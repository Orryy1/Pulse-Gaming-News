...
| 1t186u4: Reggie says Nintendo stopped selling products on Amazon in the 2010s after they asked for financial support to undercut competitors' prices | exact_subject_gameplay_still_repair | exact_subject_gameplay_still_gap | no | 0 | 0 | none | npm run media:enrich-stills -- --story 1t186u4 --dry-run --prefer-gameplay-stills --require-verified-store --max-store-search-entities 5 --max-store-assets-per-entity 3 --max-downloads-per-story 12 |
| 1t1hyqc: Even tho I can’t download you. You will always be on my phone. | exact_subject_gameplay_still_repair | exact_subject_gameplay_still_gap | no | 0 | 0 | none | npm run media:enrich-stills -- --story 1t1hyqc --dry-run --prefer-gameplay-stills --require-verified-store --max-store-search-entities 5 --max-store-assets-per-entity 3 --max-downloads-per-story 12 |
| demo_gta_xbox: GTA 6 gets a new Xbox showcase update | exact_subject_gameplay_still_repair | exact_subject_gameplay_still_gap | no | 1 | 0 | none | npm run media:enrich-stills -- --story demo_gta_xbox --dry-run --prefer-gameplay-stills --require-verified-store --max-store-search-entities 5 --max-store-assets-per-entity 3 --max-downloads-per-story 12 |
| demo_hotd_reject: House of the Dragon season 3 adds a major new cast member | exact_subject_gameplay_still_repair | exact_subject_gameplay_still_gap | no | 0 | 0 | none | npm run media:enrich-stills -- --story demo_hotd_reject --dry-run --prefer-gameplay-stills --require-verified-store --max-store-search-entities 5 --max-store-assets-per-entity 3 --max-downloads-per-story 12 |
...
| rss_4105cb7c837252c3: A New The Division PC Game Is Out Right Now, And It's Free | cover_dominated_deck_repair | cover_dominated_exact_assets | no | 12 | 0.667 | none | npm run media:enrich-stills -- --story rss_4105cb7c837252c3 --dry-run --prefer-gameplay-stills --require-verified-store --max-store-search-entities 5 --max-store-assets-per-entity 3 --max-downloads-per-story 12 |
| rss_5b3abe925b27a199: GTA 6 Owner Passed On A Sequel To A Legacy Franchise, And We're Dying To Know Which One | wrong_story_deck_rejection | wrong_story_exact_assets | yes | 39 | 0.333 | GTA, BioShock, Red Dead | npm run media:enrich-stills -- --story rss_5b3abe925b27a199 --dry-run --prefer-gameplay-stills --require-verified-store --max-store-search-entities 5 --max-store-assets-per-entity 3 --max-downloads-per-story 12 |
...
Ranked actions:
- 1. exact_subject_gameplay_still_repair (P0): The proof deck needs more exact-subject gameplay stills before proof rendering. Entities: House of the Dragon season 3
- 2. validated_clip_windows_needed (P0): Validated gameplay clip windows are below the Flash Lane threshold. Entities: House of the Dragon season 3

Commands:
- gameplay_still_dry_run: `npm run media:enrich-stills -- --story demo_hotd_reject --dry-run --prefer-gameplay-stills --require-verified-store --max-store-search-entities 5 --max-store-assets-per-entity 3 --max-downloads-per-story 12` (dry_run_only)
- gameplay_still_apply_local: `npm run media:enrich-stills -- --story demo_hotd_reject --apply-local --prefer-gameplay-stills --require-verified-store --max-store-search-entities 5 --max-store-assets-per-entity 3 --max-downloads-per-story 12` (apply_local_under_test_output_only)
- rebuild_proof_candidates: `npm run studio:v2:proof-candidates -- --story demo_hotd_reject` (report_only)
- recheck_flash_state: `npm run studio:v2:flash-state -- --story demo_hotd_reject` (report_only)
- validate_gameplay_windows: `npm run media:validate-trailer-segments -- --story-id demo_hotd_reject --apply-local --deep-scan --reference-report test/output/official_trailer_references_v1_story_demo_hotd_reject.json --previous-validation-report test/output/official_trailer_segment_validation_apply_local.json --merge-previous --exhausted-source-family-threshold 5 --max-segments 90 --candidate-windows-per-source 6` (apply_local_under_test_output_only)
