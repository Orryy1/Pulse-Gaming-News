...
function selectSubtitleScriptText(story, wordTimestamps) {
  const transcriptFromChars = Array.isArray(wordTimestamps?.characters)
    ...
    : null;
  const candidates = [
    wordTimestamps?.meta?.transcript,
    wordTimestamps?.meta?.text,
...
async function assemble() {
  ...
  // mediaPaths already declared at the top of assemble() (self-heal
  // pre-pass) — reuse the same binding here.
  for (const story of toProcess) {
    // 2026-04-24 critical fix: route audio existence check through
    // lib/media-paths so stories whose audio landed under MEDIA_ROOT
    // (`/data/media/output/audio/...`) aren't falsely skipped.
    //
    // The old `fs.pathExists(story.audio_path)` bypassed the
    // resolver entirely: when story.audio_path is a relative string
    // like `output/audio/xxx.mp3`, Node's fs.pathExists resolves it
    // against CWD (`/app` in the Railway container), not under
    // MEDIA_ROOT. Today's 08:00 UTC produce generated audio on
    // `/data/media/output/audio/` but assemble checked
    // `/app/output/audio/` — missed every one, wrote 0 MP4s, and
    // left every story in `approved_not_produced`.
    const audioExists = story.audio_path
      ? await mediaPaths.pathExists(story.audio_path)
      : false;
    if (!audioExists) {
      console.log(
    ...
    let compositeImageAbs = null;
    if (story.image_path) {
      const resolved = await mediaPaths.resolveExisting(story.image_path);
