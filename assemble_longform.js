...
async function generateLongformSubtitles(compilation, outputDir) {
  ...
  const timestampsPath = audioPath ? audioPath.replace(/\.mp3$/, '_timestamps.json') : null;
  ...
  if (wordTimestamps && wordTimestamps.characters && wordTimestamps.character_start_times_seconds && wordTimestamps.character_end_times_seconds) {
    ...
    while (idx < words.length) {
      ...
      for (let j = 0; j < chunkSize && idx + j < words.length; j++) {
        ...
        if (/[.!?]$/.test(words[idx + j].text)) { j++; break; }
      }
      phrases.push({
    ...
    events = phrases.map((p, i) => {
      ...
      const end = (i < phrases.length - 1) ? Math.max(p.start + 0.1, p.end - 0.05) : p.end;
      return `Dialogue: 0,${assTime(p.start)},${assTime(end)},Caption,,0,0,0,,${clean}`;
    }).filter(Boolean).join('\n');

    console.log(`[longform] Subtitles: ${phrases.length} phrases synced from word timestamps`);
  } else {
    // Fallback: even spacing
    const words = fullScript.replace(/\[PAUSE\]/gi, '').replace(/\[VISUAL:[^\]]*\]/gi, '').split(/\s+/).filter(Boolean);
...
async function ensureLongformMusic(duration) {
  ...
  if (suitable.length > 0) {
    ...
    console.log(`[longform] Music: using "${pick}" (will loop for ${Math.round(duration)}s)`);
  ...
  try {
    ...
    console.log(`[longform] Generating music track for longform compilation...`);
    ...
    const musicPath = path.join(MUSIC_CACHE, `${channelId}_120s_v${promptIdx}.mp3`);
    await fs.writeFile(musicPath, Buffer.from(response.data));
    console.log(`[longform] New music track saved: ${musicPath}`);
    return musicPath;
  } catch (err) {
    console.log(`[longform] Music generation failed (non-fatal): ${err.message}`);
    const allCached = (await fs.readdir(MUSIC_CACHE)).filter(f => f.endsWith('.mp3'));
    if (allCached.length > 0) return path.join(MUSIC_CACHE, allCached[0]);
    return null;
  }
...
async function assembleLongform(compilation) {
  ...
  const secondaryFFM = channel.colours.SECONDARY ? channel.colours.SECONDARY.replace('#', '0x') : '0x0D0D0F';
  ...
  inputs.push(`-f lavfi -t ${INTRO_DUR} -i "color=c=0x0D0D0F:s=1920x1080:r=30"`);
  ...
  for (let si = 0; si < segments.length; si++) {
    ...
    // Chapter title card
    inputs.push(`-f lavfi -t ${CHAPTER_CARD_DUR} -i "color=c=0x0D0D0F:s=1920x1080:r=30"`);
    const cardIdx = inputIdx++;

    const classInfo = channel.classificationColour
      ? channel.classificationColour(storyObj?.classification || storyObj?.flair || 'News')
      : { ffm: '0x6B7280', label: 'NEWS' };
    const chTitle = sanitizeDrawtext(storyObj?.title || seg.title || `Story ${si + 1}`, 60);
    ...
    let storyImages = [];
    if (storyObj && storyObj.downloaded_images && storyObj.downloaded_images.length > 0) {
    ...
    if (storyImages.length === 0 && storyObj && storyObj.image_path && await fs.pathExists(storyObj.image_path)) {
    ...
    if (storyImages.length === 0) {
      ...
      const imgSegDur = Math.max(3, Math.floor(segDuration / storyImages.length));
      const imgLabels = [];

      for (let ii = 0; ii < storyImages.length; ii++) {
        inputs.push(`-loop 1 -t ${imgSegDur} -i "${storyImages[ii].replace(/\\/g, '/')}"`);
      ...
      if (imgLabels.length > 1) {
        ...
        for (let k = 1; k < imgLabels.length; k++) {
          ...
          const outLabel = k === imgLabels.length - 1 ? `seg${si}` : `segxf${si}_${k}`;
          filterParts.push(
  ...
  // --- Outro card ---
  inputs.push(`-f lavfi -t ${OUTRO_DUR} -i "color=c=0x0D0D0F:s=1920x1080:r=30"`);
  ...
  filterParts.push(
    `${concatInput}concat=n=${concatCount}:v=1:a=0[rawvid]`
  );

  // --- Overlay: slight brightness + ASS subtitles ---
  const assPathFixed = assPath.replace(/\\/g, '/').replace(/:/g, '\\\\:');
  filterParts.push(
    `[rawvid]eq=brightness=-0.03:saturation=1.1,` +
  ...
  if (musicPath && await fs.pathExists(musicPath)) {
    ...
    inputs.push(`-stream_loop -1 -t ${Math.ceil(duration) + 5} -i "${musicPath.replace(/\\/g, '/')}"`);
    inputIdx++;
  }
  ...
  console.log(`[longform] Rendering ${segments.length} segments, ~${Math.round(duration / 60)} minutes...`);

  try {
    await execAsync(command, { timeout: 600000, maxBuffer: 20 * 1024 * 1024 });
    const stat = await fs.stat(outputPath);
    console.log(`[longform] Exported: ${outputPath} (${Math.round(stat.size / 1024 / 1024)}MB)`);
    return outputPath;
  } catch (err) {
