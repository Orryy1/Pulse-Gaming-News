...
| `USE_JOB_QUEUE`      | **Dev-only escape.** Setting this to the literal `false` in a non-prod environment reaches the quarantined legacy `_registerLegacyDevCronRegistry()` block. Ignored in `NODE_ENV=production` (or when `RAILWAY_*` env vars are set). | Default behaviour (Phase D). `lib/dispatch-mode.js` picks `queue` mode; `lib/scheduler.js` enqueues into the `jobs` table and `lib/services/jobs-runner.js` executes. In production, bootstrap failure is fatal — the legacy cron block is unreachable.                                                  |
| `USE_SCORING_ENGINE` | **Dev-only escape.** Setting this to the literal `false` in a non-prod environment makes `autoApprove()` a logged no-op. In `NODE_ENV=production` this flag is ignored.                                                              | Default behaviour. `autoApprove()` always routes through `lib/decision-engine::runScoringPass` and persists `auto` / `review` / `defer` / `reject` decisions to `story_scores`. The legacy `shouldAutoApprove() { return true }` shortcut was removed in Phase E — there is no "off" path in production. |
...
1. Railway → **Variables** → delete (or set to `false`):
   ...
   USE_SCORING_ENGINE
   ```
2. Railway redeploys. Legacy `node-cron` handlers in `run.js` / `server.js` take over. (`cloud.js` was deleted in Phase B — it is no longer part of any rollback path.)
