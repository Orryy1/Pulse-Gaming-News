...
function variantAssetsForRole(packConfig = {}, role = "") {
  const targetRole = cleanText(role);
  if (!targetRole) return [];

  const defaults = asArray(packConfig.assets).filter((asset) => cleanText(asset.role) === targetRole);
  const variantsNode = packConfig.variants || {};
  const explicitVariants = Array.isArray(variantsNode)
    ? variantsNode.filter((asset) => cleanText(asset.role) === targetRole)
    : asArray(variantsNode[targetRole] || variantsNode.roles?.[targetRole]);

  const byFilename = new Map();
  for (const asset of [...defaults, ...explicitVariants]) {
    const filename = normalisePath(asset.filename);
    if (!filename || byFilename.has(filename)) continue;
    byFilename.set(filename, {
      ...asset,
      role: cleanText(asset.role) || targetRole,
      filename,
    });
  }
...
function variantAssetCandidates(packConfig = {}, role = "", { seed = "" } = {}) {
  const variants = variantAssetsForRole(packConfig, role);
  if (!variants.length) return [];
  const selectionSeed = cleanText(seed);
  const startIndex = selectionSeed
    ? stableVariantIndex(`${packConfig.id || ""}:${role}:${selectionSeed}`, variants.length)
    : 0;
  return [...variants.slice(startIndex), ...variants.slice(0, startIndex)].map((asset, offset) => ({
    ...asset,
    variant_index: (startIndex + offset) % variants.length,
    variant_count: variants.length,
    selection_strategy: selectionSeed ? "story_id_hash" : "default_first",
  }));
}

function selectVariantAsset(packConfig = {}, role = "", { seed = "" } = {}) {
  return variantAssetCandidates(packConfig, role, { seed })[0] || null;
}

function packConfigForPack(packRow) {
  if (!packRow?.id) return null;
  return discoverPackConfigs().find((config) => config.id === packRow.id) || null;
}
...
function resolve({
  ...
  const seed = cleanText(variantSeed || storyId);
  const tryInPack = (packRow) => {
    const packConfig = packConfigForPack(packRow);
    for (const r of roleCandidates({ role, flair, breaking })) {
      if (packConfig) {
        const rootPath = absolutiseRoot(packConfig.root_path || packRow.root_path);
        for (const variant of variantAssetCandidates(packConfig, r, { seed })) {
          const abs = path.join(rootPath, variant.filename);
