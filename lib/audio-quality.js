...
function buildNarrationMusicMixFilter({
  ...
  const target = finiteNumber(targetLufs, -14);
  const peak = finiteNumber(truePeak, -1.5);
  const lra = finiteNumber(loudnessRange, 7);
  const limit = finiteNumber(limiter, 0.85);
...
function buildVoiceMasteringFilter({
  ...
  const target = finiteNumber(targetLufs, -14);
  const peak = finiteNumber(truePeak, -1);
  const lra = finiteNumber(loudnessRange, 7);
  const limit = finiteNumber(limiter, 0.96);
  ...
  const filters = [
    ...
    "equalizer=f=6500:t=q:w=1.0:g=1.5",
    "equalizer=f=9500:t=q:w=0.9:g=0.9",
    "acompressor=threshold=-22dB:ratio=2.6:attack=4:release=80:makeup=2.5",
...
async function masterTtsAudioFile({
  ...
  const targetLufs = finiteNumber(env.TTS_VOICE_TARGET_LUFS, -14);
  const truePeak = finiteNumber(env.TTS_VOICE_TRUE_PEAK, -1);
  const loudnessRange = finiteNumber(env.TTS_VOICE_LRA, 7);
