...
function buildVoiceMasteringSignature(env = process.env) {
  return {
    version: 2,
    preset: "local_presence_clarity_v2",
    targetLufs: finiteNumber(env.TTS_VOICE_TARGET_LUFS, -16),
    truePeak: finiteNumber(env.TTS_VOICE_TRUE_PEAK, -2.2),
    loudnessRange: finiteNumber(env.TTS_VOICE_LRA, 8),
    outputBitrate: resolveAudioBitrate(env.TTS_VOICE_MASTER_BITRATE, "256k"),
    denoise: isTruthy(env.TTS_VOICE_DENOISE || env.LOCAL_TTS_DENOISE),
  };
}
...
function buildVoiceMasteringFilter({
  ...
  const filters = [
    "highpass=f=75",
    "equalizer=f=220:t=q:w=1.0:g=-1",
    "equalizer=f=3000:t=q:w=1.0:g=2.4",
    "equalizer=f=5200:t=q:w=1.2:g=4.5",
    "aexciter=amount=2.2:drive=6.5:blend=0.45:freq=3600:ceil=16000",
    "acompressor=threshold=-20dB:ratio=1.45:attack=8:release=120:makeup=1.0",
