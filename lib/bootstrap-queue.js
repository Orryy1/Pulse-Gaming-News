...
async function start(opts = {}) {
  ...
  _state = { workerId, schedulerHandle, runner, maintenanceRunner, publishCriticalRunner };
  log(
    `[bootstrap-queue] up: scheduler=${!!schedulerHandle} runner=${!!runner} maintenanceRunner=${!!maintenanceRunner} publishCriticalRunner=${!!publishCriticalRunner} worker=${workerId}`,
  );
  return _state;
}
...
async function stop() {
  if (!_state) return;
  const { schedulerHandle, runner, maintenanceRunner, publishCriticalRunner } = _state;
  try {
