...
async function start(opts = {}) {
  ...
  if (autoSeed) {
    try {
      seed({ reset: resetSchedules, log, repos });
    } catch (err) {
  ...
  if (runScheduler) {
    schedulerHandle = schedulerStarter({ log, repos });
  }
  ...
  log(
    `[bootstrap-queue] up: scheduler=${_state.schedulerActive} runner=${!!runner} worker=${workerId}`,
  );
  return _state;
}
...
async function stop() {
  ...
  try {
    if (runner) {
      ...
      runnerDrained = result?.drained !== false;
    }
  } catch {
    runnerDrained = false;
  }
