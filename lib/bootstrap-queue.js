...
async function start(opts = {}) {
  ...
  _state = { workerId, schedulerHandle, runner, publishCriticalRunner };
  log(
    `[bootstrap-queue] up: scheduler=${!!schedulerHandle} runner=${!!runner} publishCriticalRunner=${!!publishCriticalRunner} worker=${workerId}`,
  );
  return _state;
}
...
async function stop() {
  if (!_state) return;
  const { schedulerHandle, runner, publishCriticalRunner } = _state;
  try {
