...
function firstSentence(value) {
  ...
  if (!text) return "";
  return clean((text.match(/[^.!?]+[.!?]?/) || [text])[0]);
}
...
async function refreshCandidate(candidate = {}, { generatedAt = new Date().toISOString() } = {}) {
  ...
  const canonicalSpokenScript = nonEmptyObject(canonicalManifest)
    ? clean(
    ...
    : "";
  const canonicalSpokenLine = firstSentence(canonicalSpokenScript);
  if (!duration) {
    return {
      candidate,
      row: {
