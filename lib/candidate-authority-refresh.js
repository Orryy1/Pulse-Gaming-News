...
async function inspectCandidateEvidence({
  ...
  if (fingerprints.timestamps?.sha256) {
    try {
      const timestampDocument = await fs.readJson(criticalPaths.timestamps);
      ...
      if (!rows.length || rows.some((row) => (
        ...
        blockers.push("timestamps_file_invalid");
      }
    } catch {
      blockers.push("timestamps_file_unreadable");
    }
  }
...
async function applyAuthorityTransaction({
  ...
  try {
    for (const fileName of AUTHORITY_FILES) {
      ...
      if (!(await fs.pathExists(filePath))) {
        throw new Error(`Authority file missing: ${filePath}`);
      }
      ...
      try {
        await fs.writeFile(backupTempPath, original, { flag: "wx", mode: stat.mode & 0o777 });
        await fs.rename(backupTempPath, backupPath);
      } finally {
        await fs.remove(backupTempPath).catch(() => {});
      }
      await fs.writeFile(nextTempPath, serialiseJson(proposed[key]), {
        flag: "wx",
    ...
    for (const entry of committed.reverse()) {
      try {
        await restoreAuthorityFile(entry.path, entry.original, entry.mode);
      } catch (error) {
...
async function refreshCandidateAuthority({
  ...
  const inspection = await inspectCandidateEvidence({
