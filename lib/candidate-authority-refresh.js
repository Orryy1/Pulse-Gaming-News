...
const ENABLED_PLATFORMS = [...LEGACY_ENABLED_PLATFORMS];
...
async function temporalQaReportPaths(artifactDir) {
  const canonicalPath = path.join(artifactDir, "temporal_video_qa_report.json");
  const paths = [];
  if (await fs.pathExists(canonicalPath)) paths.push(canonicalPath);
  const qaRoot = path.join(artifactDir, "qa");
  if (!(await fs.pathExists(qaRoot))) return paths;
  ...
  while (pending.length) {
    ...
    for (const entry of entries) {
      ...
      else if (entry.isFile() && entry.name === "temporal_video_qa_report.json") {
        paths.push(entryPath);
      }
    }
  }
  return unique(paths.map((filePath) => path.resolve(filePath)));
}

function temporalQaFinalMedia(document = {}) {
  return document.final_media || document.finalMedia || document.media || {};
}

async function resolveTemporalQaReportPath({
  artifactDir,
  storyId,
  renderFingerprint = {},
}) {
  const canonicalPath = path.join(artifactDir, "temporal_video_qa_report.json");
  const candidates = [];
  for (const filePath of await temporalQaReportPaths(artifactDir)) {
    const evidence = await readJsonEvidence(filePath);
    if (!evidence.valid) continue;
    const finalMedia = temporalQaFinalMedia(evidence.value);
    const declaredHash = normaliseSha256(
      finalMedia.sha256 ||
        finalMedia.hash ||
        evidence.value.render_sha256 ||
        evidence.value.final_render_sha256,
    );
    const declaredSize = Number(
      finalMedia.size_bytes ??
        finalMedia.file_size_bytes ??
        evidence.value.render_size_bytes ??
        evidence.value.final_render_size_bytes,
    );
    const storyMatches = clean(evidence.value.story_id) === clean(storyId);
    const hashMatches =
      Boolean(renderFingerprint.sha256) &&
      declaredHash === normaliseSha256(renderFingerprint.sha256);
    const sizeMatches =
      !Number.isFinite(declaredSize) ||
      declaredSize <= 0 ||
      declaredSize === Number(renderFingerprint.size_bytes);
    candidates.push({
      filePath,
      evidence,
      boundToCurrentRender: storyMatches && hashMatches && sizeMatches,
      generatedAt: Date.parse(clean(evidence.value.generated_at)) || 0,
    });
  }
  const bound = candidates
    .filter((candidate) => candidate.boundToCurrentRender)
    .sort((left, right) =>
      right.generatedAt - left.generatedAt ||
      left.filePath.localeCompare(right.filePath),
    );
  if (bound.length) return bound[0].filePath;
  if (candidates.some((candidate) => candidate.filePath === canonicalPath)) {
  ...
  return fallback?.filePath || canonicalPath;
}
...
async function inspectRightsLedger({
  artifactDir,
  storyId,
  rightsEvidence,
  ...
  for (const used of usedAssets) {
    ...
    blockers.push(
      ...officialYoutubeTransformativeRightsBlockers(record, used).map(
        (blocker) => `rights:${blocker}`,
      ),
    );
    for (const field of [
      "kind",
  ...
  return {
    ...
    enabled_platforms_checked: [...enabledPlatforms],
  };
}

function inspectRenderRightsReconciliation({
  artifactDir,
  renderManifest = {},
  ...
  const blockers = [];
  const warnings = [];
  const classification = classifyEvidence(reconciliation);
  const reconciliationFailures = evidenceFailures(reconciliation);
  const reconciliationWarnings = evidenceWarnings(reconciliation);
  if (classification === "RED") blockers.push("render_rights_reconciliation_not_green");
  else if (classification === "AMBER" && reconciliationWarnings.length === 0) {
    warnings.push("render_rights_reconciliation:critical_status_amber");
  }
  blockers.push(...reconciliationFailures.map((failure) =>
    `render_rights_reconciliation:${failure}`));
  warnings.push(...reconciliationWarnings.map((warning) =>
    `render_rights_reconciliation:${warning}`));
  const appliedLedgerStatus = normaliseStatus(reconciliation.applied_ledger_verdict);
  if (RED_STATUSES.has(appliedLedgerStatus)) {
    blockers.push("render_rights_reconciliation_applied_ledger_not_green");
  } else if (AMBER_STATUSES.has(appliedLedgerStatus)) {
    warnings.push("render_rights_reconciliation:applied_ledger_amber");
  }

  const declaredPath = resolveLocalPath(reconciliation.rights_ledger_path, artifactDir);
  const canonicalPath = path.resolve(path.join(artifactDir, "rights_ledger.json"));
  if (!declaredPath) blockers.push("render_rights_reconciliation_ledger_path_missing");
  else if (declaredPath.toLowerCase() !== canonicalPath.toLowerCase()) {
    blockers.push("render_rights_reconciliation_ledger_path_mismatch");
  }

  const declaredSha256 = normaliseSha256(reconciliation.applied_ledger_sha256);
  const declaredSizeBytes = Number(reconciliation.applied_ledger_size_bytes);
  if (!declaredSha256) blockers.push("render_rights_reconciliation_ledger_hash_missing");
  else if (!rightsFingerprint?.sha256 || declaredSha256 !== rightsFingerprint.sha256) {
    blockers.push("render_rights_reconciliation_ledger_hash_mismatch");
  }
  if (!Number.isFinite(declaredSizeBytes) || declaredSizeBytes <= 0) {
    blockers.push("render_rights_reconciliation_ledger_size_missing");
  } else if (!rightsFingerprint?.size_bytes ||
    declaredSizeBytes !== rightsFingerprint.size_bytes) {
    blockers.push("render_rights_reconciliation_ledger_size_mismatch");
  }
  if (reconciliation.final_state_verified !== true) {
    blockers.push("render_rights_reconciliation_final_state_not_verified");
  }
  ...
  if (!Number.isFinite(usedAssetCount) || usedAssetCount <= 0) {
    blockers.push("render_rights_reconciliation_used_asset_count_missing");
  }
  if (!Number.isFinite(reconciledRecordCount) || reconciledRecordCount <= 0) {
    blockers.push("render_rights_reconciliation_record_count_missing");
  } else if (Number.isFinite(usedAssetCount) && reconciledRecordCount !== usedAssetCount) {
    blockers.push("render_rights_reconciliation_coverage_incomplete");
  }
  if (Number(reconciliation.duplicate_record_count_after || 0) > 0) {
    blockers.push("render_rights_reconciliation_duplicate_records");
  }
  if (classification === "GREEN" && reconciliation.can_auto_publish !== true) {
    blockers.push("render_rights_reconciliation_auto_publish_not_approved");
  }
  ...
  return {
    ...
    evidence: {
      declared_statuses: evidenceStatuses(reconciliation),
      rights_ledger_path: declaredPath || null,
      rights_ledger_sha256: rightsFingerprint?.sha256 || null,
      rights_ledger_size_bytes: rightsFingerprint?.size_bytes || 0,
      declared_ledger_sha256: declaredSha256 || null,
      declared_ledger_size_bytes:
        Number.isFinite(declaredSizeBytes) ? declaredSizeBytes : null,
      used_asset_count:
        Number.isFinite(usedAssetCount) ? usedAssetCount : null,
      reconciled_record_count:
        Number.isFinite(reconciledRecordCount) ? reconciledRecordCount : null,
...
async function inspectCandidateEvidence({
  ...
  const renderManifestEvidence = await readJsonEvidence(renderManifestPath);
  const preliminaryRenderManifest = renderManifestEvidence.value || {};
  const preliminaryRenderPath = resolveLocalPath(
    preliminaryRenderManifest.output_path ||
      preliminaryRenderManifest.output ||
      "visual_v4_render.mp4",
    artifactDir,
  );
  let preliminaryRenderFingerprint = {
    path: preliminaryRenderPath || null,
    sha256: null,
    size_bytes: 0,
  };
  if (preliminaryRenderPath && await fs.pathExists(preliminaryRenderPath)) {
    try {
      preliminaryRenderFingerprint = await fingerprintFile(preliminaryRenderPath);
    } catch {
      // The normal critical-file pass below reports an unreadable render.
    }
  }
  const temporalQaPath = await resolveTemporalQaReportPath({
    artifactDir,
    storyId,
    renderFingerprint: preliminaryRenderFingerprint,
  });
  const evidenceFiles = {
    render_manifest: renderManifestPath,
    audio_manifest: path.join(artifactDir, "audio_manifest.json"),
    narration_manifest: path.join(artifactDir, "narration_manifest.json"),
    caption_manifest: path.join(artifactDir, "caption_manifest.json"),
    decoded_forensic_report: path.join(artifactDir, "decoded_forensic_report.json"),
    temporal_video_qa_report: temporalQaPath,
    final_av_review: path.join(artifactDir, "final_av_review.json"),
    rights_ledger: path.join(artifactDir, "rights_ledger.json"),
  };
  const evidence = {
    render_manifest: renderManifestEvidence,
  };
  for (const [key, filePath] of Object.entries(evidenceFiles)) {
    if (key === "render_manifest") continue;
    evidence[key] = await readJsonEvidence(filePath);
  }
  const blockers = [];
  const warnings = [];
  const platformManifestEvidence = await readJsonEvidence(
    path.join(artifactDir, "platform_publish_manifest.json"),
  );
  if (!platformManifestEvidence.present) blockers.push("platform_publish_manifest_missing");
  else if (!platformManifestEvidence.valid) blockers.push("platform_publish_manifest_unreadable");
  const platformScope = resolvePlatformPublishScope(
    platformManifestEvidence.value || {},
  );
  ...
  const criticalPaths = {
    ...
    temporal_qa_report: temporalQaPath,
  };
  ...
  const rights = await inspectRightsLedger({
    ...
    enabledPlatforms: platformScope.enabled_platforms,
  });
  blockers.push(...rights.blockers);
  warnings.push(...rights.warnings.map((warning) => `rights:${warning}`));
  const renderRightsReconciliation = inspectRenderRightsReconciliation({
    artifactDir,
    renderManifest,
    rightsFingerprint: fingerprints.rights,
  });
  blockers.push(...renderRightsReconciliation.blockers);
  ...
  return {
    ...
    platform_scope: authorityPlatformScope(platformScope),
  };
}
...
function authorityDocuments({ current, storyId, generatedAt, inspection }) {
  ...
  const authorityRefresh = {
    ...
    platform_scope: authorityPlatformScope(inspection.platform_scope),
...
async function refreshCandidateAuthority({
  ...
  const report = {
    ...
    platform_scope: inspection.platform_scope,
    render_rights_reconciliation: inspection.render_rights_reconciliation,
    proposed,
