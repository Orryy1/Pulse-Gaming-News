...
} = require("./local-generated-voice-rights");
...
function providerHistoryAudioMatchesRawResponse(generation = {}) {
  ...
  const compressedMatch =
    rawSha256 === historySha256 && rawSizeBytes === historySizeBytes;
  const method = clean(generation.audio_identity_method);
  if (compressedMatch) {
    return (
      !method ||
      (
        method === "compressed_bytes_sha256" &&
  ...
  const rawPcmSizeBytes = Number(
    generation.raw_provider_audio_decoded_pcm_size_bytes,
  );
  return (
    method === "decoded_pcm_s16le_44100_mono_sha256" &&
    generation.compressed_bytes_match === false &&
    validSha256(rawPcmSha256) &&
    clean(generation.history_audio_decoded_pcm_sha256).toLowerCase() ===
      rawPcmSha256 &&
    Number.isInteger(rawPcmSizeBytes) &&
    rawPcmSizeBytes > 0 &&
...
function decodedProviderIdentityMatchesReceipt(generation = {}, receipt = {}) {
  if (
  ...
  if (
    typeof generation.compressed_bytes_match !== "boolean" ||
    typeof receipt.compressed_bytes_match !== "boolean" ||
    receipt.compressed_bytes_match !== generation.compressed_bytes_match
  ) {
    return false;
  }
  const integerFields = [
    "history_audio_size_bytes",
    "raw_provider_audio_decoded_pcm_size_bytes",
    "history_audio_decoded_pcm_size_bytes",
    "decoded_pcm_sample_rate_hz",
    "decoded_pcm_channels",
  ];
  if (
    integerFields.some((key) => (
      typeof generation[key] !== "number" ||
      typeof receipt[key] !== "number" ||
      !Number.isInteger(generation[key]) ||
      !Number.isInteger(receipt[key]) ||
      receipt[key] !== generation[key]
    ))
  ) {
    return false;
  }
  const hashFields = [
    "history_audio_sha256",
    "raw_provider_audio_decoded_pcm_sha256",
    "history_audio_decoded_pcm_sha256",
  ];
  if (
    hashFields.some((key) => (
      !validSha256(generation[key]) ||
      !validSha256(receipt[key]) ||
      clean(receipt[key]).toLowerCase() !== clean(generation[key]).toLowerCase()
    ))
  ) {
    return false;
  }
  return [
    "audio_identity_method",
    "decoded_pcm_sample_format",
  ].every((key) => clean(receipt[key]).toLowerCase() ===
    clean(generation[key]).toLowerCase());
}
...
async function strictElevenLabsGenerationEvidenceBlockers({
  ...
  if (!providerHistoryAudioMatchesRawResponse(generation)) {
  ...
  if (!generationReceiptCurrent) {
    ...
    if (
      ...
      !decodedProviderIdentityMatchesReceipt(
        generation,
        receiptGeneration,
      ) ||
...
async function decodedVisualGateFailures(document, {
  artifactDir,
  storyId,
} = {}) {
  const failures = [];
  const status = clean(document?.status || document?.verdict || document?.result)
    .toUpperCase();
  if (!["GREEN", "PASS", "PASSED"].includes(status)) {
    failures.push("decoded_visual_status_not_green");
  }
  if (clean(document?.story_id) !== clean(storyId)) {
    failures.push("decoded_visual_story_id_mismatch");
  }
  if (document?.decoded_media_evidence !== true) {
    failures.push("decoded_visual_media_evidence_missing");
  }
  if (asArray(document?.blockers).length) {
    failures.push("decoded_visual_blockers_present");
  }
  if (document?.final_output_binding_verified !== true) {
    failures.push("decoded_visual_final_output_binding_missing");
  }
  const declaredHash = normaliseSha256(document?.mp4_sha256);
  if (!/^[a-f0-9]{64}$/.test(declaredHash)) {
    failures.push("decoded_visual_render_hash_missing");
  }
  const currentRenderPath = path.join(artifactDir, "visual_v4_render.mp4");
  if (!(await fs.pathExists(currentRenderPath))) {
    failures.push("decoded_visual_current_render_missing");
  } else if (/^[a-f0-9]{64}$/.test(declaredHash)) {
    const currentFingerprint = await fingerprintFile(currentRenderPath);
    if (currentFingerprint.sha256 !== declaredHash) {
      failures.push("decoded_visual_render_hash_mismatch");
    }
  }
...
async function inspectAuthoritativePublishEvidence({ artifactDir, storyId, aggregatePaths = [] }) {
  const evidence = [
    ...
    },
    {
      ...
      blocker: "authoritative_decoded_visual_gate_red",
      invalidBlocker: "authoritative_decoded_visual_gate_invalid",
      isRed: hasAuthoritativeRed,
      validate: (document) => decodedVisualGateFailures(document, {
        artifactDir,
        storyId,
      }),
    },
  ...
  for (const item of evidence) {
    ...
    const validationFailures = present && valid && item.validate
      ? await item.validate(document)
      : [];
    const red = present && valid && (
      await item.isRed(document) ||
      validationFailures.length > 0
    );
    if (present && !valid) blockers.push(item.invalidBlocker);
    if (red) blockers.push(item.blocker);
    const sourceFailures = present && valid
      ? unique([
          ...authoritativeFailures(document),
          ...validationFailures,
        ])
      : [];
...
function completeRightsRecord(record = {}, targetPlatforms = ENABLED_PLATFORMS) {
  ...
  const explicitLivePublishHold = Boolean(
    record.live_publish_allowed === false ||
      ...
      record.requires_human_publish_review === true,
  );
  if (explicitLivePublishHold) return false;
  return Boolean(
...
function validEditorialPolicyAcceptance(value = {}) {
  const acceptedAt = clean(value.accepted_at);
  return Boolean(
...
function boundedEditorialAssessmentDecision(
  document = {},
  record = {},
) {
  const id = clean(record.asset_id || record.id);
  if (
    clean(document.schema) !== "pulse_media_rights_assessment_v1" ||
    clean(document.verdict).toUpperCase() !== "GREEN" ||
    document.live_publish_allowed !== true ||
    asArray(document.blockers).length > 0 ||
    !id
  ) {
    return null;
  }
  const decision = asArray(document.decisions).find(
    (row) =>
      clean(row.asset_id || row.id).toLowerCase() === id.toLowerCase(),
  );
  const rightsRecord =
    asArray(document.rights_records).find(
      (row) =>
        clean(row.asset_id || row.id).toLowerCase() === id.toLowerCase(),
    ) ||
    (
      decision?.rights_record &&
      typeof decision.rights_record === "object"
        ? decision.rights_record
        : null
    );
  const acceptance =
    rightsRecord?.editorial_policy_acceptance ||
    decision?.rights_record?.editorial_policy_acceptance ||
    {};
  const declaredAcceptance = record.editorial_policy_acceptance || {};
  const acceptanceMatches = Boolean(
    validEditorialPolicyAcceptance(acceptance) &&
      validEditorialPolicyAcceptance(declaredAcceptance) &&
      clean(acceptance.accepted_by) === clean(declaredAcceptance.accepted_by) &&
      new Date(acceptance.accepted_at).toISOString() ===
        new Date(declaredAcceptance.accepted_at).toISOString(),
  );
  const decisionBasis = clean(decision?.basis)
    .toLowerCase()
    .replace(/[\s-]+/g, "_");
  const licenceBasis = rightsBasis(rightsRecord || {});
  if (
    !decision ||
    decisionBasis !== "bounded_editorial_excerpt" ||
    clean(decision.verdict).toUpperCase() !== "GREEN" ||
    decision.live_publish_allowed !== true ||
    asArray(decision.blockers).length > 0 ||
    !rightsRecord ||
    rightsRecord.legal_exception_reliance !== true ||
    rightsRecord.rights_grant === true ||
    rightsRecord.rights_grant === false ||
    rightsRecord.commercial_use_allowed !== true ||
    rightsRecord.live_publish_allowed === false ||
    !hasPlatformScope(rightsRecord, record.allowed_platforms) ||
...
async function inspectBoundTransformativeRightsEvidence(record = {}, artifactDir = "") {
  ...
  let boundedEditorialEvidence = null;
  if (isBoundedEditorialExceptionEvidenceKind(record.evidence_kind)) {
    const document = await readJson(evidencePath, null);
    boundedEditorialEvidence = boundedEditorialAssessmentDecision(
      document,
      record,
    );
    if (!boundedEditorialEvidence) {
      return {
        valid: false,
        path: evidencePath,
        fingerprint,
        bounded_editorial_evidence: null,
        blockers: ["bounded_editorial_exception_evidence_invalid"],
      };
    }
  }
  return {
    ...
    path: evidencePath,
    fingerprint,
    bounded_editorial_evidence: boundedEditorialEvidence,
    blockers: [],
  };
}

function currentValidatedOwnedProceduralMotionRights(
  used = {},
  targetPlatforms = ENABLED_PLATFORMS,
) {
  if (used.kind !== "video") return null;
  const metadata = used.metadata && typeof used.metadata === "object"
    ? used.metadata
    : {};
  if (!isPublishableStrictlyVerifiedOwnedMotionClip(metadata, { targetPlatforms })) {
    return null;
  }

  const ownedRights = metadata.owned_rights_record &&
    typeof metadata.owned_rights_record === "object"
    ? metadata.owned_rights_record
    : {};
  const ownedGrant = metadata.owned_generated_rights_grant &&
    typeof metadata.owned_generated_rights_grant === "object"
    ? metadata.owned_generated_rights_grant
    : metadata.rights_grant && typeof metadata.rights_grant === "object"
      ? metadata.rights_grant
      : {};
  const owner = clean(ownedRights.source_owner || metadata.source_owner);
  const ownedPlatforms = unique(
    asArray(ownedRights.allowed_platforms).map(normalisePlatformKey),
  );
  ...
  const record = {
    ...
    source_url: clean(ownedRights.source_url || used.source_url),
    source_type: clean(ownedRights.source_type || used.source_type),
    source_family: clean(
      used.source_family || ownedRights.source_family || metadata.source_family,
    ),
    source_owner: owner,
    allowed_use: clean(
      ownedRights.allowed_use || metadata.allowed_use || ownedGrant.allowed_use,
    ),
    allowed_platforms: ownedPlatforms,
    evidence_file: clean(
      ownedRights.evidence_file ||
        ownedRights.rights_evidence_file ||
        metadata.rights_evidence_file_path,
    ),
  ...
  if (
    !completeRightsRecord(record, targetPlatforms) ||
    explicitRightsRestrictionReasons(record, targetPlatforms).length > 0
  ) {
    return null;
  }
  return record;
...
function provisionalOwnedMotionProjectionRecord(
  record = {},
  targetPlatforms = ENABLED_PLATFORMS,
) {
  const restrictions = explicitRightsRestrictionReasons(record, targetPlatforms);
  const nestedOwnedRights = record.owned_rights_record &&
    typeof record.owned_rights_record === "object"
    ? record.owned_rights_record
    : {};
  const knownOwnedProjection = Boolean(
    clean(record.source_owner || record.creator).toLowerCase() === "pulse gaming" ||
      (
        record.current_validated_owned_procedural_motion === true &&
        clean(record.reconciliation_basis) === "current_validated_owned_procedural_motion" &&
        clean(nestedOwnedRights.source_owner).toLowerCase() === "pulse gaming" &&
        clean(nestedOwnedRights.ownership_basis) === "wholly_owned_generated_asset" &&
        nestedOwnedRights.rights_grant === true
      )
  );
  const explicitLivePublishHold = Boolean(
    record.live_publish_allowed === false ||
  ...
  return Boolean(
    restrictions.length === 1 &&
      restrictions[0] === "rights_grant_denied" &&
      !explicitLivePublishHold &&
      knownOwnedProjection &&
      clean(record.ownership_basis) === "wholly_owned_generated_asset" &&
      /internally_generated|owned_generated/i.test(clean(record.source_type)) &&
      /owned_generated/i.test(rightsBasis(record)) &&
      record.commercial_use_allowed === true &&
      hasPlatformScope(record, targetPlatforms) &&
      hasPositiveRightsDecision(record) &&
...
function provisionalElevenLabsNarrationRecord(
  record = {},
  targetPlatforms = ENABLED_PLATFORMS,
) {
  const providerText = [
  ...
  const approval = clean(record.approval_status)
    .toLowerCase()
    .replace(/[\s-]+/g, "_");
  const restrictions = explicitRightsRestrictionReasons(record, targetPlatforms);
  return Boolean(
    /elevenlabs/.test(providerText) &&
      approval === "requires_generation_bound_commercial_evidence" &&
      record.commercial_use_allowed === false &&
      record.rights_grant !== false &&
      hasPlatformScope(record, targetPlatforms) &&
      Number(record.risk_score) < 0.65 &&
      restrictions.length === 1 &&
      restrictions[0] === "commercial_use_prohibited"
  );
}

function generationBoundElevenLabsNarrationDonor({
  used = {},
  evidence = {},
  targetPlatforms = ENABLED_PLATFORMS,
} = {}) {
  ...
  if (
    ...
    !["GREEN", "PASS", "PASSED"].includes(
      clean(rightsEvidence.verdict || rightsEvidence.status).toUpperCase(),
    ) ||
    rightsEvidence.commercial_use_allowed !== true ||
    !hasPlatformScope(rightsEvidence, targetPlatforms)
  ) {
    return null;
  }
  return {
    ...
    licence_basis: rightsBasis(rightsEvidence) || rightsBasis(used),
    allowed_use: clean(used.allowed_use) || "short_form_editorial_narration",
    ...
    allowed_platforms: unique(
      asArray(rightsEvidence.allowed_platforms).map(normalisePlatformKey),
    ),
    evidence_reference: clean(used.commercial_rights_evidence_reference),
...
async function currentValidatedOfficialMotionRights(
  ...
  const documentedBoundedEditorialException = Boolean(
    hashBoundOfficialYoutube &&
      /^uk_fair_dealing_(?:criticism_review|quotation|current_events_reporting)_bounded_excerpt$/i.test(
        officialBasis,
      ) &&
      policyEvidence.valid &&
      policyEvidence.bounded_editorial_evidence &&
      metadata.legal_exception_reliance === true &&
      metadata.rights_grant !== true &&
      metadata.rights_grant !== false &&
      metadata.transformative_rights_evidence_verified === true &&
      metadata.live_publish_allowed === true &&
      metadata.commercial_use_allowed === true &&
      hasPlatformScope(metadata, targetPlatforms) &&
      typeof metadata.credit_required === "boolean" &&
      Number(metadata.risk_score) < 0.65 &&
      validEditorialPolicyAcceptance(metadata.editorial_policy_acceptance)
  );
  const documentedTransformativeRightsDecision =
    documentedPublisherVideoPolicy || documentedBoundedEditorialException;
  if (hashBoundOfficialYoutube && !documentedTransformativeRightsDecision) {
    return {
      record: null,
  ...
  const officialBasisAllowed =
    ...
    documentedTransformativeRightsDecision;
  ...
  return {
    record: {
      ...
      approval_status:
        clean(metadata.approval_status) ||
        (
          documentedBoundedEditorialException
            ? "approved_under_bounded_editorial_exception"
      ...
      evidence_reference: documentedTransformativeRightsDecision
        ? policyEvidence.path
        : "materialised_motion_clips.json",
      evidence_file: documentedTransformativeRightsDecision
        ? policyEvidence.path
        : undefined,
      evidence_kind: documentedTransformativeRightsDecision
        ? clean(metadata.evidence_kind)
        : undefined,
      evidence_sha256: documentedTransformativeRightsDecision
        ? policyEvidence.fingerprint.sha256
        : undefined,
      evidence_size_bytes: documentedTransformativeRightsDecision
      ...
      transformative_rights_evidence_verified:
        documentedTransformativeRightsDecision,
      rights_decision_basis: documentedBoundedEditorialException
        ? "validated_bounded_editorial_exception"
        : "validated_official_direct_media_editorial_policy",
      rights_status: documentedBoundedEditorialException
        ? "approved_under_bounded_editorial_exception"
        : "approved_under_transformative_editorial_policy",
      ...
      legal_exception_reliance: documentedBoundedEditorialException || undefined,
      editorial_policy_acceptance: documentedBoundedEditorialException
        ? policyEvidence.bounded_editorial_evidence.acceptance
        : undefined,
      live_publish_allowed: documentedTransformativeRightsDecision || undefined,
    },
    blockers: [],
  };
}
...
async function evidencePathForUsedAsset(used, artifactDir, donor = {}, workspaceRoot = DEFAULT_WORKSPACE_ROOT) {
  if (used.kind === "narration") {
    ...
    if (!donorEvidence || /^https?:\/\//i.test(donorEvidence)) return "";
    return resolveAssetPath(donorEvidence, artifactDir);
  }
  if (used.kind === "video" && used.generated_card_sidecar_path) return used.generated_card_sidecar_path;
  if (
    used.kind === "video" &&
    donor.current_validated_owned_procedural_motion === true
  ) {
    return resolveAssetPath(evidenceReference(donor), artifactDir);
  }
  if (
    used.kind === "video" &&
...
function narrationProviderFromRightsRecord(record = {}) {
  const providerText = [
    record.provider_id,
    record.provider_name,
    record.provider,
    ...
    .join(" ")
    .toLowerCase();
  if (providerText.includes("elevenlabs")) return "elevenlabs";
  if (
    /(?:pulse[_ -]?local[_ -]?tts|local[_ -]?tts[_ -]?voice|pulse-local-tts)/.test(
      providerText,
    )
  ) {
    return "local";
...
async function hashBoundNarrationProvider({
  rows = [],
  narrationPath = "",
  artifactDir,
  targetPlatforms,
}) {
  if (!narrationPath || !(await fs.pathExists(narrationPath))) return "";
  const current = await fingerprintFile(narrationPath);
  const providers = [];
  for (const row of rows) {
    if (
      !recordPaths(row, artifactDir).includes(
        normalisePath(narrationPath, artifactDir),
      ) ||
      !completeRightsRecord(row, targetPlatforms) ||
      !declaredFingerprintMatches(row, current)
    ) {
      continue;
    }
    const reference = evidenceReference(row);
    const evidencePath =
      reference && !/^https?:\/\//i.test(reference)
        ? resolveAssetPath(reference, artifactDir)
        : "";
    if (!evidencePath || !(await fs.pathExists(evidencePath))) continue;
    const provider = narrationProviderFromRightsRecord(row);
    if (provider) providers.push(provider);
  }
  return providers.length === 1 ? providers[0] : "";
}

function rendererAudioKind(value) {
  const kind = clean(value).toLowerCase();
  if (kind === "music") return "music";
  if (["sfx", "soundscape", "ambience"].includes(kind)) return "sfx";
  return "";
}
...
async function collectUsedAssets({
  ...
  const ownedMotionManifest = await readJson(path.join(artifactDir, "owned_motion_manifest.json"), {});
  const ownedMotion = [
    ...asArray(ownedMotionManifest.assets),
    ...materialisedRows(ownedMotionManifest),
  ];
  const used = [];
  for (const [index, scene] of asArray(renderManifest.clip_scene_plan?.scenes).entries()) {
    ...
    const ownedMetadata = metadataForPath(ownedMotion, filePath, artifactDir);
    const materialisedMetadata = metadataForPath(materialised, filePath, artifactDir);
    const metadata = isPublishableStrictlyVerifiedOwnedMotionClip(
      ownedMetadata,
      { targetPlatforms },
    )
      ? ownedMetadata
      : materialisedMetadata || ownedMetadata;
  ...
  if (narrationPath) {
    ...
    const rightsProvider = manifestProvider || audioProvider
      ? ""
      : await hashBoundNarrationProvider({
          rows,
          narrationPath,
          artifactDir,
          targetPlatforms,
        });
    const provider = manifestProvider || audioProvider || rightsProvider;
  ...
  const authoritativeRendererAudioIds = new Set(
    asArray(authoritativeRendererAssets)
      .filter((asset) => rendererAudioKind(asset.kind))
  ...
  for (const [index, selected] of asArray(renderManifest.selected_input_assets?.assets).entries()) {
    const kind = rendererAudioKind(selected.kind);
    if (!kind) continue;
    const filePath = resolveAssetPath(
      selected.local_materialized_path ||
        selected.local_materialised_path ||
...
const REPAIRABLE_OWNED_MOTION_FLAGSHIP_CLAIM_BLOCKERS = new Set([
  "flagship_rights_sidecar_source_mismatch",
  "flagship_rights_sidecar_creator_mismatch",
  "flagship_rights_sidecar_licence_mismatch",
  "flagship_rights_sidecar_platforms_mismatch",
  "flagship_inventory_source_record_mismatch",
  "flagship_inventory_creator_record_mismatch",
  "flagship_inventory_licence_record_mismatch",
  "flagship_inventory_platforms_record_mismatch",
]);

function currentValidatedOwnedMotionLedgerRecord(record = {}, targetPlatforms = ENABLED_PLATFORMS) {
  return Boolean(
    record.current_validated_owned_procedural_motion === true &&
      clean(record.reconciliation_basis) === "current_validated_owned_procedural_motion" &&
      clean(record.rights_decision_basis) ===
      ...
      completeMaterialRightsRecord(record, targetPlatforms) &&
      explicitRightsRestrictionReasons(record, targetPlatforms).length === 0
  );
}

function repairableOwnedMotionFlagshipClaimDrift(
  record = {},
  blockers = [],
  targetPlatforms = ENABLED_PLATFORMS,
) {
  return Boolean(
    blockers.length > 0 &&
      currentValidatedOwnedMotionLedgerRecord(record, targetPlatforms) &&
      blockers.every((blocker) =>
        REPAIRABLE_OWNED_MOTION_FLAGSHIP_CLAIM_BLOCKERS.has(clean(blocker).split(":")[0]),
      )
  );
}

function flagshipClaimsFromRecord(value = {}, record = {}) {
  return {
    ...value,
    source_url: clean(record.source_url),
    creator: clean(record.creator || record.source_owner),
    licence_basis: rightsBasis(record),
    commercial_use_allowed: true,
    rights_verdict: "GREEN",
    allowed_platforms: unique(
      asArray(record.allowed_platforms || record.platforms).map(normalisePlatformKey),
    ),
    credit_required: record.credit_required === true,
    risk_score: Number(record.risk_score) || 0,
    approval_status: clean(record.approval_status),
  };
}

function reconcileFlagshipInventoryClaims(inventory = {}, recordsByAssetId = new Map(), generatedAt = "") {
  const document = JSON.parse(JSON.stringify(inventory));
  const reconciledAssetIds = new Set();
  const visit = (asset) => {
    if (!asset || typeof asset !== "object" || Array.isArray(asset)) return;
    const assetId = clean(asset.asset_id || asset.id || asset.asset_identity || asset.identity);
    const record = recordsByAssetId.get(assetId);
    if (record) {
      Object.assign(asset, flagshipClaimsFromRecord(asset, record));
  ...
  for (const asset of asArray(
    document.used_assets || document.assets || document.used_asset_inventory,
  )) {
    visit(asset);
  }
  if (reconciledAssetIds.size) {
    document.rights_claims_reconciled_at = generatedAt;
    document.rights_claims_reconciled_assets = [...reconciledAssetIds].sort();
  }
  return {
    document,
    reconciled_asset_ids: [...reconciledAssetIds].sort(),
  };
}
...
async function prepareFlagshipRightsSidecarBindings({
  ...
  const inventoryClaimRecords = new Map();
  ...
  for (const asset of assets) {
    ...
    let claimBlockers = flagshipSidecarClaimBlockers({
      asset,
      document,
      record: matches[0],
    });
    const claimDriftRepaired = repairableOwnedMotionFlagshipClaimDrift(
      matches[0],
      claimBlockers,
    );
    if (claimDriftRepaired) {
      effectiveAsset = flagshipClaimsFromRecord(asset, matches[0]);
      effectiveDocument = flagshipClaimsFromRecord(document, matches[0]);
      claimBlockers = flagshipSidecarClaimBlockers({
        asset: effectiveAsset,
        document: effectiveDocument,
        record: matches[0],
      });
      if (!claimBlockers.length) inventoryClaimRecords.set(assetId, matches[0]);
    }
    ...
    const needsRebind = claimDriftRepaired ||
    ...
    discovered.push({
      ...
      claims_reconciled: claimDriftRepaired,
  ...
  if (inventoryClaimRecords.size) {
    const reconciledInventory = reconcileFlagshipInventoryClaims(
      inventory,
      inventoryClaimRecords,
      generatedAt,
    );
    if (reconciledInventory.reconciled_asset_ids.length !== inventoryClaimRecords.size) {
      blockers.push("flagship_inventory_claim_reconciliation_incomplete");
    } else {
      const inventoryBuffer = serialiseJson(reconciledInventory.document);
      inventoryUpdate = {
        path: inventoryPath,
        backup_path: await availableTransactionBackupPath(
          `${inventoryPath}.pre_candidate_evidence_reconciliation.${stamp}.bak`,
        ),
        document: reconciledInventory.document,
        sha256: sha256(inventoryBuffer),
        size_bytes: inventoryBuffer.length,
        reconciled_asset_ids: reconciledInventory.reconciled_asset_ids,
      };
    }
  }
  ...
  return {
    ...
    claim_rebound_count: inventoryClaimRecords.size,
    inventory_claim_rebound_count:
      inventoryUpdate?.reconciled_asset_ids?.length || 0,
...
async function reconcileRightsEvidence({
  ...
  for (const used of usedAssets) {
    ...
    const currentValidatedOwnedRights =
      currentValidatedOwnedProceduralMotionRights(used, targetPlatforms);
    ...
    const replaceableOwnedProjection = Boolean(
      currentValidatedOwnedRights &&
        provisionalOwnedMotionProjectionRecord(restrictiveExact, targetPlatforms),
    );
    const replaceableGenerationBoundNarration = Boolean(
      narrationCommercialRightsEvidence &&
        provisionalElevenLabsNarrationRecord(
          restrictiveExact,
          targetPlatforms,
        ),
    );
    if (
      ...
      !(
        ...
        replaceableOwnedProjection ||
        replaceableGenerationBoundNarration ||
        (
    ...
    const restrictiveNarrationDonor = used.kind === "narration"
      ? rows.find((row) =>
        sourceEquivalent(row, used) &&
          explicitRightsRestrictionReasons(row, targetPlatforms).length > 0 &&
          !(
            narrationCommercialRightsEvidence &&
            provisionalElevenLabsNarrationRecord(row, targetPlatforms)
          ),
      )
      : null;
    ...
    const generationBoundNarrationRights =
      generationBoundElevenLabsNarrationDonor({
        used,
        evidence: narrationCommercialRightsEvidence,
        targetPlatforms,
      });
    let donor = currentValidatedOwnedRights ||
      generationBoundNarrationRights ||
      ((used.kind === "narration" || used.current_owned_generated_card_sidecar === true) &&
        completeRightsRecord(used, targetPlatforms)
        ? used
    ...
    if (
      used.kind === "narration" &&
      !narrationCommercialRightsEvidence &&
      !declaredFingerprintMatches(donor, assetFingerprint)
    ) {
      blockers.push(`narration_rights_record_fingerprint_missing_or_mismatch:${used.asset_id}`);
      continue;
    }
    const evidenceFile = narrationCommercialRightsEvidencePath ||
      await evidencePathForUsedAsset(used, artifactDir, donor, workspaceRoot);
    ...
    if (
      used.kind === "narration" &&
      clean(used.provider_id).toLowerCase() === "pulse_local_tts"
    ) {
      const localNarrationEvidence = evidenceResolved
        ? await readJson(evidenceResolved, null)
        : null;
      const localNarrationValidation =
        await validateGeneratedNarrationRightsEvidence({
          evidence: localNarrationEvidence || {},
          audioPath: used.path,
          targetPlatforms,
        });
      if (localNarrationValidation.verdict !== "GREEN") {
        blockers.push(
          `narration_local_generated_rights_evidence_invalid:${used.asset_id}`,
          ...localNarrationValidation.blockers.map(
            (blocker) =>
              `narration_local_generated_rights_evidence:${blocker}:${used.asset_id}`,
          ),
        );
        continue;
      }
    ...
    const ownedProceduralMotion =
      used.kind === "video" && donor.current_validated_owned_procedural_motion === true;
    const providerText = [
      used.provider_id,
      used.source_type,
      used.source_url,
    ...
    const elevenLabsNarration =
      ownedNarration && /elevenlabs/.test(providerText);
    const epidemicAudio =
      (used.kind === "sfx" || used.kind === "music") &&
      /epidemic(?:_|\s|-)?sound/.test(providerText);
    const record = {
      ...
      path: used.path,
      local_materialized_path: used.path,
      source_url: ownedProceduralMotion
        ? clean(donor.source_url)
        : used.source_url || clean(donor.source_url),
      ...
      source_type: ownedProceduralMotion
        ? clean(donor.source_type)
        : used.source_type || clean(donor.source_type),
      source_family: used.source_family || clean(donor.source_family || donor.motion_family),
      source_owner: clean(
        (ownedProceduralMotion ? donor.source_owner || donor.creator : "") ||
          used.source_owner ||
      ...
      approval_status: clean(
        (ownedNarration ? donor.approval_status : used.approval_status) ||
      ...
      licence_basis: ownedNarration && !elevenLabsNarration
        ? rightsBasis(donor)
        : rightsBasis(used) || rightsBasis(donor),
      allowed_use: clean(
        (ownedNarration ? donor.allowed_use : used.allowed_use) ||
          donor.allowed_use,
      ) || null,
      commercial_use_allowed:
        donor.commercial_use_allowed === true ||
        (!ownedNarration && used.commercial_use_allowed === true),
      allowed_platforms: unique(
        asArray(
          (ownedNarration ? donor.allowed_platforms || donor.platforms : used.allowed_platforms) ||
            donor.allowed_platforms ||
            donor.platforms,
        ).map(normalisePlatformKey),
      ),
      credit_required: used.credit_required === true || donor.credit_required === true,
      live_publish_allowed: narrationCommercialRightsEvidence
        ? true
        : donor.live_publish_allowed,
      requires_human_legal_review_before_publish: narrationCommercialRightsEvidence
        ? false
        : donor.requires_human_legal_review_before_publish,
      requires_human_review_before_live_publish: narrationCommercialRightsEvidence
        ? false
        : donor.requires_human_review_before_live_publish,
      requires_human_publish_review: narrationCommercialRightsEvidence
        ? false
        : donor.requires_human_publish_review,
      risk_score: ownedNarration && Number.isFinite(Number(donor.risk_score))
        ? Number(donor.risk_score)
        : Number.isFinite(Number(used.risk_score))
  ...
  const safeDemotionRecovered = Boolean(
    reconciliationPassed &&
      renderManifest.safe_demotion_applied === true &&
      clean(renderManifest.status).toUpperCase() === "RED" &&
      clean(renderManifest.quality_gate_status).toLowerCase() ===
        "post_render_forensics_passed" &&
      clean(renderManifest.post_render_forensic_result).toLowerCase() === "pass" &&
      asArray(renderManifest.post_render_forensic_blockers).length === 0,
  );
  const reconciledRenderManifest = {
    ...renderManifest,
    ...(safeDemotionRecovered
      ? {
          status: "GREEN",
          safe_demotion_applied: false,
          rights_demotion_recovered_at: generatedAt,
        }
      : {}),
    rights_reconciliation: renderRightsReconciliation,
    ...(flagshipSidecarBindings.inventory_update
      ? {
          flagship_inventory_evidence: {
            ...(renderManifest.flagship_inventory_evidence || {}),
            complete: true,
            verdict: "GREEN",
            inventory_path: flagshipSidecarBindings.inventory_update.path,
            inventory_sha256: flagshipSidecarBindings.inventory_update.sha256,
            inventory_size_bytes: flagshipSidecarBindings.inventory_update.size_bytes,
            blockers: [],
          },
          flagship_inventory_reconciled_at: generatedAt,
        }
      : {}),
  };
  ...
  let flagshipRightsReportBackupPath = null;
  if (apply && !uniqueBlockers.length) {
    ...
    flagshipRightsReportBackupPath = await fs.pathExists(
      flagshipRightsReportPath,
    )
      ? await availableTransactionBackupPath(
        `${flagshipRightsReportPath}.pre_candidate_evidence_reconciliation.${stamp}.bak`,
      )
      : null;
    const refreshedFlagshipRightsReport = {
      schema_version: 1,
      story_id: storyId,
      ...
      flagship_sidecar_bindings: {
        ...
        rebuild_required:
          flagshipSidecarBindings.rebuild_required === true,
        ...
        claim_rebound_count: flagshipSidecarBindings.claim_rebound_count,
        inventory_claim_rebound_count:
          flagshipSidecarBindings.inventory_claim_rebound_count,
      ...
      final_state_verification_basis:
        "same_transaction_stored_rights_ledger_and_render_manifest",
    };
    const activeTransaction = fileTransaction || new LocalJsonFileTransaction();
    try {
      await fs.ensureDir(path.dirname(flagshipRightsReportPath));
      ...
      await activeTransaction.stageJson(
        flagshipRightsReportPath,
        refreshedFlagshipRightsReport,
        flagshipRightsReportBackupPath,
      );
      if (flagshipSidecarBindings.inventory_update) {
        await activeTransaction.stageJson(
          flagshipSidecarBindings.inventory_update.path,
          flagshipSidecarBindings.inventory_update.document,
          flagshipSidecarBindings.inventory_update.backup_path,
        );
      }
  ...
  return {
    ...
    flagship_rights_report_path: flagshipRightsReportPath,
    ...
    flagship_sidecar_bindings: {
      ...
      inventory_backup_path:
        flagshipSidecarBindings.inventory_update?.backup_path || null,
      rebound: flagshipSidecarBindings.rebound,
...
async function applySafeRightsDemotion({
  ...
  const qualityReportPaths = [...new Set([
    ...(await findFilesNamed(artifactDir, "benchmark_report.json")),
    ...(await findFilesNamed(artifactDir, "visual_quality_report.json")),
  ])].sort((left, right) => left.localeCompare(right));
  const temporalReportPaths = await findFilesNamed(
    path.join(artifactDir, "qa"),
    "temporal_video_qa_report.json",
  );
  const qualityReports = (
    await Promise.all(qualityReportPaths.map(async (reportPath) => ({
      path: reportPath,
      document: await readJson(reportPath, null),
    })))
  ).filter(({ document }) => objectHasKeys(document));
  const temporalReports = (
    await Promise.all(temporalReportPaths.map(async (reportPath) => ({
      path: reportPath,
      document: await readJson(reportPath, null),
    })))
  ).filter(({ document }) => objectHasKeys(document));
  ...
  const forensicQaBackupPath = objectHasKeys(forensicQa)
    ...
    : null;
  const qualityReportBackupPaths = await Promise.all(
    qualityReports.map(({ path: reportPath }) => (
      availableTransactionBackupPath(
        `${reportPath}.pre_candidate_evidence_reconciliation.demotion.${stamp}.bak`,
      )
    )),
  );
  const temporalReportBackupPaths = await Promise.all(
    temporalReports.map(({ path: reportPath }) => (
      availableTransactionBackupPath(
        `${reportPath}.pre_candidate_evidence_reconciliation.demotion.${stamp}.bak`,
      )
    )),
  ...
  const monotonicBlockers = unique([
  ...
  const demotedRenderManifest = {
    ...
    post_render_forensic_blockers: unique([
  ...
  const demotedForensicQa = objectHasKeys(forensicQa)
    ...
    : null;
  const demotedQualityReports = qualityReports.map((report) => ({
    ...report,
    document: {
      ...report.document,
      generated_at: generatedAt,
      ...
      publish_ready: false,
      scores: {
        ...(report.document.scores || {}),
        rights_risk_score: 0,
      },
      failures: unique([
        ...asArray(report.document.failures),
        ...monotonicBlockers,
      ]),
      blockers: unique([
        ...asArray(report.document.blockers),
        ...monotonicBlockers,
      ]),
      rights_reconciliation: demotedRenderRights,
      can_publish: false,
  ...
  const demotedTemporalReports = temporalReports.map((report) => ({
    ...report,
    document: {
      ...report.document,
      generated_at: generatedAt,
      temporal_scope_verdict: clean(
        report.document.temporal_scope_verdict || report.document.verdict,
      ) || "UNKNOWN",
      verdict: "RED",
      status: "RED",
      can_publish: false,
      ...
      global_publish_gate: {
        verdict: "RED",
        can_publish: false,
        blocker: "post_render_rights_reconciliation_not_green",
      },
      safe_demotion_applied: true,
    },
  }));
  ...
  for (let index = 0; index < demotedQualityReports.length; index += 1) {
    const report = demotedQualityReports[index];
    await transaction.stageJson(
      report.path,
      report.document,
      qualityReportBackupPaths[index],
    );
  }
  for (let index = 0; index < demotedTemporalReports.length; index += 1) {
    const report = demotedTemporalReports[index];
    await transaction.stageJson(
      report.path,
      report.document,
      temporalReportBackupPaths[index],
    );
  }
  try {
    const result = {
    ...
    return {
      ...
      rights: {
        ...
        quality_report_backup_paths: qualityReportBackupPaths,
        temporal_report_backup_paths: temporalReportBackupPaths,
        proposed_ledger: demotedLedger,
        proposed_render_rights_reconciliation: demotedRenderRights,
        transaction: result,
      },
