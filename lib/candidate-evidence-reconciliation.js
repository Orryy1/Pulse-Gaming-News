...
function verifiedMotionSourceIdentity(record = {}, artifactDir) {
  const identity = record.motion_source_identity && typeof record.motion_source_identity === "object"
    ? record.motion_source_identity
    : {};
  ...
  return {
    ...
    source_owner: strictMotionSourceOwner(record) || null,
  };
}
...
function hashBoundOfficialYoutubeIdentity(used = {}, metadata = {}) {
  ...
  const sourceVideoId = youtubeVideoIdFromUrl(used.source_url || identity.canonical_source_url);
  const authorName = clean(provenance.channel_identity?.author_name);
  const authorUrl = clean(provenance.channel_identity?.author_url);
  ...
  return Boolean(
    identity.strict_pass === true &&
      ...
      /^[a-f0-9]{64}$/i.test(clean(identity.source_master_sha256 || used.source_master_sha256)) &&
      authorName &&
      /^https:\/\/www\.youtube\.com\/@[^/]+$/i.test(authorUrl) &&
      sourceOwner &&
      sourceOwner.toLowerCase() === authorName.toLowerCase() &&
      /official_(?:youtube_channel|publisher_trailer_segment|platform_channel)/.test(sourceType) &&
      asArray(identity.blockers).length === 0
  );
}

function currentValidatedOfficialMotionRights(used = {}, targetPlatforms = ENABLED_PLATFORMS) {
  if (used.kind !== "video" || used.current_owned_generated_card_sidecar === true) return null;
  ...
  const officialBasisAllowed =
    ...
    (
      hashBoundOfficialYoutube &&
      /(?:official_publisher_promotional_video|official_platform_promotional_video)/i.test(officialBasis)
    );
  if (!validated || !officialType || !officialBasisAllowed || !trustedOfficialSource) {
    return null;
  }
  if (explicitRightsRestrictionReasons(metadata, targetPlatforms).length) return null;
  ...
  return {
    ...
    evidence_reference: "materialised_motion_clips.json",
...
async function collectUsedAssets({
  ...
  for (const [index, scene] of asArray(renderManifest.clip_scene_plan?.scenes).entries()) {
    ...
    const verifiedIdentity = verifiedMotionSourceIdentity(metadata || {}, artifactDir);
    ...
    used.push({
      ...
      story_id: storyId,
      kind: "video",
      asset_id: usedAssetId({ exact, metadata, storyId, kind: "video", index }),
      path: filePath,
      source_url: clean(
  ...
  const narrationPath = resolveAssetPath(
  ...
  if (narrationPath) {
    ...
    used.push({
      ...
      expected_asset_size_bytes: Number(
  ...
  for (const [index, selected] of asArray(sfxManifest.source_plan?.selected_assets).entries()) {
...
async function reconcileRightsEvidence({
  ...
  for (const used of usedAssets) {
    ...
    const currentValidatedRights = currentValidatedOfficialMotionRights(used, targetPlatforms);
  ...
  let updatedBridgeCandidate = null;
  ...
  if (resolvedBridgePath) {
    if (!(await fs.pathExists(resolvedBridgePath))) {
      ...
      if (bridgeCandidateIndex < 0) {
        ...
        const current = rows[bridgeCandidateIndex];
        updatedBridgeCandidate = {
          ...current,
  ...
  if (apply && !uniqueBlockers.length) {
    ...
    backupPath = `${rightsPath}.pre_candidate_evidence_reconciliation.${stamp}.bak`;
    ...
    try {
      ...
      if (resolvedBridgePath && updatedBridgeCandidate) {
        bridgeBackupPath = `${resolvedBridgePath}.pre_candidate_evidence_reconciliation.rights.${stamp}.bak`;
  ...
  if (apply && changed) {
    ...
    try {
      if (renderManifestChanged) {
        renderManifestBackupPath = `${renderManifestPath}.pre_candidate_evidence_reconciliation.${stamp}.bak`;
        await activeTransaction.stageJson(
          renderManifestPath,
      ...
      if (bridgeChanged) {
        backupPath = `${resolvedBridgePath}.pre_candidate_evidence_reconciliation.fingerprints.${stamp}.bak`;
...
async function reconcileLineageHashEvidence({
  ...
  const stamp = generatedAt.replace(/[^0-9]/g, "");
  const backups = changedKeys.map(
    (key) => `${manifestPaths[key]}.pre_candidate_lineage_reconciliation.${stamp}.bak`,
  );
  let sectionTransaction = null;
  let applied = false;
...
async function reconcileCandidateEvidence({
  ...
  if (apply && !preflightFailures.length) {
      ...
      if (!applyFailures.length) {
        ...
        try {
          ...
          const failedSection = (section, rightsSection = false) => ({
            ...
            verdict: "FAIL",
            applied: false,
            ...(rightsSection ? { bridge_rights_synced: false } : {}),
  ...
  return {
    ...
    apply_preflight: {
      ...
      writes_allowed: apply === true && preflightFailures.length === 0,
    },
    authority,
    rights,
    bridge_fingerprints,
