...
function hashBoundOfficialYoutubeIdentity(used = {}, metadata = {}) {
  ...
  const channelIdentity = resolvedYoutubeChannelIdentity(provenance, {
...
function currentValidatedOfficialMotionRights(used = {}, targetPlatforms = ENABLED_PLATFORMS) {
  ...
  const officialBasisAllowed =
    ...
    (
      hashBoundOfficialYoutube &&
      /(?:official_(?:publisher|platform)_promotional_(?:video|editorial_use))/i.test(officialBasis)
    );
...
async function prepareFlagshipRightsSidecarBindings({
  ...
  if (!assets.length) blockers.push("flagship_inventory_used_assets_empty");
  ...
  for (const asset of assets) {
    ...
    discovered.push({
      ...
      backup_path: needsRebind ? backupPath : null,
    });
    if (needsRebind) {
      updates.push({
        asset_id: assetId,
