...
function currentValidatedOfficialMotionRights(used = {}, targetPlatforms = ENABLED_PLATFORMS) {
  ...
  const policyEvidence = resolveAssetPath(metadata.evidence_reference, "");
  const documentedPublisherVideoPolicy = Boolean(
    hashBoundOfficialYoutube &&
      ...
      policyEvidence &&
      fs.existsSync(policyEvidence) &&
      fs.statSync(policyEvidence).isFile() &&
      fs.statSync(policyEvidence).size > 0 &&
