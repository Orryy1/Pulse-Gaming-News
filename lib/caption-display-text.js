...
const GRAND_THEFT_AUTO_STUTTER_SIX_RE =
  /\bGrand\s+Theft\s+Auto\s+s(?:i|y|igh)?[-\s]*six\b/gi;
const GTA_STUTTER_SIX_RE =
  /\bG\.?\s*T\.?\s*A\.?\s+s(?:i|y|igh)?[-\s]*six\b/gi;
...
function captionSixStutterPrefix(value) {
  return /^(?:si|sy|sigh|s)$/i.test(String(value || ""));
}

function captionSixStutterToken(value) {
  return /^(?:si|sy|sigh|s)-?six$/i.test(String(value || ""));
}
