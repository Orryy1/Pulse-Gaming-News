...
const GRAND_THEFT_AUTO_STUTTER_SIX_RE =
  /\bGrand\s+Theft\s+Auto\s+(?:s(?:i|y|igh)?|see|sea|c)[-\s]*six\b/gi;
const GTA_STUTTER_SIX_RE =
  /\bG\.?\s*T\.?\s*A\.?\s+(?:s(?:i|y|igh)?|see|sea|c)[-\s]*six\b/gi;
...
function captionSixStutterToken(value) {
  return /^(?:si|sy|sigh|s|see|sea|c)-?six$/i.test(String(value || ""));
}
