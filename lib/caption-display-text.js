...
const GRAND_THEFT_AUTO_STUTTER_SIX_RE =
  /\bGrand\s+Theft\s+Auto\s+(?:(?:s\s*i|s(?:i|y|igh)?|see(?:[-\s]+see)?|sea|c)[-\s]*(?:six|6)|siix)\b/gi;
const GTA_STUTTER_SIX_RE =
  /\bG\.?\s*T\.?\s*A\.?\s+(?:(?:s\s*i|s(?:i|y|igh)?|see(?:[-\s]+see)?|sea|c)[-\s]*(?:six|6)|siix)\b/gi;
...
function captionSixStutterPrefix(value) {
  return /^(?:si|sy|sigh|s|see|sea|c)$/i.test(String(value || ""));
}

function captionSixStutterToken(value) {
  return /^(?:(?:si|sy|sigh|s|see|sea|c)-?(?:six|6)|siix)$/i.test(String(value || ""));
}
