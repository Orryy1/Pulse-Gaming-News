...
function isEmphasisWord(word, storyTokens) {
  ...
  const candidates = [stripped, ...stripped.split(/\s+/).filter(Boolean)];
  for (const candidate of candidates) {
    if (storyTokens.has(candidate.toUpperCase())) return true;
 ...
 */
function tryMatchSmallNumber(tokens, tsIdx, expectNum) {
  if (tsIdx >= tokens.length) return null;
  const t0 = normWord(tokens[tsIdx].word);
  if (DIGIT_WORD[t0] === expectNum) return { lastIdx: tsIdx };
  if (TENS_WORD[t0] === expectNum) return { lastIdx: tsIdx };
  if (TENS_WORD[t0] !== undefined && tsIdx + 1 < tokens.length) {
    const t1 = normWord(tokens[tsIdx + 1].word);
    if (DIGIT_WORD[t1] !== undefined && TENS_WORD[t0] + DIGIT_WORD[t1] === expectNum) {
      return { lastIdx: tsIdx + 1 };
    }
  }
  return null;
...
const ROMAN_SPOKEN_ALIASES = {
  ii: ["two", "sequel", "number two"],
  v: ["five"],
  vi: ["six"],
};

function tryMatchRomanNumeralExpansion(tokens, tsIdx, roman) {
  if (tsIdx >= tokens.length) return null;
  const aliases = ROMAN_SPOKEN_ALIASES[String(roman || "").toLowerCase()] || [];
  const t0 = normWord(tokens[tsIdx].word);
  if (aliases.includes(t0)) return { lastIdx: tsIdx };
  if (aliases.includes("number two") && t0 === "number" && tsIdx + 1 < tokens.length) {
    const t1 = normWord(tokens[tsIdx + 1].word);
    if (t1 === "two") return { lastIdx: tsIdx + 1 };
  }
  return null;
}

function tryMatchAcronymExpansion(tokens, tsIdx, acronym) {
  const letters = String(acronym || "").split("");
  if (letters.length < 2 || letters.length > 6) return null;
  if (tsIdx + letters.length - 1 >= tokens.length) return null;

  for (let i = 0; i < letters.length; i++) {
    if (normWord(tokens[tsIdx + i].word) !== letters[i].toLowerCase()) {
      return null;
    }
  }
  return { lastIdx: tsIdx + letters.length - 1 };
}
...
function realignTimestampsToScript(scriptText, timestamps) {
  ...
  const out = [];
  let ti = 0;
  // SAFETY RESET: count consecutive non-matches. If we ever hit a
  // long run of mismatches, the realigner has clearly desynced
  // (e.g. script and audio came from different texts — filler in
  // one but not the other). When that happens, abort the
  // realignment for the rest of the stream and emit raw timestamp
  // tokens unmodified. Better to have the spoken-form text in
  // captions than 27 seconds of orphan fragments.
  ...
  for (const scriptToken of scriptTokens) {
    ...
    if (/^[A-Z]{2,6}$/.test(cleanScript)) {
      const m = tryMatchAcronymExpansion(timestamps, ti, cleanScript);
      if (m) {
        out.push({
          word: scriptToken,
    ...
    if (ROMAN_SPOKEN_ALIASES[lcScript]) {
      const m = tryMatchRomanNumeralExpansion(timestamps, ti, lcScript);
      if (m) {
        out.push({
          word: scriptToken,
          start: timestamps[ti].start,
    ...
    if (/^\d{1,2}$/.test(cleanScript)) {
      ...
      const m = tryMatchSmallNumber(timestamps, ti, n);
      if (m) {
        out.push({
          word: scriptToken,
          start: timestamps[ti].start,
    ...
    //    Track consecutive misses; if we cross MISS_THRESHOLD,
    //    abort realignment for the rest of the stream.
    out.push(t);
    ti++;
    consecutiveMisses++;
  ...
  // Append any remaining spoken tokens — either because the script
  // ran out before the timestamps did, or because the safety reset
  // aborted realignment. Raw timestamps are correct, just spoken-
  // form (may include "twenty 39" instead of "2039") — that's
  // acceptable; a 26s caption blackout is not.
...
function renderPhraseText(phrase, storyTokens) {
  ...
  for (const w of phrase.words) {
    ...
    const normalised = normaliseCaptionDisplayText(w.word.replace(/([.!?])([A-Z])/g, "$1 $2"));
...
function buildAss({ story, words, duration, scriptText }) {
  ...
  const displayWords = normaliseCaptionDisplayWords(realigned, { textKey: "word" });
  const phrases = groupIntoPhrases(displayWords);
