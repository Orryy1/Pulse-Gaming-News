...
function tryMatchRomanTwoExpansion(tokens, tsIdx) {
  if (tsIdx >= tokens.length) return null;
  const t0 = normWord(tokens[tsIdx].word);
  if (t0 === "two") return { lastIdx: tsIdx };
  if (t0 === "sequel") return { lastIdx: tsIdx };
  if (t0 === "number" && tsIdx + 1 < tokens.length) {
    const t1 = normWord(tokens[tsIdx + 1].word);
...
function realignTimestampsToScript(scriptText, timestamps) {
  ...
  for (const scriptToken of scriptTokens) {
    ...
    if (lcScript === "ii") {
      const m = tryMatchRomanTwoExpansion(timestamps, ti);
      if (m) {
        out.push({
          word: scriptToken,
          start: timestamps[ti].start,
