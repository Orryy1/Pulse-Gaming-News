...
function classifyVertical(story, text) {
  ...
  const gamingContext =
    /\b(?:game|gaming|player|players|xbox|playstation|nintendo|steam|switch|ps5|rpg|racer|trailer|release date|developer|studio|publisher|combat|gameplay|early access|survival game|survival games|canonical_game)\b/.test(
      [
        text,
  ...
  if (gamingContext) return "gaming";
...
function dominantCommercialStoryText(story = {}) {
  return [
    story.selected_title,
    story.short_title,
    story.title,
...
function detectIntent(story, vertical, text) {
  const dominantText = normaliseText(dominantCommercialStoryText(story));
  if (vertical === "crypto") {
    const highRisk =
  ...
  if (
    /\b(?:controller|wireless controller|headset|accessor(?:y|ies)|limited[-\s]?edition)\b/.test(dominantText) &&
    /\b(?:controller|wireless controller|headset|accessor(?:y|ies)|hardware)\b/.test(text)
  ) {
    return {
      type: "controller_accessory_context",
      angle: "Controller and headset buying checks",
      highRisk: false,
    };
  }

  if (/\b(?:forza|gran turismo|racing wheel|racing game|f1\s*\d*|steam numbers?)\b/.test(dominantText)) {
    return {
...
function intentOffers(intentType) {
  ...
  if (intentType === "controller_accessory_context") {
    return [
      {
        label: "Xbox controller",
        query: "Xbox wireless controller",
      ...
      },
      {
        ...
        query: "Xbox wireless headset gaming headset",
        category: "headset",
        relevance: 82,
        conversion: 72,
        commission: 54,
      ...
      {
        label: "Controller charging kit",
        query: "Xbox controller charging dock",
        category: "accessory",
        relevance: 76,
        conversion: 66,
        commission: 44,
...
function buildCandidateLink(offer, opts) {
  ...
  return {
    ...
    specificity: offer.specificity || "contextual_search",
    confidence: Number.isFinite(offer.confidence) ? offer.confidence : null,
    exact_product_claim: offer.exact_product_claim === true,
    story_specific: offer.story_specific === true,
...
function selectedPublicFields(link) {
  ...
  return {
    ...
    exact_product_claim: link.exact_product_claim,
    story_specific: link.story_specific === true,
...
function buildAffiliateLinkManifest({
  ...
  let intent = detectIntent(story, vertical, text);
  const complianceRisk =
    vertical === "crypto" && intent.highRisk ? 90 : vertical === "finance" && intent.highRisk ? 65 : 0;
  const rejectionReasons = [];
  let offers = [];
  const storySpecificOffers = buildStorySpecificLinks(story, tag).map((link) => ({
    label: link.label,
    query: link.query,
    category: link.category || "game",
    relevance: 94,
    conversion: 76,
    commission: 42,
    specificity: link.specificity,
  ...
  const hasPlatformSpecificStoryOffer = storySpecificOffers.some(
    (offer) => offer.specificity === "story_platform_search",
  );

  if (
    vertical === "gaming" &&
    intent.type === "no_safe_commercial_intent" &&
    hasPlatformSpecificStoryOffer
  ) {
    intent = {
      type: "story_specific_game_interest",
      angle: "Story-specific game and platform buying checks",
      highRisk: false,
    };
    offers = storySpecificOffers;
  } else if (intent.type === "no_safe_commercial_intent") {
    ...
    vertical === "gaming" &&
    intent.type === "gaming_setup_or_buying_context" &&
    storySpecificOffers.length > 0
  ) {
    intent = {
      type: "story_specific_game_interest",
      angle: "Story-specific game and platform buying checks",
