...
const REQUIRED_FILES = {
  ...
  productionGrammarPatterns: "production_grammar_patterns.json",
...
function modalCount(items = [], selector) {
...
function hourBucket(value) {
  ...
  return `${String(date.getUTCHours()).padStart(2, "0")}:00_utc`;
}

function medianDuration(metadata = []) {
  return median(metadata.map((item) => item.duration_s).filter((value) => value != null));
}

function buildProductionGrammarPatterns({
  metadata = [],
  titleSeo,
  hook,
  visual,
  ...
  const grammar = [
    {
      category: "title",
      observed_patterns: asArray(titleSeo?.patterns).map((pattern) => pattern.pattern),
      pulse_application: "Use named subjects, consequence and formula rotation without copying competitor phrasing.",
      quality_signal: "title_strength_score",
    },
    {
      category: "hook",
      observed_patterns: asArray(hook?.patterns).map((pattern) => pattern.pattern),
      pulse_application: "Open with subject, consequence or source-locked novelty in the first spoken beat.",
      quality_signal: "first_3_seconds_score",
    },
    {
      category: "first_frame",
      observed_patterns: asArray(visual?.patterns).map((pattern) => pattern.pattern).filter((pattern) => /subject|mobile|motion/i.test(pattern)),
      pulse_application: "Make the subject and visual proof readable immediately on mobile.",
      quality_signal: "first_frame_score",
    },
    {
      category: "shot_length",
      observed_patterns: [
        `median_duration_s:${medianDuration(metadata) || "unknown"}`,
        "shorts_native_runtime",
        "beat_change_before_viewer_fatigue",
      ],
      pulse_application: "Keep short-form runtime tight and vary shot duration instead of using static cards.",
      quality_signal: "motion_density_score",
    },
    {
      category: "caption_style",
      observed_patterns: ["large_mobile_keywords", "source_label_clear", "no_dense_caption_blocks"],
      pulse_application: "Use short keyword captions that reinforce, not duplicate, narration.",
      quality_signal: "mobile_readability_score",
    },
    {
      category: "sfx_transition",
      observed_patterns: asArray(sound?.patterns).map((pattern) => pattern.pattern),
      pulse_application: "Use voice-first beds, hook hits and varied transition cues with no silence gaps.",
      quality_signal: "sound_design_score",
    },
    {
      category: "thumbnail_layout",
      observed_patterns: ["named_subject_or_object", "high_contrast_text_hierarchy", "no_generic_dark_frame"],
      pulse_application: "Generate platform covers with clear entity, readable text and source/title parity.",
      quality_signal: "mobile_readability_score",
    },
    {
      category: "posting_time",
      observed_patterns: modalCount(metadata, (item) => hourBucket(item.published_at)).map((row) => `${row.value}:${row.count}`),
      pulse_application: "Use posting-time observations as scheduling evidence, not as a blind global rule.",
      quality_signal: "publish_cadence_score",
    },
    {
      category: "topic_choice",
      observed_patterns: modalCount(metadata, (item) => item.topic_category).map((row) => `${row.value}:${row.count}`),
      pulse_application: "Prioritise topics with named entities, fresh proof, player impact and official/source-backed motion.",
      quality_signal: "source_trust_score",
    },
    {
      category: "outlier_diagnostics",
      observed_patterns: asArray(successFailure?.success_patterns).map((pattern) => pattern.pattern),
      pulse_application: "Convert outlier patterns into original Pulse gates and test them against future uploads.",
      quality_signal: "competitor_surpass_score",
    },
    {
      category: "flop_diagnostics",
      observed_patterns: asArray(successFailure?.flop_patterns).map((pattern) => pattern.pattern),
      pulse_application: "Block generic, slow or commercially thin formats before they enter GREEN scheduling.",
      quality_signal: "competitor_parity_score",
    },
  ];
  return {
    schema_version: 1,
    goal: GOAL_ID,
    extraction_policy: "Store production grammar and Pulse-owned rules only. Do not store or copy competitor assets, transcripts, footage, thumbnails, music, branding or templates.",
    categories: grammar.map((item) => ({
      ...item,
      lawful_extraction: "structure_and_metrics_only",
      copied_asset_storage: false,
      original_pulse_rule_required: true,
    })),
    evidence_summary: {
      assessed_video_count: metadata.length,
      strong_outlier_count: strong.length,
      weak_reference_count: weak.length,
      category_count: grammar.length,
    },
    safety: {
      patterns_only: true,
      no_copied_assets_stored: true,
      no_full_transcripts_stored: true,
      no_competitor_media_downloads: true,
      no_template_clone_allowed: true,
    },
  };
}
...
async function buildCompetitorForensicsLab({
  ...
  const productionGrammarPatterns = buildProductionGrammarPatterns({
    metadata,
    titleSeo,
    hook,
    visual,
  ...
  return {
    ...
    summary: {
      ...
      production_grammar_category_count: productionGrammarPatterns.categories.length,
    ...
    production_grammar_patterns: productionGrammarPatterns,
...
function renderCompetitorForensicsLabMarkdown(report = {}) {
  ...
  lines.push(`Production grammar categories: ${report.summary?.production_grammar_category_count || 0}`);
...
module.exports = {
  ...
  _private: {
    ...
    buildProductionGrammarPatterns,
    inferTopicCategory,
    normaliseMetadata,
    titleShape,
  },
