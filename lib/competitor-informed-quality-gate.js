...
async function inspectFinalVideoFile({
  artifactDir,
  storyId = "",
  canonical = {},
  director = {},
  ...
  const mediaQa = selectedPath
    ? await runPlatformVideoQa(selectedPath, { platform: "youtube_shorts" })
    : { result: "fail", failures: ["mp4_not_on_disk"], warnings: [] };
  const rawMediaQaStatus = cleanText(mediaQa.result).toLowerCase() || "fail";
  const decodable = ["pass", "warn"].includes(rawMediaQaStatus);
  const measuredDurationSeconds = selectedPath
    ? numberOrNull(ffprobeDuration(selectedPath, { timeoutMs: 15000 }))
    : null;
  ...
  const blockers = [];
  if (!selectedPath) {
    blockers.push("media_house:final_video_missing_or_unreadable");
  }
  if (selectedPath && !decodable) {
    blockers.push("media_house:final_video_unreadable");
  }
  ...
  const mediaQaWarnings = asArray(mediaQa.warnings);
  const sha256 = selectedPath ? await sha256File(selectedPath).catch(() => null) : null;
  return {
    status: blockers.length ? "blocked" : mediaQaWarnings.length ? "amber" : "pass",
    ...
    decodable,
    media_qa_status: rawMediaQaStatus === "skip" ? "fail" : rawMediaQaStatus,
    media_qa_failures: asArray(mediaQa.failures),
    media_qa_warnings: mediaQaWarnings,
...
function applyFinalVideoGate(score = {}, finalVideoReport = {}) {
  ...
  const mediaWarnings = asArray(finalVideoReport.media_qa_warnings).map(
    (warning) => `media_house:final_video_${cleanText(warning)}`,
  );
  if (!blockers.length) {
    if (mediaWarnings.length) {
      const verdict = cleanText(score.verdict).toUpperCase() === "GREEN" ? "AMBER" : score.verdict;
      return {
        ...score,
        verdict,
        status: verdict === "AMBER" ? "human_review" : score.status,
        warnings: unique([...(score.warnings || []), ...mediaWarnings]),
        final_video_report: finalVideoReport,
        upgraded_quality_gate_report: {
          ...(score.upgraded_quality_gate_report || {}),
          verdict,
          status: verdict === "AMBER" ? "human_review" : score.upgraded_quality_gate_report?.status,
          warnings: unique([
            ...(score.upgraded_quality_gate_report?.warnings || []),
            ...mediaWarnings,
          ]),
        },
      };
...
function footageEvidenceSeverity(evidence = {}) {
  const values = [
    evidence.verdict,
    evidence.status,
    evidence.result,
  ...
  if (values.some((value) => /red|fail|blocked/.test(value))) return 3;
  if (
    values.some((value) => /amber|warn|review|pending/.test(value)) ||
    asArray(evidence.warnings).length ||
    asArray(evidence.readiness?.warnings).length
  ) {
    return 2;
  }
  if (values.some((value) => /green|pass|ready/.test(value))) return 1;
  return 0;
}

function mergeFootageEvidence(aggregateEvidence = {}, packageEvidence = {}) {
  if (!objectHasKeys(aggregateEvidence)) return packageEvidence;
  if (!objectHasKeys(packageEvidence)) return aggregateEvidence;

  const severity = Math.max(
    footageEvidenceSeverity(aggregateEvidence),
    footageEvidenceSeverity(packageEvidence),
  );
  const blockers = unique([
    ...asArray(aggregateEvidence.blockers),
    ...asArray(aggregateEvidence.hard_failures),
    ...asArray(packageEvidence.blockers),
    ...asArray(packageEvidence.hard_failures),
  ]);
  const readinessBlockers = unique([
    ...asArray(aggregateEvidence.readiness?.blockers),
    ...asArray(packageEvidence.readiness?.blockers),
  ]);
  if (severity === 3 && !blockers.length && !readinessBlockers.length) {
    blockers.push("critical_footage_evidence_not_green");
  }

  const readinessStatus = severity === 3
    ? "v4_motion_blocked"
    : severity === 2
      ? "AMBER"
      : cleanText(packageEvidence.readiness?.status || aggregateEvidence.readiness?.status);
  return {
    ...aggregateEvidence,
    ...packageEvidence,
    verdict: severity === 3 ? "RED" : severity === 2 ? "AMBER" : "GREEN",
    blockers,
    ...
    warnings: unique([
      ...asArray(aggregateEvidence.warnings),
      ...asArray(packageEvidence.warnings),
    ]),
    readiness: {
      ...(aggregateEvidence.readiness || {}),
      ...(packageEvidence.readiness || {}),
      status: readinessStatus,
      blockers: readinessBlockers,
      warnings: unique([
        ...asArray(aggregateEvidence.readiness?.warnings),
        ...asArray(packageEvidence.readiness?.warnings),
      ]),
    },
    evidence_sources: {
      aggregate_report_present: true,
      package_artifact_present: true,
      reconciliation: "monotonic_most_restrictive",
    },
  };
}
...
async function inspectStoryPackage(storyPackage = {}, context = {}) {
  ...
  const aggregateFootageEvidence = context.footageEmpireIndex instanceof Map
    ? context.footageEmpireIndex.get(storyId) || {}
    : {};
  const footageEmpireV2 = mergeFootageEvidence(
    aggregateFootageEvidence,
    packageFootageInventory,
  );
