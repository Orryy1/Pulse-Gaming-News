  ...
  return {
    ...
    director: {
      shot_plan: [
        ...
        { id: "source_lock", kind: "source_lock", startS: 2.4, durationS: 1.5 },
        { id: "payoff_card", kind: "proof_card", startS: 5.2, durationS: 2 },
      ],
    ...
    platformManifest: {
      outputs: {
        ...
        tiktok: { caption: `${candidate.canonical_subject}: the player consequence, not a recap.` },
        instagram_reels: { cover_title: title.split(" ").slice(0, 6).join(" ") },
      },
    },
...
function compareCandidate(candidate = {}, generatedAt) {
  const baseline = buildPulseMediaHouseScore({ ...baselineScoreInput(candidate), generatedAt });
  const upgraded = buildPulseMediaHouseScore({ ...upgradedScoreInput(candidate), generatedAt });
  ...
  const upgradedWins = delta > 0 && upgraded.verdict !== "RED";
...
function buildNextRenderQueue(greenCandidates = {}) {
  return {
    schema_version: 1,
    goal: GOAL_ID,
    command: "npm run ops:goal-render-inputs",
    queue: asArray(greenCandidates.candidates).map((candidate, index) => ({
      priority: index + 1,
      story_id: candidate.story_id,
...
function renderHumanReviewUpgradePack(report = {}) {
  ...
  lines.push("");
  for (const candidate of asArray(report.upgraded_green_candidates?.candidates).slice(0, 15)) {
...
async function buildCompetitorUpgradeBakeoff({
  ...
  const nextRenderQueue = buildNextRenderQueue(greenCandidates);
  ...
  const verdict =
    ...
    unsafe.length === 0 &&
    greenCandidates.candidates.length > 0
      ? "PASS"
  ...
  const report = {
    ...
    human_review_upgrade_pack: renderHumanReviewUpgradePack({
      generated_at: generatedAt,
      summary: { upgraded_green_candidate_count: greenCandidates.candidates.length },
      upgraded_green_candidates: greenCandidates,
    }),
    safety: {
