...
function isOfficialReference(reference) {
  ...
  const hashBoundOfficialYoutubeLocalMaster =
    isHashBoundOfficialYoutubeLocalMaster(reference, urlKind);
  const directOfficialPageReference =
    urlKind.segment_validation_eligible === true &&
    /(official_publisher_or_developer_trailer_page|official_game_website_media_page|platform_storefront|platform_storefront_video_reference|official_platform_product_page|official_social_media_video|licensed_direct_media_url|operator_supplied_local_video)/.test(
      sourceType,
    );
  return (
    Boolean(source) &&
    ...
    (hashBoundOfficialYoutubeLocalMaster ||
      (reference?.downloads_allowed !== true &&
        (directOfficialPageReference ||
          /(steam_movie|steam_storefront_video_reference|igdb_video|official_trailer|publisher_video|platform_video)/.test(
            sourceType,
          )))) &&
...
function resolveMaxReferencesPerEntity(references, requestedValue) {
  if (requestedValue !== undefined && requestedValue !== null) {
    return normalisePositiveInteger(requestedValue, 1);
  }

  const officialReferences = dedupeReferences(references.filter(isOfficialReference));
  const entities = new Set(
    officialReferences.map((reference) => String(reference.entity || "").toLowerCase()).filter(Boolean),
  );
  const hasAnonymousReferences = officialReferences.some((reference) => !reference.entity);

  if (entities.size === 1 && !hasAnonymousReferences && officialReferences.length >= 2) {
    return 2;
  }
  return 1;
}
...
function selectReferences(references, maxReferences = 4, options = {}) {
  ...
  const maxReferencesPerEntity = resolveMaxReferencesPerEntity(
    references,
    options.maxReferencesPerEntity,
  );
  const selected = [];
...
function buildControlledFrameExtractionPlan(motionPlan, options = {}) {
  const sourceReferences = asArray(motionPlan?.existing_references);
  const maxReferencesPerEntity = resolveMaxReferencesPerEntity(
    sourceReferences,
    options.maxReferencesPerEntity,
  );
  ...
  return {
    ...
    selected_references: selectedReferences.map((reference, index) => {
      ...
      return {
        ...
        source_duration_s: sourceDurationSeconds(reference),
