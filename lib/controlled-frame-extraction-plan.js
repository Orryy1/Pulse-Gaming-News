...
const FRAME_PERCENTS = [0.18, 0.52];
...
function selectReferences(references, maxReferences = 4) {
  ...
  for (const reference of sorted) {
    ...
    if (selected.length >= maxReferences) return selected;
  }
...
function targetFramesForReferences(references) {
  const frames = [];
  for (const reference of references) {
