...
const SHORT_OFFICIAL_REFERENCE_GRACE_S = 8;
...
function allowsShortOfficialReferenceWindow(record = {}, minimumStartSeconds = MIN_OFFICIAL_FRAME_SAMPLE_S) {
  const duration = sourceDurationSeconds(record);
  if (!Number.isFinite(duration)) return false;
  return duration <= minimumStartSeconds + SHORT_OFFICIAL_REFERENCE_GRACE_S;
}

function isOfficialTrailerFrameSource(sourceType) {
  return /\b(movie|trailer|video|steam_movie|steam_storefront_video_reference|igdb_video|official_trailer)\b/i.test(
    String(sourceType || ""),
  );
}
...
function officialTrailerFrameRejectReason(frame, qa = frame?.qa || {}) {
  ...
  const qaGuardedEarlyWindow =
    frame?.allowEarlyExploratoryWindow === true ||
    frame?.allow_early_exploratory_window === true ||
    frame?.provenance?.allowEarlyExploratoryWindow === true ||
    frame?.provenance?.allow_early_exploratory_window === true;

  if (
    Number.isFinite(targetSeconds) &&
    targetSeconds < MIN_OFFICIAL_FRAME_SAMPLE_S &&
    !allowsShortOfficialReferenceWindow(frame) &&
    !(qaGuardedEarlyWindow && targetSeconds >= 6)
  ) {
  ...
  const taste = classifyTrailerFrameTaste({
    ...prescan,
    detected_text: qa.detected_text || prescan.detected_text,
    ocr_text: qa.ocr_text || prescan.ocr_text,
    embedded_text: qa.embedded_text || prescan.embedded_text,
  });
...
function defaultExtractor({ sourceUrl, localPath, outputPath, seekSeconds, timeoutMs = 45000 }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    execFile(
      "ffmpeg",
      buildStillFrameExtractionArgs({ source, outputPath, seekSeconds }),
