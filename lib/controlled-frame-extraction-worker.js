...
const { computeContentHash, prescanImage } = require("./visual-content-prescan");
...
async function processPlan(plan, options, seenHashes) {
  ...
  for (const [index, frame] of targetFrames.entries()) {
    ...
    try {
      ...
      const duplicate = qa.content_hash && seenHashes.has(qa.content_hash);
      let status = "accepted";
      if (duplicate) {
        status = "rejected_duplicate";
      } else if (qa.verdict === "fail" || qa.thumbnail_safe === false) {
        status = "rejected_qa";
      } else if (qa.content_hash) {
        seenHashes.add(qa.content_hash);
      }
      const record = {
        ...base,
        status,
      ...
      provenance.push({
        ...
        thumbnail_safe: qa.thumbnail_safe,
        status,
        reason: status,
      });
    } catch (err) {
...
function summarisePlans(plans, applyLocal) {
  ...
  return {
    ...
    frames_rejected: allFrames.filter((frame) =>
      ["rejected_duplicate", "rejected_qa", "extract_failed"].includes(frame.status),
    ).length,
