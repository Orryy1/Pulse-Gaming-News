...
const FORMATS = [
  ...
  {
    id: "evergreen_verdict_short",
    label: "Evergreen Verdict Short",
    viewerPromise:
      "A source-backed verdict, ranking or comparison that stays useful after the news cycle.",
    idealRuntimeSeconds: { min: 61, max: 90, target: 82 },
    sourceConfidence: ["verified", "confirmed"],
    mediaInventory: {
      minClass: "standard_video",
      minExactSubjectClips: 5,
      minDistinctMotionFamilies: 2,
      requireTrailerOrGameplay: true,
    },
    scriptStructure: [
      "immediate premise (0-3s, no channel intro)",
      "explicit judging criteria (3-8s)",
      "three or more verdict beats with one concrete reason each",
      "concise final verdict and at most one contextual CTA",
    ],
    titlePatterns: [
      "Which <franchise entry> Is Best For <specific player need>?",
      "The <mechanic> That Aged Best In Every <franchise era>",
      "<entry A> Or <entry B>: Which Respects Your Time?",
      "Is <game> Still Worth Playing In <year>?",
    ],
    seo: {
      descriptionFirst200:
        "franchise + exact verdict premise + primary official sources",
      tagFamilies: [
        "franchiseTags",
        "platformTags",
        "evergreenIntentTags",
    ...
    shortsRepurposing:
      "Cross-post the governed vertical master to YouTube, Instagram and Facebook; create a continuation only after retention evidence.",
    analyticsToTrack: [
      "views_7d",
      "views_30d",
      "average_view_percentage",
    ...
    monetisation:
      "Evergreen discovery and affiliate-compatible intent after retention is established; editorial verdict comes first.",
    promotionRules:
      "Continue the two-per-week experiment only when 72-hour retention and subscriber conversion meet the governed thresholds.",
    demotionRules:
      "Pause after two policy incidents, weak retention or repeated franchise/shape fatigue.",
    reviewRequirements: [
      "all judgement claims have source bindings",
      "verified first-hand evidence for any first-person play claim",
      "at least 65% exact-subject motion",
      "complete rights ledger",
      "strict green render and human visual review",
    ],
  },
  {
    id: "daily_briefing",
...
function selectFormatForStory(story, inventory) {
  ...
  if (explicitFormat === "evergreen_verdict_short") {
    const evergreen = getFormat("evergreen_verdict_short");
    if (
      meetsRequirements(evergreen, {
        sourceConfidence,
        classification: cls,
      })
    ) {
    ...
    return {
      format: getFormat("blog_only"),
      reasons: [
        "explicit_evergreen_verdict_below_source_or_motion_bar",
      ],
    };
  }
