...
function thumbnailCandidateCheck(story, opts) {
  ...
  const exists = candidatePath
    ? fs.existsSync(
        mediaPaths.resolveExistingSync(candidatePath) || path.resolve(candidatePath),
      )
    : false;
  return {
...
function runtimeReductionCheck(inventory, runtimePlan) {
  if (!runtimePlan.shouldRender) {
    return {
      ...
      severity: "fail",
      detail: runtimePlan.blocker
        ? `should not render (${runtimePlan.blocker})`
        : `should not render (route=${runtimePlan.route})`,
    };
  }
  return {
    id: "runtime_recommendation",
...
function evaluateStoryVisualQa(story, opts = {}) {
  ...
  const runtimePlan = recommendRuntime(inventory, { story });
