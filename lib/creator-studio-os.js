...
function quotedFacts(story) {
  const facts = [];
  if (story?.title) facts.push(story.title);
...
function buildMediaInventory(story) {
  ...
  return {
    ...
    studio_v2_selected_band_eligibility:
      exactSubject.studio_v2_selected_band_eligible,
...
function buildFormatRoute({ story, dossier, sourcePack, mediaInventory }) {
  const reasons = [];
  ...
  if (explicitFormat === "evergreen_verdict_short") {
    if (
      !["verified", "confirmed"].includes(
        String(sourcePack.confidence_level || "").toLowerCase(),
      )
    ) {
      return {
        verdict: "blog_only",
        reasons: ["evergreen_verdict_requires_verified_sources"],
      };
    }
    if (
      !["premium_ready", "standard_ready"].includes(
        mediaInventory.verdict,
      )
    ) {
      return {
        verdict: "blog_only",
        reasons: ["evergreen_verdict_requires_motion_ready_media"],
      };
    }
    return {
      verdict: "evergreen_verdict_short",
      reasons: [
        "explicit_evergreen_verdict_intent",
        "verified_source_and_motion_ready_media",
      ],
    };
  }
...
function endBeatTimestamp(editorialContract, offsetSeconds = 0) {
  const maxSeconds = Number(editorialContract?.max_seconds);
  if (!Number.isFinite(maxSeconds) || maxSeconds <= 0) {
    return offsetSeconds > 0 ? `end-${offsetSeconds}s` : "end";
  }
  return `${Number(Math.max(0, maxSeconds - offsetSeconds).toFixed(1))}s`;
}
...
function buildShotList({
  ...
  shots.push({
    timestamp_estimate: endBeatTimestamp(editorialContract, 4),
    script_beat: scriptBeatText(story, "loop"),
    visual_type: "takeaway_card",
    visual_target: entities[0],
    source_asset: null,
    card_type: "takeaway",
    reason: "Summarise why the story matters before the brand close.",
    risk: "low",
  });
  const governedCta = resolveGovernedCtaCopy(story);
  shots.push({
    timestamp_estimate: endBeatTimestamp(editorialContract, 1.5),
    script_beat:
      governedCta || "Pulse Gaming News — Fast gaming news. Checked. Explained.",
    visual_type: "brand_close",
    visual_target: "Pulse Gaming News",
    source_asset: null,
    card_type: "brand_close",
    reason: governedCta
      ? "Selected story-specific CTA over the branded close."
      : "Branded close without an audience CTA.",
    risk: "governed CTA policy must be preserved in final render",
  });
...
function targetDurationForFormat(formatVerdict, editorialContract = null) {
  if (["premium_short", "standard_short"].includes(formatVerdict)) {
    const minSeconds = Number(editorialContract?.min_seconds);
    const maxSeconds = Number(editorialContract?.max_seconds);
    if (
    ...
    return Number(((minSeconds + maxSeconds) / 2).toFixed(1));
  }
  if (formatVerdict === "evergreen_verdict_short") return 82;
...
function buildRenderContract({
  ...
  const targetDuration = targetDurationForFormat(format, editorialContract);
  const visualCount =
  ...
  } else if (format === "evergreen_verdict_short") {
    renderLane = "studio_v2_candidate";
    qualityClass = "premium";
...
function storyMotionReferences(story, opts = {}) {
  ...
  const localRefs = asArray(story?.video_clips)
    ...
    .map((clip) => ({
      ...
      source_url: clip.url || clip.source_url || null,
      local_path: clip.path || clip.local_path || null,
...
function buildPublishReadiness({
  ...
  red.push(
    ...(formatLanePolicy?.blockers || []),
    ...(flashLaneContract?.blockers || []),
  );
...
function buildProductionPacket(story, opts = {}) {
  ...
  const usesPulseShortContract = ["premium_short", "standard_short", "short_only"].includes(
    format_route.verdict,
  );
  const editorialResolution = usesPulseShortContract
    ? resolveRequiredPulseEditorialContract({ story })
    : { contract: null, blocker: null };
  const shot_list = buildShotList({
    story,
    dossier: story_dossier,
    mediaInventory: media_inventory,
    formatRoute: format_route,
    editorialContract: editorialResolution.contract,
  });
  const render_contract = buildRenderContract({
    ...
    shotList: shot_list,
    editorialContract: editorialResolution.contract,
    editorialBlocker: editorialResolution.blocker,
  });
  ...
  const publish_readiness = buildPublishReadiness({
    ...
    formatLanePolicy: format_lane_policy,
    flashLaneContract: flash_lane_contract,
  });
...
function renderCreatorStudioMarkdown(report) {
  ...
  lines.push("");
  lines.push("| story | colour | verdict | format | lane | lane readiness | flash action | media | exact | selected band | runtime | motion | frames | refs | topic | render | notes |");
  lines.push("| --- | --- | --- | --- | --- | --- | --- | --- | ---: | --- | --- | --- | --- | ---: | --- | --- | --- |");
  for (const packet of report.packets) {
    lines.push(
      [
        ...
        packet.media_inventory.studio_v2_selected_band_eligibility,
...
function renderPacketMarkdown(packet) {
  ...
  lines.push(`- Studio V2 selected-band eligible: ${packet.media_inventory.studio_v2_selected_band_eligibility}`);
