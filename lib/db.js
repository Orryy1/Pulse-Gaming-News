...
const ENGAGEMENT_REPLY_LOG_PATH = path.join(__dirname, '..', 'engagement_reply_log.json');
...
const JSON_FIELDS = new Set([
  'title_variants', 'game_images', 'downloaded_images',
]);
...
function rowToStory(row) {
  ...
  for (const [key, value] of Object.entries(row)) {
    ...
    if (JSON_FIELDS.has(key) && typeof value === 'string' && value.startsWith('[')) {
      try { story[key] = JSON.parse(value); } catch { story[key] = value; }
...
async function saveStories(stories) {
  ...
  const save = db.transaction((items) => {
    ...
    const existingIds = new Set(
      db.prepare('SELECT id FROM stories').all().map(r => r.id)
    );
    const newIds = new Set(items.map(s => s.id));

    // Delete stories that are no longer in the array
...
async function getApprovedReady() {
  if (!useSqlite()) {
    const stories = await jsonGetStories();
    return stories.filter(s => s.approved && s.exported_path && !s.youtube_post_id);
  }
...
async function getPublished() {
  if (!useSqlite()) {
    const stories = await jsonGetStories();
    return stories.filter(s => s.youtube_post_id || s.tiktok_post_id || s.instagram_media_id);
  }
...
async function getAnalyticsHistory() {
  ...
  const db = getDb();
  const entries = db.prepare('SELECT * FROM analytics_snapshots ORDER BY updated_at DESC').all();
...
async function saveAnalyticsHistory(history) {
  ...
  const save = db.transaction(() => {
    ...
    for (const entry of (history.entries || [])) {
      ...
      db.prepare(sql).run(
        entry.story_id || entry.id, entry.title, entry.flair, entry.content_pillar,
        entry.youtube_post_id, entry.tiktok_post_id, entry.instagram_media_id,
        entry.youtube_views || 0, entry.youtube_likes || 0, entry.youtube_comments || 0,
        entry.tiktok_views || 0, entry.tiktok_likes || 0, entry.tiktok_comments || 0, entry.tiktok_shares || 0,
        entry.instagram_views || 0, entry.instagram_likes || 0, entry.instagram_comments || 0,
        entry.total_views || 0, entry.virality_score || 0,
    ...
    for (const [key, data] of Object.entries(history.topicStats || {})) {
      db.prepare(`INSERT INTO analytics_topic_stats (key, count, total_virality, avg_virality)
        VALUES (?, ?, ?, ?)
...
async function appendBreakingLog(entry) {
  ...
  const db = getDb();
  db.prepare(`INSERT INTO breaking_log (story_id, title, breaking_score, flair, source_type, logged_at)
    VALUES (?, ?, ?, ?, ?, ?)`).run(
    entry.story_id || entry.id, entry.title, entry.breaking_score, entry.flair,
    entry.source_type, entry.logged_at || new Date().toISOString()
  );
}
...
async function getEngagementStats() {
  if (!useSqlite()) {
    if (await fs.pathExists(ENGAGEMENT_REPLY_LOG_PATH.replace('reply_log', 'stats'))) {
      return fs.readJson(ENGAGEMENT_REPLY_LOG_PATH.replace('reply_log', 'stats'));
    }
    return {};
  }

  const db = getDb();
  const rows = db.prepare('SELECT * FROM engagement_log ORDER BY date DESC').all();
  const stats = {};
  for (const row of rows) {
    stats[row.date] = { hearted: row.hearted, replies: row.replies, pins: row.pins };
  }
  return stats;
}
...
async function recordEngagementStats(date, hearted, replies, pins) {
  ...
  const db = getDb();
  db.prepare(`INSERT INTO engagement_log (date, hearted, replies, pins)
    VALUES (?, ?, ?, ?)
    ON CONFLICT(date) DO UPDATE SET
      hearted = hearted + excluded.hearted,
      replies = replies + excluded.replies,
      pins = pins + excluded.pins`).run(date, hearted, replies, pins);
}
