...
const STORIES_COLUMNS = new Set([
  ...
  // Migration 002 added stories.channel_id but storyToRow never learned
  // about it, so every story before 014 had its channel_id dropped into
  // the _extra JSON blob instead of the real column. Adding it here
  // makes the column round-trip through saveStories/getStories and
  // lets the hunter stamp new stories with a proper channel tag —
  // precursor to the multi-channel work documented in
  // docs/channel-isolation-audit.md.
  "channel_id",
  "_extra",
]);
...
const NULLABLE_EXTRA_FIELDS = new Set([
  "youtube_error",
  "tiktok_error",
  "instagram_error",
  "facebook_error",
...
function storyToRow(story) {
  ...
  for (const [key, value] of Object.entries(story)) {
    if (STORIES_COLUMNS.has(key)) {
      ...
      if (
        value !== undefined &&
        (value !== null || NULLABLE_EXTRA_FIELDS.has(key))
      ) {
        extra[key] = value;
      }
    }
  ...
  // Default channel_id to the active channel (env-resolved) when
  // the caller didn't stamp one. Pulse-Gaming is the only live
  // channel today — this default guarantees every new row carries
  // a non-null tag so the per-channel isolation work later (see
  // docs/channel-isolation-audit.md) doesn't need a second backfill.
  // Migration 014 backfills legacy NULL rows to 'pulse-gaming'.
  if (row.channel_id === undefined || row.channel_id === null) {
    row.channel_id = process.env.CHANNEL || "pulse-gaming";
  }
