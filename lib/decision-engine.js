...
const {
  ...
  qualifiesForFreshGamePolicyRedditAutoLane,
} = require("./scoring");
const {
...
const SCRIPT_QUALITY_AUTO_MIN = 7;
// Media-inventory classes that block auto-approval. Codex's
// 2026-04-29 media-inventory report flagged that 9/10 recent
// stories scored `blog_only` from thin visual inventory — but
// hard-blocking 90% of the queue overnight would silence Pulse,
// so we start narrow:
//   reject_visuals — multiple unsafe-face candidates or zero
//                    usable visuals after filtering. Always demote.
// blog_only stories continue to render but already get surfaced
// as downgrade candidates by publisher.js's warn-only hook
// (logFormatRecommendationsForApprovedStories) so the operator
// sees them. Promote `blog_only` to this list once the operator
// confirms that demotion gives the expected daily output.
const INVENTORY_AUTO_BLOCK_CLASSES = new Set(["reject_visuals"]);
/**
...
function scoreOne(
  ...
  applyScriptQualityAutoGate(story, score);
  applyScriptReviewAutoGate(story, score);
  applyLiveScriptCoherenceAutoGate(story, score);
  applyMediaInventoryAutoGate(story, score);
  applyCommunityDiscussionAutoGate(story, score);
  ...
  const laneCheck = hasAutoApprovalBlock(score)
    ? { qualifies: false, reason: null }
    : qualifiesForTrustedRumourAutoLane(story, score);
  if (laneCheck.qualifies) {
    ...
    // Trusted-publisher auto lane (2026-04-23). Separate from the
    // rumour lane — this lane promotes review-tier stories from
    // first-party gaming press (Eurogamer / Polygon / RPS / IGN /
    // etc.) scored 70+ with clean safety and a gaming-keyword
    // gate. See `qualifiesForTrustedPublisherAutoLane` in
    // lib/scoring.js for the complete criteria. Checked AFTER the
    // rumour lane so a story that qualifies for both wins on the
    // more specific label (rumour lane = more rigorous).
    const publisherLane = hasAutoApprovalBlock(score)
      ? { qualifies: false, reason: null }
      : qualifiesForTrustedPublisherAutoLane(story, score);
    if (publisherLane.qualifies) {
      score.decision = "auto";
      score.auto_lane_reason = publisherLane.reason;
      score.inputs = {
        ...(score.inputs || {}),
        auto_lane_reason: publisherLane.reason,
      };
    } else {
      // Fresh-confirmed Reddit auto lane (2026-05-15). This is deliberately
      // narrower than generic Reddit auto-approval: only fresh review-tier
      // release/official/news posts from allow-listed gaming subreddits with
      // clean safety and concrete game/platform evidence. It exists to stop
      // high-signal same-day releases from stalling at 70-74 while discussion
      // prompts and speculative posts still stay in review.
      const freshConfirmedLane = hasAutoApprovalBlock(score)
        ? { qualifies: false, reason: null }
        : qualifiesForFreshConfirmedRedditAutoLane(story, score);
      if (freshConfirmedLane.qualifies) {
        score.decision = "auto";
        score.auto_lane_reason = freshConfirmedLane.reason;
        score.inputs = {
          ...(score.inputs || {}),
          auto_lane_reason: freshConfirmedLane.reason,
        };
      } else {
        // Fresh game-policy auto lane (2026-05-15). Game-preservation
        // law/server-shutdown stories can be important, timely and
        // source-backed without naming a single franchise. Keep it below the
        // global auto threshold only when it is fresh, high-engagement,
        // process-specific and source-backed.
        const gamePolicyLane = hasAutoApprovalBlock(score)
          ? { qualifies: false, reason: null }
          : qualifiesForFreshGamePolicyRedditAutoLane(story, score);
        if (gamePolicyLane.qualifies) {
          score.decision = "auto";
          score.auto_lane_reason = gamePolicyLane.reason;
          score.inputs = {
            ...(score.inputs || {}),
            auto_lane_reason: gamePolicyLane.reason,
          };
        }
      }
    }
...
function summariseDecision(score) {
  ...
  if (score.script_quality_auto_block) {
    return `${score.script_quality_auto_block} total=${score.total}`;
  }
  if (score.script_review_auto_block) {
    return `${score.script_review_auto_block} total=${score.total}`;
  }
  if (score.live_script_coherence_auto_block) {
    return `${score.live_script_coherence_auto_block} total=${score.total}`;
  }
  if (score.media_inventory_auto_block) {
    return `${score.media_inventory_auto_block} total=${score.total}`;
  }
  if (score.community_discussion_auto_block) {
    return `${score.community_discussion_auto_block} total=${score.total}`;
  }
...
function hydrateStoryExtra(story) {
  if (!story || !story._extra) return story;
  if (typeof story._extra !== "string") return story;
...
function hasAutoApprovalBlock(score) {
  return Boolean(
    score &&
      (score.script_quality_auto_block ||
        score.script_review_auto_block ||
        score.live_script_coherence_auto_block ||
        score.media_inventory_auto_block ||
        score.community_discussion_auto_block),
  );
}

function getScriptQualityScore(story) {
  if (
    !story ||
    story.quality_score === null ||
    story.quality_score === undefined
  ) {
    return null;
  }
  const n = Number(story.quality_score);
  return Number.isFinite(n) ? n : null;
}

function applyScriptQualityAutoGate(story, score) {
  const scriptQualityScore = getScriptQualityScore(story);
  if (scriptQualityScore === null || !score) return;
  ...
  if (scriptQualityScore >= SCRIPT_QUALITY_AUTO_MIN) return;

  score.script_quality_auto_block = `script_quality_score ${scriptQualityScore} below ${SCRIPT_QUALITY_AUTO_MIN}`;
  score.inputs.script_quality_auto_block = score.script_quality_auto_block;
...
function getScriptReviewAutoBlock(story) {
  ...
  if (
    story.script_validation_errors &&
    !Array.isArray(story.script_validation_errors)
  ) {
    return "script_review:script_validation_errors";
  }
...
function applyScriptReviewAutoGate(story, score) {
  if (!score) return;
  const block = getScriptReviewAutoBlock(story);
  if (!block) return;

  score.script_review_auto_block = block;
  score.inputs = {
    ...(score.inputs || {}),
    script_generation_status: story.script_generation_status || null,
    script_review_reason: story.script_review_reason || null,
    script_review_auto_block: block,
  };
...
function hasNarrationScript(story = {}) {
  return Boolean(
    String(story.full_script || "").trim() ||
      String(story.tts_script || "").trim() ||
      String(story.hook || "").trim() ||
      String(story.body || "").trim() ||
      String(story.loop || "").trim(),
  );
}

function applyLiveScriptCoherenceAutoGate(story, score) {
  if (!score || !hasNarrationScript(story)) return;
  const qa = runScriptCoherenceQa(story, {
    requireCtaField: false,
    requireFullScriptCta: false,
  });
  if (!qa || qa.result !== "fail" || !qa.failures?.length) return;

  const block = `script_review:${qa.failures[0]}`;
  score.live_script_coherence_auto_block = block;
  score.inputs = {
    ...(score.inputs || {}),
    live_script_coherence_failures: qa.failures,
    live_script_coherence_auto_block: block,
  };
...
function getMediaInventoryClass(story) {
  if (!story) return null;
  // The scorer requires `downloaded_images` and/or `video_clips` to
  // be present. Stories that haven't been through the image pipeline
  // yet (still in hunt → produce queue) won't have either, in which
  // case the scorer returns `blog_only` because totalUsable is 0.
  // That's OK — the gate only acts on `reject_visuals` so a
  // stories-still-in-flight row is unaffected.
  try {
    const {
      scoreStoryMediaInventory,
    } = require("./creative/media-inventory-scorer");
    const inv = scoreStoryMediaInventory(story);
    return inv?.classification || null;
  } catch (err) {
    // Pure observability — never let a scorer crash the auto-approve
    // pass.
    return null;
  }
}

function applyMediaInventoryAutoGate(story, score) {
  if (!score) return;
  const inventoryClass = getMediaInventoryClass(story);
  if (!inventoryClass) return;

  score.inputs = {
    ...(score.inputs || {}),
    media_inventory_class: inventoryClass,
  };

  if (!INVENTORY_AUTO_BLOCK_CLASSES.has(inventoryClass)) return;

  score.media_inventory_auto_block = `media_inventory_class=${inventoryClass} blocks auto-approval`;
  score.inputs.media_inventory_auto_block = score.media_inventory_auto_block;
...
function applyCommunityDiscussionAutoGate(story, score) {
  if (!score || !shouldRejectGeneralRedditForNews(story)) return;

  score.community_discussion_auto_block = "community_discussion_prompt";
  score.inputs = {
    ...(score.inputs || {}),
    community_discussion_auto_block: score.community_discussion_auto_block,
  };

  score.decision = "reject";
  score.hard_stops = Array.from(
    new Set([...(score.hard_stops || []), "community_discussion_prompt"]),
  );
}
...
module.exports = {
  ...
  applyDecision,
  applyScriptQualityAutoGate,
  applyScriptReviewAutoGate,
  applyMediaInventoryAutoGate,
  applyCommunityDiscussionAutoGate,
  hydrateStoryExtra,
  hasAutoApprovalBlock,
  getMediaInventoryClass,
  DEFAULT_BATCH_AGE_HOURS,
  DEFAULT_RECENCY_HOURS,
  SCRIPT_QUALITY_AUTO_MIN,
