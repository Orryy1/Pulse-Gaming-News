...
const COMMUNITY_DISCUSSION_TITLE_RE =
  /\b(?:what(?:'s| is| are)|which|who|how to|anyone else|do you|recommend|recommendations?|best|favourite|favorite|obscure|ever played|made me realize|feel like|in chat)\b/i;
...
function isCommunityDiscussionPrompt(story) {
  if (!story || String(story.source_type || "").toLowerCase() !== "reddit") {
    return false;
  }
  const subreddit = String(story.subreddit || "").toLowerCase().replace(/^r\//, "");
  if (!COMMUNITY_DISCUSSION_SUBREDDITS.has(subreddit)) return false;
  const title = String(story.title || "");
  if (!COMMUNITY_DISCUSSION_TITLE_RE.test(title)) return false;

  const text = `${title}\n${story.full_script || ""}\n${story.hook || ""}`;
  const hasNewsVerb =
    /\b(?:announces?|announced|confirms?|confirmed|reveals?|revealed|launches?|launched|release date|trailer|update|patch|delayed|cancelled|acquired|lawsuit|statement)\b/i.test(
      text,
    );
...
function applyCommunityDiscussionAutoGate(story, score) {
  if (!score || !isCommunityDiscussionPrompt(story)) return;
