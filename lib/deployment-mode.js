 ...
 *   2. LOCAL_PUBLIC_URL  — when DEPLOYMENT_MODE=local
 *   3. RAILWAY_PUBLIC_URL — current Railway-injected URL (still works)
 *   4. LOCAL_PUBLIC_URL  — non-local fallback, useful for smoke tests
...
function getPublicUrl(env = process.env) {
  ...
  if (getMode(env) === "local" && env.LOCAL_PUBLIC_URL) {
    return env.LOCAL_PUBLIC_URL.replace(/\/+$/, "");
  }
 ...
 */
function describeMediaRoot(env = process.env) {
  const explicit = String(env.MEDIA_ROOT || "").trim();
  if (explicit) return "(configured)";
  if (isRailway(env)) return "(railway default)";
  return "(repo-root fallback)";
}

function describeSqliteDbPath(env = process.env) {
  const explicit = String(env.SQLITE_DB_PATH || "").trim();
  if (explicit) return "(configured)";
  if (isRailway(env)) return "(railway default)";
  return "(repo-root data/pulse.db)";
}
...
function summary(env = process.env) {
  ...
  return {
    ...
    media_root: describeMediaRoot(env),
    sqlite_db_path: describeSqliteDbPath(env),
  };
}
