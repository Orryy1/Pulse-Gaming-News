...
function buildGta6PreorderLaunchAngle(facts, sourceName) {
  ...
  return {
    ...
    viewerValue:
      "For players, the useful move is to separate the confirmed preorder timing from the missing price, bonus and platform details.",
    payoff:
...
function buildGta5CurrentGenUpgradeAngle(facts, sourceName) {
  return {
    ...
    caveat:
      "The catch is that a free upgrade does not make GTA 5 new, and it does not answer every GTA 6 question.",
    viewerValue:
      "The useful split is simple: if you still own the old version, current-gen access is now easier, but the bigger story is the runway to the sequel.",
    payoff:
...
function buildGta6LaunchEndgameAngle(facts, sourceName) {
  return {
    ...
    hook: "GTA 6 just turned delay fear into a rollout watch.",
    sourceLine:
      `${sourceName} argues Rockstar is entering the endgame for launching GTA 6, with the useful claim that another delay now looks less likely.`,
    tension:
    ...
    viewerValue:
      "For players, the practical read is simple: delay anxiety may be easing, but preorders, editions and platform details still need official confirmation.",
    payoff:
...
function buildOcarinaDirectDemandAngle(facts, sourceName) {
  return {
    ...
    caveat:
      "The catch is that trailer views prove attention, not final quality, performance or design restraint.",
    viewerValue:
...
function buildHiddenOcarinaDescriptionAngle(facts, sourceName) {
  return {
    ...
    hook: "Ocarina of Time just gave Switch 2 fans a clue Nintendo pulled back.",
    sourceLine:
    ...
    stakes:
      "The debate is now sharper than whether a remake exists. It is whether faithful means preserved structure, cleaner visuals or something closer to a remaster.",
    caveat:
      "It does not confirm a remake, and removed copy should be treated as cautious evidence rather than a finished announcement.",
    viewerValue:
      "For players, the useful takeaway is narrow: the wording suggests intent, but Nintendo still has to show what Switch 2 actually changes.",
    payoff:
...
function buildGenericAngle(facts, sourceName) {
  ...
  return {
    ...
    tension:
      "The useful part is whether the detail changes what players can decide today, not whether the headline sounds big.",
    stakes:
    ...
    viewerValue:
      "For players, the practical read is whether this changes a buy, wait, reinstall or ignore decision.",
...
function padScriptSentences(angle, runtimeProfile) {
  ...
  if (angle.lane === "status_money" || angle.lane === "money") {
    ...
    pads = [
      ...
      "The smart question is not whether GTA 6 will be huge; it is which version actually makes sense to reserve first.",
      ...
      "That makes the preorder page the first real test of hype versus value.",
    ];
    ...
    pads = [
      ...
      "The sharper debate is whether this feels like a useful bridge or a late attempt to keep the old game crowded.",
    ...
    pads = [
      ...
      "The debate is whether Rockstar is building momentum deliberately or whether fans are filling gaps between official beats.",
    ...
    pads = [
      ...
      "The debate is whether fans really want a faithful remake or only think they do until the first modernised system changes.",
    ...
    pads = [
      "The removal also changes the debate, because fans now have wording to argue over instead of only vibes.",
