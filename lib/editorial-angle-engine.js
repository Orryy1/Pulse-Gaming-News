...
function detectGameTitle(story = {}, sourceMaterial = "") {
  ...
  if (/\b(?:Call of Duty\s+)?Modern Warfare 4\b/i.test(text)) return "Modern Warfare 4";
  if (/\bThe Duskbloods\b/i.test(text)) return "The Duskbloods";
  if (/\bStarfield\b/i.test(text)) return "Starfield";
  if (/\bEA\b(?=.{0,90}\bin[- ]game advertising\b)/i.test(text)) {
    return "EA in-game advertising";
  }
...
function buildBlackFlagSalesMomentumAngle(facts, sourceName) {
  return {
    ...
    hook:
      "Ubisoft says its new Black Flag sold 3 million copies in its launch week. But the number after day one tells the better story.",
    sourceLine:
      `According to ${sourceName}, 2 million copies sold on day one, then at least another million across the next six days.`,
    tension:
...
function buildModernWarfareKillBlockAngle(facts, sourceName) {
  return {
    lane: "multiplayer_first_look",
    hook: "Modern Warfare 4 just showed Kill Block before this week's multiplayer reveal.",
    sourceLine:
      `${sourceName} reports this is the first Kill Block footage, with the multiplayer gameplay debut still due this week.`,
    tension:
      "That makes this an opening glimpse of one part of the game, not a verdict on the full multiplayer package.",
    stakes:
      "Players now have a concrete reveal to watch for: whether the longer footage makes Kill Block look distinct, readable and worth learning.",
    caveat:
      "This first look does not establish how the full mode plays or whether the wider reveal changes the picture.",
    viewerValue:
      "The useful comparison is simple: what Kill Block suggests now versus what the uncut multiplayer footage actually shows.",
    payoff:
      "Kill Block starts the conversation, but this week's gameplay reveal has to earn the excitement.",
    thumbnailText: "KILL BLOCK FIRST LOOK",
    title: "Modern Warfare 4 Kill Block Sets Up The Multiplayer Reveal",
  };
}

function buildDuskbloodsPlaytestAngle(facts, sourceName) {
  return {
    lane: "limited_playtest_access",
    hook: "The Duskbloods gets a Switch 2 playtest next month, but entry may be the real fight.",
    sourceLine:
      `${sourceName} reports the promised test is happening next month and access looks difficult to secure.`,
    tension:
      "That turns the first hands-on opportunity into a limited preview while most interested players may still be watching from outside.",
    stakes:
      "For players, the value is not just seeing new footage; it is learning who can enter, when sessions run and what the test will expose.",
    caveat:
      "The report does not establish that everyone can join or how broad the closed test will be.",
    viewerValue:
      "The details worth watching are entry rules, regions, dates and whether participants can share meaningful gameplay.",
    payoff:
      "The playtest creates real interest only if it turns The Duskbloods from a promise into something players can judge.",
    thumbnailText: "WHO GETS TO PLAY?",
    title: "The Duskbloods Switch 2 Playtest May Be Hard To Enter",
  };
}

function buildStarfieldContinuedSupportAngle(facts, sourceName) {
  return {
    lane: "continued_support_roadmap",
    hook: "Bethesda says Starfield still matters to its future, but a promise is not a roadmap.",
    sourceLine:
      `${sourceName} reports Bethesda has pledged continued support for the science-fiction RPG.`,
    tension:
      "That wording keeps Starfield alive without explaining what players will actually receive or when it will arrive.",
    stakes:
      "For anyone deciding whether to return, continued support only becomes meaningful when it turns into dated updates, fixes or substantial new content.",
    caveat:
      "The pledge does not by itself confirm a release schedule, expansion or specific feature.",
    viewerValue:
      "The next Bethesda update needs to replace reassurance with a clear deliverable and a date players can judge.",
    payoff:
      "The missing roadmap decides whether Starfield's future feels real or merely reassuring.",
    thumbnailText: "WHERE IS THE ROADMAP?",
    title: "Starfield Still Matters To Bethesda But The Roadmap Is Missing",
  };
}

function buildEaInGameAdvertisingAngle(facts, sourceName) {
  return {
    lane: "in_game_advertising_trust",
    hook: "EA sees a huge opportunity in in-game ads, and players should hear the catch.",
    sourceLine:
      `${sourceName} reports EA says advertising has to make sense for the game and be done properly.`,
    tension:
      "That sounds cautious, but it leaves the biggest questions unanswered: which games, which formats and whether paid players can avoid it.",
    stakes:
      "For players, an advert that interrupts play or invades a premium game can turn a business experiment into a trust problem.",
    caveat:
      "EA's comments do not confirm a specific ad system, launch date or game that will use one.",
    viewerValue:
      "The first implementation needs clear limits around placement, frequency and whether money already spent changes the experience.",
    payoff:
      "Those limits decide whether EA's opportunity feels additive or like players are paying to watch adverts.",
    thumbnailText: "ADS INSIDE EA GAMES?",
    title: "EA Sees A Huge In Game Ad Opportunity But Players Need Limits",
  };
}

function buildFalloutModProtestAngle(facts, sourceName) {
  return {
    lane: "community_labour_protest",
    hook: "Fallout 4 modders just brought Bethesda's layoff protest inside the game.",
    sourceLine:
      `${sourceName} reports modders filled Fallout 4 with protest signs and OneBGS-branded Vault suits as unionised Microsoft workers rallied.`,
    tension:
      "That turns a workplace dispute into something players can see while exploring one of Bethesda's own worlds.",
    stakes:
      "For fans, the mods connect job cuts to the people expected to build, test and support the games they still care about.",
    caveat:
      "Player-made protest assets do not reverse the layoffs or prove which Bethesda projects will change.",
    viewerValue:
      "What they do show is how quickly a corporate decision can become part of the culture around the games themselves.",
    payoff:
      "The protest has moved from the studio gates into Fallout 4, turning Bethesda's own game into the argument.",
    thumbnailText: "PROTESTS HIT FALLOUT 4",
    title: "Fallout 4 Modders Put Bethesda Layoff Protests In Game",
  };
}
...
function buildEditorialAngle(story = {}, options = {}) {
  ...
  let angle;
  if (
    /\b(?:Call of Duty\s+)?Modern Warfare 4\b/i.test(titleText) &&
    /\bKill Block\b/i.test(titleText) &&
    /\bmultiplayer gameplay\b/i.test(titleText)
  ) {
    angle = buildModernWarfareKillBlockAngle(facts, sourceName);
  } else if (
    /\bThe Duskbloods\b/i.test(titleText) &&
    /\bplaytest\b/i.test(titleText) &&
    /\bSwitch 2\b/i.test(titleText)
  ) {
    angle = buildDuskbloodsPlaytestAngle(facts, sourceName);
  } else if (
    /\bStarfield\b/i.test(titleText) &&
    /\bimportant part of (?:our|Bethesda['\u2019]?s) future\b/i.test(titleText) &&
    /\bcontinued support\b/i.test(titleText)
  ) {
    angle = buildStarfieldContinuedSupportAngle(facts, sourceName);
  } else if (
    /\bEA\b/i.test(titleText) &&
    /\bin[- ]game advertising\b/i.test(titleText) &&
    /\bhuge opportunity\b/i.test(titleText)
  ) {
    angle = buildEaInGameAdvertisingAngle(facts, sourceName);
  } else if (
    /\bFallout 4\b/i.test(titleText) &&
    /\bprotest signs?\b/i.test(titleText) &&
    /\bBethesda layoffs?\b/i.test(titleText)
  ) {
    angle = buildFalloutModProtestAngle(facts, sourceName);
  } else if (/Stop Killing Games|server shutdowns|keep games playable|AB\s*1921/i.test(titleText)) {
