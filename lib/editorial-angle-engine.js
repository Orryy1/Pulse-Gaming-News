...
function buildHaloAccountRequirementAngle(facts, sourceName) {
  return {
    ...
    stakes:
      "For fans, the issue is friction. A remake can open the door to a bigger audience, but extra account steps can make that welcome feel awkward fast.",
    caveat:
...
function buildGenericAngle(facts, sourceName) {
  ...
  return {
    lane: "impact",
    hook: `${game} just got an update that changes the player decision.`,
    sourceLine: `${sourceName} reports a new ${game} detail around access, timing, performance or expectations.`,
    tension:
      "The point is what this changes before players spend money, storage or time.",
    stakes:
      "The stakes are direct: who is affected, what improves and what still is not confirmed.",
    caveat:
      "One report cannot settle the whole game, so the missing details still matter.",
    payoff:
      "If a later update adds footage, price or a firm date, that becomes a separate story.",
    viewerValue:
      "For players, the split is whether this changes a buy, wait, reinstall or ignore decision today.",
...
function padScriptSentences(angle, runtimeProfile) {
  ...
  if (angle.lane === "status_money" || angle.lane === "money") {
    ...
    pads = [
      "The first test is simple: does this change what players install, buy, wishlist or ignore this week?",
      "If the answer is no, the headline belongs in the backlog instead of the hype cycle.",
      "If the answer is yes, the next proof has to be visible: footage, price, date or platform detail.",
      "That is the difference between a news item people scroll past and one they can actually argue about.",
      "A strong follow-up would make the player decision obvious without needing the headline explained twice.",
    ];
  }
  const sentences = [
    angle.hook,
