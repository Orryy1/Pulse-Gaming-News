...
function normaliseTrainingRiskText(value) {
  return String(value || "")
    .toLowerCase()
    .normalize("NFKD")
    .replace(/[\u0300-\u036f]/g, "")
...
function hasRiskyGtaViTrainingText(value) {
  const text = normaliseTrainingRiskText(value);
  if (!text) return false;
  const subject = String.raw`(?:g\s*t\s*a|gta|grand\s+theft\s+auto)`;
  const stutter = String.raw`(?:s\s+i\s+(?:six|6)|(?:s|si|sy|sigh|see|sea|c|size|sice|sic|sick|sig|sixty)\s+(?:six|6)|(?:see|sea|c|s|si|suh|sir|size|sice|sic|sick|sig)\s+a\s+(?:six|6)|(?:suh|sir|size|sice|sic|sick|sig)\s+(?:six|6)|see\s+see\s+(?:six|6)|siix)`;
  return (
    /\bgtavi\b/.test(text) ||
    new RegExp(String.raw`\b${subject}\s+(?:v\s*i|vi|6|six|${stutter})\b`).test(text)
  );
}
...
async function inspectAudioManifest(
  ...
  const spokenText = cleanText(timestampPayload.meta?.text || timestampPayload.meta?.transcript || transcript);
  const blockers = [];
  const add = (blocker) => {
  ...
  if (hasRiskyGtaViTrainingText(transcript) || hasRiskyGtaViTrainingText(spokenText)) {
    add("elevenlabs_tts_corpus:risky_gta_vi_training_text");
  }
