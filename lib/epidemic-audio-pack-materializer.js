...
function plannedChannels(report = {}) {
  const channels = asArray(report.download_plan?.channels);
  if (channels.length) return channels;
  return unique(asArray(report.audio_pack_candidates).map((pack) => pack.channel_id)).map((channelId) => ({
...
function collectBlockers(report = {}, { workspaceRoot = process.cwd() } = {}) {
  const blockers = [];
  const add = (value) => {
  ...
  if (!plannedChannels(report).length) add("epidemic:no_channels_planned");
...
function buildEpidemicSoundImplementationPlan({
  ...
  const blockers = collectBlockers(report, { workspaceRoot: resolvedWorkspace });
  ...
  const musicAssets = musicByRole(report);
  const channels = plannedChannels(report);
...
async function executeEpidemicSoundImplementation({
  ...
  const plan = buildEpidemicSoundImplementationPlan({ workspaceRoot, report, generatedAt });
