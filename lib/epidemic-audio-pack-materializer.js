...
function groupByRole(assets = []) {
  ...
  for (const [role, roleAssets] of map.entries()) {
    map.set(role, roleAssets.sort((a, b) => normalisePath(a.filename || a.path || a.asset_id).localeCompare(normalisePath(b.filename || b.path || b.asset_id))));
  }
  return map;
}

function musicAssetsByRole(report = {}) {
  return groupByRole(asArray(report.music_inventory).filter((asset) => REQUIRED_MUSIC_ROLES.includes(asset.role)));
}
...
function packAssetFromMusicAsset(asset, role, variantIndex = 0, variantCount = 1) {
  return {
    role,
...
function buildMusicVariants(musicAssetsByRoleMap) {
  return REQUIRED_MUSIC_ROLES.reduce((out, role) => {
    const assets = asArray(musicAssetsByRoleMap.get(role));
    out[role] = assets.map((asset, index) => packAssetFromMusicAsset(asset, role, index, assets.length));
    return out;
  }, {});
}

function buildChannelPack({ channel, musicAssetsByRoleMap, rootPath }) {
  const variants = buildMusicVariants(musicAssetsByRoleMap);
  return {
    ...
    rotation_policy: {
      ...
      seed_fields: ["story.id", "story.url", "channel_id", "role"],
      fallback: "first_variant_per_role",
    },
    assets: REQUIRED_MUSIC_ROLES.map((role) => {
      const asset = asArray(variants[role])[0];
      return {
        ...asset,
...
function variantAssetFromSfxRecord(record = {}, role, variantIndex = 0, variantCount = 1) {
  return {
    role,
    family: record.family || role,
    provider_id: record.provider_id,
    asset_id: record.asset_id,
    source_url: record.source_url || null,
    path: record.path || null,
    license: record.licence_basis || record.rights_basis || null,
    approval_status: record.approval_status || null,
    safelist_evidence: record.safelist_evidence || null,
    variant_index: variantIndex,
    variant_count: variantCount,
  };
}

function buildSfxVariantsByRole(rightsRecords = []) {
  const grouped = groupByRole(asArray(rightsRecords).filter((record) => REQUIRED_SFX_ROLES.includes(record.role)));
  return REQUIRED_SFX_ROLES.reduce((out, role) => {
    const assets = asArray(grouped.get(role));
    out[role] = assets.map((record, index) => variantAssetFromSfxRecord(record, role, index, assets.length));
    return out;
  }, {});
}
...
function buildSfxRuntimeManifest(report = {}, blockers = []) {
  ...
  const variantAssetsByRole = buildSfxVariantsByRole(rightsRecords);
  return {
    schema_version: 1,
    ...
    variant_assets_by_role: variantAssetsByRole,
    rotation_policy: {
      strategy: "story_id_hash",
      seed_fields: ["story.id", "story.url", "role"],
      fallback: "selected_asset",
    },
...
function collectBlockers(report = {}, { workspaceRoot = process.cwd(), channelIds = [] } = {}) {
  ...
  const musicAssetGroups = musicAssetsByRole(report);
  const rights = rightsByAssetId(report);
  for (const role of REQUIRED_MUSIC_ROLES) {
    const assets = asArray(musicAssetGroups.get(role));
    if (!assets.length) {
...
function buildEpidemicSoundImplementationPlan({
  ...
  const channelPacks = ready
    ? channels.map((channel) => buildChannelPack({ channel, musicAssetsByRoleMap: musicAssetGroups, rootPath }))
    : [];
  ...
  const musicVariantCount = REQUIRED_MUSIC_ROLES.reduce((sum, role) => sum + asArray(musicAssetGroups.get(role)).length, 0);
  const sfxVariantCount = Object.values(sfxRuntimeManifest.variant_assets_by_role || {}).reduce((sum, assets) => sum + asArray(assets).length, 0);
  ...
  return {
    ...
    summary: {
      ...
      music_roles_covered: REQUIRED_MUSIC_ROLES.filter((role) => asArray(musicAssetGroups.get(role)).length).length,
      music_variant_count: musicVariantCount,
      sfx_roles_covered: asArray(report.sfx_source_plan?.covered_roles).length,
      sfx_variant_count: sfxVariantCount,
      total_audio_variant_count: musicVariantCount + sfxVariantCount,
...
function renderImplementationMarkdown(plan = {}) {
  ...
  lines.push(`- Music variants retained: ${Number(plan.summary?.music_variant_count || 0)}`);
  lines.push(`- SFX roles covered: ${Number(plan.summary?.sfx_roles_covered || 0)}`);
  lines.push(`- SFX variants retained: ${Number(plan.summary?.sfx_variant_count || 0)}`);
