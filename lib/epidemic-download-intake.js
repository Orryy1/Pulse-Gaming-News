...
function classifyDownloadedEpidemicFile(filePath, roleHint = "") {
  ...
  if (hint && MUSIC_ROLE_FOLDERS[hint]) {
    return { role: hint, target_folder: MUSIC_ROLE_FOLDERS[hint], asset_type: "music" };
  }
  if (hint && ["impact", "transition", "ui_tick", "riser", "sub_hit", "glitch"].includes(hint)) {
    return { role: hint, target_folder: "sfx", asset_type: "sfx" };
  }

  const text = normalisePath(filePath).toLowerCase();
  if (/\b(?:stem|stems|drums|bass|instrumental layer|melody)\b/.test(text)) {
    return { role: "stem", target_folder: "stems", asset_type: "music" };
  }
  if (/\b(?:rumou?r|leak)\b/.test(text) && /\b(?:sting|stinger|hit|watch)\b/.test(text)) {
    return { role: "sting_rumour", target_folder: "stings/sting_rumour", asset_type: "music" };
  }
  if (/\b(?:breaking|urgent|alert)\b/.test(text) && /\b(?:sting|stinger|hit|impact)\b/.test(text)) {
    return { role: "sting_breaking", target_folder: "stings/sting_breaking", asset_type: "music" };
  }
  if (/\b(?:verified|source|lock|confirm|confirmed|official)\b/.test(text) && /\b(?:sting|stinger|hit)\b/.test(text)) {
    return { role: "sting_verified", target_folder: "stings/sting_verified", asset_type: "music" };
  }
  if (/\b(?:breaking|urgent|alert|fast|high[-_ ]?energy)\b/.test(text) && /\b(?:bed|loop|music|underscore)\b/.test(text)) {
    return { role: "bed_breaking", target_folder: "music/bed_breaking", asset_type: "music" };
  }
  if (/\b(?:main|primary|news|background)\b/.test(text) && /\b(?:bed|loop|music|underscore)\b/.test(text)) {
    return { role: "bed_primary", target_folder: "music/bed_primary", asset_type: "music" };
  }
  if (/\b(?:glitch|static|digital|bitcrush|stutter)\b/.test(text)) {
    return { role: "glitch", target_folder: "sfx", asset_type: "sfx" };
  }
  if (/\b(?:riser|rise|swell|build|tension)\b/.test(text)) {
    return { role: "riser", target_folder: "sfx", asset_type: "sfx" };
  }
  if (/\b(?:whoosh|swoosh|swipe|transition|sweep)\b/.test(text)) {
    return { role: "transition", target_folder: "sfx", asset_type: "sfx" };
  }
  if (/\b(?:ui|tick|click|beep|button|select)\b/.test(text)) {
    return { role: "ui_tick", target_folder: "sfx", asset_type: "sfx" };
  }
  if (/\b(?:sub|boom|bass[-_ ]?drop|low[-_ ]?hit)\b/.test(text)) {
    return { role: "sub_hit", target_folder: "sfx", asset_type: "sfx" };
  }
  if (/\b(?:impact|hit|slam|thud|punch|cinematic)\b/.test(text)) {
    return { role: "impact", target_folder: "sfx", asset_type: "sfx" };
  }
  return { role: "", target_folder: "", asset_type: "" };
}
