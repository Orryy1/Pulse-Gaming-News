...
const GAME_DEFS = Object.freeze([
  ...
  entityDef("Pok\u00e9mon", ["pokemon", "pok\u00e9mon"]),
...
function buildSubjectGraph(story) {
  ...
  for (const value of [
    ...detectByDefs(text, GAME_DEFS),
    ...arrayEntities(story, "games"),
    ...arrayEntities(story, "canonical_game"),
  ]) {
    addUnique(games, value);
  }
  for (const value of inferHeadlineGameCandidates(story?.title || "")) {
    addUnique(games, value);
  }
  for (const value of [
...
function isPlatformHardwareOrAccessory(value) {
  const n = normalise(value);
  if (!n) return false;
  const platformMentioned =
    containsTerm(n, "xbox") ||
    containsTerm(n, "playstation") ||
    containsTerm(n, "ps5") ||
    containsTerm(n, "ps4") ||
    containsTerm(n, "nintendo") ||
    containsTerm(n, "switch") ||
    containsTerm(n, "steam");
  if (!platformMentioned) return false;
  return /\b(controller|headset|keyboard|mouse|wheel|accessory|accessories|capture card|storage|monitor)\b/.test(n);
}
...
function canonicalGroup(value) {
  ...
  if (containsTerm(n, "pokemon")) return "Pok\u00e9mon";
  if (isPlatformHardwareOrAccessory(value)) return String(value || "").trim();
