...
} = require("./exact-store-title-identity");
...
const GAME_DEFS = Object.freeze([
  ...
  entityDef("Silent Hill: Townfall", ["silent hill townfall"]),
]);
...
const FRANCHISE_DEFS = Object.freeze([
  ...
  entityDef("Silent Hill", ["silent hill"]),
]);
...
function classifySubjectMatch(story, asset, subjectGraph = buildSubjectGraph(story)) {
  ...
  if (
    storeVerification.store_match_verified === true &&
    titleContainsExactStoreName(
      subjectGraph.story_title,
      storeVerification.store_app_title ||
        storeVerification.store_app_slug,
    )
  ) {
    return subjectResult(
      EXACT_MATCH_QUALITIES.EXACT_GAME,
      `Verified store app title matches the complete headline entity "${storeVerification.store_app_title}".`,
      canonicalGroup(storeVerification.store_app_title),
      true,
      true,
      storeVerification,
    );
...
function verifyStoreAsset(asset, subjectGraph, sourceType = inferSourceType(asset)) {
  ...
  const specificGameSubjects = subjectGraph.games.filter((game) => {
    const normalisedGame = normalise(game);
    return subjectGraph.franchises.some((franchise) => {
      const normalisedFranchise = normalise(franchise);
      return (
        normalisedGame !== normalisedFranchise &&
        containsTerm(normalisedGame, normalisedFranchise)
      );
    });
  });
  const subjectValues = specificGameSubjects.length
    ? specificGameSubjects
    : [
        ...subjectGraph.games,
        ...subjectGraph.franchises,
        ...(subjectGraph.platform_story
          ? subjectGraph.platforms
          : []),
      ];
  const headlineTitleMatch = titleContainsExactStoreName(
    subjectGraph.story_title,
    appTitle || slug,
  )
    ? appTitle || slug
    : null;
  const subjectMatch =
    findMatch(haystack, subjectValues) ||
    headlineTitleMatch;
  if (subjectMatch) {
    return {
      store_asset: true,
...
function exactGroupCoversRequiredGroup(exactGroup, requiredGroup) {
  const exact = normalise(exactGroup);
  const required = normalise(requiredGroup);
  if (!exact || !required) return false;
  return (
    exact === required ||
    ` ${exact} `.includes(` ${required} `)
  );
}
...
function buildExactSubjectReadiness(story, candidates = null, options = {}) {
  ...
  const missingGroups = subjectGraph.required_subject_groups.filter(
    (group) =>
      !exactGroups.some((exact) =>
        exactGroupCoversRequiredGroup(exact, group),
      ),
  );
  ...
  const hasEnoughGroups =
    ...
    (subjectGraph.required_subject_groups.length === 0 ||
  ...
  if (recommended.editorial_blocker) {
    downgradeReasons.push(recommended.editorial_blocker);
  }
  ...
  return {
    ...
    studio_v2_selected_band_eligible: studioEligible,
...
function recommendRuntime({
  ...
  let recommendedFormat = "premium_short";
  if (exactSubjectAssets <= 3 && exactMotionAssets < 2) {
    recommendedFormat = "short_only";
  } else if (exactSubjectAssets <= 5 && exactMotionAssets < 2) {
    recommendedFormat = "standard_short";
  }

  const resolved = resolveRequiredPulseEditorialContract({ story: story || {} });
  if (resolved.blocker || !resolved.contract) {
    return {
      recommended_format: recommendedFormat,
      recommended_runtime_class: "pulse_editorial_contract_required",
      recommended_runtime_seconds: null,
      editorial_blocker:
        resolved.blocker || "pulse_editorial_contract_required",
    };
  }

  return {
    recommended_format: recommendedFormat,
    recommended_runtime_class: resolved.contract.duration_band_id,
    recommended_runtime_seconds: [
      Number(resolved.contract.min_seconds),
      Number(resolved.contract.max_seconds),
    ],
    editorial_blocker: null,
  };
}
...
function summariseExactSubjectPlans(plans) {
  ...
  return {
    ...
    studio_v2_selected_band_eligible: safePlans.filter(
      (plan) => plan.exact_subject_readiness?.studio_v2_selected_band_eligible,
    ).length,
