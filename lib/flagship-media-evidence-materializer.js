...
function spokenScriptTokens(value) {
  ...
  const spoken = applyGamingPronunciation(String(value || "")).replace(
    /([$£€])(\d[\d,]*)(?:\.(\d{1,2}))?/g,
...
function validateTimedCoverage(rows, durationSeconds, {
  ...
  if (coverageRatio < 0.8 && coverageBlocker) blockers.push(coverageBlocker);
  return { blockers, coverageRatio };
}

function validateWordTimestampCoverage(rows, durationSeconds, {
  beginBlocker,
  boundsBlocker,
  coverageBlocker,
  gapBlocker,
} = {}) {
  const allowedInternalGapSeconds = 1.5;
  const duration = Number(durationSeconds);
  const validRows = asArray(rows).filter((row) => (
    typeof row?.start === "number" && typeof row?.end === "number" &&
    Number.isFinite(row.start) && Number.isFinite(row.end) && row.end > row.start
  ));
  const occupancy = validateTimedCoverage(validRows, duration, {
    beginBlocker,
    boundsBlocker,
  });
  if (!(duration > 0) || !validRows.length) {
    return {
      blockers: occupancy.blockers,
      coverageRatio: 0,
      speechOccupancyRatio: occupancy.coverageRatio,
      maxInternalGapSeconds: 0,
      maxInternalGapAfterWord: null,
      allowedInternalGapSeconds,
    };
  }
  const orderedRows = [...validRows].sort(
    (left, right) => left.start - right.start || left.end - right.end,
  );
  let maxInternalGapSeconds = 0;
  let maxInternalGapAfterWord = null;
  for (let index = 1; index < orderedRows.length; index += 1) {
    const gapSeconds = Number(
      Math.max(0, orderedRows[index].start - orderedRows[index - 1].end).toFixed(6),
    );
    if (gapSeconds > maxInternalGapSeconds) {
      maxInternalGapSeconds = gapSeconds;
      maxInternalGapAfterWord = index;
    }
  }
  const firstStart = Math.min(...validRows.map((row) => row.start));
  const lastEnd = Math.max(...validRows.map((row) => row.end));
  const coverageRatio = Number(
    (Math.max(0, Math.min(duration, lastEnd) - Math.max(0, firstStart)) / duration)
      .toFixed(6),
  );
  const blockers = [...occupancy.blockers];
  if (coverageRatio < 0.8 && coverageBlocker) blockers.push(coverageBlocker);
  if (maxInternalGapSeconds > allowedInternalGapSeconds && gapBlocker) {
    blockers.push(gapBlocker);
  }
  return {
    blockers,
    coverageRatio,
...
async function materializeFlagshipMediaEvidenceInternal({
  ...
  const timestampCoverage = validateWordTimestampCoverage(
    ...
    finalMediaDurations.final_audio_seconds,
    {
      ...
      gapBlocker: "word_timestamps_internal_gap_excessive",
    },
  );
  ...
  const timestampVideoCoverage = validateWordTimestampCoverage(
    timestampInspection.words,
    finalMediaDurations.final_video_seconds,
    {
  ...
  finals.word_timestamps.speech_occupancy_ratio = timestampCoverage.speechOccupancyRatio;
  finals.word_timestamps.video_coverage_ratio = timestampVideoCoverage.coverageRatio;
  finals.word_timestamps.video_speech_occupancy_ratio =
    timestampVideoCoverage.speechOccupancyRatio;
  finals.word_timestamps.max_internal_gap_seconds =
    timestampCoverage.maxInternalGapSeconds;
  finals.word_timestamps.max_internal_gap_after_word =
    timestampCoverage.maxInternalGapAfterWord;
  finals.word_timestamps.allowed_internal_gap_seconds =
    timestampCoverage.allowedInternalGapSeconds;
  ...
  const timestampTokens = spokenScriptTokens(
    asArray(timestampInspection.words).map((row) => row.word).join(" "),
  );
