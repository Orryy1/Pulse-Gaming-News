...
const APPROVED_ISO_BMFF_BRANDS = new Set(["avc1", "iso2", "isom", "mp41", "mp42", "qt  "]);
...
async function inspectTrustedGenerationManifest({ packageDir, inventory, finals }) {
  const blockers = [];
...
async function materializeFlagshipMediaEvidenceInternal({
  ...
  if (!sameTokenSequence(timestampTokens, scriptTokens)) {
  ...
  for (const card of usedAssets.filter((asset) => asset.asset_class === "generated_card")) {
    ...
    const lineageVerified = backdrops.length > 0 && backdrops.every((asset) => asset.verified);
