...
} = require("./flagship-card-evidence");
const {
...
const ENABLED_PUBLISH_PLATFORMS = Object.freeze([
  "youtube_shorts",
  "instagram_reels",
  "facebook_reels",
]);
...
const PENDING_FINAL_MEDIA_NARRATION_TRANSITION =
...
function repairedProfessionalSourceDiversity(current, minimumRequired = 0) {
  ...
  const assessment = current.professionalSourceDiversity || {
    schema_version: 1,
    version: "missing_professional_source_diversity_assessment",
    status: "blocked",
    strict_pass: false,
    observed_genuine_base_source_count: 0,
    per_source_scene_shares: [],
    concentrated_sources: [],
    rejected_independence_claims: [],
    unresolved_clips: [],
    blockers: ["professional_source_diversity_assessment_missing"],
  };
  const strictPass = assessment.strict_pass === true && assessment.status === "pass";
  return {
    schema_version: assessment.schema_version,
    evaluator_version: assessment.version,
    verdict: strictPass ? "GREEN" : "RED",
    status: strictPass ? "GREEN" : "RED",
    strict_pass: strictPass,
    minimum_met:
      strictPass &&
      (!minimumRequired || current.genuineBaseSourceCount >= minimumRequired),
    minimum_required_genuine_base_source_count: minimumRequired,
    observed_genuine_base_source_count:
      assessment.observed_genuine_base_source_count,
    input_source_count: current.selected.length,
    accepted_source_count: assessment.observed_genuine_base_source_count,
    rejected_source_count:
      (assessment.unresolved_clips || []).length +
      (assessment.concentrated_sources || []).length,
    ...
    rejected_sources: clone([
      ...(assessment.unresolved_clips || []),
      ...(assessment.concentrated_sources || []),
    ]),
    duplicate_segment_windows: duplicateSegmentWindows,
    concentration_rule: clone(assessment.concentration_rule || null),
    per_source_scene_shares: clone(assessment.per_source_scene_shares || []),
    concentrated_sources: clone(assessment.concentrated_sources || []),
    identity_evidence: clone(assessment.identity_evidence || []),
    rejected_independence_claims: clone(
      assessment.rejected_independence_claims || [],
    ),
    blockers: clone(assessment.blockers || []),
  };
}
...
function enforceSourceRequirements(job, current) {
  ...
  const selectedDirectMotion = current.selected.filter(
    (clip) => !isGeneratedCardClip(clip.row),
  );
  const genuineBaseSources = selectedGenuineBaseSources(
    selectedDirectMotion,
  ...
  const professionalRows = selectedDirectMotion.map((clip) => clip.row);
  current.professionalSourceDiversity = assessProfessionalSourceDiversity({
    clips: professionalRows,
    scenes: professionalRows,
    requiredBaseSources: Math.max(
      PROFESSIONAL_MOTION_SOURCE_POLICY.min_genuine_base_sources,
      minGenuineBaseSources || 0,
    ),
  });
  if (current.professionalSourceDiversity.strict_pass !== true) {
    throw new Error(
      `selected_professional_source_diversity_blocked:${
        current.professionalSourceDiversity.blockers.join(",") || "unknown"
      }`,
    );
  }
  return {
...
function materializedRightsEvidence(record = {}) {
  const evidence =
    record.materialized_file_evidence ||
    record.materialised_file_evidence ||
    {};
  return {
    sha256: clean(evidence.sha256).toLowerCase(),
    size_bytes: evidence.size_bytes,
  };
}

function clipDeclaredHash(clip = {}) {
  return clean(
    clip.row?.sha256 ||
    clip.row?.file_sha256 ||
    clip.row?.asset_sha256 ||
    clip.row?.materialized_file_evidence?.sha256 ||
    clip.row?.materialised_file_evidence?.sha256,
  ).toLowerCase();
}

function clipDeclaredSize(clip = {}) {
  return (
    clip.row?.size_bytes ??
    clip.row?.file_size_bytes ??
    clip.row?.asset_size_bytes ??
    clip.row?.materialized_file_evidence?.size_bytes ??
    clip.row?.materialised_file_evidence?.size_bytes
  );
}

function transitionMotionRightsBinding({
  record,
  usedAsset,
  clip,
  transitionAllowed,
}) {
  const evidence = materializedRightsEvidence(record);
  const declaredHash = clipDeclaredHash(clip);
  const declaredSize = Number(clipDeclaredSize(clip) ?? clip.size_bytes);
  const evidenceSize = Number(evidence.size_bytes);
  if (
    !transitionAllowed ||
    !isSha256(evidence.sha256) ||
    !Number.isInteger(evidenceSize) ||
    evidenceSize <= 0 ||
    !isSha256(declaredHash) ||
    !Number.isInteger(declaredSize) ||
    declaredSize <= 0 ||
    evidence.sha256 !== declaredHash ||
    evidenceSize !== declaredSize ||
    evidenceSize !== Number(clip.size_bytes)
  ) {
    return null;
  }
  const recordSourceUrl = clean(record.source_url || record.sourceUrl);
  const usedSourceUrl = clean(usedAsset.source_url || usedAsset.sourceUrl);
  const recordSourceType = clean(record.source_type || record.sourceType);
  const usedSourceType = clean(usedAsset.source_type || usedAsset.sourceType);
  if (
    (recordSourceUrl && usedSourceUrl && recordSourceUrl !== usedSourceUrl) ||
    (recordSourceType && usedSourceType && recordSourceType !== usedSourceType)
  ) {
    return null;
  }
  return {
    ...
    basis: "record_materialized_file_evidence_bound_to_current_motion_manifest",
  };
}
...
function sameStringSet(actual = [], expected = []) {
  const left = unique(asArray(actual).map(clean).filter(Boolean)).sort();
  const right = unique(asArray(expected).map(clean).filter(Boolean)).sort();
  return left.length === right.length && left.every((value, index) => value === right[index]);
}

function candidateDerivedPreRenderRightsBlockers(storyId) {
  const narrationId = `${storyId}_audio_path`;
  return [
    `narration_commercial_rights_evidence_not_green:${narrationId}`,
    "used_asset_rights_coverage_incomplete",
    `flagship_rights_sidecar_source_record_missing:${narrationId}`,
    SCRIPT_REPAIR_RIGHTS_TRANSITION_BLOCKER,
  ];
}

function assertNarrationRightsLedgerState(ledger, narrationEvidence, storyId) {
  const verdict = clean(ledger?.verdict).toLowerCase();
  ...
  const pendingFinalMediaTransition =
    verdict === "red" &&
    failures.length === 0 &&
    sameStringSet(blockers, candidateDerivedPreRenderRightsBlockers(storyId)) &&
  ...
  if (!transitionOnly && !pendingFinalMediaTransition) {
  ...
  return pendingFinalMediaTransition
    ? PENDING_FINAL_MEDIA_NARRATION_TRANSITION
...
async function inspectCurrentRightsEvidence({
  ...
  let pendingGenerationReceipt = null;
  if (narrationTransition === PENDING_FINAL_MEDIA_NARRATION_TRANSITION) {
    const inspection = await inspectPendingElevenLabsGenerationRightsReceipt({
      artifactDir: sourceArtifactDir,
      storyId,
      requestText: timestampSpokenScript(current.timestampDocument),
      masteredAudioFingerprint: {
        sha256: current.narration.sha256,
        size_bytes: current.narration.size_bytes,
      },
      timestampDocument: current.timestampDocument,
      targetPlatforms: ENABLED_PUBLISH_PLATFORMS,
    });
    if (inspection.verdict !== "GREEN") {
      throw new Error(
        `pre_render_generation_receipt_invalid:${inspection.blockers.join(",")}`,
      );
    }
    pendingGenerationReceipt = {
      receiptPath: inspection.receiptPath,
      receiptEvidence: inspection.receiptEvidence,
      receipt: clone(inspection.receipt),
      controlFiles: inspection.supportingEvidence.map(([label, evidence]) => [
        `pre_render_${label}`,
        evidence,
      ]),
    };
  ...
  const coLocatedTransitionLedger =
    narrationTransition !== "authoritative_pass" &&
  ...
  const transitionMotionRebindingAllowed =
    coLocatedTransitionLedger &&
    narrationTransition === PENDING_FINAL_MEDIA_NARRATION_TRANSITION;
  for (const clip of current.selected) {
    ...
    const recordPathMatches =
      Boolean(recordPath) && normalPath(recordPath) === normalPath(clip.path);
    const usedAssetPathMatches =
      Boolean(usedAssetPath) && normalPath(usedAssetPath) === normalPath(clip.path);
    const transitionBinding = transitionMotionRightsBinding({
      record,
      usedAsset,
      clip,
      transitionAllowed:
        transitionMotionRebindingAllowed &&
        (!recordPathMatches || !usedAssetPathMatches),
    });
    if (!recordPathMatches && !transitionBinding) {
      throw new Error(`selected_motion_rights_path_mismatch:${clip.id}`);
    }
    if (!usedAssetPathMatches && !transitionBinding) {
      throw new Error(`selected_motion_used_asset_path_mismatch:${clip.id}`);
    }
    const recordHash = transitionBinding?.sha256 || rightsRecordHash(record);
    const usedAssetHash = transitionBinding?.sha256 || rightsRecordHash(usedAsset);
    const recordSize = transitionBinding?.size_bytes ?? rightsRecordSize(record);
    const usedAssetSize = transitionBinding?.size_bytes ?? rightsRecordSize(usedAsset);
    ...
    selectedMotion.push({
      ...
      fileEvidence,
      transitionBinding: transitionBinding ? clone(transitionBinding) : null,
    });
  }
  ...
  return {
    ...
    ]).concat(pendingGenerationReceipt?.controlFiles || []),
  };
}
...
function reconciledRightsLedger({
  ...
  const pendingFinalMedia =
    evidence.narrationTransition === PENDING_FINAL_MEDIA_NARRATION_TRANSITION;
  const narration = {
  ...
  return {
    ...
    story_id: storyId,
    verdict: pendingFinalMedia ? "AMBER" : "pass",
    status: pendingFinalMedia ? "ready_for_local_render" : "ready",
    blockers: pendingFinalMedia
      ? [SCRIPT_REPAIR_RIGHTS_TRANSITION_BLOCKER]
      : [],
    failures: [],
    ...(pendingFinalMedia
      ? {
          ...
          render_input_rights_verdict: "PASS",
          final_media_lineage_status: "PENDING",
          final_media_commercial_use_allowed: false,
        }
      : {}),
    records,
      ...
      ...(pendingFinalMedia
        ? {
            generation_receipt_path:
              evidence.pendingGenerationReceipt.receiptPath,
            generation_receipt_sha256:
              evidence.pendingGenerationReceipt.receiptEvidence.sha256,
            generation_receipt_size_bytes:
              evidence.pendingGenerationReceipt.receiptEvidence.size_bytes,
            generation_receipt_verdict:
              evidence.pendingGenerationReceipt.receipt.verdict,
            generation_verdict:
              evidence.pendingGenerationReceipt.receipt.generation_verdict,
            final_media_lineage_status: "PENDING",
            final_media_commercial_use_allowed: false,
...
function flagshipCardPath(cardsDir, storyId, kind) {
  return path.join(cardsDir, `hf_${kind}_card_${storyId}.mp4`);
}

function flagshipCardSidecarPath(cardPath) {
  return clean(cardPath).replace(/\.mp4$/i, ".shell.json");
}

function plannedFlagshipCardDuration(sidecar = {}) {
  const shell = sidecar.hyperframes_premium_shell || sidecar.premium_shell || {};
  const duration = Number(
    shell.readability_contract?.evidence?.planned_visible_duration_s
      ?? shell.readability_contract?.planned_visible_duration_s,
  );
  return Number.isFinite(duration) && duration > 0 ? duration : null;
}

async function inspectSourceFlagshipCardEvidence({
  sourceArtifactDir,
  storyId,
  canonicalSha256,
} = {}) {
  const cardsDir = path.join(sourceArtifactDir, "flagship", "cards");
  if (!(await fs.pathExists(cardsDir))) return null;
  const entries = await fs.readdir(cardsDir);
  const hasCardEvidence = entries.some(
    (entry) => /\.mp4$|\.shell\.json$/i.test(entry),
  );
  if (!hasCardEvidence) return null;

  const cards = [];
  const sidecars = new Map();
  const controlFiles = [];
  for (const kind of DEFAULT_EXPECTED_CARD_KINDS) {
    const cardPath = flagshipCardPath(cardsDir, storyId, kind);
    const sidecarPath = flagshipCardSidecarPath(cardPath);
    if (!(await fs.pathExists(cardPath))) {
      throw new Error(`flagship_card_evidence_missing:${kind}`);
    }
    if (!(await fs.pathExists(sidecarPath))) {
      throw new Error(`flagship_card_sidecar_missing:${kind}`);
    }
    const shellEvidence = evaluateHyperframesPremiumShellEvidence({
      cardPath,
      kind,
      storyId,
      channelId: "pulse-gaming",
    });
    if (shellEvidence.verdict !== "pass") {
      throw new Error(
        `flagship_card_shell_evidence_invalid:${kind}:${
          asArray(shellEvidence.blockers).join("|") || "unknown"
        }`,
      );
    }
    const [cardEvidence, sidecarDocument] = await Promise.all([
      verifyCurrentFile({
        filePath: cardPath,
        label: `flagship_card:${kind}`,
        requireHash: false,
        returnBytes: true,
      }),
      readJson(sidecarPath, `flagship_card_sidecar:${kind}`),
    ]);
    const durationSeconds = plannedFlagshipCardDuration(sidecarDocument.value);
    if (!durationSeconds) {
      throw new Error(`flagship_card_duration_evidence_missing:${kind}`);
    }
    delete cardEvidence.bytes;
    cards.push({
      card_kind: kind,
      path: cardEvidence.path,
      sha256: cardEvidence.sha256,
      size_bytes: cardEvidence.size_bytes,
      duration_seconds: durationSeconds,
    });
    sidecars.set(kind, sidecarDocument);
    controlFiles.push(
      [`flagship_card:${kind}`, cardEvidence],
      [`flagship_card_sidecar:${kind}`, sidecarDocument.fileEvidence],
    );
  }

  const generationDigest = sha256(Buffer.from(JSON.stringify({
    story_id: storyId,
    ...
    cards: cards.map((card) => ({
      ...
      sidecar_sha256: sidecars.get(card.card_kind).fileEvidence.sha256,
      sidecar_size_bytes: sidecars.get(card.card_kind).fileEvidence.size_bytes,
    })),
  })));
  const generationId = `flagship-card-generation-${generationDigest}`;
  return {
    cardsDir,
    generationId,
    manifest: {
      ...
      generation_binding: {
        policy: "canonical_and_card_sidecar_content_hashes_v1",
        canonical_sha256: canonicalSha256,
      },
      cards: cards.map((card) => ({
        ...card,
...
function finalCardEvidenceReport({
  copyReport,
  sourceCardEvidence,
  finalArtifactDir,
  generatedAt,
} = {}) {
  const report = clone(copyReport);
  const finalCardsDir = path.join(finalArtifactDir, "flagship", "cards");
  report.generated_at = generatedAt;
  report.target_artifact_dir = finalArtifactDir;
  report.evidence_path = path.join(
    finalArtifactDir,
    "flagship_card_evidence.json",
  );
  report.generation_binding = clone(
    sourceCardEvidence.manifest.generation_binding,
  );
  report.cards = asArray(report.cards).map((card) => ({
    ...card,
    target_path: path.join(finalCardsDir, card.filename),
  }));
  report.safety = {
    ...(report.safety || {}),
    publish_authorised: false,
...
async function preserveFlagshipCardEvidence({
  sourceArtifactDir,
  sourceCardEvidence,
  stagingArtifactDir,
  finalArtifactDir,
  ...
  if (!sourceCardEvidence) return null;
  const copyReport = await validateAndCopyFlagshipCardEvidence({
    artifactDir: sourceArtifactDir,
    cardManifest: sourceCardEvidence.manifest,
    currentGenerationId: sourceCardEvidence.generationId,
    expectedCardKinds: DEFAULT_EXPECTED_CARD_KINDS,
    approvedRoots: [sourceArtifactDir],
    targetArtifactDir: stagingArtifactDir,
    generatedAt,
    ...(typeof probeVideo === "function" ? { probeVideo } : {}),
  });
  const report = finalCardEvidenceReport({
    copyReport,
    sourceCardEvidence,
    finalArtifactDir,
    generatedAt,
  ...
  const reportCards = new Map(
    report.cards.map((card) => [card.card_kind, card]),
  );
  for (const kind of DEFAULT_EXPECTED_CARD_KINDS) {
    const sourceSidecar = sourceCardEvidence.sidecars.get(kind);
    const card = reportCards.get(kind);
    if (!sourceSidecar || !card) {
      throw new Error(`flagship_card_copy_evidence_incomplete:${kind}`);
    }
    const stagingCardPath = flagshipCardPath(
      path.join(stagingArtifactDir, "flagship", "cards"),
      storyId,
      kind,
    );
    const stagingSidecarPath = flagshipCardSidecarPath(stagingCardPath);
    const copiedSidecar = clone(sourceSidecar.value);
    copiedSidecar.generated_at = generatedAt;
    copiedSidecar.generation_id = sourceCardEvidence.generationId;
    copiedSidecar.output_path = card.target_path;
    copiedSidecar.hyperframes_premium_shell = {
      ...(copiedSidecar.hyperframes_premium_shell || {}),
      generation_id: sourceCardEvidence.generationId,
      output_path: card.target_path,
    };
    copiedSidecar.card_evidence_copy = {
      policy: "validated_content_bound_local_copy_v1",
      copied_at: generatedAt,
      source_path: card.source_path,
      target_path: card.target_path,
      source_sha256: card.sha256,
      source_size_bytes: card.size_bytes,
      target_sha256: card.sha256,
      target_size_bytes: card.size_bytes,
      source_sidecar_path: sourceSidecar.fileEvidence.path,
      source_sidecar_sha256: sourceSidecar.fileEvidence.sha256,
      source_sidecar_size_bytes: sourceSidecar.fileEvidence.size_bytes,
      generation_id: sourceCardEvidence.generationId,
      render_reexecuted: false,
      publish_authorised: false,
    };
    await fs.writeJson(stagingSidecarPath, copiedSidecar, { spaces: 2 });
  }
  await fs.writeJson(
    path.join(stagingArtifactDir, "flagship_card_evidence.json"),
    report,
    { spaces: 2 },
  );
  return report;
...
async function repairFlagshipRenderWorkOrder(options = {}) {
  ...
  let sourceCanonicalDocument = null;
  let sourceCanonical = null;
  if (await fs.pathExists(sourceCanonicalPath)) {
    sourceCanonicalDocument = await readJson(
      sourceCanonicalPath,
      "source_canonical_story_manifest",
    );
    sourceCanonical = sourceCanonicalDocument.value;
  ...
  const sourceCardEvidence = await inspectSourceFlagshipCardEvidence({
    sourceArtifactDir,
    storyId,
    canonicalSha256: sourceCanonicalDocument.fileEvidence.sha256,
  });
  ...
  const report = {
    ...
    preserved_flagship_card_evidence: null,
    publish_authorised: false,
    safety: {
      render_executed: false,
      no_publish_triggered: true,
  ...
  try {
    ...
    report.preserved_flagship_card_evidence = await preserveFlagshipCardEvidence({
      sourceArtifactDir,
      sourceCardEvidence,
      stagingArtifactDir,
      finalArtifactDir: artifactDir,
      storyId,
      generatedAt,
      probeVideo: options.cardProbeVideo,
    });
    ...
    await assertCurrentEvidenceUnchanged(current, [
      ...
      ...asArray(sourceCardEvidence?.controlFiles),
    ]);
