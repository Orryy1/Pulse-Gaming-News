...
function curiosityPrompt(value) {
  const line = stripTerminalPunctuation(publicNarrationLine(value));
  const pointMatch = line.match(/^The point is not that\s+(.+?)\.\s*It is whether\s+(.+)$/i);
  if (pointMatch) return `whether ${pointMatch[2]}`;
  return line
    .replace(/^The (?:point to watch|main question|unknown|remaining question) is\s+/i, "")
    .replace(/^The (?:thing to watch|real test|useful question) is\s+/i, "")
    .replace(/^Judge it on\s+/i, "")
    .replace(/^.+?;\s*the open question is\s+/i, "")
    .trim() || "whether the footage backs up the promise";
}
...
function buildNarrativeSegment(segment) {
  ...
  const curiosityAsPrompt = curiosityPrompt(segment.curiosity_gap);
  ...
  const variants = [
    [
      ...
      `It lands ${date} for ${platform}. The footage still has to settle this: ${lowerFirst(curiosityAsPrompt)}.`,
      `The catch is ${lowerFirst(risk)}.`,
      `My call is ${verdict}. ${payoff}.`,
      `Use the official page to verify the date, then use the footage to judge the game. ${segment.debate_prompt}`,
    ],
    [
      `${rank}. ${title}. This slot is about proof, not logo size.`,
      sentence(impact),
      `The pitch only works if it resolves this: ${lowerFirst(curiosityAsPrompt)}.`,
    ...
    [
      ...
      sentence(impact),
      `The sharper question: ${sentenceCaseStart(curiosityAsPrompt)}.`,
      `That matters because ${lowerFirst(risk)}.`,
      `Verdict: ${verdict}. ${payoff}.`,
      `The official sources establish the promise; the footage still has to prove the depth.`,
    ],
    [
    ...
    [
      ...
      sentence(impact),
      `The trailer still has to resolve this: ${lowerFirst(curiosityAsPrompt)}.`,
      `The weak point: ${sentenceCaseStart(risk)}.`,
      `I would keep it at ${verdict}. ${payoff}.`,
      `Treat the official page as the promise and the footage as the evidence. ${segment.debate_prompt}`,
    ],
    [
    ...
    [
      ...
      sentence(impact),
      `The remaining question: ${sentenceCaseStart(curiosityAsPrompt)}.`,
      `That is only exciting if the game avoids this problem: ${lowerFirst(risk)}.`,
      `Verdict: ${verdict}. ${payoff}.`,
      `The package is confirmed; performance footage still has to sell the return.`,
    ],
    [
    ...
    [
      ...
      sentence(impact),
      `It lands ${date} for ${platform}. The release still has to answer this: ${lowerFirst(curiosityAsPrompt)}.`,
      `The easiest way this goes wrong: ${lowerFirst(risk)}.`,
      `My call is ${verdict}. ${payoff}.`,
      `Use the demo or trailer before buying the premise. ${segment.debate_prompt}`,
    ],
    [
      `${rank}. ${title}. The final slot is the gamble.`,
      sentence(impact),
      `It only ranks this high if it resolves this: ${lowerFirst(curiosityAsPrompt)}.`,
...
function practicalVerdictSummary(segments = []) {
  const groups = new Map();
  for (const segment of segments) {
    const key = text(segment.verdict).toLowerCase();
    if (!groups.has(key)) groups.set(key, []);
    groups.get(key).push(segment.canonical_game);
  }
  const lines = ["Here is the practical split."];
  const subscriptions = [
    ...(groups.get("subscription_play") || []),
    ...(groups.get("game_pass") || []),
  ];
  const demos = groups.get("demo_first") || [];
  const wishlists = groups.get("wishlist") || [];
  const waits = groups.get("wait") || [];
  const buys = [...(groups.get("buy") || []), ...(groups.get("day_one") || [])];
  if (subscriptions.length) {
    lines.push(`Use the subscription route before paying full price for ${naturalList(subscriptions)}.`);
  }
  if (demos.length) lines.push(`Use the demo before buying ${naturalList(demos)}.`);
  if (wishlists.length) lines.push(`Wishlist ${naturalList(wishlists)} and wait for stronger launch proof.`);
  if (waits.length) lines.push(`Wait for firmer evidence on ${naturalList(waits)}.`);
  if (buys.length) lines.push(`The strongest day-one case belongs to ${naturalList(buys)}.`);
  return lines.join(" ");
}
...
function buildLongformScript({ monthLabel, segments }) {
  const monthName = text(monthLabel).split(/\s+/)[0] || "this month";
  const sleeper = segments.find((segment) => segment.rank > 3) || segments.at(-1);
  for (const segment of segments) {
    segment.script = buildNarrativeSegment(segment);
  }
  const lines = [
    `${monthLabel} has ${segments.length} games fighting for your time and money. The biggest name is not automatically the safest pick, and one free demo could save you a bad preorder.`,
    `This is not a hype countdown. It is a buyer-risk ranking built from real dates, official footage, a concrete reason to care and the weak point each trailer still has to answer.`,
    `Some of these games can be tested through a subscription or demo. Others are asking for a full-price leap on trailer evidence alone. By the end, every game gets a clear play, demo, wishlist or wait call.`,
    `The rule is simple. A famous name only gets you into the conversation. To rank highly here, the game needs a clear player reason, visible motion and a risk that is small enough to live with. If a trailer hides the loop, if the store page is doing all the work or if the pitch only sounds good as a headline, it drops.`,
    `That matters because ${monthName} is full of games people already want to believe in. Remakes, expansions, cosy sleepers and live-service comebacks all sell a feeling before they prove the routine. The useful question is not whether the logo is exciting. It is whether the game still sounds worth playing once the trailer music stops.`,
    "",
    ...segments.map((segment) => segment.script),
    "",
    practicalVerdictSummary(segments),
    ...
    `That is the ranking. Which game is too low, and which one is still living on the strength of its trailer?`,
  ];
  return lines.join("\n\n");
}
