...
function compactCanonicalSubject(story = {}) {
  ...
  const knownGameRe =
    /\b(?:GTA\s*6|GTA\s*5|Grand Theft Auto VI|Grand Theft Auto V|Forza Horizon 6|EA SPORTS FC 26|The Adventures of Elliot|Dave the Diver|Granblue Fantasy|Garfield|Free Play Days|Nintendo Switch 2)\b/i;
...
function chooseCanonicalScript(story = {}) {
  ...
  if (provided && providedQa?.result === "pass") {
    return {
      script: provided,
  ...
  if (rebuiltScript && rebuiltQa?.result === "pass") {
    return {
      ...
      repair_reason: providedQa?.failures?.[0] || "provided_script_missing_or_failed_coherence",
...
function buildCanonicalStoryManifest(story = {}, generatedAt = new Date().toISOString()) {
  ...
  const subject = compactCanonicalSubject(story) || cleanText(story.title);
  const selectedTitle = cleanText(story.selected_title || scriptChoice.repaired_title || story.title);
