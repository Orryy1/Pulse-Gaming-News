...
function publicTitleIsWeak(value = "") {
  ...
  if (!text) return true;
  return /\b(?:Official\s+Trailer|Official\s+Gameplay|Source[-\s]?Backed Update|Player Impact|Demo Test|Motion\s+Proof)\b/i.test(text);
}
...
function compactCanonicalSubject(story = {}) {
  ...
  const knownGameRe =
    /\b(?:DOOM:\s*The Dark Ages|DOOM\s+The Dark Ages|Dune:\s*Awakening|Dune\s+Awakening|Marvel\s+Rivals|Palworld|Halo:\s*Campaign Evolved|Halo Campaign Evolved|Call of Duty:\s*Black Ops|Black Ops(?:\s+Classics|\s+1\s+and\s+2)?|Onimusha:\s*Way of the Sword|Onimusha\s+Way of the Sword|Diablo\s*(?:IV|4)|Dead by Daylight(?:\s*:\s*The Life Road)?|League of Legends|Invincible VS|GUILTY\s+GEAR\s*[-:]?\s*STRIVE|Ocarina of Time|Ocarina's|Cyberpunk 2077|Lords of the Fallen\s*2|GTA\s*6|GTA\s*5|Grand Theft Auto VI|Grand Theft Auto V|Forza Horizon 6|EA SPORTS FC 26|The Adventures of Elliot|Dave the Diver|Granblue Fantasy|Garfield|Free Play Days|Nintendo Switch 2)\b/i;
