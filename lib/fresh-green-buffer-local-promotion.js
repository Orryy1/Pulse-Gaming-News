...
function buildCanonicalStoryManifest(story = {}, generatedAt = new Date().toISOString()) {
  ...
  return {
    ...
    selected_title: cleanText(story.selected_title || story.title),
    short_title: cleanText(story.short_title || story.title),
    thumbnail_headline: cleanText(story.thumbnail_headline || story.suggested_thumbnail_text || story.title),
    first_spoken_line: script.split(/(?<=[.!?])\s+/).filter(Boolean)[0] || "",
