...
function qualityFailuresForDraft(draft = {}) {
  ...
  const gameplayTrailerScaffoldSignals = [
    /\bshowed the part trailers usually hide:\s*how it plays\b/i,
    /\bjudge movement,\s*combat and camera weight\b/i,
    /\bpitch survives contact with the controller\b/i,
    /\bthe blunt test is (?:this:\s*)?(?:does|whether) the gameplay stay(?:s)? fun\b/i,
    /\bquick cuts can disguise pacing problems\b/i,
    /\bfamiliar licence in motion\b/i,
  ].filter((pattern) => pattern.test(script)).length;
  if (gameplayTrailerScaffoldSignals >= 2) {
    failures.push("local_intake:generic_gameplay_trailer_scaffold_script");
  }
  if (
...
async function buildFreshReviewLocalPromotionIntake({
  ...
  const sourceApprovedRows = approvedRows || (rows === null
    ? fetchFreshApprovedProductionRows({
        db,
        now: selectedAt,
        maxAgeHours: approvedMaxAgeHours,
        limit: Math.max(40, Number(limit || 6) * 10),
      })
    : []);
  ...
  const selectedApprovedRows = selectFreshApprovedProductionRows(
    sourceApprovedRows,
    {
      now: selectedAt,
      maxAgeHours: approvedMaxAgeHours,
  ...
  const approvedIds = new Set(selectedApprovedRows.map(storyId));
  const selectedIds = new Set();
  const selected = [
    ...selectedApprovedRows,
    ...selectedRowsFromPlan(sourceRows, repairPlan),
  ].filter((row) => {
  ...
  for (const row of selected) {
    ...
    repairResults.push({
      ...
      intake_source: approvedIds.has(storyId(row))
        ? "approved_production_backlog"
        : "review_script_repair",
  ...
  const remainingMotionSlots = Math.max(
    0,
    Number(limit || 6) - drafts.length,
  );
  const selectedMotionScorecards = remainingMotionSlots > 0
    ? dedupeMotionScorecards([...drafts, ...asArray(motionScorecards)])
        .filter((scorecard) => motionScorecardIsPromotable(scorecard, { maxAgeHours }))
        .slice(0, remainingMotionSlots)
    : [];
  ...
  return {
    ...
    summary: {
      ...
      approved_rows_seen: asArray(sourceApprovedRows).length,
      approved_production_selected_count: selectedApprovedRows.length,
