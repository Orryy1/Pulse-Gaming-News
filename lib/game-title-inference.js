...
const SUBJECT_VERB_RE =
  /\b(?:drops?|dropped|falls?|fell|exits?|exited|hits?|hit|reaches?|reached|passes?|passed|slips?|slipped|loses?|lost|gets?|got|receives?|received|launches?|launched|returns?|returned|revives?|revived|reveals?|revealed|announces?|announced|confirms?|confirmed|delays?|delayed|adds?|added|is|are|was|were|will|has|have|had)\b/i;
...
function cleanCandidate(value) {
  return normalise(value)
    ...
    .replace(
      /\b(?:pc specs?|system requirements|global release times?|release times?|launch times?|review scores?|mixed reviews?|sequel launches?|sequel|remake|remaster|remastered|update|trailer|leak)\b.*$/i,
      "",
    )
    .trim();
}
...
function directColonCandidates(base) {
  ...
  return [base.slice(0, colonIdx), base.slice(colonIdx + 1)]
    .map(cleanCandidate)
...
function leadingSubjectCandidate(base) {
  ...
  const candidate = cleanCandidate(match[1]).replace(/\b(?:just|finally)$/i, "").trim();
