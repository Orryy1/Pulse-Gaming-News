...
const SUBJECT_VERB_RE =
  /\b(?:drops?|dropped|falls?|fell|exits?|exited|hits?|hit|reaches?|reached|passes?|passed|beats?|beaten|breaks?|broke|broken|slips?|slipped|loses?|lost|gets?|got|receives?|received|launches?|launched|returns?|returned|revives?|revived|reveals?|revealed|shows?|showed|announces?|announced|confirms?|confirmed|delays?|delayed|adds?|added|prepares?|prepared|becomes?|became|becoming|is|are|was|were|will|has|have|had|won't|can't|isn't|aren't|doesn't|don't|didn't|couldn't|wouldn't|shouldn't)\b/i;

const SENTENCE_FRAGMENT_PREFIX_RE =
  /^(?:it'?s|it\s+is|this\s+is|that\s+is|there'?s|there\s+is|here'?s|here\s+is|what\s+we\s+know|why\s+it\s+matters|how)\b/i;

const COLON_LABEL_RE =
  /^(?:analysis|breaking(?:\s+news)?|exclusive|first\s+look|guide|interview|news|opinion|preview|report|review|rumou?r|roundup|state\s+of\s+play|trailer|update|watch|xbox\s+showcase|nintendo\s+direct)$/i;
...
function isLikelyGameTitleCandidate(value) {
  ...
  if (/^season\s+(?:one|two|three|four|five|six|seven|eight|nine|ten|\d+)$/i.test(normalised)) {
    return false;
  }
...
function compoundColonCandidate(base) {
  const colonIdx = base.indexOf(":");
  if (colonIdx === -1) return null;
  const left = cleanCandidate(base.slice(0, colonIdx));
  if (COLON_LABEL_RE.test(left) || !isLikelyGameTitleCandidate(left)) return null;
  const rightSource = base.slice(colonIdx + 1);
  const rightMatch =
    rightSource.match(/^\s*([A-Z0-9][A-Za-z0-9'.& -]{1,80}?)'s\b/) ||
    rightSource.match(
      /^\s*([A-Z0-9][A-Za-z0-9'.& -]{1,80}?)\s+(?:pc\s+specs?|system\s+requirements|global\s+release\s+times?|release\s+times?|launch\s+times?|review\s+scores?)\b/i,
    ) ||
    rightSource.match(
      new RegExp(
        `^\\s*([A-Z0-9][A-Za-z0-9'.& -]{1,80}?)\\s+${SUBJECT_VERB_RE.source}`,
        "i",
      ),
    );
  if (!rightMatch) return null;
  const right = cleanCandidate(rightMatch[1]);
  if (!isLikelyGameTitleCandidate(right)) return null;
...
function directColonCandidates(base) {
  ...
  const compound = compoundColonCandidate(base);
...
function howStyleColonCandidate(base) {
  const match = String(base || "").match(
    /^how\s+([A-Z][A-Za-z0-9'.& -]{1,40}:\s*[A-Z][A-Za-z0-9'.& -]{1,60}?)\s+(?:turns?|gets?|is|has|could|will|uses?|makes?|becomes?|shows?|reveals?|brings?|launches?|changes?|adds?)\b/i,
  );
...
function attributedGameCandidate(base) {
  const match = String(base || "").match(
    /^.{2,48}?\s+(?:says|reports|confirms|announces|reveals)\s+([A-Z][A-Za-z0-9:+'.& -]{2,80}?)\s+(?:is|are|has|have|will|gets?|faces?|launches?|returns?|adds?|shows?|brings?)\b/i,
  );
  if (!match) return null;
  const candidate = cleanCandidate(match[1]);
  return isLikelyGameTitleCandidate(candidate) ? candidate : null;
...
function inferHeadlineGameCandidates(rawTitle) {
  ...
  const candidates = [
    howStyleColonCandidate(base),
    attributedGameCandidate(base),
