...
function dominantStoryText(canonical = {}, storyPackage = {}) {
  return cleanText([
    canonical.selected_title,
    canonical.short_title,
    canonical.title,
...
function primaryOfferMismatch({ canonical = {}, storyPackage = {}, affiliate = {} } = {}) {
  ...
  if (isRacingOffer(offer) && !isRacingStory(dominantStoryText(canonical, storyPackage))) {
    return "racing_offer_on_non_racing_story";
  }
  return "";
}

function affiliateLandingRoute(affiliate = {}) {
  return cleanText(
    affiliate.landing_page_route ||
      affiliate.landing_page_attribution?.landing_page_route ||
      affiliate.revenue_attribution?.landing_page_route,
  );
}

function affiliateLandingSlug(affiliate = {}) {
  const route = affiliateLandingRoute(affiliate);
  return cleanText(affiliate.landing_page_slug || affiliate.landing_page_attribution?.landing_page_slug || route.replace(/^\/p\//, ""));
}

function landingRouteMismatch(landing = {}, affiliate = {}) {
  const expected = affiliateLandingRoute(affiliate);
  if (!expected) return "";
  const current = cleanText(landing.landing_page_route);
  if (current && current !== expected) return "stale_landing_route";
  return "";
}
...
function repairedLandingManifest(current = {}, repairedAffiliate = {}, generatedAt) {
  const repairedRoute = affiliateLandingRoute(repairedAffiliate);
  const repairedSlug = affiliateLandingSlug(repairedAffiliate);
  return {
    ...current,
    landing_page_slug: repairedSlug || current.landing_page_slug || current.slug || null,
    landing_page_route: repairedRoute || current.landing_page_route || null,
    link_pack: {
      ...(current.link_pack || {}),
...
async function inspectAffiliateRelevanceRepair(storyPackage = {}, { generatedAt = new Date().toISOString() } = {}) {
  ...
  const routeMismatch = landingRouteMismatch(landing, affiliate);
  if (!storyId || !artifactDir || (!mismatch && !routeMismatch)) {
    return {
      story_id: storyId,
      artifact_dir: artifactDir || null,
      status: mismatch || routeMismatch ? "blocked" : "not_required",
      mismatch_reason: mismatch || routeMismatch || null,
    };
  }
  const repairedAffiliate = mismatch
    ? buildAffiliateLinkManifest({
  ...
  return {
    ...
    mismatch_reason: mismatch || routeMismatch,
    before: {
