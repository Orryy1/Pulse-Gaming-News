...
async function compactGeneratedNarrationSilence(
  ...
  const boundedStopDurationS = Math.min(0.75, Math.max(0.1, Number(stopDurationS) || 0.75));
  try {
    await execFileImpl(
      ...
      [
        ...
        audioPath,
        "-af",
        `silenceremove=stop_periods=-1:stop_duration=${boundedStopDurationS}:stop_threshold=${silenceThreshold}:stop_silence=${retainedSilenceS}`,
        "-c:a",
        "libmp3lame",
        "-q:a",
        "2",
