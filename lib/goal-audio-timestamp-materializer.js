...
async function syncCanonicalAudioMetadata({
  ...
  const spokenText = cleanPublicText(current.spoken_narration_script || current.tts_script || narrationText);
...
async function loadStoryForAudio(job = {}) {
  ...
  const spokenText = explicitSpokenText || cleanSpokenTextForTts(text);
  return {
    artifactDir,
    canonical,
    story: {
  ...
  let jobs = asArray(workbenchReport.jobs).filter(
    (job) => {
      ...
      return (
        ...
        (force === true && status === "ready_audio_timestamp_pair")
      );
    },
  );
