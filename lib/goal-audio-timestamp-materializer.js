  ...
  try {
    await execFileImpl(
      ...
      [
        ...
        audioPath,
        "-af",
        `silenceremove=stop_periods=-1:stop_duration=0.75:stop_threshold=${silenceThreshold}:stop_silence=${retainedSilenceS}`,
        "-c:a",
        "libmp3lame",
        "-q:a",
        "2",
...
async function syncCanonicalAudioMetadata({
  ...
  const spokenText = cleanSpokenTextForTts(current.spoken_narration_script || current.tts_script || narrationText);
...
async function loadStoryForAudio(job = {}) {
  ...
  const spokenText = cleanSpokenTextForTts(explicitSpokenText || text);
  return {
    artifactDir,
    canonical,
    story: {
...
async function materializeJob(job = {}, options = {}) {
  ...
  if (!options.force && !asrAlignmentOnly) {
    ...
    const staleReasons = [
      ...
      ...failedVoiceCadenceReasons(readyPair, voiceQualityReport, voiceQualityStat),
      ...alignmentReasons,
    ];
  ...
  try {
    for (let attempt = 1; attempt <= maxGenerationAttempts; attempt += 1) {
      ...
      try {
        ...
        if (typeof options.recoverLocalTtsAfterFailure === "function") {
...
async function materializeGoalAudioTimestamps({
  ...
  for (const job of jobs) {
    ...
    try {
      const asrAlignmentOnly = cleanText(job.status) === "requires_word_timestamp_asr_alignment";
      selectedProvider = asrAlignmentOnly
