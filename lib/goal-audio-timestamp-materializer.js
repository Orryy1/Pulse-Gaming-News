...
function retryableLocalTtsGenerationFailure(error) {
  return /local_whisper_word_alignment_failed|tts_failed|Request failed with status code 500|local_tts_generation_failed:server_error|local_tts_generation_failed:server_down|recoverable generation error|server_down|local TTS server is not reachable|connection_reset|ECONNRESET|socket hang up|local TTS connection reset/i.test(String(error?.message || error));
}
...
async function syncCanonicalAudioMetadata({
  ...
  const spokenText = selectSpokenTextForTts(
    narrationText,
    current.spoken_narration_script || current.tts_script,
    { protectedTitles: canonicalProtectedTitles(current) },
  );
...
async function loadStoryForAudio(job = {}) {
  ...
  const explicitSpokenText = cleanPublicText(canonical.spoken_narration_script || canonical.tts_script);
  const spokenText = selectSpokenTextForTts(text, explicitSpokenText, {
  ...
  try {
    for (let attempt = 1; attempt <= maxGenerationAttempts; attempt += 1) {
      ...
      try {
        ...
        if (provider === "local" && typeof options.recoverLocalTtsAfterFailure === "function") {
  ...
  return {
    ...
    ...(
      generationAttempts > 1
        ? { generation_attempts: generationAttempts, reason: "tts_retry_after_strict_alignment_failure" }
        : staleExistingPair
