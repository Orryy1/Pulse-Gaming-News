...
function supportsExistingPauseCompaction(voiceQualityReport = {}) {
  ...
  return [...asArray(voiceQualityReport?.blockers), ...asArray(cadence.blockers)]
    .map((item) => cleanText(item).toLowerCase())
    .includes("voice_cadence:acoustic_pause_too_long");
}
...
async function materializeJob(job = {}, options = {}) {
  ...
  if (!options.force && !asrAlignmentOnly) {
    ...
    if (
      ...
      (supportsExistingPauseCompaction(voiceQualityReport) || titlePauseReasons.length)
    ) {
      ...
      try {
        ...
        if (compaction?.repaired === true) {
          ...
          timestampInfo.narration_silence_compaction = compaction;
          ...
          return {
            story_id: storyId,
            title: cleanText(job.title),
            status: "materialized_existing_cadence_compaction",
            provider: "existing_local_audio",
            reason: "existing_pair_excessive_pause_compacted_and_realigned",
            audio_path: relAudioPath(storyId),
            word_timestamps_path: relTimestampPath(storyId),
            word_count: timestampInfo.word_count,
            audio_size_bytes: stat.size,
            narration_silence_compaction: compaction,
          };
        }
      } finally {
