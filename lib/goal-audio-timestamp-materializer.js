...
function selectSpokenTextForTts(narrationText = "", explicitSpokenText = "") {
  const narration = cleanSpokenTextForTts(narrationText);
  const explicit = cleanSpokenTextForTts(explicitSpokenText);
...
function staleSpeechTextReasons(pair = {}, spokenText = "", displayText = "") {
  ...
  if (
    requiresTitleColonPauseProfile(displayText) &&
    profileVersion !== TTS_PRONUNCIATION_PROFILE_VERSION
  ) {
...
async function materializeJob(job = {}, options = {}) {
  ...
  if (!options.force && !asrAlignmentOnly) {
    ...
    const staleReasons = [
      ...
      ...staleSpeechTextReasons(readyPair, loaded.spokenText, loaded.text),
      ...cadenceReasons,
      ...alignmentReasons,
    ];
