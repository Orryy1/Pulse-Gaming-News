...
function timestampCoverageTokens(words = []) {
  const tokens = asArray(words)
    .flatMap((word, index) =>
      coverageTokenParts(word.word || word.text).map((part) => ({
        index,
        word: part.word,
        norm: part.norm,
        timing: word,
...
function nativeCadenceRejectionError(cadence = {}) {
  const cadenceBlockers = asArray(cadence.blockers).filter((blocker) =>
    /^voice_cadence:wpm_too_(?:fast|slow)$/.test(cleanText(blocker)),
  );
    ...
    for (let attempt = 1; attempt <= maxGenerationAttempts; attempt += 1) {
      ...
      try {
        ...
        if (shouldEnforceNativeCadenceBeforePromotion(options, provider)) {
          ...
          if (cadenceError) {
            const fastCadence = asArray(preAlignmentCadence.blockers)
              .includes("voice_cadence:wpm_too_fast");
            if (fastCadence && shouldPadFastNativeCadence(options, provider)) {
              fastNativeCadenceNeedsPadding = true;
              preAlignmentCadenceError = cadenceError;
            } else {
              throw cadenceError;
            }
          }
