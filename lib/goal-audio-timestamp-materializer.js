...
function applyDisplayCaptionTextRepairs(words = [], displayText = "") {
  const source = cleanText(displayText);
  if (!source) return { words, repaired: false, replacements: [] };
  const keys = words.map((word) => captionTokenKey(word.word || word.text || word.value));
  const repairedWords = [];
  const replacements = [];
  ...
  for (let index = 0; index < words.length; index += 1) {
    ...
    if (compactAlphanumeric) {
      const spoken = words
        ...
        .join(" ");
      repairedWords.push(
        mergeCaptionWordWindow(words, index, compactAlphanumeric.count, compactAlphanumeric.value),
      );
    ...
    if (decimal) {
      const spoken = words
        ...
        .map((word) => cleanText(word.word || word.text || word.value).replace(/[.!?,;:]+$/g, ""))
        .join(" ");
      repairedWords.push(mergeCaptionWordWindow(words, index, decimal.count, decimal.value));
    ...
    if (
      ...
      repairedWords.push(mergeCaptionWordWindow(words, index, 2, replacement));
      ...
      if (yearPart && yearPart.value >= 10 && yearPart.value <= 99) {
        ...
        if (new RegExp(`\\b${year}\\b`).test(source)) {
          ...
          const spoken = words
            ...
            .map((word) => cleanText(word.word || word.text || word.value).replace(/[.!?,;:]+$/g, ""))
            .join(" ");
          repairedWords.push(mergeCaptionWordWindow(words, index, count, String(year)));
    ...
    if (keys[index] === "g" && keys[index + 1] === "t" && keys[index + 2] === "a") {
      ...
      if (replacement) {
        repairedWords.push(mergeCaptionWordWindow(words, index, 4, replacement));
    ...
    repairedWords.push(words[index]);
  }
...
function mergeObservedSplitCompounds(expectedTokens = [], actualTokens = []) {
  const expectedCompoundNorms = new Set(
    asArray(expectedTokens)
      .filter((token) => {
        ...
        return word && norm.length >= 7 && !/[\s-]/.test(word);
      })
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    const safeObservedSplit =
      ...
      nextNorm.length >= 3 &&
      expectedCompoundNorms.has(joinedNorm);
...
function protectedTitlePauseReasons(pair = {}, protectedTitles = [], maxGapS = 0.16) {
  ...
  const threshold = Math.max(0.08, Number(maxGapS) || 0.16);
...
async function compactTimestampedNarrationPauses(
  ...
  const protectedThreshold = Math.max(0.08, Number(maxProtectedTitleGapS) || 0.16);
...
async function padTimestampedNarrationSentencePauses(
  ...
  const allocatePauseCapacity = (candidates) => {
    let active = candidates;
    while (remainingS > 0.001 && active.length) {
      const share = remainingS / active.length;
      const nextActive = [];
      for (const boundary of active) {
        const available = Math.max(0, boundary.capacity - boundary.inserted);
        const inserted = Math.min(available, share);
        boundary.inserted += inserted;
        remainingS -= inserted;
        if (available - inserted > 0.001) nextActive.push(boundary);
      }
      active = nextActive;
    }
  };
  allocatePauseCapacity(sentenceBoundaries);
  if (remainingS > 0.001) allocatePauseCapacity(clauseBoundaries);
  if (remainingS > 0.001) allocatePauseCapacity(prosodicBoundaries);
  if (remainingS > 0.001) allocatePauseCapacity(breathBoundaries);
...
async function normaliseTimestampFile(
    ...
    if (requiresWhisperWordAlignment(mode) && alignWordsWithAudio) {
      ...
      for (const model of whisperModelCandidates()) {
        ...
        if (aligned?.ok === true && Array.isArray(aligned.words) && aligned.words.length) {
          ...
          const protectedBrandMismatch = protectedBrandPhraseMismatch({
            scriptText: spokenText || text,
            transcript: aligned.transcript,
            words: usableAlignedWords,
          });
          if (protectedBrandMismatch) {
            attempts.push({
              model: aligned.model || model || null,
              error: protectedBrandMismatch.error,
              transcript: protectedBrandMismatch.transcript,
              expected_phrase: protectedBrandMismatch.expected_phrase,
              actual_phrase: protectedBrandMismatch.actual_phrase,
            });
            continue;
          }
  ...
  const rawAsrWordText = words
    ...
    .filter(Boolean)
    .join(" ");
  const displayTextRepair = applyDisplayCaptionTextRepairs(words, text);
...
async function materializeJob(job = {}, options = {}) {
  ...
  if (!options.force && !asrAlignmentOnly) {
    ...
    const fastCadencePaddingSupported = supportsExistingFastCadencePadding(voiceQualityReport);
    if (
      readyPair.ready &&
      (cadenceReasons.length || titlePauseReasons.length) &&
      ...
      try {
        ...
        if (cadenceRepair?.repaired === true || fastCadencePaddingSupported) {
          const timestampInfo = await normaliseTimestampFile(stagedTimestampPath, {
            generatedAt,
            text: loaded.text,
          ...
          if (shouldPadCadence) {
            ...
            timestampInfo.cadence_pause_padding = padding;
            timestampInfo.cadence_pause_timestamp_shift =
