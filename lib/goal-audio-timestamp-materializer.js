...
async function compactTimestampedNarrationPauses(
  ...
  for (let index = 1; index < usable.length; index += 1) {
    ...
    if (!Number.isFinite(gap) || gap <= maxGapS) continue;
    cuts.push({
      start: previousEnd + keepGap / 2,
      end: nextStart - keepGap / 2,
      original_gap_s: gap,
    });
  }
  if (!cuts.length) {
...
async function materializeJob(job = {}, options = {}) {
  ...
  if (!options.force && !asrAlignmentOnly) {
    ...
    if (readyPair.ready && cadenceReasons.length && supportsExistingPauseCompaction(voiceQualityReport)) {
