...
async function normaliseTimestampFile(
      ...
      for (const model of whisperModelCandidates()) {
        ...
        if (aligned?.ok === true && Array.isArray(aligned.words) && aligned.words.length) {
          ...
          if (reconciled.ok === true) {
            ...
            if (Number(reconciled.coverage?.inserted_actual_word_count || 0) > MAX_SAFE_ASR_INSERTED_WORDS) {
              attempts.push({
                model: aligned.model || model || null,
  ...
  const repairDuration = Number(payload.meta?.acoustic?.durationSeconds || alignment?.meta?.acoustic?.durationSeconds);
  ...
  try {
    for (let attempt = 1; attempt <= maxGenerationAttempts; attempt += 1) {
      ...
      try {
        ...
        const timestampPath = await promoteWorkspaceGeneratedMediaPath(workspaceRoot, relTimestampPath(storyId), options);
        ...
        const retryableStrictAlignmentFailure =
          provider === "local" &&
          requiresWhisperWordAlignment(options.alignmentMode) &&
          /local_whisper_word_alignment_failed|tts_failed|Request failed with status code 500/i.test(String(error?.message || error));
