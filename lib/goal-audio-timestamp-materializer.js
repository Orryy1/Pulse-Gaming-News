...
function hasRiskyGtaSixSpeechEvidence(value = "") {
  ...
  return /\b(?:g\s+t\s+a|gta|grand\s+theft\s+auto)\s+(?:v\s+i|vi|6|six|(?:s|si|sy|sigh|see|sea|c)\s+six|six\s+six)\b/.test(text);
}
...
async function syncAudioReviewManifests({
  ...
  const pronunciationAudit = gtaViSpokenPronunciationAudit(effectiveTranscript);
