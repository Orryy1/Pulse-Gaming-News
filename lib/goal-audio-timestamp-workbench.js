...
async function inspectTimestampCandidate(filePath) {
  ...
  const characterAlignment = hasCharacterAlignment(payload);
  return {
    path: path.resolve(resolvedPath),
    exists: true,
    usable: false,
    word_count: 0,
    format: characterAlignment ? "character_alignment" : "unknown",
    reason: characterAlignment
      ? "character_alignment_not_word_timestamps"
      : "word_timestamps_missing",
    requires_asr_alignment: characterAlignment,
    asr_aligned: false,
    mtime_ms: stat.mtimeMs,
  };
}
...
function statusForJob({
  ...
  requiresExistingCadenceRepair = false,
} = {}) {
  if (requiresExistingCadenceRepair && audio?.exists && timestamps?.exists) {
    return "requires_existing_audio_cadence_repair";
  }
...
function forceStaleAudioRegeneration(job = {}) {
  ...
  const scriptRewriteAudio = blockers.includes("final_narration_audio_stale_after_script_rewrite");
  const scriptRewriteTimestamps = blockers.includes("word_timestamps_stale_after_script_rewrite");
  return {
    audio: publicCopyAudio || durationAudio || pronunciationAudio || scriptRewriteAudio,
    timestamps: publicCopyTimestamps || durationTimestamps || pronunciationTimestamps || scriptRewriteTimestamps,
    audioReason: pronunciationAudio
      ? "stale_after_pronunciation_repair"
      : scriptRewriteAudio
        ? "stale_after_script_rewrite"
        : durationAudio
...
function failedVoiceCadenceQa(voiceQualityReport = {}) {
  if (!voiceQualityReport || typeof voiceQualityReport !== "object") return false;
  const cadence = voiceQualityReport.cadence && typeof voiceQualityReport.cadence === "object"
    ? voiceQualityReport.cadence
    : {};
  const evidence = [
    cleanText(voiceQualityReport.verdict || voiceQualityReport.status).toLowerCase(),
    cleanText(cadence.status).toLowerCase(),
    ...asArray(voiceQualityReport.blockers).map((item) => cleanText(item).toLowerCase()),
    ...asArray(cadence.blockers).map((item) => cleanText(item).toLowerCase()),
  ].filter(Boolean);
  return (
...
function currentFailedVoiceCadenceQa(
  voiceQualityReport = {},
  voiceQualityMtimeMs = null,
  audio = {},
  timestamps = {},
) {
  if (!failedVoiceCadenceQa(voiceQualityReport)) return false;
  const reportMtime = Number(voiceQualityMtimeMs) || 0;
  const pairMtime = Math.max(Number(audio.mtime_ms) || 0, Number(timestamps.mtime_ms) || 0);
  return !(reportMtime > 0 && pairMtime > reportMtime + 1000);
}
...
function canonicalAudioScript(canonical = {}) {
  return cleanText(
...
function comparisonTokens(value) {
  return cleanText(value)
    .toLowerCase()
    .replace(/\b(?:twenty\s+twenty\s+six|two\s+thousand\s+and\s+twenty\s+six)\b/g, "2026")
    .replace(/\b(?:twenty\s+twenty\s+seven|two\s+thousand\s+and\s+twenty\s+seven)\b/g, "2027")
    .replace(/\b(?:playstation\s+five|ps\s+five)\b/g, "ps5")
    .replace(/\b(?:g\s*t\s*a\s+five|gta\s+five)\b/g, "gta5")
    .replace(/\b(?:g\s*t\s*a\s+six|gta\s+six)\b/g, "gta6")
    .replace(/\bpaul['’]?s\s+gaming\b/g, "pulse gaming")
    .replace(/[^\p{L}\p{N}]+/gu, " ")
    .split(/\s+/)
    .filter(Boolean);
}
...
function timestampCanonicalMismatchIssue(timestamp = {}, canonical = {}) {
  const alignment = timestamp?.timestamp_whisper_alignment;
  if (!timestamp?.exists || !alignment || alignment.repaired !== true) return null;
  const script = canonicalAudioScript(canonical);
  const currentWordCount = wordCount(script);
  if (!currentWordCount) return null;
  const transcript = cleanText(alignment.transcript);
  const transcriptRatio = transcript ? lcsRatio(comparisonTokens(script), comparisonTokens(transcript)) : 0;
  const alignedExpectedWordCount = Number(alignment.script_expected_word_count);
  if (
    Number.isFinite(alignedExpectedWordCount) &&
    alignedExpectedWordCount !== currentWordCount &&
    transcriptRatio >= 0.9 &&
    transcriptRatio < 1
  ) {
    return "timestamp_current_script_word_count_mismatch";
  }
  const hasRepairMarker =
    cleanText(canonical.public_copy_repaired_at) ||
    cleanText(canonical.duration_variant_repaired_at) ||
    cleanText(canonical.pronunciation_repaired_at);
  if (hasRepairMarker) {
    if (Number.isFinite(alignedExpectedWordCount)) {
      const tolerance = Math.max(2, Math.ceil(currentWordCount * 0.05));
      if (Math.abs(alignedExpectedWordCount - currentWordCount) > tolerance) {
        if (transcriptRatio >= 0.9) return null;
        return "timestamp_transcript_mismatch_after_canonical_repair";
      }
    }
  }
  if (transcriptRatio > 0 && transcriptRatio < 0.9) return "timestamp_current_script_mismatch";
  return null;
}
...
function nextActionsForStatus(status, selectedProvider = null) {
  ...
  if (status === "requires_existing_audio_cadence_repair") {
    return [
      "compact_excessive_narration_pauses_and_realign_with_local_whisper",
      "rerun_goal_production_cutover_after_audio_materialisation",
    ];
  }
...
async function buildJobReport(job = {}, { workspaceRoot, localTts, elevenlabs, providerPreference } = {}) {
  ...
  const voiceQualityPath = artifactDir ? path.join(artifactDir, "voice_quality_report.json") : null;
  const voiceQualityReport = await readJsonIfPresent(voiceQualityPath, {});
  const voiceQualityStat = voiceQualityPath && (await fs.pathExists(voiceQualityPath))
  ...
  const requiresAsrAlignment =
    requiresWordTimestampAsrAlignment(job) ||
    (
      cleanText(timestampResult.selected?.word_timestamp_source) !== "" &&
      timestampResult.selected?.asr_aligned !== true
    );
  ...
  const canonicalMismatchReason = timestampCanonicalMismatchIssue(timestampResult.selected, canonicalManifest || {});
  const cadenceFailed = currentFailedVoiceCadenceQa(
    voiceQualityReport,
    voiceQualityStat?.mtimeMs,
    audioResult.selected,
    timestampResult.selected,
  );
  const timestampNeedsNarrationRegeneration =
    timestampResult.selected?.requires_audio_regeneration === true || Boolean(canonicalMismatchReason);
  const stale = {
    audio:
      forcedStale.audio ||
      cadenceFailed ||
      staleAfterRepairEvent(audioResult.selected, repairEvent),
    timestamps:
      forcedStale.timestamps ||
      cadenceFailed ||
      staleAfterRepairEvent(timestampResult.selected, repairEvent),
  };
  const audio = stale.audio
    ? {
        ...audioResult.selected,
        usable: false,
        reason: cadenceFailed
          ? "failed_voice_cadence_qa"
          : forcedStale.audioReason || repairEvent?.reason || "stale_after_repair",
      }
    : asrFailure
      ? { ...audioResult.selected, usable: false, reason: "asr_alignment_exhausted_regenerate_narration" }
    : timestampNeedsNarrationRegeneration
      ? { ...audioResult.selected, usable: false, reason: canonicalMismatchReason || "asr_inserted_words_regenerate_narration" }
    : audioResult.selected;
  const timestamps = stale.timestamps
    ? {
        ...
        reason: cadenceFailed
          ? "failed_voice_cadence_qa"
          : forcedStale.timestampsReason || repairEvent?.reason || "stale_after_repair",
      }
    : asrFailure
      ? { ...timestampResult.selected, usable: false, reason: "asr_alignment_exhausted_regenerate_narration" }
    : canonicalMismatchReason
      ? {
          ...timestampResult.selected,
          usable: false,
  ...
  const status = statusForJob({
    ...
    requiresAsrCoverageRepair,
    requiresExistingCadenceRepair: cadenceFailed,
  });
  const missing = [];
  if (status === "requires_word_timestamp_asr_alignment") {
  ...
  return {
    ...
    audio,
    timestamps,
    tts_provider: status === "requires_existing_audio_cadence_repair"
...
async function buildGoalAudioTimestampWorkbench({
  ...
  const requiresGeneration = jobs.filter((job) =>
    ["requires_audio_timestamp_generation", "requires_existing_audio_cadence_repair"].includes(job.status),
  );
  const requiresCadenceRepair = jobs.filter((job) => job.status === "requires_existing_audio_cadence_repair");
  ...
  return {
    ...
    summary: {
      ...
      requires_existing_cadence_repair_count: requiresCadenceRepair.length,
