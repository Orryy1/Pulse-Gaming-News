...
function buildDryRunPackageTruth({
  ...
  const stories = evidenceStories(dryRunPlan, incidentGuardReport);
  ...
  return {
    ...
    planned_action_count: numberOrZero(summary.planned_action_count || asArray(dryRunPlan.actions).length),
...
function buildPublishControlTruth({ publishVerdict = {}, dryRunPlan = {} } = {}) {
  ...
  return {
    ...
    planned_action_count: numberOrZero(publishVerdict.planned_action_count || summary.planned_action_count),
...
function renderBaselineAuditMarkdown(report = {}) {
  ...
  const lines = [
    ...
    formatCount("Planned actions", report.publish_control_truth && report.publish_control_truth.planned_action_count),
