...
function buildProofScript(story = {}, { subject, sourceName } = {}) {
  ...
  if (/\balbion\s+online\b/i.test(combined) && /\bkeepers?\b/i.test(combined)) {
    return proofScript([
      "Albion Online is turning the Keepers into a pressure test for its open world.",
      `${source} says the Keepers are rising up in Albion Online.`,
      "For lapsed players, this decides whether they reinstall, return to old zones or wait for a bigger reason.",
      "The argument is whether the faction actually shifts where guilds gather, where solo players travel and which fights feel dangerous again. If it does, Albion feels alive. If it is just louder enemies in familiar places, the update fades quickly.",
    ]);
  }
