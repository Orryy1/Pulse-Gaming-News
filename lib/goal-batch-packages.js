...
function buildProofScript(story = {}, { subject, sourceName } = {}) {
  ...
  let lead = `${subject} needs one cleaner proof point before the hype is worth trusting.`;
  let sourceBeat = `${source} says ${claim}.`;
  let impact = "The source is real, but the useful part for players is still the missing detail: what changes on screen, on the store page or in their next download.";
  let payoff = "That is the debate worth having now: treat it as early movement, or wait for the proof that changes what people actually play.";
