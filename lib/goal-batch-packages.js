...
function subjectFromTitle(title = "") {
  ...
  const known = clean.match(
    /\b(Valorant|Hades\s+II|Helldivers\s+2|Forza\s+Horizon\s+6|PlayStation\s+Plus|PlayStation|Xbox|Nintendo|Steam|Valve|Riot|Warhammer\s+40,?000|Boltgun\s+2|Subnautica\s+2|GTA\s+6|Assassin's\s+Creed(?:\s+Black\s+Flag)?|Black\s+Flag|Fable|Star\s+Fox|Final\s+Fantasy\s+7\s+Rebirth|Kingdom\s+Come|Destiny\s+2|Lego\s+Batman|Paranormal\s+Activity)\b/i,
  );
...
function canonicalSubjectForProof(story = {}) {
  ...
  const candidates = [
    ...
    subjectFromTitle(story.title || story.suggested_title || story.public_title),
    manifestSubject,
  ].map(cleanText);
...
function thumbnailTextForProof(story = {}, subject = "") {
  ...
  if (explicit && explicit.split(/\s+/).length <= 5) {
    if (
...
function buildProofScript(story = {}, { subject, sourceName } = {}) {
  ...
  if (/\b(?:trailer|gameplay|footage|preview|demo)\b/i.test(combined)) {
    lead = `${subject} is making its strangest pitch the part players might remember.`;
    ...
    lead = `${subject} just became a value test, not just a catalogue listing.`;
    impact = "The interesting question is whether this adds real value for players, or just gives a familiar game another place to sit.";
    ...
    impact = "Short delays normally sound boring, but this one says the calendar itself is now a threat.";
