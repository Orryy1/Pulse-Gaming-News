...
function ensureSubjectOpening(script = "", subject = "") {
  ...
  return cleanText(`${cleanSubject} is the part of this gaming story players need to notice. ${cleanScript}`);
}
...
function titleClaimForProof(story = {}, subject = "") {
  const title = cleanText(story.title || story.suggested_title || subject)
    ...
    .replace(/\s+/g, " ")
    .trim();
  if (!title) return `${subject} has a source-backed update`;
...
function publicTitleForProof(story = {}, subject = "") {
  const explicit = cleanText(story.public_title || story.suggested_title || asArray(story.title_options)[0]);
