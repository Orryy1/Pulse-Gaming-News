...
function buildProofScript(story = {}, { subject, sourceName } = {}) {
  ...
  if (subjectLooksGeneric(subject)) {
    return cleanText(
      `This story should stay in review until it has a named game, studio or platform subject. ` +
        `${source} says ${claim}. ` +
        `The source may be real, but Pulse needs the actual subject before this becomes narration. ` +
        `${PRIMARY_PULSE_CTA}.`,
    );
  }
  ...
  let lead = `${subject} should stay in review until it has a sharper player consequence.`;
  let sourceBeat = `${source} says ${claim}.`;
  let impact = "The source is real, but the angle does not yet have a must-watch player consequence.";
  let payoff = "It needs a concrete gameplay, price, date, platform or player-trust change before it becomes a Pulse short.";
