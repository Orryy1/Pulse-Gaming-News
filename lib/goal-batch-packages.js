...
function supplementalConfirmedClaimsForProof(story = {}, subject = "") {
  ...
  if (!/\balbion\s+online\b/i.test(evidence) || !/\bkeepers?\b/i.test(evidence)) return [];
  if (!/news\.xbox\.com\/en-us\/2026\/07\/07\/in-albion-online/i.test(evidence)) return [];
  return [
...
function buildProofScript(story = {}, { subject, sourceName } = {}) {
  ...
  if (/\b(?:the\s+)?elder\s+scrolls\s+online\b/i.test(combined) && /\bseason\s+one\b/i.test(combined)) {
    return proofScript([
      "The Elder Scrolls Online is testing whether Season One feels like a reason to return, not just another update label.",
      `${source} says Season One has a new update live now.`,
      "For lapsed players, the useful question is simple: does this change what you do tonight, or only add more things to read before logging off?",
      "If the update makes the world feel busier, ESO gets a clean re-entry point. If it feels like maintenance, most players will wait for a bigger shake-up.",
    ]);
  }
