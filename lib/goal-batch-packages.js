...
function buildProofScript(story = {}, { subject, sourceName } = {}) {
  ...
  let lead = `${subject} now has one practical question players can answer.`;
  let sourceBeat = `${source} says ${claim}.`;
  let impact = `${source} gives the story a concrete starting point, but the argument is what changes on screen, what changes in the library and what makes someone care today.`;
  let payoff = `If that change is obvious, players decide whether to install, revisit or argue before the next news cycle buries it. If not, they can safely ignore the noise.`;
