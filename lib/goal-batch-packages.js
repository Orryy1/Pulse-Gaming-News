...
function publicDescriptionDetailSentence(sentences = [], lead = "", payoff = "") {
  return asArray(sentences).find((sentence) =>
    ...
    /\b(?:try|judge|decide|test|play|download|buy|wait|skip|return|reinstall|wishlist|combat|boss|builds?|mode|map|campaign|storage|price|demo|date|score)\b/i.test(sentence)
  ) || "";
}
...
function buildProofScript(story = {}, { subject, sourceName } = {}) {
  ...
  let lead = `${subject} needs one concrete player-facing detail before it becomes more than a feed item.`;
  let sourceBeat = `${source} says ${claim}.`;
  let impact = "The useful question is what players, creators or the wider community can actually judge next: footage, release timing, price, platform access or a feature that changes how the game feels.";
  let payoff = "If the next proof gives that answer, this becomes a real debate. If it does not, it stays a watch item.";
...
function rightsLedgerForStory(story = {}, explicit = []) {
  const existing = [
    ...
    ...asArray(parseMaybeJson(story.sfx_rights_ledger, story.sfx_rights_ledger || [])),
    ...asArray(parseMaybeJson(story.sfx_rights_records, story.sfx_rights_records || [])),
    ...asArray(explicit),
  ];
