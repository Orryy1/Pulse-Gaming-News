...
function buildProofScript(story = {}, { subject, sourceName } = {}) {
  ...
  let lead = `${subject} just turned a feed update into a real player decision.`;
  let sourceBeat = `${source} says ${claim}.`;
  let impact = "The practical part is what changes now: timing, access, price, performance or whether players should actually pay attention today.";
  let payoff = "If the source detail changes one of those choices, it is worth a short; if not, it should stay below the line.";
