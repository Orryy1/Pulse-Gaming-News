...
function subjectFromTitle(title = "") {
  ...
  if (/\b(?:gta\s*vi|grand\s+theft\s+auto\s*vi)\b/i.test(clean)) return "Grand Theft Auto VI";
...
function subjectLooksFeedFragment(value = "") {
  return /\b(?:comes?\s+to|plays?\s*$|^meet\s+|^upgraded\s+|^while\s+other\s+games\b|^today'?s\s+top\s+deals\b)\b/i.test(
    cleanText(value),
...
function publicTitleForProof(story = {}, subject = "") {
  ...
  if (/\b(?:gta\s*vi|grand\s+theft\s+auto\s*vi)\b/i.test(text) && /\b(?:cover\s+art|pre[-\s]?order|preorders?|store\s+page)\b/i.test(text)) {
  ...
  if (/\bage\s+of\s+empires\s+mobile\b/i.test(text) && /\bpc\s+edition\b/i.test(text)) {
    return "Age Of Empires Mobile Has A PC Test";
  }
  if (/\binvincible\s+vs\b/i.test(text) && /\b(?:roster|universa|immortal|join)\b/i.test(text)) {
    return "Invincible VS Adds Two Roster Problems";
  }
...
function thumbnailTextForProof(story = {}, subject = "") {
  ...
  if (/\binvincible\s+vs\b/i.test(evidenceText) && /\b(?:roster|universa|immortal|join)\b/i.test(evidenceText)) {
    return "INVINCIBLE ROSTER";
  }
...
function buildProofScript(story = {}, { subject, sourceName } = {}) {
  ...
  if (/\b(?:gta\s*vi|grand\s+theft\s+auto\s*vi)\b/i.test(combined) && /\bps5\b/i.test(combined) && /\b(?:plays?\s+best|november\s+19|version)\b/i.test(combined)) {
    return proofScript([
      ...
      "That turns a release date headline into a platform argument: if the PS5 version owns the marketing, Xbox and PC players will look for what they are missing.",
      "That means the real fight is not just when GTA VI arrives; it is whether PS5 becomes the default version people expect to play.",
    ]);
  }
  ...
  if (/\binvincible\s+vs\b/i.test(combined) && /\b(?:roster|universa|immortal|join)\b/i.test(combined)) {
    return proofScript([
      "Invincible VS just made its roster argument nastier.",
      `${source} says Universa and The Immortal are joining the roster.`,
      "For a fighting game, new names only matter if the matchups change how people imagine the meta.",
      "If those kits look distinct, the reveal fuels wishlists. If not, it is just another character graphic.",
    ]);
  }
  ...
  if (/\b(?:scalper|scalpers|buying restriction|restrictions?|playtime rule|eligibility)\b/i.test(combined)) {
    ...
    lead = `${subject} finally has the reveal fans cannot dodge: real gameplay.`;
    sourceBeat = source
      ? `${source} has shown enough footage to move the debate from promise to proof.`
      : `The new footage moves the debate from promise to proof.`;
    impact = `Now the footage has to answer the details players actually feel: camera distance, hit timing, enemy pressure and whether ${subject} stays readable when the screen gets busy.`;
    payoff = /\bhalo\b/i.test(subject)
      ...
      : `If ${subject} stays clear under pressure, it has a real launch argument; if it blurs into spectacle, the reveal becomes a warning sign.`;
...
function generatedMotionClipsForStory(story = {}) {
  ...
  const readableCardDurationS = 10.5;
  const families = [
    "hook_slam",
    "source_proof",
    "subject_motion",
