...
function scriptLooksUnsafeForGoalProof(script = "") {
  return /\b(?:source-backed update|clean read|this gaming story|this story finally has something specific to judge|something specific to judge|the useful question|the player angle is simple|before you spend|wait-and-see column|named source confirms)\b/i.test(
    cleanText(script),
  );
}
...
function thumbnailTextForProof(story = {}, subject = "") {
  ...
  if (explicit && explicit.split(/\s+/).length <= 5) {
    if (!containsSubject(explicit, subject) && compact && explicit.split(/\s+/).length <= 4) {
