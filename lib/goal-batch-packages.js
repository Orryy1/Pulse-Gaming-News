...
function publicTitleForProof(story = {}, subject = "") {
  ...
  if (/\b(?:the\s+)?elder\s+scrolls\s+online\b/i.test(text) && /\b(?:thieves\s+guild|tamriel\s+tome|eight\s+(?:new\s+)?story\s+quests)\b/i.test(text)) {
    return "ESO's Thieves Guild Return Has A Reward-Track Test";
  }
...
function buildProofScript(story = {}, { subject, sourceName } = {}) {
  ...
  if (/\b(?:the\s+)?elder\s+scrolls\s+online\b/i.test(combined) && /\bseason\s+one\b/i.test(combined)) {
    return proofScript([
      "The Elder Scrolls Online just brought back the Thieves Guild, but its new reward track may decide whether players stay.",
      `${source} says Season One adds eight story quests in Glenumbra, plus Daggerfall heists, daily jobs and a new Mythic reward.`,
      "Every player gets a free Tamriel Tome, while optional paid tracks add premium rewards and can keep progressing after the season ends.",
      "That is the real split. Returning for a proper crime story feels easy; paying to turn that return into a longer rewards grind is harder to sell.",
      "If the free path feels generous, Season One is a smart comeback. If the best rewards sit behind upgrades, nostalgia becomes the sales pitch.",
    ]);
  }
