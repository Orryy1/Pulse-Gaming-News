...
function subjectLooksFeedFragment(value = "") {
  return /\b(?:comes?\s+to|plays?\s*$|^meet\s+|^upgraded\s+|^while\s+other\s+games\b|^today'?s\s+top\s+deals\b|^it'?s\s+wild\s+of\b)\b/i.test(
    cleanText(value),
...
function repairedCanonicalManifestUsable(manifest = {}) {
...
function hydrateStoryWithExistingGoalArtifacts(story = {}, { existingArtifactRoot = "" } = {}) {
  ...
  const hydrated = {
    ...normalised,
    ...(repairedCanonicalManifestUsable(canonicalManifest) ? objectSource(canonicalManifest) : {}),
