...
function subjectLooksPublisherLabel(value = "") {
  return /^(?:ubisoft|microsoft|sony|nintendo|bethesda|zenimax|electronic\s+arts|ea|capcom|sega|konami|square\s+enix|bandai\s+namco|take[- ]two|rockstar(?:\s+games)?|epic\s+games|activision|blizzard)$/i.test(
    cleanText(value),
  );
}
...
function subjectFromSourceUrl(value = "") {
  ...
  if (!clean) return "";
  if (/\bmeet-the-star-operator-1\b|\bstarward\b/.test(clean)) return "Starward";
  if (/free-play-days-07-09-2026/.test(clean)) return "Xbox Free Play Days";
  if (/\bwreck-runners\b/.test(clean)) return "Wreck Runners";
  if (/\belderscrollsonline\.com\b/.test(clean) || /\belder-scrolls-online\b/.test(clean)) {
    return "The Elder Scrolls Online";
  }
  if (/\bzaxoid\b/.test(clean)) return "Zaxoid";
  if (/\bhearthstone\b/.test(clean)) return "Hearthstone";
  if (/\bbuckshot-roulette\b/.test(clean)) return "Buckshot Roulette";
  if (/\bthe-blood-of-dawnwalker\b/.test(clean)) return "The Blood of Dawnwalker";
  if (/\balbion-online\b/.test(clean)) return "Albion Online";
  return "";
}
...
function subjectFromTitle(title = "") {
  ...
  if (!clean) return "";
  const sourceUrlSubject = subjectFromSourceUrl(clean);
  if (sourceUrlSubject) return sourceUrlSubject;
  ...
  if (/\bdigimon\s+story\s+time\s+stranger\b/i.test(clean)) return "Digimon Story Time Stranger";
  if (/\bthe\s+blood\s+of\s+dawnwalker\b/i.test(clean)) return "The Blood of Dawnwalker";
  if (/\balbion\s+online\b/i.test(clean)) return "Albion Online";
  ...
  if (/\bpit\s+of\s+goblin\b/i.test(clean)) return "Pit of Goblin";
  ...
  if (/\bdune\s*:?\s*awakening\b/i.test(clean)) return "Dune: Awakening";
...
function subjectLooksFeedFragment(value = "") {
  return /\b(?:comes?\s+to|plays?\s*$|^meet\s+|^season\s+one(?:'s)?(?:\s+update)?\b|^upgraded\s+|^updated\s+xbox\s+console\s+prices\b|^free\s+play\s+days\s+-\s+|^while\s+other\s+games\b|^today'?s\s+top\s+deals\b|^it'?s\s+wild\s+of\b)\b/i.test(
    cleanText(value),
...
function canonicalSubjectForProof(story = {}) {
  ...
  if (/\bdenshattack\b/i.test(currentEvidence)) {
    return "Denshattack";
  }
  if (
    /\bfree\s+play\s+days\b/i.test(currentEvidence) &&
    /\bthe\s+alters\b/i.test(currentEvidence) &&
    /\bstuffed\b/i.test(currentEvidence)
  ) {
    return "Xbox Free Play Days";
  }
  if (/\balbion\s+online\b/i.test([
    story.primary_source_url,
    story.article_url,
    story.url,
    story.title,
  ...
  const sourceUrlSubject = cleanText(subjectFromSourceUrl([
    story.primary_source_url,
    story.article_url,
    story.url,
    story.linked_url,
  ...
  if (
    sourceUrlSubject &&
    /^(?:Starward|Xbox Free Play Days)$/i.test(sourceUrlSubject) &&
    !containsSubject(explicitSubject, sourceUrlSubject)
  ) {
    return sourceUrlSubject;
  }
  if (
    sourceUrlSubject &&
    new RegExp(`^in\\s+${sourceUrlSubject.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`, "i").test(explicitSubject)
  ) {
    return sourceUrlSubject;
  }
  if (
    evidenceSubject &&
    explicitSubject.toLowerCase().startsWith(`in ${evidenceSubject.toLowerCase()} `)
  ) {
    return evidenceSubject;
  }
  if (
    evidenceSubject &&
    (subjectLooksGenericPlatform(explicitSubject) || subjectLooksPublisherLabel(explicitSubject)) &&
...
function titleMatchesCanonicalSubject(title = "", subject = "") {
  const inferred = cleanText(subjectFromTitle(title));
  const canonical = cleanText(subject);
  if (!inferred || !canonical) return true;
  if (
    subjectLooksGeneric(inferred) ||
    subjectLooksGenericPlatform(inferred) ||
    subjectLooksPublisherLabel(inferred)
  ) {
    return true;
  }
  return containsSubject(inferred, canonical) || containsSubject(canonical, inferred);
}
...
function affiliateManifestFromRevenuePath(pathRow = {}) {
  ...
  const storyId = cleanText(pathRow.story_id);
  const sourceLinks = revenueSourceLinks(pathRow);
  const landingPageRoute = cleanText(manifest.landing_page?.route || pathRow.route);
  const affiliateManifest = {
    story_id: storyId,
    vertical: "gaming",
    commercial_intent_type: cleanText(
      pathRow.commercial_intent_type || manifest.commercial_intent_type,
    ),
    ...
    landing_page_route: landingPageRoute,
  ...
  return materialiseGovernedCommercialEvidence({
    story: {
      id: storyId,
      title: bestRevenueTitle(pathRow),
      article_url: sourceLinks[0]?.url || "",
      source_links: sourceLinks,
    },
    affiliateManifest,
    landingPage: { landing_page_route: landingPageRoute },
  });
}
...
function firstSentence(text = "") {
  const protectedText = protectVersionDots(normaliseVersionNumberSpacing(text));
  return restoreVersionDots((protectedText.match(/[^.!?]+[.!?]?/) || [""])[0]).trim();
}

function normaliseVersionNumberSpacing(text = "") {
  return cleanText(text).replace(/\b([vV]?)(\d+)\.\s+(\d+)\b/g, (_match, prefix, major, minor) => {
    const versionPrefix = prefix ? prefix.toLowerCase() : "";
    return `${versionPrefix}${major}.${minor}`;
  });
}

function protectVersionDots(text = "") {
  return cleanText(text).replace(/\b([vV]?\d+)\.(\d+)\b/g, "$1§DOT§$2");
}

function restoreVersionDots(text = "") {
  return cleanText(text).replace(/§DOT§/g, ".");
}

function scriptSentences(text = "") {
  return protectVersionDots(normaliseVersionNumberSpacing(text))
    .replace(new RegExp(`\\b${PRIMARY_PULSE_CTA.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\.?`, "gi"), "")
    .match(/[^.!?]+[.!?]+/g)
    ?.map((sentence) => restoreVersionDots(sentence))
    .filter(Boolean) || [];
}
...
function governedPublicDescriptionForProof(story = {}, subject = "", sourceName = "") {
  const description = cleanText(story.public_description);
  const normalisedDescription = description.toLowerCase();
  const normalisedSubject = cleanText(subject).toLowerCase();
  const normalisedSource = cleanText(sourceName).toLowerCase();
  if (!description || !normalisedSubject || !normalisedDescription.includes(normalisedSubject)) {
    return "";
  }
  if (normalisedSource && !normalisedDescription.includes(normalisedSource)) return "";
  if (scriptLooksUnsafeForGoalProof(description)) return "";
  if (ADVERTISER_UNFRIENDLY_PUBLIC_RE.test(description)) return "";
  return description;
}

function publicDescriptionForProof({ story = {}, subject = "", sourceName = "", script = "" } = {}) {
  const governedDescription = governedPublicDescriptionForProof(story, subject, sourceName);
  if (governedDescription) return governedDescription;
  const descriptionEvidence = cleanText([
    subject,
    story.title,
    story.primary_source_url,
    story.article_url,
  ...
  if (/\balbion\s+online\b/i.test(descriptionEvidence) && /\bkeeper\s+uprising|keepers?\b/i.test(descriptionEvidence)) {
    return cleanText(
      `Albion Online's Keeper Uprising runs until August 31 with missions, new enemies and hidden lore. ` +
      `Keeper Memories permanently change how players explore, while the event leads into the next major update. ` +
      `Source: ${sourceName || "Xbox Wire"}.`,
    );
  }
...
function supplementalConfirmedClaimsForProof(story = {}, subject = "") {
  ...
  if (/\balbion\s+online\b/i.test(evidence) && /\bkeepers?\b/i.test(evidence) && /news\.xbox\.com\/en-us\/2026\/07\/07\/in-albion-online/i.test(evidence)) {
    return [
      "The Keeper Uprising is a season-long event running from July 4 through August 31.",
      "The event adds missions, new enemies, vanity rewards and hidden Keeper Memories.",
      "Keeper Memories are a permanent addition recorded in the Albion Journal, with hints that help players find more.",
      "The evolving Keeper Uprising leads into Albion Online's next major content update.",
    ];
  }
  if (/\bthe\s+elder\s+scrolls\s+online\b/i.test(evidence) && /elderscrollsonline\.com\/en-us\/news\/post\/70123/i.test(evidence)) {
    return [
      "Season One adds eight new story quests for the Thieves Guild in Glenumbra.",
      ...
      "Season One runs from July 8 through October 21, 2026.",
    ];
  }
  return [];
}
...
function isStrictSubjectPrefix(value, subject) {
  const prefix = normaliseForSubject(value);
  const complete = normaliseForSubject(subject);
  return Boolean(prefix && complete.length > prefix.length && complete.startsWith(`${prefix} `));
}
...
function proofSourceBeat(source = "", claim = "") {
  const cleanSource = cleanText(source);
  const cleanClaim = cleanText(claim).replace(/[.]+$/g, "");
  if (!cleanClaim) return cleanSource ? `${cleanSource} has the confirmed detail.` : "";
  if (cleanSource && new RegExp(`^${escapeRegExp(cleanSource)}\\b`, "i").test(cleanClaim)) {
    return `${cleanClaim}.`;
  }
  return cleanSource ? `${cleanSource} says ${cleanClaim}.` : `${cleanClaim}.`;
}

function safeSpokenScriptForGoalProof(script = "") {
  const pronounced = applyGamingPronunciation(cleanText(script));
  const sentenceCount = pronounced.split(/(?<=[.!?])\s+/).filter(Boolean).length;
  const cadenceSmoothed = sentenceCount >= 8
    ? pronounced.replace(
        /([A-Za-z0-9)'\"]{2,})\.\s+(?!Follow Pulse Gaming\b)(?=[A-Z])/g,
        "$1, ",
      )
    : pronounced;
  return cadenceSmoothed
    .replace(/\s+/g, " ")
    .trim();
}
...
function titleLooksGeneric(value = "") {
  return /\bhas one detail players should notice\b|\bjust got a real update\b|\bsource[-\s]?backed update\b|\bhas\s+a\s+(?:source[-\s]?proof\s+risk|footage\s+readability\s+test)\b|^why\s+.+\s+could\s+split\s+players\b/i.test(
    cleanText(value),
  );
}
...
function titleLooksMalformedGenerated(value = "") {
  ...
  return (
    ...
    /\bhas\s+a\s+(?:source[-\s]?proof\s+risk|footage\s+readability\s+test)\b/i.test(title) ||
...
function planetCrafterLaunchPlatform(story = {}) {
  const sourceIdentity = cleanText([
    sourceNameForProof(story),
    storyUrl(story),
    story.primary_source?.name,
    story.primary_source?.url,
  ].filter(Boolean).join(" "));
  const sourceEvidence = cleanText([
    story.title,
    story.source_title,
    story.article_title,
    ...asArray(story.confirmed_claims),
  ...
  const launchWord = "(?:launch(?:es|ed|ing)?|arriv(?:e|es|ed|ing)?|releas(?:e|es|ed|ing)?)";
  const xboxLaunch = new RegExp(
    `(?:\\b${launchWord}\\b[^.!?]{0,80}\\bxbox\\b|\\bxbox\\b[^.!?]{0,80}\\b${launchWord}\\b)`,
    "i",
  ).test(sourceEvidence);
  const ps5Launch = new RegExp(
    `(?:\\b${launchWord}\\b[^.!?]{0,80}\\b(?:ps5|playstation\\s*5)\\b|\\b(?:ps5|playstation\\s*5)\\b[^.!?]{0,80}\\b${launchWord}\\b)`,
    "i",
  ).test(sourceEvidence);
  if (xboxLaunch && !ps5Launch) return "xbox";
  if (ps5Launch && !xboxLaunch) return "ps5";
  if (!ps5Launch && /\b(?:xbox\s+wire|news\.xbox\.com)\b/i.test(sourceIdentity)) return "xbox";
  if (!xboxLaunch && /\b(?:playstation\s+blog|blog\.playstation\.com)\b/i.test(sourceIdentity)) return "ps5";
  return "console";
}
...
function publicTitleForProof(story = {}, subject = "") {
  ...
  const curatedExplicit = [
    ...
    .map(cleanText)
    .find((title) =>
      titleLooksPublishReady(title) &&
      !subjectLooksFeedFragment(title) &&
      !unsupportedGeneratedGameplayTitle(title, sourceEvidenceText) &&
      titleMatchesCanonicalSubject(title, base)
    ) || "";
  const explicit = cleanText(story.public_title || story.suggested_title || asArray(story.title_options)[0] || story.title);
  if (
    /\bdenshattack\b/i.test(text) &&
    /\b(?:train|carriage)\b/i.test(text) &&
    /\b(?:kickflips?|flip|trick|grind|stunt)\b/i.test(text) &&
    /\b(?:july\s+15|launch|game\s+pass|xbox\s+play\s+anywhere)\b/i.test(text)
  ) {
  ...
  if (/\bstarward\b/i.test(text) && /\b(?:pliszka|wing\s+rider|version\s*3\.?1|v3\.?1|trade[- ]off)\b/i.test(text)) {
    return "Starward's New Fighter Has A Huge Trade-Off";
  }
  if (/\bfree\s+play\s+days\b/i.test(text) && /\bthe\s+alters\b/i.test(text) && /\bstuffed\b/i.test(text)) {
    return "Xbox Free Play Days Has One Clear Winner";
  }
  ...
  if (
    /\b(?:assassin'?s\s+creed\s+)?black\s+flag\s+resynced\b/i.test(text) &&
    /\b(?:microtransactions?|steam\s+reviews?|full,?\s+complete\s+experience|standard\s+edition)\b/i.test(sourceEvidenceText || text)
  ) {
    if (
      curatedExplicit &&
      /(?:[$£€]\s*\d|\bday[- ]one\s+dlc\b|\bdlc\b[^.!?]{0,40}\bcosts?\s+more\b)/i.test(curatedExplicit)
    ) {
      return curatedExplicit;
    }
    return "Why Black Flag Resynced's Microtransactions Are Dividing Players";
  }
  if (/\b(?:assassin'?s\s+creed\s+)?black\s+flag\s+resynced\b/i.test(text) && /\bps5\s+pro\b/i.test(text)) {
  ...
  if (/\bmarvel\s+rivals\b/i.test(text) && /\bjubilee\b/i.test(text) && /\b(?:gameplay|footage|season\s*9)\b/i.test(text)) {
    return "Marvel Rivals Jubilee Gameplay Has One Problem";
  }
  if (
    /\bnte\s*:?\s*neverness\s+to\s+everness\b/i.test(text) &&
    /\b(?:steam|store|live|launch|available|out\s+now|open[-\s]?world|city)\b/i.test(sourceEvidenceText || text)
  ) {
    return "NTE: Neverness to Everness Has A Live City Test";
  }
  if (/\bthe\s+blood\s+of\s+dawnwalker\b/i.test(text) && /\b(?:dead\s+and\s+loving\s+it|vampire|hands[-\s]?on|preview)\b/i.test(text)) {
    return "The Blood Of Dawnwalker Has A Vampire Time Limit";
  }
  if (/\balbion\s+online\b/i.test(text) && /\bkeepers?\b/i.test(text)) {
  ...
  if (/\bwreck\s+runners\b/i.test(text) && /\b(?:insider|playtest|xbox)\b/i.test(text)) {
    return "Wreck Runners Has A Playtest Trust Test";
  }
  if (/\b(?:the\s+)?elder\s+scrolls\s+online\b/i.test(text) && /\b(?:thieves\s+guild|tamriel\s+tome|eight\s+(?:new\s+)?story\s+quests)\b/i.test(text)) {
    return "The Elder Scrolls Online's Thieves Guild Has A Paid Catch";
  }
  if (/\b(?:the\s+)?elder\s+scrolls\s+online\b/i.test(text) && /\b(?:season\s+one|update|live|return)\b/i.test(text)) {
    return "Elder Scrolls Online Has A Return Test";
  }
  if (/\bzaxoid\b/i.test(text)) {
    return "Zaxoid Is Reviving Arcade Pressure";
  }
  ...
  if (/\bdune\s*:?\s*awakening\b/i.test(text) && /\b(?:playstation\s*5|ps5|sept(?:ember)?\s*22|launch|brings)\b/i.test(text)) {
  ...
  if (/\bthe\s+planet\s+crafter\b/i.test(text)) {
    const platform = planetCrafterLaunchPlatform(story);
    if (platform === "xbox") return "The Planet Crafter Brings No-Pressure Survival To Xbox";
    if (platform === "ps5") return "The Planet Crafter Is Testing PS5 Survival Fans";
    return "The Planet Crafter Brings Terraforming To Console";
  }
  ...
  if (/\b(?:hands[- ]on|preview|played|four\s+hours?|combat|romance|intrigue)\b/i.test(sourceEvidenceText || text)) {
    return `${base} Has A Hands-On Combat Test`;
  }
  if (/\b(?:trailer|footage|gameplay|demo|reveal)\b/i.test(sourceEvidenceText || text)) {
    return `${base} Footage Has To Prove Itself`;
  }
  if (/\b(?:update|season|dlc|battle pass|out today|live now)\b/i.test(sourceEvidenceText || text)) {
    return `${base} Has A Return Risk`;
  }
  return `${base} Needs A Clearer Reason To Care`;
}
...
function scriptLooksCrossStoryContaminated(script = "", story = {}, subject = "") {
  ...
  const trustedContext = cleanText([
    subject,
    subjectLooksGeneric(subject) ? story.title : "",
  ...
  const openingSubject = cleanText(subjectFromTitle(firstSentence(cleanScript)));
  if (
    openingSubject &&
    !subjectLooksGeneric(openingSubject) &&
    !subjectLooksGenericPlatform(openingSubject) &&
    !subjectLooksPublisherLabel(openingSubject) &&
    !containsSubject(trustedContext, openingSubject) &&
    !containsSubject(subject, openingSubject) &&
    !containsSubject(openingSubject, subject)
  ) {
    return true;
  }
...
function albionKeeperScriptNeedsRewrite(script = "", story = {}, subject = "") {
  const context = cleanText([
    subject,
    story.title,
    story.primary_source_url,
  ...
  if (!/\balbion\s+online\b/i.test(context) || !/\bkeepers?\b/i.test(context)) return false;
  if (!/news\.xbox\.com\/en-us\/2026\/07\/07\/in-albion-online/i.test(context)) return false;
  return (
    !/\bAugust\s+31\b/i.test(script) ||
    !/\bKeeper\s+Memories\b/i.test(script) ||
    !/\bpermanent\b/i.test(script) ||
    /\bwhere guilds gather|where solo players travel\b/i.test(script)
  );
}
...
function thumbnailTextForProof(story = {}, subject = "") {
  ...
  if (/\bstarward\b/i.test(evidenceText) && /\b(?:pliszka|wing\s+rider|version\s*3\.?1|v3\.?1|trade[- ]off)\b/i.test(evidenceText)) {
    return "POWER OR SPEED?";
  }
  if (/\bfree\s+play\s+days\b/i.test(evidenceText) && /\bthe\s+alters\b/i.test(evidenceText) && /\bstuffed\b/i.test(evidenceText)) {
    return "PLAY THIS FIRST";
  }
  ...
  if (/\b(?:the\s+)?elder\s+scrolls\s+online\b/i.test(evidenceText) && /\b(?:thieves\s+guild|tamriel\s+tome|season\s+one)\b/i.test(evidenceText)) {
    return "ELDER SCROLLS: FREE OR PAID?";
  }
  ...
  if (/\balbion\s+online\b/i.test(evidenceText) && /\bkeepers?\b/i.test(evidenceText)) {
    return "ONE CHANGE STAYS";
  }
  if (
    /\bnte\s*:?\s*neverness\s+to\s+everness\b/i.test(evidenceText) &&
    /\b(?:steam|store|live|launch|available|out\s+now|open[-\s]?world|city)\b/i.test(evidenceText)
  ) {
    return "NTE LIVE CITY TEST";
  }
  ...
  const preferredExplicitBackedByEvidence = (() => {
    ...
    const explicitPrice = explicit.match(/[$£€]\s*\d+(?:[.,]\d{1,2})?/);
    const priceBacked = Boolean(
      explicitPrice &&
      evidenceText.replace(/\s+/g, "").includes(explicitPrice[0].replace(/\s+/g, "")),
    );
    const dayOneDlcBacked =
      /\bday[- ]one\s+dlc\b/i.test(explicit) &&
      /\bday[- ]one\s+dlc\b/i.test(evidenceText);
    return Boolean(
      (tokens.length && tokens.some((token) => evidence.includes(token))) ||
      priceBacked ||
      dayOneDlcBacked,
    );
  })();
...
function buildProofScript(story = {}, { subject, sourceName } = {}) {
  ...
  if (
    /\b(?:assassin'?s\s+creed\s+)?black\s+flag\s+resynced\b/i.test(combined) &&
    /\b(?:3\s*million|three\s+million|copies sold|sold more than)\b/i.test(
      sourceEvidenceText || combined,
    ) &&
    /\b(?:one week|in a week|day one|first day)\b/i.test(
      sourceEvidenceText || combined,
    )
  ) {
    const sourceBound = buildSourceBoundFallbackScript(story, {
      sourceName: source,
      sourceMaterial: sourceEvidenceText || combined,
    });
    if (cleanText(sourceBound?.full_script)) {
      return cleanText(sourceBound.full_script);
    }
  }
  ...
  if (
    /\bassassin'?s\s+creed\s+black\s+flag\s+resynced\b/i.test(combined) &&
    /\b(?:microtransactions?|steam\s+reviews?|full,?\s+complete\s+experience|standard\s+edition)\b/i.test(sourceEvidenceText || combined)
  ) {
    return proofScript([
      "Assassin's Creed Black Flag Resynced has a trust problem before players even leave port.",
      `${source} reports Ubisoft is calling the standard edition the full, complete experience while negative Steam reviews pile up over microtransactions.`,
      "That contradiction matters because this re-release is selling nostalgia, and nostalgia collapses fast when players think the old adventure has been rebuilt around a new checkout.",
      "The standard edition now has to feel whole in play, with the core pirate fantasy standing on its own instead of making every menu look like an upsell.",
      "If players stop arguing about what was held back, Ubisoft can win back the conversation. If not, prettier seas will not save it.",
    ]);
  }
  ...
  if (/\binvincible\s+vs\b/i.test(combined) && /\b(?:roster|universa|immortal|join)\b/i.test(combined)) {
    return proofScript([
      ...
      "The pre-launch decision is simple: wishlist for dangerous team combinations, or wait for proof that the fights stay readable.",
      ...
      "The payoff is simple: every new character has to prove the fights will stay readable for players when the screen gets chaotic.",
    ]);
  }
  if (
    /\bdenshattack\b/i.test(combined) &&
    /\b(?:train|carriage)\b/i.test(combined) &&
    /\b(?:kickflips?|flip|trick|grind|stunt)\b/i.test(combined) &&
    /\b(?:july\s+15|launch|game\s+pass|xbox\s+play\s+anywhere)\b/i.test(combined)
  ) {
    return proofScript([
      "Denshattack asks one ridiculous question: can a train do a kickflip?",
      "The official Xbox listing puts it on Game Pass and Xbox Play Anywhere on July 15, with players flipping, grinding and tricking a customisable train across a colourful Japanese dystopia.",
      "The trailer sells the joke instantly. The controls have to sell the next ten hours.",
      "A clean combo should feel improvised, not like watching a preset stunt reel.",
      "Players can try it without buying first, but day-one access only matters if landing a ridiculous line takes timing and improvisation.",
      "Earned stunts could make Denshattack this summer's strangest one-more-run obsession.",
      "Canned animations would derail the gimmick after one clip.",
    ]);
  }
  ...
  if (/\bsuper\s+mario\s*64\b/i.test(combined) && /\b(?:35mm|film\s+slides?|collectibles?)\b/i.test(combined)) {
    return proofScript([
      ...
      "That means buyers decide whether this is gaming history worth owning, or a novelty built entirely on Nintendo nostalgia.",
    ]);
  }
  ...
  if (/\b(?:xbox\s+console\s+prices?|updated\s+xbox\s+console\s+prices|console\s+price\s+update)\b/i.test(combined)) {
    return proofScript([
      ...
      "A hardware price change can alter a player's next move: buy now, wait for a bundle or look at PC and used hardware instead.",
      "If Xbox cannot make the value argument feel obvious, every price tag becomes another reason to hesitate.",
    ]);
  }
  if (/\bfree\s+play\s+days\b/i.test(combined) && /\bthe\s+alters\b/i.test(combined) && /\bstuffed\b/i.test(combined)) {
    return proofScript([
      "If you only try one Xbox Free Play Days game, start with The Alters.",
      `${source} says MLB The Show 26, The Alters and Stuffed are available through July 12 for eligible Game Pass members.`,
      "MLB is the safe pick if you already love baseball, while Stuffed is the quick co-op option about teddy bears fighting nightmares.",
      "The Alters is the one most likely to surprise you: a survival game where every crew member is a different version of the same man, shaped by choices he never made.",
      "Its temporary discount also halves the listed price, so this weekend is both a free trial and a clean buy-or-skip test.",
    ]);
  }
  ...
  if (/\bstar\s+fox\b/i.test(combined) && /\b(?:switch\s*2|june\s+25|overview|prologue|arcade|fox\s+mccloud)\b/i.test(combined)) {
    return proofScript([
      ...
      "For players, the question is whether to buy a focused arcade loop now or wait for reviews beside games built around endless progression.",
  ...
  if (
    /\bhalo\b[\s\S]{0,120}\b(?:remake|campaign\s+evolved)\b/i.test(combined) &&
    /\b(?:compar(?:e|ed|ison)|side[- ]by[- ]side|replayed?\s+the\s+original)\b/i.test(sourceEvidenceText)
  ) {
    return proofScript([
  ...
  if (/\bthe\s+planet\s+crafter\b/i.test(combined)) {
    const platform = planetCrafterLaunchPlatform(story);
    const platformLabel = platform === "xbox"
      ? "Xbox"
      : platform === "ps5"
        ? "PlayStation 5"
        : "console";
    const launchEvidence = cleanText([
      story.title,
      story.source_title,
      story.article_title,
      ...asArray(story.confirmed_claims),
    ...
    const hasJuly21LaunchDate = /\b(?:july\s+21|21(?:st)?(?:\s+of)?\s+july)\b/i.test(
      launchEvidence,
    );
    const platformBeat = hasJuly21LaunchDate
      ? `${source} lists July 21 as the ${platformLabel} launch date.`
      : `${source} identifies ${platformLabel} as the launch platform for this release.`;
    return proofScript([
      `The Planet Crafter is testing whether its low-pressure survival loop works on ${platformLabel}.`,
      platformBeat,
  ...
  if (/\b(?:pit\s+of\s+goblin|enter\s+the\s+pit)\b/i.test(subject)) {
    return proofScript([
  ...
  if (/\bdune\s*:?\s*awakening\b/i.test(combined) && /\b(?:playstation\s*5|ps5|sept(?:ember)?\s*22|launch|brings)\b/i.test(combined)) {
    return proofScript([
      "Dune: Awakening is about to test whether a survival MMO can survive the move to console.",
      `${source} says the PlayStation 5 version arrives on September 22.`,
      "That matters because Dune's hook is not just sandworms and spice. It is whether base building, faction pressure and long-session survival still feel readable on a controller.",
      "PC players already know the grind; PS5 players need to know if Arrakis feels like a world worth settling into, or another huge online game that looks better than it plays.",
      "The payoff is simple: if the console version feels smooth, Dune gets a second launch wave. If it feels awkward, the desert eats the hype fast.",
    ]);
  }
  ...
  if (/\balbion\s+online\b/i.test(combined) && /\bkeepers?\b/i.test(combined)) {
    return proofScript([
      "Albion Online's Keeper Uprising disappears in August, but its smartest idea is permanent.",
      `${source} says the event runs until August 31, adding missions, enemies, vanity rewards and hidden Keeper Memories.`,
      "Those Memories remain after the season. They put clues in the Albion Journal and secrets across the open world, turning familiar routes into a scavenger hunt.",
      "The event unfolds in phases before leading into Albion's next major update.",
      "For players, the risk is simple: if clues feel like checklist icons, veterans will burn through them and move on. But if discovery stays surprising, seasonal content leaves the world better than it found it.",
      "That means the uprising is temporary, but the reason to explore might not be.",
    ]);
  }
  if (/\bthe\s+blood\s+of\s+dawnwalker\b/i.test(combined)) {
    return proofScript([
      "The Blood of Dawnwalker is selling its vampire fantasy on pressure, not just mood.",
      `${source} says the hands-on preview shows combat and quest pressure around a half-human, half-vampire hero with limited time to save his family.`,
      "That gives players a clean reason to care: quests, bites and alliances need to feel costly.",
      "If the time limit makes choices sharper, Dawnwalker gets a real identity. If it only sounds dramatic in previews, it becomes another dark RPG with great lighting and no urgency.",
    ]);
  }
  if (/\bwreck\s+runners\b/i.test(combined)) {
    return proofScript([
      "Wreck Runners has a useful Game Pass-style question before anyone pretends it is the next big thing.",
      `${source} says Xbox Insiders can join the Wreck Runners playtest.`,
      "That matters because small co-op action games do not win on logos. They win when movement, hit feedback and one-more-run pressure make sense in the first few minutes.",
      "The playtest is the real filter: if players come out talking about flow, this becomes a hidden-watchlist game. If they only remember confusion, the reveal trailer did more work than the game.",
    ]);
  }
  if (/\bstarward\b/i.test(combined) && /\b(?:pliszka|wing\s+rider|version\s*3\.?1|v3\.?1)\b/i.test(combined)) {
    return proofScript([
      "Starward's new fighter gets more dangerous by making herself easier to hit.",
      `${source} says Version 3.1 adds Pliszka, whose Wing Rider Assembly boosts her firepower and lets her enter a high-speed Cruising State.`,
      "The catch is physical: the assembly makes her hitbox larger and pulls her down faster, so every aggressive push also makes her a bigger target.",
      "Players can detach it during battle and trade that firepower for a quicker, more agile style.",
      "That is the real hook. Pliszka is not one fixed character; every fight asks whether power or movement matters more right now.",
    ]);
  }
  if (/\b(?:the\s+)?elder\s+scrolls\s+online\b/i.test(combined) && /\bseason\s+one\b/i.test(combined)) {
    return proofScript([
      "The Elder Scrolls Online just brought back the Thieves Guild, but its reward track has a paid catch.",
      `${source} says Season One adds eight story quests in Glenumbra, Daggerfall heists and a new Mythic, while every player gets a free Tamriel Tome.`,
      "Optional paid tracks add premium rewards and can keep progressing after the season ends.",
      "That choice could decide whether this comeback feels generous or calculated. The crime story gives lapsed players a real reason to return.",
      "If the free path delivers the best parts, this is a smart comeback. If the standout rewards sit behind upgrades, the Thieves Guild is also the sales pitch.",
    ]);
  }
  if (/\bzaxoid\b/i.test(combined)) {
    return proofScript([
      "Zaxoid is trying to make old-school arcade pressure feel sharp again.",
      `${source} says Zaxoid is reviving golden-age arcade energy for modern players.`,
      "That pitch only works if the game is instantly readable: quick decisions, clean feedback and failure that makes you restart instead of quit.",
      "If Zaxoid nails that loop, it becomes a short-session obsession. If it only copies the look, nostalgia will not carry it past the first round.",
    ]);
  }
  let lead = `${subject} has to answer one simple thing: why should players care now?`;
  let sourceBeat = proofSourceBeat(source, claim);
  let impact = `That matters because a named source is only useful when it changes a real choice: install, wishlist, return or wait.`;
  let payoff = `If ${source} gives that choice teeth, it becomes a story. If not, it belongs in the watch pile until stronger proof lands.`;
  if (/\b(?:scalper|scalpers|buying restriction|restrictions?|playtime rule|eligibility)\b/i.test(combined)) {
    ...
    /\b(?:inherited (?:old )?computer|abandoned profiles?)\b/i.test(sourceEvidenceText) &&
    /\b(?:file explorers?|chat logs?|corrupted (?:apps?|applications?))\b/i.test(sourceEvidenceText)
  ) {
    const warningBeat = /\b(?:mental-health|mild gore|flashing imagery|photosensitivity)\b/i.test(
      sourceEvidenceText,
    )
      ? `The developer also flags mature mental-health themes, mild gore and flashing imagery.`
      : "";
    return proofScript([
      `${subject} turns an inherited computer into a mystery players solve with familiar PC tools.`,
      proofSourceBeat(source, claim),
      `The investigation moves through abandoned profiles, file explorers, chat logs and corrupted applications rather than a normal room.`,
      `Some puzzles shift into retro-styled 3D spaces, so the interface can change what counts as a clue.`,
      warningBeat,
      `The trade-off is simple: puzzle fans can enter spoiler-free now, but the familiar desktop is also where the game's mature themes surface.`,
      `That means the old PC is both the mystery and the reason to read the warnings before opening the case.`,
    ]);
  } else if (
    /\b(?:steam|store|live|launch|launches|out\s+now|available\s+now)\b/i.test(combined) &&
    /\b(?:open[-\s]?world|city|urban|online\s+world|shared\s+world|massively\s+multiplayer)\b/i.test(
      combined,
    )
  ) {
    lead = `${subject} has moved from open-world promise to a live store test.`;
    sourceBeat = proofSourceBeat(source, claim).replace(new RegExp(`\\b${escapeRegExp(subject)}\\b`, "gi"), "it");
    impact = `That changes the question from trailer budget to player trust: does this city seem readable, playable and worth installing today?`;
    payoff = `If the city feels alive beyond the edit, ${subject} can turn curiosity into wishlists fast. If the footage only sells gloss, players will scroll past and wait for proof.`;
  } else if (/\b(?:trailer|gameplay|footage|preview|demo)\b/i.test(combined)) {
    lead = `${subject} finally has footage that puts the pitch under pressure.`;
    sourceBeat = source
      ? `${source} is carrying the new footage, so the important read is not the logo; it is whether the play looks clear when the screen gets busy.`
      : `The footage makes the important read practical: whether the play looks clear when the screen gets busy.`;
    impact = `Camera distance, hit timing, enemy pressure and readable effects now matter more than the reveal edit.`;
    payoff = /\bhalo\b/i.test(subject)
      ...
      : `If the action still reads when busy, the game earns real wishlist attention. If the edit hides it, players should wait for uncut gameplay.`;
...
function rightsForRestoredMotionClip(clip = {}) {
  ...
  return {
    ...
    evidence_file: cleanText(
      clip.rights_evidence_file ||
        clip.evidence_file ||
        clip.provenance?.source_report ||
        "visual_v4_motion_pack_manifest",
    ),
    approval_status: cleanText(clip.approval_status) || "approved_for_transformative_editorial_use",
    rights_status: cleanText(clip.rights_status) || "approved_under_transformative_editorial_policy",
    usage_scope: cleanText(clip.usage_scope) || "transformative_editorial_short_form",
    rights_decision_basis:
      cleanText(clip.rights_decision_basis) ||
      "validated_official_direct_media_editorial_policy",
    rights_grant:
      typeof clip.rights_grant === "boolean" ? clip.rights_grant : undefined,
    asset_sha256: cleanText(clip.asset_sha256 || fileEvidence.sha256),
    asset_size_bytes: Number(clip.asset_size_bytes || fileEvidence.size_bytes) || null,
    evidence_sha256: cleanText(clip.rights_evidence_sha256 || clip.evidence_sha256),
    evidence_size_bytes: Number(
      clip.rights_evidence_size_bytes || clip.evidence_size_bytes,
    ) || null,
    source_media_start_s: Number(
      clip.source_media_start_s ?? clip.mediaStartS ?? clip.media_start_s,
    ) || 0,
    source_window_duration_s: Number(
      clip.source_window_duration_s ?? clip.durationS ?? clip.duration_s,
    ) || null,
  };
}

function rightsRecordsShareCurrentIdentity(left = {}, right = {}) {
  const leftPath = cleanText(left.path || left.local_path || left.local_materialized_path);
  const rightPath = cleanText(right.path || right.local_path || right.local_materialized_path);
  if (leftPath && rightPath) {
    return path.resolve(leftPath) === path.resolve(rightPath);
  }
  const leftHash = cleanText(left.asset_sha256 || left.materialized_file_evidence?.sha256);
  const rightHash = cleanText(right.asset_sha256 || right.materialized_file_evidence?.sha256);
  if (leftHash && rightHash) return leftHash === rightHash;
  const leftId = cleanText(left.asset_id || left.id);
  const rightId = cleanText(right.asset_id || right.id);
  if (leftId && rightId && leftId !== rightId) return false;
  const leftSource = cleanText(left.source_url || left.url);
  const rightSource = cleanText(right.source_url || right.url);
  return Boolean(leftSource && rightSource && leftSource === rightSource);
}

function reconcileRestoredMotionRights(clips = [], existingRecords = []) {
  const reconciled = [];
  const consumed = new Set();
  const generated = asArray(clips).map(rightsForRestoredMotionClip).filter(Boolean);
  for (const current of generated) {
    const donorIndex = asArray(existingRecords).findIndex((record) =>
      rightsRecordsShareCurrentIdentity(current, record),
    );
    const donor = donorIndex >= 0 ? existingRecords[donorIndex] : null;
    if (donorIndex >= 0) consumed.add(donorIndex);
    reconciled.push(donor
      ? {
          ...current,
          ...donor,
          asset_id: current.asset_id,
          asset_type: donor.asset_type || current.asset_type,
          kind: donor.kind || current.kind,
          path: current.path,
          source_url: current.source_url,
          source_owner: current.source_owner || donor.source_owner,
          source_type: current.source_type,
          source_family: current.source_family,
          base_source_family: current.base_source_family,
          asset_sha256: current.asset_sha256 || donor.asset_sha256,
          asset_size_bytes: current.asset_size_bytes || donor.asset_size_bytes,
          evidence_file: donor.evidence_file || current.evidence_file,
          evidence_sha256: donor.evidence_sha256 || current.evidence_sha256,
          evidence_size_bytes: donor.evidence_size_bytes || current.evidence_size_bytes,
          source_media_start_s: current.source_media_start_s,
          source_window_duration_s: current.source_window_duration_s,
        }
      : current);
  }
  const currentIds = new Set(generated.map((record) => cleanText(record.asset_id)).filter(Boolean));
  const currentPaths = new Set(generated.map((record) => cleanText(record.path)).filter(Boolean));
  const currentSources = new Set(generated.map((record) => cleanText(record.source_url)).filter(Boolean));
  const unrelated = asArray(existingRecords).filter((record, index) => {
    if (consumed.has(index)) return false;
    if (currentIds.has(cleanText(record.asset_id))) return false;
    if (currentPaths.has(cleanText(record.path || record.local_path))) return false;
    if (currentSources.has(cleanText(record.source_url || record.url))) return false;
    return true;
  });
  return {
    current: reconciled,
...
function isSfxRightsRecord(record = {}) {
  return [record.asset_type, record.kind, record.type, record.media_type].some((value) =>
    /^(?:sfx|sound[_ -]?effects?)$/i.test(cleanText(value)),
  );
}

function isMotionRightsRecord(record = {}) {
  return [record.asset_type, record.kind, record.type, record.media_type].some((value) =>
    /^(?:motion(?:[_ -]?clip)?|video|footage|render|final[_ -]?render|platform[_ -]?(?:video[_ -]?)?variant)$/i.test(
      cleanText(value),
    ),
  );
}
...
function normaliseMotionClipRecord(clip = {}) {
...
function dedupeMotionClipRecords(records = []) {
  const out = [];
  const seen = new Set();
  for (const record of asArray(records).map(normaliseMotionClipRecord).filter(Boolean)) {
    const key = motionClipIdentity(record);
    if (!key || seen.has(key)) continue;
    seen.add(key);
    out.push(record);
  }
...
function normaliseStory(story = {}) {
  ...
  return {
    ...story,
    video_clips: dedupeMotionClipRecords(parseMaybeJson(story.video_clips, story.video_clips || [])),
    visual_v4_local_motion_clips: dedupeMotionClipRecords(parseMaybeJson(
...
function looksLikeGovernedOwnedMotionClip(clip = {}) {
  return isStrictlyVerifiedOwnedMotionClip(clip);
}

function looksLikeRenderableMotionClip(clip = {}) {
  return looksLikeRealMotionClip(clip) || looksLikeGovernedOwnedMotionClip(clip);
}
...
function sha256ValueLooksValid(value = "") {
  return /^[a-f0-9]{64}$/i.test(cleanText(value));
}

function fileEvidenceMatches({ filePath = "", sha256 = "", sizeBytes = 0 } = {}) {
  const resolvedPath = firstExistingPath([filePath]);
  if (!resolvedPath || !sha256ValueLooksValid(sha256)) return false;
  const actual = fingerprintFileSyncIfPresent(resolvedPath);
  if (!actual || actual.sha256 !== cleanText(sha256).toLowerCase()) return false;
  const declaredSize = Number(sizeBytes);
  return Number.isFinite(declaredSize) && declaredSize > 0 && actual.size_bytes === declaredSize;
}

function stillSourceUrlHasImageExtension(value = "") {
...
function governedHashBoundStillMotionClip(clip = {}) {
  const sourceUrl = cleanText(clip.canonical_source_url || clip.source_url || clip.url);
  const localEvidence = objectSource(clip.materialized_file_evidence);
  const rightsEvidenceFile = cleanText(clip.rights_evidence_file || clip.evidence_file);
  const rightsEvidenceSha256 = cleanText(clip.rights_evidence_sha256 || clip.evidence_sha256);
  const rightsEvidenceSizeBytes = Number(
    clip.rights_evidence_size_bytes || clip.evidence_size_bytes,
  );
  const sourceFingerprint = cleanText(
    clip.sampled_visual_fingerprint || clip.visual_content_fingerprint?.signature,
  );
  const sourceIdentityBound =
    sha256ValueLooksValid(clip.source_master_sha256) &&
    Boolean(sourceFingerprint) &&
    (
      clip.hash_bound_still_source_identity_verified === true ||
      stillSourceUrlHasImageExtension(sourceUrl)
    );
  return Boolean(
    cleanText(clip.media_kind || clip.mediaKind).toLowerCase() === "visual_still" &&
      looksLikeRealMotionClip(clip) &&
      clip.materialized === true &&
      clip.counts_towards_motion_readiness === true &&
      clip.validated !== false &&
      clip.commercial_use_allowed === true &&
      cleanText(clip.allowed_use) === "transformative_editorial_short_form" &&
      cleanText(clip.licence_basis || clip.rights_basis) &&
      asArray(clip.allowed_platforms).map(cleanText).filter(Boolean).length > 0 &&
      /^https?:\/\//i.test(sourceUrl) &&
      sourceIdentityBound &&
      !asArray(clip.source_identity_conflicts).length &&
      !asArray(clip.validation_provenance_conflicts).length &&
      fileEvidenceMatches({
        filePath: clip.local_materialized_path || clip.path,
        sha256: localEvidence.sha256,
        sizeBytes: localEvidence.size_bytes,
      }) &&
      fileEvidenceMatches({
        filePath: rightsEvidenceFile,
        sha256: rightsEvidenceSha256,
        sizeBytes: rightsEvidenceSizeBytes,
      })
  );
}

function strictProfessionalStillMotionManifestReady(manifest = {}, manifestClips = []) {
  if (manifest.source_safety_blocked === true) return false;
  const clips = dedupeMotionClipRecords(manifestClips);
  if (clips.length < 5 || !clips.every(governedHashBoundStillMotionClip)) return false;
  const families = new Set(clips.map(motionFamilyForManifestClip).filter(Boolean));
  if (families.size < 4) return false;

  const report = objectSource(manifest.professional_source_diversity);
  const requiredSources = Number(report.required_genuine_base_source_count);
  const observedSources = Number(report.observed_genuine_base_source_count);
  if (
    report.authoritative !== true ||
    cleanText(report.status).toLowerCase() !== "pass" ||
    report.strict_pass !== true ||
    !Number.isFinite(requiredSources) ||
    requiredSources < 5 ||
    !Number.isFinite(observedSources) ||
    observedSources < requiredSources ||
    asArray(report.unresolved_clips).length ||
    asArray(report.concentrated_sources).length ||
    asArray(report.rejected_independence_claims).length ||
    asArray(report.blockers).length ||
    cleanText(report.checks?.genuine_base_source_floor?.status).toLowerCase() !== "pass" ||
    cleanText(report.checks?.identity_resolution?.status).toLowerCase() !== "pass" ||
    cleanText(report.checks?.source_concentration?.status).toLowerCase() !== "pass"
  ) {
    return false;
  }

  const sourceRows = asArray(report.per_source_scene_shares);
  const sourceIds = new Set(sourceRows.map((row) => cleanText(row.base_source_asset_id)).filter(Boolean));
  const coveredClipIds = new Set(sourceRows.flatMap((row) => asArray(row.clip_ids)).map(cleanText).filter(Boolean));
  const clipIds = clips.map((clip) => cleanText(clip.id || clip.clip_id)).filter(Boolean);
  if (
    sourceIds.size < requiredSources ||
    clipIds.length !== clips.length ||
    clipIds.some((clipId) => !coveredClipIds.has(clipId)) ||
    sourceRows.some((row) => Number(row.scene_count) > 2 || Number(row.scene_share) > 0.25)
  ) {
    return false;
  }

  const identityRows = asArray(report.identity_evidence);
  const identitySourceIds = new Set(
    identityRows.map((row) => cleanText(row.base_source_asset_id)).filter(Boolean),
  );
  if (identitySourceIds.size < requiredSources) return false;

  const rightsScope = objectSource(manifest.rights_scope_reconciliation);
  if (
    Object.keys(rightsScope).length &&
    (
      rightsScope.allowed !== true ||
      asArray(rightsScope.unresolved_failure_codes).length ||
      asArray(rightsScope.missing_excluded_asset_ids).length ||
      asArray(rightsScope.independent_ledger_blockers).length ||
      asArray(rightsScope.independent_restrictive_statuses).length
    )
  ) {
    return false;
  }
...
function materialisedMotionManifestReady(manifest = {}) {
  ...
  const manifestClips = materialisedMotionRowsFromManifest(manifest);
  const clips = manifestClips.filter(looksLikeRealMotionClip);
  ...
  const directMotionReady = clipCount >= 5 && familyCount >= 4 && directClipCount >= 5 && directFamilyCount >= 4;
  if (directMotionReady) return true;
  if (strictProfessionalStillMotionManifestReady(manifest, manifestClips)) return true;

  const ownedClips = manifestClips.filter(looksLikeGovernedOwnedMotionClip);
  const ownedFamilies = new Set(ownedClips.map(motionFamilyForManifestClip).filter(Boolean));
  const generatorProjects = new Set(ownedClips.map((clip) => cleanText(clip.generator_project_id)).filter(Boolean));
  const generatorMasters = new Set(ownedClips.map((clip) => cleanText(clip.generator_master_sha256)).filter(Boolean));
  return Boolean(
    manifest.owned_explainer_visual_plan === true &&
      manifest.source_safety_blocked !== true &&
      ownedClips.length >= 5 &&
      ownedClips.length === manifestClips.length &&
      ownedFamilies.size >= 4 &&
      generatorProjects.size >= 3 &&
      generatorMasters.size >= 3 &&
      Number(manifest.materially_distinct_generator_project_count || generatorProjects.size) >= 3 &&
      Number(manifest.distinct_generator_master_count || generatorMasters.size) >= 3,
  );
}

function materialisedMotionRowsForHydration(manifest = {}) {
  return materialisedMotionRowsFromManifest(manifest).filter(looksLikeRenderableMotionClip);
}
...
const RESTORED_MOTION_LINEAGE_FIELDS = Object.freeze([
  ...
  "sampled_visual_fingerprint_basis",
  "base_source_asset_id",
  "base_source_identity_basis",
  "motion_source_identity",
  "source_identity_provenance",
  "source_identity_conflicts",
  "validation_provenance",
  "transformation_provenance",
  "validation_provenance_conflicts",
]);

const RESTORED_MOTION_RIGHTS_FIELDS = Object.freeze([
  "licence_basis",
  "license_basis",
  "rights_basis",
  "allowed_use",
...
function explicitFieldPresent(record = {}, field = "") {
...
function mergeRestoredMotionLineage(restoredClips = [], story = {}) {
  const sourceRows = [
  ...
  return asArray(restoredClips).map((restored) => {
    const id = cleanText(restored.id || restored.clip_id || restored.asset_id);
    const source = byId.get(id);
    if (!source) return restored;
    const merged = { ...restored };
    for (const field of RESTORED_MOTION_LINEAGE_FIELDS) {
      if (!evidenceValuePresent(merged[field]) && evidenceValuePresent(source[field])) {
        merged[field] = structuredClone(source[field]);
      }
    }
    for (const field of RESTORED_MOTION_RIGHTS_FIELDS) {
      if (!explicitFieldPresent(merged, field) && explicitFieldPresent(source, field)) {
        merged[field] = structuredClone(source[field]);
      }
    }
    for (const field of ["commercial_use_allowed", "validated"]) {
      if (restored[field] === false || source[field] === false) merged[field] = false;
      else if (typeof restored[field] !== "boolean" && typeof source[field] === "boolean") {
        merged[field] = source[field];
      }
    }
    for (const field of ["provenance", "validation_provenance", "transformation_provenance"]) {
      const sourceValue = source[field];
      const restoredValue = restored[field];
      const sourceObject =
        sourceValue && typeof sourceValue === "object" && !Array.isArray(sourceValue);
      const restoredObject =
        restoredValue && typeof restoredValue === "object" && !Array.isArray(restoredValue);
      if (sourceObject || restoredObject) {
        merged[field] = {
          ...(sourceObject ? sourceValue : {}),
          ...(restoredObject ? restoredValue : {}),
        };
      }
    }
    return merged;
...
function hydrateStoryWithExistingGoalArtifacts(story = {}, { existingArtifactRoot = "" } = {}) {
  ...
  if (captionManifest && typeof captionManifest === "object") {
    hydrated.caption_manifest = captionManifest;
  }
  ...
  const existingArtifactRights = dedupeRightsRecords(
    recordsFromMaybeJson(rightsLedger, rightsLedger || []),
  );
  const existingGeneralRights = existingArtifactRights.filter(
    (record) => !isSfxRightsRecord(record),
  );
  const existingSfxRights = dedupeRightsRecords([
    ...existingArtifactRights.filter(isSfxRightsRecord),
  ...
  if (existingGeneralRights.length) {
    const storyRights = recordsFromMaybeJson(
      hydrated.rights_ledger,
      hydrated.rights_ledger || [],
    );
    hydrated.rights_ledger = dedupeRightsRecords([
      ...existingGeneralRights,
      ...storyRights,
    ]);
  }
  ...
  if (materialisedMotionManifestReady(materialisedMotion.manifest)) {
    const motionEvidence = fingerprintFileSyncIfPresent(materialisedMotion.path);
    const restoredClips = mergeRestoredMotionLineage(
      materialisedMotionRowsForHydration(materialisedMotion.manifest),
      normalised,
    )
      .map((clip) => ({
        ...clip,
        rights_evidence_file: clip.rights_evidence_file || materialisedMotion.path,
        rights_evidence_sha256:
          clip.rights_evidence_sha256 || motionEvidence?.sha256,
        rights_evidence_size_bytes:
          clip.rights_evidence_size_bytes || motionEvidence?.size_bytes,
      }));
    ...
    if (Object.keys(objectSource(materialisedMotion.manifest.professional_source_diversity)).length) {
      hydrated.professional_source_diversity =
        materialisedMotion.manifest.professional_source_diversity;
    }
...
function hydrateStoryWithMotionPack(story = {}, motionPack = {}, options = {}) {
  ...
  ].filter(looksLikeRenderableMotionClip);
...
function prepareStoryForGoalProof(story = {}, options = {}) {
  ...
  const existingMotion = dedupeMotionClipRecords([
    ...asArray(normalised.video_clips),
    ...asArray(normalised.visual_v4_local_motion_clips),
    ...asArray(normalised.motion_clips),
  ]).filter(looksLikeRenderableMotionClip);
  ...
  const shouldRewriteScript =
    ...
    (!initialScript ||
      ...
      albionKeeperScriptNeedsRewrite(initialScript, normalised, canonicalSubject) ||
      ADVERTISER_UNFRIENDLY_PUBLIC_RE.test(initialScript));
  const script = shouldRewriteScript
    ? normaliseVersionNumberSpacing(buildProofScript(normalised, { subject: canonicalSubject, sourceName }))
    : normaliseVersionNumberSpacing(initialScript);
  ...
  const publicTitle = publicTitleForProof(
    { ...normalised, full_script: script, narration_script: script },
    canonicalSubject,
  );
  ...
  const confirmedClaims = uniqueValues([
    ...
    ...supplementalConfirmedClaimsForProof(normalised, canonicalSubject),
    safeConfirmedClaim,
  ].filter(Boolean));
  ...
  const existingTtsScript = cleanText(
    normalised.tts_script || normalised.spoken_narration_script,
  );
  const spokenScript =
    !shouldRewriteScript &&
    existingTtsScript &&
    !pronunciationScriptDiverges(script, existingTtsScript)
      ? existingTtsScript
      : safeSpokenScriptForGoalProof(script);
  ...
  const prepared = {
    ...
    canonical_game: (() => {
      ...
      if (/^Denshattack!?$/i.test(existingGame) && /^Denshattack$/i.test(canonicalSubject)) {
        return canonicalSubject;
      }
      if (/^Xbox Free Play Days$/i.test(canonicalSubject)) {
        return canonicalSubject;
      }
      if (
        existingGame &&
        !/^in\s+/i.test(existingGame) &&
        !subjectLooksGeneric(existingGame) &&
        !subjectLooksFeedFragment(existingGame) &&
        containsSubject(canonicalSubject, existingGame) &&
        !isStrictSubjectPrefix(existingGame, canonicalSubject)
      ) {
        return existingGame;
      }
      return canonicalSubject;
  ...
  const existingRights = asArray(
    parseMaybeJson(normalised.rights_ledger, normalised.rights_ledger || []),
  );
  const restoredMotionRights = reconcileRestoredMotionRights(existingMotion, existingRights);
  const unrelatedCurrentScopeRights = existingMotion.length
    ? restoredMotionRights.unrelated.filter((record) => !isMotionRightsRecord(record))
    : restoredMotionRights.unrelated;
  prepared.rights_ledger = dedupeRightsRecords([
    ...unrelatedCurrentScopeRights,
    ...generatedRights,
    ...restoredMotionRights.current,
  ]);
  ...
  prepared.affiliate_link_manifest = materialiseGovernedCommercialEvidence({
    story: prepared,
    canonical: {
      canonical_subject: canonicalSubject,
      selected_title: publicTitle,
    },
    affiliateManifest: prepared.affiliate_link_manifest || {},
  });
...
function rightsLedgerForStory(story = {}, explicit = []) {
  ...
  return dedupeRightsRecords(existing);
}
...
function buildGoalBatchPackages({
  ...
  const packages = diversified.map((story) => buildGoalProofPackage({
    ...
    ...(Array.isArray(targetPlatforms) && targetPlatforms.length
...
async function writeGoalBatchPackages(batch = {}, { outputDir, contractOutDir } = {}) {
  ...
  for (const pack of asArray(batch.packages)) {
    ...
    const materialisedAcceptance = await fs.readJson(
      path.join(storyOutDir, "goal_package_summary.json"),
    );
    materialisedStoryPackages.push({
      ...materialisedAcceptance,
  ...
  return { outputDir: outDir, storyPackagesPath, batchReportPath, summary };
}
