...
function publicTitleForProof(story = {}, subject = "") {
  ...
  if (/\b(?:switch\s*2|nintendo\s+switch\s*2)\b/i.test(text) && /\b(?:screen|ghosting|lcd|oled)\b/i.test(text)) {
...
function buildProofScript(story = {}, { subject, sourceName } = {}) {
  ...
  if (/\b(?:switch\s*2|nintendo\s+switch\s*2)\b/i.test(combined) && /\b(?:screen|ghosting|lcd|oled)\b/i.test(combined)) {
    return proofScript([
  ...
  if (/\b(?:marvel\s+t[ōo]kon|fighting\s+souls)\b/i.test(combined) && /\b(?:blade|loki|deadpool)\b/i.test(combined)) {
    return proofScript([
      "MARVEL Tokon just turned its roster reveal into a pressure test.",
      `${source} says Blade, Loki and Deadpool are joining Fighting Souls.`,
      "That matters because tag fighters live on team chemistry, not famous names. Players need matchups, assists and screen control that make each character feel dangerous for a different reason.",
      "If these three change how teams are built, Tokon gets a real meta argument. If they only look good in a trailer, the roster reveal fades fast.",
    ]);
  }
  if (/\bassassin'?s\s+creed\s+black\s+flag\s+resynced\b/i.test(combined) && /\bps5\s+pro\b/i.test(combined)) {
    return proofScript([
      "Black Flag Resynced has to prove the PS5 Pro label means more than sharper water.",
      `${source} says the new version has PlayStation 5 Pro enhancements.`,
      "For players, the useful details are frame rate, image clarity, loading and whether sailing still feels smooth when the screen gets busy.",
      "If those upgrades make the old pirate loop feel cleaner, Resynced has a real return pitch. If not, nostalgia is doing too much of the work.",
    ]);
  }
  if (/\bmonopoly\b/i.test(combined) && /\bstar\s+wars\b/i.test(combined) && /\b(?:ability|abilities|heroes|villains)\b/i.test(combined)) {
    return proofScript([
      "Monopoly Star Wars is trying to make a familiar board game feel less automatic.",
      `${source} says Heroes versus Villains gives characters their own abilities.`,
      "That matters because Monopoly usually becomes a patience test once the board state is obvious. Character powers can make deals, pressure and revenge feel more personal.",
      "If the abilities change how families actually play, this becomes more than a themed box. If they barely matter, the Force is only decoration.",
    ]);
  }
