...
const OFFICIAL_RE = /\b(official|publisher|studio|platform|playstation blog|xbox wire|nintendo|steam|epic games|ea|ubisoft|capcom|sega|square enix|bethesda)\b/i;
...
function sourceKey(source = {}) {
  ...
  return host || lower(sourceName(source));
}
...
function sourceReliability(source = {}) {
  ...
  if (OFFICIAL_RE.test(joined)) return "official";
  if (RELIABLE_PUBLICATION_RE.test(joined)) return "reliable_publication";
  if (DISCUSSION_SOURCE_RE.test(joined)) return "discussion";
  return "unknown";
}
...
function buildFollowUpV4Plan(story = {}) {
  return {
    ...
    source_lock: sourceRecordForOutput(story.primary_source || story.official_source),
