...
function hasRepairableCoherenceBlocker(row = {}) {
  return asArray(row.blockers).some((blocker) =>
    /missing_artefact:coherence_report\.json|coherence_report_missing_public_copy_manifest|stale_public_output_coherence_report|coherence_report_not_pass/.test(clean(blocker)),
  );
}
...
function repairTargetsFromDryRunPlan(plan = {}) {
  const rows = [
    ...
    ...asArray(plan.public_output_coherence_report?.stories),
  ];
  const targets = [];
  const seen = new Set();
  for (const row of rows) {
    if (!hasRepairableCoherenceBlocker(row)) continue;
    const target = {
      story_id: clean(row.story_id),
      artifact_dir: clean(row.artifact_dir),
      blockers: asArray(row.blockers).map(clean).filter(Boolean),
    };
    if (!target.story_id || !target.artifact_dir) continue;
