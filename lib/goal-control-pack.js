...
function parseInsufficientCandidateBlocker(blockers = []) {
  for (const blocker of asArray(blockers)) {
    const match = clean(blocker).match(/^insufficient_restart_candidates:(\d+)\/(\d+)$/i);
    if (match) {
      return {
        selected_story_count: Number(match[1]),
        required_story_count: Number(match[2]),
      };
    }
  }
  return null;
...
function summarizeControlledRestartGate(controlledRestartPack = null) {
  if (!controlledRestartPack || typeof controlledRestartPack !== "object") return null;
  const verdict = clean(controlledRestartPack.verdict);
  const blockers = unique(controlledRestartPack.blockers);
  const blocked = verdict.toLowerCase() === "red" || blockers.length > 0;
  const insufficient = parseInsufficientCandidateBlocker(blockers);
  const selectedStoryCount = firstFiniteNumber(
    insufficient?.selected_story_count,
    controlledRestartPack.selected_restart_candidate_count,
    asArray(controlledRestartPack.selected_restart_candidates).length,
  );
  return {
    verdict,
    blocked,
    blockers: blocked && blockers.length ? blockers : blocked ? ["controlled_restart_pack_red"] : [],
    selected_story_count: selectedStoryCount,
    required_story_count: firstFiniteNumber(insufficient?.required_story_count),
    operator_can_manually_approve_now:
      controlledRestartPack.operator_can_manually_approve_now === true,
  };
}
...
function approvalCommandsFromControlledRestartPack(controlledRestartPack = {}) {
  return asArray(controlledRestartPack?.selected_restart_candidates)
    .map((candidate) => {
      const commands = candidate.operator_commands || {};
      return {
        story_id: clean(candidate.story_id),
        title: clean(candidate.title),
        enabled_platforms: unique(candidate.dispatch_enabled_platforms || candidate.enabled_platforms),
        dry_run_command: clean(commands.dry_run),
        apply_command: clean(commands.apply_after_review),
      };
    })
    .filter((row) => row.story_id && row.apply_command);
}

function approvalPendingSummary({
  controlledRestartPack = null,
  executorPlan = {},
  approvalGateReport = {},
} = {}) {
  const commands = approvalCommandsFromControlledRestartPack(controlledRestartPack);
  const pendingReviewPacketCount = Number(approvalGateReport.summary?.pending_review_packet_count || 0);
  const approvalPending =
    executorPlan.ready_for_live_executor_handoff !== true &&
    controlledRestartPack?.operator_can_manually_approve_now === true &&
    commands.length > 0 &&
    (pendingReviewPacketCount > 0 ||
      clean(executorPlan.required_next_step) === "record_operator_approved_actions_before_guarded_dispatch");
  return {
    pending: approvalPending,
    story_count: approvalPending ? commands.length : 0,
    enabled_action_count: approvalPending
      ? commands.reduce((total, command) => total + command.enabled_platforms.length, 0)
      : 0,
    commands: approvalPending ? commands : [],
  };
}
...
function buildCurrentReadinessReport({
  ...
  const restartGate = summarizeControlledRestartGate(controlledRestartPack);
  const restartBlocked = restartGate?.blocked === true;
  const approvalPending = approvalPendingSummary({
    controlledRestartPack,
    executorPlan,
    approvalGateReport,
  });
  const blockers = unique([
    ...(restartBlocked ? restartGate.blockers : []),
    ...asArray(publishReadinessReport.blockers),
  ]);
  const advisory = unique([
    ...asArray(publishReadinessReport.advisory),
    ...(restartBlocked
      ? [`controlled_restart_pack:${restartGate.blockers.join(",")}`]
      : []),
  ]);
  return {
    schema_version: 1,
    ...
    overall_verdict: restartBlocked
      ? "red"
      : clean(publishReadinessReport.overall_verdict || dryRunPlan.overall_verdict || "unknown"),
    readiness_scope: restartBlocked
      ? {
          name: "controlled_restart_blocked",
          guard_ready: false,
          upstream_readiness_scope: publishReadinessReport.readiness_scope || null,
        }
      : publishReadinessReport.readiness_scope || null,
    blocker_count: blockers.length,
    blockers,
    advisory,
    ready_story_count: restartBlocked ? 0 : currentReadyStoryCount(dryRunPlan, guardedDispatchPlan),
    ...
    guarded_dispatch_ready_action_count: restartBlocked
      ? 0
      : guardedDispatchReadyActionCount(guardedDispatchPlan, dryRunPlan),
    executor_handoff_ready_action_count: restartBlocked
      ? 0
      : Number(executorPlan.handoff_ready_action_count || 0),
    ...
    next_action: restartBlocked
      ? `Refresh fresh GREEN candidate supply before guarded dispatch: ${restartGate.blockers.join(", ")}.`
      : approvalPending.pending
        ? "Record the pending operator approval decisions, then rerun approval gate and guarded dispatch executor preflight."
        : publishReadinessReport.next_action || guardedDispatchPlan.required_next_step || null,
    controlled_restart: restartGate,
...
function buildOperatorApprovalPack({
  ...
  const restartGate = summarizeControlledRestartGate(controlledRestartPack);
  const restartBlocked = restartGate?.blocked === true;
  const suppressedHandoffReadyActions = summarizeReadyActions(executorPlan.handoff_ready_actions);
  const handoffReadyActions = restartBlocked ? [] : suppressedHandoffReadyActions;
  const approvalPending = approvalPendingSummary({
    controlledRestartPack,
    executorPlan,
    approvalGateReport,
  ...
  return {
    ...
    current_state: {
      overall_verdict: restartBlocked ? "red" : clean(publishReadinessReport.overall_verdict || "unknown"),
      readiness_scope: restartBlocked
        ? {
            name: "controlled_restart_blocked",
            guard_ready: false,
      ...
      controlled_restart_verdict: restartGate?.verdict || null,
      controlled_restart_blockers: restartGate?.blockers || [],
      controlled_restart_selected_story_count: restartGate?.selected_story_count || 0,
      ...
      executor_handoff_ready: !restartBlocked && executorPlan.ready_for_live_executor_handoff === true,
    },
    controlled_batch: {
      ...
      suppressed_ready_action_count: restartBlocked ? suppressedHandoffReadyActions.length : 0,
    },
    approval_pending: approvalPending,
    exact_approval_commands: approvalPending.commands.map((command) => command.apply_command),
    exact_next_command: command,
    exact_ui_step:
      restartBlocked
        ? "No UI dispatch step is currently available. Refresh fresh GREEN candidate supply and rerun the controlled restart pack first."
        : approvalPending.pending
          ? "No UI dispatch step is required yet. Record the approval decisions with exact_approval_commands, rerun approval gate, guarded dispatch preflight and executor preflight, then use the guarded live command if it appears."
...
function renderOperatorApprovalPackMarkdown(pack = {}) {
  ...
  if (asArray(pack.approval_pending?.commands).length) {
    lines.push("", "## Approval Commands", "");
    for (const command of asArray(pack.approval_pending.commands)) {
      lines.push(
        `### ${command.story_id} - ${command.title || "untitled"}`,
        "",
        "Dry-run decision:",
        "```powershell",
        command.dry_run_command,
        "```",
        "Apply decision after review:",
        "```powershell",
...
function renderNextActionsMarkdown({ pack = {}, readiness = {}, executorArm = {} } = {}) {
  const commandAvailable = Boolean(pack.exact_next_command && executorArm.ready_for_live_executor_handoff);
  const approvalCommands = asArray(pack.approval_pending?.commands);
  const readinessBlocked =
    clean(readiness.overall_verdict).toLowerCase() === "red" || asArray(readiness.blockers).length > 0;
  const nextStep = clean(
    commandAvailable
      ? executorArm.required_next_step || readiness.next_action
      : readinessBlocked
        ? readiness.next_action || executorArm.required_next_step
        : executorArm.required_next_step || readiness.next_action,
  );
  const lines = [
    "# Next Actions",
    ...
    approvalCommands.length
      ? "2. Record the pending approval decisions for the selected controlled-restart candidates; these commands do not publish."
  ...
  if (pack.exact_next_command && executorArm.ready_for_live_executor_handoff) {
    lines.push("```powershell", pack.exact_next_command, "```");
  } else if (approvalCommands.length) {
    lines.push("No guarded live command is currently available until those approval decisions are recorded and the guarded preflights are rerun.");
  } else {
...
async function buildGoalControlPack({
  ...
  const p = {
    ...
    controlledRestartPack: path.join(resolvedOutDir, "controlled_restart_pack.json"),
    ...paths,
  };
  ...
  const data = {
    ...
    controlledRestartPack: await readJsonOptional(p.controlledRestartPack),
  };
  ...
  const currentReadinessReport = buildCurrentReadinessReport({
    ...
    controlledRestartPack: data.controlledRestartPack,
  });
