...
function coreQualityBlockers({
  ...
  if (!platformManifestGreen(platformManifest)) blockers.push("platform_manifest_not_green");
...
function buildPackagePublishVerdict({ storyId = "", canonical = {}, platformManifest = {}, generatedAt = "" } = {}) {
  const outputs = platformOutputs(platformManifest);
  return {
    schema_version: 1,
    story_id: storyId,
    ...
    publish_action: "guarded_dispatch_eligible",
    output_count: Object.keys(outputs).length,
...
function targetFiles({
  ...
  const publishVerdict = buildPackagePublishVerdict({ storyId, canonical, platformManifest, generatedAt });
  return {
...
async function inspectArtifact(storyPackage = {}, context = {}) {
  ...
  const repairable = !blockers.length && (missingFiles.length || staleFiles.length);

  return {
    story_id: storyId,
    artifact_dir: artifactDir || null,
    status: blockers.length ? "blocked" : repairable ? "repairable" : "already_repaired",
