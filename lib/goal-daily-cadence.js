...
const DEFAULT_PUBLISH_HOURS_UTC = [9, 19];
...
function strictDryRunStatusMap(plan = {}) {
  const source = plan && typeof plan === "object" ? plan : {};
  const hasStrictDryRunArrays = [
    source.ready_stories,
    source.blocked_stories,
    source.held_stories,
    source.skipped_stories,
  ].some(Array.isArray);
  if (!hasStrictDryRunArrays) return null;

  const map = new Map();
  const addRows = (rows = [], status) => {
    for (const row of asArray(rows)) {
      const storyId = clean(row.story_id || row.id);
      if (!storyId) continue;
      const blockers = uniqueClean([
        ...asArray(row.blockers),
        row.reason,
        row.status && status !== "ready" ? `strict_dry_run_status:${row.status}` : null,
      ]);
      map.set(storyId, {
        status,
        blockers,
  ...
  addRows(source.ready_stories, "ready");
  addRows(source.blocked_stories, "blocked");
  addRows(source.held_stories, "held");
  addRows(source.skipped_stories, "skipped");
  return {
    generated_at: source.generated_at || null,
    overall_verdict: source.overall_verdict || null,
    map,
  };
}

function applyStrictDryRunGate(item = {}, strictDryRun = null) {
  if (!strictDryRun) return item;
  const storyId = clean(item.story_id);
  const row = strictDryRun.map.get(storyId);
  if (!row) {
    return {
      ...item,
      strict_dry_run_status: "missing",
      strict_dry_run_generated_at: strictDryRun.generated_at,
      strict_dry_run_blockers: ["strict_dry_run_story_not_present"],
      blockers: uniqueClean([
        ...asArray(item.blockers),
        "strict_dry_run_story_not_present",
      ]),
    };
  }
  if (row.status === "ready") {
    return {
      ...item,
      strict_dry_run_status: "ready",
      strict_dry_run_generated_at: strictDryRun.generated_at,
      strict_dry_run_blockers: [],
    };
  }
  const statusBlocker = `strict_dry_run_story_${row.status}`;
  const blockers = uniqueClean([statusBlocker, ...asArray(row.blockers)]);
  return {
    ...item,
    strict_dry_run_status: row.status,
    strict_dry_run_generated_at: strictDryRun.generated_at,
    strict_dry_run_blockers: blockers,
    blockers: uniqueClean([
      ...asArray(item.blockers),
      ...blockers,
    ]),
...
async function buildGoalDailyCadencePlan({
  ...
  const strictDryRun = strictDryRunStatusMap(dryRunPublishPlan);
  for (const item of asArray(humanReviewQueue.review_items)) {
    ...
    const upstreamGated = applyUpstreamBenchmarkGate(
      assessedItem,
      upstreamBenchmarkMap ? upstreamBenchmarkMap.get(clean(item.story_id)) : null,
    );
    assessed.push(applyStrictDryRunGate(upstreamGated, strictDryRun));
  }
  ...
  const rejected = assessed
    ...
    .map((item) => ({
      ...
      strict_dry_run_status: item.strict_dry_run_status,
      strict_dry_run_blockers: asArray(item.strict_dry_run_blockers),
    }));
  ...
  const qualityReport = {
    ...
    gates: {
      ...
      strict_dry_run_blocks_respected: selected.every((item) =>
        !item.strict_dry_run_status || item.strict_dry_run_status === "ready",
      ),
      no_repeated_subjects_scheduled: selection.subject_diversity.no_repeated_subjects_scheduled,
      operator_approval_required: true,
    },
    strict_dry_run_gate: {
      loaded: Boolean(strictDryRun),
      generated_at: strictDryRun?.generated_at || null,
      overall_verdict: strictDryRun?.overall_verdict || null,
    },
    thresholds,
