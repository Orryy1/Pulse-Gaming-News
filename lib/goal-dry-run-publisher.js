...
} = require("./platform-publish-scope");

const MIN_PREMIUM_HYPERFRAMES_CARDS = 4;
const MIN_HYPERFRAMES_READABLE_CARD_DURATION_S = V5_READABLE_CARD_TIMING.minimum_visible_duration_s;
const MIN_COMPACT_PROOF_OVERLAY_DURATION_S = 2.6;
const MAX_COMPACT_PROOF_OVERLAY_DURATION_S = 4.2;
const MAX_HYPERFRAMES_READABLE_CARD_DURATION_S = V5_READABLE_CARD_TIMING.maximum_visible_duration_s;
const MIN_HYPERFRAMES_SOURCE_CARD_DURATION_S = V5_SOURCE_CARD_TIMING.minimum_visible_duration_s;
const MAX_HYPERFRAMES_SOURCE_CARD_DURATION_S = V5_SOURCE_CARD_TIMING.maximum_visible_duration_s;
...
function passVerdict(value = "") {
  return ["pass", "passed", "green", "ready", "ok"].includes(
    cleanText(value).toLowerCase(),
  );
}

async function exactFileFingerprint(filePath = "") {
  const resolvedPath = cleanText(filePath);
  if (!resolvedPath || !(await fs.pathExists(resolvedPath))) return null;
...
function artifactHasFailures(value = {}) {
  return [
    ...asArray(value.blockers),
    ...asArray(value.failures),
    ...asArray(value.errors),
  ].map(cleanText).filter(Boolean).length > 0;
}

async function auditExactCurrentPackageAuthority({
  artifactDir = "",
  storyId = "",
  canonical = {},
  publishVerdict = {},
  renderManifest = {},
  audioSegmentReport = null,
  coherenceReport = {},
  coherenceArtifact = {},
  publicCopyQa = {},
} = {}) {
  ...
  if (
    ...
    blockers.push("current_package_authority_invalid");
  }
  ...
  for (const key of ["render", "audio", "timestamps", "rights", "temporal_qa_report"]) {
    ...
    const actual = await exactFileFingerprint(expected.path);
    exactFingerprints[key] = actual;
    if (
      !actual ||
      normaliseSha256(expected.sha256) !== actual.sha256 ||
      Number(expected.size_bytes) !== actual.size_bytes
    ) {
      blockers.push(`current_package_authority_fingerprint_mismatch:${key}`);
    }
  }

  const renderFingerprint = exactFingerprints.render;
  const renderOutputPath = cleanText(
  ...
  if (
    !renderFingerprint ||
    !renderOutputPath ||
    path.resolve(artifactDir, renderOutputPath) !== renderFingerprint.path
  ) {
    blockers.push("current_package_authority_render_path_mismatch");
  }

  const [temporalQa, finalAvReview, decodedForensic, metadataFinalisation] =
    await Promise.all([
      readJsonIfPresent(path.join(artifactDir, "temporal_video_qa_report.json"), null),
      readJsonIfPresent(path.join(artifactDir, "final_av_review.json"), null),
      readJsonIfPresent(path.join(artifactDir, "decoded_forensic_report.json"), null),
      readJsonIfPresent(
        path.join(artifactDir, "youtube_publish_metadata_finalisation.json"),
        null,
      ),
    ]);
  if (
    !temporalQa ||
    cleanText(temporalQa.story_id) !== cleanText(storyId) ||
    !passVerdict(temporalQa.verdict || temporalQa.status) ||
    temporalQa.can_publish !== true ||
    artifactHasFailures(temporalQa) ||
    normaliseSha256(temporalQa.final_media?.sha256) !== renderFingerprint?.sha256 ||
    Number(temporalQa.final_media?.size_bytes) !== renderFingerprint?.size_bytes ||
    temporalQa.validation?.render_hash_matches !== true ||
    temporalQa.validation?.render_size_matches !== true ||
    temporalQa.validation?.decode_complete !== true ||
    temporalQa.validation?.video_stream_decoded !== true ||
    temporalQa.validation?.audio_stream_decoded !== true ||
    temporalQa.validation?.temporal_scan_complete !== true
  ) {
    blockers.push("current_package_temporal_qa_invalid");
  }
  if (
    !finalAvReview ||
    cleanText(finalAvReview.story_id) !== cleanText(storyId) ||
    !passVerdict(finalAvReview.verdict || finalAvReview.status) ||
    finalAvReview.publish_ready !== true ||
    finalAvReview.can_auto_publish !== true ||
    finalAvReview.reviewer?.independent !== true ||
    normaliseSha256(finalAvReview.reviewed_artefact_fingerprints?.final_mp4) !==
      renderFingerprint?.sha256 ||
    !["full_watch", "full_listen", "av_sync", "caption_readability", "subject_match"]
      .every((field) => finalAvReview.attestations?.[field] === true) ||
    artifactHasFailures(finalAvReview)
  ) {
    blockers.push("current_package_final_av_review_invalid");
  }
  if (
    !decodedForensic ||
    cleanText(decodedForensic.story_id) !== cleanText(storyId) ||
    !passVerdict(decodedForensic.verdict || decodedForensic.status) ||
    normaliseSha256(decodedForensic.final_media?.sha256) !== renderFingerprint?.sha256 ||
    Number(decodedForensic.final_media?.audio_stream_count || 0) < 1 ||
    !passVerdict(decodedForensic.checks?.audio?.verdict) ||
    !passVerdict(decodedForensic.checks?.video?.verdict) ||
    !passVerdict(decodedForensic.checks?.av_sync?.verdict) ||
    artifactHasFailures(decodedForensic)
  ) {
    blockers.push("current_package_decoded_forensic_invalid");
  }

  const authorityValid = blockers.length === 0;
  ...
  const visualAssets = selectedAssets.filter((asset) =>
    ["image", "screenshot", "still", "visual"].some((kind) =>
      cleanText(asset.kind || asset.asset_type).toLowerCase().includes(kind),
    ),
  );
  ...
  const hyperframesAuthorityValid =
    authorityValid &&
    /hyperframes/i.test(cleanText(
    ...
    !artifactHasFailures(selected) &&
    Number(selected.asset_count) === selectedAssets.length &&
    visualAssets.length >= MIN_PREMIUM_HYPERFRAMES_CARDS &&
    visualAssets.every((asset) =>
      asset.subject_match === true &&
      /^[a-f0-9]{64}$/i.test(normaliseSha256(asset.asset_sha256)) &&
      canonicalVisualIds.has(cleanText(asset.asset_id || asset.id))) &&
    /hyperframes/i.test(cleanText(scenePlan.renderer)) &&
    scenePlan.repeat_free === true &&
    Number(scenePlan.scene_count || 0) >= MIN_PREMIUM_HYPERFRAMES_CARDS &&
    Number(scenePlan.unique_visual_asset_count || 0) >= MIN_PREMIUM_HYPERFRAMES_CARDS &&
    Number(scenePlan.placement_count || 0) ===
      Number(scenePlan.verified_placement_count || -1) &&
    passVerdict(scenePlan.temporal_qa_verdict) &&
    passVerdict(decodedGate.verdict) &&
    decodedGate.can_publish === true &&
    normaliseSha256(decodedGate.render_sha256) === renderFingerprint?.sha256 &&
    Number(decodedGate.render_size_bytes) === renderFingerprint?.size_bytes &&
    /^[a-f0-9]{64}$/i.test(normaliseSha256(hyperframesInput.composition_sha256)) &&
    passVerdict(hyperframesInput.rights_sidecar_verdict) &&
    normaliseSha256(adoption.final_render_sha256) === renderFingerprint?.sha256 &&
    passVerdict(adoption.human_av_review_verdict) &&
    passVerdict(adoption.temporal_video_qa_verdict) &&
    passVerdict(adoption.rights_placement_verdict);

  const audioAuthorityValid =
    authorityValid &&
    audioSegmentReport &&
    passVerdict(audioSegmentReport.verdict || audioSegmentReport.status) &&
    !artifactHasFailures(audioSegmentReport) &&
    normaliseSha256(renderManifest.input_fingerprint?.audio_sha256) ===
      exactFingerprints.audio?.sha256 &&
    normaliseSha256(renderManifest.input_fingerprint?.word_timestamps_sha256) ===
      exactFingerprints.timestamps?.sha256 &&
    finalAvReview?.attestations?.full_listen === true &&
    passVerdict(decodedForensic?.checks?.audio?.verdict);

  const coherenceMismatchFields = asArray(coherenceArtifact.mismatched_fields);
  const metadataGeneratedAt = timeMs(metadataFinalisation?.generated_at);
  const coherenceGeneratedAt = timeMs(coherenceReport.generated_at);
  const descriptionAuthorityValid =
    authorityValid &&
    coherenceMismatchFields.length === 1 &&
    coherenceMismatchFields[0] === "description" &&
    passVerdict(coherenceReport.result || coherenceReport.verdict || coherenceReport.status) &&
    !artifactHasFailures(coherenceReport) &&
    asArray(publicCopyQa.failures).length === 0 &&
    metadataFinalisation?.applied === true &&
    passVerdict(metadataFinalisation?.verdict) &&
    cleanText(metadataFinalisation?.story_id) === cleanText(storyId) &&
    metadataGeneratedAt != null &&
    coherenceGeneratedAt != null &&
    metadataGeneratedAt > coherenceGeneratedAt &&
    Boolean(cleanText(canonical.description));

  return {
    valid: authorityValid,
    blockers,
    exact_fingerprints: exactFingerprints,
    hyperframes_premium_shell_aliases_superseded: hyperframesAuthorityValid,
    audio_report_timestamp_alias_superseded: audioAuthorityValid,
    description_coherence_aliases_superseded: descriptionAuthorityValid,
  };
}
...
function readableCardDurationRequiredS(value = {}) {
  ...
  if (explicitMinimum != null && explicitMinimum > 0) {
    if (sourceCardLike(value)) {
      return Math.max(explicitMinimum, MIN_HYPERFRAMES_SOURCE_CARD_DURATION_S);
    }
    if (compactProofOverlayLike(value)) {
      return Math.max(explicitMinimum, MIN_COMPACT_PROOF_OVERLAY_DURATION_S);
    }
    if (compactHeadlineOverlayLike(value)) {
      return Math.max(explicitMinimum, compactHeadlineOverlayDurationRequiredS(value));
    }
    return Math.max(explicitMinimum, MIN_HYPERFRAMES_READABLE_CARD_DURATION_S);
  }
  ...
  const timing = v5CardTimingContract(value.kind || value.id || "context", text);
  return timing.required_visible_duration_s || timing.planned_visible_duration_s;
}

function cardDurationAllowedS(value = {}) {
  if (sourceCardLike(value)) return MAX_HYPERFRAMES_SOURCE_CARD_DURATION_S;
  if (compactProofOverlayLike(value)) return MAX_COMPACT_PROOF_OVERLAY_DURATION_S;
  if (compactHeadlineOverlayLike(value)) return 5.8;
  return MAX_HYPERFRAMES_READABLE_CARD_DURATION_S;
}

function cardWindowDurationAllowedS(value = {}) {
  if (sourceCardLike(value)) return MAX_HYPERFRAMES_SOURCE_CARD_DURATION_S;
  const source = cleanText(value.source || value.evidence_source).toLowerCase();
  // Scene plans describe the full shot, not the measured lifetime of an overlay.
  // Applying the ordinary-card maximum here falsely treats gameplay after a card
  // exits as additional card dwell.
  if (source === "visual_v4_scene_plan") return null;
  return cardDurationAllowedS(value);
}
...
function tooSlowSourceCardClipEvidence(renderStory = {}) {
  return hyperframesCardClips(renderStory)
    ...
    .map((clip) => ({
      ...
      duration_s: clipDurationSeconds(clip),
      minimum_required_duration_s: readableCardDurationRequiredS(clip),
      maximum_allowed_duration_s: cardDurationAllowedS(clip),
    }))
...
function renderVisibleCardWindowEvidence(renderManifest = {}) {
  ...
  return windows
    .map((window) => {
      ...
      maximum_allowed_duration_s: cardWindowDurationAllowedS(window),
...
function hyperframesReadableDwellEvidence({ renderManifest = {}, renderStory = {}, directorBeatMap = {} } = {}) {
  ...
  const directorCardShots = asArray(directorBeatMap.shot_plan)
    ...
    .map((shot) => ({
      ...
      maximum_allowed_duration_s: cardDurationAllowedS(shot),
    }));
  ...
  if (tooSlowSourceCardWindows.length || tooSlowSourceCardClips.length || tooSlowSourceCardShots.length) {
    blockers.push("hyperframes:card_visible_dwell_too_long");
    blockers.push("visual_evidence:card_visible_dwell_too_long");
    if ([...tooSlowSourceCardWindows, ...tooSlowSourceCardClips, ...tooSlowSourceCardShots].some(sourceCardLike)) {
...
async function inspectStoryPackage(storyPackage = {}, options = {}) {
  ...
  const blockers = [];
  const warnings = [];
  const authorityReconciliations = [];
  ...
  const platformManifest = artifactDir
    ...
    : {};
  const platformPublishScope = resolvePlatformPublishScope(platformManifest);
  blockers.push(...platformPublishScope.blockers);
  ...
  const stalePublishVerdictPromotedByPreflight = false;
  const effectivePublishVerdict = publishVerdict;
  ...
  const premiumShellBlockers = hyperframesPremiumShellBlockers({
    ...storyPackage,
    ...renderManifest,
    render_manifest: renderManifest,
  ...
  blockers.push(...premiumShellBlockers);
  ...
  const publicCopyQa = evaluateGoalPublicCopy({
    ...
    platform_publish_scope: platformPublishScope,
    landing_page_manifest: landingPage,
  });
  blockers.push(...asArray(publicCopyQa.failures));
  let coherenceArtifact = await auditPublicOutputCoherenceArtifact({
    artifactDir,
    canonical,
    coherenceReport,
  });
  blockers.push(...asArray(coherenceArtifact.blockers));
  const exactCurrentAuthority = await auditExactCurrentPackageAuthority({
    artifactDir,
    storyId,
    canonical,
    publishVerdict: effectivePublishVerdict,
    renderManifest,
    audioSegmentReport,
    coherenceReport,
    coherenceArtifact,
  ...
  if (exactCurrentAuthority.hyperframes_premium_shell_aliases_superseded) {
    blockers.splice(
      0,
      blockers.length,
      ...blockers.filter((blocker) => !premiumShellBlockers.includes(blocker)),
    );
    authorityReconciliations.push(
      "hyperframes_premium_shell_aliases_superseded_by_exact_current_authority",
    );
  }
  if (exactCurrentAuthority.audio_report_timestamp_alias_superseded) {
    const staleAudioBlocker = "audio_segment_loudness_report_stale_after_render";
    blockers.splice(
      0,
      blockers.length,
      ...blockers.filter((blocker) => blocker !== staleAudioBlocker),
    );
    authorityReconciliations.push(
      "audio_report_timestamp_alias_superseded_by_exact_current_authority",
    );
  }
  let effectiveCoherenceReport = coherenceReport;
  if (exactCurrentAuthority.description_coherence_aliases_superseded) {
    const staleDescriptionAliases = new Set([
    ...
    blockers.splice(
      0,
      blockers.length,
      ...blockers.filter((blocker) => !staleDescriptionAliases.has(blocker)),
    );
    coherenceArtifact = {
      ...coherenceArtifact,
      status: "fresh",
      blockers: asArray(coherenceArtifact.blockers).filter(
        (blocker) => !staleDescriptionAliases.has(blocker),
      ),
      mismatched_fields: [],
      reconciled_by: "exact_current_package_authority_and_metadata_finalisation",
    };
    effectiveCoherenceReport = {
      ...coherenceReport,
      manifest: {
        ...objectValue(coherenceReport.manifest),
  ...
  const incidentGuard = evaluateIncidentGuard({
    ...
    coherence_report: effectiveCoherenceReport,
  ...
  const inspected = {
    ...
    blockers,
    warnings,
    authority_reconciliations: authorityReconciliations,
    canonical,
    publish_verdict: effectivePublishVerdict,
    ...
    coherence_report: effectiveCoherenceReport,
    coherence_artifact_freshness: coherenceArtifact,
    exact_current_package_authority: exactCurrentAuthority,
    visual_quality_report: visualQualityReport,
    benchmark_report: benchmarkReport,
    sfx_manifest: sfxManifest,
    script_scorecard: scriptScorecard,
...
function enabledPublishPlatformsForStory(story = {}, platformOperationalConfig = null) {
  const globallyEnabled = enabledPublishPlatforms(platformOperationalConfig);
  const scope = story.platform_publish_scope;
  if (!scope?.explicit) return globallyEnabled;
  const enabled = new Set(asArray(scope.enabled_platforms).map(cleanText));
  return globallyEnabled.filter((platform) => enabled.has(platform));
}

function applyEnabledPlatformAlreadyPublishedSkips(stories = [], platformOperationalConfig = null) {
  for (const story of stories) {
    if (story.skip_status) continue;
    const enabledPlatforms = enabledPublishPlatformsForStory(
      story,
      platformOperationalConfig,
    );
    if (!enabledPlatforms.length) continue;
...
function uploaderSafeActionDescription(value = "") {
  const text = cleanText(value);
  if (!text || text.length > 420 || containsPublicUrl(text)) return false;
  return !ARTICLE_RESIDUE_PATTERNS.some((pattern) => pattern.test(text));
}

const YOUTUBE_DESCRIPTION_MAX_CHARS = 5_000;

function approvedYoutubeActionDescription(value = "", canonical = {}) {
  const text = String(value || "").trim();
  const approved = String(canonical.youtube_description || "").trim();
  if (!text || !approved || text !== approved) return "";
  return text.length <= YOUTUBE_DESCRIPTION_MAX_CHARS ? text : "";
}

function canonicalUploaderSafeDescription(canonical = {}) {
  return [
    canonical.public_description,
    canonical.description,
  ]
    .map(cleanText)
    .find(uploaderSafeActionDescription) || "";
}
...
function platformDescriptionFromConfig(platform = "", platformConfig = {}, canonical = {}, outputs = {}) {
  ...
  const uploaderSafeFallback = canonicalUploaderSafeDescription(canonical);
  ...
  if (platform === "youtube_shorts") {
    const approved = [
      ...
      .map((value) => approvedYoutubeActionDescription(value, canonical))
      .find(Boolean);
  ...
  if (!uploaderSafeActionDescription(direct)) return uploaderSafeFallback;
  const preferred = weakPlatformActionCopy(direct, canonical) && siblingDescription
    ? siblingDescription
    : direct;
  return uploaderSafeActionDescription(preferred)
    ? preferred
    : uploaderSafeFallback;
}
...
function platformActionPublicCopy({ platform = "", platformConfig = {}, canonical = {}, outputs = {} } = {}) {
  ...
  const configuredCaption = cleanText(
    platformConfig.caption ||
      platformConfig.page_caption ||
      platformConfig.post_text ||
      description,
  );
  const caption = uploaderSafeActionDescription(configuredCaption)
    ? configuredCaption
    : description;
  const configuredPageCaption = cleanText(platformConfig.page_caption || caption || description);
  const pageCaption = uploaderSafeActionDescription(configuredPageCaption)
    ? configuredPageCaption
    : description;
  return {
    title: platformTitleFromConfig(platformConfig, canonical),
    description,
...
function hasPositiveDisclosureRequirement(requirements = {}) {
  return Object.entries(objectValue(requirements)).some(([key, value]) => (
    value === true &&
    /(?:affiliate|commercial|paid|promotion|sponsor|brand|own_brand|ai|synthetic|altered)/i.test(key)
  ));
}

function positiveDisclosureRequirementKeys(requirements = {}) {
  return Object.entries(objectValue(requirements))
    .filter(([key, value]) => (
      value === true &&
      /(?:affiliate|commercial|paid|promotion|sponsor|brand|own_brand|ai|synthetic|altered)/i.test(key)
    ))
...
function affirmativeAffiliateDisclosureText(value = "") {
  const text = cleanText(value);
  if (!text) return false;
  if (
    /\b(?:not|no|never|without|isn't|aren't|doesn't)\b.{0,40}\b(?:affiliate|commission|compensation)\b/i.test(text) ||
    /\bdo(?:es)?\s+not\b.{0,40}\b(?:affiliate|commission|compensation)\b/i.test(text)
  ) {
    return false;
  }
  return (
    /(?:^|\s)#ad\b/i.test(text) ||
    /\baffiliate disclosure\b/i.test(text) ||
    (/\baffiliate links?\b/i.test(text) && /\b(?:earn|commission|compensated)\b/i.test(text))
  );
}

function platformDisclosureRequirementPresent(platformConfig = {}, requirement = "") {
  if (/(?:^|_)(?:ai|synthetic|altered)(?:_|$)/i.test(requirement)) {
    return (
      platformConfig.altered_synthetic_disclosure_present === true ||
      platformConfig.youtube_synthetic_media_disclosed === true ||
      platformConfig.synthetic_media_disclosed === true ||
      platformConfig.ai_disclosure_applied === true ||
      platformConfig.ai_generated_content_label_applied === true
    );
  }
  if (/(?:^|_)(?:affiliate)(?:_|$)/i.test(requirement)) {
    const status = objectValue(platformConfig.disclosure_status);
    return (
      platformConfig.affiliate_disclosure_present === true ||
      platformConfig.affiliate_disclosed === true ||
      (
        status.required === true &&
        /affiliate/i.test(cleanText(status.type)) &&
        affirmativeAffiliateDisclosureText(status.caption || status.copy || status.text)
      )
    );
  }
  if (/(?:^|_)(?:commercial)(?:_|$)/i.test(requirement)) {
    const explicitPresence = [
      platformConfig.commercial_content_disclosure_present,
      platformConfig.commercial_disclosure_present,
      platformConfig.commercial_disclosure_applied,
    ];
    if (explicitPresence.some((value) => value === true)) return true;
    if (explicitPresence.some((value) => value === false)) return false;
    return (
      platformConfig.commercial_disclosure_required === true &&
      cleanText(
        platformConfig.caption_copy ||
          platformConfig.disclosure_copy ||
          platformConfig.disclosure_text,
      ).length >= 10
    );
  }
  if (/(?:^|_)(?:paid|promotion|sponsor|brand|own_brand)(?:_|$)/i.test(requirement)) {
    return (
      platformConfig.paid_promotion_disclosure_present === true ||
      platformConfig.paid_promotion_disclosed === true ||
      platformConfig.paid_promotion_toggle === true
    );
  }
  return false;
}

function policyDisclosureRequirements(policy = {}) {
  const explicitRequirements = objectValue(policy.disclosure_requirements);
  const legacyDisclosures = objectValue(policy.disclosures);
  const requirements = {
    ...legacyDisclosures,
    ...explicitRequirements,
  };
  for (const key of new Set([
    ...Object.keys(legacyDisclosures),
    ...Object.keys(explicitRequirements),
  ])) {
    if (legacyDisclosures[key] === true || explicitRequirements[key] === true) {
      requirements[key] = true;
    }
  }
  if (policy.ai_disclosure_gate?.disclosure_required === true) {
    requirements.altered_or_synthetic_content = true;
  }
  return requirements;
}

function platformDisclosureResolution(platformConfig = {}, disclosureRequirements = {}) {
  const explicitStatusValue = objectValue(platformConfig.disclosure_status);
  const explicitStatus = Object.keys(explicitStatusValue).length
    ? explicitStatusValue
    : null;
  const platformDisclosures = objectValue(platformConfig.disclosures);
  const disclosureFlag = cleanText(platformConfig.disclosure_flag).toLowerCase();
  const conflictsWithNoDisclosure =
    hasPositiveDisclosureRequirement(disclosureRequirements) ||
    explicitStatus?.required === true;
  let requirementsResolved =
    platformConfig.disclosure_requirements_resolved === true ||
    platformDisclosures.requirements_resolved === true;
  let resolutionBasis = requirementsResolved
    ? "platform_manifest_explicit_resolution"
    : null;
  let disclosureStatus = explicitStatus;

  const requiredKeys = positiveDisclosureRequirementKeys(disclosureRequirements);
  if (
    !requirementsResolved &&
    requiredKeys.length > 0 &&
    requiredKeys.every((requirement) => (
      platformDisclosureRequirementPresent(platformConfig, requirement)
    ))
  ) {
    requirementsResolved = true;
    resolutionBasis = "platform_manifest_required_disclosures_present";
  }

  if (
    !requirementsResolved &&
    explicitStatus?.required === false &&
    !conflictsWithNoDisclosure
  ) {
    requirementsResolved = true;
    resolutionBasis = "platform_manifest_disclosure_status:not_required";
  }

  if (
    !requirementsResolved &&
    disclosureFlag === "not_required" &&
    !conflictsWithNoDisclosure
  ) {
    requirementsResolved = true;
    resolutionBasis = "platform_manifest_disclosure_flag:not_required";
  }

  if (!disclosureStatus && disclosureFlag === "not_required") {
    disclosureStatus = {
      required: false,
      type: "none",
  ...
  if (disclosureFlag) disclosures.disclosure_flag = disclosureFlag;
  disclosures.requirements_resolved = requirementsResolved;
  if (resolutionBasis) disclosures.resolution_basis = resolutionBasis;

  return {
    disclosure_requirements_resolved: requirementsResolved,
    disclosures,
    disclosure_status: disclosureStatus,
  };
}

function platformDisclosureRow(disclosures = {}, platform = "") {
  const rows = objectValue(disclosures);
  const direct = objectValue(rows[platform]);
  if (Object.keys(direct).length) return direct;
  const aliasKey = Object.keys(rows).find((key) => normalizePlatformKey(key) === platform);
  return aliasKey ? objectValue(rows[aliasKey]) : {};
}
...
function buildPlatformAction(story, platform, platformOperationalConfig = null) {
  ...
  const platformConfig = {
    ...platformDisclosureRow(story.platform_policy_report.platform_disclosure, platform),
    ...platformDisclosureRow(story.platform_publish_manifest.platform_disclosure, platform),
    ...objectValue(outputs[platform]),
  };
  const artifactDir = story.artifact_dir;
  const disclosureRequirements = policyDisclosureRequirements(
    story.platform_policy_report,
  );
  const disclosureResolution = platformDisclosureResolution(
    platformConfig,
  ...
  const exactAuthorityRender =
    story.exact_current_package_authority?.valid === true
      ? objectValue(
          story.exact_current_package_authority.exact_fingerprints?.render,
        )
      : {};
  const videoPath = variantVideoReference
    ? resolveArtifactReference(artifactDir, variantVideoReference)
    : cleanText(exactAuthorityRender.path) ||
      resolveArtifactReference(
        artifactDir,
        story.render_manifest.output_path ||
          story.render_manifest.final_mp4_path ||
          story.render_manifest.exported_path ||
          story.render_manifest.output,
      ) ||
  ...
  if (
    ...
    String(story.canonical.youtube_description || "").trim().length > YOUTUBE_DESCRIPTION_MAX_CHARS
  ) {
    blockers.push("youtube_description_exceeds_5000_characters");
  }
  if (
    hasPositiveDisclosureRequirement(disclosureRequirements) &&
    disclosureResolution.disclosure_requirements_resolved !== true
  ) {
    blockers.push("disclosure_requirements_unresolved");
  }
  ...
  const declaredWarnings = Array.from(new Set([
    ...asArray(platformConfig.duration_warnings || platformConfig.warnings),
    ...asArray(story.warnings),
  ]));
  const hardDurationMin = numberOrNull(durationWindow?.min);
  const hardDurationMax = numberOrNull(durationWindow?.max);
  const baseTikTokUploadEligible =
    platform === "tiktok" &&
    !creatorRewardsVariantReady &&
    videoDurationS != null &&
    (hardDurationMin == null || videoDurationS >= hardDurationMin) &&
    (hardDurationMax == null || videoDurationS <= hardDurationMax);
  const advisories = declaredWarnings.filter(
    (warning) =>
      baseTikTokUploadEligible &&
      warning === "below_creator_rewards_duration",
  );
  const warnings = declaredWarnings.filter(
    (warning) =>
      !(creatorRewardsVariantReady && warning === "below_creator_rewards_duration") &&
      !advisories.includes(warning),
  );
  ...
  return {
    ...
    video_sha256:
      !variantVideoReference && cleanText(exactAuthorityRender.path) === videoPath
        ? normaliseSha256(exactAuthorityRender.sha256) || null
        : null,
    video_size_bytes:
      !variantVideoReference && cleanText(exactAuthorityRender.path) === videoPath
        ? numberOrNull(exactAuthorityRender.size_bytes)
        : null,
    video_fingerprint_source:
      !variantVideoReference && cleanText(exactAuthorityRender.path) === videoPath
    ...
    disclosure_requirements_resolved:
      disclosureResolution.disclosure_requirements_resolved,
    disclosures: disclosureResolution.disclosures,
    disclosure_status: disclosureResolution.disclosure_status,
    commercial_promotion:
      typeof platformConfig.commercial_promotion === "boolean"
        ? platformConfig.commercial_promotion
        : null,
    affiliate_links_allowed:
      platformConfig.commercial_promotion === false &&
      platformConfig.disclosure_status?.required === false
        ? false
        : null,
    ...
    warnings,
    advisories,
    authority_reconciliations: asArray(story.authority_reconciliations),
    blockers,
...
function cleanEnabledActionReadyForGuardedDispatch(action = {}) {
  return Boolean(
    ...
    (
      !hasPositiveDisclosureRequirement(action.disclosure_requirements) ||
...
function buildPlatformStatusEvidence({
  ...
  for (const platform of PLATFORMS) {
    ...
    const scopeDisabled = asArray(scopeDisabledRows).filter(
      (row) => cleanText(row.platform) === platform,
    );
    const scopeOnlyDisabled =
      scopeDisabled.length > 0 &&
      platformActions.length === 0 &&
      platformBlockedActions.length === 0;
    ...
    const operationalReason =
      scopeOnlyDisabled
        ? cleanText(scopeDisabled[0]?.reason) || "story_platform_scope_disabled"
    ...
    platformEntries[platform] = {
      ...
      can_auto_publish: scopeOnlyDisabled ? false : publishNow.length > 0,
      ...
      scope_disabled_story_ids: uniqueCleanStrings(
        scopeDisabled.map((row) => row.story_id),
      ),
      ...
      advisories: Array.from(new Set(platformActions.flatMap((action) => asArray(action.advisories)))),
...
function applyRepairWorkOrderQuarantine(stories = [], repairWorkOrder = null) {
  ...
  for (const story of stories) {
    ...
    if (deadEnd) {
      if (!story.hold_status || story.hold_status === "quarantined_before_scheduler_preflight") {
        story.hold_status = "quarantined_by_repair_work_order";
      }
    ...
    if (!story.hold_status || story.hold_status === "quarantined_before_scheduler_preflight") {
      story.hold_status = "held_for_operator_source_review";
    }
    story.hold_reasons = Array.from(new Set([
      ...asArray(story.hold_reasons),
...
async function buildGoalDryRunPublishPlan({
  ...
  const blockedStories = inspected
    ...
    .map((story) => ({
      ...
      publish_verdict: story.publish_verdict || null,
      raw_publish_verdict: story.raw_publish_verdict || null,
      stale_publish_verdict_promoted_by_scheduler_preflight:
        story.stale_publish_verdict_promoted_by_scheduler_preflight === true,
  ...
  const platformActions = readyStories.flatMap((story) =>
    PLATFORMS
      ...
      .filter((platform) =>
        !story.platform_publish_scope?.explicit ||
        asArray(story.platform_publish_scope.enabled_platforms).includes(platform))
  ...
  const platformStatusEvidence = buildPlatformStatusEvidence({
    ...
    platformOperationalConfig,
    scopeDisabledRows: inspected.flatMap((story) =>
      story.platform_publish_scope?.explicit
        ? asArray(story.platform_publish_scope.disabled_platforms).map((platform) => ({
            story_id: story.story_id,
            platform,
            reason:
              story.platform_publish_scope.disabled_platform_reasons?.[platform] ||
              "story_platform_scope_disabled",
          }))
        : []),
  });
