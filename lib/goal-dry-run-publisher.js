...
function sfxRenderAssetMismatch({ sfxManifest = {}, renderStory = {} } = {}) {
  ...
  if (!approvedAssets.size || !renderedAssets.length) return false;
  return renderedAssets.some((assetId) => !approvedAssets.has(assetId));
}
...
async function inspectStoryPackage(storyPackage = {}, options = {}) {
  ...
  blockers.push(...externalMotionPackBlockers(externalMotionPack, renderManifest));
...
function buildPlatformAction(story, platform, platformOperationalConfig = null) {
  ...
  return {
    ...
    creator_rewards_eligible: platformConfig.creator_rewards_eligible,
...
function buildPlatformStatusEvidence({
  ...
  for (const platform of PLATFORMS) {
    ...
    const operationalReason =
      ...
      platformOperationalState(platformOperationalConfig, platform)?.reason ||
      (!platformOperationalState(platformOperationalConfig, platform)
...
function applyRepairWorkOrderQuarantine(stories = [], repairWorkOrder = null) {
  ...
  if (!deadEndJobsById.size) return stories;
  for (const story of stories) {
...
async function buildGoalDryRunPublishPlan({
  ...
  const heldStoryReports = heldStories.map((story) => ({
    ...
    render_input_requirements: asArray(story.render_input_requirements),
    repair_lanes: asArray(story.repair_work_order_dead_end?.repair_lanes),
    repair_action_ids: asArray(story.repair_work_order_dead_end?.action_ids),
    operator_approval_required: story.repair_work_order_dead_end?.operator_approval_required === true,
    incident_guard: story.incident_guard || null,
  }));
  ...
  const publishNowActions = actions.filter((action) => action.action === "would_publish");
  ...
  const activeDisasterUploadBlockers = disasterUploadBlockers.filter((story) =>
    !heldStoryIds.has(String(story.story_id)),
  );
  ...
  return {
    ...
    summary: {
      ...
      planned_action_count: actions.length,
      platform_publish_now_action_count: publishNowActions.length,
      platform_deferred_action_count: deferredActions.length,
...
function renderGoalDryRunPublishPlanMarkdown(plan = {}) {
  ...
  lines.push(`Planned actions: ${plan.summary?.planned_action_count || 0}`);
  if (plan.summary?.platform_publish_now_action_count != null) {
    lines.push(`Publish-now actions: ${plan.summary.platform_publish_now_action_count}`);
  }
  if (plan.summary?.platform_deferred_action_count) {
    lines.push(`Deferred platform actions: ${plan.summary.platform_deferred_action_count}`);
  }
