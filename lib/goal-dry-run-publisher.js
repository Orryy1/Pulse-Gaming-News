...
const MIN_HYPERFRAMES_READABLE_CARD_DURATION_S = 2.5;
...
const MAX_DIRECT_MOTION_BASE_SOURCE_SHARE = 0.45;
...
function hyperframesCardClipCount(renderStory = {}) {
  return renderStoryVisualUnits(renderStory).filter((clip) => {
    ...
    return /hyperframes|proof[_-]?card|source[_-]?card|context[_-]?card|timeline[_-]?card|quote[_-]?card|takeaway[_-]?card/.test(text);
  }).length;
}
...
function hyperframesReadableDwellEvidence({ renderManifest = {}, renderStory = {}, directorBeatMap = {} } = {}) {
  ...
  const declaresHyperframes = declaresHyperframesPremium({
    ...renderManifest,
    render_manifest: renderManifest,
  }) || cardCount > 0;
  const estimatedCardDuration = renderedDuration && finalClipCount
    ...
    : null;
  const tooFastCardShots = asArray(directorBeatMap.shot_plan)
    ...
    .filter((shot) => shot.duration_s != null && shot.duration_s < MIN_HYPERFRAMES_READABLE_CARD_DURATION_S);
  ...
  const tooFastVisibleCardWindows = visibleCardWindows
    .filter((window) => window.duration_s < MIN_HYPERFRAMES_READABLE_CARD_DURATION_S);
  ...
  if (declaresHyperframes && (visibleCardWindows.length ? tooFastVisibleCardWindows.length : tooFastCardShots.length)) {
    blockers.push(
      visibleCardWindows.length
        ? "hyperframes:rendered_card_window_dwell_too_short"
...
function directMotionBaseSourceKey(clip = {}) {
  ...
  const explicitFamily = cleanText(
    clip.base_source_family ||
      clip.original_source_family ||
      clip.provenance?.base_source_family ||
      clip.provenance?.source_family ||
  ...
  const strippedFamily = stripWindowSuffix(explicitFamily);
  if (strippedFamily && strippedFamily !== cleanText(explicitFamily).toLowerCase()) {
    return `family:${strippedFamily}`;
  }
  const sourceKey = sourceUrlFamilyKey(
    clip.source_url ||
      clip.url ||
  ...
  if (sourceKey) return sourceKey;
  if (strippedFamily) return `family:${strippedFamily}`;
  ...
  const overused = [...counts.entries()]
    ...
    .filter(
      (entry) =>
        ...
        entry.share > MAX_DIRECT_MOTION_BASE_SOURCE_SHARE,
    )
...
async function inspectStoryPackage(storyPackage = {}, options = {}) {
  ...
  const directMotionSegmentEvidenceSource = materialisedMotionEvidence.length
