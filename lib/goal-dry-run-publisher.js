...
const MIN_HYPERFRAMES_SOURCE_CARD_DURATION_S = 1.2;
const MAX_HYPERFRAMES_SOURCE_CARD_DURATION_S = 2.8;
...
function sourceCardLike(value = {}) {
  if (typeof value === "string") return /(?:^|[_\s-])source(?:[_\s-]?lock|[_\s-]?card)?(?:$|[_\s-])/i.test(value);
  ...
  return /(?:^|[\s_-])source[_\s-]?lock(?:$|[\s_-])|(?:^|[\s_-])source[_\s-]?card(?:$|[\s_-])|\bcard\.source\b/.test(text);
}
