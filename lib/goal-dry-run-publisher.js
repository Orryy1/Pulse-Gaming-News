...
function declaresHyperframesPremium(value = {}) {
  ...
  return hyperframesCardCount(value) > 0 || text.includes("hyperframes");
}
...
function hyperframesPremiumShellBlockers(value = {}) {
  ...
  const explicitRequirement =
    value.hyperframes_premium_shell_required === true ||
    value.premium_shell_required === true ||
    lane.hyperframes_premium_shell_required === true ||
    lane.premium_shell_required === true;
  if (!explicitRequirement && cardCount < requiredPassCount) return [];
  ...
  if (verdict === "pass" && !passCountTooLow) return [];
...
function narrationManifestBlockers(narrationManifest = null, audioManifest = {}) {
  if (!manifestObjectPresent(narrationManifest)) return ["narration_manifest_missing"];
  const blockers = [];
...
async function narrationReady(artifactDir, narrationManifest = {}, audioManifest = {}, fallbackTranscript = "") {
  if (
    !artifactDir ||
    !manifestObjectPresent(narrationManifest) ||
...
function storyPackagePolicyBlockers(storyPackage = {}) {
  ...
  const packageBlockers = asArray(storyPackage.blockers).map(cleanText).filter(Boolean);
