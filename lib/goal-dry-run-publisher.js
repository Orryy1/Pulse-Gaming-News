...
const MIN_HYPERFRAMES_READABLE_CARD_DURATION_S = 4;
const MAX_FINAL_RENDER_VISUAL_REUSE_RATIO = 1.5;
const MIN_FINAL_RENDER_REUSE_CLIPS = 12;
const MAX_DIRECT_MOTION_BASE_SOURCE_CLIPS = 1;
const MAX_DIRECT_MOTION_BASE_SOURCE_SHARE = 0.2;
...
function tooFastHyperframesCardClipEvidence(renderStory = {}) {
  return hyperframesCardClips(renderStory)
    ...
    .filter((clip) => clip.duration_s != null && clip.duration_s < MIN_HYPERFRAMES_READABLE_CARD_DURATION_S);
}
...
async function inspectStoryPackage(storyPackage = {}, options = {}) {
  ...
  const directMotionSegmentEvidenceSource = mergedMotionEvidenceClips(
  ...
  const directMotionBaseSourceOveruse = directMotionBaseSourceOveruseEvidence(
    renderStoryMotion.clips.length ? renderStoryMotion.clips : directMotionSegmentEvidenceSource,
  );
