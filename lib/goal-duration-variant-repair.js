...
function sourceSafeClaimForExtension(canonical = {}, subject = "") {
  ...
  if (/\b(?:gta\s*6|grand theft auto)\b/.test(text) && /\bpre[- ]?orders?\b/.test(text) && /\bjune\s*25\b/.test(text)) {
    return "Rockstar confirmed GTA 6 preorders begin on June 25, while price, editions and bonuses still need careful checking before money moves.";
  }
  if (/\bhalo\b/.test(text) && /\b(?:xbox account|gamertag|ps plus|split[- ]screen|co[- ]op|account catch)\b/.test(text)) {
...
function compactPurposeSentences(canonical = {}, subject = "", claim = "") {
  ...
  if (intent === "preorder_price_risk") {
    return [
      "The risk is not whether GTA 6 will be huge; it is whether the first store page makes the value clear.",
      "Price, editions and bonuses decide whether fans buy early, wait or argue about the premium version.",
      "That matters because a preorder is not a trailer; it is a purchase decision.",
      "Preorders open the price fight.",
    ];
  }
  if (intent === "social_mode_tradeoff") {
    return [
