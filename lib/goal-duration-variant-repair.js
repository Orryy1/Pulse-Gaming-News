...
function socialModeVoiceCadenceScript(canonical = {}, cta = PRIMARY_PULSE_CTA) {
  return normaliseProtectedNamesText(appendCtaAtEnd([
    "Sea of Thieves just made its biggest social gamble in years.",
    "Xbox Wire says Season 20's Custom Seas lets crews build private sessions, set rules, spawn treasure and enemies, change loadouts and assign up to 24 players.",
    "That sounds perfect for streamers, training runs and players who hate being ambushed.",
    "But it cuts into what makes Sea of Thieves electric: strangers can ruin your plan at any second.",
    "If the best nights go private, public servers could feel quieter and less dangerous.",
    "Players get control, but the shared ocean loses chaos.",
    "That is the trade-off.",
    "That risk turns Rare's best creator tool into Sea of Thieves' biggest community split.",
  ].join(" "), cta));
}
...
function preserveVoiceCadenceScript(canonical = {}, job = {}, baseScript = "") {
  ...
  if (classifyStoryIntent(canonical) === "social_mode_tradeoff") {
    const finalScript = socialModeVoiceCadenceScript(canonical, cta);
    return {
      script: finalScript,
      original_word_count: wordCount(baseScript),
      repaired_word_count: wordCount(finalScript),
...
function extendScriptToTarget(canonical = {}, job = {}) {
  ...
  const sourceRequiresScriptRepair = asArray(job.source_blockers).some((blocker) =>
  ...
  if (
    ...
    finalSentenceHasPulseCta(cleanBase) &&
    !durationScorecardHoldBlockers(durationRepairScriptScorecard(canonical, cleanBase)).length
  ) {
