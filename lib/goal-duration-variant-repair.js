...
function extensionSentencesFor(canonical = {}) {
  ...
  if (intent === "deal_or_access") {
    ...
    if (/\bpremium edition\b/.test(dealText)) {
      return [
        `${subject} Premium Edition is turning early access into the first real launch beat.`,
  ...
  if (intent === "release_or_showcase") {
    ...
    if (/\bhades ii\b/.test(text)) {
      ...
      return [
        claimSentence,
        `${subject} is no longer just a PC early-access story waiting for a console footnote.`,
        platformReach,
        "The feel is the pressure point: laggy dodges would blunt Hades II.",
    ...
    if (/\bthe expanse\b/.test(text)) {
      return [
        `${subject} finally has gameplay footage on screen.`,
        claimSentence,
    ...
    if (/\bstranger than heaven\b/.test(text)) {
      return [
        ...
        "Gameplay will decide whether the time jumps change the missions or just the wardrobe.",
      ];
    }
    if (/\bstar wars\b/.test(text) && /\bracer\b/.test(text)) {
      return [
        `${subject} now has a date leak attached to the nostalgia.`,
        claimSentence,
        "A racer does not need a huge lore pitch; it needs a date players can check and a reason the old arcade energy still matters.",
  ...
  if (intent === "preview_impression") {
    return [
      `${subject} sounds strongest when the squad survives between missions, not just inside them.`,
      claimSentence,
...
function durationFallbackSentencesFor(canonical = {}) {
  ...
  if (intent === "release_or_showcase") {
    ...
    if (/\bstar wars\b/.test(text) && /\bracer\b/.test(text)) {
      return [
        "An official listing would settle the date without turning the leak into a verdict.",
        "Until then, the story is the timing slip and the old arcade name coming back.",
      ];
    }
  }
  return [
...
function durationThumbnailNeedsRepair(canonical = {}) {
  ...
  const importantTokens = subject
    .split(/\s+/)
    .filter((token) => token.length >= 4 && !/^(the|and|with|from|that)$/i.test(token));
  if (!importantTokens.length) return false;
  return !importantTokens.some((token) => lowerText(headline).includes(lowerText(token)));
}
...
function compactPurposeSentences(canonical = {}, subject = "", claim = "") {
  ...
  if (intent === "release_or_showcase") {
    return [
      `A better showing would make ${subject || "the game"} readable fast: combat, UI, platform list and date.`,
...
function extendScriptToTarget(canonical = {}, job = {}) {
  ...
  const baseMentionsSubject = !subjectForBase || includesText(rawBase, subjectForBase);
  ...
  const targetWords = isNormalContentRepair
    ? Math.max(computedTargetWords, /\blocal\b/i.test(cleanText(job.provider)) ? 116 : 105)
    : computedTargetWords;
  ...
  if (wordCount(nextScript) < bodyTargetWords) {
    ...
    for (const fallback of fallbackCandidates) {
      if (wordCount(nextScript) >= bodyTargetWords) break;
      if (!includesText(nextScript, fallback)) nextScript = `${nextScript} ${fallback}`.trim();
    }
  }
...
function repairJobWithCurrentRenderDuration(job = {}, renderManifest = {}, scriptQualityNeedsRepair = false) {
  const currentDuration = renderDurationFromManifest(renderManifest);
...
async function repairDurationVariantJob(job = {}, options = {}) {
  ...
  try {
    ...
    return {
      ...
      blockers: durationStatus.blocker ? [durationStatus.blocker] : [],
      artifact_dir: validated.artifactDir,
