...
const { APPROVED_PULSE_CTAS } = require("./pulse-cta");
...
const TINY_DURATION_UNDERFLOW_PAD_MAX_SECONDS = 1.25;
const TINY_DURATION_UNDERFLOW_PAD_MARGIN_SECONDS = 0.25;
...
const FLAT_CREATIVE_RESIDUE_RE =
  /\b(?:the platform list is the point|a longer public showing would say more than another quick trailer|the console version needs to feel as sharp as the PC build|controller feel will decide whether the port lands like launch or just a late arrival|the trailer gives players a first read on pace,\s*camera and combat|the real catch is whether a full mission keeps that energy once the montage stops|a sharper play section would do more than another fast trailer sweep|that is where the next reveal can turn curiosity into trust|still needs one uninterrupted play beat before the hype fully settles)\b/i;
const SAFE_MOTION_RIGHTS_RE =
  /\b(?:owned_generated_editorial_motion_graphic|owned_generated_motion|official_reference_transformative_short|official_reference_transformative_editorial_use|source_documented_transformative_editorial_use|steam_storefront_promotional_editorial_use|screenshot_derived_editorial_motion|screenshot_derived_motion_clip|approved_for_transformative_editorial_use)\b/i;
...
function normaliseGtaViText(value = "") {
  return cleanText(value)
    .replace(
      /\b(?:G\.?\s*T\.?\s*A\.?|Grand\s+Theft\s+Auto)\s+s(?:i|y|igh)?[-\s]*six\b/gi,
      "Grand Theft Auto VI",
    )
    .replace(/\bG\.?\s*T\.?\s*A\.?\s*(?:V\s*I|VI|6|six)\b/gi, "Grand Theft Auto VI")
    .replace(/\bGrand\s+Theft\s+Auto\s*(?:V\s*I|VI|6|six)\b/gi, "Grand Theft Auto VI");
}

function normaliseAsrFragileDurationPhrases(value = "") {
  return cleanText(normaliseGtaViText(value)
    .replace(
      /\bGUILTY\s+GEAR\s*[-–—]*\s*STRIVE\s*[-–—]*\s+Robo[-\s]?Ky\s+Official\b\s*/gi,
      "Guilty Gear Strive ",
    )
    .replace(/\bGUILTY\s+GEAR\s*[-–—]*\s*STRIVE(?:\s*[-–—])?\b/gi, "Guilty Gear Strive")
    .replace(/\bGuilty\s+Gear\s*[-–—:]\s*Strive\b/gi, "Guilty Gear Strive")
    .replace(/\bGuilty\s+Gear\s+Strive\s+Robo[-\s]?Ky\s+official\s+footage\b/gi, "Guilty Gear Strive footage for the new fighter")
    .replace(/\bRobo[-\s]?Ky\s+official\b/gi, "the new fighter")
    .replace(/\bRobo[-\s]?Ky\b/gi, "the new fighter")
    .replace(/\bRobo\s+Key\b/gi, "the new fighter"));
}
...
function durationRepairThumbnailHeadline(title = "", subject = "") {
  ...
  if (/\b(?:has|had|with)\s+one\s+catch\b|\bone\s+catch\b/i.test(titleText)) {
    return durationHeadlineFromParts(subjectText, "Catch");
  }
...
function thumbnailWordCount(value = "") {
  return cleanText(value).split(/\s+/).filter(Boolean).length;
}

function isUsablePlatformNativeThumbnail(value = "") {
  const text = cleanText(value).toUpperCase();
  if (!text || text.length > 28) return false;
  const words = thumbnailWordCount(text);
  if (words < 2 || words > 4) return false;
  if (containsBannedPublicPhrase(text)) return false;
  if (/^(SOURCE|UPDATE|WATCH|GAMING NEWS|BREAKING NEWS)$/i.test(text)) return false;
  return /[A-Z0-9]/.test(text);
}

function durationRepairPreferredThumbnailHeadline(canonical = {}) {
  const firstFrame = cleanText(canonical.first_frame_text);
  const explicit = firstFrame || cleanText(canonical.suggested_thumbnail_text);
  if (!firstFrame || !isUsablePlatformNativeThumbnail(explicit)) return "";
  return explicit.toUpperCase();
}

function platformNativeThumbnailForDurationRepair(canonical = {}) {
  const explicitRepair = durationRepairPreferredThumbnailHeadline(canonical);
  if (explicitRepair) return explicitRepair;
  if (!cleanText(canonical.platform_native_thumbnail_repaired_at)) return "";
  const candidates = [
  ...
  const selected = candidates.map(cleanText).find(isUsablePlatformNativeThumbnail);
...
function classifyStoryIntent(canonical = {}) {
  const text = lowerText(
    [
      canonical.selected_title,
      canonical.canonical_title,
      cleanText(canonical.canonical_angle).replace(/\bsource_locked_update\b/gi, ""),
      canonical.description,
      ...asArray(canonical.confirmed_claims),
    ].join(" "),
  );
  ...
  if (/\b(?:vanguard|anti[- ]?cheat|cheaters?|bricking|paperweights?|trust problem)\b/.test(text)) {
    return "anti_cheat_trust";
  }
  ...
  if (/\bhalo\b/.test(text) && /\b(?:xbox account|gamertag|ps plus|split[- ]screen|co[- ]op|account catch)\b/.test(text)) {
    return "platform_account_friction";
  }
  if (/\b(?:custom seas|private sessions?|own rules|private seas|shared world)\b/.test(text)) {
    return "social_mode_tradeoff";
  }
  if (/\b(?:xcom|mass effect|permadeath|turn-based tactics|more than just star wars xcom)\b/.test(text)) {
    return "preview_impression";
  }
  if (/\b(?:playable demo|demo available|hands[- ]on report|hands[- ]on preview)\b/.test(text) && /\bdemo\b/.test(text)) {
    return "playable_demo";
  }
  if (
  ...
  if (/\b(?:pre[- ]?orders?|preorder)\b/.test(text) && /\b(?:price|edition|bonus|rockstar|gta\s*6|grand theft auto)\b/.test(text)) {
    return "preorder_price_risk";
  }
...
function scriptSubjectFor(canonical = {}) {
  ...
  const subject = normaliseAsrFragileDurationPhrases(canonical.canonical_subject || canonical.canonical_game);
...
function sourceSafeClaimForExtension(canonical = {}, subject = "") {
  ...
  if (/\b(?:gta\s*6|grand theft auto)\b/.test(text) && /\bpre[- ]?orders?\b/.test(text) && /\bjune\s*25\b/.test(text)) {
    return "Rockstar confirmed Grand Theft Auto VI preorders begin on June 25, with prices, versions and bonuses carrying the value risk.";
  }
  if (/\bhalo\b/.test(text) && /\b(?:xbox account|gamertag|ps plus|split[- ]screen|co[- ]op|account catch)\b/.test(text)) {
    return "Halo: Campaign Evolved PS5 players will need an Xbox account and gamertag, plus PS Plus for split-screen co-op.";
  }
  ...
  if (/\bguilty gear\b/.test(text) && /\brobo[-\s]?ky\b/.test(text)) {
    return "GameSpot showed new Guilty Gear Strive footage for its new fighter reveal.";
  }
  if (/\bpragmata\b/.test(text) && /\b(?:ai[- ]?look|ai generated|handmade|human developers?|new york stage)\b/.test(text)) {
  ...
  if (/\b(?:sea of thieves|custom seas|private sessions?|own rules|shared world)\b/.test(text)) {
    return "Custom Seas will let crews create private sessions with their own rules.";
  }
  ...
  if (/\bdragonwilds\b|\bsurvival spin[- ]off\b/.test(text)) {
    return "RuneScape: Dragonwilds has another major update coming later this month before full launch.";
  }
  ...
  return cleanText(normaliseAsrFragileDurationPhrases(compactWarhammer));
}
...
function sourceSafeDescriptionForDuration(canonical = {}) {
  ...
  if (
    ...
    if (!subject || durationScriptMentionsSubject(description, subject)) return description;
    return cleanText(`${subject} just turned this update into a real player debate. ${description}`);
  }
  const source = cleanText(canonical.primary_source || canonical.official_source || canonical.source_card_label || "the source");
  const claim = sourceSafeClaimForExtension(canonical, scriptSubjectFor(canonical));
...
function extensionSentencesFor(canonical = {}) {
  ...
  if (intent === "preorder_price_risk") {
    const existingScript = lowerText(
      [
        canonical.narration_script,
        canonical.full_script,
    ...
    const sourceAlreadyCovered =
      source &&
      lowerText(source) !== "the source" &&
      existingScript.includes(lowerText(source)) &&
      /\bpre[- ]?orders?\s+open\s+on\s+june\s*25\b/.test(existingScript);
    return [
      `${subject || "Grand Theft Auto VI"} just turned cover art into a checkout countdown.`,
      "Players have to decide whether Rockstar has shown enough to trust an early preorder, because if the page feels messy, the price fight starts before launch.",
      "If the store page feels fair, hype becomes a clean runway; if it feels messy, gaming's safest blockbuster starts with a value fight.",
      "That turns the cover into a value fight before release day.",
      sourceAlreadyCovered ? "" : claimSentence,
      "Jason and Lucia are the box-art promise Rockstar is asking players to trust before launch.",
      "Prices, versions and bonuses have to make early commitment feel sensible, not just inevitable.",
    ].filter(Boolean);
  }
  ...
  if (intent === "social_mode_tradeoff") {
    return [
      "Season 20 is not just adding a private lobby.",
      "Custom Seas can change creature spawns, time of day and weapon loadouts.",
      "Crews can summon enemies, spawn treasure and move ships around the map.",
      "Rare also says players can assign up to 24 players to crews and use Silver scoreboards for custom contests.",
      "That is huge for streamers, events and friend groups that want custom chaos.",
      "The catch is what happens if the best sessions move into private modes.",
      "Public seas could start feeling emptier.",
    ];
  }
  if (intent === "patch_or_performance") {
    const patchText = lowerText(
      [
        subject,
        canonical.selected_title,
        canonical.canonical_title,
    ...
    if (/\bdragonwilds\b|\bsurvival spin[- ]off\b|\bearly access\b/.test(patchText)) {
      return [
        "It has to prove this is a RuneScape game people can actually main, not a side experiment they sample for one weekend.",
        "If that trust test lands, launch starts with a reason to return.",
        "That makes full launch a real trust test, not just another patch headline.",
        "That decides whether players keep Dragonwilds installed after launch or treat it as another weekend test.",
      ];
    }
    return [
  ...
  if (intent === "playable_demo") {
    return [
      `${subject} is not just asking players to trust the trailer now.`,
      claimSentence,
      "The useful part is access: players can test the feel instead of arguing from a montage.",
      "If the demo makes the loop click, lapsed players have a real reason to come back.",
      "If it feels flat, fans find that out before putting more time into the grind.",
    ];
  }
  ...
  if (intent === "anti_cheat_trust") {
    return [
      `The catch is what this changes for ${subject} players: Riot can dunk on cheaters and still leave normal players asking how much control Vanguard should have.`,
      claimSentence,
      "Riot's punchline works because it targets cheaters, but the anti-cheat still sits at the centre of the player reaction.",
      "That pressure is why a security win can still feel uncomfortable when the software runs this deep.",
      "What this changes is how every Vanguard scare lands: players stop arguing only about cheaters and start arguing about trust.",
      "That is why one sarcastic Riot line can turn into a wider trust argument.",
    ];
  }
  if (intent === "release_or_showcase") {
    ...
    if (/\bhalo\b/.test(text) && /\bcampaign evolved\b/.test(text)) {
      return [
        claimSentence,
        "That comparison matters because Halo remakes live or die on muscle memory, not prettier lighting.",
        "If the pistol rhythm feels wrong, Xbox has a remake that looks modern but loses the trust fans needed it to prove.",
      ];
    }
    return [
...
function durationFallbackSentencesFor(canonical = {}) {
  ...
  if (intent === "anti_cheat_trust") {
    return [
      "Riot's next useful detail is whether this stays a cheater-only punchline or becomes a wider trust issue.",
      "That is the part Valorant players will carry into the next update.",
    ];
  }
  if (intent === "deal_or_access") {
    return [
  ...
  if (intent === "release_or_showcase") {
    ...
    if (/\bhalo\b/.test(text) && /\bcampaign evolved\b/.test(text)) {
      return [
        "But if that full mission keeps the pistol rhythm, the remake starts looking like trust rebuilt instead of Xbox nostalgia with prettier lighting.",
      ];
    }
...
function staleDragonwildsDurationResidue(sentence = "", fullText = "") {
  const text = lowerText(fullText);
  if (!/\bdragonwilds\b/.test(text)) return false;
  const line = lowerText(sentence);
  return (
    /\bahead of its\s*1\.?0 launch\b/.test(line) ||
    /\bdragonwilds has to show up in normal play\b/.test(line) ||
    /\breaction clips after launch will decide\b/.test(line) ||
    /\bshort a gameplay consequence instead of a checklist\b/.test(line) ||
    /\bpatch-size hype\b/.test(line) ||
    /\bweekend curiosity players either keep installed or delete\b/.test(line) ||
    /\basking if the loop has finally clicked\b/.test(line) ||
    /\bside experiment they try for a weekend and leave\b/.test(line) ||
    /\bif the loop feels sharper\b/.test(line)
  );
}
...
function stripFormulaicDurationResidue(value = "") {
  const kept = sentenceList(value).filter((sentence) =>
    ...
    !staleDragonwildsDurationResidue(sentence, value)
  );
  return cleanText(kept.join(" "));
}

function normaliseTtsRiskyDurationPhrases(value = "") {
  return normaliseAsrFragileDurationPhrases(value)
    .replace(
      /\bIf not,\s*it splits the ocean in two\./gi,
      "That is the fault line Rare has to manage.",
    )
    .replace(
      /\bGrand Theft Auto VI preorders just became a real buying decision\./gi,
      "Rockstar's preorder date just became a real buying decision.",
    )
    .replace(
      /\bThe risk is not whether Grand Theft Auto VI will be huge;/gi,
      "The risk is not whether Rockstar's next game will be huge;",
    )
    .replace(
      /\bPlayers have to decide whether Rockstar has shown enough to trust an early preorder\./gi,
      "Players have to decide whether Rockstar has shown enough to trust an early preorder, because if the page feels messy, the price fight starts before launch.",
    )
    .replace(/\bprice,\s*editions,\s*bonuses\b/gi, "prices, versions and bonuses")
    .replace(/\bprice,\s*editions\s+and\s+bonuses\b/gi, "prices, versions and bonuses");
}
...
function appendCtaAtEnd(script = "", cta = "") {
  const { body } = splitScriptAndCta(script);
  const optionalCta = sentenceWithPeriod(cta);
  return normaliseScriptPunctuation(
    optionalCta ? `${body} ${optionalCta}` : body,
  );
}
...
function needsDurationScriptQualityRepair(canonical = {}) {
  ...
  if (finalSentenceHasPulseCta(script)) return true;
...
const DURATION_SCRIPT_SCORECARD_HOLD_BLOCKERS = new Set([
  "repeated_phrase",
  "generic_reveal_catch_template",
  "vague_update_without_concrete_proof",
  "soft_payoff_language",
]);
const DURATION_CURRENT_REPAIR_BLOCKERS = new Set([
  ...DURATION_SCRIPT_SCORECARD_HOLD_BLOCKERS,
  "missing_story_specific_payoff",
  "generic_catch_is_simple",
  "internal_audience_scaffold",
  "producer_scaffold_language",
]);

const VOICE_CADENCE_PRESERVE_DISQUALIFYING_BLOCKERS = new Set([
  "duplicated_source_attribution",
  "repeated_phrase",
  "source_title_recitation",
]);

function durationScorecardHoldBlockers(scorecard = {}) {
  return asArray(scorecard.blockers)
    .map(cleanText)
    .filter((blocker) => DURATION_SCRIPT_SCORECARD_HOLD_BLOCKERS.has(blocker));
}

function durationCurrentRepairBlockers(scorecard = {}) {
  return asArray(scorecard.blockers)
    .map(cleanText)
    .filter((blocker) => DURATION_CURRENT_REPAIR_BLOCKERS.has(blocker));
}

function durationScriptScorecardNeedsRepair(canonical = {}, script = "") {
  const scorecard = buildViralScriptIntelligence({
    story: {
      id: cleanText(canonical.story_id || canonical.id),
      ...
      source_name: cleanText(canonical.primary_source || canonical.source_card_label || canonical.official_source),
      source_card_label: cleanText(canonical.source_card_label),
    },
    script,
    ctaPolicy: "optional",
  });
  return durationScorecardHoldBlockers(scorecard).length > 0;
}

function durationVoiceCadencePreserveBlockers(canonical = {}, script = "") {
  const scorecard = buildViralScriptIntelligence({
    story: {
      id: cleanText(canonical.story_id || canonical.id),
      title: cleanText(canonical.selected_title || canonical.title),
  ...
  return asArray(scorecard.blockers)
    .map(cleanText)
    .filter((blocker) => VOICE_CADENCE_PRESERVE_DISQUALIFYING_BLOCKERS.has(blocker));
}

function durationRepairStatusWithScriptScorecard(durationStatus = {}, scriptScorecard = {}) {
  const scriptBlockers = durationScorecardHoldBlockers(scriptScorecard);
...
function isVoiceCadenceFastRepair(job = {}) {
  return (
    cleanText(job.repair_lane) === "voice_cadence_compaction" ||
    asArray(job.source_blockers).some((blocker) => /voice_cadence:wpm_too_fast/i.test(blocker))
  );
}

function durationRepairTtsRate(job = {}, explicitRate = null) {
  const parsed = Number(explicitRate);
  if (Number.isFinite(parsed) && parsed > 0) return parsed;
  const jobRate = Number(job.tts_rate || job.ttsRate);
  if (Number.isFinite(jobRate) && jobRate > 0) return jobRate;
  if (isVoiceCadenceFastRepair(job)) return 0.92;
  return null;
}

function durationRepairLocalTtsSegmentationOptions(options = {}, extension = {}) {
  const provider = cleanText(options.provider || "auto").toLowerCase();
  if (options.generateTtsForStory && options.localTtsSegmentedMaterializer === undefined) return {};
  const localCapableProvider = !provider || provider === "auto" || provider === "local";
  const repairedWords = Number(extension.repaired_word_count || extension.word_count);
  const shouldDefaultSegment =
    localCapableProvider &&
    Number.isFinite(repairedWords) &&
    repairedWords >= 105 &&
    options.localTtsSegmentedMaterializer === undefined;
  if (!shouldDefaultSegment && options.localTtsSegmentedMaterializer === undefined) return {};
  return {
    localTtsSegmentedMaterializer: options.localTtsSegmentedMaterializer ?? true,
    localTtsSegmentedWordThreshold: options.localTtsSegmentedWordThreshold ?? 45,
    localTtsSegmentMaxWords: options.localTtsSegmentMaxWords ?? 26,
    localTtsRetrySegmentMaxWords: options.localTtsRetrySegmentMaxWords ?? 22,
    localTtsFinalRetrySegmentMaxWords: options.localTtsFinalRetrySegmentMaxWords ?? 18,
    localTtsSegmentGapS: options.localTtsSegmentGapS ?? 0.08,
  };
}
...
function compactOpeningSubjectAlias(value = "") {
  ...
  if (/\bhalo\s*:?\s*campaign evolved\b/i.test(raw)) return "Halo";
...
function compactPurposeSentences(canonical = {}, subject = "", claim = "") {
  ...
  if (intent === "release_or_showcase") {
    ...
    if (/\bhalo\b/.test(text) && /\bcampaign evolved\b/.test(text)) {
      return [
        "That matters because Halo lives on muscle memory.",
        "For fans, the pistol rhythm, enemy dance and movement speed are the whole point.",
        "That is where the remake either rebuilds trust or loses it.",
        "If that feel changes, Xbox has a remake that looks modern but loses trust.",
        "If it holds, old players get a reason to return.",
        "New players get the cleanest entry point.",
        `But if the rhythm survives, ${subject || "Campaign Evolved"} starts looking like trust rebuilt instead of prettier nostalgia.`,
      ];
    }
    return [
  ...
  if (intent === "platform_account_friction") {
    return [
      "The catch is friction, not cancellation.",
      "PlayStation players get the campaign, but the first setup still pulls them through Xbox identity.",
      "That matters most for split-screen co-op, because nostalgia only works if the room can start playing fast.",
      "Check the account and PS Plus rules before you buy.",
      "If setup is painless, Halo gets a cleaner cross-platform moment.",
      "If it is not, the first fight starts before the campaign does.",
    ];
  }
  if (intent === "preorder_price_risk") {
    return [
      "The risk is not whether Rockstar's next game will be huge; it is whether the first store page makes the value clear.",
      "Prices, versions and bonuses decide whether fans buy early, wait or argue about the premium version.",
      "Jason and Lucia on the cover makes the marketing feel locked, but the real test is what the announced versions actually include.",
      "If the page feels messy, preorders open the price fight before launch.",
    ];
  }
  if (intent === "social_mode_tradeoff") {
    return [
  ...
  if (intent === "patch_or_performance") {
    ...
    if (/\b(?:sea of thieves|custom seas|private sessions?|own rules)\b/.test(text)) {
      return [
        "Custom Seas can change creature spawns, time of day and weapon loadouts.",
        "Crews can summon enemies, spawn treasure and move ships around the map.",
        "That makes it a creator tool, not just a private lobby.",
      ];
    }
    if (/\bdragonwilds\b|\bsurvival spin[- ]off\b|\bearly access\b/.test(text)) {
      return [
        "It has to prove this is a RuneScape game people can actually main, not a side experiment they sample for one weekend.",
        "That makes full launch a real trust test, not just another patch headline.",
      ];
...
function compactScriptToTarget(canonical = {}, job = {}, baseScript = "") {
  const subject = scriptSubjectFor(canonical);
  const publicSubject = normaliseGtaViText(canonical.canonical_subject || canonical.canonical_game || subject);
  const source = cleanText(canonical.primary_source || canonical.source_card_label || "the source");
  const claim = sourceSafeClaimForExtension(canonical, subject);
  ...
  const declaredFirstLine = cleanText(canonical.first_spoken_line || canonical.narration_hook);
  const declaredFirstLineMatches =
    declaredFirstLine &&
    (
      (publicSubject && includesText(declaredFirstLine, publicSubject)) ||
      (subject && includesText(declaredFirstLine, subject)) ||
      (openingAlias && includesText(declaredFirstLine, openingAlias))
    );
  const firstSentence = (declaredFirstLineMatches ? declaredFirstLine : "") ||
    existingSentences.find((sentence) => publicSubject && includesText(sentence, publicSubject)) ||
  ...
  const compactFirstSentence = classifyStoryIntent(canonical) === "preorder_price_risk"
    ? "Grand Theft Auto VI just turned cover art into a preorder fight."
    : firstSentence;
  const candidates = [
    compactFirstSentence,
...
function socialModeVoiceCadenceScript(canonical = {}, baseScript = "", cta = "") {
  const sourceBoundScript = cleanText(
    baseScript ||
    canonical.narration_script ||
    canonical.full_script ||
    canonical.tts_script,
  );
  if (sourceBoundScript) {
    return normaliseProtectedNamesText(appendCtaAtEnd(sourceBoundScript, cta));
  }

  const firstLine = cleanText(canonical.first_spoken_line || canonical.narration_hook);
  const claim = asArray(canonical.confirmed_claims).map(cleanText).find(Boolean);
  const source = cleanText(canonical.primary_source || canonical.source_card_label);
  const fallback = dedupeDurationScriptSentences([
    firstLine,
    claim ? sourceLeadSentence(source || "the source", claim) : "",
  ].filter(Boolean).join(" "));
  return normaliseProtectedNamesText(appendCtaAtEnd(fallback, cta));
}

function preserveVoiceCadenceScript(canonical = {}, job = {}, baseScript = "") {
  const target = job.target_duration_seconds || DEFAULT_TARGET_SECONDS;
  const min = Number(target.min || DEFAULT_TARGET_SECONDS.min);
  const currentDuration = Number(job.current_duration_s);
  if (!isVoiceCadenceFastRepair(job) || !Number.isFinite(currentDuration) || currentDuration < min) {
    return null;
  }
  const cleanBase = dedupeDurationScriptSentences(
    normaliseDurationBaseScript(
      canonical,
      normaliseTtsRiskyDurationPhrases(stripFormulaicDurationResidue(baseScript)),
    ),
  );
  const { body: baseBody, cta } = splitScriptAndCta(cleanBase);
  if (classifyStoryIntent(canonical) === "social_mode_tradeoff") {
    const finalScript = socialModeVoiceCadenceScript(canonical, baseBody, cta);
    return {
      script: finalScript,
      original_word_count: wordCount(baseScript),
      repaired_word_count: wordCount(finalScript),
      target_word_count: wordCount(finalScript),
      appended_word_count: Math.max(0, wordCount(finalScript) - wordCount(baseScript)),
      removed_word_count: Math.max(0, wordCount(baseScript) - wordCount(finalScript)),
    };
  }
  const preserveDisqualifyingBlockers = durationVoiceCadencePreserveBlockers(canonical, cleanBase);
  if (preserveDisqualifyingBlockers.length) {
    const compact = compactScriptToTarget(
      canonical,
      {
        ...job,
        source_blockers: [
          ...asArray(job.source_blockers),
          ...preserveDisqualifyingBlockers.map((blocker) => `script_scorecard:${blocker}`),
        ],
      },
      cleanBase,
    );
    return {
      ...compact,
      original_word_count: wordCount(baseScript),
      appended_word_count: Math.max(0, compact.repaired_word_count - wordCount(baseScript)),
      removed_word_count: Math.max(0, wordCount(baseScript) - compact.repaired_word_count),
    };
  }
  if (wordCount(cleanBase) >= 98) {
    const finalScript = normaliseProtectedNamesText(appendCtaAtEnd(baseBody, cta));
    return {
      script: finalScript,
      original_word_count: wordCount(baseScript),
      repaired_word_count: wordCount(finalScript),
  ...
  const claim = sourceSafeClaimForExtension(canonical, scriptSubjectFor(canonical));
  let nextScript = baseBody;
  for (const sentence of extensionSentencesFor(canonical)) {
    if (wordCount(nextScript) >= 98 && hasRelatableDurationStakes(nextScript)) break;
    if (!extensionSentenceAlreadyCovered(nextScript, sentence, claim)) {
      nextScript = cleanText(`${nextScript} ${sentence}`);
    }
  }
...
function hasRelatableDurationStakes(script = "") {
  return /\bplayers?\b.{0,120}\b(?:get|play|try|choose|decide|lose|trust|return|care|argue|split|feel)\b/i.test(script) ||
    /\bpublic crews?\b.{0,120}\b(?:lose|get|feel|split|return|play)\b/i.test(script);
}

function durationRepairScriptScorecard(canonical = {}, script = "") {
...
function durationRepairSourceScript(canonical = {}) {
  const primary = cleanText(canonical.narration_script || canonical.full_script || canonical.tts_script);
  const seen = new Set();
  const candidates = [
    canonical.narration_script,
    canonical.full_script,
    ...
    .filter(Boolean)
    .filter((script) => {
      const key = lowerText(script);
      if (seen.has(key)) return false;
      seen.add(key);
      return true;
    })
    .map((script) => {
      const scorecard = durationRepairScriptScorecard(canonical, script);
      return {
        script,
        word_count: wordCount(script),
        has_cta: finalSentenceHasPulseCta(script),
        banned: containsBannedPublicPhrase(script),
        hold_blockers: durationScorecardHoldBlockers(scorecard),
      };
    });
  const primaryInfo = candidates.find((candidate) => candidate.script === primary) || {
    script: primary,
    word_count: wordCount(primary),
    has_cta: finalSentenceHasPulseCta(primary),
    banned: containsBannedPublicPhrase(primary),
    hold_blockers: [],
  };
  const best = candidates
    .filter((candidate) =>
      !candidate.banned &&
      candidate.hold_blockers.length === 0,
    )
  ...
  if (
    ...
    (
      primaryInfo.banned ||
      primaryInfo.hold_blockers.length > 0 ||
      primaryInfo.word_count < Math.min(85, best.word_count - 10)
    )
  ) {
    return best.script;
  }
...
function durationRepairSourceScriptIsAlternate(canonical = {}, script = "") {
  const primary = cleanText(canonical.narration_script || canonical.full_script || canonical.tts_script);
  const selected = cleanText(script);
  return Boolean(primary && selected && selected !== primary);
}

function durationScriptMentionsSubject(script = "", subject = "") {
  const text = lowerText(script);
  const cleanSubject = lowerText(subject);
  if (!cleanSubject) return true;
  if (includesText(text, cleanSubject)) return true;
  const tokens = cleanSubject
    .split(/\s+/)
    .map((token) => token.replace(/[^a-z0-9]+/g, ""))
    .filter((token) => token.length >= 4 && !/^(from|that|with|into|gets|just)$/.test(token));
...
function extendScriptToTarget(canonical = {}, job = {}) {
  const originalScript = durationRepairSourceScript(canonical);
  const preservedVoiceCadence = preserveVoiceCadenceScript(canonical, job, originalScript);
  if (preservedVoiceCadence) return preservedVoiceCadence;
  const rawBase = normaliseDurationBaseScript(
    canonical,
    normaliseTtsRiskyDurationPhrases(stripFormulaicDurationResidue(originalScript)),
  );
  ...
  const baseMentionsSubject = !subjectForBase ||
    durationScriptMentionsSubject(rawBase, subjectForBase) ||
    (subjectAliasForBase && subjectAliasForBase !== subjectForBase && durationScriptMentionsSubject(rawBase, subjectAliasForBase));
  ...
  const requestedScriptRepair = asArray(job.source_blockers).some((blocker) =>
    /(?:script_scorecard|missing_story_specific_payoff|no_curiosity_marker|repeated_phrase)/i.test(cleanText(blocker)),
  );
  const currentScriptScorecardBlockers = durationCurrentRepairBlockers(
    durationRepairScriptScorecard(canonical, cleanBase),
  );
  const sourceRequiresScriptRepair =
    requestedScriptRepair && currentScriptScorecardBlockers.length > 0;
  ...
  if (
    !sourceRequiresScriptRepair &&
    wordCount(cleanBase) >= 98 &&
    currentScriptScorecardBlockers.length === 0
  ) {
    const finalScript = normaliseProtectedNamesText(appendCtaAtEnd(baseScript, cta));
    return {
      script: finalScript,
      original_word_count: wordCount(originalScript || cleanBase),
      repaired_word_count: wordCount(finalScript),
      target_word_count: wordCount(finalScript),
      appended_word_count: 0,
      removed_word_count: Math.max(0, wordCount(originalScript || cleanBase) - wordCount(finalScript)),
    };
  }
  const target = job.target_duration_seconds || DEFAULT_TARGET_SECONDS;
  ...
  const needsPayoffCompactRepair = isNormalContentRepair &&
    asArray(job.source_blockers).some((blocker) =>
      /script_scorecard:(?:missing_story_specific_payoff|script_verdict_rewrite_required)|missing_story_specific_payoff/i.test(blocker),
    );
  if (isNormalContentRepair && (wordCount(baseScript) > bodyTargetWords || needsPayoffCompactRepair)) {
    const compact = compactScriptToTarget(canonical, job, cleanBase);
    const originalWordCount = wordCount(originalScript || cleanBase);
    return {
      ...compact,
  ...
  for (const sentence of sentences) {
    ...
    if (!extensionSentenceAlreadyCovered(nextScript, sentence, claim)) {
      const candidate = `${nextScript} ${sentence}`.trim();
      if (wordCount(nextScript) >= 72 && wordCount(candidate) > targetWords + 6) break;
      nextScript = candidate;
    }
  }
  if (wordCount(nextScript) < bodyTargetWords) {
    ...
    for (const fallback of fallbackCandidates) {
      ...
      if (!extensionSentenceAlreadyCovered(nextScript, fallback, claim)) {
        const candidate = `${nextScript} ${fallback}`.trim();
        if (wordCount(nextScript) >= 72 && wordCount(candidate) > targetWords + 6) break;
        nextScript = candidate;
      }
    }
...
async function updateCanonicalForDurationRepair({
  ...
  const headline =
    platformNativeThumbnailForDurationRepair(canonical) ||
    durationRepairThumbnailHeadline(canonical.selected_title || job.title, subject);
  ...
  const updated = normaliseProtectedNamesInDurationManifest({
    ...
    suggested_thumbnail_text: headline || canonical.suggested_thumbnail_text,
    first_frame_text: headline || canonical.first_frame_text,
...
function tinyDurationTailPaddingPlan(manifest = {}, target = DEFAULT_TARGET_SECONDS) {
  const duration = renderDurationFromManifest(manifest);
  const min = Number(target.min || DEFAULT_TARGET_SECONDS.min);
  const max = Number(target.max || DEFAULT_TARGET_SECONDS.max);
  if (!Number.isFinite(duration) || !Number.isFinite(min)) return null;
  if (duration >= min) return null;
  const underflow = min - duration;
  if (underflow <= 0 || underflow > TINY_DURATION_UNDERFLOW_PAD_MAX_SECONDS) return null;
  const targetDuration = Number.isFinite(max)
    ? Math.min(min + TINY_DURATION_UNDERFLOW_PAD_MARGIN_SECONDS, max)
    : min + TINY_DURATION_UNDERFLOW_PAD_MARGIN_SECONDS;
  if (!Number.isFinite(targetDuration) || targetDuration <= duration) return null;
  return {
    current_duration_s: roundDurationSeconds(duration),
    min_duration_s: roundDurationSeconds(min),
    target_duration_s: roundDurationSeconds(targetDuration),
    underflow_seconds: roundDurationSeconds(underflow),
    padding_seconds: roundDurationSeconds(targetDuration - duration),
  };
}

async function padRenderTailToTarget({
  artifactDir,
  renderManifest = {},
  generatedAt,
  plan,
  ...
  ffprobeDurationImpl = ffprobeDuration,
} = {}) {
  if (!artifactDir) throw new Error("tail_duration_padding_requires_artifact_dir");
  if (!plan?.padding_seconds) throw new Error("tail_duration_padding_requires_plan");
  const outputPath = path.resolve(
    renderManifest.output_path ||
      path.join(artifactDir, renderManifest.output || "visual_v4_render.mp4"),
  );
  if (!await fs.pathExists(outputPath)) {
    throw new Error(`tail_duration_padding_missing_render:${outputPath}`);
  }
  const parsed = path.parse(outputPath);
  const backupPath = path.join(parsed.dir, `${parsed.name}.pre_duration_tail_pad${parsed.ext}`);
  const tmpPath = path.join(parsed.dir, `${parsed.name}.tail_pad_tmp${parsed.ext}`);
  if (!await fs.pathExists(backupPath)) {
    await fs.copy(outputPath, backupPath);
  }
  await fs.remove(tmpPath);
  const pad = Number(plan.padding_seconds).toFixed(3);
  await execFileImpl(
    ffmpegPath,
    [
      "-y",
      ...
      "[a]",
      "-t",
      Number(plan.target_duration_s).toFixed(3),
      "-c:v",
      "libx264",
      "-pix_fmt",
      "yuv420p",
  ...
  const measuredDuration = Number(ffprobeDurationImpl(tmpPath));
  if (!Number.isFinite(measuredDuration)) {
    await fs.remove(tmpPath);
    throw new Error("tail_duration_padding_probe_failed");
  }
  if (measuredDuration < plan.min_duration_s) {
    await fs.remove(tmpPath);
    throw new Error(`tail_duration_padding_still_below_target:${measuredDuration.toFixed(3)}`);
  }
  ...
  const updatedManifest = {
    ...
    rendered_duration_s: roundDurationSeconds(measuredDuration),
    duration_s: roundDurationSeconds(measuredDuration),
    video_duration_s: roundDurationSeconds(measuredDuration),
    duration_tail_padded_at: generatedAt,
    duration_tail_padding_reason: "post_repair_tiny_underflow",
    duration_tail_padding_seconds: plan.padding_seconds,
    duration_tail_padding_source_duration_s: plan.current_duration_s,
    duration_tail_padding_target_duration_s: plan.target_duration_s,
    duration_tail_padding_backup_path: backupPath,
  };
  await fs.writeJson(renderManifestPath, updatedManifest, { spaces: 2 });
  return {
    status: "padded",
    renderManifest: updatedManifest,
...
async function maybePadTinyDurationUnderflow({
  artifactDir,
  renderManifest = {},
  target = DEFAULT_TARGET_SECONDS,
  generatedAt,
  padRenderTail = padRenderTailToTarget,
  ffmpegPath,
  execFileImpl,
  ffprobeDurationImpl,
} = {}) {
  const plan = tinyDurationTailPaddingPlan(renderManifest, target);
  if (!plan) {
    return {
      status: "not_needed",
      renderManifest,
...
function repairJobWithCurrentRenderDuration(job = {}, renderManifest = {}, scriptQualityNeedsRepair = false) {
  ...
  const preferWorkOrderDurationForVoiceCadence =
    Number.isFinite(jobDuration) &&
    isVoiceCadenceFastRepair(job) &&
    Number.isFinite(min) &&
    jobDuration >= min &&
    (!Number.isFinite(renderDuration) || renderDuration < min);
  const currentDuration = (preferWorkOrderDuration || preferWorkOrderDurationForVoiceCadence) ? jobDuration : renderDuration;
...
async function recoverVoiceCadenceScriptFromCoherence({ artifactDir, canonical = {}, job = {} } = {}) {
  if (!isVoiceCadenceFastRepair(job)) return canonical;
  const currentScript = cleanText(canonical.narration_script || canonical.full_script || canonical.tts_script);
  const currentWords = wordCount(currentScript);
  const originalWords = Number(canonical.duration_variant_extension?.original_word_count || 0);
  if (!originalWords || currentWords >= Math.max(88, originalWords - 12)) return canonical;
  const coherence = await readJsonIfPresent(path.join(artifactDir, "coherence_report.json"), {});
  const coherenceScript = cleanText(coherence.manifest?.narration_script);
  if (!coherenceScript || wordCount(coherenceScript) < Math.max(88, originalWords - 12)) return canonical;
  if (containsBannedPublicPhrase(coherenceScript)) return canonical;
  const restoredScript = normaliseProtectedNamesText(
    normaliseDurationBaseScript(canonical, normaliseTtsRiskyDurationPhrases(coherenceScript)),
  );
  if (!restoredScript || wordCount(restoredScript) <= currentWords) return canonical;
  const firstLine = sentenceList(restoredScript)[0] || cleanText(canonical.first_spoken_line || canonical.narration_hook);
  return {
    ...canonical,
    first_spoken_line: firstLine,
    narration_hook: firstLine,
    narration_script: restoredScript,
    full_script: restoredScript,
    tts_script: restoredScript,
    word_count: wordCount(restoredScript),
    voice_cadence_script_restored_from: "coherence_report",
  };
}
...
async function repairDurationVariantJob(job = {}, options = {}) {
  ...
  const voiceCadenceFastRepair = isVoiceCadenceFastRepair(job);
  const validatedCanonical = await recoverVoiceCadenceScriptFromCoherence({
    artifactDir: validated.artifactDir,
    canonical: validated.canonical,
    job,
  });
  const scriptQualityNeedsRepair = needsDurationScriptQualityRepair(validatedCanonical) ||
  ...
  const ttsRate = durationRepairTtsRate(repairJob, options.ttsRate);
  if (
    existingDurationStatus.status === "warning_held" &&
    existingDurationStatus.blocker === "post_repair_duration_still_below_target" &&
    cleanText(validatedCanonical.duration_variant_repaired_at) &&
    !isContentSignalRepair(job) &&
    !voiceCadenceFastRepair &&
    !scriptQualityNeedsRepair &&
    !hasStaleDurationMetadataBlocker(job)
  ) {
    const tailPadding = await maybePadTinyDurationUnderflow({
      artifactDir: validated.artifactDir,
      renderManifest: existingRenderManifest,
      target,
      generatedAt,
      padRenderTail: options.padRenderTail,
      ffmpegPath: options.ffmpegPath,
      execFileImpl: options.execFileImpl,
      ffprobeDurationImpl: options.ffprobeDurationImpl,
    });
    if (tailPadding.status === "padded") {
      const renderManifest = tailPadding.renderManifest;
      ...
      const captionsPath = await writeDurationRepairCaptions({
        artifactDir: validated.artifactDir,
        script: validatedCanonical.narration_script || validatedCanonical.full_script || validatedCanonical.tts_script,
        renderManifest,
      });
      ...
      const durationStatus = durationStatusFromManifest(renderManifest, target);
      ...
      return {
        ...
        duration_tail_padding_seconds: tailPadding.padding_seconds,
        render_status: "tail_padded_existing_render",
        ...
        duration_contract_status: durationContract.status ||
          ...
          null,
        output_path: tailPadding.output_path || renderManifest.output_path || null,
      };
    }
  }
  if (
  ...
  try {
    ...
    const audioReport = await materializeGoalAudioTimestamps({
      ...
      ttsRate,
      ...durationRepairLocalTtsSegmentationOptions(options, canonicalUpdate.extension),
    });
    const audioFailed = asArray(audioReport.jobs).find((audioJob) => audioJob.status === "failed");
    if (audioFailed) {
      const error = new Error(`audio_repair_failed:${audioFailed.error || "unknown"}`);
      error.audio_failure = audioFailed;
      throw error;
    }
    ...
    let renderManifest = await readJsonIfPresent(path.join(validated.artifactDir, "render_manifest.json"), {});
    const tailPadding = await maybePadTinyDurationUnderflow({
      artifactDir: validated.artifactDir,
      renderManifest,
      target,
    ...
    if (tailPadding.status === "padded") renderManifest = tailPadding.renderManifest;
    const captionsPath = await writeDurationRepairCaptions({
      artifactDir: validated.artifactDir,
    ...
    try {
      qualityRefresh = await refreshFinalRenderQualityOnly({
        artifactDir: validated.artifactDir,
        storyId,
        generatedAt,
      });
      renderManifest = await readJsonIfPresent(path.join(validated.artifactDir, "render_manifest.json"), renderManifest);
    } catch (qualityError) {
      qualityRefresh = {
        status: "quality_refresh_failed",
    ...
    return {
      ...
      tts_rate: ttsRate,
      render_status: tailPadding.status === "padded" ? "rendered_and_tail_padded" : renderReport.jobs[0]?.status || null,
      duration_tail_padding_seconds: tailPadding.status === "padded" ? tailPadding.padding_seconds : null,
      captions_path: captionsPath,
      script_scorecard: scriptScorecard,
      quality_refresh_status: qualityRefresh?.status || null,
      quality_refresh_forensic_result: qualityRefresh?.forensic_result || null,
      quality_refresh_forensic_blockers: qualityRefresh?.forensic_blockers || [],
      quality_refresh_error: qualityRefresh?.error || null,
      duration_contract_status: durationContract.status ||
        durationContract.updated?.[0]?.status ||
        durationContract.blocked?.[0]?.status ||
        null,
...
function jobsForDurationRepair(workOrder = {}, { limit = 0, storyIds = [] } = {}) {
  ...
  let jobs = asArray(workOrder.jobs).filter(
    (job) => cleanText(job.status) === "needs_duration_variant_rerender" ||
      cleanText(job.status) === "needs_platform_duration_variant" ||
...
async function materializeDurationVariantRepairs({
  ...
  for (const job of jobs) {
    try {
      ...
      if (error.audio_failure) failed.audio_failure = error.audio_failure;
      results.push(failed);
    }
  }
  const externalTtsProviderUsed = results.some((job) =>
    job.audio_provider === "elevenlabs" ||
    job.audio_failure?.provider === "elevenlabs"
  )
    ? "elevenlabs"
    : null;
  return {
    ...
    safety: {
      no_publish_triggered: true,
      no_network_uploads: externalTtsProviderUsed ? false : true,
      no_db_mutation: true,
      no_oauth_or_token_change: true,
      no_gate_weakened: true,
      local_audio_only: !externalTtsProviderUsed,
      external_tts_provider_used: externalTtsProviderUsed,
