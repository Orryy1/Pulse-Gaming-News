...
function strictSchemaBlockers(review = {}, expectedStoryId = "") {
  ...
  for (const field of ["status", "final_verdict"]) {
    if (["RED", "AMBER"].includes(clean(review[field]).toUpperCase())) {
      blockers.push(`final_av_review_${field}_not_green`);
    }
  }
