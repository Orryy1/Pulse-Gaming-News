...
function requiredSelectedHyperframesCardCount(renderManifest = {}) {
  const gate = premiumShellGate(renderManifest);
  const explicitGateRequired = numberOrNull(
    gate.requiredSelectedCardCount ?? gate.required_selected_card_count,
  );
  const directMotionSubstitution =
    gate.cardsOmittedForDirectMotion === true &&
    clean(gate.selectionMode || gate.selection_mode).toLowerCase() === "direct_motion_substitution" &&
    ["pass", "green"].includes(clean(gate.verdict).toLowerCase());
  if (explicitGateRequired === 0 && directMotionSubstitution) return 0;
  const candidates = [
...
function captionSrtLastEndSeconds(filePath = "") {
  const resolved = resolveMaybeRelative(filePath);
  if (!resolved || !fs.existsSync(resolved)) return null;
  ...
  for (const line of text.split(/\r?\n/g)) {
    ...
    if (!match) continue;
    const end = srtTimeToSeconds(match[1]);
    if (Number.isFinite(end)) lastEnd = end;
  }
  return lastEnd;
}

function timestampLastWordEndSeconds(payload = {}) {
  const words = asArray(
  ...
  const maxEnd = Math.max(
    0,
    ...words.map((word) => Number(word.end ?? word.end_s ?? word.endTime ?? word.end_time) || 0),
  );
  return maxEnd > 0 ? maxEnd : null;
}

function captionTimelineEvidenceBlockers(action = {}) {
  const captionsEnd = captionSrtLastEndSeconds(action.captions_path);
  if (!Number.isFinite(captionsEnd) || captionsEnd <= 0) return [];

  const manifestLoad = canonicalManifestForAction(action);
  let maxTimestampEnd = null;
  for (const candidate of timestampCandidatesForAction(action, manifestLoad)) {
    const payload = readJsonSyncIfExists(candidate);
    const timestampEnd = payload ? timestampLastWordEndSeconds(payload) : null;
    if (!Number.isFinite(timestampEnd) || timestampEnd <= 0) continue;
    maxTimestampEnd = Math.max(maxTimestampEnd || 0, timestampEnd);
  }

  if (!Number.isFinite(maxTimestampEnd) || maxTimestampEnd <= 0) return [];
  const tailGap = maxTimestampEnd - captionsEnd;
  if (maxTimestampEnd >= 10 && tailGap > 3 && captionsEnd / maxTimestampEnd < 0.82) {
    return ["captions_srt_timeline_truncated_vs_word_timestamps"];
  }
  return [];
}
...
function actionHasTitleColonPronunciationContext(action = {}, story = {}) {
  return [
    story.title,
    story.public_title,
    story.upload_title,
...
function gtaViPronunciationEvidenceBlockers(action = {}) {
  ...
  const storyHasTitleColonContext = actionHasTitleColonPronunciationContext(action, story);
  ...
  if (!storyHasGtaContext && !storyHasTitleColonContext && !loadedTimestampPayload) return [];
  ...
  if (!storyHasGtaContext && !storyHasTitleColonContext && !voiceHasGtaContext) return [];

  const blockers = [];
  if (!loadedTimestampPayload) {
    blockers.push(
      storyHasTitleColonContext
        ? "title_colon_timestamp_evidence_missing"
        : "gta_vi_timestamp_evidence_missing",
    );
    return blockers;
  }

  if (timestampPronunciationProfile(loadedTimestampPayload) !== TTS_PRONUNCIATION_PROFILE_VERSION) {
    if (storyHasGtaContext || voiceHasGtaContext) blockers.push("gta_vi_timestamp_profile_stale");
    if (storyHasTitleColonContext) blockers.push("title_colon_timestamp_profile_stale");
  }
...
function evidenceBlockers(action = {}) {
  ...
  blockers.push(...captionTimelineEvidenceBlockers(action));
