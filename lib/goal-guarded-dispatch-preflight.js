...
function governedPlatformMetadata(action = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(action, "disclosure_requirements")) {
    metadata.disclosure_requirements = action.disclosure_requirements;
  }
  return metadata;
}
...
function normaliseDispatchPlatform(value) {
...
function actionKey(action = {}) {
  return `${clean(action.story_id)}:${normaliseDispatchPlatform(action.platform)}`;
}
...
function autonomousDryRunActionsFrom(plan = {}, manualActions = []) {
  ...
  return asArray(plan.actions)
    ...
    .map((action) => ({
      ...
      ...governedPlatformMetadata(action),
...
function publishedEvidenceIsTerminal(row = {}) {
  const status = clean(row.status).toLowerCase();
  return status === "published";
}

function alreadyPublishedActionKeysFrom(plan = {}, publishedPlatformEvidence = []) {
  const keys = new Set();
  for (const story of asArray(plan.ready_stories)) {
    ...
    for (const platform of unique([
      ...
      keys.add(`${storyId}:${normaliseDispatchPlatform(platform)}`);
    }
  }
  for (const row of asArray(publishedPlatformEvidence)) {
    const storyId = clean(row.story_id || row.storyId || row.id);
    const platform = normaliseDispatchPlatform(row.platform);
    if (!storyId || !platform || !publishedEvidenceIsTerminal(row)) continue;
...
function terminalDuplicateActionKeysFrom(report = {}) {
  ...
  for (const action of asArray(report.blocked_actions)) {
    const blockers = unique(action.blockers);
    const terminalDuplicate =
      clean(action.outcome) === "duplicate_blocked" ||
...
function terminalDuplicateActionKeysFromStrictDryRun(plan = {}) {
  const keys = new Set();
  for (const story of asArray(plan.skipped_stories)) {
    const storyId = clean(story.story_id || story.id);
    if (!storyId) continue;
    for (const platform of unique(story.terminal_duplicate_blocked_platforms)) {
      keys.add(`${storyId}:${normaliseDispatchPlatform(platform)}`);
    }
    const reason = clean(story.reason || story.skip_reason);
    if (!/^enabled_platforms_already_public_or_terminal_duplicate:/i.test(reason)) continue;
    const detail = reason.replace(/^enabled_platforms_already_public_or_terminal_duplicate:/i, "");
    for (const part of detail.split(",")) {
      const match = clean(part).match(/^([^:]+):duplicate_blocked$/i);
      if (!match) continue;
      keys.add(`${storyId}:${normaliseDispatchPlatform(match[1])}`);
    }
  }
  return keys;
}
...
function validateApprovedAction({
  ...
  if (transcriptAudienceReportPresent && strictAction) {
...
function isStaleApprovalOnlyBlockers(blockers = []) {
  ...
  const missingFromCurrentStrictDryRun = rows.includes("approved_action_missing_from_current_strict_dry_run");
  return rows.length > 0 && rows.every((blocker) =>
    blocker === "approved_action_missing_from_current_strict_dry_run" ||
    (missingFromCurrentStrictDryRun && /^platform_not_ready_now:/i.test(blocker)) ||
    /^platform_status_missing_story:/i.test(blocker)
  );
}

function buildDispatchReadyAction(action = {}, strictAction = {}) {
  return {
    story_id: clean(action.story_id),
    platform: clean(action.platform),
    title: clean(action.title),
    ...
    platform_publish_manifest_path: clean(action.platform_publish_manifest_path),
    ...governedPlatformMetadata(action),
    ...governedPlatformMetadata(strictAction),
...
function buildGuardedDispatchPreflight({
  ...
  publishedPlatformEvidenceError = null,
  ...
  const alreadyPublishedActionKeys = alreadyPublishedActionKeysFrom(strictDryRunPlan, publishedPlatformEvidence);
  const terminalDuplicateActionKeys = new Set([
    ...terminalDuplicateActionKeysFrom(guardedLiveDispatchExecutorReport || {}),
    ...terminalDuplicateActionKeysFromStrictDryRun(strictDryRunPlan),
  ]);
  ...
  const ignoredTerminalDuplicateActions = [];
  ...
  if (publishedPlatformEvidenceError) {
    safetyBlockers.push(`published_platform_evidence_read_failed:${clean(publishedPlatformEvidenceError) || "unknown"}`);
  }
  ...
  for (const action of candidateActions) {
    ...
    if (terminalDuplicateActionKeys.has(key)) {
      ignoredTerminalDuplicateActions.push({
        story_id: clean(action.story_id),
        platform: clean(action.platform),
        title: clean(action.title),
        reason: strictAction
          ? "terminal_duplicate_ignored_from_current_strict_dry_run"
          : "terminal_duplicate_outside_current_strict_dry_run",
      });
      continue;
    }
    ...
    if (blockers.length) {
      ...
      dispatchReadyActions.push(buildDispatchReadyAction(action, strictAction));
    }
  }
  ...
  if (ignoredAlreadyPublishedActions.length) {
    advisory.push("approved_already_published_actions_ignored");
    advisory.push("already_published_actions_ignored");
  }
  if (terminalDuplicateActionKeys.size) advisory.push("terminal_duplicate_blocked_actions_present");
  if (ignoredTerminalDuplicateActions.length) advisory.push("stale_terminal_duplicate_actions_ignored");
  ...
  const requiredNextStep = verdict === "GREEN"
    ...
    : blockedActions.length
      ...
      : (ignoredAlreadyPublishedActions.length || ignoredTerminalDuplicateActions.length) && candidateActions.length
  ...
  const guardedDispatchPlan = {
    ...
    ignored_terminal_duplicate_action_count: ignoredTerminalDuplicateActions.length,
    ...
    ignored_terminal_duplicate_actions: ignoredTerminalDuplicateActions,
  ...
  return {
    ...
    summary: {
      ...
      ignored_already_published_action_count: ignoredAlreadyPublishedActions.length,
      ignored_terminal_duplicate_action_count: ignoredTerminalDuplicateActions.length,
      terminal_duplicate_blocked_action_count:
        blockedActions.filter((action) => asArray(action.blockers).includes("terminal_duplicate_blocked")).length,
      published_platform_evidence_count: asArray(publishedPlatformEvidence).length,
      published_platform_evidence_error: publishedPlatformEvidenceError ? clean(publishedPlatformEvidenceError) : null,
...
function renderGuardedDispatchPreflightMarkdown(report = {}) {
  ...
  if (asArray(report.ignored_terminal_duplicate_actions).length) {
    lines.push("## Terminal Duplicate Approvals Ignored", "");
    for (const action of asArray(report.ignored_terminal_duplicate_actions)) {
      lines.push(`- ${action.story_id || "unknown"} -> ${action.platform || "unknown"}: ${action.reason || "terminal_duplicate_outside_current_strict_dry_run"}`);
    }
    lines.push("");
  }
