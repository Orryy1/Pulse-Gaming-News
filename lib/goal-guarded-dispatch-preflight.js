...
function buildGuardedDispatchPreflight({
  ...
  const transcriptRows = transcriptAudienceByStory(transcriptAudienceReport || {});
  ...
  for (const action of approvedActions) {
    ...
    if (!strictAction && alreadyPublishedActionKeys.has(key)) {
    ...
    const blockers = validateApprovedAction({
      ...
      strictAction,
      platformStatusMatrix,
      transcriptAudienceRow: transcriptRows.get(clean(action.story_id)),
