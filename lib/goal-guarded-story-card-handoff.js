...
async function storyFromAction({ root, action, storiesById }) {
  ...
  const imagePath = storyCardPath(row);
  let premiumCampaignManifestPath = clean(row.premium_visual_campaign_manifest_path);
  if (!premiumCampaignManifestPath && imagePath) {
    const derivedPath = path.join(path.dirname(imagePath), storyId, "premium_visual_campaign_manifest.json");
    if (await fs.pathExists(resolvePath(root, derivedPath))) premiumCampaignManifestPath = derivedPath;
  }
  const title = clean(
    row.title ||
  ...
  return {
    ...
    story_image_path: imagePath,
    premium_visual_campaign_manifest_path: premiumCampaignManifestPath,
  };
}
...
function buildStoryCardAction({
  ...
  return {
    ...
    visual_campaign_tier: clean(campaignManifestPath) ? "premium" : "legacy_compatible",
    premium_visual_campaign_manifest_path: clean(campaignManifestPath),
    ...
    for (const [storyId, storyActions] of actionsByStory.entries()) {
      ...
      for (const derivative of derivatives) {
        ...
        appended.push(buildStoryCardAction({
          ...
          campaignManifestPath: clean(card.story.premium_visual_campaign_manifest_path),
          generatedAt,
        }));
      }
