...
function currentDryRunActionByKey(plan = {}) {
  if (!dryRunSafetyIsIntact(plan)) return new Map();
  const map = new Map();
  for (const action of asArray(plan.actions)) {
    if (clean(action.action) !== "would_publish") continue;
    if (!clean(action.story_id) || !clean(action.platform)) continue;
...
function autonomousGreenDryRunStoryIds(plan = {}) {
  ...
  for (const action of asArray(plan.actions)) {
    if (
      ...
      ids.add(clean(action.story_id));
    }
  }
  return ids;
}

function isStaleReviewEvidenceOnlyBlockers(blockers = []) {
  const rows = unique(blockers);
  return rows.length > 0 && rows.every((blocker) =>
    /^reviewed_artefact_fingerprint_mismatch:/i.test(blocker) ||
    blocker === "human_review_visual_strip_stale_for_review_packet" ||
    blocker === "human_review_visual_strip_qa_stale_for_review_packet" ||
    blocker === "human_review_visual_strip_qa_not_from_current_strip"
  );
}
...
function artefactAliases(key) {
  const aliases = {
    ...
    human_review_visual_strip_report_path: ["human_review_visual_strip_report_path", "visual_strip_report", "first_three_second_visual_strip"],
    human_review_visual_strip_qa_report_path: ["human_review_visual_strip_qa_report_path", "visual_strip_qa_report", "first_three_second_visual_strip_qa"],
  };
...
function reviewArtefactKeys(packet = {}) {
  ...
  return [
    ...
    ].filter((key) => clean(artefacts[key])),
  ];
}
...
function qaCardHasFrameRisks(card = {}) {
  return (
    asArray(card.risk_reasons).length > 0 ||
    asArray(card.frame_risks).length > 0 ||
    asArray(card.warnings).length > 0 ||
    Number(card.warning_frame_count || 0) > 0 ||
    Number(card.missing_frame_count || 0) > 0
  );
}
...
function visualReviewEvidenceBlockers({ packet = {}, reviewPacketManifest = {} } = {}) {
  ...
  else if (qaCardHasFrameRisks(qaCard)) {
    blockers.push("human_review_visual_strip_qa_has_frame_risks");
  }
  return unique(blockers);
}

function validateDecision({ decision = {}, packet = null, reviewPacketManifest = {} } = {}) {
  const blockers = [];
  ...
  if (clean(packet.verdict).toUpperCase() === "RED") {
    blockers.push("red_packet_cannot_be_approved");
  }
  ...
  blockers.push(...visualReviewEvidenceBlockers({ packet, reviewPacketManifest }));
  return unique(blockers);
}

function buildApprovedActions({ decision = {}, packet = {}, strictActionByKey = new Map() } = {}) {
  const artefacts = packet.artefacts || {};
  return unique(decision.approved_platforms).map((platform) => {
    const strictAction = strictActionByKey.get(actionKey({ story_id: packet.story_id, platform })) || {};
    const videoPath = clean(strictAction.video_path) || clean(artefacts.video_path);
    const captionsPath = clean(strictAction.captions_path) || clean(artefacts.captions_path);
    const firstFrameSource = clean(strictAction.cover_frame_source || strictAction.video_path)
      || clean(artefacts.first_frame_source);
    return {
      ...
      platform_media_source: strictAction.video_path ? "strict_dry_run_platform_action" : "review_packet_artefacts",
      reviewed_review_packet_video_path: clean(artefacts.video_path),
      reviewed_review_packet_captions_path: clean(artefacts.captions_path),
...
function buildHumanReviewApprovalGate({
  ...
  const strictActionByKey = currentDryRunActionByKey(strictDryRunPlan);
  const autonomousGreenStoryIds = autonomousGreenDryRunStoryIds(strictDryRunPlan);
  const approvedActions = [];
  const approvedStories = [];
  const blockedDecisions = [];
  const ignoredStaleDecisions = [];
  ...
  for (const decision of decisions) {
    ...
    if (storyId && !packet) {
      ignoredStaleDecisions.push({
        story_id: storyId,
        decision: decisionType || null,
        operator: clean(decision.operator),
        decided_at: clean(decision.decided_at),
        reason: "decision_story_not_in_current_review_packet_manifest",
      });
      continue;
    }
    ...
    const blockers = validateDecision({ decision, packet, reviewPacketManifest });
    if (blockers.length) {
      if (
        storyId &&
        autonomousGreenStoryIds.has(storyId) &&
        isStaleReviewEvidenceOnlyBlockers(blockers)
      ) {
        ignoredStaleDecisions.push({
          story_id: storyId,
          decision: decisionType || null,
          operator: clean(decision.operator),
          decided_at: clean(decision.decided_at),
          reason: "stale_decision_superseded_by_autonomous_green_dry_run",
          blockers,
        });
        continue;
      }
    ...
    approvedActions.push(...buildApprovedActions({ decision, packet, strictActionByKey }));
  }
  ...
  if (ignoredStaleDecisions.length) advisory.push("stale_operator_decisions_ignored");
  ...
  const safePublishPlan = {
    ...
    ignored_stale_decision_count: ignoredStaleDecisions.length,
    required_next_step:
      verdict === "RED"
  ...
  return {
    ...
    ignored_stale_decisions: ignoredStaleDecisions,
