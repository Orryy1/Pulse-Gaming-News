...
function requiredArtefacts(packet = {}) {
  ...
  const keys = reviewArtefactKeys(packet);
  const fingerprints = fingerprintArtefacts(artefacts, keys);
  return keys.map((key) => ({
    key,
...
function reviewArtefactPaths(packet = {}) {
  ...
  return Object.fromEntries(
    reviewArtefactKeys(packet).map((key) => [key, clean(artefacts[key])]),
  );
}
...
function buildRecorderCommands({ storyId, enabled = [], artefacts = [] } = {}) {
  ...
  return {
    ...
    reject_apply_template: buildRecorderCommand({
      storyId,
      decision: "reject",
      rejectedPlatforms: enabled,
      reviewedArtefacts,
    ...
    request_repairs_apply_template: buildRecorderCommand({
      storyId,
      decision: "request_repairs",
      reviewedArtefacts,
      reviewedArtefactFingerprints,
...
function buildDecisionSlot(packet = {}, existingDecision = null) {
  ...
  return {
    ...
    validation_rules: [
      ...
      "Fresh first-three-second visual strip and QA reports must be reviewed before approval.",
...
function buildHumanReviewDecisionSheet({
  ...
  const sourceDryRunGeneratedAt = clean(reviewPacketManifest.source_dry_run_generated_at);
  const sourceReviewPacketManifestGeneratedAt = clean(reviewPacketManifest.generated_at);
...
function renderHumanReviewDecisionSheetMarkdown(sheet = {}) {
  ...
  for (const slot of asArray(sheet.decision_slots)) {
    ...
    lines.push(
      ...
      "This sheet records decisions only. It cannot publish, upload, change OAuth, touch tokens or mutate the production database.",
      "",
      "Approve enabled platforms:",
      "```powershell",
      slot.operator_decision_recorder_commands?.approve_enabled_platforms_dry_run || "",
      "```",
      "Approval apply template:",
      "```powershell",
      slot.operator_decision_recorder_commands?.approve_enabled_platforms_apply_template || "",
      "```",
      "Reject template:",
      "```powershell",
      slot.operator_decision_recorder_commands?.reject_dry_run || "",
      "```",
      "Reject apply template:",
      "```powershell",
