...
const REVIEW_CARD_BLOCKING_ARTEFACT_KEYS = new Set(REVIEW_ARTEFACT_KEYS);
...
function reviewArtefactKeys(slot = {}) {
  const required = asArray(slot.required_reviewed_artefacts)
    .map((artefact) => clean(artefact.key || artefact))
    .filter(Boolean);
  return required.length ? required : REVIEW_ARTEFACT_KEYS;
}

function buildRecommendedReviewDecision({
  slot = {},
  blockers = [],
  pending = false,
  alreadyDecided = false,
} = {}) {
  const enabledCount = unique(slot.allowed_approval_platforms).length;
  const deferredCount = unique(slot.non_approvable_platforms).length;
  if (blockers.length) {
    return {
      decision: "request_repairs_missing_review_artefacts",
      reason: `${blockers.length}_missing_review_artefacts`,
    };
  }
  if (alreadyDecided) {
    return {
      decision: "no_operator_action_needed",
      reason: "decision_already_recorded",
    };
  }
  if (pending) {
    return {
      decision: "watch_first_3s_then_choose_approve_reject_or_repair_enabled_platforms",
      reason: `${enabledCount}_enabled_platforms_reviewable; ${deferredCount}_deferred_or_disabled_platforms_excluded`,
    };
  }
  return {
    decision: "no_operator_action_needed",
    reason: "decision_status_not_pending",
  };
}

function buildReviewCard(slot = {}, index = 0) {
  const openTargets = Object.fromEntries(
    reviewArtefactKeys(slot).map((key) => [key, artefactTarget(slot, key)]),
  );
  const blockers = [];
  for (const target of Object.values(openTargets)) {
    if (REVIEW_CARD_BLOCKING_ARTEFACT_KEYS.has(target.key) && (!target.path || !target.exists)) {
      blockers.push(`missing_review_artefact:${target.key}`);
    }
  }
  ...
  const recommendation = buildRecommendedReviewDecision({
    slot,
    blockers,
    pending,
    alreadyDecided,
  ...
  return {
    ...
    recommended_next_decision: recommendation.decision,
    recommended_decision_reason: recommendation.reason,
    public_copy: {
...
function buildHumanReviewOperatorIndex({
  ...
  const sourceDryRunGeneratedAt = clean(
    decisionSheet.source_dry_run_generated_at ||
      reviewPacketManifest?.source_dry_run_generated_at,
  );
  const sourceDecisionSheetGeneratedAt = clean(decisionSheet.generated_at);
  const sourceReviewPacketManifestGeneratedAt = clean(
    decisionSheet.source_review_packet_manifest_generated_at ||
      reviewPacketManifest?.generated_at,
  );
  const blockers = [];
  ...
  return {
    ...
    source_decision_sheet_generated_at: sourceDecisionSheetGeneratedAt,
    source_review_packet_manifest_generated_at: sourceReviewPacketManifestGeneratedAt,
...
function renderHumanReviewOperatorIndexMarkdown(index = {}) {
  ...
  for (const card of asArray(index.review_cards)) {
    lines.push(
      ...
      `Recommendation reason: ${card.recommended_decision_reason || "not recorded"}`,
