...
function sourceRecord(source = null) {
  if (!source) return null;
  if (typeof source === "string") return { name: clean(source), url: null };
  return {
    name: clean(source.name || source.source_name || source.label || source.title || source.url) || null,
    url: clean(source.url || source.source_url || source.href) || null,
  };
}
...
async function buildReviewItem({
  ...
  return {
    ...
    source_list: {
      ...
      secondary: asArray(canonical.secondary_sources).map(sourceRecord).filter(Boolean),
    },
    evidence: {
...
function renderGoalHumanReviewQueueMarkdown(queue = {}) {
  ...
  for (const item of asArray(queue.review_items)) {
    ...
    `Thumbnail: ${item.public_copy?.thumbnail_headline || "missing"}`,
      ...
      `Primary source: ${item.source_list?.primary?.name || "missing"}`,
