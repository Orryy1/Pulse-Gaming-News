...
function normaliseSourceDisplayName(value) {
  const text = clean(value);
  if (/^youtu(?:be)?$/i.test(text) || /^youtube$/i.test(text)) return "YouTube";
  return text;
}
...
function sourceRecord(source = null, urlFallbacks = []) {
  ...
  if (typeof source === "string") {
    return { name: normaliseSourceDisplayName(source), url: fallbackUrl || null };
  }
  return {
    name: normaliseSourceDisplayName(
      source.name || source.source_name || source.label || source.title || source.url,
    ) || null,
...
function normaliseSha256(value) {
  const sha256 = clean(value).toLowerCase();
...
async function exactDescriptionCoherenceReconciliation({
  actions = [],
  incident = {},
  artifactDir = "",
} = {}) {
  const reviewableActions = reviewablePublishActions(actions);
  if (!reviewableActions.length) return null;
  const reconciliationCode =
    "description_coherence_aliases_superseded_by_exact_current_authority";
  const actionIsSafelyReconciled = (candidate = {}) => {
    const candidateAuthority =
      candidate.package_checks?.exact_current_package_authority || {};
    const candidateExplicitlyReconciled =
      asArray(candidate.authority_reconciliations).includes(reconciliationCode);
    const candidatePackageExplicitlyReconciled =
      candidateAuthority.valid === true &&
      candidateAuthority.description_coherence_aliases_superseded === true;
    const guardedDispatchReady =
      clean(candidate.live_execution_gate) === "guarded_dispatch_ready" &&
      candidate.autonomous_green_lit_by_dry_run === true &&
      !asArray(candidate.live_execution_gate_reasons).length;
    const operatorHumanReviewRequired =
      clean(candidate.live_execution_gate) === "operator_human_review_required" &&
      candidate.requires_human_review_before_live_publish === true;
    return (
      (candidateExplicitlyReconciled || candidatePackageExplicitlyReconciled) &&
      candidate.action === "would_publish" &&
      candidate.platform_enabled === true &&
      !asArray(candidate.blockers).length &&
      (guardedDispatchReady || operatorHumanReviewRequired)
    );
  };
  if (!reviewableActions.every(actionIsSafelyReconciled)) return null;
  const action = reviewableActions.find((candidate) =>
    clean(candidate.video_fingerprint_source) === "exact_current_package_authority" &&
    normaliseSha256(candidate.video_sha256)
  );
  if (!action) return null;
  const packageChecks = action.package_checks || {};
  const authority = packageChecks.exact_current_package_authority || {};
  const effectiveCoherence = packageChecks.effective_public_output_coherence || {};
  const actionExplicitlyReconciled =
    asArray(action.authority_reconciliations).includes(reconciliationCode);
  const packageExplicitlyReconciled =
    authority.valid === true &&
    authority.description_coherence_aliases_superseded === true;
  if (!actionExplicitlyReconciled && !packageExplicitlyReconciled) return null;

  const videoPath = clean(action.video_path);
  const videoSha256 = normaliseSha256(action.video_sha256);
  if (!videoPath || !videoSha256 || !await fs.pathExists(videoPath)) return null;
  const stats = await fs.stat(videoPath);
  if (
    !stats.isFile() ||
    (Number.isFinite(Number(action.video_size_bytes)) &&
      Number(action.video_size_bytes) !== stats.size)
  ) {
    return null;
  }
  const authorityRender = authority.exact_fingerprints?.render || {};
  if (
    packageExplicitlyReconciled &&
    (
      !sameResolvedPath(authorityRender.path, videoPath) ||
      normaliseSha256(authorityRender.sha256) !== videoSha256 ||
      (Number.isFinite(Number(authorityRender.size_bytes)) &&
        Number(authorityRender.size_bytes) !== stats.size)
    )
  ) {
    return null;
  }
  if (await sha256File(videoPath) !== videoSha256) return null;

  const coherence = incident.public_output_coherence_report || {};
  const freshness = coherence.artifact_freshness || {};
  const reconciledBy = clean(freshness.reconciled_by);
  if (
    incident.verdict !== "pass" ||
    incident.safe_to_publish_boolean !== true ||
    asArray(incident.disaster_upload_blockers).length ||
    clean(coherence.verdict) !== "pass" ||
    asArray(coherence.blockers).length ||
    clean(freshness.status) !== "fresh" ||
    clean(freshness.result) !== "pass" ||
    asArray(freshness.blockers).length ||
    asArray(freshness.mismatched_fields).length ||
    asArray(freshness.missing_fields).length ||
    reconciledBy !== "exact_current_package_authority_and_metadata_finalisation"
  ) {
    return null;
  }
  if (
    Object.keys(effectiveCoherence).length &&
    (
      clean(effectiveCoherence.status) !== "fresh" ||
      clean(effectiveCoherence.verdict) !== "pass" ||
      asArray(effectiveCoherence.blockers).length ||
      asArray(effectiveCoherence.mismatched_fields).length
    )
  ) {
    return null;
  }
  if (
    clean(incident.artifact_dir) &&
    artifactDir &&
    !sameResolvedPath(incident.artifact_dir, artifactDir)
  ) {
    return null;
  }
  return {
    ...
    authority: "exact_current_package_authority",
  };
}
...
function isReviewablePublishAction(action = {}) {
  return (
    action.action === "would_publish" &&
    action.platform_enabled !== false &&
    clean(action.live_execution_gate) !== "blocked" &&
    !asArray(action.blockers).length
  );
}

function reviewablePublishActions(actions = []) {
  return asArray(actions).filter(isReviewablePublishAction);
}
...
function platformBlockedRequirements(blockedActions = []) {
  const requirements = [];
  const seen = new Set();
  for (const action of asArray(blockedActions)) {
    const platform = clean(action.platform);
    if (!platform || seen.has(platform)) continue;
    seen.add(platform);
    const blockers = unique([
      ...
      clean(action.live_execution_gate) === "blocked" && !asArray(action.blockers).length
        ? "platform_action_blocked"
        : "",
    ]);
    requirements.push({
      platform,
      platform_enabled: action.platform_enabled === true,
      operational_state: clean(action.platform_operational_state) || null,
      operational_reason: clean(action.platform_operational_reason) || null,
      enablement_gaps: unique(action.platform_enablement_gaps),
      enablement_next_action: clean(action.platform_enablement_next_action) || null,
      blockers,
      action: clean(action.action) || "blocked",
      live_execution_gate: clean(action.live_execution_gate) || "blocked",
      required_before_counting_platform_ready: true,
      live_publish_allowed_before_repair: false,
      live_publish_allowed_before_enablement: action.platform_enabled === false ? false : undefined,
    });
  }
  return requirements;
}

function platformBlockedReasonCodes(blockedActions = []) {
  return unique(asArray(blockedActions).flatMap((action) => [
    ...asArray(action.blockers),
    ...asArray(action.live_execution_gate_reasons),
    clean(action.live_execution_gate) === "blocked" ? `platform_action_blocked:${clean(action.platform)}` : "",
  ]));
}
...
async function buildReviewItem({
  ...
  const publishActions = reviewablePublishActions(actions);
  const publishNowPlatforms = unique(publishActions.map((action) => action.platform));
  ...
  const firstAction = publishActions[0] || asArray(actions)[0] || {};
  const title = clean(
  ...
  const platformBlocked = platformBlockedRequirements(blockedActions);
  const fullPlatformVerdict = !publishNowPlatforms.length
    ? "RED"
    : blockedPlatforms.length || deferredPlatforms.length || warnings.length
      ? "AMBER"
      : "GREEN";
  ...
  if (platformBlocked.length) {
    approvalRequirements.push("Do not approve blocked platform variants until their repair lane passes preflight.");
  }
  ...
  return {
    ...
    enabled_platform_verdict: publishNowPlatforms.length ? "GREEN" : "RED",
    ...
    platform_blocked_requirements: platformBlocked,
    public_copy: {
      title,
    ...
    evidence: {
      ...
      video_sha256: coherenceReconciliation?.video_sha256 || normaliseSha256(firstAction.video_sha256) || null,
      public_output_coherence_reconciled_by: coherenceReconciliation?.reconciled_by || null,
    },
    approval: {
...
function buildReviewPacketManifest({
  generatedAt,
  sourceDryRunGeneratedAt = "",
  sourceDryRunVerdict = "",
  reviewItems = [],
  blockedItems = [],
  visualReviewEvidenceDir = path.join(process.cwd(), "output", "goal-contract"),
} = {}) {
  const visualEvidenceDir = path.resolve(visualReviewEvidenceDir);
  return {
    schema_version: 1,
    generated_at: generatedAt,
    source_dry_run_generated_at: clean(sourceDryRunGeneratedAt),
    source_dry_run_verdict: clean(sourceDryRunVerdict),
    mode: "HUMAN_REVIEW",
    review_packets: asArray(reviewItems).map((item) => ({
      ...
      platform_blocked_requirements: asArray(item.platform_blocked_requirements),
      ...
      artefacts: {
        ...
        human_review_visual_strip_report_path: path.join(visualEvidenceDir, "human_review_visual_strip_report.json"),
        human_review_visual_strip_qa_report_path: path.join(visualEvidenceDir, "human_review_visual_strip_qa_report.json"),
      },
...
async function buildGoalHumanReviewQueue({
  ...
  const sourceDryRunGeneratedAt = clean(dryRunPlan.generated_at || dryRunPlan.generatedAt);
  const sourceDryRunVerdict = clean(dryRunPlan.overall_verdict || dryRunPlan.verdict);
  ...
  for (const storyId of storyIds) {
    ...
    const storyReviewableActions = reviewablePublishActions(storyActions);
    if (
      blockedStory ||
    ...
    if (!storyReviewableActions.length || incident.verdict !== "pass") {
      blockedItems.push(blockedItemFrom(
        {
          story_id: storyId,
          blockers: [
            "human_review_missing_safe_dry_run_evidence",
            ...platformBlockedReasonCodes(storyBlockedActions),
          ],
        },
        incident,
        renderInputDetailsByStory.get(storyId),
    ...
    const coherenceReconciliation = await exactDescriptionCoherenceReconciliation({
      actions: storyActions,
      incident,
      artifactDir,
    });
    const coherenceBlockers = asArray(coherenceArtifact.blockers).filter((blocker) =>
      !(
        coherenceReconciliation &&
        [
          "stale_public_output_coherence_report",
          "stale_public_output_coherence_field:description",
        ].includes(clean(blocker))
      )
    );
    if (coherenceBlockers.length) {
      blockedItems.push(blockedItemFrom(
        {
          story_id: storyId,
          artifact_dir: artifactDir,
  ...
  const approvalRequirements = {
    ...
    stories: limitedReviewItems.map((item) => ({
      ...
      platform_blocked_requirements: item.platform_blocked_requirements,
  ...
  return {
    schema_version: 1,
    generated_at: generatedAt,
    source_dry_run_generated_at: sourceDryRunGeneratedAt,
    source_dry_run_verdict: sourceDryRunVerdict,
    mode: "HUMAN_REVIEW",
    summary: {
      review_item_count: limitedReviewItems.length,
      blocked_item_count: blockedItems.length,
      ...
      source_dry_run_generated_at: sourceDryRunGeneratedAt,
      source_dry_run_verdict: sourceDryRunVerdict || null,
    },
...
function renderGoalHumanReviewQueueMarkdown(queue = {}) {
  ...
  for (const item of asArray(queue.review_items)) {
    ...
    `Blocked platforms: ${item.blocked_platforms.join(", ") || "none"}`,
    ...
    `Platform blocked requirements: ${asArray(item.platform_blocked_requirements).map((requirement) => `${requirement.platform}:${asArray(requirement.blockers).join("+") || "blocked"}`).join(", ") || "none"}`,
...
async function writeGoalHumanReviewQueue(queue = {}, { outputDir } = {}) {
  ...
  let preservedOperatorDecisions = null;
  try {
    if (await fs.pathExists(operatorDecisionLogPath)) {
      const existingOperatorDecisionLog = await fs.readJson(operatorDecisionLogPath);
      if (Array.isArray(existingOperatorDecisionLog.decisions) && existingOperatorDecisionLog.decisions.length) {
        preservedOperatorDecisions = existingOperatorDecisionLog.decisions;
      }
    }
  } catch {}
  const mergedOperatorDecisionLog = preservedOperatorDecisions
    ? {
        ...operatorDecisionLog,
        decisions: preservedOperatorDecisions,
        safety: {
          no_live_publish_from_log: true,
          no_network_uploads: true,
          no_db_mutation: true,
          no_oauth_or_token_change: true,
          ...(operatorDecisionLog.safety || {}),
        },
      }
    : operatorDecisionLog;
  ...
  await fs.writeJson(operatorDecisionLogPath, mergedOperatorDecisionLog, { spaces: 2 });
  return {
    outputDir: outDir,
    jsonPath,
    markdownPath,
