...
function buildHumanReviewVisualRepairWorkOrder(cards = [], {
  generatedAt = new Date().toISOString(),
  sourceVisualStripQaGeneratedAt = null,
} = {}) {
  const jobs = asArray(cards)
    .filter((card) => asArray(card.risk_reasons).length > 0)
    .map((card) => {
      ...
      return {
        ...
        recommended_command: commandForVisualRepairJob(storyId, lane),
        expected_output:
...
async function buildHumanReviewVisualStripQaReport({
  ...
  return {
    ...
    visual_repair_work_order: buildHumanReviewVisualRepairWorkOrder(cards, {
      generatedAt,
...
async function writeHumanReviewVisualStripQaReport(report = {}, { outputDir } = {}) {
  ...
  await fs.writeJson(workOrderJsonPath, report.visual_repair_work_order || buildHumanReviewVisualRepairWorkOrder([]), { spaces: 2 });
  await fs.writeFile(
    workOrderMarkdownPath,
    renderHumanReviewVisualRepairWorkOrderMarkdown(report.visual_repair_work_order || buildHumanReviewVisualRepairWorkOrder([])),
    "utf8",
  );
