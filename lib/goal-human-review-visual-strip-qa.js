...
function evaluateFrameAnalysis(frame = {}, analysis = {}) {
  ...
  const darkEdge = numberOrZero(border.dark_edge_ratio);
  ...
  const brandedDarkEdgeMask =
    darkEdge >= 0.18 &&
    brightEdge < 0.06 &&
    textOverlay < 0.025 &&
    whiteTextOnDark < 0.05;
  ...
  if (!brandedDarkEdgeMask && (borderRisk >= 0.42 || edgeTouch >= 0.15 || brightEdge >= 0.06)) {
...
async function usableMotionRowsEvidence(rows = [], { artifactDir, workspaceRoot, honourReadinessFlag = true } = {}) {
  const paths = [];
  const families = new Set();
  for (const [index, row] of rows.entries()) {
    if (row && typeof row === "object" && row.validated === false) continue;
    if (honourReadinessFlag && row && typeof row === "object" && row.counts_towards_motion_readiness === false) continue;
...
async function usableMaterialisedMotionEvidence(manifest = {}, { artifactDir, workspaceRoot } = {}) {
  return usableMotionRowsEvidence(materialisedMotionRows(manifest), {
    artifactDir,
    workspaceRoot,
    honourReadinessFlag: true,
  });
}

function schedulerBridgeCandidatesRows(report = {}) {
...
function bridgeCandidateMatchesCard(candidate = {}, card = {}, artifactDir = "") {
  const storyId = clean(card.story_id);
  if (storyId && clean(candidate.story_id || candidate.id) === storyId) return true;
  const candidateArtifactDir = clean(candidate.artifact_dir || candidate.artifactDir);
  return artifactDir && candidateArtifactDir && path.resolve(candidateArtifactDir) === path.resolve(artifactDir);
}

async function usableSchedulerBridgeMotionEvidence(card = {}, { artifactDir, workspaceRoot } = {}) {
  const bridgePath = path.join(workspaceRoot, "output", "goal-contract", "scheduler_bridge_candidates.json");
  const bridgeReport = await readJsonIfPresent(bridgePath, {});
  const candidate = schedulerBridgeCandidatesRows(bridgeReport).find((row) =>
    bridgeCandidateMatchesCard(row, card, artifactDir),
  );
  if (!candidate) {
    return { materialised_motion_clip_paths: [], distinct_motion_families: [] };
  }
  const rows = [
  ...
  return usableMotionRowsEvidence(rows, {
    artifactDir,
    workspaceRoot,
    honourReadinessFlag: false,
  });
}
...
async function finalRenderEvidenceForCard(card = {}, { workspaceRoot = process.cwd() } = {}) {
  ...
  const bridgeMotionEvidence = motionEvidence.materialised_motion_clip_paths.length >= 3 &&
    motionEvidence.distinct_motion_families.length >= 3
    ? { materialised_motion_clip_paths: [], distinct_motion_families: [] }
    : await usableSchedulerBridgeMotionEvidence(card, { artifactDir, workspaceRoot });
  const effectiveMotionEvidence = bridgeMotionEvidence.materialised_motion_clip_paths.length >= 3 &&
    bridgeMotionEvidence.distinct_motion_families.length >= 3
    ? bridgeMotionEvidence
    : motionEvidence;
  ...
  if (effectiveMotionEvidence !== motionEvidence) {
    for (let index = blockers.length - 1; index >= 0; index -= 1) {
      if (
        blockers[index] === "materialised_motion_clip_paths_missing_or_insufficient" ||
        blockers[index] === "distinct_motion_families_missing_or_insufficient"
      ) {
  ...
  return {
    ...
    evidence: {
      ...
      materialised_motion_clip_count: effectiveMotionEvidence.materialised_motion_clip_paths.length,
      distinct_motion_family_count: effectiveMotionEvidence.distinct_motion_families.length,
      materialised_motion_clip_paths: effectiveMotionEvidence.materialised_motion_clip_paths,
      motion_evidence_source: effectiveMotionEvidence === motionEvidence
