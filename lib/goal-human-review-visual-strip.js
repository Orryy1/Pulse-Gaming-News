...
function consoleInputIsBlocked(consoleBundle = {}) {
  const summary = consoleBundle.summary || {};
  return (
    clean(consoleBundle.verdict).toUpperCase() === "RED" ||
    asArray(consoleBundle.blockers).length > 0 ||
    Number(summary.blocked_input_count || 0) > 0
  );
}
...
function buildVisualStripCard(card = {}, { outputDir }) {
  ...
  const extractable = blockers.length === 0;
...
function buildHumanReviewVisualStripPlan({
  ...
  if (consoleInputIsBlocked(consoleBundle)) blockers.push("human_review_console_input_blocked");
  ...
  return {
    ...
    source_console_dry_run_generated_at: clean(
      consoleBundle.source_operator_index_dry_run_generated_at ||
        consoleBundle.source_dry_run_generated_at ||
        consoleBundle.freshness_reference_generated_at,
    ),
    source_strict_dry_run_generated_at: clean(consoleBundle.source_strict_dry_run_generated_at),
