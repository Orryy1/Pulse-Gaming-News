...
function timestampRows(payload = {}) {
  return asArray(payload.words || payload.word_timestamps)
    .map((row) => ({
      ...
      ...(asArray(row?.spoken_words ?? row?.spokenWords).length
        ? {
            spoken_words: asArray(row?.spoken_words ?? row?.spokenWords)
              .map(clean)
              .filter(Boolean),
          }
        : {}),
...
function timestampSpokenWords(row = {}) {
  const spokenWords = asArray(row.spoken_words ?? row.spokenWords)
    .flatMap((value) => transcriptWords(value));
  if (spokenWords.length) return spokenWords;
  return transcriptWords(row.word);
}

function timestampAcousticWordCount(rows = []) {
  return asArray(rows).reduce((total, row) => {
    const spokenWords = asArray(row.spoken_words ?? row.spokenWords)
      .map(clean)
      .filter(Boolean);
    return total + Math.max(1, spokenWords.length);
  }, 0);
}
...
function planFlagshipTimestampRepair({
  ...
  const alignedWords = rows.flatMap((row) => timestampSpokenWords(row));
  const alignedAcousticWordCount = timestampAcousticWordCount(rows);
  ...
  for (const field of [
    ...
    if (Number(alignment[field]) !== alignedAcousticWordCount) {
...
async function writeFlagshipNarrationQaEvidence({
  ...
  let alignedWords = words.flatMap(timestampSpokenWords);
  ...
  if (timestampRepair.repaired === true) {
    ...
    alignedWords = words.flatMap(timestampSpokenWords);
  ...
  const captionManifest = {
    ...
    timestamp_row_count: words.length,
    spoken_word_timestamp_count: alignedWords.length,
  ...
  const cadence = await analyseNarrationCadence({
    audioManifest: { word_timestamp_count: timestampAcousticWordCount(words) },
    narrationManifest: { word_timestamp_count: timestampAcousticWordCount(words) },
    timestampPayload,
  ...
  const voiceChecks = {
    ...
    cadence_passed:
      cadence.status !== "fail" &&
      asArray(cadence.blockers).length === 0,
