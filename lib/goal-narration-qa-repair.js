...
function timestampTimelineAudit(rows = [], {
  ...
  return {
    ...
    audio_coverage_sufficient: audioCoverageRatio >= 0.8,
    video_coverage_sufficient: videoCoverageRatio >= 0.8,
  };
}
...
function planFlagshipTimestampRepair({
  ...
  for (let index = 0; index < sourceWords.length - 1; index += 1) {
    if (!(Number(sourceWords[index + 1].start) > Number(sourceWords[index].start))) {
  ...
  const repairedOrdinals = [];
  const overlapRepairOrdinals = [];
  const acousticBoundaryRepairOrdinals = [];
  const repairedWords = sourceWords.map((row, index) => {
    if (index === sourceWords.length - 1) return { ...row };
    ...
    if (Number(row.end) !== authoritativeEnd) repairedOrdinals.push(index + 1);
  ...
  const repairEvidence = {
    ...
    word_start_times_unchanged: true,
