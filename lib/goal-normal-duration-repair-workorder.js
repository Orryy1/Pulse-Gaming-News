...
async function buildNormalDurationRepairJob(blockedStory = {}, {
  ...
  if (!floorBlocker && !ceilingBlocker && !contentBlocker && manifestDuration != null) {
    if (manifestDuration > Number(targetDurationSeconds.max)) {
      ceilingBlocker = `manifest_duration_above_target:${manifestDuration}`;
    } else if (candidateMissing && manifestDuration < Number(targetDurationSeconds.min)) {
...
async function buildNormalDurationRepairWorkOrder({
  ...
  const jobs = [];
  const skipped = [];
  const blockedStories = Array.isArray(dryRunPlan)
    ? asArray(dryRunPlan)
    : asArray(dryRunPlan.blocked_stories);
  const heldStories = Array.isArray(dryRunPlan) ? [] : asArray(dryRunPlan.held_stories);
  const cutoverReadyStories = Array.isArray(cutoverPlan) ? [] : asArray(cutoverPlan.ready);
  const cutoverQueueStories = Array.isArray(cutoverPlan) ? asArray(cutoverPlan) : asArray(cutoverPlan.queue);
  const cutoverDurationJobs = Array.isArray(cutoverPlan)
    ? []
    : asArray(cutoverPlan.normal_duration_rerender_work_order?.jobs);
  ...
  const addCandidate = (story = {}, source = "") => {
    ...
    repairCandidates.push({
      ...story,
      blockers: asArray(story.blockers).length ? story.blockers : story.source_blockers,
      _normal_duration_source: source,
    });
  };
  cutoverDurationJobs.forEach((story) => addCandidate(story, "cutover_duration_rerender"));
  blockedStories.forEach((story) => addCandidate(story, "blocked"));
  heldStories.forEach((story) => addCandidate(story, "held"));
  cutoverReadyStories.forEach((story) => addCandidate(story, "cutover_ready"));
  ...
  return {
    ...
    summary: {
      ...
      cutover_ready_story_count: cutoverReadyStories.length,
      cutover_queue_story_count: cutoverQueueStories.length,
      cutover_duration_job_count: cutoverDurationJobs.length,
