...
const MAX_OWNED_EXPLAINER_CARD_DURATION_S = 8.5;
...
function clipDuration(clip = {}) {
  ...
  const fallback = isOwnedExplainer ? MIN_OWNED_EXPLAINER_CARD_DURATION_S : 2.8;
  ...
  return isOwnedExplainer ? Math.max(capped, MIN_OWNED_EXPLAINER_CARD_DURATION_S) : capped;
}
...
function ownedExplainerClipPlan({ storyId, canonical = {} } = {}) {
  ...
  return rows.map(([assetClass, headline, purpose], index) => ({
    ...
    durationS: MIN_OWNED_EXPLAINER_CARD_DURATION_S,
    validated: true,
