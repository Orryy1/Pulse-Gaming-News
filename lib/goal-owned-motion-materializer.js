...
const {
  _private: { governedExistingStillMotionRows },
...
function sourceLabelKey(value) {
  return cleanText(value).toLowerCase().replace(/[^a-z0-9]+/g, "");
}

function isReadablePrimarySourceCard(clip = {}, canonical = {}) {
  if (!isOwnedGeneratedMotion(clip)) return false;
  const cardKind = cleanText(clip.readable_card_kind || clip.card_kind).toLowerCase();
  const assetClass = cleanText(clip.asset_class).toLowerCase();
  if (cardKind !== "source" && assetClass !== "animated_source_card") return false;
  const expectedSource = sourceLabelKey(canonical.primary_source || canonical.source_card_label);
  const readableSource = sourceLabelKey(clip.readable_text || clip.source_relationship);
  if (!expectedSource || expectedSource !== readableSource) return false;
  const duration = finiteNumber(clip.durationS ?? clip.duration);
  return Boolean(
    clip.materialized === true &&
      clip.counts_towards_motion_readiness !== false &&
      duration != null &&
      duration >= MIN_OWNED_EXPLAINER_CARD_DURATION_S,
  );
}

function isPremiumExternalMotionPack(clips = []) {
  const ready = asArray(clips).filter((clip) => clip.counts_towards_motion_readiness === true);
  const families = unique(ready.map((clip) => clip.motion_family || clip.source_family));
  return ready.length >= 5 && families.length >= 4;
}
...
async function updateOwnedMotionEvidence({
  ...
  const governedStillMotionClips = sourceSafetyBlocked
    ? []
    : governedExistingStillMotionRows(rightsLedger, { root });
  const preservedExternalMotionClips = mergeClipRows(
    preservedDirectVideoClips,
    governedStillMotionClips,
  );
  const premiumExternalPackPreserved = isPremiumExternalMotionPack(
    preservedExternalMotionClips,
  );
  const premiumGovernedStillPackPreserved = isPremiumExternalMotionPack(
    governedStillMotionClips,
  );
  const selectedOwnedMotionRows = premiumGovernedStillPackPreserved
    ? clipRows.filter((clip) => isReadablePrimarySourceCard(clip, canonical))
    : clipRows;
  ...
  const finalMotionRows = sourceSafetyBlocked
    ? clipRows
    : mergeClipRows(preservedExternalMotionClips, selectedOwnedMotionRows);
  ...
  await fs.writeJson(path.join(artifactDir, "materialised_motion_clips.json"), {
    ...
    selected_materialised_motion_clip_ids: finalMotionRows.map(clipIdentity).filter(Boolean),
    distinct_motion_families: families,
    clip_count: finalMotionRows.length,
    distinct_motion_family_count: families.length,
    governed_still_motion_asset_count: governedStillMotionClips.length,
    preserved_external_motion_asset_count: preservedExternalMotionClips.length,
    premium_external_motion_pack_preserved: premiumExternalPackPreserved,
    premium_governed_still_motion_pack_preserved: premiumGovernedStillPackPreserved,
    owned_support_motion_asset_count: selectedOwnedMotionRows.length,
    owned_motion_audit_asset_count: clipRows.length,
    direct_video_motion_asset_count: directVideoClipCount,
    direct_video_motion_family_count: directVideoFamilyCount,
    quarantined_direct_video_clip_count: quarantinedDirectVideoAudit.length,
    quarantined_direct_video_clips: quarantinedDirectVideoAudit,
