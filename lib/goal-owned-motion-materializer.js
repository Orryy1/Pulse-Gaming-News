...
const MIN_OWNED_EXPLAINER_CARD_DURATION_S = 6.5;
const MAX_OWNED_EXPLAINER_CARD_DURATION_S = 10;
...
function clipDuration(clip = {}) {
  ...
  const isOwnedExplainer = [
    ...
    clip.source_url,
    clip.rights_risk_class,
  ].map(cleanText).join(" ").toLowerCase().includes("owned") &&
    [
      clip.source_kind,
      clip.media_kind,
      clip.source_url,
      clip.owned_explainer_visual_plan === true ? "owned_explainer_visual_plan" : "",
    ].map(cleanText).join(" ").toLowerCase().includes("explainer");
  const readableFloor = isOwnedExplainer
    ? ownedExplainerReadableDurationS(clip)
    : MIN_OWNED_EXPLAINER_CARD_DURATION_S;
  const fallback = isOwnedExplainer ? readableFloor : 2.8;
  if (!Number.isFinite(duration) || duration <= 0.5) return fallback;
  const capped = Math.min(duration, isOwnedExplainer ? MAX_OWNED_EXPLAINER_CARD_DURATION_S : 6);
  return isOwnedExplainer ? Math.max(capped, readableFloor) : capped;
}
...
function ownedExplainerReadableDurationS(clip = {}) {
  ...
  if (/source_proof|platform_proof|proof card|quote|article screenshot|confirmed claim/.test(text)) return 10;
  if (/stat|chart|carousel|image-card|x_image|breaking-news|fast card|claim card/.test(text)) return 8;
...
function ownedExplainerClipPlan({ storyId, canonical = {} } = {}) {
  ...
  return rows.map(([assetClass, headline, purpose], index) => ({
    ...
    durationS: ownedExplainerReadableDurationS({
      asset_class: assetClass,
...
function isReadyDirectVideoClip(clip = {}) {
  ...
  if (clip.source_safety_blocked === true || clip.counts_towards_motion_readiness === false) return false;
  const text = [
    clip.media_kind,
    clip.source_type,
    clip.source_kind,
  ...
  if (text.includes("official_reference_only")) return false;
  ...
  return hasDirectVideoEvidence && Boolean(cleanText(clip.path || clip.local_materialized_path || clip.file_path));
}
