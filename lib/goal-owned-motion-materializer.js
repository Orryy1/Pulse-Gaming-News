...
const DEFAULT_OWNED_EXPLAINER_CARD_DURATION_S = 4.2;
...
function clipDuration(clip = {}) {
  ...
  const fallback = isOwnedExplainer ? DEFAULT_OWNED_EXPLAINER_CARD_DURATION_S : 2.8;
  if (!Number.isFinite(duration) || duration <= 0.5) return fallback;
  const capped = Math.min(duration, 6);
  return isOwnedExplainer ? Math.max(capped, DEFAULT_OWNED_EXPLAINER_CARD_DURATION_S) : capped;
}
...
function ownedExplainerClipPlan({ storyId, canonical = {} } = {}) {
  ...
  return rows.map(([assetClass, headline, purpose], index) => ({
    ...
    durationS: DEFAULT_OWNED_EXPLAINER_CARD_DURATION_S,
    validated: true,
