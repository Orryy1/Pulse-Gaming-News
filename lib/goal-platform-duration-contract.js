...
const LONGEST_PLATFORM_HARD_MAX_SECONDS = Math.max(...Object.values(PLATFORM_HARD_MAX_SECONDS));
...
function activeStorySet(activeStoryIds = null) {
  if (!Array.isArray(activeStoryIds)) return null;
  const ids = activeStoryIds.map((id) => cleanText(id)).filter(Boolean);
  return ids.length ? new Set(ids) : null;
}

function readinessScopeForStory(storyId = "", activeSet = null) {
  if (!activeSet) return "active";
  return activeSet.has(cleanText(storyId)) ? "active" : "quarantined";
}
...
function mergePlatformOutput(existing = {}, contract = {}) {
  const uploadWindow = contract.publish_duration_seconds || contract.duration_seconds;
  return {
    ...existing,
    ...contract,
    duration_seconds: uploadWindow || existing.duration_seconds || contract.target_duration_seconds,
    strategic_duration_seconds: contract.target_duration_seconds || existing.strategic_duration_seconds,
  };
}
...
function durationBlockers(renderedDurationS) {
  ...
  if (duration > LONGEST_PLATFORM_HARD_MAX_SECONDS) {
    blockers.push(`render_duration_above_longest_platform_max:${LONGEST_PLATFORM_HARD_MAX_SECONDS}`);
  }
  return blockers;
}
...
function variantFreshAgainstRender({ output = {}, renderManifest = {}, renderedDurationS = null } = {}) {
  ...
  if (renderGeneratedAt) {
    if (!variantGeneratedAt || variantGeneratedAt < renderGeneratedAt) return false;
  }
  const currentDuration = round(renderedDurationS);
...
async function platformVariantAlreadyInWindow({
  output = {},
  artifactDir = "",
  maxDurationS = null,
  renderManifest = {},
  ...
  if (!variantFreshAgainstRender({ output, renderManifest, renderedDurationS })) return false;
...
async function tiktokCreatorRewardsVariantAlreadyReady({
  output = {},
  artifactDir = "",
  renderManifest = {},
  renderedDurationS = null,
...
async function buildPlatformDurationVariantRepairJobs({
  ...
  for (const [platform, output] of Object.entries(existingOutputs || {})) {
    ...
    if (!Number.isFinite(hardMax)) continue;
    const existingVariantPath = variantVideoPath(output);
    if (duration <= hardMax && !existingVariantPath) continue;
    if (await platformVariantAlreadyInWindow({
      output,
      artifactDir,
      maxDurationS: hardMax,
      renderManifest,
      renderedDurationS,
    })) continue;
    jobs.push({
      ...
      target_duration_s: round(Math.min(duration, Math.max(HARD_MIN_SECONDS, hardMax - 0.2))),
      generated_at: generatedAt,
      actions: [
...
async function buildTiktokCreatorRewardsVariantRepairJob({
  ...
  if (await tiktokCreatorRewardsVariantAlreadyReady({
    output: tiktokOutput,
    artifactDir,
    renderManifest,
    renderedDurationS,
...
async function repairStoryPackageDurationContract(
  storyPackage = {},
  { generatedAt = new Date().toISOString(), readinessScope = "active" } = {},
) {
  ...
  const tiktokCreatorRewardsReady = await tiktokCreatorRewardsVariantAlreadyReady({
    output: existingOutputs.tiktok || {},
    artifactDir,
    renderManifest,
    renderedDurationS,
  });
  if (tiktokCreatorRewardsReady) {
    contracts.tiktok = {
      ...contracts.tiktok,
      creator_rewards_eligible: true,
      creator_rewards_duration_seconds: { min: 61, max: 90 },
      duration_warnings: asArray(contracts.tiktok.duration_warnings)
  ...
  const syncedPlatformPackFiles = [];
  for (const [platform, basename] of Object.entries(PLATFORM_PACK_FILES)) {
    const packPath = path.join(artifactDir, basename);
    if (!(await fs.pathExists(packPath)) || !outputs[platform]) continue;
    await fs.writeJson(packPath, outputs[platform], { spaces: 2 });
    syncedPlatformPackFiles.push(packPath);
  }
  ...
  return {
    ...
    synced_platform_pack_files: syncedPlatformPackFiles,
  };
}
...
async function repairGoalPlatformDurationContracts({
  ...
  generatedAt = new Date().toISOString(),
  activeStoryIds = null,
  activeStoryPackageOverrides = null,
} = {}) {
  const updated = [];
  const blocked = [];
  const activeSet = activeStorySet(activeStoryIds);
  const overrideMap =
    activeStoryPackageOverrides instanceof Map
      ? activeStoryPackageOverrides
      : new Map(Object.entries(activeStoryPackageOverrides || {}));
  for (const storyPackage of asArray(storyPackages)) {
    const storyId = storyPackage?.story_id || storyPackage?.id || "unknown";
    const readinessScope = readinessScopeForStory(storyId, activeSet);
    const override = readinessScope === "active" ? overrideMap.get(cleanText(storyId)) : null;
    const effectiveStoryPackage = override
      ? {
          ...storyPackage,
          ...override,
          original_artifact_dir: storyPackage.artifact_dir || storyPackage.output_dir || storyPackage.package_dir || null,
        }
      : storyPackage;
    const result = await repairStoryPackageDurationContract(effectiveStoryPackage, {
      generatedAt,
      readinessScope,
    });
  ...
  const tiktokCreatorRewardsVariantJobs = updated
    ...
    .filter(Boolean);
  const activeVariantRepairJobs = variantRepairJobs.filter((job) => job.readiness_scope === "active");
  const quarantinedVariantRepairJobs = variantRepairJobs.filter((job) => job.readiness_scope !== "active");
  const activeTiktokCreatorRewardsVariantJobs = tiktokCreatorRewardsVariantJobs.filter(
    (job) => job.readiness_scope === "active",
  );
  const quarantinedTiktokCreatorRewardsVariantJobs = tiktokCreatorRewardsVariantJobs.filter(
    (job) => job.readiness_scope !== "active",
  );
  ...
  return {
    ...
    summary: {
      ...
      active_variant_repair_required_count: activeVariantRepairJobs.length,
      quarantined_variant_repair_required_count: quarantinedVariantRepairJobs.length,
      tiktok_creator_rewards_variant_required_count: tiktokCreatorRewardsVariantJobs.length,
      active_tiktok_creator_rewards_variant_required_count:
        activeTiktokCreatorRewardsVariantJobs.length,
      quarantined_tiktok_creator_rewards_variant_required_count:
        quarantinedTiktokCreatorRewardsVariantJobs.length,
    },
    updated,
    blocked,
...
function renderGoalPlatformDurationContractMarkdown(report = {}) {
  ...
  lines.push(`Active variant rerenders required: ${report.summary?.active_variant_repair_required_count || 0}`);
  lines.push(`Quarantined variant rerenders required: ${report.summary?.quarantined_variant_repair_required_count || 0}`);
  lines.push(`TikTok creator-rewards variants required: ${report.summary?.tiktok_creator_rewards_variant_required_count || 0}`);
  lines.push(`Active TikTok creator-rewards variants required: ${report.summary?.active_tiktok_creator_rewards_variant_required_count || 0}`);
  lines.push(`Quarantined TikTok creator-rewards variants required: ${report.summary?.quarantined_tiktok_creator_rewards_variant_required_count || 0}`);
  lines.push("");
