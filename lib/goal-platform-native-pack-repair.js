...
async function refreshStoryPackageEntriesFromArtifacts(storyPackages = [], {
  ...
  for (const storyPackage of asArray(storyPackages)) {
    ...
    const mediaHouseScore = await readJsonIfPresent(path.join(artifactDir, "pulse_media_house_score.json"), {});
    ...
    const blockers = [
      ...
      ...asArray(publishVerdict.reason_codes),
    ...
    const canPromoteGreen =
      ...
      blockers.length === 0 &&
      platformNativePass &&
      cleanText(mediaHouseScore.verdict).toUpperCase() !== "RED";
    const next = canPromoteGreen
      ? {
          ...storyPackage,
          verdict: "GREEN",
          blockers: [],
