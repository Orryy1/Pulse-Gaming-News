...
const CORE_PUBLISH_PLATFORMS = Object.freeze([
  "youtube_shorts",
  "instagram_reels",
  "facebook_reels",
  "tiktok",
...
function normalisePublishScopePlatforms(platforms = DEFAULT_ENABLED_RIGHTS_PLATFORMS) {
  const aliases = {
  ...
  return [
    ...new Set(
      asArray(platforms)
        ...
        .filter(Boolean)
        .map((platform) => aliases[platform] || platform)
        .filter((platform) => CORE_PUBLISH_PLATFORMS.includes(platform)),
    ),
  ].sort();
}

function withExplicitPlatformScope(outputs = {}, platforms = DEFAULT_ENABLED_RIGHTS_PLATFORMS) {
  const enabledPlatforms = normalisePublishScopePlatforms(platforms);
  const next = { ...outputs };
  for (const platform of CORE_PUBLISH_PLATFORMS) {
    if (enabledPlatforms.includes(platform)) {
      ...
      if (cleanText(enabledOutput.status).toLowerCase() === "disabled") {
        delete enabledOutput.status;
      }
      delete enabledOutput.disabled_reason;
      if (enabledOutput.can_auto_publish === false) delete enabledOutput.can_auto_publish;
      if (enabledOutput.planned_action == null) delete enabledOutput.planned_action;
      if (!asArray(enabledOutput.planned_actions).length) delete enabledOutput.planned_actions;
      next[platform] = enabledOutput;
      continue;
    }
    next[platform] = {
      ...(next[platform] || {}),
      ...
      status: "disabled",
      disabled_reason:
        "Candidate rights evidence is not cleared for commercial publication on this platform.",
...
async function inspectArtifact(storyPackage = {}, {
  ...
  const scopedNative = withExplicitPlatformScope(native.outputs, rightsPlatforms);
  const targetOutputs = scopedNative.outputs;
  ...
  const targetPlatformManifest = {
    ...platformManifest,
    enabled_platforms: scopedNative.enabled_platforms,
  ...
  const targetCoverHeadline = cleanText(targetOutputs?.youtube_shorts?.cover_frame?.headline);
  const targetTitle = cleanText(targetOutputs?.youtube_shorts?.title);
  const targetDescription = cleanText(targetOutputs?.youtube_shorts?.description);
  ...
  const affiliateOutputStale = snapshotsDiffer(
    ...
    affiliateOutputSnapshot(targetOutputs || {}),
  );
  const platformScopeStale =
    JSON.stringify(asArray(platformManifest.enabled_platforms).map(cleanText).sort()) !==
      JSON.stringify(scopedNative.enabled_platforms) ||
    CORE_PUBLISH_PLATFORMS.some((platform) => {
      const enabled = scopedNative.enabled_platforms.includes(platform);
      const output = platformManifest.outputs?.[platform] || {};
      return enabled
        ? cleanText(output.operational_state).toLowerCase() !== "enabled" ||
          cleanText(output.status).toLowerCase() === "disabled" ||
          Boolean(cleanText(output.disabled_reason)) ||
          output.can_auto_publish === false ||
          (Object.hasOwn(output, "planned_action") && output.planned_action == null) ||
          (Object.hasOwn(output, "planned_actions") && !asArray(output.planned_actions).length)
        : cleanText(output.operational_state || output.status).toLowerCase() !== "disabled" ||
          output.can_auto_publish !== false ||
          Boolean(cleanText(output.planned_action)) ||
          asArray(output.planned_actions).length > 0 ||
          !cleanText(output.disabled_reason);
    });
  ...
  return {
    ...
    target_affiliate_output: affiliateOutputSnapshot(targetOutputs || {}),
    ...
    target_enabled_platforms: scopedNative.enabled_platforms,
    platform_scope_stale: platformScopeStale,
...
async function reconcileCurrentPackageDerivedEvidence({
  artifactDir,
  storyId,
  generatedAt = new Date().toISOString(),
  finalAvReviewValidator = validateFinalAvReviewFile,
  ...
  const resolvedArtifactDir = artifactDir ? path.resolve(artifactDir) : "";
  const resolvedStoryId = cleanText(storyId || path.basename(resolvedArtifactDir));
  if (!resolvedArtifactDir || !(await fs.pathExists(resolvedArtifactDir))) {
  ...
  const mediaHouseScorePath = path.join(resolvedArtifactDir, "pulse_media_house_score.json");
  const publishVerdictPath = path.join(resolvedArtifactDir, "publish_verdict.json");
  const [platformManifest, renderManifest] = await Promise.all([
    readJsonIfPresent(platformManifestPath, {}),
    readJsonIfPresent(path.join(resolvedArtifactDir, "render_manifest.json"), {}),
  ]);
  const finalRenderEvidence = await inspectFinalPublishRender(resolvedArtifactDir, renderManifest);
  const rightsReport = await inspectRightsStoryPackage(
    { story_id: resolvedStoryId, artifact_dir: resolvedArtifactDir },
    { workspaceRoot: process.cwd(), platforms: normaliseRightsPlatforms(rightsPlatforms) },
  );
  const rightsBlockers = rightsPublishBlockers(rightsReport);
  const declaredFinalMp4 = cleanText(
    renderManifest.output_path || renderManifest.output || renderManifest.render_path,
  );
  const finalAvReview = await finalAvReviewValidator(
  ...
  const publishControlTower = finalAvReviewGreen
    ? { verdict: "GREEN", can_auto_publish: true, reason_codes: [], blockers: [] }
    : {
        verdict: "RED",
        can_auto_publish: false,
        reason_codes: finalAvReviewBlockers.map((blocker) => `control:${blocker}`),
  ...
  const mediaHouseScore = await refreshPulseMediaHouseScore({
    ...
    storyId: resolvedStoryId,
    generatedAt,
    platformManifestOverride: platformManifest,
  });
  const [
    scriptScorecard,
    audioManifest,
    ...
    readJsonIfPresent(path.join(resolvedArtifactDir, "script_scorecard.json"), {}),
    readJsonIfPresent(path.join(resolvedArtifactDir, "audio_manifest.json"), {}),
    readJsonIfPresent(path.join(resolvedArtifactDir, "footage_inventory.json"), {}),
    readJsonIfPresent(path.join(resolvedArtifactDir, "director_beat_map.json"), {}),
    readJsonIfPresent(path.join(resolvedArtifactDir, "benchmark_report.json"), {}),
    readJsonIfPresent(path.join(resolvedArtifactDir, "materialised_motion_clips.json"), {}),
  ]);
  const publishVerdict = buildPackagePublishVerdict({
    ...
    directorBeatMap,
    benchmarkReport,
    platformNativeEvidence: platformManifest.platform_native_evidence,
    mediaHouseScore,
    finalPublishBlockers: [
      ...finalPublishEvidenceBlockers({
        story: {
  ...
  const mediaHouseGreen =
    (statusLabel(mediaHouseScore.verdict) === "GREEN" || statusLabel(mediaHouseScore.status) === "PASS") &&
    !asArray(mediaHouseScore.hard_failures).length;
  const packageGreen =
    statusLabel(publishVerdict.verdict || publishVerdict.status) === "GREEN" &&
    publishVerdict.can_auto_publish === true &&
    !asArray(publishVerdict.blockers).length;
  const canAutoPublish =
    ...
    finalAvReviewGreen;
  const reconciledPlatformManifest = {
    ...platformManifest,
    publish_status: canAutoPublish ? "GREEN" : "RED",
    can_auto_publish: canAutoPublish,
    blockers: asArray(publishVerdict.blockers).map(cleanText).filter(Boolean),
    reason_codes: asArray(publishVerdict.reason_codes).map(cleanText).filter(Boolean),
    derived_evidence_reconciled_at: generatedAt,
    no_publish_triggered: true,
  };
  await Promise.all([
    fs.writeJson(mediaHouseScorePath, mediaHouseScore, { spaces: 2 }),
    fs.writeJson(publishVerdictPath, publishVerdict, { spaces: 2 }),
    fs.writeJson(platformManifestPath, reconciledPlatformManifest, { spaces: 2 }),
  ]);
  return {
    story_id: resolvedStoryId,
    artifact_dir: resolvedArtifactDir,
    platform_native_verdict: platformManifest.platform_native_evidence?.verdict || "missing",
    media_house_verdict: mediaHouseScore.verdict || mediaHouseScore.status || "RED",
    media_house_hard_failures: asArray(mediaHouseScore.hard_failures).map(cleanText).filter(Boolean),
    publish_verdict: publishVerdict.verdict || publishVerdict.status || "RED",
    publish_blockers: asArray(publishVerdict.blockers).map(cleanText).filter(Boolean),
    final_render_ready: finalRenderEvidence.ready === true,
    rights_status: rightsReport.status || "blocked",
    final_av_review_green: finalAvReviewGreen,
    can_auto_publish: canAutoPublish,
    safety: {
      no_publish_triggered: true,
...
async function applyRepair(item = {}, { generatedAt = new Date().toISOString(), backupRoot = "" } = {}) {
  ...
  const repairedPlatformManifest = {
    ...
    enabled_platforms: asArray(item.target_enabled_platforms).map(cleanText).filter(Boolean),
...
module.exports = {
  ...
  PLATFORM_PACK_FILES,
  reconcileCurrentPackageDerivedEvidence,
