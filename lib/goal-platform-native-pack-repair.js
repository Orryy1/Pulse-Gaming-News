...
async function inspectArtifact(storyPackage = {}, { generatedAt = new Date().toISOString() } = {}) {
  ...
  const targetPublishStatus =
    greenGovernance && native.platformNativeEvidence.verdict === "pass"
      ? "GREEN"
  ...
  const publishStatusStale =
    Boolean(currentPublishStatus && targetPublishStatus) &&
    currentPublishStatus !== targetPublishStatus;
  const needsRepair =
