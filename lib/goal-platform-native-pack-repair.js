...
function canonicalWithNativeCover(canonical = {}, coverHeadline = "") {
  const headline = cleanText(coverHeadline);
  if (!headline) return canonical;
  return {
    ...canonical,
...
async function inspectArtifact(storyPackage = {}, { generatedAt = new Date().toISOString() } = {}) {
  ...
  const targetMediaHouseScore = artifactDir
    ? buildPulseMediaHouseScore({
        story_id: storyId,
        canonical: canonicalWithNativeCover(canonical, targetCoverHeadline),
...
async function applyRepair(item = {}, { generatedAt = new Date().toISOString(), backupRoot = "" } = {}) {
  ...
  if (Object.keys(canonical).length && targetCoverHeadline) {
    await fs.writeJson(
      canonicalPath,
      {
        ...canonicalWithNativeCover(canonical, targetCoverHeadline),
