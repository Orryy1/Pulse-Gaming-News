...
function platformRequiresSafeVariant(platform) {
  return ["instagram_reels", "facebook_reels"].includes(cleanText(platform));
}
...
function variantFreshAgainstRender({ output = {}, renderManifest = {}, baseDuration = null, platform = "" } = {}) {
...
function buildVariantJobsForPackage({ storyId, artifactDir, renderManifest = {}, platformManifest = {}, generatedAt } = {}) {
  ...
  for (const platform of PLATFORM_KEYS) {
    ...
    const existingVariantIsStale =
      existingVariantPath &&
      !variantFreshAgainstRender({ output, renderManifest, baseDuration, platform });
    const needsSafePlatformVariant = platformRequiresSafeVariant(platform) && !existingVariantPath;
