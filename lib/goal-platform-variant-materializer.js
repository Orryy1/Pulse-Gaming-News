...
} = require("./youtube-shorts-cover-frame");
...
function buildPlatformVariantFfmpegArgs({
  inputPath,
  outputPath,
  targetDurationS,
  platform,
  ...
  if (cleanText(platform) === "youtube_shorts" && cleanText(coverFramePath)) {
    const coverDuration = Math.max(0.3, Math.min(Number(coverFrameDurationS) || 0.6, 1));
    const fadeDuration = Math.min(0.15, coverDuration / 3);
    const fadeStart = Math.max(0, coverDuration - fadeDuration);
    const filter = [
      "[0:v]scale=1080:1920:force_original_aspect_ratio=decrease," +
        "pad=1080:1920:(ow-iw)/2:(oh-ih)/2,fps=30,format=yuv420p[base]",
      `[1:v]scale=1080:1920:flags=lanczos,trim=duration=${coverDuration.toFixed(3)},` +
        `setpts=PTS-STARTPTS,fps=30,format=rgba,` +
        `fade=t=out:st=${fadeStart.toFixed(3)}:d=${fadeDuration.toFixed(3)}:alpha=1[cover]`,
      "[base][cover]overlay=0:0:eof_action=pass:shortest=0,format=yuv420p[vout]",
    ].join(";");
    return [
      "-y",
      "-i",
...
async function variantFreshAgainstRender({
  ...
  if (
    cleanText(platformKey) === "youtube_shorts" &&
    coverFrameEvidence?.status === "ready"
  ) {
    const embedded = variant.embedded_cover_frame || {};
    if (
      cleanText(embedded.status) !== "ready" ||
      cleanText(embedded.cover_frame_sha256).toLowerCase() !==
        cleanText(coverFrameEvidence.cover_frame_sha256).toLowerCase() ||
      cleanText(embedded.campaign_manifest_sha256).toLowerCase() !==
        cleanText(coverFrameEvidence.campaign_manifest_sha256).toLowerCase() ||
      numberOrNull(embedded.cover_frame_size_bytes) !==
        numberOrNull(coverFrameEvidence.cover_frame_size_bytes)
    ) {
      return false;
    }
  }
...
async function buildVariantJobsForPackage({
  ...
  const youtubeOutput = platformManifest.outputs?.youtube_shorts || {};
  const youtubeCoverFrameEvidence = await resolveYouTubeShortsCoverFrame({
    artifactDir,
    workspaceRoot,
    storyId,
    expectedHeadline: cleanText(
      youtubeOutput.cover_frame?.headline ||
        youtubeOutput.cover_headline ||
        youtubeOutput.thumbnail_headline,
    ),
    explicitManifestPath: cleanText(
      storyPackage.premium_visual_campaign_manifest_path ||
        storyPackage.youtube_shorts_cover_manifest_path,
    ),
  });
  const jobs = [];
  for (const platform of PLATFORM_KEYS) {
    ...
    const existingVariantIsStale =
      ...
      !(await variantFreshAgainstRender({
        ...
        storyId,
        coverFrameEvidence:
          platform === "youtube_shorts" ? youtubeCoverFrameEvidence : null,
      }));
    ...
    jobs.push({
      ...
      ...(platform === "youtube_shorts"
        ? { youtube_shorts_cover_frame: youtubeCoverFrameEvidence }
        : {}),
    });
  }
  return jobs;
...
async function inspectStoryPackage(
  storyPackage = {},
  {
...
async function materializeVariantJob(job = {}, { variantRenderer, probeDuration, generatedAt } = {}) {
  ...
  const coverFrameEvidence =
    job.platform === "youtube_shorts" &&
    job.youtube_shorts_cover_frame?.status === "ready"
      ? job.youtube_shorts_cover_frame
      : null;
  const coverBefore = coverFrameEvidence
    ? await fileFingerprint(coverFrameEvidence.cover_frame_path)
    : null;
  if (
    coverFrameEvidence &&
    (
      !coverBefore ||
      coverBefore.sha256 !== cleanText(coverFrameEvidence.cover_frame_sha256).toLowerCase() ||
      coverBefore.size_bytes !== numberOrNull(coverFrameEvidence.cover_frame_size_bytes)
    )
  ) {
    throw new Error("youtube_shorts_cover_frame_fingerprint_changed_before_render");
  }
  await variantRenderer({
    ...
    coverFramePath: coverFrameEvidence?.cover_frame_path || "",
    coverFrameDurationS: coverFrameEvidence?.embed_duration_s || 0.6,
  });
  ...
  if (coverFrameEvidence) {
    const coverAfter = await fileFingerprint(coverFrameEvidence.cover_frame_path);
    if (
      !coverAfter ||
      coverAfter.sha256 !== coverBefore.sha256 ||
      coverAfter.size_bytes !== coverBefore.size_bytes
    ) {
      throw new Error("youtube_shorts_cover_frame_changed_during_variant_render");
    }
  }
  ...
  const variantEvidence = {
    ...
    transformation_mode: coverFrameEvidence
...
async function materializeGoalPlatformVariants({
  ...
  for (const storyPackage of asArray(storyPackages)) {
    inspected.push(await inspectStoryPackage(storyPackage, {
      generatedAt,
      workspaceRoot,
    }));
  }
