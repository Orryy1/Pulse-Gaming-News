...
const { PRIMARY_PULSE_CTA, normalisePulseCtaText } = require("./pulse-cta");
const {
...
const FINAL_RENDER_MISSING_ONLY_BLOCKERS = new Set([
  "render:final_publish_render_missing",
  "render:final_render_audio_missing",
  "render:final_render_not_decodable",
  "render:final_render_duration_too_short",
  "render:final_render_path_invalid",
  "render:final_render_missing",
  "render:final_render_fingerprint_unavailable",
...
function premiumLaneCandidatesFor(source = {}) {
  const renderManifest = objectValue(source.render_manifest || source.renderManifest);
  return [
    source.premiumLane,
    source.premium_lane,
    source.studio?.premiumLane,
    source.studio?.premium_lane,
    renderManifest.premiumLane,
    renderManifest.premium_lane,
  ].map(objectValue).filter(hasObjectKeys);
}

function looksLikeHyperframesLane(value = {}) {
  return Boolean(
    hasObjectKeys(value) &&
      (
        cleanText(value.rendererSplit || value.renderer_split).toLowerCase().includes("hyperframes") ||
...
function hyperframesGateSources({ storyPackage = {}, renderManifest = {}, renderStory = {} } = {}) {
  const base = [
    objectValue(storyPackage),
    objectValue(renderManifest),
    objectValue(renderStory),
    objectValue(storyPackage.studio),
    objectValue(renderStory.studio),
  ];
  const nested = base.flatMap(premiumLaneCandidatesFor);
  return [...base, ...nested].filter(hasObjectKeys);
}

function premiumShellGateFromSources(sources = []) {
  for (const source of sources) {
    const gate = objectValue(
      source.hyperframesPremiumShellGate ||
        source.premiumShellGate ||
        source.hyperframes_premium_shell_gate ||
        source.premium_shell_gate,
    );
    if (hasObjectKeys(gate)) return gate;
  }
  return {};
}

function hyperframesCardCountFromSources(sources = []) {
  let count = 0;
  for (const source of sources) {
    count = Math.max(
      count,
      firstNumber(source.hyperframesCardCount, source.hyperframes_card_count) || 0,
    );
  }
  return count;
}

function premiumShellPassCountFromSources(sources = [], gate = {}) {
  for (const source of sources) {
    const count = firstNumber(source.premiumShellPassCount, source.premium_shell_pass_count);
    if (count != null) return count;
  }
  return firstNumber(gate.passCount, gate.pass_count);
}

function premiumShellRequiredPassCountFromSources(sources = [], gate = {}) {
  for (const source of sources) {
    const count = firstNumber(
      source.premiumShellRequiredPassCount,
      source.premium_shell_required_pass_count,
      source.hyperframesPremiumShellRequiredPassCount,
      source.hyperframes_premium_shell_required_pass_count,
    );
    if (count != null) return count;
  }
  return firstNumber(gate.requiredPassCount, gate.required_pass_count) || MIN_PREMIUM_HYPERFRAMES_CARDS;
}

function premiumShellVerdictFromSources(sources = [], gate = {}) {
  for (const source of sources) {
    const verdict = cleanText(source.premiumShellVerdict || source.premium_shell_verdict).toLowerCase();
    if (verdict) return verdict;
  }
  return cleanText(gate.verdict || gate.status).toLowerCase();
}

function hyperframesPremiumShellRequiredFromSources(sources = []) {
  return sources.some((source) =>
    truthyFlag(
      source.hyperframesPremiumShellRequired ||
        source.hyperframes_premium_shell_required ||
        source.requireHyperframesPremiumShell ||
        source.require_hyperframes_premium_shell,
    ),
  );
}

function declaresHyperframesPremiumFromSources(sources = []) {
  const text = sources
    .flatMap((source) => [
      source.render_lane,
      source.lane,
      source.renderer,
      source.render_renderer,
      source.rendererSplit,
      source.renderer_split,
      typeof source.premiumLane === "string" ? source.premiumLane : "",
      typeof source.premium_lane === "string" ? source.premium_lane : "",
    ])
    .map(cleanText)
    .join(" ")
    .toLowerCase();
  return (
    hyperframesPremiumShellRequiredFromSources(sources) ||
    hyperframesCardCountFromSources(sources) > 0 ||
    text.includes("hyperframes") ||
    sources.some(looksLikeHyperframesLane)
  );
}

function hyperframesPremiumShellBlockers({ storyPackage = {}, renderManifest = {}, renderStory = {} } = {}) {
  const sources = hyperframesGateSources({ storyPackage, renderManifest, renderStory });
  if (!declaresHyperframesPremiumFromSources(sources)) return [];
  const gate = premiumShellGateFromSources(sources);
  const required = hyperframesPremiumShellRequiredFromSources(sources);
  const requiredPassCount = premiumShellRequiredPassCountFromSources(sources, gate);
  const passCount = premiumShellPassCountFromSources(sources, gate);
  const verdict = premiumShellVerdictFromSources(sources, gate);
  const passCountMissing = passCount == null;
  const passCountTooLow = !passCountMissing && passCount < requiredPassCount;
  if (verdict === "pass" && !passCountMissing && !passCountTooLow) return [];
  return unique([
    "hyperframes_premium_shell_not_passed",
    required ? "hyperframes_premium_shell_required" : "",
    ...
    passCountTooLow ? `hyperframes_premium_shell_pass_count_below_required:${passCount}/${requiredPassCount}` : "",
    ...sources.flatMap((source) =>
      [
        ...asArray(source.premiumShellBlockers),
        ...asArray(source.premium_shell_blockers),
      ].map((blocker) => `hyperframes_premium_shell:${blocker}`),
    ),
...
const SPOKEN_NUMBER_REPLACEMENTS = [
  [/\btwenty\s+twenty\s+one\b/g, "2021"],
  [/\btwenty\s+twenty\s+two\b/g, "2022"],
  [/\btwenty\s+twenty\s+three\b/g, "2023"],
  [/\btwenty\s+twenty\s+four\b/g, "2024"],
  [/\btwenty\s+twenty\s+five\b/g, "2025"],
  [/\btwenty\s+twenty\s+six\b/g, "2026"],
  [/\btwenty\s+twenty\s+seven\b/g, "2027"],
  [/\btwenty\s+twenty\s+eight\b/g, "2028"],
  [/\btwenty\s+twenty\s+nine\b/g, "2029"],
  [/\btwenty\s+twenty\b/g, "2020"],
  [/\btwenty\s+thirty\b/g, "2030"],
  [/\btwenty\s+seventy\s+seven\b/g, "2077"],
];

function normalizeSpokenNumberPhrases(value = "") {
  let text = value;
  for (const [pattern, replacement] of SPOKEN_NUMBER_REPLACEMENTS) {
...
function spokenFreshnessKey(value = "") {
  return normalizeSpokenNumberPhrases(cleanText(value).toLowerCase())
    .replace(/\bplaystation\s+five\b/g, "ps5")
    .replace(/\bplaystation\s+5\b/g, "ps5")
    .replace(/\bplaystation\s+four\b/g, "ps4")
    .replace(/\bplaystation\s+4\b/g, "ps4")
...
function asrWordTimestampCoverage({ payload = {}, canonical = {} } = {}) {
  ...
  const strictAlignment = payload?.meta?.timestampWhisperAlignment ||
    payload?.meta?.timestamp_whisper_alignment || null;
  if (
    strictAlignment?.repaired === true &&
    Number(strictAlignment.script_inserted_actual_word_count || 0) === 0 &&
    Number(strictAlignment.script_trailing_actual_word_count || 0) === 0 &&
    strictAlignment.script_opening_covered === true &&
    Number(strictAlignment.script_coverage_ratio) > 0
  ) {
    return {
      ratio: Number(strictAlignment.script_coverage_ratio),
      expected_word_count: Number(strictAlignment.script_expected_word_count) || 0,
      actual_word_count: Number(strictAlignment.script_actual_word_count) || 0,
      opening_covered: true,
      expected_opening: null,
      actual_opening: null,
      strict_alignment_evidence: true,
    };
  }
...
async function refreshSchedulerCaptionSrt({
  ...
  const voiceQuality = await readJsonIfPresent(path.join(artifactDir, "voice_quality_report.json"), {});
  const voiceQualityGeneratedAt = cleanText(
    voiceQuality.generated_at || voiceQuality.generatedAt || voiceQuality.checked_at,
  );
  const voiceQualityPass = /^(pass|passed|green|ok)$/i.test(
    cleanText(voiceQuality.verdict || voiceQuality.status),
  );
  const voiceQualityWordCount = Number(voiceQuality.word_timestamp_count || voiceQuality.word_count);
  const preserveVoiceQualityFreshness =
    voiceQualityPass &&
    voiceQualityGeneratedAt &&
    (!Number.isFinite(voiceQualityWordCount) || voiceQualityWordCount === words.length);
  const captionGeneratedAt = preserveVoiceQualityFreshness ? voiceQualityGeneratedAt : generatedAt;
  ...
  await fs.writeJson(path.join(artifactDir, "caption_manifest.json"), {
    ...
    generated_at_source: preserveVoiceQualityFreshness
      ? "preserved_voice_quality_timestamp_for_same_word_timestamps"
      : "production_cutover_refresh",
...
function isLocalProofTransitionPackage(storyPackage = {}) {
  const verdict = cleanText(storyPackage.verdict || storyPackage.status).toLowerCase();
  return (
    storyPackage.local_promotion_only === true ||
    storyPackage.localPromotionOnly === true ||
    LOCAL_PROOF_PENDING_VERDICTS.has(verdict)
  );
}

function isFinalRenderMissingOnlyPackage(storyPackage = {}) {
  const blockers = asArray(storyPackage.blockers).map(cleanText).filter(Boolean);
  return blockers.length > 0 && blockers.every((blocker) => FINAL_RENDER_MISSING_ONLY_BLOCKERS.has(blocker));
}
...
function renderInputRequirements(footageInventory = {}, director = {}) {
  ...
  const availableMotionClips = Number(motionBudget.available_motion_clips);
  const clips =
    Number.isFinite(availableMotionClips) &&
    availableMotionClips >= DEFAULT_FINAL_RENDER_INPUT_REQUIREMENTS.min_materialised_motion_clips &&
    availableMotionClips < rawClips
      ? availableMotionClips
      : rawClips;
  const families =
  ...
  for (const clip of candidates) {
    ...
    if (readyPath) {
      ...
      materialised.push({
        ...
        path: readyPath,
        source_url: cleanText(clip.asset?.source_url || clip.asset?.url),
        source_type: cleanText(clip.asset?.source_type || clip.asset?.type),
        media_kind: cleanText(clip.asset?.media_kind || clip.asset?.kind),
        source_url_kind: cleanText(clip.asset?.source_url_kind || clip.asset?.url_kind),
        ...
        direct_video: isDirectVideoMotionAsset(clip.asset),
      });
    }
  }
...
function realMotionInputReadiness({
  ...
  const directVideoMotionClipFloor = productPageHybridMotionReady({ canonical, motion })
    ? Math.min(2, realVisualMotionClipFloor)
    : realVisualMotionClipFloor;
...
function productPageHybridMotionReady({ canonical = {}, motion = {} } = {}) {
  const text = [
    ...
    canonical.official_confirmation_source,
    ...asArray(canonical.confirmed_claims),
  ].map(cleanText).join(" ").toLowerCase();
  const accessoryStory =
    /\b(?:controller|headset|accessor(?:y|ies)|hardware|product|store|deal|price|setup|edition)\b/.test(text) &&
    /\b(?:xbox|playstation|nintendo|steam|storefront|official)\b|(?:xbox|playstation|nintendo|steam)\.com/.test(text);
  if (!accessoryStory) return false;
  const clips = asArray(motion.clips);
  const officialProductDirect = clips.filter((clip) => {
    if (clip.direct_video !== true) return false;
    const clipText = [
      clip.source_type,
      clip.media_kind,
    ...
    return /official_platform_product_page|official.*product|xboxservices\.com|playstation\.com|nintendo\.com|steampowered\.com/.test(clipText);
  }).length;
  const officialStillMotion = clips.filter((clip) => {
    const clipText = [
      clip.source_type,
      clip.media_kind,
      clip.source_url_kind,
    ...
    return (
      clip.real_media === true &&
      /official_press_kit_stills|source_documented_transformative_editorial_use|visual_still|screenshot|cms-assets\.xboxservices\.com|storefront/.test(clipText)
    );
  }).length;
  return officialProductDirect >= 2 && officialStillMotion >= 4 && clips.length >= 6;
}
...
function productionRenderTarget(storyId, artifactDir) {
  return {
    ...
    hyperframes_premium_shell_required_pass_count: MIN_PREMIUM_HYPERFRAMES_CARDS,
    ...currentRenderPolicyManifest(),
...
function schedulerExportedRenderPath({ artifactDir = "", renderManifest = {} } = {}) {
  const outputName = path.basename(cleanText(renderManifest.output) || "visual_v4_render.mp4");
  const artifactOutputPath = artifactDir ? path.join(artifactDir, outputName) : "";
  if (artifactOutputPath && fs.existsSync(artifactOutputPath)) return artifactOutputPath;
  return firstText(renderManifest.output_path, artifactOutputPath);
}
...
function normaliseSchedulerRightsRecord(record = {}) {
...
function schedulerPublishableRightsRecords(records = []) {
  return asArray(records)
    .map(normaliseSchedulerRightsRecord)
    .filter((record) => {
      const licenceBasis = cleanText(record.licence_basis);
      if (!licenceBasis || record.commercial_use_allowed === false) return false;
      const evidence = [
        ...
        .map(cleanText)
        .join(" ");
      return /approved|owned|official|licensed|source_documented|storefront|steam|publisher/i.test(evidence);
    });
}
...
function bridgeClipFromAsset(asset = {}, index = 0, fallbackSourceType = "owned_motion_clip") {
  ...
  const directVideoLike = clipLooksDirectVideo(normalizedAsset);
  const stillDerived = clipLooksStillDerived(normalizedAsset);
  const fallbackRightsBasis = directVideoLike
    ? "official_reference_transformative_editorial_use"
    : stillDerived && clipHasDocumentedSource(normalizedAsset)
      ? "source_documented_transformative_editorial_use"
      : stillDerived
        ? "missing_source_documentation_for_still_derived_motion"
  ...
  return {
    ...
    source_owner: cleanText(asset.source_owner || asset.owner || asset.rights_owner || ""),
    source_title: cleanText(asset.source_title || asset.title || asset.asset_title || ""),
...
function sourceOwnerSlug(value = "") {
  return cleanText(value)
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "_")
    .replace(/^_+|_+$/g, "")
...
function enrichBridgeClipWithRights(clip = {}, rightsRecords = []) {
  const record = rightsRecords.find((candidate) => existingRightsRecordCoversClip(candidate, clip)) || {};
  const sourceOwner = cleanText(clip.source_owner || record.source_owner || record.owner || record.rights_owner);
  const sourceTitle = cleanText(clip.source_title || record.source_title || record.title || record.asset_title);
  const sourceFamily = cleanText(clip.source_family);
  const ownerSlug = sourceOwnerSlug(sourceOwner || sourceTitle);
  return {
    ...clip,
    source_owner: sourceOwner,
    source_title: sourceTitle,
    source_family:
      ownerSlug && /^url:/i.test(sourceFamily)
        ? `${ownerSlug}_${sourceFamily}`
        : sourceFamily,
  };
}

function enrichBridgeClipsWithRights(clips = [], rightsLedger = {}) {
  const rightsRecords = [
    ...rightsAssetsFromLedger(rightsLedger),
    ...rightsRecordsFromLedger(rightsLedger),
  ];
  if (!rightsRecords.length) return clips;
  return clips.map((clip) => enrichBridgeClipWithRights(clip, rightsRecords));
}
...
function clipLooksDirectVideo(clip = {}) {
  return /direct_video|hls_manifest|dash_manifest|official_trailer_segment|official_video|official_platform_product_page|official_social_media_video|official_game_page_direct_video|steam_storefront_video_reference|steam_movie|gameplay|trailer/i.test(
    clipRightsText(clip),
  );
}

function clipLooksStillDerived(clip = {}) {
  const text = clipRightsText(clip);
  return /visual_still|screenshot|key_art|hero|capsule|storefront_image|still/i.test(text) &&
    !clipLooksDirectVideo(clip);
}

function clipHasDocumentedSource(clip = {}) {
  const sourceUrl = cleanText(clip.source_url || clip.url);
  return /^https?:\/\//i.test(sourceUrl);
}

function clipLooksOwnedGenerated(clip = {}) {
  return /owned|generated|internally_generated|pulse-generated|visual_v4_production_render/i.test(
    clipRightsText(clip),
  );
}

function recordLooksMotionRights(record = {}) {
  return /video|motion|clip|trailer|render|screenshot_derived/i.test(
    [
      record.kind,
      record.type,
      record.asset_type,
...
function sameRightsKey(left = "", right = "") {
  const a = cleanText(left).replace(/\\/g, "/").toLowerCase();
  const b = cleanText(right).replace(/\\/g, "/").toLowerCase();
  return Boolean(a && b && a === b);
}

function existingRightsRecordCoversClip(record = {}, clip = {}) {
  if (sameRightsKey(record.path, clip.path)) return true;
  if (sameRightsKey(record.source_url || record.url, clip.source_url || clip.url) && recordLooksMotionRights(record)) {
    return true;
  }
  if (
    cleanText(record.asset_id) &&
    cleanText(record.asset_id) === cleanText(clip.id || clip.asset_id) &&
    recordLooksMotionRights(record)
  ) {
    return true;
  }
  return false;
...
function bridgeVideoClips({
  ...
  if (selectedRenderClips.length) return dedupeBridgeClips(enrichBridgeClipsWithRights(selectedRenderClips, rightsLedger));
  ...
  if (footageClips.length) return dedupeBridgeClips(enrichBridgeClipsWithRights(footageClips, rightsLedger));
...
function buildFinalRenderRightsRecord(clip = {}, storyId = "") {
  const stillDerived = clipLooksStillDerived(clip);
  const directVideoLike = clipLooksDirectVideo(clip);
  const documentedSource = clipHasDocumentedSource(clip);
  const ownedGenerated = clipLooksOwnedGenerated(clip) || cleanText(clip.source_url).startsWith("local://");
  const sourceType = cleanText(clip.source_type || "visual_v4_production_render");
  let assetType = "motion_clip";
  let licenceBasis = cleanText(clip.licence_basis || clip.license_basis || clip.rights_basis || clip.rights_risk_class);
  let allowedUse = cleanText(clip.allowed_use || "transformative_editorial_short_form");
  let approvalStatus = cleanText(clip.approval_status || "approved_for_transformative_editorial_use");
  let commercialUseAllowed = clip.commercial_use_allowed === false ? false : true;
  let riskScore = Number.isFinite(Number(clip.risk_score)) ? Number(clip.risk_score) : 0.28;
  let transformationNotes = cleanText(
    clip.transformation_notes ||
      "Trimmed into a short, source-labelled Pulse Gaming editorial motion beat for a governed V4 render.",
  );

  if (stillDerived && documentedSource) {
    assetType = "screenshot_derived_motion_clip";
    licenceBasis = "source_documented_transformative_editorial_use";
    allowedUse = "screenshot_derived_editorial_motion";
    transformationNotes =
      "Source-documented still transformed into a short, source-labelled Pulse Gaming editorial motion beat for a governed V4 render.";
    riskScore = Number.isFinite(Number(clip.risk_score)) ? Number(clip.risk_score) : 0.32;
  } else if (stillDerived) {
    assetType = "screenshot_derived_motion_clip";
    licenceBasis = "missing_source_documentation_for_still_derived_motion";
    allowedUse = "not_publishable_until_source_rights_are_documented";
    approvalStatus = "review_required";
    commercialUseAllowed = false;
    transformationNotes =
      "Still-derived motion clip is present, but the source URL is not documented enough for scheduler publication.";
    riskScore = Number.isFinite(Number(clip.risk_score)) ? Number(clip.risk_score) : 0.72;
  } else if (directVideoLike) {
    licenceBasis = licenceBasis || "official_reference_transformative_editorial_use";
    allowedUse = allowedUse || "short_transformative_editorial_reference";
  } else if (ownedGenerated) {
    assetType = "generated_motion_clip";
    licenceBasis = licenceBasis || "owned_generated_editorial_motion_graphic";
    allowedUse = allowedUse || "owned_editorial_motion_graphic";
    transformationNotes =
      "Generated Pulse Gaming motion graphic used inside a governed editorial render.";
    riskScore = Number.isFinite(Number(clip.risk_score)) ? Number(clip.risk_score) : 0.02;
  } else {
    licenceBasis = licenceBasis || "missing_source_documentation_for_motion_clip";
    allowedUse = "not_publishable_until_source_rights_are_documented";
    approvalStatus = "review_required";
    commercialUseAllowed = false;
    riskScore = Number.isFinite(Number(clip.risk_score)) ? Number(clip.risk_score) : 0.72;
...
function envEnabled(name) {
  return /^(true|1|yes|on)$/i.test(String(process.env[name] || ""));
}

function normaliseGovernancePlatform(value = "") {
  const key = cleanText(value).toLowerCase();
  if (key === "youtube_shorts" || key === "youtube_short" || key === "yt_shorts") return "youtube";
  if (key === "instagram_reels" || key === "instagram_reel" || key === "ig_reels") return "instagram";
  if (key === "facebook_reels" || key === "facebook_reel" || key === "fb_reels") return "facebook";
  if (key === "twitter") return "x";
  return key;
}

function governanceRefreshPlatforms(candidate = {}) {
  const explicit = asArray(
    candidate.enabled_platforms ||
      candidate.live_enabled_platforms ||
      candidate.governance_refresh_platforms,
  )
    .map(normaliseGovernancePlatform)
    .filter(Boolean);
  if (explicit.length) return unique(explicit);

  const platforms = ["youtube", "instagram", "facebook"];
  if (envEnabled("TIKTOK_ENABLED") || envEnabled("TIKTOK_AUTO_UPLOAD_ENABLED")) {
...
function refreshedGovernanceForBridgeCandidate(candidate = {}, { generatedAt = new Date().toISOString() } = {}) {
  try {
    ...
    return buildStudioGovernanceReport({
      ...
      platforms: governanceRefreshPlatforms(candidate),
      generatedAt,
...
function refreshPlatformDurationEvidence(
  platformManifest = {},
  {
    duration = null,
    durationLane = "",
    normalProductionDuration = false,
    generatedAt = new Date().toISOString(),
  } = {},
) {
  const renderedDuration = numberOrNull(duration);
  if (renderedDuration == null) return platformManifest;

  const outputs = {};
  for (const [platform, existing] of Object.entries(platformManifest.outputs || {})) {
    if (!existing || typeof existing !== "object" || Array.isArray(existing)) {
      outputs[platform] = existing;
      continue;
    }
    const durationWarnings = new Set(
      asArray(existing.duration_warnings).filter(
        (warning) =>
          warning !== "below_normal_production_duration_floor" &&
          warning !== "below_creator_rewards_duration",
      ),
    );
    if (normalProductionDuration && renderedDuration < NORMAL_PRODUCTION_TARGET_SECONDS.min) {
      durationWarnings.add("below_normal_production_duration_floor");
    }
    const creatorRewardsFields = {};
    if (platform === "tiktok") {
      const creatorRewardsEligible = renderedDuration >= 61 && renderedDuration <= 90;
    ...
    outputs[platform] = {
      ...existing,
      ...(normalProductionDuration
        ? {
            duration_strategy: "normal_production_safe_script_expansion",
            target_duration_seconds: { ...NORMAL_PRODUCTION_TARGET_SECONDS },
            strategic_duration_seconds: { ...NORMAL_PRODUCTION_TARGET_SECONDS },
          }
        : {}),
  ...
  return {
    ...
    rendered_duration_s: renderedDuration,
    duration_lane: durationLane || platformManifest.duration_lane || null,
    ...(normalProductionDuration
      ? {
...
function applyGovernanceRefreshToBridgeCandidate(candidate = {}, { generatedAt = new Date().toISOString() } = {}) {
  ...
  const refreshedPlatformManifest = refreshPlatformDurationEvidence(
    {
    ...
    },
    {
      ...
      durationLane: candidate.duration_lane,
      normalProductionDuration: candidate.duration_lane === "normal_production",
      generatedAt,
    },
  );
  return {
    ...
    platform_publish_manifest: refreshedPlatformManifest,
  };
}
...
function buildSchedulerBridgeCandidate({
  ...
  const exportedPath = schedulerExportedRenderPath({ artifactDir, renderManifest });
  const captionsPath = path.join(artifactDir, "captions.srt");
  ...
  const bridgeAudioManifest = {
    ...narrationManifest,
    ...audioManifest,
    narration_audio_path: audioPath || firstText(audioManifest.narration_audio_path, narrationManifest.narration_audio_path),
    audio_path: audioPath || firstText(audioManifest.audio_path, narrationManifest.audio_path, narrationManifest.narration_audio_path),
    word_timestamps_path: timestampsPath || firstText(audioManifest.word_timestamps_path, narrationManifest.word_timestamps_path),
    timestamps_path: timestampsPath || firstText(audioManifest.timestamps_path, narrationManifest.timestamps_path),
    resolved_word_timestamps_path:
      timestampsPath ||
      firstText(audioManifest.resolved_word_timestamps_path, narrationManifest.resolved_word_timestamps_path),
  };
  ...
  const existingSchedulerRecords = schedulerPublishableRightsRecords(existingRecords);
  const finalRecords = videoClips
    .filter((clip) => !existingSchedulerRecords.some((record) => existingRightsRecordCoversClip(record, clip)))
    .map((clip) => buildFinalRenderRightsRecord(clip, storyId));
  const audioRecord = buildAudioRightsRecord({ storyId, audioPath, audioManifest: bridgeAudioManifest });
  const audioRecordNeeded = audioRecord && !existingSchedulerRecords.some((record) =>
  ...
  const productionRenderer = PRODUCTION_RENDERERS.has(cleanText(renderManifest.renderer));
  const productionTier = PRODUCTION_TIERS.has(cleanText(renderManifest.visual_tier));
  const inferredNormalProductionDuration =
    !explicitDurationLane &&
    Number.isFinite(duration) &&
    duration > 45 &&
    duration <= 60 &&
    (productionRenderer || productionTier);
  const normalProductionDuration =
    explicitDurationLane === "normal_production" ||
    inferredNormalProductionDuration ||
  ...
  const sourceLabel = firstText(
    canonical.source_card_label,
    canonical.primary_source,
    sourceManifest.primary_source?.name,
  );
  const hyperframesShellSources = hyperframesGateSources({ renderManifest, renderStory });
  const hyperframesPremiumShellGate = premiumShellGateFromSources(hyperframesShellSources);
  const hyperframesPremiumShellRequired = hyperframesPremiumShellRequiredFromSources(hyperframesShellSources);
  const hyperframesCardCount = hyperframesCardCountFromSources(hyperframesShellSources);
  const premiumShellVerdict = premiumShellVerdictFromSources(
    hyperframesShellSources,
    hyperframesPremiumShellGate,
  );
  const premiumShellPassCount = premiumShellPassCountFromSources(
    hyperframesShellSources,
    hyperframesPremiumShellGate,
  );
  const premiumShellRequiredPassCount = premiumShellRequiredPassCountFromSources(
    hyperframesShellSources,
    hyperframesPremiumShellGate,
  );
  const premiumShellBlockers = unique([
    ...asArray(hyperframesPremiumShellGate.blockers),
    ...hyperframesShellSources.flatMap((source) => [
      ...asArray(source.premiumShellBlockers),
      ...asArray(source.premium_shell_blockers),
    ]),
  ]);
  const primarySourceUrl = firstText(
    canonical.primary_source_url,
    canonical.source_url,
    canonical.url,
    sourceManifest.primary_source?.url,
    sourceManifest.source_url,
  );
  const sourcePublishedAt = firstText(
    canonical.source_published_at,
    canonical.primary_source_published_at,
    canonical.published_at,
    canonical.timestamp,
    sourceManifest.source_published_at,
    sourceManifest.primary_source?.published_at,
  );
  ...
  const candidate = {
    ...
    primary_source: firstText(canonical.primary_source, canonical.source_card_label, sourceManifest.primary_source?.name),
    primary_source_url: primarySourceUrl,
    source_published_at: sourcePublishedAt,
    confirmed_event_window:
      canonical.confirmed_event_window || sourceManifest.confirmed_event_window || null,
    source_age_policy_hours:
      numberOrNull(canonical.source_age_policy_hours) ??
      numberOrNull(sourceManifest.source_age_policy_hours) ??
      168,
    ...
    hyperframes_premium_shell_required: hyperframesPremiumShellRequired,
    hyperframes_card_count: hyperframesCardCount,
    hyperframes_premium_shell_gate: hyperframesPremiumShellGate,
    premium_shell_verdict: premiumShellVerdict || null,
    premium_shell_pass_count: premiumShellPassCount ?? null,
    premium_shell_required_pass_count: premiumShellRequiredPassCount,
    ...
    audio_duration: numberOrNull(bridgeAudioManifest.duration_seconds) ?? duration,
...
function buildSchedulerBridgeCtaUniquenessGate(candidates = []) {
  const inspectedCandidates = asArray(candidates);
  const canonicalCta = normalisePulseCtaText(PRIMARY_PULSE_CTA);
  const canonicalSpokenOutroCount = inspectedCandidates.filter((candidate) => {
    const narration = normalisePulseCtaText(
      firstText(candidate.full_script, candidate.tts_script, candidate.body),
    );
    return narration && narration.endsWith(canonicalCta);
  }).length;
  const blocked =
    inspectedCandidates.length > 1 &&
    canonicalSpokenOutroCount === inspectedCandidates.length;
  return {
    status: blocked ? "blocked" : "pass",
    blocker: blocked ? "anti_spam:canonical_spoken_cta_cohort_saturation" : null,
    inspected_candidate_count: inspectedCandidates.length,
    canonical_spoken_outro_count: canonicalSpokenOutroCount,
    canonical_cadence_allowed: true,
  };
}

function buildSchedulerBridge(inspected = [], { generatedAt = new Date().toISOString() } = {}) {
  const inspectedCandidates = inspected
    .filter((item) => item.status === "ready_for_dry_run_publish" && item.scheduler_candidate)
    .map((item) => item.scheduler_candidate);
  const ctaUniquenessGate = buildSchedulerBridgeCtaUniquenessGate(inspectedCandidates);
  const candidates = ctaUniquenessGate.status === "blocked" ? [] : inspectedCandidates;
  return {
    schema_version: 1,
    generated_at: generatedAt,
    mode: "SCHEDULER_BRIDGE_DRY_RUN",
    status:
      ctaUniquenessGate.status === "blocked"
        ? "blocked_cta_cohort_saturation"
        : candidates.length
          ? "ready_for_dry_run_preflight"
          : "no_ready_candidates",
    inspected_candidate_count: inspectedCandidates.length,
    candidate_count: candidates.length,
    blocked_candidate_ids:
      ctaUniquenessGate.status === "blocked"
        ? inspectedCandidates.map((candidate) => cleanText(candidate.id)).filter(Boolean)
        : [],
    cta_uniqueness_gate: ctaUniquenessGate,
    candidates,
    safety: {
      read_only: true,
      db_mutation: false,
...
function authoritativePublishControlBlockers(platformManifest = {}, publishVerdict = {}) {
  const platformStatus = cleanText(
    platformManifest.publish_status || platformManifest.status || platformManifest.verdict,
  ).toLowerCase();
  const controlStatus = cleanText(
    publishVerdict.verdict || publishVerdict.status || publishVerdict.publish_status,
  ).toLowerCase();
  const blockers = [];
  if (platformStatus === "red" && !isFinalRenderRepairOnlyAuthority(platformManifest)) {
    blockers.push("authoritative_platform_publish_manifest_red");
  }
  if (controlStatus === "red" && !isFinalRenderRepairOnlyAuthority(publishVerdict)) {
    blockers.push("authoritative_publish_verdict_red");
  }
  return blockers;
}

function authorityFailureCodes(value = {}) {
  return unique([
...
function isFinalRenderRepairOnlyAuthority(value = {}) {
  const blockers = authorityFailureCodes(value);
  return blockers.length > 0 && blockers.every((blocker) =>
    FINAL_RENDER_MISSING_ONLY_BLOCKERS.has(blocker)
  );
}

function authoritativePackageSummaryBlockers(packageSummary = {}) {
  const statusObjects = [
    packageSummary,
  ...
  const hasRed = statusObjects.some((value) => {
    const red = [
      value.final_verdict,
      ...
      value.status,
      value.result,
    ].some((status) => cleanText(status).toLowerCase() === "red") ||
      authorityFailureCodes(value).length > 0;
    return red && !isFinalRenderRepairOnlyAuthority(value);
  });
  return hasRed ? ["authoritative_goal_package_summary_red"] : [];
}

function storyPackagePolicyBlockers(storyPackage = {}) {
  const blockers = [];
  const verdict = cleanText(storyPackage.verdict || storyPackage.status).toLowerCase();
  const localProofTransition = isLocalProofTransitionPackage(storyPackage);
  const finalRenderMissingOnly = isFinalRenderMissingOnlyPackage(storyPackage);
  if (
    verdict &&
    !(localProofTransition && LOCAL_PROOF_PENDING_VERDICTS.has(verdict)) &&
    !(finalRenderMissingOnly && verdict === "red") &&
    ![
      "green",
      "pass",
      "passed",
  ...
  for (const blocker of asArray(storyPackage.blockers).map(cleanText).filter(Boolean)) {
    if (localProofTransition && LOCAL_PROOF_TRANSITION_BLOCKERS.has(blocker)) continue;
    if (finalRenderMissingOnly && FINAL_RENDER_MISSING_ONLY_BLOCKERS.has(blocker)) continue;
...
async function inspectStoryPackage(storyPackage = {}, options = {}) {
  ...
  const packagePolicyBlockers = storyPackagePolicyBlockers(storyPackage);
  const missingInputs = [];
  ...
  const [
    ...
    readJsonIfPresent(path.join(artifactDir, "source_manifest.json")),
    ...
    readJsonIfPresent(path.join(artifactDir, "narration_manifest.json")),
  ...
  packagePolicyBlockers.push(
    ...authoritativePublishControlBlockers(platformManifest, publishVerdict),
    ...authoritativePackageSummaryBlockers(goalPackageSummary),
  );
  ...
  const hyperframesShellBlockers = hyperframesPremiumShellBlockers({
    storyPackage,
    renderManifest,
    renderStory,
  });
  const finalReady = renderManifest.final_publish_render === true;
  const blockingMissingInputs = finalReady
    ? missingInputs.filter((item) => item !== "publish_verdict.json")
    : missingInputs;
  ...
  const blockers = [
    ...blockingMissingInputs.map((item) => `missing_input:${item}`),
    ...(finalReady ? renderBlockers.filter((item) => item !== "render_not_final_publish_ready") : []),
    ...(finalReady ? hyperframesShellBlockers : []),
  ...
  if (packagePolicyBlockers.length) {
    return {
      ...common,
      status: "blocked",
      blockers: packagePolicyBlockers,
  ...
  if (!finalReady && isLocalProofTransitionPackage(storyPackage)) {
    ...
    return {
      ...
      blockers: unique(["render_not_final_publish_ready", ...blockers]),
  ...
  if (finalReady && !blockingMissingInputs.length) {
  ...
  if (finalReady && !blockingMissingInputs.length && staleRenderBlockers.length && !blockers.length) {
  ...
  if (finalReady && !blockingMissingInputs.length && durationBlockers.length) {
    const renderInputs = renderInputsWithSelectedRenderMotionEvidence(
      finalReadyRenderInputs || await inspectFinalRenderInputs({
        artifactDir,
        audioManifest,
  ...
  if (finalReady && !blockingMissingInputs.length && !blockers.length) {
...
module.exports = {
  _testables: {
    asrWordTimestampCoverage,
    buildSchedulerBridge,
    buildSchedulerBridgeCandidate,
  },
  DEFAULT_THRESHOLDS,
  FINAL_RENDER_INPUTS,
  REQUIRED_INPUTS,
