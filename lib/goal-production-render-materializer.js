...
function preferredMaterialisedClips({ materialisedMotion = {}, footageInventory = {}, rightsLedger = {}, job = {} } = {}) {
  ...
  const materialisedOnlyRealClips = filterLowPayloadSlateClipsWhenSafe(
    rotateFirstClipForRepair(
      prioritiseOpeningMotionClips(materialisedOnlyClips.filter(isRealMaterialisedClip)),
      job,
    ),
  );
  if (manifestReadyStatus(materialisedMotion) && materialisedOnlyRealClips.length >= 3) {
    const hasDirectVideoMotion = materialisedOnlyRealClips.some((clip) => openingMotionPriority(clip) < 2);
    if (hasDirectVideoMotion) return materialisedOnlyRealClips;
  ...
  const realClips = filterLowPayloadSlateClipsWhenSafe(prioritisedRealClips);
...
function hyperframesPremiumShellEvidenceForStory({
  ...
  return {
    ...
    card_clips: passingCards.map((card, index) => ({
      ...
      durationS: MIN_READABLE_HYPERFRAMES_CARD_DURATION_S,
      duration_s: MIN_READABLE_HYPERFRAMES_CARD_DURATION_S,
      duration: MIN_READABLE_HYPERFRAMES_CARD_DURATION_S,
...
async function refreshPostRenderQualityArtifacts({
  ...
  await fs.writeJson(
    path.join(artifactDir, "render_manifest.json"),
    {
      ...
      overlay_card_windows: asArray(renderReport.overlay_card_windows || renderManifest.overlay_card_windows),
    },
    { spaces: 2 },
  );
  return {
...
async function writeFinalRenderManifest({
  ...
  const manifest = {
    ...
    overlay_card_windows: asArray(renderReport.overlay_card_windows),
    ...currentRenderPolicyManifest(),
