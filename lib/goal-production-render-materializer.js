...
function clipBaseSourceKey(clip = {}) {
  ...
  if (
    ...
    /\b(?:official_trailer_segment|official_game_website_media_page|official_social_media_video|licensed_direct_media|steam_movie|hls_manifest|dash_manifest|publisher_trailer)\b/i.test(clipText)
  ) {
...
function preferredMaterialisedClips({ materialisedMotion = {}, footageInventory = {}, rightsLedger = {}, job = {} } = {}) {
  ...
  const preservedEvidenceRealClips = dedupeClipsByBaseSource(filterLowPayloadSlateClipsWhenSafe(
    rotateFirstClipForRepair(
