...
function clipBaseSourceKey(clip = {}) {
  ...
  if (
    ...
    !/generated|owned_generated|pulse-generated|rss_story_v4_clip/i.test(clipText) &&
    /\b(?:official_trailer_segment|official_game_website_media_page|official_social_media_video|licensed_direct_media)\b/i.test(clipText)
  ) {
...
function renderedScenePlanClipObjects({ renderReport = {}, renderManifest = {}, fallbackClips = [] } = {}) {
  ...
  return scenes.map((scene, index) => {
    ...
    const duration = numberOrNull(
      scene.durationS ?? scene.duration_s ?? scene.duration ?? fallback.durationS ?? fallback.duration_s ?? fallback.duration,
    );
    return {
      ...fallback,
      ...scene,
...
const HYPERFRAMES_CARD_SELECTION_PRIORITY = ["source", "quote", "takeaway", "context", "timeline"];
  ...
  return asArray(windows)
    ...
    .filter(
      (window) =>
        ...
        window.duration_s + 0.01 < window.minimum_readable_duration_s,
    );
}
...
function maxReadableHyperframesCardsForMotion(primaryClipCount = 0, shellClipCount = 0) {
  ...
  const byRatio = Math.floor(
    (motionCount * MAX_HYPERFRAMES_CARD_DURATION_RATIO) /
      (1 - MAX_HYPERFRAMES_CARD_DURATION_RATIO),
  );
...
async function buildRendererStoryJson(job = {}, { generatedAt, workspaceRoot = process.cwd() } = {}) {
  const artifactDir = path.resolve(job.artifact_dir || "");
  const [canonical, director, materialisedMotion, footageInventory, rightsLedger, sfxManifest, voiceQualityReport] = await Promise.all([
  ...
  const premiumShellPrimaryClips = preferredClips.filter((clip) => !isApprovedOwnedExplainerClip(clip));
  ...
  const preferredDirectorClips = blendPremiumShellClips(
    primaryClipsForRender.map(directorClipFromProductionClip),
