...
function rightsRecordForSelectedClip(clip = {}) {
  const licenceBasis = cleanText(
    clip.licence_basis || clip.license_basis || clip.rights_basis || inferredStorefrontLicenceBasis(clip),
  );
  if (!licenceBasis) return null;
  const assetId = cleanText(clip.asset_id || clip.id || clip.path || clip.source_url);
  ...
  return normaliseRightsRecord({
    ...
    commercial_use_allowed: clip.commercial_use_allowed ?? true,
...
function augmentRightsLedgerForSelectedClips(rightsLedger = {}, clips = []) {
  ...
  const additions = [];
  for (const clip of asArray(clips)) {
    if (matchingRightsRecord(clip, rightsByKey)) continue;
    const record = rightsRecordForSelectedClip(clip);
    if (!record) continue;
    additions.push(record);
    for (const key of [
      ...keySuffixes(record.path),
      ...keySuffixes(record.source_url),
      cleanText(record.asset_id).toLowerCase(),
      cleanText(record.source_family).toLowerCase(),
    ].filter(Boolean)) rightsByKey.set(key, record);
  }
  if (!additions.length) return rightsLedger;
  if (Array.isArray(rightsLedger)) return [...existing, ...additions];
  return {
    ...(rightsLedger || {}),
...
async function writeFlagshipGenerationEvidence({
  ...
  const captionSourcePath = resolveWorkspacePath(
    job.evidence?.captions_path ||
      job.evidence?.caption_srt_path ||
  ...
  if (!artifactRelativePath(artifactDir, captionSourcePath) || !(await fs.pathExists(captionSourcePath || ""))) {
  ...
  if (!blockers.includes("word_timestamps_missing_or_outside_story_package")) {
    ...
    if (!sourceTimestamps || !asArray(sourceTimestamps.words).length) {
      ...
      const scriptWords = flagshipTranscriptWords(scriptText);
      const timestampWords = asArray(sourceTimestamps.words)
        .flatMap((row) => flagshipTranscriptWords(row?.word ?? row?.text ?? row));
      if (!scriptWords.length || scriptWords.join(" ") !== timestampWords.join(" ")) {
  ...
  const timestampPath = path.join(evidenceDir, "word_timestamps.json");
  const captionPath = path.join(evidenceDir, "captions.srt");
  if (scriptText) await fs.writeFile(scriptPath, `${scriptText}\n`, "utf8");
  if (!blockers.includes("captions_missing_or_outside_story_package")) {
...
function flagshipRightsIdentity(row = {}) {
  return cleanText(row.asset_id || row.id).toLowerCase();
}
...
function flagshipRightsCreator(row = {}) {
  const explicit = cleanText(
    row.creator || row.source_owner || row.rights_holder || row.publisher || row.provider_name,
  );
  if (explicit) return explicit;
  const provider = cleanText(row.provider_id || row.provider);
  if (provider) return provider.replace(/[_-]+/g, " ");
  const sourceUrl = cleanText(row.source_url || row.url).toLowerCase();
  if (sourceUrl.startsWith("elevenlabs://")) return "Pulse Gaming via ElevenLabs";
  return "";
}
...
async function writeFlagshipInventoryEvidence({
  ...
  const usedRows = asArray(rightsLedger?.used_assets).filter((row) => row && typeof row === "object");
  if (!usedRows.length) blockers.push("used_asset_inventory_missing");
  ...
  for (let index = 0; index < usedRows.length; index += 1) {
    ...
    if (!statusValues.some(flagshipRightsStatusIsPositive)) {
    ...
    const merged = Object.assign({}, ...allRows);
    ...
    const evidence = {
      ...
      source_record_sha256: stableObjectSha256(matchedRows),
      generated_at: generatedAt,
    };
