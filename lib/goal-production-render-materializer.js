...
function selectNonReadableOwnedExplainerTopUpForMotionBalance(ownedClips = [], primaryClips = []) {
  ...
  const desiredTotalClipCount = Math.max(9, primary.length);
...
function rightsRecordForSelectedClip(clip = {}) {
  ...
  const licenceBasis = cleanText(
    selectedOwnsGeneratedAsset
      ? selectedRightsBasis
      : clip.licence_basis || clip.license_basis || selectedRightsBasis || inferredStorefrontLicenceBasis(clip),
  );
  ...
  const inferredCommercialBasis = Boolean(inferredStorefrontLicenceBasis(clip));
  ...
  return normaliseRightsRecord({
    ...
    rights_status: explicitlyOwnedGenerated
      ? "approved"
      : rightsStatus || (localProofOnly ? "operator_legal_review_required" : "approved"),
    usage_scope: explicitlyOwnedGenerated
      ? "declared_licensed_scope"
      : usageScope || (localProofOnly ? "local_proof_only" : "declared_licensed_scope"),
    rights_grant: clip.rights_grant === true,
    rights_verdict: explicitlyOwnedGenerated ? "pass" : localProofOnly ? "RED" : "pass",
    sha256: cleanText(clip.sha256 || clip.content_sha256).replace(/^sha256:/i, "").toLowerCase(),
    size_bytes: Number.isFinite(Number(clip.size_bytes)) ? Number(clip.size_bytes) : null,
    ...
    source_info_path: cleanText(clip.source_info_path || clip.info_json_path),
    source_info_sha256: cleanText(clip.source_info_sha256).replace(/^sha256:/i, "").toLowerCase(),
    creator: cleanText(clip.creator || clip.source_channel),
    source_channel: cleanText(clip.source_channel),
    source_channel_url: cleanText(clip.source_channel_url),
    risk_score: explicitlyOwnedGenerated
      ? 0
      : localProofOnly
      ? 1
      : Number.isFinite(Number(clip.risk_score)) ? Number(clip.risk_score) : 0.28,
    evidence_file: cleanText(clip.evidence_file || "final_clip_scene_plan"),
  });
}
...
function exactRightsRecordIndex(records = [], clip = {}) {
  ...
  return asArray(records).findIndex((record) => {
    ...
    if (clipPath && recordPath) return clipPath === recordPath && (!clipId || !recordId || clipId === recordId);
...
function augmentRightsLedgerForSelectedClips(rightsLedger = {}, clips = []) {
  const existing = recordsFromRightsLedger(rightsLedger);
  const completed = existing.map((record) => ({ ...record }));
  for (const clip of asArray(clips)) {
    const record = rightsRecordForSelectedClip(clip);
    if (!record) continue;
    const exactIndex = exactRightsRecordIndex(completed, clip);
    if (exactIndex >= 0) completed[exactIndex] = mergeDefinedRightsRecord(completed[exactIndex], record);
    else completed.push(record);
  }
  if (Array.isArray(rightsLedger)) return completed;

  const selectedRecords = asArray(clips)
    .map((clip) => completed[exactRightsRecordIndex(completed, clip)])
    .filter(Boolean);
  const inheritedBlockers = [
    ...asArray(rightsLedger?.blockers),
  ...
  const blockers = uniqueCleanTexts([
    ...inheritedBlockers,
    ...(selectedRecords.some((record) => !/^[a-f0-9]{64}$/i.test(cleanText(record.sha256)))
  ...
  return {
    ...
    records: completed,
    blockers,
    failures: uniqueCleanTexts(asArray(rightsLedger?.failures)),
    verdict: red ? "RED" : inheritedStatus || "AMBER",
    result: red ? "RED" : inheritedStatus || "AMBER",
    can_auto_publish: red ? false : rightsLedger?.can_auto_publish === true,
  };
}
...
function authoritativePostRenderNarrationQa({
  ...
  return Boolean(
    ...
    hashMatches &&
    (!requireCadence || cleanText(report.cadence?.status).toLowerCase() === "pass")
  );
}
...
function hyperframesPremiumShellEvidenceForStory({
  ...
  const assets = resolveCardAssetsForArtifact({
    artifactDir,
    workspaceRoot,
    storyId,
    channelId,
  ...
  for (const kind of HYPERFRAMES_PREMIUM_SHELL_CARD_KINDS) {
    ...
    const evaluatedResult = evaluateHyperframesPremiumShellEvidence({
...
async function applyFinalDirectMotionQa({
  ...
  const filteredClips = selectedClips.filter((clip) => {
    if (!isRealMaterialisedClip(clip)) return true;
    return acceptedPaths.has(normalisePathKey(clipPathValue(clip)));
  });
  if (filteredClips.filter(isRealMaterialisedClip).length !== accepted.length) {
...
async function buildRendererStoryJson(
  ...
  const rawMaterialisedRealClips = filterOverlappingStrictMotionWindows(dedupeClipsByStrictMotionWindow(
    filterLowPayloadSlateClipsWhenSafe(
      prioritiseOpeningMotionClips(
  ...
  const finalDirectMotionQa = await applyFinalDirectMotionQa({
    clips: coverageReadyPrimaryClips,
    artifactDir,
    directMotionFilter,
  });
...
async function refreshPostRenderQualityArtifacts({
  ...
  const finalRightsLedger = augmentRightsLedgerForSelectedClips(normalisedRightsLedger, clips);
...
async function writeFlagshipGenerationEvidence({
  ...
  if (!blockers.includes("word_timestamps_missing_or_outside_story_package")) {
    ...
    if (!sourceTimestamps || !asArray(sourceTimestamps.words).length) {
      ...
      const alignedTimestampWords = timestampWords
        .flatMap((row) => flagshipTranscriptWords(row?.word ?? row?.text ?? row));
      if (!scriptWords.length || scriptWords.join(" ") !== alignedTimestampWords.join(" ")) {
  ...
  if (authoritativeCaption && !blockers.includes("captions_missing_or_outside_story_package")) {
...
async function refreshFlagshipInventoryEvidence({
  ...
  const renderManifest = await readJsonIfPresent(
    path.join(resolvedArtifactDir, "render_manifest.json"),
    {},
  );
