...
function hyperframesPremiumShellEvidenceForStory({
  ...
  const selectedCardBlockers =
    selectedCount >= MIN_PREMIUM_HYPERFRAMES_CARDS
      ? []
      : [`selected_hyperframes_card_count_below_required:${selectedCount}/${MIN_PREMIUM_HYPERFRAMES_CARDS}`];
  const allBlockers = uniqueCleanTexts([...blockers, ...selectedCardBlockers]);
  const gateVerdict =
