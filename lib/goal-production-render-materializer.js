...
const MIN_SOURCE_LOCK_CARD_DURATION_S = 1.2;
const MIN_OVERLAY_PROOF_CARD_DURATION_S = 4.2;
  ...
  return {
    ...
    card_clips: selectedCards.map((card, index) => {
      ...
      return {
        ...
        max_readable_card_duration_s: maxReadable || durationS,
        maximum_visible_duration_s: maxReadable || durationS,
        durationS,
...
async function buildRendererStoryJson(job = {}, { generatedAt, workspaceRoot = process.cwd() } = {}) {
  ...
  const audioDurationS = numberOrNull(
    voiceQualityReport.cadence?.duration_seconds ??
      voiceQualityReport.cadence?.duration_s ??
      voiceQualityReport.duration_seconds ??
      voiceQualityReport.duration_s,
  ...
  const primaryClipsForRender = asArray(premiumShell?.card_clips).length
    ...
    : preferredClips;
  const shellClipsForRender = stretchReadableShellClipsForAudioCoverage({
  ...
  const premiumShellGate = premiumShell
    ...
    : null;
  const coverageReadyPrimaryClips = filterSteamAppOutlierClips(
