...
function governedReadableShellCardPaths({ materialisedMotion = {}, job = {} } = {}) {
  if (!manifestReadyStatus(materialisedMotion)) return [];
  const selectedIds = new Set(
    asArray(job.evidence?.selected_materialised_motion_clip_ids)
  ...
  return uniqueCleanTexts(
    materialisedMotionClips(materialisedMotion)
      .filter((clip) => {
        ...
        return !selectedIds.size || (id && selectedIds.has(id));
      })
      .filter((clip) => {
        const evidence = [
          clip.source_url,
...
function currentHashBoundReconciledRightsRecord(record = {}, clip = {}, {
  ...
  return Boolean(
    (legacyValidatedOfficialRecord || sameRunExactCurrentRecord) &&
...
function augmentRightsLedgerForSelectedClips(rightsLedger = {}, clips = [], options = {}) {
  ...
  return {
    ...
    records: completed,
    blockers,
    failures: uniqueCleanTexts(asArray(normalisedRightsLedger?.failures)),
...
async function applyFinalDirectMotionQa({
  ...
  const visuallyAcceptedClips = directClips.filter((clip) =>
  ...
  const filteredClips = selectedClips.filter((clip) => {
    ...
    return professionallySelectedPaths.has(normalisePathKey(clipPathValue(clip)));
  });
  if (
...
async function hydrateCanonicalEpidemicSfxRuntime({
  ...
  const runtimeManifestPath = resolveWorkspacePath(
...
async function buildRendererStoryJson(
  ...
  const governedSelectedCardPaths = governedReadableShellCardPaths({
    materialisedMotion,
    job,
  });
...
function bindAppliedPostRenderRightsLedger(report = {}, ledgerState = {}, {
  ...
  if (ledgerState.green) return bound;
  return {
    ...bound,
    verdict: "FAIL",
