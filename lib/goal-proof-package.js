...
function platformAttentionDescription({ canonical = {}, subject = "", angle = "", sourceLine = "", title = "", firstLine = "" } = {}) {
  ...
  const evidenceText = cleanText([description, angle, title, firstLine, narrationEvidence].join(" "));
  ...
  const body = !forceAttentionBody && usefulPublicAngle(description) && !sourceSafeSocialBodyTooPlain(description) && platformAudiencePullLanguage(description)
    ...
    : attentionBodyFromEvidence({
        ...
        angle,
        title,
        description: cleanText([description, narrationEvidence].join(" ")),
        firstLine,
      });
...
function buildAcceptanceEntry({
  ...
  if (!materialisedEvidenceClearsStaleFootageBlocker(footageInventory, materialisedMotionEvidence)) {
    const footageBlockers = footageInventoryBlockers(footageInventory);
    blockers.push(...(footageBlockers.length
      ? footageBlockers.map((blocker) => `footage:${blocker}`)
      : [`footage:${footageInventory.readiness?.status || "unknown"}`]));
  }
