...
function platformConsequenceLanguage(value = "") {
  return /\b(?:problem|risk|catch|warning|changed|broke|broken|revealed|confirmed|finally|pushback|why|before|after|ceiling|impact|cost|deal|launch|lands?|hits?|date|proof|payoff|trust|pressure|fight|split|splits|splitting|bloat|dangerous|delete|wins?|timing|trial|argument|balance|balanced|polish|half[- ]finished|more than|handmade|leaked?|tactics?|xcom|exposes?|spike|peak|demand|bet|custom seas|private seas|private sessions?|private mode|rule controls?|ea\s+play|ps5|jungle|combat|discovery|readability|readable|lab[- ]worthy|roster|meta|matchups?|team building|screen control|map|choices?|empathy|atmosphere|horror|jump[- ]?scares?)\b/i.test(
    cleanText(value),
  );
}
...
function platformAudiencePullLanguage(value = "") {
  ...
  const hasSpecificity =
    /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|one|only|first|last|free|paid|delay(?:ed)?|release calendar|release window|leak(?:ed)?|launch|ending|before|after|price|trial|demo|early[- ]access|steam peak|steam spike|steam demand|custom seas|private seas|private sessions?|private mode|rule controls?|set (?:their own )?rules|ea\s+play|ps5|jungle|combat|discovery|readability|readable|lab[- ]worthy|balanced|polish|roster|meta|matchups?|team building|screen control|map|choices?|empathy|atmosphere|horror|jump[- ]?scares?|roguelite|podracing|repeat[- ]run|wipeout|handling)\b/i.test(text);
  const hasCuriosity =
    /\b(?:why|how|what|secret|hidden|real|catch|risk|problem|fight|trial|argument|pressure|warning|trust|ceiling|broke|turns?|changed|into|test|timing|balance|polish|more than|handmade|xcom|tactics?|exposes?|spike|peak|demand|bet)\b/i.test(text);
...
function attentionPlatformTitle({ subject = "", title = "", angle = "", description = "", firstLine = "" } = {}) {
  ...
  if (
    /\bhas\s+(?:a|an)\s+(?:stage\s+clarity|studio\s+risk|closure\s+risk|showcase\s+watchlist|extraction\s+map|ghosting|kenshiro\s+roster)\b/i.test(title) ||
    /\b(?:screen\s+rumou?r\s+has\s+a\s+ghosting\s+test|kenshiro\s+roster\s+(?:test|fight))\b/i.test(title)
  ) {
  ...
  if (/\bassassin'?s\s+creed\s+black\s+flag\s+resynced\b/i.test(subjectEvidenceText) && /\bps5\s+pro\b/i.test(evidenceText)) {
    return "Black Flag Resynced Has A PS5 Pro Test";
  }
  if (/\bmonopoly\b/i.test(subjectEvidenceText) && /\bstar\s+wars\b/i.test(evidenceText) && /\b(?:ability|abilities|heroes|villains)\b/i.test(evidenceText)) {
    return "Monopoly Star Wars Has A Character Ability Fight";
  }
...
function weakCoverHeadline(value = "", subject = "") {
  ...
  return tokens.length <= 3 && !/\b(?:risk|test|fight|trust|bloat|danger|pressure|problem|wins?|demo|spike|demand|momentum|ceiling|ps5|ea\s+play|jungle|combat|discovery|readability|map)\b/i.test(text);
}
...
function buildPlatformNativePublishPacks({
  ...
  const outputs = {
    ...
    facebook_reels: mergePlatformOutput(platformOutputs.facebook_reels, {
      ...
      explanatory_framing: `${subject} matters because ${reasonClause}.`,
...
function materialisedEvidenceClearsStaleFootageBlocker(footageInventory = {}, evidence = {}) {
  ...
  if (status === "v4_motion_ready") return true;
  if (status !== "v4_motion_blocked") return false;
  const blockers = asArray(readiness.blockers).map(cleanText).filter(Boolean);
  if (blockers.some((blocker) => blocker !== "distinct_motion_source_assets_minimum_not_met")) {
    return false;
  }
...
function buildGoalProofPackage({
  ...
  const finalPublishBlockers = finalPublishEvidenceBlockers({ story, audioManifest });
  ...
  const storyRenderManifest = objectHasKeys(story.render_manifest) ? story.render_manifest : {};
...
async function writeGoalProofPackageArtifacts(pack = {}, { outputDir } = {}) {
  ...
  written.materialised_motion_clips = await writeJson(
    ...
    materialisedMotionEvidenceForPack(pack),
  );
  ...
  if (pack.render_manifest?.final_publish_render === true && await fs.pathExists(finalRenderPath)) {
    written.visual_v4_render = finalRenderPath;
  } else {
    written.visual_v4_render = await writeLocalProofMp4(finalRenderPath, pack);
  }
  return written;
}
