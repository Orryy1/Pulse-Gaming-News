...
function platformConsequenceLanguage(value = "") {
  return /\b(?:problem|risk|catch|warning|changed|turns?|makes?|threat|ends?|ended|best|worst|worse|permanent(?:ly)?|broke|broken|revealed|confirmed|finally|pushback|why|before|after|ceiling|impact|cost|deal|launch|lands?|hits?|date|proof|payoff|trust|pressure|fight|split|splits|splitting|bloat|dangerous|delete|wins?|timing|trial|argument|balance|balanced|polish|half[- ]finished|more than|handmade|leaked?|tactics?|xcom|exposes?|spike|peak|demand|bet|comeback|second wave|custom seas|private seas|private sessions?|private mode|rule controls?|ea\s+play|game pass|ps5|jungle|combat|discovery|readability|readable|lab[- ]worthy|roster|meta|matchups?|team building|screen control|map|choices?|empathy|atmosphere|horror|jump[- ]?scares?|ruins?|replayable|chaos|game night|family night|force powers?)\b/i.test(
    cleanText(value),
  );
}
...
function buildPlatformNativePublishPacks({
  ...
  const explanatoryFraming = /\bdenshattack\b/i.test(`${subject} ${title} ${shortAngle}`)
    ? `${socialClaim}.`
