...
function attentionBodyFromEvidence({ subject = "", angle = "", title = "", description = "", firstLine = "" } = {}) {
  ...
  if (/\b(?:diddy\s+kong\s+racing|yooka[-\s]?laylee\s+kart|mascot\s+kart|handling|nostalgia)\b/i.test(evidenceText)) {
...
function attentionCoverHeadline({ canonical = {}, story = {}, subject = "", title = "", angle = "", firstLine = "" } = {}) {
  ...
  const repairedHeadline = [canonical.thumbnail_headline, story.suggested_thumbnail_text, canonical.suggested_thumbnail_text]
    ...
    .find((candidate) =>
      candidate &&
      coverHeadlineMatchesSubject(candidate, subject) &&
