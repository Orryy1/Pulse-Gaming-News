...
function platformConsequenceLanguage(value = "") {
  return /\b(?:problem|risk|catch|warning|changed|permanent(?:ly)?|broke|broken|revealed|confirmed|finally|pushback|why|before|after|ceiling|impact|cost|deal|launch|lands?|hits?|date|proof|payoff|trust|pressure|fight|split|splits|splitting|bloat|dangerous|delete|wins?|timing|trial|argument|balance|balanced|polish|half[- ]finished|more than|handmade|leaked?|tactics?|xcom|exposes?|spike|peak|demand|bet|comeback|second wave|custom seas|private seas|private sessions?|private mode|rule controls?|ea\s+play|game pass|ps5|jungle|combat|discovery|readability|readable|lab[- ]worthy|roster|meta|matchups?|team building|screen control|map|choices?|empathy|atmosphere|horror|jump[- ]?scares?|ruins?|replayable|chaos|game night|family night|force powers?)\b/i.test(
    cleanText(value),
  );
}
...
function platformAudiencePullLanguage(value = "") {
  ...
  const hasSpecificity =
    /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|one|only|first|last|free|paid|delay(?:ed)?|release calendar|release window|leak(?:ed)?|launch|ending|before|after|price|trial|demo|early[- ]access|steam peak|steam spike|steam demand|comeback|game pass|subscription|custom seas|private seas|private sessions?|private mode|rule controls?|set (?:their own )?rules|ea\s+play|ps5|jungle|combat|discovery|readability|readable|lab[- ]worthy|balanced|polish|roster|meta|matchups?|team building|screen control|map|choices?|empathy|atmosphere|horror|jump[- ]?scares?|roguelite|podracing|repeat[- ]run|wipeout|handling|force powers?|heroes?|villains?|board game|family night|game night|replayable|licensed board)\b/i.test(text);
  const hasCuriosity =
    /\b(?:why|how|what|secret|hidden|hides?|real|catch|risk|problem|fight|trial|argument|pressure|warning|trust|ceiling|broke|turns?|changed|into|test|timing|balance|polish|more than|handmade|xcom|tactics?|exposes?|spike|peak|demand|bet|comeback|second wave)\b/i.test(text);
...
function attentionPlatformTitle({ subject = "", title = "", angle = "", description = "", firstLine = "" } = {}) {
  ...
  if (
    /\bassassin'?s\s+creed\s+black\s+flag\s+resynced\b/i.test(subjectEvidenceText) &&
