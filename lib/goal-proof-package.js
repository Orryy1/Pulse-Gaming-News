...
const INTERNAL_REVIEW_PUBLIC_COPY_RE =
  /\breal source detail,\s*but not enough practical consequence for a strong pulse short yet\b/i;
...
function weakCoverHeadline(value = "", subject = "") {
  ...
  return headlineTokens(text).length <= 3 && !/\b(?:risk|test|fight|trust|bloat|danger|pressure|problem|wins?|demo|spike|demand|momentum|ceiling)\b/i.test(text);
}
