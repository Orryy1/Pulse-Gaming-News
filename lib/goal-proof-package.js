...
function attentionBodyFromEvidence({ subject = "", angle = "", title = "", description = "", firstLine = "" } = {}) {
  ...
  if (/\b(?:demo|demos|next fest|playable)\b/i.test(evidenceText)) {
    return `${subject} is getting a demo trust test before launch. The playable slice can win wishlists fast or expose the problem while players are still deciding.`;
  }
  if (/\b(?:retro|throwback|old-school|exploration|combat|discovery|square enix)\b/i.test(evidenceText)) {
    return `${subject} has a retro trust problem to solve. The style grabs attention, but the real test is whether combat, exploration and discovery still feel modern.`;
  }
...
function platformConsequenceLanguage(value = "") {
  return /\b(?:problem|risk|catch|warning|changed|broke|broken|revealed|confirmed|finally|pushback|why|before|after|ceiling|impact|cost|deal|launch|date|proof|payoff|trust|pressure|fight|bloat|dangerous|delete|wins?|timing|more than|handmade|leaked?|tactics?|xcom)\b/i.test(
    cleanText(value),
  );
}
...
function attentionPlatformTitle({ subject = "", title = "", angle = "", description = "", firstLine = "" } = {}) {
  ...
  if (/\b(?:demo|demos|next fest|playable)\b/i.test(evidenceText)) {
    return `${subject} Has A Demo Trust Test`;
  }
  if (/\b(?:retro|throwback|old-school|exploration|combat|discovery|square enix)\b/i.test(evidenceText)) {
  ...
  return `${subject} Has A Player Trust Test`;
}
