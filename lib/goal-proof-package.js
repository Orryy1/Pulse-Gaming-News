...
function normaliseDirectMediaCandidate(candidate = {}, index = 0) {
  ...
  if (!/\.(?:mp4|mov|m4v|webm|m3u8|mpd)(?:[?#]|$)/i.test(directMediaUrl)) return null;
  return {
    direct_media_url: directMediaUrl,
    direct_media_url_if_available: directMediaUrl,
...
function sourceDirectMediaFromStory(story = {}) {
  return sourceDirectMediaCandidatesFromStory(story)[0]?.direct_media_url || "";
}
