...
function usefulPublicAngle(value = "") {
  ...
  if (!text || INTERNAL_ANGLE_RE.test(text) || LABEL_ANGLE_RE.test(text)) return "";
  return text;
}
...
function platformAttentionDescription({ canonical = {}, subject = "", angle = "", sourceLine = "" } = {}) {
  const description = stripSourceAdmin(canonical.description || "");
  const body = usefulPublicAngle(description) && wordCount(description) >= 8
    ? description
    : cleanText(`${subject}: ${angle}.`);
