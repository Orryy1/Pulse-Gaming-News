...
function decodedRgbFrame(filePath, timeSeconds = null) {
  const args = ["-hide_banner", "-loglevel", "error", "-i", filePath];
  if (Number.isFinite(timeSeconds)) args.push("-ss", Number(timeSeconds).toFixed(6));
...
function probeVisualAsset(filePath) {
  ...
  const stream = asArray(parsed.streams)[0] || {};
  return {
    codec_name: cleanText(stream.codec_name).toLowerCase(),
...
function lineageConsistencyBlockers({
  ...
  const captionAlignmentAudioHashes = new Set();
  const programmeAudioHashes = new Set();
  const timestampHashes = new Set();
  const programmeAudioContractPresent = Boolean(
    cleanText(audioManifest.programme_audio_sha256) ||
    cleanText(renderManifest.input_fingerprint?.programme_audio_sha256),
  );
  for (const value of [
    audioManifest.caption_alignment_audio_sha256,
    audioManifest.narration_audio_sha256,
    !programmeAudioContractPresent ? audioManifest.audio_sha256 : null,
    narrationManifest.caption_alignment_audio_sha256,
    narrationManifest.audio_sha256,
    narrationManifest.narration_audio_sha256,
    narrationManifest.lineage?.final_audio_sha256,
    captionManifest.alignment_audio_sha256,
    captionManifest.audio_sha256,
    captionManifest.narration_audio_sha256,
    captionManifest.lineage?.final_audio_sha256,
    renderManifest.input_fingerprint?.caption_alignment_audio_sha256,
    !programmeAudioContractPresent
      ? renderManifest.input_fingerprint?.audio_sha256
      : null,
  ]) declaredSha256(value, "audio", blockers, captionAlignmentAudioHashes);
  for (const value of [
    audioManifest.programme_audio_sha256,
    programmeAudioContractPresent ? audioManifest.audio_sha256 : null,
    renderManifest.input_fingerprint?.programme_audio_sha256,
    programmeAudioContractPresent
      ? renderManifest.input_fingerprint?.audio_sha256
      : null,
  ]) declaredSha256(value, "programme_audio", blockers, programmeAudioHashes);
  for (const value of [
  ...
  if (captionAlignmentAudioHashes.size > 1) blockers.push("lineage:audio_sha256_conflict");
  if (programmeAudioHashes.size > 1) blockers.push("lineage:programme_audio_sha256_conflict");
  ...
  if (finalGreenNarration) {
    const captionAlignmentAudio =
      captionManifest.alignment_audio_sha256 ||
      captionManifest.audio_sha256 ||
      captionManifest.narration_audio_sha256 ||
      captionManifest.lineage?.final_audio_sha256;
    const renderProgrammeAudio = programmeAudioContractPresent
      ? renderManifest.input_fingerprint?.programme_audio_sha256
      : renderManifest.input_fingerprint?.audio_sha256;
    const requiredClaims = [
      ...
      ["caption_manifest_audio", captionAlignmentAudio],
      [
        programmeAudioContractPresent
          ? "render_manifest_programme_audio"
          : "render_manifest_audio",
        renderProgrammeAudio,
      ],
      ...(programmeAudioContractPresent
        ? [
            ["audio_manifest_programme_audio", audioManifest.programme_audio_sha256],
            [
              "render_manifest_caption_alignment_audio",
              renderManifest.input_fingerprint?.caption_alignment_audio_sha256,
            ],
          ]
        : []),
...
function declaredVisualCtaRenderIdentity(story = {}) {
  const renderManifest = story.render_manifest || story.renderManifest || {};
  const values = [
    renderManifest.reviewed_sha256,
    renderManifest.final_media?.sha256,
    renderManifest.final_media_sha256,
    renderManifest.output_sha256,
    renderManifest.render_sha256,
    renderManifest.video_sha256,
    renderManifest.final_render_sha256,
    renderManifest.final_mp4_sha256,
    renderManifest.media_sha256,
  ]
    .map((value) => cleanText(value).toLowerCase())
    .filter(Boolean);
...
function visualCtaManifestContract(story = {}) {
  const manifest = story.visual_cta_manifest || story.visualCtaManifest;
  if (!objectHasKeys(manifest)) {
    return {
      claimed: false,
      valid: false,
      blockers: [],
  ...
  const exactText = String(manifest.exact_text || "").trim();
  const deliveryMode = cleanText(manifest.delivery_mode);
  const startSeconds = Number(manifest.start_seconds);
  const endSeconds = Number(manifest.end_seconds);
  const renderManifest = story.render_manifest || story.renderManifest || {};
  const renderedDurationSeconds = Number(
    renderManifest.rendered_duration_s ||
      renderManifest.duration_seconds ||
      renderManifest.duration_s ||
      renderManifest.duration,
  );
  const renderIdentities = declaredVisualCtaRenderIdentity(story);
  const renderSha256 = cleanText(manifest.render_sha256).toLowerCase();
  const reviewedRenderSha256 = cleanText(renderManifest.reviewed_sha256).toLowerCase();
  const renderPath = cleanText(
  ...
  const evidencePath = cleanText(manifest.evidence_path);
  const evidenceSha256 = cleanText(manifest.evidence_sha256).toLowerCase();

  if (Number(manifest.schema_version) !== 1) blockers.push("schema_version_mismatch");
  if (!storyId || manifestStoryId !== storyId) blockers.push("story_id_mismatch");
  if (cleanText(manifest.verdict).toUpperCase() !== "GREEN") blockers.push("verdict_not_green");
  if (exactText !== PRIMARY_PULSE_CTA) blockers.push("exact_text_mismatch");
  if (deliveryMode !== "visual_end_card") blockers.push("delivery_mode_mismatch");
  if (!Number.isFinite(startSeconds) || !Number.isFinite(endSeconds) || endSeconds <= startSeconds) {
    blockers.push("timing_invalid");
  } else if (endSeconds - startSeconds < 0.75 || endSeconds - startSeconds > 3) {
    blockers.push("timing_duration_out_of_range");
  }
  if (!Number.isFinite(renderedDurationSeconds) || renderedDurationSeconds <= 0) {
    blockers.push("render_duration_missing");
  } else if (
    Number.isFinite(startSeconds) &&
    Number.isFinite(endSeconds) &&
    (startSeconds < Math.max(0, renderedDurationSeconds - 3) ||
      endSeconds > renderedDurationSeconds + 1 / 30 ||
      endSeconds < renderedDurationSeconds - 0.25)
  ) {
    blockers.push("timing_not_bound_to_end_card");
  }
  if (!/^[a-f0-9]{64}$/.test(renderSha256)) blockers.push("render_sha256_invalid");
  if (!/^[a-f0-9]{64}$/.test(reviewedRenderSha256)) {
    blockers.push("reviewed_render_sha256_missing_or_invalid");
  } else if (reviewedRenderSha256 !== renderSha256) {
    blockers.push("reviewed_render_sha256_mismatch");
  }
  if (!renderIdentities.length) blockers.push("reviewed_render_sha256_missing");
  if (renderIdentities.some((value) => !/^[a-f0-9]{64}$/.test(value))) {
    blockers.push("reviewed_render_sha256_invalid");
  }
  if (renderIdentities.length > 1) blockers.push("reviewed_render_sha256_conflict");
  if (renderIdentities.length === 1 && renderIdentities[0] !== renderSha256) {
    blockers.push("render_sha256_mismatch");
  }
  if (!path.isAbsolute(renderPath)) {
    blockers.push("render_path_not_absolute");
  } else if (/^[a-f0-9]{64}$/.test(renderSha256)) {
    try {
      const renderStat = fs.lstatSync(renderPath);
      if (!renderStat.isFile() || renderStat.isSymbolicLink() || renderStat.size <= 0) {
        blockers.push("render_file_invalid");
      } else {
        const actualRenderSha256 = sha256FileSync(renderPath);
        if (actualRenderSha256 !== renderSha256) blockers.push("render_file_sha256_mismatch");
      }
    } catch (_error) {
      blockers.push("render_file_unreadable");
    }
  }
  if (manifest.dom_verified !== true) blockers.push("dom_not_verified");
  if (manifest.sampled_visual_verified !== true) blockers.push("sampled_visual_not_verified");
  if (!path.isAbsolute(evidencePath)) blockers.push("evidence_path_not_absolute");
  if (!/^[a-f0-9]{64}$/.test(evidenceSha256)) blockers.push("evidence_sha256_invalid");

  let evidence = null;
  if (path.isAbsolute(evidencePath) && /^[a-f0-9]{64}$/.test(evidenceSha256)) {
    try {
      const stat = fs.lstatSync(evidencePath);
      if (!stat.isFile() || stat.isSymbolicLink() || stat.size <= 0 || stat.size > 1024 * 1024) {
        blockers.push("evidence_file_invalid");
      } else {
        const evidenceBytes = fs.readFileSync(evidencePath);
        const actualEvidenceSha256 = crypto.createHash("sha256").update(evidenceBytes).digest("hex");
        if (actualEvidenceSha256 !== evidenceSha256) {
          blockers.push("evidence_sha256_mismatch");
        } else {
          evidence = JSON.parse(evidenceBytes.toString("utf8"));
          const evidenceFields = [
            ["schema_version", Number(evidence.schema_version), Number(manifest.schema_version)],
            ["story_id", cleanText(evidence.story_id), manifestStoryId],
            ["verdict", cleanText(evidence.verdict).toUpperCase(), cleanText(manifest.verdict).toUpperCase()],
            ["exact_text", String(evidence.exact_text || "").trim(), exactText],
            ["delivery_mode", cleanText(evidence.delivery_mode), deliveryMode],
            ["start_seconds", Number(evidence.start_seconds), startSeconds],
            ["end_seconds", Number(evidence.end_seconds), endSeconds],
            ["render_sha256", cleanText(evidence.render_sha256).toLowerCase(), renderSha256],
            ["dom_verified", evidence.dom_verified, manifest.dom_verified],
            ["sampled_visual_verified", evidence.sampled_visual_verified, manifest.sampled_visual_verified],
          ];
          for (const [field, actual, expected] of evidenceFields) {
            if (actual !== expected) blockers.push(`evidence_${field}_mismatch`);
          }
          if (!cleanText(evidence.reviewer?.id) || evidence.reviewer?.independent !== true) {
            blockers.push("independent_visual_review_missing");
          }
          if (evidence.full_visual_review !== true) blockers.push("full_visual_review_missing");
          const sampledFrame = evidence.sampled_frame || {};
          const sampledFrameTime = Number(sampledFrame.time_seconds);
          const sampledFramePath = cleanText(sampledFrame.path);
          const sampledFrameSha256 = cleanText(sampledFrame.sha256).toLowerCase();
          if (
            !Number.isFinite(sampledFrameTime) ||
            sampledFrameTime < startSeconds ||
            sampledFrameTime > endSeconds
          ) {
            blockers.push("sampled_frame_time_outside_cta");
          }
          if (!path.isAbsolute(sampledFramePath)) blockers.push("sampled_frame_path_not_absolute");
          if (!/^[a-f0-9]{64}$/.test(sampledFrameSha256)) {
            blockers.push("sampled_frame_sha256_invalid");
          }
          if (path.isAbsolute(sampledFramePath) && /^[a-f0-9]{64}$/.test(sampledFrameSha256)) {
            try {
              const sampleStat = fs.lstatSync(sampledFramePath);
              if (
                !sampleStat.isFile() ||
                sampleStat.isSymbolicLink() ||
                sampleStat.size <= 0 ||
                sampleStat.size > 50 * 1024 * 1024
              ) {
                blockers.push("sampled_frame_file_invalid");
              } else {
                const actualSampleSha256 = sha256FileSync(sampledFramePath);
                if (actualSampleSha256 !== sampledFrameSha256) {
                  blockers.push("sampled_frame_sha256_mismatch");
                } else {
                  try {
                    const renderFrame = decodedRgbFrame(renderPath, sampledFrameTime);
                    const suppliedFrame = decodedRgbFrame(sampledFramePath);
                    if (
                      !renderFrame.length ||
                      renderFrame.length !== suppliedFrame.length ||
                      !crypto.timingSafeEqual(
                        crypto.createHash("sha256").update(renderFrame).digest(),
                        crypto.createHash("sha256").update(suppliedFrame).digest(),
                      )
                    ) {
                      blockers.push("sampled_frame_not_from_render");
                    }
                  } catch (_error) {
                    blockers.push("sampled_frame_not_from_render");
                  }
              ...
              blockers.push("sampled_frame_file_unreadable");
            }
          }
        }
      }
    } catch (_error) {
      blockers.push("evidence_file_unreadable");
    }
  }
  ...
  return {
    ...
    render_sha256: renderSha256 || null,
...
function captionParityText(value) {
  return cleanText(captionText(value));
}

function captionTimelineError(code) {
  const error = new Error(code);
  error.code = code;
  return error;
}

function authoritativeCaptionLineageSha256(values = [], kind = "lineage") {
  const declared = [...new Set(values
    .map((value) => cleanText(value).toLowerCase())
    .filter(Boolean))];
  if (!declared.length || declared.some((value) => !/^[a-f0-9]{64}$/.test(value))) {
    throw captionTimelineError(`authoritative_caption_${kind}_sha256_missing`);
  }
  if (declared.length !== 1) {
    throw captionTimelineError(`authoritative_caption_${kind}_sha256_conflict`);
  }
  return declared[0];
}

async function buildAuthoritativeCaptionSrt(pack = {}) {
  const manifest = pack.caption_manifest || {};
  const finalPublishRender = pack.render_manifest?.final_publish_render === true;
  if (manifest.caption_timeline_authoritative !== true) {
    if (finalPublishRender) {
      throw captionTimelineError("final_publish_caption_timeline_authority_required");
    }
    return null;
  }

  const mappingPath = cleanText(manifest.karaoke_mapping_path);
  const expectedSha256 = cleanText(manifest.karaoke_mapping_sha256).toLowerCase();
  if (!mappingPath) throw captionTimelineError("authoritative_caption_mapping_path_missing");
  if (!/^[a-f0-9]{64}$/.test(expectedSha256)) {
    throw captionTimelineError("authoritative_caption_mapping_sha256_missing");
  }

  const resolvedMappingPath = path.resolve(mappingPath);
  let mappingBytes;
  try {
    const stat = await fs.stat(resolvedMappingPath);
    if (!stat.isFile() || stat.size <= 0 || stat.size > 10 * 1024 * 1024) {
      throw captionTimelineError("authoritative_caption_mapping_file_invalid");
    }
    mappingBytes = await fs.readFile(resolvedMappingPath);
  } catch (error) {
    if (error?.code?.startsWith?.("authoritative_caption_")) throw error;
    throw captionTimelineError("authoritative_caption_mapping_file_unreadable");
  }
  const actualSha256 = crypto.createHash("sha256").update(mappingBytes).digest("hex");
  if (actualSha256 !== expectedSha256) {
    throw captionTimelineError("authoritative_caption_mapping_sha256_mismatch");
  }

  let mapping;
  try {
    mapping = JSON.parse(mappingBytes.toString("utf8"));
  } catch {
    throw captionTimelineError("authoritative_caption_mapping_json_invalid");
  }
  const groups = asArray(mapping?.groups);
  if (!groups.length || groups.length > 1000) {
    throw captionTimelineError("authoritative_caption_mapping_groups_invalid");
  }

  const expectedDisplayText = captionParityText(manifest.display_text);
  const mappingDisplayText = captionParityText(mapping.display_text);
  if (!expectedDisplayText || mappingDisplayText !== expectedDisplayText) {
    throw captionTimelineError("authoritative_caption_display_text_mismatch");
  }
  const canonicalDisplayText = captionParityText(
    pack.canonical_story_manifest?.caption_display_text ||
      pack.canonical_story_manifest?.display_script ||
      pack.canonical_story_manifest?.narration_script ||
      pack.canonical_story_manifest?.spoken_narration_script ||
      pack.canonical_story_manifest?.tts_script,
  );
  if (!canonicalDisplayText || canonicalDisplayText !== expectedDisplayText) {
    throw captionTimelineError("authoritative_caption_canonical_display_text_mismatch");
  }

  const renderInputFingerprint = pack.render_manifest?.input_fingerprint || {};
  const renderedProgrammeAudioSha256 = cleanText(
    renderInputFingerprint.programme_audio_sha256,
  ).toLowerCase();
  const renderedAudioSha256 = cleanText(
    renderInputFingerprint.audio_sha256,
  ).toLowerCase();
  const renderedCaptionAlignmentAudioSha256 = cleanText(
    renderInputFingerprint.caption_alignment_audio_sha256,
  ).toLowerCase();
  const manifestProgrammeAudioSha256 = cleanText(
    pack.audio_manifest?.programme_audio_sha256,
  ).toLowerCase();
  const programmeAudioContractPresent = Boolean(
    renderedProgrammeAudioSha256 || manifestProgrammeAudioSha256,
  );
  if (
    renderedProgrammeAudioSha256 &&
    renderedAudioSha256 &&
    renderedProgrammeAudioSha256 !== renderedAudioSha256
  ) {
    throw captionTimelineError("render_programme_audio_sha256_conflict");
  }
  if (
    renderedProgrammeAudioSha256 &&
    manifestProgrammeAudioSha256 &&
    renderedProgrammeAudioSha256 !== manifestProgrammeAudioSha256
  ) {
    throw captionTimelineError("audio_manifest_programme_audio_sha256_mismatch");
  }
  const expectedAudioSha256 = authoritativeCaptionLineageSha256([
    manifest.alignment_audio_sha256,
    manifest.audio_sha256,
    manifest.narration_audio_sha256,
    manifest.lineage?.final_audio_sha256,
    pack.audio_manifest?.caption_alignment_audio_sha256,
    !programmeAudioContractPresent ? pack.audio_manifest?.audio_sha256 : null,
    pack.audio_manifest?.narration_audio_sha256,
    pack.narration_manifest?.caption_alignment_audio_sha256,
    pack.narration_manifest?.audio_sha256,
    pack.narration_manifest?.narration_audio_sha256,
    pack.narration_manifest?.lineage?.final_audio_sha256,
    renderedCaptionAlignmentAudioSha256 ||
      (!renderedProgrammeAudioSha256 ? renderedAudioSha256 : null),
  ], "audio");
  if (
    renderedProgrammeAudioSha256 &&
    renderedProgrammeAudioSha256 !== expectedAudioSha256 &&
    renderedCaptionAlignmentAudioSha256 !== expectedAudioSha256
  ) {
    throw captionTimelineError(
      "render_caption_alignment_audio_sha256_missing_or_mismatch",
    );
  }
  const expectedTimestampsSha256 = authoritativeCaptionLineageSha256([
    manifest.word_timestamps_sha256,
    manifest.lineage?.frozen_word_timestamps_sha256,
    pack.audio_manifest?.word_timestamps_sha256,
    pack.narration_manifest?.word_timestamps_sha256,
    pack.narration_manifest?.lineage?.frozen_word_timestamps_sha256,
    pack.render_manifest?.input_fingerprint?.word_timestamps_sha256,
  ], "alignment");
  const mappingAudioSha256 = cleanText(mapping?.alignment_provenance?.audio_sha256).toLowerCase();
  const mappingTimestampsSha256 = cleanText(mapping?.sources?.alignment?.sha256).toLowerCase();
  if (mappingAudioSha256 !== expectedAudioSha256) {
    throw captionTimelineError("authoritative_caption_audio_sha256_mismatch");
  }
  if (mappingTimestampsSha256 !== expectedTimestampsSha256) {
    throw captionTimelineError("authoritative_caption_alignment_sha256_mismatch");
  }

  const durationSeconds = Number(
    pack.render_manifest?.duration_seconds ||
      pack.render_manifest?.rendered_duration_s ||
      pack.render_manifest?.duration_s ||
      pack.render_manifest?.duration,
  );
  if (finalPublishRender && (!Number.isFinite(durationSeconds) || durationSeconds <= 0)) {
    throw captionTimelineError("final_publish_caption_render_duration_missing");
  }
  let previousEnd = 0;
  const cues = groups.map((group, index) => {
    const text = captionText(group?.text);
    ...
    if (
      ...
      (index > 0 && start < previousEnd - 1e-6) ||
      (Number.isFinite(durationSeconds) && durationSeconds > 0 && end > durationSeconds + 0.001)
    ) {
      throw captionTimelineError("authoritative_caption_mapping_cue_invalid");
    }
    previousEnd = end;
    return { text, start, end };
  });
  if (captionParityText(cues.map((cue) => cue.text).join(" ")) !== expectedDisplayText) {
    throw captionTimelineError("authoritative_caption_cue_text_mismatch");
  }
...
function editorialMetadataBody(value = "") {
  const raw = String(value || "").trim();
  if (!raw) return "";
  const paragraphs = raw.split(/\r?\n\s*\r?\n/);
  const administrationIndex = paragraphs.findIndex((paragraph, index) =>
    index > 0 &&
    /^(?:source|sources|footage|additional\s+footage|music|synthetic\s+narration|game\s+information|credits?|links?)\s*:/i.test(
      paragraph.trim(),
    ),
  );
  return (administrationIndex > 0 ? paragraphs.slice(0, administrationIndex) : paragraphs)
    .join("\n\n")
    .trim();
}

function sourceSafeSocialBodyTooPlain(value = "", { maxWords = 45 } = {}) {
  const rightsNotice = splitMicrosoftGameContentNotice(editorialMetadataBody(value));
  ...
  if (wordCount(text) > maxWords) return true;
...
function verifiedExactFirstFrameHeadline(canonical = {}, story = {}) {
  const evidence = story.first_frame_text_evidence || canonical.first_frame_text_evidence || {};
  const exactText = cleanText(evidence.exact_text);
  const suppliedText = cleanText(story.first_frame_text || canonical.first_frame_text);
  const evidenceStatus = cleanText(evidence.status).toLowerCase();
  const evidenceRenderSha = cleanText(evidence.render_sha256).toLowerCase();
  const sourceMasterSha = cleanText(evidence.source_master_sha256).toLowerCase();
  const boundRenderSha = cleanText(
    story.render_manifest?.reviewed_sha256 ||
      story.render_manifest?.output_sha256 ||
      story.render_manifest?.final_media?.sha256 ||
      canonical.render_manifest?.reviewed_sha256 ||
      canonical.render_manifest?.output_sha256,
  ).toLowerCase();
  if (
    evidence.verified !== true ||
    !["pass", "green", "verified"].includes(evidenceStatus) ||
    !exactText ||
    normaliseSignatureText(exactText) !== normaliseSignatureText(suppliedText) ||
    !/^[a-f0-9]{64}$/.test(evidenceRenderSha) ||
    !/^[a-f0-9]{64}$/.test(sourceMasterSha) ||
    evidenceRenderSha !== boundRenderSha
  ) {
    return "";
  }
  return exactText.toUpperCase();
}

function verifiedPlatformCoverContract(story = {}, canonical = {}) {
  const supplied = story.platform_cover_manifest || canonical.platform_cover_manifest || {};
  if (!objectHasKeys(supplied)) {
    return { claimed: false, valid: false, manifest: {}, blockers: [] };
  }

  const blockers = [];
  const block = (code) => blockers.push(code);
  const storyId = cleanText(story.id || story.story_id || canonical.story_id);
  const suppliedStoryId = cleanText(supplied.story_id);
  const status = cleanText(supplied.status).toUpperCase();
  const headline = cleanText(supplied.headline).toUpperCase();
  const subject = storySubject(story, canonical);
  const sourceLabel = cleanText(supplied.source_label);
  const primarySource = story.primary_source && typeof story.primary_source === "object"
    ? cleanText(
        story.primary_source.name ||
          story.primary_source.label ||
          story.primary_source.source_name,
      )
    : cleanText(story.primary_source || canonical.primary_source);
  const renderManifest = objectHasKeys(story.render_manifest)
    ? story.render_manifest
    : objectHasKeys(story.renderManifest)
      ? story.renderManifest
      : canonical.render_manifest || {};
  const boundRenderSha = cleanText(
    renderManifest.reviewed_sha256 ||
      renderManifest.output_sha256 ||
      renderManifest.final_media?.sha256,
  ).toLowerCase();
  const acceptedRenderSha = cleanText(supplied.accepted_render_sha256).toLowerCase();
  const acceptedRenderPath = cleanText(
    supplied.accepted_render_path ||
      renderManifest.output_path ||
      renderManifest.output ||
      renderManifest.final_media?.path,
  );
  const frameTimeSeconds = Number(supplied.frame_time_seconds);
  const frameAsset = supplied.frame_asset || {};
  const framePath = cleanText(frameAsset.path);
  const frameSha = cleanText(frameAsset.sha256).toLowerCase();
  const visibleTextEvidence = asArray(supplied.visible_text_evidence).map(cleanText).filter(Boolean);
  const visibleTextKey = normaliseSignatureText(visibleTextEvidence.join(" "));
  const derivation = supplied.derivation || {};
  const youtube = supplied.platforms?.youtube_shorts || {};
  const instagram = supplied.platforms?.instagram_reels || {};
  const facebook = supplied.platforms?.facebook_reels || {};

  if (Number(supplied.schema_version) !== 1) block("schema_version_invalid");
  if (!storyId || suppliedStoryId !== storyId) block("story_id_mismatch");
  if (supplied.verified !== true || !["PASS", "GREEN", "VERIFIED"].includes(status)) {
    block("verification_not_passed");
  }
  if (!headline || headlineTokens(headline).length > 8 || weakCoverHeadline(headline, subject)) {
    block("headline_invalid");
  }
  if (!coverHeadlineMatchesSubject(headline, subject)) block("headline_subject_mismatch");
  if (!primarySource || normaliseSignatureText(sourceLabel) !== normaliseSignatureText(primarySource)) {
    block("source_label_mismatch");
  }
  if (!/^[a-f0-9]{64}$/.test(acceptedRenderSha) || acceptedRenderSha !== boundRenderSha) {
    block("accepted_render_sha256_mismatch");
  }
  if (!regularNonSymlinkFile(acceptedRenderPath)) block("accepted_render_file_invalid");
  if (!Number.isFinite(frameTimeSeconds) || frameTimeSeconds < 0) block("frame_time_invalid");
  if (!regularNonSymlinkFile(framePath)) block("frame_asset_file_invalid");
  if (!/^[a-f0-9]{64}$/.test(frameSha)) block("frame_asset_sha256_invalid");
  if (
    Number(frameAsset.width) !== 1080 ||
    Number(frameAsset.height) !== 1920 ||
    cleanText(frameAsset.mime_type).toLowerCase() !== "image/png"
  ) {
    block("frame_asset_contract_invalid");
  }
  if (
    derivation.type !== "exact_frame_extracted_from_accepted_render" ||
    derivation.external_generated_imagery_used !== false ||
    derivation.pulse_authored_graphics_only !== true
  ) {
    block("derivation_contract_invalid");
  }
  if (
    !headline ||
    !visibleTextKey.includes(normaliseSignatureText(headline)) ||
    !visibleTextKey.includes(normaliseSignatureText(subject))
  ) {
    block("visible_text_evidence_mismatch");
  }
  if (
    cleanText(youtube.selection_mode) !== "in_video_frame" ||
    Math.abs(Number(youtube.frame_time_seconds) - frameTimeSeconds) > 0.001 ||
    youtube.custom_thumbnail_upload_allowed !== false ||
    cleanText(youtube.application_status).toUpperCase() !== "NOT_APPLIED"
  ) {
    block("youtube_shorts_contract_invalid");
  }
  if (
    cleanText(instagram.selection_mode) !== "separate_cover_image" ||
    cleanText(instagram.application_status).toUpperCase() !== "NOT_APPLIED"
  ) {
    block("instagram_reels_contract_invalid");
  }
  if (
    cleanText(facebook.selection_mode) !== "in_video_frame" ||
    Math.abs(Number(facebook.frame_time_seconds) - frameTimeSeconds) > 0.001 ||
    cleanText(facebook.application_status).toUpperCase() !== "NOT_APPLIED"
  ) {
    block("facebook_reels_contract_invalid");
  }

  let renderProbe = {};
  let frameProbe = {};
  let decodedPixelSha256 = "";
  if (regularNonSymlinkFile(acceptedRenderPath)) {
    try {
      if (sha256FileSync(acceptedRenderPath) !== acceptedRenderSha) {
        block("accepted_render_file_sha256_mismatch");
      }
      renderProbe = probeVisualAsset(acceptedRenderPath);
      const declaredDuration = Number(
        renderManifest.rendered_duration_s ||
          renderManifest.duration_seconds ||
          renderManifest.duration_s ||
          renderProbe.duration_seconds,
      );
      if (!Number.isFinite(declaredDuration) || frameTimeSeconds >= declaredDuration) {
        block("frame_time_outside_render");
      }
    } catch {
      block("accepted_render_decode_failed");
    }
  }
  if (regularNonSymlinkFile(framePath)) {
    try {
      if (sha256FileSync(framePath) !== frameSha) block("frame_asset_sha256_mismatch");
      frameProbe = probeVisualAsset(framePath);
      if (
        frameProbe.codec_name !== "png" ||
        frameProbe.width !== 1080 ||
        frameProbe.height !== 1920
      ) {
        block("frame_asset_probe_mismatch");
      }
    } catch {
      block("frame_asset_decode_failed");
    }
  }
  if (
    regularNonSymlinkFile(acceptedRenderPath) &&
    regularNonSymlinkFile(framePath) &&
    Number.isFinite(frameTimeSeconds)
  ) {
    try {
      const renderPixels = decodedRgbFrame(acceptedRenderPath, frameTimeSeconds);
      const framePixels = decodedRgbFrame(framePath);
      decodedPixelSha256 = crypto.createHash("sha256").update(framePixels).digest("hex");
      if (!renderPixels.equals(framePixels)) block("frame_pixels_do_not_match_accepted_render");
    } catch {
      block("frame_pixel_verification_failed");
    }
  }
  ...
  return {
    ...
    manifest: {
      ...
      accepted_render_path: acceptedRenderPath ? path.resolve(acceptedRenderPath) : null,
      accepted_render_sha256: acceptedRenderSha,
      frame_time_seconds: Number.isFinite(frameTimeSeconds) ? frameTimeSeconds : null,
      visible_text_evidence: visibleTextEvidence,
      frame_asset: {
        ...frameAsset,
        path: framePath ? path.resolve(framePath) : null,
        sha256: frameSha,
        width: Number(frameAsset.width),
        height: Number(frameAsset.height),
        mime_type: cleanText(frameAsset.mime_type).toLowerCase(),
      },
      verification: {
        accepted_render_hash_bound: valid,
        exact_frame_pixels_match: valid,
        actual_frame_codec: frameProbe.codec_name || null,
        actual_frame_width: Number.isFinite(frameProbe.width) ? frameProbe.width : null,
        actual_frame_height: Number.isFinite(frameProbe.height) ? frameProbe.height : null,
        decoded_frame_rgb_sha256: decodedPixelSha256 || null,
        render_duration_seconds: Number.isFinite(renderProbe.duration_seconds)
          ? renderProbe.duration_seconds
          : null,
      },
      blockers: uniqueBlockers,
...
function attentionCoverHeadline({ canonical = {}, story = {}, subject = "", title = "", angle = "", firstLine = "" } = {}) {
  const verifiedFirstFrame = verifiedExactFirstFrameHeadline(canonical, story);
  if (verifiedFirstFrame) return verifiedFirstFrame;
...
function canonicalAttentionCopy(canonical = {}, story = {}, { platformCoverContract = {} } = {}) {
  const subject = storySubject(story, canonical);
  ...
  const verifiedFirstFrame = verifiedExactFirstFrameHeadline(canonical, story);
  const headline = platformCoverContract.valid === true
    ? cleanText(platformCoverContract.manifest?.headline).toUpperCase()
    : attentionCoverHeadline({ canonical, story, subject, title, angle, firstLine });
  ...
  return {
    ...
    first_frame_text: verifiedFirstFrame || (firstFrameMatchesHeadline ? existingFirstFrame : headline),
...
function buildPlatformNativePublishPacks({
  ...
  const headline = platformCoverContract.valid === true
    ? cleanText(platformCoverContract.manifest?.headline).toUpperCase()
    : attentionCoverHeadline({ canonical, story, subject, title, angle, firstLine });
  const verifiedCoverAsset = platformCoverContract.valid === true
    ? { ...(platformCoverContract.manifest?.frame_asset || {}) }
    : null;
  const coverPlatformContract = (platform) => {
    if (!verifiedCoverAsset) return {};
    const selection = platformCoverContract.manifest?.platforms?.[platform] || {};
    return {
      asset: verifiedCoverAsset,
      selection_mode: cleanText(selection.selection_mode),
      ...(Number.isFinite(Number(selection.frame_time_seconds))
        ? { frame_time_seconds: Number(selection.frame_time_seconds) }
        : {}),
      ...(typeof selection.custom_thumbnail_upload_allowed === "boolean"
        ? { custom_thumbnail_upload_allowed: selection.custom_thumbnail_upload_allowed }
        : {}),
      application_status: cleanText(selection.application_status).toUpperCase(),
      ...(cleanText(selection.contract_basis)
        ? { contract_basis: cleanText(selection.contract_basis) }
        : {}),
      ...(cleanText(selection.contract_url)
        ? { contract_url: cleanText(selection.contract_url) }
        : {}),
      ...(typeof selection.custom_cover_capability_claimed === "boolean"
        ? { custom_cover_capability_claimed: selection.custom_cover_capability_claimed }
        : {}),
    };
  };
  ...
  const outputs = {
    youtube_shorts: mergePlatformOutput(cleanPlatformOutputs.youtube_shorts, {
      ...
      cover_frame: {
        ...
        ...coverPlatformContract("youtube_shorts"),
      },
      captions: {
        file: "captions.srt",
    ...
    instagram_reels: mergePlatformOutput(cleanPlatformOutputs.instagram_reels, {
      ...
      cover_frame: {
        ...
        text_word_limit: 7,
        source_label: sourceName,
        ...coverPlatformContract("instagram_reels"),
      },
      caption: attentionDescription,
      carousel_companion: {
        required: true,
    ...
    facebook_reels: mergePlatformOutput(cleanPlatformOutputs.facebook_reels, {
      ...
      cover_frame: {
        ...
        text_word_limit: 7,
        source_label: sourceName,
        ...coverPlatformContract("facebook_reels"),
      },
...
function platformNativeQualityFailures(outputs = {}) {
  ...
  for (const platform of PLATFORM_NATIVE_ORDER) {
    ...
    const qualityBody = platform === "youtube_shorts"
    ...
    if (sourceSafeSocialBodyTooPlain(qualityBody, {
      maxWords: platform === "youtube_shorts" ? 70 : 45,
    })) {
...
function buildGoalProofPackage({
  ...
  const visualCtaContract = visualCtaManifestContract(story);
  const baseScriptScorecard = buildViralScriptIntelligence({
    story,
    script: story.full_script || story.tts_script || "",
    ctaPolicy: visualCtaContract.valid ? "optional" : "required",
  });
  const scriptScorecard = visualCtaContract.valid
    ? {
        ...baseScriptScorecard,
        verdict:
          Number(baseScriptScorecard.cta?.count || 0) === 0
            ? baseScriptScorecard.verdict
            : "rewrite_required",
        blockers:
          Number(baseScriptScorecard.cta?.count || 0) === 0
            ? asArray(baseScriptScorecard.blockers)
            : [...new Set([...asArray(baseScriptScorecard.blockers), "duplicated_cta"])],
        cta: {
          ...baseScriptScorecard.cta,
          exact: PRIMARY_PULSE_CTA,
          spoken_count: Number(baseScriptScorecard.cta?.count || 0),
          visual_count: 1,
          count: Number(baseScriptScorecard.cta?.count || 0) + 1,
          delivery_mode: visualCtaContract.delivery_mode,
          start_seconds: visualCtaContract.start_seconds,
          end_seconds: visualCtaContract.end_seconds,
          render_sha256: visualCtaContract.render_sha256,
          evidence_sha256: visualCtaContract.evidence_sha256,
        },
      }
    : baseScriptScorecard;
  ...
  const platformCoverContract = verifiedPlatformCoverContract(
    story,
    initialGovernanceReport.canonical_story_manifest,
  );
  const canonicalStoryManifest = canonicalAttentionCopy(
    initialGovernanceReport.canonical_story_manifest,
    story,
    { platformCoverContract },
  );
  ...
  const finalPublishBlockers = [
    ...
    ...platformCoverContract.blockers.map((blocker) => `platform_cover:${blocker}`),
    ...visualCtaContract.blockers.map((blocker) => `visual_cta:${blocker}`),
  ...
  const pack = {
    ...
    ...(platformCoverContract.claimed
      ? { platform_cover_manifest: platformCoverContract.manifest }
      : {}),
...
function closeGovernedYouTubeUploadRequest({
  publishPack = {},
  storyId = "",
  videoPath = "",
  captionPath = "",
} = {}) {
  const disclosureSetting = cleanText(
    publishPack.altered_synthetic_disclosure_setting,
  ).toUpperCase();
  if (!["YES", "NO"].includes(disclosureSetting)) {
    throw new Error("youtube_upload_disclosure_setting_required");
  }
  const hashtags = asArray(publishPack.hashtags)
    .map((value) => cleanText(value).replace(/^#+/, ""))
    .filter(Boolean);
  const explicitTags = asArray(publishPack.tags).map(cleanText).filter(Boolean);
  const tags = [...new Set([...explicitTags, ...hashtags])];
  const videoFile = path.basename(videoPath);
  const captionFile = path.basename(captionPath);
  const containsSyntheticMedia = disclosureSetting === "YES";
  const coverFrame = publishPack.cover_frame || {};
  const coverSelection = cleanText(coverFrame.selection_mode) === "in_video_frame"
    ? {
        selection_mode: "in_video_frame",
        frame_time_seconds: Number(coverFrame.frame_time_seconds),
        custom_thumbnail_upload_allowed: coverFrame.custom_thumbnail_upload_allowed === true,
        application_status: cleanText(coverFrame.application_status).toUpperCase() || "NOT_APPLIED",
        frame_asset_sha256: cleanText(coverFrame.asset?.sha256).toLowerCase() || null,
      }
    : null;
  const basePack = {
    ...publishPack,
    ...
    platform: "youtube_shorts",
    captions: {
      ...(publishPack.captions || {}),
      file: captionFile,
      clean_manual_captions_required: true,
    },
  };
  if (!containsSyntheticMedia) return basePack;
  return {
    ...basePack,
    youtube_upload_request: {
      schema_version: 1,
      closed: true,
      authority_scope: "request_shape_only",
      publish_authority: "NOT_EVALUATED",
      video_file: videoFile,
      video_sha256: sha256FileSync(videoPath),
      notifySubscribers: false,
      snippet: {
        title: String(publishPack.title || ""),
        description: String(publishPack.description || ""),
        tags,
        categoryId: "20",
      ...
      ...(coverSelection ? { cover_selection: coverSelection } : {}),
      captions: {
        file: captionFile,
        sha256: sha256FileSync(captionPath),
        language: "en-GB",
...
function publishPackWithCoverAsset(publishPack = {}, frameAsset = {}) {
  if (!objectHasKeys(publishPack.cover_frame)) return publishPack;
  return {
    ...publishPack,
    cover_frame: {
      ...publishPack.cover_frame,
      asset: { ...frameAsset },
    },
  };
}

async function materialisePlatformCoverAssetForPackage(pack = {}, outDir = "") {
  const manifest = pack.platform_cover_manifest || {};
  if (manifest.verified !== true || cleanText(manifest.status).toUpperCase() !== "PASS") {
    return pack;
  }
  const sourcePath = cleanText(manifest.frame_asset?.path);
  const expectedSha256 = cleanText(manifest.frame_asset?.sha256).toLowerCase();
  if (!regularNonSymlinkFile(sourcePath) || !/^[a-f0-9]{64}$/.test(expectedSha256)) {
    throw new Error("verified_platform_cover_asset_invalid_at_materialisation");
  }
  if (sha256FileSync(sourcePath) !== expectedSha256) {
    throw new Error("verified_platform_cover_asset_changed_before_materialisation");
  }
  const coverDir = path.resolve(outDir, "platform-cover");
  const targetPath = path.resolve(coverDir, "accepted-cover-frame.png");
  const relativeTarget = path.relative(path.resolve(outDir), targetPath);
  if (relativeTarget.startsWith("..") || path.isAbsolute(relativeTarget)) {
    throw new Error("platform_cover_target_outside_package");
  }
  await fs.ensureDir(coverDir);
  if (path.resolve(sourcePath) !== targetPath) {
    await fs.copy(sourcePath, targetPath, { overwrite: false });
  }
  if (sha256FileSync(targetPath) !== expectedSha256) {
    throw new Error("materialised_platform_cover_sha256_mismatch");
  }
  const frameAsset = {
    ...manifest.frame_asset,
    path: targetPath,
    sha256: expectedSha256,
  };
  const outputs = Object.fromEntries(
    Object.entries(pack.platform_publish_manifest?.outputs || {}).map(([platform, publishPack]) => [
      platform,
      publishPackWithCoverAsset(publishPack, frameAsset),
    ]),
  );
  const variantOutputs = Object.fromEntries(
    Object.entries(pack.platform_variant_scorecard?.outputs || {}).map(([platform, publishPack]) => [
      platform,
      publishPackWithCoverAsset(publishPack, frameAsset),
    ]),
  );
  return {
    ...
    youtube_publish_pack: publishPackWithCoverAsset(pack.youtube_publish_pack, frameAsset),
    instagram_publish_pack: publishPackWithCoverAsset(pack.instagram_publish_pack, frameAsset),
    facebook_publish_pack: publishPackWithCoverAsset(pack.facebook_publish_pack, frameAsset),
    platform_publish_manifest: {
      ...(pack.platform_publish_manifest || {}),
      outputs,
    },
    platform_variant_scorecard: {
      ...(pack.platform_variant_scorecard || {}),
...
async function writeGoalProofPackageArtifacts(pack = {}, { outputDir } = {}) {
  ...
  materialisedPack = await materialisePlatformCoverAssetForPackage(materialisedPack, outDir);
  const authoritativeCaptionSrt = await buildAuthoritativeCaptionSrt(materialisedPack);
  const mapping = {
    ...
    platform_cover_manifest: "platform-cover-manifest.json",
    acceptance_entry: "goal_package_summary.json",
  };
  const written = {};
  for (const [key, basename] of Object.entries(mapping)) {
    if (key === "platform_cover_manifest" && !objectHasKeys(materialisedPack[key])) continue;
  ...
  const closedYoutubePack = closeGovernedYouTubeUploadRequest({
    publishPack: materialisedPack.youtube_publish_pack || {},
    storyId: materialisedPack.story_id,
    videoPath: written.visual_v4_render,
    captionPath: written.captions,
  });
  materialisedPack = {
    ...materialisedPack,
    youtube_publish_pack: closedYoutubePack,
    platform_publish_manifest: {
      ...(materialisedPack.platform_publish_manifest || {}),
      outputs: {
        ...(materialisedPack.platform_publish_manifest?.outputs || {}),
        youtube_shorts: closedYoutubePack,
      },
    },
  };
  written.youtube_publish_pack = await writeJson(
    path.join(outDir, mapping.youtube_publish_pack),
    closedYoutubePack,
  );
  written.platform_publish_manifest = await writeJson(
    path.join(outDir, mapping.platform_publish_manifest),
    materialisedPack.platform_publish_manifest,
  );
