...
function platformConsequenceLanguage(value = "") {
  return /\b(?:problem|risk|catch|warning|changed|broke|broken|revealed|confirmed|finally|pushback|why|before|after|ceiling|impact|cost|deal|launch|lands?|hits?|date|proof|payoff|trust|pressure|fight|split|splits|splitting|bloat|dangerous|delete|wins?|timing|more than|handmade|leaked?|tactics?|xcom|exposes?|spike|peak|demand|bet|custom seas|private seas|private sessions?|private mode|rule controls?|ea\s+play|ps5|jungle|combat|discovery|readability|map|choices?|empathy|atmosphere|horror|jump[- ]?scares?)\b/i.test(
    cleanText(value),
  );
}
...
function platformAudiencePullLanguage(value = "") {
  ...
  const hasSpecificity =
    /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|one|only|first|last|free|paid|delay(?:ed)?|leak(?:ed)?|launch|ending|before|after|price|trial|demo|early[- ]access|steam peak|steam spike|steam demand|custom seas|private seas|private sessions?|private mode|rule controls?|set (?:their own )?rules|ea\s+play|ps5|jungle|combat|discovery|readability|map|choices?|empathy|atmosphere|horror|jump[- ]?scares?|roguelite|podracing|repeat[- ]run|wipeout|handling)\b/i.test(text);
  const hasCuriosity =
...
function buildPlatformNativePublishPacks({
  ...
  const outputs = {
    ...
    x: mergePlatformOutput(platformOutputs.x, {
      ...
      hot_take_post: `${subjectClaim}. That is the part of this story everyone will argue about.`,
      source_safe_post: `${title}\n\n${sourceLine} Full source list: ${landingPageLink}`,
      concise_news_post: `${subjectClaim}.`,
      thread_posts: [
        title,
        `${sourceLine} ${subjectClaim}.`,
    ...
    pinterest: {
      ...
      pin_description: `${subjectClaim}. Sources, related links and safer buying routes are on the story page.`,
      evergreen_only: true,
