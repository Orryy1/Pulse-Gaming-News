...
const HEADLINE_DANGLE_RE = /\b(?:a|an|the|of|for|to|into|with|without|has|have|had|is|are|was|were|make|makes|made|turns|turned|gets|get|got|feel)$/i;
...
function attentionBodyFromEvidence({ subject = "", angle = "", title = "", description = "", firstLine = "" } = {}) {
  ...
  if (/\b(?:demo|demos|next fest|playable)\b/i.test(evidenceText)) {
    return `${subject} is where trailers stop doing the work. The playable slice can win wishlists or expose the problem before launch.`;
  }
  if (/\b(?:expansion|dlc|update|season|battle pass)\b/i.test(evidenceText)) {
    return `${subject} is getting more content, but the useful test is whether it gives players a real reason to come back now.`;
  }
  ...
  if (/\b(?:score|metacritic|review)\b/i.test(evidenceText)) {
    return `${subject} has the kind of score players notice, but the useful question is whether that number turns into real momentum.`;
  }
  ...
  return `${subject} has a useful question behind the headline: ${fallback}.`;
}

function platformConsequenceLanguage(value = "") {
  return /\b(?:problem|risk|catch|warning|changed|broke|broken|revealed|confirmed|finally|pushback|why|before|after|ceiling|impact|cost|deal|launch|date|proof|payoff|trust|pressure|fight|bloat|dangerous|delete|wins?|test|timing|more than|handmade|leaked?|tactics?|xcom)\b/i.test(
    cleanText(value),
  );
}
...
function platformAudiencePullLanguage(value = "") {
  ...
  const hasSpecificity =
    /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|one|only|first|last|free|paid|delay(?:ed)?|leak(?:ed)?|launch|ending|before|after|price|trial|demo)\b/i.test(text);
  const hasCuriosity =
    /\b(?:why|how|what|secret|hidden|real|catch|risk|problem|fight|pressure|warning|trust|ceiling|broke|turns?|changed|into|test|timing|more than|handmade|xcom|tactics?)\b/i.test(text);
...
function attentionPlatformTitle({ subject = "", title = "", angle = "", description = "", firstLine = "" } = {}) {
  ...
  if (/\b(?:ea play|game pass|ps plus|subscription)\b/i.test(evidenceText)) {
    return `${subject} Just Became A Low-Risk Trial`;
  }
  if (/\b(?:demo|demos|next fest|playable)\b/i.test(evidenceText)) {
    return `${subject} Has To Prove Itself In Public`;
  }
  if (/\b(?:deal|discount|sale|price|reserved?|reservation|preorder|pre-order)\b/i.test(evidenceText)) {
    return `${subject} Has A Price Timing Problem`;
  }
  if (/\b(?:expansion|dlc|update|season|battle pass)\b/i.test(evidenceText)) {
    return `${subject} Needs More Than Another Update`;
  }
  if (/\b(?:delay|delayed|moved|avoids?|crowded)\b/i.test(evidenceText)) {
    return `${subject} Just Dodged A Release-Date Fight`;
  }
  if (/\b(?:score|metacritic|review)\b/i.test(evidenceText)) {
    return `${subject} Has A Momentum Problem`;
  }
  return `${subject} Has A Real Player Test Now`;
}
...
function platformAttentionDescription({ canonical = {}, subject = "", angle = "", sourceLine = "", title = "", firstLine = "" } = {}) {
  ...
  const body = usefulPublicAngle(description) && !sourceSafeSocialBodyTooPlain(description)
    ? description
...
function weakCoverHeadline(value = "", subject = "") {
  ...
  return headlineTokens(text).length <= 3 && !/\b(?:risk|test|fight|trust|bloat|danger|pressure|problem|wins?|demo)\b/i.test(text);
}
...
function attentionCoverHeadline({ canonical = {}, story = {}, subject = "", title = "", angle = "", firstLine = "" } = {}) {
  ...
  if (/\btrust\b/i.test(evidenceText)) return `${subjectHead} TRUST TEST`;
  if (/\bdemo\b/i.test(evidenceText)) return `${subjectHead} DEMO TEST`;
  if (/\b(?:deal|discount|sale|price|reserved?|reservation|preorder|pre-order)\b/i.test(evidenceText)) return `${subjectHead} PRICE TEST`;
  if (/\b(?:score|metacritic|review)\b/i.test(evidenceText)) return `${subjectHead} MOMENTUM TEST`;
