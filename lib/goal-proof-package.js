...
function platformConsequenceLanguage(value = "") {
  return /\b(?:problem|risk|catch|warning|changed|broke|broken|revealed|confirmed|finally|pushback|why|before|after|ceiling|impact|cost|deal|launch|lands?|hits?|date|proof|payoff|trust|pressure|fight|bloat|dangerous|delete|wins?|timing|more than|handmade|leaked?|tactics?|xcom|exposes?|spike|peak|demand|bet|ea\s+play|ps5|jungle|combat|discovery|readability|map)\b/i.test(
    cleanText(value),
  );
}
...
function platformAudiencePullLanguage(value = "") {
  ...
  const hasSpecificity =
    /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|one|only|first|last|free|paid|delay(?:ed)?|leak(?:ed)?|launch|ending|before|after|price|trial|demo|early[- ]access|steam peak|steam spike|steam demand|ea\s+play|ps5|jungle|combat|discovery|readability|map)\b/i.test(text);
  const hasCuriosity =
