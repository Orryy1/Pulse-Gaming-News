...
const FORMULAIC_PUBLIC_NARRATION_PATTERNS = [
  ...
  /\bfamiliar logo\b/i,
  /\bmontage pace\b/i,
  /\bthe catch is what matters after the reveal cut\b/i,
...
const PUBLIC_URL_PATTERN_SOURCE =
  String.raw`(?:https?:)?\/\/[^\s<>"']+|\bwww\.[^\s<>"']+|\b(?:[a-z0-9](?:[a-z0-9-]{0,62}[a-z0-9])?\.)+[a-z]{2,24}(?:[/?#][^\s<>"']*)?`;

function containsPublicUrl(value = "") {
  return new RegExp(PUBLIC_URL_PATTERN_SOURCE, "i").test(String(value || ""));
}

function stripPublicUrls(value = "") {
  return String(value || "").replace(new RegExp(PUBLIC_URL_PATTERN_SOURCE, "gi"), "");
}
...
const UNSUPPORTED_DETAIL_TERMS = [
  {
    publicPattern: /\bframe[- ]?rates?(?:\s+clips?)?\b|\b\d{2,3}\s*fps\b/i,
    evidencePattern: /\bframe[- ]?rates?\b|\bfps\b|\bperformance mode\b/i,
  },
  {
...
function subjectAliasKeys(subject) {
  const key = normaliseKey(subject);
  ...
  if (/\bdenshattack\b/.test(key)) {
    aliases.add("densha attack");
    aliases.add("denshaattack");
  }
...
const PLATFORM_HEADLINE_ENTITY_STOPWORDS = new Set([
  "game",
  "games",
  "gaming",
  "news",
...
function meaningfulPlatformHeadlineTokens(value = "") {
  return normaliseKey(value)
    .split(/\s+/)
    .filter((token) => token.length >= 4 && !PLATFORM_HEADLINE_ENTITY_STOPWORDS.has(token));
}

function platformCoverHeadlineCarriesStoryEntity({ value = "", subject = "", manifest = {}, outputs = {} } = {}) {
  if (!value || !subject) return false;
  const titleEvidence = clean([
  ...
  if (!textContainsSubject(titleEvidence, subject)) return false;
  const rawEvidence = clean([
    titleEvidence,
    ...
    ...(Array.isArray(manifest.allowed_public_wording) ? manifest.allowed_public_wording : []),
    ...flattenPublicStrings(outputs).map((item) => item.value),
  ].join(" "));
  const priceTokens = clean(value).match(/[$£€]\s?\d+(?:[.,]\d{1,2})?/g) || [];
  if (
    priceTokens.some((price) =>
      rawEvidence.replace(/\s+/g, "").includes(price.replace(/\s+/g, ""))
    )
  ) {
    return true;
  }
  if (
    /\bday[- ]one\s+dlc\b/i.test(clean(value)) &&
    /\bday[- ]one\s+dlc\b/i.test(rawEvidence)
  ) {
    return true;
  }
  const evidence = normaliseKey(rawEvidence);
  const tokens = meaningfulPlatformHeadlineTokens(value);
...
function platformPublicCopyFailures(manifest = {}) {
  ...
  if (subject && !hasGenericSubject(subject)) {
    const subjectMisses = flattenPublicStrings(outputs)
      ...
      .filter(({ path, value }) => {
        if (textContainsSubject(value, subject)) return false;
        if (
          /\.cover_frame\.headline$/i.test(path) &&
          platformCoverHeadlineCarriesStoryEntity({ value, subject, manifest, outputs })
        ) {
          return false;
        }
        return true;
...
function hasUnanchoredReviewScoreLanguage(manifest = {}, script = "") {
  const text = clean(script).replace(/\b(?:viewership|views?|trailer views?|attention|demand|hype|audience)\s+(?:is|are|was|were)\s+not\s+(?:a\s+)?review score\b/gi, "");
...
function hasSemanticallyTruncatedThumbnail(manifest = {}, title = "") {
  ...
  return missingMeaningfulWords.length === 1 && /^(?:catch|silence|problem|question)$/i.test(missingMeaningfulWords[0]);
}
...
const DANGLING_THUMBNAIL_VERB_TERMINALS = new Set([
  "broke",
  "breaks",
  "calls",
  "chased",
...
function hasDanglingThumbnailHeadline(manifest = {}) {
  ...
  return (
    DANGLING_THUMBNAIL_TERMINALS.has(terminal) ||
    ((previous === "the" || previous === "a" || previous === "an") &&
      DANGLING_THUMBNAIL_VERB_TERMINALS.has(terminal))
  );
}
...
function normaliseScriptComparisonText(value = "") {
  return clean(normaliseCaptionDisplayText(value))
    .replace(/\bofficial\s+cover\s+art\s+for\s+(?:GTA\s+VI|Grand\s+Theft\s+Auto\s+VI)\b/gi, "official GTA VI cover art")
    .replace(/\bcover\s+art\s+for\s+(?:GTA\s+VI|Grand\s+Theft\s+Auto\s+VI)\b/gi, "GTA VI cover art")
    .replace(/\breesynced\b/gi, "Resynced")
    .toLowerCase();
}

function normaliseScriptWordSequence(value = "") {
  return normaliseScriptComparisonText(value)
    .replace(/[^\p{L}\p{N}']+/gu, " ")
    .replace(/\s+/g, " ")
    .trim();
}

const SPOKEN_SINGLE_DIGIT_ALIASES = new Map([
  ["zero", "0"],
  ["one", "1"],
  ["two", "2"],
  ["three", "3"],
...
function pronunciationWordSequencesEquivalent(primary = "", alternate = "") {
  const primaryWords = normaliseScriptWordSequence(primary).split(/\s+/).filter(Boolean);
  const alternateWords = normaliseScriptWordSequence(alternate).split(/\s+/).filter(Boolean);
  if (primaryWords.length !== alternateWords.length) return false;
  return primaryWords.every((word, index) => {
    const alternateWord = alternateWords[index];
    if (word === alternateWord) return true;
    return (
      (SPOKEN_SINGLE_DIGIT_ALIASES.get(word) || word) ===
      (SPOKEN_SINGLE_DIGIT_ALIASES.get(alternateWord) || alternateWord)
    );
  });
}

function applyCurrencyPronunciation(value = "") {
  return String(value || "").replace(
    /([$£€])\s*(\d+)(?:[.,](\d{1,2}))?/g,
    (_, symbol, whole, fraction) => {
      const unit = symbol === "£" ? "pounds" : symbol === "€" ? "euros" : "dollars";
      return `${whole} ${unit}${fraction ? ` ${fraction}` : ""}`;
    },
  );
}

function scriptFieldDiverges(primaryScript = "", alternateScript = "", options = {}) {
  ...
  const comparablePrimary = normaliseScriptComparisonText(primary);
  const comparableAlternate = normaliseScriptComparisonText(alternate);
  if (options.allowPronunciationOnly) {
    if (normaliseScriptWordSequence(primary) === normaliseScriptWordSequence(alternate)) return false;
    const pronunciationSafePrimaryWords = normaliseScriptWordSequence(
      applyGamingPronunciation(applyCurrencyPronunciation(primary)),
    );
    const pronunciationSafeAlternateWords = normaliseScriptWordSequence(
      applyGamingPronunciation(applyCurrencyPronunciation(alternate)),
    );
    if (pronunciationSafeAlternateWords === pronunciationSafePrimaryWords) return false;
    if (
      pronunciationWordSequencesEquivalent(
        pronunciationSafePrimaryWords,
        pronunciationSafeAlternateWords,
      )
    ) {
      return false;
    }
  }
  return Boolean(comparablePrimary !== comparableAlternate);
}

function pronunciationScriptDiverges(narrationScript = "", ttsScript = "") {
  return scriptFieldDiverges(narrationScript, ttsScript, { allowPronunciationOnly: true });
}
...
function evaluateGoalPublicCopy(manifest = {}) {
  ...
  if (scriptFieldDiverges(script, ttsScript, { allowPronunciationOnly: true })) {
    failures.push("public_copy:tts_script_diverges_from_narration");
  }
