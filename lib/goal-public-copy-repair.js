...
function curiosityGapLine({ subject = "", title = "", claim = "" } = {}) {
  ...
  if (/xbox|playstation|nintendo|steam/i.test(joined) && /exclusive|platform|hardware|controller/i.test(joined)) {
    return "The catch is whether that platform clue turns into timing, access or hardware players can actually verify.";
  }
  return "The catch is whether players get a concrete next step, or just another vague update.";
}
...
function storySpecificNarrationPlan({ claim = "", source = "", subject = "", title = "" } = {}) {
  ...
  if (/runescape:?\s*dragonwilds|dragonwilds/i.test(joined) && /\b(?:1\.0|early access|update|survival)\b/i.test(joined)) {
    const firstLine = "RuneScape Dragonwilds is getting one last chance to win back Early Access players.";
    return {
      firstLine,
      script: clean(
        [
          firstLine,
          `${source || "Rock Paper Shotgun"} says the survival spin-off has another major update coming later this month before full launch.`,
