    ...
    return {
      ...
      script: clean(
        [
          ...
          "The catch is raid timing: Niantic still has to explain who gets access and when.",
          "If access holds, Niantic turns Go Fest into a comeback test for lapsed players.",
          `${EXACT_CTA}.`,
        ].join(" "),
      ),
    };
...
function repairGoalPublicCopyManifest(
  ...
  const preserveExistingNarration = !forceNarrationRewrite && thumbnailOnlyPublicCopyRepair(before);
  ...
  const updated = normaliseProtectedNamesInManifest({
    ...
    suggested_thumbnail_text: subjectAnchoredThumbnailHeadline(title, subject),
    first_frame_text: subjectAnchoredThumbnailHeadline(title, subject),
    first_spoken_line: firstLine,
...
async function hydrateManifestFromFreshArtifactEvidence(
  ...
  const coherenceReport = await readJsonIfPresent(path.join(artifactDir, "coherence_report.json"), null);
...
async function repairGoalPublicCopyPackages({
  ...
  for (const storyPackage of asArray(storyPackages)) {
    ...
    const repairedScriptScorecard = scriptScorecardRepairNeeded
