...
function storySpecificNarrationPlan({ claim = "", source = "", subject = "", title = "" } = {}) {
  ...
  if (/hades\s*(?:ii|2)/i.test(joined) && /\bxbox\b/i.test(joined) && /\bplaystation\b/i.test(joined)) {
    const firstLine = "Hades 2 is not just leaving early access.";
    return {
      firstLine,
      script: clean(
        [
          firstLine,
          "Xbox's trailer lists Hades 2 for Xbox and PlayStation, with an April 14 date.",
          "The catch is why controller feel matters: both console crowds get the same launch window, so Supergiant has one shot to make dodge timing and build chaos land cleanly.",
          "If it feels as sharp on pad as it does on PC, April becomes a real console fight instead of a late port.",
          "If it feels off, Hades 2 loses the one thing it cannot fake.",
          `${EXACT_CTA}.`,
        ].join(" "),
      ),
    };
