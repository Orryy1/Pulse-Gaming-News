...
function firstLineForTitle(title, subject, manifest = {}) {
  ...
  if (subject === "PS5" && hasOfficialAttribution(manifest)) {
    return "PS5 prices went up across Europe and the UK.";
  }
  ...
  if (subject === "V Rising") return "V Rising's developers are already building another vampire game.";
  ...
  if (subject === "Crimson Desert" && /\b(?:already live|out now|launched|released|march 19)\b/i.test(text)) {
    return "Crimson Desert is already live after years of glossy showcase footage.";
  }
  ...
  if (subject === "Forza Horizon 6" && /\breview thread\b/i.test(text)) {
    return "Forza Horizon 6 reviews are finally in.";
  }
  if (subject === "Forza Horizon 6" && /\breview\b/i.test(text)) {
    return "Forza Horizon 6 just landed a strong PC Gamer review.";
  }
  if (subject === "Forza Horizon 6" && /\bsteam record|concurrent players\b/i.test(text)) {
...
function playerValueLine(subject = "", title = "", claim = "") {
  ...
  if (/forza horizon 6/i.test(joined) && /review/i.test(joined)) {
    return "Strong reviews matter here because this is when fence-sitters decide whether another Horizon is enough.";
  }
  ...
  if (/crimson desert/i.test(joined)) {
    return "Now the shipped build has to carry the spectacle: combat, performance and scale, not just trailer shots.";
  }
  ...
  if (/\bps5\b|\bplaystation 5\b/i.test(joined) && /\b(?:price hike|more expensive|prices? went up|recommended retail prices|price increase)\b/i.test(joined)) {
    return "That lands hardest on new buyers in markets where the console was already a premium purchase.";
  }
  ...
  return `${subject} now has one concrete change worth remembering after the scroll moves on.`;
}
...
function repairGoalPublicCopyManifest(manifest = {}, { generatedAt = new Date().toISOString(), usedTitles = null } = {}) {
  ...
  const narrationScript = buildNarrationScript({ firstLine, claim, source, subject, title });
