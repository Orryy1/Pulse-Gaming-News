...
function playerValueLine(subject = "", title = "", claim = "") {
  ...
  if (/mega mewtwo|pokemon go|pokémon go/i.test(joined)) {
    return "That turns Go Fest from a paywall flex into a comeback test.";
  }
...
function curiosityGapLine({ subject = "", title = "", claim = "" } = {}) {
  ...
  if (/v rising/i.test(joined)) {
    return "The catch is what matters now: if Stunlock moves beyond V Rising, the original's support plan becomes part of the next game story.";
  }
    ...
    return {
      ...
      script: clean(
        [
          ...
          "Racers live or die in the first lap, not lore.",
          "If the official reveal lands next, it has to show speed, track identity and why this version exists now.",
          `${EXACT_CTA}.`,
        ].join(" "),
      ),
    };
