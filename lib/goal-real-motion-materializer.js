...
const DEFAULT_MAX_DIRECT_MOTION_CLIPS_PER_BASE_SOURCE = 2;
...
function dynamicMaxDirectClipsPerBaseSource(candidates = [], {
  ...
  const defaultCap = DEFAULT_MAX_DIRECT_MOTION_CLIPS_PER_BASE_SOURCE;
  const directCandidates = asArray(candidates).filter((candidate) => cleanText(candidate.media_kind) === "direct_video");
  const requiredClips = Math.max(
    defaultCap,
    Math.min(Number(maxClips || 8), jobDirectVideoFloor(job, minClips)),
  );
  if (directCandidates.length < requiredClips) return defaultCap;
  const windowsByBase = new Map();
  directCandidates.forEach((candidate, index) => {
    const base = directMotionBaseSourceFamily(candidate, index);
    const window = directMotionWindowKey(candidate, index);
    if (!base || !window) return;
    if (!windowsByBase.has(base)) windowsByBase.set(base, new Set());
    windowsByBase.get(base).add(window);
  });
  const usableBaseCount = [...windowsByBase.values()].filter((windows) => windows.size > 0).length;
  if (usableBaseCount < 2) return defaultCap;
  const uniqueWindowCount = [...windowsByBase.values()].reduce((sum, windows) => sum + windows.size, 0);
  if (uniqueWindowCount < requiredClips) return defaultCap;
  for (let cap = defaultCap; cap <= 4; cap += 1) {
    const capacity = [...windowsByBase.values()].reduce((sum, windows) => sum + Math.min(windows.size, cap), 0);
    if (capacity >= requiredClips) return cap;
  }
  return defaultCap;
}
...
async function materializeRealMotionJob(job = {}, options = {}) {
  ...
  const minClips = Number(options.minClips || DEFAULT_MIN_CLIPS);
  const minFamilies = Number(options.minFamilies || DEFAULT_MIN_FAMILIES);
  const candidates = expandDirectVideoWindowCandidates(
