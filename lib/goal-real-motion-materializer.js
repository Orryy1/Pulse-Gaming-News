...
const LIVE_PUBLISH_NOT_ALLOWED_BLOCKER = "rights:live_publish_not_allowed";
const HUMAN_LEGAL_REVIEW_BLOCKER = "rights:human_legal_review_required_before_publish";
const MOTION_SCOPE_LEDGER_REBUILD_BLOCKER =
  "rights:global_ledger_rebuild_required_after_motion_scope_exclusions";
const LIVE_PUBLISH_HOLD_CODES = Object.freeze([
  LIVE_PUBLISH_NOT_ALLOWED_BLOCKER,
  HUMAN_LEGAL_REVIEW_BLOCKER,
]);
...
function rightsFieldPresence(asset = {}) {
  return {
    ...
    source_audio_allowed: hasOwn(asset, "source_audio_allowed"),
    local_materialization_allowed: hasOwn(asset, "local_materialization_allowed"),
    live_publish_allowed: hasOwn(asset, "live_publish_allowed"),
    requires_human_legal_review_before_publish:
      hasOwn(asset, "requires_human_legal_review_before_publish"),
  };
}
...
function genuineBaseSourceFamilies(rows = []) {
  return [
    ...new Set(
      asArray(rows)
        .map((clip, index) => {
          const mediaKind = cleanText(clip.media_kind || clip.mediaKind || "direct_video");
          if (mediaKind === "direct_video" && clipLooksDirectVideo(clip)) {
            return directMotionBaseSourceFamily(clip, index);
          }
          const governedStillMotion =
            ...
            Boolean(cleanText(clip.licence_basis || clip.rights_basis)) &&
            (
              isSafeStillImageUrl(clip.source_url || clip.canonical_source_url) ||
              (
                clip.hash_bound_still_source_identity_verified === true &&
                isSafeOutboundUrl(clip.source_url || clip.canonical_source_url)
              )
            );
          if (!governedStillMotion) return "";
          const identity = compoundMotionSourceIdentity(clip, index);
          return identity.strict_pass === true ? identity.base_source_asset_id : "";
        })
        .filter(Boolean),
    ),
  ];
...
function isExplicitNonCommercialLocalProofHold(asset = {}) {
  const sourceUrl = cleanText(asset.source_url || asset.path);
  const sourceUrlKind = cleanText(asset.source_url_kind) ||
    (sourceUrl && !/^https?:\/\//i.test(sourceUrl) ? "local_video_file" : "");
  const sourceType = lowerText(asset.source_type);
  const licenceBasis = lowerText(
    asset.licence_basis || asset.license_basis || asset.rights_basis,
  );
  const allowedUse = lowerText(asset.allowed_use || asset.allowed_render_use);
  const rightsStatus = lowerText(asset.rights_status);
  const rightsVerdict = lowerText(asset.rights_verdict || asset.verdict);
  const canonicalSourceUrl = cleanText(asset.canonical_source_url);
  const youtubeVideoId = cleanText(asset.youtube_video_id);
  const sourceMasterSha256 = lowerText(asset.source_master_sha256);
  const risk = numberOrNull(asset.risk_score);
  return Boolean(
    sourceType === "official_youtube_channel_url" &&
      sourceUrlKind === "local_video_file" &&
      sourceUrl &&
      !/^https?:\/\//i.test(sourceUrl) &&
      licenceBasis.includes("local_proof_only") &&
      allowedUse === "local_proof_only" &&
      Array.isArray(asset.allowed_platforms) &&
      asset.allowed_platforms.length === 0 &&
      asset.commercial_use_allowed === false &&
      asset.local_materialization_allowed === true &&
      asset.live_publish_allowed === false &&
      asset.requires_human_legal_review_before_publish === true &&
      asset.source_audio_allowed === false &&
      asset.rights_grant === false &&
      rightsStatus === "local_proof_only" &&
      rightsVerdict === "red" &&
      /^https:\/\/(?:www\.)?youtube\.com\/watch\?v=[a-z0-9_-]{6,}(?:&|$)/i.test(
        canonicalSourceUrl,
      ) &&
      /^[a-z0-9_-]{6,}$/i.test(youtubeVideoId) &&
...
function isValidatedLocalOfficialMotionSource(asset = {}, { root = process.cwd() } = {}) {
  ...
  if (
    asset.commercial_use_allowed === false &&
    !isExplicitNonCommercialLocalProofHold(asset)
  ) {
    return false;
  }
  const risk = numberOrNull(asset.risk_score);
...
function isExplicitNonCommercialLocalStillProofHold(asset = {}) {
  const pathRef = cleanText(asset.path);
  const sourceType = lowerText(
    [asset.source_type, asset.type, asset.kind, asset.asset_type].join(" "),
  );
  const licenceBasis = lowerText(
    asset.licence_basis || asset.license_basis || asset.rights_basis,
  );
  ...
  const approvalStatus = lowerText(asset.approval_status);
  const rightsStatus = lowerText(asset.rights_status);
  const evidencePath = cleanText(asset.evidence_file || asset.rights_evidence_file);
  const evidenceSha256 = lowerText(
    asset.evidence_sha256 || asset.rights_evidence_sha256,
  );
  const risk = numberOrNull(asset.risk_score);
  return Boolean(
    sourceType.includes("official_press_kit_stills") &&
      isLocalImagePath(pathRef) &&
      fs.existsSync(pathRef) &&
      (licenceBasis.includes("pending_human_rights_review") ||
        licenceBasis.includes("local_proof_only")) &&
      ["local_transformative_editorial_proof", "local_proof_only"].includes(allowedUse) &&
      Array.isArray(asset.allowed_platforms) &&
      asset.allowed_platforms.length === 0 &&
      asset.commercial_use_allowed === false &&
      asset.local_materialization_allowed === true &&
      asset.live_publish_allowed === false &&
      asset.requires_human_legal_review_before_publish === true &&
      approvalStatus === "approved_for_local_materialization_only" &&
      /rights_unresolved|local_proof_only|pending_human/.test(rightsStatus) &&
...
function isValidatedRealStillAsset(asset = {}, { root = process.cwd() } = {}) {
  if (isPreviouslyMaterializedMotionRecord(asset)) return false;
  if (asset.commercial_use_allowed === false) {
    if (!isExplicitNonCommercialLocalStillProofHold(asset)) return false;
    if (verifiedHashBoundStillSourceIdentity(asset, { root }).verified !== true) {
      return false;
    }
  }
  const risk = numberOrNull(asset.risk_score);
...
function normaliseCandidate(asset = {}, index = 0) {
  ...
  return {
    ...
    source_audio_allowed:
      typeof asset.source_audio_allowed === "boolean"
        ? asset.source_audio_allowed
        : undefined,
    automated_segment_motion_class:
      cleanText(asset.automated_segment_motion_class) || undefined,
    segment_motion_class: cleanText(asset.segment_motion_class) || undefined,
    editorial_semantic_role: cleanText(asset.editorial_semantic_role) || undefined,
    current_event_footage_claim_allowed:
      typeof asset.current_event_footage_claim_allowed === "boolean"
        ? asset.current_event_footage_claim_allowed
        : undefined,
    selection_reason: cleanText(asset.selection_reason) || undefined,
    semantic_review_basis: cleanText(asset.semantic_review_basis) || undefined,
    ...
    local_materialization_allowed:
      typeof asset.local_materialization_allowed === "boolean"
        ? asset.local_materialization_allowed
        : undefined,
...
function normaliseStillCandidate(asset = {}, index = 0) {
  ...
  return {
    ...
    base_source_family: cleanText(
      asset.base_source_family || asset.provenance?.base_source_family,
    ) || undefined,
    path: ref,
    ...
    focal_point_x: normaliseFocalPoint(
      asset.focal_point_x ??
        asset.focalPointX ??
        asset.framing?.focal_point_x ??
        asset.provenance?.focal_point_x,
    ),
    focal_point_y: normaliseFocalPoint(
      asset.focal_point_y ??
        asset.focalPointY ??
        asset.framing?.focal_point_y ??
        asset.provenance?.focal_point_y,
    ),
    licence_basis: cleanText(
      asset.licence_basis ||
        asset.license_basis ||
  ...
  return asArray(segmentValidationReport.segments)
    ...
    .map((segment, index) => {
      ...
      return {
        ...
        source_audio_allowed:
          typeof segment.source_audio_allowed === "boolean"
            ? segment.source_audio_allowed
            : undefined,
        automated_segment_motion_class:
          cleanText(segment.automated_segment_motion_class) || undefined,
        segment_motion_class: cleanText(segment.segment_motion_class) || undefined,
        editorial_semantic_role:
          cleanText(segment.editorial_semantic_role) || undefined,
        current_event_footage_claim_allowed:
          typeof segment.current_event_footage_claim_allowed === "boolean"
            ? segment.current_event_footage_claim_allowed
            : undefined,
        selection_reason: cleanText(segment.selection_reason) || undefined,
        semantic_review_basis: cleanText(segment.semantic_review_basis) || undefined,
        ...
        local_materialization_allowed:
          typeof segment.local_materialization_allowed === "boolean"
            ? segment.local_materialization_allowed
            : undefined,
        live_publish_allowed:
...
function candidateRows({
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    else if (isValidatedRealStillAsset(row, { root })) candidate = normaliseStillCandidate(row, index);
    if (!candidate) continue;
    if (candidate.media_kind === "visual_still") {
      const sidecarSourceIdentity = verifiedHashBoundStillSourceIdentity(row, { root });
      const existingSourceMasterSha256 = lowerText(candidate.source_master_sha256);
      if (
        sidecarSourceIdentity.verified === true &&
        (
          !existingSourceMasterSha256 ||
          existingSourceMasterSha256 === sidecarSourceIdentity.source_master_sha256
        )
      ) {
        candidate = {
          ...candidate,
          canonical_source_url: cleanText(candidate.canonical_source_url || candidate.source_url),
          source_master_path:
            cleanText(candidate.source_master_path) || sidecarSourceIdentity.source_master_path,
          source_master_sha256:
            existingSourceMasterSha256 || sidecarSourceIdentity.source_master_sha256,
...
function mergePreservedMotionEvidence(existing = {}, incoming = {}) {
  ...
  if (existing.source_audio_allowed === false || incoming.source_audio_allowed === false) {
    merged.source_audio_allowed = false;
  } else if (
    existing.source_audio_allowed === true &&
    incoming.source_audio_allowed === true
  ) {
    merged.source_audio_allowed = true;
  }
  if (
    existing.current_event_footage_claim_allowed === false ||
    incoming.current_event_footage_claim_allowed === false
  ) {
    merged.current_event_footage_claim_allowed = false;
  } else if (
    existing.current_event_footage_claim_allowed === true &&
    incoming.current_event_footage_claim_allowed === true
  ) {
    merged.current_event_footage_claim_allowed = true;
  }
  for (const field of [
    "automated_segment_motion_class",
...
function existingMotionClipRows(footageInventory = {}) {
  ...
  for (const [index, asset] of rows.entries()) {
    ...
    const row = {
      ...
      source_identity_provenance: sourceIdentityProvenance(asset),
      source_identity_conflicts: sourceIdentityConflicts(asset),
      hash_bound_still_source_identity_verified:
        asset.hash_bound_still_source_identity_verified === true,
...
function localFileMatchesEvidence(filePath, expectedSha256, expectedSizeBytes) {
  if (!filePath || !/^[a-f0-9]{64}$/i.test(cleanText(expectedSha256))) return false;
  let stat;
  try {
  ...
  const expectedSize = numberOrNull(expectedSizeBytes);
  if (!stat.isFile() || expectedSize == null || expectedSize <= 0 || stat.size !== expectedSize) {
    return false;
  }
  const actualSha256 = crypto
    ...
    .digest("hex");
  return actualSha256 === lowerText(expectedSha256);
}

function hashBoundRightsSidecarStillSourceIdentity({
  asset = {},
  evidencePath = "",
  sourceUrl = "",
  root = process.cwd(),
  ...
  let sidecar;
  try {
    sidecar = fs.readJsonSync(evidencePath);
  } catch {
    return { verified: false };
  }
  const decision = sidecar.decision || {};
  const source = sidecar.source || {};
  const expectedAssetId = lowerText(asset.id || asset.asset_id);
  const sidecarAssetId = lowerText(sidecar.asset_id);
  const contentType = lowerText(source.content_type);
  const platformMatch = stableJson(
    normalisedRightsPlatformSet(decision.allowed_platforms),
  ) === stableJson(normalisedRightsPlatformSet(asset.allowed_platforms));
  const risk = numberOrNull(asset.risk_score);
  const sidecarRisk = numberOrNull(decision.risk_score);
  const sourceMasterPath = resolveMaybeRootedPath(
    path.resolve(root),
    source.local_path || source.source_evidence_path,
  );
  const sourceRelative = path.relative(path.resolve(root), sourceMasterPath);
  const sourceMasterSha256 = lowerText(source.sha256);
  const sourceMasterSizeBytes = numberOrNull(source.size_bytes);
  const sourceFileVerified = Boolean(
    sourceMasterPath &&
      sourceRelative !== ".." &&
      !sourceRelative.startsWith(`..${path.sep}`) &&
      !path.isAbsolute(sourceRelative) &&
      /^(?:output|test[\\/]output)[\\/]/i.test(sourceRelative) &&
      /\.(?:jpe?g|png|webp)$/i.test(sourceMasterPath) &&
      localFileMatchesEvidence(
        sourceMasterPath,
        sourceMasterSha256,
        sourceMasterSizeBytes,
      )
  );
  const verified = Boolean(
    expectedAssetId &&
      sidecarAssetId === expectedAssetId &&
      cleanText(source.source_url) === cleanText(sourceUrl) &&
      /^image\/(?:jpeg|png|webp)$/.test(contentType) &&
      cleanText(decision.licence_basis) === cleanText(
        asset.licence_basis || asset.license_basis || asset.rights_basis,
      ) &&
      lowerText(decision.allowed_use) === lowerText(asset.allowed_use) &&
      platformMatch &&
      typeof decision.commercial_use_allowed === "boolean" &&
      typeof asset.commercial_use_allowed === "boolean" &&
      decision.commercial_use_allowed === asset.commercial_use_allowed &&
      decision.local_materialization_allowed === true &&
      asset.local_materialization_allowed === true &&
      decision.live_publish_allowed === false &&
      asset.live_publish_allowed === false &&
      decision.requires_human_legal_review_before_publish === true &&
      asset.requires_human_legal_review_before_publish === true &&
      lowerText(decision.approval_status) === lowerText(asset.approval_status) &&
      lowerText(decision.rights_status) === lowerText(asset.rights_status) &&
      risk != null &&
      sidecarRisk != null &&
      Math.abs(risk - sidecarRisk) < 0.0001 &&
      Boolean(cleanText(sidecar.policy?.required_rules_link || sidecar.policy?.policy_evidence_path)) &&
      sidecar.safety?.no_publish_triggered === true &&
      sourceFileVerified
  );
  return verified
    ? {
        ...
        source_master_size_bytes: sourceMasterSizeBytes,
      }
    : { verified: false };
}

function verifiedHashBoundStillSourceIdentity(asset = {}, { root = process.cwd() } = {}) {
  const resolvedRoot = path.resolve(root);
  const evidencePath = resolveMaybeRootedPath(
    resolvedRoot,
    asset.evidence_file || asset.rights_evidence_file,
  );
  const evidenceRelative = path.relative(resolvedRoot, evidencePath);
  if (
    !evidencePath ||
    evidenceRelative === ".." ||
    evidenceRelative.startsWith(`..${path.sep}`) ||
  ...
  return hashBoundRightsSidecarStillSourceIdentity({
    asset,
    evidencePath,
    sourceUrl: cleanText(asset.source_url || asset.canonical_source_url),
    root: resolvedRoot,
  });
}

function governedExistingStillMotionRows(rightsLedger = {}, { root = process.cwd() } = {}) {
  const resolvedRoot = path.resolve(root);
  const sourceRows = [
    ...asArray(rightsLedger.assets),
    ...asArray(rightsLedger.records),
  ...
  const accepted = [];
  for (const asset of sourceRows) {
    const mediaKind = lowerText(asset.media_kind || asset.mediaKind);
    const assetType = lowerText([asset.asset_type, asset.type, asset.kind].join(" "));
    const sourceType = lowerText([
      asset.source_type,
      asset.source_kind,
      asset.asset_class,
    ...
    const canonicalHashBoundStillMotionRecord = Boolean(
      /(?:screenshot_derived_motion_clip|still_editorial_motion|game_content_still)/.test(
        assetType,
      ) &&
        /\.(?:mp4|mov|mkv|webm)$/i.test(
          cleanText(asset.local_materialized_path || asset.path || asset.file_path),
        ),
    );
    if (
      !(
        /(?:visual_still|still_editorial_motion|game_content_still)/.test(mediaKind) ||
        canonicalHashBoundStillMotionRecord
      ) ||
      !/(?:official|press_kit)/.test(sourceType) ||
      !/(?:still|image)/.test(sourceType)
    ) {
      continue;
    }
    if (
      !(
        (
          asset.materialized === true &&
          asset.counts_towards_motion_readiness === true
        ) ||
        canonicalHashBoundStillMotionRecord
      ) ||
      ...
      asset.live_publish_allowed !== false ||
      asset.requires_human_legal_review_before_publish !== true ||
      lowerText(asset.approval_status) !== "approved_for_local_materialization_only" ||
      !/(?:conditional|pending_human)/.test(lowerText(asset.rights_status)) ||
      lowerText(asset.allowed_use) !== "transformative_editorial_short_form" ||
      !cleanText(asset.licence_basis || asset.license_basis || asset.rights_basis) ||
      !asArray(asset.allowed_platforms).map(normaliseRightsPlatform).includes("youtube_shorts")
    ) {
      continue;
    }
    const risk = numberOrNull(asset.risk_score);
    if (risk == null || risk >= 0.65) continue;
    const sourceUrl = cleanText(asset.source_url || asset.canonical_source_url);
    if (!isSafeOutboundUrl(sourceUrl)) continue;

    const mediaPath = resolveMaybeRootedPath(
      resolvedRoot,
      asset.local_materialized_path || asset.path || asset.file_path,
    );
    const mediaRelative = path.relative(resolvedRoot, mediaPath);
    if (
      !mediaPath ||
      mediaRelative === ".." ||
      mediaRelative.startsWith(`..${path.sep}`) ||
      path.isAbsolute(mediaRelative) ||
      !/^(?:output|test[\\/]output)[\\/]/i.test(mediaRelative) ||
      !/\.(?:mp4|mov|mkv|webm)$/i.test(mediaPath)
    ) {
      continue;
    }
    const mediaEvidence = asset.materialized_file_evidence || {};
    if (!localFileMatchesEvidence(
      mediaPath,
      mediaEvidence.sha256 || asset.asset_sha256 || asset.sha256,
      mediaEvidence.size_bytes || asset.asset_size_bytes || asset.size_bytes,
    )) {
      continue;
    }
    ...
    const sidecarSourceIdentity = hashBoundRightsSidecarStillSourceIdentity({
      asset,
      evidencePath,
      sourceUrl,
      root: resolvedRoot,
    });
    if (!isSafeStillImageUrl(sourceUrl) && sidecarSourceIdentity.verified !== true) {
      continue;
    }
    const existingSourceMasterSha256 = lowerText(asset.source_master_sha256);
    if (
      sidecarSourceIdentity.verified === true &&
      /^[a-f0-9]{64}$/.test(existingSourceMasterSha256) &&
      existingSourceMasterSha256 !== sidecarSourceIdentity.source_master_sha256
    ) {
      continue;
    }
    accepted.push({
      ...
      materialized: true,
      counts_towards_motion_readiness: true,
      canonical_source_url: cleanText(asset.canonical_source_url || sourceUrl),
      source_master_path:
        cleanText(asset.source_master_path) || sidecarSourceIdentity.source_master_path,
      source_master_sha256:
        existingSourceMasterSha256 || sidecarSourceIdentity.source_master_sha256,
      hash_bound_still_source_identity_verified:
        sidecarSourceIdentity.verified === true,
  ...
  return existingMotionClipRows({
    motion_inventory: { production_motion_clips: accepted },
  });
}
...
function rightsRecordForClip(clip = {}) {
  ...
  return {
    ...
    source_kind: cleanText(clip.source_kind) || undefined,
    source_url_kind: cleanText(clip.source_url_kind) || undefined,
    ...
    source_audio_allowed:
      typeof clip.source_audio_allowed === "boolean"
        ? clip.source_audio_allowed
        : undefined,
    automated_segment_motion_class:
      cleanText(clip.automated_segment_motion_class) || undefined,
    segment_motion_class: cleanText(clip.segment_motion_class) || undefined,
    editorial_semantic_role: cleanText(clip.editorial_semantic_role) || undefined,
    current_event_footage_claim_allowed:
      typeof clip.current_event_footage_claim_allowed === "boolean"
        ? clip.current_event_footage_claim_allowed
        : undefined,
    selection_reason: cleanText(clip.selection_reason) || undefined,
    semantic_review_basis: cleanText(clip.semantic_review_basis) || undefined,
    ...
    local_materialization_allowed:
      typeof clip.local_materialization_allowed === "boolean"
        ? clip.local_materialization_allowed
        : undefined,
    live_publish_allowed:
...
function materializedRightsPublishHolds(records = [], rightsLedger = {}) {
  const inherited = new Set(
    asArray(rightsLedger.publish_blockers).map(cleanText).filter(Boolean),
  );
  const rows = asArray(records);
  const active = new Set();
  if (
    inherited.has(LIVE_PUBLISH_NOT_ALLOWED_BLOCKER) ||
    rows.some((record) => record.live_publish_allowed === false)
  ) {
    active.add(LIVE_PUBLISH_NOT_ALLOWED_BLOCKER);
  }
  if (
    inherited.has(HUMAN_LEGAL_REVIEW_BLOCKER) ||
    rows.some((record) => record.requires_human_legal_review_before_publish === true)
  ) {
    active.add(HUMAN_LEGAL_REVIEW_BLOCKER);
  }
  return LIVE_PUBLISH_HOLD_CODES.filter((code) => active.has(code));
}

function localMaterializationHoldFailures(records = [], publishHolds = []) {
  if (!asArray(publishHolds).length) return [];
  const heldRecords = asArray(records).filter((record) => (
    record.live_publish_allowed === false ||
    record.requires_human_legal_review_before_publish === true
  ));
  if (
    heldRecords.length > 0 &&
    heldRecords.every((record) => record.local_materialization_allowed === true)
  ) {
    return [];
  }
  return [{
    reason: "rights_local_materialization_not_allowed",
    error: "rights_local_materialization_not_allowed:explicit_local_permission_required_for_held_assets",
  }];
}

function rightsEvidenceAssetId(record = {}) {
  return cleanText(
    record.asset_id || record.id || record.clip_id || record.rights_record_id,
  );
}

function recordTriggersMotionScopeFailure(record = {}, failureCode = "") {
  if (failureCode === "rights:licence_basis_missing") {
    return !cleanText(
      record.licence_basis || record.license_basis || record.rights_basis,
    );
  }
  if (failureCode === "rights:commercial_use_not_allowed") {
    return record.commercial_use_allowed === false;
  }
  return false;
}

function excludedMotionRightsScopeReconciliation({
  rightsLedger = {},
  excludedClipIds = [],
  generatedAt = null,
} = {}) {
  ...
  const failures = [...new Set(
    asArray(rightsLedger.failures).map(cleanText).filter(Boolean),
  )];
  const supportedFailures = new Set([
    "rights:licence_basis_missing",
    "rights:commercial_use_not_allowed",
  ]);
  const evidenceRows = rightsLedgerEvidenceRows(rightsLedger);
  const excludedRows = evidenceRows.filter((record) =>
    excluded.has(lowerText(rightsEvidenceAssetId(record))),
  );
  const observedExcludedIds = new Set(
    excludedRows.map(rightsEvidenceAssetId).map(lowerText).filter(Boolean),
  );
  const observedMissingExcludedAssetIds = [...excluded]
    .filter((id) => !observedExcludedIds.has(id))
    .sort();
  const priorScopeReconciliation = (
    rightsLedger.motion_scope_reconciliation &&
    typeof rightsLedger.motion_scope_reconciliation === "object" &&
    !Array.isArray(rightsLedger.motion_scope_reconciliation)
  )
    ? rightsLedger.motion_scope_reconciliation
    : {};
  const priorExcludedAssetIds = excludedMotionClipIdSet(
    priorScopeReconciliation.excluded_asset_ids,
  );
  const priorScopeReconciliationCanContinue = Boolean(
    priorScopeReconciliation.allowed === true &&
      cleanText(priorScopeReconciliation.status) ===
        "excluded_assets_removed_from_motion_scope" &&
      cleanText(priorScopeReconciliation.scope) ===
        "local_motion_materialization_only" &&
      priorScopeReconciliation.source_ledger_preserved === true &&
      priorExcludedAssetIds.size > 0 &&
      [...priorExcludedAssetIds].every((id) => excluded.has(id)) &&
      asArray(priorScopeReconciliation.unresolved_failure_codes).length === 0 &&
      asArray(priorScopeReconciliation.missing_excluded_asset_ids).length === 0 &&
      asArray(priorScopeReconciliation.independent_ledger_blockers).length === 0 &&
      asArray(priorScopeReconciliation.independent_restrictive_statuses).length === 0
  );
  const previouslyReconciledAbsentAssetIds = priorScopeReconciliationCanContinue
    ? observedMissingExcludedAssetIds
        .filter((id) => priorExcludedAssetIds.has(id))
        .sort()
    : [];
  const previouslyReconciledAbsentAssetIdSet = new Set(
    previouslyReconciledAbsentAssetIds,
  );
  const missingExcludedAssetIds = observedMissingExcludedAssetIds
    .filter((id) => !previouslyReconciledAbsentAssetIdSet.has(id));
  const resolvedFailureCodes = [];
  const unresolvedFailureCodes = [];
  for (const failureCode of failures) {
    if (!supportedFailures.has(failureCode)) {
      unresolvedFailureCodes.push(failureCode);
      continue;
    }
    const offenders = evidenceRows.filter((record) =>
      recordTriggersMotionScopeFailure(record, failureCode),
    );
    if (
      offenders.length === 0 ||
      offenders.some((record) =>
        !excluded.has(lowerText(rightsEvidenceAssetId(record))),
      )
    ) {
      unresolvedFailureCodes.push(failureCode);
    ...
    resolvedFailureCodes.push(failureCode);
  }
  const independentLedgerBlockers = asArray(rightsLedger.blockers)
    .map(cleanText)
    .filter((blocker) => (
      blocker &&
      !failures.includes(blocker) &&
      !(
        blocker === MOTION_SCOPE_LEDGER_REBUILD_BLOCKER &&
        priorScopeReconciliationCanContinue
      )
    ));
  const statuses = rightsStatusFields(rightsLedger);
  const independentRestrictiveStatuses = [
    statuses.approval_status,
    statuses.rights_status,
    statuses.usage_status,
  ].map(restrictiveRightsStatus).filter(Boolean);
  const ledgerStatus = lowerText(statuses.status);
  const ledgerVerdict = lowerText(statuses.verdict);
  const aggregateFailureStatusOnly = (
    ["", "blocked"].includes(ledgerStatus) &&
    ["", "fail", "failed", "red"].includes(ledgerVerdict)
  );
  const allowed = Boolean(
    excluded.size > 0 &&
      failures.length > 0 &&
      resolvedFailureCodes.length === failures.length &&
      unresolvedFailureCodes.length === 0 &&
      missingExcludedAssetIds.length === 0 &&
      independentLedgerBlockers.length === 0 &&
      independentRestrictiveStatuses.length === 0 &&
      aggregateFailureStatusOnly
  );
  return {
    schema_version: 1,
    status: allowed
      ? "excluded_assets_removed_from_motion_scope"
      : "motion_scope_exclusion_not_authorised",
    allowed,
    scope: "local_motion_materialization_only",
    source_ledger_preserved: true,
    source_ledger_verdict: cleanText(rightsLedger.verdict) || null,
    source_ledger_status: cleanText(rightsLedger.status) || null,
    prior_scope_reconciliation_extended:
      priorScopeReconciliationCanContinue,
    excluded_asset_ids: [...excluded].sort(),
    resolved_failure_codes: resolvedFailureCodes.sort(),
    unresolved_failure_codes: unresolvedFailureCodes.sort(),
    previously_reconciled_absent_asset_ids:
      previouslyReconciledAbsentAssetIds,
    missing_excluded_asset_ids: missingExcludedAssetIds,
    independent_ledger_blockers: independentLedgerBlockers,
    independent_restrictive_statuses: independentRestrictiveStatuses,
...
function reconcileMaterializedRightsRecords(clips = [], rightsLedger = {}) {
  ...
  for (const clip of asArray(clips)) {
    ...
    const generatedLocalProofHold = isExplicitNonCommercialLocalProofHold(generated);
    const generatedRestrictiveStatus = restrictiveRightsRecordStatus(generated);
    if (
      generatedRestrictiveStatus &&
      !(generatedLocalProofHold && generatedRestrictiveStatus === "red")
    ) {
      failures.push({
        id: cleanText(clip.id),
        source_url: cleanText(clip.source_url),
    ...
    const restrictiveEvidenceRows = governingEvidence.filter((record) =>
      restrictiveRightsRecordStatus(record),
    );
    const governingLocalProofHold = generatedLocalProofHold &&
      restrictiveEvidenceRows.every(isExplicitNonCommercialLocalProofHold);
    if (
      restrictiveStatus &&
      !(governingLocalProofHold && restrictiveStatus === "red")
    ) {
      failures.push({
        id: cleanText(clip.id),
        source_url: cleanText(clip.source_url),
    ...
    if (
      !governingLocalProofHold &&
      governingEvidence.some((record) => record.commercial_use_allowed === false)
    ) {
      failures.push({
        id: cleanText(clip.id),
        source_url: cleanText(clip.source_url),
    ...
    if (
      !governingLocalProofHold &&
      governingEvidence.some((record) =>
        Array.isArray(record.allowed_platforms) && record.allowed_platforms.length === 0,
      )
    ) {
      failures.push({
        id: cleanText(clip.id),
    ...
    const reconciledLocalProofHold = isExplicitNonCommercialLocalProofHold(reconciled);
    if (
      reconciled.commercial_use_allowed !== true &&
      !reconciledLocalProofHold
    ) {
    ...
    if (
      (!Array.isArray(reconciled.allowed_platforms) || reconciled.allowed_platforms.length === 0) &&
      !reconciledLocalProofHold
    ) {
...
async function materializeCandidate({
  ...
  if (
    restrictiveCandidateRightsStatus &&
    !(
      restrictiveCandidateRightsStatus === "red" &&
      isExplicitNonCommercialLocalProofHold(candidate)
    )
  ) {
    return {
      status: "failed",
...
function buildStillMotionFfmpegArgs({
  input,
  output,
  durationS,
  focalPointX,
  ...
  const zoomFrameDenominator = Math.max(1, frames - 1);
  const focalX = normaliseFocalPoint(focalPointX);
  const focalY = normaliseFocalPoint(focalPointY);
  const crop = focalX === undefined && focalY === undefined
    ? "crop=1080:1920"
    : `crop=1080:1920:x='${
      focalX === undefined
        ? "(iw-1080)/2"
        : `max(0,min(iw-1080,iw*${focalX.toFixed(6)}-540))`
    }':y='${
      focalY === undefined
        ? "(ih-1920)/2"
        : `max(0,min(ih-1920,ih*${focalY.toFixed(6)}-960))`
    }'`;
  const zoompan =
    `scale=1240:2200:force_original_aspect_ratio=increase,` +
    `${crop},setsar=1,` +
    `zoompan=z='min(1.32,1.08+0.24*on/${zoomFrameDenominator})':` +
    `x='(iw-iw/zoom)*(0.08+0.84*on/${zoomFrameDenominator})':` +
    `y='(ih-ih/zoom)*(0.78-0.56*on/${zoomFrameDenominator})':` +
...
async function updateArtifactMotionEvidence({
  ...
  const rightsPublishHolds = materializedRightsPublishHolds(records, rightsLedger);
  const rightsScopeTransition = excludedMotionRightsScopeReconciliation({
    rightsLedger,
    excludedClipIds,
    generatedAt,
  });
  const motionScopeExclusionAllowed = rightsScopeTransition.allowed === true;
  ...
  const unresolvedRightsFailures = (
    motionScopeExclusionAllowed ? [] : remainingRightsFailures
  ).map((failure) => ({
  ...
  const rightsFailures = [
    ...
    ...localMaterializationHoldFailures(records, rightsPublishHolds),
  ];
  ...
  const publishBlockers = [...new Set([
    ...asArray(rightsLedger.publish_blockers)
      .map(cleanText)
      .filter((blocker) => blocker && blocker !== SCRIPT_REPAIR_RIGHTS_TRANSITION_BLOCKER),
    ...rightsPublishHolds,
    ...(motionScopeExclusionAllowed ? [MOTION_SCOPE_LEDGER_REBUILD_BLOCKER] : []),
    ...(scriptRepairRenderHold ? [SCRIPT_REPAIR_RIGHTS_TRANSITION_BLOCKER] : []),
  ])];
  const hasPublishHold = publishBlockers.length > 0;
  const updatedFootage = {
    ...footageInventory,
    status: ready ? "ready" : "blocked",
    ready,
    motion_ready: ready,
    not_publishable: hasPublishHold || !ready,
    counts_towards_final_render_readiness: ready,
    readiness: {
      ...reconciledReadiness,
      can_publish: hasPublishHold
        ? false
    ...
    motion_scope_reconciliation: motionScopeExclusionAllowed
      ? rightsScopeTransition
      : footageInventory.motion_scope_reconciliation,
  };
  const {
  ...
  const updatedRights = {
    ...canonicalLedgerBase,
    story_id: storyId,
    verdict: motionScopeExclusionAllowed
      ? cleanText(rightsLedger.verdict) || "fail"
      : scriptRepairRenderHold
        ? "RED"
        : hasPublishHold
          ? "warn"
          : "pass",
    status: motionScopeExclusionAllowed
      ? cleanText(rightsLedger.status) || "blocked"
      : scriptRepairRenderHold
        ? "blocked"
        : hasPublishHold
          ? "local_materialization_only"
          : "ready",
    blockers: motionScopeExclusionAllowed
      ? [...new Set([
          ...asArray(rightsLedger.blockers).map(cleanText).filter(Boolean),
          MOTION_SCOPE_LEDGER_REBUILD_BLOCKER,
        ])]
      : scriptRepairRenderHold
        ? [SCRIPT_REPAIR_RIGHTS_TRANSITION_BLOCKER]
        : [],
    failures: motionScopeExclusionAllowed
      ? asArray(rightsLedger.failures).map(cleanText).filter(Boolean)
      : remainingRightsFailures,
    publish_blockers: publishBlockers,
    not_publishable: hasPublishHold,
    records,
    ...
    ...(motionScopeExclusionAllowed
      ? {
          motion_scope_reconciliation: rightsScopeTransition,
          motion_scope_reconciled_at: generatedAt,
        }
      : {
          rights_ledger_repaired_at: generatedAt,
          rights_ledger_repair_strategy:
            "materialised_validated_direct_media_to_explicit_rights_records",
        }),
    narration_transition_reconciled:
      repairableScriptTransition && !scriptRepairRenderHold
        ? SCRIPT_REPAIR_RIGHTS_TRANSITION_BLOCKER
        : null,
    can_auto_publish: hasPublishHold
      ? false
      : canonicalLedgerBase.can_auto_publish,
    result: motionScopeExclusionAllowed
      ? cleanText(rightsLedger.result) || "FAIL"
      : scriptRepairRenderHold
        ? "BLOCKED"
        : hasPublishHold
          ? "AMBER"
  ...
  await fs.writeJson(materialisedPath, {
    ...
    distinct_genuine_base_source_count: baseSourceFamilies.length,
    professional_source_diversity: professionalSourceDiversity,
    rights_scope_reconciliation: motionScopeExclusionAllowed
      ? rightsScopeTransition
      : undefined,
    minimum_requirements: {
      min_clips: DEFAULT_MIN_CLIPS,
  ...
  return {
    status: "ready",
    publish_hold: publishBlockers[0] || null,
    publish_holds: publishBlockers,
    ...
    direct_video_motion_family_count: directVideoMotionFamilyCountValue,
    professional_source_diversity: professionalSourceDiversity,
    rights_scope_transition: motionScopeExclusionAllowed
      ? rightsScopeTransition
      : null,
    motion_pack_path: motionPackPath,
  };
...
async function materializeRealMotionJob(job = {}, options = {}) {
    ...
    if (options.refreshExistingMotion === true) {
      ...
      if (refreshedClips.length >= minClips) {
        ...
        return {
          ...
          publish_holds: updateSummary.publish_holds,
          rights_transition_status: updateSummary.rights_transition_status,
          rights_scope_transition: updateSummary.rights_scope_transition,
          candidate_count: 0,
          materialized_count: refreshedClips.length,
  ...
  const recoveredStillMotionRows = withoutExcludedMotionClips(
    governedExistingStillMotionRows(rightsLedger, { root: options.root }),
    options.excludedClipIds,
  );
  let governedExistingRows = mergeMotionRows(
    mergeMotionRows(
      governedExistingDirectMotionRows(footageInventory),
      recoveredStillMotionRows,
    ),
  ...
  const incrementalMergedFamilies = motionReadinessFamilies(incrementalMergedRows);
  const incrementalMergedBaseSourceFamilies = genuineBaseSourceFamilies(incrementalMergedRows);
  ...
  const incrementalMotionCompletion =
    ...
    incrementalMergedFamilies.length >= minFamilies &&
    (
      requiredBaseSourceCount === 0 ||
      incrementalMergedBaseSourceFamilies.length >= requiredBaseSourceCount
    ) &&
  ...
  if (
    ...
    return {
      ...
      recovered_still_motion_clip_count: recoveredStillMotionRows.length,
      incremental_merged_clip_count: incrementalMergedRows.length,
      incremental_merged_family_count: incrementalMergedFamilies.length,
      incremental_merged_base_source_count: incrementalMergedBaseSourceFamilies.length,
