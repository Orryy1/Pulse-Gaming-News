...
async function captureMaterializedClipEvidence(filePath, {
  ...
  if (expectedEvidence && typeof expectedEvidence === "object" && Object.keys(expectedEvidence).length) {
    const expected = {
    ...
    const mismatches = [];
    if (expected.sha256 !== evidence.sha256) mismatches.push("sha256");
    if (expected.size_bytes !== evidence.size_bytes) mismatches.push("size_bytes");
    if (Math.abs(expected.duration_seconds - evidence.duration_seconds) > 0.05) {
      mismatches.push("duration_seconds");
    }
    if (expected.video_codec !== evidence.video_codec) mismatches.push("video_codec");
    if (expected.width !== evidence.width) mismatches.push("width");
    if (expected.height !== evidence.height) mismatches.push("height");
    if (mismatches.length) {
...
function explicitCanonicalSourceUrl(value = {}) {
  ...
  if (canonicaliseMotionSourceUrl(explicit)) return explicit;
...
async function readPulseSourceIdentitySidecar(masterPath = "", currentMasterSha256 = "") {
  ...
  return {
    ...
    fields: {
      canonical_source_url: canonicalSourceUrl,
      youtube_video_id: youtubeVideoId,
      source_master_sha256: sidecarMasterSha256,
    },
    conflicts: [],
    provenance,
  };
...
function buildProfessionalSourceDiversity(rows = [], requiredBaseSourceCount = 2) {
  const evaluatedRows = asArray(rows).map((row, index) => ({
    row,
...
function mergePreservedMotionEvidence(existing = {}, incoming = {}) {
  ...
  if (existingAllowedPlatforms && incomingAllowedPlatforms) {
    const incomingSet = new Set(incomingAllowedPlatforms.map(lowerText));
    merged.allowed_platforms = existingAllowedPlatforms.filter((platform) =>
      incomingSet.has(lowerText(platform)),
    );
...
function selectorClipRecoveryEvidenceComplete(clip = {}) {
  ...
  const rightsComplete = (
    ...
    rightsPresence.evidence_reference &&
...
async function governedSelectorRecoveryRows(artifactDir = "") {
  ...
  if (
    ...
    !clips.every(selectorClipRecoveryEvidenceComplete)
  ) {
    return [];
  }
...
function rightsRecordForClip(clip = {}) {
  ...
  return {
    ...
    allowed_platforms: Array.isArray(clip.allowed_platforms)
    ...
    restricted_platforms: Array.isArray(clip.restricted_platforms)
      ...
      : undefined,
    commercial_use_allowed:
      typeof clip.commercial_use_allowed === "boolean" ? clip.commercial_use_allowed : true,
    ...
    approval_status:
      rightsStatuses.approval_status || "approved_for_transformative_editorial_use",
...
function rightsSourceWindow(record = {}) {
  const sourceReference = cleanText(
    record.source_url || record.url || (/^https?:/i.test(cleanText(record.path)) ? record.path : ""),
  );
...
function explicitRightsComparableValue(record = {}, field) {
  ...
  if (field === "licence_basis") {
    ...
    value = record.evidence_reference;
  } else {
    value = record[field];
  }
  if (Array.isArray(value)) return stableJson(normalisedStringSet(value));
  if (value && typeof value === "object") return stableJson(value);
  if (typeof value === "string") return lowerText(value);
  return value;
}

function aggregateRightsEvidenceContradictions(records = []) {
  const conflicts = [];
  for (const field of [
    "licence_basis",
    "allowed_use",
    ...
    const values = asArray(records)
      .map((record) => explicitRightsComparableValue(record, field))
...
function rightsContradictions(clip = {}, existingRecord = {}) {
  ...
  for (const field of ["licence_basis", "allowed_use", "evidence_reference"]) {
    ...
    if (presence[field] && clipValue && existingValue && lowerText(clipValue) !== lowerText(existingValue)) {
      conflicts.push(field);
    }
  }
  ...
  for (const field of ["allowed_platforms", "restricted_platforms"]) {
    if (
      ...
      stableJson(normalisedStringSet(clip[field])) !== stableJson(normalisedStringSet(existingRecord[field]))
    ) {
      conflicts.push(field);
    }
  }
  ...
  if (
    ...
    stableJson(clip.validation_provenance) !== stableJson(existingRecord.validation_provenance)
  ) {
...
function reconcileMaterializedRightsRecords(clips = [], rightsLedger = {}) {
  ...
  for (const clip of asArray(clips)) {
    ...
    const collidingRecord = byIdentity.get(collidingIdentity) ||
      existing.find((record) => rightsRecordIdentity(record) === collidingIdentity);
    if (
      collidingRecord &&
    ...
    const existingRecord = byIdentity.get(identity) ||
      ...
      matchingEvidence[0];
    const restrictiveStatus = matchingEvidence
    ...
    const matchingHighRisk = matchingEvidence
    ...
    if (matchingEvidence.some((record) => record.commercial_use_allowed === false)) {
      failures.push({
        id: cleanText(clip.id),
        source_url: cleanText(clip.source_url),
    ...
    if (matchingEvidence.some((record) => Array.isArray(record.allowed_platforms) && record.allowed_platforms.length === 0)) {
      failures.push({
        id: cleanText(clip.id),
        source_url: cleanText(clip.source_url),
    ...
    const contradictionEvidence = existingRecord && !matchingEvidence.includes(existingRecord)
    ...
    const contradictions = [...new Set([
      ...
      ...aggregateRightsEvidenceContradictions(contradictionEvidence),
    ])];
    ...
    const reconciled = existingRecord
      ? {
          ...generated,
          ...existingRecord,
      ...
      : generated;
    if (reconciled.commercial_use_allowed === false) {
      failures.push({
        id: cleanText(clip.id),
        source_url: cleanText(clip.source_url),
        reason: "rights_evidence_restricts_commercial_use",
    ...
    if (existingIdentity && existingIdentity !== identity) byIdentity.delete(existingIdentity);
...
async function balancedRestorableArtifactClips(footageInventory = {}, {
  ...
  const excluded = new Set(normaliseStoryIds(excludedClipIds).map(lowerText));
  const candidates = allCandidates.filter(
    (candidate) => !excluded.has(lowerText(candidate.id || candidate.clip_id)),
  );
...
function exactRefreshWindowCandidate(candidates = [], selected = [], entry = {}, index = 0) {
    ...
    try {
      const materializedFileEvidence = await captureMaterializedClipEvidence(
        clip.local_materialized_path || clip.path,
        {
          ...
          materializedClipProbe,
          materializedClipDecode,
          expectedEvidence: existingMaterializedFileEvidence(clip),
          expectedMedia: {
            duration_seconds: sourceWindowDurationS,
            video_codec: "h264",
            width: 1080,
...
async function updateArtifactMotionEvidence({
  ...
  const mergedCandidateRows = preserveExistingMotion ? mergeMotionRows(preservedRows, clipRows) : clipRows;
...
async function materializeRealMotionJob(job = {}, options = {}) {
  ...
  const requiredBaseSourceCount = requiredGenuineBaseSourceCount(job, options);
  const blockers = [];
  ...
  const recoveredSelectorRows = await governedSelectorRecoveryRows(artifactDir);
  const governedExistingRows = mergeMotionRows(
    governedExistingDirectMotionRows(footageInventory),
  ...
  if (
    ...
    const partialEvidence = materialized.length
      ...
      : null;
    const invalidatedEvidence = materialized.length > 0 ||
      staleReadyArtifactDetected ||
      staleReadyCentralPackDetected ||
      staleReadyOwnedMotionDetected ||
      staleReadyFootageDetected
  ...
  const updateSummary = await updateArtifactMotionEvidence({
    ...
    recoveredExistingRows: recoveredSelectorRows,
    materializedClipProbe: options.materializedClipProbe,
    materializedClipDecode: options.materializedClipDecode,
    clipVisualFingerprint,
    requiredBaseSourceCount,
