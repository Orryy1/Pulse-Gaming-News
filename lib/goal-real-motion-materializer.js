...
function segmentValidationRows(segmentValidationReport = {}, storyId = "") {
  ...
  return asArray(segmentValidationReport.segments)
    ...
    .map((segment, index) => {
      const baseSourceFamily = cleanText(segment.source_family || `segment_source_family_${index + 1}`);
...
function directVideoMotionFamilyCount(rows = []) {
  return new Set(
    directVideoMotionRows(rows)
      .map((clip) => cleanText(clip.source_family || clip.motion_family || clip.source_url))
      .filter(Boolean),
  ).size;
}
...
function uniqueFamiliesFromRows(rows = []) {
  return [...new Set(asArray(rows).map((clip) => cleanText(clip.source_family)).filter(Boolean))];
}
...
async function writePartialMotionEvidence({
  ...
  const families = [...new Set(clipRows.map((clip) => cleanText(clip.source_family)).filter(Boolean))];
