...
function sampledVisualFingerprintValue(value = {}) {
  ...
  if (typeof fingerprint === "string") return cleanText(fingerprint);
  return cleanText(fingerprint?.signature);
}
...
function buildProfessionalSourceDiversity(rows = [], requiredBaseSourceCount = 2) {
  const identityRows = asArray(rows).map((row, index) => {
    const identity = row.motion_source_identity || compoundMotionSourceIdentity(row, index);
  ...
  return assessProfessionalSourceDiversity({
    clips: identityRows,
    scenes: identityRows,
