...
function genuineBaseSourceFamilies(rows = []) {
  return [
    ...new Set(
      asArray(rows)
        .filter((clip) => clipLooksDirectVideo(clip))
...
function onlyRepairableMissingRightsRecordFailure(rightsLedger = {}) {
  ...
  const nonVerdictRestrictiveStatuses = [
    rightsLedger.approval_status,
    rightsLedger.rights_status,
    rightsLedger.usage_status,
    rightsLedger.status,
  ].map(restrictiveRightsStatus).filter(Boolean);
  const verdictStatus = restrictiveRightsStatus(rightsLedger.verdict);
...
function rightsSourceWindow(record = {}) {
  const sourceUrl = lowerText(record.source_url || record.url || (/^https?:/i.test(cleanText(record.path)) ? record.path : ""));
...
function reconcileMaterializedRightsRecords(clips = [], rightsLedger = {}) {
  ...
  const byIdentity = new Map(existing.map((record) => [rightsRecordIdentity(record), record]));
  const failures = [];
  for (const clip of asArray(clips)) {
...
async function updateArtifactMotionEvidence({
  ...
  const updatedRights = {
    ...
    records,
    rights_ledger: records,
    matched_assets: mergeRecords(records.map((record) => ({
      ...
      platform_restrictions: record.platform_restrictions,
