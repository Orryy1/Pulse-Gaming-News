...
function rightsEvidenceMatchesClip(record = {}, generated = {}) {
  ...
  if (!recordWindow.source_url || recordWindow.source_url !== generatedWindow.source_url) return false;
  if (!recordWindow.complete || !generatedWindow.complete) return true;
  return Math.abs(recordWindow.start - generatedWindow.start) < 0.01 &&
...
function reconcileMaterializedRightsRecords(clips = [], rightsLedger = {}) {
  ...
  for (const clip of asArray(clips)) {
    ...
    const contradictions = [...new Set([
      ...matchingEvidence.flatMap((record) => rightsContradictions(clip, record)),
      ...aggregateRightsEvidenceContradictions(matchingEvidence),
    ])];
