...
function fileExistsForEvidence(values = [], artifactDir = "") {
  return Boolean(firstExistingEvidencePath(values, artifactDir));
}
...
function materialisedMotionEvidenceForPackage(item = {}) {
  ...
  const clips = materialisedMotionClipRows(manifest);
  ...
  const familyValues = localClips
    ...
    .filter(Boolean);
  const clipCount = positiveMax(manifest.clip_count, manifest.materialised_motion_clip_count, localClips.length);
  const distinctFamilyCount = positiveMax(
    manifest.distinct_motion_family_count,
    manifest.materialised_motion_family_count,
    manifest.distinct_motion_families,
...
function audioTimestampEvidenceForPackage(item = {}) {
  ...
  const narrationAudioPath = firstExistingEvidencePath([
    manifest.resolved_narration_audio_path,
    manifest.narration_audio_path,
    manifest.audio_path,
  ], artifactDir);
  const wordTimestampsPath = firstExistingEvidencePath([
...
function jobForQueueItem(item = {}, context = {}) {
  ...
  const readyForFinalRender =
    renderInputStatus === "ready_for_final_render_job" && !hasNormalProductionDurationBlocker;
...
function queueItemFromCutoverBlockedItem(item = {}) {
  ...
  if (cutoverBlockedItemNeedsForcedRerender({ item, rawBlockers, blockers })) {
    return {
