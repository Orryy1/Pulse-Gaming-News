...
function repairedPackageInputsFreshness(item = {}) {
  ...
  const renderGeneratedMs = timeMs(renderManifest.generated_at);
  if (!renderGeneratedMs) {
    return {
      newer_than_render: false,
      reason: "render_manifest_generated_at_missing",
    };
  }
  ...
  const audioMs = latestManifestTimeMs(audioManifest, ["materialized_at", "generated_at", "audio_materialized_at"]);
  const sfxMs = latestManifestTimeMs(sfxManifest, ["generated_at", "materialized_at"]);
  const captionMs = latestManifestTimeMs(captionManifest, ["generated_at", "materialized_at"]);
  const canonicalMs = latestManifestTimeMs(canonicalManifest, [
    "public_copy_regeneration_completed_at",
    "public_copy_final_render_regenerated_at",
    "audio_materialized_at",
    "generated_at",
  ]);
  const latestInputMs = Math.max(audioMs, sfxMs, captionMs, canonicalMs);
...
function forcedRerenderTargetManifest(item = {}) {
  ...
  return {
    ...
    renderer: cleanText(item.target_render_manifest?.renderer || renderManifest.renderer || "visual_v4_production"),
    visual_tier: cleanText(item.target_render_manifest?.visual_tier || renderManifest.visual_tier || "production_v4_motion"),
    final_publish_render: true,
