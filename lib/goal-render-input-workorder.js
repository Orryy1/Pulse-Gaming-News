...
function classifyMotionRepairLane(item = {}, context = {}) {
  ...
  const directVideoMotionClipFloor =
    ...
    5;
  const directVideoMotionAssetCount =
    Number(motionReadiness.direct_video_motion_asset_count) ||
    Number(renderEvidence.visual_evidence_profile?.direct_video_motion_asset_count) ||
    0;
  ...
  if (
    ...
    return {
      ...
      recommended_command:
        `Find one more rights-backed official direct-video source for ${storyId}, then rerun source intake and real-motion materialisation.`,
      post_repair_validation_command:
      ...
      evidence: {
        ...
        direct_video_motion_family_count: Number(motionReadiness.direct_video_motion_family_count || 0),
        materialised_real_motion_clip_floor_met:
          motionReadiness.materialised_real_motion_clip_floor_met === true,
        real_visual_motion_clip_count: Number(renderEvidence.real_visual_motion_clip_count || 0),
      },
      output_expectations: [
...
function queueItemFromDryRunBlockedStory(item = {}) {
  ...
  const blockers = [...new Set(rawBlockers.map(mapDryRunPreflightBlocker).filter(Boolean))];
...
function buildGoalRenderInputWorkOrder({
  ...
  const cutoverBlockedQueue = asArray(cutoverPlan.blocked)
    ...
    .filter(Boolean);
  const dryRunBlockedQueue = asArray(dryRunPlan?.blocked_stories)
  ...
  const repairBacklog = buildRepairBacklog(jobs, generatedAt);
  ...
  return {
    ...
    summary: {
      ...
      auto_repairable_jobs: autoRepairableJobs.length,
      operator_required_jobs: operatorRequiredJobs.length,
      dead_end_blocker_jobs: deadEndBlockerJobs.length,
    },
