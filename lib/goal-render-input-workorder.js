...
function commandPath(value = "") {
  const text = cleanText(value);
  if (!text) return "";
  return /[\s"]/i.test(text) ? commandArg(text) : text;
}

function repairCommandContext(item = {}) {
  return {
    ...(item.repair_command_context || {}),
    ...(item.render_input_evidence?.repair_command_context || {}),
  };
}

function repairContextPath(item = {}, field = "", fallback = "") {
  return cleanText(repairCommandContext(item)[field]) || fallback;
}

function repairStoryPackagesPath(item = {}) {
  return repairContextPath(item, "story_packages_path", "output/goal-contract/production_cutover_story_packages.json");
}

function repairWorkOrderPath(item = {}) {
  return repairContextPath(item, "render_input_work_order_path", "output/goal-contract/render_input_work_order.json");
}

function repairOutputDir(item = {}) {
  return repairContextPath(item, "out_dir", "output/goal-contract");
}

function repairCutoverPlanPath(item = {}) {
  return repairContextPath(item, "production_cutover_plan_path", "output/goal-contract/production_render_cutover_plan.json");
}

function repairMotionOutDir(item = {}) {
  return repairContextPath(item, "real_motion_out_dir", "output/studio-v4/motion-packs");
}

function repairArtifactRoot(item = {}) {
  return repairContextPath(item, "real_motion_artifact_root", "");
}

function repairSegmentReportPath(item = {}) {
  return repairContextPath(item, "segment_report_path", "");
}

function hasLocalPromotionMotionValidationInput(item = {}) {
  return Boolean(repairSegmentReportPath(item));
}

function finalRenderPrerequisiteBlockers(item = {}) {
  const blockers = asArray(item.render_input_blockers).map(cleanText).filter(Boolean);
  ...
  return blockers.filter((blocker) => prerequisiteBlockers.has(blocker));
}

function renderInputValidationCommand(item = {}) {
  return `npm run ops:goal-render-inputs -- --cutover-plan ${commandPath(repairCutoverPlanPath(item))} --out-dir ${commandPath(repairOutputDir(item))} --json`;
}

function dryRunValidationCommand(item = {}) {
  return `npm run ops:goal-dry-run-publish -- --out-dir ${commandPath(repairOutputDir(item))} --json`;
}

function productionRenderCommand(item = {}) {
  const ownedMotionOnly =
    item.rights_safe_owned_motion_only === true ||
    item.render_input_evidence?.rights_safe_owned_motion_only === true ||
    item.evidence?.rights_safe_owned_motion_only === true;
  return `npm run ops:goal-production-render -- --work-order ${commandPath(repairWorkOrderPath(item))} --out-dir ${commandPath(repairOutputDir(item))}${ownedMotionOnly ? " --owned-motion-only" : ""} --json`;
}

function audioTimestampCommand(item = {}, storyId = cleanText(item.story_id)) {
  return `npm run ops:goal-audio-timestamps -- --story-id ${storyId} --work-order ${commandPath(repairWorkOrderPath(item))} --out-dir ${commandPath(repairOutputDir(item))} --json`;
}

function publicCopyRepairCommand(item = {}, storyId = cleanText(item.story_id), extra = "") {
  return `npm run ops:goal-public-copy-repair -- --story-packages ${commandPath(repairStoryPackagesPath(item))} --story-id ${storyId}${extra} --out-dir ${commandPath(repairOutputDir(item))} --json`;
}

function productionCutoverCommand(item = {}) {
  return `npm run ops:goal-production-cutover -- --story-packages ${commandPath(repairStoryPackagesPath(item))} --out-dir ${commandPath(repairOutputDir(item))} --json`;
}

function realMotionMaterialisationCommand(item = {}, storyId = cleanText(item.story_id)) {
  const extras = [];
  const artifactRoot = repairArtifactRoot(item);
  const segmentReport = repairSegmentReportPath(item);
  if (artifactRoot) extras.push("--artifact-root", commandPath(artifactRoot));
  if (segmentReport) {
    extras.push("--segment-report", commandPath(segmentReport));
    extras.push("--min-clips", "8", "--min-families", "5", "--max-clips", "8");
  }
  return [
    "npm run ops:goal-real-motion --",
    "--story-id",
    storyId,
    "--work-order",
    commandPath(repairWorkOrderPath(item)),
    "--out-dir",
    commandPath(repairMotionOutDir(item)),
    ...extras,
    "--json",
  ].join(" ");
}
...
function sourceUrlFamilyKey(value) {
  ...
  try {
    ...
    if (host === "youtu.be") {
      const videoId = cleanText(url.pathname.split("/").filter(Boolean)[0]).toLowerCase();
...
function hyperframesReadableDwellBlockers(blockers = []) {
  return asArray(blockers).filter((blocker) =>
    blocker === "hyperframes_readable_dwell_repair_required" ||
    blocker === "hyperframes_card_family_repair_required",
  );
}

function visualMotionRepeatBlockers(blockers = []) {
  return asArray(blockers).filter((blocker) =>
    blocker === "visual_motion_repeat_repair_required",
  );
}

function finalRenderInputFingerprintStale(evidence = {}) {
  return evidence.audio_fingerprint_matches_render === false ||
    evidence.word_timestamps_fingerprint_matches_render === false ||
    evidence.audio_size_matches_render === false ||
    evidence.word_timestamps_size_matches_render === false;
}

function readStoryJsonForQueueItem(item = {}, filename = "") {
  if (!filename) return null;
  const filePath = storyArtifactPath(item, filename);
  try {
...
function readJsonPath(filePath = "") {
  const resolved = cleanText(filePath);
...
function scriptScorecardForManifest(manifest = {}) {
  const storyId = cleanText(manifest.story_id || manifest.id);
  const title = cleanText(manifest.selected_title || manifest.short_title || manifest.canonical_title || manifest.title);
  ...
  const script = cleanText(manifest.narration_script || manifest.full_script || manifest.tts_script);
...
function scriptScorecardNeedsRepair(scorecard = null) {
  ...
  const verdict = cleanText(scorecard.verdict).toLowerCase();
  ...
  const warnings = asArray(scorecard.warnings).map(cleanText);
  return (
...
function scriptScorecardFailureCodes(scorecard = {}) {
  ...
  const curiosityGap = Number(scorecard.scores?.curiosity_gap ?? scorecard.curiosity_gap);
  const failures = [];
  for (const blocker of asArray(scorecard.blockers).map(cleanText).filter(Boolean)) {
    failures.push(`script_scorecard:${blocker}`);
  }
  const blockingWarningSet = new Set(["internal_qa_language", "weak_fallback_narration", "generic_public_copy"]);
  for (const warning of asArray(scorecard.warnings).map(cleanText).filter(Boolean)) {
    if (blockingWarningSet.has(warning)) failures.push(`script_scorecard:${warning}`);
  }
...
function scriptScorecardQaForQueueItem(item = {}, manifest = {}) {
  const storedScorecard = readStoryJsonForQueueItem(item, "script_scorecard.json");
  if (!storedScorecard || typeof storedScorecard !== "object") {
    return {
      verdict: "pass",
      failures: [],
      selected_scorecard_source: "missing_script_scorecard_not_blocking",
      stored_scorecard_verdict: null,
      stored_scorecard_score: null,
      current_scorecard_verdict: null,
      current_scorecard_score: null,
    };
  }
  const currentScorecard = scriptScorecardForManifest(manifest);
  const storedNeedsRepair = scriptScorecardNeedsRepair(storedScorecard);
  const currentNeedsRepair = scriptScorecardNeedsRepair(currentScorecard);
  const selectedScorecard =
    storedNeedsRepair ? storedScorecard :
    currentNeedsRepair ? currentScorecard :
    null;
  const failures = selectedScorecard ? scriptScorecardFailureCodes(selectedScorecard) : [];
  return {
    verdict: failures.length ? "fail" : "pass",
    failures,
    selected_scorecard_source:
      storedNeedsRepair ? "stored_script_scorecard" :
      currentNeedsRepair ? "current_manifest_script" :
      storedScorecard ? "stored_script_scorecard" : "current_manifest_script",
    stored_scorecard_verdict: cleanText(storedScorecard?.verdict) || null,
    stored_scorecard_score: Number.isFinite(Number(storedScorecard?.viral_score))
      ? Number(storedScorecard.viral_score)
      : null,
    current_scorecard_verdict: cleanText(currentScorecard?.verdict) || null,
    current_scorecard_score: Number.isFinite(Number(currentScorecard?.viral_score))
      ? Number(currentScorecard.viral_score)
      : null,
  };
}

function artefactModifiedTimeMs(item = {}, filename = "") {
  if (!filename) return null;
  try {
    const stats = fs.statSync(storyArtifactPath(item, filename));
...
function existingEvidenceModifiedTimeMs(values = [], artifactDir = "") {
  const existingPath = existingEvidencePath(values, artifactDir);
  if (!existingPath) return null;
  try {
    const stats = fs.statSync(existingPath);
    const ms = Number(stats.mtimeMs);
    return stats.isFile() && Number.isFinite(ms) && ms > 0 ? ms : null;
  } catch {
    return null;
  }
}

function evidencePathCandidates(value = "", artifactDir = "") {
  const text = cleanText(value);
  if (!text) return [];
  const candidates = [text];
  if (!path.isAbsolute(text) && cleanText(artifactDir)) {
    candidates.push(path.join(artifactDir, text));
  }
  if (!path.isAbsolute(text)) {
    const mediaCandidate = mediaPaths.resolveExistingSync(text, { fs });
    if (mediaCandidate) candidates.push(mediaCandidate);
  }
  return [...new Set(candidates)];
}

function existingEvidencePath(values = [], artifactDir = "") {
  for (const value of asArray(values)) {
    for (const candidate of evidencePathCandidates(value, artifactDir)) {
      try {
...
function firstExistingEvidencePath(values = [], artifactDir = "") {
  const existing = existingEvidencePath(values, artifactDir);
  if (existing) return existing;
  return cleanText(asArray(values)[0]);
}

function fileExistsForEvidence(values = [], artifactDir = "") {
  return Boolean(existingEvidencePath(values, artifactDir));
}

function nonEmptyEvidenceFields(evidence = {}) {
...
function currentReadableHyperframesEvidenceReady(fileEvidence = {}) {
  return (
    cleanText(fileEvidence.readable_hyperframes_evidence_source) === "current_owned_motion_manifest" &&
    fileEvidence.readable_hyperframes_ready === true &&
    Number(fileEvidence.readable_hyperframes_too_fast_count || 0) === 0 &&
    Number(fileEvidence.readable_hyperframes_clip_count || 0) >= 3
  );
}

function currentReadableHyperframesEvidenceOverrides(hyperframes = {}) {
  if (!hyperframes.ready) return {};
  return {
    hyperframes_effective_too_fast_card_shots: [],
    hyperframes_too_fast_card_shots: [],
...
function currentInputsReadyForFinalRender(evidence = {}) {
  const distinctMotionFamiliesReady =
    evidence.distinct_motion_families_ready === true ||
    Number(evidence.distinct_motion_family_count || 0) >= 3;
  const directMotionReady =
    evidence.owned_explainer_exception_approved === true ||
    evidence.owned_explainer_motion_exception_approved === true ||
    (
      Number(evidence.direct_video_motion_clip_count || 0) >= 5 &&
      Number(evidence.direct_video_motion_family_count || 0) >= 4
    );
  return (
    evidence.narration_ready === true &&
    evidence.word_timestamps_ready === true &&
    evidence.materialised_motion_ready === true &&
    distinctMotionFamiliesReady &&
    directMotionReady &&
    evidence.readable_hyperframes_ready !== false
  );
}

function hardFinalRenderBlockers(blockers = []) {
  const nonInputReadyRenderBlockers = new Set([
    "final_mp4_missing",
    "caption_file_missing",
    "render_manifest_missing",
    "visual_design_policy_rerender_required",
    "public_copy_newer_than_render",
  ]);
  return asArray(blockers)
    .map(cleanText)
    .filter((blocker) => blocker && !nonInputReadyRenderBlockers.has(blocker));
}

function stalePackageBlockerClearedByCurrentEvidence(blocker = "", evidence = {}) {
  const code = cleanText(blocker);
  const scriptQaPass = cleanText(evidence.script_scorecard_qa?.verdict).toLowerCase() === "pass" &&
    asArray(evidence.script_scorecard_qa?.failures).length === 0;
  if (["script:rewrite_required", "media_house:script_sounds_ai_generic"].includes(code)) {
    return scriptQaPass;
  }
  if (code === "director:director_blocked") {
    return currentInputsReadyForFinalRender(evidence);
  }
  return false;
}

function removeStalePackageBlockersClearedByCurrentEvidence(blockers = [], evidence = {}) {
  return asArray(blockers)
    .map(cleanText)
    .filter((blocker) => blocker && !stalePackageBlockerClearedByCurrentEvidence(blocker, evidence));
}

function manifestStatusReady(manifest = {}) {
  const status = cleanText(manifest.status || manifest.state || manifest.verdict || manifest.result).toLowerCase();
  if (!status) return true;
  return ["ready", "pass", "passed", "green", "ok", "complete", "materialized", "materialised"].includes(status);
}

function materialisedMotionClipRows(manifest = {}) {
...
function distinctMotionFamilyReportEvidenceForPackage(item = {}, manifest = {}) {
  const report = readStoryJsonForQueueItem(item, "distinct_motion_family_report.json") || {};
  if (!Object.keys(report).length) {
    return { ready: false, report: null };
  }
  const artifactDir = cleanText(item.artifact_dir);
  const storyId = cleanText(item.story_id || item.id);
  const reportStoryId = cleanText(report.story_id || report.storyId);
  const summary = report.summary || {};
  const familyRows = [
    ...asArray(report.distinct_motion_families),
    ...asArray(report.families),
  ].filter(Boolean);
  const manifestGeneratedTime = timeMs(manifest.generated_at || manifest.generatedAt);
  const manifestModifiedTime = artefactModifiedTimeMs(item, "materialised_motion_clips.json");
  const reportGeneratedTime = timeMs(report.generated_at || report.generatedAt);
  const reportModifiedTime = artefactModifiedTimeMs(item, "distinct_motion_family_report.json");
  const manifestTime = manifestGeneratedTime || manifestModifiedTime;
  const reportTime = manifestGeneratedTime
    ? (reportGeneratedTime || reportModifiedTime)
    : reportModifiedTime;
  const staleComparedWithManifest =
    Boolean(manifestTime && reportTime && reportTime < manifestTime);
  const storyMatches = !storyId || !reportStoryId || storyId === reportStoryId;
  const distinctMotionFamilyCount = positiveMax(
    summary.distinct_motion_family_count,
    report.distinct_motion_family_count,
    report.distinct_motion_families_count,
    familyRows.length,
  );
  const directVideoMotionFamilyCount = positiveMax(
    summary.direct_video_motion_family_count,
    report.direct_video_motion_family_count,
    report.direct_video_motion_families,
  );
  const clipCount = positiveMax(
    summary.clip_count,
    report.clip_count,
    report.materialised_motion_clip_count,
    report.materialized_motion_clip_count,
  );
  const ready =
    manifestStatusReady(report) &&
    storyMatches &&
    !staleComparedWithManifest &&
    distinctMotionFamilyCount > 0;
  return {
    ready,
    report,
    report_path: storyArtifactPath(item, "distinct_motion_family_report.json"),
    story_matches: storyMatches,
    stale_compared_with_manifest: staleComparedWithManifest,
    report_time_ms: reportTime || null,
    manifest_time_ms: manifestTime || null,
    distinct_motion_family_count: ready ? distinctMotionFamilyCount : 0,
    direct_video_motion_family_count: ready ? directVideoMotionFamilyCount : 0,
    clip_count: ready ? clipCount : 0,
    artifact_dir: artifactDir,
  };
}

function clipEvidencePath(clip = {}) {
...
function dedupeClipsByEvidencePath(clips = [], artifactDir = "") {
  const seen = new Set();
  const deduped = [];
  for (const clip of asArray(clips)) {
    const clipPath = firstExistingEvidencePath([clipEvidencePath(clip)], artifactDir);
    const key = cleanText(clipPath).replace(/\\/g, "/").toLowerCase();
...
function clipLooksOwnedGenerated(clip = {}) {
  const text = [
    clip.source_type,
    clip.source_kind,
    clip.media_kind,
  ...
  return (
    ...
    /owned|internally_generated|source_card|proof_card|stat_card|quote_card|title_card|lower_third|chart_slam|motion_background|branded_wipe|carousel|image_card|fast_card/.test(text)
  );
}

function directMotionClips(clips = []) {
  return asArray(clips).filter((clip) => !clipLooksOwnedGenerated(clip));
}
...
function directMotionReadinessFamily(clip = {}, index = 0) {
  ...
  if (cleanText(clip.media_kind || "direct_video") === "direct_video" && sourceKey) {
    return sourceKey;
  }
  ...
  return familyBaseKey(clip.source_family || clip.motion_family || clip.family) ||
    cleanText(`direct_motion_family_${index + 1}`);
}

function motionReadinessFamily(clip = {}, index = 0) {
  return directMotionReadinessFamily(clip, index) ||
    familyBaseKey(clip.source_family || clip.motion_family || clip.asset_id || clip.id) ||
    cleanText(`motion_family_${index + 1}`);
}

function materialisedMotionEvidenceForPackage(item = {}) {
  const artifactDir = cleanText(item.artifact_dir);
  const manifest = readStoryJsonForQueueItem(item, "materialised_motion_clips.json") || {};
  const partial = readStoryJsonForQueueItem(item, "partial_real_motion_evidence.json") || {};
  const partialBlocksReadiness =
    cleanText(partial.status).toLowerCase() === "blocked" ||
    partial.not_publishable === true ||
    partial.counts_towards_final_render_readiness === false;
  const manifestGeneratedTime = timeMs(manifest.generated_at);
  const manifestModifiedTime = artefactModifiedTimeMs(item, "materialised_motion_clips.json");
  const manifestModifiedLooksLikeRefresh =
    Boolean(
      manifestGeneratedTime &&
        manifestModifiedTime &&
        manifestModifiedTime >= manifestGeneratedTime &&
        manifestModifiedTime - manifestGeneratedTime <= 24 * 60 * 60 * 1000,
    );
  const manifestTime = manifestModifiedLooksLikeRefresh
    ? positiveMax(manifestGeneratedTime, manifestModifiedTime)
    : (manifestGeneratedTime || manifestModifiedTime);
  const partialGeneratedTime = timeMs(partial.generated_at);
  const partialTime = partialGeneratedTime || artefactModifiedTimeMs(item, "partial_real_motion_evidence.json");
  const partialOverrides =
    partialBlocksReadiness &&
    (!manifestStatusReady(manifest) || !manifestTime || !partialTime || partialTime >= manifestTime);
  const evidenceManifest = partialOverrides ? partial : manifest;
  const distinctMotionFamilyReport = distinctMotionFamilyReportEvidenceForPackage(item, manifest);
  const distinctMotionFamilyReportReady = !partialOverrides && distinctMotionFamilyReport.ready === true;
  const clips = materialisedMotionClipRows(evidenceManifest);
  const localClips = dedupeClipsByEvidencePath(clips.filter((clip) => {
    const clipPath = clipEvidencePath(clip);
    return /\.mp4$/i.test(clipPath) && fileExistsForEvidence([clipPath], artifactDir);
  }), artifactDir);
  const directClips = directMotionClips(localClips);
  const familyValues = localClips
    .map((clip, index) => motionReadinessFamily(clip, index))
    .filter(Boolean);
  const directFamilyValues = directClips
    .map((clip, index) => directMotionReadinessFamily(clip, index))
    .filter(Boolean);
  const localDirectMotionFamilyCount = new Set(directFamilyValues).size;
  const localDistinctFamilyCount = new Set(familyValues).size;
  const directMotionFamilyCount = directClips.length
    ? positiveMax(
        localDirectMotionFamilyCount,
        distinctMotionFamilyReportReady ? distinctMotionFamilyReport.direct_video_motion_family_count : 0,
      )
    : positiveMax(
        evidenceManifest.direct_video_motion_family_count,
        evidenceManifest.direct_video_motion_families,
        distinctMotionFamilyReportReady ? distinctMotionFamilyReport.direct_video_motion_family_count : 0,
        localDirectMotionFamilyCount,
      );
  const directMotionClipCount = directClips.length
    ? directClips.length
    : positiveMax(
        evidenceManifest.direct_video_motion_asset_count,
        evidenceManifest.direct_video_motion_clip_count,
        directClips.length,
      );
  const clipCount = localClips.length
    ? localClips.length
    : positiveMax(evidenceManifest.clip_count, evidenceManifest.materialised_motion_clip_count, localClips.length);
  const distinctFamilyCount = localClips.length
    ? positiveMax(
        localDistinctFamilyCount,
        distinctMotionFamilyReportReady ? distinctMotionFamilyReport.distinct_motion_family_count : 0,
      )
    : positiveMax(
        evidenceManifest.distinct_motion_family_count,
        evidenceManifest.materialised_motion_family_count,
        evidenceManifest.distinct_motion_families,
        distinctMotionFamilyReportReady ? distinctMotionFamilyReport.distinct_motion_family_count : 0,
        localDistinctFamilyCount,
      );
  const motionReady =
    !partialOverrides &&
    manifestStatusReady(manifest) &&
    localClips.length >= 3 &&
    clipCount >= 3;
  return {
    motionReady,
    manifest,
    partial,
    partialRealMotionOverride: partialOverrides,
    partialRealMotionStatus: cleanText(partial.status),
    clips: localClips,
    clipCount: motionReady ? Math.max(clipCount, localClips.length) : clipCount,
    distinctFamilyCount,
    directMotionClipCount,
    directMotionFamilyCount,
    distinctMotionFamilyReportReady,
    distinctMotionFamilyReportPath: distinctMotionFamilyReport.report_path || null,
    ownedGeneratedOnly: motionReady && directClips.length === 0,
    clipPaths: localClips.map((clip) => firstExistingEvidencePath([clipEvidencePath(clip)], artifactDir)).filter(Boolean),
  };
}

function audioTimestampEvidenceForPackage(item = {}) {
  const artifactDir = cleanText(item.artifact_dir);
  const manifest = readStoryJsonForQueueItem(item, "audio_manifest.json") || {};
  const narrationAudioPath = existingEvidencePath([
    manifest.resolved_narration_audio_path,
    manifest.narration_audio_path,
    manifest.audio_path,
  ], artifactDir);
  const wordTimestampsPath = existingEvidencePath([
  ...
  return {
    ...
    narrationAudioPath,
    wordTimestampsPath,
    narrationReady: Boolean(narrationAudioPath),
    wordTimestampsReady: Boolean(wordTimestampsPath),
    wordTimestampSource: cleanText(
      manifest.word_timestamp_source ||
        manifest.word_timestamp_provenance?.word_timestamp_source,
    ),
  };
}

function canonicalTitleForPackage(item = {}) {
  const canonical = readStoryJsonForQueueItem(item, "canonical_story_manifest.json") || {};
  return cleanText(
    item.title ||
      item.public_title ||
      item.selected_title ||
...
function targetRenderManifestForPackage(item = {}) {
  const artifactDir = cleanText(item.artifact_dir);
  const storyId = cleanText(item.story_id);
  return {
    ...
    output_path: artifactDir ? path.join(artifactDir, "visual_v4_render.mp4") : null,
    manifest_path: artifactDir ? path.join(artifactDir, "render_manifest.json") : null,
    story_id: storyId,
  };
}

function queueItemFromStoryPackage(item = {}) {
  const storyId = cleanText(item.story_id || item.id);
  if (!storyId) return null;
  const packageBlockers = asArray(item.blockers || item.reason_codes).map(cleanText).filter(Boolean);
  const audio = audioTimestampEvidenceForPackage(item);
  const motion = materialisedMotionEvidenceForPackage(item);
  const blockers = [];
  for (const blocker of packageBlockers) {
    if (blocker === "render:final_publish_render_missing") continue;
    if (blocker === "audio:narration_audio_missing") {
      if (!audio.narrationReady) blockers.push("final_narration_audio_missing");
      continue;
    }
    if (blocker === "captions:word_timestamps_missing") {
      if (!audio.wordTimestampsReady) blockers.push("word_timestamps_missing");
      continue;
    }
    if (blocker === "footage:v4_motion_blocked") {
      if (!motion.motionReady) blockers.push("materialised_motion_clips_missing");
      continue;
    }
  ...
  const readyForRender =
    packageBlockers.includes("render:final_publish_render_missing") &&
    uniqueBlockers.length === 0 &&
    audio.narrationReady &&
    audio.wordTimestampsReady &&
    motion.motionReady;
  return {
    story_id: storyId,
    title: canonicalTitleForPackage(item),
    artifact_dir: item.artifact_dir || null,
    force_final_render: readyForRender,
    render_input_status: readyForRender ? "ready_for_final_render_job" : "blocked",
    render_input_blockers: uniqueBlockers,
    target_render_manifest: readyForRender ? targetRenderManifestForPackage(item) : null,
    render_input_evidence: {
      package_verdict: cleanText(item.verdict) || null,
      package_blockers: packageBlockers,
      narration_audio_path: audio.narrationAudioPath,
      word_timestamps_path: audio.wordTimestampsPath,
      word_timestamp_source: audio.wordTimestampSource || null,
      materialised_motion_clip_count: motion.clipCount,
      distinct_motion_family_count: motion.distinctFamilyCount,
      materialised_motion_clip_paths: motion.clipPaths,
      story_package_input_normalized: true,
    },
  };
}

function cutoverPlanFromStoryPackagesIfNeeded(cutoverPlan = {}) {
  const packages = Array.isArray(cutoverPlan)
    ? cutoverPlan
    : asArray(cutoverPlan.story_packages || cutoverPlan.packages || cutoverPlan.stories);
  if (!packages.length || !packages.some((item) =>
    item &&
    cleanText(item.story_id || item.id) &&
    cleanText(item.artifact_dir) &&
    asArray(item.blockers || item.reason_codes).length,
  )) return cutoverPlan || {};
  const queue = packages.map(queueItemFromStoryPackage).filter(Boolean);
  return {
    schema_version: 1,
    generated_at: cutoverPlan.generated_at || null,
    source_mode: "story_package_array",
    queue,
    blocked: [],
  };
}

function latestManifestTimeMs(manifest = {}, fields = []) {
  return Math.max(
    0,
    ...fields
      .map((field) => timeMs(manifest?.[field]))
      .filter((ms) => Number.isFinite(ms) && ms > 0),
  );
}

function repairedPackageInputsFreshness(item = {}) {
  const renderManifest = item.render_manifest || readStoryJsonForQueueItem(item, "render_manifest.json") || {};
  const artifactDir = cleanText(item.artifact_dir);
  const declaredRenderGeneratedMs = timeMs(renderManifest.generated_at);
  const renderOutputModifiedMs = existingEvidenceModifiedTimeMs([
    renderManifest.output_path,
    renderManifest.output,
    item.render_output_path,
    item.output_path,
    "visual_v4_render.mp4",
  ], artifactDir);
  const renderGeneratedMs = declaredRenderGeneratedMs || renderOutputModifiedMs;
  if (!renderGeneratedMs) {
    return {
      newer_than_render: false,
      reason: "render_time_evidence_missing",
      render_time_source: null,
    };
  }

  const audioManifest = readStoryJsonForQueueItem(item, "audio_manifest.json") || {};
  const sfxManifest = readStoryJsonForQueueItem(item, "sfx_manifest.json") || {};
  const captionManifest = readStoryJsonForQueueItem(item, "caption_manifest.json") || {};
  const motionManifest = readStoryJsonForQueueItem(item, "materialised_motion_clips.json") || {};
  const canonicalManifest = readStoryJsonForQueueItem(item, "canonical_story_manifest.json") || {};
  const audioMs = latestManifestTimeMs(
    audioManifest,
    ["materialized_at", "generated_at", "audio_materialized_at"],
  ) || artefactModifiedTimeMs(item, "audio_manifest.json") || 0;
  const sfxMs = latestManifestTimeMs(
    sfxManifest,
    ["generated_at", "materialized_at"],
  ) || artefactModifiedTimeMs(item, "sfx_manifest.json") || 0;
  const captionMs = latestManifestTimeMs(
    captionManifest,
    ["generated_at", "materialized_at"],
  ) ||
    artefactModifiedTimeMs(item, "caption_manifest.json") ||
    artefactModifiedTimeMs(item, "captions.srt") ||
    0;
  const motionMs = latestManifestTimeMs(
    motionManifest,
    ["generated_at", "materialized_at", "materialised_at"],
  ) || artefactModifiedTimeMs(item, "materialised_motion_clips.json") || 0;
  const canonicalMs = latestManifestTimeMs(
    canonicalManifest,
    [
      "public_copy_regeneration_completed_at",
      "public_copy_final_render_regenerated_at",
      "audio_materialized_at",
      "generated_at",
    ],
  ) || artefactModifiedTimeMs(item, "canonical_story_manifest.json") || 0;
  const latestInputMs = Math.max(audioMs, sfxMs, captionMs, motionMs, canonicalMs);
  const currentScript = cleanText(
    canonicalManifest.narration_script ||
      canonicalManifest.full_script ||
      canonicalManifest.tts_script,
  );
  const renderScript = cleanText(renderManifest.input_fingerprint?.canonical_snapshot?.narration_script);
  const script_changed_since_render = Boolean(currentScript && renderScript && currentScript !== renderScript);
  return {
    newer_than_render: latestInputMs > renderGeneratedMs || script_changed_since_render,
    render_generated_at: renderManifest.generated_at || null,
    render_output_modified_at_ms: renderOutputModifiedMs || null,
    render_time_source: declaredRenderGeneratedMs
      ? "render_manifest_generated_at"
      : "render_output_mtime",
    latest_input_ms: latestInputMs || null,
    audio_manifest_newer_than_render: audioMs > renderGeneratedMs,
    sfx_manifest_newer_than_render: sfxMs > renderGeneratedMs,
    caption_manifest_newer_than_render: captionMs > renderGeneratedMs,
    motion_manifest_newer_than_render: motionMs > renderGeneratedMs,
    canonical_manifest_newer_than_render: canonicalMs > renderGeneratedMs,
    script_changed_since_render,
  };
}
...
function classifyMotionRepairLane(item = {}, context = {}) {
  ...
  if (
    ...
    return {
      ...
      recommended_command: productionCutoverCommand(item),
      post_repair_validation_command: renderInputValidationCommand(item),
  ...
  if (searchBlockers.length > 0) {
    if (requiresOwnedVisualPlan(searchBlockers)) {
      return {
        ...
        recommended_command:
          `npm run ops:goal-owned-motion -- --story-id ${storyId} --work-order ${commandPath(repairWorkOrderPath(item))} --out-dir ${commandPath(repairOutputDir(item))} --json`,
        post_repair_validation_command: productionCutoverCommand(item),
        evidence: {
          source_search_blockers: searchBlockers,
          primary_story_entity: sourceRow.primary_story_entity || null,
        },
  ...
  if (
    renderEvidence.local_promotion_only === true &&
    !hasLocalPromotionMotionValidationInput(item) &&
    !validation &&
    !realMotionJob
  ) {
    return {
      repair_lane: "official_motion_segment_validation_required",
      status: "operator_required",
      auto_repairable: false,
      operator_approval_required: true,
      dead_end_blocker: false,
      exact_missing_input:
        "validated official direct-motion segment report before materialising or rendering this local promotion candidate",
      recommended_command:
        `Run official-source intake and segment validation for ${storyId}; do not materialise or render until the segment report exists.`,
      post_repair_validation_command:
        `npm run ops:goal-real-motion -- --story-id ${storyId} --work-order ${commandPath(repairWorkOrderPath(item))} --out-dir ${commandPath(repairMotionOutDir(item))} --segment-report <validated-segment-report> --json`,
      evidence: {
        local_promotion_only: true,
        segment_report_path: "",
        trailer_reference_count: Number(renderEvidence.trailer_reference_count || 0),
        materialised_motion_clip_count: Number(renderEvidence.materialised_motion_clip_count || 0),
        distinct_motion_family_count: Number(renderEvidence.distinct_motion_family_count || 0),
      },
      output_expectations: [
        "official/direct-media source evidence exists before motion materialisation",
        "segment validation report contains enough readable gameplay/trailer windows",
        "Visual V4 render is not attempted from missing or unvalidated motion inputs",
      ],
      allowed_routes: [
      ...
      blocked_when: [
        "segment_report_missing",
        "official_motion_reference_missing",
        "validated_direct_motion_clip_floor_not_met",
      ],
    };
  }
  ...
  return {
    ...
    recommended_command: realMotionMaterialisationCommand(item, storyId),
    post_repair_validation_command: renderInputValidationCommand(item),
    evidence: {},
  };
}
...
function actionForAudioAndTimestamps(item = {}) {
  ...
  return {
    ...
    operator_approval_required: false,
    dead_end_blocker: false,
    recommended_command: audioTimestampCommand(item, storyId),
    post_repair_validation_command: renderInputValidationCommand(item),
...
function actionForReadableHyperframesMotionRepair(item = {}) {
  const storyId = cleanText(item.story_id);
  const fileEvidence = item.render_input_evidence?.file_evidence || item.render_input_evidence || {};
  const tooFastCards = [
    ...asArray(fileEvidence.hyperframes_effective_too_fast_card_shots),
    ...asArray(fileEvidence.hyperframes_too_fast_card_shots),
    ...asArray(fileEvidence.hyperframes_too_fast_card_clips),
    ...asArray(fileEvidence.rendered_too_fast_card_windows),
  ];
  const missingDurationCards = asArray(fileEvidence.hyperframes_missing_duration_card_clips);
  const repeatedCardFamilies = [
    ...asArray(fileEvidence.hyperframes_repeated_card_families),
    ...asArray(fileEvidence.repeated_readable_card_kinds),
  ];
  return {
    action_id: "materialise_owned_generated_motion_clips",
    status: "auto_repairable",
    repair_lane: "readable_hyperframes_card_motion_rematerialisation",
    exact_missing_input: "fresh non-repeating owned HyperFrames/source-card motion clips with a 12s+ readable dwell floor",
    required_artefact_path: storyArtifactPath(item, "owned_motion_manifest.json"),
    required_artefact_paths: [
      storyArtifactPath(item, "materialised_motion_clips.json"),
      storyArtifactPath(item, "distinct_motion_family_report.json"),
      storyArtifactPath(item, "owned_motion_manifest.json"),
    ...
    recommended_command:
      `npm run ops:goal-owned-motion -- --story-id ${storyId} --work-order output/goal-contract/render_input_work_order.json --out-dir output/goal-contract --refresh-existing --json`,
    post_repair_validation_command:
      "npm run ops:goal-dry-run-publish -- --out-dir output/goal-contract --json",
    reason_codes: hyperframesReadableDwellBlockers(item.render_input_blockers),
    evidence: {
      minimum_readable_card_duration_s:
        Number(fileEvidence.minimum_readable_card_duration_s || 0) || 12,
      too_fast_card_count: tooFastCards.length,
      missing_duration_card_count: missingDurationCards.length,
      repeated_card_family_count: repeatedCardFamilies.length,
      too_fast_cards: tooFastCards,
      missing_duration_cards: missingDurationCards,
      repeated_card_families: repeatedCardFamilies,
    },
    output_expectations: [
      "owned HyperFrames/source-card clips are regenerated after the current script and source-lock state",
      "each readable card clip has durationS >= 12 unless its own readability contract requires longer",
      "readable card families/kinds are not repeated to pad the render",
      "strict dry-run no longer reports hyperframes/card dwell blockers",
    ],
    allowed_routes: [
      "rematerialise_owned_source_locked_explainer_motion",
      "regenerate_story_specific_cards_after_public_copy_repair",
      "rerun_strict_dry_run_after_card_dwell_repair",
    ],
    blocked_when: [
      "primary_source_is_discovery_only",
      "source_claim_missing",
...
function actionForRepeatedDirectMotionRepair(item = {}) {
  const storyId = cleanText(item.story_id);
  const fileEvidence = item.render_input_evidence?.file_evidence || item.render_input_evidence || {};
  return {
    ...
    required_artefact_paths: [
      storyArtifactPath(item, "materialised_motion_clips.json"),
      storyArtifactPath(item, "distinct_motion_family_report.json"),
      storyArtifactPath(item, "footage_inventory.json"),
    ],
    auto_repairable: false,
    operator_approval_required: true,
    dead_end_blocker: false,
    recommended_command:
      `Find additional official direct-media families for ${storyId}; do not reuse the same trailer/source windows, then rerun media intake and ops:v4-motion-pack.`,
    post_repair_validation_command:
      `npm run ops:v4-motion-pack -- --story-id ${storyId} --out-dir output/studio-v4/motion-packs --json`,
    reason_codes: visualMotionRepeatBlockers(item.render_input_blockers),
    evidence: {
      repeated_direct_motion_segments: asArray(fileEvidence.repeated_direct_motion_segments),
      repeated_direct_motion_segment_count:
        Number(fileEvidence.repeated_direct_motion_segment_count || 0) ||
        asArray(fileEvidence.repeated_direct_motion_segments).length,
      direct_motion_base_source_overuse: asArray(fileEvidence.direct_motion_base_source_overuse),
      direct_motion_base_source_overuse_max_clips:
        Number(fileEvidence.direct_motion_base_source_overuse_max_clips || 0) || null,
      direct_motion_base_source_overuse_max_share:
        Number(fileEvidence.direct_motion_base_source_overuse_max_share || 0) || null,
    },
    output_expectations: [
      "replacement motion pack uses distinct official/licensed source families",
      "no base source exceeds the direct-motion overuse limit",
      "strict dry-run no longer reports repeated direct motion or base-source overuse",
    ],
    allowed_routes: [
      "official_direct_media_source_family_intake",
      "operator_supplied_licensed_motion_files",
      "validated_distinct_trailer_or_gameplay_windows",
    ],
    blocked_when: [
      "only_one_official_video_exists",
      "source_family_duplicates_only",
      "rights_ledger_missing",
      "clip_path_missing_on_disk",
    ],
...
function actionForPublicOutputCoherence(item = {}) {
  ...
  return {
    ...
    recommended_command: unrecoverableImagePost
      ...
      : publicCopyRepairCommand(item, storyId),
    post_repair_validation_command: renderInputValidationCommand(item),
    reason_codes: asArray(item.render_input_blockers).filter((blocker) =>
...
function actionForDuplicateTitleRepair(item = {}) {
  ...
  return {
    ...
    operator_approval_required: true,
    dead_end_blocker: false,
    recommended_command: publicCopyRepairCommand(
      item,
      storyId,
      duplicateTitle ? ` --reserved-title ${commandArg(duplicateTitle)}` : "",
    ),
    post_repair_validation_command:
      `npm run ops:goal-dry-run-publish -- --out-dir ${commandPath(repairOutputDir(item))} --candidate-report test/output/next_publish_candidates.json --json`,
    reason_codes: asArray(item.render_input_blockers).filter((blocker) =>
...
function actionForScriptScorecardRepair(item = {}) {
  ...
  return {
    ...
    operator_approval_required: false,
    dead_end_blocker: false,
    recommended_command: publicCopyRepairCommand(item, storyId),
...
function actionForCommercialDisclosureRepair(item = {}) {
  return {
    ...
    dead_end_blocker: false,
    recommended_command:
      "npm run ops:goal-commercial-disclosure-repair -- --story-packages output/goal-contract/production_cutover_story_packages.json --work-order output/goal-contract/render_input_work_order.json --out-dir output/goal-contract --json",
    post_repair_validation_command: `npm run ops:goal-render-inputs -- --cutover-plan output/goal-contract/production_render_cutover_plan.json --out-dir output/goal-contract --json`,
...
function actionForProductionRender(item = {}) {
  return {
    ...
    operator_approval_required: false,
    dead_end_blocker: false,
    recommended_command: productionRenderCommand(item),
    post_repair_validation_command: dryRunValidationCommand(item),
...
function actionForFinalMp4Repair(item = {}) {
  const prerequisiteBlockers = finalRenderPrerequisiteBlockers(item);
  const autoRepairable = prerequisiteBlockers.length === 0;
  return {
    action_id: "materialise_final_mp4",
    status: autoRepairable ? "required" : "blocked_on_render_inputs",
    ...
    evidence: prerequisiteBlockers.length
...
function actionForCaptionFileRepair(item = {}) {
  return {
    ...
    operator_approval_required: false,
    dead_end_blocker: false,
    recommended_command: audioTimestampCommand(item),
    post_repair_validation_command: renderInputValidationCommand(item),
...
function actionForAudioSegmentLoudnessQaRefresh(item = {}) {
  return {
    action_id: "refresh_audio_segment_loudness_qa",
    status: "required",
    repair_lane: "audio_segment_loudness_qa_refresh",
    exact_missing_input: "fresh rendered-audio segment loudness QA for the latest final MP4",
    required_artefact_path: storyArtifactPath(item, "audio_segment_loudness_report.json"),
    required_artefact_paths: [
      storyArtifactPath(item, "audio_segment_loudness_report.json"),
      "output/goal-contract/render_audio_segment_audit.json",
    ],
    auto_repairable: true,
    operator_approval_required: false,
    dead_end_blocker: false,
    recommended_command:
      "npm run ops:render-audio-segment-audit -- --bridge output/goal-contract/scheduler_bridge_candidates.json --out-dir output/goal-contract --json",
    post_repair_validation_command: "npm run ops:goal-dry-run-publish -- --json",
    reason_codes: asArray(item.render_input_blockers).filter((blocker) =>
      blocker === "audio_segment_loudness_report_stale_after_render",
    ),
    output_expectations: [
      "audio_segment_loudness_report.json is generated after the current visual_v4_render.mp4",
      "aggregate render_audio_segment_audit.json reports pass for the candidate",
      "strict dry-run no longer reports audio_segment_loudness_report_stale_after_render",
    ],
    allowed_routes: ["rerun_render_audio_segment_audit"],
    blocked_when: ["final_mp4_missing", "render_manifest_missing", "audio_probe_failed"],
  };
}

function actionForNarrationVoiceQualityQaRefresh(item = {}) {
  return {
    action_id: "refresh_narration_voice_quality_qa",
    status: "required",
    repair_lane: "narration_voice_quality_qa_refresh",
    exact_missing_input: "fresh voice quality QA after the latest audio and caption manifests",
    required_artefact_path: storyArtifactPath(item, "voice_quality_report.json"),
    required_artefact_paths: [
      storyArtifactPath(item, "voice_quality_report.json"),
      storyArtifactPath(item, "narration_manifest.json"),
      storyArtifactPath(item, "caption_manifest.json"),
    ],
    auto_repairable: true,
    operator_approval_required: false,
    dead_end_blocker: false,
    recommended_command:
      "npm run ops:goal-narration-qa-repair -- --dry-run-plan output/goal-contract/dry_run_publish_plan.json --out-dir output/goal-contract --apply --json",
    post_repair_validation_command: "npm run ops:goal-dry-run-publish -- --json",
    reason_codes: asArray(item.render_input_blockers).filter((blocker) =>
      blocker === "voice_quality_report_stale_after_audio" ||
      blocker === "voice_quality_report_stale_after_captions",
    ),
    output_expectations: [
      "voice_quality_report.json is newer than audio_manifest.json and caption_manifest.json",
      "narration_manifest.json and caption_manifest.json remain ready",
      "strict dry-run no longer reports stale voice quality QA",
    ],
    allowed_routes: ["rerun_goal_narration_qa_repair"],
    blocked_when: ["audio_manifest_missing", "caption_file_missing", "word_timestamps_missing"],
  };
}
...
function jobForQueueItem(item = {}, context = {}) {
  const staleFinalRenderInputFingerprint = finalRenderInputFingerprintStale(item.render_input_evidence || {});
  const fileEvidenceBlockers = visualRepairBlockersFromFileEvidence(
    item.render_input_evidence?.file_evidence || item.render_input_evidence || {},
  );
  const initialBlockers = [
    ...asArray(item.render_input_blockers),
    ...fileEvidenceBlockers,
    ...(staleFinalRenderInputFingerprint ? [] : normalProductionDurationBlockers(item.blockers)),
  ];
  const resolved = removeResolvedCurrentArtifactBlockers(new Set(initialBlockers), item);
  let blockers = Array.from(resolved.blockers);
  const renderInputEvidence = {
    ...(item.render_input_evidence || {}),
    ...nonEmptyEvidenceFields(resolved.current_evidence),
  };
  for (const blocker of visualRepairBlockersFromFileEvidence(renderInputEvidence)) {
    if (!blockers.includes(blocker)) blockers.push(blocker);
  }
  blockers = removeStalePackageBlockersClearedByCurrentEvidence(blockers, renderInputEvidence);
  const itemWithResolvedRenderEvidence = {
    ...item,
    render_input_blockers: blockers,
    render_input_evidence: renderInputEvidence,
  };
  const actions = [];
  ...
  const renderInputStatus = cleanText(item.render_input_status);
  const hardBlockers = hardFinalRenderBlockers(blockers);
  const repairedCurrentInputsReady =
    hardBlockers.length === 0 &&
    currentInputsReadyForFinalRender(renderInputEvidence);
  const itemForActions =
    repairedCurrentInputsReady && !item.target_render_manifest
      ? {
          ...item,
          force_final_render: true,
          target_render_manifest: forcedRerenderTargetManifest(item),
        }
      : item;
  const readyForSchedulerPreflight =
    renderInputStatus === "ready_for_scheduler_preflight" &&
    !hasNormalProductionDurationBlocker &&
    blockers.length === 0;
  const readyForFinalRender =
    !readyForSchedulerPreflight &&
    (
      (renderInputStatus === "ready_for_final_render_job" && hardBlockers.length === 0) ||
      (item.force_final_render === true && hardBlockers.length === 0) ||
      repairedCurrentInputsReady
    ) &&
    !hasNormalProductionDurationBlocker;
  if (hasNormalProductionDurationBlocker) {
    pushUniqueAction(actions, actionForNormalProductionDurationRepair(item, blockers));
  }
  const itemWithCurrentBlockers = {
    ...item,
    render_input_blockers: blockers,
  };
  if (hasAny(blockers, ["final_narration_audio_missing", "word_timestamps_missing"])) {
    pushUniqueAction(actions, actionForAudioAndTimestamps(itemWithCurrentBlockers));
  }
  if (hasAny(blockers, [
    "final_narration_audio_stale_after_public_copy_repair",
    "word_timestamps_stale_after_public_copy_repair",
  ...
  if (hasAny(blockers, ["visual_motion_repeat_repair_required"]) && !hasTerminalPublicCopyBlocker && !hasMotionMaterialisationAction()) {
    pushUniqueAction(actions, actionForRepeatedDirectMotionRepair(itemWithResolvedRenderEvidence));
  }
  ...
  if (
    hasAny(blockers, [
      ...
      "hyperframes_card_family_repair_required",
    ]) &&
    !hasTerminalPublicCopyBlocker
  ) {
    pushUniqueAction(actions, actionForReadableHyperframesMotionRepair(itemWithResolvedRenderEvidence));
  }
  if (hasAny(blockers, [
  ...
  if (hasAny(blockers, ["audio_segment_loudness_report_stale_after_render"])) {
    pushUniqueAction(actions, actionForAudioSegmentLoudnessQaRefresh(item));
  }
  if (hasAny(blockers, [
    "voice_quality_report_stale_after_audio",
    "voice_quality_report_stale_after_captions",
  ])) {
    pushUniqueAction(actions, actionForNarrationVoiceQualityQaRefresh(item));
  }
  ...
  if (readyForFinalRender) {
    pushUniqueAction(actions, actionForProductionRender(itemForActions));
  }
  return {
    story_id: cleanText(item.story_id),
    title: cleanText(item.title),
    artifact_dir: item.artifact_dir || null,
    force_final_render: item.force_final_render === true || repairedCurrentInputsReady,
    target_render_manifest: readyForFinalRender
      ? (itemForActions.target_render_manifest || item.target_render_manifest || null)
      : item.target_render_manifest || null,
    status:
      readyForSchedulerPreflight
          ? "ready_for_scheduler_preflight"
          : readyForFinalRender
            ? "ready_for_final_render_job"
            : "blocked_on_render_inputs",
    blockers,
    evidence: renderInputEvidence,
    actions,
  };
}

const OWNED_MOTION_ONLY_REPLACEABLE_BLOCKERS = new Set([
  "direct_video_motion_clip_floor_not_met",
  "real_visual_motion_clips_missing",
  "real_visual_motion_families_insufficient",
  "visual_evidence:generated_only_motion_deck",
...
function defaultOwnedMotionOnlyValidator(materialisedMotion = {}, options = {}) {
  const { _private } = require("./goal-production-render-materializer");
  return _private.selectGovernedOwnedMotionOnlyClips(materialisedMotion, options);
}

function ownedMotionOnlySelectionSummary(selection = {}) {
  return {
    verified_clip_count: Number(selection.verified_clip_count || 0),
    primary_clip_count: Number(selection.primary_clip_count || 0),
    readable_card_count: Number(selection.readable_card_count || 0),
    source_card_count: Number(selection.source_card_count || 0),
    generator_project_count: Number(selection.generator_project_count || 0),
    generator_project_ids: asArray(selection.generator_project_ids).map(cleanText).filter(Boolean),
    target_platforms: asArray(selection.target_platforms).map(cleanText).filter(Boolean),
    blockers: asArray(selection.blockers).map(cleanText).filter(Boolean),
  };
}

function promoteGovernedOwnedMotionOnlyRenderJob(job = {}, {
  enabled = false,
  materialisedMotion = null,
  validator = defaultOwnedMotionOnlyValidator,
  targetPlatforms = ["youtube_shorts", "instagram_reels", "facebook_reels"],
} = {}) {
  if (!enabled || cleanText(job.status) !== "blocked_on_render_inputs") return job;
  const blockers = asArray(job.blockers).map(cleanText).filter(Boolean);
  if (
    !blockers.length ||
    blockers.some((blocker) => !OWNED_MOTION_ONLY_REPLACEABLE_BLOCKERS.has(blocker))
  ) {
    return job;
  }
  ...
  const manifest = materialisedMotion || readStoryJsonForQueueItem(job, "materialised_motion_clips.json") || {};
  const selection = validator(manifest, { targetPlatforms });
  const selectionSummary = ownedMotionOnlySelectionSummary(selection);
  if (selectionSummary.blockers.length || !asArray(selection.clips).length) {
    return {
      ...job,
      evidence: {
        ...evidence,
        owned_motion_only_selection: selectionSummary,
      },
    };
  }
  const targetRenderManifest = job.target_render_manifest || forcedRerenderTargetManifest(job);
  const renderItem = {
    ...job,
    force_final_render: true,
  ...
  return {
    ...
    blockers: [],
    actions: [actionForProductionRender(renderItem)],
  };
}
...
function mapIncidentBlocker(blocker = "") {
  ...
  const mappings = {
    ...
    "incident:repeated_direct_motion_segment": "visual_motion_repeat_repair_required",
...
function missingInputRenderBlockers(blocker = "") {
  const match = cleanText(blocker).match(/^missing_input:(.+)$/i);
  if (!match) return [];
  const filename = path.basename(match[1]).toLowerCase();
  const blockers = [];
  if (filename === "visual_v4_render.mp4") {
    blockers.push(
      "final_narration_audio_missing",
      "word_timestamps_missing",
      "materialised_motion_clips_missing",
  ...
  if (filename === "captions.srt") {
    blockers.push("word_timestamps_missing", "caption_file_missing");
  }
  if (filename === "rights_ledger.json") blockers.push("rights_ledger_missing");
  if (filename === "sfx_manifest.json") blockers.push("sound_design_benchmark_repair_required");
  if (filename === "benchmark_report.json" || filename === "visual_quality_report.json") {
    blockers.push("aggregate_benchmark_repair_required");
  }
  if (filename === "director_beat_map.json") blockers.push("render_manifest_missing");
  if (filename === "platform_publish_manifest.json" || filename === "publish_verdict.json") {
    blockers.push("stale_qa_state");
  }
...
function blockersFromFileEvidence(fileEvidence = {}) {
  ...
  if (
    !currentReadableHyperframesEvidenceReady(fileEvidence) &&
    (
      asArray(fileEvidence.hyperframes_effective_too_fast_card_shots).length ||
      asArray(fileEvidence.hyperframes_too_fast_card_shots).length ||
      asArray(fileEvidence.hyperframes_too_fast_card_clips).length ||
      asArray(fileEvidence.hyperframes_missing_duration_card_clips).length ||
      asArray(fileEvidence.rendered_too_fast_card_windows).length
    )
  ) {
    blockers.push("hyperframes_readable_dwell_repair_required");
  }
  if (
    asArray(fileEvidence.hyperframes_repeated_card_families).length ||
    asArray(fileEvidence.repeated_readable_card_kinds).length
  ) {
    blockers.push("hyperframes_card_family_repair_required");
  }
  if (
    asArray(fileEvidence.repeated_direct_motion_segments).length ||
    Number(fileEvidence.repeated_direct_motion_segment_count || 0) > 0 ||
    asArray(fileEvidence.direct_motion_base_source_overuse).length
  ) {
    blockers.push("visual_motion_repeat_repair_required");
  }
  return blockers;
}

function visualRepairBlockersFromFileEvidence(fileEvidence = {}) {
  const currentReadableCardsReady = currentReadableHyperframesEvidenceReady(fileEvidence);
  const hasActiveHyperframesDwellFailure =
    !currentReadableCardsReady &&
    (
      asArray(fileEvidence.hyperframes_effective_too_fast_card_shots).length > 0 ||
      asArray(fileEvidence.hyperframes_too_fast_card_shots).length > 0 ||
      asArray(fileEvidence.hyperframes_too_fast_card_clips).length > 0 ||
      asArray(fileEvidence.hyperframes_missing_duration_card_clips).length > 0 ||
      asArray(fileEvidence.rendered_too_fast_card_windows).length > 0
    );
  const readableHyperframesReady =
    (
      currentReadableCardsReady ||
      (
        fileEvidence.readable_hyperframes_ready === true &&
        Number(fileEvidence.readable_hyperframes_too_fast_count || 0) === 0 &&
        Number(fileEvidence.readable_hyperframes_clip_count || 0) > 0
      )
    ) &&
    !hasActiveHyperframesDwellFailure;
  return blockersFromFileEvidence(fileEvidence).filter((blocker) => {
    if (blocker === "hyperframes_readable_dwell_repair_required") {
      return !readableHyperframesReady;
    }
    return blocker === "visual_motion_repeat_repair_required" ||
      blocker === "hyperframes_card_family_repair_required";
  });
}
...
function cutoverHasFailedRenderQa(rawBlockers = [], item = {}) {
  return (
    asArray(rawBlockers).some((blocker) =>
      /benchmark_not_pass|benchmark_below_production_threshold|visual_quality_failed|voice_quality_failed|gold_standard:|sfx_impact|post_render_forensics_failed/i.test(
        blocker,
      ),
    ) ||
    /failed|fail/i.test(cleanText(item.render_manifest?.quality_gate_status)) ||
    /fail/i.test(cleanText(item.benchmark_summary?.result || item.benchmark_summary?.visual_quality_result))
  );
}

function cutoverNeedsFirstProductionRender(rawBlockers = [], item = {}) {
  const renderManifest = item.render_manifest || {};
  if (renderManifest.final_publish_render === true) return false;
  const renderer = cleanText(renderManifest.renderer).toLowerCase();
  const visualTier = cleanText(renderManifest.visual_tier).toLowerCase();
  const localProof = renderer === "visual_v4_local_proof" || visualTier === "local_proof_motion_graphic";
  if (!localProof) return false;
  return asArray(rawBlockers).some((blocker) =>
    /final_publish_render_(?:missing|not_proven)|render_not_final_publish_ready/i.test(cleanText(blocker)),
  );
}

function localRenderRightsEvidence(item = {}, motion = {}) {
  const ledger = readStoryJsonForQueueItem(item, "rights_ledger.json") || {};
  if (!Object.keys(ledger).length) {
    return {
      ready: false,
      scope: "current_local_render_inputs",
      blocker_codes: ["rights:ledger_missing"],
      selected_motion_clip_ids: [],
      complete_selected_motion_clip_ids: [],
      ignored_unselected_rights_record_ids: [],
      publish_holds_retained: [],
      local_render_only: true,
      publish_permitted: false,
    };
  }
  const allowedPublishHolds = new Set([
    "rights:live_publish_not_allowed",
    "rights:human_legal_review_required_before_publish",
    "rights:direct_motion_live_publish_hold",
  ]);
  const rightsCodes = [...new Set([
    ...asArray(ledger.failures),
    ...asArray(ledger.blockers),
    ...asArray(ledger.publish_blockers),
  ].map(cleanText).filter((code) => code.startsWith("rights:")))];
  ...
  for (const record of records) {
    const assetId = cleanText(record.asset_id || record.id || record.clip_id);
    if (assetId && !recordsById.has(assetId)) recordsById.set(assetId, record);
  }
  const clipIds = asArray(motion.clips)
    .map((clip) => cleanText(clip.id || clip.clip_id || clip.asset_id))
    .filter(Boolean);
  const recordComplete = (record) => Boolean(
    record &&
      cleanText(record.licence_basis || record.license_basis) &&
      cleanText(record.allowed_use) &&
      record.commercial_use_allowed === true &&
      asArray(record.allowed_platforms).map(cleanText).filter(Boolean).length > 0
  );
  const completeClipIds = clipIds.filter((clipId) => recordComplete(recordsById.get(clipId)));
  const incompleteClipIds = clipIds.filter((clipId) => !recordComplete(recordsById.get(clipId)));
  const missingLicenceRecords = Array.from(recordsById.entries())
    .filter(([, record]) => !cleanText(record.licence_basis || record.license_basis));
  const ignorableUnselectedRecords = missingLicenceRecords.filter(([assetId, record]) => {
    const kind = cleanText(record.kind || record.asset_type || record.type).toLowerCase();
    const localPath = cleanText(record.path || record.asset_path || record.local_path);
    return (
      !clipIds.includes(assetId) &&
      /visual|image|logo/.test(kind) &&
      !localPath
    );
  });
  const missingLicenceCanBeScopedToUnselectedRecords =
    missingLicenceRecords.length > 0 &&
    missingLicenceRecords.length === ignorableUnselectedRecords.length;
  const blockerCodes = rightsCodes.filter((code) => {
    if (allowedPublishHolds.has(code)) return false;
    if (code === "rights:licence_basis_missing") {
      return !missingLicenceCanBeScopedToUnselectedRecords;
    }
    return true;
  });
  const ready = Boolean(
    clipIds.length >= 5 &&
      incompleteClipIds.length === 0 &&
      blockerCodes.length === 0
  );
  return {
    ready,
    scope: "current_local_render_inputs",
    global_ledger_verdict: cleanText(ledger.verdict) || null,
    blocker_codes: blockerCodes,
    selected_motion_clip_ids: clipIds,
    complete_selected_motion_clip_ids: completeClipIds,
    incomplete_selected_motion_clip_ids: incompleteClipIds,
    ignored_unselected_rights_record_ids: ignorableUnselectedRecords.map(([assetId]) => assetId),
    publish_holds_retained: rightsCodes.filter((code) => allowedPublishHolds.has(code)),
    local_render_only: true,
    publish_permitted: false,
  };
}

function localRenderRightsEvidenceReady(item = {}, motion = {}) {
  const evidence = localRenderRightsEvidence(item, motion);
  if (!evidence.ready) return false;
  return asArray(motion.clips).every((clip) => {
    const clipId = cleanText(clip.id || clip.clip_id || clip.asset_id);
    return Boolean(clipId && evidence.complete_selected_motion_clip_ids.includes(clipId));
  });
}

function visualEvidenceReadyForForcedRerender(item = {}) {
  const selected =
  ...
  const selectedHasEvidence = Boolean(
    selected &&
      (
        selected.has_selected_render_assets === true ||
        positiveMax(
          selected.direct_video_motion_asset_count,
          selected.real_motion_asset_count,
          selected.real_media_asset_count,
          selected.motion_asset_count,
        ) > 0
      ),
  );
  const profile = selectedHasEvidence
    ? selected
    : (
  ...
  if (asArray(profile.blockers).length) return false;
  if (profile.generated_only_motion_deck === true) return false;
  const directVideoCount = positiveMax(
    profile.direct_video_motion_asset_count,
    profile.real_motion_asset_count,
    profile.real_media_asset_count,
    profile.motion_asset_count,
  );
  const directVideoFamilyCount = positiveMax(
    profile.direct_video_motion_family_count,
    profile.real_media_family_count,
  );
  return directVideoCount >= 5 && directVideoFamilyCount >= 4;
}

function cutoverBlockedItemNeedsForcedRerender({ item = {}, rawBlockers = [], blockers = new Set() } = {}) {
  const firstProductionRender = cutoverNeedsFirstProductionRender(rawBlockers, item);
  if (!cutoverHasFailedRenderQa(rawBlockers, item) && !firstProductionRender) return false;
  const unresolved = Array.from(blockers).map(cleanText).filter((blocker) => blocker && blocker !== "stale_qa_state");
  if (unresolved.length) return false;
  if (!visualEvidenceReadyForForcedRerender(item)) return false;
  if (firstProductionRender) {
    const audio = currentPackageAudioEvidence(item);
    const motion = materialisedMotionEvidenceForPackage(item);
    return Boolean(
      audio.narration_ready &&
        audio.word_timestamps_ready &&
        audio.stale_after_script_rewrite !== true &&
        motion.motionReady &&
        motion.directMotionClipCount >= 5 &&
        motion.directMotionFamilyCount >= 4 &&
        localRenderRightsEvidenceReady(item, motion)
    );
  }
  return repairedPackageInputsFreshness(item).newer_than_render === true;
}

function forcedRerenderTargetManifest(item = {}) {
  const storyId = cleanText(item.story_id || item.id);
  const artifactDir = cleanText(item.artifact_dir);
  const renderManifest = item.render_manifest || {};
  const requiredPassCount = positiveMax(
    item.target_render_manifest?.hyperframes_premium_shell_required_pass_count,
    item.target_render_manifest?.premium_shell_required_pass_count,
    renderManifest.hyperframes_premium_shell_required_pass_count,
    renderManifest.premium_shell_required_pass_count,
    4,
  );
  const requestedRenderer = cleanText(
    item.target_render_manifest?.renderer || renderManifest.renderer,
  );
  const requestedVisualTier = cleanText(
    item.target_render_manifest?.visual_tier || renderManifest.visual_tier,
  );
  return {
    ...(item.target_render_manifest || {}),
    renderer: requestedRenderer === "visual_v4_production"
      ? requestedRenderer
      : "visual_v4_production",
    visual_tier: requestedVisualTier === "production_v4_motion"
      ? requestedVisualTier
      : "production_v4_motion",
    final_publish_render: true,
    hyperframes_premium_shell_required:
      item.target_render_manifest?.hyperframes_premium_shell_required !== false &&
      renderManifest.hyperframes_premium_shell_required !== false,
    hyperframes_premium_shell_required_pass_count: requiredPassCount,
    output: cleanText(item.target_render_manifest?.output || renderManifest.output || "visual_v4_render.mp4"),
    output_path:
      cleanText(item.target_render_manifest?.output_path || renderManifest.output_path) ||
      (artifactDir ? path.join(artifactDir, "visual_v4_render.mp4") : null),
    manifest_path:
      cleanText(item.target_render_manifest?.manifest_path || renderManifest.manifest_path) ||
      (artifactDir ? path.join(artifactDir, "render_manifest.json") : null),
    story_id: storyId,
  };
}

function currentPackageAudioEvidence(item = {}) {
  const artifactDir = cleanText(item.artifact_dir);
  const audioManifest = readStoryJsonForQueueItem(item, "audio_manifest.json") || {};
  const canonicalManifest = readStoryJsonForQueueItem(item, "canonical_story_manifest.json") || {};
  const storedNarrationAudioPath =
    cleanText(audioManifest.resolved_narration_audio_path) ||
    cleanText(audioManifest.narration_audio_path) ||
    cleanText(audioManifest.audio_path) ||
    null;
  const storedWordTimestampsPath =
    cleanText(audioManifest.resolved_word_timestamps_path) ||
    cleanText(audioManifest.word_timestamps_path) ||
    cleanText(audioManifest.timestamps_path) ||
    null;
  const narrationAudioPath = existingEvidencePath([
    storedNarrationAudioPath,
  ], artifactDir);
  const wordTimestampsPath = existingEvidencePath([
    storedWordTimestampsPath,
  ], artifactDir);
  const canonicalScript = cleanText(
  ...
  const canonicalWordCount =
    wordCount(canonicalScript) ||
    Number(canonicalManifest.tts_word_count || canonicalManifest.word_count);
  const canonicalDisplayScript = cleanText(
    canonicalManifest.narration_script ||
      canonicalManifest.full_script ||
      canonicalManifest.spoken_narration_script ||
      canonicalManifest.tts_script ||
  ...
  const timestampPayload = readJsonPath(wordTimestampsPath);
  const timestampDisplayScript = cleanText(
    timestampPayload?.meta?.display_text ||
      timestampPayload?.meta?.displayText ||
      timestampPayload?.meta?.text,
  );
  const timestampDisplayScriptMatches = Boolean(
    canonicalDisplayScript &&
    timestampDisplayScript &&
    canonicalDisplayScript === timestampDisplayScript,
  );
  const timestampExpectedWordCount = positiveMax(
    audioManifest.timestamp_whisper_alignment?.script_expected_word_count,
    audioManifest.word_timestamp_provenance?.timestamp_whisper_alignment?.script_expected_word_count,
    audioManifest.word_timestamp_count,
    audioManifest.audio_word_timestamp_count,
  );
  const staleTolerance = canonicalWordCount ? Math.max(2, Math.ceil(canonicalWordCount * 0.05)) : 0;
  const staleAfterScriptRewrite = !timestampDisplayScriptMatches &&
    canonicalWordCount > 0 &&
    timestampExpectedWordCount > 0 &&
    Math.abs(timestampExpectedWordCount - canonicalWordCount) > staleTolerance;
  return {
    narration_audio_path: narrationAudioPath || storedNarrationAudioPath,
    word_timestamps_path: wordTimestampsPath || storedWordTimestampsPath,
    narration_ready: Boolean(narrationAudioPath) && !staleAfterScriptRewrite,
    word_timestamps_ready: Boolean(wordTimestampsPath) && !staleAfterScriptRewrite,
    word_timestamp_source:
      cleanText(audioManifest.word_timestamp_source || audioManifest.wordTimestampSource) ||
      cleanText(audioManifest.word_timestamp_provenance?.word_timestamp_source) ||
      null,
    stale_after_script_rewrite: staleAfterScriptRewrite,
    canonical_script_word_count: canonicalWordCount || null,
    timestamp_expected_word_count: timestampExpectedWordCount || null,
    timestamp_display_script_matches: timestampDisplayScriptMatches,
  };
}

function readableHyperframesEvidenceForPackage(item = {}) {
  const artifactDir = cleanText(item.artifact_dir);
  const ownedManifest = readStoryJsonForQueueItem(item, "owned_motion_manifest.json") || {};
  const materialisedManifest = readStoryJsonForQueueItem(item, "materialised_motion_clips.json") || {};
  const clips = [
    ...materialisedMotionClipRows(ownedManifest),
    ...materialisedMotionClipRows(materialisedManifest),
  ];
  const readableCardClips = clips.filter((clip) =>
    clip?.owned_explainer_visual_plan === true ||
    /owned_(source_card_)?explainer|source_card|proof|card/i.test(cleanText(
      clip.source_kind ||
        clip.media_kind ||
        clip.asset_class ||
        clip.visual_purpose ||
  ...
  const existingReadableClips = readableCardClips.filter((clip) => {
    const clipPath = clipEvidencePath(clip);
    return /\.mp4$/i.test(clipPath) && fileExistsForEvidence([clipPath], artifactDir);
  });
  const tooFast = existingReadableClips.filter((clip) => {
    const minDuration = 12;
    const duration = Number(clip.durationS ?? clip.duration_s ?? clip.duration_seconds ?? 0);
    return !Number.isFinite(duration) || duration < minDuration;
  });
  const existingSourceCards = existingReadableClips.filter((clip) =>
    /(?:^|[_-])source(?:[_-]|$)/i.test(cleanText(
  ...
  const tooFastSourceCardIds = new Set(
    tooFast
      .filter((clip) => existingSourceCards.includes(clip))
      .map((clip) => cleanText(clip.id || clip.clip_id || clip.path))
      .filter(Boolean),
  );
  return {
    ready:
      manifestStatusReady(ownedManifest) &&
      existingReadableClips.length >= 3 &&
      tooFast.length === 0,
    source_card_ready:
      existingSourceCards.length > 0 &&
      existingSourceCards.every((clip) => !tooFastSourceCardIds.has(
        cleanText(clip.id || clip.clip_id || clip.path),
      )),
    source_card_clip_count: existingSourceCards.length,
    readable_card_clip_count: existingReadableClips.length,
    too_fast_readable_card_count: tooFast.length,
    min_duration_s: existingReadableClips.length
      ? Math.min(...existingReadableClips.map((clip) => Number(clip.durationS ?? clip.duration_s ?? clip.duration_seconds ?? 0)).filter(Number.isFinite))
      : null,
  };
}

function removeResolvedCurrentArtifactBlockers(blockers = new Set(), item = {}) {
  const next = new Set(blockers);
  const audio = currentPackageAudioEvidence(item);
  const motion = materialisedMotionEvidenceForPackage(item);
  const localRenderRights = localRenderRightsEvidence(item, motion);
  const hyperframes = readableHyperframesEvidenceForPackage(item);
  const rawCutoverBlockers = asArray(item.blockers || item.render_input_blockers)
    .map(cleanText)
    .filter(Boolean);
  const firstProductionRenderNeedsSourceCard = Boolean(
    cutoverNeedsFirstProductionRender(rawCutoverBlockers, item) &&
      forcedRerenderTargetManifest(item).hyperframes_premium_shell_required !== false &&
      motion.directMotionClipCount >= 5 &&
      !hyperframes.source_card_ready
  );

  if (audio.narration_ready) {
    next.delete("final_narration_audio_missing");
    next.delete("audio_manifest_missing");
  }
  if (audio.word_timestamps_ready) {
    next.delete("word_timestamps_missing");
  }
  if (audio.stale_after_script_rewrite) {
    next.add("final_narration_audio_stale_after_script_rewrite");
    next.add("word_timestamps_stale_after_script_rewrite");
  }
  if (motion.motionReady) {
    next.delete("materialised_motion_clips_missing");
    next.delete("materialised_motion_families_insufficient");
    if (motion.directMotionClipCount >= 5) {
      next.delete("real_visual_motion_clips_missing");
      next.delete("visual_evidence:no_real_visual_media_asset");
      next.delete("visual_evidence:direct_video_motion_missing");
    }
    if (motion.directMotionFamilyCount >= 4) {
      next.delete("real_visual_motion_families_insufficient");
    }
    if (motion.directMotionClipCount >= 5 && motion.directMotionFamilyCount >= 4) {
      next.delete("visual_evidence:insufficient_real_visual_source_families");
    }
  } else if (motion.partialRealMotionOverride) {
    next.add("materialised_motion_clips_missing");
  }
  const ownedExplainerException =
  ...
  if (motion.motionReady && !ownedExplainerException) {
    if (motion.directMotionClipCount === 0) {
      next.add("visual_evidence:no_real_visual_media_asset");
    } else if (motion.directMotionClipCount < 5 || motion.directMotionFamilyCount < 4) {
      next.add("visual_evidence:insufficient_real_visual_source_families");
    }
  }
  if (hyperframes.ready) {
    next.delete("hyperframes_readable_dwell_repair_required");
  }
  if (firstProductionRenderNeedsSourceCard) {
    next.add("hyperframes_readable_dwell_repair_required");
  }
  ...
  return {
    ...
    current_evidence: {
      ...audio,
      materialised_motion_ready: motion.motionReady,
      materialised_motion_clip_count: motion.clipCount,
      distinct_motion_family_count: motion.distinctFamilyCount,
      ...(motion.clipPaths.length ? { materialised_motion_clip_paths: motion.clipPaths } : {}),
      direct_video_motion_clip_count: motion.directMotionClipCount,
      direct_video_motion_family_count: motion.directMotionFamilyCount,
      distinct_motion_family_report_ready: motion.distinctMotionFamilyReportReady === true,
      distinct_motion_family_report_path: motion.distinctMotionFamilyReportPath || null,
      owned_generated_only_motion_deck: motion.ownedGeneratedOnly,
      partial_real_motion_override: motion.partialRealMotionOverride,
      partial_real_motion_status: motion.partialRealMotionStatus || null,
      local_render_rights: localRenderRights,
      readable_hyperframes_ready: hyperframes.ready ? true : null,
      readable_hyperframes_evidence_source: hyperframes.ready ? "current_owned_motion_manifest" : null,
      readable_hyperframes_source_card_ready: hyperframes.source_card_ready,
      readable_hyperframes_source_card_count: hyperframes.source_card_clip_count,
      readable_hyperframes_clip_count: hyperframes.readable_card_clip_count,
      readable_hyperframes_too_fast_count: hyperframes.too_fast_readable_card_count,
      readable_hyperframes_min_duration_s: hyperframes.min_duration_s,
      ...currentReadableHyperframesEvidenceOverrides(hyperframes),
    },
  };
}
...
function queueItemFromCutoverBlockedItem(item = {}) {
  ...
  for (const blocker of rawBlockers) {
    ...
    for (const missingInputBlocker of missingInputRenderBlockers(blocker)) {
      blockers.add(missingInputBlocker);
    }
  ...
  const resolved = removeResolvedCurrentArtifactBlockers(blockers, item);
  const resolvedBlockers = resolved.blockers;
  if (cutoverBlockedItemNeedsForcedRerender({ item, rawBlockers, blockers: resolvedBlockers })) {
    return {
      ...
      render_input_evidence: {
        ...
        ...currentPackageAudioEvidence(item),
        cutover_blockers: rawBlockers,
        repaired_package_input_freshness: repairedPackageInputsFreshness(item),
  ...
  if (!resolvedBlockers.size) return null;
  return {
    story_id: cleanText(item.story_id || item.id),
    title: cleanText(item.title || item.public_title || item.selected_title),
    artifact_dir: item.artifact_dir || null,
    ...
    render_input_status: "blocked",
    render_input_blockers: Array.from(resolvedBlockers),
    render_input_evidence: {
      ...(item.render_input_evidence || {}),
      ...resolved.current_evidence,
      cutover_blockers: rawBlockers,
...
function mapDryRunPreflightBlocker(blocker = "") {
  const code = cleanText(blocker);
  if (/^normal_production_duration_below_quality_floor:/i.test(code)) {
    return code;
  }
  const shortDurationMatch = code.match(/duration_too_short(?:_|\s*\()(\d+(?:\.\d+)?)s?/i);
  if (shortDurationMatch) {
    return `normal_production_duration_below_quality_floor:${Math.round(Number(shortDurationMatch[1]))}`;
  }
  if (/^audio_segment_loudness_report_stale_after_render$/i.test(code)) {
    return "audio_segment_loudness_report_stale_after_render";
  }
  if (/^voice_quality_report_stale_after_audio$/i.test(code)) {
    return "voice_quality_report_stale_after_audio";
  }
  if (/^voice_quality_report_stale_after_captions$/i.test(code)) {
    return "voice_quality_report_stale_after_captions";
  }
  if (/bridge_metadata_stale:/i.test(code)) {
    return "stale_qa_state";
  }
  ...
  if (/hyperframes:.*dwell|card_visible_dwell|director_card_dwell|rendered_card_window_dwell|card_clip_dwell/i.test(code)) {
    return "hyperframes_readable_dwell_repair_required";
  }
  if (/hyperframes:repeated_card_family|visual_evidence:repeated_card_family|readable_card_kind_repeated/i.test(code)) {
    return "hyperframes_card_family_repair_required";
  }
  if (/direct_motion_base_source_overused|repeated_direct_motion_segment/i.test(code)) {
    return "visual_motion_repeat_repair_required";
  }
...
function hasVisualDesignPolicyStale(codes = []) {
  return asArray(codes).some((code) => /(?:^|:)visual_design_policy_stale$/i.test(cleanText(code)));
}
...
function isVisualDesignPolicyRerenderOnly(codes = []) {
  const cleaned = asArray(codes).map(cleanText).filter(Boolean);
  if (!hasVisualDesignPolicyStale(cleaned)) return false;
  return cleaned.every((code) =>
    /(?:^|:)visual_design_policy_stale$/i.test(code) ||
    /^preflight_candidate_not_publish_ready:/i.test(code),
  );
}
...
function mapDryRunPreflightBlockers(rawBlockers = [], detailedFailures = []) {
  ...
  const visualDesignPolicyRerenderOnly = isVisualDesignPolicyRerenderOnly(codes);
  const aggregateVisualDirectorFailure = !visualDesignPolicyRerenderOnly && hasAggregateVisualDirectorFailure(codes);
  const mapped = [];
  if (visualDesignPolicyRerenderOnly) mapped.push("visual_design_policy_rerender_required");
  if (aggregateVisualDirectorFailure) mapped.push("aggregate_benchmark_repair_required");
  for (const code of codes) {
    if (visualDesignPolicyRerenderOnly && /(?:^|:)visual_design_policy_stale$/i.test(code)) {
      continue;
    }
    ...
    if (
      /approved_voice:.*(?:gta_vi_spoken_stutter|gta_vi_spoken_roman_split|gta_vi_opening_spoken_six_risk|non_native_rate_applied|tempo_stretch_applied|spoken_outro_misheard|spoken_outro_missing)/i.test(code) ||
      /content:approved_voice:.*(?:gta_vi_spoken_stutter|gta_vi_spoken_roman_split|gta_vi_opening_spoken_six_risk|non_native_rate_applied|tempo_stretch_applied|spoken_outro_misheard|spoken_outro_missing)/i.test(code)
    ) {
      mapped.push("final_narration_audio_stale_after_pronunciation_repair");
      mapped.push("word_timestamps_stale_after_pronunciation_repair");
      continue;
    }
    if (
...
function dryRunOnlyNeedsPublicCopyRerender(rawBlockers = [], detailedFailures = []) {
  const codes = [...new Set([...asArray(rawBlockers), ...asArray(detailedFailures)].map(cleanText).filter(Boolean))];
  return codes.length > 0 && codes.every((code) => /^public_copy_newer_than_render$/i.test(code));
}

function fileEvidenceHasFreshFinalInputs(fileEvidence = {}) {
  return fileEvidence.narration_ready === true &&
    fileEvidence.word_timestamps_ready === true &&
    fileEvidence.captions_ready === true &&
...
function queueItemFromDryRunBlockedStory(item = {}) {
  ...
  let blockers = [
    ...mapDryRunPreflightBlockers(rawBlockers, preflightFailures),
    ...blockersFromFileEvidence(evidence.file_evidence || {}),
  ];
  blockers = [...new Set(blockers)];
  const finalInputsFresh = fileEvidenceHasFreshFinalInputs(evidence.file_evidence || {});
  const publicCopyRerenderOnly = dryRunOnlyNeedsPublicCopyRerender(rawBlockers, preflightFailures) && finalInputsFresh;
  if (publicCopyRerenderOnly) {
    blockers = ["public_copy_newer_than_render"];
  }
  if (!blockers.length) return null;
  const storyId = cleanText(item.story_id || item.id);
  const artifactDir = item.artifact_dir || null;
  const visualPolicyRerender =
    blockers.length === 1 && blockers[0] === "visual_design_policy_rerender_required";
  const finalRenderRerender = publicCopyRerenderOnly || visualPolicyRerender;
  const bridgeFreshnessEvidence = item.scheduler_preflight?.checks?.bridge_artifact_freshness?.evidence || {};
  const targetRenderManifest = finalRenderRerender
    ? {
        output_path: artifactDir ? path.join(artifactDir, "visual_v4_render.mp4") : null,
        manifest_path:
          cleanText(bridgeFreshnessEvidence.render_manifest_path) ||
          (artifactDir ? path.join(artifactDir, "render_manifest.json") : null),
      }
    : null;
  return {
    ...
    artifact_dir: artifactDir,
    preflight_only: true,
    force_final_render: item.force_final_render === true || finalRenderRerender,
    target_render_manifest: targetRenderManifest,
    render_input_status: finalRenderRerender ? "ready_for_final_render_job" : "blocked",
    render_input_blockers: blockers,
    ...
    render_input_evidence: {
      ...
      ...(finalRenderRerender
        ? {
            narration_audio_path: cleanText(item.audio_path) || `output/audio/${storyId}.mp3`,
            word_timestamps_path: cleanText(item.timestamps_path) || `output/audio/${storyId}_timestamps.json`,
            ...(visualPolicyRerender ? { visual_design_policy_rerender_required: true } : {}),
            ...(publicCopyRerenderOnly ? { public_copy_rerender_required: true } : {}),
          }
        : {}),
...
function queueItemWithPublicCopyQa(item = {}) {
  ...
  const scriptScorecardQa = scriptScorecardQaForQueueItem(item, manifest);
  ...
  const publicCopyFailed = publicCopyQa.verdict === "fail";
  const scriptScorecardFailed = asArray(scriptScorecardQa.failures).length > 0;
  if (!publicCopyFailed && !scriptScorecardFailed) {
    const resolvedBlockers = asArray(item.render_input_blockers)
      .map(cleanText)
      .filter((blocker) =>
        blocker &&
        ![
        ...
        !blocker.startsWith("public_copy:"),
      );
    return {
      ...item,
      title,
      render_input_blockers: resolvedBlockers,
      render_input_evidence: {
        ...(item.render_input_evidence || {}),
        public_copy_qa: publicCopyQa,
        script_scorecard_qa: scriptScorecardQa,
        public_copy_repair_evidence_source: "current_canonical_story_manifest",
      },
    };
  }
  const blockers = new Set(asArray(item.render_input_blockers));
  if (publicCopyFailed) blockers.add("public_copy_repair_required");
  if (scriptScorecardFailed) blockers.add("script_scorecard_repair_required");
  if (publicCopyFailed && asArray(publicCopyQa.failures).some((failure) =>
    [
      "public_copy:reddit_discovery_label_used_as_primary_source",
      "public_copy:non_news_image_post_source",
      "public_copy:platform_host_source_label",
...
function buildGoalRenderInputWorkOrder({
  ...
  ownedMotionOnlyValidator = defaultOwnedMotionOnlyValidator,
  ownedMotionOnlyTargetPlatforms = ["youtube_shorts", "instagram_reels", "facebook_reels"],
  generatedAt = new Date().toISOString(),
} = {}) {
  cutoverPlan = cutoverPlanFromStoryPackagesIfNeeded(cutoverPlan);
  ...
  const jobs = queue
    .map((item) => jobForQueueItem(item, context))
    .map((job) => promoteGovernedOwnedMotionOnlyRenderJob(job, {
      enabled: rightsSafeOwnedMotionOnly,
      validator: ownedMotionOnlyValidator,
      targetPlatforms: ownedMotionOnlyTargetPlatforms,
    }));
  ...
  const audioSegmentQaRefreshJobs = jobs.filter((job) =>
    job.actions.some((action) => action.action_id === "refresh_audio_segment_loudness_qa"),
  );
  const narrationQaRefreshJobs = jobs.filter((job) =>
    job.actions.some((action) => action.action_id === "refresh_narration_voice_quality_qa"),
  );
  ...
  const schedulerPreflightReadyJobs = jobs.filter((job) => job.status === "ready_for_scheduler_preflight");
  ...
  return {
    ...
    summary: {
      ...
      ready_for_scheduler_preflight_count: schedulerPreflightReadyJobs.length,
      ...
      audio_segment_qa_refresh_jobs: audioSegmentQaRefreshJobs.length,
      narration_qa_refresh_jobs: narrationQaRefreshJobs.length,
...
function renderGoalRenderInputWorkOrderMarkdown(workOrder = {}) {
  ...
  lines.push(`Ready for scheduler preflight: ${workOrder.summary?.ready_for_scheduler_preflight_count || 0}`);
  ...
  lines.push(`Audio segment QA refresh jobs: ${workOrder.summary?.audio_segment_qa_refresh_jobs || 0}`);
  lines.push(`Narration QA refresh jobs: ${workOrder.summary?.narration_qa_refresh_jobs || 0}`);
...
function rewriteCommandOutputPaths(value, { outputDir } = {}) {
  if (!outputDir) return value;
  const outDir = commandPath(outputDir);
  const workOrderPath = `${outDir}/render_input_work_order.json`;
  const rewriteText = (text = "") =>
    String(text)
      .replaceAll("output/goal-contract/render_input_work_order.json", workOrderPath)
      .replaceAll("--out-dir output/goal-contract", `--out-dir ${outDir}`);

  if (typeof value === "string") return rewriteText(value);
  if (Array.isArray(value)) return value.map((item) => rewriteCommandOutputPaths(item, { outputDir }));
  if (value && typeof value === "object") {
    return Object.fromEntries(
      Object.entries(value).map(([key, item]) => [
        key,
        rewriteCommandOutputPaths(item, { outputDir }),
      ]),
    );
  }
  return value;
...
async function writeGoalRenderInputWorkOrder(workOrder = {}, { outputDir } = {}) {
  ...
  const normalizedWorkOrder = rewriteCommandOutputPaths(workOrder, { outputDir: outDir });
  ...
  await fs.writeJson(jsonPath, normalizedWorkOrder, { spaces: 2 });
  await fs.writeJson(
    repairBacklogPath,
    normalizedWorkOrder.repair_backlog || buildRepairBacklog(normalizedWorkOrder.jobs),
    { spaces: 2 },
  );
  await fs.writeJson(
    autoRepairPlanPath,
    normalizedWorkOrder.auto_repair_plan || buildAutoRepairPlan(normalizedWorkOrder.repair_backlog),
    { spaces: 2 },
  );
  await fs.writeJson(
    postRepairValidationPlanPath,
    normalizedWorkOrder.post_repair_validation_plan || buildPostRepairValidationPlan(normalizedWorkOrder.repair_backlog),
    { spaces: 2 },
  );
  await fs.writeFile(markdownPath, renderGoalRenderInputWorkOrderMarkdown(normalizedWorkOrder), "utf8");
