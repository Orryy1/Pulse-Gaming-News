...
function materialisedMotionEvidenceForPackage(item = {}) {
  ...
  const directFamilyValues = directClips
    ...
    .filter(Boolean);
  const directMotionFamilyCount = positiveMax(
    evidenceManifest.direct_video_motion_family_count,
    evidenceManifest.direct_video_motion_families,
    new Set(directFamilyValues).size,
  );
  const directMotionClipCount = positiveMax(
    evidenceManifest.direct_video_motion_asset_count,
    evidenceManifest.direct_video_motion_clip_count,
    directClips.length,
  );
  const clipCount = positiveMax(evidenceManifest.clip_count, evidenceManifest.materialised_motion_clip_count, localClips.length);
  const distinctFamilyCount = positiveMax(
...
function actionForReadableHyperframesMotionRepair(item = {}) {
  ...
  return {
    ...
    exact_missing_input: "fresh owned HyperFrames/source-card motion clips with a 6.5s+ readable dwell floor",
    ...
    evidence: {
      minimum_readable_card_duration_s:
        Number(fileEvidence.minimum_readable_card_duration_s || 0) || 6.5,
    ...
    output_expectations: [
      ...
      "each readable card clip has durationS >= 6.5 unless its own readability contract requires longer",
...
function jobForQueueItem(item = {}, context = {}) {
  ...
  const readyForFinalRender =
    (
      renderInputStatus === "ready_for_final_render_job" ||
      (item.force_final_render === true && blockers.length === 0) ||
...
function readableHyperframesEvidenceForPackage(item = {}) {
  ...
  const tooFast = existingReadableClips.filter((clip) => {
    const minDuration = /proof/i.test(cleanText(clip.asset_class || clip.visual_purpose || clip.id)) ? 8 : 6.5;
