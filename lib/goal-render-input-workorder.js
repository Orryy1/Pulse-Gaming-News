...
function actionForReadableHyperframesMotionRepair(item = {}) {
  ...
  return {
    ...
    exact_missing_input: "fresh owned HyperFrames/source-card motion clips with a 10.5s+ readable dwell floor",
...
function jobForQueueItem(item = {}, context = {}) {
  ...
  if (hasAny(blockers, ["visual_motion_repeat_repair_required"]) && !hasTerminalPublicCopyBlocker && !hasMotionMaterialisationAction()) {
    pushUniqueAction(actions, actionForRepeatedDirectMotionRepair(item));
  }
  ...
  if (hasAny(blockers, ["hyperframes_readable_dwell_repair_required"]) && !hasTerminalPublicCopyBlocker) {
    pushUniqueAction(actions, actionForReadableHyperframesMotionRepair(item));
  }
  if (hasAny(blockers, [
...
function queueItemFromDryRunBlockedStory(item = {}) {
  ...
  let blockers = mapDryRunPreflightBlockers(rawBlockers, preflightFailures);
