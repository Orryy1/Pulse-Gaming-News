...
function materialisedMotionEvidenceForPackage(item = {}) {
  ...
  const manifestTime = timeMs(manifest.generated_at) || artefactModifiedTimeMs(item, "materialised_motion_clips.json");
  const partialTime = timeMs(partial.generated_at) || artefactModifiedTimeMs(item, "partial_real_motion_evidence.json");
...
function currentPackageAudioEvidence(item = {}) {
  ...
  return {
    ...
    narration_ready: Boolean(narrationAudioPath),
    word_timestamps_ready: Boolean(wordTimestampsPath),
...
function removeResolvedCurrentArtifactBlockers(blockers = new Set(), item = {}) {
  ...
  return {
    ...
    current_evidence: {
      ...
      readable_hyperframes_ready: hyperframes.ready,
