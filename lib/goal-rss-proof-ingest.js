...
function proofScriptForItem(item = {}) {
  ...
  const claim = ADVERTISER_UNFRIENDLY_PUBLIC_RE.test(title)
    ...
    : title;
  return cleanText(
    `${subject} finally has something concrete to judge. ` +
      `${source} says ${claim}. ` +
      "The next real test is what changes on screen, in the store or in the platform plan. " +
      "If the official page moves, the angle moves with it. " +
      `${PRIMARY_PULSE_CTA}.`,
  );
}
