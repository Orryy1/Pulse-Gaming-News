...
function subjectForItem(item = {}) {
  ...
  const fallbackSubject = titleSubjectFallback(item.title);
  const manifestTokens = manifestSubject
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, " ")
    .trim()
    .split(/\s+/)
    .filter(Boolean);
  const fallbackTokens = fallbackSubject
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, " ")
    .trim()
    .split(/\s+/)
    .filter(Boolean);
  const manifestSubjectIsTruncated =
    manifestTokens.length > 0 &&
    fallbackTokens.length > manifestTokens.length &&
    manifestTokens.every((token, index) => fallbackTokens[index] === token);
      ...
      .map((candidate) => {
        ...
        return {
          ...
          source_family:
            ...
            `rss_video_enclosure_${sourceFamilySlug(item.source_name)}_${sourceFamilySlug(subject)}`,
          source_url: item.url,
          title: item.title,
