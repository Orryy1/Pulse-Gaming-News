...
const CREATOR_STUDIO_PROVIDER_NAMES = new Map(
  creatorStudioSfxSourceRegistry().map((provider) => [
    cleanText(provider.provider_id).toLowerCase(),
    cleanText(provider.name),
  ]),
);

function providerAttribution(asset = {}) {
  const providerId = cleanText(asset.provider_id || asset.provider);
  const registeredProviderName = CREATOR_STUDIO_PROVIDER_NAMES.get(providerId.toLowerCase()) || "";
  const providerName = cleanText(asset.provider_name || asset.source_owner || asset.creator || registeredProviderName);
  const sourceOwner = cleanText(asset.source_owner || asset.creator || providerName);
  return {
...
function mergeMissingRecordFields(existing = {}, incoming = {}) {
  const merged = { ...existing };
  for (const [key, value] of Object.entries(incoming)) {
    if (!hasRecordValue(merged[key]) && hasRecordValue(value)) merged[key] = value;
  }
  return merged;
}
...
async function discoverPackageEntries({ root = process.cwd(), storyPackagesPath = null, packageRoot = null } = {}) {
  if (storyPackagesPath || !packageRoot) {
  ...
  const isExactPackage =
    (await fs.pathExists(path.join(dir, "canonical_story_manifest.json"))) ||
    (await fs.pathExists(path.join(dir, "audio_manifest.json"))) ||
    (await fs.pathExists(path.join(dir, "render_manifest.json")));
  if (isExactPackage) {
    const canonical = (await readJsonIfPresent(path.join(dir, "canonical_story_manifest.json"), {})) || {};
    const render = (await readJsonIfPresent(path.join(dir, "render_manifest.json"), {})) || {};
    const audio = (await readJsonIfPresent(path.join(dir, "audio_manifest.json"), {})) || {};
    const storyId = cleanText(
      canonical.story_id ||
        canonical.id ||
        canonical.story?.story_id ||
        canonical.story?.id ||
        render.story_id ||
        render.id ||
        audio.story_id ||
    ...
    return storyId ? [{ story_id: storyId, artifact_dir: dir }] : [];
  }
...
function mergeRightsRecords(existing = {}, sfxRights = {}) {
  ...
  for (const record of existingRecords) {
    ...
    byId.set(key, mergeMissingRecordFields(byId.get(key), record));
  }
  for (const record of rightsLedgerEntries(sfxRights)) {
    const key = cleanText(record.asset_id || record.id || record.path || record.source_url);
    if (!key) continue;
    byId.set(key, byId.has(key) ? mergeMissingRecordFields(byId.get(key), record) : record);
  }
  return {
...
function selectedAssetEvidenceKeys(value = {}) {
  return asArray(value.selected_assets || value.source_plan?.selected_assets)
    .map((asset) => {
      return [
        cleanText(asset.asset_id || asset.id),
        roleForAsset(asset),
        cleanText(asset.provider_id || asset.provider),
        cleanText(asset.provider_name),
        cleanText(asset.source_owner || asset.creator),
        cleanText(asset.source_url || asset.path || asset.file_path),
      ].join("|");
    })
    .filter(Boolean)
    .sort();
...
function sameSelectedAssets(a = {}, b = {}) {
  const left = selectedAssetEvidenceKeys(a);
  const right = selectedAssetEvidenceKeys(b);
...
function rightsRecordKeys(value = {}) {
  ...
  return records
    .map((record) => [
      ...
      cleanText(record.provider_id || record.provider),
      cleanText(record.provider_name),
      cleanText(record.source_owner),
      cleanText(record.creator),
    ].join("|"))
    .sort();
}
...
function normaliseSfxAsset(asset = {}, { packageScoped = false } = {}) {
  ...
  const role = roleForAsset(asset);
  const attribution = providerAttribution(asset);
...
function lockRenderedSfxSourcePlan({
  plannedSourcePlan = {},
  ...
  const required = asArray(requiredRoles).map(cleanText).filter(Boolean);
  const selectedByRole = new Map(
    asArray(existingSelectedAssets)
      .map((asset) => [roleForAsset(asset), asset])
      .filter(([role]) => role && required.includes(role)),
  );
  if (!required.length || required.some((role) => !selectedByRole.has(role))) return plannedSourcePlan;

  const rightsAssets = dedupeSfxAssets([
    ...rightsLedgerEntries(packageRightsLedger),
    ...rightsLedgerEntries(sfxRightsLedger),
  ]);
  const rightsById = new Map(
    rightsAssets
      .map((asset) => [cleanText(asset.asset_id || asset.id), asset])
      .filter(([id]) => id),
  );
  const rightsBySource = new Map(
    rightsAssets
      .map((asset) => [cleanText(asset.source_url || asset.path || asset.file_path), asset])
      .filter(([source]) => source),
  );
  const locked = [];
  for (const role of required) {
    const selected = selectedByRole.get(role);
    const selectedId = cleanText(selected.asset_id || selected.id);
    const selectedSource = cleanText(selected.source_url || selected.path || selected.file_path);
    const donor = rightsById.get(selectedId) || rightsBySource.get(selectedSource);
    if (!donor || !isApprovedSfxAsset(donor)) return plannedSourcePlan;
    locked.push(normaliseSfxAsset(mergeMissingRecordFields(selected, donor), { packageScoped: true }));
  }
  const selectedSignature = selectedSfxSignature(locked);
  return {
    ...plannedSourcePlan,
    required_roles: required,
    covered_roles: Array.from(new Set(locked.map(roleForAsset))).sort(),
    selected_assets: locked,
    anti_repetition: {
      ...(plannedSourcePlan.anti_repetition || {}),
      selected_signature: selectedSignature,
      selection_lock: "materialised_render",
    },
    readiness: {
      ...(plannedSourcePlan.readiness || {}),
      warnings: [
        ...new Set([
          ...asArray(plannedSourcePlan.readiness?.warnings),
          "sfx_source:post_render_selection_locked",
        ]),
      ],
    },
  };
...
function syncSfxAssetAttribution(assets = [], selectedAssets = []) {
  ...
  return asArray(assets).map((asset) => {
    const id = cleanText(asset.asset_id || asset.id);
    const source = cleanText(asset.source_url || asset.path || asset.file_path);
    const donor = selectedById.get(id) || selectedBySource.get(source);
    if (!donor) return asset;
    return mergeMissingRecordFields(asset, providerAttribution(donor));
  });
}

function syncSfxSourcePlanAttribution(sourcePlan, selectedAssets) {
  if (!sourcePlan || typeof sourcePlan !== "object" || Array.isArray(sourcePlan)) return sourcePlan;
  if (!asArray(sourcePlan.selected_assets).length) return sourcePlan;
  return {
    ...sourcePlan,
    selected_assets: syncSfxAssetAttribution(sourcePlan.selected_assets, selectedAssets),
  };
}

function syncSoundTransitionPlanAttribution(plan, selectedAssets) {
  if (!plan || typeof plan !== "object" || Array.isArray(plan)) return plan;
  if (!plan.sfx || typeof plan.sfx !== "object" || Array.isArray(plan.sfx)) return plan;
  return {
    ...plan,
    sfx: {
      ...plan.sfx,
      source_plan: syncSfxSourcePlanAttribution(plan.sfx.source_plan, selectedAssets),
    },
  };
}

function syncRenderStorySfxAttribution(renderStory = {}, selectedAssets = []) {
  const updated = {
    ...renderStory,
    ...(asArray(renderStory.sfx_asset_inventory).length
      ? { sfx_asset_inventory: syncSfxAssetAttribution(renderStory.sfx_asset_inventory, selectedAssets) }
      : {}),
    ...(renderStory.sfx_source_plan
      ? { sfx_source_plan: syncSfxSourcePlanAttribution(renderStory.sfx_source_plan, selectedAssets) }
      : {}),
    ...(renderStory.sfx_manifest && typeof renderStory.sfx_manifest === "object"
      ? {
        sfx_manifest: {
          ...renderStory.sfx_manifest,
          selected_assets: syncSfxAssetAttribution(renderStory.sfx_manifest.selected_assets, selectedAssets),
          source_plan: syncSfxSourcePlanAttribution(renderStory.sfx_manifest.source_plan, selectedAssets),
        },
      }
      : {}),
    ...(renderStory.sound_transition_plan
      ? {
        sound_transition_plan: syncSoundTransitionPlanAttribution(
          renderStory.sound_transition_plan,
          selectedAssets,
        ),
      }
  ...
  if (renderStory.visual_v4_director_plan && typeof renderStory.visual_v4_director_plan === "object") {
    const director = renderStory.visual_v4_director_plan;
    updated.visual_v4_director_plan = {
      ...director,
      sound_transition_plan: syncSoundTransitionPlanAttribution(
        director.sound_transition_plan,
        selectedAssets,
      ),
      ...(director.sfx_plan && typeof director.sfx_plan === "object"
        ? {
          sfx_plan: {
            ...director.sfx_plan,
            source_plan: syncSfxSourcePlanAttribution(director.sfx_plan.source_plan, selectedAssets),
          },
        }
        : {}),
    };
  ...
  return {
    story: updated,
    changed: JSON.stringify(updated) !== JSON.stringify(renderStory),
  };
}

async function materialisedRenderExists(artifactDir, renderManifest = {}) {
  const declaredOutput = cleanText(renderManifest.output_path || renderManifest.output);
  const candidates = [
    declaredOutput
      ? (path.isAbsolute(declaredOutput) ? declaredOutput : path.join(artifactDir, declaredOutput))
      : "",
  ...
  for (const candidate of new Set(candidates)) {
    try {
      ...
      // Missing output means the package is still safe for pre-render variant rotation.
    }
  }
  return false;
}
...
function buildStorySfxManifest({ storyId, sourcePlan = {}, audioManifest = {}, generatedAt } = {}) {
  ...
  return {
    ...
    selected_assets: selectedAssets.map((asset) => ({
      ...
      provider_name: cleanText(asset.provider_name),
      source_owner: cleanText(asset.source_owner || asset.creator || asset.provider_name),
...
async function repairPackageSfxEvidence({
  ...
  const renderStoryPath = path.join(artifactDir, "visual_v4_render_story.json");
  ...
  const existingSfxSourcePlan = (await readJsonIfPresent(sfxSourcePlanPath, {})) || {};
  ...
  const renderStory = (await readJsonIfPresent(renderStoryPath, {})) || {};
  const requiredRoles = activeRenderRequiredRolesFromDirector(director);
  const plannedStorySourcePlan = buildStoryVariantSourcePlan({
    storyId,
    sourcePlan,
    sfxRightsLedger,
  ...
  const renderMaterialised = await materialisedRenderExists(artifactDir, renderManifest);
  const storySourcePlan = renderMaterialised
    ? lockRenderedSfxSourcePlan({
      plannedSourcePlan: plannedStorySourcePlan,
      existingSelectedAssets:
        existingSfxSourcePlan.selected_assets ||
        existingSfxManifest.source_plan?.selected_assets ||
        existingSfxManifest.selected_assets,
      packageRightsLedger: originalRights,
      sfxRightsLedger,
      requiredRoles,
    })
    : plannedStorySourcePlan;
  ...
  if (renderMaterialised && Object.keys(renderStory).length) {
    const syncedRenderStory = syncRenderStorySfxAttribution(renderStory, storySourcePlan.selected_assets);
    if (syncedRenderStory.changed) {
      writes.push({ path: renderStoryPath, data: syncedRenderStory.story });
    }
  }
