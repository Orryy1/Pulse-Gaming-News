...
function buildStoryVariantSourcePlan({
  ...
  for (const role of requiredRoles) {
    ...
    const packageScopedRoleRecords = roleRecords.filter((asset) => asset.package_scope === "story_package");
    const candidateBasis = packageScopedRoleRecords.length ? packageScopedRoleRecords : roleRecords;
    const rankedCandidates = candidateBasis
      .sort((a, b) =>
    ...
    const candidates = rankedCandidates
      .filter((asset) => sfxSelectionScore(asset) >= topTierFloor)
      .slice(0, 5);
