...
function resolveArtifactEvidencePath(filePath, artifactDir, workspaceRoot = process.cwd()) {
  const text = cleanText(filePath);
  if (!text || path.isAbsolute(text)) return resolveWorkspacePath(text, workspaceRoot);
  const packageRelative = path.resolve(artifactDir, text);
  if (fs.existsSync(packageRelative)) return packageRelative;
  return resolveWorkspacePath(text, workspaceRoot);
}
...
async function buildJobForCandidate(candidate = {}, {
  ...
  const narrationAudioPath = resolveArtifactEvidencePath(cleanText(
  ...
  const wordTimestampsPath = resolveArtifactEvidencePath(cleanText(
