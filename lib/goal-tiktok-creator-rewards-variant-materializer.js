...
function isCoopParanoiaStory(canonical = {}) {
  const text = canonicalEvidenceText(canonical);
  return /\b(?:co[-\s]?op|team|group|voice chat)\b/i.test(text) &&
    /\b(?:madness system|paranoia|distorts? what players see and hear|hallucinations?|swears? they saw)\b/i.test(text);
}
...
function isAntiCheatTrustStory(canonical = {}) {
  const text = canonicalEvidenceText(canonical);
  return /\b(?:valorant|vanguard|riot)\b/i.test(text) &&
    /\b(?:anti[-\s]?cheat|cheaters?|bricking|paperweights?|trust|control)\b/i.test(text);
}

function isHandmadeAiArtDirectionStory(canonical = {}) {
  const text = canonicalEvidenceText(canonical);
  return /\b(?:pragmata|ai[-\s]?looking|ai generated|handmade|human developers|art direction|new york stage)\b/i.test(text) &&
    /\b(?:handmade|human developers|built by hand|art direction)\b/i.test(text) &&
    /\b(?:ai[-\s]?looking|ai generated|machine shortcut|uncanny)\b/i.test(text);
}
...
function isSquadTacticsStory(canonical = {}) {
  const text = canonicalEvidenceText(canonical);
  return /\b(?:xcom|turn[-\s]?based tactics|squad|permadeath|mass effect|crew losses|between missions)\b/i.test(text) &&
    /\b(?:star wars|zero company|tactics|permadeath)\b/i.test(text);
}

function minimumWordsForTiktokCreatorRewards(canonical = {}, script = "") {
  if (isCoopParanoiaStory(canonical)) return 175;
  ...
  if (isAntiCheatTrustStory(canonical)) return 190;
  if (isHandmadeAiArtDirectionStory(canonical)) return 175;
  if (isPokemonGoEventStory(canonical)) return 210;
  if (isSquadTacticsStory(canonical)) return 195;
  if (isReviewScoreStory(canonical)) return 185;
  return Math.max(165, Math.min(180, Math.max(115, wordCount(script) - 15)));
}
...
function tiktokLongContextSentences(canonical = {}) {
  ...
  if (isCoopParanoiaStory(canonical)) {
    return [
      "Voice chat becomes part of the level design because one player's warning can be real to them and invisible to everyone else.",
      "That creates a sharper choice than a scripted scare: waste supplies checking, split the group or ignore the one warning that was real.",
      "The make-or-break detail is consistency; hallucinations need rules players can learn without becoming predictable.",
      "If every scare feels random, teams stop trusting the game instead of distrusting each other.",
      "If the system stays fair, every confident call becomes suspicious and every ignored warning can become a story the group remembers.",
      "A calm team can beat a brave one, because panic is exactly what the system is trying to create.",
      source ? `${source} confirms the mechanic, so the longer cut should test what it changes between players rather than padding the launch date.` : "",
    ];
  }
  ...
  if (isDateLeakStory(canonical)) {
    return [
      ...
      "The next real proof is the official date itself.",
  ...
  if (isSteamLaunchAccessStory(canonical)) {
    return [
      ...
      "Steam's review graph will show whether that opening momentum lasts.",
  ...
  if (isCorporateStakeStory(canonical)) {
    return [
      ...
      "This matters because it changes the power map around a gaming name.",
      source ? `${source} anchors the stake percentage and investor ranking.` : "",
    ];
  }
  ...
  if (isRetailDiscountStory(canonical)) {
    return [
      ...
      "That keeps the TikTok version useful without pressuring anyone to buy.",
      ...
      "The fifteen-dollar figure is a useful benchmark if the listing moves.",
  ...
  if (isAntiCheatTrustStory(canonical)) {
    return [
      `The real player question is not whether Riot should punish cheaters; it is how much trust ${subject} gets while it runs this deep.`,
      "For cheaters, Riot's line is funny. For regular players, the uncomfortable part is that an anti-cheat story can turn into a software-control story fast.",
      "Competitive integrity needs Vanguard to work, but player confidence needs the software staying explainable when something goes wrong.",
      "If Riot proves this only hit cheat boxes, the joke lands cleaner.",
      "If ordinary players keep worrying about edge cases, the trust argument stays alive.",
      source ? `${source} anchors the report, so the longer cut should stay on the cheat-device claim and the wider trust argument.` : "",
    ];
  }
  if (isHandmadeAiArtDirectionStory(canonical)) {
    return [
      `${subject} is an art-direction story because the AI look was built by people, not generated by a prompt.`,
      "That makes the weird city texture feel less like a shortcut and more like a deliberate aesthetic choice.",
      "Players are not just judging whether it looks strange; they are judging whether that strangeness pays off once the level is playable.",
      `For ${subject}, that matters because the long wait has turned every new detail into a proof point.`,
      "If the handmade weirdness carries into puzzles, combat and traversal, the stage becomes a signature instead of a screenshot argument.",
      "If it does not, the AI-look debate can swallow the actual game again.",
      source ? `${source} names the handmade stage detail; the player question is whether the full mission turns that idea into something playable.` : "",
    ];
  }
  ...
  if (isSquadTacticsStory(canonical)) {
    return [
      "The squad layer is the pressure point, because a tactics game dies fast if choices only matter during the fight.",
      "The Mass Effect comparison starts to make sense only if the crew, downtime and consequences survive between missions.",
      "Permadeath matters here because it can turn a clean win into a story you cannot simply reload away.",
      "If losses follow the squad back to base, the licence becomes more than a skin over XCOM.",
      "If that layer is thin, Star Wars Zero Company risks feeling like a familiar tactics pitch with a bigger logo.",
      "That is the real curiosity gap: whether the drama around the team is strong enough to make every mission feel costly.",
      "Players do not just need another grid battle; they need a reason to care who makes it out.",
      source ? `${source} gives the comparison, but the long cut has to keep the argument on squad pressure and consequences.` : "",
    ];
  }
  return [
...
function contextSentenceWouldBloatNearTarget(
  currentText = "",
  cta = "",
  sentence = "",
  minWords = 0,
) {
  const currentWords = wordCount(`${currentText} ${cta}`);
  if (currentWords < Number(minWords) - 5) return false;
  return wordCount(`${currentText} ${sentence} ${cta}`) > Number(minWords) + 5;
}
...
function expandScriptForLocalTiktokCreatorRewards(script = "", canonical = {}, { minWords = 165 } = {}) {
  ...
  for (const sentence of tiktokLongContextSentences(canonical)) {
    ...
    if (
      contextSentenceWouldBloatNearTarget(
        nextBody,
        cta,
        cleanSentence,
        minWords,
...
function unsafeTiktokVariantPublicSentence(sentence = "", canonical = {}) {
  ...
  if (/\b(?:one concrete change worth remembering|clean shape:\s*what changed|source visible and no extra lore|players have to judge the footage|finally has footage players can judge|clip puts the pitch on screen|one reveal cannot settle|longer play section|release timing and platform detail turn curiosity|at phone speed|movement,\s*interface pressure|details keep showing up|footage has to make it clear|direct play segment|combat rhythm|camera weight)\b/i.test(text)) {
    return true;
  }
...
async function restoreVariantPublicCopyAfterAudio({ variantDir = "", canonical = {} } = {}) {
  const canonicalPath = path.join(variantDir, "canonical_story_manifest.json");
  const audioCanonical = await readJsonIfPresent(canonicalPath, {});
  const displayScript = cleanText(canonical.narration_script || canonical.full_script);
  const spokenScript = cleanText(
    audioCanonical.spoken_narration_script ||
      audioCanonical.local_tts_spoken_script ||
      canonical.spoken_narration_script ||
      canonical.local_tts_spoken_script,
  );
  const updated = {
    ...audioCanonical,
    ...
    caption_display_text: displayScript,
...
async function materializeTiktokCreatorRewardsJob(job = {}, options = {}) {
  ...
  await restoreVariantPublicCopyAfterAudio({
    variantDir,
    canonical: variantCanonical.canonical,
  });
