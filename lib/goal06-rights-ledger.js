...
} = require("./stabilisation/originality-disclosure-contract");
...
function lowerPlatformList(value) {
  return unique(asArray(value).map(platformRightsFamily).filter(Boolean));
}
...
function normaliseAsset(raw = {}, index = 0, fallbackKind = "asset") {
  ...
  return {
    ...
    asset_sha256: normaliseSha256(asset.asset_sha256 || asset.sha256 || asset.content_hash),
  };
}
...
function normaliseRecord(raw = {}, index = 0) {
  ...
  return {
    ...
    asset_sha256: normaliseSha256(record.asset_sha256 || record.sha256 || record.content_hash),
  };
}
...
function collectLedgerAssets(ledger = {}) {
  const hasExplicitUsedAssets = Object.prototype.hasOwnProperty.call(ledger, "used_assets");
  return dedupeByKey(
    [
      ...asArray(hasExplicitUsedAssets ? ledger.used_assets : ledger.assets),
      ...asArray(ledger.missing_assets),
      ...(hasExplicitUsedAssets
        ? []
...
function rightsRecordMatchScore(record = {}, asset = {}) {
  ...
  const assetSourceUrl = normaliseText(asset.source_url);
  const recordIds = [record.asset_id, record.id, record.clip_id, record.image_id, record.rights_record_id]
    .map(normaliseText)
    .filter(Boolean);
  const recordPaths = [record.path, record.local_path, record.file, record.output_path]
    .map(normaliseText)
    .filter(Boolean);
  const recordSourceUrls = [record.source_url, record.url].map(normaliseText).filter(Boolean);
...
function assignRightsRecords(assets = [], records = []) {
  const assignments = new Map();
  const assignedRecords = new Set();
  const candidates = assets.flatMap((asset, assetIndex) =>
  ...
  for (const candidate of candidates) {
    if (assignments.has(candidate.assetIndex) || assignedRecords.has(candidate.recordIndex)) continue;
    assignments.set(candidate.assetIndex, candidate.record);
    assignedRecords.add(candidate.recordIndex);
  }
  return assignments;
}

function finalUsedHashBlockers(asset = {}, record = {}) {
  const assetSha256 = normaliseSha256(asset.asset_sha256);
  const recordSha256 = normaliseSha256(record.asset_sha256);
  const blockers = [];
  if (!assetSha256) blockers.push("rights:used_asset_hash_missing");
  else if (!SHA256_RE.test(assetSha256)) blockers.push("rights:used_asset_hash_invalid");
  if (!recordSha256) blockers.push("rights:rights_record_hash_missing");
  else if (!SHA256_RE.test(recordSha256)) blockers.push("rights:rights_record_hash_invalid");
  if (SHA256_RE.test(assetSha256) && SHA256_RE.test(recordSha256) && assetSha256 !== recordSha256) {
    blockers.push("rights:asset_hash_mismatch");
  }
  return blockers;
}
...
async function inspectStoryPackage(storyPackage = {}, options = {}) {
  ...
  const recordAssignments = assignRightsRecords(assets, records);
  const requiresFinalUsedHashes = Object.prototype.hasOwnProperty.call(ledger, "used_assets");
  const invalidAssignedAssets = new Set();
  const originalityDisclosureManifest =
    canonical?.originality_disclosure_manifest ||
    ledger?.originality_disclosure_manifest ||
    {};
  const originalityDisclosure = validateOriginalityAndDisclosure(
    originalityDisclosureManifest,
  );

  if (!assets.length) blockers.push("rights:used_asset_inventory_missing");
  if (!records.length) blockers.push("rights:rights_record_inventory_missing");
  blockers.push(...originalityDisclosure.blockers);
  ...
  for (const [assetIndex, asset] of assets.entries()) {
    const matchedRecord = recordAssignments.get(assetIndex);
    const explicitlyMissing = assetKeyParts(asset).map(normaliseText).some((key) => missingKeys.has(key));
    const assetBlockers = !matchedRecord || explicitlyMissing
      ? unique([
        "rights:no_rights_record",
        ...(explicitlyMissing ? ["rights:declared_missing_asset"] : []),
      ])
      : requiresFinalUsedHashes
        ? finalUsedHashBlockers(asset, matchedRecord)
        : [];
    if (assetBlockers.length) {
      if (matchedRecord) invalidAssignedAssets.add(assetIndex);
      ...
      rejectedAssets.push(
        rejectionForAsset({
          ...
          requiredAction: matchedRecord && !explicitlyMissing
            ? "Reconcile the rights record against the exact final-used asset SHA-256."
            : "Attach a rights record with licence basis, platform scope, commercial permission and evidence for this asset.",
        }),
      );
    }
  }
  ...
  const matchedAssetCount = recordAssignments.size - invalidAssignedAssets.size;
  return {
    story_id: storyId || "unknown",
    title,
    artifact_dir: artifactDir || null,
    ...
    originality_and_transformation_verdict:
      cleanText(
        originalityDisclosureManifest.originality_and_transformation_verdict,
      ) || null,
    synthetic_disclosure: {
      required:
        typeof originalityDisclosureManifest.synthetic_disclosure_required ===
        "boolean"
          ? originalityDisclosureManifest.synthetic_disclosure_required
          : null,
      operator_decision:
        cleanText(originalityDisclosureManifest.operator_decision) || null,
      youtube_field_value:
        typeof originalityDisclosureManifest.youtube_field_value === "boolean"
          ? originalityDisclosureManifest.youtube_field_value
          : null,
      reviewed_at:
        cleanText(originalityDisclosureManifest.reviewed_at) || null,
    },
    originality_disclosure_status: originalityDisclosure.status,
...
function buildRightsLedgerManifest(report = {}) {
  return {
    ...
    stories: asArray(report.stories).map((story) => ({
      ...
      originality_and_transformation_verdict:
        story.originality_and_transformation_verdict || null,
      originality_disclosure_status:
        story.originality_disclosure_status || "blocked",
      synthetic_disclosure: story.synthetic_disclosure || null,
      blockers: story.blockers || [],
    })),
