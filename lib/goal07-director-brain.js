...
// Goal07 audits V4 director output, whose narrative proof generator still allows 5.8s.
const DIRECTOR_PROOF_BEAT_MAX_VISIBLE_DURATION_S = 5.8;
...
function mergeIntervals(intervals = []) {
  const sorted = intervals
    .map((interval) => ({
      start: numberOr(interval.start, 0),
      end: numberOr(interval.end, 0),
    }))
...
function intervalSeconds(intervals = []) {
  return mergeIntervals(intervals).reduce((sum, interval) => sum + Math.max(0, interval.end - interval.start), 0);
}

function subtractIntervals(baseIntervals = [], subtractingIntervals = []) {
  let remaining = mergeIntervals(baseIntervals);
  const subtracting = mergeIntervals(subtractingIntervals);
  for (const subtractingInterval of subtracting) {
    const next = [];
    for (const interval of remaining) {
      if (subtractingInterval.end <= interval.start || subtractingInterval.start >= interval.end) {
        next.push(interval);
        continue;
      }
      if (subtractingInterval.start > interval.start) {
        next.push({ start: interval.start, end: subtractingInterval.start });
      }
      if (subtractingInterval.end < interval.end) {
        next.push({ start: subtractingInterval.end, end: interval.end });
      }
    }
    remaining = next;
  }
...
function readableCardText(shot = {}) {
  return cleanText([
    shot.label,
    shot.detail,
    shot.text,
...
function readableCardTimingContract(shot = {}) {
  const policy = v5CardTimingContract(
    shot.kind || shot.id || "context",
    readableCardText(shot),
  );
  return {
    role: policy.kind,
    minimum_visible_duration_s: policy.minimum_visible_duration_s,
    target_visible_duration_s: policy.planned_visible_duration_s,
    maximum_visible_duration_s: policy.kind === "source"
      ? policy.maximum_visible_duration_s
      : Math.max(
        policy.maximum_visible_duration_s,
        DIRECTOR_PROOF_BEAT_MAX_VISIBLE_DURATION_S,
      ),
...
function motionSourceAssetKey(shot = {}) {
  return cleanText(
    shot.base_source_family ||
      shot.original_source_family ||
      shot.provenance?.base_source_family ||
      shot.source_url ||
      shot.reference_url ||
      shot.source_family,
  );
...
function motionClipAssetKey(shot = {}) {
  const pathKey = cleanText(
    shot.media_path ||
      shot.clip_path ||
      shot.video_path ||
      shot.local_path ||
  ...
  if (pathKey) return `path:${pathKey.toLowerCase()}`;

  const source = cleanText(
    shot.source_url ||
      shot.reference_url ||
      shot.provenance?.source_url ||
      shot.provenance?.reference_url,
  );
  if (!source) return "";

  const start = cleanText(shot.media_start_s ?? shot.mediaStartS ?? shot.start_offset_s ?? shot.start_offset);
  const end = cleanText(shot.media_end_s ?? shot.mediaEndS ?? shot.end_offset_s ?? shot.end_offset);
  if (start || end) return `source-window:${source.toLowerCase()}#${start}-${end}`;
  return `source:${source.toLowerCase()}`;
}

function repeatedMotionClipAssetStats(motionShots = []) {
  const counts = new Map();
  for (const shot of motionShots) {
    const key = motionClipAssetKey(shot);
    if (!key) continue;
    counts.set(key, (counts.get(key) || 0) + 1);
  }
  const repeated = [...counts.entries()].filter(([, count]) => count > 1);
  return {
    repeated_motion_clip_asset_count: repeated.length,
    repeated_motion_clip_instance_count: repeated.reduce((sum, [, count]) => sum + count, 0),
  };
}
...
function cardStats(shots = [], durationS = 0) {
  const cardIntervals = shots
    .filter(isCardOnlyShot)
    .map((shot) => ({ start: shotStart(shot), end: shotEnd(shot) }));
  const motionIntervals = shots
    .filter(isMotionShot)
    .map((shot) => ({ start: shotStart(shot), end: shotEnd(shot) }));
  const cardOverlaySeconds = intervalSeconds(cardIntervals);
  const cardOnlySeconds = intervalSeconds(subtractIntervals(cardIntervals, motionIntervals));
  const totalDuration = Math.max(durationS, ...shots.map(shotEnd), 1);
  return {
    card_seconds: round(cardOnlySeconds, 3),
    card_overlay_seconds: round(cardOverlaySeconds, 3),
    card_ratio: round(cardOnlySeconds / totalDuration, 3),
    total_duration_s: round(totalDuration, 3),
  };
}

function readableCardTimingEvidence(shots = []) {
  return shots
    .filter(isCardOnlyShot)
    .map((shot) => {
      const contract = readableCardTimingContract(shot);
      const durationS = round(shotDuration(shot), 3);
      const status = durationS < contract.minimum_visible_duration_s
        ? "too_short"
        : durationS > contract.maximum_visible_duration_s
          ? "too_long"
          : "pass";
      return {
        id: cleanText(shot.id || shot.kind || "card"),
        kind: cleanText(shot.kind || "card"),
        role: contract.role,
        duration_s: durationS,
        minimum_visible_duration_s: contract.minimum_visible_duration_s,
        target_visible_duration_s: contract.target_visible_duration_s,
        maximum_visible_duration_s: contract.maximum_visible_duration_s,
...
function validateDirectorPlan(plan = {}) {
  ...
  const distinctSourceAssets = new Set(motionShots.map(motionSourceAssetKey).filter(Boolean));
  ...
  const cardTimingEvidence = readableCardTimingEvidence(shots);
  const shortCards = cardTimingEvidence.filter((shot) => shot.status === "too_short");
  const longCards = cardTimingEvidence.filter((shot) => shot.status === "too_long");
  const repeatedMotionAssets = repeatedMotionClipAssetStats(motionShots);
  ...
  if (shortCards.length) blockers.push("director:card_dwell_too_short");
  if (shortCards.some((shot) => shot.role === "source")) {
    blockers.push("director:source_lock_dwell_too_short");
  }
  if (longCards.length) blockers.push("director:card_dwell_too_long");
  if (longCards.some((shot) => shot.role === "source")) {
    blockers.push("director:source_lock_dwell_too_long");
  }
  ...
  if (distinctSourceAssets.size < minSourceAssets) blockers.push("director:distinct_motion_source_assets_minimum_not_met");
  if (repeatedMotionAssets.repeated_motion_clip_asset_count > 0) {
    blockers.push("director:repeated_motion_clip_asset");
  }
  ...
  return {
    ...
    metrics: {
      ...
      distinct_motion_source_asset_count: distinctSourceAssets.size,
      duration_s: round(duration, 3),
      ...cards,
      ...repeatedMotionAssets,
      card_timing_contract_version: PREMIUM_CARD_TIMING_V5_VERSION,
      card_timing_evidence: cardTimingEvidence,
      minimum_readable_card_duration_s: V5_READABLE_CARD_TIMING.minimum_visible_duration_s,
      too_short_readable_card_count: shortCards.length,
      too_long_readable_card_count: longCards.length,
