...
const MIN_READABLE_CARD_SHOT_DURATION_S = 12;
const MAX_READABLE_CARD_SHOT_DURATION_S = 12;
...
function readableCardDurationRequiredS(shot = {}) {
  ...
  const computed = Math.max(
    ...
    words ? words * 1.05 + 1.2 : MIN_READABLE_CARD_SHOT_DURATION_S,
  );
  return Math.min(MAX_READABLE_CARD_SHOT_DURATION_S, Math.ceil(computed * 10) / 10);
}
...
function tooShortReadableCards(shots = []) {
  return shots
    .filter(isCardOnlyShot)
    .map((shot) => ({
    ...
    .filter((shot) => shot.duration_s < shot.minimum_required_duration_s);
}
...
function validateDirectorPlan(plan = {}) {
  ...
  const shortCards = tooShortReadableCards(shots);
  ...
  if (shortCards.some((shot) => shot.kind === "source_lock")) {
  ...
  return {
    ...
    metrics: {
      ...
      minimum_readable_card_duration_s: MIN_READABLE_CARD_SHOT_DURATION_S,
