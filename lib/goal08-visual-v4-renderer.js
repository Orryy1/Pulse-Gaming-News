...
function cardRatio(director = {}) {
  ...
  const cardIntervals = shots.filter(isCardShot).map((shot) => ({ start: shotStart(shot), end: shotEnd(shot) }));
  const motionIntervals = shots.filter(isMotionShot).map((shot) => ({ start: shotStart(shot), end: shotEnd(shot) }));
  const cardSeconds = intervalSeconds(subtractIntervals(cardIntervals, motionIntervals));
...
function renderManifestBlockers(renderManifest = {}, fileState = {}) {
  ...
  if (cleanText(renderManifest.creative_system_version) === "pulse_visual_identity_v5") {
    const decodedGate = renderManifest.decoded_visual_gate;
    if (!decodedGate || decodedGate.decoded_media_evidence !== true) {
      blockers.push("render:decoded_visual_gate_missing");
    } else if (
      cleanText(decodedGate.status) !== "pass" ||
      asArray(decodedGate.blockers).length > 0 ||
      numberOr(decodedGate.frame_count, 0) < 3
    ) {
      blockers.push("render:decoded_visual_gate_failed");
    }
  }
...
async function inspectStoryPackage(storyPackage = {}, options = {}) {
  ...
  return {
    ...
    render: {
      ...
      creative_system_version: cleanText(renderManifest?.creative_system_version) || null,
      creative_identity: renderManifest?.creative_identity || null,
      decoded_visual_gate: renderManifest?.decoded_visual_gate || null,
    },
    quality: {
