...
function buildDryRunSkippedIndex(dryRunPlan = {}) {
  const index = new Map();
  for (const story of asArray(dryRunPlan.skipped_stories)) {
    const storyId = cleanText(story.story_id || story.id);
    if (!storyId) continue;
    index.set(storyId, {
      status: cleanText(story.status) || "skipped",
      reason: cleanText(story.reason) || "strict_dry_run_skipped",
    });
  }
  return index;
}
...
function isElevenLabsSfxAsset(asset = {}) {
  const provider = cleanText(asset.provider_id || asset.provider || asset.source_type).toLowerCase();
  return provider === "elevenlabs_sfx" || provider === "elevenlabs" || provider.includes("elevenlabs_generated");
}

function elevenLabsGovernanceBlockers(selectedAssets = []) {
  const blockers = [];
  for (const asset of asArray(selectedAssets).filter(isElevenLabsSfxAsset)) {
    const governance = asset.elevenlabs_governance || asset.governance || {};
    const hasRequiredProof =
      governance.sha256_verified === true &&
      cleanText(governance.sha256) &&
      cleanText(governance.sidecar_path) &&
      cleanText(governance.prompt) &&
      cleanText(governance.model) &&
      cleanText(governance.generation_time) &&
      cleanText(governance.rights_note) &&
      cleanText(governance.loudness_qc_status) === "pass" &&
      cleanText(governance.reuse_status) === "within_reuse_limit";
    if (!hasRequiredProof) {
      blockers.push("sound:elevenlabs_sfx_governance_missing");
    }
  }
  return unique(blockers);
}
...
function audioEvidenceManifest(audioManifest = {}, narrationManifest = {}) {
  const narrationReady = cleanText(narrationManifest.status).toLowerCase() === "ready";
  return {
    ...narrationManifest,
    ...audioManifest,
    narration_audio_path: firstText(
    ...
    word_timestamps_path: firstText(
    ...
    voice_status: firstText(
      audioManifest.voice_status,
      narrationManifest.voice_status,
      narrationReady ? "materialized" : "",
    ),
    word_timestamp_count:
      numberOr(audioManifest.word_timestamp_count, 0) ||
      numberOr(narrationManifest.word_timestamp_count, 0),
    mix_rules: {
      ...(narrationManifest.mix_rules || {}),
      ...(audioManifest.mix_rules || {}),
    },
    safety: {
      ...(narrationManifest.safety || {}),
      ...(audioManifest.safety || {}),
    },
  };
}
...
function sfxManifestBlockers(sfxManifest = {}, director = {}) {
  ...
  blockers.push(...elevenLabsGovernanceBlockers(selectedAssets));
...
function upstreamVisualReady(storyId, upstreamVisualIndex = new Map()) {
  const row = upstreamVisualIndex.get(cleanText(storyId));
  return cleanText(row?.status) === "ready";
}
...
async function inspectStoryPackage(storyPackage = {}, options = {}) {
  ...
  const [audioManifest, narrationManifest, sfxManifest, loudnessReport, director, renderManifest] = await Promise.all([
  ...
  const effectiveAudioManifest = audioManifest ? audioEvidenceManifest(audioManifest, narrationManifest) : null;
  const missing = [];
  ...
  const directBlockers = unique([
    ...missing,
    ...(effectiveAudioManifest ? audioManifestBlockers(effectiveAudioManifest) : []),
  ...
  return {
    ...
    audio: {
      ...
      narration_audio_path: cleanText(effectiveAudioManifest?.narration_audio_path),
      word_timestamps_path: cleanText(effectiveAudioManifest?.word_timestamps_path),
      voice_status: cleanText(effectiveAudioManifest?.voice_status),
      word_timestamp_count: numberOr(effectiveAudioManifest?.word_timestamp_count, 0),
      mix_rules: effectiveAudioManifest?.mix_rules || {},
    },
    sfx: {
...
function buildAggregateSfxManifest(report = {}) {
  const isSfxBlocker = (blocker) =>
    blocker.startsWith("sound:sfx") ||
    blocker.startsWith("sfx:") ||
    blocker.startsWith("sound:elevenlabs_sfx");
  return {
    schema_version: 1,
    goal: GOAL_ID,
    generated_at: report.generated_at || null,
    ...
    stories: asArray(report.stories).map((story) => ({
      ...
      status: story.status === "skipped"
        ? "skipped"
        : asArray(story.direct_sound_blockers).some(isSfxBlocker)
          ? "blocked"
          : "pass",
      sfx: story.sfx,
      blockers: asArray(story.direct_sound_blockers).filter(isSfxBlocker),
    })),
  };
}
...
function buildLoudnessReport(report = {}) {
  return {
    ...
    stories: asArray(report.stories).map((story) => ({
      ...
      status: story.status === "skipped"
        ? "skipped"
        : asArray(story.direct_sound_blockers).some((blocker) => blocker.startsWith("sound:loudness") || blocker.startsWith("loudness:") || blocker.startsWith("sound:unsafe") || blocker.startsWith("sound:buried") || blocker.startsWith("sound:late"))
          ? "blocked"
          : "pass",
...
function buildAudioQualityScorecard(report = {}) {
  return {
    ...
    stories: asArray(report.stories).map((story) => ({
      ...
      status: story.status === "skipped"
        ? "skipped"
        : story.direct_sound_status === "pass" ? "pass" : "blocked",
...
async function buildGoal09SoundDesignEngine({
  ...
  const dryRunSkippedIndex = buildDryRunSkippedIndex(dryRunPlan);
  const stories = [];
  for (const storyPackage of asArray(storyPackages)) {
    stories.push(await inspectStoryPackage(storyPackage, { workspaceRoot, upstreamVisualIndex, dryRunSkippedIndex }));
  }
  ...
  const directSoundPassStories = activeStories.filter((story) => story.direct_sound_status === "pass");
  const directSoundBlockedStories = activeStories.filter((story) => story.direct_sound_status === "blocked");
  ...
  const report = {
    ...
    summary: {
      ...
      sfx_source_pass_story_count: activeStories.filter((story) => story.sfx.status === "pass").length,
      loudness_pass_story_count: activeStories.filter((story) => story.loudness.verdict === "pass").length,
      current_sound_policy_story_count: activeStories.filter((story) =>
