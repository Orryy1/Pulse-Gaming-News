...
function commercialIntegrationValue(affiliateManifest = {}) {
  const route = cleanText(affiliateManifest.landing_page_route);
  const linkTracking = asArray(affiliateManifest.landing_page_attribution?.link_tracking);
  if (affiliateManifest.disclosure_required === true) return "disclosed_story_page_route";
  if (route && linkTracking.length) return "source_first_tracked_story_page";
  return "";
}
