...
const COMMERCIAL_ATTRIBUTION_PLATFORMS = [
  "youtube",
  "tiktok",
  "instagram",
  "facebook",
...
function upstreamSkippedInfo(storyId, upstreamSoundIndex = new Map()) {
  ...
  return {
    ...
    reason: cleanText(row.skipped_reason || row.reason) || "upstream_sound_skipped",
  };
}
...
function normalisedCommercialIntent(affiliateManifest = {}) {
  return cleanText(affiliateManifest.commercial_intent_type || affiliateManifest.intent_verdict)
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "_")
    .replace(/^_+|_+$/g, "");
}

function noDirectOfferIntent(affiliateManifest = {}) {
  return [
    "no_safe_commercial_intent",
    "no_direct_offer",
    "no_safe_direct_offer",
    "no_safe_direct_affiliate_offer",
  ].includes(normalisedCommercialIntent(affiliateManifest));
}

function commercialLandingRoute(affiliateManifest = {}) {
  const route = cleanText(
    affiliateManifest.landing_page_route ||
      affiliateManifest.landing_page_attribution?.landing_page_route,
  );
  return /^\/p\/[a-z0-9][a-z0-9/_-]*(?:[?#].*)?$/i.test(route) ? route.split(/[?#]/)[0] : "";
}

function attributionPlatform(value) {
...
function attributionRowsByPlatform(affiliateManifest = {}) {
  const attribution = affiliateManifest.landing_page_attribution || {};
  const rows = new Map();
  for (const [platform, row] of Object.entries(attribution.platforms || {})) {
    if (isRecord(row)) rows.set(attributionPlatform(platform), row);
  }
  for (const row of asArray(attribution.link_tracking)) {
    const platform = attributionPlatform(row?.platform);
    if (platform) rows.set(platform, { ...(rows.get(platform) || {}), ...row });
  }
  return rows;
}

function validPlatformLandingAttribution(row = {}, platform, route) {
  const landingPageUrl = cleanText(row.landing_page_url || row.url);
  const trackingKey = cleanText(row.tracking_key || row.trackingKey);
  if (!landingPageUrl || !trackingKey) return false;
  try {
    const url = new URL(landingPageUrl, "https://pulse-gaming.local");
    return (
      url.pathname === route &&
      cleanText(url.searchParams.get("utm_source")).toLowerCase() === platform &&
      Boolean(cleanText(url.searchParams.get("utm_medium"))) &&
      Boolean(cleanText(url.searchParams.get("utm_campaign")))
    );
  } catch {
    return false;
  }
...
function sourceFirstPerPlatformAttributionComplete(affiliateManifest = {}) {
  const route = commercialLandingRoute(affiliateManifest);
  const attribution = affiliateManifest.landing_page_attribution || {};
  if (!route || cleanText(attribution.verdict).toLowerCase() !== "pass") return false;
  if (attribution.safety?.source_first_story_page !== true) return false;
  const rows = attributionRowsByPlatform(affiliateManifest);
  return COMMERCIAL_ATTRIBUTION_PLATFORMS.every((platform) =>
    validPlatformLandingAttribution(rows.get(platform), platform, route));
}

function explicitEmptyOfferTracking(affiliateManifest = {}) {
  const trackingMap = affiliateManifest.affiliate_tracking_map;
  if (!isRecord(trackingMap)) return false;
  const mapIsEmpty =
    hasOwn(trackingMap, "primary_offer_id") &&
    !cleanText(trackingMap.primary_offer_id) &&
    hasOwn(trackingMap, "story_page") &&
    !cleanText(trackingMap.story_page) &&
    isRecord(trackingMap.platforms) &&
    !Object.values(trackingMap.platforms).some((value) => cleanText(value)) &&
    Array.isArray(trackingMap.fallback_offer_ids) &&
    trackingMap.fallback_offer_ids.length === 0;
  if (!mapIsEmpty) return false;
  ...
  return COMMERCIAL_ATTRIBUTION_PLATFORMS.every((platform) => {
    ...
    return Boolean(
      isRecord(row) &&
        ...
        hasOwn(row, "offer_tracking_url") &&
        !cleanText(row.offer_tracking_url),
    );
  });
}

function noDirectOfferDecisionComplete(affiliateManifest = {}) {
  const reason = cleanText(
    affiliateManifest.no_direct_offer_reason ||
      affiliateManifest.no_offer_reason ||
      affiliateManifest.decision_reason ||
      asArray(affiliateManifest.rejection_reasons)[0],
  );
  return Boolean(
    noDirectOfferIntent(affiliateManifest) &&
      reason &&
      hasOwn(affiliateManifest, "primary_link") &&
      affiliateManifest.primary_link === null &&
      Array.isArray(affiliateManifest.fallback_links) &&
      affiliateManifest.fallback_links.length === 0 &&
      asArray(affiliateManifest.offers).length === 0 &&
      affiliateManifest.disclosure_required === false &&
      explicitEmptyOfferTracking(affiliateManifest) &&
      sourceFirstPerPlatformAttributionComplete(affiliateManifest)
  );
}

function directOfferDecisionComplete(affiliateManifest = {}) {
  const primaryLink = affiliateManifest.primary_link;
  const trackingMap = affiliateManifest.affiliate_tracking_map || {};
  const hasOfferTracking = Boolean(
    cleanText(primaryLink?.tracking_url) ||
      Object.values(primaryLink?.platform_tracking_urls || {}).some((value) => cleanText(value)) ||
      cleanText(trackingMap.story_page) ||
      Object.values(trackingMap.platforms || {}).some((value) => cleanText(value)) ||
      asArray(affiliateManifest.landing_page_attribution?.link_tracking)
        .some((row) => cleanText(row?.offer_tracking_url)),
  );
  return Boolean(
    isRecord(primaryLink) &&
      !noDirectOfferIntent(affiliateManifest) &&
      cleanText(affiliateManifest.commercial_intent_type) &&
      cleanText(primaryLink.id) &&
      cleanText(primaryLink.label) &&
      cleanText(primaryLink.merchant || affiliateManifest.merchant) &&
      cleanText(primaryLink.product_category || affiliateManifest.product_category) &&
...
function commercialIntegrationValue(affiliateManifest = {}) {
  if (directOfferDecisionComplete(affiliateManifest)) return "disclosed_story_page_route";
  if (noDirectOfferDecisionComplete(affiliateManifest)) return "source_first_tracked_story_page";
  return "";
}
...
function extractPatternData({
  ...
  return {
    ...
    commercial_integration: patternEntry("present", commercialIntegrationValue(affiliateManifest), {
      landing_page_route: commercialLandingRoute(affiliateManifest),
      link_tracking_count: asArray(affiliateManifest.landing_page_attribution?.link_tracking).length,
      attributed_platform_count: attributionRowsByPlatform(affiliateManifest).size,
      decision_type: directOfferDecisionComplete(affiliateManifest)
        ? "governed_direct_offer"
        : noDirectOfferDecisionComplete(affiliateManifest)
          ? "governed_no_offer"
          : "incomplete",
    }),
...
async function inspectStoryPackage(storyPackage = {}, options = {}) {
  ...
  const skipped = upstreamSkippedInfo(storyId, options.upstreamSoundIndex);
  if (skipped) {
    return {
      ...
      pattern_coverage: {
        ...
        present_count: 0,
        missing: REQUIRED_PATTERN_FIELDS,
      },
      scores: {},
      thresholds: {},
      safety: {
...
async function buildGoal10GoldStandardForensicsEngine({
  ...
  const directPassStories = activeStories.filter((story) => story.direct_benchmark_status === "pass");
  const directBlockedStories = activeStories.filter((story) => story.direct_benchmark_status === "blocked");
  ...
  const report = {
    ...
    summary: {
      ...
      full_pattern_coverage_story_count: activeStories.filter((story) => story.pattern_coverage.present_count === REQUIRED_PATTERN_FIELDS.length).length,
    },
