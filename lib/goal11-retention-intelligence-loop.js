...
function buildLearningRules(stories = [], generatedAt = new Date().toISOString()) {
  ...
  const pendingStories = activeStories.filter((story) => story.metrics_status === "pending");
...
async function inspectStoryPackage(storyPackage = {}, context = {}) {
  ...
  const analyticsPending = !metrics && upstream.length === 0;
  ...
  const directBlockers = unique([
    ...
    ...(analyticsPending ? [] : missing.map((field) => `retention:${field}_missing`)),
  ...
  return {
    ...
    metrics_status: hasCompleteMetrics ? "complete" : metrics ? "incomplete" : analyticsPending ? "pending" : "missing",
