...
function upstreamSkippedInfo(storyId, experimentIndex = new Map()) {
  ...
  return {
    ...
    reason: cleanText(row.skipped_reason || row.reason) || "upstream_experiment_skipped",
  };
}
...
function affiliateDisclosureRequired({ affiliateManifest = {}, landingPage = {}, platformManifest = {} } = {}) {
  if (noAffiliateCommercialIntent(affiliateManifest, landingPage)) return false;
  if (explicitNoAffiliateDisclosureContext({ affiliateManifest, landingPage, platformManifest })) return false;
...
function manifestHasAffiliateLinks(affiliateManifest = {}) {
  return Boolean(
    affiliateManifest.primary_link ||
      affiliateManifest.primaryLink ||
      asArray(affiliateManifest.candidate_links).length ||
      asArray(affiliateManifest.fallback_links).length,
  );
}

function attributionRequiresDisclosure(landingPage = {}) {
  return Object.values(landingPage.attribution_manifest?.platforms || {}).some((row = {}) =>
    row?.disclosure_required === true ||
      /affiliate|commission|commercial/i.test(cleanText(row?.disclosure_copy)),
  );
}

function explicitNoAffiliateDisclosureContext({ affiliateManifest = {}, landingPage = {}, platformManifest = {} } = {}) {
  if (manifestHasAffiliateLinks(affiliateManifest)) return false;
  if (affiliateManifest.disclosure_required === true || landingPage.disclosure_block?.required === true) return false;
  if (attributionRequiresDisclosure(landingPage)) return false;
  if (affiliateManifest.disclosure_required === false || landingPage.disclosure_block?.required === false) return true;
  const outputsText = cleanText(JSON.stringify(platformOutputs(platformManifest) || {}));
  return /\b(?:no affiliate|no commercial link|affiliate_not_required|not_required|no affiliate links are attached)\b/i.test(outputsText);
}

function noAffiliateCommercialIntent(affiliateManifest = {}, landingPage = {}) {
  const intent = cleanText(affiliateManifest.commercial_intent_type || affiliateManifest.primary_affiliate_angle).toLowerCase();
  const noIntent = /no[_\s-]*(safe|natural)?[_\s-]*commercial[_\s-]*intent|no natural affiliate angle/.test(intent);
  return (
    affiliateManifest.disclosure_required === false &&
    landingPage.disclosure_block?.required !== true &&
    !manifestHasAffiliateLinks(affiliateManifest) &&
    noIntent
  );
}
...
function affiliateLinkRelated(affiliateManifest = {}) {
  ...
  const intent = cleanText(affiliateManifest.commercial_intent_type || affiliateManifest.primary_affiliate_angle).toLowerCase();
  if (!hasLinks && /no[_\s-]*(safe|natural)?[_\s-]*commercial[_\s-]*intent|no natural affiliate angle/.test(intent)) {
    return true;
  }
...
async function inspectStoryPackage(storyPackage = {}, context = {}) {
  ...
  const skipped = upstreamSkippedInfo(storyId, context.experimentIndex);
  if (skipped) {
    return {
      story_id: storyId,
      ...
      status: "skipped",
      direct_platform_status: "skipped",
      upstream_status: "skipped",
      skipped_status: skipped.status,
      skipped_reason: skipped.reason,
      blockers: [],
      upstream_blockers: [],
      direct_platform_blockers: [],
      risks: [],
      platform_rows: [],
      platform_outputs: {},
      platform_native_evidence: null,
      duplicate_pairs: [],
...
function actionStatus(story = {}) {
  if (story.status === "skipped") return "skipped";
...
function buildPlatformPublishManifest(report = {}) {
  return {
    ...
    stories: asArray(report.stories).map((story) => ({
      ...
      platform_plan: story.status === "skipped"
        ? []
...
function buildPlatformVariantScorecard(report = {}) {
  ...
  const platformSummary = Object.fromEntries(REQUIRED_PLATFORMS.map((platform) => {
    const rows = activeStories.map((story) => story.platform_rows.find((row) => row.platform === platform));
    return [platform, {
      platform,
      ...
      direct_pass_count: activeStories.filter((story) =>
...
function buildScheduledPosts(report = {}) {
  const posts = asArray(report.stories).filter((story) => story.status !== "skipped").flatMap((story) =>
    REQUIRED_PLATFORMS.map((platform) => ({
      story_id: story.story_id,
      platform,
      status: actionStatus(story),
...
async function buildGoal13MultiPlatformPublisherEngine({
  ...
  const directPassStories = activeStories.filter((story) => story.direct_platform_status === "pass");
  const directBlockedStories = activeStories.filter((story) => story.direct_platform_status !== "pass");
  const allOutputsPresentStories = activeStories.filter((story) =>
  ...
  const verdict = !activeStories.length
    ...
    : blockedStories.length && readyStories.length
      ...
      : blockedStories.length
        ? "BLOCKED"
        : "PASS";
  const directPlatformVerdict = !activeStories.length
    ? "FAIL"
  ...
  const report = {
    ...
    summary: {
      ...
      planned_dry_run_post_count: activeStories.length * REQUIRED_PLATFORMS.length,
      publish_now_count: 0,
    },
    required_platforms: REQUIRED_PLATFORMS,
