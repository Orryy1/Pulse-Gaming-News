...
function upstreamSkippedInfo(storyId, publisherIndex = new Map()) {
  ...
  return {
    ...
    reason: cleanText(row.skipped_reason || row.reason) || "upstream_publisher_skipped",
  };
}
...
function angleRole(title = "") {
  const text = normaliseBasic(title);
  if (/\b(?:score|scores|metacritic|opencritic|review|reviews|rated)\b/.test(text)) return "score_check";
  if (/\b(?:leak|leaked|reportedly|rumour|rumor|claimed)\b/.test(text)) return "claim_boundary";
  if (/\b(?:deal|price|drops?|sale|\$|£|discount|subscription)\b/.test(text)) return "price_access";
  if (/\b(?:gameplay|trailer|shows|showed|reveal|revealed)\b/.test(text)) return "footage_proof";
  if (/\b(?:dev|developer|studio|composer|jobs?|layoff|board)\b/.test(text)) return "studio_context";
  if (/\b(?:steam|xbox|playstation|switch|pc|platform)\b/.test(text)) return "platform_context";
  return "player_impact";
}

function sourceRole(source = "") {
  const text = normaliseBasic(source);
  if (/\b(?:xbox|playstation|nintendo|steam|gamestop|metacritic|opencritic)\b/.test(text)) return "official_or_store_source";
  if (/\b(?:ign|pc gamer|eurogamer|gamespot|rock paper shotgun|insider gaming|aftermath|automaton)\b/.test(text)) return "press_source";
  return "source_proof";
}

function promptRole(title = "") {
  const text = normaliseBasic(title);
  if (/\b(?:deal|price|drops?|sale|\$|£|discount|subscription)\b/.test(text)) return "buy_or_wait_prompt";
  if (/\b(?:leak|leaked|reportedly|rumour|rumor|claimed)\b/.test(text)) return "believe_or_wait_prompt";
  if (/\b(?:score|review|rated|metacritic|opencritic)\b/.test(text)) return "score_argument_prompt";
  return "watchlist_prompt";
}

function statLabelFor(title = "") {
  const role = angleRole(title);
  if (role === "score_check") return "review score";
  if (role === "claim_boundary") return "reported claim";
  if (role === "price_access") return "price check";
  if (role === "footage_proof") return "footage shown";
  if (role === "studio_context") return "studio signal";
  if (role === "platform_context") return "platform move";
  return "player impact";
}

function staleGenericXPost(value = "") {
  return /\bI want the next official beat to show whether this actually changes the game, the launch or the platform plan\b/i.test(value) ||
    /\buseful next beat\b/i.test(value);
}

function xHotTakeFor({ title, subject, source }) {
  const target = cleanText(subject || title || "This story");
  const headline = cleanText(title || target);
  const role = angleRole(title);
  if (role === "score_check") {
    return `${headline}. The argument is whether that number changes the launch conversation.`;
  }
  if (role === "claim_boundary") {
    return `${target} is already in leak-control mode. The next official response matters more than the leak itself.`;
  }
  if (role === "price_access") {
    return `${target} at this price is tempting. The catch is whether the deal beats waiting for the next sale.`;
  }
  if (role === "footage_proof") {
    return `${target} finally has footage people can judge. That is a different story from another logo reveal.`;
  }
  if (role === "studio_context") {
    return `${target} just became a studio story. The question is what this says about the next game, not only this headline.`;
  }
  if (role === "platform_context") {
    return `${headline}. This is now a platform story, and that is where the next argument starts.`;
  }
  return `${headline}. The part to watch is whether this changes what players do next.`;
}

function carouselSlots({ storyId, title, source }) {
  const proof = sourceRole(source);
  const angle = angleRole(title);
  const prompt = promptRole(title);
  const middle = [proof, angle, "quote_card", "stat_card", prompt];
  let seed = hashText(`${storyId}:${title}:${source}`);
  for (let index = middle.length - 1; index > 0; index -= 1) {
    seed = (seed * 1664525 + 1013904223) >>> 0;
    const swapIndex = seed % (index + 1);
    [middle[index], middle[swapIndex]] = [middle[swapIndex], middle[index]];
  }
  return ["cover", ...middle, "related_links"];
}

function carouselFormatSignature(slots = []) {
  return slots.map((slot) => cleanText(slot).toLowerCase()).join(">");
}
...
function buildInstagramDerivative({ storyId, title, subject, source, instagramPack = {}, route }) {
  const slots = carouselSlots({ storyId, title, source });
  const formatSignature = carouselFormatSignature(slots);
  return {
    story_id: storyId,
    platform: "instagram",
    ...
    carousel_companion: {
      ...
      source: instagramPack.carousel_companion ? "goal14_variation_repair" : "goal14_generated",
    },
...
function buildThreadsDerivative({ storyId, title, subject, source, threadsPack = {}, route }) {
  return {
    ...
    discussion_post: cleanText(threadsPack.discussion_post) ||
      `${title}. ${source || "The source"} has the report.`,
...
function pinterestPinIdeas({ title, subject, source, pinterestPack = {} }) {
  const existing = asArray(pinterestPack.pin_ideas || pinterestPack.pins).filter((pin) =>
    pin && typeof pin === "object" && cleanText(pin.title || pin.headline),
  );
  if (existing.length >= 3) {
    return existing.slice(0, 5).map((pin) => ({
      type: cleanText(pin.type || "evergreen_pin"),
      title: cleanText(pin.title || pin.headline),
      description: cleanText(pin.description || pin.copy || `${subject || title}. Source: ${source || "source manifest"}.`),
      image_card_type: cleanText(pin.image_card_type || "source_backed_story_card"),
    }));
  }
  const target = subject || title;
  return [
    {
      type: "evergreen_context_pin",
      title: `${target}: what changed`,
      description: `${title}. Source: ${source || "source manifest"}.`,
      image_card_type: "source_backed_story_card",
    },
    {
      type: "watchlist_pin",
      title: `${target}: player watchlist check`,
      description: `Save the story page for sources, context and the follow-up angle.`,
      image_card_type: "watchlist_story_card",
    },
    {
      type: "source_context_pin",
      title: `${target}: source notes`,
      description: `The useful angle is the source trail and what it means for players next.`,
      image_card_type: "source_notes_card",
    },
  ];
}

function pinterestBreakingSpamRisk(pinterestPack = {}) {
  const text = collectStrings(pinterestPack).join(" ");
  return /\b(?:breaking now|urgent|watch now|go viral|viral pin|pin this now)\b/i.test(text);
}

function buildPinterestDerivative({ storyId, title, subject, source, pinterestPack = {}, route }) {
  const landing = cleanText(pinterestPack.landing_page_link || pinterestPack.story_page_link || route || `/p/${storyId}`);
  return {
    story_id: storyId,
    platform: "pinterest",
    landing_page_required: true,
    landing_page_link: landing,
    evergreen_strategy: cleanText(pinterestPack.evergreen_strategy) || "source_backed_story_page_pin",
    board_suggestions: asArray(pinterestPack.board_suggestions).length
      ? asArray(pinterestPack.board_suggestions).map(cleanText)
      : ["Gaming News", "Upcoming Games", "Game Trailers"],
    pin_ideas: pinterestPinIdeas({ title, subject, source, pinterestPack }),
    affiliate_route_allowed: pinterestPack.affiliate_route_allowed === true,
    no_breaking_news_spam: true,
    no_crude_engagement_bait: true,
  };
}
...
function buildCarouselPlan({ storyId, title, source, instagramDerivative }) {
  ...
  const cardsByType = new Map([
    ...
    [sourceRole(source), {
      type: sourceRole(source),
      source_label: source || null,
      role: "source_proof",
    }],
    [angleRole(title), {
      type: angleRole(title),
      headline: title,
      source_label: source || null,
      role: "story_angle",
    }],
    ["quote_card", instagramDerivative.quote_card],
    ["stat_card", instagramDerivative.stat_card],
    [promptRole(title), {
      type: "story_prompt",
      prompt: instagramDerivative.story_prompt_card.prompt,
      role: promptRole(title),
    }],
    ["related_links", {
      type: "related_links",
      cta: "story page",
  ...
  return {
    story_id: storyId,
    platform: "instagram",
    format_signature: carouselFormatSignature(slots),
    cards: slots.map((slot) => cardsByType.get(slot)).filter(Boolean),
  };
}
...
async function inspectStoryPackage(storyPackage = {}, context = {}) {
  ...
  const skipped = upstreamSkippedInfo(storyId, context.publisherIndex);
  if (skipped) {
    return {
      story_id: storyId,
      ...
      status: "skipped",
      direct_derivative_status: "skipped",
      upstream_status: "skipped",
      skipped_status: skipped.status,
      skipped_reason: skipped.reason,
      blockers: [],
      upstream_blockers: [],
      direct_derivative_blockers: [],
      risks: [],
      x_derivative: null,
      instagram_derivative: null,
      threads_derivative: null,
      image_card_plan: null,
      carousel_plan: null,
      source_material: {
        x_pack_present: false,
        instagram_pack_present: false,
        threads_pack_present: false,
        image_card_source_ready: false,
        carousel_source_ready: false,
      },
      safety: {
  ...
  const pinterestPack = await readJsonIfPresent(path.join(artifactDir, "pinterest_publish_pack.json"), outputs.pinterest || {});
  ...
  const route = cleanText(
    pinterestPack.landing_page_link ||
      xPack.landing_page_link ||
      threadsPack.landing_page_link ||
      instagramPack.bio_link_cta ||
      `/p/${storyId}`,
  );
  ...
  if (containsEngagementBait(xPack, instagramPack, threadsPack, pinterestPack)) {
    addBlocker("social:engagement_bait", "Crude engagement bait appears in social derivative copy.");
  }
  if (pinterestBreakingSpamRisk(pinterestPack)) {
    addBlocker("social:pinterest_breaking_news_spam", "Pinterest package uses urgent/breaking spam language instead of evergreen routing.");
  }
  ...
  const pinterestDerivative = buildPinterestDerivative({ storyId, title, subject, source, pinterestPack, route });
  ...
  return {
    ...
    pinterest_derivative: pinterestDerivative,
    ...
    source_material: {
      ...
      pinterest_pack_present: Object.keys(pinterestPack || {}).length > 0,
...
function buildInstagramPublishPack(report = {}) {
  return {
    ...
    stories: asArray(report.stories)
      .filter((story) => story.status !== "skipped")
      .map((story) => story.instagram_derivative),
    safety: {
      no_publish_triggered: true,
      no_external_posting: true,
    },
...
function buildThreadsPublishPack(report = {}) {
  return {
    ...
    stories: asArray(report.stories)
      .filter((story) => story.status !== "skipped")
      .map((story) => story.threads_derivative),
    safety: {
      no_publish_triggered: true,
      no_external_posting: true,
      no_automated_replies: true,
...
function buildPinterestPublishPack(report = {}) {
  return {
    schema_version: 1,
    goal: GOAL_ID,
    generated_at: report.generated_at || null,
    ...
    stories: asArray(report.stories)
      .filter((story) => story.status !== "skipped")
      .map((story) => story.pinterest_derivative),
    safety: {
      no_publish_triggered: true,
      no_external_posting: true,
...
function buildImageCardManifest(report = {}) {
  return {
    ...
    stories: asArray(report.stories)
      .filter((story) => story.status !== "skipped")
      .map((story) => story.image_card_plan),
    safety: {
      no_external_generation: true,
      no_external_posting: true,
      no_publish_triggered: true,
...
function buildEngagementRiskReport(report = {}) {
  const risks = asArray(report.stories)
    .filter((story) => story.status !== "skipped")
    .flatMap((story) => asArray(story.risks));
  return {
    schema_version: 1,
    goal: GOAL_ID,
    generated_at: report.generated_at || null,
    ...
    hard_fail_categories: [
      ...
      "social:pinterest_breaking_news_spam",
    ],
...
async function buildGoal14SocialDerivativesEngine({
  ...
  const directPassStories = activeStories.filter((story) => story.direct_derivative_status === "pass");
  const directBlockedStories = activeStories.filter((story) => story.direct_derivative_status !== "pass");
  const verdict = !activeStories.length
    ...
    : blockedStories.length && readyStories.length
      ...
      : blockedStories.length
        ? "BLOCKED"
        : "PASS";
  const directDerivativeVerdict = !activeStories.length
    ? "FAIL"
  ...
  const report = {
    ...
    summary: {
      ...
      x_pack_story_count: activeStories.filter((story) => story.source_material.x_pack_present).length,
      instagram_pack_story_count: activeStories.filter((story) => story.source_material.instagram_pack_present).length,
      threads_pack_story_count: activeStories.filter((story) => story.source_material.threads_pack_present).length,
      pinterest_pack_story_count: activeStories.filter((story) => story.pinterest_derivative).length,
      image_card_story_count: activeStories.length,
      carousel_story_count: activeStories.length,
    },
  ...
  report.pinterest_publish_pack = buildPinterestPublishPack(report);
...
function renderGoal14SocialDerivativesEngineMarkdown(report = {}) {
  ...
  lines.push(`Pinterest derivative stories: ${report.summary?.pinterest_pack_story_count || 0}`);
  lines.push("");
  lines.push("## Blockers");
...
async function writeGoal14SocialDerivativesEngine(report = {}, { outputDir } = {}) {
  ...
  const pinterestPublishPack = path.join(outDir, "pinterest_publish_pack.json");
  ...
  await fs.writeJson(pinterestPublishPack, report.pinterest_publish_pack || buildPinterestPublishPack(report), { spaces: 2 });
