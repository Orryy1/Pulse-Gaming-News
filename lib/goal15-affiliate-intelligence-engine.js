...
function upstreamSkippedInfo(storyId, socialIndex = new Map()) {
  ...
  return {
    ...
    reason: cleanText(row.skipped_reason || row.reason) || "upstream_social_skipped",
  };
}
...
function dominantAffiliateStoryText(canonical = {}, storyPackage = {}) {
  return cleanText([
    canonical.selected_title,
    canonical.short_title,
    canonical.title,
...
function linkMismatchesStorySubject({ canonical = {}, storyPackage = {}, link = {}, affiliate = {} } = {}) {
  const storyText = dominantAffiliateStoryText(canonical, storyPackage);
...
function noDirectOfferIntent(affiliate = {}) {
  const intent = cleanText(affiliate.commercial_intent_type || affiliate.intent_verdict)
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "_")
    .replace(/^_+|_+$/g, "");
  return [
...
function directOfferManifestComplete(affiliate = {}, landing = {}, links = []) {
  const primary = affiliate.primary_link;
  if (!isRecord(primary) || links[0] !== primary) return false;
  if (!cleanText(affiliate.commercial_intent_type) || noDirectOfferIntent(affiliate)) return false;
  if (!Array.isArray(affiliate.fallback_links)) return false;
  if (!cleanText(primary.id) || !cleanText(primary.label) || !cleanText(merchantName(affiliate, primary))) return false;
  if (!cleanText(primary.product_category || affiliate.product_category) || !validUrlish(primary.url)) return false;
  if (affiliate.disclosure_required !== true || !disclosureCopyPresent(affiliate, landing).affiliate) return false;
  return !trackingMissing(affiliate, landing, links);
}

function noDirectOfferTrackingState(affiliate = {}, landing = {}) {
  const trackingMap = affiliate.affiliate_tracking_map;
  const explicitEmptyMap =
    isRecord(trackingMap) &&
    ...
    asArray(trackingMap.fallback_offer_ids).length === 0;
  if (!explicitEmptyMap) return false;

  const attribution = affiliate.landing_page_attribution || landing.attribution_manifest || {};
  const attributionRows = [
    ...asArray(attribution.link_tracking),
    ...Object.values(attribution.platforms || {}),
  ];
  return Boolean(
    !cleanText(attribution.primary_offer_id) &&
      !attributionRows.some((row) => cleanText(row?.offer_id) || cleanText(row?.offer_tracking_url)),
  );
}

function noDirectOfferDecisionComplete(affiliate = {}, landing = {}, links = []) {
  const reason = cleanText(
    affiliate.no_direct_offer_reason ||
      affiliate.no_offer_reason ||
      affiliate.decision_reason ||
      asArray(affiliate.rejection_reasons)[0],
  );
  const noLinksDeclared =
    links.length === 0 &&
    hasOwn(affiliate, "primary_link") &&
    affiliate.primary_link === null &&
    Array.isArray(affiliate.fallback_links) &&
    affiliate.fallback_links.length === 0;
  const platformDisclosureSafe = Object.values(affiliate.platform_disclosure || {}).every(
    (row) => row?.affiliate_disclosure_required !== true,
  );
  return Boolean(
    noLinksDeclared &&
      noDirectOfferIntent(affiliate) &&
      reason &&
      affiliate.disclosure_required === false &&
      landing.disclosure_block?.required !== true &&
      platformDisclosureSafe &&
      noDirectOfferTrackingState(affiliate, landing),
  );
}

function affiliateManifestDecisionComplete(affiliate = {}, landing = {}, links = []) {
  return directOfferManifestComplete(affiliate, landing, links) || noDirectOfferDecisionComplete(affiliate, landing, links);
}
...
async function inspectStoryPackage(storyPackage = {}, context = {}) {
  ...
  const skipped = upstreamSkippedInfo(storyId, context.socialIndex);
  if (skipped) {
    return {
      story_id: storyId,
      ...
      source_material: {
        ...
        direct_offer_count: 0,
        platform_tracking_count: 0,
      },
      safety: {
  ...
  } else if (!affiliateManifestDecisionComplete(affiliateManifest, landing, links)) {
    addBlocker("affiliate:manifest_incomplete", "affiliate_link_manifest.json must contain a governed offer or explicit no-offer decision.");
  }
...
function buildAffiliateLinkManifest(report = {}) {
  return {
    ...
    stories: asArray(report.stories)
      ...
      .map((story) => story.affiliate_link_manifest),
    safety: {
      local_proof_only: true,
...
function buildCommercialOpportunityScore(report = {}) {
  return {
    ...
    stories: asArray(report.stories)
      .filter((story) => story.status !== "skipped")
      .map((story) => story.commercial_opportunity_score),
    safety: {
      no_network_link_checking: true,
...
function buildDisclosureManifest(report = {}) {
  const rows = asArray(report.stories)
    .filter((story) => story.status !== "skipped")
    .map((story) => story.disclosure_row);
  return {
    schema_version: 1,
    goal: GOAL_ID,
    generated_at: report.generated_at || null,
...
async function buildGoal15AffiliateIntelligenceEngine({
  ...
  const directPassStories = activeStories.filter((story) => story.direct_affiliate_status === "pass");
  const directBlockedStories = activeStories.filter((story) => story.direct_affiliate_status !== "pass");
  const verdict = !activeStories.length
    ...
    : blockedStories.length && readyStories.length
      ...
      : blockedStories.length
        ? "BLOCKED"
        : "PASS";
  const directAffiliateVerdict = !activeStories.length
    ? "FAIL"
  ...
  const report = {
    ...
    summary: {
      ...
      commercial_score_story_count: activeStories.filter((story) => story.commercial_opportunity_score).length,
      disclosure_story_count: activeStories.filter((story) => story.disclosure_row).length,
      tracking_story_count: activeStories.filter((story) => story.tracking_row).length,
      direct_offer_story_count: activeStories.filter((story) => story.source_material.direct_offer_count > 0).length,
    },
