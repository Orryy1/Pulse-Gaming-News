...
function buildLandingPageManifest(report = {}) {
  return {
    ...
    mode: "LOCAL_PROOF",
    stories: asArray(report.stories).map((story) => story.landing_page_manifest),
    safety: {
      local_proof_only: true,
      no_publish_triggered: true,
      no_external_posting: true,
...
function buildDisclosureBlockArtifact(report = {}) {
  const rows = asArray(report.stories).map((story) => ({
    story_id: story.story_id,
...
function buildRevenueTracking(report = {}) {
  return {
    ...
    generated_at: report.generated_at || null,
    mode: "LOCAL_PROOF",
    stories: asArray(report.stories).map((story) => story.revenue_tracking),
    safety: {
      no_affiliate_network_reporting_pull: true,
...
async function buildGoal16LandingPageEngine({
  ...
  const directPassStories = stories.filter((story) => story.direct_landing_status === "pass");
  const directBlockedStories = stories.filter((story) => story.direct_landing_status !== "pass");
  const verdict = !stories.length
    ...
    : blockedStories.length && readyStories.length
      ...
      : blockedStories.length
        ? "BLOCKED"
        : "PASS";
  const directLandingVerdict = !stories.length
    ? "FAIL"
  ...
  const report = {
    ...
    summary: {
      ...
      source_list_story_count: stories.filter((story) => story.source_material.source_count > 0).length,
      local_embed_story_count: stories.filter((story) => story.source_material.embed_status === "local_proof_asset_available").length,
      upstream_blocked_embed_story_count: stories.filter((story) => story.source_material.embed_status === "upstream_blocked").length,
      related_story_count: stories.filter((story) => story.source_material.related_story_count > 0).length,
      related_product_story_count: stories.filter((story) => story.source_material.related_product_count > 0).length,
      newsletter_capture_story_count: stories.filter((story) => story.landing_page_manifest.newsletter_capture?.enabled === true).length,
    },
