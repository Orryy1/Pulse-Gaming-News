...
function normaliseRegionRoute(region = {}, landingPageRoute = "") {
  const offerRoute = cleanText(region.route);
  return {
    route: offerRoute || landingPageRoute || null,
    offer_route: offerRoute || null,
    status: cleanText(region.status) || (offerRoute ? "offer_route_available" : "story_page_only"),
    fallback_to_story_page: !offerRoute,
  };
}

function buildCommerceRouteMap(report = {}) {
  const stories = asArray(report.stories)
    .filter((story) => story.status !== "skipped")
    .map((story) => {
      const page = story.landing_page_manifest || {};
      const geo = page.geo_routing || {};
      const disclosure = page.disclosure_block || {};
      const newsletter = page.newsletter_capture || {};
      return {
        story_id: story.story_id,
        title: story.title,
        landing_page_route: page.landing_page_route || null,
        route_status: story.direct_landing_status === "pass" ? "prepared" : "blocked",
        regions: {
          UK: normaliseRegionRoute(geo.regions?.UK, page.landing_page_route),
          US: normaliseRegionRoute(geo.regions?.US, page.landing_page_route),
        },
        platform_routes: page.tracking_utm?.platforms || {},
        disclosure: {
          required: disclosure.required === true,
          status: disclosure.status || "unknown",
          copy_present: Boolean(cleanText(collectStrings(disclosure.copy).join(" "))),
        },
        newsletter_capture: {
          enabled: newsletter.enabled === true,
          placement: newsletter.placement || null,
        },
        source_link_count: asArray(page.source_list).length,
        direct_offer_present: Boolean(story.link_pack?.primary_link || asArray(story.link_pack?.fallback_links).length),
        revenue_tracking_present: Boolean(story.revenue_tracking),
        safety: {
          local_proof_only: true,
          no_external_posting: true,
          no_live_geo_redirect_mutation: true,
  ...
  return {
    ...
    generated_at: report.generated_at || null,
    mode: "LOCAL_PROOF",
    verdict: stories.some((story) => story.route_status === "blocked") ? "blocked" : "pass",
    stories,
    safety: {
      local_proof_only: true,
      no_external_posting: true,
...
function zeroedPlatformAttribution(story = {}) {
  const page = story.landing_page_manifest || {};
  const tracking = story.revenue_tracking || {};
  const clicks = tracking.platform_clicks || {};
  const revenue = tracking.revenue || {};
  return PLATFORM_KEYS.reduce((out, platform) => {
    out[platform] = {
      landing_page_url: page.tracking_utm?.platforms?.[platform] || null,
      clicks: Number(clicks[platform] || 0),
      affiliate_clicks: 0,
      conversions: 0,
      revenue: {
        amount: 0,
        currency: revenue.currency || "GBP",
      },
    };
    return out;
  }, {});
...
function buildRevenueAttributionManifest(report = {}) {
  const stories = asArray(report.stories)
    .filter((story) => story.status !== "skipped")
    .map((story) => {
      const page = story.landing_page_manifest || {};
      ...
      return {
        story_id: story.story_id,
        landing_page_route: page.landing_page_route || null,
        primary_offer_id: story.link_pack?.primary_link?.id || null,
        platforms: zeroedPlatformAttribution(story),
        landing_page_visits: Number(tracking.landing_page_visits || 0),
        newsletter_signups: Number(tracking.newsletter_signups || 0),
        affiliate_clicks: Number(tracking.affiliate_clicks || 0),
        conversions: Number(tracking.conversions || 0),
        revenue: {
          amount: 0,
          currency: tracking.revenue?.currency || "GBP",
          source: tracking.revenue?.source || "waiting_for_affiliate_network_reporting",
        },
...
async function buildGoal16LandingPageEngine({
  ...
  const report = {
    ...
    summary: {
      ...
      uk_us_route_map_story_count: activeStories.filter((story) => story.landing_page_manifest.geo_routing?.regions?.UK && story.landing_page_manifest.geo_routing?.regions?.US).length,
      revenue_attribution_manifest_story_count: activeStories.filter((story) => story.revenue_tracking).length,
    },
  ...
  report.commerce_route_map = buildCommerceRouteMap(report);
  report.revenue_attribution_manifest = buildRevenueAttributionManifest(report);
  return report;
}
...
function renderGoal16LandingPageEngineMarkdown(report = {}) {
  ...
  lines.push(`UK/US commerce route map stories: ${report.summary?.uk_us_route_map_story_count || 0}`);
  lines.push(`Revenue attribution manifest stories: ${report.summary?.revenue_attribution_manifest_story_count || 0}`);
  lines.push("");
  lines.push("## Blockers");
...
async function writeGoal16LandingPageEngine(report = {}, { outputDir } = {}) {
  ...
  const commerceRouteMap = path.join(outDir, "uk_us_route_map.json");
  const revenueAttributionManifest = path.join(outDir, "revenue_attribution_manifest.json");
  ...
  await fs.writeJson(commerceRouteMap, report.commerce_route_map || buildCommerceRouteMap(report), { spaces: 2 });
  await fs.writeJson(
    revenueAttributionManifest,
    report.revenue_attribution_manifest || buildRevenueAttributionManifest(report),
    { spaces: 2 },
  );
  return {
...
module.exports = {
  ...
  buildRevenueAttributionManifest,
