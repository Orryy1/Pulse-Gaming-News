...
function buildPublishBlockersArtifact(report = {}) {
  const stories = asArray(report.stories).map((story) => ({
    story_id: story.story_id,
...
async function buildGoal17PlatformPolicyEngine({
  ...
  const directPassStories = stories.filter((story) => story.direct_policy_status === "pass");
  const directBlockedStories = stories.filter((story) => story.direct_policy_status !== "pass");
  const verdict = !stories.length
    ...
    : blockedStories.length && readyStories.length
      ...
      : blockedStories.length
        ? "BLOCKED"
        : "PASS";
  const directPolicyVerdict = !stories.length
    ? "FAIL"
  ...
  const report = {
    ...
    summary: {
      ...
      youtube_reused_content_checked_story_count: stories.filter((story) => story.policy_checks.youtube_reused_content_risk).length,
      affiliate_disclosure_required_story_count: stories.filter((story) => story.evidence.affiliate_required).length,
      finance_crypto_review_required_story_count: stories.filter((story) => story.evidence.finance_crypto_vertical !== "non_financial").length,
      publish_now_count: 0,
    },
