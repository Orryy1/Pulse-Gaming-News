...
const FINANCE_TOPIC_RE =
  /\b(?:finance|financial|stock|stocks|shares|shareholder|investor|investment fund|investment trust|trading platform|pension|isa|mortgage|earnings|market cap|asset manager)\b/i;
...
function buildApprovedWording(report = {}) {
  return {
    ...
    generated_at: report.generated_at || null,
    mode: "LOCAL_PROOF",
    stories: asArray(report.stories).map((story) => story.approved_wording),
    safety: {
...
async function buildGoal18FinanceCryptoFirewall({
  ...
  const directPassStories = stories.filter((story) => story.direct_finance_crypto_status === "pass");
  const directBlockedStories = stories.filter((story) => story.direct_finance_crypto_status !== "pass");
  const verdict = !stories.length
    ...
    : blockedStories.length && readyStories.length
      ...
      : blockedStories.length
        ? "BLOCKED"
        : "PASS";
  const directFinanceCryptoVerdict = !stories.length
    ? "FAIL"
  ...
  const report = {
    ...
    summary: {
      ...
      finance_crypto_topic_story_count: stories.filter((story) => story.finance_crypto_risk.topic).length,
      safe_finance_format_story_count: stories.filter((story) => story.finance_crypto_risk.status === "allowed_safe_format").length,
      unsafe_finance_crypto_story_count: stories.filter((story) => story.finance_crypto_risk.status === "blocked").length,
      publish_now_count: 0,
    },
