...
function statusPasses(value) {
  return ["ready", "pass", "passed", "green", "clear", "ok", "approved", "local_proof_prepared"].includes(
...
function policyReportPasses(policy = {}) {
  if (!policy || typeof policy !== "object" || !Object.keys(policy).length) return false;
  const failures = unique([
    ...asArray(policy.failures),
    ...asArray(policy.blockers),
    ...asArray(policy.publish_blockers),
    ...gateFailures(policy.finance_crypto_firewall || {}),
  ]);
  const status = policy.verdict || policy.status || policy.result || policy.overall_verdict;
  return !failures.length && (statusPasses(status) || !["fail", "failed", "red", "blocked"].includes(normaliseStatus(status)));
}

function upstreamSatisfiedByPackageEvidence({ storyId = "", policyIndex = new Map(), policy = {}, affiliate = {}, landing = {} } = {}) {
  const row = policyIndex.get(cleanText(storyId));
  if (!row) return false;
  const directStatus = row.direct_policy_status || row.direct_policy_verdict;
  const directBlockers = asArray(row.direct_policy_blockers || row.direct_policy_failures);
  return (
    statusPasses(directStatus) &&
    !directBlockers.length &&
    policyReportPasses(policy) &&
...
function upstreamBlockers(storyId, policyIndex = new Map(), packageEvidence = {}) {
  const row = policyIndex.get(cleanText(storyId));
  ...
  if (packageEvidence.policy_satisfied_by_package_evidence === true) return [];
...
async function inspectStoryPackage(storyPackage = {}, context = {}) {
  ...
  const policySatisfiedByPackageEvidence = upstreamSatisfiedByPackageEvidence({
    storyId,
    policyIndex: context.policyIndex,
    policy: platformPolicyReport,
    affiliate,
    landing,
  });
  const upstream = upstreamBlockers(storyId, context.policyIndex, {
    policy_satisfied_by_package_evidence: policySatisfiedByPackageEvidence,
  });
  ...
  return {
    ...
    source_material: {
      ...
      upstream_policy_satisfied_by_package_evidence: policySatisfiedByPackageEvidence,
    },
    safety: {
      local_proof_only: true,
      no_publish_triggered: true,
