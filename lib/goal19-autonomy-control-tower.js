...
function usedSfxAssets(sfxManifest = {}) {
  return asArray(sfxManifest.source_plan?.selected_assets)
    .filter((asset) => asset && typeof asset === "object" && asset.used_in_final_render !== false)
    .map((asset, index) => ({
...
async function inspectRightsAssignment(
  ...
  const assetDeclaredPath = cleanText(
    record.path || record.local_path || record.file || record.output_path || asset.path,
  );
  ...
  const assetFile = await hashReadableFile(
    rightsEvidencePathCandidates(assetDeclaredPath, artifactDir, workspaceRoot),
    cache,
  );
...
function upstreamBlockers(storyId, firewallIndex = new Map()) {
  ...
  const status = normaliseStatus(statusFrom(row));
...
async function inspectStoryPackage(storyPackage = {}, context = {}) {
  ...
  const finalMedia = await inspectCriticalFinalMedia(renderManifest, context.workspaceRoot, artifactDir);
  ...
  const finalVerdict = directBlockers.length || upstream.length
    ? "RED"
    : approval.required || directWarnings.length
      ? "AMBER"
      : "GREEN";
