...
const {
  materialisePulseEditorialAuthorityBundle,
} = require("./pulse-editorial-authority-materializer");
const {
  verifyPulseEditorialExceptionPlanAuthority,
} = require("./studio/v5/pulse-visual-editorial-policy");

const GOAL_ID = "19_autonomy_control_tower";
const DEFAULT_LIVE_PLATFORM_RIGHTS_FAMILIES = ["youtube", "instagram", "facebook"];
const PLATFORM_EVIDENCE_MAX_AGE_MS = 24 * 60 * 60 * 1000;
const PLATFORM_EVIDENCE_MAX_FUTURE_SKEW_MS = 5 * 60 * 1000;
const MIN_PLATFORM_VARIANT_SSIM = 0.9;
...
function canonicalPlatformKey(value) {
  const platform = normaliseStatus(platformKey(value));
  if (/^youtube(?:_|$)/.test(platform)) return "youtube_shorts";
  if (/^instagram(?:_|$)/.test(platform)) return "instagram_reels";
  if (/^facebook(?:_|$)/.test(platform)) return "facebook_reels";
  if (/^tiktok(?:_|$)/.test(platform)) return "tiktok";
  if (/^(?:x|twitter)(?:_|$)/.test(platform)) return "x";
  if (/^threads(?:_|$)/.test(platform)) return "threads";
  if (/^pinterest(?:_|$)/.test(platform)) return "pinterest";
...
function enabledPlatformAuthority(platformStatusMatrix = {}) {
  if (!hasObject(platformStatusMatrix)) {
    return {
      present: false,
      generated_at: null,
      enabled_platforms: [],
      failures: ["platform_pack:enabled_platform_authority_missing"],
    };
  }
  const generatedAt = cleanText(
    platformStatusMatrix.generated_at || platformStatusMatrix.generatedAt,
  );
  const rows = hasObject(platformStatusMatrix.platforms)
    ? Object.entries(platformStatusMatrix.platforms)
    : hasObject(platformStatusMatrix.operational)
      ? Object.entries(platformStatusMatrix.operational)
      : [];
  const enabledPlatforms = unique(
    rows
      .filter(([, row]) =>
        normaliseStatus(row?.operational_state || row?.state || row?.status) === "enabled")
      .map(([key, row]) => canonicalPlatformKey(row?.platform || key))
      .filter(Boolean),
  );
  const failures = unique([
    ...(!timestampMs(generatedAt)
      ? ["platform_pack:enabled_platform_authority_generated_at_missing_or_invalid"]
      : []),
    ...(rows.length
      ? []
      : ["platform_pack:enabled_platform_authority_platforms_missing"]),
    ...(enabledPlatforms.length
      ? []
      : ["platform_pack:enabled_platform_authority_has_no_enabled_platforms"]),
  ]);
  return {
    present: true,
...
async function verifyDirectorEditorialAuthority(directorPlan = {}) {
  const plan = directorPlan.editorial_exception_plan || {};
  const entries = asArray(plan.entries);
  const required = plan.used === true || entries.length > 0;
  if (!required) {
    return {
      required: false,
      valid: true,
  ...
  const bundleId = cleanText(integration.bundle_id);
  const motionCanvasContract = directorPlan.motion_canvas_contract || {};
  const boundedExceptionContract =
    motionCanvasContract.bounded_editorial_exception || {};
  const blockers = unique([
    ...(motionCanvasContract.contract_id ===
      "pulse_motion_canvas_contract_v1" &&
    plan.contract_id === motionCanvasContract.contract_id &&
    motionCanvasContract.required_timeline_coverage_ratio === 1 &&
    motionCanvasContract.required_full_bleed_coverage_ratio === 1 &&
    motionCanvasContract.rights_ledger_still_authoritative === true &&
    boundedExceptionContract.source_rights_verdict_required === "AMBER" &&
    boundedExceptionContract.human_review_required === true &&
    boundedExceptionContract.exact_window_required === true
      ? []
      : ["director:motion_canvas_contract_missing_or_mismatched"]),
    ...(plan.planning_valid === true &&
    plan.verdict === "GOVERNANCE_RESOLVED_FOR_EPISODE"
      ? []
      : ["director:editorial_exception_plan_unresolved"]),
    ...(integration.schema_version === 1 &&
    integration.schema ===
      "pulse_visual_v4_editorial_authority_integration_v1" &&
    integration.verdict === "PASS" &&
    ["LOCAL_PROOF", "DRY_RUN_PUBLISH"].includes(
      cleanText(integration.operating_mode).toUpperCase(),
    ) &&
    bundleId
      ? []
      : ["director:editorial_authority_materialisation_missing_or_invalid"]),
    ...(cleanText(plan.episode_id) &&
    cleanText(plan.episode_id) === cleanText(directorPlan.story_id) &&
    cleanText(integration.episode_id) === cleanText(directorPlan.story_id)
      ? []
      : ["director:editorial_authority_episode_binding_mismatch"]),
  ]);
  if (!bundleId) {
    return {
      required: true,
  ...
  let materialised;
  try {
    materialised = await materialisePulseEditorialAuthorityBundle({
      bundle_id: bundleId,
    });
  } catch {
    blockers.push("director:editorial_authority_registered_bundle_unavailable");
    return {
      required: true,
      valid: false,
  ...
  if (
    canonicalJson(integration.materialisation_report || {}) !==
    canonicalJson(materialised.report || {})
  ) {
    blockers.push("director:editorial_authority_materialisation_report_stale_or_forged");
  }
  if (
    cleanText(integration.operating_mode).toUpperCase() !==
    cleanText(materialised.report?.mode).toUpperCase()
  ) {
    blockers.push("director:editorial_authority_operating_mode_mismatch");
  }
  const authorityVerification = verifyPulseEditorialExceptionPlanAuthority({
    plan,
    trusted_rights_authority: materialised.trusted_rights_authority,
    trusted_acceptance_authority: materialised.trusted_acceptance_authority,
  });
  blockers.push(
    ...asArray(authorityVerification.blockers).map(
      (blocker) => `director:${blocker}`,
    ),
  );
  const serializedAuthorityEvidence = plan.authority_evidence || {};
  if (
    canonicalJson(serializedAuthorityEvidence) !==
    canonicalJson(authorityVerification.authority_evidence || {})
  ) {
    blockers.push("director:editorial_authority_plan_evidence_mismatch");
  }
  const integrationEvidence = integration.evidence_binding || {};
  const expectedIntegrationEvidence = {
    rights: {
      raw_sha256:
        cleanText(authorityVerification.authority_evidence?.rights?.raw_sha256).toLowerCase() ||
        null,
      canonical_sha256:
        cleanText(
          authorityVerification.authority_evidence?.rights?.canonical_sha256,
        ).toLowerCase() || null,
      size_bytes:
        Number(authorityVerification.authority_evidence?.rights?.bytes) || null,
      actor: authorityVerification.authority_evidence?.rights?.actor || null,
    },
    acceptance: {
      raw_sha256:
        cleanText(
          authorityVerification.authority_evidence?.acceptance?.raw_sha256,
        ).toLowerCase() || null,
      canonical_sha256:
        cleanText(
          authorityVerification.authority_evidence?.acceptance
            ?.canonical_sha256,
        ).toLowerCase() || null,
      size_bytes:
        Number(authorityVerification.authority_evidence?.acceptance?.bytes) ||
        null,
      actor:
        authorityVerification.authority_evidence?.acceptance?.actor || null,
    },
    blockers: [],
  };
  if (
    canonicalJson(integrationEvidence) !==
    canonicalJson(expectedIntegrationEvidence)
  ) {
    blockers.push("director:editorial_authority_integration_evidence_mismatch");
  }
  return {
    required: true,
    valid:
      blockers.length === 0 &&
      authorityVerification.valid === true &&
      authorityVerification.verified_entry_count === entries.length,
    blockers: unique(blockers),
    bundle_id: bundleId,
    verified_entry_count: authorityVerification.verified_entry_count || 0,
    materialisation_report: materialised.report,
  };
}

async function checkDirectorPlan(directorPlan = {}, visualQuality = {}, renderManifest = {}) {
  ...
  const finalMotionPasses = finalVisualMotionPasses({ visualQuality, renderManifest, requiredMotionCount, requiredFamilyCount });
  const authorityVerification = await verifyDirectorEditorialAuthority(directorPlan);
  const blockers = unique([
    ...failuresFrom(directorPlan),
    ...failuresFrom(readiness),
    ...authorityVerification.blockers,
  ]).filter((blocker) => {
  ...
  return buildCheck({
    ...
    evidence: {
      ...
      blockers,
      final_post_render_motion_evidence_passed: finalMotionPasses,
      authority_required: authorityVerification.required,
      authority_valid: authorityVerification.valid,
      authority_bundle_id: authorityVerification.bundle_id,
      authority_verified_entry_count:
        authorityVerification.verified_entry_count,
      authority_failures: authorityVerification.blockers,
    },
  });
}
...
function checkPulseMediaHouseScore(
  score = {},
  storyId = "",
  finalMedia = {},
  renderManifest = {},
  ...
  const scoreStoryId = cleanText(score.story_id || score.storyId);
  const scoreGeneratedAt = cleanText(score.generated_at || score.generatedAt);
  const scoreGeneratedAtMs = timestampMs(scoreGeneratedAt);
  const renderGeneratedAt = cleanText(
    renderManifest.generated_at || renderManifest.generatedAt || renderManifest.rendered_at,
  );
  const renderGeneratedAtMs = timestampMs(renderGeneratedAt);
  const scoreRenderRunId = cleanText(
    score.source_render_run_id ||
      score.render_run_id ||
      score.final_video_report?.source_render_run_id,
  );
  const currentRenderRunId = cleanText(
    renderManifest.run_id || renderManifest.render_run_id || renderManifest.generation_id,
  );
  const scoreVideoSha256 = cleanText(
    score.final_video_report?.sha256 ||
      score.final_video_report?.output_sha256 ||
      score.final_media?.sha256,
  ).replace(/^sha256:/i, "").toLowerCase();
  const scoreVideoSize = numeric(
    score.final_video_report?.size_bytes ||
      score.final_video_report?.bytes ||
      score.final_media?.size_bytes,
    0,
  );
  const bindingFailures = unique([
    ...(!scoreStoryId
      ? ["media_house:story_id_missing"]
      : !storyId || scoreStoryId !== storyId
        ? ["media_house:story_id_mismatch"]
        : []),
    ...(!renderGeneratedAtMs
      ? ["media_house:render_generated_at_missing_or_invalid"]
      : []),
    ...(!scoreGeneratedAtMs
      ? ["media_house:score_generated_at_missing_or_invalid"]
      : renderGeneratedAtMs && scoreGeneratedAtMs < renderGeneratedAtMs
        ? ["media_house:score_generated_before_render"]
        : []),
    ...(currentRenderRunId && !scoreRenderRunId
      ? ["media_house:source_render_run_id_missing"]
      : currentRenderRunId && scoreRenderRunId !== currentRenderRunId
        ? ["media_house:source_render_run_id_mismatch"]
        : []),
    ...(!/^[a-f0-9]{64}$/.test(scoreVideoSha256)
      ? ["media_house:final_video_sha256_missing_or_invalid"]
      : scoreVideoSha256 !== cleanText(finalMedia.output_sha256).toLowerCase()
        ? ["media_house:final_video_sha256_mismatch"]
        : []),
    ...(scoreVideoSize <= 0
      ? ["media_house:final_video_size_missing_or_invalid"]
      : scoreVideoSize !== numeric(finalMedia.output_bytes, 0)
        ? ["media_house:final_video_size_mismatch"]
        : []),
    ...(score.final_video_report?.decodable === true
      ? []
      : ["media_house:final_video_decode_not_verified"]),
  ]);
  const passed =
    hasObject(score) &&
  ...
  return buildCheck({
    ...
    evidence: {
      ...
      story_id: scoreStoryId || null,
      expected_story_id: storyId || null,
      generated_at: scoreGeneratedAt || null,
      render_generated_at: renderGeneratedAt || null,
      source_render_run_id: scoreRenderRunId || null,
      current_render_run_id: currentRenderRunId || null,
      final_video_sha256: scoreVideoSha256 || null,
      current_final_video_sha256: finalMedia.output_sha256 || null,
      final_video_size_bytes: scoreVideoSize || null,
      current_final_video_size_bytes: finalMedia.output_bytes || null,
...
function platformVariantStoryId(output = {}) {
  return cleanText(
    output.story_id ||
      output.storyId ||
      output.platform_variant_render?.story_id ||
      output.platform_variant_render?.storyId,
  );
}

function platformVariantGeneratedAt(output = {}) {
  return cleanText(
    output.generated_at ||
      output.generatedAt ||
      output.platform_variant_render?.generated_at ||
      output.platform_variant_render?.generatedAt ||
      output.platform_variant_render?.rendered_at,
  );
}

function platformVariantSourceRenderRunId(output = {}) {
  return cleanText(
    output.source_render_run_id ||
      output.render_run_id ||
      output.platform_variant_render?.source_render_run_id ||
      output.platform_variant_render?.render_run_id,
  );
}

function publishVerdictSourceRenderHash(publishVerdict = {}) {
  return cleanText(
    publishVerdict.source_render_sha256 ||
      publishVerdict.source_video_sha256 ||
      publishVerdict.final_render_sha256 ||
      publishVerdict.render_sha256,
  ).replace(/^sha256:/i, "").toLowerCase();
}

function publishVerdictSourceRenderRunId(publishVerdict = {}) {
  return cleanText(
    publishVerdict.source_render_run_id ||
      publishVerdict.render_run_id ||
      publishVerdict.generation_id,
  );
}
...
function evidenceExpired(evidenceTimestampMs, controlTowerTimestampMs) {
  return Boolean(
    evidenceTimestampMs &&
      controlTowerTimestampMs &&
      controlTowerTimestampMs - evidenceTimestampMs > PLATFORM_EVIDENCE_MAX_AGE_MS,
  );
}

function evidenceGeneratedInFuture(evidenceTimestampMs, controlTowerTimestampMs) {
  return Boolean(
    evidenceTimestampMs &&
      controlTowerTimestampMs &&
      evidenceTimestampMs - controlTowerTimestampMs > PLATFORM_EVIDENCE_MAX_FUTURE_SKEW_MS,
  );
}

async function verifyPlatformVariantDerivation(
  sourcePath,
  variantPath,
  {
    transformationMode = "",
  ...
  if (
    transformationMode === "validated_passthrough" &&
    sourceSha256 &&
    sourceSha256 === variantSha256
  ) {
    return {
      verified: true,
      method: "byte_identical_validated_passthrough",
      ssim: 1,
      failures: [],
    };
  }
  if (transformationMode !== "transcode") {
    return {
      verified: false,
      method: "unsupported_transformation_mode",
      ssim: null,
      failures: ["transformation_mode_missing_or_unsupported"],
    };
  }
  if (!sourcePath || !variantPath) {
    return {
      ...
      method: "decoded_ssim",
      ssim: null,
      failures: ["derivation_input_missing"],
    };
  }
  try {
    const result = await runExecFile(
      ...
      [
        ...
        variantPath,
        "-filter_complex",
        "[0:v]scale=160:284:flags=bilinear,setsar=1[src];[1:v]scale=160:284:flags=bilinear,setsar=1[variant];[src][variant]ssim=shortest=0:repeatlast=1",
        "-an",
        "-f",
        "null",
        "-",
    ...
    const match = output.match(/\bAll:([0-9.]+)/i);
    const ssim = match ? Number(match[1]) : Number.NaN;
    if (!Number.isFinite(ssim)) {
      return {
        verified: false,
        method: "decoded_ssim",
        ssim: null,
        failures: ["derivation_similarity_missing"],
      };
    }
    return {
      verified: ssim >= MIN_PLATFORM_VARIANT_SSIM,
      method: "decoded_ssim",
      ssim,
      minimum_ssim: MIN_PLATFORM_VARIANT_SSIM,
      failures: ssim >= MIN_PLATFORM_VARIANT_SSIM ? [] : ["derivation_similarity_below_minimum"],
    };
  } catch (error) {
    return {
      verified: false,
      method: "decoded_ssim",
      ssim: null,
      failures: [`derivation_probe_failed:${cleanText(error?.code || error?.message) || "unknown"}`],
    };
  }
}

function enabledPlatformOutputEntries(platformManifest = {}, expectedEnabledPlatforms = []) {
  ...
  const authoritativeEnabled = unique(
    asArray(expectedEnabledPlatforms).map(canonicalPlatformKey).filter(Boolean),
  );
  const exactOutputRequired = authoritativeEnabled.length > 0 || explicitlyEnabled.length > 0;
  const required = authoritativeEnabled.length
    ? authoritativeEnabled
    : explicitlyEnabled.length
      ? unique(explicitlyEnabled.map(canonicalPlatformKey))
      : requiredRightsPlatforms(platformManifest);
  return required.map((platform) => {
    const exact = outputs[platform];
    if (exact && typeof exact === "object") return [platform, exact];
    if (exactOutputRequired) return [platform, {}];
...
async function inspectPlatformVariant(
  ...
  controlTowerGeneratedAt = "",
) {
  const failures = [];
  const warnings = [];
  const materialization = output.platform_variant_render;
  const hasMaterializationEvidence = hasObject(materialization);
  const scorecardMaterialization =
    platformVariantScorecard.variants?.[platform] &&
    typeof platformVariantScorecard.variants[platform] === "object"
      ? platformVariantScorecard.variants[platform]
      : {};
  const hasIndependentMaterializationEvidence = hasObject(scorecardMaterialization);
  ...
  const variantStoryId = platformVariantStoryId(output);
  const variantGeneratedAt = platformVariantGeneratedAt(output);
  const variantGeneratedAtMs = timestampMs(variantGeneratedAt);
  const controlTowerGeneratedAtMs = timestampMs(controlTowerGeneratedAt);
  const renderGeneratedAt = cleanText(
    renderManifest.generated_at || renderManifest.generatedAt || renderManifest.rendered_at,
  );
  const renderGeneratedAtMs = timestampMs(renderGeneratedAt);
  const sourceRenderRunId = platformVariantSourceRenderRunId(output);
  const currentRenderRunId = cleanText(
    renderManifest.run_id || renderManifest.render_run_id || renderManifest.generation_id,
  );
  const producerId = cleanText(materialization?.producer_id);
  const materializationRunId = cleanText(materialization?.materialization_run_id);
  const materializationPlatform = canonicalPlatformKey(materialization?.platform);
  const encoderProfile = cleanText(materialization?.encoder_profile);
  const transformationMode = normaliseStatus(materialization?.transformation_mode);
  const passthroughApproved = materialization?.passthrough_approved === true;
  const materializationGeneratedAt = platformVariantGeneratedAt({
    platform_variant_render: materialization,
  });
  const materializationGeneratedAtMs = timestampMs(materializationGeneratedAt);
  const materializationFingerprint = {
    producer_id: producerId,
    materialization_run_id: materializationRunId,
    story_id: platformVariantStoryId({ platform_variant_render: materialization }),
    platform: materializationPlatform,
    encoder_profile: encoderProfile,
    transformation_mode: transformationMode,
    passthrough_approved: passthroughApproved,
    source_render_run_id: platformVariantSourceRenderRunId({
      platform_variant_render: materialization,
    }),
    source_render_sha256: platformVariantSourceRenderHash({
      platform_variant_render: materialization,
    }),
    output_sha256: platformVariantExpectedHash({
      platform_variant_render: materialization,
    }),
    output_size_bytes: platformVariantExpectedSize({
      platform_variant_render: materialization,
    }),
    generated_at: platformVariantGeneratedAt({
      platform_variant_render: materialization,
    }),
  };
  const scorecardFingerprint = {
    producer_id: cleanText(scorecardMaterialization.producer_id),
    materialization_run_id: cleanText(scorecardMaterialization.materialization_run_id),
    story_id: platformVariantStoryId({
      platform_variant_render: scorecardMaterialization,
    }),
    platform: canonicalPlatformKey(scorecardMaterialization.platform),
    encoder_profile: cleanText(scorecardMaterialization.encoder_profile),
    transformation_mode: normaliseStatus(scorecardMaterialization.transformation_mode),
    passthrough_approved: scorecardMaterialization.passthrough_approved === true,
    source_render_run_id: platformVariantSourceRenderRunId({
      platform_variant_render: scorecardMaterialization,
    }),
    source_render_sha256: platformVariantSourceRenderHash({
  ...
  const outputFingerprint = {
    story_id: cleanText(output.story_id || output.storyId),
    source_render_run_id: cleanText(output.source_render_run_id || output.render_run_id),
    source_render_sha256: cleanText(
      output.source_render_sha256 ||
        output.source_video_sha256 ||
        output.parent_render_sha256 ||
        output.final_render_sha256,
    ).toLowerCase(),
    output_sha256: cleanText(
      output.variant_sha256 || output.video_sha256 || output.output_sha256,
    ).toLowerCase(),
    output_size_bytes: numeric(
      output.variant_size_bytes ||
        output.video_size_bytes ||
        output.output_size_bytes ||
        output.size_bytes,
  ...
  const receiptOutputFingerprint = {
    story_id: materializationFingerprint.story_id,
    source_render_run_id: materializationFingerprint.source_render_run_id,
    source_render_sha256: materializationFingerprint.source_render_sha256,
    output_sha256: materializationFingerprint.output_sha256,
    output_size_bytes: materializationFingerprint.output_size_bytes,
  };

  if (!hasMaterializationEvidence) {
    failures.push(`platform_pack:${platform}:materialization_evidence_missing`);
  }
  if (!hasIndependentMaterializationEvidence) {
    failures.push(`platform_pack:${platform}:materialization_scorecard_evidence_missing`);
  } else if (
    JSON.stringify(materializationFingerprint) !== JSON.stringify(scorecardFingerprint)
  ) {
    failures.push(`platform_pack:${platform}:materialization_scorecard_mismatch`);
  }
  if (JSON.stringify(outputFingerprint) !== JSON.stringify(receiptOutputFingerprint)) {
    failures.push(`platform_pack:${platform}:output_receipt_mismatch`);
  }
  if (producerId !== "pulse-goal-platform-variant-materializer") {
    failures.push(`platform_pack:${platform}:materializer_producer_id_untrusted`);
  }
  if (!materializationRunId) {
    failures.push(`platform_pack:${platform}:materialization_run_id_missing`);
  }
  if (!materializationPlatform) {
    failures.push(`platform_pack:${platform}:materialization_platform_missing`);
  } else if (materializationPlatform !== canonicalPlatformKey(platform)) {
    failures.push(`platform_pack:${platform}:materialization_platform_mismatch`);
  }
  if (!encoderProfile) {
    failures.push(`platform_pack:${platform}:encoder_profile_missing`);
  }
  if (!materializationGeneratedAtMs) {
    failures.push(`platform_pack:${platform}:materialization_generated_at_missing_or_invalid`);
  } else if (renderGeneratedAtMs && materializationGeneratedAtMs < renderGeneratedAtMs) {
    failures.push(`platform_pack:${platform}:materialization_generated_before_render`);
  } else if (
    evidenceGeneratedInFuture(materializationGeneratedAtMs, controlTowerGeneratedAtMs)
  ) {
    failures.push(`platform_pack:${platform}:materialization_generated_in_future`);
  } else if (evidenceExpired(materializationGeneratedAtMs, controlTowerGeneratedAtMs)) {
    failures.push(`platform_pack:${platform}:materialization_evidence_expired`);
  }
  if (!fingerprint) failures.push(`platform_pack:${platform}:variant_missing_or_unreadable`);
  if (!variantStoryId) {
    failures.push(`platform_pack:${platform}:variant_story_id_missing`);
  } else if (!storyId || variantStoryId !== storyId) {
    failures.push(`platform_pack:${platform}:variant_story_id_mismatch`);
  }
  if (!renderGeneratedAtMs) {
    failures.push(`platform_pack:${platform}:render_generated_at_missing_or_invalid`);
  }
  if (!variantGeneratedAtMs) {
    failures.push(`platform_pack:${platform}:variant_generated_at_missing_or_invalid`);
  } else if (renderGeneratedAtMs && variantGeneratedAtMs < renderGeneratedAtMs) {
    failures.push(`platform_pack:${platform}:variant_generated_before_render`);
  } else if (evidenceGeneratedInFuture(variantGeneratedAtMs, controlTowerGeneratedAtMs)) {
    failures.push(`platform_pack:${platform}:variant_generated_in_future`);
  } else if (evidenceExpired(variantGeneratedAtMs, controlTowerGeneratedAtMs)) {
    failures.push(`platform_pack:${platform}:variant_evidence_expired`);
  }
  if (currentRenderRunId) {
    if (!sourceRenderRunId) {
      failures.push(`platform_pack:${platform}:source_render_run_id_missing`);
    } else if (sourceRenderRunId !== currentRenderRunId) {
      failures.push(`platform_pack:${platform}:source_render_run_id_mismatch`);
    }
  }
  if (!/^[a-f0-9]{64}$/.test(expectedHash)) {
  ...
  if (
    fingerprint &&
    currentRenderHash &&
    fingerprint.sha256 === currentRenderHash &&
    !(transformationMode === "validated_passthrough" && passthroughApproved)
  ) {
    failures.push(`platform_pack:${platform}:master_identical_without_passthrough_proof`);
  }
  ...
  const derivation = fingerprint && finalMedia.resolved_path
    ? await verifyPlatformVariantDerivation(finalMedia.resolved_path, fingerprint.path, {
        transformationMode,
        sourceSha256: currentRenderHash,
        variantSha256: fingerprint.sha256,
      })
    : {
        verified: false,
        method: "decoded_ssim",
  ...
  if (!derivation.verified) {
    failures.push(`platform_pack:${platform}:variant_derivation_unproven`);
  }
  ...
  return {
    ...
    story_id: variantStoryId || null,
    expected_story_id: storyId || null,
    generated_at: variantGeneratedAt || null,
    render_generated_at: renderGeneratedAt || null,
    source_render_run_id: sourceRenderRunId || null,
    current_render_run_id: currentRenderRunId || null,
    materializer_producer_id: producerId || null,
    materialization_run_id: materializationRunId || null,
    materialization_platform: materializationPlatform || null,
    encoder_profile: encoderProfile || null,
    transformation_mode: transformationMode || null,
    passthrough_approved: passthroughApproved,
    independent_materialization_evidence_present: hasIndependentMaterializationEvidence,
    decodable,
...
async function checkPlatformPack(
  ...
  const manifestStoryId = cleanText(platformManifest.story_id || platformManifest.storyId);
  const manifestGeneratedAt = cleanText(
    platformManifest.generated_at ||
      platformManifest.generatedAt ||
      platformManifest.platform_variant_materialized_at,
  );
  const manifestGeneratedAtMs = timestampMs(manifestGeneratedAt);
  const renderGeneratedAt = cleanText(
    renderManifest.generated_at || renderManifest.generatedAt || renderManifest.rendered_at,
  );
  const renderGeneratedAtMs = timestampMs(renderGeneratedAt);
  const controlTowerGeneratedAtMs = timestampMs(controlTowerGeneratedAt);
  const verdictStoryId = cleanText(publishVerdict.story_id || publishVerdict.storyId);
  const verdictGeneratedAt = cleanText(
    publishVerdict.generated_at || publishVerdict.generatedAt,
  );
  const verdictGeneratedAtMs = timestampMs(verdictGeneratedAt);
  const verdictSourceRenderHash = publishVerdictSourceRenderHash(publishVerdict);
  const verdictSourceRenderRunId = publishVerdictSourceRenderRunId(publishVerdict);
  const currentRenderHash = cleanText(finalMedia.output_sha256).toLowerCase();
  const currentRenderRunId = cleanText(
    renderManifest.run_id || renderManifest.render_run_id || renderManifest.generation_id,
  );
  const scorecardStatus = cleanText(statusFrom(platformVariantScorecard));
  const scorecardStoryId = cleanText(
    platformVariantScorecard.story_id || platformVariantScorecard.storyId,
  );
  const scorecardGeneratedAt = cleanText(
    platformVariantScorecard.generated_at ||
      platformVariantScorecard.generatedAt ||
      platformVariantScorecard.platform_variant_materialized_at,
  );
  const scorecardGeneratedAtMs = timestampMs(scorecardGeneratedAt);
  const scorecardProducerId = cleanText(platformVariantScorecard.producer_id);
  const authority = enabledPlatformAuthority(platformStatusMatrix);
  const declaredEnabledPlatforms = unique([
    ...asArray(platformManifest.enabled_platforms),
    ...asArray(platformManifest.live_enabled_platforms),
    ...asArray(platformManifest.publishable_platforms),
  ].map(canonicalPlatformKey).filter(Boolean));
  const requiredOutputs = enabledPlatformOutputEntries(
    platformManifest,
    authority.enabled_platforms,
  );
  ...
  const failures = unique([
    ...
    ...failuresFrom(platformVariantScorecard).map(
      (failure) => `platform_pack:variant_scorecard:${failure}`,
    ),
    ...(statusFails(scorecardStatus)
      ? ["platform_pack:variant_scorecard_authoritative_red"]
      : []),
    ...(!scorecardStatus
      ? ["platform_pack:variant_scorecard_status_missing"]
      : scorecardStatus.toUpperCase() !== "GREEN"
        ? ["platform_pack:variant_scorecard_not_green"]
        : []),
    ...(!scorecardStoryId
      ? ["platform_pack:variant_scorecard_story_id_missing"]
      : !storyId || scorecardStoryId !== storyId
        ? ["platform_pack:variant_scorecard_story_id_mismatch"]
        : []),
    ...(!scorecardGeneratedAtMs
      ? ["platform_pack:variant_scorecard_generated_at_missing_or_invalid"]
      : renderGeneratedAtMs && scorecardGeneratedAtMs < renderGeneratedAtMs
        ? ["platform_pack:variant_scorecard_generated_before_render"]
        : evidenceGeneratedInFuture(scorecardGeneratedAtMs, controlTowerGeneratedAtMs)
          ? ["platform_pack:variant_scorecard_generated_in_future"]
        : evidenceExpired(scorecardGeneratedAtMs, controlTowerGeneratedAtMs)
          ? ["platform_pack:variant_scorecard_evidence_expired"]
          : []),
    ...(scorecardProducerId !== "pulse-goal-platform-variant-materializer"
      ? ["platform_pack:variant_scorecard_producer_id_untrusted"]
      : []),
    ...authority.failures,
    ...authority.enabled_platforms
      .filter((platform) => !declaredEnabledPlatforms.includes(platform))
      .map((platform) => `platform_pack:enabled_platform_missing_from_manifest:${platform}`),
    ...variantEvidence.flatMap((row) => row.failures),
    ...(!manifestStoryId
      ? ["platform_pack:manifest_story_id_missing"]
      : !storyId || manifestStoryId !== storyId
        ? ["platform_pack:manifest_story_id_mismatch"]
        : []),
    ...(!manifestGeneratedAtMs
      ? ["platform_pack:manifest_generated_at_missing_or_invalid"]
      : renderGeneratedAtMs && manifestGeneratedAtMs < renderGeneratedAtMs
        ? ["platform_pack:manifest_generated_before_render"]
        : evidenceGeneratedInFuture(manifestGeneratedAtMs, controlTowerGeneratedAtMs)
          ? ["platform_pack:manifest_generated_in_future"]
        : evidenceExpired(manifestGeneratedAtMs, controlTowerGeneratedAtMs)
          ? ["platform_pack:manifest_evidence_expired"]
        : []),
    ...(!verdictStoryId
      ? ["platform_pack:verdict_story_id_missing"]
      : !storyId || verdictStoryId !== storyId
        ? ["platform_pack:verdict_story_id_mismatch"]
        : []),
    ...(!verdictGeneratedAtMs
      ? ["platform_pack:verdict_generated_at_missing_or_invalid"]
      : renderGeneratedAtMs && verdictGeneratedAtMs < renderGeneratedAtMs
        ? ["platform_pack:verdict_generated_before_render"]
        : evidenceGeneratedInFuture(verdictGeneratedAtMs, controlTowerGeneratedAtMs)
          ? ["platform_pack:verdict_generated_in_future"]
        : evidenceExpired(verdictGeneratedAtMs, controlTowerGeneratedAtMs)
          ? ["platform_pack:verdict_evidence_expired"]
        : []),
    ...(currentRenderRunId && !verdictSourceRenderRunId
      ? ["platform_pack:verdict_source_render_run_id_missing"]
      : currentRenderRunId && verdictSourceRenderRunId !== currentRenderRunId
        ? ["platform_pack:verdict_source_render_run_id_mismatch"]
        : []),
    ...(!/^[a-f0-9]{64}$/.test(verdictSourceRenderHash)
      ? ["platform_pack:verdict_source_render_hash_missing_or_invalid"]
      : !currentRenderHash || verdictSourceRenderHash !== currentRenderHash
        ? ["platform_pack:verdict_source_render_hash_mismatch"]
        : []),
  ...
  return buildCheck({
    ...
    evidence: {
      ...
      enabled_platform_authority_present: authority.present,
      enabled_platform_authority_generated_at: authority.generated_at,
      authoritative_enabled_platforms: authority.enabled_platforms,
      manifest_enabled_platforms: declaredEnabledPlatforms,
      story_id: manifestStoryId || null,
      expected_story_id: storyId || null,
      generated_at: manifestGeneratedAt || null,
      render_generated_at: renderGeneratedAt || null,
      verdict_story_id: verdictStoryId || null,
      verdict_generated_at: verdictGeneratedAt || null,
      verdict_source_render_sha256: verdictSourceRenderHash || null,
      verdict_source_render_run_id: verdictSourceRenderRunId || null,
      variant_scorecard_status: scorecardStatus || null,
      variant_scorecard_story_id: scorecardStoryId || null,
      variant_scorecard_generated_at: scorecardGeneratedAt || null,
      variant_scorecard_producer_id: scorecardProducerId || null,
      evidence_max_age_hours: PLATFORM_EVIDENCE_MAX_AGE_MS / (60 * 60 * 1000),
...
async function inspectStoryPackage(storyPackage = {}, context = {}) {
  ...
  const platformVariantScorecard = await readJsonIfPresent(
    path.join(artifactDir, "platform_variant_scorecard.json"),
    {},
  );
  ...
  const controlInputs = {
    ...
    director_plan: await checkDirectorPlan(
      directorPlan,
      visualQuality,
      renderManifest,
    ),
    ...
    pulse_media_house_score: checkPulseMediaHouseScore(
...
module.exports = {
  ...
  verifyDirectorEditorialAuthority,
