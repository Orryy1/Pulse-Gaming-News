...
function canonicalSpokenOutro(value) {
  const narration = normalisePulseCtaText(value);
  const canonical = normalisePulseCtaText(PRIMARY_PULSE_CTA);
  return narration && narration.endsWith(canonical) ? PRIMARY_PULSE_CTA : "";
}
...
function collectStorySignals({
  ...
  const spokenOutro = canonicalSpokenOutro(
    canonical.narration_script || canonical.full_script,
  );
  return {
    story_id: storyId,
    title,
    ...
    spoken_outro: spokenOutro,
    spoken_outro_signature: normalisePulseCtaText(spokenOutro),
...
function batchIndex(stories = []) {
  ...
  for (const story of asArray(stories)) {
    ...
    addToMap(index.spoken_outro_signature, story.signals.spoken_outro_signature, story.story_id);
...
function buildStoryChecks(story = {}, index = {}) {
  ...
  const spokenOutroMatches = duplicatesFor(
    index.spoken_outro_signature,
    signals.spoken_outro_signature,
    story.story_id,
  );
  const canonicalSpokenCtaSignature = normalisePulseCtaText(PRIMARY_PULSE_CTA);
  const canonicalSpokenCtaCohortSaturated = Boolean(
    index.active_story_count > 1 &&
      signals.spoken_outro_signature === canonicalSpokenCtaSignature &&
      unique(index.spoken_outro_signature.get(canonicalSpokenCtaSignature) || []).length ===
        index.active_story_count,
  );
  ...
  const checks = {
    ...
    repeated_ctas: buildCheck({
      status: canonicalSpokenCtaCohortSaturated
        ? "fail"
        : ctaMatches.length || spokenOutroMatches.length
          ? "warn"
          : "pass",
      blocker: "anti_spam:canonical_spoken_cta_cohort_saturation",
      warning: "anti_spam:cta_reused",
      matches: [
        ...ctaMatches.map((id) => ({ story_id: id, cta: signals.cta, scope: "platform_copy" })),
        ...spokenOutroMatches.map((id) => ({
          story_id: id,
          cta: signals.spoken_outro,
          scope: "spoken_outro",
        })),
      ],
      evidence: {
        cta: signals.cta,
        spoken_outro: signals.spoken_outro,
        active_story_count: index.active_story_count,
        canonical_spoken_outro_count: unique(
          index.spoken_outro_signature.get(canonicalSpokenCtaSignature) || [],
        ).length,
        cohort_saturated: canonicalSpokenCtaCohortSaturated,
      },
    }),
