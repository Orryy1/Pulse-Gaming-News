...
function carouselSignature(outputs = {}) {
  const cards = outputs.instagram_reels?.carousel_companion?.cards || outputs.instagram_reels?.carousel_cards || [];
  return signature(asArray(cards).map((card) => (typeof card === "string" ? card : card?.type || card?.label || objectText(card))));
}
...
function collectStorySignals({
  ...
  const xPost = cleanText(outputs.x?.hot_take_post || outputs.x?.source_safe_post || outputs.x?.post || outputs.x?.text);
  const threadsPost = cleanText(outputs.threads?.discussion_post || outputs.threads?.post || outputs.threads?.text);
  ...
  return {
    ...
    layout_signature: normaliseText(directorPlan.layout_template || directorPlan.layout || directorPlan.execution_mode || platformManifest.visual_template),
    ...
    duplicate_x_wording_allowed: outputs.threads?.duplicate_x_wording_allowed === true,
    instagram_carousel_signature: carouselSignature(outputs),
...
function buildRepetitionRiskScore(report = {}) {
  return {
    ...
    generated_at: report.generated_at || null,
    mode: "LOCAL_PROOF",
    batch_signature_counts: buildSignatureCounts(report.stories),
    stories: asArray(report.stories).map((story) => story.repetition_risk),
    safety: {
      local_proof_only: true,
      no_publish_triggered: true,
    },
...
async function buildGoal20AntiSpamUniquenessEngine({
  ...
  const index = batchIndex(loadedStories);
  const stories = loadedStories.map((story) => finaliseStory(story, index, controlIndex));
  ...
  const directPassStories = stories.filter((story) => story.direct_uniqueness_status === "pass");
  const directReviewStories = stories.filter((story) => story.direct_uniqueness_status === "review");
  const directBlockedStories = stories.filter((story) => story.direct_uniqueness_status === "blocked");
    ...
    : blockedStories.length && readyStories.length + reviewStories.length
      ...
      : blockedStories.length
        ...
        : reviewStories.length
          ? "PARTIAL"
          : "PASS";
  const directUniquenessVerdict = !stories.length
    ? "FAIL"
