...
function buildStoryIndex(report = {}) {
  const index = new Map();
  for (const row of asArray(report.stories || report.rows)) {
    const storyId = cleanText(row.story_id || row.id);
    if (storyId) index.set(storyId, row);
  }
  return index;
...
function upstreamBlockers(storyId, antiSpamIndex = new Map()) {
  ...
  if ((passLike(status) || ["review", "partial", "skipped"].includes(status)) && blockers.length === 0) return [];
  return unique(["upstream:goal20_anti_spam_uniqueness_blocked", ...blockers]);
}

function upstreamSkippedInfo(storyId, antiSpamIndex = new Map()) {
  const row = antiSpamIndex.get(cleanText(storyId));
  if (!row) return null;
  const status = normaliseStatus(row.status || row.verdict || row.direct_uniqueness_status || row.final_verdict);
  ...
  return {
    ...
    reason: cleanText(row.skipped_reason || row.reason || "upstream skipped before Goal 21"),
  };
}
...
async function loadStoryPackage(storyPackage = {}, context = {}) {
  ...
  const localPlatformPolicyReport = await readJsonIfPresent(path.join(artifactDir, "platform_policy_report.json"), {});
  const platformPolicyReport = hasObject(localPlatformPolicyReport)
    ? localPlatformPolicyReport
    : context.platformPolicyIndex?.get(storyId) || {};
...
function finaliseStory(story = {}, antiSpamIndex = new Map()) {
  const skipped = upstreamSkippedInfo(story.story_id, antiSpamIndex);
  if (skipped) {
    return {
      ...story,
      status: "skipped",
      upstream_status: "skipped",
      direct_observability_status: "skipped",
      skipped_status: skipped.status,
      skipped_reason: skipped.reason,
      blockers: [],
      upstream_blockers: [],
      direct_observability_blockers: [],
      warnings: [],
      metrics: {
        ...story.metrics,
        rejected_stories: metric(0),
...
async function buildGoal21ObservabilityDashboard({
  ...
  const platformPolicyIndex = buildStoryIndex(upstreamPlatformPolicyReport);
  const loadedStories = [];
  for (const storyPackage of asArray(storyPackages)) {
    loadedStories.push(await loadStoryPackage(storyPackage, { workspaceRoot, platformPolicyIndex }));
  }
  ...
  const directPassStories = activeStories.filter((story) => story.direct_observability_status === "pass");
  const directBlockedStories = activeStories.filter((story) => story.direct_observability_status === "blocked");
  ...
  const verdict = !activeStories.length
    ...
    : blockedStories.length && readyStories.length
      ...
      : blockedStories.length
        ? "BLOCKED"
        : "PASS";
  const directObservabilityVerdict = !activeStories.length
    ? "FAIL"
    : directBlockedStories.length && directPassStories.length
      ...
      : directBlockedStories.length
        ? "BLOCKED"
        : "PASS";
  const recurringFailureReasons = blockerCounts(activeStories);
  const report = {
    schema_version: 1,
    goal: GOAL_ID,
