...
function upstreamSkippedInfo(storyId, observabilityIndex = new Map()) {
  const row = observabilityIndex.get(cleanText(storyId));
  if (!row) return null;
  const status = normaliseStatus(row.status || row.verdict || row.direct_observability_status || row.final_verdict);
  ...
  return {
    ...
    reason: cleanText(row.skipped_reason || row.reason || "upstream skipped before Goal 22"),
  };
}
...
function finaliseStory(story = {}, observabilityIndex = new Map()) {
  const skipped = upstreamSkippedInfo(story.story_id, observabilityIndex);
  if (skipped) {
    return {
      ...story,
      status: "skipped",
      upstream_status: "skipped",
      direct_registry_status: "skipped",
      skipped_status: skipped.status,
      skipped_reason: skipped.reason,
      blockers: [],
      upstream_blockers: [],
      direct_registry_blockers: [],
      warnings: [],
...
function buildProductionAuditLog(report = {}) {
  return {
    ...
    entries: asArray(report.stories).map((story) => ({
      ...
      skipped_status: story.skipped_status || null,
      skipped_reason: story.skipped_reason || null,
...
function buildModelPromptRegistry(report = {}) {
  ...
  return {
    ...
    entries,
    missing_field_counts: directRiskCounts(activeStories),
    safety: {
      no_publish_triggered: true,
      no_secret_values_exposed: true,
    },
...
function buildVideoLineageManifest(report = {}) {
  return {
    ...
    videos: asArray(report.stories).map((story) => ({
      ...
      skipped_by_upstream: story.upstream_status === "skipped",
      skipped_status: story.skipped_status || null,
      skipped_reason: story.skipped_reason || null,
      blockers: story.blockers,
      version_fields: compactFields(story.version_fields),
...
async function buildGoal22VersionedPromptModelRegistry({
  ...
  const directPassStories = activeStories.filter((story) => story.direct_registry_status === "pass");
  const directBlockedStories = activeStories.filter((story) => story.direct_registry_status === "blocked");
  ...
  const verdict = !activeStories.length
    ...
    : blockedStories.length && readyStories.length
      ...
      : blockedStories.length
        ? "BLOCKED"
        : "PASS";
  const directRegistryVerdict = !activeStories.length
    ? "FAIL"
