...
function upstreamSkippedInfo(storyId, registryIndex = new Map()) {
  const row = registryIndex.get(cleanText(storyId));
  if (!row) return null;
  const status = normaliseStatus(row.status || row.verdict || row.final_verdict);
  ...
  return {
    status: "skipped",
    reason: cleanText(row.skipped_reason || row.reason || "upstream skipped before Goal 23"),
  };
}
...
function finaliseStory(storyPackage = {}, registryIndex = new Map(), directSafety = {}) {
  ...
  if (skipped && directBlockers.length === 0) {
    return {
      ...
      direct_safety_status: "pass",
      blockers: [],
      upstream_blockers: [],
      direct_safety_blockers: [],
