...
function upstreamSkippedInfo(storyId, correctionsIndex = new Map()) {
  const row = correctionsIndex.get(cleanText(storyId));
  if (!row) return null;
  const status = normaliseStatus(row.status || row.verdict || row.final_verdict);
  ...
  return {
    status: "skipped",
    reason: cleanText(row.skipped_reason || row.reason || "upstream skipped before Goal 25"),
  };
}
...
function buildSponsorPerformanceSnapshotFromSignals({
  channelStats = {},
  analyticsSnapshots = [],
  retentionReports = [],
  generatedAt = new Date().toISOString(),
} = {}) {
  const rows = asArray(analyticsSnapshots);
  const retentionByStory = new Map(asArray(retentionReports).map((report) => [cleanText(report.story_id || report.id), report]));
  const totalViews = rows.reduce((sum, row) => sum + numberValue(row.total_views, 0), 0);
  const youtubeViews = rows.reduce((sum, row) => sum + numberValue(row.youtube_views, 0), 0);
  const instagramViews = rows.reduce((sum, row) => sum + numberValue(row.instagram_views, 0), 0);
  const tiktokViews = rows.reduce((sum, row) => sum + numberValue(row.tiktok_views, 0), 0);
  const totalComments = rows.reduce(
    (sum, row) =>
      sum +
      numberValue(row.youtube_comments, 0) +
      numberValue(row.instagram_comments, 0) +
      numberValue(row.tiktok_comments, 0),
    0,
  );
  const verticalBreakdown = {};
  for (const row of rows) {
    const vertical = cleanText(row.content_pillar || row.vertical || "gaming_news")
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "_")
      .replace(/^_|_$/g, "") || "gaming_news";
    verticalBreakdown[vertical] = (verticalBreakdown[vertical] || 0) + 1;
  }
  const retentionPercents = [];
  const retentionDurations = [];
  const storyMetrics = {};
  for (const row of rows) {
    const storyId = cleanText(row.story_id || row.id);
    const retention = retentionByStory.get(storyId) || {};
    const percent = percentValue(
      retention.traffic?.weighted_average_percentage_viewed ??
        retention.traffic?.shorts_average_percentage_viewed ??
        retention.average_view_percentage,
    );
    if (percent > 0) retentionPercents.push(percent);
    const duration = numberValue(retention.durationS ?? retention.duration_seconds, 0);
    const avd = numberValue(retention.average_view_duration_seconds, 0) || (duration > 0 && percent > 0 ? duration * (percent / 100) : 0);
    if (avd > 0) retentionDurations.push(avd);
    if (storyId) {
      storyMetrics[storyId] = {
        views: numberValue(row.total_views, 0),
        average_view_duration_seconds: avd || null,
        platform: numberValue(row.youtube_views, 0) >= numberValue(row.instagram_views, 0) ? "youtube_shorts" : "instagram_reels",
      };
    }
  }
  const averageViewPercentage = average(retentionPercents);
  const averageViewDuration = average(retentionDurations);
  const sortedPlatforms = Object.entries({
    youtube_shorts: youtubeViews,
    instagram_reels: instagramViews,
    tiktok: tiktokViews,
  }).sort((a, b) => b[1] - a[1]);
  ...
  return {
    ...
    subscribers: numberValue(channelStats.subscribers ?? channelStats.subscriberCount, 0),
    shorts_views_90d: totalViews,
    average_views: rows.length ? Math.round(totalViews / rows.length) : 0,
    average_view_duration_seconds: Number(averageViewDuration.toFixed(2)),
    average_view_percentage: Number(averageViewPercentage.toFixed(2)),
    comments_per_view: totalViews > 0 ? totalComments / totalViews : 0,
    platform_reach: {
      youtube_shorts: youtubeViews,
      instagram_reels: instagramViews,
      tiktok: tiktokViews,
    ...
    audience_summary: {
      core: "gaming news viewers",
      evidence: "public channel statistics plus local analytics snapshots",
      top_platforms: sortedPlatforms.filter(([, views]) => views > 0).map(([platform]) => platform),
    ...
    provenance: {
      channel_stats: cleanText(channelStats.source || "not_available"),
      analytics_rows: rows.length,
      retention_reports: retentionByStory.size,
      report_only: true,
    },
  };
}

function defaultPulseDataDbPath() {
  const configured = cleanText(process.env.SQLITE_DB_PATH);
  if (configured) return path.isAbsolute(configured) ? configured : path.resolve(process.cwd(), configured);
  return "D:\\pulse-data\\pulse.db";
}

function readAnalyticsSnapshotsFromDb({ dbPath = defaultPulseDataDbPath(), now = new Date() } = {}) {
  if (!dbPath || !fs.existsSync(dbPath)) return { rows: [], warnings: ["analytics_db_missing"] };
  let db;
  try {
    ...
    const rows = db.prepare(`
      SELECT
        ...
        COALESCE(youtube_comments, 0) AS youtube_comments,
        COALESCE(instagram_comments, 0) AS instagram_comments,
        COALESCE(tiktok_comments, 0) AS tiktok_comments,
        COALESCE(total_views, 0) AS total_views,
        published_at,
        updated_at
      FROM analytics_snapshots
      WHERE COALESCE(total_views, 0) > 0
        AND datetime(COALESCE(published_at, updated_at)) >= datetime(?)
      ORDER BY COALESCE(total_views, 0) DESC
    `).all(cutoff90);
    return { rows, warnings: [] };
  } catch (error) {
    return { rows: [], warnings: [`analytics_db_read_failed:${error.message}`] };
  } finally {
    if (db) db.close();
  }
}

async function readRetentionReportsFromDir(retentionDir = "D:\\pulse-data\\learning\\retention-intelligence") {
  if (!retentionDir || !(await fs.pathExists(retentionDir))) return { reports: [], warnings: ["retention_dir_missing"] };
  const reports = [];
  const warnings = [];
  const entries = await fs.readdir(retentionDir);
  for (const entry of entries) {
    if (!entry.toLowerCase().endsWith(".json")) continue;
    const filePath = path.join(retentionDir, entry);
    try {
      const report = await fs.readJson(filePath);
      if (report && typeof report === "object") reports.push(report);
    } catch (error) {
      warnings.push(`retention_report_unreadable:${entry}:${error.message}`);
    }
  }
  return { reports, warnings };
}

async function buildSponsorPerformanceSnapshotFromLocalSources({
  dbPath = defaultPulseDataDbPath(),
  retentionDir = "D:\\pulse-data\\learning\\retention-intelligence",
  channelStats = {},
  generatedAt = new Date().toISOString(),
  now = new Date(),
} = {}) {
  const analytics = readAnalyticsSnapshotsFromDb({ dbPath, now });
  const retention = await readRetentionReportsFromDir(retentionDir);
  const snapshot = buildSponsorPerformanceSnapshotFromSignals({
    channelStats,
    analyticsSnapshots: analytics.rows,
    retentionReports: retention.reports,
    generatedAt,
  });
  snapshot.provenance = {
    ...snapshot.provenance,
    db_path: dbPath ? path.basename(dbPath) : null,
    retention_dir: retentionDir ? path.basename(retentionDir) : null,
    warnings: unique([...analytics.warnings, ...retention.warnings]),
  };
  return snapshot;
}
...
function finaliseSkippedStory(story = {}, skipped = {}) {
  return {
    ...story,
    status: "skipped",
    skipped_reason: skipped.reason || "upstream skipped before Goal 25",
    upstream_status: "skipped",
    direct_sponsor_status: "skipped",
    blockers: [],
    upstream_blockers: [],
    direct_sponsor_blockers: [],
    sponsor_safe: false,
    safety: {
      no_sponsor_outreach_sent: true,
      no_external_posting: true,
...
async function buildGoal25SponsorReadinessPack({
  ...
  const stories = loadedStories.map((story) => {
    const skipped = upstreamSkippedInfo(story.story_id, correctionsIndex);
...
async function writeGoal25SponsorReadinessPack(report = {}, { outputDir } = {}) {
  ...
  const paths = {
    ...
    audienceStats: path.join(outputDir, "sponsor_audience_stats.json"),
    sponsorSafeExamples: path.join(outputDir, "sponsor_safe_examples.json"),
    pricingBands: path.join(outputDir, "sponsor_pricing_bands.json"),
    disclosurePlan: path.join(outputDir, "sponsor_disclosure_plan.json"),
  };
  const kit = report.sponsor_media_kit || {};
  ...
  await fs.writeJson(paths.sponsorMediaKit, kit, { spaces: 2 });
  ...
  await fs.writeJson(paths.audienceStats, {
    schema_version: 1,
    subscribers: kit.public_metrics?.subscribers ?? null,
    shorts_views_90d: kit.public_metrics?.shorts_views_90d ?? null,
    comments_per_view: kit.public_metrics?.comments_per_view ?? null,
    average_views: kit.average_views ?? null,
    retention_stats: kit.retention_stats || {},
    platform_reach: kit.platform_reach || {},
    vertical_breakdown: kit.vertical_breakdown || {},
    audience_summary: kit.audience_summary || null,
    best_performing_videos: kit.best_performing_videos || [],
    safety: {
      local_proof_only: true,
      no_public_claims_changed: true,
      no_sponsor_outreach_sent: true,
  ...
  await fs.writeJson(paths.sponsorSafeExamples, {
    schema_version: 1,
    sponsor_safe_examples: kit.sponsor_safe_examples || [],
    safety: {
      operator_review_required: true,
      no_sponsor_outreach_sent: true,
    },
  }, { spaces: 2 });
  await fs.writeJson(paths.pricingBands, kit.pricing_recommendations || {}, { spaces: 2 });
  await fs.writeJson(paths.disclosurePlan, kit.disclosure_plan || {}, { spaces: 2 });
  return paths;
}
