...
const GOAL_ID = "27_goal_definition_missing";
const GOAL_NUMBER = 27;
...
function directBlockersFor(goalDefinition = {}) {
  const blockers = [];
  if (!goalDefinition.found) blockers.push("campaign:goal27_definition_missing");
  if (!asArray(goalDefinition.outputs).length) blockers.push("campaign:goal27_outputs_missing");
  if (!goalDefinition.acceptance_criteria_found) blockers.push("campaign:goal27_acceptance_criteria_missing");
  return blockers;
}

function upstreamBlockers(upstreamGoal26Report = {}) {
  if (!upstreamGoal26Report || typeof upstreamGoal26Report !== "object") {
    return ["upstream:goal26_creator_studio_brand_system_missing"];
  }
  const verdict = upstreamGoal26Report.verdict || upstreamGoal26Report.status;
  if (passLike(verdict)) return [];
  return ["upstream:goal26_creator_studio_brand_system_blocked"];
}
  ...
  return {
    ...
    required_operator_action:
      asArray(report.direct_blockers).length > 0
        ? "Add the Goal 27 contract to docs/codex-main-goal.md, including heading, scope, outputs and acceptance criteria."
        : "No definition gap detected for Goal 27.",
    safety: {
...
async function buildGoal27DefinitionMissing({
  ...
  const upstream = upstreamBlockers(upstreamGoal26Report);
  ...
  const report = {
    ...
    operator_request: {
      ...
      requested_update: directBlockers.length > 0
        ? "Define Goal 27 in docs/codex-main-goal.md before implementation continues, including the goal heading, scope, outputs and acceptance criteria."
        : "No Goal 27 definition update is required.",
      next_safe_action:
        directBlockers.length > 0
          ? "After the contract is added, rerun this gate before implementing any Goal 27 production behaviour."
          : "Proceed to the Goal 27 implementation gate in LOCAL_PROOF or DRY_RUN_PUBLISH.",
    },
    safety: {
      local_proof_only: true,
...
function renderGoal27DefinitionMissingMarkdown(report = {}) {
  const lines = [];
  lines.push("# Goal 27 - Definition Missing Gate");
  lines.push("");
...
function renderOperatorRequest(report = {}) {
  const lines = [];
  lines.push("# Goal 27 Operator Request");
  lines.push("");
  lines.push("Goal 27 cannot be implemented from the current campaign docs.");
  lines.push("");
  lines.push("## Required Update");
  lines.push("- Add `### 27. <Goal Title>` to `docs/codex-main-goal.md`.");
...
async function writeGoal27DefinitionMissing(report = {}, { outputDir } = {}) {
  if (!outputDir) throw new Error("writeGoal27DefinitionMissing requires outputDir");
  await fs.ensureDir(outputDir);
  const paths = {
    readinessJson: path.join(outputDir, "goal27_readiness_report.json"),
    readinessMarkdown: path.join(outputDir, "goal27_readiness_report.md"),
    contractGapReport: path.join(outputDir, "goal27_contract_gap_report.json"),
    operatorRequest: path.join(outputDir, "goal27_operator_request.md"),
  };
  await fs.writeJson(paths.readinessJson, report, { spaces: 2 });
  await fs.outputFile(paths.readinessMarkdown, renderGoal27DefinitionMissingMarkdown(report));
