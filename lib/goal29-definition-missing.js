...
const GOAL_ID = "29_goal_definition_missing";
const GOAL_NUMBER = 29;
...
function directBlockersFor(goalDefinition = {}) {
  const blockers = [];
  if (!goalDefinition.found) blockers.push("campaign:goal29_definition_missing");
  if (!asArray(goalDefinition.outputs).length) blockers.push("campaign:goal29_outputs_missing");
  if (!goalDefinition.acceptance_criteria_found) blockers.push("campaign:goal29_acceptance_criteria_missing");
  return blockers;
}

function upstreamBlockers(upstreamGoal28Report = {}) {
  if (!upstreamGoal28Report || typeof upstreamGoal28Report !== "object") {
    return ["upstream:goal28_goal_definition_missing_missing"];
  }
  const verdict = upstreamGoal28Report.verdict || upstreamGoal28Report.status;
  if (passLike(verdict)) return [];
  return ["upstream:goal28_goal_definition_missing_blocked"];
}
  ...
  return {
    ...
    required_operator_action:
      asArray(report.direct_blockers).length > 0
        ? "Add the Goal 29 contract to docs/codex-main-goal.md, including heading, scope, outputs and acceptance criteria."
        : "No definition gap detected for Goal 29.",
    safety: {
...
async function buildGoal29DefinitionMissing({
  campaignDocPath = path.join(process.cwd(), "docs", "codex-main-goal.md"),
  storyPackages = [],
  upstreamGoal28Report = {},
  ...
  const upstream = upstreamBlockers(upstreamGoal28Report);
  ...
  const report = {
    ...
    operator_request: {
      ...
      requested_update: directBlockers.length > 0
        ? "Define Goal 29 in docs/codex-main-goal.md before implementation continues, including the goal heading, scope, outputs and acceptance criteria."
        : "No Goal 29 definition update is required.",
      next_safe_action:
        directBlockers.length > 0
          ? "After the contract is added, rerun this gate before implementing any Goal 29 production behaviour."
          : "Proceed to the Goal 29 implementation gate in LOCAL_PROOF or DRY_RUN_PUBLISH.",
    },
    safety: {
      local_proof_only: true,
...
function renderGoal29DefinitionMissingMarkdown(report = {}) {
  const lines = [];
  lines.push("# Goal 29 - Definition Missing Gate");
  lines.push("");
...
function renderOperatorRequest(report = {}) {
  const lines = [];
  lines.push("# Goal 29 Operator Request");
  lines.push("");
  lines.push("Goal 29 cannot be implemented from the current campaign docs.");
  lines.push("");
  lines.push("## Required Update");
  lines.push("- Add `### 29. <Goal Title>` to `docs/codex-main-goal.md`.");
...
async function writeGoal29DefinitionMissing(report = {}, { outputDir } = {}) {
  if (!outputDir) throw new Error("writeGoal29DefinitionMissing requires outputDir");
  await fs.ensureDir(outputDir);
  const paths = {
    readinessJson: path.join(outputDir, "goal29_readiness_report.json"),
    readinessMarkdown: path.join(outputDir, "goal29_readiness_report.md"),
    contractGapReport: path.join(outputDir, "goal29_contract_gap_report.json"),
    operatorRequest: path.join(outputDir, "goal29_operator_request.md"),
  };
  await fs.writeJson(paths.readinessJson, report, { spaces: 2 });
  await fs.outputFile(paths.readinessMarkdown, renderGoal29DefinitionMissingMarkdown(report));
...
module.exports = {
  ...
  extractGoalSection,
  renderGoal29DefinitionMissingMarkdown,
