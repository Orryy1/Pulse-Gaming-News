...
const GOAL_ID = "30_goal_definition_missing";
...
function directBlockersFor(goalDefinition = {}) {
  const blockers = [];
  if (!goalDefinition.found) blockers.push("campaign:goal30_definition_missing");
  if (!asArray(goalDefinition.outputs).length) blockers.push("campaign:goal30_outputs_missing");
  if (!goalDefinition.acceptance_criteria_found) blockers.push("campaign:goal30_acceptance_criteria_missing");
  return blockers;
}

function upstreamBlockers(upstreamGoal29Report = {}) {
  if (!upstreamGoal29Report || typeof upstreamGoal29Report !== "object") {
    return ["upstream:goal29_goal_definition_missing_missing"];
  }
  const verdict = upstreamGoal29Report.verdict || upstreamGoal29Report.status;
  if (passLike(verdict)) return [];
  return ["upstream:goal29_goal_definition_missing_blocked"];
}
  ...
  return {
    ...
    required_operator_action:
      asArray(report.direct_blockers).length > 0
        ? "Add the Goal 30 contract to docs/codex-main-goal.md, including heading, scope, outputs and acceptance criteria."
        : "No definition gap detected for Goal 30.",
    safety: {
...
async function buildGoal30DefinitionMissing({
  ...
  const upstream = upstreamBlockers(upstreamGoal29Report);
  ...
  const report = {
    ...
    operator_request: {
      ...
      requested_update: directBlockers.length > 0
        ? "Define Goal 30 in docs/codex-main-goal.md before implementation continues, including the goal heading, scope, outputs and acceptance criteria."
        : "No Goal 30 definition update is required.",
      next_safe_action:
        directBlockers.length > 0
          ? "After the contract is added, rerun this gate before implementing any Goal 30 production behaviour."
          : "Proceed to the Goal 30 implementation gate in LOCAL_PROOF or DRY_RUN_PUBLISH.",
    },
    safety: {
      local_proof_only: true,
...
function renderGoal30DefinitionMissingMarkdown(report = {}) {
  const lines = [];
  lines.push("# Goal 30 - Definition Missing Gate");
  lines.push("");
...
function renderOperatorRequest(report = {}) {
  const lines = [];
  lines.push("# Goal 30 Operator Request");
  lines.push("");
  lines.push("Goal 30 cannot be implemented from the current campaign docs.");
  lines.push("");
  lines.push("## Required Update");
  lines.push("- Add `### 30. <Goal Title>` to `docs/codex-main-goal.md`.");
...
async function writeGoal30DefinitionMissing(report = {}, { outputDir } = {}) {
  if (!outputDir) throw new Error("writeGoal30DefinitionMissing requires outputDir");
  await fs.ensureDir(outputDir);
  const paths = {
    readinessJson: path.join(outputDir, "goal30_readiness_report.json"),
    readinessMarkdown: path.join(outputDir, "goal30_readiness_report.md"),
    contractGapReport: path.join(outputDir, "goal30_contract_gap_report.json"),
    operatorRequest: path.join(outputDir, "goal30_operator_request.md"),
  };
  await fs.writeJson(paths.readinessJson, report, { spaces: 2 });
  await fs.outputFile(paths.readinessMarkdown, renderGoal30DefinitionMissingMarkdown(report));
