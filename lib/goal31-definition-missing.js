...
const GOAL_ID = "31_goal_definition_missing";
const GOAL_NUMBER = 31;
...
function directBlockersFor(goalDefinition = {}) {
  const blockers = [];
  if (!goalDefinition.found) blockers.push("campaign:goal31_definition_missing");
  if (!asArray(goalDefinition.outputs).length) blockers.push("campaign:goal31_outputs_missing");
  if (!goalDefinition.acceptance_criteria_found) blockers.push("campaign:goal31_acceptance_criteria_missing");
  return blockers;
}

function upstreamBlockers(upstreamGoal30Report = {}) {
  if (!upstreamGoal30Report || typeof upstreamGoal30Report !== "object") {
    return ["upstream:goal30_goal_definition_missing_missing"];
  }
  const verdict = upstreamGoal30Report.verdict || upstreamGoal30Report.status;
  if (passLike(verdict)) return [];
  return ["upstream:goal30_goal_definition_missing_blocked"];
}
  ...
  return {
    ...
    required_operator_action:
      asArray(report.direct_blockers).length > 0
        ? "Add the Goal 31 contract to docs/codex-main-goal.md, including heading, scope, outputs and acceptance criteria."
        : "No definition gap detected for Goal 31.",
    safety: {
...
async function buildGoal31DefinitionMissing({
  campaignDocPath = path.join(process.cwd(), "docs", "codex-main-goal.md"),
  storyPackages = [],
  upstreamGoal30Report = {},
  ...
  const upstream = upstreamBlockers(upstreamGoal30Report);
  ...
  const report = {
    ...
    operator_request: {
      ...
      requested_update: directBlockers.length > 0
        ? "Define Goal 31 in docs/codex-main-goal.md before implementation continues, including the goal heading, scope, outputs and acceptance criteria."
        : "No Goal 31 definition update is required.",
      next_safe_action:
        directBlockers.length > 0
          ? "After the contract is added, rerun this gate before implementing any Goal 31 production behaviour."
          : "Proceed to the Goal 31 implementation gate in LOCAL_PROOF or DRY_RUN_PUBLISH.",
    },
    safety: {
      local_proof_only: true,
...
function renderGoal31DefinitionMissingMarkdown(report = {}) {
  const lines = [];
  lines.push("# Goal 31 - Definition Missing Gate");
  lines.push("");
...
function renderOperatorRequest(report = {}) {
  const lines = [];
  lines.push("# Goal 31 Operator Request");
  lines.push("");
  lines.push("Goal 31 cannot be implemented from the current campaign docs.");
  lines.push("");
  lines.push("## Required Update");
  lines.push("- Add `### 31. <Goal Title>` to `docs/codex-main-goal.md`.");
...
async function writeGoal31DefinitionMissing(report = {}, { outputDir } = {}) {
  if (!outputDir) throw new Error("writeGoal31DefinitionMissing requires outputDir");
  await fs.ensureDir(outputDir);
  const paths = {
    readinessJson: path.join(outputDir, "goal31_readiness_report.json"),
    readinessMarkdown: path.join(outputDir, "goal31_readiness_report.md"),
    contractGapReport: path.join(outputDir, "goal31_contract_gap_report.json"),
    operatorRequest: path.join(outputDir, "goal31_operator_request.md"),
  };
  await fs.writeJson(paths.readinessJson, report, { spaces: 2 });
  await fs.outputFile(paths.readinessMarkdown, renderGoal31DefinitionMissingMarkdown(report));
...
module.exports = {
  ...
  extractGoalSection,
  renderGoal31DefinitionMissingMarkdown,
