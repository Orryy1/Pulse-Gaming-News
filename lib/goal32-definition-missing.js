...
const GOAL_ID = "32_goal_definition_missing";
const GOAL_NUMBER = 32;
...
function directBlockersFor(goalDefinition = {}) {
  const blockers = [];
  if (!goalDefinition.found) blockers.push("campaign:goal32_definition_missing");
  if (!asArray(goalDefinition.outputs).length) blockers.push("campaign:goal32_outputs_missing");
  if (!goalDefinition.acceptance_criteria_found) blockers.push("campaign:goal32_acceptance_criteria_missing");
  return blockers;
}

function upstreamBlockers(upstreamGoal31Report = {}) {
  if (!upstreamGoal31Report || typeof upstreamGoal31Report !== "object") {
    return ["upstream:goal31_goal_definition_missing_missing"];
  }
  const verdict = upstreamGoal31Report.verdict || upstreamGoal31Report.status;
  if (passLike(verdict)) return [];
  return ["upstream:goal31_goal_definition_missing_blocked"];
}
  ...
  return {
    ...
    required_operator_action:
      asArray(report.direct_blockers).length > 0
        ? "Add the Goal 32 contract to docs/codex-main-goal.md, including heading, scope, outputs and acceptance criteria."
        : "No definition gap detected for Goal 32.",
    safety: {
...
async function buildGoal32DefinitionMissing({
  ...
  const upstream = upstreamBlockers(upstreamGoal31Report);
  ...
  const report = {
    ...
    operator_request: {
      ...
      requested_update: directBlockers.length > 0
        ? "Define Goal 32 in docs/codex-main-goal.md before implementation continues, including the goal heading, scope, outputs and acceptance criteria."
        : "No Goal 32 definition update is required.",
      next_safe_action:
        directBlockers.length > 0
          ? "After the contract is added, rerun this gate before implementing any Goal 32 production behaviour."
          : "Proceed to the Goal 32 implementation gate in LOCAL_PROOF or DRY_RUN_PUBLISH.",
    },
    safety: {
      local_proof_only: true,
...
function renderGoal32DefinitionMissingMarkdown(report = {}) {
  const lines = [];
  lines.push("# Goal 32 - Definition Missing Gate");
  lines.push("");
...
function renderOperatorRequest(report = {}) {
  const lines = [];
  lines.push("# Goal 32 Operator Request");
  lines.push("");
  lines.push("Goal 32 cannot be implemented from the current campaign docs.");
  lines.push("");
  lines.push("## Required Update");
  lines.push("- Add `### 32. <Goal Title>` to `docs/codex-main-goal.md`.");
...
async function writeGoal32DefinitionMissing(report = {}, { outputDir } = {}) {
  if (!outputDir) throw new Error("writeGoal32DefinitionMissing requires outputDir");
  await fs.ensureDir(outputDir);
  const paths = {
    readinessJson: path.join(outputDir, "goal32_readiness_report.json"),
    readinessMarkdown: path.join(outputDir, "goal32_readiness_report.md"),
    contractGapReport: path.join(outputDir, "goal32_contract_gap_report.json"),
    operatorRequest: path.join(outputDir, "goal32_operator_request.md"),
  };
  await fs.writeJson(paths.readinessJson, report, { spaces: 2 });
  await fs.outputFile(paths.readinessMarkdown, renderGoal32DefinitionMissingMarkdown(report));
