...
const GOAL_ID = "33_goal_definition_missing";
const GOAL_NUMBER = 33;
...
function directBlockersFor(goalDefinition = {}) {
  const blockers = [];
  if (!goalDefinition.found) blockers.push("campaign:goal33_definition_missing");
  if (!asArray(goalDefinition.outputs).length) blockers.push("campaign:goal33_outputs_missing");
  if (!goalDefinition.acceptance_criteria_found) blockers.push("campaign:goal33_acceptance_criteria_missing");
  return blockers;
}

function upstreamBlockers(upstreamGoal32Report = {}) {
  if (!upstreamGoal32Report || typeof upstreamGoal32Report !== "object") {
    return ["upstream:goal32_goal_definition_missing_missing"];
  }
  const verdict = upstreamGoal32Report.verdict || upstreamGoal32Report.status;
  if (passLike(verdict)) return [];
  return ["upstream:goal32_goal_definition_missing_blocked"];
}
  ...
  return {
    ...
    required_operator_action:
      asArray(report.direct_blockers).length > 0
        ? "Add the Goal 33 contract to docs/codex-main-goal.md, including heading, scope, outputs and acceptance criteria."
        : "No definition gap detected for Goal 33.",
    safety: {
...
async function buildGoal33DefinitionMissing({
  campaignDocPath = path.join(process.cwd(), "docs", "codex-main-goal.md"),
  storyPackages = [],
  upstreamGoal32Report = {},
  ...
  const upstream = upstreamBlockers(upstreamGoal32Report);
  ...
  const report = {
    ...
    operator_request: {
      ...
      requested_update: directBlockers.length > 0
        ? "Define Goal 33 in docs/codex-main-goal.md before implementation continues, including the goal heading, scope, outputs and acceptance criteria."
        : "No Goal 33 definition update is required.",
      next_safe_action:
        directBlockers.length > 0
          ? "After the contract is added, rerun this gate before implementing any Goal 33 production behaviour."
          : "Proceed to the Goal 33 implementation gate in LOCAL_PROOF or DRY_RUN_PUBLISH.",
    },
    safety: {
      local_proof_only: true,
...
function renderGoal33DefinitionMissingMarkdown(report = {}) {
  const lines = [];
  lines.push("# Goal 33 - Definition Missing Gate");
  lines.push("");
...
function renderOperatorRequest(report = {}) {
  const lines = [];
  lines.push("# Goal 33 Operator Request");
  lines.push("");
  lines.push("Goal 33 cannot be implemented from the current campaign docs.");
  lines.push("");
  lines.push("## Required Update");
  lines.push("- Add `### 33. <Goal Title>` to `docs/codex-main-goal.md`.");
...
async function writeGoal33DefinitionMissing(report = {}, { outputDir } = {}) {
  if (!outputDir) throw new Error("writeGoal33DefinitionMissing requires outputDir");
  await fs.ensureDir(outputDir);
  const paths = {
    readinessJson: path.join(outputDir, "goal33_readiness_report.json"),
    readinessMarkdown: path.join(outputDir, "goal33_readiness_report.md"),
    contractGapReport: path.join(outputDir, "goal33_contract_gap_report.json"),
    operatorRequest: path.join(outputDir, "goal33_operator_request.md"),
  };
  await fs.writeJson(paths.readinessJson, report, { spaces: 2 });
  await fs.outputFile(paths.readinessMarkdown, renderGoal33DefinitionMissingMarkdown(report));
...
module.exports = {
  ...
  extractGoalSection,
  renderGoal33DefinitionMissingMarkdown,
