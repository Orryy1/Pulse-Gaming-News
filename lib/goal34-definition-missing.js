...
const GOAL_ID = "34_goal_definition_missing";
const GOAL_NUMBER = 34;
...
function directBlockersFor(goalDefinition = {}) {
  const blockers = [];
  if (!goalDefinition.found) blockers.push("campaign:goal34_definition_missing");
  if (!asArray(goalDefinition.outputs).length) blockers.push("campaign:goal34_outputs_missing");
  if (!goalDefinition.acceptance_criteria_found) blockers.push("campaign:goal34_acceptance_criteria_missing");
  return blockers;
}

function upstreamBlockers(upstreamGoal33Report = {}) {
  if (!upstreamGoal33Report || typeof upstreamGoal33Report !== "object") {
    return ["upstream:goal33_goal_definition_missing_missing"];
  }
  const verdict = upstreamGoal33Report.verdict || upstreamGoal33Report.status;
  if (passLike(verdict)) return [];
  return ["upstream:goal33_goal_definition_missing_blocked"];
}
  ...
  return {
    ...
    required_operator_action:
      asArray(report.direct_blockers).length > 0
        ? "Add the Goal 34 contract to docs/codex-main-goal.md, including heading, scope, outputs and acceptance criteria."
        : "No definition gap detected for Goal 34.",
    safety: {
...
async function buildGoal34DefinitionMissing({
  ...
  const upstream = upstreamBlockers(upstreamGoal33Report);
  ...
  const report = {
    ...
    operator_request: {
      ...
      requested_update: directBlockers.length > 0
        ? "Define Goal 34 in docs/codex-main-goal.md before implementation continues, including the goal heading, scope, outputs and acceptance criteria."
        : "No Goal 34 definition update is required.",
      next_safe_action:
        directBlockers.length > 0
          ? "After the contract is added, rerun this gate before implementing any Goal 34 production behaviour."
          : "Proceed to the Goal 34 implementation gate in LOCAL_PROOF or DRY_RUN_PUBLISH.",
    },
    safety: {
      local_proof_only: true,
...
function renderGoal34DefinitionMissingMarkdown(report = {}) {
  const lines = [];
  lines.push("# Goal 34 - Definition Missing Gate");
  lines.push("");
...
function renderOperatorRequest(report = {}) {
  const lines = [];
  lines.push("# Goal 34 Operator Request");
  lines.push("");
  lines.push("Goal 34 cannot be implemented from the current campaign docs.");
  lines.push("");
  lines.push("## Required Update");
  lines.push("- Add `### 34. <Goal Title>` to `docs/codex-main-goal.md`.");
...
async function writeGoal34DefinitionMissing(report = {}, { outputDir } = {}) {
  if (!outputDir) throw new Error("writeGoal34DefinitionMissing requires outputDir");
  await fs.ensureDir(outputDir);
  const paths = {
    readinessJson: path.join(outputDir, "goal34_readiness_report.json"),
    readinessMarkdown: path.join(outputDir, "goal34_readiness_report.md"),
    contractGapReport: path.join(outputDir, "goal34_contract_gap_report.json"),
    operatorRequest: path.join(outputDir, "goal34_operator_request.md"),
  };
  await fs.writeJson(paths.readinessJson, report, { spaces: 2 });
  await fs.outputFile(paths.readinessMarkdown, renderGoal34DefinitionMissingMarkdown(report));
