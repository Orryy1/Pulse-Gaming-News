...
const GOAL_ID = "35_goal_definition_missing";
const GOAL_NUMBER = 35;
...
function directBlockersFor(goalDefinition = {}) {
  const blockers = [];
  if (!goalDefinition.found) blockers.push("campaign:goal35_definition_missing");
  if (!asArray(goalDefinition.outputs).length) blockers.push("campaign:goal35_outputs_missing");
  if (!goalDefinition.acceptance_criteria_found) blockers.push("campaign:goal35_acceptance_criteria_missing");
  return blockers;
}

function upstreamBlockers(upstreamGoal34Report = {}) {
  if (!upstreamGoal34Report || typeof upstreamGoal34Report !== "object") {
    return ["upstream:goal34_goal_definition_missing_missing"];
  }
  const verdict = upstreamGoal34Report.verdict || upstreamGoal34Report.status;
  if (passLike(verdict)) return [];
  return ["upstream:goal34_goal_definition_missing_blocked"];
}
  ...
  return {
    ...
    required_operator_action:
      asArray(report.direct_blockers).length > 0
        ? "Add the Goal 35 contract to docs/codex-main-goal.md, including heading, scope, outputs and acceptance criteria."
        : "No definition gap detected for Goal 35.",
    safety: {
...
async function buildGoal35DefinitionMissing({
  campaignDocPath = path.join(process.cwd(), "docs", "codex-main-goal.md"),
  storyPackages = [],
  upstreamGoal34Report = {},
  ...
  const upstream = upstreamBlockers(upstreamGoal34Report);
  ...
  const report = {
    ...
    operator_request: {
      ...
      requested_update: directBlockers.length > 0
        ? "Define Goal 35 in docs/codex-main-goal.md before implementation continues, including the goal heading, scope, outputs and acceptance criteria."
        : "No Goal 35 definition update is required.",
      next_safe_action:
        directBlockers.length > 0
          ? "After the contract is added, rerun this gate before implementing any Goal 35 production behaviour."
          : "Proceed to the Goal 35 implementation gate in LOCAL_PROOF or DRY_RUN_PUBLISH.",
    },
    safety: {
      local_proof_only: true,
...
function renderGoal35DefinitionMissingMarkdown(report = {}) {
  const lines = [];
  lines.push("# Goal 35 - Definition Missing Gate");
  lines.push("");
...
function renderOperatorRequest(report = {}) {
  const lines = [];
  lines.push("# Goal 35 Operator Request");
  lines.push("");
  lines.push("Goal 35 cannot be implemented from the current campaign docs.");
  lines.push("");
  lines.push("## Required Update");
  lines.push("- Add `### 35. <Goal Title>` to `docs/codex-main-goal.md`.");
...
async function writeGoal35DefinitionMissing(report = {}, { outputDir } = {}) {
  if (!outputDir) throw new Error("writeGoal35DefinitionMissing requires outputDir");
  await fs.ensureDir(outputDir);
  const paths = {
    readinessJson: path.join(outputDir, "goal35_readiness_report.json"),
    readinessMarkdown: path.join(outputDir, "goal35_readiness_report.md"),
    contractGapReport: path.join(outputDir, "goal35_contract_gap_report.json"),
    operatorRequest: path.join(outputDir, "goal35_operator_request.md"),
  };
  await fs.writeJson(paths.readinessJson, report, { spaces: 2 });
  await fs.outputFile(paths.readinessMarkdown, renderGoal35DefinitionMissingMarkdown(report));
...
module.exports = {
  ...
  extractGoalSection,
  renderGoal35DefinitionMissingMarkdown,
