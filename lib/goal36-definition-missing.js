...
const GOAL_ID = "36_goal_definition_missing";
const GOAL_NUMBER = 36;
...
function directBlockersFor(goalDefinition = {}) {
  const blockers = [];
  if (!goalDefinition.found) blockers.push("campaign:goal36_definition_missing");
  if (!asArray(goalDefinition.outputs).length) blockers.push("campaign:goal36_outputs_missing");
  if (!goalDefinition.acceptance_criteria_found) blockers.push("campaign:goal36_acceptance_criteria_missing");
  return blockers;
}

function upstreamBlockers(upstreamGoal35Report = {}) {
  if (!upstreamGoal35Report || typeof upstreamGoal35Report !== "object") {
    return ["upstream:goal35_goal_definition_missing_missing"];
  }
  const verdict = upstreamGoal35Report.verdict || upstreamGoal35Report.status;
  if (passLike(verdict)) return [];
  return ["upstream:goal35_goal_definition_missing_blocked"];
}
  ...
  return {
    ...
    required_operator_action:
      asArray(report.direct_blockers).length > 0
        ? "Add the Goal 36 contract to docs/codex-main-goal.md, including heading, scope, outputs and acceptance criteria."
        : "No definition gap detected for Goal 36.",
    safety: {
...
async function buildGoal36DefinitionMissing({
  ...
  const upstream = upstreamBlockers(upstreamGoal35Report);
  ...
  const report = {
    ...
    operator_request: {
      ...
      requested_update: directBlockers.length > 0
        ? "Define Goal 36 in docs/codex-main-goal.md before implementation continues, including the goal heading, scope, outputs and acceptance criteria."
        : "No Goal 36 definition update is required.",
      next_safe_action:
        directBlockers.length > 0
          ? "After the contract is added, rerun this gate before implementing any Goal 36 production behaviour."
          : "Proceed to the Goal 36 implementation gate in LOCAL_PROOF or DRY_RUN_PUBLISH.",
    },
    safety: {
      local_proof_only: true,
...
function renderGoal36DefinitionMissingMarkdown(report = {}) {
  const lines = [];
  lines.push("# Goal 36 - Definition Missing Gate");
  lines.push("");
...
function renderOperatorRequest(report = {}) {
  const lines = [];
  lines.push("# Goal 36 Operator Request");
  lines.push("");
  lines.push("Goal 36 cannot be implemented from the current campaign docs.");
  lines.push("");
  lines.push("## Required Update");
  lines.push("- Add `### 36. <Goal Title>` to `docs/codex-main-goal.md`.");
...
async function writeGoal36DefinitionMissing(report = {}, { outputDir } = {}) {
  if (!outputDir) throw new Error("writeGoal36DefinitionMissing requires outputDir");
  await fs.ensureDir(outputDir);
  const paths = {
    readinessJson: path.join(outputDir, "goal36_readiness_report.json"),
    readinessMarkdown: path.join(outputDir, "goal36_readiness_report.md"),
    contractGapReport: path.join(outputDir, "goal36_contract_gap_report.json"),
    operatorRequest: path.join(outputDir, "goal36_operator_request.md"),
  };
  await fs.writeJson(paths.readinessJson, report, { spaces: 2 });
  await fs.outputFile(paths.readinessMarkdown, renderGoal36DefinitionMissingMarkdown(report));
