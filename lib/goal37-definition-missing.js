...
const GOAL_ID = "37_goal_definition_missing";
const GOAL_NUMBER = 37;
...
function directBlockersFor(goalDefinition = {}) {
  const blockers = [];
  if (!goalDefinition.found) blockers.push("campaign:goal37_definition_missing");
  if (!asArray(goalDefinition.outputs).length) blockers.push("campaign:goal37_outputs_missing");
  if (!goalDefinition.acceptance_criteria_found) blockers.push("campaign:goal37_acceptance_criteria_missing");
  return blockers;
}

function upstreamBlockers(upstreamGoal36Report = {}) {
  if (!upstreamGoal36Report || typeof upstreamGoal36Report !== "object") {
    return ["upstream:goal36_goal_definition_missing_missing"];
  }
  const verdict = upstreamGoal36Report.verdict || upstreamGoal36Report.status;
  if (passLike(verdict)) return [];
  return ["upstream:goal36_goal_definition_missing_blocked"];
}
  ...
  return {
    ...
    required_operator_action:
      asArray(report.direct_blockers).length > 0
        ? "Add the Goal 37 contract to docs/codex-main-goal.md, including heading, scope, outputs and acceptance criteria."
        : "No definition gap detected for Goal 37.",
    safety: {
...
async function buildGoal37DefinitionMissing({
  ...
  const upstream = upstreamBlockers(upstreamGoal36Report);
  ...
  const report = {
    ...
    operator_request: {
      ...
      requested_update: directBlockers.length > 0
        ? "Define Goal 37 in docs/codex-main-goal.md before implementation continues, including the goal heading, scope, outputs and acceptance criteria."
        : "No Goal 37 definition update is required.",
      next_safe_action:
        directBlockers.length > 0
          ? "After the contract is added, rerun this gate before implementing any Goal 37 production behaviour."
          : "Proceed to the Goal 37 implementation gate in LOCAL_PROOF or DRY_RUN_PUBLISH.",
    },
    safety: {
      local_proof_only: true,
...
function renderGoal37DefinitionMissingMarkdown(report = {}) {
  const lines = [];
  lines.push("# Goal 37 - Definition Missing Gate");
  lines.push("");
...
function renderOperatorRequest(report = {}) {
  const lines = [];
  lines.push("# Goal 37 Operator Request");
  lines.push("");
  lines.push("Goal 37 cannot be implemented from the current campaign docs.");
  lines.push("");
  lines.push("## Required Update");
  lines.push("- Add `### 37. <Goal Title>` to `docs/codex-main-goal.md`.");
...
async function writeGoal37DefinitionMissing(report = {}, { outputDir } = {}) {
  if (!outputDir) throw new Error("writeGoal37DefinitionMissing requires outputDir");
  await fs.ensureDir(outputDir);
  const paths = {
    readinessJson: path.join(outputDir, "goal37_readiness_report.json"),
    readinessMarkdown: path.join(outputDir, "goal37_readiness_report.md"),
    contractGapReport: path.join(outputDir, "goal37_contract_gap_report.json"),
    operatorRequest: path.join(outputDir, "goal37_operator_request.md"),
  };
  await fs.writeJson(paths.readinessJson, report, { spaces: 2 });
  await fs.outputFile(paths.readinessMarkdown, renderGoal37DefinitionMissingMarkdown(report));
