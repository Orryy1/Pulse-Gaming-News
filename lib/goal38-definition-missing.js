...
const GOAL_ID = "38_goal_definition_missing";
const GOAL_NUMBER = 38;
...
function directBlockersFor(goalDefinition = {}) {
  const blockers = [];
  if (!goalDefinition.found) blockers.push("campaign:goal38_definition_missing");
  if (!asArray(goalDefinition.outputs).length) blockers.push("campaign:goal38_outputs_missing");
  if (!goalDefinition.acceptance_criteria_found) blockers.push("campaign:goal38_acceptance_criteria_missing");
  return blockers;
}

function upstreamBlockers(upstreamGoal37Report = {}) {
  if (!upstreamGoal37Report || typeof upstreamGoal37Report !== "object") {
    return ["upstream:goal37_goal_definition_missing_missing"];
  }
  const verdict = upstreamGoal37Report.verdict || upstreamGoal37Report.status;
  if (passLike(verdict)) return [];
  return ["upstream:goal37_goal_definition_missing_blocked"];
}
  ...
  return {
    ...
    required_operator_action:
      asArray(report.direct_blockers).length > 0
        ? "Add the Goal 38 contract to docs/codex-main-goal.md, including heading, scope, outputs and acceptance criteria."
        : "No definition gap detected for Goal 38.",
    safety: {
...
async function buildGoal38DefinitionMissing({
  campaignDocPath = path.join(process.cwd(), "docs", "codex-main-goal.md"),
  storyPackages = [],
  upstreamGoal37Report = {},
  ...
  const upstream = upstreamBlockers(upstreamGoal37Report);
  ...
  const report = {
    ...
    operator_request: {
      ...
      requested_update: directBlockers.length > 0
        ? "Define Goal 38 in docs/codex-main-goal.md before implementation continues, including the goal heading, scope, outputs and acceptance criteria."
        : "No Goal 38 definition update is required.",
      next_safe_action:
        directBlockers.length > 0
          ? "After the contract is added, rerun this gate before implementing any Goal 38 production behaviour."
          : "Proceed to the Goal 38 implementation gate in LOCAL_PROOF or DRY_RUN_PUBLISH.",
    },
    safety: {
      local_proof_only: true,
...
function renderGoal38DefinitionMissingMarkdown(report = {}) {
  const lines = [];
  lines.push("# Goal 38 - Definition Missing Gate");
  lines.push("");
...
function renderOperatorRequest(report = {}) {
  const lines = [];
  lines.push("# Goal 38 Operator Request");
  lines.push("");
  lines.push("Goal 38 cannot be implemented from the current campaign docs.");
  lines.push("");
  lines.push("## Required Update");
  lines.push("- Add `### 38. <Goal Title>` to `docs/codex-main-goal.md`.");
...
async function writeGoal38DefinitionMissing(report = {}, { outputDir } = {}) {
  if (!outputDir) throw new Error("writeGoal38DefinitionMissing requires outputDir");
  await fs.ensureDir(outputDir);
  const paths = {
    readinessJson: path.join(outputDir, "goal38_readiness_report.json"),
    readinessMarkdown: path.join(outputDir, "goal38_readiness_report.md"),
    contractGapReport: path.join(outputDir, "goal38_contract_gap_report.json"),
    operatorRequest: path.join(outputDir, "goal38_operator_request.md"),
  };
  await fs.writeJson(paths.readinessJson, report, { spaces: 2 });
  await fs.outputFile(paths.readinessMarkdown, renderGoal38DefinitionMissingMarkdown(report));
...
module.exports = {
  ...
  buildGoal38DefinitionMissing,
  extractGoalSection,
  renderGoal38DefinitionMissingMarkdown,
  writeGoal38DefinitionMissing,
};
