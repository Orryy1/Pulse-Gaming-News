...
const GOAL_ID = "39_goal_definition_missing";
const GOAL_NUMBER = 39;
...
function directBlockersFor(goalDefinition = {}) {
  const blockers = [];
  if (!goalDefinition.found) blockers.push("campaign:goal39_definition_missing");
  if (!asArray(goalDefinition.outputs).length) blockers.push("campaign:goal39_outputs_missing");
  if (!goalDefinition.acceptance_criteria_found) blockers.push("campaign:goal39_acceptance_criteria_missing");
  return blockers;
}

function upstreamBlockers(upstreamGoal38Report = {}) {
  if (!upstreamGoal38Report || typeof upstreamGoal38Report !== "object") {
    return ["upstream:goal38_goal_definition_missing_missing"];
  }
  const verdict = upstreamGoal38Report.verdict || upstreamGoal38Report.status;
  if (passLike(verdict)) return [];
  return ["upstream:goal38_goal_definition_missing_blocked"];
}
  ...
  return {
    ...
    required_operator_action:
      asArray(report.direct_blockers).length > 0
        ? "Add the Goal 39 contract to docs/codex-main-goal.md, including heading, scope, outputs and acceptance criteria."
        : "No definition gap detected for Goal 39.",
    safety: {
...
async function buildGoal39DefinitionMissing({
  ...
  const upstream = upstreamBlockers(upstreamGoal38Report);
  ...
  const report = {
    ...
    operator_request: {
      ...
      requested_update: directBlockers.length > 0
        ? "Define Goal 39 in docs/codex-main-goal.md before implementation continues, including the goal heading, scope, outputs and acceptance criteria."
        : "No Goal 39 definition update is required.",
      next_safe_action:
        directBlockers.length > 0
          ? "After the contract is added, rerun this gate before implementing any Goal 39 production behaviour."
          : "Proceed to the Goal 39 implementation gate in LOCAL_PROOF or DRY_RUN_PUBLISH.",
    },
    safety: {
      local_proof_only: true,
...
function renderGoal39DefinitionMissingMarkdown(report = {}) {
  const lines = [];
  lines.push("# Goal 39 - Definition Missing Gate");
  lines.push("");
...
function renderOperatorRequest(report = {}) {
  const lines = [];
  lines.push("# Goal 39 Operator Request");
  lines.push("");
  lines.push("Goal 39 cannot be implemented from the current campaign docs.");
  lines.push("");
  lines.push("## Required Update");
  lines.push("- Add `### 39. <Goal Title>` to `docs/codex-main-goal.md`.");
...
async function writeGoal39DefinitionMissing(report = {}, { outputDir } = {}) {
  if (!outputDir) throw new Error("writeGoal39DefinitionMissing requires outputDir");
  await fs.ensureDir(outputDir);
  const paths = {
    readinessJson: path.join(outputDir, "goal39_readiness_report.json"),
    readinessMarkdown: path.join(outputDir, "goal39_readiness_report.md"),
    contractGapReport: path.join(outputDir, "goal39_contract_gap_report.json"),
    operatorRequest: path.join(outputDir, "goal39_operator_request.md"),
  };
  await fs.writeJson(paths.readinessJson, report, { spaces: 2 });
  await fs.outputFile(paths.readinessMarkdown, renderGoal39DefinitionMissingMarkdown(report));
