...
const GOAL_ID = "40_goal_definition_missing";
const GOAL_NUMBER = 40;
...
function directBlockersFor(goalDefinition = {}) {
  const blockers = [];
  if (!goalDefinition.found) blockers.push("campaign:goal40_definition_missing");
  if (!asArray(goalDefinition.outputs).length) blockers.push("campaign:goal40_outputs_missing");
  if (!goalDefinition.acceptance_criteria_found) blockers.push("campaign:goal40_acceptance_criteria_missing");
  return blockers;
}

function upstreamBlockers(upstreamGoal39Report = {}) {
  if (!upstreamGoal39Report || typeof upstreamGoal39Report !== "object") {
    return ["upstream:goal39_goal_definition_missing_missing"];
  }
  const verdict = upstreamGoal39Report.verdict || upstreamGoal39Report.status;
  if (passLike(verdict)) return [];
  return ["upstream:goal39_goal_definition_missing_blocked"];
}
  ...
  return {
    ...
    required_operator_action:
      asArray(report.direct_blockers).length > 0
        ? "Add the Goal 40 contract to docs/codex-main-goal.md, including heading, scope, outputs and acceptance criteria."
        : "No definition gap detected for Goal 40.",
    safety: {
...
async function buildGoal40DefinitionMissing({
  campaignDocPath = path.join(process.cwd(), "docs", "codex-main-goal.md"),
  storyPackages = [],
  upstreamGoal39Report = {},
  ...
  const upstream = upstreamBlockers(upstreamGoal39Report);
  ...
  const report = {
    ...
    operator_request: {
      ...
      requested_update: directBlockers.length > 0
        ? "Define Goal 40 in docs/codex-main-goal.md before implementation continues, including the goal heading, scope, outputs and acceptance criteria."
        : "No Goal 40 definition update is required.",
      next_safe_action:
        directBlockers.length > 0
          ? "After the contract is added, rerun this gate before implementing any Goal 40 production behaviour."
          : "Proceed to the Goal 40 implementation gate in LOCAL_PROOF or DRY_RUN_PUBLISH.",
    },
    safety: {
      local_proof_only: true,
...
function renderGoal40DefinitionMissingMarkdown(report = {}) {
  const lines = [];
  lines.push("# Goal 40 - Definition Missing Gate");
  lines.push("");
...
function renderOperatorRequest(report = {}) {
  const lines = [];
  lines.push("# Goal 40 Operator Request");
  lines.push("");
  lines.push("Goal 40 cannot be implemented from the current campaign docs.");
  lines.push("");
  lines.push("## Required Update");
  lines.push("- Add `### 40. <Goal Title>` to `docs/codex-main-goal.md`.");
...
async function writeGoal40DefinitionMissing(report = {}, { outputDir } = {}) {
  if (!outputDir) throw new Error("writeGoal40DefinitionMissing requires outputDir");
  await fs.ensureDir(outputDir);
  const paths = {
    readinessJson: path.join(outputDir, "goal40_readiness_report.json"),
    readinessMarkdown: path.join(outputDir, "goal40_readiness_report.md"),
    contractGapReport: path.join(outputDir, "goal40_contract_gap_report.json"),
    operatorRequest: path.join(outputDir, "goal40_operator_request.md"),
  };
  await fs.writeJson(paths.readinessJson, report, { spaces: 2 });
  await fs.outputFile(paths.readinessMarkdown, renderGoal40DefinitionMissingMarkdown(report));
...
module.exports = {
  ...
  extractGoalSection,
  renderGoal40DefinitionMissingMarkdown,
