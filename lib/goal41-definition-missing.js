...
const GOAL_ID = "41_goal_definition_missing";
const GOAL_NUMBER = 41;
...
function directBlockersFor(goalDefinition = {}) {
  const blockers = [];
  if (!goalDefinition.found) blockers.push("campaign:goal41_definition_missing");
  if (!asArray(goalDefinition.outputs).length) blockers.push("campaign:goal41_outputs_missing");
  if (!goalDefinition.acceptance_criteria_found) blockers.push("campaign:goal41_acceptance_criteria_missing");
  return blockers;
}

function upstreamBlockers(upstreamGoal40Report = {}) {
  if (!upstreamGoal40Report || typeof upstreamGoal40Report !== "object") {
    return ["upstream:goal40_goal_definition_missing_missing"];
  }
  const verdict = upstreamGoal40Report.verdict || upstreamGoal40Report.status;
  if (passLike(verdict)) return [];
  return ["upstream:goal40_goal_definition_missing_blocked"];
}
  ...
  return {
    ...
    required_operator_action:
      asArray(report.direct_blockers).length > 0
        ? "Add the Goal 41 contract to docs/codex-main-goal.md, including heading, scope, outputs and acceptance criteria."
        : "No definition gap detected for Goal 41.",
    safety: {
...
async function buildGoal41DefinitionMissing({
  ...
  const upstream = upstreamBlockers(upstreamGoal40Report);
  ...
  const report = {
    ...
    operator_request: {
      ...
      requested_update: directBlockers.length > 0
        ? "Define Goal 41 in docs/codex-main-goal.md before implementation continues, including the goal heading, scope, outputs and acceptance criteria."
        : "No Goal 41 definition update is required.",
      next_safe_action:
        directBlockers.length > 0
          ? "After the contract is added, rerun this gate before implementing any Goal 41 production behaviour."
          : "Proceed to the Goal 41 implementation gate in LOCAL_PROOF or DRY_RUN_PUBLISH.",
    },
    safety: {
      local_proof_only: true,
...
function renderGoal41DefinitionMissingMarkdown(report = {}) {
  const lines = [];
  lines.push("# Goal 41 - Definition Missing Gate");
  lines.push("");
...
function renderOperatorRequest(report = {}) {
  const lines = [];
  lines.push("# Goal 41 Operator Request");
  lines.push("");
  lines.push("Goal 41 cannot be implemented from the current campaign docs.");
  lines.push("");
  lines.push("## Required Update");
  lines.push("- Add `### 41. <Goal Title>` to `docs/codex-main-goal.md`.");
...
async function writeGoal41DefinitionMissing(report = {}, { outputDir } = {}) {
  if (!outputDir) throw new Error("writeGoal41DefinitionMissing requires outputDir");
  await fs.ensureDir(outputDir);
  const paths = {
    readinessJson: path.join(outputDir, "goal41_readiness_report.json"),
    readinessMarkdown: path.join(outputDir, "goal41_readiness_report.md"),
    contractGapReport: path.join(outputDir, "goal41_contract_gap_report.json"),
    operatorRequest: path.join(outputDir, "goal41_operator_request.md"),
  };
  await fs.writeJson(paths.readinessJson, report, { spaces: 2 });
  await fs.outputFile(paths.readinessMarkdown, renderGoal41DefinitionMissingMarkdown(report));
