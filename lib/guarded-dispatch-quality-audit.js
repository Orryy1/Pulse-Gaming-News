...
function requiredHyperframesCardCount(renderManifest = {}) {
  const gate = premiumShellGate(renderManifest);
  const value =
  ...
  if (available !== null && available < MIN_HYPERFRAMES_CARDS) return MIN_HYPERFRAMES_CARDS;
  ...
  if (duration !== null) {
    ...
    if (feasibleByDuration > 0) return Math.max(1, Math.min(MIN_HYPERFRAMES_CARDS, feasibleByDuration));
  }
  return MIN_HYPERFRAMES_CARDS;
}
...
async function inspectStory({ storyId, actions = [] } = {}) {
  ...
  const requiredCardCount = requiredHyperframesCardCount(renderManifest);
  const blockers = [
  ...
  if (cardCount < requiredCardCount) {
    blockers.push(`hyperframes_card_count_below_target:${cardCount}/${requiredCardCount}`);
  }
  ...
  return {
    ...
    hyperframes_required_card_count: requiredCardCount,
...
async function buildGuardedDispatchQualityAudit({
  ...
  const blockedActions = asArray(guardedDispatchExecutorPreflight.blocked_selected_actions);
  ...
  for (const action of blockedActions) {
    const storyId = cleanText(action.story_id || action.action_id);
    if (!storyId) continue;
    const existing = stories.find((story) => cleanText(story.story_id) === storyId);
    const blockers = asArray(action.blockers).map(cleanText).filter(Boolean);
    if (existing) {
      existing.blockers = Array.from(new Set([...asArray(existing.blockers), ...blockers]));
      continue;
    }
    stories.push({
      story_id: storyId,
      ...
      artifact_dir: "",
      platforms: [cleanText(action.platform)].filter(Boolean),
      hyperframes_card_count: null,
      scene_count: null,
      blockers,
      warnings: [],
    });
  ...
  return {
    ...
    summary: {
      action_count: actions.length + blockedActions.length,
