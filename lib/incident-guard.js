...
const MIN_REPEAT_GUARD_READABLE_CARD_DURATION_S = 7;
...
function subjectAliasKeys(subject) {
  const key = normaliseSubjectKey(subject);
...
function hasSubject(text, subject) {
  ...
  const haystackKey = normaliseSubjectKey(text);
  const subjectLower = lower(subjectText);
  if (haystack.includes(subjectLower)) return true;
  if (subjectAliasKeys(subjectText).some((alias) => haystackKey.includes(alias))) return true;
...
const EVIDENCE_BACKED_COVER_STAKE_TOKENS = new Set([
  "boss",
  "catch",
  "danger",
  "date",
...
const EVIDENCE_BACKED_COVER_STOP_TOKENS = new Set([
  "about",
  "after",
  "again",
  "ahead",
  ...
  "trailer",
  "with",
  ...EVIDENCE_BACKED_COVER_STAKE_TOKENS,
]);
...
function evidenceBackedCoverTokens(value = "") {
  return normaliseTokenText(value).split(/\s+/).filter(Boolean);
}

function manifestEvidenceText(canonical = {}, platformManifest = {}) {
  return normaliseTokenText([
    ...
    asArray(canonical.confirmed_claims).join(" "),
    asArray(canonical.allowed_public_wording).join(" "),
    JSON.stringify(platformManifest.outputs || {}),
  ].join(" "));
}

function hasEvidenceBackedCoverEntity(headline = "", canonical = {}, platformManifest = {}) {
  const tokens = evidenceBackedCoverTokens(headline);
  if (!tokens.some((token) => EVIDENCE_BACKED_COVER_STAKE_TOKENS.has(token))) return false;
  const evidence = manifestEvidenceText(canonical, platformManifest);
  if (!evidence) return false;
  const entityTokens = tokens.filter((token) =>
    token.length >= 4 &&
    !/^\d+$/.test(token) &&
    !EVIDENCE_BACKED_COVER_STOP_TOKENS.has(token)
  );
  const adjacentEntityPhrases = tokens
    .slice(0, -1)
    .map((token, index) => [token, tokens[index + 1]])
    .filter((phrase) =>
      phrase.every((token) =>
        !EVIDENCE_BACKED_COVER_STAKE_TOKENS.has(token) &&
        !EVIDENCE_BACKED_COVER_STOP_TOKENS.has(token),
      ),
    )
    .map((phrase) => phrase.join(" "));
  return (
    entityTokens.some((token) => evidence.includes(token)) ||
    adjacentEntityPhrases.some((phrase) => evidence.includes(phrase))
  );
}
...
function hasAppliedPlatformDisclosure(disclosureObjects = []) {
  return disclosureObjects.some((disclosure) =>
    flattenObjectValues(disclosure).some(({ key, value }) => {
      ...
      if (/(?:required|requirement)/i.test(key)) return false;
      return /(?:^|_)(?:commercial|platform|branded)(?:_content)?_disclosure_(?:present|applied|disclosed|toggle)$/i.test(key);
    }),
  );
}

function platformOutputRows(platformManifest = {}) {
  return Object.values(platformManifest.outputs || {}).filter(
    (row) => row && typeof row === "object" && !Array.isArray(row),
  );
}

function syntheticDisclosureRequiredInOutput(output = {}) {
  return (
    output.synthetic_media_required === true ||
    output.altered_synthetic_disclosure_required === true ||
    output.ai_disclosure_required === true
  );
}

function syntheticDisclosurePresentInOutput(output = {}) {
  return (
    output.altered_synthetic_disclosure_present === true ||
    output.youtube_synthetic_media_disclosed === true ||
    output.synthetic_media_disclosed === true ||
    output.ai_disclosure_applied === true ||
    output.ai_generated_content_label_applied === true
  );
}

function paidPromotionRequiredInOutput(output = {}) {
  return output.paid_promotion_disclosure_required === true;
}

function paidPromotionPresentInOutput(output = {}) {
  return (
    output.paid_promotion_disclosure_present === true ||
    output.paid_promotion_disclosed === true ||
    output.paid_promotion_toggle === true
  );
}

function monotonicRequirement(sources = [], pattern) {
  return sources.some((source) => (
    Object.entries(source || {}).some(([key, value]) => value === true && pattern.test(key))
  ));
}
...
function platformDisclosureMissing({
  ...
  const outputs = platformOutputRows(platformManifest);
  ...
  const syntheticRequiredOutputs = outputs.filter(syntheticDisclosureRequiredInOutput);
  const syntheticRequired = Boolean(
    policyReport.ai_disclosure_gate?.disclosure_required === true ||
    monotonicRequirement(requirementSources, /(?:^|_)(?:ai|synthetic|altered)(?:_|$)/i) ||
    syntheticRequiredOutputs.length > 0
  );
  const syntheticPresent = syntheticRequiredOutputs.length > 0
    ? syntheticRequiredOutputs.every(syntheticDisclosurePresentInOutput)
    : Boolean(
        policyReport.ai_disclosure_gate?.disclosure_present === true ||
        policyReport.disclosure_evidence?.altered_or_synthetic_content?.present === true ||
        outputs.some(syntheticDisclosurePresentInOutput)
      );
  if (syntheticRequired && !syntheticPresent) return true;

  const paidRequiredOutputs = outputs.filter(paidPromotionRequiredInOutput);
  const paidPromotionRequired = Boolean(
    policyReport.platform_policy_gate?.paid_promotion_required === true ||
    monotonicRequirement(requirementSources, /(?:^|_)(?:paid|promotion|sponsor|brand)(?:_|$)/i) ||
    paidRequiredOutputs.length > 0
  );
  const paidPromotionPresent = paidRequiredOutputs.length > 0
    ? paidRequiredOutputs.every(paidPromotionPresentInOutput)
    : Boolean(
        policyReport.platform_policy_gate?.paid_promotion_present === true ||
        policyReport.disclosure_evidence?.paid_promotion?.present === true ||
        outputs.some(paidPromotionPresentInOutput)
      );
  if (paidPromotionRequired && !paidPromotionPresent) return true;

  const platformRequired = requirementSources.some((requirements) => (
    Object.entries(requirements || {}).some(([key, value]) => {
      if (value !== true) return false;
      if (/affiliate/i.test(key)) return false;
      if (/(?:^|_)(?:paid|promotion|sponsor|brand|ai|synthetic|altered)(?:_|$)/i.test(key)) return false;
      return /(?:platform|commercial|branded)/i.test(key);
    })
  ));
  ...
  if (
    hasAppliedPlatformDisclosure([
      ...
      policyReport.disclosure_evidence,
...
function collectVisualQualityBlockers({
  ...
  if (objectHasEvidence(frameRules) && frameRules.text_objects_within_safe_bounds === false) {
    blockers.push("incident:text_cutoff_risk");
  }
  if (objectHasEvidence(frameRules) && frameRules.source_labels_mobile_readable === false) {
    blockers.push("incident:source_label_unreadable");
  }
  if (objectHasEvidence(frameRules) && frameRules.caption_overlay_clear === false) {
    blockers.push("incident:caption_overlay_collision_risk");
  }
...
function collectSfxSourceBlockers(sfxManifest = {}) {
  ...
  for (const asset of selectedAssets) {
    ...
    const score = Number(editorialSfxScore(asset, undefined, role));
...
function evaluateIncidentGuard({
  ...
  if (
    canonicalSubject &&
    thumbnailHeadline &&
    !hasSubject(thumbnailHeadline, canonicalSubject) &&
    !hasEvidenceBackedCoverEntity(thumbnailHeadline, canonical, platformManifest)
  ) {
  ...
  if (renderPolicyRequired(renderManifest)) {
    ...
    const repeatGuardCardFloor = numericValue(
    ...
    if (
      ...
      repeatGuardCardFloor + 0.001 < MIN_REPEAT_GUARD_READABLE_CARD_DURATION_S
    ) {
      add(blockers, "incident:hyperframes_repeat_guard_card_floor_too_low");
    }
  }
  ...
  const tooFastHyperframesEvidence = [
  ...
  const hyperframesEstimatedCardVisibleDuration = numericValue(
    fileEvidence.hyperframes_estimated_card_visible_duration_s ??
      fileEvidence.hyperframes_estimated_card_dwell_s ??
      fileEvidence.estimated_card_visible_duration_s,
  );
  const hyperframesCardCount = numericValue(fileEvidence.hyperframes_card_count);
  const renderedCardWindowCount = numericValue(fileEvidence.rendered_card_window_count);
  const hasReadableRenderedCardWindowEvidence =
    (renderedCardWindowCount !== null && renderedCardWindowCount > 0) ||
    asArray(fileEvidence.rendered_card_windows).length > 0 ||
    asArray(fileEvidence.card_visible_windows).length > 0 ||
    asArray(renderManifest.card_visible_windows).length > 0 ||
    asArray(renderManifest.rendered_card_windows).length > 0 ||
    asArray(renderManifest.clip_scene_plan?.card_visible_windows).length > 0;
  if (
    tooFastHyperframesEvidence.length ||
    (
      !hasReadableRenderedCardWindowEvidence &&
      hyperframesCardCount !== null &&
      hyperframesCardCount > 0 &&
      hyperframesEstimatedCardVisibleDuration !== null &&
      hyperframesEstimatedCardVisibleDuration + 0.001 < MIN_REPEAT_GUARD_READABLE_CARD_DURATION_S
    )
  ) {
    add(blockers, "incident:hyperframes_card_dwell_too_short");
  }
  if (asArray(fileEvidence.hyperframes_repeated_card_families).length) {
    add(blockers, "incident:hyperframes_repeated_card_family");
  }
  if (
    asArray(fileEvidence.repeated_direct_motion_segments).length ||
    Number(fileEvidence.repeated_direct_motion_segment_count || 0) > 0
  ) {
    add(blockers, "incident:repeated_direct_motion_segment");
  }
