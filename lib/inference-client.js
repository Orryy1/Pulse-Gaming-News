...
/**
 * Raised by waitForReady when /health reports phase='failed'. Distinct
 * from InferTimeoutError so the caller can tell "the service never
 * came up" from "I stopped waiting". Contains the last_error string
 * from the server so the log line alone is enough to diagnose.
 */
class InferFailedStateError extends Error {
  constructor(lastError, health) {
    super(
      `[inference-client] inference service reported phase='failed' ` +
        `(last_error=${lastError || "unknown"}) — not polling further; ` +
        "restart the service and capture a py-spy dump before retrying",
    );
    this.name = "InferFailedStateError";
...
/**
 * Block until the inference service reports phase='ready' (or
 * 'ready-skipped' if the caller accepts the first-request cold-start
 * trade-off). Polls /health on an exponential backoff up to
 * deadlineMs total. Used by the jobs-runner boot path to prevent
 * GPU-only workers from claiming jobs before the engine is resident.
 *
 * Accepts {
 *   acceptSkipped   — treat phase='ready-skipped' as ready (default false)
 *   pollIntervalMs  — starting poll interval (default 2000, doubles up to 15000)
 *   deadlineMs      — give up after this long (default DEFAULT_TIMEOUT_MS)
 *   baseUrl, log    — as elsewhere
 * }.
 *
 * Returns the final /health payload on success, throws on deadline.
 * Never throws on transient network errors — those just get retried
 * until the deadline.
 */
async function waitForReady({
  ...
  const limitMs = deadlineMs || DEFAULT_TIMEOUT_MS;
  const startedAt = Date.now();
  let interval = pollIntervalMs;

  while (Date.now() - startedAt < limitMs) {
    const h = await health({ baseUrl, timeoutMs: 3_000 });
    if (h.ok) {
      // Phase 1B: hard-fail on phase='failed'. The service has flipped
      // itself into a state where no amount of further polling will
      // recover (watchdog expired or load threw); blocking the runner
      // here just delays the operator seeing the incident.
      if (h.phase === "failed") {
        log(
          `[inference-client] /health reports phase=failed last_error=${h.last_error || "-"} ` +
            `— bailing out of waitForReady`,
        );
        throw new InferFailedStateError(h.last_error, h);
      }
      const ready =
        h.phase === "ready" || (acceptSkipped && h.phase === "ready-skipped");
      if (ready) {
        log(
          `[inference-client] ready phase=${h.phase} last_load_ms=${h.last_load_ms ?? "-"}`,
        );
        return h;
      }
      log(
        `[inference-client] waiting phase=${h.phase} warming=${!!h.warming} ` +
          `engine_count=${h.engine_count ?? 0} last_heartbeat=${h.last_heartbeat_ts || "-"}`,
      );
    } else {
      log(
        `[inference-client] /health unreachable: ${h.error || "unknown"} — still waiting`,
      );
    }
    await new Promise((r) => setTimeout(r, interval));
    interval = Math.min(Math.floor(interval * 1.5), 15_000);
  }
  throw new InferTimeoutError(
    "waitForReady",
    Date.now() - startedAt,
    limitMs,
    null,
