"use strict";

/**
 * lib/intelligence/analytics-client.js — Session 3 (intelligence pass).
 *
 * Read-only YouTube analytics client with two modes:
 *   - mode: 'fixture' (default) — synthetic snapshot data, no network
 *   - mode: 'real'              — calls youtubeAnalytics.reports.query
 *                                 ONLY when the operator has
 *                                 explicitly authorised yt-analytics
 *                                 scopes. The function refuses to run
 *                                 without `INTELLIGENCE_REAL_MODE=true`
 *                                 set by the operator.
 *
 * This module never triggers OAuth, never prints tokens, and never
 * mutates production data. It only READS. Snapshot rows it produces
 * land in the local `video_performance_snapshots` table via the
 * caller; the client itself returns plain objects.
 *
 * Required scopes for real mode:
 *   - https://www.googleapis.com/auth/yt-analytics.readonly
 * The scope is listed in upload_youtube.js for future consent refreshes, but
 * existing tokens still need operator-approved re-auth before real mode works.
 */

const REQUIRED_REAL_SCOPES = [
  "https://www.googleapis.com/auth/yt-analytics.readonly",
];
const {
  buildRetentionQuery,
  buildTrafficSourceQuery,
  mapRetentionReportRows,
  mapTrafficSourceReportRows,
} = require("./youtube-analytics-ingestion-packet");

const SNAPSHOT_LABELS = ["+1h", "+3h", "+24h", "+72h", "+7d", "+28d"];

const TARGET_METRICS = [
  "views",
  "estimatedMinutesWatched",
  "averageViewDuration",
  "averageViewPercentage",
  "likes",
  "comments",
  "subscribersGained",
];

function numberOrNull(value) {
  const n = Number(value);
  return Number.isFinite(n) ? n : null;
}

function metricMapFromReport(data = {}) {
  const headers = Array.isArray(data.columnHeaders) ? data.columnHeaders : [];
  const row = Array.isArray(data.rows) && Array.isArray(data.rows[0])
    ? data.rows[0]
    : [];
  const out = {};
  headers.forEach((header, index) => {
    const name = header && header.name;
    if (!name) return;
    out[name] = row[index];
  });
  return out;
}

function mapYouTubeAnalyticsReportToSnapshot({
  videoId,
  label = "+24h",
  data = {},
  snapshotAt = new Date().toISOString(),
} = {}) {
  const metrics = metricMapFromReport(data);
  const estimatedMinutes = numberOrNull(metrics.estimatedMinutesWatched);
  const watchTimeSeconds =
    estimatedMinutes === null ? null : Number((estimatedMinutes * 60).toFixed(3));
  return {
    video_id: videoId,
    snapshot_label: label,
    snapshot_at: snapshotAt,
    views: numberOrNull(metrics.views),
    watch_time_seconds: watchTimeSeconds,
    average_view_duration_seconds: numberOrNull(metrics.averageViewDuration),
    average_percentage_viewed: numberOrNull(metrics.averageViewPercentage),
    likes: numberOrNull(metrics.likes),
    comments: numberOrNull(metrics.comments),
    subscribers_gained: numberOrNull(metrics.subscribersGained),
    raw_json: JSON.stringify(data || {}),
    fixture: false,
  };
}

function fixtureSnapshot({ videoId, label, baseSeed = 1 }) {
  // Deterministic pseudo-random — same videoId/label always returns
  // the same numbers so tests don't flap. No real metric data.
  let h = 0;
  const key = `${videoId}|${label}|${baseSeed}`;
  for (let i = 0; i < key.length; i++) {
    h = ((h << 5) - h + key.charCodeAt(i)) | 0;
  }
  const rand = (n) => {
    h = (h * 9301 + 49297) % 233280;
    return Math.abs(h) % n;
  };
  const stage = SNAPSHOT_LABELS.indexOf(label) + 1;
  return {
    video_id: videoId,
    snapshot_label: label,
    snapshot_at: new Date().toISOString(),
    views: 100 * stage + rand(60),
    watch_time_seconds: (28 + rand(8)) * (100 * stage + rand(60)) * 0.001,
    average_view_duration_seconds: 28 + rand(8),
    average_percentage_viewed: 0.45 + rand(20) / 100,
    likes: Math.round((100 * stage + rand(60)) * (0.05 + rand(3) / 100)),
    comments: Math.round((100 * stage + rand(60)) * (0.012 + rand(2) / 100)),
    subscribers_gained: rand(stage * 4),
    traffic_source: ["SHORTS", "BROWSE", "SUBSCRIBER", "SEARCH"][rand(4)],
    shorts_feed_views: Math.round((100 * stage + rand(60)) * 0.6),
    raw_json: JSON.stringify({ source: "fixture", baseSeed }),
    fixture: true,
  };
}

function fixturePullForVideo(videoId, baseSeed = 1) {
  return SNAPSHOT_LABELS.map((label) =>
    fixtureSnapshot({ videoId, label, baseSeed }),
  );
}

function defaultDateWindow() {
  return {
    startDate: new Date(Date.now() - 30 * 86400000).toISOString().slice(0, 10),
    endDate: new Date().toISOString().slice(0, 10),
  };
}

function fixtureRetentionIntelligenceInputsForVideo(videoId, baseSeed = 1) {
  const seed = String(videoId || "").length + Number(baseSeed || 1);
  const softness = (seed % 8) / 100;
  return {
    videoId,
    retentionRows: [
      {
        video_id: videoId,
        elapsed_video_time_ratio: 0,
        audience_watch_ratio: 1,
        relative_retention_performance: 1.05,
      },
      {
        video_id: videoId,
        elapsed_video_time_ratio: 0.05,
        audience_watch_ratio: Number((0.84 - softness).toFixed(3)),
        relative_retention_performance: 0.98,
      },
      {
        video_id: videoId,
        elapsed_video_time_ratio: 0.1,
        audience_watch_ratio: Number((0.74 - softness).toFixed(3)),
        relative_retention_performance: 0.92,
      },
      {
        video_id: videoId,
        elapsed_video_time_ratio: 0.35,
        audience_watch_ratio: Number((0.58 - softness).toFixed(3)),
        relative_retention_performance: 0.88,
      },
    ],
    trafficRows: [
      {
        video_id: videoId,
        traffic_source_type: "SHORTS",
        views: 1000 + seed * 10,
        estimated_minutes_watched: 42 + seed,
        average_view_duration_seconds: 25.2,
        average_percentage_viewed: 58,
      },
    ],
    fixture: true,
    safety: {
      oauth_triggered: false,
      network_called: false,
      token_values_printed: false,
      production_db_mutated: false,
      social_posting_triggered: false,
    },
  };
}

function buildAnalyticsClient(opts = {}) {
  const mode =
    opts.mode || process.env.INTELLIGENCE_ANALYTICS_MODE || "fixture";
  if (mode !== "fixture" && mode !== "real") {
    throw new Error(`unknown analytics mode "${mode}"`);
  }
  if (mode === "real" && process.env.INTELLIGENCE_REAL_MODE !== "true") {
    throw new Error(
      "real mode requires INTELLIGENCE_REAL_MODE=true plus an OAuth token with yt-analytics.readonly. See PULSE_INTELLIGENCE_MONETISATION_PASS.md §C.",
    );
  }
  return {
    mode,
    requiredScopes: REQUIRED_REAL_SCOPES,
    snapshotLabels: SNAPSHOT_LABELS.slice(),
    targetMetrics: TARGET_METRICS.slice(),
    async pullSnapshotsForVideo(videoId, options = {}) {
      if (mode === "fixture") {
        return fixturePullForVideo(videoId, options.baseSeed ?? 1);
      }
      // Real mode: structurally implement the call but refuse to fire
      // without an explicit authorised google client. The caller is
      // expected to pre-flight scope verification — this client does
      // not initiate OAuth.
      const auth = options.authClient;
      if (!auth) {
        throw new Error(
          "real mode requires options.authClient (a googleapis OAuth2 client with yt-analytics.readonly granted). This client never initiates OAuth.",
        );
      }
      let ya = options.youtubeAnalyticsClient;
      if (!ya) {
        const { google } = require("googleapis");
        ya = google.youtubeAnalytics({ version: "v2", auth });
      }
      const start =
        options.startDate ||
        new Date(Date.now() - 30 * 86400000).toISOString().slice(0, 10);
      const end = options.endDate || new Date().toISOString().slice(0, 10);
      const resp = await ya.reports.query({
        ids: "channel==MINE",
        startDate: start,
        endDate: end,
        metrics: TARGET_METRICS.join(","),
        filters: `video==${videoId}`,
      });
      return [
        mapYouTubeAnalyticsReportToSnapshot({
          videoId,
          label: options.label || "+24h",
          data: resp.data,
          snapshotAt: new Date().toISOString(),
        }),
      ];
    },
    async pullSnapshotsForVideos(videoIds = [], options = {}) {
      const out = [];
      for (const id of videoIds) {
        try {
          const rows = await this.pullSnapshotsForVideo(id, options);
          for (const r of rows) out.push(r);
        } catch (err) {
          out.push({
            video_id: id,
            error: err.message,
            snapshot_label: options.label || "+24h",
            fixture: mode === "fixture",
          });
        }
      }
      return out;
    },
    async pullRetentionIntelligenceInputsForVideo(videoId, options = {}) {
      if (mode === "fixture") {
        return fixtureRetentionIntelligenceInputsForVideo(
          videoId,
          options.baseSeed ?? 1,
        );
      }
      const auth = options.authClient;
      if (!auth) {
        throw new Error(
          "real mode requires options.authClient (a googleapis OAuth2 client with yt-analytics.readonly granted). This client never initiates OAuth.",
        );
      }
      let ya = options.youtubeAnalyticsClient;
      if (!ya) {
        const { google } = require("googleapis");
        ya = google.youtubeAnalytics({ version: "v2", auth });
      }
      const dates = {
        ...defaultDateWindow(),
        startDate: options.startDate || defaultDateWindow().startDate,
        endDate: options.endDate || defaultDateWindow().endDate,
      };
      const retentionResp = await ya.reports.query(
        buildRetentionQuery({ videoId, ...dates }),
      );
      const trafficResp = await ya.reports.query(
        buildTrafficSourceQuery({ videoId, ...dates }),
      );
      return {
        videoId,
        startDate: dates.startDate,
        endDate: dates.endDate,
        retentionRows: mapRetentionReportRows({
          videoId,
          data: retentionResp.data,
        }),
        trafficRows: mapTrafficSourceReportRows({
          videoId,
          data: trafficResp.data,
        }),
        fixture: false,
        safety: {
          oauth_triggered: false,
          network_called: true,
          token_values_printed: false,
          production_db_mutated: false,
          social_posting_triggered: false,
        },
      };
    },
    async pullRetentionIntelligenceInputsForVideos(videoIds = [], options = {}) {
      const out = [];
      for (const id of videoIds) {
        try {
          out.push(await this.pullRetentionIntelligenceInputsForVideo(id, options));
        } catch (err) {
          out.push({
            videoId: id,
            error: err.message,
            fixture: mode === "fixture",
          });
        }
      }
      return out;
    },
    describe() {
      return {
        mode,
        requiredScopes: REQUIRED_REAL_SCOPES,
        snapshotLabels: SNAPSHOT_LABELS,
        targetMetrics: TARGET_METRICS,
      };
    },
  };
}

module.exports = {
  buildAnalyticsClient,
  fixtureSnapshot,
  fixturePullForVideo,
  fixtureRetentionIntelligenceInputsForVideo,
  mapYouTubeAnalyticsReportToSnapshot,
  SNAPSHOT_LABELS,
  TARGET_METRICS,
  REQUIRED_REAL_SCOPES,
};
