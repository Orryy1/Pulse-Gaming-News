...
function externalIds(story = {}) {
  return {
    youtube: cleanText(story.youtube_post_id || story.youtube_id) || null,
    instagram_reel: cleanText(story.instagram_media_id) || null,
    facebook_reel: cleanText(story.facebook_post_id) || null,
  };
}
...
function buildCommercialLearningDigest({
  ...
  const offerBreakdown = [...offerMap.values()]
    ...
    .sort((a, b) => b.clicks - a.clicks);
  const clickedStoryIndex = new Map(topStories.map((story) => [story.story_id, story]));
  const trackedStories = [...manifestIndex.entries()]
    .map(([storyId, manifest]) => {
      const story = storyIndex.get(storyId) || {};
      const clicked = clickedStoryIndex.get(storyId);
      const views = storyViews(story);
      const clicks = Number(clicked?.clicks) || 0;
      const ids = externalIds(story);
      return {
        story_id: storyId,
        title:
          story.title ||
          manifest.short_title ||
          manifest.story_entities?.[0] ||
          storyId,
        manifest_generated_at: manifest.generated_at || null,
        learning_status: clicks > 0 ? "signal_available" : "waiting_for_click_data",
        clicks,
        views,
        affiliate_click_rate: clickRate(clicks, views),
        external_ids: ids,
        published_platforms: Object.entries(ids)
  ...
  return {
    ...
    totals: {
      ...
      tracked_stories: trackedStories.length,
    },
    top_stories: topStories,
    tracked_stories: trackedStories,
...
function renderCommercialLearningMarkdown(digest = {}) {
  ...
  lines.push("");
  lines.push("## Tracked published cohorts");
  if (!asArray(digest.tracked_stories).length) {
    lines.push("- No commercial story manifests are currently tracked.");
  } else {
    for (const story of asArray(digest.tracked_stories).slice(0, 10)) {
      lines.push(
        `- ${story.title}: ${story.learning_status}, ${story.clicks || 0} click(s), platforms ${asArray(story.published_platforms).join(", ") || "not linked"}`,
      );
    }
  }
  lines.push("");
