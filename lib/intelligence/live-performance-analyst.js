...
// Cross-tick dedupe window. After we Discord-notify about a given
// (story_id, metric, signal_kind), suppress further Discord posts for
// the same tuple for this many hours. The signal row is still
// persisted so analytics + the morning digest see the full timeline —
// only the operator-facing chat post is rate-limited.
const NOTIFY_DEDUPE_HOURS = 12;
...
async function runLiveAnalystPass({
  ...
  // 2026-04-30 fix (Discord report): collapse fan-out signals so
  // the same outlier doesn't appear 7x in one tick. We keep ALL the
  // raw signal rows in the persistence step (audit trail), but the
  // Discord poster only sees the per-tuple strongest.
  const collapsedForNotify = collapseFanoutSignals(allSignals);

  // Persist signals (raw, pre-collapse, so the morning digest sees
  // the full feature attribution)
  try {
  ...
  // - Per-tick: collapse fan-out signals to one per (story, metric,
  //   kind), then cap at 5 per pass.
  // - Cross-tick: skip when the same (story, metric, kind) was
  //   already notified within NOTIFY_DEDUPE_HOURS — same outlier
  //   doesn't re-spam every 30 minutes after the model converges.
  const notifySend =
    notify ||
    (async (msg) => {
      try {
  ...
  const strong = collapsedForNotify
    .filter((s) => Math.abs(s.severity) >= NOTIFY_SIGMA)
    .sort((a, b) => Math.abs(b.severity) - Math.abs(a.severity))
    .slice(0, 5);

  const markNotified =
    activeRepos && activeRepos.db
      ? activeRepos.db.prepare(
          `UPDATE live_performance_signals
         SET notified_at = datetime('now')
         WHERE story_id = ? AND metric = ? AND signal_kind = ?
           AND notified_at IS NULL
           AND id = (
             SELECT id FROM live_performance_signals
             WHERE story_id = ? AND metric = ? AND signal_kind = ?
  ...
  for (const s of strong) {
    const fresh = isFreshToNotify({
      db: activeRepos && activeRepos.db,
      storyId: s.story_id,
      metric: s.metric,
      signalKind: s.signal_kind,
      windowHours: NOTIFY_DEDUPE_HOURS,
    });
    if (!fresh) {
      summary.suppressed_dupes = (summary.suppressed_dupes || 0) + 1;
      continue;
    }
...
/**
 * Collapse fan-out signals into one row per (story_id, metric,
 * signal_kind). The analyser emits a separate signal for each
 * (feature_kind, metric) cell — that's 7 * 3 rows for one outlier.
 * Operationally those describe one story-level event ("rss_X is
 * overperforming on views"), not seven independent signals. Pick
 * the strongest |severity| and merge the feature_kind list into the
 * raw_json so the audit trail is preserved.
 *
 * Pure — exported for unit testing.
 */
function collapseFanoutSignals(signals) {
  const byKey = new Map();
  for (const s of signals || []) {
    if (!s) continue;
    const key = `${s.story_id}|${s.metric}|${s.signal_kind}`;
    const prev = byKey.get(key);
    if (!prev || Math.abs(s.severity) > Math.abs(prev.severity)) {
      byKey.set(key, s);
    }
  }
  return [...byKey.values()];
...
/**
 * Check whether a (story_id, metric, signal_kind) tuple was already
 * notified inside the dedupe window. Returns true when SAFE to
 * notify, false when we should suppress.
 *
 * Pure DB read against live_performance_signals.notified_at —
 * exported for unit testing.
 */
function isFreshToNotify({ db, storyId, metric, signalKind, windowHours }) {
  if (!db || !storyId || !metric || !signalKind) return true;
  try {
    const row = db
      .prepare(
        `SELECT 1 FROM live_performance_signals
         WHERE story_id = ?
           AND metric = ?
           AND signal_kind = ?
      ...
      .get(storyId, metric, signalKind, `-${windowHours} hours`);
    return !row;
  } catch {
    // If the query fails (table missing on a fresh deploy, etc), do
    // not silently flood — fail-safe by suppressing.
    return false;
  }
}
