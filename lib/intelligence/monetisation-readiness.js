...
function naturallySupportsAffiliate(story) {
  ...
  const productAngle =
    /\b(release|launch|price|deal|sale|discount|hardware|console|controller|accessory|peripheral|headset|storage|download|(?:pokemon|pok[eé]mon)\s+go|steam deck|game pass)\b/.test(
      text,
    );
...
function firstYppBlocker(snapshotReport) {
  const blockers = snapshotReport.summary.ypp_blockers || [];
...
function buildRevenuePathReport(snapshot = {}) {
  ...
  return {
    current_stage: yppEligible ? "ypp_ready_for_operator_review" : "pre_monetisation",
