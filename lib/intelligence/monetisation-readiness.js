...
function buildSponsorMediaKitDraft(snapshot = {}, options = {}) {
  const provenance = options.stateProvenance || null;
  const valueMissing = SPONSOR_REQUIRED_METRICS.filter(
    (key) => snapshot[key] === undefined || snapshot[key] === null,
  );
  const provenanceMissing = SPONSOR_REQUIRED_METRICS.filter(
    (key) => provenance?.fields?.[key]?.present === false,
  );
  const missing = [...new Set([...valueMissing, ...provenanceMissing])];
  const ready =
    missing.length === 0 &&
...
function buildMonetisationReadiness({
  ...
  const sponsorMediaKit = buildSponsorMediaKitDraft(snapshot, { stateProvenance });
...
function renderMonetisationReadinessMarkdown(readiness) {
  ...
  lines.push("");
  if (readiness.state_provenance) {
    lines.push("## State Source");
    lines.push("");
    lines.push(`Mode: ${readiness.state_provenance.mode}`);
    lines.push(`Source: ${readiness.state_provenance.source}`);
    const warnings = readiness.state_provenance.warnings || [];
    ...
    for (const key of importantFields) {
      const field = readiness.state_provenance.fields?.[key];
      if (!field) continue;
      lines.push(
        `- ${key}: source=${field.source}, present=${field.present}, value=${field.public_value}`,
      );
    }
    lines.push("");
  }
