...
const { describeLlmState } = require("../llm-key");
...
async function runOvernightClaudeAnalyst({
  ...
  if (!isEnabled(env)) return { enabled: false, skipped: "disabled_by_env" };
  const llmState = describeLlmState(env);
  if (!llmState.ok && !anthropicCall) {
    return {
      enabled: true,
      skipped: "llm_unavailable",
      provider: llmState.provider,
      key_state: llmState.state,
  ...
  try {
    ...
    return { enabled: true, error: `llm: ${err.message}` };
  }
  ...
  const recent = (stories || []).filter((s) => {
    const ts =
      Date.parse(
        s.exported_at ||
          s.rendered_at ||
          s.render_fallback_at ||
          s.published_at ||
          s.youtube_published_at ||
          s.created_at ||
          "",
      ) || 0;
    return ts >= sinceMs;
