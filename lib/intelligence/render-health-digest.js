...
function directVideoSubjectEvidence(candidate = {}) {
  ...
  for (const asset of directAssets) {
    ...
    if (!sidecarHasSubjectBearingText(asset)) continue;
    if (subjectTokenAppears(trustedAssetSubjectText(asset), subjectTokens)) continue;
    if (!subjectTokenAppears(sidecarSubjectText(asset), subjectTokens)) {
...
function buildBridgeSummary(candidates, sinceMs, context = {}) {
  ...
  for (const candidate of stamped) {
    ...
    if (subjectEvidence.subject_mismatch === true) {
