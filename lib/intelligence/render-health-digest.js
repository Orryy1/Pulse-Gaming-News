"use strict";

const fs = require("node:fs");
const path = require("node:path");

/**
 * lib/intelligence/render-health-digest.js
 *
 * Builds a per-day summary of render-quality metadata stamped by
 * assemble.js. The metadata fields the digest reads are:
 *
 *   story.render_lane                 // legacy_multi_image | legacy_single_image_fallback
 *   story.distinct_visual_count       // number — count of real images used
 *   story.render_quality_class        // premium | standard | fallback | reject
 *   story.outro_present               // boolean — was OUTRO_CARD attached?
 *   story.thumbnail_candidate_present // boolean
 *
 * Output:
 *   - JSON object with per-bucket counts
 *   - Markdown digest suitable for posting to Discord
 *
 * Why: the visual-count gate (content-qa.js) is currently warn-only by
 * operator choice. Before flipping BLOCK_THIN_VISUALS=true the operator
 * needs a daily picture of what fraction of recent renders would be
 * blocked. This digest turns the per-story stamps into that picture.
 *
 * Pure: takes stories in, returns a result. No DB writes, no Discord
 * posts. The handler in lib/job-handlers.js posts the markdown line.
 *
 * Stamp-less (pre-2026-04-29) stories are deliberately excluded from
 * percentages — they don't reflect current rendering and would dilute
 * the signal. Counted separately as `unstamped`.
 */

const QUALITY_CLASSES = ["premium", "standard", "fallback", "reject"];
const RENDER_LANES = ["legacy_multi_image", "legacy_single_image_fallback"];
const {
  isDirectVideoMotionAsset,
  visualEvidenceProfile,
} = require("../visual-evidence-classifier");

const ROOT = path.resolve(__dirname, "..", "..");
const DEFAULT_WINDOW_HOURS = 24;

function summariseNumeric(arr) {
  if (arr.length === 0) {
    return { min: null, max: null, median: null, mean: null };
  }
  const sorted = arr.slice().sort((a, b) => a - b);
  const min = sorted[0];
  const max = sorted[sorted.length - 1];
  const mid = Math.floor(sorted.length / 2);
  const median =
    sorted.length % 2 === 0
      ? (sorted[mid - 1] + sorted[mid]) / 2
      : sorted[mid];
  const mean = sorted.reduce((a, b) => a + b, 0) / sorted.length;
  return {
    min,
    max,
    median,
    mean: Math.round(mean * 10) / 10,
  };
}

function renderHealthTimestamp(story) {
  if (!story) return "";
  return (
    story.exported_at ||
    story.rendered_at ||
    story.render_fallback_at ||
    story.published_at ||
    story.youtube_published_at ||
    story.created_at ||
    ""
  );
}

function isWithinWindow(story, sinceMs) {
  if (!story) return false;
  // Prefer explicit render/export stamps. Older rows do not carry
  // exported_at, so fall back through publish timestamps before
  // created_at; otherwise a newly published legacy row created weeks
  // ago looks invisible to the daily Discord digest. Do not use
  // updated_at here: analytics and repair passes can update old rows
  // without producing a new render.
  const t = Date.parse(renderHealthTimestamp(story)) || 0;
  return t >= sinceMs;
}

function isStamped(story) {
  return (
    story &&
    typeof story.render_quality_class === "string" &&
    QUALITY_CLASSES.includes(story.render_quality_class)
  );
}

function bridgeHealthTimestamp(candidate) {
  if (!candidate) return "";
  return (
    candidate.scheduler_bridge_generated_at ||
    candidate.bridge_generated_at ||
    candidate.approved_at ||
    candidate.exported_at ||
    candidate.rendered_at ||
    candidate.published_at ||
    candidate.created_at ||
    candidate._bridge_loaded_at ||
    ""
  );
}

function isBridgeCandidateWithinWindow(candidate, sinceMs) {
  if (!candidate) return false;
  const t = Date.parse(bridgeHealthTimestamp(candidate)) || 0;
  return t >= sinceMs;
}

function normaliseVisualCount(candidate) {
  const direct =
    candidate.distinct_visual_count ??
    candidate.qa_visual_count ??
    candidate.visual_count;
  const n = Number(direct);
  if (Number.isFinite(n)) return n;

  if (Array.isArray(candidate.video_clips)) return candidate.video_clips.length;
  if (Array.isArray(candidate.visual_v4_bridge_video_clips)) {
    return candidate.visual_v4_bridge_video_clips.length;
  }
  if (Array.isArray(candidate.downloaded_images)) {
    return candidate.downloaded_images.length;
  }
  return null;
}

function asArray(value) {
  return Array.isArray(value) ? value.filter(Boolean) : [];
}

function cleanText(value) {
  return String(value || "").replace(/\s+/g, " ").trim();
}

function tokeniseSubject(value = "") {
  const stopWords = new Set([
    "the",
    "a",
    "an",
    "and",
    "or",
    "of",
    "to",
    "for",
    "with",
    "has",
    "have",
    "gets",
    "just",
    "new",
    "one",
    "risk",
  ]);
  return [
    ...new Set(
      cleanText(value)
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, " ")
        .split(/\s+/)
        .filter((token) => token && !stopWords.has(token) && (token.length >= 3 || /\d/.test(token))),
    ),
  ];
}

const CHARACTER_SPECIFIC_MOTION_CONTEXT_RE =
  /\b(?:character|fighter|roster|new[-_\s]?fighter|new[-_\s]?character|dlc[-_\s]?fighter|gameplay[-_\s]?reveal|ranked|rushdown|zoner|footsie|footsies|meter|combo|combos|knife[-_\s]?feints?|eskrima|space[-_\s]?control)\b/i;
const SPECIFIC_MOTION_SUBJECT_STOP_TOKENS = new Set([
  "ahead",
  "beat",
  "capcom",
  "character",
  "combat",
  "control",
  "dangerous",
  "defenders",
  "eskrima",
  "fairness",
  "fighter",
  "footage",
  "headline",
  "just",
  "knife",
  "looks",
  "made",
  "mode",
  "players",
  "pressure",
  "problem",
  "ranked",
  "reveal",
  "revealed",
  "rushdown",
  "space",
  "street",
  "videos",
  "while",
  "zoner",
  "zoners",
]);

function bridgeSubjectTokens(candidate = {}) {
  const subjectText = cleanText(
    candidate.canonical_subject ||
      candidate.canonical_game ||
      candidate.game_title ||
      candidate.entity ||
      candidate.title ||
      candidate.public_title,
  );
  return tokeniseSubject(subjectText);
}

function bridgeSpecificMotionSubjectTokens(candidate = {}) {
  const context = [
    candidate.title,
    candidate.selected_title,
    candidate.public_title,
    candidate.short_title,
    candidate.suggested_thumbnail_text,
    candidate.thumbnail_headline,
    candidate.full_script,
    candidate.tts_script,
    candidate.narration_script,
    candidate.primary_source_url,
    candidate.source_url,
    candidate.url,
  ]
    .map(cleanText)
    .filter(Boolean)
    .join(" ");
  if (!CHARACTER_SPECIFIC_MOTION_CONTEXT_RE.test(context)) return [];

  let urlPath = "";
  const url = cleanText(candidate.primary_source_url || candidate.source_url || candidate.url);
  if (url) {
    try {
      urlPath = new URL(url).pathname;
    } catch {
      urlPath = url;
    }
  }

  const gameTokens = new Set(tokeniseSubject(candidate.canonical_game || candidate.game_title || ""));
  const sourceTokens = new Set([
    ...tokeniseSubject(candidate.source_name),
    ...tokeniseSubject(candidate.primary_source_name),
  ]);
  const seen = new Set();
  return tokeniseSubject([
    candidate.motion_subject,
    candidate.featured_character,
    candidate.character_name,
    candidate.title,
    candidate.selected_title,
    candidate.public_title,
    candidate.short_title,
    candidate.suggested_thumbnail_text,
    candidate.thumbnail_headline,
    urlPath,
  ].map(cleanText).filter(Boolean).join(" "))
    .filter((token) => token.length >= 4)
    .filter((token) => !/^\d+$/.test(token))
    .filter((token) => !gameTokens.has(token))
    .filter((token) => !sourceTokens.has(token))
    .filter((token) => !SPECIFIC_MOTION_SUBJECT_STOP_TOKENS.has(token))
    .filter((token) => {
      if (seen.has(token)) return false;
      seen.add(token);
      return true;
    })
    .slice(0, 4);
}

function visualAssetPathCandidates(asset = {}) {
  const values = typeof asset === "string"
    ? [asset]
    : [
        asset.path,
        asset.local_path,
        asset.local_materialized_path,
        asset.local_materialised_path,
        asset.file_path,
        asset.media_path,
      ];
  return [...new Set(values.map(cleanText).filter(Boolean))];
}

function resolveLocalVisualAssetPath(value = "") {
  const text = cleanText(value);
  if (!text || /^local:\/\//i.test(text) || /^https?:\/\//i.test(text) || /^file:\/\//i.test(text)) return "";
  const normalised = text.replace(/\//g, path.sep);
  return path.isAbsolute(normalised) ? normalised : path.resolve(ROOT, normalised);
}

function readVisualAssetSidecar(asset = {}) {
  for (const candidatePath of visualAssetPathCandidates(asset)) {
    const resolved = resolveLocalVisualAssetPath(candidatePath);
    if (!resolved) continue;
    const sidecarPath = `${resolved}.json`;
    try {
      if (!fs.existsSync(sidecarPath)) continue;
      const sidecar = JSON.parse(fs.readFileSync(sidecarPath, "utf8"));
      if (sidecar && typeof sidecar === "object" && !Array.isArray(sidecar)) {
        return { ...sidecar, source_sidecar_path: sidecarPath };
      }
    } catch {
      // Render health is an observability path. A bad sidecar should not crash
      // the digest; scheduler preflight owns hard blocking.
    }
  }
  return null;
}

function enrichVisualAssetWithSidecar(asset = {}) {
  const normalised = typeof asset === "string" ? { path: asset } : { ...asset };
  const sidecar = readVisualAssetSidecar(normalised);
  if (!sidecar) return normalised;
  return {
    ...normalised,
    source_sidecar_path: cleanText(sidecar.source_sidecar_path),
    sidecar_source_url: cleanText(sidecar.source_url || sidecar.url),
    sidecar_source_family: cleanText(sidecar.source_family || sidecar.motion_family || sidecar.family),
    sidecar_entity: cleanText(sidecar.entity || sidecar.canonical_subject || sidecar.canonical_game),
  };
}

function subjectTokenAppears(text = "", subjectTokens = []) {
  const haystack = cleanText(text).toLowerCase();
  if (!haystack || !subjectTokens.length) return false;
  return subjectTokens.some((token) =>
    new RegExp(`(^|[^a-z0-9])${token.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}([^a-z0-9]|$)`).test(haystack),
  );
}

function sidecarSubjectText(asset = {}) {
  return cleanText([
    asset.sidecar_entity,
    asset.sidecar_source_family,
    asset.sidecar_source_url,
  ].filter(Boolean).join(" "));
}

function trustedAssetSubjectText(asset = {}) {
  return cleanText([
    asset.source_owner,
    asset.source_title,
    asset.entity,
    ...asArray(asset.entities),
    asset.canonical_subject,
    asset.canonical_game,
  ].filter(Boolean).join(" "));
}

function directAssetIdentityKeys(asset = {}) {
  return [
    asset.path,
    asset.local_path,
    asset.local_materialized_path,
    asset.local_materialised_path,
    asset.file_path,
    asset.media_path,
    asset.source_url,
    asset.url,
    asset.source_family,
    asset.motion_family,
    asset.family,
    asset.sidecar_source_url,
    asset.sidecar_source_family,
  ]
    .map((value) => cleanText(value).toLowerCase())
    .filter(Boolean);
}

function trustedAssetMatchesSubject(asset = {}, subjectTokens = [], matchTokens = []) {
  const trusted = trustedAssetSubjectText(asset);
  return subjectTokenAppears(trusted, subjectTokens) || subjectTokenAppears(trusted, matchTokens);
}

function sidecarHasSubjectBearingText(asset = {}) {
  if (cleanText(asset.sidecar_entity)) return true;
  const family = cleanText(asset.sidecar_source_family).toLowerCase();
  if (!family) return false;
  if (/^segment_source_family_\d+_window_\d+_\d+$/.test(family)) return false;
  if (/^family-\d+$/.test(family)) return false;
  if (/^(?:url:)?https?:\/\/[^ ]*steamstatic\.com\//.test(family)) return false;
  if (/^steam_\d+_/.test(family)) {
    const genericSteamTokens = new Set([
      "app",
      "apps",
      "asset",
      "assets",
      "clip",
      "clips",
      "direct",
      "extras",
      "family",
      "hash",
      "hls",
      "master",
      "media",
      "motion",
      "movie",
      "movies",
      "segment",
      "source",
      "steam",
      "store",
      "storefront",
      "trailer",
      "trailers",
      "url",
      "video",
      "videos",
      "webm",
      "window",
    ]);
    const meaningfulTokens = family
      .replace(/^steam_\d+_/, "")
      .split(/[^a-z0-9]+/)
      .filter((token) =>
        token &&
        !genericSteamTokens.has(token) &&
        !/^\d+$/.test(token) &&
        !/^[a-f0-9]{8,}$/.test(token),
      );
    return meaningfulTokens.length > 0;
  }
  return /[a-z]/.test(family);
}

function candidateRightsLedger(candidate = {}) {
  if (Array.isArray(candidate.rights_ledger)) return candidate.rights_ledger;
  if (Array.isArray(candidate.rights_records)) return candidate.rights_records;
  if (candidate.rights_ledger && typeof candidate.rights_ledger === "object") {
    return candidate.rights_ledger;
  }
  return {};
}

function motionEvidenceAssets(candidate = {}) {
  return [
    ...asArray(candidate.visual_v4_bridge_video_clips),
    ...asArray(candidate.video_clips),
    ...asArray(candidate.motion_clips),
    ...asArray(candidate.footage_inventory?.motion_inventory?.accepted_local_clips),
    ...asArray(candidate.footage_inventory?.motion_inventory?.production_motion_clips),
    ...asArray(candidate.rights_ledger),
    ...asArray(candidate.rights_records),
  ];
}

function directVideoSubjectEvidence(candidate = {}) {
  const subjectTokens = bridgeSubjectTokens(candidate);
  const requiredSpecificSubjectTokens = bridgeSpecificMotionSubjectTokens(candidate);
  const matchTokens = requiredSpecificSubjectTokens.length ? requiredSpecificSubjectTokens : subjectTokens;
  const directAssets = motionEvidenceAssets(candidate)
    .map(enrichVisualAssetWithSidecar)
    .filter((asset) => isDirectVideoMotionAsset(asset) || hasDirectVideoMotion({ video_clips: [asset] }));
  const trustedSubjectMatchAssetKeys = new Set();
  for (const asset of directAssets) {
    if (!trustedAssetMatchesSubject(asset, subjectTokens, matchTokens)) continue;
    for (const key of directAssetIdentityKeys(asset)) trustedSubjectMatchAssetKeys.add(key);
  }
  const mismatchedAssets = [];
  for (const asset of directAssets) {
    if (!asset.source_sidecar_path) continue;
    if (!requiredSpecificSubjectTokens.length && !sidecarHasSubjectBearingText(asset)) continue;
    if (trustedAssetMatchesSubject(asset, subjectTokens, matchTokens)) continue;
    if (directAssetIdentityKeys(asset).some((key) => trustedSubjectMatchAssetKeys.has(key))) continue;
    if (!subjectTokenAppears(sidecarSubjectText(asset), matchTokens)) {
      mismatchedAssets.push({
        id: cleanText(asset.id || asset.asset_id),
        path: cleanText(asset.path || asset.local_path || asset.file_path || asset.media_path),
        sidecar_source_family: cleanText(asset.sidecar_source_family),
        sidecar_source_url: cleanText(asset.sidecar_source_url),
        source_sidecar_path: cleanText(asset.source_sidecar_path),
      });
    }
  }
  return {
    direct_asset_count: directAssets.length,
    subject_mismatch: mismatchedAssets.length > 0,
    subject_tokens: subjectTokens,
    required_specific_subject_tokens: requiredSpecificSubjectTokens,
    mismatched_assets: mismatchedAssets,
  };
}

function hasDirectVideoMotion(candidate = {}) {
  return motionEvidenceAssets(candidate).some((asset) => {
    const text = [
      asset.path,
      asset.local_path,
      asset.file_path,
      asset.media_path,
      asset.source_url,
      asset.url,
      asset.source_type,
      asset.source_kind,
      asset.source_url_kind,
      asset.media_kind,
      asset.asset_type,
      asset.kind,
      asset.type,
      asset.licence_basis,
      asset.license_basis,
      asset.rights_basis,
      asset.rights_risk_class,
      asset.approval_status,
      asset.transformation_notes,
    ]
      .map(cleanText)
      .filter(Boolean)
      .join(" ")
      .toLowerCase();
    if (!text) return false;
    if (/screenshot|visual_still|still|image|jpg|jpeg|png|gif/.test(text)) return false;
    return /\.(?:mp4|mov|webm|mkv)(?:$|[?#])/i.test(text) ||
      /\b(?:direct_video|official_direct_media|official_trailer_segment|official_video|gameplay|trailer|steam_movie|hls_manifest)\b/i.test(text);
  });
}

function bridgeVisualEvidence(candidate = {}) {
  const profile = visualEvidenceProfile({
    story: candidate,
    rightsLedger: candidateRightsLedger(candidate),
    footageInventory: candidate.footage_inventory || {},
    directorPlan: candidate.visual_v4_director_plan || candidate.director_plan || {},
  });
  const subjectEvidence = directVideoSubjectEvidence(candidate);
  const hasDirectMotion =
    !subjectEvidence.subject_mismatch &&
    (Number(profile.direct_video_motion_asset_count || 0) > 0 ||
      hasDirectVideoMotion(candidate));
  return {
    profile,
    has_direct_video_motion: hasDirectMotion,
    direct_video_subject_evidence: subjectEvidence,
  };
}

function bridgeStoryId(candidate = {}) {
  const id = cleanText(candidate.id || candidate.story_id || candidate.slug);
  return id || "unknown";
}

function bridgeStoryTitle(candidate = {}) {
  return cleanText(
    candidate.title ||
      candidate.public_title ||
      candidate.upload_title ||
      candidate.suggested_title ||
      candidate.canonical_subject ||
      candidate.canonical_game ||
      "Untitled bridge candidate",
  );
}

function safeStoryIdForCommand(storyId = "") {
  return cleanText(storyId).replace(/[^a-zA-Z0-9_-]/g, "");
}

function directVideoEnrichmentCommands(storyId = "") {
  const safeStoryId = safeStoryIdForCommand(storyId);
  const storyFlag = safeStoryId ? ` --story-id ${safeStoryId}` : "";
  return [
    {
      step: "find_or_confirm_official_source_family",
      command:
        `npm run ops:v4-source-family-acquisition --${storyFlag} ` +
        "--story-packages output/goal-contract/story-packages.json " +
        "--work-order output/goal-contract/direct_video_enrichment_work_order.json " +
        "--output-json test/output/studio_v4_source_family_acquisition.json " +
        "--output-md test/output/studio_v4_source_family_acquisition.md " +
        "--intake-template test/output/visual_v4_source_family_intake_template.json",
      expected_output:
        "test/output/visual_v4_source_family_acquisition.json and test/output/visual_v4_source_family_intake_template.json",
    },
    {
      step: "discover_direct_media_from_official_pages",
      command:
        `npm run media:discover-direct-media -- --input test/output/visual_v4_source_family_intake_template.json${storyFlag}`,
      expected_output:
        "test/output/official_direct_media_discovery.json and test/output/official_direct_media_intake_template.json",
    },
    {
      step: "classify_licensed_direct_media_readiness",
      command: `npm run ops:v4-licensed-direct-media --${storyFlag}`,
      expected_output: "test/output/studio_v4_licensed_direct_media_acquisition.json",
    },
    {
      step: "rebuild_visual_v4_motion_pack",
      command: `npm run ops:v4-motion-pack --${storyFlag}`,
      expected_output: `output/studio-v4/motion-packs/${safeStoryId || "<story_id>"}_motion_pack_manifest.json`,
    },
    {
      step: "rematerialise_owned_motion",
      command: `npm run ops:goal-owned-motion --${storyFlag}`,
      expected_output: `output/goal-proof/batch/${safeStoryId || "<story_id>"}/owned_motion_manifest.json`,
    },
    {
      step: "refresh_render_input_work_order",
      command: `npm run ops:goal-render-inputs --${storyFlag}`,
      expected_output: "output/goal-contract/render_input_work_order.json",
    },
  ];
}

function buildDirectVideoEnrichmentJob(candidate = {}, profile = {}) {
  return buildDirectVideoEnrichmentJobWithContext(candidate, profile, {});
}

function buildDirectVideoEnrichmentJobWithContext(candidate = {}, profile = {}, context = {}) {
  const storyId = bridgeStoryId(candidate);
  return {
    story_id: storyId,
    title: bridgeStoryTitle(candidate),
    blocker_type: "visual_evidence:direct_video_motion_missing",
    repair_lane: "direct_video_enrichment",
    quality_gap: true,
    blocking_current_dry_run: context.blockingDryRunStoryIds?.has(storyId) === true,
    exact_missing_input:
      "Official, licensed or operator-approved direct-video gameplay/trailer motion materialised into the Visual V4 motion pack.",
    current_evidence: {
      real_media_asset_count: Number(profile.real_media_asset_count || 0),
      real_motion_asset_count: Number(profile.real_motion_asset_count || 0),
      direct_video_motion_asset_count: Number(profile.direct_video_motion_asset_count || 0),
      direct_video_motion_family_count: Number(profile.direct_video_motion_family_count || 0),
    },
    recommended_commands: directVideoEnrichmentCommands(storyId),
    expected_outputs: [
      `output/studio-v4/motion-packs/${safeStoryIdForCommand(storyId) || "<story_id>"}_motion_pack_manifest.json`,
      `output/goal-proof/batch/${safeStoryIdForCommand(storyId) || "<story_id>"}/owned_motion_manifest.json`,
      `output/goal-proof/batch/${safeStoryIdForCommand(storyId) || "<story_id>"}/render_manifest.json`,
    ],
    db_mutation_required: false,
    operator_approval_required: true,
    production_publish_allowed_by_this_work_order: false,
    post_repair_validation_command: "npm run ops:render-health -- --json",
    post_repair_validation_commands: [
      "npm run ops:render-health -- --json",
      "npm run ops:next-publish-candidates -- --preflight-qa --json",
    ],
  };
}

function storyReasonCodes(item = {}) {
  return [
    ...asArray(item.blockers),
    ...asArray(item.reason_codes),
    ...asArray(item.preflight_blockers),
    ...asArray(item.blocked_reasons),
    ...asArray(item.hold_reasons),
    ...asArray(item.qa_failures),
  ].map(cleanText).filter(Boolean);
}

function directVideoBlockedDryRunStoryIds(dryRunPlan = {}) {
  const ids = new Set();
  const rows = [
    ...asArray(dryRunPlan.blocked_stories),
    ...asArray(dryRunPlan.blocked),
    ...asArray(dryRunPlan.blocked_candidates),
  ];
  for (const row of rows) {
    const storyId = cleanText(row.story_id || row.id);
    if (!storyId) continue;
    const reasons = storyReasonCodes(row).join(" ").toLowerCase();
    if (reasons.includes("direct_video_enrichment_required")) {
      ids.add(storyId);
    }
  }
  return ids;
}

function buildDirectVideoEnrichmentWorkOrder(jobs = []) {
  const workOrderJobs = asArray(jobs);
  return {
    schema_version: 1,
    generated_at: new Date().toISOString(),
    execution_mode: "local_repair_planning_only",
    summary: {
      job_count: workOrderJobs.length,
      quality_gap_count: workOrderJobs.filter((job) => job.quality_gap === true).length,
      blocking_current_dry_run_count: workOrderJobs.filter(
        (job) => job.blocking_current_dry_run === true,
      ).length,
    },
    safety: {
      local_only: true,
      external_download_started: false,
      production_db_mutated: false,
      oauth_or_token_mutated: false,
      social_posting_triggered: false,
      publish_permission_granted: false,
    },
    jobs: workOrderJobs,
  };
}

function buildBridgeSummary(candidates, sinceMs, context = {}) {
  const input = Array.isArray(candidates) ? candidates : [];
  const inWindow = input.filter((candidate) =>
    isBridgeCandidateWithinWindow(candidate, sinceMs),
  );
  const stamped = inWindow.filter(isStamped);
  const quality = Object.fromEntries(QUALITY_CLASSES.map((k) => [k, 0]));
  quality.other = 0;
  const lane = {};
  const visualCounts = [];
  let thinCount = 0;
  const visualEvidence = {
    real_media_ready_count: 0,
    generated_only_motion_deck_count: 0,
    no_real_visual_media_asset_count: 0,
    direct_video_motion_count: 0,
    direct_video_subject_mismatch_count: 0,
    screenshot_derived_only_count: 0,
    direct_video_story_ids: [],
    direct_video_gap_story_ids: [],
    direct_video_subject_mismatch_story_ids: [],
    generated_only_story_ids: [],
    no_real_visual_media_story_ids: [],
    screenshot_derived_only_story_ids: [],
  };
  const directVideoEnrichmentJobs = [];

  for (const candidate of stamped) {
    const storyId = bridgeStoryId(candidate);
    if (QUALITY_CLASSES.includes(candidate.render_quality_class)) {
      quality[candidate.render_quality_class]++;
    } else {
      quality.other++;
    }

    const laneKey =
      typeof candidate.render_lane === "string" && candidate.render_lane.trim()
        ? candidate.render_lane.trim()
        : "unknown";
    lane[laneKey] = (lane[laneKey] || 0) + 1;

    const visualCount = normaliseVisualCount(candidate);
    if (Number.isFinite(visualCount)) {
      visualCounts.push(visualCount);
      if (visualCount < 3) thinCount++;
    }

    const evidence = bridgeVisualEvidence(candidate);
    const profile = evidence.profile || {};
    const subjectEvidence = evidence.direct_video_subject_evidence || {};
    if (Number(profile.real_media_asset_count) > 0) {
      visualEvidence.real_media_ready_count++;
    }
    if (profile.generated_only_motion_deck === true) {
      visualEvidence.generated_only_motion_deck_count++;
      visualEvidence.generated_only_story_ids.push(storyId);
    }
    if (asArray(profile.blockers).includes("visual_evidence:no_real_visual_media_asset")) {
      visualEvidence.no_real_visual_media_asset_count++;
      visualEvidence.no_real_visual_media_story_ids.push(storyId);
    }
    const hasSubjectMismatch = subjectEvidence.subject_mismatch === true;
    if (hasSubjectMismatch) {
      visualEvidence.direct_video_subject_mismatch_count++;
      visualEvidence.direct_video_subject_mismatch_story_ids.push(storyId);
    }
    if (evidence.has_direct_video_motion) {
      visualEvidence.direct_video_motion_count++;
      visualEvidence.direct_video_story_ids.push(storyId);
    } else if (!hasSubjectMismatch && Number(profile.real_motion_asset_count) > 0) {
      visualEvidence.screenshot_derived_only_count++;
      visualEvidence.screenshot_derived_only_story_ids.push(storyId);
    }
    if (!evidence.has_direct_video_motion) {
      visualEvidence.direct_video_gap_story_ids.push(storyId);
      directVideoEnrichmentJobs.push(
        buildDirectVideoEnrichmentJobWithContext(candidate, profile, context),
      );
    }
  }

  const total = stamped.length;
  const pct = (n) => (total > 0 ? Math.round((n / total) * 100) : 0);

  return {
    candidate_count: inWindow.length,
    total_in_window: inWindow.length,
    stamped: total,
    unstamped: inWindow.length - total,
    quality,
    lane,
    visual_count: summariseNumeric(visualCounts),
    thin_count: thinCount,
    visual_evidence: visualEvidence,
    direct_video_enrichment_work_order: buildDirectVideoEnrichmentWorkOrder(
      directVideoEnrichmentJobs,
    ),
    percentages: {
      quality: Object.fromEntries(
        Object.entries(quality).map(([k, v]) => [k, pct(v)]),
      ),
      lane: Object.fromEntries(
        Object.entries(lane).map(([k, v]) => [k, pct(v)]),
      ),
      thin: pct(thinCount),
    },
  };
}

/**
 * Compute the render-health summary over the supplied story rows. The
 * digest looks at stories whose exported_at / created_at falls within
 * the last `windowHours` (default 24h). Only stories with a stamped
 * `render_quality_class` are counted in percentages — old (unstamped)
 * rows are reported separately.
 *
 * Returns:
 *   {
 *     window_hours,
 *     total_in_window,
 *     stamped,
 *     unstamped,
 *     quality: { premium, standard, fallback, reject },
 *     lane:    { legacy_multi_image, legacy_single_image_fallback, other },
 *     outro:   { present, missing, unknown },
 *     thumbnail: { present, missing },
 *     visual_count: { min, max, median, mean }, // among stamped
 *     thin_count: number,    // distinct_visual_count < 3
 *     percentages: { quality, lane, outro, thumbnail, thin }
 *   }
 */
function buildRenderHealthSummary(stories, opts = {}) {
  const windowHours = Number(opts.windowHours) || DEFAULT_WINDOW_HOURS;
  const now = opts.now ? Date.parse(opts.now) || Date.now() : Date.now();
  const sinceMs = now - windowHours * 60 * 60 * 1000;

  const inWindow = (Array.isArray(stories) ? stories : []).filter((s) =>
    isWithinWindow(s, sinceMs),
  );
  const stamped = inWindow.filter(isStamped);
  const unstamped = inWindow.length - stamped.length;
  const bridge = buildBridgeSummary(opts.bridgeCandidates, sinceMs, {
    blockingDryRunStoryIds: directVideoBlockedDryRunStoryIds(opts.dryRunPlan),
  });

  const quality = Object.fromEntries(QUALITY_CLASSES.map((k) => [k, 0]));
  const lane = Object.fromEntries(RENDER_LANES.map((k) => [k, 0]));
  lane.other = 0;
  const outro = { present: 0, missing: 0, unknown: 0 };
  const thumbnail = { present: 0, missing: 0 };
  const visualCounts = [];
  let thinCount = 0;

  for (const s of stamped) {
    quality[s.render_quality_class]++;
    if (RENDER_LANES.includes(s.render_lane)) {
      lane[s.render_lane]++;
    } else {
      lane.other++;
    }
    if (s.outro_present === true) outro.present++;
    else if (s.outro_present === false) outro.missing++;
    else outro.unknown++;
    if (s.thumbnail_candidate_present === true) thumbnail.present++;
    else thumbnail.missing++;

    if (Number.isFinite(s.distinct_visual_count)) {
      visualCounts.push(s.distinct_visual_count);
      if (s.distinct_visual_count < 3) thinCount++;
    }
  }

  const total = stamped.length;
  const pct = (n) => (total > 0 ? Math.round((n / total) * 100) : 0);

  const percentages = {
    quality: Object.fromEntries(
      Object.entries(quality).map(([k, v]) => [k, pct(v)]),
    ),
    lane: Object.fromEntries(Object.entries(lane).map(([k, v]) => [k, pct(v)])),
    outro: {
      present: pct(outro.present),
      missing: pct(outro.missing),
      unknown: pct(outro.unknown),
    },
    thumbnail: {
      present: pct(thumbnail.present),
      missing: pct(thumbnail.missing),
    },
    thin: pct(thinCount),
  };

  return {
    window_hours: windowHours,
    total_in_window: inWindow.length,
    stamped: total,
    unstamped,
    quality,
    lane,
    outro,
    thumbnail,
    visual_count: summariseNumeric(visualCounts),
    thin_count: thinCount,
    percentages,
    bridge,
  };
}

function splitRenderHealthSummary(summary = {}, { generatedAt = new Date().toISOString() } = {}) {
  const { bridge, ...liveDbHealthReport } = summary || {};
  const bridgeHealthReport = {
    ...(bridge || {}),
    candidate_count_meaning: "scheduler_bridge_candidates",
    readiness_note:
      "Counts only governed scheduler bridge candidates supplied to render-health, separate from live DB stamped rows.",
  };
  const directVideoEnrichmentWorkOrder =
    bridgeHealthReport.direct_video_enrichment_work_order ||
    buildDirectVideoEnrichmentWorkOrder([]);
  return {
    render_health_report: summary,
    live_db_health_report: liveDbHealthReport,
    bridge_health_report: bridgeHealthReport,
    direct_video_enrichment_work_order: directVideoEnrichmentWorkOrder,
    discord_digest_payload: {
      generated_at: generatedAt,
      summary: {
        live_db_total_in_window: Number(liveDbHealthReport.total_in_window || 0),
        live_db_stamped: Number(liveDbHealthReport.stamped || 0),
        live_db_unstamped: Number(liveDbHealthReport.unstamped || 0),
        scheduler_bridge_candidate_count: Number(bridgeHealthReport.candidate_count || 0),
        scheduler_bridge_stamped: Number(bridgeHealthReport.stamped || 0),
        scheduler_bridge_direct_video_motion_count: Number(
          bridgeHealthReport.visual_evidence?.direct_video_motion_count || 0,
        ),
        scheduler_bridge_direct_video_gap_count: Number(
          bridgeHealthReport.visual_evidence?.direct_video_gap_story_ids?.length || 0,
        ),
        scheduler_bridge_direct_video_subject_mismatch_count: Number(
          bridgeHealthReport.visual_evidence?.direct_video_subject_mismatch_count || 0,
        ),
        direct_video_enrichment_job_count: Number(
          directVideoEnrichmentWorkOrder.summary?.job_count || 0,
        ),
      },
    },
  };
}

function appendBridgeDigest(lines, summary) {
  const bridge = summary && summary.bridge;
  if (!bridge || bridge.candidate_count <= 0) return;

  lines.push(
    `Bridge V4 final renders: ${bridge.stamped} stamped (${bridge.candidate_count} candidates).`,
  );
  if (bridge.stamped > 0 && bridge.visual_evidence) {
    const evidence = bridge.visual_evidence;
    lines.push(
      `Bridge visual evidence: real media ${evidence.real_media_ready_count}/${bridge.stamped}; ` +
        `direct-video motion ${evidence.direct_video_motion_count}/${bridge.stamped}; ` +
        `generated-only ${evidence.generated_only_motion_deck_count}; ` +
        `screenshot-derived only ${evidence.screenshot_derived_only_count}; ` +
        `direct-video subject mismatch ${evidence.direct_video_subject_mismatch_count || 0}.`,
    );
    if (evidence.direct_video_subject_mismatch_count > 0) {
      lines.push(
        "Bridge warning: direct-video sidecar provenance does not match the story subject; rebuild motion before publishing.",
      );
    }
    if (evidence.generated_only_motion_deck_count > 0) {
      lines.push(
        "Bridge warning: generated-only motion decks are not publish-ready even if they are stamped premium.",
      );
    }
    if (evidence.direct_video_motion_count === 0 && evidence.real_media_ready_count > 0) {
      lines.push(
        "Bridge warning: current bridge candidates use still-derived motion only; treat them as safer than cards but below rich gameplay-video standard.",
      );
    } else if (evidence.direct_video_motion_count < Math.ceil(bridge.stamped / 2)) {
      lines.push(
        "Bridge warning: direct-video motion coverage is low; do not treat still-derived V4 as rich gameplay-video standard.",
      );
    }
    const directVideoGaps = asArray(evidence.direct_video_gap_story_ids).slice(0, 8);
    if (directVideoGaps.length) {
      lines.push(`Direct-video gap sample: ${directVideoGaps.join(", ")}.`);
    }
    const enrichmentJobs = Number(
      bridge.direct_video_enrichment_work_order?.summary?.job_count || 0,
    );
    if (enrichmentJobs > 0) {
      lines.push(
        `Direct-video enrichment work orders: ${enrichmentJobs} local repair jobs written separately; this is a quality upgrade signal, not publish permission.`,
      );
    }
  }

  if (summary.stamped === 0 && bridge.stamped > 0) {
    lines.push(
      "live DB still has no stamped rows; bridge candidates are scheduler-visible dry-run inputs until operator-confirmed cutover persists them.",
    );
  }
}

/**
 * Render the summary as a Discord-friendly markdown block.
 */
function formatDigest(summary) {
  if (!summary) return "";
  const lines = [];
  const bridge = summary.bridge || {};
  const hasBridgeCandidates = Number(bridge.candidate_count || 0) > 0;
  lines.push(`**Render health (last ${summary.window_hours}h)**`);
  if (summary.stamped === 0) {
    if (hasBridgeCandidates) {
      appendBridgeDigest(lines, summary);
      lines.push(
        `Legacy DB rows: 0 stamped, ${summary.unstamped} unstamped ` +
          `(${summary.total_in_window} total in window; not scheduler readiness).`,
      );
      return lines.join("\n");
    }
    lines.push(
      `No stamped stories in window (${summary.total_in_window} total, ${summary.unstamped} unstamped pre-2026-04-29 rows).`,
    );
    appendBridgeDigest(lines, summary);
    return lines.join("\n");
  }
  lines.push(
    `Stamped renders: ${summary.stamped}` +
      (summary.unstamped > 0
        ? ` (+${summary.unstamped} unstamped legacy)`
        : ""),
  );
  appendBridgeDigest(lines, summary);

  // Quality class
  const q = summary.quality;
  const qp = summary.percentages.quality;
  lines.push("Quality class:");
  for (const k of QUALITY_CLASSES) {
    if (q[k] === 0 && k !== "premium") continue;
    lines.push(`  • ${k}: ${q[k]} (${qp[k]}%)`);
  }

  // Lane
  const l = summary.lane;
  const lp = summary.percentages.lane;
  lines.push("Render lane:");
  for (const k of RENDER_LANES) {
    if (l[k] === 0) continue;
    lines.push(`  • ${k}: ${l[k]} (${lp[k]}%)`);
  }
  if (l.other > 0) lines.push(`  • other: ${l.other} (${lp.other}%)`);

  // Outro
  const o = summary.outro;
  const op = summary.percentages.outro;
  lines.push(
    `Outro present: ${o.present}/${summary.stamped} (${op.present}%)` +
      (o.missing > 0 ? ` — ⚠ ${o.missing} missing` : "") +
      (o.unknown > 0 ? `, ${o.unknown} unknown` : ""),
  );

  // Thin count headline — drives the BLOCK_THIN_VISUALS decision
  lines.push(
    `Thin (visuals <3): ${summary.thin_count}/${summary.stamped} (${summary.percentages.thin}%)`,
  );

  // Visual count distribution
  const v = summary.visual_count;
  if (v.median !== null) {
    lines.push(
      `Visual count: median ${v.median}, mean ${v.mean}, range ${v.min}–${v.max}`,
    );
  }

  // Operator hint
  if (summary.percentages.thin >= 50) {
    lines.push(
      `⚠ Over half of stamped renders are thin. Hold off on BLOCK_THIN_VISUALS=true until upstream image enrichment lands more wins.`,
    );
  } else if (summary.percentages.thin <= 10 && summary.stamped >= 10) {
    lines.push(
      `✓ Thin-visual rate is low. BLOCK_THIN_VISUALS=true is approval-ready for a controlled next-window pilot; do not flip it silently because it changes live publish gating.`,
    );
  }

  return lines.join("\n");
}

/**
 * Run the digest end-to-end. Reads stories via the supplied db handle
 * (defaults to the canonical lib/db) and returns both summary + the
 * formatted markdown.
 */
async function runRenderHealthDigest({
  db = require("../db"),
  windowHours = DEFAULT_WINDOW_HOURS,
  now,
  bridgeCandidates = [],
  dryRunPlan = {},
} = {}) {
  let stories = [];
  try {
    stories = await db.getStories();
  } catch {
    stories = [];
  }
  const summary = buildRenderHealthSummary(stories, {
    windowHours,
    now,
    bridgeCandidates,
    dryRunPlan,
  });
  return { summary, markdown: formatDigest(summary) };
}

module.exports = {
  buildRenderHealthSummary,
  formatDigest,
  runRenderHealthDigest,
  isWithinWindow,
  isStamped,
  QUALITY_CLASSES,
  RENDER_LANES,
  DEFAULT_WINDOW_HOURS,
  renderHealthTimestamp,
  bridgeHealthTimestamp,
  isBridgeCandidateWithinWindow,
  normaliseVisualCount,
  buildBridgeSummary,
  splitRenderHealthSummary,
};
