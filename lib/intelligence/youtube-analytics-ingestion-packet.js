...
function isFixtureVideoId(videoId) {
  return /^fixture[-_]/i.test(String(videoId || "").trim());
}
...
function buildYouTubeAnalyticsIngestionPacket({
  ...
  const fixtureIds = ids.filter(isFixtureVideoId);
  const dates =
  ...
  if (scopeStatus === "granted") {
    if (realMode && !ids.length) {
      status = "no_real_youtube_video_ids";
      nextActions.push(
        "Select recent real published YouTube video IDs before running real analytics ingestion.",
      );
    } else if (realMode && fixtureIds.length) {
      status = "fixture_video_ids_not_real_mode_ready";
      nextActions.push(
        "Replace fixture IDs with recent real published YouTube video IDs before running real analytics ingestion.",
      );
    } else {
      verdict = realMode ? "READY_REAL_MODE" : "READY_DRY_RUN";
      status = realMode ? "real_mode_ready" : "dry_run_ready";
    }
    if (!realMode && verdict !== "BLOCKED") {
      nextActions.push(
        "Keep this as dry-run until Martin approves real read-only ingestion.",
      );
    } else if (realMode && verdict !== "BLOCKED") {
      nextActions.push(
  ...
  return {
    ...
    fixture_video_id_count: fixtureIds.length,
