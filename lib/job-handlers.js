    ...
    if (
      ...
      editorialInventoryJob = ctx.repos.jobs.enqueue({
        ...
        payload: {
          ...
          generated_at:
            job?.payload?.source_evidence_generated_at ||
            job?.payload?.generated_at,
...
async function handlePublish(job, ctx) {
  ...
  const result = await publishNextStory({
    ...
    runtimeAuthority: ctx.runtimeAuthority,
    claimedJobAuthority: ctx.claimedJobAuthority,
  });
  ctx.assertLeaseHealthy?.();
  if (result?.publish_dispatch_blocked) {
    const reason = result.top_reason || "durable_publish_dispatch_blocked";
...
async function handleGovernedPublicationAdmission(job, ctx) {
  ...
  const blockers = [
    ...
    (!Number.isSafeInteger(attemptCount) || attemptCount < 1)
      ? ["autonomous_jit_attempt_count_invalid"]
      : []),
  ];
  if (blockers.length) {
    return {
  ...
  if (
    runtime.contract.mode === "LIVE_GUARDED" &&
    !autonomousJit
  ) {
    return {
      status: "held",
      blockers: ["publication_admission_authority_required"],
      story_id: String(payload.story_id || "").trim() || null,
      lifecycle_mutation_attempted: false,
      no_external_posting: true,
    };
  ...
  if (autonomousJit) {
    const {
      ...
      RESULT_SCHEMA_VERSION:
        AUTONOMOUS_OFFICIAL_JIT_RESULT_SCHEMA_VERSION,
      RESOLVED_PLAN_SCHEMA_VERSION:
        AUTONOMOUS_OFFICIAL_JIT_RESOLVED_PLAN_SCHEMA_VERSION,
    } = require(
      "./services/autonomous-official-jit-admission-packet"
    );
    const {
      runWithPublicationAdmissionLease:
        defaultRunWithPublicationAdmissionLease,
    } = require("./services/publication-admission-lock");
    const materialiseJitPacket =
      ctx.materialiseAutonomousOfficialJitAdmissionPacket ||
      defaultMaterialiseAutonomousOfficialJitAdmissionPacket;
    const runWithPublicationAdmissionLease =
      ctx.runWithPublicationAdmissionLease ||
      defaultRunWithPublicationAdmissionLease;
    ...
    const leased = await runWithPublicationAdmissionLease({
      db: ctx.repos?.db,
      ...
      claimedJobAuthority: ctx.claimedJobAuthority,
      ownerId:
        `${ctx.workerId || "governed-admission"}:` +
        `autonomous-t75-job-${Number(job.id)}`,
      task: async ({
        assertHealthy,
        ...
        assertHealthy();
        ctx.assertLeaseHealthy?.();
        const resolvedPlan = jitResult?.resolved_plan;
        const resolvedPlanFields =
          resolvedPlan &&
          typeof resolvedPlan === "object" &&
          !Array.isArray(resolvedPlan)
            ? Object.keys(resolvedPlan).sort()
            : [];
        if (
          jitResult?.schema_version !==
            AUTONOMOUS_OFFICIAL_JIT_RESULT_SCHEMA_VERSION ||
          jitResult?.verdict !== "GREEN" ||
          !Number.isSafeInteger(jitResult?.attempt_count) ||
          jitResult.attempt_count <= 0 ||
          jitResult.attempt_count !== attemptCount ||
          resolvedPlanFields.join("\0") !==
            "resolved_plan_sha256\0schema_version" ||
          resolvedPlan.schema_version !==
            AUTONOMOUS_OFFICIAL_JIT_RESOLVED_PLAN_SCHEMA_VERSION ||
          validRunwaySha256(resolvedPlan.resolved_plan_sha256) !==
            resolvedPlan.resolved_plan_sha256 ||
    ...
    if (leased?.publication_admission_blocked) {
      return {
        status: "held",
        blockers: [
          leased.top_reason ||
            "autonomous_t75_publication_admission_lease_unavailable",
        ],
        story_id: String(payload.story_id).trim(),
        lifecycle_mutation_attempted: false,
        no_external_posting: true,
...
function governedEditorialEvidenceRetryOutcome({
  job,
  reportBlockers = [],
  sourceEvidence,
}) {
  const transientBlockers = new Set([
    "source_claim_extraction_failed",
    "source_fetch_failed",
    "breaking_source_evidence_capture_deadline_exceeded",
    "breaking_source_evidence_persist_failed",
  ]);
  const transientConsequences = new Set([
    "official_or_corroborated_body_evidence_required",
    "persisted_breaking_source_evidence_required",
  ]);
  const observedBlockers = [
    ...(Array.isArray(reportBlockers) ? reportBlockers : []),
    ...(Array.isArray(sourceEvidence?.blockers)
      ? sourceEvidence.blockers
      : []),
    ...(Array.isArray(sourceEvidence?.sources)
      ? sourceEvidence.sources.flatMap((source) =>
    ...
    .filter(Boolean);
  const transientFailureObserved = observedBlockers.some(
    (blocker) => transientBlockers.has(blocker),
  );
  const onlyAllowlistedTransientBlockers =
    observedBlockers.length > 0 &&
    observedBlockers.every(
      (blocker) =>
        transientBlockers.has(blocker) ||
        transientConsequences.has(blocker),
    );
  const attemptCount = Number(job?.attempt_count);
  ...
  const retryable =
    ...
    onlyAllowlistedTransientBlockers &&
    attemptsKnown &&
    attemptCount < maxAttempts;
  return {
    status: "held",
    job_outcome: retryable ? "RETRY" : "TERMINAL",
    retryable,
    retry_classification: retryable
      ? "TRANSIENT_SOURCE_EVIDENCE_FAILURE"
      : "NON_RETRYABLE_SOURCE_EVIDENCE_HOLD",
    attempt_count: attemptsKnown ? attemptCount : null,
    max_attempts: attemptsKnown ? maxAttempts : null,
    ...(retryable
      ? {
...
async function handleBreakingStoryDiscovery(job, ctx) {
  const payload = job?.payload || {};
  const attemptObservedValue =
    typeof ctx.now === "function"
      ? ctx.now()
      : ctx.now === undefined
        ? new Date()
        : ctx.now;
  const attemptObservedDate =
    attemptObservedValue instanceof Date
      ? new Date(attemptObservedValue.getTime())
      : new Date(attemptObservedValue);
  if (Number.isNaN(attemptObservedDate.getTime())) {
    const error = new Error(
      "breaking_source_evidence_trusted_time_invalid",
    );
    error.code =
      "BREAKING_SOURCE_EVIDENCE_TRUSTED_TIME_INVALID";
    throw error;
  }
  const attemptObservedAt =
    attemptObservedDate.toISOString();
  const {
  ...
  if (editorialEvidenceOnly) {
    ...
    envelope = buildBreakingEventEnvelope({
      story: {
        ...(payload.story || {}),
        timestamp:
          payload.story?.timestamp ||
          payload.story?.discovered_at,
      },
      now: attemptObservedAt,
    });
  }
  ...
  if (evidenceValidation.valid) {
    const persistBreakingSourceEvidencePacket =
      ctx.persistBreakingSourceEvidencePacket ||
      breakingEvidenceService.persistBreakingSourceEvidencePacket;
    try {
      persistedEvidence =
  ...
  const report = {
    ...envelope,
    attempt_observed_at: attemptObservedAt,
    verdict:
      blockers.length > 0
        ? "HOLD"
        : editorialEvidenceOnly
  ...
  return {
    ...
    ...(editorialEvidenceOnly &&
    report.verdict === "HOLD"
      ? governedEditorialEvidenceRetryOutcome(
          {
            job,
...
function laneProductionBlockers(
  ...
  laneId,
  {
    ...
    governedAutonomousProduction = false,
  } = {},
) {
  const blockers = [];
  const extra = parseStoryExtra(story);
  ...
  if (laneId === "breaking_short") {
    if (governedAutonomousProduction !== true) {
...
async function handleExactShortProduction(job, ctx) {
  ...
  const blockers = laneProductionBlockers(governedStory, laneId, {
    localProofReviewDraft:
      payload.publish_authority === false &&
      payload.human_review_required === true,
    governedAutonomousProduction: autonomousRequested,
  });
  if (laneId === "breaking_short") {
    blockers.push(
...
const AUTONOMOUS_FAST_NEWS_MAXIMUM_AGE_MS =
  72 * 60 * 60 * 1000;
const AUTONOMOUS_NON_FAST_NEWS_FORMATS = new Set([
  "evergreen_short",
  "evergreen_verdict_short",
  "longform",
  "monthly_release_radar",
...
function governedAutonomousFastNewsLaneBlockers({
  story,
  inventoryStory,
  scheduledTimestamp,
}) {
  ...
  const explicitFormats = [
    ...
    .filter(Boolean);
  if (
    explicitFormats.some(
      (format) =>
        AUTONOMOUS_NON_FAST_NEWS_FORMATS.has(format) ||
        format.includes("longform") ||
        format.includes("evergreen"),
    )
  ) {
    return [
  ...
  if (
    ...
    publishedTimestamp >
      scheduledTimestamp + 5 * 60 * 1000 ||
    scheduledTimestamp - publishedTimestamp >
      AUTONOMOUS_FAST_NEWS_MAXIMUM_AGE_MS
  ) {
    return [
      "governed_autonomous_window_recent_official_news_required",
    ];
  }
  return [];
}
...
const AUTONOMOUS_WINDOW_PLAN_RETRY_SECONDS = 5 * 60;
const AUTONOMOUS_WINDOW_PLAN_RETRY_GUARD_MS =
  60 * 1000;
const AUTONOMOUS_WINDOW_PRE_T94_DEPENDENCY_MS =
  94 * 60 * 1000;
const AUTONOMOUS_WINDOW_MINIMUM_PRODUCTION_RUNWAY_SECONDS =
  15 * 60;
const AUTONOMOUS_WINDOW_MINIMUM_PRODUCTION_RUNWAY_MS =
  AUTONOMOUS_WINDOW_MINIMUM_PRODUCTION_RUNWAY_SECONDS *
  1000;

function governedAutonomousWindowPlanningRetry({
  job,
  attemptTimestamp,
  scheduledTimestamp,
}) {
  ...
  const attemptsKnown =
    ...
    Number.isInteger(maxAttempts) &&
    maxAttempts >= 1;
  const retryDeadlineTimestamp =
    scheduledTimestamp -
    AUTONOMOUS_WINDOW_PRE_T94_DEPENDENCY_MS -
    AUTONOMOUS_WINDOW_MINIMUM_PRODUCTION_RUNWAY_MS;
  const preT94DependencyTimestamp =
    scheduledTimestamp -
    AUTONOMOUS_WINDOW_PRE_T94_DEPENDENCY_MS;
  const latestRetryTimestamp =
    retryDeadlineTimestamp -
    AUTONOMOUS_WINDOW_PLAN_RETRY_GUARD_MS;
  const secondsUntilLatestRetry = Math.floor(
    (latestRetryTimestamp - attemptTimestamp) / 1000,
  );
  const retryable =
    attemptsKnown &&
    attemptCount < maxAttempts &&
    secondsUntilLatestRetry >= 1;
  return {
    job_outcome: retryable ? "RETRY" : "TERMINAL",
    retryable,
    ...
    pre_t94_dependency_at: new Date(
      preT94DependencyTimestamp,
    ).toISOString(),
    minimum_production_runway_seconds:
      AUTONOMOUS_WINDOW_MINIMUM_PRODUCTION_RUNWAY_SECONDS,
    attempt_count: attemptsKnown ? attemptCount : null,
    max_attempts: attemptsKnown ? maxAttempts : null,
    catch_up_allowed: false,
    ...(retryable
      ? {
          retry_after_seconds: Math.min(
            AUTONOMOUS_WINDOW_PLAN_RETRY_SECONDS,
            secondsUntilLatestRetry,
          ),
        }
      : {}),
  };
...
async function handleGovernedAutonomousWindowProductionPlan(
  ...
  const trustedAttemptValue =
    typeof ctx.now === "function"
      ? ctx.now()
      : ctx.now === undefined
        ? new Date()
        : ctx.now;
  const trustedAttemptDate =
    trustedAttemptValue instanceof Date
      ? new Date(trustedAttemptValue.getTime())
      : new Date(trustedAttemptValue);
  const attemptTimestamp = trustedAttemptDate.getTime();
  const generatedAt = Number.isFinite(attemptTimestamp)
    ? trustedAttemptDate.toISOString()
    : "";
  const generatedTimestamp = attemptTimestamp;
  ...
  if (
    ...
    inputBlockers.push(
      "governed_autonomous_window_planning_trusted_time_invalid",
    );
  } else if (
    Number.isFinite(scheduledTimestamp) &&
    generatedTimestamp >=
  ...
  if (inputBlockers.length) {
    const productionCutoffReached =
      inputBlockers.includes(
        "governed_autonomous_window_planning_must_precede_production_cutoff",
      ) &&
      Number.isFinite(generatedTimestamp) &&
      Number.isFinite(scheduledTimestamp);
    return governedAutonomousWindowPlanningHold({
      ...
      counts,
      rejected,
      retry: productionCutoffReached
        ? governedAutonomousWindowPlanningRetry({
            job,
            attemptTimestamp: generatedTimestamp,
            scheduledTimestamp,
          })
        : {},
    });
  ...
  const inventoryEntryById = new Map(
    exactReadyEntries.map((entry) => [
      String(entry.story.id),
      entry,
    ]),
  );
  ...
  for (const entry of exactReadyEntries) {
    ...
    const laneBlockers =
      governedAutonomousFastNewsLaneBlockers({
        story,
        inventoryStory: entry.story,
        scheduledTimestamp,
      });
    if (laneBlockers.length) {
      counts.lane_eligibility_rejected += 1;
      rejected.push({
        story_id: storyId,
  ...
  const compileCandidate =
    ...
    defaultCompileCandidate;
  const timingStateRoot = String(
    ctx.governedAutonomousNarrationTimingStateRoot ||
      ctx.env?.PULSE_STATE_ROOT ||
      process.env.PULSE_STATE_ROOT ||
      "",
  ).trim();
  const discoverNarrationTiming =
    ctx.discoverGovernedAutonomousNarrationTimingEvidence ||
    require("./services/governed-autonomous-narration-timing-evidence")
    ...
    try {
      ...
      const canonicalIdentitySource =
        story.canonical_identity_url ||
          ...
          story.url ||
          story.article_url ||
          inventoryEntryById.get(storyId)?.story
            ?.primary_source_url ||
          "";
      const canonicalIdentityUrl = canonicalUrl(
        canonicalIdentitySource,
      );
      const canonicalStoryId = canonicalIdentityUrl
        ? `official_${canonicalHash(canonicalIdentitySource)}`
        : "";
      const narrationTimingEvidence = timingStateRoot
        && canonicalStoryId
        ? await discoverNarrationTiming({
            stateRoot: path.resolve(timingStateRoot),
            legacyStoryId: storyId,
            storyId: canonicalStoryId,
            scriptSha256: crypto
              .createHash("sha256")
              .update(normalisedScript)
              .digest("hex"),
      ...
      compiled = await compileCandidate({
        ...
        allowed_roots: narrationTimingEvidence
          ? [
              ...allowedRoots,
              path.resolve(timingStateRoot),
            ]
          : allowedRoots,
        candidate,
        story,
  ...
  if (eligible.length < 2) {
    return governedAutonomousWindowPlanningHold({
      ...
      retry: governedAutonomousWindowPlanningRetry({
        job,
        attemptTimestamp: generatedTimestamp,
        scheduledTimestamp,
      }),
  ...
  const planWindowProduction =
    ...
    defaultPlanWindowProduction;
  let planResult;
  let prePlannerAttemptTimestamp = null;
  let durableBoundaryAttemptTimestamp = null;
  try {
    ctx.assertLeaseHealthy?.();
    const prePlannerAttemptValue =
      typeof ctx.now === "function"
        ? ctx.now()
        : ctx.now === undefined
          ? new Date()
          : ctx.now;
    const prePlannerAttemptDate =
      prePlannerAttemptValue instanceof Date
        ? new Date(prePlannerAttemptValue.getTime())
        : new Date(prePlannerAttemptValue);
    prePlannerAttemptTimestamp =
      prePlannerAttemptDate.getTime();
    if (!Number.isFinite(prePlannerAttemptTimestamp)) {
      return governedAutonomousWindowPlanningHold({
        blockers: [
          "governed_autonomous_window_planning_trusted_time_invalid",
        ],
    ...
    if (
      ...
      return governedAutonomousWindowPlanningHold({
        ...
        retry: governedAutonomousWindowPlanningRetry({
          job,
          attemptTimestamp: prePlannerAttemptTimestamp,
          scheduledTimestamp,
        }),
      });
    }
    planResult = await planWindowProduction(
      ...
      },
      {
        ...
        beforeDurableEnqueue() {
          ctx.assertLeaseHealthy?.();
          const durableBoundaryAttemptValue =
            typeof ctx.now === "function"
              ? ctx.now()
              : ctx.now === undefined
                ? new Date()
                : ctx.now;
          const durableBoundaryAttemptDate =
            durableBoundaryAttemptValue instanceof Date
              ? new Date(
                  durableBoundaryAttemptValue.getTime(),
                )
              : new Date(durableBoundaryAttemptValue);
          durableBoundaryAttemptTimestamp =
            durableBoundaryAttemptDate.getTime();
          if (
            !Number.isFinite(
              durableBoundaryAttemptTimestamp,
            )
    ...
    return governedAutonomousWindowPlanningHold({
      ...
      retry:
        blocker ===
          "governed_autonomous_window_planning_must_precede_production_cutoff" &&
        Number.isFinite(durableBoundaryAttemptTimestamp)
          ? governedAutonomousWindowPlanningRetry({
              job,
              attemptTimestamp:
                durableBoundaryAttemptTimestamp,
          ...
          : blocker ===
              ...
              Number.isFinite(prePlannerAttemptTimestamp)
            ? governedAutonomousWindowPlanningRetry({
                job,
                attemptTimestamp:
                  prePlannerAttemptTimestamp,
...
async function attemptConfirmedDisarmReservePromotion({
  ...
  try {
    ...
    const runWithPublicationAdmissionLease =
      ...
      require("./services/publication-admission-lock")
        .runWithPublicationAdmissionLease;
    const reserveAdmitPublication =
      ctx.admitPublication ||
      require("./services/publication-admission")
        .admitPublication;
    const result = await runWithPublicationAdmissionLease({
      db: ctx.repos?.db,
      leases: ctx.repos?.runtimeLeases,
      operation:
      ...
      task: async ({
        ...
        const promoted = await promote({
          ...
          admitPublication: (options) =>
            reserveAdmitPublication({
              ...options,
              publicationAdmissionLease,
            }),
        });
  ...
  if (
    ...
    try {
      ...
      const reserveAdmitPublication =
        ...
        require("./services/publication-admission")
          .admitPublication;
      reservePromotion = await runWithPublicationAdmissionLease({
        db: ctx.repos?.db,
        leases: ctx.repos?.runtimeLeases,
        operation:
      ...
      if (reservePromotion.promoted !== true) {
        const promotionBlocker =
          ...
          (reservePromotion.publication_admission_blocked === true
            ? "publication_admission_lease_unavailable"
            : null) ||
