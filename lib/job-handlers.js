...
async function handlePublishWindowWatchdog(job, ctx) {
  ...
  if (
    clean(payload.phase).toUpperCase() === "T-90" &&
    ...
    try {
      ...
      ctx.repos.jobs.enqueue({
        ...
        payload: {
          ...
          fresh_review_script_repair_limit: Number(payload.fresh_review_script_repair_limit || 6),
        ...
        priority: 70,
        idempotency_key: `publish_window_candidate_refill:${date}:${label}`,
      });
      candidateRefillEnqueued = true;
    } catch (err) {
