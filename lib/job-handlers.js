...
async function readGuardedLiveExecutorPlanForScheduler(executorPlanPath, generatedAt = new Date().toISOString()) {
  ...
  if (executorHasActions) {
    if (guardedDispatchPlanReady) {
      ...
      if (dispatchGeneratedAt !== null && (executorGeneratedAt === null || dispatchGeneratedAt > executorGeneratedAt)) {
...
async function handleCandidateSupplyMonitor(job, ctx) {
  ...
  if (payload.post_discord_on_red === true && report.verdict === "red") {
    try {
      const sendDiscord = require("../notify");
      await sendDiscord([
        ...
        `V4-ready: ${report.summary?.v4_ready_candidates || 0}/${report.targets?.v4_ready_candidates || 0}`,
        `Blockers: ${(report.blockers || []).slice(0, 4).join("; ") || "none"}`,
        "Safety: read-only monitor; no publish, no token changes, no DB mutation.",
      ].join("\n"));
    } catch (err) {
      const log = (ctx && ctx.log) || console.log;
