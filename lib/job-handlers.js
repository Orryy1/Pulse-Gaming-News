    ...
    try {
      ...
      const sourceRefillCommandWithMode =
        sourceRefillCommandBase && !/\s--repair-evidence-mode(?:=|\s+)/.test(sourceRefillCommandBase)
          ? `${sourceRefillCommandBase} --repair-evidence-mode plan`
...
function freshRefillRepairQuarantineReasons({ row = {}, canonical = {}, sourceManifest = {}, scriptScorecard = {} } = {}) {
  ...
  if (
    ...
    blockers.includes("script:rewrite_required") ||
    blockers.some((blocker) => /\bscript_verdict_rewrite_required\b/i.test(blocker)) ||
    blockers.some((blocker) => /\bscript_score_below_threshold\b/i.test(blocker))
  ) {
...
async function freshRefillRepairAttemptScope({
  ...
  const allRepairStoryPackageRows = packageFilter.eligibleRows || [];
  ...
  const repairDeferredByLimitRows = repairStoryLimitValue
    ? allRepairStoryPackageRows.slice(repairStoryLimitValue)
    : [];
  ...
  if (repairStoryLimitValue && repairDeferredByLimitRows.length) {
