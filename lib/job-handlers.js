...
function officialFreshRefillSourceType(sourceName, sourceUrl) {
  ...
  if (
    /^(?:ea|electronic arts|ubisoft|capcom|sega|bandai namco|square enix|bethesda|blizzard|rockstar(?:\s+newswire)?|2k)$/.test(name) &&
  ...
  for (const row of rows) {
    ...
    for (const [index, candidate] of candidateEntries.entries()) {
      ...
      entries.push({
        ...
        source_family: freshRefillSafeSlug(
          `${sourceName}_${entity}_${storyId}_${candidate.source_family || index + 1}`,
        ),
