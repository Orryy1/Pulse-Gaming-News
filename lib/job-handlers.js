...
function runProduceChildProcess({
  spawn = defaultSpawn,
  cwd = path.resolve(__dirname, ".."),
  env = process.env,
  log = null,
  timeoutMs = Number(env.PULSE_PRODUCE_CHILD_TIMEOUT_MS || 45 * 60 * 1000),
} = {}) {
  return new Promise((resolve, reject) => {
    let settled = false;
    let stdoutTail = "";
    let stderrTail = "";
    const child = spawn(process.execPath, ["run.js", "produce"], {
      cwd,
      env: {
        ...env,
        PULSE_JOB_CHILD_KIND: "produce",
      },
      windowsHide: true,
    ...
    const timer = setTimeout(() => {
      ...
      finish(() =>
        reject(
          new Error(`produce child process timed out after ${timeoutMs}ms`),
        ),
      );
    }, timeoutMs);
    ...
    if (child.stdout) {
      ...
      child.stdout.on("data", (chunk) => {
        ...
        streamLines("[produce-child]", text, log);
      });
    }
    if (child.stderr) {
      ...
      child.stderr.on("data", (chunk) => {
        ...
        streamLines("[produce-child:err]", text, log);
      });
    }
    ...
    child.on("close", (code, signal) => {
      finish(() => {
        ...
        reject(new Error(`produce child process failed with ${suffix}: ${detail}`));
      });
    });
  });
}
...
function renderGuardedLiveDispatchSummary(report = {}, { jobId, actionId } = {}) {
  const action = Array.isArray(report.actions) && report.actions.length
    ? report.actions[0]
    : Array.isArray(report.blocked_actions) && report.blocked_actions.length
      ? report.blocked_actions[0]
      : null;
  const lines = [
    `**Pulse Gaming Guarded Publish**${jobId ? ` (job #${jobId})` : ""}`,
    `Status:    ${String(report.verdict || "unknown").toLowerCase()}`,
    `Action:    ${actionId || action?.action_id || "none"}`,
  ];
  if (action) {
