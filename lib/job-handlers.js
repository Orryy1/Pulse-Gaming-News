...
async function buildFreshProductionRefillRepairEvidence({
  ...
  const storyPackageRows = await readFreshRefillEvidenceRows(storyPackagesPath);
  const effectiveStoryPackageCount = Math.max(storyPackageRows.length, Number(storyCount || 0) || 0);
  ...
  const report = {
    ...
    summary: {
      story_package_count: storyPackageRows.length,
