...
async function freshRefillRepairAttemptScope({
  ...
  const directRunwayRows = priorityRows.filter((item) => item.has_direct_motion_runway === true);
  const discoveryRows =
    motionRunwayRequired && allowOfficialSourceDiscoveryWithoutRunway && directRunwayRows.length === 0
      ? priorityRows.filter((item) => item.source_type)
      : [];
  ...
  const attemptPriorityRows = motionRunwayRequired
    ? (directRunwayRows.length ? directRunwayRows : discoveryRows)
    : priorityRows;
  const motionRunwayDeferredPriorityRows = motionRunwayRequired
    ? priorityRows.filter((item) => {
  ...
  if (repairDir) {
    ...
    await fs.writeJson(repairPriorityReportPath, {
      ...
      ranked: priorityRows.map((item, rank) => ({
        ...
        defer_reason: motionRunwayRequired && item.has_direct_motion_runway !== true
...
async function freshRefillHyperframesStoryIdsAfterMotion({
  ...
  if (!reportPath || !(await fs.pathExists(reportPath))) {
    return motionReadyIds.length ? uniqueClean([...motionReadyIds, ...sourceCardFallbackIds]) : fallbackIds;
  }
  ...
  return uniqueClean([...materializedIds, ...motionReadyIds, ...sourceCardFallbackIds]);
}
  ...
  if (!storyPackageRows.length) {
    ...
    const report = {
      ...
      outputs: {
        ...
        official_source_intake_report: sourceEvidence.officialSourceIntakeJsonPath,
        official_source_intake_markdown: sourceEvidence.officialSourceIntakeMdPath,
        next_action: "source_motion_first_intake_required",
      },
      child_processes: [],
      safety: {
        local_only: true,
