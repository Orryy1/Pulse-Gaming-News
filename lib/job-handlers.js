...
async function handlePublishScheduleRecoveryMonitor(job, ctx) {
  ...
  const publish = await recoverPublish({
    now: new Date(),
    schedules: require("./scheduler").DEFAULT_SCHEDULES,
    repos,
    env: process.env,
    persist: true,
...
function freshRefillRepairQuarantineReasons({
  ...
  if (mediaHouseBlockers.some((blocker) => /\bfeed_title_template_fatigue\b/i.test(blocker))) {
...
function relevantFreshRefillSourceEvidenceClaims(sourceEvidenceClaims, canonical) {
  ...
  return sourceEvidenceClaims
    .map((claim, index) => {
      const text = clean(claim);
      if (
        !text ||
        /\b(?:did you like this|like this|latest news|learn more|share of the week)\b/i.test(text) ||
        text.split(/\s+/).filter(Boolean).length > 45
      ) {
        return null;
      }
    ...
    .filter((claim) => claim && claim.overlap_count >= 2)
    .sort(
      (left, right) =>
...
async function buildFreshProductionRefillRepairEvidence({
  ...
  if (shouldRunOfficialDiscovery) {
    ...
    const segmentValidationReportPresent =
      segmentValidationOk && await fs.pathExists(segmentValidationJsonPath);
    if (!segmentValidationReportPresent) {
      ...
      segmentValidationStatus = "validated";
      await runFreshRefillRepairChild({
        runChild,
        log,
        childProcesses,
...
async function handleFreshProductionRefill(job, ctx) {
  ...
  const zeroYieldQuarantinePath = path.resolve(
    ROOT,
    clean(
      payload.zero_yield_quarantine_path ||
        payload.zeroYieldQuarantinePath,
    ) || path.join("output", "runtime", "refill-zero-yield-quarantine.json"),
  );
  ...
  const zeroYieldDetected = strictOutcome
    ? strictOutcome.target_met !== true
    : greenCount === 0;
  ...
  const zeroYieldRecovery = {
    ...
    outcome: strictOutcome?.outcome || (greenCount === 0 ? "zero_local_package_yield" : "target_met"),
