...
function renderFreshRefillHyperframesCardMarkdown(report = {}) {
  ...
  for (const item of report.stories || []) {
    lines.push(`- ${item.story_id}: ${item.status}`);
  }
...
async function buildFreshRefillHyperframesCardEvidence({
  ...
  const report = {
    schema_version: 1,
    generated_at: new Date().toISOString(),
    status: !storyIds.length ? "skipped" : failedCount ? "partial" : "generated",
    summary: {
...
async function buildFreshProductionRefillRepairEvidence({
  ...
  if (sourceEvidence.official_source_entries_count > 0) {
    ...
    if (Array.isArray(officialSearchAutofillTemplate?.entries) && officialSearchAutofillTemplate.entries.length > 0) {
