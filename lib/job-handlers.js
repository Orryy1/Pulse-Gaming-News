 ...
 * an overall status ("ok" / "degraded" / "failed") driven by CORE
 * platform outcomes only. Optional platforms (Twitter/X) cannot make the
 * publish look broken — they get their own line.
 *
...
function renderPublishSummary(result, { jobId } = {}) {
  ...
  const icon = (platformKey) => {
    if (result[platformKey]) return "✅";
    if (skipped[platformKey]) return "⏸";
    return "❌";
  };
  const label = {
    youtube: "YT",
  ...
  // live under Optional and shouldn't pollute the main summary.
  ...
  const lines = [
    ...
    `Optional: ${optionalPlatformsLine}`,
    `Status:   ${status}`,
  ];
