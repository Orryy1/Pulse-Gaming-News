...
const MIN_HYPERFRAMES_READABLE_CARD_DURATION_S = 8.5;
const MAX_HYPERFRAMES_READABLE_CARD_DURATION_S = 12;
...
function internalHyperframesReadableHoldFloorS({ readableText = "", wordCount = null } = {}) {
  ...
  const computed = Math.max(
    ...
    1.15 * Math.max(1, words) + 1.5 + longTokenPenalty,
  );
  return Number(
    Math.min(MAX_HYPERFRAMES_READABLE_CARD_DURATION_S, Math.ceil(computed * 10) / 10).toFixed(1),
  );
}
...
function ownerFromOfficialDirectMedia(candidate = {}) {
  ...
  if (/(?:^|\.)steampowered\.com$/i.test(host)) return "Steam";
  return "";
}
...
function freshRefillRepairQuarantineReasons({ row = {}, canonical = {}, sourceManifest = {}, scriptScorecard = {} } = {}) {
  ...
  const blockers = [
    ...
    ...(Array.isArray(sourceManifest.blockers) ? sourceManifest.blockers : []),
  ...
  if (/\bgeneric_(?:title|could_split_title|player_test)_template\b/i.test(combined)) {
    reasons.push("script_generic_template_blocker");
  }
  if (clean(scriptScorecard.verdict).toLowerCase() === "rewrite_required" || blockers.includes("script:rewrite_required")) {
  ...
  if (/\bmedia_house:script_sounds_ai_generic\b/i.test(combined)) {
    reasons.push("media_house_script_generic");
  }
  if (/\b(?:public_output|platform):generic_title\b/i.test(combined)) {
...
async function buildFreshRefillRepairPackageFilter({ storyPackagesPath, outputDir }) {
  const root = path.resolve(__dirname, "..");
  ...
  for (const row of rows) {
    ...
    for (const [index, candidate] of candidateEntries.entries()) {
      ...
      rowEntries.push({
        ...
        direct_media_url_if_available: candidate.direct_media_url || "",
        ...
        evidence_of_officialness: candidate.direct_media_url
        ...
        direct_media_provided: Boolean(candidate.direct_media_url),
...
function freshRefillSegmentValidationBudget(storyPackageCount) {
  const count = Math.max(1, Number(storyPackageCount || 0) || 1);
  return {
    maxSegments: Math.min(240, Math.max(48, count * 8)),
    candidateWindowsPerSource: 8,
...
async function buildFreshProductionRefillRepairEvidence({
  ...
  const storyPackageRows = packageFilter.eligibleRows;
  const repairStoryPackagesPath = packageFilter.eligibleStoryPackagesPath;
  const declaredEligibleStoryCount = Math.max(
    0,
    (Number(storyCount || 0) || 0) - packageFilter.quarantinedRows.length,
  );
  const effectiveStoryPackageCount = Math.max(storyPackageRows.length, declaredEligibleStoryCount);
  ...
  if (sourceEvidence.official_source_entries_count > 0) {
    ...
    if (freshRefillSearchTemplateRows(officialSearchAutofillTemplate).length > 0) {
      directMediaDiscoveryInputPath = officialSearchAutofillTemplatePath;
    }
    ...
    const hyperframesStoryIds = uniqueClean(
  ...
  const report = {
    ...
    summary: {
      ...
      repair_eligible_story_package_count: storyPackageRows.length,
    ...
    outputs: {
      ...
      official_search_autofill_report:
        sourceEvidence.official_source_entries_count > 0 ? officialSearchAutofillJsonPath : null,
      official_search_autofill_markdown:
        sourceEvidence.official_source_entries_count > 0 ? officialSearchAutofillMdPath : null,
      official_search_template:
        sourceEvidence.official_source_entries_count > 0 ? supplementalOfficialSearchEvidence.template_path : null,
      official_search_autofill_template:
        sourceEvidence.official_source_entries_count > 0 ? officialSearchAutofillTemplatePath : null,
...
async function handleFreshProductionRefill(job, ctx) {
  ...
  if (
    ...
    const candidateStoryIds = uniqueClean(candidateStories.map((story) => freshRefillStoryId(story)));
...
async function handleLocalTtsRetryRecovery(job, ctx) {
  ...
  await run({
    ...
    args: ["tools/local-tts-doctor.js", "--json", "--restart", "--prewarm"],
    timeoutEnvName: "PULSE_LOCAL_TTS_DOCTOR_CHILD_TIMEOUT_MS",
  });
  await run({
