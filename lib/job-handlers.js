...
async function buildFreshProductionRefillRepairEvidence({
  ...
  if (sourceEvidence.official_source_entries_count > 0) {
    ...
    await runFreshRefillRepairChild({
      ...
      args: [
        ...
        sourceEvidence.officialSourceIntakeJsonPath,
        "--output-dir",
        repairDir,
        "--output-basename",
  ...
  const report = {
    ...
    outputs: {
      ...
      trailer_reference_report: sourceEvidence.official_source_entries_count > 0
        ? path.join(repairDir, "official_trailer_references_fresh_refill.json")
        : null,
    },
...
async function handleFreshProductionRefill(job, ctx) {
  ...
  const result = await runGoalBatchPackages(args);
