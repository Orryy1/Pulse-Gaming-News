...
async function readGuardedLiveExecutorPlanForScheduler(executorPlanPath, generatedAt = new Date().toISOString()) {
  ...
  if (await fs.pathExists(executorPlanPath)) {
    ...
    if (Array.isArray(executorPlan.handoff_ready_actions) && executorPlan.handoff_ready_actions.length) {
      return executorPlan;
    }
  }
  ...
  if (
    guardedDispatchPlan.mode !== "GUARDED_DISPATCH_PREFLIGHT" ||
    guardedDispatchPlan.ready_for_guarded_dispatch !== true ||
    !guardedDispatchPlanSafetyOk(guardedDispatchPlan)
  ) {
