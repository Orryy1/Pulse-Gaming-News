...
function platformPackPresence(platformPublishManifest = {}, standalonePacks = {}) {
  const presence = {};
  for (const platform of CORE_PUBLISH_PLATFORMS) {
    presence[platform] = {
      manifest: objectHasKeys(platformPublishManifest.outputs?.[platform]),
      standalone: objectHasKeys(standalonePacks[platform]),
    };
    presence[platform].present =
      presence[platform].manifest || presence[platform].standalone;
  }
  return presence;
}
...
function mediaHouseScoreNeedsRepair(mediaHouseScore = {}) {
  return (
    !verdictGreen(mediaHouseScore.verdict) ||
    cleanText(mediaHouseScore.status).toLowerCase() !== "pass"
  );
}
...
async function fingerprintFile(filePath, artifactDir = "") {
  const resolved = filePathForLocalCheck(filePath, artifactDir);
  if (!resolved) return { path: null, sha256: null, size_bytes: 0 };
  try {
    ...
    return { path: resolved, sha256: null, size_bytes: 0 };
  }
}
...
function authorityRefreshFor(document = {}) {
  return document && typeof document === "object" && !Array.isArray(document)
    ? document.authority_refresh || null
    : null;
}

function authorityDocumentGreen(document = {}) {
  return (
    verdictGreen(document.verdict || document.publish_status) &&
    document.can_auto_publish === true &&
    asArray(document.blockers).length === 0 &&
    asArray(document.hard_blockers).length === 0 &&
    asArray(document.failures).length === 0 &&
    asArray(document.errors).length === 0 &&
    asArray(document.warnings).length === 0 &&
    asArray(document.advisories).length === 0
  );
}

async function validateMonotonicAuthority({
  artifactDir,
  storyId,
  generatedAt,
  goalPackageSummary = null,
  ...
  const blockers = [];
  const documents = {
    goal_package_summary: goalPackageSummary,
    platform_publish_manifest: platformPublishManifest,
    publish_verdict: publishVerdict,
  };
  const currentPlatformScope = resolvePlatformPublishScope(platformPublishManifest);
  let platformScopeMismatch = false;
  const refreshes = {};
  for (const [label, document] of Object.entries(documents)) {
    ...
    if (!authorityDocumentGreen(document)) blockers.push(`${label}_authority_not_green`);
    if (cleanText(document.story_id) && cleanText(document.story_id) !== cleanText(storyId)) {
      blockers.push(`${label}_story_id_mismatch`);
    }
    const refresh = authorityRefreshFor(document);
    if (!refresh) {
      blockers.push(`${label}_authority_refresh_missing`);
      continue;
    }
    refreshes[label] = refresh;
    if (refresh.monotonic_verdict !== true) blockers.push(`${label}_authority_not_monotonic`);
    if (cleanText(refresh.source) !== "current_independently_verified_artifact_evidence") {
      blockers.push(`${label}_authority_source_invalid`);
    }
    if (currentPlatformScope.explicit) {
      const frozenScope = refresh.platform_scope;
      if (!frozenScope || typeof frozenScope !== "object" || Array.isArray(frozenScope)) {
        blockers.push(`${label}_authority_platform_scope_missing`);
        platformScopeMismatch = true;
      } else if (
        normaliseSha256(frozenScope.sha256) !== currentPlatformScope.sha256
      ) {
        platformScopeMismatch = true;
      }
    }
    const refreshedAt = Date.parse(cleanText(refresh.refreshed_at));
    if (!Number.isFinite(refreshedAt)) {
      blockers.push(`${label}_authority_refreshed_at_missing`);
    }
  }

  const refreshValues = Object.values(refreshes);
  if (refreshValues.length === Object.keys(documents).length) {
    const canonical = JSON.stringify(refreshValues[0]);
    if (refreshValues.some((refresh) => JSON.stringify(refresh) !== canonical)) {
      blockers.push("authority_refresh_documents_disagree");
    }
  }
  if (platformScopeMismatch) blockers.push("authority_platform_scope_mismatch");

  const actual = {
    render: await fingerprintFile(renderPath, artifactDir),
    audio: await fingerprintFile(audioPath, artifactDir),
    timestamps: await fingerprintFile(timestampsPath, artifactDir),
    rights: await fingerprintFile(path.join(artifactDir, "rights_ledger.json"), artifactDir),
  };
  const frozen = refreshValues[0]?.frozen_hashes || {};
  for (const key of ["render", "audio", "timestamps", "rights"]) {
    const declared = frozen[key] || {};
    if (!normaliseSha256(declared.sha256)) {
      blockers.push(`authority_frozen_hash_missing:${key}`);
      continue;
    }
    if (
      normaliseSha256(declared.sha256) !== actual[key].sha256 ||
      Number(declared.size_bytes) !== actual[key].size_bytes
    ) {
      blockers.push(`authority_frozen_evidence_mismatch:${key}`);
    }
  }
  ...
  return {
    ...
    evidence: {
      authority_refreshed_at: refreshValues[0]?.refreshed_at || null,
      platform_scope: currentPlatformScope,
...
function ledgerRows(rightsLedgerRaw) {
  return asArray(rightsLedgerRaw);
}
...
function mergeStandalonePlatformPacks(platformPublishManifest = {}, standalonePacks = {}, canonical = {}) {
  ...
  const scope = resolvePlatformPublishScope(platformPublishManifest);
  let outputsEnriched = false;
  for (const platform of scope.enabled_platforms) {
    if (!CORE_PUBLISH_PLATFORMS.includes(platform)) continue;
    ...
    if (before !== after) outputsEnriched = true;
  }
  const originalStatus = cleanText(platformPublishManifest.publish_status);
  return {
    ...platformPublishManifest,
    outputs,
    local_bridge_outputs_enriched_from_standalone_packs: outputsEnriched,
...
function motionClipPathKeys(clip = {}) {
  return [
    clip.path,
    clip.local_materialized_path,
    clip.local_materialised_path,
    clip.media_path,
  ].map(normaliseAssetKey).filter(Boolean);
}

function enrichFinalRenderScenes(sceneRows = [], availableRows = []) {
  const availableByPath = new Map();
  for (const row of motionClipsFromManifest({ clips: availableRows })) {
    for (const key of motionClipPathKeys(row)) {
      if (!availableByPath.has(key)) availableByPath.set(key, row);
    }
  }
  return asArray(sceneRows).map((scene) => {
    const source = motionClipPathKeys(scene)
      .map((key) => availableByPath.get(key))
      .find(Boolean);
    return {
      ...(source || {}),
      ...scene,
      id: cleanText(scene.id || source?.id || scene.asset_id || source?.asset_id),
      source_family: cleanText(
        scene.source_family ||
          scene.motion_family ||
          scene.baseSourceKey ||
          source?.source_family ||
          source?.motion_family,
      ),
      motion_family: cleanText(
        scene.motion_family ||
          scene.source_family ||
          scene.baseSourceKey ||
          source?.motion_family ||
...
function selectedRenderMotionClips({ renderStory = {}, renderManifest = {} } = {}) {
  ...
  const renderManifestRows = [
    ...asArray(renderManifest.visual_v4_bridge_video_clips),
    ...asArray(renderManifest.video_clips),
  ];
  const finalSceneRows = asArray(renderManifest.clip_scene_plan?.scenes);
  const rows = finalSceneRows.length
    ? enrichFinalRenderScenes(finalSceneRows, [...renderStoryRows, ...renderManifestRows])
    : (renderStoryRows.length ? renderStoryRows : renderManifestRows);
...
function staleDirectorMotionMinimumCanBeIgnored(directorBeatMap = {}) {
  const directorBlockers = asArray(directorBeatMap.readiness?.blockers || directorBeatMap.blockers)
    .map(cleanText)
    .filter(Boolean);
  const quantitativeMotionMinimumBlockers = new Set([
  ...
  return (
    ...
    directorBlockers.every((blocker) => quantitativeMotionMinimumBlockers.has(blocker)) &&
...
async function validateLocalBridgePackage({
  ...
  if (
    ...
    platformPublishManifest.can_auto_publish !== true
  ) blockers.push("platform_publish_manifest_not_green");
  if (needsMediaHouseScoreRepair) blockers.push("media_house_score_not_green");
  if (needsPublishVerdictRepair) blockers.push("publish_verdict_not_green");
  blockers.push(...asArray(authorityValidation.blockers));
  ...
  const outputs = platformPublishManifest.outputs || {};
  const platformScope = resolvePlatformPublishScope(platformPublishManifest);
  blockers.push(...platformScope.blockers);
  for (const platform of platformScope.enabled_platforms) {
    if (!CORE_PUBLISH_PLATFORMS.includes(platform)) continue;
    if (platformPackEvidence[platform]?.present !== true) {
      blockers.push(`platform_pack_missing:${platform}`);
    }
  ...
  return {
    ...
    evidence: {
      ...
      enabled_platforms_checked: platformScope.enabled_platforms,
      disabled_platforms_checked: platformScope.disabled_platforms,
      platform_pack_evidence: platformPackEvidence,
      rights_rows: rightsRows.length,
      monotonic_authority: authorityValidation.evidence || null,
...
async function buildLocalBridgeCandidate({
  ...
  const captionManifest = await readJsonIfPresent(path.join(resolvedArtifactDir, "caption_manifest.json"));
  ...
  const platformPackEvidence = platformPackPresence(
  ...
  const goalPackageSummary = await readJsonIfPresent(
    path.join(resolvedArtifactDir, "goal_package_summary.json"),
    null,
  );
  ...
  const authorityValidation = await validateMonotonicAuthority({
    artifactDir: resolvedArtifactDir,
    storyId,
    generatedAt,
    goalPackageSummary,
  ...
  const captionChecks = captionManifest.checks || {};
  const captionEvidenceWordCount = Number(
    captionManifest.word_count ||
      captionManifest.display_word_count ||
      captionManifest.word_timestamp_count ||
      audioManifest.word_timestamp_count ||
      0,
  );
  const captionEvidenceStatus = [
    captionManifest.verdict,
    captionManifest.status,
    captionManifest.quality_gate_status,
  ]
    .map((value) => cleanText(value).toLowerCase())
    .filter(Boolean);
  const captionEvidenceVerified =
    captionEvidenceStatus.some((value) => ["pass", "green", "ready"].includes(value)) &&
    asArray(captionManifest.blockers).length === 0 &&
    captionChecks.caption_file_verified === true &&
    captionChecks.display_script_verified === true &&
    captionChecks.display_alignment_exact === true &&
    captionEvidenceWordCount > 0 &&
    (await fileSize(captionPath, resolvedArtifactDir)) > 0;
  const publicDescription = cleanText(
    canonical.public_description ||
      canonical.description ||
      platformPublishManifest.outputs?.youtube_shorts?.description,
  );
  const youtubeDescription = cleanText(
    platformPublishManifest.outputs?.youtube_shorts?.description ||
  ...
  return {
    ...
    manual_caption_generated: captionEvidenceVerified,
    clean_manual_captions: captionEvidenceVerified,
    subtitle_timing_source: captionEvidenceVerified
      ? "timestamps"
      : cleanText(canonical.subtitle_timing_source),
    subtitle_timing_inspection: captionEvidenceVerified
      ? {
          usable: true,
          reason: null,
          word_count: captionEvidenceWordCount,
          timestamp_row_count: Number(
            captionManifest.word_timestamp_count ||
              audioManifest.word_timestamp_count ||
              captionEvidenceWordCount,
          ),
          source: "authoritative_caption_manifest",
        }
      : canonical.subtitle_timing_inspection || null,
    ...
    platform_publish_manifest: platformPublishManifest,
    enabled_platforms: platformScope.enabled_platforms,
    disabled_platforms: platformScope.disabled_platforms,
    disabled_platform_reasons: platformScope.disabled_platform_reasons,
...
async function persistSanitizedPackageEvidence({
  ...
  const updatedFiles = [];
  const skippedFiles = candidate.rights_ledger === undefined || candidate.rights_ledger === null
    ? []
    : [{
        file_name: "rights_ledger.json",
        updated: false,
        file_path: path.join(resolvedArtifactDir, "rights_ledger.json"),
