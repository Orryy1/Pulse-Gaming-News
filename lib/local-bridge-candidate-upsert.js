...
function mergeStandalonePlatformPacks(platformPublishManifest = {}, standalonePacks = {}, canonical = {}) {
  ...
  return {
    ...
    publish_status: repaired ? "GREEN" : platformPublishManifest.publish_status,
    outputs,
...
async function buildLocalBridgeCandidate({
  ...
  return {
    ...
    hook: cleanText(canonical.first_spoken_line || canonical.narration_hook || firstSentence(script)),
    body: script,
    ...
    audio_path: cleanText(canonical.narration_audio_path || audioManifest.narration_audio_path),
    narration_audio_path: cleanText(canonical.narration_audio_path || audioManifest.narration_audio_path),
    resolved_narration_audio_path: cleanText(
      canonical.resolved_narration_audio_path ||
        audioManifest.resolved_narration_audio_path ||
        canonical.narration_audio_path ||
        audioManifest.narration_audio_path,
    ),
    timestamps_path: cleanText(canonical.word_timestamps_path || audioManifest.word_timestamps_path),
    word_timestamps_path: cleanText(canonical.word_timestamps_path || audioManifest.word_timestamps_path),
    resolved_word_timestamps_path: cleanText(
      canonical.resolved_word_timestamps_path ||
        audioManifest.resolved_word_timestamps_path ||
        canonical.word_timestamps_path ||
        audioManifest.word_timestamps_path,
