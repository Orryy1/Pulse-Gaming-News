...
const MAX_SPAWN_STDERR_BYTES = 16 * 1024;
      ...
      : useOutputFileSpawn
        ? await new Promise((resolve, reject) => {
            ...
            const child = spawnImpl(invocation.executable, invocation.args, {
              ...
              // output handles are detached. Keep stdout inherited and capture only
              // bounded stderr so production failures retain their real root cause.
              stdio: ["ignore", "inherit", "pipe"],
            });
            child.stderr?.on("data", (chunk) => {
              stderr = `${stderr}${String(chunk || "")}`.slice(-MAX_SPAWN_STDERR_BYTES);
            });
            const timeout = setTimeout(() => {
              child.kill();
              const timeoutError = new Error(`whisper_alignment_timeout_after_${timeoutMs}ms`);
              timeoutError.stderr = stderr;
              reject(timeoutError);
            }, timeoutMs);
            child.once("error", (error) => {
              clearTimeout(timeout);
              error.stderr = cleanText(error.stderr || stderr);
              reject(error);
            });
            child.once("close", (code, signal) => {
              ...
              if (code === 0) resolve("");
                ...
                const processError = new Error(
                  `whisper_alignment_process_failed:code=${code}:signal=${signal || "none"}`,
                );
