...
function searchPotential(story) {
  ...
  if (/\bGTA\b|Pokemon|Final Fantasy|Xbox|Nintendo|PlayStation|Steam/i.test(title)) value += 8;
...
function franchiseDemand(story) {
  const title = normaliseText(story?.title);
  if (/\bGTA\b|Grand Theft Auto|Pokemon|Final Fantasy|Resident Evil|Mario|Zelda/i.test(title)) {
    return 90;
  }
...
function buildSegmentList(format, candidates) {
  const usable = candidates.filter((candidate) => candidate.scores.total >= 45);
...
function buildDossierFactCheckFlags(segments, candidates) {
  return [
    ...new Set([
      ...segments.flatMap((segment) => segment.required_fact_checks),
      ...candidates.flatMap((candidate) => candidate.fact_check_flags || []),
    ]),
  ];
}
