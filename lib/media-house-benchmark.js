...
function gtaSubjectAliases(normalised = "") {
  const tokens = normalised.split(/\s+/).filter(Boolean);
  if (tokens[0] !== "grand" || tokens[1] !== "theft" || tokens[2] !== "auto") return [];
  const suffix = tokens.slice(3).join(" ");
  const aliases = new Set(["gta", "grand theft auto"]);
  if (suffix) {
    aliases.add(`gta ${suffix}`);
    aliases.add(`grand theft auto ${suffix}`);
    const numericSuffix = suffix
      .replace(/\bvi\b/g, "6")
      .replace(/\bsix\b/g, "6")
      .replace(/\bv\b/g, "5")
      .replace(/\bfive\b/g, "5");
    const romanSuffix = suffix
      .replace(/\b6\b/g, "vi")
      .replace(/\bsix\b/g, "vi")
      .replace(/\b5\b/g, "v")
      .replace(/\bfive\b/g, "v");
    const spokenSuffix = suffix
      ...
      .replace(/\bv\b/g, "five")
      .replace(/\b5\b/g, "five");
    for (const variant of [numericSuffix, romanSuffix, spokenSuffix]) {
      if (!variant) continue;
      aliases.add(`gta ${variant}`);
      aliases.add(`grand theft auto ${variant}`);
    }
  }
  return [...aliases];
}
...
function subjectMatchAliases(subject) {
  ...
  for (const alias of gtaSubjectAliases(normalised)) aliases.add(alias);
...
function first3SecondsHookScore({ story = {}, shots = [], manifest = {} }) {
  ...
  if (/\b(?:problem|risk|trap|catch|fight|exploded?|skyrocket|dodged|avoided|exposed|beats?|biggest|ridiculous|won't vanish)\b/i.test(`${hook} ${title}`)) {
    score += 12;
  }
  return clampScore(score);
}
...
function normalisedAssetIdentityKeys(asset = {}) {
  if (typeof asset === "string") asset = { path: asset };
  if (!asset || typeof asset !== "object") return [];
  const keys = [];
  const add = (value) => {
    const key = cleanText(value).replace(/\\/g, "/").toLowerCase();
    if (key) keys.push(key);
    const outputIndex = key.lastIndexOf("/output/");
    if (outputIndex >= 0) keys.push(key.slice(outputIndex + 1));
  };
  add(asset.path || asset.local_path || asset.media_path || asset.file_path);
  add(asset.source_url || asset.url);
  add(asset.asset_id || asset.id);
  add(asset.source_family || asset.motion_family);
  return [...new Set(keys)];
}

function rightsLedgerByAssetKey(story = {}) {
  const byKey = new Map();
  const declaredLedger = story.rights_ledger;
  const records = Array.isArray(declaredLedger)
    ? declaredLedger
    : asArray(declaredLedger?.records || declaredLedger?.rights_records);
  for (const record of records) {
    const keys = normalisedAssetIdentityKeys(record);
...
function governedBoundedEditorialException(asset = {}) {
  if (!asset || typeof asset !== "object") return false;
  const basis = [
    asset.rights_basis_category,
    asset.licence_basis,
    asset.license_basis,
    asset.allowed_use,
    asset.evidence_kind,
    ...
    .join(" ")
    .toLowerCase();
  const approval = cleanText(asset.approval_status).toLowerCase();
  const verdict = cleanText(asset.verdict || asset.status).toLowerCase();
  const acceptance = asset.editorial_policy_acceptance || {};
  const risk = numericRiskScore(asset);
  return (
    /bounded_editorial_excerpt/.test(basis) &&
    /uk_fair_dealing_(?:criticism|review|quotation|reporting)/.test(basis) &&
    /approved/.test(approval) &&
    verdict === "green" &&
    asset.commercial_use_allowed === true &&
    asset.live_publish_allowed === true &&
    asset.legal_exception_reliance === true &&
    asset.transformative_rights_evidence_verified === true &&
    cleanText(asset.evidence_reference) !== "" &&
    cleanText(asset.evidence_kind) === "bounded_editorial_excerpt_policy" &&
    asset.credit_required === true &&
    cleanText(asset.on_screen_credit) !== "" &&
    cleanText(acceptance.accepted_by) !== "" &&
    Number.isFinite(Date.parse(cleanText(acceptance.accepted_at))) &&
    risk !== null &&
    risk <= 0.5
  );
}

function authoritativeSelectedMediaAssets(story = {}) {
  const selected =
    story.render_manifest?.selected_input_assets ||
    story.renderManifest?.selected_input_assets ||
    {};
  const assets = asArray(selected.assets);
  const declaredCount = Number(selected.asset_count);
  const completeForScope =
    selected.complete_for_declared_media_scope === true ||
    selected.complete === true;
  if (
    selected.authoritative !== true ||
    !completeForScope ||
    !cleanText(selected.inventory_scope) ||
    !assets.length ||
    (Number.isFinite(declaredCount) && declaredCount !== assets.length) ||
    asArray(selected.blockers).length ||
    asArray(selected.failures).length
  ) {
    return [];
  }
  return assets;
...
function assetWithRightsRecord(asset, ledgerByKey) {
  ...
  for (const key of normalisedAssetIdentityKeys(asset)) {
    if (ledgerByKey.has(key)) return ledgerByKey.get(key);
  }
  return asset;
}

function rightsRiskAssessment(story = {}) {
  ...
  const authoritativeAssets = authoritativeSelectedMediaAssets(story);
  const declaredLedger = story.rights_ledger;
  const ledgerRecords = Array.isArray(declaredLedger)
    ? declaredLedger
    : asArray(declaredLedger?.records || declaredLedger?.rights_records);
  const assets = authoritativeAssets.length
    ? authoritativeAssets.map((asset) => assetWithRightsRecord(asset, ledgerByKey))
    : [
        ...asArray(story.downloaded_images),
        ...videoClipAssets,
        ...asArray(story.media_provenance),
        ...(videoClipAssets.length === 0 ? ledgerRecords : []),
      ];
  if (assets.length === 0) {
    return {
      score: 55,
      evaluation_scope: "legacy_available_media_evidence",
      evaluated_asset_count: 0,
      governed_editorial_exception_count: 0,
      licence_or_ownership_claim_inferred: false,
    };
  }
  let safe = 0;
  let risky = 0;
  let missing = 0;
  let governedEditorialExceptions = 0;
  for (const asset of assets) {
    const klass = rightRiskClass(asset);
    const governedException = governedBoundedEditorialException(asset);
    if (governedException) governedEditorialExceptions += 1;
    if (
      governedException ||
  ...
  return {
    score: clampScore((safe / assets.length) * 100 - risky * 18 - missing * 12),
    evaluation_scope: authoritativeAssets.length
      ? "authoritative_selected_media_inputs"
      : "legacy_available_media_evidence",
    evaluated_asset_count: assets.length,
    safe_asset_count: safe,
    risky_asset_count: risky,
    missing_provenance_count: missing,
    governed_editorial_exception_count: governedEditorialExceptions,
    licence_or_ownership_claim_inferred: false,
  };
}

function rightsRiskScore(story = {}) {
  return rightsRiskAssessment(story).score;
}
...
function runMediaHouseBenchmark(options = {}) {
  ...
  const rightsEvidence = rightsRiskAssessment(story);
  ...
  const scores = {
    ...
    rights_risk_score: rightsEvidence.score,
