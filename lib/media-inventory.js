...
function normaliseInventoryTitle(value) {
...
function classifyStoryVisualInventory(story) {
  ...
  const runtimePlan = recommendRuntime(className, { story });
  const recommendedRuntimeSeconds = runtimePlan.runtimeSeconds?.target ?? 0;
  const blockerReasons = compactReasons([
    ...reasons,
    ...detailed.classificationReasons,
    ...(runtimePlan.blocker ? [runtimePlan.blocker] : []),
  ...
  const nextBestAction =
    runtimePlan.blocker || recommendations[0] || "ready_for_render_qa";

  return {
    storyId: story?.id || null,
    title: normaliseInventoryTitle(story?.title || ""),
    score,
    className,
    ...
    renderDecision: {
      shouldRender: runtimePlan.shouldRender === true,
      route: runtimeRouteFor(className, runtimePlan),
      runtimeTargetSeconds: recommendedRuntimeSeconds,
      durationBandId: runtimePlan.durationBandId || null,
      blocker: runtimePlan.blocker || null,
      nextBestAction,
    },
    scores: {
      legacyScore: score,
...
function recommendationsFor({ className, clips, gameAssets, stock, unsafeFaces, detailed }) {
  ...
  if (className === "briefing_item" || className === "blog_only") {
    out.push("do_not_force_standalone_video");
  }
