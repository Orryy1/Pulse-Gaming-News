...
const POLICY_VERSION = "pulse_media_rights_policy_2026-07-26";
const INTERNAL_EDITORIAL_GUARDRAILS = Object.freeze({
  max_continuous_motion_excerpt_seconds: 8,
  max_total_third_party_media_seconds: 36,
...
const BASIS_ALIASES = Object.freeze({
  ...
  transformative_gameplay: "bounded_editorial_excerpt",
  transformative_gameplay_excerpt: "bounded_editorial_excerpt",
  official_gameplay_excerpt: "bounded_editorial_excerpt",
  official_trailer_excerpt: "bounded_editorial_excerpt",
  official_trailer_segment: "bounded_editorial_excerpt",
  official_direct_media_editorial_excerpt: "bounded_editorial_excerpt",
});

const EDITORIAL_PURPOSES = new Set([
  "criticism_review",
  "quotation",
...
function assessEditorialExcerpt({
  ...
  if (type === "photograph" && purpose === "current_events_reporting") {
    ...
    blockers.push(`${id}:editorial_purpose_not_supported`);
  }
  ...
  const licenceBasis =
    purpose === "quotation"
      ...
      : purpose === "current_events_reporting"
    ...
    : positiveRecord({
        ...
        allowedUse:
          purpose === "current_events_reporting"
            ...
            : "bounded_transformative_editorial_excerpt",
...
function assessMediaRightsPackage({
  ...
  const assetRows = asArray(assets);
  const totalEditorialMediaSeconds = Number(
    assetRows
      .filter(
        (asset) =>
          normaliseBasis(asset) === "bounded_editorial_excerpt" &&
          (VIDEO_TYPES.has(
            normaliseAssetType(asset.asset_type || asset.kind),
          ) ||
            STILL_TYPES.has(
              normaliseAssetType(asset.asset_type || asset.kind),
            )),
      ...
      .reduce((total, asset) => {
        const seconds = finiteNumber(asset.total_use_seconds);
        return total + (seconds != null && seconds > 0 ? seconds : 0);
      }, 0)
      .toFixed(3),
  );
  if (
    totalEditorialMediaSeconds >
    INTERNAL_EDITORIAL_GUARDRAILS.max_total_third_party_media_seconds
  ) {
    packageBlockers.push(
      "package:total_editorial_media_over_internal_guardrail",
    );
  }
  const decisions = assetRows.map((asset, index) =>
  ...
  return {
    ...
    summary: {
      ...
      total_editorial_media_seconds: totalEditorialMediaSeconds,
    ...
    legal_basis: {
      ...
      editorial_exception:
        "UK fair dealing for criticism, review, quotation or current-events reporting assessed case by case; photographs remain separately restricted for current-events reporting",
      attribution:
