...
function checksumOfString(value) {
  return crypto.createHash("sha256").update(value).digest("hex");
}

function lineEndingEquivalentChecksums(filepath) {
  const body = fs.readFileSync(filepath, "utf8");
  const lf = body.replace(/\r\n/g, "\n");
  const crlf = lf.replace(/\n/g, "\r\n");
  return new Set([checksumOfString(body), checksumOfString(lf), checksumOfString(crlf)]);
}
  ...
  for (const filename of files) {
    ...
    if (existing) {
      if (existing.checksum !== checksum) {
        const equivalentLineEndings = lineEndingEquivalentChecksums(filepath);
        if (!equivalentLineEndings.has(existing.checksum)) {
          throw new Error(
