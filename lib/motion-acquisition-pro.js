...
} = require("./hash-bound-official-youtube-local-master");
...
function normaliseReference(reference) {
  return {
    ...
    source_family: reference?.source_family || null,
    reference_url: reference?.reference_url || reference?.official_source_url || null,
    source_url_kind: reference?.source_url_kind || null,
    segment_validation_eligible: reference?.segment_validation_eligible,
    segment_validation_ineligible_reason: reference?.segment_validation_ineligible_reason || null,
    ...
    reference_title: reference?.reference_title || reference?.movie_name || reference?.name || reference?.title || null,
    ...
    source_duration_s:
      reference?.source_duration_s ||
      reference?.sourceDurationS ||
      reference?.duration_seconds ||
      reference?.durationSeconds ||
      reference?.provenance?.source_duration_s ||
      null,
    rights_risk_class: reference?.rights_risk_class || null,
    allowed_render_use: reference?.allowed_render_use || null,
    source_tier: reference?.source_tier || null,
    trusted_footage_source_id: reference?.trusted_footage_source_id || reference?.registry_id || null,
    trusted_footage_registry_status:
      reference?.trusted_footage_registry_status || reference?.trusted_registry_status || null,
    video_use_style_plan: reference?.video_use_style_plan || null,
    downloads_allowed: reference?.downloads_allowed === true,
    source_verified: reference?.source_verified,
    autonomous_use_approved: reference?.autonomous_use_approved,
    rights_grant: reference?.rights_grant,
    allowed_platforms: Array.isArray(reference?.allowed_platforms)
      ? [...reference.allowed_platforms]
      : reference?.allowed_platforms,
    commercial_use_allowed: reference?.commercial_use_allowed,
    source_audio_allowed: reference?.source_audio_allowed,
    rights_status: reference?.rights_status || null,
    rights_verdict: reference?.rights_verdict || null,
    approval_status: reference?.approval_status || null,
    provenance:
      reference?.provenance && typeof reference.provenance === "object"
...
function officialPlansForStory(story, options = {}) {
  const storyId = story?.id;
  return officialPlansFromOptions(options).filter((plan) => plan?.story_id === storyId || plan?.storyId === storyId);
}

function officialReferencesForStory(story, options = {}) {
  return officialPlansForStory(story, options)
    .flatMap((plan) => (Array.isArray(plan?.references) ? plan.references : []))
    .filter(Boolean)
    .filter(
      (reference) =>
        reference.downloads_allowed !== true ||
        isHashBoundOfficialYoutubeLocalMaster(reference),
    );
}
...
function resolverCoverageForStory(story, options = {}) {
  const plans = officialPlansForStory(story, options);
  return {
    target_entities: uniqueStrings(plans.flatMap((plan) => plan.target_entities || [])),
    covered_target_entities: uniqueStrings(plans.flatMap((plan) => plan.covered_target_entities || [])),
    missing_target_entities: uniqueStrings(plans.flatMap((plan) => plan.missing_target_entities || [])),
    partial: plans.some((plan) => plan.motion_reference_readiness === "partial_official_reference_found"),
  };
}
...
function motionReadiness({
  ...
  if (references.length > 0 && hasPartialResolverCoverage) {
    return "partial_reference_needs_targeted_search";
  }
  if (
    references.length > 0 &&
...
function buildMotionAcquisitionPlan(story, options = {}) {
  ...
  const resolverCoverage = resolverCoverageForStory(story, options);
    ...
    ? [
        {
          ...
          accepted_sources: [
            "Steam movie metadata",
            "IGDB video metadata",
            "trusted footage registry references",
          ],
        },
      ]
    : [];
  const targetedResolverSearchActions = resolverCoverage.missing_target_entities.map((entity) => ({
    type: "targeted_official_reference_search",
    priority: "high",
    target: entity,
    reason: "missing_official_motion_reference_for_story_target",
    will_download: false,
    mutates: false,
    queries: [`${entity} official trailer`, `${entity} gameplay trailer`, `${entity} Steam trailer`],
    accepted_sources: ["Steam movie metadata", "IGDB video metadata", "official publisher channel"],
  }));
  const plannedActions = [
    ...officialSearchTasks,
    ...targetedResolverSearchActions,
    ...extractionTasks,
    ...resolverPlanActions,
  ].map((task) => ({
    type: task.type,
  ...
  const readiness = motionReadiness({
    ...
    hasResolverReferences,
    hasPartialResolverCoverage:
      resolverCoverage.partial || resolverCoverage.missing_target_entities.length > 0,
  });
  ...
  return {
    ...
    resolver_reference_coverage: {
      target_entities: resolverCoverage.target_entities,
      covered_target_entities: resolverCoverage.covered_target_entities,
      missing_target_entities: resolverCoverage.missing_target_entities,
      partial: resolverCoverage.partial,
    },
    ...
    blockers: buildMotionBlockers(readiness, counts, references, resolverCoverage),
...
function buildMotionBlockers(readiness, counts, references, resolverCoverage = {}) {
  const blockers = [];
  if (readiness === "reject") blockers.push("off_brand_or_rejected_story");
  if ((resolverCoverage.missing_target_entities || []).length > 0) {
    blockers.push("missing_official_reference_entities");
  }
...
function buildMotionAcquisitionReport(stories = [], options = {}) {
  ...
  return {
    ...
    summary: {
      ...
      partial_reference_needs_targeted_search:
        byReadiness.partial_reference_needs_targeted_search || 0,
...
function renderMotionAcquisitionMarkdown(report) {
  ...
  lines.push(
    `- partial references needing targeted search: ${report.summary.partial_reference_needs_targeted_search || 0}`,
  );
  lines.push(`- official trailer search required: ${report.summary.official_reference_search_required}`);
  lines.push("");
  lines.push("| story | readiness | refs | missing targets | clips | frames | actions | blockers |");
  lines.push("| --- | --- | ---: | --- | ---: | ---: | --- | --- |");
  for (const plan of report.plans) {
    lines.push(
      [
        ...
        plan.resolver_reference_coverage?.missing_target_entities?.join(", ") || "none",
