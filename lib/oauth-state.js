...
/** @type {Map<string, { provider: string, createdAt: number, metadata?: Record<string, unknown> }>} */
const store = new Map();
...
function enforceMaxPending(maxPendingStates = MAX_PENDING_STATES) {
  while (store.size > maxPendingStates) {
 ...
 * @param {{ now?: number, ttlMs?: number, metadata?: Record<string, unknown> }} [opts]
 ...
 * @returns {{ ok: true, metadata?: Record<string, unknown> } | { ok: false, reason: string }}
 */
