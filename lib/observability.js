...
function getScoringDigest({ repos, sinceHours = 24, channelId = null } = {}) {
  ...
  const whereChannel = channelId
    ? `AND COALESCE(sc.channel_id, s_filter.channel_id, 'pulse-gaming') = ?`
    : "";
  const joinChannelFilter = channelId
    ? `LEFT JOIN stories s_filter ON s_filter.id = sc.story_id`
    : "";
  ...
  const latestScoresCte = `
    WITH latest_scores AS (
      SELECT sc.*
        ...
        JOIN (
          SELECT story_id, MAX(id) AS latest_id
            FROM story_scores
           WHERE datetime(scored_at) >= datetime(?)
           GROUP BY story_id
        ) latest ON latest.latest_id = sc.id
    )
  `;
  ...
  const byDecision = db
    .prepare(
      `${latestScoresCte}
       SELECT decision, COUNT(*) AS n FROM latest_scores sc
        ${joinChannelFilter}
        WHERE 1=1 ${whereChannel}
        GROUP BY decision`,
    )
    .all(...params);
  ...
  const avgTotalRow = db
    .prepare(
      `${latestScoresCte}
       SELECT AVG(total) AS avg FROM latest_scores sc
        ${joinChannelFilter}
        WHERE 1=1 ${whereChannel}`,
    )
    .get(...params);
  ...
  const hardStops = db
    .prepare(
      `${latestScoresCte}
       SELECT COUNT(*) AS n FROM latest_scores sc
        ${joinChannelFilter}
        WHERE 1=1 ${whereChannel}
      ...
      `${latestScoresCte}
       SELECT sc.story_id, sc.total, sc.decision, s.title
        ...
        WHERE 1=1 ${whereChannel}
          ...
          AND sc.total BETWEEN 60 AND 74
          AND COALESCE(sc.inputs, '') NOT LIKE '%community_discussion_auto_block%'
        ORDER BY sc.total DESC LIMIT 3`,
    )
    .all(...params);
...
function buildScoringDigestMessage(summary) {
  ...
  const lines = [
    ...
    "Review means: scored well enough to inspect, but not safe for auto-publish yet. Open the review queue/control room, approve or reject it, then let preflight decide.",
  ...
  if (summary.top && summary.top.length) {
    ...
    for (const t of summary.top) {
      lines.push(formatDigestStoryLine(t, { includeDecision: true }));
    }
  }
  if (summary.near_miss && summary.near_miss.length) {
    ...
    for (const t of summary.near_miss) {
      lines.push(formatDigestStoryLine(t));
    }
  }
  return lines.join("\n");
}

function formatDigestStoryLine(row = {}, { includeDecision = false } = {}) {
  const score = row.total ?? "n/a";
  const decision = includeDecision && row.decision ? ` (${row.decision})` : "";
  const title = normaliseDigestTitle(row.title || row.story_id || "untitled story");
  return `- \`${score}\`${decision}\n  ${title}`;
}

function normaliseDigestTitle(value = "") {
  return String(value || "")
    .replace(/\s+/g, " ")
    .trim();
}
