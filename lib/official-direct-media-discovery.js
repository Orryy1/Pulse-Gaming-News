...
const { headlineFromHtml } = require("./official-source-page-evidence");
...
const YOUTUBE_REFERENCE_RE =
  /(?:https?:)?\/\/(?:www\.)?(?:youtube\.com\/(?:embed\/|watch\?v=)|youtu\.be\/)([A-Za-z0-9_-]{11})/gi;
...
const GTA_VI_OFFICIAL_SITE_MOTION_URLS = Object.freeze([
...
function discoverYouTubeReferenceUrlsFromText({ text = "" } = {}) {
  const decoded = cleanText(decodeEscapedText(text));
  const found = [];
  const seen = new Set();
  for (const match of decoded.matchAll(YOUTUBE_REFERENCE_RE)) {
    const url = youtubeWatchUrl(match[1]);
    if (!url) continue;
    ...
    found.push({
      url,
      source_url_kind: kind.source_url_kind,
      segment_validation_eligible: kind.segment_validation_eligible,
      segment_validation_ineligible_reason: kind.segment_validation_ineligible_reason,
      source: "youtube_embed_reference",
    });
  }
  return found;
}
...
function isTrustedOfficialStorefrontUrl(value = "") {
  try {
    const parsed = new URL(value);
    const host = parsed.hostname.toLowerCase().replace(/^www\./, "");
    const pathname = parsed.pathname.toLowerCase();
    if (host === "store.steampowered.com" && /\/app\/\d+\b/.test(pathname)) return true;
    if ((host === "xbox.com" || host.endsWith(".xbox.com")) && /\/games\/store\//.test(pathname)) return true;
    if ((host === "microsoft.com" || host.endsWith(".microsoft.com")) && /\/(?:store|p)\//.test(pathname)) return true;
    if ((host === "playstation.com" || host.endsWith(".playstation.com")) && /\/games\//.test(pathname)) return true;
    if (host === "store.playstation.com" && /\/product\//.test(pathname)) return true;
    if ((host === "nintendo.com" || host.endsWith(".nintendo.com")) && /\/store\/products\//.test(pathname)) return true;
    return false;
  } catch {
    return false;
  }
...
function linkContextMatchesEntity({ url = "", context = "", entity = "" } = {}) {
  const entityTokens = [...new Set(textTokens(entity))];
  if (!entityTokens.length) return false;
  const searchableTokens = [...new Set(textTokens(`${url} ${context}`))];
  const overlap = entityTokens.filter((token) => searchableTokens.includes(token)).length;
  if (!overlap) return false;
  return overlap >= Math.min(2, entityTokens.length) || overlap / entityTokens.length >= 0.5;
}
...
function discoverOfficialMediaPageLinksFromText({ text = "", baseUrl = "", entity = "", limit = 2 } = {}) {
  ...
  function add(raw, index = 0) {
    ...
    const sameOrigin = sameOriginUrl(resolved, baseUrl);
    const trustedStorefront =
      !sameOrigin &&
      isTrustedOfficialStorefrontUrl(resolved) &&
      linkContextMatchesEntity({ url: resolved, context, entity });
    if (!sameOrigin && !trustedStorefront) return;
    if (sameOrigin && !mediaPageCandidate(resolved, context)) return;
    seen.add(key);
    found.push(resolved);
  }
...
function linkedMediaPageMatchesEntity({ url = "", text = "", entity = "" } = {}) {
  if (candidateMatchesGtaViAlias({ url }, entity)) return true;
  if (isGtaViOfficialPageEntry({ entity, official_source_url: url })) return true;
  return linkContextMatchesEntity({
    url,
    context: headlineFromHtml(text),
    entity,
  });
}
...
function candidateMatchesGtaViAlias(candidate = {}, entity = "") {
  const entityCompact = compactSearchText(entity);
  const isGtaViEntity =
    entityCompact.includes("grandtheftautovi") ||
    entityCompact.includes("grandtheftauto6") ||
    entityCompact.includes("gtavi") ||
    entityCompact.includes("gta6");
  if (!isGtaViEntity) return false;
  const mediaCompact = compactSearchText(candidate.direct_media_url || candidate.url);
  return (
    mediaCompact.includes("grandtheftautovi") ||
    mediaCompact.includes("grandtheftauto6") ||
    mediaCompact.includes("gtavi") ||
    mediaCompact.includes("gta6")
  );
}

function candidateMatchesKnownOfficialCatalogue(candidate = {}, entity = "") {
  const source = cleanText(candidate.discovery_source || candidate.source);
  if (source !== "known_official_catalogue") return false;
  const entityCompact = compactSearchText(entity);
  return (
    entityCompact.includes("grandtheftautovi") ||
    entityCompact.includes("grandtheftauto6") ||
...
function candidateMatchesEntity(candidate = {}, entity = "") {
  if (candidateMatchesGtaViAlias(candidate, entity)) return true;
  if (candidateMatchesKnownOfficialCatalogue(candidate, entity)) return true;
  const entityTokens = [...new Set(textTokens(entity))];
  ...
  if (!mediaTokens.length) return true;
  if (overlap === 0) return false;
  if (mediaTokens.length <= 2) return true;
  return overlap / entityTokens.length >= 0.6;
}

function steamStoreAppIdFromUrl(value = "") {
  const text = cleanText(value);
  const match = text.match(/store\.steampowered\.com\/app\/(\d+)\b/i);
  return match ? match[1] : "";
}

function steamMediaAppIdFromUrl(value = "") {
  const text = cleanText(value);
  const trailerMatch = text.match(/steamstatic\.com\/store_trailers\/(\d+)\//i);
  if (trailerMatch) return trailerMatch[1];
  const assetMatch = text.match(/store_item_assets\/steam\/apps\/(\d+)\//i);
  return assetMatch ? assetMatch[1] : "";
}

function candidateMatchesOfficialStorefront(candidate = {}, entry = {}) {
  const storeAppId = steamStoreAppIdFromUrl(entry.official_source_url);
  if (!storeAppId) return false;
  const mediaAppId = steamMediaAppIdFromUrl(candidate.direct_media_url || candidate.url);
  return Boolean(mediaAppId && mediaAppId === storeAppId);
}
...
function isGtaViOfficialPageEntry(entry = {}) {
  const entityCompact = compactSearchText(entry.entity);
  if (
    !(
      entityCompact.includes("grandtheftautovi") ||
      entityCompact.includes("grandtheftauto6") ||
  ...
  try {
    const parsed = new URL(cleanText(entry.official_source_url));
    const host = parsed.hostname.toLowerCase().replace(/^www\./, "");
    const pathname = parsed.pathname.toLowerCase();
    return host === "rockstargames.com" && /^\/vi(?:\/|$)/.test(pathname);
  } catch {
    return false;
  }
}

function supplementalOfficialDirectMediaCandidates(entry = {}) {
  if (!isGtaViOfficialPageEntry(entry)) return [];
  return GTA_VI_OFFICIAL_SITE_MOTION_URLS.map((url) => directMediaCandidate(url, "known_official_catalogue")).filter(
    Boolean,
  );
}

function appendUniqueDirectMediaCandidates(target = [], additions = []) {
  const seen = new Set(target.map((candidate) => canonicalUrl(candidate.url)).filter(Boolean));
  for (const candidate of asArray(additions)) {
    const key = canonicalUrl(candidate.url);
    if (!key || seen.has(key)) continue;
    seen.add(key);
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        "user-agent": "PulseGamingDirectMediaDiscovery/1.0 report-only",
      },
    });
    const text = await response.text();
    if (!response.ok) {
...
function steamAppdetailsUrl(appId) {
  const id = cleanText(appId);
  if (!/^\d+$/.test(id)) return "";
  return `https://store.steampowered.com/api/appdetails?appids=${id}&l=english&cc=us&filters=movies,basic`;
}

function steamMovieUrls(movie = {}) {
  const urls = [];
  function add(value) {
    const candidate = directMediaCandidate(value, "steam_appdetails_movie_metadata");
  ...
  add(movie.mp4?.["480"]);
  add(movie.webm?.["480"]);
  add(movie.highlighted_movie_webm);
  add(movie.highlighted_movie_mp4);
  return urls;
}

async function discoverSteamAppdetailsMovieCandidates(entry = {}, options = {}) {
  const appId = steamStoreAppIdFromUrl(entry.official_source_url);
  const url = steamAppdetailsUrl(appId);
  if (!url) return [];
  const fetchJson = options.fetchJson || defaultFetchJson;
  let fetched;
  try {
    fetched = await fetchJson(url, options);
  } catch {
    return [];
  }
  if (!fetched?.ok || !fetched.json) return [];
  const appPayload = fetched.json?.[appId];
  const movies = asArray(appPayload?.data?.movies);
  const out = [];
  const seen = new Set();
  for (const movie of movies) {
    for (const candidate of steamMovieUrls(movie)) {
      const key = canonicalUrl(candidate.url);
      if (!key || seen.has(key)) continue;
      seen.add(key);
      out.push({
        ...candidate,
        media_title: cleanText(movie.name || movie.title),
      });
    }
  }
  return out;
...
function templateEntryForCandidate(entry = {}, row = {}, candidate = null, index = 0) {
  ...
  if (out.direct_media_url_if_available) {
    Object.assign(out, mediaSourceUrlKindFields(out.direct_media_url_if_available));
  }
...
function templateEntryForReferenceOnlyCandidate(entry = {}, candidate = {}, index = 0) {
  const url = cleanText(candidate.url || candidate.official_source_url);
  const kind = mediaSourceUrlKindFields(url);
  const baseFamily = cleanText(entry.source_family);
  const sourceFamily =
    index > 0
      ? `${baseFamily}__youtube_reference_${String(index).padStart(2, "0")}`
      : `${baseFamily}__youtube_reference`;
  return {
    ...entry,
    source_family: sourceFamily,
    source_type: "official_youtube_channel_url",
    source_owner: `${cleanText(entry.source_owner) || "Official source"} embedded YouTube reference`,
    official_source_url: url,
    direct_media_url_if_available: "",
    source_url_kind: kind.source_url_kind,
    segment_validation_eligible: kind.segment_validation_eligible,
    segment_validation_ineligible_reason: kind.segment_validation_ineligible_reason,
    discovery_source: candidate.source || "youtube_embed_reference",
    media_identity: mediaIdentityFromUrl(url, index),
    downloads_allowed: false,
    direct_media_provided: false,
    evidence_of_officialness:
      cleanText(entry.evidence_of_officialness) ||
      "This YouTube reference was embedded by the official source page.",
    entity_match_notes:
      cleanText(entry.entity_match_notes) ||
      `The embedded video appears on the official source page for ${cleanText(entry.entity) || "this story"}.`,
  };
}
...
function templateEntriesForRow(entry = {}, row = {}, options = {}) {
  ...
  const directEntries =
    limit <= 1 || candidates.length <= 1
      ? [templateEntry(entry, row)]
      : candidates.slice(0, limit).map((candidate, index) =>
          templateEntryForCandidate(entry, row, candidate, index),
        );
  const referenceEntries = asArray(row.reference_only_candidates).map((candidate, index) =>
    templateEntryForReferenceOnlyCandidate(entry, candidate, index),
  );
  return [...directEntries, ...referenceEntries];
}
...
async function rowForEntry(entry = {}, index, options = {}) {
  ...
  try {
    ...
    const referenceOnlyCandidates = discoverYouTubeReferenceUrlsFromText({
      text: fetched.text,
    });
    ...
    let linkedPageEntityMismatchCount = 0;
    const rejectedLinkedPageUrls = [];
    ...
    if (!selected) {
      for (const linkedPageUrl of discoverOfficialMediaPageLinksFromText({
        ...
        if (!linkedMediaPageMatchesEntity({
          url: linkedPageUrl,
          text: linkedFetched.text,
          entity: entry.entity,
        })) {
          linkedPageEntityMismatchCount += 1;
          rejectedLinkedPageUrls.push(linkedPageUrl);
          continue;
        }
        ...
        discoverySource = sameOriginUrl(linkedPageUrl, officialUrl)
          ? "official_same_origin_media_page"
          : "trusted_official_linked_media_page";
    ...
    if (!selected) {
      const steamCandidates = await discoverSteamAppdetailsMovieCandidates(entry, options);
      if (steamCandidates.length) {
        candidates.push(...steamCandidates);
        selected = steamCandidates[0];
        discoverySource = "steam_appdetails_movie_metadata";
      }
    }

    appendUniqueDirectMediaCandidates(candidates, supplementalOfficialDirectMediaCandidates(entry));
    if (!selected && candidates.length) {
      selected = candidates[0];
      discoverySource = selected.source || "known_official_catalogue";
    }

    const enrichedCandidates = selected
      ? await enrichDirectMediaCandidates(candidates, options)
      : [];
    const effectiveStorefrontEntry = discoveredPageUrl
      ? { ...entry, official_source_url: discoveredPageUrl }
      : entry;
    const entityMatchedCandidates = enrichedCandidates.filter((candidate) =>
      candidateMatchesEntity(candidate, entry.entity) ||
      candidateMatchesOfficialStorefront(candidate, effectiveStorefrontEntry),
    );
    ...
    return {
      ...
      rejection_reason: directMediaFound
        ...
        : shortDurationCandidateCount > 0
          ...
          : linkedPageEntityMismatchCount > 0
          ? "linked_media_page_entity_mismatch"
          : "no_validation_eligible_media_url_found",
      candidate_count: candidates.length,
      entity_mismatch_candidate_count: entityMismatchCandidateCount,
      linked_page_entity_mismatch_count: linkedPageEntityMismatchCount,
      rejected_linked_page_urls: rejectedLinkedPageUrls,
      short_duration_candidate_count: shortDurationCandidateCount,
      direct_media_candidates: directMediaCandidates,
      reference_only_candidates: referenceOnlyCandidates,
      ...metadata,
    };
  } catch (err) {
    return {
...
async function buildOfficialDirectMediaDiscoveryReport({
  ...
  return {
    ...
    summary: {
      ...
      reference_only_youtube_embeds: rows.reduce(
        (sum, row) => sum + asArray(row.reference_only_candidates).length,
        0,
      ),
...
module.exports = {
  ...
  discoverYouTubeReferenceUrlsFromText,
  discoverSteamAppdetailsMovieCandidates,
