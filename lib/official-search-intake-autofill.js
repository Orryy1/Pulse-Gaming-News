...
async function autofillSteamEntry(entry = {}, options = {}) {
  ...
  const searchUrl = steamApiSearchUrl(entity);
  let fetched;
  try {
  ...
  const match = bestSteamResult(results, entity, options.minimumSteamScore || 0.9);
  if (!match) {
    return {
      provider: "steam_storesearch",
...
async function buildOfficialSearchIntakeAutofillReport({
  ...
  for (const [index, rawEntry] of asArray(entries).entries()) {
    ...
    const steam = await autofillSteamEntry(entry, { fetchJson, timeoutMs, minimumSteamScore });
    rows.push({
      index,
  ...
  return {
    ...
    safety: {
      local_only: true,
      provider_scope: ["steam_storesearch"],
...
function renderOfficialSearchIntakeAutofillMarkdown(report = {}) {
  ...
  lines.push("");
  lines.push("Safety: report-only Steam/storefront metadata lookup. No downloads, browser scraping, DB mutation, OAuth or posting.");
  lines.push("");
