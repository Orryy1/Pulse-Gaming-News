...
function acceptedReferenceFromEntry(entry, index) {
  const canonical = canonicalSourceUrl(entry);
  const urlKind = mediaSourceUrlKindFields(entry.official_source_url);
  return {
...
function renderOfficialSourceIntakeMarkdown(report) {
  ...
  lines.push("");
  lines.push("| story | entity | source type | source | URL kind | segment validation | use |");
  lines.push("| --- | --- | --- | --- | --- | --- | --- |");
  for (const entry of report.accepted_entries || []) {
    lines.push(
      ...
      } | ${entry.segment_validation_eligible ? "eligible" : "reference-only"} | reference-only |`,
    );
  }
  if (!report.accepted_entries?.length) lines.push("| none | none | none | none | none | none | none |");
  lines.push("");
