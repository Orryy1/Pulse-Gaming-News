...
function labelAliases(value) {
  ...
  if (/\b(?:gta\s*6|gta\s*vi|grand theft auto\s*(?:6|vi)?)\b/.test(normalised)) {
    aliases.add("gta 6");
    aliases.add("gta vi");
  ...
  const garfieldEscape = /\bgarfield\b/.test(normalised) && /\bescape\b/.test(normalised) && /\bmonday\b/.test(normalised);
  if (garfieldEscape) {
    aliases.add("garfield");
    aliases.add("garfield escape from monday");
  }
  return [...aliases].filter(Boolean);
}

function labelAppearsInText(label, text) {
  const normalisedText = normaliseLabel(text);
  const compactText = compactLabel(text);
  for (const alias of labelAliases(label)) {
    const normalisedAlias = normaliseLabel(alias);
    const compactAlias = compactLabel(alias);
    if (normalisedAlias && normalisedText.includes(normalisedAlias)) return true;
    if (compactAlias.length >= 3 && compactText.includes(compactAlias)) return true;
  }
  return false;
}
...
function entityAppearsInStory(entity, story) {
  ...
  if (storyEntities(story).some((candidate) => labelAliases(entity).includes(normaliseLabel(candidate)))) return true;
  const storyText = [
    ...
    .filter(Boolean)
    .join(" ");
  return labelAppearsInText(entity, storyText);
}
...
function hasNonNoteEntityEvidence(entry) {
  const entity = String(entry?.entity || "").trim();
  const combined = [
    entry?.official_source_url,
    entry?.direct_media_url_if_available,
...
function explicitReferencePageUrl(entry) {
  return cleanText(entry?.reference_page_url || entry?.official_reference_page_url);
}
...
function rejectionReasonsForEntry(entry, context) {
  ...
  const suppliedReferencePageUrl = explicitReferencePageUrl(entry);
  if (suppliedReferencePageUrl && parseHttpUrl(suppliedReferencePageUrl).reason) {
    reasons.push("invalid_reference_page_url");
  }
  ...
  if (story && (!hasEntityEvidence(entry) || !hasNonNoteEntityEvidence(entry))) {
...
function acceptedReferenceFromEntry(entry, index) {
  ...
  const productPageEvidence = hashBoundFileEvidence(entry, "product_page_evidence");
  const policyEvidence = hashBoundFileEvidence(entry, "policy_evidence");
  const livePublishAllowed = entry.live_publish_allowed === true;
  const requiresHumanLegalReview =
    entry.requires_human_legal_review_before_publish === false
      ? false
      : true;
  return {
    ...
    reference_page_url: suppliedReferencePageUrl || (directMediaUrl ? referenceUrl : null),
    ...
    licence_basis: cleanText(entry.licence_basis || entry.license_basis) || "official_reference_only",
    allowed_use: cleanText(entry.allowed_use) || "reference_validation_only",
    allowed_platforms: cleanStringArray(entry.allowed_platforms),
    restricted_platforms: cleanStringArray(entry.restricted_platforms),
    platform_restrictions:
      entry.platform_restrictions && typeof entry.platform_restrictions === "object"
        ? structuredClone(entry.platform_restrictions)
        : null,
    commercial_use_allowed: entry.commercial_use_allowed === true,
    local_materialization_allowed: entry.local_materialization_allowed !== false,
    live_publish_allowed: livePublishAllowed,
    requires_human_legal_review_before_publish: requiresHumanLegalReview,
    approval_status:
      cleanText(entry.approval_status) || "approved_for_reference_validation_only",
    rights_status:
      cleanText(entry.rights_status) || "official_identity_verified_rights_unresolved",
    required_public_notice: cleanText(entry.required_public_notice) || null,
    required_rules_link: cleanText(entry.required_rules_link) || null,
    local_source_path: cleanText(
      entry.local_source_path || entry.source_asset_path || entry.captured_asset_path,
    ) || null,
    source_asset_sha256: cleanText(entry.source_asset_sha256 || entry.asset_sha256) || null,
    source_asset_size_bytes:
      positiveNumberOrNull(entry.source_asset_size_bytes || entry.asset_size_bytes),
    ...(productPageEvidence
      ? {
          product_page_evidence_path: productPageEvidence.path,
          product_page_evidence_sha256: productPageEvidence.sha256,
          product_page_evidence_size_bytes: productPageEvidence.size_bytes,
        }
      : {}),
    ...(policyEvidence
      ? {
          policy_evidence_path: policyEvidence.path,
          policy_evidence_sha256: policyEvidence.sha256,
          policy_evidence_size_bytes: policyEvidence.size_bytes,
        }
      : {}),
    provenance: {
      ...
      rights_status:
        cleanText(entry.rights_status) || "official_identity_verified_rights_unresolved",
      product_page_evidence: productPageEvidence,
...
function hashBoundFileEvidence(entry = {}, prefix = "") {
  const evidencePath = cleanText(entry[`${prefix}_path`]);
  const evidenceSha256 = cleanText(entry[`${prefix}_sha256`]).toLowerCase();
  const evidenceSizeBytes = Number(entry[`${prefix}_size_bytes`]);
  if (
    !evidencePath ||
    !/^[a-f0-9]{64}$/.test(evidenceSha256) ||
    !Number.isFinite(evidenceSizeBytes) ||
    evidenceSizeBytes <= 0
  ) {
    return null;
  }
