...
function stripTitleQualifier(value) {
  return String(value || "")
    .replace(
      /\b(?:gameplay\s+trailer|official\s+trailer|announcement\s+trailer|launch\s+trailer|trailer|review|release\s+date|pre[- ]?orders?|patch\s+notes|system\s+update|available|launches?|gets?|shows?|teases?|reveals?|revealed|opens?|begins?)\b.*$/i,
      "",
    )
    .replace(/\s+[-:|,]\s*$/g, "")
    .replace(/\s+/g, " ")
...
function specificCanonicalTitleEntity(story) {
  ...
  const title = String(story?.canonical_title || "").trim();
  if (!canonical || !title) return null;
  const confidence = String(
    story?.canonical_subject_confidence || story?.canonical_game_confidence || "",
  )
    .trim()
    .toLowerCase();
  if (
    confidence === "explicit" &&
    (String(story?.canonical_game || "").trim() || String(story?.canonical_subject || "").trim())
  ) {
    return null;
  }
  const canonicalKey = normaliseLabel(canonical);
  const titleKey = normaliseLabel(title);
  if (!canonicalKey || !titleKey.startsWith(canonicalKey)) return null;

  const candidate = stripTitleQualifier(title);
  const candidateKey = normaliseLabel(candidate);
  if (!candidateKey || candidateKey === canonicalKey) return null;
  if (!candidateKey.startsWith(canonicalKey)) return null;
  const words = candidate.split(/\s+/).filter(Boolean);
  if (words.length > 8 || candidate.length > 80) return null;
  return candidate;
}

function inferredStoryEntity(story) {
  return (
    specificCanonicalTitleEntity(story) ||
    story?.canonical_game ||
    story?.canonical_subject ||
    story?.game_title ||
    story?.primary_entity ||
...
function officialSourceIntakeRows(report = {}) {
  if (Array.isArray(report)) return report;
  if (!report || typeof report !== "object") return [];
  return [
    ...(Array.isArray(report.accepted_references) ? report.accepted_references : []),
    ...(Array.isArray(report.entries) ? report.entries : []),
    ...(Array.isArray(report.items) ? report.items : []),
    ...(Array.isArray(report.rows) ? report.rows : []),
    ...(Array.isArray(report.output_template?.entries) ? report.output_template.entries : []),
  ];
}

function officialIntakeReferenceUrl(reference = {}) {
  const localOperatorFile = String(
    reference.local_operator_file_path ||
      reference.localOperatorFilePath ||
  ...
  if (
    ...
    mediaSourceUrlKindFields(localOperatorFile).segment_validation_eligible === true
  ) {
    return localOperatorFile;
  }

  const approvedDirectMedia = String(
    reference.approved_direct_media_url ||
      reference.approvedDirectMediaUrl ||
      "",
  ).trim();
  if (approvedDirectMedia) return approvedDirectMedia;

  const discoveredDirectMedia = String(
    reference.direct_media_url_if_available ||
      reference.directMediaUrlIfAvailable ||
      reference.direct_media_url ||
      "",
  ).trim();
  if (discoveredDirectMedia) {
    const discoveredKind = mediaSourceUrlKindFields(discoveredDirectMedia);
    if (
      discoveredKind.segment_validation_eligible === true ||
      reference.segment_validation_eligible === true
    ) {
      return discoveredDirectMedia;
    }
  }
...
function officialIntakeSteamTarget(reference = {}, story = {}) {
  if (!reference || typeof reference !== "object") return null;
  if (reference.source_verified === false) return null;
  const sourceUrl =
    reference.official_source_url ||
    reference.source_url ||
    reference.sourceUrl ||
    reference.reference_url ||
    "";
  const appId = extractSteamAppId(sourceUrl);
  if (!appId) return null;
  const providerText = String(
    `${reference.provider || ""} ${reference.provenance?.source || ""} ${reference.source_type || ""}`,
  ).toLowerCase();
  if (!/official_intake|operator_official_source_intake|platform_storefront/.test(providerText)) {
    return null;
  }
  const entity =
    reference.entity ||
    reference.exact_subject_group ||
  ...
  return {
    provider: "steam",
    story_id: story?.id || reference.story_id || null,
    entity,
    store_app_id: String(appId),
    store_app_title: reference.movie_name || reference.source_title || entity || null,
    store_app_slug: null,
    store_matched_query: reference.entity || entity || null,
    provenance_source_type: "official_source_intake_steam_storefront",
  };
}

function buildOfficialIntakeSteamTargets(references = [], story = {}) {
  return (Array.isArray(references) ? references : [])
    .map((reference) => officialIntakeSteamTarget(reference, story))
    .filter(Boolean);
}

function dedupeSteamTargets(targets = []) {
  const seen = new Set();
  const out = [];
  for (const target of Array.isArray(targets) ? targets : []) {
    const key = String(target?.store_app_id || "").trim();
...
function isOfficialIntakeSteamTarget(target = {}) {
  return String(target?.provenance_source_type || "").toLowerCase() === "official_source_intake_steam_storefront";
}

function steamTargetSubjectLabel(target = {}) {
  return (
    target.entity ||
    target.exact_subject_group ||
    target.store_app_title ||
    target.store_matched_query ||
    ""
  );
}

function preferExactOfficialIntakeSteamTargets(targets = []) {
  const list = Array.isArray(targets) ? targets : [];
  const officialIntakeTargets = list.filter(isOfficialIntakeSteamTarget);
  if (!officialIntakeTargets.length) return list;

  return list.filter((target) => {
    if (isOfficialIntakeSteamTarget(target)) return true;
    const targetLabel = steamTargetSubjectLabel(target);
    if (!normaliseLabel(targetLabel)) return true;

    return !officialIntakeTargets.some((officialTarget) => {
      if (String(officialTarget.store_app_id || "") === String(target.store_app_id || "")) return false;
      const officialLabel = steamTargetSubjectLabel(officialTarget);
      return isNestedTargetEntity(targetLabel, officialLabel);
    });
  });
}

function preferExactOfficialIntakeTargetEntities(targetEntities = [], officialIntakeSteamTargets = []) {
  const existingEntities = uniqueStrings(targetEntities);
  const officialEntities = uniqueStrings(
    (Array.isArray(officialIntakeSteamTargets) ? officialIntakeSteamTargets : [])
      .map(steamTargetSubjectLabel)
      .filter(Boolean),
  );
  if (!officialEntities.length) return existingEntities;
  if (!existingEntities.length) return collapseNestedTargetEntities(officialEntities);

  const promotedEntities = [];
  const retainedEntities = [];
  for (const entity of existingEntities) {
    const exactOfficialEntities = officialEntities.filter((officialEntity) =>
      !sameGameEntity(entity, officialEntity) && isNestedTargetEntity(entity, officialEntity),
    );
    if (exactOfficialEntities.length) {
      promotedEntities.push(...exactOfficialEntities);
    } else {
      retainedEntities.push(entity);
    }
  }
  return collapseNestedTargetEntities([...promotedEntities, ...retainedEntities]);
}
...
function comparisonSignalText(story) {
  return storyComparisonText(story)
    .replace(/\b(?:never|don t|dont|do not)\s+miss\s+a\s+beat\b/g, " ")
    .replace(/\bmiss\s+a\s+beat\b/g, " ");
}
...
function isRankComparisonOnlyTarget(story, entity) {
  ...
  const text = comparisonSignalText(story);
...
function storyTargetEntities(story) {
  const graphEntities = collapseNestedTargetEntities(buildSubjectGraph(story).required_subject_groups).filter(
    (entity) => !isRankComparisonOnlyTarget(story, entity),
  );
  const canonical = String(inferredStoryEntity(story) || "").trim();
  if (!canonical) return graphEntities;
  const canonicalKey = normaliseLabel(canonical);
  if (!canonicalKey) return graphEntities;
  const explicitCanonical =
    story?.canonical_game ||
    story?.canonical_subject ||
    story?.game_title ||
    story?.primary_entity;
  if (!explicitCanonical) return graphEntities;
  return collapseNestedTargetEntities([
    canonical,
    ...graphEntities.filter((entity) => sameGameEntity(entity, canonical)),
  ]);
}

function hasExplicitCanonicalStoryTarget(story = {}) {
  return Boolean(
    String(story.canonical_game || "").trim() ||
      String(story.canonical_subject || "").trim() ||
      String(story.game_title || "").trim() ||
      String(story.primary_entity || "").trim(),
  );
}

function sameGameEntity(left, right) {
  const leftKey = normaliseLabel(left);
  const rightKey = normaliseLabel(right);
  if (!leftKey || !rightKey) return false;
  if (leftKey === rightKey) return true;
  const aliases = [
    ["gta", "gta 5", "grand theft auto v", "grand theft auto v enhanced"],
    ["gta 6", "grand theft auto vi"],
  ];
  return aliases.some((group) => group.includes(leftKey) && group.includes(rightKey));
}

function steamTargetMatchesStoryTargets(target, targetEntities = []) {
  const allowed = Array.isArray(targetEntities) ? targetEntities : [];
  if (!allowed.length) return true;
  return allowed.some((entity) =>
    [
      target.entity,
      target.store_app_title,
      target.store_app_slug,
      target.store_matched_query,
    ].some((value) => sameGameEntity(value, entity)),
  );
}
  ...
  return officialSourceIntakeRows(report)
    ...
    .map((reference) => {
      ...
      return {
        ...
        downloads_allowed:
          localOfficialYoutubeMaster && reference.downloads_allowed === true,
        ...
        provenance: {
          ...provenance,
          source: provenance.source || "operator_official_source_intake",
          source_url_kind: urlKind.source_url_kind,
          segment_validation_eligible: urlKind.segment_validation_eligible,
          segment_validation_ineligible_reason: urlKind.segment_validation_ineligible_reason,
        },
...
async function buildOfficialTrailerReferencePlan(story, options = {}) {
  ...
  const initialTargetEntities = storyTargetEntities(story);
  const officialIntakeReferences = officialIntakeReferencesForStory(
    options.officialSourceIntakeReport,
    story,
  );
  const officialIntakeSteamTargets = buildOfficialIntakeSteamTargets(officialIntakeReferences, story);
  const targetEntities = preferExactOfficialIntakeTargetEntities(initialTargetEntities, officialIntakeSteamTargets);
  const verifiedStoreTargets = dedupeSteamTargets(preferExactOfficialIntakeSteamTargets([
    ...officialIntakeSteamTargets,
    ...buildSteamTargets(story),
  ])).filter((target) =>
    !hasExplicitCanonicalStoryTarget(story) || steamTargetMatchesStoryTargets(target, targetEntities),
  );
