...
function buildSearchQueries(story, acquisitionPlan) {
...
function readinessFor({ topicality, references, targets, searchQueries }) {
...
async function buildOfficialTrailerReferencePlan(story, options = {}) {
  ...
  const igdbReferences = sourceAssets(story)
    ...
    .filter(Boolean);
  const references = [...steamReferences, ...igdbReferences];
  const searchQueries = buildSearchQueries(story, acquisitionPlan);
  ...
  return {
    ...
    references,
    lookup_results,
    planned_searches: searchQueries.map((query) => ({
      query,
...
function renderOfficialTrailerReferenceMarkdown(report) {
  ...
  lines.push("");
  lines.push("| story | readiness | targets | refs | searches | blockers |");
  lines.push("| --- | --- | ---: | ---: | ---: | --- |");
  for (const plan of report.plans) {
    lines.push(
      [
        ...
        plan.verified_store_targets.length,
