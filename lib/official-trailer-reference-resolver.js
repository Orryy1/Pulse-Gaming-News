...
function ratingReferenceText(movie) {
  return [
    movie?.name,
    movie?.title,
    movie?.description,
...
function ratingBoardRejectReason(movie) {
  return RATING_REFERENCE_RE.test(ratingReferenceText(movie)) ? "rating_board_reference" : null;
}
...
async function buildOfficialTrailerReferencePlan(story, options = {}) {
  ...
  const igdbReferences = sourceAssets(story)
    ...
    .filter(Boolean);
  const resolvedReferences = [...steamReferences, ...igdbReferences];
...
async function buildOfficialTrailerReferenceReport(stories = [], options = {}) {
  ...
  return {
    ...
    summary: {
      ...
      alternate_official_reference_required: byReadiness.alternate_official_reference_required || 0,
...
function renderOfficialTrailerReferenceMarkdown(report) {
  ...
  lines.push("");
  lines.push("| story | readiness | targets | missing targets | refs | excluded | searches | blockers | warnings |");
  lines.push("| --- | --- | ---: | --- | ---: | ---: | ---: | --- | --- |");
