...
function buildSteamTargets(story) {
  ...
  for (const asset of sourceAssets(story)) {
    ...
    targets.push({
      ...
      entity,
      store_app_id: appId,
      store_app_title: asset.store_app_title || exactSubject || null,
...
function buildSearchQueries(story, acquisitionPlan, targetEntities = []) {
  ...
  const hasTargets = Array.isArray(targetEntities) && targetEntities.length > 0;
  const titleWords = normaliseLabel(title).split(" ").filter(Boolean);
  const looksTitleFragmentLed = (query) => {
    if (!hasTargets || titleWords.length < 3) return false;
  ...
  const entityQuery = (query) => {
    ...
    return targetEntities.some((entity) => queryKey.startsWith(`${normaliseLabel(entity)} `));
  };
  return Array.from(
    new Set(
      [
        ...
        !hasTargets && title ? `${title} official trailer` : null,
        !hasTargets && title ? `${title} gameplay trailer` : null,
      ].filter(Boolean),
    ),
  ).slice(0, 18);
}
