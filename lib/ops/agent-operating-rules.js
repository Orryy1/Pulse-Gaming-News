...
function documentedNpmCommands(content = "") {
  return [
    ...new Set(
      [...String(content).matchAll(/`npm run ([^`\s]+)/g)].map(
        (match) => match[1],
      ),
    ),
  ].sort();
...
async function inspectDocumentedNpmCommands({
  rootDir,
  content,
} = {}) {
  const names = documentedNpmCommands(content);
  const packagePath = path.join(rootDir, "package.json");
  let scripts = {};
  if (names.length > 0 && (await fs.pathExists(packagePath))) {
    try {
      scripts = (await fs.readJson(packagePath))?.scripts || {};
    } catch {
      scripts = {};
    }
  }
  return names.map((name) => {
    ...
    const nodeMatch =
      typeof command === "string"
        ? command.match(/^node\s+([^\s]+\.js)(?:\s|$)/)
        : null;
    const entrypoint = nodeMatch ? nodeMatch[1] : null;
    const entrypointExists = entrypoint
      ? fs.existsSync(path.join(rootDir, entrypoint))
      : true;
    return {
      name,
      command: typeof command === "string" ? command : null,
      entrypoint,
      resolved: typeof command === "string" && entrypointExists,
    };
  });
}
...
async function buildAgentOperatingRulesReport({ rootDir = process.cwd(), generatedAt = new Date().toISOString() } = {}) {
  ...
  const npmCommands = await inspectDocumentedNpmCommands({
    rootDir,
    content,
  });
  const missingNpmCommands = npmCommands.filter((command) => !command.resolved);
  ...
  if (missingNpmCommands.length > 0) {
    rejectionReasons.push("documented_npm_command_missing");
  }
  ...
  return {
    ...
    summary: {
      ...
      documented_npm_commands: npmCommands.length,
      missing_documented_npm_commands: missingNpmCommands.length,
    },
    safety: {
    ...
    documented_npm_commands: npmCommands,
    missing_documented_npm_commands: missingNpmCommands.map(
...
function renderAgentOperatingRulesMarkdown(report) {
  const lines = [
    ...
    `- Documented npm commands: ${report.summary.documented_npm_commands}`,
    `- Missing documented npm commands: ${report.summary.missing_documented_npm_commands}`,
    "",
    "## Safety defaults",
    "",
  ...
  if (report.missing_documented_npm_commands.length > 0) {
    lines.push("", "## Missing documented npm commands", "");
    for (const command of report.missing_documented_npm_commands) {
      lines.push(`- ${command}`);
    }
  }
...
module.exports = {
  REQUIRED_SECTIONS,
  CRITICAL_RULES,
  documentedNpmCommands,
  inspectDocumentedNpmCommands,
