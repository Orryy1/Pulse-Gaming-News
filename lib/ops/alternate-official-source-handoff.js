...
function manualSourceIntake({ entity, storyId, hasExhaustedSteam = false, verifiedTargets = [] } = {}) {
  return {
    ...
    safe_next_commands: [
      `npm run media:resolve-trailers -- --story-id ${storyId} --no-latest-report --segment-validation-report test/output/official_trailer_segment_validation_apply_local.json --exhausted-source-family-threshold 5`,
  ...
  return entities.map((entity) => {
    ...
    return {
      ...
      next_actions: [
        ...
        `Rerun: npm run media:resolve-trailers -- --story-id ${gap.story_id} --no-latest-report --segment-validation-report test/output/official_trailer_segment_validation_apply_local.json --exhausted-source-family-threshold 5`,
