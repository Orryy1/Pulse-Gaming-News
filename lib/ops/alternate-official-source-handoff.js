...
function manualSourceIntake({ entity, storyId, hasExhaustedSteam = false, verifiedTargets = [] } = {}) {
  ...
  return {
    ...
    optional_fields: ["direct_media_url_if_available", "direct_media_url_kind", "direct_media_url_notes"],
    url_handling: {
      official_source_url:
        "Official provenance/reference page or URL. HTML pages and official YouTube links are allowed here but remain reference-only.",
      direct_media_url_if_available:
        "Optional direct .mp4/.webm/.mov/.m3u8/.mpd media or manifest URL. Leave blank unless the official source exposes a direct playable media URL.",
      direct_media_url_notes:
        "Do not paste YouTube watch URLs, embed URLs, storefront pages or publisher HTML pages into the direct media field.",
      direct_media_url_required_for_segment_validation: true,
      segment_validation_eligible_url_kinds: ["direct_video", "hls_manifest", "dash_manifest"],
      reference_only_url_kinds: [
        "youtube_watch",
        "youtube_short",
        "youtube_embed",
        "youtube_page",
...
function buildSourceIntakeTemplate(rows = []) {
  return array(rows).map((row) => ({
    ...
    entity: row.entity,
    official_source_url: "",
    official_source_url_usage:
      "Use for provenance/reference URLs. HTML pages, storefront pages and official YouTube links are allowed here but remain reference-only.",
    direct_media_url_if_available: "",
    direct_media_url_kind: "",
    direct_media_url_notes:
      "Optional. Only fill with a direct .mp4/.webm/.mov/.m3u8/.mpd media or manifest URL exposed by the official source; otherwise leave blank.",
    source_owner: "",
    source_type: "official_publisher_or_developer_trailer_page",
  ...
  return entities.map((entity) => {
    ...
    return {
      ...
      next_actions: [
        ...
        `Validate operator intake: npm run media:intake-official-sources -- --input test/output/official_source_intake_template.json --story-id ${gap.story_id}`,
        ...
        `If a new official reference exists, rerun: npm run media:validate-trailer-segments -- --story-id ${gap.story_id} --apply-local --deep-scan --reference-report ${storyReferenceReportRelativePath(gap.story_id)} --previous-validation-report test/output/official_trailer_segment_validation_apply_local.json --merge-previous --exhausted-source-family-threshold 5 --max-segments 90 --candidate-windows-per-source 6`,
...
function renderAlternateOfficialSourceHandoffMarkdown(report = {}) {
  ...
  for (const row of array(report.rows)) {
    ...
    if (row.manual_source_intake) {
      ...
      if (array(row.manual_source_intake.optional_fields).length) {
        lines.push("", "Optional fields:");
        for (const field of row.manual_source_intake.optional_fields) lines.push(`- ${field}`);
      }
      if (row.manual_source_intake.url_handling) {
        lines.push(
          "",
          "URL handling:",
          `- official_source_url: ${row.manual_source_intake.url_handling.official_source_url}`,
          `- direct_media_url_if_available: ${row.manual_source_intake.url_handling.direct_media_url_if_available}`,
  ...
  if (array(report.source_intake_template?.entries).length) {
    ...
    lines.push("");
    lines.push("| Story | Entity | Source type | Source family | Direct media field |");
    lines.push("| --- | --- | --- | --- | --- |");
    for (const entry of report.source_intake_template.entries) {
      lines.push(
        `| ${mdCell(entry.story_id)} | ${mdCell(entry.entity)} | ${mdCell(entry.source_type)} | ${mdCell(
          entry.source_family,
        )} | ${mdCell(entry.direct_media_url_if_available || "blank unless direct media URL is available")} |`,
      );
    }
  }
