...
function manualSourceIntake({ entity, storyId, hasExhaustedSteam = false, verifiedTargets = [] } = {}) {
  return {
    ...
    safe_next_commands: [
      `npm run media:resolve-trailers -- --story-id ${storyId} --segment-validation-report test/output/official_trailer_segment_validation_apply_local.json --exhausted-source-family-threshold 5`,
...
function entityHandoffRows(gap = {}, referencePlan = {}) {
  ...
  return entities.map((entity) => {
    ...
    return {
      ...
      entity,
      blocker,
      motion_status: entityStatus.status || "unknown",
      ...
      next_actions: [
        ...
        `Rerun: npm run media:resolve-trailers -- --story-id ${gap.story_id} --segment-validation-report test/output/official_trailer_segment_validation_apply_local.json --exhausted-source-family-threshold 5`,
...
function buildAlternateOfficialSourceHandoffReport({ motionGapReport = {}, referenceReport = {} } = {}) {
