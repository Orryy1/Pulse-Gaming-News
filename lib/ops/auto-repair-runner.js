...
function extractAutoRepairPlanItems(autoRepairPlan = {}) {
  ...
  const sourceBoundRewriteItems = asArray(autoRepairPlan.source_bound_rewrite_work_orders)
    .map((item) => normaliseRepairPlanItem(item, {
      source: "local_script_extension.source_bound_rewrite_work_orders",
    }));
  if (sourceBoundRewriteItems.length) return dedupeRepairPlanItems(sourceBoundRewriteItems);
...
function mergeRunPlanItems(items = []) {
  const merged = [];
  const byCommand = new Map();
  for (const item of asArray(items)) {
    const key = [
      cleanText(item.story_id).toLowerCase(),
      cleanText(item.command).toLowerCase(),
    ].join("|");
    const existing = byCommand.get(key);
    if (!existing) {
      const prepared = {
        ...item,
        covered_repair_lanes: unique([item.repair_lane]),
        covered_blocker_types: unique([item.blocker_type]),
        covered_validation_commands: unique([item.validation_command]),
        expected_output: unique(asArray(item.expected_output)),
        required_artefact_paths: unique([
          item.required_artefact_path,
          ...asArray(item.required_artefact_paths),
        ]),
      };
      prepared.required_artefact_path =
        prepared.required_artefact_path ||
        prepared.required_artefact_paths[0] ||
        "";
      byCommand.set(key, prepared);
      merged.push(prepared);
      continue;
    }
    existing.covered_repair_lanes = unique([
      ...existing.covered_repair_lanes,
      item.repair_lane,
    ]);
    existing.covered_blocker_types = unique([
      ...existing.covered_blocker_types,
      item.blocker_type,
    ]);
    existing.covered_validation_commands = unique([
      ...existing.covered_validation_commands,
      item.validation_command,
    ]);
    existing.expected_output = unique([
      ...asArray(existing.expected_output),
      ...asArray(item.expected_output),
    ]);
    existing.required_artefact_paths = unique([
      ...asArray(existing.required_artefact_paths),
      item.required_artefact_path,
      ...asArray(item.required_artefact_paths),
    ]);
    existing.required_artefact_path =
      existing.required_artefact_path ||
      item.required_artefact_path ||
      existing.required_artefact_paths[0] ||
      "";
  }
  return merged;
}
...
function buildAutoRepairRunPlan(autoRepairPlan = {}, {
  ...
  const selected = selectAutoRepairItems(sourceItems, { lane, storyIds });
  ...
  const deduplicatedItems = mergeRunPlanItems(preparedItems);
  const max = numberOrZero(limit);
  const items = max > 0 ? deduplicatedItems.slice(0, max) : deduplicatedItems;
  ...
  return {
    ...
    summary: {
      ...
      duplicate_commands_collapsed: preparedItems.length - deduplicatedItems.length,
...
function findNpmCliPath(env = process.env, fileExists = fs.existsSync) {
  const explicit = cleanText(env.npmCliPath || env.npm_cli_path);
...
function resolveExecutableCommand(command = {}, env = process.env) {
  ...
  const nodeExecPath = cleanText(env.nodeExecPath || process.execPath);
  const inferredNpmCliPath = findNpmCliPath(env);
  const npmCliPath = npmExecPath || inferredNpmCliPath;
  if (npmCliPath && nodeExecPath) {
    return {
      executable: nodeExecPath,
      args: [npmCliPath, ...asArray(command.args)],
      shell: false,
    };
  }
  return {
...
function executionStructuredFailure(stdout = "", stderr = "") {
  const output = cleanText(`${stdout} ${stderr}`);
  const failedCount = /"failed"\s*:\s*([1-9]\d*)/.exec(output);
  if (failedCount) return `child_report_failed:${failedCount[1]}`;
  if (/"status"\s*:\s*"failed"/.test(output)) {
    return "child_report_failed";
  }
  return "";
}
...
async function executeAutoRepairRunPlan(runPlan = {}, {
  ...
  for (const item of asArray(runPlan.items)) {
    ...
    const structuredFailure = exitCode === 0
      ? executionStructuredFailure(result?.stdout, result?.stderr || result?.error)
      : "";
    const noEffect = exitCode === 0 && !structuredFailure && executionNoEffect(result?.stdout, result?.stderr || result?.error);
    const planGenerated = exitCode === 0 && !structuredFailure && !noEffect && executionPlanGenerated(result?.stdout, result?.stderr || result?.error);
    const status = exitCode !== 0 || structuredFailure
      ? "failed"
      : noEffect
        ? "executed_no_effect"
        : planGenerated
    ...
    results.push({
      ...
      command: item.command,
      exit_code: exitCode,
      reason: structuredFailure || (exitCode !== 0 ? cleanText(result?.error || result?.stderr) : ""),
