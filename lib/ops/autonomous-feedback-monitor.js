...
function transcriptAudienceStoryKey(story = {}) {
  ...
  return [storyId, artifactDir, title].join("|");
}

function mergeTranscriptAudienceReports(baseReport = {}, selectedReport = null) {
  if (!selectedReport || !asArray(selectedReport.stories).length) return baseReport || {};
  const mergedStories = [];
  const byKey = new Map();
  for (const story of [...asArray(baseReport?.stories), ...asArray(selectedReport?.stories)]) {
    const key = transcriptAudienceStoryKey(story);
    if (!key.replaceAll("|", "")) continue;
    byKey.set(key, story);
  }
  for (const story of byKey.values()) mergedStories.push(story);
  const pass = mergedStories.filter((story) => clean(story.verdict) === "pass").length;
  const rewriteRequired = mergedStories.filter((story) => clean(story.verdict) && clean(story.verdict) !== "pass").length;
  const viralVerdictCounts = {};
  for (const story of mergedStories) {
    const verdict = clean(story.viral_verdict || story.viralVerdict);
    if (!verdict) continue;
    viralVerdictCounts[verdict] = (viralVerdictCounts[verdict] || 0) + 1;
  }
  return {
    ...(baseReport || {}),
    generated_at: selectedReport.generated_at || baseReport?.generated_at || new Date().toISOString(),
    summary: {
      ...(baseReport?.summary || {}),
      total: mergedStories.length,
      pass,
      rewrite_required: rewriteRequired,
      viral_verdict_counts: Object.keys(viralVerdictCounts).length
        ? viralVerdictCounts
        : baseReport?.summary?.viral_verdict_counts,
    },
    stories: mergedStories,
    selected_artifact_audit: {
      generated_at: selectedReport.generated_at || null,
      story_count: asArray(selectedReport.stories).length,
      pass: Number(selectedReport.summary?.pass || 0),
      rewrite_required: Number(selectedReport.summary?.rewrite_required || 0),
    },
  };
}
...
async function runAutonomousFeedbackMonitor({
  ...
  if (!loadedTranscriptAudienceReport) {
    try {
      ...
      const selectedArtifactDirs = selectedArtifactDirsFromGuardedPreflight(
        loadedGuardedDispatchPreflightReport,
        loadedGuardedSelectedAction,
      );
      if (selectedArtifactDirs.length) {
        const selectedTranscriptAudienceReport = await auditGeneratedTranscripts({
          root: ROOT,
          artifactDirs: selectedArtifactDirs,
        });
        loadedTranscriptAudienceReport = mergeTranscriptAudienceReports(
          loadedTranscriptAudienceReport,
          selectedTranscriptAudienceReport,
        );
      }
