...
function buildTranscriptAudienceFeedback({
  ...
  const items = stories
    ...
    .map((story) => {
      ...
      const currentCandidate = candidates.has(storyId);
      const blocksPublishing = selectedForWindow;
      return {
        story_id: storyId,
...
function buildTtsCaptionFeedback(materializationReport = {}, { candidateReport = {} } = {}) {
...
function buildAutonomousFeedbackReport({
  ...
  const discordFeedback = classifyDiscordFeedback({
    ...
    candidateReport,
    generatedAt,
    selectedStoryIds: [storyIdFromActionId(guardedSelection.action_id)].filter(Boolean),
  });
  ...
  const ttsCaptionFeedback = buildTtsCaptionFeedback(ttsCaptionReport, { candidateReport });
...
async function runAutonomousFeedbackMonitor({
  ...
  const loadedCandidateReport = candidateReport || (await readJsonIfExists(candidatePath)) || {};
  ...
  if (!loadedIngestedDiscordFeedbackReport) {
    try {
      ...
      loadedIngestedDiscordFeedbackReport = await runDiscordFeedbackIngestion({
        ...
        selectedStoryIds: [
          storyIdFromActionId(loadedNormalOperationsReport.guarded_selection?.action_id),
        ].filter(Boolean),
