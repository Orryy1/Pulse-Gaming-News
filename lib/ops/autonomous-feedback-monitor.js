...
function classifyDiscordFeedback({ discordDigestPayload = null, candidateReport = {}, generatedAt = new Date().toISOString() } = {}) {
...
function hasPlatformEvidenceAfter(platformPosts = [], isoDate) {
  const date = parseDate(isoDate);
  if (!date) return false;
  return asArray(platformPosts).some((post) => {
    const created = parseDate(post.created_at || post.published_at || post.updated_at);
    return created && created.getTime() >= date.getTime();
  });
}
...
function buildPostWindowFeedback({ recentJobs = [], platformPosts = [] } = {}) {
  ...
  if (job && lower(job.status) === "done") {
    ...
    if (!hasPlatformEvidenceAfter(platformPosts, completedAt)) {
      anomalies.push({
...
function buildTtsCaptionFeedback(materializationReport = {}) {
  const failedJobs = asArray(materializationReport.jobs).filter((job) =>
    clean(job.status).toLowerCase() === "failed" || clean(job.error),
  );
  const failedCount = Number(materializationReport.summary?.failed_count || failedJobs.length || 0);
  const items = failedJobs.map((job) => {
    ...
    return {
      story_id: clean(job.story_id || job.id || "unknown"),
...
function buildAutonomousFeedbackReport({
  ...
  const ttsCaptionFeedback = buildTtsCaptionFeedback(ttsCaptionReport);
  ...
  const schedulerWindow = schedulerWindowReadiness || {};
  ...
  const blockers = [
    ...
    ...asArray(readiness.blockers).map((item) => `publish_readiness:${item}`),
  ...
  else if (normaliseVerdict(readiness.verdict) === "red") currentAction = "hold_scheduler_and_clear_publish_readiness";
  ...
  return {
    ...
    publish_readiness: {
      verdict: normaliseVerdict(readiness.verdict),
      blockers: asArray(readiness.blockers),
      advisory: asArray(readiness.advisory),
    },
    candidate_buffer: {
    ...
    scheduler: {
      ...
      next_safe_publish_at_utc: schedulerWindow.next_publish_window_utc || postWindow.next_safe_publish_at_utc || null,
      selected_action: schedulerWindow.selected_action?.action_id || guardedSelection.action_id || null,
...
function formatAutonomousFeedbackMarkdown(report = {}) {
  ...
  lines.push(
    ...
    "## Market Intelligence",
    "",
    `- Candidate supply: ${report.market_intelligence?.candidate_supply?.verdict || "unknown"} (${report.market_intelligence?.candidate_supply?.green_ready_candidates || 0} GREEN, ${report.market_intelligence?.candidate_supply?.fresh_source_backed_stories_24h || 0} fresh source-backed)`,
...
function formatAutonomousFeedbackDiscord(report = {}) {
  ...
  const lines = [
    ...
    `Market: candidate=${report.market_intelligence?.candidate_supply?.verdict || "unknown"} | competitors=${report.market_intelligence?.competitor_forensics?.verdict || "unknown"} | quality=${report.market_intelligence?.competitor_quality_gate?.verdict || "unknown"} | commercial=${report.market_intelligence?.commercial_learning?.verdict || "unknown"}`,
  ];
...
async function recentJobsFromDb({ limit = 20 } = {}) {
  try {
    ...
    return db
      .prepare(
        `SELECT id, kind, status, created_at, updated_at, completed_at, result_summary
           FROM jobs
...
async function runAutonomousFeedbackMonitor({
  ...
  const candidatePath = path.join(ROOT, "test", "output", "next_publish_candidates.json");
  ...
  const report = buildAutonomousFeedbackReport({
    generatedAt,
    normalOperationsReport: normalOperationsReport || (await readJsonIfExists(normalOpsPath)) || {},
    schedulerWindowReadiness: await readJsonIfExists(schedulerWindowPath),
    candidateReport: candidateReport || (await readJsonIfExists(candidatePath)) || {},
