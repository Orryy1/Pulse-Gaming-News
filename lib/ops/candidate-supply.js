...
function buildCandidateSupplyReport({
  ...
  const authoritativeCandidateIds = new Set(
    candidates
      .map((candidate) => clean(candidate.id || candidate.story_id))
      .filter(Boolean),
  );
  const freshSourceBackedIntakeRows = freshSourceBacked.map((story) => {
    const storyId = clean(story.id || story.story_id);
    const schedulerCandidate = authoritativeCandidateIds.has(storyId);
    return {
      story_id: storyId,
      title: clean(story.title),
      source_name: clean(
        story.source_name || story.subreddit || story.primary_source,
      ),
      scheduler_candidate: schedulerCandidate,
      reason: schedulerCandidate
        ? "in_authoritative_candidate_report"
        : "not_in_authoritative_candidate_report",
    };
  });
  const freshSourceBackedSchedulerCandidateCount =
    freshSourceBackedIntakeRows.filter(
      (row) => row.scheduler_candidate,
    ).length;
  ...
  const blockers = [];
  const warnings = [];
  const freshSourceBackedIntakeWarnings = [];
  ...
  if (
    freshSourceBacked.length > 0 &&
    freshSourceBackedSchedulerCandidateCount === 0
  ) {
    freshSourceBackedIntakeWarnings.push(
      `fresh_source_backed_discovered_but_not_scheduler_candidates:${freshSourceBacked.length}`,
    );
  }
  ...
  return {
    ...
    summary: {
      ...
      fresh_source_backed_scheduler_candidate_count:
        freshSourceBackedSchedulerCandidateCount,
    ...
    fresh_source_backed_intake_truth: {
      discovered_count: freshSourceBacked.length,
      scheduler_candidate_count:
        freshSourceBackedSchedulerCandidateCount,
      warnings: freshSourceBackedIntakeWarnings,
      rows: freshSourceBackedIntakeRows,
    },
