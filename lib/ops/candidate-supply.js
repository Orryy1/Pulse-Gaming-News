...
function buildCandidateSupplyReport({
  ...
  const cleanGreenReadyCandidates = transcriptCleanReadyCandidates.filter((candidate) => {
  ...
  if (motionRepairableCandidates.length && cleanGreenReadyCandidates.length < greenReadyTarget) {
...
function formatCandidateSupplyMonitorDiscord(report = {}) {
  ...
  const attentionHeld = Math.max(0, transcriptCleanGreen - cleanGreen);
  return [
