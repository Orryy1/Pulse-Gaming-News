...
function buildRefillActionPlan({
  ...
  const recommendedRefillLimit = needed
    ...
    : 0;
  const recommendedRssPerFeed = recommendedRefillLimit >= 16 ? 6 : 4;
  ...
  return {
    ...
    safe_refill_command: needed
      ? `npm run ops:fresh-production-refill -- --json --limit ${recommendedRefillLimit} --rss-per-feed ${recommendedRssPerFeed}`
      : "",
    safety: {
...
function proofPackageVisualSafetyBlockers(renderManifest = {}, motionManifest = {}) {
  ...
  if (cardWindows.some((window) => window.duration_s < MIN_PROOF_PACKAGE_READABLE_CARD_DURATION_S)) {
