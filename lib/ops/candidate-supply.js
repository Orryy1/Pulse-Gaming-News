...
function candidateSupplyMonitorNeedsRepair(report = {}) {
  ...
  return asArray(report.warnings).some((warning) =>
    /green_ready_candidates_below_target|durable_green_ready_candidates_below_target|source_safe_candidates_below_target|ready_candidates_expiring|non_ready_candidates_expiring|publish_window_reserve_empty/i.test(warning),
  );
}
...
function candidateSupplyMonitorNeedsFreshIntake(report = {}) {
  ...
  return asArray(report.warnings).some((warning) =>
    /fresh_source_backed_stories_below_daily_target|durable_green_ready_candidates_below_target|ready_candidates_expiring|publish_window_reserve_empty|green_ready_candidates_below_target/i.test(warning),
  );
}
