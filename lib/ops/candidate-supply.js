...
function proofPackageSearchDirsForStory(storyId = "", root = process.cwd()) {
  const base = path.join(root, "output", "fresh-green-refill");
  if (!storyId || !fs.existsSync(base)) return [];
  const runs = fs
...
function sourceStoryFromProofPackageDir(storyId = "", artifactDir = "") {
  ...
  return {
    ...
    source: clean(primarySource.name || canonicalManifest.primary_source || sourceManifest.source || "source_manifest"),
    source_name: clean(primarySource.name || canonicalManifest.primary_source || sourceManifest.source || "source_manifest"),
    source_type: "rss",
    url: sourceUrl,
  ...
  if (capacity.source_manifest || capacity.canonical_story_manifest) {
    ...
    if (sourceUrl && publishedAt) {
      return {
        ...
        source: clean(primarySource.name || canonicalStoryManifest.primary_source || "source_manifest"),
        source_name: clean(primarySource.name || canonicalStoryManifest.primary_source || "source_manifest"),
        source_type: "rss",
        url: sourceUrl,
        source_url: sourceUrl,
        source_published_at: publishedAt,
