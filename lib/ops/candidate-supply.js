...
function candidateAlreadyPublishedOn(candidate = {}, platform) {
  return candidatePlatformList(candidate, "already_published_platforms").includes(lower(platform));
}

function candidateCanCreateNewPlatformUpload(candidate = {}, platform) {
  const target = lower(platform);
...
function motionCapacityFromRow(row = {}) {
  ...
  const currentMotionClips = nonNegativeNumber(
    row.current_motion_clips ?? row.clip_count ?? row.clips ?? row.accepted_clip_count,
  );
  ...
  const readinessStatus = clean(row.readiness_status || row.status || row.render_decision || "unknown");
  const readyByStatus = /ready|green|pass/i.test(readinessStatus) && !/blocked|hold|operator/i.test(readinessStatus);
  const motionReady =
    readyByStatus ||
...
function mergeMotionCapacity(current = null, next = null) {
  ...
  const currentRank = { none: 0, operator_required: 1, medium: 2, high: 3, ready: 4 }[current.repair_priority] || 0;
  const nextRank = { none: 0, operator_required: 1, medium: 2, high: 3, ready: 4 }[next.repair_priority] || 0;
  const nextIsBlockingEvidence = /blocked|hold|failed/i.test(next.readiness_status) || asArray(next.blockers).length > 0;
  const primary = nextIsBlockingEvidence || nextRank >= currentRank ? next : current;
  ...
  return {
    ...
    blockers: unique([...asArray(current.blockers), ...asArray(next.blockers)]),
...
function scoreMotionCapacityProspect(capacity = {}) {
  ...
  const namedEntity = /\b(?:Nintendo|Xbox|PlayStation|Steam|Capcom|Square Enix|Sega|Ubisoft|Bethesda|Doom|Mario|Halo|Forza|Final Fantasy|Resident Evil|Granblue|Dave|Planet Crafter|EA Sports)\b/i.test(capacity.title) ? 8 : 2;
  const score = Math.min(100, Math.round(20 + visualAvailability + namedEntity + (capacity.near_ready ? 12 : 0) + (capacity.direct_media_ready > 0 ? 10 : 0)));
  return {
    story_id: clean(capacity.story_id),
    title: clean(capacity.title || capacity.story_id),
    score,
    freshness_score: 0,
...
function buildCandidateSupplyReport({
  ...
  const sourceSafeCandidates = transcriptCleanReadyCandidates.filter(candidateIsSourceSafe);
  const v4ReadyCandidates = transcriptCleanReadyCandidates.filter(candidateIsV4Ready);
  const cleanCandidateReport = {
    ...candidateReport,
    candidates: candidates.filter((candidate) => !transcriptBlockedReadyIds.has(clean(candidate.id))),
  };
  ...
  const motionCapacityById = buildMotionCapacityIndex(motionCapacityReports);
  const candidatePriorityScorecards = candidates
    .map((candidate) => {
      ...
      const cleanGreen = Boolean(scorecard.publish_ready && !transcriptBlock);
      return {
        ...scorecard,
  ...
  const motionOnlyScorecards = Array.from(motionCapacityById.values())
    ...
    .map(scoreMotionCapacityProspect);
  const priorityScorecards = [...candidatePriorityScorecards, ...motionOnlyScorecards].sort((a, b) => b.score - a.score);
  const readyIds = new Set(transcriptCleanReadyCandidates.map((candidate) => clean(candidate.id)));
  ...
  const freshYoutubeReadyCandidateIds = new Set(
    transcriptCleanReadyCandidates
      .filter((candidate) => candidateCanCreateNewPlatformUpload(candidate, "youtube_shorts"))
  ...
  const catchUpOnlyGreenCandidates = transcriptCleanReadyCandidates.filter(
  ...
  if (transcriptCleanReadyCandidates.length === 0) blockers.push("green_ready_candidate_buffer_empty");
  if (freshYoutubeReadyScorecards.length === 0) blockers.push("fresh_youtube_upload_candidate_buffer_empty");
  if (transcriptCleanReadyCandidates.length < greenReadyTarget) warnings.push(`green_ready_candidates_below_target:${transcriptCleanReadyCandidates.length}/${greenReadyTarget}`);
  ...
  if (motionRepairableCandidates.length && transcriptCleanReadyCandidates.length < greenReadyTarget) {
    warnings.push(`motion_repairable_candidates_available:${motionRepairableCandidates.length}`);
  }
  ...
  return {
    ...
    summary: {
      ...
      green_ready_candidates: transcriptCleanReadyCandidates.length,
    ...
    next_action: blockers.length
      ...
      : motionRepairableCandidates.length && transcriptCleanReadyCandidates.length < greenReadyTarget
...
function formatCandidateSupplyMonitorDiscord(report = {}) {
  ...
  const transcriptHeld = Math.max(0, rawGreen - cleanGreen);
  return [
    ...
    `Clean GREEN: ${cleanGreen}/${report.targets?.green_ready_candidates || 0} (raw preflight ${rawGreen}; transcript-held ${transcriptHeld})`,
