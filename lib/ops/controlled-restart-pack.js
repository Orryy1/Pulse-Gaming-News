...
function storyArtifactDir(root, storyId, storyPackages = [], candidate = {}) {
  const fromPackage = storyPackages.find((item) => clean(item.story_id || item.id) === storyId);
  const exportedPath = clean(candidate.source?.exported_path || candidate.exported_path || candidate.final_mp4_path);
  const dir = clean(
    fromPackage?.artifact_dir ||
      fromPackage?.artifactDir ||
  ...
  if (exportedPath) return path.dirname(rel(root, exportedPath));
  return path.join(root, "output", "goal-proof", "batch", storyId);
}

function readStoryArtefacts(root, storyId, storyPackages = [], candidate = {}) {
  const dir = storyArtifactDir(root, storyId, storyPackages, candidate);
...
function readyStoryFor(plan = {}, storyId = "") {
  return asArray(plan.ready_stories || plan.readyStories)
    .find((story) => clean(story.story_id || story.storyId || story.id) === storyId) || {};
}

function alreadyPublishedEnabledPlatforms(candidate = {}, strictDryRunPlan = {}, storyId = "") {
  const readyStory = readyStoryFor(strictDryRunPlan, storyId);
  return new Set([
    ...asArray(candidate.source?.already_published_platforms),
    ...asArray(candidate.already_published_platforms),
    ...asArray(readyStory.already_published_platforms),
  ].map(clean).filter((platform) => REQUIRED_ENABLED_PLATFORMS.includes(platform)));
}

function enabledActionIsReviewReady(action = {}) {
  const liveGate = clean(action.live_execution_gate);
  const legacyOperatorReviewReady =
    action.requires_human_review_before_live_publish === true &&
    liveGate === "operator_human_review_required";
  const autonomousGuardedDispatchReady =
    liveGate === "guarded_dispatch_ready" &&
  ...
  return (
    ...
    (legacyOperatorReviewReady || autonomousGuardedDispatchReady) &&
...
function internalPublicCopyReason(script = "", title = "") {
  ...
  return qa.failures.some((failure) =>
    ...
    failure === "script_coherence:vague_filler:public_narration_meta_language" ||
...
function timestampEvidence(candidate = {}, artefacts = {}) {
  ...
  const captions = artefacts.captions || {};
  const narration = artefacts.narration || {};
  return {
    preflight_result: clean(timestampCheck.result || candidate.preflight_qa?.status),
    preflight_evidence: evidence,
    word_timestamp_source: clean(
      narration.word_timestamp_source ||
        captions.timing_source ||
        evidence.source,
    ),
    word_timestamp_count: Number(
      narration.word_timestamp_count ||
        captions.word_timestamp_count ||
        captions.word_count ||
        evidence.word_timestamp_count ||
        0,
    ),
    caption_status: clean(captions.status || captions.verdict),
    caption_blockers: asArray(captions.blockers),
  };
}
...
function timestampEvidencePasses(evidence = {}) {
  ...
  return (
    ...
    /whisper|forced|word|alignment/.test(source) &&
...
function premiumLaneObject(artefacts = {}) {
  const render = artefacts.render || {};
  return render.premiumLane && typeof render.premiumLane === "object"
    ? render.premiumLane
    : {};
}

function hyperframesShellGate(artefacts = {}) {
  const render = artefacts.render || {};
  const lane = premiumLaneObject(artefacts);
  return (
    render.hyperframesPremiumShellGate ||
    render.hyperframes_premium_shell_gate ||
    render.premium_shell_gate ||
...
function hyperframesCardCount(artefacts = {}) {
  const render = artefacts.render || {};
  const lane = premiumLaneObject(artefacts);
  return firstNumber(
    render.hyperframesCardCount,
    render.hyperframes_card_count,
    lane.hyperframesCardCount,
    lane.hyperframes_card_count,
  );
}

function hasHyperframesPremiumLane(artefacts = {}) {
  const render = artefacts.render || {};
  const lane = premiumLaneObject(artefacts);
  const text = [
    render.renderer,
    render.rendererSplit,
    render.renderer_split,
    render.render_lane,
    render.lane,
    render.premiumLane,
    lane.renderer,
    ...
    .join(" ");
  return (
    hyperframesCardCount(artefacts) > 0 ||
    Boolean(render.hyperframesPremiumShellGate || render.hyperframes_premium_shell_gate || render.premium_shell_gate) ||
    text.includes("hyperframes")
  );
}

function hyperframesPremiumShellPasses(artefacts = {}) {
  if (!hasHyperframesPremiumLane(artefacts)) return true;
  const render = artefacts.render || {};
  const lane = premiumLaneObject(artefacts);
  const gate = hyperframesShellGate(artefacts);
  const verdict = clean(
    gate.verdict ||
      gate.status ||
      render.premium_shell_verdict ||
      lane.premium_shell_verdict,
  ).toLowerCase();
  const passCount = firstOptionalNumber(
    gate.passCount,
    gate.pass_count,
    render.premium_shell_pass_count,
    lane.premium_shell_pass_count,
  );
  const requiredPassCount = Math.max(
    firstOptionalNumber(
      gate.requiredPassCount,
      gate.required_pass_count,
      render.premium_shell_required_pass_count,
      lane.premium_shell_required_pass_count,
    ) || MIN_PREMIUM_HYPERFRAMES_CARDS,
    MIN_PREMIUM_HYPERFRAMES_CARDS,
  );
  return (
    ["pass", "green", "ready"].includes(verdict) &&
    passCount != null &&
    passCount >= requiredPassCount &&
    asArray(gate.blockers).length === 0 &&
    asArray(render.premium_shell_blockers).length === 0 &&
    asArray(lane.premium_shell_blockers).length === 0
  );
}
...
function materialisedMotionPasses(artefacts = {}) {
  ...
  const visualProfile = artefacts.visualQuality?.visual_evidence_profile || {};
  const benchmarkProfile = artefacts.benchmark?.visual_evidence_profile || {};
  const profileBlockers = [
    ...asArray(visualProfile.blockers),
    ...asArray(benchmarkProfile.blockers),
  ];
  const profiledMotionCount = Math.max(
    firstNumber(visualProfile.motion_asset_count, benchmarkProfile.motion_asset_count),
    firstNumber(visualProfile.real_motion_asset_count, benchmarkProfile.real_motion_asset_count),
    firstNumber(visualProfile.direct_video_motion_asset_count, benchmarkProfile.direct_video_motion_asset_count),
  );
  const profiledFamilyCount = Math.max(
    firstNumber(visualProfile.direct_video_motion_family_count, benchmarkProfile.direct_video_motion_family_count),
    firstNumber(visualProfile.real_media_family_count, benchmarkProfile.real_media_family_count),
  );
  const directMotionCount = Number(
    visualProfile.direct_video_motion_asset_count ||
      benchmarkProfile.direct_video_motion_asset_count ||
      0,
  );
  const materialisedCount = Math.max(materialised.length, profiledMotionCount);
  const distinctFamilyCount = Math.max(families.size, profiledFamilyCount);
  return {
    materialised_count: materialisedCount,
    distinct_motion_family_count: distinctFamilyCount,
    direct_video_motion_asset_count: directMotionCount,
    generated_only_motion_deck:
      visualProfile.generated_only_motion_deck === true ||
      benchmarkProfile.generated_only_motion_deck === true,
    passes: profileBlockers.length === 0 && materialisedCount >= 8 && distinctFamilyCount >= 3 && directMotionCount > 0,
  };
}
...
function validateCandidate({
  ...
  strictDryRunPlan,
  storyPackages = [],
  guardedReadyPlatformsByStory = null,
  terminalDuplicatePlatformsByStory = null,
  requireGuardedDispatchReady = false,
} = {}) {
  ...
  const alreadyPublished = alreadyPublishedEnabledPlatforms(candidate, strictDryRunPlan, storyId);
  const guardedReadyPlatforms = requireGuardedDispatchReady
    ? guardedReadyPlatformsByStory?.get(storyId) || new Set()
    : null;
  const terminalDuplicatePlatforms = requireGuardedDispatchReady
    ? terminalDuplicatePlatformsByStory?.get(storyId) || new Set()
    : new Set();
  const dispatchEnabledPlatforms = [];
  const artefacts = readStoryArtefacts(root, storyId, storyPackages, candidate);
  ...
  for (const platform of REQUIRED_ENABLED_PLATFORMS) {
    ...
    if (terminalDuplicatePlatforms.has(platform) && !alreadyPublished.has(platform)) {
      blockers.push(`terminal_duplicate_action_ignored:${platform}`);
    }
    if (guardedReadyPlatforms) {
      if (guardedReadyPlatforms.has(platform)) {
        dispatchEnabledPlatforms.push(platform);
      } else if (!alreadyPublished.has(platform)) {
        blockers.push(`guarded_dispatch_action_not_ready:${platform}`);
      }
    } else if (!action) {
      if (!alreadyPublished.has(platform)) blockers.push(`missing_enabled_platform_action:${platform}`);
    } else if (!enabledActionIsReviewReady(action)) {
      blockers.push(`enabled_platform_action_not_review_ready:${platform}`);
    } else {
      dispatchEnabledPlatforms.push(platform);
    }
  ...
  if (dispatchEnabledPlatforms.length === 0) blockers.push("no_enabled_platform_action_ready");
  ...
  if (!hyperframesPremiumShellPasses(artefacts)) blockers.push("hyperframes_premium_shell_not_passed");

  const rightsVerdict = clean(artefacts.rights?.verdict || artefacts.rights?.status);
  const rightsCount = [
    ...asArray(artefacts.rights?.assets),
    ...asArray(artefacts.rights?.records),
    ...asArray(artefacts.rights?.entries),
  ].length;
  ...
  return {
    ...
    dispatch_enabled_platforms: dispatchEnabledPlatforms,
    already_published_enabled_platforms: [...alreadyPublished],
    platform_packages: Object.fromEntries(
      dispatchEnabledPlatforms.map((platform) => {
    ...
    manual_approval_checkbox: `[ ] Approve ${storyId} for ${dispatchEnabledPlatforms.join(", ")} only after watching the final MP4 and checking title, source, captions and cover.`,
    operator_commands: operatorCommands(storyId, dispatchEnabledPlatforms),
  };
}

function operatorCommands(storyId, platformsList = REQUIRED_ENABLED_PLATFORMS) {
  const platforms = asArray(platformsList).map(clean).filter(Boolean).join(",");
  const artefacts = [
    "video_path",
    "captions_path",
...
function candidateStoryId(candidate = {}) {
  return clean(candidate.id || candidate.story_id);
}

function candidateReportCandidates(report = {}) {
  return asArray(report.candidates);
}

function candidateReportStoryIds(report = {}) {
  return new Set(candidateReportCandidates(report).map(candidateStoryId).filter(Boolean));
}

function strictDryRunReadyStoryIds(plan = {}) {
  return new Set(
    asArray(plan.actions)
      ...
      .map((action) => clean(action.story_id))
      .filter(Boolean),
  );
}

function guardedDispatchReadyActions(report = {}) {
  return [
    ...asArray(report.dispatch_ready_actions),
    ...asArray(report.guarded_dispatch_plan?.dispatch_ready_actions),
    ...asArray(report.executor_plan?.handoff_ready_actions),
    ...asArray(report.handoff_ready_actions),
  ];
}

function guardedDispatchPreflightIsAuthoritative(report = {}) {
  return Boolean(
    report &&
      (
        guardedDispatchReadyActions(report).length > 0 ||
        asArray(report.ignored_terminal_duplicate_actions).length > 0 ||
        asArray(report.guarded_dispatch_plan?.ignored_terminal_duplicate_actions).length > 0 ||
        asArray(report.ignored_already_published_actions).length > 0 ||
        asArray(report.guarded_dispatch_plan?.ignored_already_published_actions).length > 0 ||
        report.summary ||
        report.guarded_dispatch_plan
      ),
  );
}

function guardedDispatchReadyPlatformsByStory(report = {}) {
  const map = new Map();
  for (const action of guardedDispatchReadyActions(report)) {
    ...
    map.get(storyId).add(platform);
  }
  return map;
}

function guardedDispatchTerminalDuplicatePlatformsByStory(report = {}) {
  const map = new Map();
  const actions = [
    ...asArray(report.ignored_terminal_duplicate_actions),
    ...asArray(report.guarded_dispatch_plan?.ignored_terminal_duplicate_actions),
  ];
...
function candidateReportGeneratedMs(report = {}, filePath = "") {
  const parsed = Date.parse(clean(report.generated_at || report.generatedAt || report.timestamp));
...
function workspaceCandidateReportPaths({ output, testOutput } = {}) {
  const candidates = [
    path.join(testOutput, "next_publish_candidates.json"),
    path.join(output, "next_publish_candidates.json"),
    path.join(output, "scheduler_preflight_report.json"),
  ];
  try {
    for (const entry of fs.readdirSync(output, { withFileTypes: true })) {
      if (!entry.isDirectory()) continue;
      candidates.push(path.join(output, entry.name, "scheduler_preflight_report.json"));
    }
  } catch {
    // Missing output directories are normal in fresh workspaces.
  }
  const seen = new Set();
...
function selectWorkspaceCandidateReport({ output, testOutput, strictDryRunPlan } = {}) {
  const readyIds = strictDryRunReadyStoryIds(strictDryRunPlan);
  const scored = workspaceCandidateReportPaths({ output, testOutput })
    .map((filePath) => {
      const report = readJsonIfExists(filePath);
      if (!report) return null;
      const storyIds = candidateReportStoryIds(report);
      const overlap = [...storyIds].filter((id) => readyIds.has(id)).length;
      return {
        report,
        overlap,
        candidateCount: storyIds.size,
        generatedMs: candidateReportGeneratedMs(report, filePath),
      };
    })
    .filter(Boolean)
    .sort((a, b) =>
      ...
      b.generatedMs - a.generatedMs
    );
  return scored[0]?.report || {};
}
...
function liveGateState({ localHealth = {}, publicHealth = {}, platformStatusMatrix = {}, publishReadinessReport = {}, guardedDispatchPreflight = {} } = {}) {
  ...
  const primaryRuntimeHold = health?.runtime?.primary_runtime_hold === true;
...
function buildGuardedDispatchPlan(selected = []) {
  const actions = selected.flatMap((candidate) =>
    asArray(candidate.dispatch_enabled_platforms).map((platform) => ({
...
function buildControlledRestartPack({
  ...
  const guardedDispatchAuthoritative = guardedDispatchPreflightIsAuthoritative(guardedDispatchPreflight);
  const guardedReadyPlatforms = guardedDispatchReadyPlatformsByStory(guardedDispatchPreflight);
  const terminalDuplicatePlatforms = guardedDispatchTerminalDuplicatePlatformsByStory(guardedDispatchPreflight);
  const evaluated = asArray(candidateReport.candidates)
    .map((candidate) => validateCandidate({
      ...
      strictDryRunPlan,
      storyPackages,
      guardedReadyPlatformsByStory: guardedReadyPlatforms,
      terminalDuplicatePlatformsByStory: terminalDuplicatePlatforms,
      requireGuardedDispatchReady: guardedDispatchAuthoritative,
    }));
...
async function writeControlledRestartPack(report, { outputDir }) {
  ...
  await write("controlled_restart_guarded_dispatch_plan.json", report.guarded_dispatch_plan || {});
...
async function buildControlledRestartPackFromWorkspace({
  ...
  const strictDryRunPlan = readJsonIfExists(path.join(output, "dry_run_publish_plan.json")) || {};
  const report = buildControlledRestartPack({
    root: cwd,
    generatedAt,
    candidateLimit,
    candidateReport: selectWorkspaceCandidateReport({ output, testOutput, strictDryRunPlan }),
    strictDryRunPlan,
