...
function resolveChannelNames(env = process.env) {
  return clean(
    env.DISCORD_FEEDBACK_CHANNEL_NAME ||
      env.DISCORD_CHANNEL_NAME ||
      env.DISCORD_PUBLIC_FEEDBACK_CHANNEL_NAME ||
      "pulse-gaming,video-drops",
  )
    .split(/[,\n;]/)
...
async function fetchDiscordChannelMessages({
  ...
  if (!channelId) {
    const discovered = await discoverDiscordFeedbackChannel({ env, token, fetchImpl });
