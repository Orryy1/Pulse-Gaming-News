...
} = require("../studio/v5/pulse-visual-identity");
...
const CONCRETE_COVER_STAKE_RE =
  /\b(?:[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|demo|demos|storage|ssd|pc port|pc specs?|free access|free trial|free weekend|price|subscription|game pass|review score|release date|launch date|delay(?:ed)?|gameplay|early access|wait[- ]?list|pre[- ]?order|reservation|hollywood|movie|film|steam ceiling)\b/i;
const ABSTRACT_TRUST_COVER_RE =
  /\b(?:player\s+)?trust\s+(?:test|problem)|player test|story test|source lock|gameplay check|update check\b/i;
...
const COVER_IDENTITY_PARITY_FIELDS = [
  "version",
  "category",
  "palette_id",
  "primary",
...
function analyseCoverText(value) {
  ...
  if (ABSTRACT_TRUST_COVER_RE.test(text) && !CONCRETE_COVER_STAKE_RE.test(text)) {
    blockers.push("cover_text_too_abstract");
  }
...
function identitySnapshot(identity = {}) {
  return {
    version: clean(identity.version || identity.creative_system_version) || null,
    category: clean(identity.category || identity.id) || null,
    palette_id: clean(identity.palette_id || identity.palette?.id) || null,
    primary: clean(identity.primary || identity.palette?.primary || identity.accent) || null,
    secondary: clean(identity.secondary || identity.palette?.secondary || identity.accent2) || null,
    segment_name: clean(identity.segment_name || identity.segment?.name) || null,
    code: clean(identity.code || identity.segment?.code || identity.segment_code) || null,
  };
}

function hasCompleteV5Identity(identity = {}) {
  const snapshot = identitySnapshot(identity);
  return snapshot.version === CREATIVE_SYSTEM_VERSION &&
    COVER_IDENTITY_PARITY_FIELDS.every((field) => clean(snapshot[field]));
}

function identitiesMatch(expected = {}, actual = {}) {
  const expectedSnapshot = identitySnapshot(expected);
  const actualSnapshot = identitySnapshot(actual);
  return COVER_IDENTITY_PARITY_FIELDS.every(
    (field) => clean(expectedSnapshot[field]).toLowerCase() === clean(actualSnapshot[field]).toLowerCase(),
  );
}

function resolveStoryCreativeIdentity(story = {}, canonical = {}) {
  const suppliedIdentity =
    canonical.creative_identity ||
    canonical.visual_identity ||
    story.creative_identity ||
    story.visual_identity ||
    {};
  const resolved = resolvePulseVisualIdentity({
    ...story,
    ...canonical,
    creative_category: clean(
      suppliedIdentity.category ||
        canonical.creative_category ||
        canonical.visual_category ||
        story.creative_category ||
        story.visual_category,
    ),
    title: titleForStory(story, canonical),
    canonical_subject: subjectForStory(story, canonical),
    content_pillar: clean(canonical.content_pillar || story.content_pillar),
  });
  return {
    ...resolved,
    transition_cycle: [...(resolved.transition_cycle || [])],
  };
}
...
function inspectPremiumVisualCampaign(manifest = {}, { expectedIdentity = {} } = {}) {
  if (!manifest || !Object.keys(manifest).length) {
    return {
      ...
      identity_parity: {
        verdict: "not_evaluated",
        story_identity: identitySnapshot(expectedIdentity),
        campaign_identity: null,
        cover_outputs: [],
      },
    };
  }
  const outputEntries = manifest.outputs && typeof manifest.outputs === "object"
    ? Object.entries(manifest.outputs)
    : [];
  const campaignIdentity = manifest.creative_identity || manifest.visual_identity || manifest.identity || {};
  const storyIdentityPresent = hasCompleteV5Identity(expectedIdentity);
  const parityIdentity = storyIdentityPresent ? expectedIdentity : campaignIdentity;
  const blockers = [];
  if (clean(manifest.verdict).toLowerCase() !== "green") blockers.push("premium_visual_campaign_not_green");
  if (outputEntries.length < 5) blockers.push("premium_visual_campaign_output_matrix_incomplete");
  if (!hasCompleteV5Identity(campaignIdentity)) {
    blockers.push("premium_visual_campaign_identity_missing");
  } else if (storyIdentityPresent && !identitiesMatch(expectedIdentity, campaignIdentity)) {
    blockers.push("premium_visual_campaign_story_identity_mismatch");
  }
  const coverOutputs = outputEntries.map(([platform, output]) => {
    const coverIdentity = output?.creative_identity || output?.visual_identity || output?.identity || {};
    let verdict = "pass";
    if (!hasCompleteV5Identity(coverIdentity)) {
      verdict = "fail";
      blockers.push(`premium_visual_campaign_cover_identity_missing:${platform}`);
    } else if (!identitiesMatch(parityIdentity, coverIdentity)) {
      verdict = "fail";
      blockers.push(`premium_visual_campaign_cover_identity_mismatch:${platform}`);
    }
    return {
      platform,
      verdict,
      identity: identitySnapshot(coverIdentity),
    };
  });
  const identityBlockers = blockers.filter((blocker) => blocker.includes("identity"));
  return {
    verdict: blockers.length ? "fail" : "pass",
    blockers: unique(blockers),
    output_count: outputEntries.length,
    visual_fingerprint: clean(manifest.visual_fingerprint) || null,
    identity_parity: {
      verdict: identityBlockers.length ? "fail" : "pass",
      story_identity: storyIdentityPresent ? identitySnapshot(expectedIdentity) : null,
      campaign_identity: identitySnapshot(campaignIdentity),
      cover_outputs: coverOutputs,
    },
  };
}
...
function scoreFirstFrameThumbnailStory({
  ...
  const creativeIdentity = resolveStoryCreativeIdentity(story, canonicalManifest);
  const premiumCampaign = inspectPremiumVisualCampaign(premiumVisualCampaign, {
    expectedIdentity: creativeIdentity,
  });
  ...
  return {
    ...
    premium_visual_campaign: premiumCampaign,
    blockers,
    warnings,
...
function buildFirstFrameThumbnailReport({
  ...
  const rows = candidateRows.map((story) => {
    ...
    return scoreFirstFrameThumbnailStory({
      ...
      premiumVisualCampaign: premiumVisualCampaigns[id] || {},
