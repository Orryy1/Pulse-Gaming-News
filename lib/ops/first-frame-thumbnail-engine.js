...
function coverTextForStory(story = {}, canonical = {}) {
  return clean(
...
function titleSourceParity({ story = {}, canonical = {}, actions = [] } = {}) {
  ...
  const cover = coverTextForStory(story, canonical);
...
function buildPlatformCoverMatrix({ story = {}, actions = [], requiredPlatforms = REQUIRED_ENABLED_PLATFORMS } = {}) {
  const id = storyId(story);
  const rows = requiredPlatforms.map((platform) => {
    const row = actions.find((action) => clean(action.platform) === platform) || null;
    const cover = clean(row?.cover_frame_source || row?.cover_path || row?.thumbnail_path);
    const video = clean(row?.video_path || row?.exported_path);
    const ready = Boolean(row && cover);
    return {
      platform,
      ...
      ready,
      blocker: ready ? null : `platform_cover_source_missing:${platform}`,
    };
  });
...
function scoreFirstFrameThumbnailStory({
  ...
  const coverText = coverTextForStory(story, canonicalManifest);
  const cover = analyseCoverText(coverText);
  const parity = titleSourceParity({ story, canonical: canonicalManifest, actions });
  const platformMatrix = buildPlatformCoverMatrix({ story: { ...story, id }, actions, requiredPlatforms });
