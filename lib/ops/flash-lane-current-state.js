...
const OFFICIAL_SOURCE_INTAKE_TEMPLATE_PATH = "test/output/official_source_intake_template.json";
const STALE_OFFICIAL_SOURCE_INTAKE_PATH_RE = /test[\\/]+input[\\/]+official_sources\.json/g;
const RESOLVE_TRAILERS_REFRESH_COMMAND =
...
function buildInputFreshness({ motionGapReport = {}, alternateSourceReport = {} } = {}) {
  const motionGapGeneratedAt = motionGapReport.generated_at || null;
  const alternateSourceGeneratedAt = alternateSourceReport.generated_at || null;
  const motionGapMs = timestampMs(motionGapGeneratedAt);
  const alternateSourceMs = timestampMs(alternateSourceGeneratedAt);
  const warnings = [];
  const alternateFreshness = alternateSourceReport.input_freshness || {};

  if (motionGapMs && alternateSourceMs && alternateSourceMs < motionGapMs) {
    warnings.push({
      code: "alternate_source_report_older_than_motion_gap",
      severity: "warning",
      message:
        "The alternate-source handoff is older than the motion-gap report; alternate source entities may be incomplete.",
      recommended_command: "npm run studio:v2:alternate-sources",
    });
  }
  for (const warning of array(alternateFreshness.warnings)) {
    if (!warning?.code) continue;
    warnings.push({
      code: warning.code,
      severity: warning.severity || "warning",
      message:
        warning.message ||
        "The alternate-source handoff has provisional reference counts; refresh trailer resolution before trusting remaining/excluded refs.",
      recommended_command: warning.recommended_command || null,
    });
  }
  const referenceCountsProvisional =
    Boolean(alternateFreshness.reference_counts_provisional) ||
    warnings.some((warning) => warning.code === "reference_report_older_than_motion_gap");

  return {
    motion_gap_report_generated_at: motionGapGeneratedAt,
    alternate_source_report_generated_at: alternateSourceGeneratedAt,
    reference_counts_provisional: referenceCountsProvisional,
    warnings,
  };
}
...
function formatSeconds(value) {
  ...
  return number === null ? "unknown" : `${number.toFixed(1)}s`;
}
...
function visualEvidenceBlocked(candidate = {}) {
  const blockers = array(candidate.blockers).map(String);
  return (
    candidate.visuals?.visual_evidence_gate_ready === false ||
    blockers.includes("flash_proof_requires_verified_store_exact_assets") ||
    blockers.includes("flash_proof_blocks_cover_dominated_exact_assets") ||
    blockers.includes("flash_proof_blocks_wrong_story_exact_assets")
  );
}

function visualEvidenceRepairAction(candidate = {}) {
  const blockers = array(candidate.blockers).map(String);
  if (blockers.includes("flash_proof_blocks_cover_dominated_exact_assets")) {
    return "replace_cover_dominated_assets_with_screenshots_or_gameplay_frames";
  }
  if (blockers.includes("flash_proof_blocks_wrong_story_exact_assets")) {
    return "rerun_exact_subject_acquisition_with_story_entity_filter";
  }
  if (blockers.includes("flash_proof_requires_verified_store_exact_assets")) {
    return "rerun_store_verification_before_counting_exact_assets";
  }
  return "repair_visual_evidence_before_flash_proof";
}

function normaliseOperatorCommand(command) {
  return String(command || "").replace(STALE_OFFICIAL_SOURCE_INTAKE_PATH_RE, OFFICIAL_SOURCE_INTAKE_TEMPLATE_PATH);
}

function recommendedCommands(gap = {}, candidate = {}, alternateRows = [], inputFreshness = {}) {
  const commands = [];
  if (inputFreshness.reference_counts_provisional) {
    const refreshCommand =
      array(inputFreshness.warnings).find((warning) => /media:resolve-trailers/.test(String(warning.recommended_command || "")))
        ?.recommended_command || RESOLVE_TRAILERS_REFRESH_COMMAND;
    commands.push({
      label: "Refresh resolver before trusting remaining/excluded refs",
  ...
  const seen = new Set();
  return commands.map((item) => ({ ...item, command: normaliseOperatorCommand(item.command) })).filter((item) => {
...
function classifyCurrentState({ candidate = {}, gap = {}, footage = {}, alternateRows = [] } = {}) {
  const candidateReadyFlashProof = candidate.verdict === "ready_flash_proof";
  ...
  const hasVisualEvidenceBlock = visualEvidenceBlocked(candidate);
  const targetEntities = storyEntities(candidate, gap, footage);
  const hasExactSubjectTarget = targetEntities.length > 0;
  const missingEntities = candidateReadyFlashProof
    ? unique(candidate.visuals?.missing_validated_clip_entities)
    : missingMotionEntities(candidate, gap, footage);
  ...
  if (hasVisualEvidenceBlock) blockingDimensions.push("visual_evidence");
  ...
  } else if (hasVisualEvidenceBlock) {
    stage = "needs_visual_evidence_repair";
    operator_next_action = visualEvidenceRepairAction(candidate);
  } else if (hasAlternateNeed) {
    stage = "needs_alternate_official_motion_source";
    operator_next_action = missingEntities.length
      ? "find_non_exhausted_official_motion_source"
      : "find_more_validated_gameplay_seconds_or_downgrade_story";
...
function buildCurrentStateRow({ candidate = {}, gap = {}, footage = {}, alternateRows = [], inputFreshness = {} } = {}) {
  const state = classifyCurrentState({ candidate, gap, footage, alternateRows });
  const alternateEntities = unique([
    ...array(gap.motion_gap?.acquisition_strategy?.alternate_source_entities),
    ...alternateRows.map((row) => row.entity),
  ]);
  const topAlternateSearches = alternateRows
    ...
    .slice(0, 12);
  const commands =
    state.stage === "needs_format_router_decision"
      ? []
      : recommendedCommands(gap, candidate, alternateRows, inputFreshness).slice(0, 8);
  ...
  return {
    ...
    visuals: {
      ...
      clip_dominance_shortfall_seconds: nullableNumberValue(gap.motion_gap?.missing_clip_seconds_for_dominance),
      visual_evidence_gate_ready: candidate.visuals?.visual_evidence_gate_ready !== false,
      unverified_store_exact_asset_count: numberValue(candidate.visuals?.unverified_store_exact_asset_count),
      cover_dominated_exact_asset_count: numberValue(candidate.visuals?.cover_dominated_exact_asset_count),
      cover_dominated_exact_asset_share: numberValue(candidate.visuals?.cover_dominated_exact_asset_share),
      wrong_story_exact_asset_count: numberValue(candidate.visuals?.wrong_story_exact_asset_count),
      wrong_story_exact_asset_share: numberValue(candidate.visuals?.wrong_story_exact_asset_share),
      wrong_story_exact_asset_groups: unique(candidate.visuals?.wrong_story_exact_asset_groups),
    },
    acquisition: {
      ...
      reference_counts_provisional: Boolean(inputFreshness.reference_counts_provisional),
      requires_reference_refresh: Boolean(inputFreshness.reference_counts_provisional),
    },
...
function buildFlashLaneCurrentStateReport({
  ...
  const inputFreshness = buildInputFreshness({ motionGapReport, alternateSourceReport });
  ...
  return {
    ...
    summary: {
      ...
      needs_visual_evidence_repair: rows.filter((row) => row.stage === "needs_visual_evidence_repair").length,
...
function renderFlashLaneCurrentStateMarkdown(report = {}) {
  const lines = [
    ...
    `- Need visual evidence repair: ${report.summary?.needs_visual_evidence_repair || 0}`,
    ...
    `- Alternate source report: ${report.input_freshness?.alternate_source_report_generated_at || "unknown"}`,
    `- Reference counts: ${report.input_freshness?.reference_counts_provisional ? "provisional - refresh resolver before trusting remaining/excluded refs" : "current"}`,
    "",
    ...array(report.input_freshness?.warnings).flatMap((warning) => [
      `- Warning: ${warning.code} - ${warning.message}`,
      ...(warning.recommended_command ? [`  Recommended: \`${warning.recommended_command}\``] : []),
    ]),
    ...(array(report.input_freshness?.warnings).length ? [""] : []),
    "## Current Queue",
    "",
  ...
  for (const row of array(report.rows)) {
    ...
    const visualGate = row.visuals?.visual_evidence_gate_ready
      ? "pass"
      : `block cover ${row.visuals?.cover_dominated_exact_asset_share ?? 0}`;
    lines.push(
      `| ${mdCell(`${row.story_id}: ${row.title}`)} | ${mdCell(row.stage)} | ${mdCell(row.distance_to_local_proof)} | ${mdCell(audio)} | ${row.visuals?.exact_subject_count || 0} | ${mdCell(visualGate)} | ${row.visuals?.validated_clip_ref_count || 0}/${row.visuals?.validated_clip_source_count || 0} | ${mdCell(formatSeconds(row.visuals?.clip_dominance_shortfall_seconds))} | ${mdCell(array(row.visuals?.missing_motion_entities).join(", ") || "none")} | ${mdCell(row.operator_next_action)} |`,
    );
  }
