...
function topicScriptFor(story = {}, sourceMaterial = "") {
  ...
  if (/black\s+flag|resynced/i.test(text)) {
    return scriptObject({
      ...
      thumbnailText: "BLACK FLAG TRUST TEST",
      fullScript:
        "Black Flag Resynced has to sell more than prettier water. PlayStation Blog details PS5 Pro enhancements, so the early test is sailing, boarding, combat and whether the Caribbean still feels sharp in motion. That turns the remake argument into a trust test: is Ubisoft giving players a cleaner pirate classic, or asking nostalgia to do all the work? Black Flag is loved because open-sea chaos still has a clear identity. A technical upgrade only matters if it makes those moments feel cleaner without sanding the game flat. Players will look for frame rate, image clarity and danger, not just nicer screenshots. If Resynced makes the old rhythm feel immediate again, the upgrade has a case. If it only looks cleaner in stills, the price argument gets ugly fast. Follow Pulse Gaming so you never miss a beat.",
    });
  }
  ...
  if (/marvel\s+tokon/i.test(text) && /blade|loki|deadpool/i.test(text)) {
    return scriptObject({
      ...
      fullScript:
        "Marvel Tokon just made the roster argument louder. PlayStation Blog confirmed Blade, Loki and Deadpool, so players now have three very different combat styles to judge before launch. It is about whether each hero brings a different kind of movement problem. Blade should feel direct and violent. Loki should mess with space and decisions. Deadpool has to be annoying without becoming unreadable. That mix is exactly what Marvel fighters need: characters that create arguments before launch, then prove those arguments in matches. The danger is obvious too. If everyone becomes visual fireworks with different skins, the names stop mattering. If those playstyles actually clash, Tokon starts looking like a real fighting-game problem, not just a Marvel celebration reel. Follow Pulse Gaming so you never miss a beat.",
    });
  }

  if (/monopoly|heroes\s+vs\.?\s+villains|character abilities/i.test(text)) {
    return scriptObject({
      title: "Star Wars Monopoly Needs Proper Chaos",
      thumbnailText: "STAR WARS MONOPOLY CHAOS",
      fullScript:
        "Monopoly with Star Wars powers sounds ridiculous, but the abilities are the fight. Xbox Wire breaks down Heroes versus Villains character abilities, and players can now judge whether this is proper table chaos or just a branded board. The Star Wars skin earns its place when the powers create arguments: who gets momentum, who blocks a comeback and who feels unfair in the funniest way. If the abilities are too weak, it is Monopoly with a costume. If they are too strong, the whole board turns into noise. That tension is the reason this story works at all. If the powers split the table without killing the strategy, Star Wars Monopoly becomes messy family-night drama instead of another shelf-filler. Follow Pulse Gaming so you never miss a beat.",
    });
  }

  if (/fatal\s+fury|city\s+of\s+the\s+wolves|kenshiro|fist\s+of\s+the\s+north\s+star/i.test(text)) {
    return scriptObject({
      title: "Kenshiro Gives Fatal Fury A Real Roster Fight",
      thumbnailText: "KENSHIRO ROSTER FIGHT",
      fullScript:
        "City of the Wolves just got a crossover that fighting-game fans will not ignore. Xbox Wire says Kenshiro from Fist of the North Star is coming to Fatal Fury, and players now get a real roster argument, not just a guest-star poster. Kenshiro brings a different weight: brutal manga history colliding with SNK's clean fighting-game rhythm. The question is whether he feels like a proper character or a novelty skin built for trailer reactions. Guest fighters can revive attention, but they can also make a roster feel desperate if the moveset does not belong. If Kenshiro lands with real style, City of the Wolves gets a new audience fight. If he feels pasted in, the crossover becomes noise. Follow Pulse Gaming so you never miss a beat.",
    });
  }
...
function buildFreshRefillViewerScript({ job = {}, manifest = {} } = {}) {
  ...
  const verdict =
    quality.verdict === "viral_ready" && coherence.result === "pass"
      ? "viral_ready"
      : "rewrite_required";
...
function updatePlatformManifest(platform = {}, script) {
...
function removeScriptBlockers(values = []) {
  return asArray(values).map(cleanText).filter((value) => value && !SCRIPT_BLOCKER_RE.test(value));
}

function updatePublishVerdict(verdict = {}, script) {
  const reasonCodes = removeScriptBlockers(verdict.reason_codes);
  const blockers = removeScriptBlockers(verdict.blockers);
  const packageQualityBlockers = removeScriptBlockers(verdict.package_quality_gate?.blockers);
...
function updateGoalPackageSummary(summary = {}, script) {
  const blockers = removeScriptBlockers(summary.blockers);
  return {
    ...summary,
...
function updatePulseMediaHouseScore(score = {}, script) {
  ...
  next.hard_failures = removeScriptBlockers(score.hard_failures);
  if (next.competitor_parity_report) {
    next.competitor_parity_report = {
      ...next.competitor_parity_report,
      blockers: removeScriptBlockers(next.competitor_parity_report.blockers),
    };
  }
...
async function applyRewriteToArtifact({ artifactDir, job, script, generatedAt }) {
  ...
  await writeJson(manifestPath, updateCanonicalManifest(manifest, script, generatedAt));
  await writeJson(platformPath, updatePlatformManifest(platform, script));
  ...
  const updatedPlatform = updatePlatformManifest(platform, script);
  await Promise.all(
