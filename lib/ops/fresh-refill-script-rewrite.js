...
function topicScriptFor(story = {}, sourceMaterial = "") {
  ...
  if (/doom\b/i.test(text) && /\b(?:dark ages|chain spear|revelations|supersonic)\b/i.test(text)) {
    return scriptObject({
      ...
      fullScript:
        "Doom The Dark Ages just made its next DLC about speed, not size. Xbox Wire says the Revelations update adds a Chain Spear built around fast movement, and that is the useful detail for players. Doom only works when aggression feels readable. A new weapon has to pull you into danger, create pressure and keep the fight clear instead of turning combat into effects spam. If the Chain Spear sharpens that push-forward combat, this DLC gives lapsed players a real reason to come back. If it is only a flashy tool, the novelty dies after the first arena. Follow Pulse Gaming so you never miss a beat.",
    });
  }
