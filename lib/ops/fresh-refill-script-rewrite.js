...
function preservedCurrentScriptCandidate(job = {}, manifest = {}, story = {}) {
  ...
  const sourceClaims = structuredSourceEvidenceClaims(job, manifest)
  ...
  return {
    ...scriptObject({
      ...
      confirmedClaims: sourceClaims.length
        ? sourceClaims
        : [
...
function generatedSubjectIsSourceBrand(subject = "", sourceName = "") {
  const subjectComparison = comparisonText(subject);
  const sourceComparison = comparisonText(sourceName);
  if (!subjectComparison || !sourceComparison) return false;
  if (subjectComparison.split(/\s+/).length > 2) return false;
  return (
    sourceComparison === subjectComparison ||
    sourceComparison.startsWith(`${subjectComparison} `)
  );
}
...
function storyFromJob(job = {}, manifest = {}) {
  ...
  const replaceGeneratedIdentity =
    ...
    (
      ...
      (
        recordedSubject &&
        comparisonText(recordedSubject) !== comparisonText(existingSubject) &&
        generatedSubjectIsSourceBrand(existingSubject, sourceName)
      )
    );
  return {
...
function structuredSourceEvidenceClaims(job = {}, manifest = {}) {
  ...
  return structuredSourceEvidenceEntries(job, manifest)
    ...
    .filter((claim) => {
      const key = [
        cleanText(claim.text).toLowerCase(),
        cleanText(claim.evidence_text).toLowerCase(),
        normalizedSourceUrl(claim.source_url),
        cleanText(claim.origin).toLowerCase(),
      ].join("|");
...
function datedSentenceFor(sourceMaterial = "", cue = /$^/) {
  const sentences = sentenceList(sourceMaterial);
  for (let index = 0; index < sentences.length; index += 1) {
    if (!cue.test(sentences[index])) continue;
    for (let lookahead = 0; lookahead <= 2; lookahead += 1) {
      const combined = sentences.slice(index, index + lookahead + 1).join(" ");
      if (datesFromSentence(combined).length >= 2) return combined;
    }
  }
  return "";
}

function datesFromSentence(sentence = "") {
  return unique(
    sentence.match(new RegExp(`\\b${MONTH_NAME_RE}\\s+\\d{1,2}(?:st|nd|rd|th)?\\b`, "gi")) || [],
  ).map((value) => value.replace(/(\\d)(?:st|nd|rd|th)\\b/i, "$1"));
}

function firstDateFromSentence(sourceMaterial = "", cue = /$^/) {
  const sentence = sentenceList(sourceMaterial).find((value) => cue.test(value)) || "";
  return datesFromSentence(sentence)[0] || "";
}

function twoWindowBetaScheduleScript(story = {}, sourceMaterial = "", source = "") {
  if (!/\bbeta\b/i.test(sourceMaterial)) return null;

  const earlySentence = datedSentenceFor(sourceMaterial, /\bearly access beta\b/i);
  const openSentence = datedSentenceFor(
    sourceMaterial,
    /\bweekend two\b[^.]{0,180}\bopen beta\b|\bopen beta\b\s*,?\s*(?:is\s+)?(?:open to all|begins?|starts?)/i,
  );
  const [earlyStart, earlyEnd] = datesFromSentence(earlySentence);
  const [openStart, openEnd] = datesFromSentence(openSentence);
  if (!earlyStart || !earlyEnd || !openStart || !openEnd) return null;

  const subject = cleanText(story.canonical_subject || story.canonical_game || "");
  if (!subject || /^(?:this game|this update|game|gaming news)$/i.test(subject)) return null;

  const nextDate = firstDateFromSentence(sourceMaterial, /\bCall of Duty:\s*NEXT\b/i);
  const switchPreorderDate = firstDateFromSentence(
    sourceMaterial,
    /\bpre[- ]orders?\b[^.]{0,80}\bSwitch\s*2\b[^.]{0,40}\b(?:begin|begins|open|opens|start|starts)\b|\bSwitch\s*2\b[^.]{0,80}\bpre[- ]orders?\b[^.]{0,40}\b(?:begin|begins|open|opens|start|starts)\b/i,
  );
  const hasSwitchOpenWindow =
    /\bopen beta\b[^.]*\bSwitch\s*2\b|\bSwitch\s*2\b[^.]*\bopen beta\b/i.test(sourceMaterial);
  const noPreorderRequired =
    /\b(?:without|no) (?:a )?pre[- ]order\b|\bregardless of (?:their )?pre[- ]order status\b/i.test(sourceMaterial);
  const sourceLabel = source || "The official announcement";
  const sentences = [
    `${subject}'s beta has two entry points, and Switch 2 players get the later one.`,
    `${sourceLabel} says early access starts ${earlyStart} and runs through ${earlyEnd} for pre-orders on Xbox Series X and S, PlayStation 5 and PC.`,
  ];
  if (nextDate) {
    sentences.push(`Call of Duty: NEXT lands on ${nextDate}, with the beta beginning after the broadcast.`);
  }
  sentences.push(
    `The free open beta runs ${openStart} through ${openEnd} on every supported platform${hasSwitchOpenWindow ? ", including Nintendo Switch 2" : ""}${noPreorderRequired ? ", with no pre-order required" : ""}.`,
  );
  if (hasSwitchOpenWindow && switchPreorderDate) {
    sentences.push(`Switch 2 pre-orders open ${switchPreorderDate}, just before its beta window.`);
  }
  sentences.push(
    "The trade-off: Switch 2 loses early access but keeps the free test; others can pre-order or wait.",
  );
  if (/\bsubject to change\b/i.test(sourceMaterial)) {
    sentences.push("The publisher says dates and platform availability remain subject to change.");
  }
  sentences.push(
    hasSwitchOpenWindow
      ? "That means Switch 2 players can ignore early-access pressure and judge the game in the free weekend."
      : "That means players can choose earlier paid access or wait for the free weekend.",
  );
  sentences.push(EXACT_CTA);

  return scriptObject({
    title: `${subject} Beta Has Two Different Entry Points`,
    thumbnailText: "TWO BETA WINDOWS",
    canonicalSubject: subject,
    confirmedClaims: unique([earlySentence, openSentence].filter(Boolean)),
    fullScript: sentences.join(" "),
    source: "source_bound_two_window_beta_timeline",
  });
}
...
function topicScriptFor(story = {}, sourceMaterial = "") {
  ...
  const betaSchedule = twoWindowBetaScheduleScript(story, sourceMaterial, source);
  if (betaSchedule) return betaSchedule;

  if (
    /\barkheron\b/i.test(identityText) &&
    /\bxbox\s+insiders?\b/i.test(sourceMaterial) &&
    /\bspires\b/i.test(sourceMaterial) &&
    /\bprestige\b/i.test(sourceMaterial)
  ) {
    return scriptObject({
      title: "Arkheron Beta Rewards Survive The Test",
      thumbnailText: "BETA REWARDS STAY",
      canonicalSubject: "Arkheron",
      confirmedClaims: sentenceList(sourceMaterial).filter((sentence) =>
        /\b(?:Arkheron|Spires|Ascension|Prestige|Xbox Insider Hub)\b/i.test(sentence),
      ),
      fullScript:
        "Arkheron's Xbox beta is live, and the rewards will not vanish when it ends. Xbox Wire says Insiders on Series X and S or Windows PC can play until midnight Pacific on July 27. Spires is a focused three-versus-three arena for trying item builds quickly. Ascension scales up to 15 teams of three during limited regional weekend windows. Both modes award Pages, which unlock beta-only Prestige cosmetics that stay on your account. This is a technical test, so expect bugs and active updates. To join, open Xbox Insider Hub, choose Previews, then Arkheron Insider. The real test starts once the screen fills with players: can a class-free item system stay readable under pressure? If it can, Arkheron may leave the beta with a crowd worth keeping. Follow Pulse Gaming so you never miss a beat.",
      source: "source_bound_arkheron_beta_access_and_rewards",
    });
  }
...
function approvedPrimarySourceFromJob(job = {}, sourceEvidence = null) {
  const source = job.source && typeof job.source === "object" ? job.source : null;
  if (
    !source ||
    cleanText(sourceEvidence?.status).toLowerCase() !== "pass" ||
    !sourceEvidenceHasCaptureHash(sourceEvidence) ||
    normalizedSourceUrl(source.url) !== normalizedSourceUrl(sourceEvidence.source_url)
  ) {
    return null;
  }
  ...
  return {
    ...
    url,
    type,
    ...(cleanText(source.published_at) ? { published_at: cleanText(source.published_at) } : {}),
...
async function applyRewriteToArtifact({ artifactDir, job, script, generatedAt }) {
  ...
  const sourceEvidence = sourceEvidenceSummaryFromJob(job, manifest, generatedAt);
  const approvedPrimarySource = approvedPrimarySourceFromJob(job, sourceEvidence);
  const confirmedEventWindow =
    sourceEvidence?.confirmed_event_window || job.source?.confirmed_event_window || null;
  const updatedManifest = {
    ...updateCanonicalManifest(manifest, script, generatedAt),
    ...(approvedPrimarySource
      ? {
          primary_source: approvedPrimarySource.name,
          primary_source_url: approvedPrimarySource.url,
          source_type: approvedPrimarySource.type,
          ...(approvedPrimarySource.published_at
            ? { source_published_at: approvedPrimarySource.published_at }
            : {}),
        }
      : {}),
    ...(sourceEvidence ? { source_evidence: sourceEvidence } : {}),
  ...
  if (sourceManifest && (sourceEvidence || confirmedEventWindow)) {
    await writeJson(sourceManifestPath, {
      ...sourceManifest,
      ...(approvedPrimarySource ? { primary_source: approvedPrimarySource } : {}),
      ...(sourceEvidence ? { source_evidence: sourceEvidence } : {}),
      ...(confirmedEventWindow ? { confirmed_event_window: confirmedEventWindow } : {}),
    });
  }
...
async function runFreshRefillScriptRewrite({
  ...
  for (const job of jobs) {
    ...
    try {
      ...
      const script = buildFreshRefillViewerScript({ job, manifest, now: generatedAt });
      const sourceEvidence = sourceEvidenceSummaryFromJob(job, manifest, generatedAt);
