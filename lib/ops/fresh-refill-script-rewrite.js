...
function structuredSourceEvidenceClaims(job = {}, manifest = {}) {
  return [
    ...asArray(job.source_evidence?.claims),
    ...asArray(manifest.source_evidence?.claims),
  ].filter((claim) => {
    if (!claim || typeof claim !== "object") return false;
    const origin = cleanText(claim.origin).toLowerCase();
    return cleanText(claim.text) &&
      cleanText(claim.evidence_text) &&
      /^https?:\/\//i.test(cleanText(claim.source_url)) &&
...
function sourceMaterialFrom(job = {}, manifest = {}) {
  ...
  const verifiedBody = structuredClaims.length
    ? [job.source?.body, job.source?.summary, job.source?.description]
    : [];
  return [
    job.source?.title,
    manifest.source_title,
...
function topicScriptFor(story = {}, sourceMaterial = "") {
  ...
  if (
    ...
    return scriptObject({
      ...
      thumbnailText: "MARVEL'S 4V4 GAMBLE",
      canonicalSubject: "MARVEL Tokon",
      fullScript:
        "Marvel Tokon has 20 fighters, but the number that could make or break it is four. PlayStation Blog says the base game has 20 playable heroes and villains, from Black Panther and Doctor Doom to Spider-Man and Carnage. Each match is four-versus-four, with easier inputs, combo assists, a tutorial and a single-player mode for learning the system. That sounds welcoming. Here is the catch: eight-character teams can turn every match into a wall of effects. Players deciding whether to buy at launch or wait will judge the readability first. If swaps stay clear, Tokon gets something rare: team depth without the usual fighting-game entry wall. The roster sells the fantasy. The real payoff is whether new players understand the fight fast enough to want one more match. Follow Pulse Gaming so you never miss a beat.",
    });
  }
...
function buildFreshRefillViewerScript({ job = {}, manifest = {}, now = new Date() } = {}) {
  ...
  const sourceEvidenceClaimCount = structuredClaims.length + legacyClaims.length;
