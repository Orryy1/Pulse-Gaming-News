...
function scriptObject({ fullScript, title, thumbnailText, description = "", source = "fresh_refill_viewer_script" }) {
...
function topicScriptFor(story = {}, sourceMaterial = "") {
  ...
  if (/\bstarward\b/i.test(text) && /\b(?:pliszka|wing rider assembly|larger hitbox|detach)\b/i.test(text)) {
    return scriptObject({
  ...
  if (/wreck\s+runners/i.test(text) && /(?:xbox\s+insider|playtest)/i.test(text)) {
    return scriptObject({
  ...
  if (/enter\s+the\s+pit|pit\s+of\s+goblin/i.test(text)) {
    return scriptObject({
  ...
  if (/elder\s+scrolls\s+online|\beso\b/i.test(text) && /season\s+one|thieves\s+guild|tamriel\s+tome/i.test(text)) {
    return scriptObject({
