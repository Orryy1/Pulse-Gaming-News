...
function topicScriptFor(story = {}, sourceMaterial = "") {
  ...
  if (/black\s+flag|resynced/i.test(text)) {
    return scriptObject({
      ...
      fullScript:
        "Assassin's Creed Black Flag Resynced has one job. Make the pirate loop feel dangerous again. PlayStation Blog says PlayStation 5 Pro upgrades are coming, but the real test is motion, not screenshots. Sailing needs speed. Boarding needs chaos. Combat needs risk. Black Flag worked because every chase and cannon shot was readable. If this restores that rhythm, lapsed players get a reason to reinstall. If not, this is prettier water. Follow Pulse Gaming so you never miss a beat.",
    });
  }
