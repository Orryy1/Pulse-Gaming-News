...
function isCommerceDealsRoundup(row = {}) {
  ...
  return (
    /\b(?:best deals today|today'?s top deals|deal roundup|gift cards?)\b/i.test(text) ||
    /\bairpods\b/i.test(text) ||
    (/\bdeals?\b/i.test(text) && /\b(?:lego|switch 2 cameras?|accessor(?:y|ies)|headset|controller)\b/i.test(text))
  );
}
...
function buildFreshReviewScriptRepairPlan({
  ...
  const workOrders = [...dbWorkOrders, ...transcriptWorkOrders].filter((item) => {
