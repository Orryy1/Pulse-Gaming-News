...
function sourceEvidenceTopicalityVerdict(row = {}) {
  return evaluatePulseGamingTopicality(
    {
      title: row.title,
      body: [row.top_comment, row.description, row.summary]
        .filter(Boolean)
        .join(" "),
      subreddit: row.subreddit,
      flair: row.flair,
...
function approvedProductionHoldReason(row = {}, {
  now = new Date(),
  maxAgeHours = 36,
  minScore = 65,
} = {}) {
  ...
  if (!id) return "missing_story_id";
  if (!commandSafeStoryId(id)) return "unsafe_story_id_for_repair_command";
  if (Number(row.approved || 0) !== 1) return "not_approved";
  if (Number(row.total || 0) < minScore) return `score_below_threshold:${Number(row.total || 0)}`;
  if (hardStopCount(row) > 0) return "hard_stop_present";
  if (!isSourceBacked(row)) return "not_source_backed";
  if (isCommerceDealsRoundup(row)) return "commerce_deals_roundup_not_editorial_story";
  if (isMotionPoorRetailOrServiceRow(row)) return "motion_runway_unfit_for_automatic_refill";
  const topicality = sourceEvidenceTopicalityVerdict(row);
  if (topicality.decision === "reject") return `topicality_reject:${topicality.reason}`;
  if (topicality.decision === "review") return `topicality_review:${topicality.reason}`;
  if (!hasStrongGamingRepairSignal(row, topicality)) return "topicality_reject:weak_generic_gaming_signal";
  if (hasPublishedPlatformPost(row)) return "already_has_platform_post";
  if (cleanText(row.exported_path)) return "already_has_exported_media";
  const age = sourceAgeHours(row, now);
  if (age === null) return "unknown_source_age";
  if (age > maxAgeHours) return `source_age_expired:${Math.round(age)}h`;

  const status = lower(row.publish_status);
  const error = cleanText(row.publish_error);
  if (!status) return "";
  if (
    status === "failed" &&
    /^audio_generation_failed:\s*(?:server_down|timeout|tts_busy|connection_error)\b/i.test(error)
  ) {
    return "";
  }
  if (status === "failed" && /^qa_blocked:\s*audio_duration_too_long\b/i.test(error)) {
    return "";
  }
  return `production_state_not_retryable:${status}`;
}

function selectFreshApprovedProductionRows(rows = [], {
  now = new Date(),
  maxAgeHours = 36,
  minScore = 65,
  limit = 10,
  ...
  return asArray(rows)
    ...
    .filter((row) => approvedProductionHoldReason(row, {
      now: generatedAt,
      maxAgeHours,
      minScore,
    }) === "")
...
function fetchFreshApprovedProductionRows({
  db = null,
  now = new Date(),
  maxAgeHours = 36,
  limit = 80,
  ...
  const cutoff = new Date(
    (now instanceof Date ? now : new Date(now)).getTime() - maxAgeHours * 36e5,
  ).toISOString();
  return liveDb
    .prepare(`
      WITH latest AS (
      ...
      JOIN stories s ON s.id = sc.story_id
      WHERE s.approved = 1
        AND datetime(COALESCE(s.timestamp, s.created_at, sc.scored_at)) >= datetime(?)
        AND COALESCE(TRIM(s.exported_path), '') = ''
        AND COALESCE(TRIM(s.youtube_post_id), '') = ''
        AND COALESCE(TRIM(s.youtube_url), '') = ''
        AND COALESCE(TRIM(s.tiktok_post_id), '') = ''
        AND COALESCE(TRIM(s.instagram_media_id), '') = ''
        AND COALESCE(TRIM(s.facebook_post_id), '') = ''
        AND COALESCE(TRIM(s.twitter_post_id), '') = ''
        AND COALESCE(TRIM(s.published_at), '') = ''
        AND NOT EXISTS (
          SELECT 1
          FROM platform_posts pp
          WHERE pp.story_id = s.id
