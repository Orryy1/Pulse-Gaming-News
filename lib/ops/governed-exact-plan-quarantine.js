...
function assertExactTransitionLease(db, lease, fingerprint, planSha256, now) {
...
async function assertLeasedDatabaseFence({
  ...
  assertExactTransitionLease(db, lease, fingerprint, planSha256, now);
  return observedSha256;
}
...
function auditEvidence(request, binding, fingerprint) {
  return {
...
async function writeEvidence(
  ...
  await assertRuntimeFence();
  const commitStatus = await writeNoClobber(
    fileSystem,
    commitPath,
    commit,
    identityProbe,
  ...
  await assertRuntimeFence();
  await assertEvidenceGuardBound(guard);
  const commitDirectoryFsyncStatus = await syncDirectory(
    fileSystem,
    directory,
    guard,
  );
...
function replayAudit(existing, request, binding, fingerprint, now) {
  let evidence;
  try {
  ...
  const expectedEvidence = auditEvidence(request, binding, fingerprint);
  if (
...
async function quarantineExactGovernedProductionPlan(value, dependencies = {}) {
  ...
  operationAttempt: try {
    ...
    if (existing) {
      ...
      if (lease.renew(leaseAcquiredAt) !== true) fail("quarantine_lease_lost");
      ...
      try {
        ...
        const evidence = auditEvidence(request, fresh, fingerprint);
        const inserted = db
          .prepare(
  ...
  if (operationError || replay?.verdict === "HOLD") {
    const releaseError = releaseRuntimeResources();
    if (releaseError) {
  ...
  const assertFinalRuntimeFence = async () => {
    ...
    if (lease.renew(fenceNow) !== true) fail("quarantine_lease_lost");
    await assertOpenedDatabaseIdentity(
      db,
      fileSystem,
    ...
    assertQuiescent(
      await inspectQuiescence({
        ...
        workspaceRoot: finalRuntimeFence.workspaceRoot,
        expectedCommit: request.expected_checkout_commit,
      }),
    );
    await absentNoLink(
