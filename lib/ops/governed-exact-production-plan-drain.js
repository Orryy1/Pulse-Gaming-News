...
} = require("../stabilisation/windows-task-authority");
const {
...
const CLAIM_AUTHORITY_PERMIT_MAX_AGE_MS = 5_000;
...
const QUIESCENCE_DIAGNOSTIC_PROBES = new Set([
  ...PROBE_NAMES,
  "aggregate",
]);
const QUIESCENCE_DIAGNOSTIC_KINDS = new Set([
  "AMBIGUOUS",
  "UNAVAILABLE",
  "OCCUPIED",
]);
const QUIESCENCE_DIAGNOSTIC_REASONS = new Set([
  "OPAQUE_ENCODED_POWERSHELL_HOST",
  "OPAQUE_RUNTIME_ENVIRONMENT_AUTHORITY",
  "PROBE_ATTESTATION_INCOMPLETE",
  "INSPECTION_FAILED",
  "OWNER_RECEIPT_INVALID",
  "UNREVIEWED_NODE_STARTUP_CODE_AUTHORITY",
  "OWNER_PIDS_PRESENT",
  "LISTENER_PIDS_PRESENT",
  "SCHEDULER_PROCESS_PIDS_PRESENT",
  "ENABLED_TASKS_PRESENT",
  "RUNNING_TASKS_PRESENT",
  "EXPECTED_TASK_NOT_ABSENT",
  "UNREVIEWED_PULSE_TASK_PRESENT",
]);
const MAX_QUIESCENCE_DIAGNOSTIC_PIDS = 32;
const MAX_QUIESCENCE_DIAGNOSTIC_TASK_IDENTITIES = 16;
const UNREVIEWED_PULSE_TASK_IDENTITY = "UNREVIEWED_PULSE_TASK";
const QUIESCENCE_TASK_STATE_LABELS = new Map([
  ["0", "Unknown"],
  ["unknown", "Unknown"],
  ["1", "Disabled"],
  ["disabled", "Disabled"],
...
function boundedPositivePids(values) {
  return [...new Set(Array.isArray(values) ? values : [])]
    .filter((value) => Number.isInteger(value) && value > 0)
    .sort((left, right) => left - right)
    .slice(0, MAX_QUIESCENCE_DIAGNOSTIC_PIDS);
}

function boundedTaskIdentities(values) {
  return [...new Set(Array.isArray(values) ? values : [])]
    .filter(
      (value) =>
        typeof value === "string" &&
    ...
    .sort()
    .slice(0, MAX_QUIESCENCE_DIAGNOSTIC_TASK_IDENTITIES);
}

function boundedReviewedTaskIdentities(values, profile = null) {
  const reviewed = boundedTaskIdentities([
    profile?.task_name,
    ...(Array.isArray(profile?.conflicting_task_names)
      ? profile.conflicting_task_names
      : []),
  ]);
  const reviewedByFoldedIdentity = new Map(
    reviewed.map((identity) => [identity.toLowerCase(), identity]),
  );
  return boundedTaskIdentities(values)
    .map((identity) =>
      reviewedByFoldedIdentity.get(identity.toLowerCase()),
    )
    .filter(Boolean)
    .sort()
    .slice(0, MAX_QUIESCENCE_DIAGNOSTIC_TASK_IDENTITIES);
...
function reviewedTaskIdentity(value, profile = null) {
  const identity = String(value || "").trim();
  if (!identity) return null;
  const foldedIdentity = identity.replace(/^\\/, "").toLowerCase();
  const reviewed = boundedTaskIdentities([
    profile?.task_name,
    ...(Array.isArray(profile?.conflicting_task_names)
      ? profile.conflicting_task_names
  ...
  return (
    reviewed.find(
      (candidate) => candidate.toLowerCase() === foldedIdentity,
    ) || null
  );
}

function redactedTaskIdentity(value, profile = null) {
  return (
    reviewedTaskIdentity(value, profile) || UNREVIEWED_PULSE_TASK_IDENTITY
  );
}

function normaliseTaskIdentityEvidence(values, profile = null) {
  return [
    ...new Set(
      (Array.isArray(values) ? values : [])
        .filter((value) => typeof value === "string" && value.trim())
        .map((value) => redactedTaskIdentity(value, profile)),
    ),
  ]
    .sort()
    .slice(0, MAX_QUIESCENCE_DIAGNOSTIC_TASK_IDENTITIES);
...
function normaliseTaskStateEvidence(values, profile = null) {
  return (Array.isArray(values) ? values : [])
    .filter(plainObject)
    .slice(0, MAX_QUIESCENCE_DIAGNOSTIC_TASK_IDENTITIES)
    .map((entry) => {
      const reviewed =
        reviewedTaskIdentity(entry.task_name, profile) ||
        reviewedTaskIdentity(entry.task_path, profile);
      const identity = reviewed || UNREVIEWED_PULSE_TASK_IDENTITY;
      const state =
        QUIESCENCE_TASK_STATE_LABELS.get(
          String(entry.state ?? "")
            .trim()
            .toLowerCase(),
        ) || "Unknown";
...
function quiescenceObservationMalformed(value) {
  if (value?.available !== true) return false;
  if (
    value.schema !== BOUNDED_AUTHORITY_SCHEMA ||
    value.verdict !== "GREEN" ||
    value.state !== "STOPPED_BOUND" ||
    !SHA256_PATTERN.test(String(value.authority_fingerprint || "")) ||
    value.runtime_instance_id !== null ||
    !SHA256_PATTERN.test(String(value.observation_sha256 || "")) ||
    !SHA256_PATTERN.test(String(value.database_snapshot_sha256 || "")) ||
  ...
  if (
    !plainObject(value.probe_attestations) ||
    PROBE_NAMES.some(
      (probe) => typeof value.probe_attestations[probe] !== "boolean",
    ) ||
    pidFields.some(
      (field) =>
        !Array.isArray(value[field]) ||
        value[field].length > MAX_QUIESCENCE_DIAGNOSTIC_PIDS ||
        value[field].some(
          (entry) => !Number.isInteger(entry) || entry <= 0,
        ),
    ) ||
    taskIdentityFields.some(
      (field) =>
        !Array.isArray(value[field]) ||
        value[field].length > MAX_QUIESCENCE_DIAGNOSTIC_TASK_IDENTITIES ||
        value[field].some(
          (entry) => typeof entry !== "string" || !entry.trim(),
        ),
    ) ||
    !Array.isArray(value.task_states) ||
    value.task_states.length > MAX_QUIESCENCE_DIAGNOSTIC_TASK_IDENTITIES
  ) {
    return true;
  }
  const taskStateFields = new Set([
    "task_name",
    "task_path",
    "state",
    "enabled",
  ]);
  return value.task_states.some((entry) => {
    if (
      !plainObject(entry) ||
      Object.keys(entry).length !== taskStateFields.size ||
      Object.keys(entry).some((field) => !taskStateFields.has(field)) ||
      typeof entry.task_name !== "string" ||
      !entry.task_name.trim() ||
      typeof entry.task_path !== "string" ||
      !entry.task_path.trim() ||
      typeof entry.enabled !== "boolean"
    ) {
      return true;
    }
    return !QUIESCENCE_TASK_STATE_LABELS.has(
      String(entry.state ?? "")
        .trim()
        .toLowerCase(),
    );
...
function normaliseQuiescenceDiagnostics(value, profile = null) {
  if (!plainObject(value)) return null;
  const probe = String(value.probe || "");
  const kind = String(value.kind || "");
  if (
    !QUIESCENCE_DIAGNOSTIC_PROBES.has(probe) ||
    !QUIESCENCE_DIAGNOSTIC_KINDS.has(kind)
  ) {
    return null;
  }
  const reasons = [...new Set(Array.isArray(value.reasons) ? value.reasons : [])]
    .filter((reason) => QUIESCENCE_DIAGNOSTIC_REASONS.has(reason))
    .sort();
  if (!reasons.length) return null;
  return {
    probe,
    kind,
    pids: boundedPositivePids(value.pids),
    task_identities: boundedReviewedTaskIdentities(
      value.task_identities,
      profile,
    ),
    reasons,
  };
...
function quiescenceDiagnosticsForObservation(value, profile = null) {
  if (
    value?.available === true &&
    PROBE_NAMES.some(
      (probe) => value?.probe_attestations?.[probe] !== true,
    )
  ) {
    return {
      probe: "aggregate",
  ...
  if (value?.available !== true) {
    return (
      normaliseQuiescenceDiagnostics(value?.diagnostics, profile) || {
        probe: "aggregate",
        kind: "UNAVAILABLE",
        pids: [],
        task_identities: [],
  ...
  const taskStates = normaliseTaskStateEvidence(value.task_states, profile);
  if (
    taskStates.some(
      (entry) =>
        entry.task_name === UNREVIEWED_PULSE_TASK_IDENTITY ||
        entry.task_path === UNREVIEWED_PULSE_TASK_IDENTITY,
    )
  ) {
    return {
  ...
  const ownerPids = boundedPositivePids(value.owner_pids);
  const listenerPids = boundedPositivePids(value.listener_pids);
  const schedulerPids = boundedPositivePids(value.scheduler_process_pids);
  const enabledTasks = normaliseTaskIdentityEvidence(
    value.enabled_tasks,
    profile,
  );
  const runningTasks = normaliseTaskIdentityEvidence(
    value.running_tasks,
    profile,
  );
  const reasons = [];
  if (ownerPids.length) reasons.push("OWNER_PIDS_PRESENT");
  if (listenerPids.length) reasons.push("LISTENER_PIDS_PRESENT");
  if (schedulerPids.length) reasons.push("SCHEDULER_PROCESS_PIDS_PRESENT");
  if (enabledTasks.length) reasons.push("ENABLED_TASKS_PRESENT");
  if (runningTasks.length) reasons.push("RUNNING_TASKS_PRESENT");

  const requiredTasks = boundedTaskIdentities([
    profile?.task_name,
    ...(Array.isArray(profile?.conflicting_task_names)
      ? profile.conflicting_task_names
      : []),
  ]);
  const absentTasks = new Set(
    boundedReviewedTaskIdentities(value.absent_task_names, profile).map(
      (identity) => identity.toLowerCase(),
    ),
  );
  const expectedButNotAbsent = requiredTasks.filter(
    (identity) => !absentTasks.has(identity.toLowerCase()),
  );
  if (expectedButNotAbsent.length) reasons.push("EXPECTED_TASK_NOT_ABSENT");
  if (!reasons.length) return null;

  return {
    probe: "aggregate",
    kind: "OCCUPIED",
    pids: boundedPositivePids([
      ...ownerPids,
      ...listenerPids,
      ...schedulerPids,
    ]),
    task_identities: boundedReviewedTaskIdentities(
      [...enabledTasks, ...runningTasks, ...expectedButNotAbsent],
      profile,
    ),
    reasons,
  };
...
function normaliseQuiescence(value, profile = null) {
  const malformed = quiescenceObservationMalformed(value);
  const taskStates = normaliseTaskStateEvidence(value?.task_states, profile);
  const presentReviewedTasks = new Set(
    taskStates
      .flatMap((entry) => [entry.task_name, entry.task_path])
      .map((identity) => reviewedTaskIdentity(identity, profile))
      .filter(Boolean)
  ...
  const report = {
    ...
    verdict:
      value?.available === true && !malformed ? "GREEN" : "HOLD",
    state:
      value?.available === true && !malformed ? "STOPPED_BOUND" : "HOLD",
    ...
    database_snapshot_sha256:
      ...
      !malformed &&
      SHA256_PATTERN.test(String(value?.database_snapshot_sha256 || ""))
        ? value.database_snapshot_sha256
        : null,
    blockers:
      value?.available === true && !malformed
        ? []
        : ["exact_plan_runtime_quiescence_inspection_unavailable"],
    available: value?.available === true && !malformed,
    probe_attestations: {},
  };
  for (const probe of PROBE_NAMES) {
    report.probe_attestations[probe] =
      !malformed && value?.probe_attestations?.[probe] === true;
  }
  report.owner_pids = boundedPositivePids(value?.owner_pids);
  report.listener_pids = boundedPositivePids(value?.listener_pids);
  report.scheduler_process_pids = boundedPositivePids(
    value?.scheduler_process_pids,
  );
  report.enabled_tasks = normaliseTaskIdentityEvidence(
    [
      ...(Array.isArray(value?.enabled_tasks) ? value.enabled_tasks : []),
      ...taskStates
        .filter((entry) => entry.enabled)
        .map((entry) => entry.task_path),
    ],
    profile,
  );
  report.running_tasks = normaliseTaskIdentityEvidence(
    [
      ...(Array.isArray(value?.running_tasks) ? value.running_tasks : []),
      ...taskStates
        .filter((entry) => ["Unknown", "Queued", "Running"].includes(entry.state))
        .map((entry) => entry.task_path),
    ],
    profile,
  );
  report.task_states = taskStates;
  report.absent_task_names = boundedReviewedTaskIdentities(
    value?.absent_task_names,
    profile,
  ).filter(
    (identity) => !presentReviewedTasks.has(identity.toLowerCase()),
  );
  const derivedDiagnostics = quiescenceDiagnosticsForObservation(
    { ...report, diagnostics: null },
    profile,
  );
  const suppliedDiagnosticsPresent =
    value?.diagnostics !== undefined && value?.diagnostics !== null;
  const suppliedDiagnostics = suppliedDiagnosticsPresent
    ? normaliseQuiescenceDiagnostics(value.diagnostics, profile)
    : null;
  const comparableDerivedDiagnostics = derivedDiagnostics
    ? normaliseQuiescenceDiagnostics(derivedDiagnostics, profile)
    : null;
  const diagnosticsContradictObservation =
    report.available &&
    suppliedDiagnosticsPresent &&
    (!suppliedDiagnostics ||
      JSON.stringify(suppliedDiagnostics) !==
        JSON.stringify(comparableDerivedDiagnostics));
  const derivedInspectionUnavailable =
    report.available &&
    ["AMBIGUOUS", "UNAVAILABLE"].includes(derivedDiagnostics?.kind);
  if (diagnosticsContradictObservation) {
    report.available = false;
    for (const probe of PROBE_NAMES) {
      report.probe_attestations[probe] = false;
    }
    report.diagnostics =
      suppliedDiagnostics ||
  ...
  } else if (derivedInspectionUnavailable) {
    report.available = false;
    if (PROBE_NAMES.includes(derivedDiagnostics.probe)) {
      report.probe_attestations[derivedDiagnostics.probe] = false;
    } else {
      for (const probe of PROBE_NAMES) {
        report.probe_attestations[probe] = false;
      }
    }
    report.diagnostics = derivedDiagnostics;
  } else if (report.available) {
    report.diagnostics = derivedDiagnostics;
  } else {
    report.diagnostics = quiescenceDiagnosticsForObservation(
      {
        ...report,
        diagnostics: malformed ? null : value?.diagnostics,
      },
      profile,
    );
  }
...
function assertQuiescent(value, profile = null) {
  ...
  const absentTasks = new Set(
    boundedTaskIdentities(report.absent_task_names).map((identity) =>
...
function unavailableWindowsQuiescence() {
  return {
    schema: BOUNDED_AUTHORITY_SCHEMA,
    verdict: "HOLD",
    state: "HOLD",
    ...
    blockers: ["exact_plan_runtime_quiescence_inspection_unavailable"],
    available: false,
    probe_attestations: Object.fromEntries(
      PROBE_NAMES.map((name) => [name, false]),
    ),
    owner_pids: [],
    listener_pids: [],
    scheduler_process_pids: [],
...
function canonicalBoundedExecutable(
  executablePath,
  fileSystemSync = fs,
) {
  const resolved = path.resolve(String(executablePath || ""));
  const stat = fileSystemSync.lstatSync(resolved, { bigint: true });
  if (
    !stat.isFile() ||
    stat.isSymbolicLink() ||
    (typeof stat.nlink === "bigint" && stat.nlink !== 1n)
  ) {
    throw new Error("bounded_node_identity_invalid");
  }
  const real = path.resolve(fileSystemSync.realpathSync.native(resolved));
  if (!samePath(real, resolved)) {
    throw new Error("bounded_node_identity_link_forbidden");
  }
  return real;
}

function canonicalBoundedCheckout(
  workspaceRoot,
  fileSystemSync = fs,
) {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink()) {
    throw new Error("bounded_checkout_identity_invalid");
  }
  const real = path.resolve(fileSystemSync.realpathSync.native(resolved));
  if (!samePath(real, resolved)) {
    throw new Error("bounded_checkout_identity_link_forbidden");
  }
  return real;
}

function fixedBoundedPowerShellRunner(execFileSyncImpl = execFileSync) {
  return async ({ script } = {}) =>
    String(
      execFileSyncImpl(
        "powershell.exe",
...
async function inspectBoundedWindowsTaskPresence({
  taskName,
  runPowerShell,
} = {}) {
  if (taskName !== LIVE_TASK_NAME || typeof runPowerShell !== "function") {
    return { available: false, present: null };
  }
  ...
  const script = [
    ...
    `$matches = @($folder.GetTasks(1) | Where-Object { $_.Name -ieq ${literalTaskName} })`,
    "$exactName = if ($matches.Count -eq 1) { [string]$matches[0].Name } else { $null }",
    "[pscustomobject]@{Present=($matches.Count -eq 1);ExactName=$exactName;MatchCount=[int]$matches.Count} | ConvertTo-Json -Compress",
  ].join("; ");
  try {
    const value = JSON.parse(await runPowerShell({ script }));
    const keys = Object.keys(value || {}).sort();
    if (
      !plainObject(value) ||
      JSON.stringify(keys) !==
        JSON.stringify(["ExactName", "MatchCount", "Present"]) ||
      typeof value.Present !== "boolean" ||
      typeof value.MatchCount !== "number" ||
      !Number.isInteger(Number(value.MatchCount)) ||
      Number(value.MatchCount) < 0 ||
      Number(value.MatchCount) > 1 ||
      value.Present !== (Number(value.MatchCount) === 1) ||
      (value.Present && value.ExactName !== taskName) ||
      (!value.Present && value.ExactName !== null)
    ) {
      return { available: false, present: null };
    }
    return {
...
async function inspectExactPlanInFlightDatabaseAuthority({
  db,
  mode,
  expected,
  now,
  ...
  const inspected = await baseInspector({ mode, expected, now });
  if (!allowedExactClaim) return inspected;
  const jobId = Number(allowedExactClaim.job_id);
  const jobRunId = Number(allowedExactClaim.job_run_id);
  const workerId = String(allowedExactClaim.worker_id || "");
  const invalid = (blocker) => ({
    ...(plainObject(inspected) ? inspected : {}),
    ok: false,
    database_identity_sha256:
      inspected?.database_identity_sha256 ||
  ...
  if (
    ...
    jobId <= 0 ||
    !Number.isInteger(jobRunId) ||
    jobRunId <= 0 ||
    !WORKER_ID_PATTERN.test(workerId) ||
    !plainObject(inspected) ||
    inspected.schema !== BOUNDED_RUNTIME_DB_AUTHORITY_SCHEMA ||
    inspected.mode !== "QUIESCENT" ||
    inspected.database_identity_sha256 !==
      expected?.database_identity_sha256 ||
    !SHA256_PATTERN.test(String(inspected.snapshot_sha256 || "")) ||
    !Array.isArray(inspected.blockers)
  ) {
    return invalid("runtime_db_exact_plan_inflight_authority_invalid");
  }
  const blockers = [...new Set(inspected.blockers)].sort();
  if (
    inspected.ok !== false ||
    ...
    return invalid("runtime_db_exact_plan_inflight_authority_mismatch");
  }
  try {
    const activeJobs = db
      .prepare(
        `SELECT id, kind, status, claimed_by
           FROM main.jobs
    ...
    if (
      activeJobs.length !== 1 ||
      Number(activeJobs[0].id) !== jobId ||
      activeJobs[0].kind !== "produce_breaking_short" ||
      !["claimed", "running"].includes(activeJobs[0].status) ||
      activeJobs[0].claimed_by !== workerId ||
      openRuns.length !== 1 ||
      Number(openRuns[0].id) !== jobRunId ||
      Number(openRuns[0].job_id) !== jobId ||
      openRuns[0].worker_id !== workerId ||
      openRuns[0].status !== "running"
    ) {
      return invalid("runtime_db_exact_plan_inflight_authority_mismatch");
    }
  } catch {
    return invalid("runtime_db_exact_plan_inflight_authority_unavailable");
  }
  return {
    ...inspected,
    ok: true,
...
async function boundedAbsentReceipt({
  receiptPath,
  fileSystem = defaultFileSystem,
  owner = false,
} = {}) {
  try {
    await inspectAbsentNoLinkPath(receiptPath, fileSystem);
    return owner
      ? { state: "absent", blockers: [] }
      : { present: false, blockers: [] };
  } catch {
    return owner
      ? { state: "invalid", blockers: ["bounded_owner_receipt_not_absent"] }
      : {
          present: true,
          blockers: ["bounded_activation_receipt_not_absent"],
        };
  }
}

function exactStoppedBoundedAuthority(value) {
  return (
    plainObject(value) &&
    value.schema === BOUNDED_AUTHORITY_SCHEMA &&
    value.verdict === "GREEN" &&
...
async function inspectWindowsPulseQuiescence({
  profile,
  workspaceRoot,
  ...
  now = new Date(),
  allowedExactClaim = null,
  boundedAuthorityInspector = inspectBoundedWindowsAuthority,
  taskDefinitionInspector = null,
  taskInstancesInspector = null,
  conflictInspector = null,
  activationReceiptInspector = null,
  ownerReceiptInspector = null,
  listenerInspector = null,
  databaseAuthorityInspector = null,
} = {}) {
  if (platform !== "win32") {
    return unavailableWindowsQuiescence();
  }
  try {
    const exactCheckout =
      checkoutRealPath ||
      canonicalBoundedCheckout(workspaceRoot, fileSystemSync);
    const exactNode =
      nodeRealPath ||
      canonicalBoundedExecutable(nodeExecutable, fileSystemSync);
    ...
    const expected = {
      ...
      nodePath: exactNode,
      checkoutRealPath: exactCheckout,
      releaseSha: String(expectedCommit || "")
        .trim()
        .toLowerCase(),
      profileSha256: profileFingerprint(profile),
      databaseIdentitySha256: databaseIdentity,
      conflictingTaskNames: Array.isArray(profile?.conflicting_task_names)
        ? [...profile.conflicting_task_names]
        : [],
    };
    const runPowerShell = fixedBoundedPowerShellRunner(execFileSyncImpl);
    const inspectExactTaskPresence = () =>
      inspectBoundedWindowsTaskPresence({
        taskName: expected.taskName,
        runPowerShell,
      });
    const inspectDatabaseAuthority =
    ...
    const probes = {
      taskDefinitionInspector:
        taskDefinitionInspector ||
        (async () => {
          const presence = await inspectExactTaskPresence();
          ...
          const inspected = inspectLiveWindowsTask({
            profile,
            repoRoot: exactCheckout,
            expectedCommit: expected.releaseSha,
            nodeExecutable: exactNode,
            execFileSyncImpl,
          });
          if (["absent", "unavailable"].includes(inspected?.state)) {
            return {
              task_name: expected.taskName,
              state: "unavailable",
              blockers: ["windows_task_inspection_unavailable"],
      ...
      taskInstancesInspector:
        ...
        (async ({ taskName }) => {
          ...
          return inspectExactWindowsTaskInstances({
            taskName,
            runPowerShell,
          });
        }),
      ...
      listenerInspector:
        listenerInspector ||
        ((options) => {
          const result = inspectWindowsListeners({
            ...options,
            execFileSyncImpl,
          });
          return {
      ...
      databaseAuthorityInspector:
        ({ mode, expected: authorityExpected }) =>
          inspectExactPlanInFlightDatabaseAuthority({
            db,
            mode,
            expected: authorityExpected,
            now,
            allowedExactClaim,
            baseInspector: inspectDatabaseAuthority,
          }),
    };
    const bounded = await boundedAuthorityInspector({
      mode: "QUIESCENT",
      expected,
      probes,
    });
    if (!exactStoppedBoundedAuthority(bounded)) {
      return unavailableWindowsQuiescence();
    }
    return normaliseQuiescence(
      {
        ...bounded,
        available: true,
        probe_attestations: {
...
async function exactRoot(rootPath, fileSystem) {
  ...
  return Object.freeze({
    path: rootPath,
    real_path: real,
    device: exactBigIntStatField(
      stat.dev,
      "exact_plan_drain_workspace_root_identity_invalid",
    ).toString(),
    inode: exactBigIntStatField(
      stat.ino,
      "exact_plan_drain_workspace_root_identity_invalid",
    ).toString(),
  });
...
async function assertExactRootBinding(expected, fileSystem) {
  const observed = await exactRoot(expected.path, fileSystem);
  if (
    !samePath(observed.real_path, expected.real_path) ||
    observed.device !== expected.device ||
    observed.inode !== expected.inode
  ) {
    fail("exact_plan_drain_workspace_root_changed");
  }
  return expected;
}
...
async function exactFile(filePath, root, fileSystem, outsideCode) {
  ...
  if (
    exactBigIntStatField(
      before.nlink,
      "exact_plan_drain_input_file_identity_invalid",
    ) !== 1n
  ) {
    fail("exact_plan_drain_input_hardlink_forbidden");
  }
  const beforeReal = path.resolve(await fileSystem.realpath(filePath));
  if (!samePath(beforeReal, filePath)) {
    fail("exact_plan_drain_input_link_forbidden");
  }
  if (!pathWithin(root.real_path, beforeReal)) fail(outsideCode);
  const bytes = await fileSystem.readFile(filePath);
  let after;
  try {
    after = await fileSystem.lstat(filePath, { bigint: true });
  } catch (error) {
    fail("exact_plan_drain_input_changed_during_read", error);
  }
  const afterReal = path.resolve(await fileSystem.realpath(filePath));
  const beforeIdentity = {
    ...
    link_count: exactBigIntStatField(
      before.nlink,
      "exact_plan_drain_input_file_identity_invalid",
    ),
    size: exactBigIntStatField(
      before.size,
      "exact_plan_drain_input_file_identity_invalid",
    ),
  };
  ...
  if (
    ...
    afterIdentity.link_count !== 1n ||
    beforeIdentity.device !== afterIdentity.device ||
    beforeIdentity.inode !== afterIdentity.inode ||
    beforeIdentity.size !== afterIdentity.size ||
    beforeIdentity.size !== BigInt(bytes.length) ||
    !samePath(afterReal, beforeReal)
  ) {
    fail("exact_plan_drain_input_changed_during_read");
  }
  return bytes;
}
...
async function ensureOutputRoot(root, outputDir, fileSystem) {
  ...
  for (const part of relative.split(path.sep).filter(Boolean)) {
    ...
    binding = {
      ...
      device: exactBigIntStatField(
        stat.dev,
        "exact_plan_drain_output_root_identity_invalid",
      ).toString(),
      inode: exactBigIntStatField(
        stat.ino,
        "exact_plan_drain_output_root_identity_invalid",
...
async function assertOutputRootBinding(root, outputDir, expected, fileSystem) {
  if (!expected || !samePath(expected.path, outputDir)) {
    fail("exact_plan_drain_output_root_changed");
  }
  let stat;
  try {
    stat = await fileSystem.lstat(outputDir, { bigint: true });
  } catch (error) {
    fail("exact_plan_drain_output_root_changed", error);
  }
  if (!stat.isDirectory() || stat.isSymbolicLink()) {
    fail("exact_plan_drain_output_root_changed");
  }
  const real = path.resolve(await fileSystem.realpath(outputDir));
  if (
    !samePath(real, outputDir) ||
    !samePath(real, expected.real_path) ||
    !pathWithin(root.real_path, real) ||
    exactBigIntStatField(
      stat.dev,
      "exact_plan_drain_output_root_identity_invalid",
    ).toString() !== expected.device ||
    exactBigIntStatField(
      stat.ino,
      "exact_plan_drain_output_root_identity_invalid",
    ).toString() !== expected.inode
  ) {
    fail("exact_plan_drain_output_root_changed");
  }
  return expected;
...
function runnerReposProvider(repos, state, hooks = {}) {
  ...
  const workers = repos.workers;
  const claimPermitClock =
    typeof hooks.claimPermitClock === "function"
      ? hooks.claimPermitClock
      : () => Date.now();
  const authorityContextProvider =
    typeof hooks.authorityContextProvider === "function"
      ? hooks.authorityContextProvider
      : () => state.authority_context_sha256;
    ...
    get(target, property) {
      if (property === "claim") {
        return (workerId, options) => {
          ...
          const permit = state.claim_permit;
          if (!permit) return null;
          state.claim_permit = null;
          const nowMs = Number(claimPermitClock());
          const currentAuthorityContext = authorityContextProvider();
          if (
            !Number.isFinite(nowMs) ||
            permit.consumed !== false ||
            permit.plan_index !== state.next_index ||
            permit.job_id !== Number(expected.job_id) ||
            permit.worker_id !== workerId ||
            permit.authority_fingerprint !== currentAuthorityContext ||
            permit.authority_fingerprint !==
              state.authority_context_sha256 ||
            !SHA256_PATTERN.test(String(permit.observation_sha256 || "")) ||
            !SHA256_PATTERN.test(
              String(permit.database_snapshot_sha256 || ""),
            ) ||
            nowMs < permit.issued_at_ms ||
            nowMs > permit.expires_at_ms
          ) {
            state.execution_error = "exact_plan_claim_authority_permit_invalid";
            return null;
          }
          ...
          if (!claimed) {
            state.execution_error = "exact_plan_claim_authority_permit_refused";
            return null;
          }
          ...
          state.active_claim = {
            job_id: Number(claimed.id),
            job_run_id: Number(claimed.claim_token),
            worker_id: workerId,
          };
      ...
      if (property === "complete") {
        return (jobId, workerId, claimToken, options) => {
          ...
          if (
            !expected ||
            Number(jobId) !== Number(expected.job_id) ||
            state.active_claim?.job_id !== Number(jobId) ||
            state.active_claim?.job_run_id !== Number(claimToken) ||
            state.active_claim?.worker_id !== workerId
          ) {
      ...
      if (property === "fail") {
        return (jobId, workerId, claimToken, error, options) => {
          const failed = target.fail(
            jobId,
            workerId,
            claimToken,
            error,
  ...
  // JobsRunner normally writes dashboard-only worker presence before it tries
  // to claim. This governed LOCAL_PROOF lane authorises only the two exact job
  // transitions, so keep worker reads available while fencing every registry
  // mutation outside the synchronous permit + claimExact boundary.
  const localProofWorkers = new Proxy(workers, {
    get(target, property) {
      if (["register", "heartbeat", "logEvent"].includes(property)) {
        return () => null;
      }
      const value = Reflect.get(target, property);
      return typeof value === "function" ? value.bind(target) : value;
  ...
  return () => ({ ...repos, jobs: gatedJobs, workers: localProofWorkers });
}
...
function renderMarkdown(report) {
  ...
  lines.push(
    ...
    "- Durable database mutations were limited to that transition lease and ordinary lease-fenced status transitions for the two exact production jobs; worker-registry writes were fenced out.",
    "",
  );
  return `${lines.join("\n")}\n`;
}
...
async function readExactOptionalFile(filePath, fileSystem) {
  ...
  const beforeLinks = exactBigIntStatField(
    before.nlink,
    "exact_plan_drain_evidence_conflict",
  );
  if (beforeLinks !== 1n) {
    fail("exact_plan_drain_evidence_hardlink_forbidden");
  }
  const beforeReal = path.resolve(await fileSystem.realpath(filePath));
  if (!samePath(beforeReal, filePath)) {
    fail("exact_plan_drain_evidence_conflict");
  ...
  try {
    ...
    fail("exact_plan_drain_evidence_conflict", error);
  }
  const afterReal = path.resolve(await fileSystem.realpath(filePath));
  if (
    !after.isFile() ||
...
async function assertNoCrashLeftEvidenceStagingFiles(
  outputDir,
  fileSystem,
  assertOutputRoot,
) {
  const stagingNamePattern =
    /^exact-plan-drain(?:-reservation)?(?:\.commit)?(?:\.json|\.md)\.[a-f0-9]{64}\.tmp$/;
  await assertOutputRoot();
  const entries = await fileSystem.readdir(outputDir, { withFileTypes: true });
  await assertOutputRoot();
  if (entries.some((entry) => stagingNamePattern.test(String(entry?.name || "")))) {
    fail("exact_plan_drain_evidence_staging_conflict");
  }
}
...
function validateFinalReport(value, attemptSha256, expectedGeneratedAt) {
  if (
    ...
    !SHA256_PATTERN.test(
      String(
        value.runtime_transition_lease.authority_context_sha256 || "",
      ),
    ) ||
...
async function validateExactEvidenceComponent(filePath, expectedBytes, fileSystem) {
  const observed = await readExactOptionalFile(filePath, fileSystem);
  if (!observed?.equals(expectedBytes)) {
    fail("exact_plan_drain_evidence_changed_during_commit");
  }
  return observed;
}

async function invalidateExactEvidenceSet(
  paths,
  fileSystem,
  assertOutputRoot,
) {
  for (const filePath of [paths.commit, paths.markdown, paths.json]) {
    await assertOutputRoot();
    try {
      await fileSystem.unlink(filePath);
    } catch (error) {
      if (error?.code !== "ENOENT") {
        fail("exact_plan_drain_evidence_invalidation_failed", error);
      }
    }
  }
  await assertOutputRoot();
...
async function writeEvidence(
  ...
  const markdownPath = path.join(outputDir, "exact-plan-drain.md");
  const commitPath = path.join(outputDir, "exact-plan-drain.commit.json");
  const paths = { json: jsonPath, markdown: markdownPath, commit: commitPath };
  const fence = async () => {
    await assertOutputRoot();
    await assertFinalAuthority({ record: false });
    await assertOutputRoot();
  };
  ...
  await validateExactEvidenceComponent(jsonPath, jsonBytes, fileSystem);
  ...
  await validateExactEvidenceComponent(markdownPath, markdownBytes, fileSystem);
  await fence();
  ...
  try {
    commitStatus = await writeCrashSafeNoClobber(
      ...
      fileSystem,
      {
        beforeLink: async () => {
          await fence();
          await validateExactEvidenceComponent(jsonPath, jsonBytes, fileSystem);
          await validateExactEvidenceComponent(
            markdownPath,
            markdownBytes,
            fileSystem,
          );
    ...
    await invalidateExactEvidenceSet(paths, fileSystem, assertOutputRoot);
    throw error;
  }
  try {
    await fence();
    ...
    await validateExactEvidenceComponent(commitPath, commitBytes, fileSystem);
  } catch (error) {
    await invalidateExactEvidenceSet(paths, fileSystem, assertOutputRoot);
    fail("exact_plan_drain_evidence_changed_during_commit", error);
  }
  return {
    json_path: jsonPath,
    json_file_sha256: sha256Bytes(jsonBytes),
...
async function finaliseReplayedEvidence(
  ...
  const paths = {
    json: existing.jsonPath,
    markdown: existing.markdownPath,
    commit: existing.commitPath,
  };
  const fence = async () => {
    await assertOutputRoot();
    await assertFinalAuthority({ record: false });
...
async function drainExactGovernedProductionPlan(value, dependencies = {}) {
  ...
  const outputRootBinding = await ensureOutputRoot(
    root,
    request.output_dir,
    fileSystem,
  );
  const assertOutputRoot = () =>
    assertOutputRootBinding(
      root,
      request.output_dir,
      outputRootBinding,
      fileSystem,
  ...
  let lineageBinding = null;
  let validateFinalAuthority = null;
  ...
  let transitionAuthorityContextSha256 = null;
  ...
  try {
    ...
    lineageBinding = await validatePlanLineageBinding({
      plan,
      planPath: request.plan_path,
      root,
      fileSystem,
    ...
    const inspectWorkspaceAuthority = () => {
      const observed = (dependencies.workspaceInspector || inspectWorkspace)({
        workspaceRoot: root.path,
      });
      if (
        observed?.available !== true ||
        observed.commit !== request.expected_checkout_commit ||
        observed.tracked_clean !== true
      ) {
        fail("exact_plan_workspace_checkout_mismatch");
      }
      workspaceInspection = observed;
      return observed;
    };
    const bindRuntimeProfileAuthority = async () => {
      await assertExactRootBinding(root, fileSystem);
      const reboundBytes = await exactFile(
        request.runtime_profile_path,
        root,
        fileSystem,
        "exact_plan_drain_runtime_profile_outside_workspace",
      );
      const reboundSha256 = sha256Bytes(reboundBytes);
      if (reboundSha256 !== request.expected_runtime_profile_file_sha256) {
        fail("exact_plan_runtime_profile_file_sha256_mismatch");
      }
      const reboundProfile = parseJsonBytes(
        reboundBytes,
        "exact_plan_runtime_profile_json_invalid",
      );
      ...
      if (
        reboundValidation?.valid !== true ||
        (Array.isArray(reboundValidation?.blockers) &&
          reboundValidation.blockers.length)
      ) {
        fail("exact_plan_runtime_profile_invalid");
      }
      if (
        !samePath(reboundProfile.database_path, request.database_path) ||
        !samePath(
          normalEmbeddedPath(
            reboundProfile.activation_receipt_path,
            "exact_plan_activation_receipt_path_invalid",
          ),
      ...
      return reboundProfile;
    };
    const validateRuntimeExecutionAuthority = async () => {
      await bindRuntimeProfileAuthority();
      inspectWorkspaceAuthority();
      await assertOutputRoot();
    };
    const validateImmutableFileAuthorities = async () => {
      await assertExactRootBinding(root, fileSystem);
      const reboundPlanBytes = await exactFile(
        request.plan_path,
        root,
        fileSystem,
        "exact_plan_drain_plan_outside_workspace",
      );
      if (sha256Bytes(reboundPlanBytes) !== request.expected_plan_file_sha256) {
        fail("exact_plan_file_sha256_mismatch");
      }
      const reboundPlan = validatePlan(
        parseJsonBytes(reboundPlanBytes, "exact_plan_json_invalid"),
        request.expected_plan_sha256,
      );
      const reboundLineage = await validatePlanLineageBinding({
        plan: reboundPlan,
        planPath: request.plan_path,
        root,
        fileSystem,
      });
      const reboundReservation = await validateReservationBinding({
        plan: reboundPlan,
        root,
        fileSystem,
      });
      if (
        canonicalSha256(reboundLineage.inspection) !==
          canonicalSha256(lineageInspection) ||
        canonicalSha256(reboundReservation) !==
          canonicalSha256(reservationInspection)
      ) {
        fail("exact_plan_immutable_authority_binding_mismatch");
      }
      await validateRuntimeExecutionAuthority();
      return reboundLineage;
    };
    inspectWorkspaceAuthority();
    ...
    let liveDatabaseIdentity;
    try {
      liveDatabaseIdentity = (
        dependencies.liveDatabaseIdentityInspector ||
        inspectLiveDatabaseIdentity
      )({
        databasePath: requestedDatabase.path,
      });
    } catch {
      fail("exact_plan_runtime_database_identity_unavailable");
    }
    if (
      !SHA256_PATTERN.test(
        String(liveDatabaseIdentity?.database_identity_sha256 || ""),
      )
    ) {
      fail("exact_plan_runtime_database_identity_unavailable");
    }
    let boundedCheckoutRealPath;
    let boundedNodeRealPath;
    try {
      boundedCheckoutRealPath = (
        dependencies.boundedCheckoutInspector || canonicalBoundedCheckout
      )(root.path);
      boundedNodeRealPath = (
        dependencies.boundedNodeInspector || canonicalBoundedExecutable
      )(dependencies.nodeExecutable || process.execPath);
    } catch {
      fail("exact_plan_runtime_static_authority_unavailable");
    }
    ...
    const inspectNow = async ({
      record = true,
      requireTransitionLease = true,
      allowedExactClaim = null,
    } = {}) => {
      await validateRuntimeExecutionAuthority();
      if (requireTransitionLease) {
        assertTransitionLease();
      }
      ...
      try {
        observed = normaliseQuiescence(
          await inspectQuiescence({
            profile,
            workspaceRoot: root.path,
            checkoutRealPath: boundedCheckoutRealPath,
            expectedCommit: request.expected_checkout_commit,
            nodeExecutable: dependencies.nodeExecutable || process.execPath,
            nodeRealPath: boundedNodeRealPath,
            databaseIdentitySha256:
              liveDatabaseIdentity.database_identity_sha256,
            db,
            fileSystem,
            ...
            boundedAuthorityInspector:
              dependencies.boundedAuthorityInspector,
            taskDefinitionInspector:
              dependencies.taskDefinitionInspector,
            taskInstancesInspector: dependencies.taskInstancesInspector,
            conflictInspector: dependencies.conflictInspector,
            activationReceiptInspector:
              dependencies.activationReceiptInspector,
            ownerReceiptInspector: dependencies.ownerReceiptInspector,
            listenerInspector: dependencies.listenerInspector,
            databaseAuthorityInspector:
              dependencies.databaseAuthorityInspector,
        ...
        if (record) {
          quiescenceChecks.push({
            sequence,
            ...normaliseQuiescence(null, profile),
          });
        }
        throw error;
      }
      ...
      if (
        transitionAuthorityContextSha256 !== null &&
        observed.authority_fingerprint !==
          transitionAuthorityContextSha256
      ) {
        fail("exact_plan_runtime_authority_context_drift");
      }
    ...
    const initialStoppedAuthority = await inspectNow({
      requireTransitionLease: false,
    });
    transitionAuthorityContextSha256 =
      initialStoppedAuthority.authority_fingerprint;
    try {
      transitionLease = (
        ...
        authorityContextSha256: transitionAuthorityContextSha256,
        metadata: {
    ...
    await assertNoCrashLeftEvidenceStagingFiles(
      request.output_dir,
      fileSystem,
      assertOutputRoot,
    );
    ...
    if (
      ...
      state = {
        ...
        authority_context_sha256: transitionAuthorityContextSha256,
        claim_permit: null,
        active_claim: null,
      ...
      const reposProvider = runnerReposProvider(repos, state, {
        ...
        claimPermitClock: dependencies.claimPermitClock,
        authorityContextProvider:
          dependencies.claimPermitAuthorityContextProvider,
      });
      ...
      const claimPermitClock =
        typeof dependencies.claimPermitClock === "function"
          ? dependencies.claimPermitClock
          : () => Date.now();
      const issueClaimPermit = async () => {
        if (
          state.active_claim ||
          state.claim_permit ||
          state.next_index >= expectedJobs.length
        ) {
          fail("exact_plan_claim_authority_permit_state_invalid");
        }
        const expected = expectedJobs[state.next_index];
        const observed = await inspectNow();
        const issuedAtMs = Number(claimPermitClock());
        if (!Number.isFinite(issuedAtMs)) {
          fail("exact_plan_claim_authority_permit_clock_invalid");
        }
        state.claim_permit = {
          consumed: false,
          plan_index: state.next_index,
          job_id: Number(expected.job_id),
          worker_id: request.worker_id,
          authority_fingerprint: observed.authority_fingerprint,
          observation_sha256: observed.observation_sha256,
          database_snapshot_sha256: observed.database_snapshot_sha256,
          issued_at_ms: issuedAtMs,
          expires_at_ms:
            issuedAtMs + CLAIM_AUTHORITY_PERMIT_MAX_AGE_MS,
        };
      };
      await runner.start();
      await issueClaimPermit();
      while (
        state.next_index < expectedJobs.length &&
        !state.execution_error &&
        Date.now() < deadline
        ...
        if (
          ...
          await inspectNow({
            allowedExactClaim: state.active_claim,
          });
    ...
    validateFinalAuthority = async ({ record = true } = {}) => {
      const reboundLineage = await validateImmutableFileAuthorities();
      await inspectNow({ record });
      validatePredecessorQueueBindings({
        db,
        predecessorPlans: reboundLineage.predecessor_plans,
        workspaceRoot: root.path,
      });
      validateQueueBindings({ db, plan, workspaceRoot: root.path });
      const rebound = await recoverDurablePrefix({
        db,
        plan,
        attemptSha256,
        root,
      ...
      const reboundJobs = executionJobsFromRows(plan, rebound.rows);
      if (
        rebound.recovered.length !== plan.production_jobs.length ||
        canonicalSha256(reboundJobs) !== canonicalSha256(execution.jobs)
      ) {
        fail("exact_plan_final_result_attestation_mismatch");
      }
      await inspectNow({ record });
      await validateImmutableFileAuthorities();
      return rebound;
    };
  ...
  try {
    return await (async () => {
      ...
      const evidence = await writeEvidence(report, {
        ...
        fileSystem,
        assertTransitionLease,
        assertFinalAuthority: validateFinalAuthority,
        assertOutputRoot,
      });
      finalEvidenceDatabaseFence.commit();
