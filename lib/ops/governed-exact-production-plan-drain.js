...
const {
  ...
  "../stabilisation/windows-local-runtime-supervisor"
);
const {
  acquireLiveRuntimeTransitionLease,
  LIVE_RUNTIME_TRANSITION_LEASE_NAME,
} = require(
  "../stabilisation/live-runtime-transition-lease"
);
...
function beginFinalEvidenceDatabaseFence(db) {
  ...
  try {
    ...
    fail(
      "exact_plan_final_evidence_database_fence_unavailable",
      error,
    );
  }
  ...
  return {
    ...
    commit() {
      ...
      try {
        ...
        if (db.inTransaction) {
          fail(
            "exact_plan_final_evidence_database_fence_commit_failed",
          );
        }
        active = false;
      } catch (error) {
    ...
    rollback() {
      ...
      try {
        ...
        if (db.inTransaction) {
          fail(
            "exact_plan_final_evidence_database_fence_rollback_failed",
          );
        }
        active = false;
      } catch (error) {
...
function isCanonicalCodexPowerShellProcessIdentity(row) {
  return (
    String(row?.name || "").trim().toLowerCase() === "powershell.exe" &&
...
function isPulseRuntimeCommand(value, hints) {
  ...
  if (
    ...
    POWERSHELL_ENCODED_SWITCH_PATTERN.test(
      encodedSwitchDetectionCommand,
    )
  ) {
    return true;
  }
...
function isExactSafeOllamaTask(row) {
  ...
  const workingDirectory = normaliseWindowsCommand(
    action?.WorkingDirectory,
  );
  ...
  return (
    String(row?.TaskName || "").trim().toLowerCase() ===
    ...
    argumentMatch &&
    workingDirectory ===
      argumentMatch[1].replace(
        /\/tools\/windows-ollama-watchdog\.js$/,
        "",
      )
  );
}

function isExactSafeDesktopCommanderSupervisorTask(
  row,
  fileSystemSync,
) {
  const actions = taskActions(row);
...
function taskIsActive(row) {
  ...
  return ["0", "2", "4", "unknown", "queued", "running"].includes(
    state,
  );
}
...
function assertCanonicalLiveOwnerReceipt(
  ...
  if (
    ...
    String(owner.commit_sha || "").trim().toLowerCase() !== commit ||
    Number(owner.port) !== Number(profile?.port) ||
    String(owner.profile_sha256 || "").trim().toLowerCase() !==
  ...
  try {
    ...
    return {
      ...
      scheduler_process_pids: pulseProcesses
        .filter((row) =>
          isPulseSchedulerProcess(row, hints),
        )
...
function validateEvidenceCommit(
  value,
  ...
  const expected = buildEvidenceCommit(
    report,
    jsonBytes,
    markdownBytes,
  );
...
async function inspectFinalEvidence({
  ...
  const commitBytes = await readExactOptionalFile(
    commitPath,
    fileSystem,
  );
  if (!jsonBytes) {
...
async function drainExactGovernedProductionPlan(value, dependencies = {}) {
  ...
  try {
    ...
    if (blockers.length === 0) {
      finalEvidenceDatabaseFence =
        beginFinalEvidenceDatabaseFence(dependencies.db);
      await inspectNow();
      renewTransitionLease();
      validatePredecessorQueueBindings({
        db: dependencies.db,
  ...
  const evidence = await writeEvidence(report, {
    ...
    outputDir: request.output_dir,
    fileSystem,
    assertTransitionLease: renewTransitionLease,
  });
  finalEvidenceDatabaseFence.commit();
