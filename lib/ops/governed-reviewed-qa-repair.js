...
function inspectionBlockers(inspection, generatedAt) {
  ...
  const storedExtra = parseObject(inspection.raw_story?._extra);
  if (
    text(inspection.raw_story?.publish_error) !==
      EXPECTED_QA_PUBLISH_ERROR ||
    text(storedExtra.publish_error) !== EXPECTED_QA_PUBLISH_ERROR
  ) {
