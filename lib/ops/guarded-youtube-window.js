...
} = require("../services/youtube-safety");
...
function normalisePath(value) {
  const resolved = path.resolve(text(value)).replace(/\\/g, "/");
...
function inspectGuardedYoutubeDatabase({
  ...
  snapshot.schema_blockers.push(
    ...checkpointedSqliteBlockers(
      sqliteSidecarSnapshot(resolvedPath),
      { allowSharedMemory: false },
    ),
  );
  try {
    return withReadOnlySqliteSnapshot(resolvedPath, (db) => {
...
function isNormalCadenceSchedule(schedule) {
  return (
...
function outsideCadenceAuthorisation(value = {}) {
  return {
    authorisation_id: text(value.outsideCadenceAuthorisationId),
    confirmed_authorisation_id: text(
      value.confirmOutsideCadenceAuthorisationId,
    ),
    one_shot: value.confirmOutsideCadenceOneShot === true,
  };
}
...
function evaluateWindow(
  ...
  if (!isNormalCadenceSchedule(schedule)) {
    const authorisation = outsideCadenceAuthorisation(
      authorisationOptions,
    );
...
function evaluateScheduledRow(row, { generatedAt } = {}) {
  ...
  const persistedAuthorisation =
    evidence.outside_cadence_authorisation || null;
  ...
  const window = evaluateWindow(evidence.scheduled_for, generatedAt, {
    outsideCadenceAuthorisationId:
      persistedAuthorisation?.authorisation_id,
    confirmOutsideCadenceAuthorisationId:
      persistedAuthorisation?.authorisation_id,
    confirmOutsideCadenceOneShot:
      persistedAuthorisation?.one_shot === true,
  });
  blockers.push(...window.blockers);
  if (
    !Number.isNaN(scheduledFor.getTime()) &&
    !isNormalCadenceSchedule(scheduledFor)
  ) {
    blockers.push(
      ...validatePersistedOutsideCadenceAuthorisation(
        persistedAuthorisation,
        {
          ...
          now: generatedAt,
          dispatchIdempotencyKey: evidence.dispatch_idempotency_key,
  ...
  if (
    !isNormalCadenceSchedule(new Date(evidence.scheduled_for)) &&
    persistedAuthorisation?.basis !==
      "explicit_operator_goal_authorisation"
  ) {
    blockers.push("scheduled_outside_cadence_authorisation_invalid");
  }
  if (
...
function expectedIdentity(options, review) {
  return {
    ...
    outside_cadence_authorisation_id: text(
      options.outsideCadenceAuthorisationId,
    ),
    review_manifest_sha256: text(review?.review_manifest_sha256).toLowerCase(),
  };
}

function expectedConfirmations(options, review, fingerprint) {
  let scheduledFor = null;
  try {
    scheduledFor = new Date(options.scheduledFor).toISOString();
  } catch {
    return null;
  }
  const storyId = text(options.storyId || review?.story_id);
  if (!storyId) return null;
  const confirmations = {
    story_id: storyId,
    scheduled_for: scheduledFor,
    media_sha256: text(review?.media_sha256).toLowerCase(),
    script_sha256: text(review?.script_sha256).toLowerCase(),
    request_fingerprint: text(fingerprint?.request_fingerprint).toLowerCase(),
    renderer_manifest_sha256: text(
      review?.preflight_evidence?.renderer_manifest_sha256,
    ).toLowerCase(),
    source_evidence_sha256: text(
      review?.preflight_evidence?.source_evidence_sha256,
    ).toLowerCase(),
    dispatch_idempotency_key: `${PLATFORM}:${storyId}:${scheduledFor}`,
  };
  if (!isNormalCadenceSchedule(new Date(scheduledFor))) {
    confirmations.outside_cadence_authorisation_id = text(
      options.outsideCadenceAuthorisationId,
    );
    confirmations.outside_cadence_one_shot = true;
  }
...
function validateScheduledBinding(snapshot, identity, generatedAt) {
  ...
  if (targetEvaluation) {
    const evidence = targetEvaluation.row.evidence || {};
    const expectedPairs = [
      [
    ...
    if (!isNormalCadenceSchedule(new Date(identity.scheduled_for))) {
      expectedPairs.push([
        text(
          evidence.outside_cadence_authorisation
            ?.authorisation_id,
        ),
        identity.outside_cadence_authorisation_id,
...
function databaseBoundaryError(blockers) {
  const codes = unique(blockers);
  const error = new Error(codes[0] || "guarded_database_boundary_failed");
  error.code = codes[0] || "guarded_database_boundary_failed";
  error.blockers = codes;
  return error;
}

function databaseBoundaryBlockersFromError(
  error,
  fallback = "guarded_database_boundary_failed",
) {
  if (Array.isArray(error?.blockers) && error.blockers.length) {
    return unique(error.blockers);
  }
  if (
    text(error?.code).startsWith("source_database_") ||
    text(error?.code).startsWith("guarded_database_")
  ) {
...
function runGuardedSqliteBoundaryCheck({
  databasePath,
  ...
  try {
    ...
    return ["guarded_database_boundary_inspection_failed"];
  }
}

function assertGuardedSqliteTransactionBoundary({
  repos,
  databasePath,
  expectedSourceSha256,
  mutationBoundaryBlockers = sqliteMutationBoundaryBlockers,
} = {}) {
  if (!repos?.db || typeof repos.db.transaction !== "function") {
    throw databaseBoundaryError([
      "guarded_database_transaction_required",
    ]);
  }
  const transaction = repos.db.transaction(() => {
    const blockers = runGuardedSqliteBoundaryCheck({
      databasePath,
      expectedSourceSha256,
      allowSharedMemory: true,
      mutationBoundaryBlockers,
    });
    if (blockers.length) throw databaseBoundaryError(blockers);
    return readGuardedSqliteDataVersion(repos.db);
  });
  return transaction.immediate();
}

function readGuardedSqliteDataVersion(db) {
...
function resultBase({
  ...
  youtubeAuthTelemetry = createYoutubeAuthTelemetry(),
} = {}) {
  const authTelemetry =
    normaliseYoutubeAuthTelemetry(youtubeAuthTelemetry);
  return {
    ...
    media_sha256: identity?.media_sha256 || null,
    script_sha256: identity?.script_sha256 || null,
    request_fingerprint: identity?.request_fingerprint || null,
    renderer_manifest_sha256: identity?.renderer_manifest_sha256 || null,
    source_evidence_sha256: identity?.source_evidence_sha256 || null,
    dispatch_idempotency_key:
      identity?.dispatch_idempotency_key || null,
    outside_cadence_authorisation_id:
      identity?.outside_cadence_authorisation_id || null,
    review_manifest_sha256: identity?.review_manifest_sha256 || null,
    ...
    safety: {
      ...
      oauth_or_tokens_mutated:
        authTelemetry.durable_oauth_or_token_mutated,
      durable_oauth_or_token_mutated:
        authTelemetry.durable_oauth_or_token_mutated,
      ephemeral_youtube_access_token_refresh: {
        ...authTelemetry.ephemeral_access_token_refresh,
      },
...
async function executeGuardedYoutubeWindow(options = {}) {
  ...
  const earlyBlockers = [];
  const hasInjectedGeneratedAt =
    options.generatedAt !== undefined && options.generatedAt !== null;
  const testClockAllowed =
    text(env.NODE_ENV).toLowerCase() === "test" &&
    Object.keys(dependencies).length > 0;
  let generatedAt = new Date().toISOString();
  if (hasInjectedGeneratedAt && testClockAllowed) {
    const injectedClock = new Date(options.generatedAt);
    if (Number.isNaN(injectedClock.getTime())) {
      earlyBlockers.push("test_generated_at_invalid");
    } else {
      generatedAt = injectedClock.toISOString();
    }
  } else if (hasInjectedGeneratedAt) {
    earlyBlockers.push("operator_generated_at_forbidden");
  }
  const effective = {
    ...options,
    action,
  ...
  const mutationBoundaryBlockers =
    dependencies.mutationBoundaryBlockers ||
    sqliteMutationBoundaryBlockers;
  const assertTransactionBoundary =
    dependencies.assertTransactionBoundary ||
    assertGuardedSqliteTransactionBoundary;
  const expectedSourceSha256 = text(
    backup?.evidence?.source_database_sha256,
  ).toLowerCase();
  const preOpenDatabaseBlockers = () =>
    runGuardedSqliteBoundaryCheck({
      databasePath: effective.databasePath,
      expectedSourceSha256,
      allowSharedMemory: false,
      mutationBoundaryBlockers,
    });
  const transactionBoundaryCheck = () => {
    const boundaryBlockers = runGuardedSqliteBoundaryCheck({
      databasePath: effective.databasePath,
      expectedSourceSha256,
      allowSharedMemory: true,
      mutationBoundaryBlockers,
    ...
    if (boundaryBlockers.length) {
      throw databaseBoundaryError(boundaryBlockers);
    }
  };

  const preBlockers = unique(blockers);
  if (action === "inspect") {
    const confirmations = expectedConfirmations(effective, review, fingerprint);
    ...
    if (scheduledRows.length === 0) {
      ...
      result.expected_confirmations = confirmations;
      result.request_fingerprint =
        confirmations?.request_fingerprint || result.request_fingerprint;
      return result;
    }
  ...
  if (action === "admit") {
    ...
    const preOpenBlockers = preOpenDatabaseBlockers();
    ...
    let admission;
    try {
      handle = openRepositories(effective.databasePath);
      ...
      admission = await admitPublication({
        ...
        outsideCadenceAuthorisation: {
          authorisationId: text(
            effective.outsideCadenceAuthorisationId,
          ),
          confirmAuthorisationId: text(
            effective.confirmOutsideCadenceAuthorisationId,
          ),
          oneShotConfirmed:
            effective.confirmOutsideCadenceOneShot === true,
        },
      });
    } catch (error) {
      return resultBase({
        options: effective,
        identity,
        blockers: databaseBoundaryBlockersFromError(
          error,
    ...
    if (!isNormalCadenceSchedule(new Date(identity.scheduled_for))) {
      admissionPairs.push([
        admission?.outside_cadence_authorisation
          ?.authorisation_id,
        identity.outside_cadence_authorisation_id,
      ]);
      admissionPairs.push([
        admission?.outside_cadence_authorisation?.one_shot,
        true,
      ]);
    }
    if (
  ...
  if (dispatchBlockers.length === 0) {
    dispatchBlockers.push(...preOpenDatabaseBlockers());
  }
  ...
  let handle = null;
  let databaseDataVersion = null;
  try {
    handle = openRepositories(effective.databasePath);
    databaseDataVersion = assertTransactionBoundary({
      repos: handle.repos,
      databasePath: effective.databasePath,
      expectedSourceSha256,
      mutationBoundaryBlockers,
    ...
    if (
      !Number.isSafeInteger(databaseDataVersion) ||
    ...
    const result = resultBase({
      options: effective,
      identity,
      blockers: databaseBoundaryBlockersFromError(error),
    });
    result.selection_binding = scheduled.binding;
    result.scheduled_candidate = scheduled.candidate;
    return result;
  ...
  try {
    publisherResult = await publishNextStory({
      ...
      env: effective.env,
      actorId: text(effective.actorId),
      onYoutubeAuthTelemetry: reportYoutubeAuthTelemetry,
      exactDispatchBinding: Object.freeze({
        storyId: identity.story_id,
        platform: PLATFORM,
        scheduledFor: identity.scheduled_for,
        scheduledEventId: scheduled.candidate.scheduled_event_id,
        dispatchIdempotencyKey: identity.dispatch_idempotency_key,
        requestFingerprint: identity.request_fingerprint,
        databaseDataVersion,
      }),
    });
    if (publisherResult?.safety?.youtube_auth) {
      reportYoutubeAuthTelemetry(
        publisherResult.safety.youtube_auth,
      );
    }
  } catch {
    publisherError = true;
  ...
  if (youtubeAuthTelemetry.durable_oauth_or_token_mutated) {
    finalBlockers.push("durable_oauth_or_token_mutation_detected");
  }
...
function renderGuardedYoutubeWindowMarkdown(result) {
  ...
  if (result.expected_confirmations) {
    lines.push(
      "",
      "## Expected confirmations",
      "",
      "Copy these exact values into a second inspect/admit invocation only after reviewing this HOLD result:",
      "",
    );
    for (const [key, value] of Object.entries(
      result.expected_confirmations,
    )) {
      lines.push(`- ${key}: ${value || "unavailable"}`);
    }
  }
  lines.push("", "## Blockers", "");
