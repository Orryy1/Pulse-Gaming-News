...
function resolveLocalAcousticProbePythonPath({
  root = ROOT,
  env = process.env,
  platform = process.platform,
} = {}) {
  return resolveLocalTtsRuntimePaths({ root, env, platform }).pythonPath;
}
...
function probeMedianPitch(file) {
  const python = resolveLocalAcousticProbePythonPath();
  const code = [
    "import json, sys",
    "import numpy as np",
    "import librosa",
