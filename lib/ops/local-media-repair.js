...
const REPAIR_STORY_SUBJECTS = [
  { id: "gta_6", re: /\b(?:gta\s*6|gta\s*vi|grand\s+theft\s+auto\s+(?:6|vi)|rockstar'?s?\s+next\s+grand\s+theft\s+auto)\b/i },
  { id: "gta_5", re: /\b(?:gta\s*5|gta\s*v|grand\s+theft\s+auto\s+(?:5|v))\b/i },
  { id: "silent_hill", re: /\bsilent\s+hill\b/i },
  { id: "splatoon", re: /\bsplatoon\b/i },
  { id: "phasmophobia", re: /\bphasmophobia\b/i },
  { id: "avatar_legends", re: /\bavatar\s+legends\b/i },
  { id: "marvel_tokon", re: /\bmarvel\s+tokon\b/i },
  { id: "halo", re: /\bhalo\b/i },
  { id: "gears_of_war", re: /\bgears\s+of\s+war\b/i },
  { id: "fable", re: /\bfable\b/i },
  { id: "sea_of_thieves", re: /\bsea\s+of\s+thieves\b/i },
];

function repairSubjects(text = "") {
  const source = String(text || "");
  return REPAIR_STORY_SUBJECTS
    .filter((subject) => subject.re.test(source))
    .map((subject) => subject.id);
}

function localRepairScriptIssue(story = {}, text = "") {
  const titleText = [
    story.title,
    ...
    .filter(Boolean)
    .join(" ");
  const titleSubjects = new Set(repairSubjects(titleText));
  const scriptSubjects = new Set(repairSubjects(text));
  if (titleSubjects.size > 0) {
    const orphanSubjects = [...scriptSubjects].filter((subject) => !titleSubjects.has(subject));
    if (orphanSubjects.length > 0) {
      return {
        failure_code: "script_coherence:cross_story_subject_mismatch",
        blockers: orphanSubjects.map((subject) => `script_coherence:orphan_subject:${subject}`),
      };
    }
  }

  if (
    titleSubjects.has("splatoon") &&
    /\b(?:new fighter|fighting game|roster|matchups?|frame data|counter picks?|mains\b|lab time)\b/i.test(
      String(text || ""),
    )
  ) {
    return {
      failure_code: "script_coherence:wrong_genre_contamination",
      blockers: ["script_coherence:splatoon_fighting_game_contamination"],
    };
  }
...
function mediaStatusFor(story = {}, mediaByStoryId = {}) {
  ...
  return {
    ...
    recoveredLocalRepairAudio: bool(media.recoveredLocalRepairAudio),
    localRepairAudioPath: media.localRepairAudioPath || null,
    localRepairTimestampsPath: media.localRepairTimestampsPath || null,
    localRepairTimestampsExist: bool(media.localRepairTimestampsExist),
  };
}
...
function storyHasPublicPlatformEvidence(story = {}) {
  if (/^published$/i.test(String(story.publish_status || ""))) return true;
  return [
    "youtube_post_id",
    "youtube_url",
    "instagram_media_id",
    ...
    "threads_post_id",
    "pinterest_pin_id",
  ].some((field) => String(story[field] || "").trim());
}
...
function classifyRepairItem(story, opts = {}) {
  ...
  if (storyHasPublicPlatformEvidence(story)) {
    return {
      story_id: story.id || null,
      title: story.title || "",
      approved: true,
      action: "skip_already_published",
      blockers: ["already_has_public_platform_evidence"],
      warnings,
      needs,
      media,
      voice_audit: audit,
  ...
  const scriptIssue = localRepairScriptIssue(story, runtimeText);
  if (scriptIssue) {
    return {
      story_id: story.id || null,
      title: story.title || "",
      ...
      blockers: scriptIssue.blockers,
      warnings,
      needs: ["rewrite_script_before_local_repair"],
      media,
      voice_audit: audit,
      topicality,
      runtime,
      failure_code: scriptIssue.failure_code,
      priority: Number(story.breaking_score || 0),
    };
  }
...
function buildLocalMediaRepairQueue({
  ...
  const items = (Array.isArray(stories) ? stories : [])
    .map((story) =>
      ({
        ...
        repair_scope: story.repair_scope || voiceAuditByStoryId[story.id]?.readiness_scope || null,
        local_proof_repair:
          bool(story.local_proof_repair) ||
          voiceAuditByStoryId[story.id]?.readiness_scope === "active_local_proof",
        publish_status: story.publish_status || null,
      }),
    )
    .map((item) => ({
      ...item,
      repair_work_order: buildRepairWorkOrder(item),
    }))
    .sort((a, b) => {
  ...
  for (const item of items) {
    ...
    else if (item.action === "no_action" || item.action === "skip_already_published") increment(counts, "no_action");
...
function repairBlockerType(item = {}) {
  if (item.failure_code) return item.failure_code;
  if (Array.isArray(item.blockers) && item.blockers[0]) return item.blockers[0];
  if (item.needs?.includes("regenerate_audio_with_sleepy_liam")) {
    return "missing_or_unapproved_narration";
  }
  if (item.needs?.includes("extend_script_for_64_70s_local_voice")) {
    return "script_duration_too_short";
  }
  if (item.needs?.includes("rerender_video_local")) return "missing_final_render";
  return item.action || "unknown";
}

function repairLaneForItem(item = {}) {
  if (item.action === "blocked_local_tts_unavailable") return "local_tts_doctor_recovery";
  if (item.action === "extend_script_before_local_repair") {
    return "script_extension_before_local_voice";
  }
  if (item.action === "rewrite_or_route_before_render") return "script_rewrite_or_route_review";
  if (item.action === "route_longform_not_short_repair") return "longform_or_briefing_route";
  if (item.action === "ready_local_audio_render_repair") {
    if (item.needs?.includes("regenerate_audio_with_sleepy_liam")) {
      return "local_audio_regeneration";
    }
    if (item.needs?.includes("rerender_video_local")) return "local_final_render_repair";
  }
  if (item.action === "no_action" || item.action === "skip_already_published") return "none";
  return "operator_review";
}

function missingInputsForItem(item = {}) {
  const inputs = [];
  if (item.needs?.includes("regenerate_audio_with_sleepy_liam")) {
    inputs.push("approved Sleepy Liam narration audio");
  }
  if (item.needs?.includes("extend_script_for_64_70s_local_voice")) {
    inputs.push("64-70 second local voice script");
  }
  if (item.needs?.includes("shorten_or_route_to_briefing")) {
    inputs.push("short-form-safe script or briefing route decision");
  }
  if (item.needs?.includes("rewrite_script_before_local_repair")) {
    inputs.push("coherent source-matched narration script");
  }
  if (item.needs?.includes("rerender_video_local")) inputs.push("local final render rerun");
  if (item.blockers?.includes("local_tts_not_ready")) {
    inputs.push("ready local Sleepy Liam TTS service");
  }
  return inputs.length ? inputs : ["none"];
}

function repairCommandForItem(item = {}) {
  const id = safeStoryId(item.story_id);
  const lane = repairLaneForItem(item);
  if (lane === "local_audio_regeneration") {
    return `npm run ops:local-media-repair -- --story-id ${id} --apply-local-audio --apply-limit 1`;
  }
  if (lane === "local_final_render_repair") {
    return `npm run ops:local-tts-publish-refresh -- --story-id ${id} --apply-local --rerender`;
  }
  if (lane === "script_extension_before_local_voice" || lane === "script_rewrite_or_route_review") {
    return `npm run ops:local-script-extension -- --story-id ${id} --dry-run`;
  }
  if (lane === "local_tts_doctor_recovery") {
    return "npm run tts:doctor -- --restart --prewarm";
  }
  return `npm run ops:local-media-repair -- --story-id ${id} --dry-run`;
}

function expectedOutputForItem(item = {}) {
  const id = safeStoryId(item.story_id);
  const lane = repairLaneForItem(item);
  if (lane === "local_audio_regeneration") {
    return `test/output/local-media-repair/audio/${id}_liam.mp3`;
  }
  if (lane === "local_final_render_repair") {
    return "test/output/local_tts_publish_refresh_rerender.json";
  }
  if (lane === "script_extension_before_local_voice" || lane === "script_rewrite_or_route_review") {
    return "test/output/local_script_extension_plan.json";
  }
  if (lane === "local_tts_doctor_recovery") return "test/output/local_tts_doctor.json";
  return "test/output/local_media_repair_queue.json";
}

function buildRepairWorkOrder(item = {}) {
  const lane = repairLaneForItem(item);
  const dbMutationRequired = lane === "local_final_render_repair";
  const operatorApprovalRequired =
    dbMutationRequired || lane === "script_rewrite_or_route_review" || lane === "longform_or_briefing_route";
  return {
    story_id: item.story_id || null,
    blocker_type: repairBlockerType(item),
    repair_lane: lane,
    exact_missing_input: missingInputsForItem(item),
    recommended_command: repairCommandForItem(item),
    expected_output: expectedOutputForItem(item),
    db_mutation_required: dbMutationRequired,
    db_mutation_status: dbMutationRequired ? "requires_controlled_local_db_update" : "not_required",
    operator_approval_required: operatorApprovalRequired,
    operator_approval_status: operatorApprovalRequired ? "required" : "not_required",
    post_repair_validation_command: `npm run ops:local-media-repair -- --story-id ${safeStoryId(item.story_id)} --dry-run`,
  };
}
...
function clearStaleAudioQaFailure(story = {}) {
  ...
  next.publish_status = derivePublishStatusFromPlatformEvidence(next) || null;
...
function segmentOutputPath(outputRel, label, index) {
  const safeLabel = safeStoryId(label || `segment_${index + 1}`);
  return String(outputRel || "").replace(/\.mp3$/i, `_${safeLabel}.mp3`);
}

function buildLocalRepairTtsSegments({ text, rate = 1.0, segmentLongText = null } = {}) {
  const base = [
    {
      label: "repair",
  ...
  if (typeof segmentLongText !== "function") return base;
  const segmented = segmentLongText(base)
    .map((segment, index) => ({
      label: segment.label || `repair_${index + 1}`,
      text: String(segment.text || "").replace(/\s+/g, " ").trim(),
      rate: Number.isFinite(Number(segment.rate)) ? Number(segment.rate) : rate,
    }))
    .filter((segment) => segment.text);
  return segmented.length ? segmented : base;
}

async function generateLocalRepairTtsWithOptionalSegmentation({
  storyId,
  text,
  outputRel,
  rate = 1.0,
  ...
  segmentLongText = null,
  concatAudioFiles = null,
  mergeSegmentAlignments = null,
} = {}) {
  const segments = buildLocalRepairTtsSegments({ text, rate, segmentLongText });
  const canSegment =
    segments.length > 1 &&
    typeof concatAudioFiles === "function" &&
    typeof mergeSegmentAlignments === "function";

  if (!canSegment) {
    return generateLocalTtsWithOptionalRecovery({
      storyId,
      text,
      outputRel,
      rate,
  ...
  for (let index = 0; index < segments.length; index += 1) {
    ...
    const segmentRel = segmentOutputPath(outputRel, segment.label, index);
    const attempt = await generateLocalTtsWithOptionalRecovery({
      storyId,
      text: segment.text,
      outputRel: segmentRel,
      rate: segment.rate,
      generateTts,
      recoverLocalTts,
      recoverTimeouts: true,
    });
    attempts += Number(attempt.attempts || 1);
    if (!attempt.ok) {
      return {
        ...attempt,
        attempts,
        ...
        failed_segment_label: segment.label,
      };
    }
    segmentPaths.push(segmentRel);
  }

  await concatAudioFiles(segmentPaths, outputRel, { interSegmentGapS: 0 });
  const mergedAlignment = await mergeSegmentAlignments(segmentPaths, {
    interSegmentGapS: 0,
  });
  await fsExtra.writeJson(
    mediaPaths.writePath(outputRel.replace(/\.mp3$/i, "_timestamps.json")),
    mergedAlignment,
    { spaces: 2 },
  );
...
async function applyLocalAudioRepairs({
  ...
  for (const item of selected) {
    ...
    const ttsAttempt = await generateLocalRepairTtsWithOptionalSegmentation({
    ...
    applied.push({
      ...
      tts_segmented: ttsAttempt.segmented === true,
      tts_segment_count: ttsAttempt.segment_count || null,
      tts_segment_paths: ttsAttempt.segment_paths || [],
      server_recovery: ttsAttempt.recovery || null,
...
function renderLocalMediaRepairApplyMarkdown(report) {
  ...
  if (!report.applied.length) {
    ...
    for (const item of report.applied) {
      lines.push(
        `- ${item.story_id}: ${item.duration_verdict} ${item.duration_seconds ?? "unknown"}s${item.tts_segmented ? ` | segmented=${item.tts_segment_count}` : ""}${item.failure_code ? ` | failure=${item.failure_code}` : ""} | ${item.resolved_audio_path || item.output_audio_path}`,
      );
    }
  }
