...
async function applyLocalAudioRepairs({
  ...
  for (const item of selected) {
    ...
    const resolvedAudioPath = await resolveOutputPath(outputRel);
...
function renderLocalMediaRepairApplyMarkdown(report) {
  ...
  if (!report.applied.length) {
    ...
    for (const item of report.applied) {
      lines.push(
        `- ${item.story_id}: ${item.duration_verdict} ${item.duration_seconds ?? "unknown"}s${item.failure_code ? ` | ${item.failure_code}` : ""} | ${item.resolved_audio_path || item.output_audio_path}`,
      );
    }
  }
  if (report.skipped.length) {
    ...
    for (const item of report.skipped) {
      lines.push(`- ${item.story_id}: ${item.reason}${item.error ? ` (${item.error})` : ""}`);
    }
  }
  lines.push("");
  lines.push("## Safety");
