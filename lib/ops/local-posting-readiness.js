...
function preferFreshHealth(primaryHealth, cutoverHealth, tunnelHealth) {
  if (healthOk(tunnelHealth)) return tunnelHealth;
  if (healthOk(primaryHealth)) return primaryHealth;
  if (healthOk(cutoverHealth)) return cutoverHealth;
  return primaryHealth || tunnelHealth || cutoverHealth || null;
}

function tunnelReadinessConnected(tunnelReadiness = null) {
  if (!tunnelReadiness) return null;
  if (publicLocalDeployment(tunnelReadiness.health?.public)) return true;
  const status = String(tunnelReadiness.tunnel?.status || "").trim().toLowerCase();
  if (status === "active") return true;
  if (status === "inactive") return false;
  return normaliseVerdict(tunnelReadiness.verdict) === "green" ? true : null;
}
...
function doctorHealth(report = {}, source = "doctor_report") {
  ...
  const ready =
    ...
    current.ready === true;
  const serviceReady = ready && voice.loaded === true && voiceRefResolved(voice);
  const generationCapacityBusy =
    normalisedVerdict === "amber" &&
    (
      report?.failure_code === "gpu_saturated" ||
      report?.action === "wait_for_gpu"
    );
  const green =
    serviceReady &&
    (
      normalisedVerdict === "green" ||
      generationCapacityBusy
    );
  return {
    green,
    source,
    ...
    generation_capacity: generationCapacityBusy ? "amber" : normalisedVerdict,
    generation_capacity_reason: generationCapacityBusy ? report?.reason || "gpu_saturated" : null,
  };
}
...
function localTtsNativeCrashQuarantined(report = {}) {
  return (
    normaliseVerdict(report?.verdict) === "red" &&
    String(report?.action || "").trim().toLowerCase() === "quarantine_native_crash" &&
    String(report?.failure_code || "").trim().toLowerCase() ===
      "native_inference_access_violation"
  );
}
...
function buildLocalPostingReadiness({
  ...
  const localHealth = preferFreshHealth(
    primaryReadiness?.health?.local,
    cutoverPlan?.health?.local,
    tunnelReadiness?.health?.local,
  );
  const publicHealth = preferFreshHealth(
    primaryReadiness?.health?.public,
    cutoverPlan?.health?.public,
    tunnelReadiness?.health?.public,
  );
  ...
  const localTtsQuarantined = localTtsNativeCrashQuarantined(ttsDoctorReport);
  const completedRenderDispatchOnly =
    !localTtsGreen && localTtsQuarantined && localVoiceReadyCount > 0;
  ...
  const tunnelConnected =
    tunnelReadinessConnected(tunnelReadiness) ?? hasTunnelConnection(cutoverPlan || {}, publicHealth);
  ...
  if (!localTtsGreen && !completedRenderDispatchOnly) {
    blockers.push("local Liam TTS readiness is not green");
  }
  ...
  if (localTtsGreen && localTtsEvidence.generation_capacity === "amber") {
    warnings.push(
      `local Liam generation capacity is temporarily degraded; completed renders can publish but fresh narration generation may wait: ${localTtsEvidence.generation_capacity_reason || "generation capacity amber"}`,
    );
  }
  ...
  if (completedRenderDispatchOnly) {
    warnings.push(
      "local Liam fresh narration generation is quarantined; only completed candidates with independent strict media evidence may dispatch",
    );
  }
  ...
  const completedRenderDispatchReady =
    blockers.length === 0 && completedRenderDispatchOnly && healthOk(publicHealth);
  const verdict = readyForLocalPrimary
    ? "green"
    : completedRenderDispatchReady || (healthOk(localHealth) && localTtsGreen)
      ? "amber"
      : "red";
  const status = readyForLocalPrimary
    ? "ready_to_resume_local_posting"
    : completedRenderDispatchReady
      ? "completed_render_dispatch_ready_generation_blocked"
  ...
  return {
    ...
    readiness: {
      ...
      local_tts_generation_capacity: localTtsEvidence.generation_capacity || null,
      local_tts_generation_capacity_reason: localTtsEvidence.generation_capacity_reason || null,
      local_tts_quarantined: localTtsQuarantined,
      fresh_narration_generation_ready: localTtsGreen,
      completed_render_dispatch_only: completedRenderDispatchOnly,
