...
async function fetchJsonWithRetry(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    result = await fetchJsonImpl(url, { timeoutMs });
    if (result?.ok === true) {
      return { ...result, probe_attempts: attempt };
    }
    if (attempt < maxAttempts) await wait(delayMs);
  }
  return { ...(result || { ok: false }), probe_attempts: maxAttempts };
}
...
async function buildLocalPrimaryReadiness({
  ...
  if (!localHealth) {
    health.local = await fetchJsonWithRetry(`${localUrl}/api/health`, {
      fetchJsonImpl,
  ...
  if (!publicHealth && publicUrl) {
    health.public = await fetchJsonWithRetry(`${publicUrl}/api/health`, {
      fetchJsonImpl,
      attempts: healthProbeAttempts,
      timeoutMs: healthProbeTimeoutMs,
      delayMs: healthProbeDelayMs,
