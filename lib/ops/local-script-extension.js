...
const DEFAULT_LOCAL_EXTENSION_TARGET_WORDS = 212;
const DEFAULT_LOCAL_EXTENSION_SECONDS_PER_WORD = 0.315;
const LOCAL_EXTENSION_TARGET_MIN_SECONDS = 64;
const LOCAL_EXTENSION_TARGET_MAX_SECONDS = 70;
const MAX_LOCAL_EXTENSION_SENTENCES = 5;
const PUBLIC_COPY_BLOCKERS = [
  {
    flag: "public_copy_blocked:internal_source_policy_language",
    re: /\b(?:the key is not to overclaim it|treat the headline as confirmed only where|where the named source confirms|named source confirms|everything else stays in the wait[- ]and[- ]see column|wait[- ]and[- ]see column|core detail plainly)\b/i,
  },
  {
  ...
  {
    flag: "public_copy_blocked:generic_duration_padding",
    re: /\b(?:keep unsupported claims out|official listing, patch note or platform page changes the picture|strongest angle is the concrete consequence|thing players can actually act on today|decision point is whether|open question is what changes next|that is why the timing matters|that is the clean consequence)\b/i,
  },
];
const YEAR_SUFFIX_ONES = [
  "zero",
  "one",
  "two",
  "three",
...
function publicCopySafetyFlags(text = "") {
  const flags = [...publicCopyBlockerFlags(text)];
  const lint = lintScript(String(text || ""), {
    minWords: 1,
    maxWords: Number.MAX_SAFE_INTEGER,
  });
  for (const failure of lint.failures || []) {
    const flag = `script_lint:${failure}`;
    if (!flags.includes(flag)) flags.push(flag);
  }
  return flags;
}

function yearSuffixToWords(suffix) {
  const value = Number(suffix);
  if (!Number.isInteger(value) || value < 0 || value > 99) return suffix;
  if (value === 0) return "two thousand";
  if (value < 10) return `two thousand and ${YEAR_SUFFIX_ONES[value]}`;
  if (value < 20) return `twenty ${YEAR_SUFFIX_TEENS[value]}`;
  const tens = Math.floor(value / 10) * 10;
  const ones = value % 10;
  return `twenty ${YEAR_SUFFIX_TENS[tens]}${ones ? ` ${YEAR_SUFFIX_ONES[ones]}` : ""}`;
}

function repairHybridSpokenYears(text = "") {
  return String(text || "").replace(/\btwenty\s+(\d{2})\b/gi, (_, suffix) =>
    yearSuffixToWords(suffix),
  );
}

function repairPublicScriptText(text = "") {
  return repairHybridSpokenYears(
    String(text || "")
      .replace(/\[PAUSE\]/gi, " ")
      .replace(/\[VISUAL:[^\]]*\]/gi, " ")
      .replace(/\s+/g, " ")
...
function localLiamSpokenText(text = "", cleanText = (value) => value) {
  const cleaned = String(cleanText(String(text || "")) || "").replace(/\s+/g, " ").trim();
  return cleaned
    .replace(/\bHades\s+(?:II|2|two|number\s+two)\b/gi, "Hades sequel")
    .replace(/\s+/g, " ")
    .trim();
}

function proofCompareText(text = "") {
  return String(text || "")
    .toLowerCase()
    .replace(/[^\p{L}\p{N}]+/gu, " ")
    .replace(/\s+/g, " ")
...
function proofMatchesDraft(proof = {}, draft = {}, cleanText = (value) => value) {
  const transcript = proofCompareText(
    proof.transcript || proof.final_transcript || proof.tts_spoken_script || proof.text,
  );
  if (!transcript) return false;
  const spoken = proofCompareText(localLiamSpokenText(draft.proposed_full_script, cleanText));
  return Boolean(spoken && transcript === spoken);
}

function normaliseRejectedProofsById(input = {}) {
  if (!input) return new Map();
  if (input instanceof Map) return input;
  if (Array.isArray(input)) {
    return new Map(
      input
        .filter((proof) => proof?.story_id)
        .map((proof) => [safeStoryId(proof.story_id), proof]),
    );
  }
  if (typeof input === "object") {
    return new Map(
      Object.entries(input)
        .filter(([, proof]) => proof)
        .map(([id, proof]) => [safeStoryId(id), { story_id: id, ...proof }]),
    );
  }
  return new Map();
}

function isActiveLocalProofQueueItem(item = {}) {
  return (
...
function sourceBoundRewriteBlockerType(flags = []) {
  ...
  if (flags.some((flag) => /^local_audio_proof:duration_too_(?:long|short)/.test(String(flag)))) {
    return "local_audio_duration_repair";
  }
...
function buildSourceBoundRewriteWorkOrder(story = {}, flags = []) {
  ...
  return {
    ...
    auto_repair_scope: "dry_run_source_bound_script_candidate",
...
function sourceBoundExtensionSentences(story = {}, keys = []) {
  const sentences = [];
  for (const key of keys) {
    sentences.push(...sentenceList(story[key]));
  }
  if (story.local_script_extension && typeof story.local_script_extension === "object") {
    for (const key of keys) {
      sentences.push(...sentenceList(story.local_script_extension[key]));
    }
  }
  return sentences
...
function contextSentences(story = {}) {
  return sourceBoundExtensionSentences(story, [
    "source_bound_extension_sentences",
    "local_script_extension_sentences",
    "narration_extension_sentences",
  ]);
}

function compactContextSentences(story = {}) {
  return sourceBoundExtensionSentences(story, [
    "source_bound_compact_sentences",
    "local_script_extension_compact_sentences",
    "narration_compact_extension_sentences",
  ]);
}

function localExtensionSecondsPerWord(env = process.env) {
  const override = Number(
    env.LOCAL_SCRIPT_EXTENSION_SECONDS_PER_WORD ||
      env.LOCAL_EXTENSION_SECONDS_PER_WORD,
  );
  if (Number.isFinite(override) && override > 0) return override;
  return DEFAULT_LOCAL_EXTENSION_SECONDS_PER_WORD;
}
...
function scriptTextCandidates(story = {}) {
  return [
    { source: "tts_script", text: story.tts_script },
    { source: "full_script", text: story.full_script },
    {
      source: "hook_body_loop",
      text: [story.hook, story.body, story.loop].filter(Boolean).join(" "),
    },
  ]
    .map((candidate) => ({
      ...candidate,
      text: repairPublicScriptText(normaliseText(candidate.text || "")),
    }))
    .filter((candidate) => candidate.text);
}

function selectLocalBaseScript(story = {}, { cleanText = (value) => value, maxWords = Infinity } = {}) {
  const candidates = scriptTextCandidates(story)
    .map((candidate) => ({
      ...candidate,
      words: countSpokenWords(cleanText(candidate.text)),
    }))
    .filter((candidate) => candidate.words > 0);
  if (!candidates.length) return "";

  const withinMax = candidates
    .filter((candidate) => candidate.words <= maxWords)
    .sort((a, b) => b.words - a.words);
  if (withinMax.length) return withinMax[0].text;

  return candidates.sort((a, b) => a.words - b.words)[0].text;
}
...
function extendScriptToLocalFlash({
  ...
  const localSecondsPerWord = localExtensionSecondsPerWord(env);
  ...
  const raw = selectLocalBaseScript(story, { cleanText, maxWords });
  ...
  const buildProposed = () => repairPublicScriptText(`${sentences.join(" ")} ${REQUIRED_CTA}`);

  let proposed = buildProposed();
  ...
  if (
    (
      (extras.length === 0 && compactExtras.length === 0) ||
      extensionSentencesAdded >= MAX_LOCAL_EXTENSION_SENTENCES
    ) &&
...
function buildLocalScriptExtensionPlan({
  ...
  const readyStoryIds = new Set(
    (Array.isArray(existingReadyStoryIds) ? existingReadyStoryIds : [])
      .map((id) => safeStoryId(id))
      .filter(Boolean),
  );
  const rejectedProofs = normaliseRejectedProofsById(existingRejectedProofsById);
  let candidates = (queueReport.items || []).filter(
    (item) =>
      item.action === "extend_script_before_local_repair" &&
      (!storyId || item.story_id === storyId) &&
      (!activeLocalProofOnly || isActiveLocalProofQueueItem(item)),
  );
  ...
  candidates = candidates.filter((item) => {
    const itemId = safeStoryId(item.story_id);
    if (!readyStoryIds.has(itemId)) return true;
    skipped.push({
      story_id: item.story_id || null,
      reason: "existing_voice_ready_proof",
      failure_code: "existing_voice_ready_proof",
      action: "skip_existing_voice_ready_proof",
    });
    return false;
  });
  const skippedExistingReady = skipped.length;
  const selected =
  ...
  for (const item of selected) {
    try {
      ...
      const proof = rejectedProofs.get(safeStoryId(draft.story_id));
      if (
        draft.action === "ready_for_local_liam_audio" &&
        proof?.failure_code &&
        proofMatchesDraft(proof, draft, cleanText)
      ) {
        const flags = [
          ...draft.manual_review_flags,
          `local_audio_proof:${proof.failure_code}`,
        ];
        draft.action = "review_extended_script";
        draft.manual_review_flags = [...new Set(flags)];
        draft.source_bound_rewrite_work_order = buildSourceBoundRewriteWorkOrder(
          { id: draft.story_id, title: draft.title },
          draft.manual_review_flags,
        );
      }
      drafts.push(draft);
  ...
  const counts = drafts.reduce(
    ...
    {
      total: selected.length + skippedExistingReady,
      ready: 0,
      review: 0,
      failed: Math.max(0, skipped.length - skippedExistingReady),
      skipped_existing_ready: skippedExistingReady,
      source_bound_rewrite_work_orders: 0,
    },
  );
  const sourceBoundRewriteWorkOrders = drafts
    .map((draft) => draft.source_bound_rewrite_work_order)
    .filter(Boolean);
  ...
  return {
    ...
    drafts,
    skipped,
    source_bound_rewrite_work_orders: sourceBoundRewriteWorkOrders,
...
function renderLocalScriptExtensionMarkdown(plan) {
  ...
  const sourceBoundRewriteWorkOrders = plan.source_bound_rewrite_work_orders ||
    (plan.drafts || [])
      .map((draft) => draft.source_bound_rewrite_work_order)
      .filter(Boolean);
  if (sourceBoundRewriteWorkOrders.length) {
    lines.push("");
    lines.push("## Source-Bound Rewrite Work Orders");
    for (const workOrder of sourceBoundRewriteWorkOrders.slice(0, 20)) {
      lines.push(
        `- ${workOrder.story_id}: ${workOrder.blocker_type} | \`${workOrder.recommended_command}\``,
      );
    }
  }
...
async function applyLocalScriptExtensionAudio({
  ...
  cleanText = (value) => value,
  stampVoiceMeta = stampLocalVoiceTimestampMeta,
} = {}) {
  ...
  for (const draft of selected) {
    ...
    const publicCopyFlags = publicCopySafetyFlags(draft.proposed_full_script);
    if (publicCopyFlags.length) {
      skipped.push({
        story_id: draft.story_id,
        reason: "public_copy_blocked_before_tts",
        failure_code: "public_copy_blocked",
        blocking_flags: publicCopyFlags,
      });
      continue;
    }
    const outputRel = `${outputRelDir}/${safeStoryId(draft.story_id)}_liam_extended.mp3`;
    const spokenText = localLiamSpokenText(draft.proposed_full_script, cleanText);
    ...
    let voiceMeta;
    try {
      voiceMeta = await stampVoiceMeta({
