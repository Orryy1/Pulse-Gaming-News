...
const DEFAULT_LOCAL_EXTENSION_TARGET_WORDS = 175;
...
function compactContextSentences() {
  return [
    "That matters because players are voting with their time.",
    "The consequence is simple: momentum is already slipping.",
    "For players, this changes what to watch next.",
    "That is the useful signal, not just the headline.",
    "The next move needs an official answer.",
  ];
}
...
function chooseExtensionSentence({
  ...
  const closeToMinimum = currentWords >= minWords - 20;
  const ordered = [];
