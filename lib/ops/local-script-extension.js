...
const DEFAULT_LOCAL_EXTENSION_TARGET_WORDS = 195;
...
function contextSentences(story = {}) {
  ...
  return [
    "The useful bit is whether this changes what players buy, download, patch, preorder or ignore next.",
    `The source context is ${source}, and the safest read is to stick to what has actually been reported.`,
    `The story angle is ${classification}, so Pulse should explain the consequence without pretending unknowns are settled.`,
    "The next useful update will be an official statement, patch note, store listing or platform response.",
    "If this story moves again, the follow-up should lead with the confirmed change, not speculation.",
    "That keeps the short useful without stretching beyond what the source actually supports.",
    "For players, the decision point is simple: does this change what deserves their attention today?",
  ];
}

function compactContextSentences() {
  return [
    "That is the player consequence.",
    "The useful bit is what changes next.",
    "The safe read is to watch official wording.",
    "That is the signal, not just the headline.",
    "For players, the decision point is simple.",
  ];
}
...
function extendScriptToLocalFlash({
  ...
  const minWords = Number(queueItem.runtime?.minWords || 0) || Math.ceil(61 / localSecondsPerWord);
  const maxWords = Number(queueItem.runtime?.maxWords || 0) || Math.floor(75 / localSecondsPerWord);
...
function buildLocalScriptExtensionPlan({
  ...
  const counts = drafts.reduce(
    ...
    { total: selected.length, ready: 0, review: 0, failed: skipped.length },
  );
