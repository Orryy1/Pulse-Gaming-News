...
function contextSentences(story = {}) {
  ...
  return [
    ...
    `The source context is ${source}, so the script should only use what has actually been reported.`,
    `The story angle is ${classification}, so the player consequence needs to stay clear without pretending unknowns are settled.`,
    ...
    "That keeps the update useful without stretching beyond what the source actually supports.",
...
function compactContextSentences() {
  return [
    ...
    "Use official wording for the follow-up.",
