...
function contextSentences(story = {}) {
  ...
  const rawClassification = normaliseText(
    story.content_pillar || story.classification || story.story_type || "gaming update",
  );
  const classification = /\bmanual review\b/i.test(rawClassification)
    ? "gaming update"
    : rawClassification;
  return [
    "For players, the immediate question is whether this changes what they buy, download, patch, preorder or ignore next.",
    `The reported source is ${source}, so only the reported details are treated as confirmed.`,
    `This is a ${classification} story, so unknown details stay labelled until a store page, patch note or official statement settles them.`,
    "The strongest follow-up would be a named developer statement, a platform listing, patch notes or hard player numbers.",
    "That keeps the update useful without stretching beyond the published facts.",
    "The player-facing question is simple: does this change what deserves attention today?",
  ];
}

function compactContextSentences() {
  return [
    "That is the confirmed player impact.",
    "The useful bit is what changes next.",
    "Keep the next step concrete.",
    "Treat rumour and fact separately.",
    "Keep the unknowns clearly labelled.",
