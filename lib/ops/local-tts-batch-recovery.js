...
async function withLocalTtsGpuTurn({
  storyId,
  env = process.env,
  waitForGpuTurn = waitForPulseGpuTurn,
  releaseGpuTurn = releasePulseGpuTurn,
  stopLocalTtsServer = stopPulseLocalTtsServer,
  operation,
} = {}) {
  if (typeof operation !== "function") {
    throw new TypeError("local_tts_gpu_turn_operation_required");
  }

  const gpuTurn = await waitForGpuTurn({
    env,
    workload: `local-tts:${storyId || "story"}`,
  });

  try {
    return await operation(gpuTurn);
  } finally {
    if (gpuTurn?.coordinated) {
      try {
        await stopLocalTtsServer({ env });
      } catch {
        // The lease still needs releasing if server cleanup fails.
      }
      try {
        await releaseGpuTurn(gpuTurn, {
          env,
          reason: `Pulse local TTS complete: ${storyId || "story"}`,
        });
      } catch {
        // Lease expiry is the final recovery path if release fails.
      }
    }
  }
}
    ...
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      try {
        ...
        const shouldRecover =
          ...
          (recoverTimeouts === true || lastFailure.code !== "tts_timeout") &&
    ...
    try {
      ...
      if (String(before.status || "").toLowerCase() === "unreachable") {
        ...
        report.started = await startServer({
          ...
          env: {
            ...env,
            LOCAL_TTS_RECENT_BOOT_COOLDOWN_MS:
              env.LOCAL_TTS_RECOVERY_RECENT_BOOT_COOLDOWN_MS || "1000",
          },
        });
