...
function classifyStoryForLocalTtsRefresh(story = {}, opts = {}) {
  ...
  return {
    ...
    blockers,
    warnings,
    needs: [...new Set(["regenerate_local_liam_audio", "clear_exported_path_for_rerender", ...needs])],
    platform_ids: platformIds,
    audio_path: story.audio_path || path.join("output", "audio", `${safeStoryId(story.id)}.mp3`),
    timestamps_path: timestampPathForAudio(
      story.audio_path || path.join("output", "audio", `${safeStoryId(story.id)}.mp3`),
    ),
...
function buildLocalTtsPublishRefreshPlan({
  ...
  const items = selected.map((story) => {
    ...
    return classifyStoryForLocalTtsRefresh(story, { allowPublishedRepair });
  });
...
async function applyLocalTtsPublishRefresh({
  ...
  if (typeof generateTtsForStory !== "function") {
