...
async function buildLocalTtsRerenderOnlyPlan({
  stories = [],
  storyIds = [],
  pathExists = mediaPaths.pathExists,
  generatedAt = new Date().toISOString(),
} = {}) {
  const wanted = new Set((storyIds || []).map((id) => String(id).trim()).filter(Boolean));
  const byId = new Map((stories || []).map((story) => [String(story.id), story]));
  ...
  for (const story of selected) {
    ...
    if (!story.id) blockers.push("story_id_missing");
    if (!isApproved(story)) blockers.push("story_not_approved");
    if (!story.audio_path) blockers.push("audio_path_missing");
    if (!story.image_path) blockers.push("image_path_missing");
    if (story.exported_path) blockers.push("exported_path_already_present");
    if (platformIds.length > 0) blockers.push("already_has_platform_ids");
    if (story.local_tts_publish_refresh?.audio_path && story.audio_path !== story.local_tts_publish_refresh.audio_path) {
      warnings.push("audio_path_differs_from_last_local_tts_refresh");
    }

    const audioPath = story.audio_path || path.join("output", "audio", `${safeStoryId(storyId)}.mp3`);
    const timestampsPath = timestampPathForAudio(audioPath);
    if (audioPath && !(await pathExists(audioPath))) blockers.push("audio_file_missing");
    if (timestampsPath && !(await pathExists(timestampsPath))) blockers.push("word_timestamps_missing");

    items.push({
      story_id: story.id || null,
      title: story.title || "",
      action: blockers.length ? "blocked" : "rerender_video_local",
      blockers,
      warnings,
      platform_ids: platformIds,
      audio_path: audioPath,
  ...
  return {
    ...
    counts: {
      total: items.length,
      rerenderable: items.filter((item) => item.action === "rerender_video_local").length,
      blocked: items.filter((item) => item.action === "blocked").length,
    },
    items,
    safety: {
...
function renderLocalTtsPublishRefreshMarkdown(report) {
  ...
  if (report.mode === "rerender_only") {
    lines.push("## Resume Mode");
    lines.push("- Rerender-only: existing local audio and word timestamps must already exist.");
    lines.push("- Audio is not regenerated in this mode.");
    lines.push("");
  }
  if (report.applied) {
    lines.push("## Applied");
