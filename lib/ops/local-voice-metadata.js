...
async function stampLocalVoiceTimestampMeta({
  ...
  let diagnostics = normaliseVoiceDiagnostics(
    existingMeta.voiceDiagnostics || existingMeta.voice_diagnostics,
  );
  let acoustic = acousticFromMeta(existingMeta);
  if (!acoustic && typeof acousticProbe === "function") {
    try {
      const audioAbs = await mediaPaths.resolveExisting(outputAudioPath);
      const probed = await acousticProbe(audioAbs || outputAudioPath);
      const probedDiagnostics = normaliseVoiceDiagnostics({
        source: "local_acoustic_probe",
        metrics: probed || {},
      });
      diagnostics = probedDiagnostics || diagnostics;
      acoustic = probedDiagnostics?.acoustic || null;
    } catch (err) {
      diagnostics = diagnostics || {
        source: "local_acoustic_probe",
        error: String(err?.message || err || "acoustic probe failed").slice(0, 240),
      };
    }
  }
