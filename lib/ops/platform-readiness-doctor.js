...
function packNeedsTikTokCreativeReview(pack = {}) {
  ...
  if (
    pack.creativeReviewRequired === true &&
    !/^(ready_for_operator_review|tiktok_auth_action_required)$/i.test(status)
  ) {
    return true;
  }
...
function liveTikTokLocalTokenState(tokenStatus = {}) {
  const inspected =
    Object.prototype.hasOwnProperty.call(tokenStatus, "ok") ||
    Object.prototype.hasOwnProperty.call(tokenStatus, "reason") ||
    Object.prototype.hasOwnProperty.call(tokenStatus, "refresh_available") ||
    Object.prototype.hasOwnProperty.call(tokenStatus, "needs_refresh_or_sync") ||
    Object.prototype.hasOwnProperty.call(tokenStatus, "needs_reauth");
  ...
  if (tokenStatus.needs_reauth === true) {
    return {
      status: "operator_reauth_required",
      next_action: "operator_reauth_required",
      proven: false,
      refresh_available: tokenStatus.refresh_available === true,
      needs_reauth: true,
    };
  ...
  return {
    status: tokenStatus.reason || "unknown",
    next_action: "inspect_token_state",
    proven: false,
...
function normaliseTikTokNoPostReadiness(
  ...
  const liveToken = liveTikTokLocalTokenState(tokenStatus);
  const fallbackLocalTokenStatus =
    tokenStatus.ok === true
      ? "usable"
  ...
  return {
    browser_oauth: {
      ...
      local_token_proven:
        liveToken ? liveToken.proven : browserOAuth.local_token_proven === true,
      local_token_status:
        liveToken?.status || browserOAuth.local_token_status || fallbackLocalTokenStatus,
    },
    local_token: {
      status: liveToken?.status || localToken.status || fallbackLocalTokenStatus,
      next_action: liveToken?.next_action || localToken.next_action || "inspect_token_state",
      refresh_available:
        liveToken?.refresh_available === true ||
        (!liveToken && localToken.refresh_available === true) ||
        tokenStatus.refresh_available === true,
      needs_reauth:
        liveToken?.needs_reauth === true ||
        (!liveToken && localToken.needs_reauth === true) ||
        tokenStatus.needs_reauth === true,
    },
    official_inbox: {
...
function buildTikTokReadiness({ tokenStatus = {}, tiktokAutomationReport = {} } = {}) {
  ...
  const hasUsablePack =
    ...
    (pack.status === "ready_for_operator_review" ||
      (pack.status === "tiktok_auth_action_required" && tokenStatus.ok !== true && !creativeBlocked) ||
