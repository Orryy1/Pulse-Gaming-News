...
function normaliseTikTokNoPostReadiness(
  ...
  tokenStatus = {},
  { officialInboxRoute = "not_checked" } = {},
) {
  ...
  return {
    ...
    dispatch_creative: {
      status: dispatchCreative.status || "not_checked",
      story_id: dispatchCreative.storyId || dispatchCreative.story_id || null,
      blockers: Array.isArray(dispatchCreative.blockers)
...
function buildTikTokReadiness({ tokenStatus = {}, tiktokAutomationReport = {} } = {}) {
  ...
  const creativeBlocked =
    /creative|promotion|review/i.test(packStatus) ||
  ...
  return {
    ...
    pack: pack
      ? {
          ...
          duration_seconds: numberOrNull(pack.eligibility?.durationSeconds),
...
function buildFacebookReadiness({ platformConfig = {}, facebookManualProof = {} } = {}) {
  ...
  return {
    ...
    manual_reel_upload_observed: facebookManualProof.observed === true,
    manual_proof_note: facebookManualProof.note || null,
    verifier_contract: {
...
function buildPlatformReadinessDoctor({
  ...
  const facebook = buildFacebookReadiness({ platformConfig: operational, facebookManualProof });
  const blockers = [];
