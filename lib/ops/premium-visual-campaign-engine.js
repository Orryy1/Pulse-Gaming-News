...
function resolveCampaignCreativeIdentity(input = {}) {
  const resolved = resolvePulseVisualIdentity({
    ...input,
    creative_category: clean(
      input.creative_category ||
        input.visual_category ||
        input.category ||
        input.classification,
    ),
    content_pillar: clean(input.content_pillar || input.classification),
  });
  return {
    ...resolved,
...
function withRendererIdentityAliases(identity = {}) {
  return {
    ...identity,
    id: identity.category,
    accent: identity.primary,
    accent2: identity.secondary,
    signal: `${clean(identity.segment_name).toUpperCase()} / ${clean(identity.code)}`,
  };
}

function hasCompleteV5Identity(identity = {}) {
  return identity.version === CREATIVE_SYSTEM_VERSION &&
    COVER_IDENTITY_PARITY_FIELDS.every((field) => clean(identity[field]));
}

function identitiesMatch(expected = {}, actual = {}) {
  return COVER_IDENTITY_PARITY_FIELDS.every(
    (field) => clean(expected[field]).toLowerCase() === clean(actual[field]).toLowerCase(),
  );
}
...
function normaliseIdentity(value) {
  return withRendererIdentityAliases(resolveCampaignCreativeIdentity({
...
function buildPremiumVisualCampaignSpec(input = {}) {
  ...
  const creativeIdentity = resolveCampaignCreativeIdentity(input);
  const identity = withRendererIdentityAliases(creativeIdentity);
  const outputs = Object.fromEntries(Object.entries(PLATFORM_VISUAL_SPECS).map(([platform, spec]) => [platform, {
    ...
    creative_identity: {
      ...creativeIdentity,
      transition_cycle: [...creativeIdentity.transition_cycle],
    },
  }]));
  const visualFingerprint = hash([
    subject,
    headline,
    CREATIVE_SYSTEM_VERSION,
    creativeIdentity.category,
    creativeIdentity.palette_id,
    creativeIdentity.primary,
    creativeIdentity.secondary,
    creativeIdentity.segment_name,
    creativeIdentity.code,
  ].join("|"));
  return {
    schema_version: 1,
    story_id: storyId,
...
function validatePremiumVisualCampaign(campaign = {}, { heroImagePresent = false, recentCampaigns = [] } = {}) {
  ...
  const storyIdentity = campaign.creative_identity || {};
  if (
    !hasCompleteV5Identity(storyIdentity) ||
    !campaign.identity?.accent ||
    !campaign.identity?.accent2
  ) {
    blockers.push("premium_visual_identity_missing");
  }
  ...
  for (const row of outputRows) {
    ...
    if (!hasCompleteV5Identity(row.creative_identity)) {
      blockers.push(`premium_visual_cover_identity_missing:${row.platform || "unknown"}`);
    } else if (!identitiesMatch(storyIdentity, row.creative_identity)) {
      blockers.push(`premium_visual_cover_identity_mismatch:${row.platform || "unknown"}`);
    }
  }
  ...
  return {
    ...
    checks: {
      ...
      creative_system_version: clean(storyIdentity.version) || null,
      cover_identity_parity: !uniqueBlockers.some((blocker) =>
        blocker.startsWith("premium_visual_cover_identity_"),
      ),
    },
  };
}
