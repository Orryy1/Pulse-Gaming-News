...
const DEFAULT_EXPECTED_HOURS_UTC = [9, 19];
const DEFAULT_WINDOW_HOURS = 24;
const DEFAULT_TOLERANCE_MINUTES = 20;
const DEFAULT_MIN_GAP_MINUTES = 240;
const DEFAULT_SCHEDULED_MIN_GAP_GRACE_SECONDS = 120;
const DEFAULT_MAX_POSTS_PER_24H = 2;
...
function platformsForStory(story) {
  if (Array.isArray(story?.platforms_override) && story.platforms_override.length > 0) {
    return [
      ...new Set(
        story.platforms_override
          .map((platform) => normalisePlatformPostPlatform(platform))
          .filter(Boolean),
      ),
    ];
  }
...
function normalisePlatformPostPlatform(platform) {
  const value = String(platform || "").trim().toLowerCase();
  if (value === "instagram_reel" || value === "instagram_reels") return "instagram";
  if (value === "facebook_reel" || value === "facebook_reels") return "facebook";
  if (value === "twitter" || value === "twitter_video") return "x";
  return value;
}

function storyFieldForPlatform(platform) {
  switch (normalisePlatformPostPlatform(platform)) {
    case "youtube":
      return "youtube_post_id";
    case "instagram":
...
function platformPostPublishedAt(post = {}) {
  return post.published_at || post.updated_at || post.created_at || null;
}

function isPublishedPlatformPost(post = {}) {
  if (String(post.status || "").toLowerCase() !== "published") return false;
  if (!isRealPlatformValue(post.external_id) && !isRealPlatformValue(post.external_url)) {
    return false;
  }
  return !!parseDate(platformPostPublishedAt(post));
}

function platformPostToStory(post = {}, story = {}) {
  const platform = normalisePlatformPostPlatform(post.platform);
  const field = storyFieldForPlatform(platform);
  const pseudo = {
    id: post.story_id || post.storyId || `platform_post_${post.id || "unknown"}`,
    title: post.title || story.title || "(untitled)",
    published_at: platformPostPublishedAt(post),
    publish_status: "published",
  };
  if (field) pseudo[field] = post.external_id || post.external_url;
  if (platform === "youtube" && post.external_url) pseudo.youtube_url = post.external_url;
  return pseudo;
}

function platformPostGroupToStory(items = [], story = {}) {
  const first = items[0]?.post || {};
  const storyId = first.story_id || first.storyId || story.id || `platform_post_${first.id || "unknown"}`;
  const pseudo = {
    id: storyId,
    title: first.title || story.title || "(untitled)",
    published_at: null,
    publish_status: "published",
    platforms_override: [],
    platform_post_ids: [],
  ...
  for (const item of items || []) {
    ...
    const platform = item?.platform || normalisePlatformPostPlatform(post.platform);
    const field = storyFieldForPlatform(platform);
    const at = parseDate(item?.at || platformPostPublishedAt(post));
    if (at && (!earliest || at.getTime() < earliest.getTime())) {
      earliest = at;
    }
    if (platform && !pseudo.platforms_override.includes(platform)) {
      pseudo.platforms_override.push(platform);
    }
    if (post.id != null) {
      pseudo.platform_post_ids.push(post.id);
    }
    if (field) {
      pseudo[field] = post.external_id || post.external_url;
    }
    if (platform === "youtube" && post.external_url) {
      pseudo.youtube_url = post.external_url;
    }
  }

  if (earliest) {
    pseudo.published_at = earliest.toISOString();
  }
  return pseudo;
}

function maxIso(values = [], accessor = (value) => value) {
  let best = null;
  for (const value of values || []) {
    const date = parseDate(accessor(value));
    if (!date) continue;
...
function isRepairedFailedPlatformRow(story = {}) {
  const publishError = String(story.publish_error || "");
  return (
    publishError.includes("script_validation_review_required_public_row_repair") ||
    Boolean(story.public_row_repair)
  );
}

function isLocalTtsRepairPollutedPublishedRow(story = {}, { env = process.env } = {}) {
  const publishError = String(story.publish_error || "");
  const qaFailures = Array.isArray(story.qa_failures)
    ? story.qa_failures.join(" ")
    : String(story.qa_failures || "");
  const localFailureCode = String(story.local_tts_failure?.code || "");
  const hasLocalTtsRepairFailure =
    /^audio_generation_failed:\s*server_down\b/i.test(publishError) ||
    /audio_generation_failed:server_down/i.test(qaFailures) ||
    /^server_down$/i.test(localFailureCode);
  if (!hasLocalTtsRepairFailure) return false;
  return derivePublishStatusFromPlatformEvidence(story, { env }) === "published";
}
...
function isScheduledWindowPublishEvent(
  event,
  expectedHoursUtc = DEFAULT_EXPECTED_HOURS_UTC,
  toleranceMinutes = DEFAULT_TOLERANCE_MINUTES,
) {
  if (!event) return false;
  if (event.classification === "scheduled_window") return true;
  const nearest = nearestExpectedWindow(event.published_at || event.at, expectedHoursUtc);
  return nearest.minutes !== null && nearest.minutes <= Number(toleranceMinutes);
}
...
function computeNextSafePublishWindow({
  ...
  scheduledMinGapGraceSeconds = DEFAULT_SCHEDULED_MIN_GAP_GRACE_SECONDS,
  ...
  if (sorted.length > allowedBeforeNextPost) {
    ...
    if (capClearsAt.getTime() > now.getTime()) {
      blockers.push({
        type: "post_cap",
  ...
  if (last) {
    ...
    const nextWindowFromNow = nextExpectedWindowAfter(now, expectedHoursUtc);
    const scheduledGraceMs =
      Math.max(0, Number(scheduledMinGapGraceSeconds) || 0) * ONE_SECOND_MS;
    const canApplyScheduledGrace =
      nextWindowFromNow &&
      gapClearsAt.getTime() > nextWindowFromNow.getTime() &&
      gapClearsAt.getTime() - nextWindowFromNow.getTime() <= scheduledGraceMs &&
      isScheduledWindowPublishEvent(last, expectedHoursUtc, toleranceMinutes);
    const effectiveGapClearsAt = canApplyScheduledGrace ? nextWindowFromNow : gapClearsAt;

    if (effectiveGapClearsAt.getTime() > earliest.getTime()) {
      earliest = effectiveGapClearsAt;
    }
    if (canApplyScheduledGrace) {
      grace.push({
        type: "scheduled_min_gap_grace",
        scheduled_window_at: nextWindowFromNow.toISOString(),
        raw_gap_clears_at: gapClearsAt.toISOString(),
        story_id: last.id || null,
        seconds_short: Math.ceil(
          (gapClearsAt.getTime() - nextWindowFromNow.getTime()) / ONE_SECOND_MS,
        ),
      });
    } else if (gapClearsAt.getTime() > now.getTime()) {
      blockers.push({
        type: "minimum_gap",
...
function buildPublishCadenceReport({
  ...
  const storyPublicPlatformKeys = new Set();
  const storyEvents = publishedStories.map(({ story }) => {
    for (const platform of platformsForStory(story)) {
      storyPublicPlatformKeys.add(`${story.id}:${platform}`);
    }
    return classifyPublishEvent({
      story,
      publishJobs,
      expectedHoursUtc,
      toleranceMinutes,
  ...
  const platformPostGroups = new Map();
  for (const item of (platformPosts || [])
    .filter(isPublishedPlatformPost)
    .map((post) => {
      const storyId = post.story_id || post.storyId || null;
      const platform = normalisePlatformPostPlatform(post.platform);
      return {
        post,
        storyId,
        platform,
        at: parseDate(platformPostPublishedAt(post)),
      };
    })
    .filter(({ storyId, platform, at }) => {
      if (!storyId || !platform || !at) return false;
      if (storyPublicPlatformKeys.has(`${storyId}:${platform}`)) return false;
      return at.getTime() >= sinceMs && at.getTime() <= nowDate.getTime();
    })) {
    if (!platformPostGroups.has(item.storyId)) {
      platformPostGroups.set(item.storyId, []);
    }
    platformPostGroups.get(item.storyId).push(item);
  }

  const platformPostEvents = [...platformPostGroups.entries()]
    .map(([storyId, items]) => {
      items.sort((a, b) => a.at.getTime() - b.at.getTime());
      const event = classifyPublishEvent({
        story: platformPostGroupToStory(items, storyById.get(storyId)),
        publishJobs,
        expectedHoursUtc,
        toleranceMinutes,
      });
      return {
        ...
        platform_post_id: items[0]?.post?.id || null,
        platform_post_ids: items.map((item) => item.post.id).filter((id) => id != null),
      };
    });

  const publishEvents = [...storyEvents, ...platformPostEvents].sort((a, b) => {
  ...
  const allFailedRowsWithPlatformIds = (stories || [])
    ...
    .map((story) => {
      ...
      return {
        ...
        age_hours: touched
          ...
          : null,
        repaired_public_row: isRepairedFailedPlatformRow(story),
        tts_repair_polluted_public_row: isLocalTtsRepairPollutedPublishedRow(story, { env }),
      };
    });
  const repairedFailedRowsWithPlatformIds = allFailedRowsWithPlatformIds.filter(
    (row) => row.repaired_public_row,
  );
  const ttsRepairPollutedPublicRowsWithPlatformIds = allFailedRowsWithPlatformIds.filter(
    (row) => row.tts_repair_polluted_public_row && !row.repaired_public_row,
  );
  const failedRowsWithPlatformIds = allFailedRowsWithPlatformIds.filter(
    (row) => !row.repaired_public_row && !row.tts_repair_polluted_public_row,
  );
  ...
  const storyRowsWithPlatformIds = (stories || []).filter(hasPlatformId);
  const storyRowsWithPublishTimestamp = storyRowsWithPlatformIds.filter((story) =>
    parseDate(explicitPublishedAt(story)),
  );
  const publishedPlatformPostRows = (platformPosts || []).filter(isPublishedPlatformPost);
  const historicalPublicStoryRows = storyRowsWithPlatformIds
    .filter((story) => !isFailedStory(story))
    .filter((story) => parseDate(explicitPublishedAt(story)));
  const telemetryLimitations = ["external_platform_inventory_not_checked"];
  const zeroRecentDbEventsButHistoricalPublicRows =
    publishEvents.length === 0 &&
    (historicalPublicStoryRows.length > 0 || publishedPlatformPostRows.length > 0);
  if (zeroRecentDbEventsButHistoricalPublicRows) {
    telemetryLimitations.push("zero_recent_db_events_is_not_zero_platform_truth");
  }
  const telemetry = {
    source: "local_db_stories_jobs_platform_posts",
    platform_truth_checked: false,
    stories_seen: (stories || []).length,
    jobs_seen: (jobs || []).length,
    platform_posts_seen: (platformPosts || []).length,
    story_rows_with_platform_ids: storyRowsWithPlatformIds.length,
    story_rows_with_publish_timestamps: storyRowsWithPublishTimestamp.length,
    published_platform_posts_seen: publishedPlatformPostRows.length,
    newest_known_db_publish_at: maxIso(storyRowsWithPublishTimestamp, explicitPublishedAt),
    newest_known_platform_post_at: maxIso(publishedPlatformPostRows, platformPostPublishedAt),
  ...
  if (zeroRecentDbEventsButHistoricalPublicRows) {
    advisory.push(
      "No recent DB-stamped public posts were found; this is not proof that no platform posts happened because YouTube/Meta inventory was not checked.",
    );
  }
  ...
  return {
    ...
    thresholds: {
      ...
      scheduled_min_gap_grace_seconds: DEFAULT_SCHEDULED_MIN_GAP_GRACE_SECONDS,
    ...
    summary: {
      ...
      platform_post_events: platformPostEvents.length,
      ...
      repaired_failed_rows_with_platform_ids: repairedFailedRowsWithPlatformIds.length,
      tts_repair_polluted_public_rows_with_platform_ids:
        ttsRepairPollutedPublicRowsWithPlatformIds.length,
    ...
    repaired_failed_rows_with_platform_ids: repairedFailedRowsWithPlatformIds,
    tts_repair_polluted_public_rows_with_platform_ids:
      ttsRepairPollutedPublicRowsWithPlatformIds,
...
function formatPublishCadenceMarkdown(report) {
  ...
  lines.push(
    `  - Repaired public rows: ${report.summary?.repaired_failed_rows_with_platform_ids || 0}`,
  );
  lines.push(
    `  - TTS repair-polluted public rows: ${report.summary?.tts_repair_polluted_public_rows_with_platform_ids || 0}`,
  );
  lines.push(
    `- Invalid public story rows: ${report.summary?.invalid_public_story_rows || 0}`,
  );
  lines.push(
    `- Structured platform-post events: ${report.summary?.platform_post_events || 0}`,
  );
  ...
  if (report.telemetry) {
    lines.push("");
    lines.push("## Telemetry Scope");
    lines.push(`- Source: ${report.telemetry.source || "unknown"}`);
    lines.push(
      `- Platform inventory checked: ${report.telemetry.platform_truth_checked ? "yes" : "no"}`,
    );
    lines.push(
      `- Story rows with platform IDs: ${report.telemetry.story_rows_with_platform_ids ?? 0}`,
    );
    lines.push(
      `- Published platform_posts rows: ${report.telemetry.published_platform_posts_seen ?? 0}`,
    );
    lines.push(
      `- Newest DB publish timestamp: ${report.telemetry.newest_known_db_publish_at || "none"}`,
    );
    lines.push(
      `- Newest platform_posts timestamp: ${report.telemetry.newest_known_platform_post_at || "none"}`,
    );
    if (report.telemetry.limitations?.length) {
      lines.push(
        `- Limitations: ${report.telemetry.limitations.join(", ")}`,
      );
    }
  }
  ...
  if (report.repaired_failed_rows_with_platform_ids?.length) {
    lines.push("");
    lines.push("## Repaired Failed Rows Carrying Platform IDs");
    lines.push(
      `- ${report.repaired_failed_rows_with_platform_ids.length} repaired row(s) are retained for audit, but do not count against current cadence.`,
    );
    for (const row of report.repaired_failed_rows_with_platform_ids.slice(0, 5)) {
      lines.push(
        `- ${row.id}: ${row.platforms.join(", ")} | ${row.last_touched_at || "unknown time"} | ${shortTitle(row.title)}`,
      );
    }
  ...
  if (report.tts_repair_polluted_public_rows_with_platform_ids?.length) {
    lines.push("");
    lines.push("## TTS Repair-Polluted Public Rows Carrying Platform IDs");
    lines.push(
      `- ${report.tts_repair_polluted_public_rows_with_platform_ids.length} row(s) have complete public platform evidence but were later touched by local TTS repair failures; retained for audit only.`,
    );
    for (const row of report.tts_repair_polluted_public_rows_with_platform_ids.slice(0, 5)) {
      lines.push(
        `- ${row.id}: ${row.platforms.join(", ")} | ${row.last_touched_at || "unknown time"} | ${shortTitle(row.title)}`,
      );
    }
...
async function loadPublishCadenceInputs({ windowHours = DEFAULT_WINDOW_HOURS, now = new Date() } = {}) {
  ...
  try {
    ...
    platformPosts = repos.db
      .prepare(
        `SELECT pp.id, pp.story_id, s.title, pp.channel_id, pp.platform,
                pp.external_id, pp.external_url, pp.status, pp.block_reason,
                pp.error_message, pp.published_at, pp.created_at, pp.updated_at
         FROM platform_posts pp
         LEFT JOIN stories s ON s.id = pp.story_id
         WHERE COALESCE(pp.published_at, pp.updated_at, pp.created_at) >= ?
         ORDER BY COALESCE(pp.published_at, pp.updated_at, pp.created_at), pp.id`,
      )
      .all(formatSqliteUtc(since));
  } catch {
    jobs = [];
  ...
  return { stories, jobs, platformPosts };
}
...
module.exports = {
  ...
  DEFAULT_SCHEDULED_MIN_GAP_GRACE_SECONDS,
