...
function summariseRecentFailedCandidates(stories = [], { limit = 10 } = {}) {
...
function pillarRecentFailedCandidates({ stories } = {}) {
  const summary = summariseRecentFailedCandidates(stories, { limit: 10 });
  let verdict = GREEN;
  if (summary.count >= 5) verdict = AMBER;
  const reason =
    verdict === AMBER
      ? `${summary.count}_qa_failed_total; top_recent: ` +
        summary.examples
          .slice(0, 3)
          .map((s) => `${s.id}:${s.reason}`)
          .join(", ")
