...
function summariseLocalTtsRecoveryContext(report = null, doctorReport = null) {
  ...
  const safeRetryCount = numberFromAny(
    recovery.safe_retry_work_order_count,
    safeRetryWorkOrders.length,
  );
  const retryPreflightCount = numberFromAny(
    recovery.retry_preflight_work_order_count,
    safeRetryWorkOrders.filter((order) => order.preflight_required === true).length,
  );
  const retryApplyReadyCount = numberFromAny(
    recovery.retry_apply_ready_work_order_count,
    safeRetryWorkOrders.filter((order) => order.preflight_required !== true).length,
  );
  ...
  return {
    ...
    safe_retry_story_ids: safeRetryStoryIds.slice(0, 10),
