...
} = require("./local-readiness-evidence-root");
...
const DEFAULT_LOCAL_POSTING_ARTIFACT_MAX_AGE_HOURS = 24;
...
function summariseLocalPostingReadinessReason(report = {}) {
  const parts = [];
  const artifactFreshness = report.artifact_freshness || {};
  const staleArtifacts = Array.isArray(artifactFreshness.artifacts)
    ? artifactFreshness.artifacts.filter((artifact) => artifact?.stale === true)
    : [];
  const missingArtifacts = Array.isArray(artifactFreshness.artifacts)
    ? artifactFreshness.artifacts.filter((artifact) => artifact?.missing === true)
    : [];
  if (staleArtifacts.length > 0) {
    parts.push(
      `local_posting_artifacts_stale=${staleArtifacts
  ...
  if (missingArtifacts.length > 0) {
    parts.push(
      `local_posting_artifacts_missing=${missingArtifacts
        .map((artifact) => artifact.name)
        .filter(Boolean)
        .slice(0, 3)
        .join(", ")}`,
  ...
  const blockers = Array.isArray(report.blockers) ? report.blockers.filter(Boolean).map(String) : [];
  const warnings = Array.isArray(report.warnings) ? report.warnings.filter(Boolean).map(String) : [];
  if (blockers.length > 0) parts.push(blockers.slice(0, 4).join("; "));
  const readiness = report.readiness || {};
  const runtimeFacts = [];
  if (readiness.safe_observation_mode === true) runtimeFacts.push("safe_observation_mode=true");
  if (readiness.running_primary_enabled === false) runtimeFacts.push("running_primary=false");
  if (readiness.running_auto_publish_enabled === false) runtimeFacts.push("running_auto_publish=false");
  if (readiness.public_health === false) runtimeFacts.push("public_health=false");
  if (readiness.tunnel_connected === false) runtimeFacts.push("tunnel_connected=false");
  if (runtimeFacts.length > 0) parts.push(runtimeFacts.join(", "));
  if (parts.length === 0 && warnings.length > 0) parts.push(warnings.slice(0, 3).join("; "));
  if (parts.length === 0 && report.status) parts.push(String(report.status));
...
function defaultLocalPostingArtifactPath(cwd, fileName, env = process.env) {
  return path.join(
    resolveLocalReadinessOutputDir({
...
function buildArtifactFreshness(entries = [], {
  now = Date.now(),
  maxAgeHours = DEFAULT_LOCAL_POSTING_ARTIFACT_MAX_AGE_HOURS,
} = {}) {
  const nowMs = Number.isFinite(Number(now)) ? Number(now) : new Date(now).getTime();
  const maxAge = Number.isFinite(Number(maxAgeHours))
    ? Number(maxAgeHours)
    : DEFAULT_LOCAL_POSTING_ARTIFACT_MAX_AGE_HOURS;
  const artifacts = entries
    ...
    .map((entry) => {
      ...
      const ageHours = Math.max(0, (nowMs - stat.mtimeMs) / 36e5);
      return {
        name: entry.name,
        path: filePath,
        exists: true,
  ...
  return {
    ...
    stale_count: artifacts.filter((artifact) => artifact.stale === true).length,
    missing_count: artifacts.filter((artifact) => artifact.missing === true).length,
    artifacts,
  };
}

function pillarLocalPostingReadiness(opts = {}) {
  if (opts.report) {
    return {
      ok: true,
      verdict: opts.report.verdict || UNKNOWN,
      reason: summariseLocalPostingReadinessReason(opts.report),
      raw: opts.report,
    };
  }
  const mod = safeRequire("./local-posting-readiness");
  if (!mod || typeof mod.buildLocalPostingReadiness !== "function") {
    return { ok: false, verdict: UNKNOWN, reason: "module_unavailable" };
  }
  try {
    const cwd = opts.cwd || path.join(__dirname, "..", "..");
    const reportPath = opts.reportPath || opts.localPostingReadinessPath;
    const artifactPaths = reportPath
      ? [{ name: "local_posting_readiness", path: reportPath }]
      : [
          {
            name: "local_cutover_plan",
            path: opts.localCutoverPlanPath || defaultLocalPostingArtifactPath(cwd, "local_cutover_plan.json", opts.env),
          },
          {
            name: "local_primary_readiness",
            path: opts.localPrimaryReadinessPath || defaultLocalPostingArtifactPath(cwd, "local_primary_readiness.json", opts.env),
          },
          {
            name: "local_tunnel_readiness",
            path: opts.localTunnelReadinessPath || defaultLocalPostingArtifactPath(cwd, "local_tunnel_readiness.json", opts.env),
          },
          {
            name: "local_tts_overnight_report",
            path: opts.localTtsReportPath || defaultLocalPostingArtifactPath(cwd, "local_tts_overnight_report.json", opts.env),
          },
          {
            name: "local_tts_doctor",
            path: opts.localTtsDoctorPath || defaultLocalPostingArtifactPath(cwd, "local_tts_doctor.json", opts.env),
          },
        ];
    const report = reportPath ? readJsonIfPresent(reportPath) : null;
    const built = report || mod.buildLocalPostingReadiness({
      cutoverPlan:
        opts.cutoverPlan ||
        readJsonIfPresent(opts.localCutoverPlanPath || defaultLocalPostingArtifactPath(cwd, "local_cutover_plan.json", opts.env)),
      primaryReadiness:
        opts.primaryReadiness ||
        readJsonIfPresent(opts.localPrimaryReadinessPath || defaultLocalPostingArtifactPath(cwd, "local_primary_readiness.json", opts.env)),
      tunnelReadiness:
        opts.tunnelReadiness ||
        readJsonIfPresent(opts.localTunnelReadinessPath || defaultLocalPostingArtifactPath(cwd, "local_tunnel_readiness.json", opts.env)),
      ttsReport:
        opts.ttsReport ||
        readJsonIfPresent(opts.localTtsReportPath || defaultLocalPostingArtifactPath(cwd, "local_tts_overnight_report.json", opts.env)),
      ttsDoctorReport:
        opts.ttsDoctorReport ||
        readJsonIfPresent(opts.localTtsDoctorPath || defaultLocalPostingArtifactPath(cwd, "local_tts_doctor.json", opts.env)),
      now: opts.now ? new Date(opts.now) : new Date(),
    });
    const artifactFreshness = buildArtifactFreshness(artifactPaths, {
      now: opts.now || Date.now(),
      maxAgeHours: opts.maxArtifactAgeHours,
    });
    const raw = {
      ...built,
      artifact_freshness: artifactFreshness,
    };
    const normalisedVerdict = normaliseReadinessVerdict(built.verdict);
    const verdict =
      artifactFreshness.stale_count > 0 && normalisedVerdict === GREEN
        ? AMBER
        : built.verdict || UNKNOWN;
    return {
      ok: true,
      verdict,
      reason: summariseLocalPostingReadinessReason(raw),
      raw,
    };
  } catch (err) {
    return {
      ok: false,
      verdict: AMBER,
      reason: `local_posting_readiness: ${err.message}`,
    };
  }
}

function localPostingPillarNeedsArtifactRefresh(pillar = {}) {
  const freshness = pillar.raw?.artifact_freshness || {};
  const readiness = pillar.raw?.readiness || {};
  const runtimeHealthFailed =
    readiness.local_health === false || readiness.public_health === false;
  return (
    runtimeHealthFailed ||
    Number(freshness.stale_count || 0) > 0 ||
    Number(freshness.missing_count || 0) > 0
  );
}

async function buildLocalPostingReadinessPillar(opts = {}) {
  const initial = pillarLocalPostingReadiness(opts);
  if (
    opts.refreshLocalPostingArtifacts !== true ||
    typeof opts.localPostingArtifactRefresh !== "function" ||
    !localPostingPillarNeedsArtifactRefresh(initial)
  ) {
    return initial;
  }

  const freshness = initial.raw?.artifact_freshness || {};
  const staleArtifacts = Array.isArray(freshness.artifacts)
    ? freshness.artifacts.filter((artifact) => artifact?.stale === true || artifact?.missing === true)
    : [];
  let refreshResult = null;
  try {
    refreshResult = await opts.localPostingArtifactRefresh({
      cwd: opts.cwd || path.join(__dirname, "..", ".."),
      now: opts.now || Date.now(),
      staleArtifacts,
      initialPillar: initial,
    });
  } catch (err) {
    return {
      ...initial,
      reason: [initial.reason, `local_posting_artifact_refresh_failed=${err.message}`]
        .filter(Boolean)
        .join("; "),
      raw: {
        ...(initial.raw || {}),
  ...
  const refreshed = pillarLocalPostingReadiness(opts);
  return {
    ...refreshed,
    raw: {
      ...(refreshed.raw || {}),
...
const CORE_PLATFORM_STATUS_KEYS = ["youtube", "instagram_reel", "facebook_reel"];
const OPTIONAL_DEFERRED_PLATFORM_STATUS_KEYS = new Set([
  "tiktok",
  "twitter",
  "x",
  "threads",
...
function pillarIsGreen(pillar = {}) {
  return normaliseReadinessVerdict(pillar.verdict) === GREEN;
}

function rawNumber(pillar = {}, key, fallback = 0) {
  return numberFromAny(pillar?.raw?.[key], fallback);
}

function guardedEnabledPlatformHandoffReady(pillars = {}) {
  return (
    guardedEnabledPlatformApprovalHandoffReady(pillars) ||
    guardedEnabledPlatformAutonomousHandoffReady(pillars)
  );
}

function guardedEnabledPlatformApprovalHandoffReady(pillars = {}) {
  const approval = pillars.human_review_approval_gate || {};
  const dispatch = pillars.guarded_dispatch_preflight || {};
  const executor = pillars.guarded_dispatch_executor_preflight || {};
  const dispatchVerdict = normaliseReadinessVerdict(dispatch.verdict);
  const dispatchHasSelectedSafeCoverage =
    pillarIsGreen(dispatch) ||
    (
      dispatchVerdict === RED &&
      dispatch.reason === "guarded_dispatch_preflight_blocked" &&
      rawNumber(dispatch, "dispatch_ready_action_count") > 0 &&
      rawNumber(dispatch, "safety_blocker_count") === 0
    );
  return (
    pillarIsGreen(approval) &&
    dispatchHasSelectedSafeCoverage &&
    pillarIsGreen(executor) &&
    rawNumber(approval, "approved_action_count") > 0 &&
    rawNumber(approval, "invalid_decision_count") === 0 &&
    approval.raw?.guarded_dispatch_eligible === true &&
    rawNumber(dispatch, "dispatch_ready_action_count") > 0 &&
    rawNumber(dispatch, "safety_blocker_count") === 0 &&
    rawNumber(executor, "handoff_ready_action_count") > 0 &&
    rawNumber(executor, "blocked_selected_action_count") === 0 &&
    executor.raw?.ready_for_live_executor_handoff === true
  );
}

function strictDryRunHasAutonomousGuardedActions(
  pillar = {},
  { allowNonSelectedBlockedStories = false } = {},
) {
  const raw = pillar.raw || {};
  const blockedStoriesSafe =
    allowNonSelectedBlockedStories || rawNumber(pillar, "blocked_story_count") === 0;
  return (
    raw.safety_intact === true &&
    raw.ready_for_unattended_publish !== true &&
    rawNumber(pillar, "ready_story_count") > 0 &&
    blockedStoriesSafe &&
    rawNumber(pillar, "blocked_action_count") === 0 &&
    rawNumber(pillar, "platform_publish_now_action_count") > 0 &&
    rawNumber(pillar, "platform_enabled_dry_run_action_count") > 0 &&
    rawNumber(pillar, "human_review_required_action_count") === 0 &&
    rawNumber(pillar, "live_publish_allowed_action_count") === 0 &&
    rawNumber(pillar, "guarded_dispatch_ready_action_count") > 0 &&
    rawNumber(pillar, "publish_now_warning_action_count") === 0 &&
    rawNumber(pillar, "reviewable_enabled_action_count") === 0
  );
}

function guardedDispatchHasAutonomousDryRunCoverage(pillar = {}) {
  return (
    pillarIsGreen(pillar) &&
    rawNumber(pillar, "autonomous_dry_run_action_count") > 0 &&
    rawNumber(pillar, "dispatch_ready_action_count") > 0 &&
    rawNumber(pillar, "blocked_action_count") === 0 &&
    rawNumber(pillar, "safety_blocker_count") === 0 &&
    pillar.raw?.ready_for_guarded_dispatch === true
  );
}

function guardedEnabledPlatformAutonomousHandoffReady(pillars = {}) {
  const strictDryRun = pillars.strict_dry_run_control || {};
  const dispatch = pillars.guarded_dispatch_preflight || {};
  const executor = pillars.guarded_dispatch_executor_preflight || {};
  return (
    strictDryRunHasAutonomousGuardedActions(strictDryRun, {
      allowNonSelectedBlockedStories: true,
    }) &&
    guardedDispatchHasAutonomousDryRunCoverage(dispatch) &&
    pillarIsGreen(executor) &&
    rawNumber(executor, "handoff_ready_action_count") > 0 &&
    rawNumber(executor, "blocked_selected_action_count") === 0 &&
    executor.raw?.ready_for_live_executor_handoff === true
...
function guardedDispatchExecutorAwaitingSelection(executor = {}) {
  const executorVerdict = normaliseReadinessVerdict(executor.verdict);
  return (
    executorVerdict === AMBER &&
    executor.reason === "explicit_action_ids_required" &&
    rawNumber(executor, "dispatch_ready_action_count") > 0 &&
    rawNumber(executor, "selected_action_count") === 0 &&
    rawNumber(executor, "blocked_selected_action_count") === 0
  );
}

function guardedEnabledPlatformApprovalSchedulerReady(pillars = {}) {
  const approval = pillars.human_review_approval_gate || {};
  const dispatch = pillars.guarded_dispatch_preflight || {};
  const executor = pillars.guarded_dispatch_executor_preflight || {};
  return (
    pillarIsGreen(approval) &&
    pillarIsGreen(dispatch) &&
    (pillarIsGreen(executor) || guardedDispatchExecutorAwaitingSelection(executor)) &&
    rawNumber(approval, "approved_action_count") > 0 &&
    rawNumber(approval, "invalid_decision_count") === 0 &&
    approval.raw?.guarded_dispatch_eligible === true &&
    rawNumber(dispatch, "dispatch_ready_action_count") > 0 &&
    rawNumber(dispatch, "blocked_action_count") === 0 &&
    rawNumber(dispatch, "safety_blocker_count") === 0 &&
    dispatch.raw?.ready_for_guarded_dispatch === true
  );
}

function guardedEnabledPlatformAutonomousSchedulerReady(pillars = {}) {
  const strictDryRun = pillars.strict_dry_run_control || {};
  const dispatch = pillars.guarded_dispatch_preflight || {};
  const executor = pillars.guarded_dispatch_executor_preflight || {};
  return (
    ...
    guardedDispatchHasAutonomousDryRunCoverage(dispatch) &&
    guardedDispatchExecutorAwaitingSelection(executor)
  );
}

function guardedEnabledPlatformSchedulerReady(pillars = {}) {
  return (
    guardedEnabledPlatformApprovalSchedulerReady(pillars) ||
    guardedEnabledPlatformAutonomousSchedulerReady(pillars)
  );
}

function strictDryRunHasNoCurrentPublishableActions(pillar = {}) {
  const raw = pillar.raw || {};
  const hasCurrentDryRunEvidence = [
    "ready_story_count",
    "platform_publish_now_action_count",
    "reviewable_enabled_action_count",
    "live_publish_allowed_action_count",
  ].some((key) => Object.prototype.hasOwnProperty.call(raw, key));
  if (!hasCurrentDryRunEvidence) return false;
  return (
    rawNumber(pillar, "ready_story_count") <= 0 &&
    rawNumber(pillar, "platform_publish_now_action_count") <= 0 &&
    rawNumber(pillar, "reviewable_enabled_action_count") <= 0 &&
...
function scopedGreenPillar(pillar = {}, overrideReason, extraRaw = {}) {
  return {
    ...pillar,
    ok: true,
    verdict: GREEN,
    ...
    raw: {
      ...(pillar.raw || {}),
      previous_verdict: normaliseReadinessVerdict(pillar.verdict),
      previous_reason: pillar.reason || null,
      enabled_platform_scope_override: overrideReason,
      ...extraRaw,
    },
  };
}

function scopedAmberPillar(pillar = {}, reason, extraRaw = {}) {
  return {
    ...pillar,
    ok: true,
    verdict: AMBER,
    ...
    raw: {
      ...
      previous_verdict: normaliseReadinessVerdict(pillar.verdict),
      previous_reason: pillar.reason || null,
      enabled_platform_scope_override: reason,
      ...extraRaw,
    },
  };
}

function cadenceIsOnlyHistoricalScheduleAdvisory(pillar = {}) {
  if (normaliseReadinessVerdict(pillar.verdict) !== AMBER) return false;
  const summary = pillar.raw?.summary || {};
  const thresholds = pillar.raw?.thresholds || {};
  const publishedCount = numberFromAny(summary.published_count);
  const maxRecommended = numberFromAny(thresholds.max_recommended_posts_per_24h, 3);
  return (
    publishedCount <= maxRecommended &&
    numberFromAny(summary.off_schedule_count) > 0 &&
    numberFromAny(summary.burst_pairs) === 0 &&
    numberFromAny(summary.failed_rows_with_platform_ids_recent) === 0 &&
    numberFromAny(summary.invalid_public_story_rows) === 0
  );
}

function strictDryRunIsSupersededByGuardedHandoff(pillar = {}) {
  const raw = pillar.raw || {};
  const common =
    normaliseReadinessVerdict(pillar.verdict) === AMBER &&
    raw.safety_intact === true &&
    raw.ready_for_unattended_publish !== true &&
    numberFromAny(raw.ready_story_count) > 0 &&
    numberFromAny(raw.blocked_story_count) === 0 &&
    numberFromAny(raw.blocked_action_count) === 0 &&
    numberFromAny(raw.platform_publish_now_action_count) > 0 &&
    numberFromAny(raw.live_publish_allowed_action_count) === 0;
  if (!common) return false;
  return (
    (
      numberFromAny(raw.human_review_required_action_count) > 0 &&
      numberFromAny(raw.reviewable_enabled_action_count) > 0
    ) ||
    strictDryRunHasAutonomousGuardedActions(pillar)
  );
}

function strictDryRunRedIsHeldByGuardedSchedulerScope(pillar = {}) {
  const raw = pillar.raw || {};
  return (
    normaliseReadinessVerdict(pillar.verdict) === RED &&
    raw.safety_intact === true &&
    raw.ready_for_unattended_publish !== true &&
    numberFromAny(raw.ready_story_count) > 0 &&
    (
...
function strictDryRunRedIsHeldByAutonomousGuardedSchedulerScope(pillar = {}) {
  return (
    normaliseReadinessVerdict(pillar.verdict) === RED &&
    strictDryRunHasAutonomousGuardedActions(pillar, {
      allowNonSelectedBlockedStories: true,
...
function strictDryRunRedIsHeldByGuardedExecutorHandoff(pillar = {}, pillars = {}) {
  const raw = pillar.raw || {};
  if (normaliseReadinessVerdict(pillar.verdict) !== RED) return false;
  if (raw.safety_intact !== true) return false;
  if (raw.ready_for_unattended_publish === true) return false;
  if (numberFromAny(raw.ready_story_count) <= 0) return false;
  if (numberFromAny(raw.platform_publish_now_action_count) <= 0) return false;
  if (numberFromAny(raw.live_publish_allowed_action_count) !== 0) return false;
  const humanReviewScoped =
    numberFromAny(raw.human_review_required_action_count) > 0 &&
    numberFromAny(raw.reviewable_enabled_action_count) > 0;
  const autonomousScoped = strictDryRunHasAutonomousGuardedActions(pillar, {
    allowNonSelectedBlockedStories: true,
  });
  if (!humanReviewScoped && !autonomousScoped) return false;
  return guardedEnabledPlatformHandoffReady(pillars);
}

function guardedDispatchPreflightRedIsHeldByExecutorHandoff(pillar = {}, pillars = {}) {
  if (normaliseReadinessVerdict(pillar.verdict) !== RED) return false;
  if (pillar.reason !== "guarded_dispatch_preflight_blocked") return false;
  if (rawNumber(pillar, "dispatch_ready_action_count") <= 0) return false;
  if (rawNumber(pillar, "safety_blocker_count") !== 0) return false;
  return guardedEnabledPlatformHandoffReady(pillars);
}

function platformStateIsEnabled(entry = {}) {
  return entry.state === "enabled" || entry.state === "enabled_via_scheduler";
}

function platformStatusIsEnabledPlatformScoped(pillar = {}) {
  if (normaliseReadinessVerdict(pillar.verdict) !== AMBER) return false;
  const raw = pillar.raw || {};
  const summary = raw.summary || {};
  const operational = raw.operational || {};
  const corePlatformsReady = CORE_PLATFORM_STATUS_KEYS.every((platform) =>
    platformStateIsEnabled(operational[platform] || {}),
  );
  if (!corePlatformsReady) return false;

  const needsCredentials = Array.isArray(summary.needs_credentials_platforms)
    ? summary.needs_credentials_platforms
    : [];
  if (needsCredentials.length > 0 || numberFromAny(summary.needs_credentials_platform_count) > 0) {
    return false;
  }

  const disabled = Array.isArray(summary.disabled_platforms) ? summary.disabled_platforms : [];
  const disabledOptionalOnly = disabled.every((platform) =>
    OPTIONAL_DEFERRED_PLATFORM_STATUS_KEYS.has(String(platform || "")),
  );
  if (!disabledOptionalOnly) return false;

  const external = Array.isArray(summary.blocked_external_platforms)
    ? summary.blocked_external_platforms
    : [];
  const externalDeferredOnly = external.every((platform) => String(platform || "") === "tiktok");
  return externalDeferredOnly;
}

function applyEnabledPlatformAutoPublishReadinessScope(pillars = {}) {
  const scopedPillars = Object.fromEntries(
    Object.entries(pillars).map(([name, pillar]) => [name, { ...pillar }]),
  );
  const handoffReady = guardedEnabledPlatformHandoffReady(scopedPillars);
  const autonomousHandoffReady = guardedEnabledPlatformAutonomousHandoffReady(scopedPillars);
  const schedulerReady = guardedEnabledPlatformSchedulerReady(scopedPillars);
  const scope = {
    name: "all_platforms",
    guard_ready: false,
    overridden_pillars: [],
  ...
  if (!handoffReady && !schedulerReady) {
    return { pillars: scopedPillars, scope };
  }

  scope.name = handoffReady
    ? "enabled_platform_guarded_handoff"
    : "enabled_platform_guarded_scheduler_window";
  scope.guard_ready = true;

  if (autonomousHandoffReady) {
    scopedPillars.human_review_approval_gate = scopedGreenPillar(
      scopedPillars.human_review_approval_gate,
      "autonomous_green_dry_run_actions_supersede_human_review_approval_gate",
      {
        autonomous_green_dry_run_actions_used: true,
        invalid_decision_count_still_visible: rawNumber(
          scopedPillars.human_review_approval_gate,
          "invalid_decision_count",
        ),
        approved_action_count_still_visible: rawNumber(
          scopedPillars.human_review_approval_gate,
          "approved_action_count",
        ),
      },
    );
    scope.overridden_pillars.push("human_review_approval_gate");
  }

  if (cadenceIsOnlyHistoricalScheduleAdvisory(scopedPillars.publish_cadence)) {
    scopedPillars.publish_cadence = scopedGreenPillar(
      scopedPillars.publish_cadence,
      "guarded_queue_controls_current_enabled_platform_cadence",
      { cadence_history_still_visible: true },
    );
    scope.overridden_pillars.push("publish_cadence");
  }

  if (strictDryRunIsSupersededByGuardedHandoff(scopedPillars.strict_dry_run_control)) {
    scopedPillars.strict_dry_run_control = scopedGreenPillar(
      scopedPillars.strict_dry_run_control,
      autonomousHandoffReady
        ? "autonomous_guarded_executor_handoff_supersedes_deferred_platform_dry_run_amber"
        : "approved_guarded_executor_handoff_supersedes_pre_approval_dry_run_amber",
      { deferred_platforms_still_excluded: true },
    );
    scope.overridden_pillars.push("strict_dry_run_control");
  } else if (
    strictDryRunRedIsHeldByGuardedSchedulerScope(scopedPillars.strict_dry_run_control) ||
    (
      schedulerReady &&
      strictDryRunRedIsHeldByAutonomousGuardedSchedulerScope(
        scopedPillars.strict_dry_run_control,
      )
    ) ||
    strictDryRunRedIsHeldByGuardedExecutorHandoff(scopedPillars.strict_dry_run_control, scopedPillars)
  ) {
    scopedPillars.strict_dry_run_control = scopedAmberPillar(
      scopedPillars.strict_dry_run_control,
      "non_selected_candidate_blockers_held_by_guarded_scheduler_scope",
      {
        deferred_platforms_still_excluded: true,
        blocked_story_count_still_visible: numberFromAny(
          scopedPillars.strict_dry_run_control?.raw?.blocked_story_count,
        ),
        blocked_action_count_still_visible: numberFromAny(
          scopedPillars.strict_dry_run_control?.raw?.blocked_action_count,
        ),
      },
    );
    scope.overridden_pillars.push("strict_dry_run_control");
  ...
  if (guardedDispatchPreflightRedIsHeldByExecutorHandoff(scopedPillars.guarded_dispatch_preflight, scopedPillars)) {
    scopedPillars.guarded_dispatch_preflight = scopedAmberPillar(
      scopedPillars.guarded_dispatch_preflight,
      "non_selected_guarded_preflight_blockers_held_by_executor_handoff",
      {
        dispatch_ready_action_count_still_visible: rawNumber(
          scopedPillars.guarded_dispatch_preflight,
          "dispatch_ready_action_count",
        ),
        blocked_action_count_still_visible: rawNumber(
          scopedPillars.guarded_dispatch_preflight,
          "blocked_action_count",
        ),
        held_action_count_still_visible: rawNumber(
          scopedPillars.guarded_dispatch_preflight,
          "held_action_count",
        ),
      },
    );
    scope.overridden_pillars.push("guarded_dispatch_preflight");
  }

  if ([RED, AMBER].includes(normaliseReadinessVerdict(scopedPillars.repair_backlog?.verdict))) {
    scopedPillars.repair_backlog = scopedAmberPillar(
      scopedPillars.repair_backlog,
      "repair_backlog_held_outside_current_enabled_platform_scope",
      {
        scoped_repair_items_visible: rawNumber(scopedPillars.repair_backlog, "total_items"),
        auto_repairable_items_still_visible: rawNumber(
          scopedPillars.repair_backlog,
          "auto_repairable_items",
        ),
        operator_required_items_still_visible: rawNumber(
          scopedPillars.repair_backlog,
          "operator_required_items",
        ),
        dead_end_items_still_visible: rawNumber(scopedPillars.repair_backlog, "dead_end_items"),
        active_publish_blocker_items_still_visible: rawNumber(
          scopedPillars.repair_backlog,
          "active_publish_blocker_items",
        ),
      },
    );
    scope.overridden_pillars.push("repair_backlog");
  }

  if (platformStatusIsEnabledPlatformScoped(scopedPillars.platform_status)) {
    scopedPillars.platform_status = scopedGreenPillar(
      scopedPillars.platform_status,
      "enabled_platforms_ready_optional_platforms_deferred",
      { optional_platforms_still_excluded: true },
    );
    scope.overridden_pillars.push("platform_status");
  }

  if (
    normaliseReadinessVerdict(scopedPillars.tiktok_external_block?.verdict) === AMBER &&
    pillarIsGreen(scopedPillars.platform_status)
  ) {
    scopedPillars.tiktok_external_block = scopedGreenPillar(
      scopedPillars.tiktok_external_block,
      "tiktok_direct_post_gate_deferred_outside_enabled_platform_scope",
      {
        deferred_reason: scopedPillars.tiktok_external_block.reason || null,
        tiktok_still_not_counted_publishable: true,
      },
    );
    scope.overridden_pillars.push("tiktok_external_block");
  }
...
function strictDryRunQuarantineContext({
  ...
  const platformPublishNowActionCount = numberFromAny(
    summary.platform_publish_now_action_count,
    plan.platform_publish_now_action_count,
  );
  const platformEnabledDryRunActionCount = numberFromAny(
    summary.platform_enabled_dry_run_action_count,
    plan.platform_enabled_dry_run_action_count,
  );
  const humanReviewRequiredActionCount = numberFromAny(
    summary.human_review_required_action_count,
    plan.human_review_required_action_count,
  );
  const livePublishAllowedActionCount = numberFromAny(
    summary.live_publish_allowed_action_count,
    plan.live_publish_allowed_action_count,
  );
  ...
  const guardedSchedulerScopeClean =
    readyStoryCount > 0 &&
    activeBlockedActionCount === 0 &&
    (platformPublishNowActionCount > 0 || platformEnabledDryRunActionCount > 0);
  ...
  return {
    ...
    platform_publish_now_action_count: platformPublishNowActionCount,
    platform_enabled_dry_run_action_count: platformEnabledDryRunActionCount,
    ...
    guarded_scheduler_scope_clean: guardedSchedulerScopeClean,
...
function renderManifestFrom(row = {}) {
  if (hasObjectKeys(row.render_manifest)) return objectValue(row.render_manifest);
  if (hasObjectKeys(row.renderManifest)) return objectValue(row.renderManifest);
  return {};
}
...
function looksLikePremiumLane(value = {}) {
  return Boolean(
    hasObjectKeys(value) &&
      (
        String(value.rendererSplit || value.renderer_split || "").toLowerCase().includes("hyperframes") ||
...
function premiumLaneObject(row = {}) {
  const renderManifest = renderManifestFrom(row);
  const candidates = [
    row.premiumLane,
    row.premium_lane,
    row.studio?.premiumLane,
    row.studio?.premium_lane,
...
function hyperframesCardCount(row = {}) {
  const lane = premiumLaneObject(row);
  const renderManifest = renderManifestFrom(row);
  return firstFiniteNumber(
    row.hyperframesCardCount,
    row.hyperframes_card_count,
    row.studio?.hyperframesCardCount,
    row.studio?.hyperframes_card_count,
...
function premiumShellGateFrom(row = {}) {
  const lane = premiumLaneObject(row);
  const renderManifest = renderManifestFrom(row);
  return (
    row.hyperframesPremiumShellGate ||
    row.premiumShellGate ||
    row.hyperframes_premium_shell_gate ||
    row.premium_shell_gate ||
    row.studio?.hyperframesPremiumShellGate ||
    row.studio?.premiumShellGate ||
...
function premiumShellVerdictFrom(row = {}) {
  const gate = premiumShellGateFrom(row);
  const lane = premiumLaneObject(row);
  const renderManifest = renderManifestFrom(row);
  return String(
    row.premiumShellVerdict ||
      row.premium_shell_verdict ||
      row.studio?.premiumShellVerdict ||
      lane.premiumShellVerdict ||
      lane.premium_shell_verdict ||
      renderManifest.premiumShellVerdict ||
      renderManifest.premium_shell_verdict ||
      gate.verdict ||
      gate.status ||
      "",
  ).trim().toLowerCase();
...
function premiumShellPassCountFrom(row = {}) {
  const gate = premiumShellGateFrom(row);
  const lane = premiumLaneObject(row);
  const renderManifest = renderManifestFrom(row);
  return firstFiniteNumber(
    row.premiumShellPassCount,
    row.premium_shell_pass_count,
    row.studio?.premiumShellPassCount,
    lane.premiumShellPassCount,
    lane.premium_shell_pass_count,
    renderManifest.premiumShellPassCount,
...
function rowDeclaresHyperframesPremium(row = {}) {
  const laneObject = premiumLaneObject(row);
  const renderManifest = renderManifestFrom(row);
  const lane = String(row.render_lane || row.lane || "").toLowerCase();
  const renderer = String(row.renderer || row.render_renderer || "").toLowerCase();
  const split = String(
    row.rendererSplit ||
      row.renderer_split ||
      laneObject.rendererSplit ||
      laneObject.renderer_split ||
      renderManifest.rendererSplit ||
      renderManifest.renderer_split ||
      "",
  ).toLowerCase();
  const premiumLane = String(
    typeof row.premiumLane === "string"
      ? row.premiumLane
      : typeof row.premium_lane === "string"
        ? row.premium_lane
        : "",
  ).toLowerCase();
  const renderText = String(
    [
      ...
      renderManifest.render_renderer,
    ].filter(Boolean).join(" "),
  ).toLowerCase();
  return (
    hyperframesCardCount(row) > 0 ||
    lane.includes("hyperframes") ||
    renderer.includes("hyperframes") ||
    split.includes("hyperframes") ||
    premiumLane === "hyperframes" ||
    renderText.includes("hyperframes")
  );
}

function hyperframesPremiumShellIssues(stories = []) {
  return (Array.isArray(stories) ? stories : [])
    .map((row) => {
      if (!rowDeclaresHyperframesPremium(row)) return null;
      const cardCount = hyperframesCardCount(row);
      const requiredPassCount = firstFiniteNumber(
        row.premiumShellRequiredPassCount,
        row.premium_shell_required_pass_count,
        row.studio?.premiumShellRequiredPassCount,
        premiumLaneObject(row).premiumShellRequiredPassCount,
        premiumLaneObject(row).premium_shell_required_pass_count,
        renderManifestFrom(row).premiumShellRequiredPassCount,
        renderManifestFrom(row).premium_shell_required_pass_count,
        premiumShellGateFrom(row).requiredPassCount,
        premiumShellGateFrom(row).required_pass_count,
      ) || MIN_PREMIUM_HYPERFRAMES_CARDS;
      const verdict = premiumShellVerdictFrom(row);
      const passCount = premiumShellPassCountFrom(row);
      const blockers = [
        ...(Array.isArray(row.premiumShellBlockers) ? row.premiumShellBlockers : []),
        ...(Array.isArray(row.premium_shell_blockers) ? row.premium_shell_blockers : []),
        ...(Array.isArray(premiumShellGateFrom(row).blockers) ? premiumShellGateFrom(row).blockers : []),
      ].filter(Boolean);
      const passCountMissing = passCount == null;
      const passCountTooLow = !passCountMissing && passCount < requiredPassCount;
      if (verdict === "pass" && !passCountMissing && !passCountTooLow && blockers.length === 0) return null;

      return {
        story_id: storyId(row) || row.id || "unknown",
        title: String(row.title || row.public_title || row.suggested_title || "").slice(0, 120),
        hyperframes_card_count: cardCount,
        required_pass_count: requiredPassCount,
        premium_shell_verdict: verdict || "missing",
...
function evidenceVerdictPasses(value = "") {
  return ["green", "pass", "passed"].includes(
    String(value || "").trim().toLowerCase(),
  );
}

function exactAuthorityReconcilesMissingPremiumShellAlias({
  row = {},
  issue = {},
  plan = {},
} = {}) {
  if (
    issue.premium_shell_verdict !== "missing" ||
    issue.premium_shell_pass_count != null ||
    Number(issue.hyperframes_card_count || 0) !== 0 ||
    (Array.isArray(issue.blockers) && issue.blockers.length > 0)
  ) {
    return false;
  }
  const id = storyId(row);
  const action = (Array.isArray(plan.actions) ? plan.actions : []).find(
    (candidate) =>
      storyId(candidate) === id &&
      ["would_publish", "would_queue_when_enabled"].includes(
        String(candidate?.action || "").trim(),
      ),
  );
  if (
    !action ||
    action.live_execution_gate !== "guarded_dispatch_ready" ||
    action.video_fingerprint_source !== "exact_current_package_authority" ||
    (Array.isArray(action.blockers) && action.blockers.length > 0) ||
    (Array.isArray(action.warnings) && action.warnings.length > 0) ||
    !Array.isArray(action.authority_reconciliations) ||
    !action.authority_reconciliations.includes(
  ...
  const publishVerdict = objectValue(row.publish_verdict);
  const authority = objectValue(publishVerdict.authority_refresh);
  const frozenRender = objectValue(authority.frozen_hashes?.render);
  const actionPath = normaliseComparablePath(action.video_path);
  const frozenPath = normaliseComparablePath(frozenRender.path);
  const exportedPath = normaliseComparablePath(row.exported_path);
  const manifestPath = normaliseComparablePath(
    renderManifest.output_path ||
      renderManifest.final_mp4_path ||
      renderManifest.exported_path ||
      renderManifest.output,
  );
  const actionSha256 = normaliseSha256(action.video_sha256);
  const frozenSha256 = normaliseSha256(frozenRender.sha256);
  const adoptionSha256 = normaliseSha256(adoption.final_render_sha256);
  const actionSize = Number(action.video_size_bytes);
  const frozenSize = Number(frozenRender.size_bytes);
  const adoptionSize = Number(adoption.final_render_size_bytes);
  ...
  return Boolean(
    id &&
      ...
      actionPath &&
      actionPath === frozenPath &&
      actionPath === exportedPath &&
      actionPath === manifestPath &&
      /^[a-f0-9]{64}$/.test(actionSha256) &&
      actionSha256 === frozenSha256 &&
      actionSha256 === adoptionSha256 &&
      Number.isFinite(actionSize) &&
      actionSize > 0 &&
      actionSize === frozenSize &&
      actionSize === adoptionSize &&
      evidenceVerdictPasses(adoption.human_av_review_verdict) &&
      evidenceVerdictPasses(adoption.temporal_video_qa_verdict) &&
      evidenceVerdictPasses(adoption.rights_placement_verdict)
  );
}
...
function pillarRenderMetadata({
  ...
  if (recent.length === 0 && bridgeCandidates.length === 0) {
    return { ok: false, verdict: UNKNOWN, reason: "no_exported_stories" };
  }
  const aggregate = renderMetadataCounts(recent.length ? recent : bridgeCandidates);
  let verdict = GREEN;
  ...
  if (plan && dryRunSafetyIsIntact(plan)) {
    ...
    if (ids.active.size || ids.quarantined.size) {
      ...
      const rawActiveHyperframesShellIssues = hyperframesPremiumShellIssues(active);
      const activeRowsById = new Map(active.map((row) => [storyId(row), row]));
      const reconciledHyperframesShellIssues = rawActiveHyperframesShellIssues.filter(
        (issue) => exactAuthorityReconcilesMissingPremiumShellAlias({
          row: activeRowsById.get(issue.story_id),
          issue,
          plan,
        }),
      );
      const reconciledStoryIds = new Set(
        reconciledHyperframesShellIssues.map((issue) => issue.story_id),
      );
      const activeHyperframesShellIssues = rawActiveHyperframesShellIssues.filter(
        (issue) => !reconciledStoryIds.has(issue.story_id),
      );
      const quarantinedHyperframesShellIssues = hyperframesPremiumShellIssues(quarantined);
      ...
      } else if (activeHyperframesShellIssues.length > 0) {
        scopedVerdict = RED;
        reason = `active_hyperframes_premium_shell_not_passed:${activeHyperframesShellIssues.length}`;
      ...
      return {
        ...
        raw: {
          ...
          active_hyperframes_premium_shell_blocker_count: activeHyperframesShellIssues.length,
          active_hyperframes_premium_shell_blockers: activeHyperframesShellIssues,
          active_hyperframes_premium_shell_reconciled_count:
            reconciledHyperframesShellIssues.length,
          active_hyperframes_premium_shell_reconciled_story_ids:
            [...reconciledStoryIds],
          quarantined_hyperframes_premium_shell_blocker_count: quarantinedHyperframesShellIssues.length,
          quarantined_hyperframes_premium_shell_blockers: quarantinedHyperframesShellIssues,
        },
      };
    }
  }
...
function summariseLocalTtsDoctorOverride(doctorReport = null) {
  if (!doctorReport || typeof doctorReport !== "object") return null;
  const verdict = normaliseReadinessVerdict(doctorReport.verdict || doctorReport.status);
  const generationSmoke = doctorReport.generation_smoke || null;
  const generationSmokeFailed = generationSmoke && generationSmoke.ok === false;
  const ready = verdict === GREEN && !generationSmokeFailed;
  return {
    ready,
    verdict,
    ...
    generation_smoke_ok:
      generationSmoke && Object.prototype.hasOwnProperty.call(generationSmoke, "ok")
        ? generationSmoke.ok === true
        : null,
    generation_smoke_error: generationSmoke?.error || null,
  };
}

function storyHasPublicPlatformPost(story = {}) {
  return Boolean(
    story &&
    (
      ...
      story.facebook_reel_id
    )
  );
}

function summariseLocalTtsRecoveryContext(report = null, doctorReport = null, options = {}) {
  if (!report || typeof report !== "object") return null;
  const doctor = report.doctor || {};
  const recovery = report.autonomous_recovery || {};
  const plan = report.recovery_plan || {};
  const doctorOverride = summariseLocalTtsDoctorOverride(doctorReport);
  const alreadyPublicStoryIds = options.alreadyPublicStoryIds instanceof Set
    ? options.alreadyPublicStoryIds
    : new Set(options.alreadyPublicStoryIds || []);
  const workOrders = Array.isArray(plan.work_orders) ? plan.work_orders : [];
  const safeRetryWorkOrders = workOrders.filter((order) =>
    order &&
    order.repair_lane === "local_tts_retry" &&
    order.operator_approval_required !== true &&
    order.external_posting_risk !== true &&
    order.db_mutation_required !== true &&
    !alreadyPublicStoryIds.has(order.story_id),
  );
  const excludedAlreadyPublicRetryIds = [...new Set(workOrders
    .filter((order) =>
      order &&
      order.repair_lane === "local_tts_retry" &&
      alreadyPublicStoryIds.has(order.story_id)
    )
    .map((order) => order.story_id)
    .filter(Boolean))];
  const operatorRequiredWorkOrders = workOrders.filter((order) =>
    order && order.operator_approval_required === true,
  );
  const safeRetryStoryIds = Array.isArray(recovery.safe_retry_story_ids)
    ? recovery.safe_retry_story_ids
    : Array.isArray(plan.retry_tts_story_ids)
      ? plan.retry_tts_story_ids
      : safeRetryWorkOrders.map((order) => order.story_id).filter(Boolean);
  const filteredSafeRetryStoryIds = safeRetryStoryIds.filter(
    (storyId) => !alreadyPublicStoryIds.has(storyId),
  );
  let currentLocalTtsReady =
    recovery.current_local_tts_ready === true ||
    doctor.local_ready === true ||
    (
      normaliseReadinessVerdict(doctor.verdict) === GREEN &&
      doctor.voice?.loaded === true &&
      (doctor.voice?.ref_resolved === true || doctor.voice?.refResolved === true)
    );
  if (doctorOverride) {
    currentLocalTtsReady = doctorOverride.ready === true;
  }
  const hasRecoveryWorkOrders = workOrders.length > 0;
  const retryPreflightWorkOrders = safeRetryWorkOrders.filter((order) =>
    order && order.preflight_required === true,
  );
  const retryApplyReadyWorkOrders = safeRetryWorkOrders.filter((order) =>
    order && order.preflight_required !== true,
  );
  const hasPreflightMetadata = safeRetryWorkOrders.some((order) =>
    order && Object.prototype.hasOwnProperty.call(order, "preflight_required"),
  );
  const safeRetryCount = hasRecoveryWorkOrders
    ? safeRetryWorkOrders.length
    : numberFromAny(recovery.safe_retry_work_order_count, safeRetryWorkOrders.length);
  const retryPreflightCount = hasRecoveryWorkOrders && hasPreflightMetadata
    ? retryPreflightWorkOrders.length
    : Math.max(
        0,
        numberFromAny(recovery.retry_preflight_work_order_count, retryPreflightWorkOrders.length) -
          excludedAlreadyPublicRetryIds.length,
      );
  const retryApplyReadyCount = hasRecoveryWorkOrders && hasPreflightMetadata
    ? retryApplyReadyWorkOrders.length
    : Math.max(
        0,
        numberFromAny(recovery.retry_apply_ready_work_order_count, retryApplyReadyWorkOrders.length) -
          excludedAlreadyPublicRetryIds.length,
      );
  const operatorRequiredCount = numberFromAny(
    recovery.operator_required_work_order_count,
    operatorRequiredWorkOrders.length,
  );
  const derivedReadyStatus =
    currentLocalTtsReady && retryApplyReadyCount > 0
      ? "ready_for_local_tts_retry_apply"
      : currentLocalTtsReady && retryPreflightCount > 0
      ? "ready_for_local_tts_retry_preflight"
      : currentLocalTtsReady && safeRetryCount > 0
      ? "ready_for_local_tts_retry"
      : currentLocalTtsReady
        ? "no_local_tts_recovery_work_required"
        : "blocked_tts_not_ready";
  const staleBlockedRecoveryStatus =
    currentLocalTtsReady &&
    doctorOverride &&
    String(recovery.status || "").trim().toLowerCase() === "blocked_tts_not_ready";
  const status =
    !currentLocalTtsReady && doctorOverride
      ? "blocked_tts_not_ready"
      : staleBlockedRecoveryStatus
        ? derivedReadyStatus
      : (
          recovery.status ||
          derivedReadyStatus
  ...
  return {
    status,
    current_local_tts_ready: currentLocalTtsReady,
    safe_to_run_local_tts_retry:
      currentLocalTtsReady &&
      (
        recovery.safe_to_run_local_tts_retry === true ||
        retryApplyReadyCount > 0
      ),
    safe_to_run_local_tts_retry_preflight:
      currentLocalTtsReady &&
      (
        recovery.safe_to_run_local_tts_retry_preflight === true ||
        safeRetryCount > 0
      ),
    safe_retry_work_order_count: safeRetryCount,
    retry_preflight_work_order_count: retryPreflightCount,
    retry_apply_ready_work_order_count: retryApplyReadyCount,
    operator_required_work_order_count: operatorRequiredCount,
    safe_retry_story_ids: filteredSafeRetryStoryIds.slice(0, 10),
    excluded_already_public_retry_count: excludedAlreadyPublicRetryIds.length,
    excluded_already_public_retry_story_ids: excludedAlreadyPublicRetryIds.slice(0, 10),
    report_verdict: report.verdict || null,
    report_generated_at: report.generated_at || null,
    ...(doctorOverride
      ? {
          doctor_verdict: doctorOverride.verdict,
          doctor_failure_code: doctorOverride.failure_code,
          doctor_reason: doctorOverride.reason,
          doctor_generation_smoke_ok: doctorOverride.generation_smoke_ok,
          doctor_generation_smoke_error: doctorOverride.generation_smoke_error,
        }
      : {}),
  };
}
...
function summariseRecentFailedCandidates(
  ...
  const alreadyPublicStoryIds = new Set(
    (stories || [])
      .filter(storyHasPublicPlatformPost)
      .map((s) => s.id)
      .filter(Boolean),
  );
  const localTtsRecovery = summariseLocalTtsRecoveryContext(
    localTtsReport,
    localTtsDoctorReport,
    { alreadyPublicStoryIds },
  );
  ...
  return {
    ...
    examples,
    ...(localTtsRecovery ? { local_tts_recovery: localTtsRecovery } : {}),
  };
}
...
function isPlatformVariantOnlyBlockedAction(action = {}) {
  ...
  return (
    ...
    reasons.every((reason) =>
      /^platform_variant_(?:stale_after_render|source_duration_mismatch):[a-z0-9_]+$/i.test(reason)
    ) &&
...
function pillarStrictDryRunControl({
  ...
  const raw = {
    ...
    platform_enabled_dry_run_action_count: Number(
      summary.platform_enabled_dry_run_action_count || 0,
    ),
    blocked_action_count: Number(summary.blocked_action_count || 0),
    warning_action_count: Number(summary.warning_action_count || 0),
    publish_now_warning_action_count: Number(summary.publish_now_warning_action_count || 0),
    ...
    guarded_dispatch_ready_action_count: Number(
      summary.guarded_dispatch_ready_action_count ||
        plan.safe_publish_plan?.guarded_dispatch_ready_action_count ||
        0,
    ),
...
function pillarRepairBacklog({
  ...
  if (totalItems > 0) {
    ...
    const guardedSchedulerScopeHoldsBacklog =
      publishBlockerResolutionItems === 0 &&
      quarantineContext.available === true &&
      quarantineContext.guarded_scheduler_scope_clean === true;
    if (guardedSchedulerScopeHoldsBacklog) {
      return {
        ok: true,
        verdict: AMBER,
        reason: `guarded_scheduler_scope_holds_repair_backlog:${summariseRepairBacklogReason(report)}`,
        raw: {
          ...raw,
          readiness_scope: "guarded_scheduler_scope_held_repair_backlog",
          active_publish_blocker_items: 0,
          scoped_repair_items_visible: totalItems,
          strict_dry_run_context: quarantineContext,
        },
      };
    }
...
function activeVideoPathSetFromStrictDryRunPlan(plan = {}) {
  if (!dryRunSafetyIsIntact(plan)) return new Set();
  const actions = Array.isArray(plan.actions) ? plan.actions : [];
  const paths = new Set();
  for (const action of actions) {
    const actionType = String(action?.action || "").trim();
    if (!["would_publish", "would_queue_when_enabled"].includes(actionType)) continue;
    const normalised = normaliseComparablePath(
      action.video_path || action.exported_path || action.media_path,
    );
    if (normalised) paths.add(normalised);
  }
  return paths;
...
function pillarFinalVoiceAudit({
  ...
  const dryRunActivePaths = activeVideoPathSetFromStrictDryRunPlan(strictDryRunPlan || {});
  const manifestActivePaths = activeVideoPathSetFromManifest(manifest || {});
  const strictDryRunPlanPresent = strictDryRunPlan !== null;
  const activePaths = strictDryRunPlanPresent ? dryRunActivePaths : manifestActivePaths;
  const scopedToActivePaths = strictDryRunPlanPresent || activePaths.size > 0;
  const activeScopeSource = strictDryRunPlanPresent
    ? "strict_dry_run_plan"
    : manifestActivePaths.size > 0
      ? "local_test_video_manifest"
      : "all_rows";
  const activeRows = scopedToActivePaths
    ? rows.filter((row) => activePaths.has(normaliseComparablePath(row?.mp4_path)))
    : rows;
  const quarantinedRows = scopedToActivePaths
    ? rows.filter((row) => !activePaths.has(normaliseComparablePath(row?.mp4_path)))
    : [];
  ...
  const raw = {
    ...
    local_test_manifest_path: localTestManifestPath,
    strict_dry_run_plan_path: strictDryRunPlanPath,
    active_scope_source: activeScopeSource,
    generated_at: report.generated_at || null,
    age_hours: ageHours,
    scoped_to_active_paths: scopedToActivePaths,
    scoped_to_local_test_manifest: !strictDryRunPlanPresent && manifestActivePaths.size > 0,
    scoped_to_strict_dry_run_plan: strictDryRunPlanPresent,
...
function pillarGuardedDispatchPreflight({
  ...
  const raw = {
    ...
    approved_action_count: Number(summary.approved_action_count || 0),
    autonomous_dry_run_action_count: Number(summary.autonomous_dry_run_action_count || 0),
...
function pillarRecentFailedCandidates({
  stories,
  now,
  localTtsReportPath,
  localTtsReport,
  ...
  let ttsReport = localTtsReport || null;
  if (!ttsReport && localTtsReportPath) {
    try {
      ttsReport = readJsonIfPresent(localTtsReportPath);
    } catch {
      ttsReport = null;
    }
  }
  let ttsDoctorReport = localTtsDoctorReport || null;
  if (!ttsDoctorReport && localTtsDoctorPath) {
    try {
      ttsDoctorReport = readJsonIfPresent(localTtsDoctorPath);
    } catch {
  ...
  const summary = summariseRecentFailedCandidates(stories, {
    ...
    now,
    recentWindowHours: 24,
    localTtsReport: ttsReport,
    localTtsDoctorReport: ttsDoctorReport,
  });
  let verdict = GREEN;
  if (summary.recent_count >= 5) verdict = AMBER;
  const recoveryReason = summary.local_tts_recovery
    ? "; local_tts_recovery=" +
      summary.local_tts_recovery.status +
      `; current_local_tts_ready=${summary.local_tts_recovery.current_local_tts_ready}` +
      `; retry_preflight_work_orders=${summary.local_tts_recovery.retry_preflight_work_order_count}` +
      `; retry_apply_ready_work_orders=${summary.local_tts_recovery.retry_apply_ready_work_order_count}`
    : "";
  const reason =
    verdict === AMBER
...
function resolvePublishReadinessNextAction({ overall, pillars = {} } = {}) {
  ...
  if (strictDryRunHasNoCurrentPublishableActions(pillars.strict_dry_run_control)) {
    return "Rebuild the fresh GREEN candidate buffer before expecting uploads. Current strict dry-run has no enabled-platform publish actions, so stale guarded-dispatch evidence must not be used as live publish authority.";
  }

  if (guardedEnabledPlatformHandoffReady(pillars)) {
    return "Guarded enabled-platform dispatch is ready. Let the scheduler or guarded executor run selected YouTube, Instagram Reels and Facebook Reels actions with the kill switch and final platform recheck; disabled or deferred platforms stay excluded.";
  }

  if (guardedEnabledPlatformSchedulerReady(pillars)) {
    return "Let the normal guarded scheduler select the approved enabled-platform actions at the next window. Select explicit action IDs only for a manual executor handoff; disabled or deferred platforms stay excluded.";
  }
  ...
  if (guardedDispatchExecutorPreflight?.verdict === AMBER) {
    ...
    if (Number(raw.dispatch_ready_action_count || 0) > 0 && Number(raw.selected_action_count || 0) <= 0) {
      const strictRaw = strictDryRun?.raw || {};
      if (
        strictDryRun?.reason === "non_selected_candidate_blockers_held_by_guarded_scheduler_scope" ||
        strictRaw.enabled_platform_scope_override === "non_selected_candidate_blockers_held_by_guarded_scheduler_scope"
      ) {
        return "Let the normal guarded scheduler select the approved enabled-platform actions at the next window. Select explicit action IDs only for a manual executor handoff; disabled or deferred platforms stay excluded.";
      }
...
async function buildPublishReadinessReport(opts = {}) {
  ...
  if (skipOperationalPillars) {
    ...
    sd = rd = qh = lr = lp = ps = mv = mi = docs = skipped;
  } else {
    // The local restart probe hits /api/health. Keep it out of the
    // heavier parallel batch so a slow local runtime cannot become a
    // false RED while DB and media audits are also running.
    lr = await pillarLocalRestartReadiness({ env, now });
    // Run the remaining async pillars in parallel.
    ...
    [sd, lp, rd, qh, ps, mv, mi, docs] = await Promise.all([
      pillarSystemDoctor(),
      buildLocalPostingReadinessPillar({
        cwd: opts.cwd,
        env: opts.env,
        now,
        reportPath: opts.localPostingReadinessPath,
        localCutoverPlanPath: opts.localCutoverPlanPath,
        localPrimaryReadinessPath: opts.localPrimaryReadinessPath,
        localTunnelReadinessPath: opts.localTunnelReadinessPath,
        localTtsReportPath: opts.localTtsReportPath,
        localTtsDoctorPath: opts.localTtsDoctorPath,
        refreshLocalPostingArtifacts: opts.refreshLocalPostingArtifacts,
        localPostingArtifactRefresh: opts.localPostingArtifactRefresh,
      }),
      pillarRailwayDeploy(),
      pillarQueueHealth(),
  ...
  const rawPillars = {
    ...
    recent_failed_candidates: pillarRecentFailedCandidates({
      stories,
      now,
      localTtsReportPath:
        opts.localTtsReportPath ||
        defaultLocalPostingArtifactPath(opts.cwd, "local_tts_overnight_report.json", opts.env),
      localTtsReport: opts.localTtsReport,
      localTtsDoctorPath:
        opts.localTtsDoctorPath ||
        defaultLocalPostingArtifactPath(opts.cwd, "local_tts_doctor.json", opts.env),
      localTtsDoctorReport: opts.localTtsDoctorReport,
    }),
  ...
  const basePillars = Object.fromEntries(
    Object.entries(rawPillars).map(([name, pillar]) => [name, normalisePillar(pillar)]),
  );
  const scopedReadiness = applyEnabledPlatformAutoPublishReadinessScope(basePillars);
  const pillars = scopedReadiness.pillars;
  ...
  return {
    ...
    recently_improved,
    next_action,
    readiness_scope: scopedReadiness.scope,
...
module.exports = {
  ...
  applyEnabledPlatformAutoPublishReadinessScope,
  pillarPublishCadence,
  pillarLocalRestartReadiness,
  pillarLocalPostingReadiness,
  buildLocalPostingReadinessPillar,
  localPostingPillarNeedsArtifactRefresh,
  ...
  summariseLocalPostingReadinessReason,
