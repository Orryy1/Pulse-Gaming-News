...
function defaultLocalPostingArtifactPath(cwd, fileName) {
  return path.join(cwd || path.join(__dirname, "..", ".."), "test", "output", fileName);
}
...
function pillarLocalPostingReadiness(opts = {}) {
  ...
  try {
      ...
      : [
          {
            ...
            path: opts.localCutoverPlanPath || defaultLocalPostingArtifactPath(cwd, "local_cutover_plan.json"),
          },
          {
            name: "local_primary_readiness",
            path: opts.localPrimaryReadinessPath || defaultLocalPostingArtifactPath(cwd, "local_primary_readiness.json"),
          },
          {
            name: "local_tunnel_readiness",
            path: opts.localTunnelReadinessPath || defaultLocalPostingArtifactPath(cwd, "local_tunnel_readiness.json"),
          },
          {
            name: "local_tts_overnight_report",
            path: opts.localTtsReportPath || defaultLocalPostingArtifactPath(cwd, "local_tts_overnight_report.json"),
          },
          {
            name: "local_tts_doctor",
            path: opts.localTtsDoctorPath || defaultLocalPostingArtifactPath(cwd, "local_tts_doctor.json"),
          },
        ];
    ...
    const built = report || mod.buildLocalPostingReadiness({
      cutoverPlan:
        opts.cutoverPlan ||
        readJsonIfPresent(opts.localCutoverPlanPath || defaultLocalPostingArtifactPath(cwd, "local_cutover_plan.json")),
      primaryReadiness:
        opts.primaryReadiness ||
        readJsonIfPresent(opts.localPrimaryReadinessPath || defaultLocalPostingArtifactPath(cwd, "local_primary_readiness.json")),
      tunnelReadiness:
        opts.tunnelReadiness ||
        readJsonIfPresent(opts.localTunnelReadinessPath || defaultLocalPostingArtifactPath(cwd, "local_tunnel_readiness.json")),
      ttsReport:
        opts.ttsReport ||
        readJsonIfPresent(opts.localTtsReportPath || defaultLocalPostingArtifactPath(cwd, "local_tts_overnight_report.json")),
      ttsDoctorReport:
        opts.ttsDoctorReport ||
        readJsonIfPresent(opts.localTtsDoctorPath || defaultLocalPostingArtifactPath(cwd, "local_tts_doctor.json")),
      now: opts.now ? new Date(opts.now) : new Date(),
    });
...
function localPostingPillarNeedsArtifactRefresh(pillar = {}) {
  ...
  return Number(freshness.stale_count || 0) > 0 || Number(freshness.missing_count || 0) > 0;
}
...
function pillarFinalVoiceAudit({
  ...
  const activePaths = dryRunActivePaths.size > 0 ? dryRunActivePaths : manifestActivePaths;
  const scopedToActivePaths = activePaths.size > 0;
  const activeScopeSource = dryRunActivePaths.size > 0
    ? "strict_dry_run_plan"
  ...
  const raw = {
    ...
    scoped_to_local_test_manifest: manifestActivePaths.size > 0 && dryRunActivePaths.size === 0,
    scoped_to_strict_dry_run_plan: dryRunActivePaths.size > 0,
  ...
  const rawPillars = {
    ...
    recent_failed_candidates: pillarRecentFailedCandidates({
      ...
      localTtsReportPath:
        ...
        defaultLocalPostingArtifactPath(opts.cwd, "local_tts_overnight_report.json"),
      localTtsReport: opts.localTtsReport,
      localTtsDoctorPath:
        opts.localTtsDoctorPath ||
        defaultLocalPostingArtifactPath(opts.cwd, "local_tts_doctor.json"),
