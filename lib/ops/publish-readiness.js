...
async function pillarQueueHealth() {
  ...
  try {
    ...
    let verdict = GREEN;
    if (report.stale_running > 0 || (report.failed_terminal || 0) > 5) {
      verdict = AMBER;
    }
    return { ok: true, verdict, raw: report };
  } catch (err) {
...
function pillarFacebookReelEligibility() {
  return {
    ...
    verdict: AMBER,
    raw: {
      mode: "page_not_eligible",
      cause: "meta_page_eligibility_gate_for_new_pages",
      action: "facebook_card_fallback_active",
    },
  };
}
...
function pillarRecentFailedCandidates({ stories } = {}) {
  const failed = (stories || [])
    ...
    .slice(0, 10);
  let verdict = GREEN;
  if (failed.length >= 5) verdict = AMBER;
  return {
    ok: true,
    verdict,
    raw: {
      count: failed.length,
      ids: failed.map((s) => s.id),
    },
  };
}
...
function pillarSecurityBlockers() {
  // Static check: scan server.js for known token-logging anti-patterns.
  // The audit's flagged P0 was fixed in commit 5facfd3 (fingerprint-only
  // logging). This pillar reverifies.
  const fs = require("fs-extra");
  const path = require("node:path");
  try {
    const serverPath = path.join(__dirname, "..", "..", "server.js");
    const txt = fs.readFileSync(serverPath, "utf-8");
    // Patterns that would leak a token
    const patterns = [
      {
      ...
      { re: /console\.log\([^)]*pageToken[^)]*\)/, label: "page_token_log" },
    ];
    const hits = [];
    for (const p of patterns) if (p.re.test(txt)) hits.push(p.label);
    if (hits.length === 0) {
      return { ok: true, verdict: GREEN, raw: { token_log_patterns_found: 0 } };
    }
    return {
      ok: true,
      verdict: RED,
...
async function buildPublishReadinessReport(opts = {}) {
  ...
  const pillars = {
    ...
    recent_failed_candidates: pillarRecentFailedCandidates({ stories }),
