  ...
  return (Array.isArray(stories) ? stories : [])
    .map((row) => {
      ...
      const explicitRequirement =
        row.hyperframes_premium_shell_required === true ||
        row.premium_shell_required === true ||
        premiumLaneObject(row).premium_shell_required === true ||
        premiumLaneObject(row).hyperframes_premium_shell_required === true ||
        renderManifestFrom(row).premium_shell_required === true ||
        renderManifestFrom(row).hyperframes_premium_shell_required === true;
      const requiresShell = explicitRequirement || cardCount >= requiredPassCount;
      if (!requiresShell) return null;
      ...
      const passCountTooLow = passCount != null && passCount < requiredPassCount;
      if (verdict === "pass" && !passCountTooLow) return null;
