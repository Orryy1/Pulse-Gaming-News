...
function hyperframesCardCount(row = {}) {
  return firstFiniteNumber(
    ...
    row.premiumLane?.hyperframesCardCount,
    row.premium_lane?.hyperframes_card_count,
  ) || 0;
}
...
function premiumShellGateFrom(row = {}) {
  return (
    ...
    row.premiumLane?.hyperframesPremiumShellGate ||
    row.premiumLane?.premiumShellGate ||
    {}
  );
}
...
function premiumShellVerdictFrom(row = {}) {
  ...
  return String(
    row.premiumShellVerdict ||
      ...
      row.premiumLane?.premiumShellVerdict ||
      gate.verdict ||
      gate.status ||
      "",
  ).trim().toLowerCase();
...
function premiumShellPassCountFrom(row = {}) {
  ...
  return firstFiniteNumber(
    ...
    row.premiumLane?.premiumShellPassCount,
    gate.passCount,
    gate.pass_count,
  );
}
...
function rowDeclaresHyperframesPremium(row = {}) {
  ...
  const split = String(
    row.rendererSplit ||
      row.renderer_split ||
      row.premiumLane?.rendererSplit ||
      row.premiumLane?.renderer_split ||
      "",
  ).toLowerCase();
  const premiumLane = String(row.premiumLane || row.premium_lane || "").toLowerCase();
  return (
  ...
  return (Array.isArray(stories) ? stories : [])
    .map((row) => {
      ...
      const requiredPassCount = firstFiniteNumber(
        ...
        row.premiumLane?.premiumShellRequiredPassCount,
      ...
      const explicitRequirement =
        ...
        row.premiumLane?.premium_shell_required === true ||
        row.premiumLane?.hyperframes_premium_shell_required === true;
