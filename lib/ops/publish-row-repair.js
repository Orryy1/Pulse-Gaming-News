...
function buildScriptFallbackRepairSql(rows = [], { now = new Date().toISOString() } = {}) {
  ...
  return [
    ...
    "    script_review_reason = COALESCE(NULLIF(script_review_reason, ''), 'script_validation_review_required'),",
    `    updated_at = ${ts}`,
    `WHERE id IN (${idList})`,
    ...
    "    OR hook LIKE '%Script validation failed%'",
    ...
    "    OR script_review_reason LIKE '%script_validation%'",
    "  );",
    "COMMIT;",
  ].join("\n");
}
