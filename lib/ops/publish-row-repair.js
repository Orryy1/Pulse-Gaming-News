...
const SCRIPT_FALLBACK_REPAIR_MARKER =
...
function repairedScriptFallbackError(existing) {
  const value = String(existing || "").trim();
  if (!value) return SCRIPT_FALLBACK_REPAIR_MARKER;
  if (value.includes(SCRIPT_FALLBACK_REPAIR_MARKER)) return value;
  return `${SCRIPT_FALLBACK_REPAIR_MARKER}: ${value}`;
}
...
function buildScriptFallbackRepairSql(rows = [], { now = new Date().toISOString() } = {}) {
  ...
  return [
    ...
    "    qa_failed = 1,",
    "    publish_error = CASE",
    `      WHEN publish_error IS NULL OR TRIM(publish_error) = '' THEN ${sqlQuote(SCRIPT_FALLBACK_REPAIR_MARKER)}`,
    `      WHEN publish_error LIKE '%${SCRIPT_FALLBACK_REPAIR_MARKER}%' THEN publish_error`,
    `      ELSE ${sqlQuote(`${SCRIPT_FALLBACK_REPAIR_MARKER}: `)} || publish_error`,
    "    END,",
    `    updated_at = ${ts}`,
    `WHERE id IN (${idList})`,
  ...
  for (const id of redIds) {
    ...
    applied.push({
      ...
      next: {
        ...
        qa_failed: true,
        publish_error: repairedScriptFallbackError(story.publish_error),
