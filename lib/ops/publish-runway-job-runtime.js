...
function buildRunwaySourceDocuments({
  ...
  const reserve = strictCandidates
    .slice(STRICT_IMMEDIATE_COUNT, STRICT_BUFFER_COUNT)
    .map((row) => strictCandidateRow(row, "reserve"));
  const candidateBlockers = [];
  if (strictCandidates.length < STRICT_BUFFER_COUNT) {
    candidateBlockers.push(
      `strict_green_candidate_buffer_below_10:${strictCandidates.length}/${STRICT_BUFFER_COUNT}`,
    );
  }
...
function runtimeConfig({
  ...
  const storeRoot = path.resolve(
    clean(payload.runway_store_root || env.PULSE_PUBLISH_RUNWAY_ROOT) ||
      path.join(root, "output", "runtime", "publish-runway"),
  );
  const sourceRoot = path.resolve(
    clean(payload.runway_source_root || env.PULSE_PUBLISH_RUNWAY_SOURCE_ROOT) ||
      path.join(root, "output", "runtime", "publish-runway-source"),
    generationId,
  );
  return {
    phase,
    ...
    goal_contract_root: path.join(root, "output", "goal-contract"),
  };
}
