...
// Canonical intake refreshes at most 30 stories: five immediate plus 25 reserve.
const MAXIMUM_RESERVE_STORIES = 25;
...
function normaliseReserveTarget(value) {
  const supplied = value !== undefined && value !== null && clean(value) !== "";
  const requested = supplied ? Number(value) : MINIMUM_RESERVE_STORIES;
  if (!Number.isFinite(requested)) {
    fail(
      "INVALID_RESERVE_TARGET",
      "target reserve count must be a finite number",
      { target_reserve_count: value },
    );
  }
  const requestedStoryCount = Math.ceil(requested);
  return {
    requested_story_count: requestedStoryCount,
    effective_story_count: Math.min(
      MAXIMUM_RESERVE_STORIES,
      Math.max(MINIMUM_RESERVE_STORIES, requestedStoryCount),
    ),
    minimum_story_count: MINIMUM_RESERVE_STORIES,
    maximum_story_count: MAXIMUM_RESERVE_STORIES,
  };
}
...
function strictReconciliation(
  report,
  {
    targetRunwayCount = report?.targets?.runway_story_count ??
      MINIMUM_RUNWAY_STORIES,
    targetReserveCount = report?.targets?.reserve_story_count ??
      MINIMUM_RESERVE_STORIES,
  } = {},
) {
  return (
    ...
    report.summary?.executable_youtube_first_story_count >=
      targetRunwayCount &&
    report.summary?.reserve_story_count >= targetReserveCount &&
    Array.isArray(report.blockers) &&
    report.blockers.length === 0
  );
}

function assertStrictReconciliation(report, targets) {
  if (!strictReconciliation(report, targets)) {
    const runwayTarget =
      targets?.targetRunwayCount ??
      report?.targets?.runway_story_count ??
      MINIMUM_RUNWAY_STORIES;
    const reserveTarget =
      targets?.targetReserveCount ??
      report?.targets?.reserve_story_count ??
      MINIMUM_RESERVE_STORIES;
    fail(
      "RUNWAY_RECONCILIATION_NOT_STRICT_GREEN",
      `immutable generation requires strict GREEN reconciliation with ${runwayTarget} immediate and ${reserveTarget} reserve stories`,
...
function exactActionBinding({
  ...
  if (
    selectedActionIds.length !== targetRunwayCount ||
    unique(selectedActionIds).length !== selectedActionIds.length
  ) {
    fail(
      "RUNWAY_ACTION_BINDING_INVALID",
      `reconciliation must bind exactly ${targetRunwayCount} unique immediate action ids`,
  ...
  if (
    reserveStoryIds.length !== targetReserveCount ||
    ...
    fail(
      ...
      `reconciliation must bind ${targetReserveCount} unique reserve stories disjoint from the immediate runway`,
...
class PublishRunwaySchedulerCoordinator {
  ...
  async generateAtT180({
    ...
    const effectiveReserveTarget = reserveTarget.effective_story_count;
    ...
    let reconciliation = reconcilePublishRunway(
      this.reconciliationInput({
        ...
        generatedAt: explicitGeneratedAt,
        targetWindow,
        targetReserveCount: effectiveReserveTarget,
      }),
    );
