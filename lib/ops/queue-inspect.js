...
function runAgeHours(run, nowMs) {
  const ts = parseTimeMs(run?.started_at);
  if (!ts) return null;
  return Math.max(0, (nowMs - ts) / (60 * 60 * 1000));
}
...
function inspectStaleJobRuns(db, { nowMs = Date.now(), staleAfterHours = 6 } = {}) {
  if (!hasTable(db, "job_runs")) return [];
  const columns = getTableColumns(db, "job_runs");
  if (!columns.has("status") || !columns.has("started_at")) return [];
  return db
    .prepare(
      `SELECT ${selectColumn(columns, "id")},
              ${selectColumn(columns, "job_id")},
              ${selectColumn(columns, "worker_id")},
              ${selectColumn(columns, "attempt")},
              ${selectColumn(columns, "status")},
              ${selectColumn(columns, "started_at")},
              ${selectColumn(columns, "finished_at")}
       FROM job_runs
    ...
    .map((run) => ({
      ...run,
      age_hours: runAgeHours(run, nowMs),
    }))
    .filter((run) => run.age_hours === null || run.age_hours >= staleAfterHours)
    .slice(0, 20);
}
...
function inspectQueue({ db, now = Date.now(), recentFailedWindowHours = 24 } = {}) {
  ...
  const staleJobRuns = inspectStaleJobRuns(db, { nowMs });
  ...
  if (staleJobRuns.length > 0) warnings.push("stale_job_runs_present");
...
function renderQueueInspectMarkdown(report) {
  ...
  const lines = [
    ...
    "",
    "## Stale Job Runs",
    ...(report.staleJobRuns?.length
      ? report.staleJobRuns.map((run) =>
          `- #${run.job_id || run.id} worker=${run.worker_id || "unknown"} started=${run.started_at || "unknown"}`,
        )
      : ["- none"]),
    "",
    "## Content Runway",
