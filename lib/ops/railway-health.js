...
function summariseQueueStats(queueStats) {
  if (!queueStats || typeof queueStats !== "object") return null;
  const jobs = queueStats.jobs || {};
  const derivatives = queueStats.derivatives || {};
  return {
    checked: true,
    generated_at: queueStats.generated_at || null,
    jobs: {
      total: Number(jobs.total || 0),
      by_status: jobs.by_status || {},
      gpu_pending: Number(jobs.gpu_pending || 0),
      oldest_pending_minutes:
        jobs.oldest_pending_minutes === null ||
        jobs.oldest_pending_minutes === undefined
          ? null
          : Number(jobs.oldest_pending_minutes),
      stale_claims: Number(jobs.stale_claims || 0),
      recent_failed: Array.isArray(jobs.recent_failed)
        ? jobs.recent_failed.slice(0, 5).map((job) => ({
            id: job.id,
            kind: job.kind,
    ...
    derivatives: {
      total: Number(derivatives.total || 0),
      by_status: derivatives.by_status || {},
    },
  };
}
...
function classifyFailedQueueJobs(queueSummary, latestDeployment) {
  const failedJobs = Number(queueSummary?.jobs?.by_status?.failed || 0);
  if (failedJobs <= 0) {
    return { failedJobs: 0, allSampled: true, afterDeploy: [], historical: [] };
  }

  const recentFailed = queueSummary.jobs.recent_failed || [];
  const deployMs = timestampMs(latestDeployment?.createdAt);
  const allSampled = recentFailed.length >= failedJobs;

  if (!deployMs) {
    return {
      failedJobs,
      allSampled,
      afterDeploy: recentFailed,
      historical: [],
    };
  }
  ...
  for (const job of recentFailed) {
    ...
    if (updatedMs !== null && updatedMs < deployMs) historical.push(job);
    else afterDeploy.push(job);
  }

  return { failedJobs, allSampled, afterDeploy, historical };
}
...
function classifyLogIssues(entries, { source = "app" } = {}) {
  ...
  for (const entry of safeEntries) {
    ...
    if (status >= 400) {
      if (
        status === 401 &&
        String(entry.path || "").replace(/\/$/, "") === "/api/queue/stats"
      ) {
        advisories.push({
          code: "queue_stats_auth_advisory",
          source,
          timestamp: entry.timestamp,
          status,
          method: entry.method,
          path: entry.path,
          message: "Optional queue stats auth failed.",
        });
        continue;
      }
      warnings.push({
    ...
    if (/NODE-CRON[\s\S]*missed execution/i.test(msg)) {
      warnings.push({
        code: "scheduler_missed_execution",
        source,
        timestamp: entry.timestamp,
        message: msg,
      });
...
function resolveExpectedCommit({ env = process.env, gitHead = null } = {}) {
  ...
  const expectLocal = /^(1|true|yes)$/i.test(
    String(env.RAILWAY_HEALTH_EXPECT_LOCAL_COMMIT || "").trim(),
  );
  if (!expectLocal) return null;
...
function buildRailwayHealthReport({
  ...
  deployments = [],
  deploymentAccessIssue = null,
  health = null,
  queueStats = null,
  queueStatsIssue = null,
  appLogs = [],
  buildLogs = [],
  httpLogs = [],
  expectedCommit = null,
  ...
  const queueSummary = summariseQueueStats(queueStats);
  ...
  if (!latest && deploymentAccessIssue) {
    warnings.push({
      code: "railway_deployment_access_unavailable",
      message: redactSensitive(deploymentAccessIssue),
    });
  } else if (!latest) {
  ...
  if (queueSummary) {
    const failedClassification = classifyFailedQueueJobs(queueSummary, latest);
    const failedJobs = failedClassification.failedJobs;
    const staleClaims = Number(queueSummary.jobs.stale_claims || 0);
    const oldestPending = queueSummary.jobs.oldest_pending_minutes;
    if (failedJobs > 0) {
      if (
        failedClassification.allSampled &&
        failedClassification.afterDeploy.length === 0
      ) {
        advisories.push({
          code: "queue_historical_failed_jobs_present",
          message:
            `Production queue has ${failedJobs} historical failed job(s), ` +
            "but none sampled after the latest successful deployment.",
        });
        green.push("queue_no_active_failed_jobs");
      } else {
        const activeCount = failedClassification.afterDeploy.length;
        warnings.push({
          code: "queue_failed_jobs_present",
          message:
            `Production queue has ${failedJobs} failed job(s)` +
            (activeCount > 0
              ? `, including ${activeCount} sampled after the latest deployment.`
              : "."),
        });
      }
    }
    if (staleClaims > 0) {
      warnings.push({
        code: "queue_stale_claims_present",
        message: `Production queue has ${staleClaims} stale claimed job(s).`,
      });
    }
    if (oldestPending !== null && oldestPending > 120) {
      warnings.push({
        code: "queue_old_pending_jobs",
        message: `Oldest runnable pending job is ${oldestPending} minute(s) old.`,
      });
    }
    if (failedJobs === 0 && staleClaims === 0) {
      green.push("queue_stats_ok");
    }
  } else if (queueStatsIssue) {
    const issue = String(queueStatsIssue);
    const item = {
      code: "queue_stats_not_checked",
      message: issue,
    };
    if (/not available|skipped|HTTP 401|HTTP 403|auth|token/i.test(issue)) {
      advisories.push(item);
    } else if (/HTTP 5\d\d|timeout|aborted|ECONN|failed/i.test(issue)) {
      warnings.push(item);
    } else {
      advisories.push(item);
    }
  ...
  return {
    ...
    deploymentAccess: {
      ok: !deploymentAccessIssue,
      issue: deploymentAccessIssue ? redactSensitive(deploymentAccessIssue) : null,
    },
    health: healthSummary,
    queue: queueSummary || {
      checked: false,
      issue: queueStatsIssue || null,
    },
    hardFails,
    warnings,
    advisories,
...
function renderRailwayHealthMarkdown(report) {
  ...
  if (report.deploymentAccess?.issue) {
    lines.push(`- deploymentAccessIssue: ${redactSensitive(report.deploymentAccess.issue)}`);
  }
  ...
  lines.push("", "## Queue", "");
  if (report.queue?.checked) {
    lines.push(`- checked: true`);
    lines.push(`- jobsTotal: ${report.queue.jobs.total}`);
    lines.push(
      `- jobsByStatus: ${JSON.stringify(report.queue.jobs.by_status || {})}`,
    );
    lines.push(`- staleClaims: ${report.queue.jobs.stale_claims}`);
    lines.push(
      `- oldestPendingMinutes: ${report.queue.jobs.oldest_pending_minutes ?? "none"}`,
    );
    if (report.queue.jobs.recent_failed?.length) {
      lines.push("- recentFailed:");
      for (const job of report.queue.jobs.recent_failed) {
        lines.push(
          `  - #${job.id} ${job.kind}${job.story_id ? ` story=${job.story_id}` : ""}${job.updated_at ? ` updated=${job.updated_at}` : ""}: ${redactSensitive(job.last_error || "failed")}`.slice(
            0,
            500,
          ),
        );
    ...
    lines.push(`- derivativesTotal: ${report.queue.derivatives.total}`);
  } else {
    lines.push(`- checked: false`);
    if (report.queue?.issue) {
      lines.push(`- issue: ${redactSensitive(report.queue.issue)}`);
    }
  }
