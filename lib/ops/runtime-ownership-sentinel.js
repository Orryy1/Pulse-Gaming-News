...
async function fetchRuntimeHealthJson(
  url,
  {
    attempts = 1,
    timeoutMs = 5000,
  ...
  for (let attempt = 1; attempt <= safeAttempts; attempt += 1) {
    lastResult = await fetchJsonImpl(url, { timeoutMs });
    if (lastResult?.ok) {
      return {
        ...lastResult,
        attempts: attempt,
    ...
    previousErrors.push(cleanText(lastResult?.error || lastResult?.status || "unreachable"));
    if (attempt < safeAttempts) await delay(retryDelayMs);
  }
  return {
    ...(lastResult || { ok: false, status: null, json: null, error: "unreachable" }),
    attempts: safeAttempts,
    previous_errors: previousErrors.slice(0, Math.max(0, previousErrors.length - 1)),
  };
}
...
function guardedDispatchActionKeys(plan = {}) {
  const actions = Array.isArray(plan.dispatch_ready_actions) ? plan.dispatch_ready_actions : [];
  return new Set(actions.map(asActionKey).filter(Boolean));
}

function setCoversAll(source = new Set(), target = new Set()) {
...
function inspectSchedulerProof({ schedulerProof = {}, env = {} } = {}) {
  ...
  const guardedDispatchKeys = guardedDispatchActionKeys(guardedDispatchPlan || {});
  const guardedDispatchCoversDryRun =
    guardedDispatchPlan?.ready_for_guarded_dispatch === true &&
    setCoversAll(guardedDispatchKeys, dryRunKeys);
  ...
  if (dryRunKeys.size > 0 && executorKeys.size === 0) {
    if (guardedDispatchCoversDryRun) {
      advisory.push("manual executor handoff has no actions, but current guarded dispatch plan covers enabled dry-run actions for scheduler mode");
    } else {
  ...
  if (
    ...
    if (guardedDispatchCoversDryRun) {
      advisory.push(
        `manual executor handoff stale: dry-run is newer by ${Math.round((dryRunGeneratedMs - executorGeneratedMs) / 60_000)} minutes, but current guarded dispatch plan covers enabled dry-run actions`,
      );
      advisory.push(
        `manual executor handoff missing current enabled actions outside scheduler path: ${missingFromExecutor.slice(0, 8).join(", ")}`,
      );
    } else {
      blockers.push(
  ...
  if (
    ...
    if (guardedDispatchCoversDryRun) {
      advisory.push(
        `guarded dispatch plan is newer than manual executor handoff by ${Math.round((guardedGeneratedMs - executorGeneratedMs) / 60_000)} minutes; scheduler can use the current guarded plan`,
      );
    } else {
      blockers.push(
  ...
  return {
    ...
    facts: {
      ...
      guarded_dispatch_action_count: guardedDispatchKeys.size,
      guarded_dispatch_covers_dry_run: guardedDispatchCoversDryRun,
...
function inspectHealth({
  ...
  return {
    ...
    facts: {
      ...
      primary_runtime_hold: runtime?.primary_runtime_hold ?? null,
      protected_primary_runtime: runtime?.protected_primary_runtime ?? null,
      no_scheduler_mode: runtime?.controlled_restart_no_scheduler_mode ?? null,
    },
  };
}

function protectedPrimaryHealthSurfaceAttestation(inspection) {
  const facts = inspection?.facts || {};
  return (
    inspection?.ok === true &&
    Number(facts.status) >= 200 &&
    Number(facts.status) < 300 &&
    Boolean(facts.commit_sha) &&
    Boolean(facts.branch) &&
    facts.mode === "local" &&
    facts.primary === true &&
    boolFromRuntime(facts.auto_publish) === true &&
    boolFromRuntime(facts.use_job_queue_explicit) === true &&
    boolFromRuntime(facts.guarded_live_dispatch_enabled) === true &&
    boolFromRuntime(facts.emergency_kill_switch_clear) === true &&
    boolFromRuntime(facts.schedulerActive) === true &&
    facts.dispatch_mode === "queue" &&
    facts.dispatch_strict === true &&
    boolFromRuntime(facts.safe_observation_mode) === false &&
    boolFromRuntime(facts.primary_runtime_hold) === false &&
    boolFromRuntime(facts.protected_primary_runtime) === true
  );
}

function matchingProtectedPrimaryHealthAttestation(local, publicRuntime) {
  if (
    !protectedPrimaryHealthSurfaceAttestation(local) ||
    !protectedPrimaryHealthSurfaceAttestation(publicRuntime)
  ) {
    return false;
  }
  const localFacts = local.facts || {};
  const publicFacts = publicRuntime.facts || {};
  return (
    cleanText(localFacts.commit_sha) === cleanText(publicFacts.commit_sha) &&
    cleanText(localFacts.branch) === cleanText(publicFacts.branch)
  );
}

function inspectProcessOwnership({
  processSnapshot,
  env = {},
  healthAttestedProtectedPrimary = false,
} = {}) {
  ...
  const conflictingServerPids = snapshot.server_processes
    .filter((process) => process.pid && process.pid !== snapshot.port_owner_pid)
  ...
  } else if (!snapshot.port_owner.is_node) {
    blockers.push(`port ${snapshot.port} owner PID ${snapshot.port_owner_pid} is not node server.js`);
  } else if (!snapshot.port_owner.is_server_js) {
    const commandLineUnavailable = !String(snapshot.port_owner.command_line || "").trim();
    if (commandLineUnavailable && healthAttestedProtectedPrimary) {
      if (conflictingServerPids.length) {
        blockers.push(
          `port ${snapshot.port} owner PID ${snapshot.port_owner_pid} is opaque; conflicting node server.js process PID ${conflictingServerPids.join(", ")} detected`,
        );
      } else {
        advisory.push(
          `port ${snapshot.port} owner PID ${snapshot.port_owner_pid} node command line is unavailable; matching local/public protected-primary endpoint attestation accepted`,
        );
      }
    } else {
      blockers.push(`port ${snapshot.port} owner PID ${snapshot.port_owner_pid} is not node server.js`);
...
function inspectContentRunway({ queueInspection = null } = {}) {
  const contentRunway = queueInspection?.contentRunway || null;
  const blockers = [];
  const warnings = [];
  if (!contentRunway) {
    return {
      ...
      facts: {
        inspected: false,
        reason: "queue_inspection_unavailable",
      },
    };
  }

  const pendingCount = Number(contentRunway.pending_count || 0);
  const runningCount = Number(contentRunway.running_count || 0);
  const activeWorkerCount = Number(contentRunway.active_fresh_worker_count || 0);
  const occupiedWorkerCount = Number(contentRunway.occupied_fresh_worker_count || 0);
  const saturated = Boolean(contentRunway.saturated);
  const noActiveWorker = Boolean(contentRunway.no_active_worker);

  if (pendingCount > 0 && noActiveWorker) {
    warnings.push(`content runway jobs pending without active worker: ${pendingCount}`);
  } else if (pendingCount > 0 && saturated) {
    warnings.push(`content runway worker capacity saturated: pending=${pendingCount}, active_workers=${activeWorkerCount}`);
  }
  ...
  return {
    ...
    facts: {
      ...
      saturated,
      no_active_worker: noActiveWorker,
    },
  };
}
...
function buildRuntimeOwnershipSentinel({
  ...
  const healthAttestedProtectedPrimary =
    matchingProtectedPrimaryHealthAttestation(local, publicRuntime);
  const processOwnership = inspectProcessOwnership({
    processSnapshot,
    env,
    healthAttestedProtectedPrimary,
  });
  const schedulerProofInspection = inspectSchedulerProof({ schedulerProof, env });
  const contentRunwayInspection = inspectContentRunway({ queueInspection });
  ...
  const blockers = [
    ...
    ...contentRunwayInspection.blockers,
  ];
  const warnings = [
    ...local.warnings,
    ...
    ...contentRunwayInspection.warnings,
  ];
  const advisory = [
    ...asArray(local.advisory),
    ...asArray(publicRuntime.advisory),
    ...asArray(processOwnership.advisory),
    ...asArray(schedulerProofInspection.advisory),
  ];
  ...
  const contentRunwayAction = contentRunwayInspection.facts?.no_active_worker
    ? "restore_content_worker_lane_before_buffer_runs_dry"
    : contentRunwayInspection.facts?.saturated
      ? "let_content_workers_drain_backlog"
      : null;
  ...
  return {
    ...
    content_runway: contentRunwayInspection.facts,
    blockers,
    warnings,
    advisory,
...
async function buildRuntimeOwnershipSentinelFromEnvironment({
  ...
  const retryDelayMs = numberOrNull(env.PULSE_RUNTIME_HEALTH_RETRY_DELAY_MS) ?? 1000;
  const [localHealth, publicHealth] = await Promise.all([
    fetchRuntimeHealthJson(`http://localhost:${port}/api/health`, {
      attempts: numberOrNull(env.PULSE_RUNTIME_LOCAL_HEALTH_ATTEMPTS) ?? 2,
      timeoutMs: numberOrNull(env.PULSE_RUNTIME_LOCAL_HEALTH_TIMEOUT_MS) ?? 5000,
      retryDelayMs,
    }),
    publicUrl
      ? fetchRuntimeHealthJson(`${publicUrl}/api/health`, {
          attempts: numberOrNull(env.PULSE_RUNTIME_PUBLIC_HEALTH_ATTEMPTS) ?? 3,
          timeoutMs: numberOrNull(env.PULSE_RUNTIME_PUBLIC_HEALTH_TIMEOUT_MS) ?? 15000,
          retryDelayMs,
        })
      : Promise.resolve({ ok: false, status: null, error: "public_url_not_configured" }),
  ]);
  ...
  try {
    const { buildQueueReport } = require("./queue-inspect");
    queueInspection = await buildQueueReport();
  } catch (err) {
    queueInspection = {
      verdict: "skip",
      reason: `queue_inspection_failed: ${err?.message || String(err)}`,
    };
  }
