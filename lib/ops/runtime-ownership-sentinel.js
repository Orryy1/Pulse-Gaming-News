...
function exactProtectedPrimaryHealthAttestation(inspection, expectedBuild) {
  const facts = inspection?.facts || {};
  return (
    inspection?.ok === true &&
    asArray(inspection?.blockers).length === 0 &&
    commitsMatch(facts.commit_sha, expectedBuild?.commit_sha) &&
    Boolean(facts.branch) &&
    facts.branch === expectedBuild?.branch &&
    facts.mode === "local" &&
    facts.primary === true &&
...
function inspectProcessOwnership({
  ...
  if (!snapshot.port_owner_pid) {
    ...
    if (commandLineUnavailable && healthAttestedProtectedPrimary) {
      advisory.push(
        `port ${snapshot.port} owner PID ${snapshot.port_owner_pid} SYSTEM-owned node command line is unavailable; exact local/public protected-primary health attestation accepted`,
      );
    } else {
      blockers.push(`port ${snapshot.port} owner PID ${snapshot.port_owner_pid} is not node server.js`);
    }
...
function buildRuntimeOwnershipSentinel({
  ...
  const healthAttestedProtectedPrimary =
    exactProtectedPrimaryHealthAttestation(local, build) &&
    exactProtectedPrimaryHealthAttestation(publicRuntime, build);
