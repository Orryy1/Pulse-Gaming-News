...
const EXPLICIT_STORY_REPROCESS_REASON = "explicit_story_reprocess";
const REPROCESSABLE_FAILURE_PATTERNS = [
  ...
  "tts_duration_too_long",
  "script_runtime_extended_review_required",
  "script_runtime_unknown",
];
const SOURCE_BACKED_PUBLIC_COPY_REPAIR_PATTERNS = [
  "glued_sentence",
  "internal_qa_language",
  "placeholder_title",
  "public_output:placeholder_title",
  "public_output:internal_qa_language",
  "public_output:generic_title",
];
...
const NON_SHORT_REPROCESS_FORMAT_ROUTES = new Set([
...
function isReprocessableFailureReason(reason = "", story = {}) {
  ...
  if (
    SOURCE_BACKED_PUBLIC_COPY_REPAIR_PATTERNS.some((pattern) =>
      text.includes(pattern),
    )
  ) {
    return isSourceBackedRepairCandidate(story);
  }
...
function isAdvertiserSafeRepairCandidate(story = {}) {
  ...
  return (
    !asArray(scored.hard_stops).includes("advertiser_unfriendly_language") &&
    Number(scored.breakdown?.advertiser_safety || 0) > 0
  );
}

function nonShortReprocessRoute(story = {}) {
  const route = String(story.format_route || story.runtime_route || "")
    .trim()
    .toLowerCase();
  return NON_SHORT_REPROCESS_FORMAT_ROUTES.has(route) ? route : "";
}
...
function selectReprocessableScriptFailureStories({
  ...
  const wanted = new Set((storyIds || []).filter(Boolean));
  if (forceStoryIds === true && wanted.size > 0) {
    return (stories || [])
      .filter((story) => story && story.id)
      .filter((story) => wanted.has(story.id))
      .filter((story) => !storyHasPlatformPost(story))
      .filter(isAdvertiserSafeRepairCandidate)
      .slice(0, Math.max(0, Number(limit) || 0))
      .map((story) => ({
        ...story,
        script_failure_reprocess_reason: EXPLICIT_STORY_REPROCESS_REASON,
      }));
  }
...
function classifyReprocessedStory(story = {}) {
  if (story.reprocess_persisted === false) {
    return {
      status: "still_review",
      reason: story.reprocess_persist_skip_reason || "not_script_ready",
    };
  }
  ...
  const nonShortRoute = nonShortReprocessRoute(story);
  if (nonShortRoute) {
    return {
      status: "still_review",
      reason: `format_route_not_short:${nonShortRoute}`,
    };
  }
  return {
    status: "script_ready",
...
function formatScriptFailureReprocessMarkdown(report = {}) {
  ...
  if (!report.excluded || report.excluded.length === 0) {
    ...
    for (const row of report.excluded) {
      lines.push(
        `- ${row.story_id}: ${row.reason} - ${row.title || "untitled"}`,
      );
    }
  }
...
module.exports = {
  EXPLICIT_STORY_REPROCESS_REASON,
  FAILURE_NEEDLE,
  NON_SHORT_REPROCESS_FORMAT_ROUTES,
  REPROCESSABLE_COHERENCE_FAILURE_PATTERNS,
  REPROCESSABLE_FAILURE_PATTERNS,
  SOURCE_BACKED_PUBLIC_COPY_REPAIR_PATTERNS,
