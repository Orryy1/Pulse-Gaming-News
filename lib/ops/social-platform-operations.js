  ...
  const routes = [
    {
      ...
      note:
        "Best near-term route outside Pulse app approval, provided the vendor truly auto-publishes for the connected account.",
    },
    {
...
function buildSocialPlatformOperationsReport({
  ...
  if (tiktok.state === "blocked_external") {
    operatorActions.push(
      "Choose a TikTok route: audited scheduler test first, official inbox upload second, phone dispatch pack as immediate fallback.",
    );
  }
  ...
  const platforms = {
    ...
    facebook_reel: {
      state:
        facebook.state === "enabled" && fbAssessment.verdict !== "blocked"
          ? "review"
          : "blocked_external",
      reason: facebook.reason || fbAssessment.reason,
