...
function statusAfterTokenGate(baseStatus, tiktokTokenStatus = null) {
  if (!tiktokTokenStatus) return baseStatus;
  if (tiktokTokenStatus.ok === true) return baseStatus;
  if (tiktokTokenStatus.needs_reauth === true) return "needs_reauth";
  if (tiktokTokenStatus.refresh_available === true) return "needs_token_refresh_or_sync";
  return "needs_token_recovery";
}

function buildTikTokRoutes({
  tiktokDiagnosis = {},
  dispatchManifest = {},
  tiktokTokenStatus = null,
} = {}) {
  const inboxBaseStatus = tiktokDiagnosis?.evidence?.uploadScopeRequested
  ...
  const routes = [
    {
      ...
      status: statusAfterTokenGate(inboxBaseStatus, tiktokTokenStatus),
      note:
...
function buildSocialPlatformOperationsReport({
  ...
  const tiktokSafeRoutes = buildTikTokRoutes({
    tiktokDiagnosis,
    dispatchManifest,
    tiktokTokenStatus,
  });
  ...
  } else if (tiktokTokenStatus && tiktokTokenStatus.ok !== true && tiktokTokenStatus.refresh_available) {
    operatorActions.push(
      `Refresh or sync the local TikTok token before testing official inbox upload (reason: ${tiktokTokenStatus.reason || "not_ok"}).`,
    );
  } else if (tiktokTokenStatus && tiktokTokenStatus.ok !== true) {
    operatorActions.push(
      `TikTok token is not usable locally and cannot be refreshed automatically from this report (reason: ${tiktokTokenStatus.reason || "not_ok"}).`,
    );
  }
...
function renderSocialPlatformOperationsMarkdown(report) {
  ...
  if (tiktok) {
    ...
    if (tiktok.token) {
      lines.push(
        `- Token: ok=${tiktok.token.ok} reason=${tiktok.token.reason || "unknown"} refresh_available=${tiktok.token.refresh_available} needs_reauth=${tiktok.token.needs_reauth}`,
      );
    }
