...
function segmentSourceFamily(segment = {}) {
  const provider = normaliseSourceFamilyValue(segment.provider || segment.source || segment.provenance?.provider);
  const storeAppId = normaliseSourceFamilyValue(
    segment.store_app_id || segment.storeAppId || segment.provenance?.store_app_id || segment.provenance?.storeAppId,
  );
  ...
  const movieId = normaliseSourceFamilyValue(
    segment.movie_id || segment.movieId || segment.video_id || segment.provenance?.movie_id || segment.provenance?.movieId,
  );
