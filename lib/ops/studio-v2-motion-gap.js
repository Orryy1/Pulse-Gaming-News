...
function sourceFamiliesFromSegmentInventory(segment = {}) {
  const byKey = new Map();
  for (const [entity, inventory] of Object.entries(segment.entity_inventory || {})) {
    for (const family of array(inventory.source_families)) {
      const key = family.key || [family.provider, family.store_app_id, family.movie_id, family.source_url]
        .map((item) => String(item || "").trim())
        .join("|");
      if (!key.trim()) continue;
      const existing = byKey.get(key);
      if (existing) {
        existing.entities = unique([...existing.entities, entity]);
        existing.attempted_segments += Number(family.attempted_segments || 0);
        existing.validated_segments += Number(family.validated_segments || 0);
        existing.rejected_segments += Number(family.rejected_segments || 0);
      } else {
        byKey.set(key, {
          ...family,
...
function validatedSourceFamiliesFromSegmentInventory(segment = {}) {
  return sourceFamiliesFromSegmentInventory(segment)
    .filter((family) => Number(family.validated_segments || 0) > 0)
    .sort((a, b) => {
      const validated = Number(b.validated_segments || 0) - Number(a.validated_segments || 0);
      if (validated) return validated;
      return String(a.reference_title || a.movie_id || a.key).localeCompare(
        String(b.reference_title || b.movie_id || b.key),
      );
    });
}

function sourceCoverageWarnings(storyEntities = [], validatedEntities = [], missingEntities = []) {
  const warnings = [];
  if (missingEntities.length) {
    warnings.push(`missing_validated_motion_entities:${missingEntities.join(",")}`);
  }
  const validatedKeys = new Set(validatedEntities.map(normaliseSourceFamilyValue));
  const missingKeys = new Set(missingEntities.map(normaliseSourceFamilyValue));
  const labelSatisfied = storyEntities.filter((entity) => {
    const key = normaliseSourceFamilyValue(entity);
    return key && !validatedKeys.has(key) && !missingKeys.has(key);
  });
  if (labelSatisfied.length) {
    warnings.push(`multi_entity_coverage_satisfied_by_validated_labels:${labelSatisfied.join(",")}`);
  }
  return warnings;
}
...
function buildMotionBackboneGap({
  threshold,
  clipDominanceThreshold,
  validatedClipRefs,
  validatedClipSources,
  missingValidatedClipRefs,
  missingValidatedClipSources,
  storyEntities,
  validatedEntities,
  missingValidatedEntities,
  validatedSourceFamilies,
  needsFootageBackboneDominance,
  projectedClipDominance,
  requiredClipSecondsForDominance,
  missingClipSecondsForDominance,
} = {}) {
  let status = "motion_backbone_ready";
  if (missingValidatedClipSources > 0) status = "needs_additional_validated_source_family";
  else if (missingValidatedClipRefs > 0) status = "needs_additional_validated_clip_window";
  else if (missingValidatedEntities.length) status = "needs_entity_motion_coverage";
  else if (needsFootageBackboneDominance) status = "needs_more_validated_gameplay_seconds";

  const actionEntities = missingValidatedEntities.length
    ? missingValidatedEntities
    : validatedEntities.length
      ? validatedEntities
      : storyEntities;
  const nextActions = [];
  if (missingValidatedClipSources > 0) {
    nextActions.push(
      `add_${missingValidatedClipSources}_validated_official_source_${plural(
        missingValidatedClipSources,
        "family",
        "families",
      )}_for:${actionEntities.join(",") || "story"}`,
    );
    nextActions.push("prefer_a_new_official_source_family_over_more_windows_from_existing_validated_sources");
  }
  if (missingValidatedClipRefs > 0) {
    nextActions.push(
      `add_${missingValidatedClipRefs}_validated_gameplay_clip_${plural(missingValidatedClipRefs, "window")}_for:${
        actionEntities.join(",") || "story"
      }`,
    );
  }
  if (missingValidatedEntities.length) {
    nextActions.push(`cover_missing_motion_entities:${missingValidatedEntities.join(",")}`);
  }
  if (needsFootageBackboneDominance) {
    nextActions.push("increase_validated_gameplay_seconds_to_meet_flash_lane_dominance");
  }
  if (!nextActions.length) nextActions.push("motion_backbone_source_requirements_clear");

  return {
    status,
    required_validated_clip_refs: threshold,
    current_validated_clip_refs: validatedClipRefs,
    missing_validated_clip_refs: missingValidatedClipRefs,
    required_validated_clip_sources: threshold,
    current_validated_clip_sources: validatedClipSources,
    missing_validated_clip_sources: missingValidatedClipSources,
    required_clip_dominance: clipDominanceThreshold,
    current_projected_clip_dominance: projectedClipDominance ?? null,
    ...
    validated_source_families: validatedSourceFamilies,
...
function buildGap(candidate = {}, context = {}) {
  ...
  const missingValidatedClipRefs = Math.max(0, threshold - validatedClipRefs);
  const missingValidatedClipSources = Math.max(0, threshold - validatedClipSources);
  ...
  const motionContradictsReadyProof =
    ...
    (missingValidatedClipRefs > 0 ||
      missingValidatedClipSources > 0 ||
  ...
  const validatedSourceFamilies = validatedSourceFamiliesFromSegmentInventory(segment);
  const sourceWarnings = sourceCoverageWarnings(storyEntities, validatedEntities, missingValidatedEntities);
  const motionBackboneGap = buildMotionBackboneGap({
    threshold,
    clipDominanceThreshold,
    validatedClipRefs,
    validatedClipSources,
    ...
    validatedSourceFamilies,
    needsFootageBackboneDominance,
    projectedClipDominance: candidate.visuals?.projected_clip_dominance,
    requiredClipSecondsForDominance,
    missingClipSecondsForDominance,
  });
  ...
  const motionGap = {
    ...
    source_coverage_warnings: sourceWarnings,
    motion_backbone_gap: motionBackboneGap,
...
function renderStudioV2MotionGapMarkdown(report = {}) {
  ...
  for (const gap of array(report.gaps)) {
    lines.push(
      ...
      `- Source coverage warnings: ${array(gap.motion_gap.source_coverage_warnings).join(", ") || "none"}`,
      ...
      "### Motion Backbone Gap",
      "",
      `- Status: ${gap.motion_gap.motion_backbone_gap?.status || "unknown"}`,
      `- Validated clip windows: ${gap.motion_gap.motion_backbone_gap?.current_validated_clip_refs ?? "unknown"} / ${
        gap.motion_gap.motion_backbone_gap?.required_validated_clip_refs ?? "unknown"
      }+`,
      `- Validated source families: ${
        gap.motion_gap.motion_backbone_gap?.current_validated_clip_sources ?? "unknown"
      } / ${gap.motion_gap.motion_backbone_gap?.required_validated_clip_sources ?? "unknown"}+`,
    );
    if (Number(gap.motion_gap.motion_backbone_gap?.missing_validated_clip_sources || 0) > 0) {
      const missingSources = Number(gap.motion_gap.motion_backbone_gap.missing_validated_clip_sources);
      lines.push(
        `- Need ${missingSources} more validated official source ${plural(missingSources, "family", "families")} before Flash Lane source diversity is clear.`,
      );
    }
    if (Number(gap.motion_gap.motion_backbone_gap?.missing_validated_clip_refs || 0) > 0) {
      const missingRefs = Number(gap.motion_gap.motion_backbone_gap.missing_validated_clip_refs);
      lines.push(
        `- Need ${missingRefs} more validated gameplay clip ${plural(missingRefs, "window")} before the 3+ clip-window threshold is clear.`,
      );
    }
    const backboneActions = array(gap.motion_gap.motion_backbone_gap?.next_actions);
    if (backboneActions.length) {
      lines.push("- Backbone actions:");
      for (const action of backboneActions) lines.push(`  - ${action}`);
    }
    const sourceWarnings = array(gap.motion_gap.source_coverage_warnings);
    if (sourceWarnings.length) {
      lines.push("- Source coverage warnings:");
      for (const warning of sourceWarnings) lines.push(`  - ${warning}`);
    }
    const validatedFamilies = array(gap.motion_gap.motion_backbone_gap?.validated_source_families);
    if (validatedFamilies.length) {
      lines.push("", "#### Validated Source Families", "");
      lines.push("| Provider | App | Movie/source | Validated windows | Entities |");
      lines.push("| --- | --- | --- | ---: | --- |");
      for (const family of validatedFamilies.slice(0, 12)) {
        const sourceLabel =
        ...
        lines.push(
          ...
          )} | ${mdCell(sourceLabel)} | ${family.validated_segments || 0} | ${mdCell(
            array(family.entities).join(", ") || "unknown",
          )} |`,
        );
      }
    }
    ...
    if (sourceFamilyRows.length) {
      ...
      lines.push("| Entity | Provider | App | Movie/source | Attempts | Validated | Rejected | Top rejection |");
      lines.push("| --- | --- | --- | --- | ---: | ---: | ---: | --- |");
      for (const family of sourceFamilyRows.slice(0, 12)) {
        ...
        lines.push(
          `| ${mdCell(family.entity)} | ${mdCell(family.provider || "unknown")} | ${mdCell(
            family.store_app_title || family.store_app_id || "unknown",
          )} | ${mdCell(sourceLabel)} | ${family.attempted_segments} | ${
            family.validated_segments || 0
          } | ${family.rejected_segments} | ${mdCell(
