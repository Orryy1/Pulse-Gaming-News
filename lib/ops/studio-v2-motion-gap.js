...
function renderStudioV2MotionGapMarkdown(report = {}) {
  ...
  for (const gap of array(report.gaps)) {
    ...
    if (entityRows.length) {
      lines.push("", "| Entity | Status | Attempts | Validated | Top rejection | Recommendation |");
      lines.push("| --- | --- | ---: | ---: | --- | --- |");
      for (const [entity, row] of entityRows) {
        lines.push(
          `| ${entity} | ${row.status} | ${row.attempted_segments} | ${row.validated_segments} | ${row.top_rejection_reason || "none"} | ${row.recommendation} |`,
        );
      }
    }
    lines.push(
