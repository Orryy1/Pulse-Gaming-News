...
function buildSafeCommands(candidate, row) {
  ...
  if (row.motion_gap.acquisition_strategy?.alternate_source_entities?.length) {
    commands.push({
      ...
      command: `npm run media:resolve-trailers -- --story-id ${storyId} --no-latest-report --segment-validation-report test/output/official_trailer_segment_validation_apply_local.json --exhausted-source-family-threshold 5`,
    ...
    commands.push(
      {
        ...
        command: `npm run media:plan-frames -- --story-id ${storyId} --trailer-references test/output/official_trailer_references_v1.json`,
        safety: "report_only",
      },
      {
      ...
      },
      {
        ...
        command: `npm run media:validate-trailer-segments -- --story-id ${storyId} --apply-local --deep-scan --reference-report test/output/official_trailer_references_v1.json --previous-validation-report test/output/official_trailer_segment_validation_apply_local.json --merge-previous --exhausted-source-family-threshold 5 --max-segments 90 --candidate-windows-per-source 6`,
        safety: "apply_local_under_test_output_only",
      },
    );
  }
...
function buildGap(candidate = {}, context = {}) {
  ...
  const blockers = array(candidate.blockers);
