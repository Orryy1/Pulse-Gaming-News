    ...
    commands.push(
      {
        ...
        command: `npm run media:resolve-trailers -- --story ${storyId}`,
        safety: "report_or_local_only",
      },
      {
        purpose: "plan_frame_sampling",
        command: `npm run media:plan-frames -- --story ${storyId}`,
        safety: "report_only",
      },
      {
        purpose: "extract_safe_local_frames",
        command: `npm run media:extract-frames -- --story ${storyId} --apply-local`,
        safety: "apply_local_under_test_output_only",
      },
      {
        purpose: "validate_gameplay_clip_windows",
        command: `npm run media:validate-trailer-segments -- --story ${storyId} --apply-local --deep-scan`,
        safety: "apply_local_under_test_output_only",
      },
    );
  }
...
function prioritySteps(candidate, motionGap, audioGap) {
  const steps = [];
  if (motionGap.needs_more_motion) {
...
function buildGap(candidate = {}, context = {}) {
  ...
  const motionGap = {
    ...
    needs_more_motion: array(candidate.blockers).includes("flash_proof_requires_motion_backbone") ||
