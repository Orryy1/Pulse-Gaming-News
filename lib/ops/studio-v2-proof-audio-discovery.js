  ...
  for (const root of roots) {
    for (const config of discoveryDirs) {
      ...
      for (const entry of entries) {
        ...
        const proofName = entry.replace(/\.(mp3|wav|m4a)$/i, "_proof.json");
        const proofAbs = path.join(absDir, proofName);
        let proofSidecar = null;
        let proofSidecarError = null;
        try {
          proofSidecar = await readJsonIfExists(fs, proofAbs);
        } catch (err) {
          proofSidecarError = String(err?.message || err || "unknown_error").slice(0, 240);
        }
        ...
        const item = {
          ...
          timestamps_path: (await fs.pathExists(timestampsAbs))
            ...
            : null,
          proof_sidecar_path: proofSidecar
            ? normaliseRel(path.join(config.relDir, proofName))
            : null,
          proof_sidecar_error: proofSidecarError,
        ...
        if (proofSidecar) {
          item.text_word_count = proofSidecar.text_word_count ?? proofSidecar.word_count ?? null;
          item.word_count = proofSidecar.word_count ?? proofSidecar.text_word_count ?? null;
          item.wpm = proofSidecar.wpm ?? null;
          item.acoustic = proofSidecar.acoustic || proofSidecar.local_voice_evidence?.acoustic || null;
          item.transcript =
            typeof proofSidecar.transcript === "string"
              ? proofSidecar.transcript
              : proofSidecar.local_voice_evidence?.transcript || "";
          item.local_voice_metadata =
            proofSidecar.local_voice_metadata || proofSidecar.voice_metadata || "stamped";
          item.local_voice_reference =
            proofSidecar.local_voice_reference || proofSidecar.voice_reference || null;
          item.local_voice_evidence = proofSidecar.local_voice_evidence || null;
        }
