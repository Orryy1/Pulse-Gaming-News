...
function buildRuntimeTarget(audio = {}) {
  ...
  const pass = durationInFlashRange(duration);
...
function renderStudioV2ProofCandidatesMarkdown(report = {}) {
  ...
  for (const candidate of array(report.candidates)) {
    ...
    lines.push(
      ...
      `- Runtime target 61-75s: ${readiness.runtime_target?.status || "unknown"} (${readiness.runtime_target?.duration_seconds ?? "unknown"}s)`,
