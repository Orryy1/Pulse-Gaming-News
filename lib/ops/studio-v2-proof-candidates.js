...
function buildRuntimeTarget(audio = {}) {
  ...
  const pass =
    ...
    duration >= LOCAL_PROOF_TARGET_MIN_SECONDS &&
    duration <= LOCAL_PROOF_TARGET_MAX_SECONDS;
  return statusCheck(pass ? "pass" : "fail", {
    duration_seconds: duration,
    target_seconds: [LOCAL_PROOF_TARGET_MIN_SECONDS, LOCAL_PROOF_TARGET_MAX_SECONDS],
...
function renderStudioV2ProofCandidatesMarkdown(report = {}) {
  ...
  for (const candidate of array(report.candidates)) {
    ...
    lines.push(
      ...
      `- Runtime target ${readiness.runtime_target?.target_seconds?.join("-") || "64-70"}s: ${readiness.runtime_target?.status || "unknown"} (${readiness.runtime_target?.duration_seconds ?? "unknown"}s)`,
