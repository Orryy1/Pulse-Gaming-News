  ...
  for (const [storyId, segments] of byStory.entries()) {
    ...
    out.set(storyId, {
      ...
      validated_clip_ready:
        ...
        uniqueSources.length >= FLASH_MIN_VALIDATED_CLIP_REFS,
    });
  }
  return out;
}
...
function classifyCandidate({ story, audio, assets, frames, segments, stillDeckBlock }) {
  ...
  if (!segments.validated_clip_ready) blockers.push("flash_proof_requires_three_validated_clip_refs");
