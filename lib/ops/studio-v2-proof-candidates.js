...
function buildRecommendedCommand(storyId, audio, segments = {}) {
  ...
  if (segments.validated_clip_ready) {
    parts.push(
...
function collectExactSubjectEntityCoverage(assets = {}) {
  const exactGroups = uniqueStrings(assets.exact_subject_groups);
  const storyTargets = uniqueStrings(assets.story_target_entities);
  const exactSet = new Set(exactGroups.map(normaliseLabel));
  ...
  return {
    exact_subject_entity_coverage_count: storyTargets.filter((group) => exactSet.has(normaliseLabel(group))).length,
...
function classifyCandidate({ story, audio, assets, frames, segments, stillDeckBlock, latestRenderProof }) {
  ...
  const exactEntityCoverage = collectExactSubjectEntityCoverage(assetsWithStoryTargets);
