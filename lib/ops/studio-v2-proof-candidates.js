...
function buildRecommendedCommand(storyId, audio) {
...
function collectEntityCoverage(assets = {}, segments = {}) {
  ...
  const coveredGroups = exactGroups.filter((group) => validatedSet.has(normaliseLabel(group)));
  const requiredCoverage =
    exactGroups.length >= FLASH_MIN_VALIDATED_ENTITY_COVERAGE
...
function classifyCandidate({ story, audio, assets, frames, segments, stillDeckBlock }) {
  const blockers = [];
  const warnings = [];
  const entityCoverage = collectEntityCoverage(assets, segments);
  ...
  return {
    ...
    recommended_command: verdict === "ready_flash_proof"
      ? buildRecommendedCommand(story.id, audio)
      : null,
  };
}
