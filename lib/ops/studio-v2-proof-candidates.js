...
function collectAssetStrength(assetReports = []) {
  ...
  for (const [storyId, value] of byStory.entries()) {
    ...
    const exactCoverageLabels = uniqueStrings(exact.flatMap(assetCoverageLabels));
    ...
    byStory.set(storyId, {
      ...
      exact_subject_coverage_labels: exactCoverageLabels,
...
function collectFrameStrength(frameReports = []) {
  ...
  for (const [storyId, frameSets] of byStory.entries()) {
    ...
    out.set(storyId, {
      ...
      frame_coverage_labels: uniqueStrings(uniqueFrames.flatMap(frameCoverageLabels)),
...
function collectSegmentStrength(segmentValidationReports = []) {
  ...
  for (const storyId of storyIds) {
    ...
    out.set(storyId, {
      ...
      validated_clip_coverage_labels: uniqueStrings(uniqueSegments.flatMap(segmentCoverageLabels)),
...
function buildLocalRepairPromotionPacket(candidate = {}) {
  const audio = candidate.audio || {};
  const readiness = candidate.proof_readiness || {};
  const canRender = readiness.final_recommendation === "render_local_proof" && Boolean(candidate.recommended_command);
  return {
    story_id: candidate.story_id || null,
    title: candidate.title || "",
    verdict: candidate.verdict || "unknown",
    final_recommendation: readiness.final_recommendation || "unknown",
    approved_sleepy_liam_audio: {
      path: audio.output_audio_path || null,
      timestamps_path: audio.timestamps_path || null,
      ...
      spoken_outro_present: audio.spoken_outro_present === true,
    },
    rerender: {
      required: canRender,
      command: canRender ? candidate.recommended_command : null,
      blockers: canRender ? [] : array(candidate.blockers),
      warnings: array(candidate.warnings),
    },
    db_changes_needed_later: [
      {
        field: "stories.audio_path",
        value: audio.output_audio_path || null,
        status: "deferred_until_clean_mp4_qa",
      },
      {
        field: "stories.exported_path",
        value: "future test/output/studio-v2-still-deck clean MP4 path",
        status: "deferred_until_clean_mp4_qa",
      },
      {
        field: "platform upload ids and posted markers",
        value: "preserve existing values",
        status: "must_not_clear_without_explicit_approval",
      },
      {
        field: "extended script fields",
        value: /local-script-extension/i.test(String(audio.output_audio_path || ""))
          ? "set full_script, tts_script and word_count from matching extension draft before live use"
          : "not_required_for_media_repair_audio",
        status: "deferred_until_clean_mp4_qa",
      },
    ],
    safety: {
...
function buildLocalRepairPromotionPacketReport({
  candidates = [],
  generatedAt = new Date().toISOString(),
} = {}) {
  const items = array(candidates)
    .filter((candidate) => candidate.audio?.ready === true)
    .map(buildLocalRepairPromotionPacket);
  const readyToRerender = items.filter((item) => item.rerender.required);
  const audioReadyBlocked = items.filter((item) => !item.rerender.required);
  return {
    schema_version: 1,
    generated_at: generatedAt,
    summary: {
      total_audio_ready: items.length,
      ready_to_rerender_count: readyToRerender.length,
      audio_ready_blocked_count: audioReadyBlocked.length,
    },
    items,
    safety:
      "local-only promotion packet; does not render, post, mutate DB rows, touch Railway, mutate tokens or trigger OAuth",
  };
}

function renderLocalRepairPromotionPacketMarkdown(packet = {}) {
  const lines = [
    "# Studio V2 Local Repair Promotion Packet",
    "",
    "This is a local-only packet. It does not render, post or mutate the DB.",
    "",
    "## Summary",
    "",
    `- Audio-ready candidates: ${packet.summary?.total_audio_ready || 0}`,
    `- Ready to rerender locally: ${packet.summary?.ready_to_rerender_count || 0}`,
    `- Audio-ready but still blocked: ${packet.summary?.audio_ready_blocked_count || 0}`,
    "",
    "## Candidates",
    "",
  ];
  if (!array(packet.items).length) {
    lines.push("- No approved Sleepy Liam audio proofs are ready for local repair promotion.", "");
  }
  for (const item of array(packet.items)) {
    lines.push(
      `### ${item.story_id}`,
      "",
      ...
      `- approved Sleepy Liam audio: ${item.approved_sleepy_liam_audio.path || "missing"} (${item.approved_sleepy_liam_audio.duration_seconds ?? "unknown"}s)`,
      `- Rerender command available: ${Boolean(item.rerender.command)}`,
      `- Blockers: ${array(item.rerender.blockers).length ? item.rerender.blockers.join(", ") : "none"}`,
      `- Safety: posts=${item.safety.posts_to_platforms}; db_mutation=${item.safety.mutates_production_db}; render_requested=${item.safety.render_requested}`,
      "- deferred DB changes:",
      ...array(item.db_changes_needed_later).map((change) => `  - ${change.field}: ${change.status}`),
    );
    if (item.rerender.command) {
      lines.push("", "Local rerender command:", "", "```powershell", item.rerender.command, "```");
    }
    lines.push("");
  }
  return lines.join("\n").trimEnd() + "\n";
...
function mediaProgressScore({ assets = {}, frames = {}, segments = {} } = {}) {
  const exactCount = Number(assets.exact_subject_count || 0);
  const acceptedFrames = Number(frames.accepted_frame_count || 0);
  const validatedClips = Number(segments.validated_clip_ref_count || 0);
  const validatedSources = Number(segments.validated_clip_source_count || 0);
  const projectedClipSeconds = Number(segments.projected_clip_seconds || 0);
  const entityCoverage = Number(segments.validated_entity_coverage_count || 0);
  const coverCount = Number(assets.cover_dominated_exact_asset_count || 0);
  const wrongStoryCount = Number(assets.wrong_story_exact_asset_count || 0);
  const unverifiedCount = Number(assets.unverified_store_exact_asset_count || 0);
  const score =
    exactCount * 4 +
    acceptedFrames * 3 +
    validatedClips * 6 +
    validatedSources * 4 +
    Math.min(30, projectedClipSeconds) +
    entityCoverage * 3 -
    coverCount -
    wrongStoryCount * 20 -
    unverifiedCount * 6;
...
function buildRankingSignals({ audio = {}, assets = {}, frames = {}, segments = {} } = {}) {
  return {
    audio_ready: audio?.ready === true,
    media_progress_score: mediaProgressScore({ assets, frames, segments }),
    exact_subject_count: Number(assets.exact_subject_count || 0),
    accepted_frame_count: Number(frames.accepted_frame_count || 0),
    validated_clip_ref_count: Number(segments.validated_clip_ref_count || 0),
    validated_clip_source_count: Number(segments.validated_clip_source_count || 0),
    cover_dominated_exact_asset_count: Number(assets.cover_dominated_exact_asset_count || 0),
    wrong_story_exact_asset_count: Number(assets.wrong_story_exact_asset_count || 0),
  };
}
...
function collectEntityCoverage(assets = {}, segments = {}) {
  ...
  const coverageLabels = uniqueStrings([...validatedEntities, ...array(segments.validated_clip_coverage_labels)]);
  const { labels: validatedLabels, coveredGroups, missingGroups } = coverageForGroups(requiredGroups, coverageLabels);
  ...
  return {
    ...
    validated_clip_coverage_labels: validatedLabels,
...
function collectExactSubjectEntityCoverage(assets = {}, frames = {}, segments = {}) {
  ...
  const coverageLabels = uniqueStrings([
    ...exactGroups,
    ...array(assets.exact_subject_coverage_labels),
    ...motionGroups,
    ...array(frames.frame_coverage_labels),
    ...array(segments.validated_clip_coverage_labels),
  ]);
  const { labels: exactSubjectCoverageLabels, coveredGroups, missingGroups } = coverageForGroups(
    storyTargets,
    coverageLabels,
  );
  ...
  return {
    ...
    exact_subject_coverage_labels: exactSubjectCoverageLabels,
...
function storyTargetEntities(story = {}) {
  return uniqueStrings(
    buildSubjectGraph(story).required_subject_groups.flatMap((entity) => {
      const parts = normaliseText(entity)
        .split(":")
        .map((part) => part.trim())
        .filter(Boolean);
      return parts.length > 1 ? parts : [entity];
    }),
  );
}
...
function classifyCandidate({ story, audio, assets, frames, segments, stillDeckReadiness, latestRenderProof }) {
  ...
  const visuals = {
    ...
    exact_subject_coverage_labels: exactEntityCoverage.exact_subject_coverage_labels,
    ...
    validated_clip_coverage_labels: entityCoverage.validated_clip_coverage_labels,
  ...
  const rankingSignals = buildRankingSignals({ audio, assets, frames, segments });
  const candidate = {
    story_id: story.id || null,
    title: normaliseText(story.title || ""),
    priority: storyPriority(story),
    media_progress_score: rankingSignals.media_progress_score,
...
function buildStudioV2ProofCandidateReport({
  ...
  for (const story of array(stories)) {
    ...
    const baseSegments =
      segmentsByStory.get(storyId) ||
      {
        ...
        validated_clip_coverage_labels: [],
  ...
  candidates.sort((a, b) => {
    ...
    if (lane) return lane;
    const audioReady = Number(b.audio?.ready === true) - Number(a.audio?.ready === true);
    if (audioReady) return audioReady;
    const mediaProgress = Number(b.media_progress_score || 0) - Number(a.media_progress_score || 0);
    if (mediaProgress) return mediaProgress;
  ...
  const localRepairPromotionPacket = buildLocalRepairPromotionPacketReport({
    candidates: limited,
    generatedAt,
  });
  ...
  return {
    ...
    summary,
    candidates: limited,
    local_repair_promotion_packet: localRepairPromotionPacket,
    thresholds: {
...
module.exports = {
  ...
  buildLocalRepairPromotionPacket,
