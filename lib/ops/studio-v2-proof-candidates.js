...
const FLASH_MAX_WRONG_STORY_EXACT_ASSET_SHARE = 0.5;
...
function buildVisualEvidenceGate({ story = {}, assets = {} } = {}) {
  ...
  if (
    ...
    wrongStoryShare > FLASH_MAX_WRONG_STORY_EXACT_ASSET_SHARE
  ) {
    blockers.push("flash_proof_blocks_wrong_story_exact_assets");
  } else if (wrongStoryAssets.length > 0) {
    warnings.push("some_exact_assets_do_not_match_story_targets");
  }
...
function classifyCandidate({ story, audio, assets, frames, segments, stillDeckBlock, latestRenderProof }) {
  ...
  if (stillDeckBlock && !motionBackboneReady) {
...
function buildStudioV2ProofCandidateReport({
  ...
  return {
    ...
    thresholds: {
      ...
      flash_max_wrong_story_exact_asset_share: FLASH_MAX_WRONG_STORY_EXACT_ASSET_SHARE,
