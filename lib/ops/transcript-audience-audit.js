...
function subjectAliases(subject = "") {
  ...
  if (!raw) return [];
  const aliases = [raw];
  const colonPrefix = clean(raw.split(":")[0]);
  if (
    colonPrefix !== raw &&
    colonPrefix.length >= 3 &&
    /[a-z]/i.test(colonPrefix) &&
    !/^(?:breaking|exclusive|report|rumour|update)$/i.test(colonPrefix)
  ) {
    aliases.push(colonPrefix);
  }
