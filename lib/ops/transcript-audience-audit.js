...
function scriptContainsSubject(script = "", subject = "") {
  const subjectKey = normaliseLooseSubject(subject);
  if (!subjectKey) return true;
  const scriptKey = normaliseLooseSubject(script);
  return new RegExp(`(?:^|\\s)${escapeRegExp(subjectKey)}(?:\\s|$)`, "i").test(scriptKey);
}
...
function auditMassAudienceClarity({ script = "", title = "", sourceName = "", canonicalSubject = "" } = {}) {
  ...
  const concreteMatches = text.match(
    /\b(?:gameplay|trailer|demo|release date|date|delay|price|discount|score|review|mission|combat|gunfight|camera|boss|map|mode|co[- ]?op|campaign|deckbuild(?:er|ing)?|cards?|restaurant|monster|kitchen|platform|xbox|playstation|switch|steam|pc|game pass|beta|patch|update|download|gb|fps|hours?|minutes?|\d{2,4})\b/gi,
  ) || [];
  ...
  if (
    titleWords.length >= 2 &&
    !new RegExp(titleWords.map(escapeRegExp).join("\\s+"), "i").test(text)
  ) {
    warnings.push("mass_audience:title_subject_not_obvious");
  }
  if (sourceName && !new RegExp(escapeRegExp(sourceName), "i").test(text)) {
...
async function auditGeneratedTranscripts({
  ...
  const seen = new Set();
  const storyDirs = [batchDir, ...extraBatchDirs, ...discoverProofBatchDirs(root)]
    .flatMap(storyDirsFromBatchDir)
    .filter((dir) => {
