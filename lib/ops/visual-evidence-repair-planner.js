...
function proofCandidateToRepairRow(candidate = {}, index = 0) {
  return {
    story_id: candidate.story_id,
    title: normaliseText(candidate.title),
    ...
    proof_candidate_rank: numberValue(candidate.proof_candidate_rank || candidate.rank || index + 1),
    media_progress_score: numberValue(candidate.media_progress_score),
...
function currentStateRepairRow(row = {}) {
  return {
    ...
    media_progress_score: numberValue(row.media_progress_score),
    ranking_signals: row.ranking_signals || {},
...
function mergeRows(base = {}, extra = {}) {
  return {
    ...
    media_progress_score: Math.max(numberValue(base.media_progress_score), numberValue(extra.media_progress_score)),
    proof_candidate_rank:
      numberValue(base.proof_candidate_rank) && numberValue(extra.proof_candidate_rank)
        ? Math.min(numberValue(base.proof_candidate_rank), numberValue(extra.proof_candidate_rank))
        : numberValue(base.proof_candidate_rank || extra.proof_candidate_rank),
    ranking_signals: { ...(base.ranking_signals || {}), ...(extra.ranking_signals || {}) },
...
function classifyRepair(row = {}) {
  ...
  const exactSubjectCount = numberValue(visuals.exact_subject_count);
  const coverShare =
    visuals.cover_dominated_exact_asset_share === undefined || visuals.cover_dominated_exact_asset_share === null
      ? exactSubjectCount > 0
        ? coverCount / exactSubjectCount
        : 0
      : numberValue(visuals.cover_dominated_exact_asset_share);
  ...
  if (coverCount > 0 && (coverShare >= 0.5 || blockers.includes("flash_proof_blocks_cover_dominated_exact_assets"))) {
    return {
...
function repairRowSort(a = {}, b = {}) {
  const priority = priorityWeight(a.priority) - priorityWeight(b.priority);
  if (priority) return priority;

  const aAudio = Number(a.audio_ready === true);
  const bAudio = Number(b.audio_ready === true);
  if (aAudio !== bAudio) return bAudio - aAudio;

  const mediaProgress = numberValue(b.media_progress_score) - numberValue(a.media_progress_score);
  if (mediaProgress) return mediaProgress;

  const aValidatedMotion = Number(a.validated_motion_ready === true);
  const bValidatedMotion = Number(b.validated_motion_ready === true);
  if (aValidatedMotion !== bValidatedMotion) return bValidatedMotion - aValidatedMotion;

  const exactSubject = numberValue(b.exact_subject_count) - numberValue(a.exact_subject_count);
  if (exactSubject) return exactSubject;

  const aProofRank = numberValue(a.proof_candidate_rank);
  const bProofRank = numberValue(b.proof_candidate_rank);
  if (aProofRank && bProofRank && aProofRank !== bProofRank) return aProofRank - bProofRank;
...
function buildVisualEvidenceRepairPlan({
  ...
  for (const row of array(proofCandidateReport.candidates).map((candidate, index) =>
    proofCandidateToRepairRow(candidate, index),
  )) {
    if (!row.story_id) continue;
    byStory.set(row.story_id, mergeRows(byStory.get(row.story_id), row));
  }
  ...
  const repairRows = rows
    ...
    .map((row) => {
      ...
      return {
        ...
        proof_candidate_rank: numberValue(row.proof_candidate_rank),
  ...
  return {
    ...
    rows: repairRows.sort(repairRowSort).slice(0, Math.max(1, Number(limit) || 20)),
  };
}
...
function primaryOperatorCommand(row = {}) {
  const actionCommand = array(row.ranked_actions)
    .flatMap((item) => array(item.commands))
    .find((item) => item?.command);
  return actionCommand || array(row.commands).find((item) => item?.command) || null;
}
...
function renderVisualEvidenceRepairMarkdown(report = {}) {
  ...
  for (const row of array(report.rows)) {
    lines.push(
      `| ${mdCell(`${row.story_id}: ${row.title}`)} | ${mdCell(row.primary_action_type || row.action)} | ${mdCell(row.repair_class)} | ${
        row.audio_ready ? "ready" : "missing"
      } | ${row.media_progress_score || 0} | ${
      ...
      )} | ${mdCell(primaryOperatorCommand(row)?.command || "none")} |`,
    );
  }
  if (!array(report.rows).length) {
    lines.push("| none | no visual repair candidates | no_visual_repair_needed | missing | 0 | no | 0 | 0 | none | none |");
  }
  ...
  for (const row of array(report.rows)) {
    ...
    lines.push(`Audio ready: ${row.audio_ready ? "yes" : "no"}`);
    lines.push(`Media progress score: ${row.media_progress_score || 0}`);
