 ...
 *
 * STATUS (2026-04-28): EFFECTIVELY DECOMMISSIONED.
 *
 *   Buffer has retired their public developer API for new users.
 *   `https://buffer.com/developers/apps/create` now returns:
 *
 *     "Sorry, Buffer no longer supports the creation of new
 *      developer apps."
 *
 *   The auth docs at /developers/api/authentication 404. New
 *   accounts cannot generate a `BUFFER_ACCESS_TOKEN`, so the
 *   `USE_BUFFER_TIKTOK=true` gate can never trip on a fresh
 *   account. Pulse Gaming was not grandfathered (My Apps shows
 *   "You have not created any apps").
 *
 *   The code below is left intact for two reasons:
 *     1. If a legacy Buffer token surfaces (e.g. operator's older
 *        account), the existing fallback still works as designed.
 *     2. The `not-enabled` short-circuit means leaving the code in
 *        place has zero runtime cost — `isEnabled()` returns false
 *        when env is unset, every entry point bails gracefully.
 *
 *   With both direct TikTok API (audit rejected) and Buffer (API
 *   closed) unavailable, the only remaining TikTok path is the
 *   Playwright-based `upload_tiktok_browser.js` browser fallback,
 *   gated behind TIKTOK_BROWSER_FALLBACK=true. See publisher.js for
 *   the full TikTok routing.
 *
