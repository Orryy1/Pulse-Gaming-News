...
const DEFAULT_FRESH_DISPATCH_MAX_AGE_HOURS = 36;

function inspectFreshDispatchPackAge(
  freshDispatchPack = null,
  generatedAt = new Date().toISOString(),
  maxAgeHours = DEFAULT_FRESH_DISPATCH_MAX_AGE_HOURS,
) {
  if (!freshDispatchPack?.dispatchPack) return null;
  const packGeneratedAt = freshDispatchPack.generatedAt || freshDispatchPack.generated_at || null;
  if (!packGeneratedAt) return null;
  const packTime = Date.parse(packGeneratedAt);
  const reportTime = Date.parse(generatedAt);
  if (!Number.isFinite(packTime) || !Number.isFinite(reportTime)) {
    return {
      reason: "fresh_dispatch_pack_timestamp_invalid",
      generated_at: packGeneratedAt,
      age_hours: null,
      max_age_hours: maxAgeHours,
    };
  }
  const ageHours = (reportTime - packTime) / 3_600_000;
  if (ageHours <= maxAgeHours && ageHours >= -1) return null;
  return {
    reason: ageHours < -1 ? "fresh_dispatch_pack_timestamp_in_future" : "fresh_dispatch_pack_expired",
    generated_at: packGeneratedAt,
    age_hours: ageHours,
    max_age_hours: maxAgeHours,
  };
...
function freshPackSummary(freshDispatchPack = null) {
  ...
  if (!pack) return null;
  const inboxBlockers = Array.isArray(inboxPlan?.blockers) ? inboxPlan.blockers : [];
  const creativeInboxBlockers = inboxBlockers.filter(
    (blocker) => !/^dispatch_pack_tiktok_auth_action_required$/i.test(String(blocker || "")),
  );
  const creativeBlockers = unique([
    ...
    ...(Array.isArray(freshDispatchPack?.creativeReview?.blockers)
      ...
      : []),
    ...creativeInboxBlockers,
  ]);
  return {
    ...pack,
    source: pack.source || "fresh_local_dispatch_pack",
...
function freshTokenOnlyReadyPack(pack = null, freshDispatchPack = null, tokenGate = {}) {
  const inboxPlan = freshDispatchPack?.inboxPlan || null;
  if (!pack || pack.status !== "tiktok_auth_action_required") return false;
  if (tokenGate?.ok === true) return false;
  if (inboxPlan && inboxPlan.dry_run === false) return false;
  if (inboxPlan && inboxPlan.will_upload_to_tiktok === true) return false;
  if (freshDispatchPack?.safety?.public_post_created === true) return false;
  if (pack.creativeGate?.blocks_dispatch === true) return false;
  if (pack.voiceGate?.do_not_reuse_for_tiktok_dispatch === true) return false;
  if (Array.isArray(pack.voiceGate?.blockers) && pack.voiceGate.blockers.length > 0) return false;
  if (pack.eligibility?.captionReady === false) return false;
  if (pack.eligibility?.dispatchLengthReady === false) return false;
  if (pack.eligibility?.hasMp4 === false || pack.eligibility?.hasCover === false) return false;
  const blockers = Array.isArray(inboxPlan?.blockers) ? inboxPlan.blockers : [];
  return blockers.every((blocker) =>
    /^dispatch_pack_tiktok_auth_action_required$/i.test(String(blocker || "")),
  );
}

function freshReadyPack(freshDispatchPack = null, tokenGate = {}) {
  const pack = freshPackSummary(freshDispatchPack);
  const inboxPlan = freshDispatchPack?.inboxPlan || null;
  if (!pack) return null;
  if (pack.status !== "ready_for_operator_review") {
    return freshTokenOnlyReadyPack(pack, freshDispatchPack, tokenGate) ? pack : null;
  }
...
function findReadyPack(dispatchManifest = {}, freshDispatchPack = null, tokenGate = {}) {
  const freshPack = freshReadyPack(freshDispatchPack, tokenGate);
...
function buildTikTokAutomationReport({
  ...
  dispatchManifest = {},
  freshDispatchPack = null,
  freshDispatchMaxAgeHours = DEFAULT_FRESH_DISPATCH_MAX_AGE_HOURS,
} = {}) {
  const ignoredFreshDispatchPack = inspectFreshDispatchPackAge(
    freshDispatchPack,
    generatedAt,
    freshDispatchMaxAgeHours,
  );
  const usableFreshDispatchPack = ignoredFreshDispatchPack ? null : freshDispatchPack;
  const tokenGate = tokenGateFromReports(authDoctorReport, dispatchManifest);
  const readyPack = findReadyPack(dispatchManifest, usableFreshDispatchPack, tokenGate);
  const topPack = firstPack(dispatchManifest, usableFreshDispatchPack);
...
function renderTikTokAutomationMarkdown(report) {
  ...
  if (report.diagnostics.ignoredFreshDispatchPack) {
    const ignored = report.diagnostics.ignoredFreshDispatchPack;
    lines.push(
      `- Ignored fresh dispatch proof: ${ignored.reason}; age_hours=${Number.isFinite(Number(ignored.age_hours)) ? Number(ignored.age_hours).toFixed(2) : "unknown"}; max_age_hours=${ignored.max_age_hours}`,
    );
  }
