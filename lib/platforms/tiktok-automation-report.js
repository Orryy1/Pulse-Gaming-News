...
function buildNoPostReadiness({
  ...
  return {
    ...
    dispatchCreative: {
      ...
      storyId: topPack?.storyId || readyPack?.storyId || null,
      pack_status: topPack?.status || null,
      durationSeconds: topPack?.eligibility?.durationSeconds ?? null,
      blockers: topPack?.creativeBlockers || [],
      operator_visual_review_required:
        topPack?.creativeReviewRequired !== false && Boolean(topPack || readyPack),
    },
    safety: {
      no_post_report: true,
...
function buildTikTokAutomationReport({
  ...
  return {
    ...
    tokenGate,
    dispatchGate: {
      source: usingFreshPack ? "fresh_local_dispatch_pack" : "dispatch_manifest",
      ...
      topPack: topPack
        ? {
            ...
            source: topPack.source || null,
          dryRunOnly: topPack.dryRunOnly ?? null,
          creativeReviewRequired: topPack.creativeReviewRequired ?? null,
          creativeBlockers: topPack.creativeBlockers || [],
        }
        : null,
      ...
      legacyManifestWarnings:
        usingFreshPack && Object.keys(dispatchManifest?.statusCounts || {}).length
