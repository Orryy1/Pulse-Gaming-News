...
function statusWithGates(baseStatus, voiceGate, renderFreshness, tiktokTokenGate) {
...
function buildTikTokDispatchPack(
  ...
  const pack = {
    ...
    urgencyScore: score,
    status: statusWithGates(baseStatus, voiceGate, freshness, tiktokTokenGate),
    voiceGate,
...
function buildTikTokDispatchManifest(
  ...
  function readinessRank(pack) {
    ...
    if (pack.status === "tiktok_auth_action_required") return 4;
    if (pack.status === "voice_review_required") return 5;
