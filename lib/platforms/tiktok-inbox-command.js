...
function publicPostIdsFromStatus(tiktokStatus = {}) {
  return uniqueStrings(
    tiktokStatus.publicly_available_post_id ||
      tiktokStatus.publicaly_available_post_id ||
      tiktokStatus.publiclyAvailablePostIds ||
      [],
  );
}
...
function buildTikTokInboxCommandPlan({
  ...
  const statusValue = String(tiktokStatus?.status || result?.status || "")
    .trim()
    .toUpperCase() || null;
  const publicPostIds = publicPostIdsFromStatus(tiktokStatus || result || {});
  const publicVerified =
    statusValue === "PUBLISH_COMPLETE" && publicPostIds.length > 0;
  const publicProfileUrl = String(args.publicProfileUrl || "")
    .trim()
    .replace(/\/+$/, "");
  const publicUrls = publicProfileUrl
    ? publicPostIds.map(
        (postId) => `${publicProfileUrl}/video/${encodeURIComponent(postId)}`,
      )
    : [];
  const completionState =
    publicVerified
      ? "public_post_verified"
      : statusValue === "PUBLISH_COMPLETE"
        ? "published_not_publicly_verified"
  ...
  const discordSummary = publishId
    ? [
        `${publicVerified ? "TikTok Public Post Verified" : "TikTok Inbox OK"} ${storyId || "unknown story"}`,
        `Status: ${statusValue || "unknown"}`,
        `Publish ID: ${publishId}`,
        publicVerified
          ? `Public post ID: ${publicPostIds[0]}`
          : "Manual action: open TikTok inbox, tap the upload notification, then finish and post through TikTok's editing flow.",
  ...
  return {
    ...
    requires_manual_completion: !publicVerified,
    public_verified: publicVerified,
    public_post_ids: publicPostIds,
    public_urls: publicUrls,
    official_status_documentation:
    ...
    warnings,
    safety: {
      no_public_post: !publicVerified,
      no_browser_automation: true,
      no_oauth_triggered: true,
      no_token_mutation_by_planning: true,
      status_fetch_read_only: statusOnly,
      operator_must_finish_in_tiktok_app: !publicVerified,
    },
    next_operator_step: statusOnly
      ? publicVerified
        ? "No manual completion remains. Preserve the public post ID and URL as publication evidence."
        : statusValue === "PUBLISH_COMPLETE"
          ? "TikTok reports the editing flow was completed but returned no publicly available post ID. Confirm the post visibility is Everyone, then poll again after moderation."
          : statusValue === "SEND_TO_USER_INBOX"
            ? "Open the TikTok mobile app, tap Inbox, open the upload notification, finish TikTok's editing flow, set visibility to Everyone and tap Post."
            : "Poll the same publish ID again. Do not upload another copy."
      : sendInbox
      ? blockers.length
...
function renderTikTokInboxCommandMarkdown(plan) {
  ...
  lines.push(`Public verified: ${plan.public_verified === true}`);
  if (plan.public_post_ids?.length) {
    lines.push(`Public post IDs: ${plan.public_post_ids.join(", ")}`);
  }
  if (plan.public_urls?.length) {
    lines.push(`Public URLs: ${plan.public_urls.join(", ")}`);
  }
  if (plan.upload_error) {
    lines.push("");
    lines.push("## Upload Failure");
    lines.push(`- Reason: ${plan.upload_error.reason || "unknown"}`);
    if (plan.upload_error.http_status) lines.push(`- HTTP status: ${plan.upload_error.http_status}`);
    if (plan.upload_error.raw_error_code) lines.push(`- TikTok error code: ${plan.upload_error.raw_error_code}`);
    if (plan.upload_error.raw_error_message) lines.push(`- TikTok error message: ${plan.upload_error.raw_error_message}`);
    if (plan.upload_error.log_id) lines.push(`- TikTok log ID: ${plan.upload_error.log_id}`);
    if (plan.upload_error.operator_action) lines.push(`- Operator action: ${plan.upload_error.operator_action}`);
  }
  lines.push("");
  lines.push("## Story");
