...
function buildCommentSwoop({ story, fontOpt, startS = 12.0, durationS = 6.0 }) {
  ...
  // Bigger card (820 wide vs 680) + balanced wrapping. The previous
  // box was too narrow at fontsize 24 and tended to leave orphan
  // tokens like "I" alone on a final line. Fix:
  //   - Wider card (820)
  //   - Wrap target ≈ 32 chars/line — comfortable read length
  //   - 2 lines max (was 3); if comment overflows, truncate with …
  //     so the box stays compact and never has a one-word orphan
  //   - Right-pad each line to avoid single-word last-line dangle
  const cardW = 820;
  const lineMax = 32;
  const maxLines = 2;
  ...
  // If we didn't fit everything, mark with ellipsis on the last line.
  if (consumedCount < rawWords.length && lines.length > 0) {
    const last = lines[lines.length - 1].replace(/[.,;:!?]+$/, "");
    lines[lines.length - 1] = `${last}…`;
  }
  // Anti-orphan: if the last line has just 1-2 short words AND the
  // previous line has space, pull the orphan back up. Avoids the
  // "trailing 'I'" look the user flagged.
  if (lines.length === 2) {
    const lastWords = lines[1].split(/\s+/).filter(Boolean);
    const lastLineShort = lastWords.length <= 2 || lines[1].length <= 6;
    const prevHasRoom = (lines[0] + " " + lines[1]).length <= lineMax + 8;
