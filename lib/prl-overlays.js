...
function buildCommentSwoop({ story, fontOpt, startS = 12.0, durationS = 6.0 }) {
  if (!hasVerifiedRedditReaction(story)) return [];
  const top = story?.reddit_comments?.[0]?.body || story?.top_comment;
  const author =
  ...
  // Never truncate. Aggressive lineMax=48 packs ~50 chars/line at
  // fontsize 26, so a 144-char Reddit top-comment lands in 3 lines
  // cleanly. Sanity ceiling at 6 lines just in case a comment is
  // huge — at that point we'd switch strategy anyway. The previous
  // smaller lineMax + maxLines=3 + ellipsis was cutting comments
  // off; this version fits the FULL comment.
  const cardW = 880;
  const lineMax = 48;
  const maxLines = 6;
  const rawWords = String(top).replace(/\s+/g, " ").trim().split(" ");
  const lines = [];
  let cur = "";
  let consumedCount = 0;
  ...
  if (cur && lines.length < maxLines) {
    lines.push(cur);
    consumedCount = rawWords.length;
  }
  // No ellipsis truncation — with lineMax=48 and maxLines=6 the
  // entire comment fits. If a comment is so long that we somehow
  // hit the 6-line ceiling, the renderer will simply show the
  // first 6 lines and the rest is dropped silently. Better than
  // a dangling "…".
  // Anti-orphan: if the LAST line has just 1-2 short words AND
  // the previous line has space, pull the orphan back up. Avoids
  // the "trailing 'I'" look. Applies regardless of total line
  // count — the orphan is on whichever line is currently last.
  while (lines.length >= 2) {
    const last = lines[lines.length - 1];
    const lastWords = last.split(/\s+/).filter(Boolean);
    const lastLineShort = lastWords.length <= 2 || last.length <= 6;
    const prevHasRoom =
      (lines[lines.length - 2] + " " + last).length <= lineMax + 8;
    if (lastLineShort && prevHasRoom) {
      lines[lines.length - 2] = `${lines[lines.length - 2]} ${last}`.trim();
      lines.pop();
    } else {
      break;
    }
  ...
  // Card body: dark bg with amber stripe. Starts below the top-left
  // badge/source stack with a mobile-safe buffer.
  const yBase = 260;
  const headerH = 36;
  const lineH = 38;
  const padTop = 18;
  const padBot = 22;
  const gap = 10;
  const totalH = padTop + headerH + gap + lines.length * lineH + padBot;
  const out = [];
  // Card background
  out.push(
    `drawbox=x=(w-${cardW})/2:y=${yBase}:w=${cardW}:h=${totalH}:color=${COLOURS.CARD_BG}@0.82:t=fill:${enable}`,
  );
  // Amber side stripe
  out.push(
    `drawbox=x=(w-${cardW})/2:y=${yBase}:w=4:h=${totalH}:color=${COLOURS.PRIMARY}@1.0:t=fill:${enable}`,
  );
  // Header line: u/handle in amber
  out.push(
    `drawtext=text='${ffEscape(handle)}':${fontOpt}:fontcolor=${COLOURS.PRIMARY}:fontsize=30:` +
      `${xExpr}:y=${yBase + padTop}:${alpha}:${enable}`,
  );
  // Body lines — start below the header + gap. Slightly bumped
  // fontsize (26) to read better on phone screens.
  const bodyTop = yBase + padTop + headerH + gap;
  for (let i = 0; i < lines.length; i++) {
    out.push(
      `drawtext=text='${ffEscape(lines[i])}':${fontOpt}:fontcolor=white:fontsize=26:` +
        `${xExpr}:y=${bodyTop + i * lineH}:${alpha}:${enable}`,
    );
  }
  return out;
}
