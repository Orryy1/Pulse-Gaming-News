...
function collectPublicMetadataFailures(story = {}, opts = {}) {
  const { extraText = "" } = opts;
  const failures = [];
  ...
  const publicOutput = runPublicOutputCoherenceGate({
    story,
    publicTitle: opts.publicTitle,
    ...
    requireCaptionEvidence: opts.requireCaptionEvidence === true,
  });
  failures.push(...publicOutput.failures);

  const publicCopyManifest =
    publicOutput.manifest ||
    buildStoryManifest(story, {
      publicTitle: opts.publicTitle,
      script: canonicalScript,
    });
  const publicCopyQa = evaluateGoalPublicCopy(publicCopyManifest);
  const fallbackSubject =
    publicCopyManifest.canonical_subject_confidence === "fallback" &&
    String(publicCopyManifest.canonical_subject || "").trim().toLowerCase() === "this story";
  failures.push(
    ...publicCopyQa.failures.filter(
      (failure) => !(fallbackSubject && failure === "public_copy:first_line_too_weak"),
    ),
  );
