...
function ttsScriptForManifest(script = "") {
...
const THUMBNAIL_ENTITY_STOPWORDS = new Set([
  "game",
  "games",
  "gaming",
  "news",
...
function meaningfulThumbnailTokens(value = "") {
  return normaliseKey(value)
    .split(/\s+/)
    .filter((token) => token.length >= 4 && !THUMBNAIL_ENTITY_STOPWORDS.has(token));
}

function thumbnailCarriesStoryEntity({ thumbnailText = "", publicTitle = "", script = "", description = "", subject = "" } = {}) {
  if (!thumbnailText || !publicTitle || !subject) return false;
  if (!textContainsSubject(publicTitle, subject)) return false;
  const rawEvidence = cleanText([publicTitle, script, description].join(" "));
  const priceTokens = cleanText(thumbnailText).match(/[$£€]\s?\d+(?:[.,]\d{1,2})?/g) || [];
  if (
  ...
  if (
    /\bday[- ]one\s+dlc\b/i.test(cleanText(thumbnailText)) &&
    /\bday[- ]one\s+dlc\b/i.test(rawEvidence)
  ) {
    return true;
  }
  const evidence = normaliseKey(rawEvidence);
  const tokens = meaningfulThumbnailTokens(thumbnailText);
...
function verifiedMeasuredAudioAuthorityForStory(story = {}) {
  const audio =
    story.audio_manifest && typeof story.audio_manifest === "object" && !Array.isArray(story.audio_manifest)
      ? story.audio_manifest
      : null;
  const contract =
    audio?.breaking_news_audio_contract &&
    typeof audio.breaking_news_audio_contract === "object" &&
    !Array.isArray(audio.breaking_news_audio_contract)
      ? audio.breaking_news_audio_contract
      : null;
  if (!audio || !contract) return {};

  const duration = Number(audio.audio_duration_seconds);
  const contractDuration = Number(contract.audio_duration_seconds);
  const lane = cleanText(audio.duration_lane);
  const route = cleanText(audio.runtime_route);
  const narrationSha256 = cleanText(audio.narration_audio_sha256).toLowerCase();
  const timestampsSha256 = cleanText(audio.word_timestamps_sha256).toLowerCase();
  const narrationPath = cleanText(
    audio.resolved_narration_audio_path || audio.narration_audio_path,
  );
  const timestampsPath = cleanText(
    audio.resolved_word_timestamps_path || audio.word_timestamps_path,
  );
  const verifiedAt = cleanText(audio.audio_duration_verified_at);
  const contractBlockers = arrayify(contract.blockers).filter(Boolean);
  const hashBound = /^[a-f0-9]{64}$/.test(narrationSha256) && /^[a-f0-9]{64}$/.test(timestampsSha256);
  const contractMatches =
    cleanText(contract.duration_lane) === lane &&
    cleanText(contract.runtime_route) === route &&
    Number.isFinite(contractDuration) &&
    Math.abs(contractDuration - duration) < 0.001;
  const verified =
    lane === "breaking_news" &&
    route === "breaking_news_short" &&
    Number.isFinite(duration) &&
    duration > 0 &&
    audio.audio_duration_verification_status === "pass" &&
    audio.final_audio_authority === true &&
    contract.audio_duration_verification_status === "pass" &&
    contract.final_audio_authority === true &&
    contractBlockers.length === 0 &&
    contractMatches &&
    hashBound &&
    narrationPath &&
    timestampsPath &&
    cleanText(audio.word_timestamp_source) === "local_whisper_word_alignment" &&
  ...
  return {
    ...
    audio_duration_verified_at: verifiedAt,
    final_audio_authority: true,
    narration_audio_path: cleanText(audio.narration_audio_path) || narrationPath,
    resolved_narration_audio_path: narrationPath,
    narration_audio_sha256: narrationSha256,
    narration_audio_size_bytes: Number(audio.narration_audio_size_bytes) || null,
    word_timestamps_path: cleanText(audio.word_timestamps_path) || timestampsPath,
    resolved_word_timestamps_path: timestampsPath,
    word_timestamps_sha256: timestampsSha256,
    word_timestamps_size_bytes: Number(audio.word_timestamps_size_bytes) || null,
    audio_word_timestamp_count: Number(audio.word_timestamp_count) || 0,
...
function buildStoryManifest(story = {}, options = {}) {
  ...
  const ttsScript = ttsScriptForManifest(script);
  ...
  const commercialIntelligence =
    ...
    null;
  const narrationHook =
    ttsScriptForManifest(story.first_spoken_line || story.narration_hook || story.hook) ||
    firstFiveSecondsText(ttsScript);
  ...
  return {
    ...
    ...verifiedMeasuredAudioAuthorityForStory(story),
  };
}
...
function runPublicOutputCoherenceGate(pack = {}, options = {}) {
  ...
  if (
    ...
    !thumbnailCarriesStoryEntity({
      thumbnailText,
      publicTitle,
      script,
      description,
      subject: manifest.canonical_subject,
    })
  ) {
