...
} = require("./viral-script-intelligence");
...
function titlePassesSourceFactLock(value, story = {}) {
  const title = cleanPublicTitle(value);
  if (!title) return false;
  if (!classifyVerifiedFirstPartyAnnouncement(story).qualifies) return true;
  return findUnsupportedUniversalClaims(story, title).length === 0;
}
...
function resolvePublicTitle(story = {}) {
  ...
  for (const candidate of candidates) {
    ...
    seen.add(key);
    if (
      isSafePublicTitle(title) &&
      titleMatchesStorySubject(title, story) &&
