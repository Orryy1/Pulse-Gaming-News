...
function consequenceLanguage(text = "") {
  return /\b(?:problem|risk|catch|warning|changed|broke|broken|revealed|confirmed|finally|pushback|why|before|after|ceiling|impact|cost|deal|launch|lands?|hits?|date|proof|payoff|trust|pressure|fight|bloat|dangerous|delete|wins?|spike|peak|demand|momentum|exposes?|early[- ]access|ea\s+play|ps5|jungle|combat|discovery|readability|map)\b/i.test(text);
}
...
function audiencePullLanguage(text = "") {
  ...
  const hasSpecificity =
    /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|one|only|first|last|free|paid|delay(?:ed)?|leak(?:ed)?|launch|ending|before|after|steam peak|peak|early[- ]access|ea\s+play|ps5|jungle|combat|discovery|readability|map)\b/i.test(value);
  const hasCuriosity =
