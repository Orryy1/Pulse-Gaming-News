...
function weakSourceHousekeepingCopy(text = "") {
  ...
  if (!value) return false;
  const housekeeping =
    /\b(?:confirmed drop|source-backed update|full source list|sources and related links)\b|source\s*:/i.test(value);
...
function platformCopyTooPlain(platformManifest = {}) {
  ...
  return descriptions.some((description) => {
    ...
    if (!audiencePullLanguage(descriptionAttentionBody(description))) return true;
