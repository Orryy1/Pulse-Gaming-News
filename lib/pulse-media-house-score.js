...
function audiencePullLanguage(text = "") {
  ...
  const hasSpecificity =
    /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|one|only|first|last|free|paid|delay(?:ed)?|release calendar|release window|leak(?:ed)?|launch|ending|before|after|steam peak|peak|early[- ]access|ea\s+play|ps5|footage|performance proof|platform comparisons?|default\s+version|jungle|combat|discovery|readability|readable|lab[- ]worthy|balanced|polish|roster|meta|matchups?|team building|screen control|tag fighters?|wishlist|map|custom seas|private seas|private sessions?|private mode|rule controls?|set (?:their own )?rules|jump[- ]?scares?|choices?|atmosphere|horror|force powers?|heroes?|villains?|board game|family night|game night|replayable|licensed board)\b/i.test(value);
  const hasCuriosity =
    /\b(?:why|how|what|secret|hidden|real|catch|risk|problem|fight|trial|pressure|warning|trust|ceiling|broke|turns?|changed|into|test|exposes?|bet|prove|proves|wait|decid(?:e|ing|es)|argument|balance|polish|sharp|sharper)\b/i.test(value);
...
function viewerStakeLanguage(text = "") {
  ...
  const hasAudienceStake =
    /\b(?:players?|fans?|you|your|owners?|buyers?|subscribers?|pc games?|pc players?|xbox|playstation|nintendo|switch|steam|game pass|console|wishlist|demos?|campaign|storage|ssd|launch|reviews?|scores?|price|subscription|free|paid|famil(?:y|ies)|kids?|parents?|game night)\b/i.test(value);
...
function feedDescriptionHasSpecificPayoff(text = "") {
  ...
  return feedConcreteDetail(value) || /\b(?:campaign|remake|storage|wishlist|players remember|wins? the week|worth keeping installed|force powers?|family night|game night|replayable|board game|kids?|parents?|families)\b/i.test(value);
}
