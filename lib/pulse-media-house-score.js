...
function consequenceLanguage(text = "") {
  return /\b(?:problem|risk|catch|warning|changed|broke|broken|revealed|confirmed|finally|pushback|why|before|after|ceiling|impact|cost|deal|launch|date|proof|payoff|trust|pressure|fight|bloat|dangerous|delete|wins?)\b/i.test(text);
}
...
function concreteThumbnailStake(text = "") {
  return /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|problem|risk|catch|warning|pressure|fight|broke|broken|why|before|after|free|paid|delay|leak|deal|price|storage|ssd|launch)\b/i.test(
    cleanText(text),
  );
}
...
function feedTitleHardDetail(text = "") {
  return /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|demo|demos|storage|ssd|pc specs?|free|price|subscription|game pass|review|score|release date|delay(?:ed)?|gameplay|steam|xbox|playstation|ps5|switch)\b/i.test(
    cleanText(text),
  );
}
