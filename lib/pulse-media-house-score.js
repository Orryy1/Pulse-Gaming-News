...
function consequenceLanguage(text = "") {
  return /\b(?:problem|risk|catch|warning|changed|broke|broken|revealed|confirmed|finally|pushback|why|before|after|ceiling|impact|cost|deal|launch|lands?|hits?|date|proof|payoff|trust|pressure|fight|bloat|dangerous|delete|wins?|spike|demand|momentum|ea\s+play|ps5|jungle|combat|discovery|readability|map)\b/i.test(text);
}
...
function audiencePullLanguage(text = "") {
  ...
  const hasSpecificity =
    /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|one|only|first|last|free|paid|delay(?:ed)?|leak(?:ed)?|launch|ending|before|after|ea\s+play|ps5|jungle|combat|discovery|readability|map)\b/i.test(value);
  const hasCuriosity =
    /\b(?:why|how|what|secret|hidden|real|catch|risk|problem|fight|pressure|warning|trust|ceiling|broke|turns?|changed|into|test)\b/i.test(value);
...
function titleLacksCuriosityGap(canonical = {}) {
...
function viewerStakeLanguage(text = "") {
  ...
  const hasAudienceStake =
    /\b(?:players?|fans?|you|your|owners?|buyers?|subscribers?|pc games?|pc players?|xbox|playstation|nintendo|switch|steam|game pass|console|wishlist|demos?|campaign|storage|ssd|launch|reviews?|scores?|price|subscription|free|paid)\b/i.test(value);
  const hasConcreteHook =
    /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|demo|trailer|campaign|storage|ssd|pc specs?|release date|review score|free trial|subscription|boss|map|mode|launch|delay|score)\b/i.test(value) ||
    consequenceLanguage(value);
  const hasPayoffTurn =
    /\b(?:because|so|means|turns?|turned|turning|changes?|changed|changing|wins?|loses?|exposes?|proves?|before launch|after|instead|but|whether|why|asks?|remember|judge|question)\b/i.test(value);
...
function platformTitlesTooPlain(platformManifest = {}, canonical = {}) {
  const titles = platformOutputs(platformManifest)
    .map(platformTitle)
    .filter(Boolean);
...
function firstFrameCopyCandidates(canonical = {}, platformManifest = {}) {
  ...
  return unique([
    ...
    ...outputs.map((output) => output.cover_frame?.headline || output.thumbnail_headline || output.cover_headline),
  ].map(cleanText));
}
...
function concreteThumbnailStake(text = "") {
  return /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|problem|risk|catch|warning|pressure|fight|broke|broken|why|before|after|free|paid|delay|leak|deal|price|storage|ssd|launch|spike|demand|momentum|ea\s+play|ps5|jungle|combat|discovery|readability|map)\b/i.test(
    cleanText(text),
  );
}
...
function weakFirstFrameOrThumbnailCopy(canonical = {}, platformManifest = {}) {
  ...
  return candidates.some((candidate) => {
    ...
    if (words(candidate).length <= 3 && !consequenceLanguage(candidate)) return true;
    return false;
  });
}

function feedConcreteDetail(text = "") {
  return /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|demo|demos|storage|ssd|pc specs?|free|price|subscription|game pass|review|score|launch|release date|delay(?:ed)?|trailer|gameplay|campaign|ending|boss|map|mode|steam|xbox|playstation|ps5|switch)\b/i.test(
    cleanText(text),
  );
}

function feedTitleHardDetail(text = "") {
  return /\b(?:[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|demo|demos|storage|ssd|pc specs?|pc port|free access|free trial|free weekend|price|subscription|game pass|review score|release date|launch date|delay(?:ed)?|gameplay|early access|wait[- ]?list|pre[- ]?order|reservation|hollywood|movie|film)\b/i.test(
    cleanText(text),
  );
}
...
function feedDescriptionHasSpecificPayoff(text = "") {
  ...
  return feedConcreteDetail(value) || /\b(?:campaign|remake|storage|wishlist|players remember|wins? the week|worth keeping installed)\b/i.test(value);
}

function feedCompetitionScore({ canonical = {}, platformManifest = {} } = {}) {
  const title = titleText(canonical);
  const subject = subjectText(canonical);
  const platformTitleList = platformOutputs(platformManifest).map(platformTitle).filter(Boolean);
  ...
  if (coverSet.length) {
    const weakCovers = coverSet.filter((cover) => feedCoverTooAbstract(cover) || weakFirstFrameOrThumbnailCopy(canonical, { outputs: { probe: { cover_frame: { headline: cover } } } }));
...
function titleStrengthScore(canonical = {}, uniqueness = {}) {
  ...
  if (titleLacksCuriosityGap(canonical)) score -= 26;
...
function footageEmpireVerdict(footageEmpireV2 = {}) {
  return cleanText(footageEmpireV2.verdict || footageEmpireV2.status || footageEmpireV2.result);
}

function footageEmpireBlockers(footageEmpireV2 = {}) {
...
function sourceLockScore({ canonical = {}, visualQuality = {}, benchmark = {}, footageEmpireV2 = {} } = {}) {
  let score = average([
    scoreFrom(visualQuality.scores?.source_lock_quality_score, 65),
    scoreFrom(visualQuality.scores?.rights_risk_score, 70),
  ...
  if (hasObject(footageEmpireV2)) {
    const motion = footageEmpireV2.motion || {};
    const trustedSources = footageEmpireV2.trusted_sources || {};
    const rights = footageEmpireV2.rights_coverage || {};
    const verdict = footageEmpireVerdict(footageEmpireV2);
    const blockers = footageEmpireBlockers(footageEmpireV2);
    const motionClips = Number(motion.available_motion_clips || motion.available_official_product_motion_clips || 0);
    const motionFamilies = Number(motion.available_distinct_families || motion.available_official_product_motion_families || 0);
    const trustedRefs = Number(trustedSources.references_found || trustedSources.story_candidate_count || 0);
    ...
    if (verdictFails(verdict)) score = Math.min(score, 45);
  }
...
function buildScores(input = {}) {
  ...
  if (titleLacksCuriosityGap(canonical) || platformTitlesTooPlain(platformManifest, canonical)) {
    scores.title_strength_score = Math.min(scores.title_strength_score, 64);
  }
  if (weakFirstFrameOrThumbnailCopy(canonical, platformManifest)) {
...
function hardFailures(input = {}, scores = {}) {
  ...
  if (titleLacksCuriosityGap(canonical)) failures.push("media_house:title_lacks_curiosity_gap");
  if (subject && title && !hasSubject(title, subject)) failures.push("media_house:source_title_mismatch");
  if (platformTitlesTooPlain(input.platformManifest || {}, canonical)) failures.push("media_house:platform_title_too_plain");
  if (platformCopyTooPlain(input.platformManifest || {})) failures.push("media_house:platform_copy_too_plain");
  if (weakFirstFrameOrThumbnailCopy(canonical, input.platformManifest || {})) failures.push("media_house:first_frame_or_thumbnail_not_attention_led");
  ...
  if (scores.source_lock_score < THRESHOLDS.source_lock_score || footageEmpireBlockers(input.footageEmpireV2 || input.footageEmpire || {}).length) {
...
function buildSourceLockReport(input = {}, score = {}) {
  ...
  const blocked = scoreValue < THRESHOLDS.source_lock_score || blockers.length > 0 || verdictFails(verdict);
  return {
    schema_version: 1,
    goal: GOAL_ID,
...
function buildShortsAttentionReport(input = {}, score = {}) {
  ...
  const platformManifest = input.platformManifest || {};
  const title = titleText(canonical);
  const platformTitles = platformOutputs(platformManifest).map(platformTitle).filter(Boolean);
  const firstFrames = firstFrameCopyCandidates(canonical, platformManifest);
  const descriptions = platformOutputs(platformManifest).map(platformDescription).filter(Boolean);
  const blockers = [];
  if (genericTitle(title)) blockers.push("generic_title");
  if (titleLacksCuriosityGap(canonical)) blockers.push("title_lacks_curiosity_gap");
  ...
  return {
    ...
    title_attention_led: title ? audiencePullLanguage(title) && !titleLacksCuriosityGap(canonical) : false,
    platform_titles_attention_led:
      platformTitles.length > 0 && !platformTitlesTooPlain(platformManifest, canonical),
    first_frame_attention_led:
      firstFrames.length > 0 && !weakFirstFrameOrThumbnailCopy(canonical, platformManifest),
    scores: {
