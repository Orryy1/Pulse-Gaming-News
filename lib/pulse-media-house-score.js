...
function consequenceLanguage(text = "") {
  return /\b(?:problem|risk|catch|warning|changed|broke|broken|revealed|confirmed|finally|pushback|why|before|after|ceiling|impact|cost|deal|launch|lands?|hits?|date|proof|payoff|trust|pressure|fight|split|splits|splitting|bloat|dangerous|delete|wins?|test|reinstall|lapsed|returning|progression|prestige|weapon prestige|zombies?|endgame|spike|peak|demand|momentum|exposes?|early[- ]access|ea\s+play|ps5|jungle|combat|discovery|readability|map|jump[- ]?scares?|choices?|atmosphere|horror)\b/i.test(text);
}
