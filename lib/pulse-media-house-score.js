...
function consequenceLanguage(text = "") {
  return /\b(?:problem|risk|catch|warning|changed|broke|broken|revealed|confirmed|finally|pushback|why|before|after|ceiling|impact|cost|deal|launch|lands?|hits?|date|proof|payoff|trust|pressure|fight|split|splits|splitting|bloat|dangerous|delete|wins?|spike|peak|demand|momentum|exposes?|early[- ]access|ea\s+play|ps5|jungle|combat|discovery|readability|map|jump[- ]?scares?|choices?|atmosphere|horror)\b/i.test(text);
}
...
function audiencePullLanguage(text = "") {
  ...
  const hasSpecificity =
    /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|one|only|first|last|free|paid|delay(?:ed)?|leak(?:ed)?|launch|ending|before|after|steam peak|peak|early[- ]access|ea\s+play|ps5|jungle|combat|discovery|readability|map|custom seas|private seas|private sessions?|private mode|rule controls?|set (?:their own )?rules|jump[- ]?scares?|choices?|atmosphere|horror)\b/i.test(value);
  const hasCuriosity =
...
function viewerStakeLanguage(text = "") {
  ...
  const hasConcreteHook =
    /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|demo|trailer|campaign|storage|ssd|pc specs?|release date|review score|free trial|subscription|boss|map|mode|custom seas|private seas|private sessions?|private mode|rule controls?|set (?:their own )?rules|launch|delay|score|jump[- ]?scares?|choices?|atmosphere|horror)\b/i.test(value) ||
...
function concreteThumbnailStake(text = "") {
  return /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|problem|risk|catch|warning|pressure|fight|broke|broken|why|before|after|free|paid|delay|leak|deal|price|storage|ssd|launch|spike|demand|momentum|ea\s+play|ps5|jungle|combat|discovery|readability|map|custom seas|private seas|private sessions?|private mode|rule controls?|diddy\s+kong|mascot\s+kart|handling|wishlist|nostalgia|roguelite|podracing|repeat[- ]run|wipeout)\b/i.test(
    cleanText(text),
  );
}
...
function feedConcreteDetail(text = "") {
  return /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|demo|demos|storage|ssd|pc specs?|free|price|subscription|game pass|review|score|launch|release date|delay(?:ed)?|trailer|gameplay|campaign|ending|boss|map|mode|custom seas|private seas|private sessions?|private mode|rule controls?|steam|xbox|playstation|ps5|switch|diddy\s+kong|mascot\s+kart|handling|wishlist|nostalgia|choices?|empathy|atmosphere|horror|jump[- ]?scares?|roguelite|podracing|repeat[- ]run|wipeout)\b/i.test(
    cleanText(text),
  );
}

function feedTitleHardDetail(text = "") {
  return /\b(?:[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|demo|demos|storage|ssd|pc specs?|pc port|free access|free trial|free weekend|price|subscription|game pass|review score|release date|launch date|delay(?:ed)?|gameplay|early access|wait[- ]?list|pre[- ]?order|reservation|hollywood|movie|film|custom seas|private seas|private sessions?|private mode|rule controls?|diddy\s+kong|mascot\s+kart|handling|wishlist|nostalgia|roguelite|podracing|repeat[- ]run|wipeout)\b/i.test(
    cleanText(text),
  );
}
