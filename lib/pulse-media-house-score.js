...
function consequenceLanguage(text = "") {
  return /\b(?:problem|risk|catch|warning|changes?|changed|turns?|makes?|threat|ends?|ended|best|worst|worse|guarantee(?:d|s)?|permanent(?:ly)?|broke|broken|revealed|confirmed|finally|pushback|why|before|after|ceiling|impact|costs?|deal|launch|lands?|hits?|date|proof|payoff|trust|pressure|fight|trial|argument|balance|balanced|polish|half[- ]finished|split|splits|splitting|bloat|dangerous|delete|wins?|test|reinstall|lapsed|returning|progression|prestige|weapon prestige|zombies?|endgame|spike|peak|demand|momentum|exposes?|early[- ]access|comeback|second wave|ea\s+play|game pass|ps5|default\s+version|platform comparisons?|performance proof|jungle|combat|discovery|readability|readable|lab[- ]worthy|roster|meta|matchups?|team building|screen control|wishlist|map|jump[- ]?scares?|choices?|atmosphere|horror|ruins?|replayable|chaos|game night|family night|force powers?)\b/i.test(text);
}

function vividConcreteHeadline(text = "") {
  const value = cleanText(text);
  return (
    /\bturns?\b.{1,50}\binto\b/i.test(value) ||
    /\bmakes?\b.{1,50}\b(?:threat|enemy|weapon|problem|risk)\b/i.test(value) ||
    /\b(?:best|worst)\b/i.test(value) ||
    /\b(?:better|worse|more|less)\s+than\b/i.test(value)
  );
}
...
function audiencePullLanguage(text = "") {
  ...
  const hasSpecificity =
    /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|one|only|first|last|best|worst|your|guarantee(?:d|s)?|free|paid|delay(?:ed)?|release calendar|release window|leak(?:ed)?|launch|ending|before|after|steam peak|peak|early[- ]access|comeback|second wave|game pass|subscription|ea\s+play|ps5|footage|performance proof|platform comparisons?|default\s+version|jungle|combat|discovery|readability|readable|lab[- ]worthy|balanced|polish|roster|meta|matchups?|team building|screen control|tag fighters?|wishlist|map|custom seas|private seas|private sessions?|private mode|rule controls?|set (?:their own )?rules|jump[- ]?scares?|choices?|atmosphere|horror|force powers?|heroes?|villains?|board game|family night|game night|replayable|licensed board)\b/i.test(value) ||
    pulseHardDetail(value) ||
    vividConcreteHeadline(value);
  const hasCuriosity =
    /\b(?:why|how|what|secret|hidden|hides?|real|catch|risk|problem|fight|trial|pressure|warning|trust|threat|guarantee(?:d|s)?|ceiling|broke|turns?|makes?|ends?|ended|best|worst|worse|changes?|changed|into|test|exposes?|bet|prove|proves|wait|decid(?:e|ing|es)|argument|balance|polish|sharp|sharper|comeback|second wave)\b/i.test(value);
...
function platformDescription(output = {}) {
  return cleanText(platformDescriptionRaw(output));
}

function platformDescriptionRaw(output = {}) {
  return String(
    output.description || output.caption || output.metadata?.description || output.metadata?.caption || "",
  ).trim();
}
...
function viewerStakeLanguage(text = "") {
  ...
  const hasPayoffTurn =
    /\b(?:because|so|means|turns?|turned|turning|changes?|changed|changing|removes?|guarantee(?:d|s)?|determines?|wins?|loses?|exposes?|proves?|before launch|after|instead|but|whether|why|asks?|remember|judge|question|reason|wait|decid(?:e|ing|es)|argument)\b/i.test(value);
...
function platformCopyTooPlain(platformManifest = {}) {
  const descriptions = platformOutputs(platformManifest)
    .map(platformDescriptionRaw)
    .filter(Boolean);
  if (!descriptions.length) return false;
  return descriptions.some((description) => {
    const publicBody = splitMicrosoftGameContentNotice(editorialMetadataBody(description)).editorialText;
    if (articleBoilerplateCopy(publicBody)) return true;
    if (words(publicBody).length > 55) return true;
    if (weakSourceHousekeepingCopy(publicBody)) return true;
    const attentionBody = descriptionAttentionBody(publicBody);
    if (!audiencePullLanguage(attentionBody)) return true;
    if (!viewerStakeLanguage(attentionBody)) return true;
    if (words(publicBody).length < 9 && !consequenceLanguage(publicBody)) return true;
    return false;
  });
}

function platformCopyStoryAngleMismatch(canonical = {}, platformManifest = {}) {
  const storyText = cleanText([
    titleText(canonical),
    firstLine(canonical),
  ...
  const storyCarriesDemoAngle = /\b(?:demo|wishlist(?:s|ed|ing)?)\b/i.test(storyText);
  return platformOutputs(platformManifest)
    ...
    .some((description) => {
      ...
      return staleDemoTemplate && !storyCarriesDemoAngle;
    });
}
...
function directMotionRepeatReport(input = {}) {
  ...
  const blockers = [];
  if (!clips.length) {
    blockers.push("premium_output:direct_motion_not_verified");
  }
...
function renderManifestReport(input = {}) {
  ...
  const outputPath = cleanText(manifest.output_path || manifest.output || manifest.path || input.canonical?.final_render_path);
  const blockers = [];
  if (finalFlag !== true || !outputPath || !Number.isFinite(bytes) || bytes <= 0) {
    blockers.push("premium_output:final_publish_render_not_proven");
  }
  return {
    status: blockers.length ? "blocked" : "pass",
    blockers,
...
function captionDisplayReport(input = {}) {
  ...
  const blockers = [];
  if (!text) blockers.push("premium_output:caption_display_not_verified");
...
function premiumOutputHardFailures(input = {}) {
  ...
  if (contract.blockers.includes("premium_output:direct_motion_not_verified")) {
    failures.push("media_house:direct_motion_not_verified");
  }
  ...
  if (contract.blockers.includes("premium_output:caption_display_not_verified")) {
    failures.push("media_house:caption_display_not_verified");
  }
  return failures;
}

function isStaleWindowFamilyBlocker(blocker) {
  return [
...
const NORMAL_SOURCE_DIVERSITY_TIER = "normal_strict_green";
const PROFESSIONAL_SOURCE_DIVERSITY_TIER = "ultimate_professional";

function professionalSourceDiversityEvidence(input = {}, footageEmpireV2 = {}) {
  return input.professional_source_diversity ||
    input.professionalSourceDiversity ||
    footageEmpireV2.professional_source_diversity ||
    footageEmpireV2.professionalSourceDiversity ||
    {};
}

function sourceDiversityTier(input = {}, evidence = {}, footageEmpireV2 = {}) {
  const values = [
    input.source_diversity_tier,
    input.sourceDiversityTier,
    input.quality_tier,
    input.qualityTier,
    evidence.policy_tier,
    evidence.policyTier,
    footageEmpireV2.source_diversity_tier,
    footageEmpireV2.sourceDiversityTier,
  ].map((value) => cleanText(value).toLowerCase()).filter(Boolean);
  return values.some((value) => ["ultimate_professional", "professional", "flagship"].includes(value))
    ? PROFESSIONAL_SOURCE_DIVERSITY_TIER
    : NORMAL_SOURCE_DIVERSITY_TIER;
}

function professionalSourceRows(evidence = {}) {
  const rows = [
    ...asArray(evidence.base_sources),
    ...asArray(evidence.genuine_base_sources),
    ...asArray(evidence.source_records),
    ...asArray(evidence.identity_evidence),
  ];
  if (rows.length) return rows;
  if (evidence.identity_evidence && typeof evidence.identity_evidence === "object") {
    return Object.entries(evidence.identity_evidence).map(([baseSourceAssetId, row]) => ({
...
function professionalSourceRowAssetId(row = {}) {
  return cleanText(
    row.base_source_asset_id || row.baseSourceAssetId || row.asset_id || row.id,
  );
}

function professionalSourceRowDurableIdentity(row = {}) {
  const directIdentity = cleanText(
    row.sampled_visual_fingerprint ||
      row.sampledVisualFingerprint ||
      row.master_sha256 ||
      row.masterSha256,
  ).toLowerCase();
  if (directIdentity) return directIdentity;

  const assetId = professionalSourceRowAssetId(row).toLowerCase();
  const identityBasis = cleanText(
    row.base_source_identity_basis || row.baseSourceIdentityBasis || row.identity_basis || row.identityBasis,
  ).toLowerCase();
  const assetHash = /^sha256:([a-f0-9]{64})$/i.exec(assetId);
  if (assetHash && identityBasis === "master_sha256") {
    const matchingEvidence = asArray(row.identity_evidence).some((evidence) => {
      const kind = cleanText(evidence.kind || evidence.type).toLowerCase();
      const alias = cleanText(evidence.alias || evidence.value).toLowerCase();
      return kind === "master_sha256" && alias === assetId;
    });
    return matchingEvidence ? assetHash[1].toLowerCase() : "";
  }

  const sampledFingerprint = /^fingerprint:([a-f0-9]{16}(?::[a-f0-9]{16})+)$/i.exec(assetId);
  if (!sampledFingerprint || identityBasis !== "sampled_visual_fingerprint") return "";
  const matchingFingerprintEvidence = asArray(row.identity_evidence).some((evidence) => {
    const kind = cleanText(evidence.kind || evidence.type).toLowerCase();
    const alias = cleanText(evidence.alias || evidence.value).toLowerCase();
    return kind === "sampled_visual_fingerprint" && alias === assetId;
  });
  return matchingFingerprintEvidence ? sampledFingerprint[1].toLowerCase() : "";
}

function professionalSourceRowHasStrictIdentity(row = {}) {
  if (!row || typeof row !== "object" || Array.isArray(row)) return false;
  const assetId = professionalSourceRowAssetId(row);
  const identityBasis = cleanText(
    row.base_source_identity_basis || row.baseSourceIdentityBasis || row.identity_basis || row.identityBasis,
  ).toLowerCase();
  const durableIdentity = professionalSourceRowDurableIdentity(row);
  return Boolean(
    assetId &&
      identityBasis &&
      !/(?:source_family|motion_family|derivative_path|window_name|path_only)/.test(identityBasis) &&
      durableIdentity,
  );
}

function buildProfessionalSourceDiversityReport(input = {}, footageEmpireV2 = {}) {
  const evidence = professionalSourceDiversityEvidence(input, footageEmpireV2);
  const policyTier = sourceDiversityTier(input, evidence, footageEmpireV2);
  const strict = policyTier === PROFESSIONAL_SOURCE_DIVERSITY_TIER;
  if (!strict) {
    const evidencePresent = hasObject(evidence);
    const evidenceBlockers = asArray(evidence.blockers).map(cleanText).filter(Boolean);
    const evidenceStatus = cleanText(evidence.status || evidence.verdict || evidence.result).toLowerCase();
    if (
      evidencePresent &&
      (evidenceBlockers.length || ["red", "fail", "failed", "blocked", "amber", "warn", "warning"].includes(evidenceStatus))
    ) {
      return {
        schema_version: 1,
        policy_tier: NORMAL_SOURCE_DIVERSITY_TIER,
        status: "blocked",
        authoritative_evidence_present: evidence.authoritative === true,
        blockers: unique([
          ...evidenceBlockers,
          ...(!evidenceBlockers.length ? ["professional_source_diversity_not_green"] : []),
        ]),
      };
    }
    return {
      schema_version: 1,
      policy_tier: NORMAL_SOURCE_DIVERSITY_TIER,
      status: evidencePresent && ["pass", "green", "ready"].includes(evidenceStatus)
        ? "pass"
        : "not_required",
      authoritative_evidence_present: evidence.authoritative === true,
      blockers: [],
  ...
  if (!hasObject(evidence)) {
    return {
      schema_version: 1,
      policy_tier: PROFESSIONAL_SOURCE_DIVERSITY_TIER,
      status: "blocked",
      authoritative_evidence_present: false,
      blockers: ["professional_source_diversity_evidence_missing"],
    };
  }

  const blockers = asArray(evidence.blockers).map(cleanText).filter(Boolean);
  const rows = professionalSourceRows(evidence);
  const unresolvedClips = asArray(evidence.unresolved_clips || evidence.unresolvedClips);
  const ambiguities = asArray(
    evidence.ambiguous_base_sources || evidence.ambiguousBaseSources || evidence.identity_ambiguities,
  );
  const required = Number(
    evidence.required_genuine_base_source_count ||
      evidence.required_genuine_base_sources ||
      evidence.minimum_required_genuine_base_sources ||
      2,
  );
  const observed = Number(
    evidence.observed_genuine_base_source_count ||
      evidence.distinct_genuine_base_source_count ||
      evidence.observed_genuine_base_sources ||
      rows.length,
  );
  const evidenceStatus = cleanText(evidence.status || evidence.verdict || evidence.result).toLowerCase();
  const completeRows = rows.filter(professionalSourceRowHasStrictIdentity);
  const distinctAssetIds = new Set(completeRows.map(professionalSourceRowAssetId));
  const distinctDurableIdentities = new Set(completeRows.map(professionalSourceRowDurableIdentity));
  if (evidence.authoritative !== true) blockers.push("professional_source_diversity_not_authoritative");
  if (!["pass", "green", "ready"].includes(evidenceStatus)) {
    blockers.push("professional_source_diversity_not_green");
  }
  if (!Number.isFinite(required) || required < 2) {
    blockers.push("professional_source_diversity_requirement_ambiguous");
  }
  if (!Number.isFinite(observed) || observed < required) {
    blockers.push("genuine_base_source_minimum_not_met");
  }
  if (unresolvedClips.length) blockers.push("professional_source_diversity_unresolved_clips");
  if (ambiguities.length) blockers.push("professional_source_diversity_identity_ambiguous");
  if (rows.length < observed || rows.some((row) => !professionalSourceRowHasStrictIdentity(row))) {
    blockers.push("professional_source_diversity_provenance_incomplete");
  }
  if (distinctAssetIds.size < required || distinctDurableIdentities.size < required) {
    blockers.push("professional_source_diversity_duplicate_base_identities");
  }
  ...
  return {
    ...
    authoritative_evidence_present: evidence.authoritative === true,
    required_genuine_base_source_count: Number.isFinite(required) ? required : null,
    observed_genuine_base_source_count: Number.isFinite(observed) ? observed : null,
    unresolved_clip_count: unresolvedClips.length,
    ambiguous_identity_count: ambiguities.length,
    complete_identity_record_count: completeRows.length,
    distinct_verified_base_identity_count: distinctDurableIdentities.size,
...
function footageEmpireBlockers(footageEmpireV2 = {}, input = {}) {
  ...
  const professionalDiversity = buildProfessionalSourceDiversityReport(input, footageEmpireV2);
  let blockers = [
    ...
    ...asArray(professionalDiversity.blockers),
  ];
  if (distinctProof.ready) {
    blockers = blockers.filter((blocker) => !isStaleWindowFamilyBlocker(blocker));
  }
...
function hardFailures(input = {}, scores = {}) {
  ...
  if (platformCopyStoryAngleMismatch(canonical, platformManifest)) {
    failures.push("media_house:platform_copy_story_angle_mismatch");
  }
  ...
  if (
    asArray(visualProfile.blockers).includes(
      "visual_evidence:selected_render_subject_matched_editorial_media_missing",
    )
  ) {
    failures.push("media_house:subject_matched_editorial_media_missing");
  }
  ...
  if (buildProfessionalSourceDiversityReport(
    input,
    input.footageEmpireV2 || input.footageEmpire || {},
  ).status === "blocked") {
    failures.push("media_house:professional_source_diversity_not_verified");
  }
...
function buildShortsAttentionReport(input = {}, score = {}) {
  ...
  if (platformCopyStoryAngleMismatch(canonical, platformManifest)) {
    blockers.push("platform_copy_story_angle_mismatch");
  }
  if (weakFirstFrameOrThumbnailCopy(displayCanonical, platformManifest)) {
...
function withAuthoritativeFinalVisualEvidence(input = {}) {
  ...
  const materialisedMotion =
    input.materialisedMotionClips ||
    input.materializedMotionClips ||
    input.materialised_motion_clips ||
    input.materialized_motion_clips ||
    {};
  const footageEmpire =
    input.footageEmpireV2 ||
    input.footageEmpire ||
    {};
  const supportingClips = [
    ...
    ...asArray(footageEmpire.motion_inventory?.accepted_local_clips),
    ...asArray(footageEmpire.motion_inventory?.production_motion_clips),
  ];
  const profile = visualEvidenceProfile({
    story: {
      ...(input.canonical || {}),
      ...(input.renderStory || input.render_story || {}),
    },
    rightsLedger: input.rightsLedger || input.rights_ledger || {},
    footageInventory: {
      clips: supportingClips,
    },
    directorPlan: input.director || {},
    renderManifest,
  });
  return {
    ...input,
    visualQuality: {
      ...(input.visualQuality || {}),
      visual_evidence_profile: profile,
    },
  };
}

function buildPulseMediaHouseScore(input = {}) {
  const evaluatedInput = withAuthoritativeFinalVisualEvidence(input);
  const generatedAt = evaluatedInput.generatedAt || new Date().toISOString();
  const scores = buildScores(evaluatedInput);
  const failures = hardFailures(evaluatedInput, scores);
  ...
  const report = {
    ...
    story_id: cleanText(
      evaluatedInput.story_id ||
        evaluatedInput.canonical?.story_id ||
        evaluatedInput.canonical?.id ||
        null,
    ) || null,
    source_render_run_id: cleanText(
      evaluatedInput.renderManifest?.run_id ||
        evaluatedInput.renderManifest?.render_run_id ||
        evaluatedInput.renderManifest?.generation_id,
    ) || null,
    verdict,
    ...
    competitor_similarity: evaluatedInput.competitorSimilarity || {},
    safety: {
  ...
  report.selected_render_visual_evidence_profile =
    evaluatedInput.visualQuality?.visual_evidence_profile || null;
  report.source_lock_report = buildSourceLockReport(evaluatedInput, report);
  report.professional_source_diversity_report = buildProfessionalSourceDiversityReport(
    evaluatedInput,
    evaluatedInput.footageEmpireV2 || evaluatedInput.footageEmpire || {},
  );
  report.production_grammar_alignment_report = buildProductionGrammarAlignmentReport(
    evaluatedInput,
    report,
  );
  ...
  report.shorts_attention_report = buildShortsAttentionReport(evaluatedInput, report);
  report.shorts_feed_competition_report = buildShortsFeedCompetitionReport(evaluatedInput);
  report.premium_output_contract = buildPremiumOutputContract(evaluatedInput);
...
module.exports = {
  ...
  buildProfessionalSourceDiversityReport,
  ...
  _private: {
    ...
    platformCopyStoryAngleMismatch,
