...
function consequenceLanguage(text = "") {
  return /\b(?:problem|risk|catch|warning|changed|broke|broken|revealed|confirmed|finally|pushback|why|before|after|ceiling|impact|cost|deal|launch|lands?|hits?|date|proof|payoff|trust|pressure|fight|split|splits|splitting|bloat|dangerous|delete|wins?|test|reinstall|lapsed|returning|progression|prestige|weapon prestige|zombies?|endgame|spike|peak|demand|momentum|exposes?|early[- ]access|ea\s+play|ps5|jungle|combat|discovery|readability|readable|roster|meta|matchups?|team building|screen control|wishlist|map|jump[- ]?scares?|choices?|atmosphere|horror)\b/i.test(text);
}
...
function audiencePullLanguage(text = "") {
  ...
  const hasSpecificity =
    /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|one|only|first|last|free|paid|delay(?:ed)?|leak(?:ed)?|launch|ending|before|after|steam peak|peak|early[- ]access|ea\s+play|ps5|jungle|combat|discovery|readability|readable|roster|meta|matchups?|team building|screen control|tag fighters?|wishlist|map|custom seas|private seas|private sessions?|private mode|rule controls?|set (?:their own )?rules|jump[- ]?scares?|choices?|atmosphere|horror)\b/i.test(value);
  const hasCuriosity =
    /\b(?:why|how|what|secret|hidden|real|catch|risk|problem|fight|pressure|warning|trust|ceiling|broke|turns?|changed|into|test|exposes?|bet|prove|proves|sharp|sharper)\b/i.test(value);
...
function viewerStakeLanguage(text = "") {
  ...
  const hasConcreteHook =
    /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|demo|trailer|campaign|storage|ssd|pc specs?|release date|review score|free trial|subscription|boss|map|mode|custom seas|private seas|private sessions?|private mode|rule controls?|set (?:their own )?rules|launch|delay|score|roster|meta|matchups?|team building|screen control|tag fighters?|jump[- ]?scares?|choices?|atmosphere|horror)\b/i.test(value) ||
