...
function cardTimingReport(input = {}) {
  ...
  const sourceCards = cardShots.filter((shot) => /source|source_lock|proof/i.test(shotKind(shot)));
  const readableCards = cardShots.filter((shot) => !/source_lock/i.test(shotKind(shot)));
  const blockers = [];
  ...
  if (sourceCardDwell.some((duration) => duration > 6.5)) {
  ...
  if (sourceCards.some((shot) => shotStart(shot) <= 0.5 && shotDuration(shot) > 2.2)) {
