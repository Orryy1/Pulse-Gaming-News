...
function consequenceLanguage(text = "") {
  return /\b(?:problem|risk|catch|warning|changed|broke|broken|revealed|confirmed|finally|pushback|why|before|after|ceiling|impact|cost|deal|launch|lands?|hits?|date|proof|payoff|trust|pressure|fight|trial|argument|balance|balanced|polish|half[- ]finished|split|splits|splitting|bloat|dangerous|delete|wins?|test|reinstall|lapsed|returning|progression|prestige|weapon prestige|zombies?|endgame|spike|peak|demand|momentum|exposes?|early[- ]access|ea\s+play|ps5|default\s+version|platform comparisons?|performance proof|jungle|combat|discovery|readability|readable|lab[- ]worthy|roster|meta|matchups?|team building|screen control|wishlist|map|jump[- ]?scares?|choices?|atmosphere|horror)\b/i.test(text);
}
