...
function consequenceLanguage(text = "") {
  return /\b(?:problem|risk|catch|warning|changed|broke|broken|revealed|confirmed|finally|pushback|why|before|after|ceiling|impact|cost|deal|launch|lands?|hits?|date|proof|payoff|trust|pressure|fight|bloat|dangerous|delete|wins?|spike|peak|demand|momentum|exposes?|early[- ]access|ea\s+play|ps5|jungle|combat|discovery|readability|map|jump[- ]?scares?|choices?|atmosphere|horror)\b/i.test(text);
}
...
function audiencePullLanguage(text = "") {
  ...
  const hasSpecificity =
    /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|one|only|first|last|free|paid|delay(?:ed)?|leak(?:ed)?|launch|ending|before|after|steam peak|peak|early[- ]access|ea\s+play|ps5|jungle|combat|discovery|readability|map|jump[- ]?scares?|choices?|atmosphere|horror)\b/i.test(value);
  const hasCuriosity =
    /\b(?:why|how|what|secret|hidden|real|catch|risk|problem|fight|pressure|warning|trust|ceiling|broke|turns?|changed|into|test|exposes?|bet)\b/i.test(value);
...
function viewerStakeLanguage(text = "") {
  ...
  const hasConcreteHook =
    /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|demo|trailer|campaign|storage|ssd|pc specs?|release date|review score|free trial|subscription|boss|map|mode|launch|delay|score|jump[- ]?scares?|choices?|atmosphere|horror)\b/i.test(value) ||
...
function sourceLockScore({ canonical = {}, visualQuality = {}, benchmark = {}, footageEmpireV2 = {} } = {}) {
  ...
  if (hasObject(footageEmpireV2)) {
    ...
    const readiness = cleanText(footageEmpireV2.readiness?.status || verdict).toLowerCase();
    ...
    if (verdictFails(verdict) || readiness === "v4_motion_blocked") score = Math.min(score, 45);
  }
