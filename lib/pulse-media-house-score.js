...
function consequenceLanguage(text = "") {
  return /\b(?:problem|risk|catch|warning|changed|broke|broken|revealed|confirmed|finally|pushback|why|before|after|ceiling|impact|cost|deal|launch|date|proof|payoff|trust|pressure|fight|bloat|dangerous|delete|wins?|spike|demand|momentum)\b/i.test(text);
}
...
function audiencePullLanguage(text = "") {
  ...
  const hasSpecificity =
    /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|one|only|first|last|free|paid|delay(?:ed)?|leak(?:ed)?|launch|ending|before|after)\b/i.test(value);
  const hasCuriosity =
...
function concreteThumbnailStake(text = "") {
  return /\b(?:\d+|[0-9]+ ?(?:gb|fps|k|million|billion|hours?)|problem|risk|catch|warning|pressure|fight|broke|broken|why|before|after|free|paid|delay|leak|deal|price|storage|ssd|launch|spike|demand|momentum)\b/i.test(
    cleanText(text),
  );
}
