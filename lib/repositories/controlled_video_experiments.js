...
const ASSIGNMENT_POLICY = "canonical-ordinal-v1";
...
function bind(db) {
  ...
  const assignNextVideoTransaction = db.transaction(
    ({
      ...
      const next = getNextCell.get(
        normalisedExperimentId,
        normalisedChannelId,
      );
      if (!next) throw new Error("controlled_experiment_matrix_full");
