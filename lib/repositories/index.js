...
const factories = {
  ...
  runtimeLeases: require("./runtime_leases"),
  killSwitch: require("./kill_switch"),
  circuitBreakers: require("./circuit_breakers"),
  runtimeComponents: require("./runtime_components"),
  publicationGovernance: require("./publication_governance"),
  controlledExperiments: require("./controlled_video_experiments"),
  youtubeAnalyticsExperimentSnapshots: require(
    "./youtube_analytics_experiment_snapshots",
  ),
};
...
function bindRepositories(handle) {
  if (!handle || typeof handle.prepare !== "function") {
    throw new Error("[repositories] A migrated SQLite database is required.");
  }
...
function getRepos() {
  ...
  _cached = bindRepositories(handle);
  return _cached;
}
...
module.exports = { bindRepositories, getRepos, resetReposCache };
