...
function bind(db) {
  ...
  const nextClaimable = db.prepare(`
    ...
    LIMIT 1
  `);
  const exactClaimable = db.prepare(`
    SELECT * FROM jobs
    WHERE id = @id
      AND status = 'pending'
      AND attempt_count < max_attempts
  ...
  return {
    ...
    /**
     * Claim one exact due job while retaining the same transaction,
     * claim-token and lease fencing as claim(). This is intentionally not
     * a fallback selector: if the requested row is not due or eligible it
     * returns null and cannot claim a different queued job.
     */
    claimExact(
      jobId,
      workerId,
      ...
      const normalisedJobId = requiredJobId(jobId);
      const normalisedWorkerId = requiredWorkerId(workerId);
      const normalisedLeaseMs = positiveLeaseDuration(leaseMs);
      const kindsJson = kinds && kinds.length ? JSON.stringify(kinds) : null;
      const txn = db.transaction(() => {
        const candidate = exactClaimable.get({
          id: normalisedJobId,
          gpuOnly: gpu ? 1 : 0,
          kindsJson,
          channelId,
