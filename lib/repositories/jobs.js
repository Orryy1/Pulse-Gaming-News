...
function bind(db) {
  ...
  `);
  const bindClaimToken = db.prepare(`
    UPDATE jobs
  ...
  return {
    ...
    /**
     * Atomically enqueue a complete batch through the same validation and
     * idempotency rules as enqueue(). Any error rolls the entire batch back.
     */
    enqueueBatch(batch) {
      if (!Array.isArray(batch)) {
        throw new Error("job_enqueue_batch_array_required");
      }
      const enqueueTransaction = db.transaction(() =>
        batch.map((job) => enqueueInternal(job)),
      );
      return enqueueTransaction.immediate().map(hydrate);
    },
    ...
    claim(
      ...
      const txn = db.transaction(() => {
        ...
        const claimToken = Number(run.lastInsertRowid);
        const bound = bindClaimToken.get({
          id: claimed.id,
          workerId: normalisedWorkerId,
        ...
        if (!bound) throw new Error("stale_job_claim:claim_token_bind_failed");
        bound.claim_token = String(bound.claim_token);
        return bound;
      });
    ...
    complete(jobId, workerId, claimToken, { log = null } = {}) {
      ...
      const txn = db.transaction(() => {
        ...
        if (!completed) throw new Error("stale_job_claim:job_lease_not_held");
        const runRow = db
          .prepare(
            `SELECT id
             FROM job_runs
    ...
    fail(
      ...
      const txn = db.transaction(() => {
        ...
        if (!attempt) throw new Error("stale_job_claim:job_lease_not_held");
        const idx = Math.min(
        ...
        if (!res) throw new Error("stale_job_claim:job_lease_not_held");
        const runRow = db
          .prepare(
            `SELECT id
             FROM job_runs
