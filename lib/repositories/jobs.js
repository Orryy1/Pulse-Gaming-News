...
function bind(db) {
  ...
  const latestActiveByKind = db.prepare(`
    SELECT *
    FROM jobs
    WHERE kind = ?
    ...
    LIMIT 1
  `);
  const newerActiveByKind = db.prepare(`
    SELECT *
    FROM jobs
    WHERE kind = ?
      AND id > ?
  ...
  return {
    ...
    findLatestActiveByKind(kind) {
      return hydrate(latestActiveByKind.get(kind));
    },

    findNewerActiveByKind(kind, jobId) {
      return hydrate(newerActiveByKind.get(kind, jobId));
    },
