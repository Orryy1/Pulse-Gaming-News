...
const REPOSITORY_DATABASE_BINDINGS = new WeakMap();
...
function bind(db) {
  ...
  REPOSITORY_DATABASE_BINDINGS.set(repository, db);
  return repository;
}

function isRuntimeLeaseRepositoryBoundToDatabase(repository, db) {
  return Boolean(
    repository &&
      db &&
      (typeof repository === "object" || typeof repository === "function") &&
      REPOSITORY_DATABASE_BINDINGS.get(repository) === db,
  );
}

module.exports = { bind, iso, isRuntimeLeaseRepositoryBoundToDatabase };
