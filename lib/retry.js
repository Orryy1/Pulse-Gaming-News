...
function isRetriable(err) {
  if (err?.nonRetriable === true || err?.retriable === false) return false;
  const status = err.response?.status;
  const responseData = err.response?.data;
  const debugInfo = responseData?.debug_info || responseData?.error?.debug_info;
  if (debugInfo?.retriable === false) return false;
  // Don't retry client errors (auth failures, bad requests, not found)
  if (status && status >= 400 && status < 500) return false;
  const msg = (err.message || "").toLowerCase();
  if (
    /instagram binary upload failed/i.test(err.message || "") &&
    /"retriable"\s*:\s*false/i.test(err.message || "") &&
    /processingfailederror|request processing failed/i.test(err.message || "")
  ) {
    return false;
  }
  if (
    /instagram (?:url )?processing failed/i.test(err.message || "") &&
    /2207076|2207052|media upload has failed|unsupported|codec|pix_fmt|profile|aspect/i.test(
      err.message || "",
    )
  ) {
    return false;
  ...
  if (
    /facebook reel did not go live within\s+\d+\s*(?:min|minute|minutes)\b/i.test(err.message || "") ||
    /facebook reel processing errored\b/i.test(err.message || "")
  ) {
    return false;
  }
  if (
...
async function withRetry(
  ...
  if (lastError && typeof lastError === "object") {
    if (!lastError.originalMessage) lastError.originalMessage = originalMessage;
    lastError.message = `${label} failed after ${maxAttempts} attempts: ${originalMessage}`;
    throw lastError;
  }
  throw new Error(`${label} failed after ${maxAttempts} attempts: ${originalMessage}`);
}
