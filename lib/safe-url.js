 ...
 *   - DNS rebinding between validation and fetch. Redirect targets
 *     can be guarded by using `safeRedirectConfig(maxRedirects)`,
 *     which wires axios/follow-redirects beforeRedirect validation.
 *
 * Usage:
...
function assertSafeRedirectTarget(options = {}) {
  const protocol = options.protocol || "https:";
  const host = options.hostname || options.host;
  const targetPath = options.path || "/";
  if (!host) {
    throw new Error("unsafe outbound URL rejected (missing_host)");
  }
  assertSafeOutboundUrl(`${protocol}//${host}${targetPath}`);
}

function safeRedirectConfig(maxRedirects = 0) {
  return {
    maxRedirects,
    beforeRedirect: (options) => assertSafeRedirectTarget(options),
  };
}
