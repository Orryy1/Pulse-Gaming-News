 ...
 * Studio shorts run 12â€“16 cuts in 60s for a fast-paced feel. With
...
function composeStudioSlate({ story, media, audioDurationS, opts = {} }) {
  ...
  const flashStillPool = flashLane && reusableClips.length < 6
    ? interleaveAssetPools(
        frames.filter((asset) => asset?.path),
        heroes.filter((asset) => asset?.path),
      )
    : [];
  ...
  const flashStillRepeatCap = flashLane
    ...
    : 2;
  const allowFlashExpansionCards = expandScarceFlashLane;
