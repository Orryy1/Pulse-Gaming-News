...
function pickFillerScene(scenes, story, duration, cardBackdrop, idx) {
  ...
  if (sourceCount >= 2) return null; // cap reached
...
function composeStudioSlate({ story, media, audioDurationS, opts = {} }) {
  ...
  for (let i = 0; i < middleTarget; i++) {
    ...
    if (wantClip) {
      ...
      // Out of fresh media. Try a card-filler first (capped at 2
      // source-cards total); if the cap is reached, fall back to
      // RE-USING a previously emitted clip.frame with a different
      // motion preset. The no-adjacent-source pass scatters the
      // duplicate. Better a soft repeat than a third source-card.
      const sub = pickFillerScene(scenes, story, sceneDur, cardBackdrop, i);
      if (sub) {
        scenes.push(sub);
      } else {
        // Find a previously-emitted clip.frame to re-use.
        const reuse = scenes.find((s) => s.type === SCENE_TYPES.CLIP_FRAME);
        if (reuse) {
          scenes.push({
            type: SCENE_TYPES.CLIP_FRAME,
            duration: sceneDur,
            label: `frame_reuse_${i}`,
            source: reuse.source,
            motion: pickMotion(scenes.length + 7), // different preset
            _reused: true,
          });
        }
        // If even that fails (no frames at all) — skip the slot.
        // The composer caps target by audio duration so a missing
        // slot just shortens the runtime slightly.
      }
    }
  }
