...
function reusableClipVariant(clips, useIndex) {
  ...
  const baseStart = Number.isFinite(Number(clip?.mediaStartS))
    ? Number(clip.mediaStartS)
    : Number.isFinite(Number(clip?.media_start_s))
      ? Number(clip.media_start_s)
      : 12;
  const lap = Math.floor(useIndex / clips.length);
  return {
    ...clip,
    mediaStartS: Number((baseStart + lap * 4.5).toFixed(2)),
  };
}
...
function composeStudioSlate({ story, media, audioDurationS, opts = {} }) {
  ...
  let frameIdx = 0;
  let heroIdx = 0;
  const minFlashClipScenes = flashLane ? Math.ceil(targetCount * 0.55) : 0;
  ...
  if (flashLane) deferFlashSourceCardAfterHook(scenes);
