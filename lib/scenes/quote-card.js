...
function buildQuoteBodyLayout(body) {
  ...
  const variants = [
    { lineMax: 28, fontSize: 42, lineH: 56, maxLines: 6 },
    { lineMax: 32, fontSize: 38, lineH: 50, maxLines: 7 },
    { lineMax: 36, fontSize: 34, lineH: 46, maxLines: 8 },
  ];

  for (const variant of variants) {
    const lines = wrapBody(cleaned, variant.lineMax);
    if (lines.length <= variant.maxLines) {
      const blockTop = Math.round(1920 / 2 - (lines.length * variant.lineH) / 2);
      return {
        ...variant,
        lines,
        truncated: false,
  ...
  const blockTop = Math.round(1920 / 2 - (lines.length * fallback.lineH) / 2);
  return {
    ...fallback,
    lines,
    truncated: true,
