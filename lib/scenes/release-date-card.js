...
function buildReleaseDateCardFilter({
  ...
  return [
    ...
    // Big date drop-shadow
    `drawtext=text='${date}':${fontOpt}:fontcolor=black@0.7:fontsize=140:x=(w-tw)/2+5:y=h/2-50+5:${fadeIn(0.4, 0.6)}`,
    // Big date text
    `drawtext=text='${date}':${fontOpt}:fontcolor=white:fontsize=140:x=(w-tw)/2:y=h/2-50:${fadeIn(0.4, 0.6)}`,
    // Sublabel
    sub
