...
function buildFlashLaneSourceCardFilter({
  ...
  const labelFontSize = label.length > 18 ? 42 : label.length > 12 ? 48 : 56;
  ...
  return [
    ...
    `drawtext=text='SOURCE CHECK':${fontOpt}:fontcolor=${ACCENT}:fontsize=24:x=88:y=220:${fadeIn(0.08, 0.2)}`,
    `drawtext=text='${label}':${fontOpt}:fontcolor=black@0.64:fontsize=${labelFontSize}:x=91:y=258:${fadeIn(0.18, 0.35)}`,
    `drawtext=text='${label}':${fontOpt}:fontcolor=white:fontsize=${labelFontSize}:x=88:y=255:${fadeIn(0.18, 0.35)}`,
    `drawtext=text='${sub}':${fontOpt}:fontcolor=white@0.78:fontsize=22:x=88:y=316:${fadeIn(0.36, 0.25)}`,
