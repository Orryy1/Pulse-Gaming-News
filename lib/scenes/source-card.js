...
function buildFlashLaneSourceCardFilter({
  ...
  return [
    ...
    `fps=30`,
    `boxblur=8:3`,
    `eq=brightness=-0.10:saturation=0.82:contrast=1.20`,
    `drawbox=x=52:y=458:w=976:h=360:color=black@0.48:t=fill:${enableAfter(0.05)}`,
    `drawbox=x=52:y=458:w=8:h=360:color=${ACCENT}@0.98:t=fill:${enableAfter(0.05)}`,
    `drawbox=x=52:y=458:w='if(lt(t\\,0.45)\\,1+(976-1)*t/0.45\\,976)':h=5:color=${ACCENT}@0.98:t=fill`,
    `drawtext=text='FLASH SOURCE':${fontOpt}:fontcolor=${ACCENT}:fontsize=34:x=102:y=508:${fadeIn(0.12, 0.25)}`,
    `drawtext=text='SOURCE CHECK':${fontOpt}:fontcolor=white@0.78:fontsize=28:x=102:y=552:${fadeIn(0.22, 0.25)}`,
    `drawtext=text='${label}':${fontOpt}:fontcolor=black@0.70:fontsize=${labelFontSize}:x=107:y=621:${fadeIn(0.32, 0.45)}`,
    `drawtext=text='${label}':${fontOpt}:fontcolor=white:fontsize=${labelFontSize}:x=102:y=616:${fadeIn(0.32, 0.45)}`,
    `drawtext=text='${sub}':${fontOpt}:fontcolor=white@0.82:fontsize=30:x=102:y=718:${fadeIn(0.62, 0.35)}`,
    `drawtext=text='  PULSE VERIFIED  ':${fontOpt}:fontcolor=black:fontsize=28:x=102:y=770:box=1:boxcolor=${ACCENT}@0.98:boxborderw=12:${fadeIn(0.82, 0.35)}`,
    `drawbox=x=52:y=850:w='if(lt(t\\,1.1)\\,1\\,1+(976-1)*(t-0.7)/0.4)':h=4:color=white@0.72:t=fill`,
    trim,
    `format=yuv420p,setsar=1[v${slot}]`,
  ].join(",");
}
