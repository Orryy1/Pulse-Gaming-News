...
function buildFlashLaneSourceCardFilter({
  ...
  const trim = `trim=duration=${duration},setpts=PTS-STARTPTS`;
  const label = ffEscape(String(sourceLabel || "NEWSROOM").toUpperCase());
  const sub = ffEscape(String(sublabel || "SOURCE VERIFIED").toUpperCase());
  const labelFontSize = label.length > 18 ? 62 : label.length > 12 ? 76 : 92;
  ...
  return [
    ...
    `fps=30`,
    `boxblur=18:5`,
    `eq=brightness=-0.24:saturation=0.82:contrast=1.18`,
    `drawbox=x=0:y=0:w=iw:h=ih:color=black@0.22:t=fill`,
    `drawbox=x=0:y=0:w=iw:h=340:color=black@0.48:t=fill`,
    `drawbox=x=0:y=h-470:w=iw:h=470:color=black@0.55:t=fill`,
    `drawbox=x=72:y=430:w=936:h=420:color=black@0.62:t=fill:${enableAfter(0.05)}`,
    `drawbox=x=72:y=430:w=8:h=420:color=${ACCENT}@0.98:t=fill:${enableAfter(0.05)}`,
    `drawbox=x=72:y=430:w='if(lt(t\\,0.45)\\,1+(936-1)*t/0.45\\,936)':h=5:color=${ACCENT}@0.98:t=fill`,
    `drawtext=text='SOURCE CHECK':${fontOpt}:fontcolor=${ACCENT}:fontsize=34:x=118:y=484:${fadeIn(0.12, 0.25)}`,
    `drawtext=text='${label}':${fontOpt}:fontcolor=black@0.70:fontsize=${labelFontSize}:x=123:y=563:${fadeIn(0.32, 0.45)}`,
    `drawtext=text='${label}':${fontOpt}:fontcolor=white:fontsize=${labelFontSize}:x=118:y=558:${fadeIn(0.32, 0.45)}`,
    `drawtext=text='${sub}':${fontOpt}:fontcolor=white@0.78:fontsize=30:x=118:y=666:${fadeIn(0.62, 0.35)}`,
    `drawtext=text='  PULSE VERIFIED  ':${fontOpt}:fontcolor=black:fontsize=28:x=118:y=736:box=1:boxcolor=${ACCENT}@0.98:boxborderw=12:${fadeIn(0.82, 0.35)}`,
    `drawbox=x=72:y=904:w='if(lt(t\\,1.1)\\,1\\,1+(936-1)*(t-0.7)/0.4)':h=4:color=white@0.72:t=fill`,
    trim,
    `format=yuv420p,setsar=1[v${slot}]`,
  ].join(",");
}

module.exports = { buildSourceCardFilter, buildFlashLaneSourceCardFilter };
