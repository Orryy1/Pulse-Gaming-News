...
function standardSourceCardLayout(sourceLabel = "") {
  const label = compactDisplayLabel(sourceLabel, 26);
  const labelFontSize =
    label.length > 24 ? 52 : label.length > 20 ? 56 : label.length > 16 ? 72 : 96;
  return { label, labelFontSize };
}
...
function buildSourceCardFilter({
  ...
  const layout = standardSourceCardLayout(sourceLabel);
  ...
  return [
    ...
    `drawtext=text='${label}':${fontOpt}:fontcolor=black@0.7:fontsize=${layout.labelFontSize}:x=(w-tw)/2+4:y=${shadowY}:${fadeIn(0.3, 0.6)}`,
    // Big label (white)
    `drawtext=text='${label}':${fontOpt}:fontcolor=white:fontsize=${layout.labelFontSize}:x=(w-tw)/2:y=${labelY}:${fadeIn(0.3, 0.6)}`,
...
module.exports = {
  ...
  buildFlashLaneSourceCardFilter,
  flashLaneSourceCardLayout,
  standardSourceCardLayout,
};
