...
const { startLeaseHeartbeat } = require("./services/lease-heartbeat");
const {
...
function start({
  ...
  cronImpl = cron,
  nowProvider = () => new Date(),
  leaseHeartbeatFactory = startLeaseHeartbeat,
} = {}) {
  ...
  const deactivate = () => {
    const heartbeat = heartbeatHandle;
  ...
  const heartbeatNow = () => {
    if (!active || !heartbeatHandle) return false;
    const result = heartbeatHandle.heartbeatNow();
    if (result && typeof result.then === "function") {
      log("[scheduler] asynchronous scheduler lease heartbeat is unsupported");
      deactivate();
      return false;
    }
    return result;
  ...
  const retryNow = ({ throwOnRegistrationFailure = false } = {}) => {
    ...
    active = true;
    try {
      heartbeatHandle = leaseHeartbeatFactory({
        heartbeat: () => heartbeatSchedulerLease({
          leases: resolvedRepos.runtimeLeases,
          ownerId,
          fencingToken: lease?.fencing_token,
        }),
        intervalMs: Math.max(1000, Number(heartbeatIntervalMs) || 30_000),
        onLost() {
  ...
  return {
    ...
    stop() {
      ...
      const owned = Boolean(lease?.acquired);
      deactivate();
      if (owned) {
        releaseSchedulerLease({
          leases: resolvedRepos.runtimeLeases,
