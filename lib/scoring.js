...
} = require("./analytics-recommendation");
...
// Crude sexual/body-part wording is advertiser-unfriendly even when the
// surrounding story is technically about a game, patch or platform rule.
// Match with word boundaries so harmless words such as "dongle" do not trip.
const CRUDE_SEXUAL_TERMS = [
  "dong",
  "penis",
  "genitals",
  "nudity",
...
const DIMENSION_SCORERS = {
  source_confidence(story) {
    ...
    const firstPartySource = firstPartySourceForStory(story);
    let base = 12;
    ...
    if (firstPartySource) base = DIM_WEIGHTS.source_confidence;
  ...
  story_importance(story) {
    ...
    const hay = storyScoringText(story).toLowerCase();
    ...
    if (firstPartyAnnouncement.qualifies) {
      pts = Math.max(
        pts,
        10 + Math.min(3, firstPartyAnnouncement.signals.length),
      );
    }
  ...
  search_demand(story) {
    ...
    let score = 2;
    if (proxy >= 250) score = 10;
    else if (proxy >= 120) score = 8;
    else if (proxy >= 50) score = 6;
    else if (proxy >= 15) score = 4;

    const firstPartyAnnouncement = classifyVerifiedFirstPartyAnnouncement(story);
    if (firstPartyAnnouncement.qualifies) {
      const contentFloor = firstPartyAnnouncement.signals.length >= 3 ? 8 : 6;
      score = Math.max(score, contentFloor);
    }
    return score;
  },
  ...
  advertiser_safety(story) {
    ...
    for (const term of CRUDE_SEXUAL_TERMS) {
...
function scoreStory(story, ctx = {}) {
  ...
  const latestRecommendation =
    ctx.analyticsRecommendation !== undefined
      ? { recommendation: ctx.analyticsRecommendation || null }
      : readLatestRecommendation();
  const analyticsRecommendationSignal = evaluateHookRecommendationSignal(
    story,
  ...
  return {
    ...
    inputs: {
      ...
      analytics_recommendation_signal: analyticsRecommendationSignal,
      first_party_announcement: {
        qualifies: firstPartyAnnouncement.qualifies,
        ...
        evidence_text_present: firstPartyAnnouncement.evidence_text_present,
      },
    },
  };
}
...
const FRESH_CONFIRMED_REDDIT_MIN_SCORE = 70;
const FRESH_CONFIRMED_REDDIT_MAX_AGE_HOURS = 36;
const FRESH_CONFIRMED_REDDIT_SUBS = new Set([
  "games",
  "pcgaming",
  "nintendoswitch",
  "xboxseriesx",
...
const FRESH_CONFIRMED_REDDIT_FLAIR_RE =
  /\b(?:release|news|official|industry news|news\/article)\b/i;
const FRESH_CONFIRMED_REDDIT_CLAIM_RE =
  /\b(?:officially|official|confirmed|released|release|launches|launched|available now|early access|demo|update|patch|price|sale|bundle)\b/i;
const FRESH_CONFIRMED_REDDIT_SPECULATION_RE =
  /\b(?:reportedly|rumou?r|could|might|maybe|possibly|speculation|unverified|anonymous)\b/i;
const FRESH_GAME_POLICY_REDDIT_MIN_SCORE = 65;
const FRESH_GAME_POLICY_CORE_RE =
  /\b(?:stop killing games|game preservation|server shutdowns|games playable|abandoned games|online servers|access to the games|games they(?:'|’)ve already paid for)\b/i;
const FRESH_GAME_POLICY_PROCESS_RE =
  /\b(?:bill|law|legislation|assembly|committee|vote|refunds?|maintain online servers|shut them down)\b/i;
const FRESH_GAME_POLICY_SOURCE_RE =
  /\b(?:according to|rock paper shotgun|eurogamer|ign|gamespot|polygon|pc gamer|pcgamer|vgc|video games chronicle)\b/i;
const FRESH_GAME_POLICY_SPECULATION_RE =
  /\b(?:reportedly|rumou?r|maybe|possibly|speculation|unverified|anonymous)\b/i;

function storyAgeHours(story) {
  const raw = story?.timestamp || story?.created_at || story?.createdAt || null;
...
function qualifiesForFreshConfirmedRedditAutoLane(story, score) {
  if (!story || !score) return { qualifies: false, reason: null };
  if (score.decision !== "review") return { qualifies: false, reason: null };
  if (Array.isArray(score.hard_stops) && score.hard_stops.length > 0)
    return { qualifies: false, reason: null };
  ...
  if (b.advertiser_safety !== 5) return { qualifies: false, reason: null };
  if ((b.duplicate_safety || 0) < 8) return { qualifies: false, reason: null };
  if ((score.total || 0) < FRESH_CONFIRMED_REDDIT_MIN_SCORE)
    return { qualifies: false, reason: null };

  const sourceType = (story.source_type || "").toLowerCase();
  if (sourceType !== "reddit") return { qualifies: false, reason: null };

  const subreddit = (story.subreddit || "").toLowerCase().replace(/\s+/g, "");
  if (!FRESH_CONFIRMED_REDDIT_SUBS.has(subreddit))
    return { qualifies: false, reason: null };

  if (storyAgeHours(story) > FRESH_CONFIRMED_REDDIT_MAX_AGE_HOURS)
    return { qualifies: false, reason: null };

  if (!FRESH_CONFIRMED_REDDIT_FLAIR_RE.test(String(story.flair || "")))
    return { qualifies: false, reason: null };
  ...
  if (!FRESH_CONFIRMED_REDDIT_CLAIM_RE.test(hay))
    return { qualifies: false, reason: null };
  if (FRESH_CONFIRMED_REDDIT_SPECULATION_RE.test(hay))
    return { qualifies: false, reason: null };
  for (const phrase of AUTO_LANE_OFF_BRAND_BLOCKERS) {
    if (hay.includes(phrase)) return { qualifies: false, reason: null };
  }
  ...
  const wordBoundaryMatch = (kw) =>
    new RegExp(`\\b${escapeRegExp(kw)}\\b`, "i").test(hay);
  const matchedFranchise = FRANCHISE_KEYWORDS.find(wordBoundaryMatch) || null;
  const matchedPlatform =
    PLATFORM_SERVICE_KEYWORDS.find(wordBoundaryMatch) || null;
  if (!matchedFranchise && !matchedPlatform)
    return { qualifies: false, reason: null };

  const parts = [`subreddit=${subreddit}`, `score=${score.total}`];
  if (matchedFranchise) parts.push(`franchise=${matchedFranchise}`);
  if (matchedPlatform) parts.push(`platform=${matchedPlatform}`);

  return {
    qualifies: true,
    reason: `fresh_confirmed_reddit_auto_lane: ${parts.join(", ")}`,
  };
}

function qualifiesForFreshGamePolicyRedditAutoLane(story, score) {
  if (!story || !score) return { qualifies: false, reason: null };
  if (score.decision !== "review") return { qualifies: false, reason: null };
  if (Array.isArray(score.hard_stops) && score.hard_stops.length > 0)
    return { qualifies: false, reason: null };
  ...
  if (b.advertiser_safety !== 5) return { qualifies: false, reason: null };
  if ((b.duplicate_safety || 0) < 8) return { qualifies: false, reason: null };
  if ((score.total || 0) < FRESH_GAME_POLICY_REDDIT_MIN_SCORE)
    return { qualifies: false, reason: null };
  ...
  if (!FRESH_GAME_POLICY_CORE_RE.test(hay)) return { qualifies: false, reason: null };
  if (!FRESH_GAME_POLICY_PROCESS_RE.test(hay)) return { qualifies: false, reason: null };
  if (!FRESH_GAME_POLICY_SOURCE_RE.test(hay)) return { qualifies: false, reason: null };
  if (FRESH_GAME_POLICY_SPECULATION_RE.test(hay))
    return { qualifies: false, reason: null };
  for (const phrase of AUTO_LANE_OFF_BRAND_BLOCKERS) {
    if (hay.includes(phrase)) return { qualifies: false, reason: null };
  }
  ...
  return {
    qualifies: true,
    reason: `fresh_game_policy_reddit_auto_lane: subreddit=${subreddit}, score=${score.total}`,
  };
}
...
module.exports = {
  ...
  FRESH_GAME_POLICY_REDDIT_MIN_SCORE,
  scoreStory,
  recordScore,
