...
const VAGUE_FILLER_PATTERNS = [
  ...
  },
  {
    ...
    re: /\b(?:here(?: is|'?s) what matters(?: now)?|the catch is what matters|the catch is simple|the debate is simple|the useful part is simple|the player-facing part is|the player impact is|for players,\s*the takeaway is simple|for players,\s*the point is simple|the payoff is knowing|players will judge the practical change first|what feels better,\s*what lasts longer)\b/i,
  },
  {
  ...
  },
  {
    ...
    re: /\b(?:new detail players should clock|new player-facing detail to judge|player-facing detail still worth separating from the noise|another update exists|content push that has to prove it is more than maintenance|reason to exist beyond repeating the feed|stays a tight update instead of a hype cycle|next thing to watch is whether the official follow-up|small update either becomes useful or fades into the feed|headline fades before the patch notes do|stronger short keeps the subject named|consequence visible from the first line|new source detail,\s*but the real question is still what players can do with it|if it changes when people buy,\s*download,\s*wishlist or return|if it only repeats a headline|needs stronger proof before it deserves attention|the next official detail has to make that choice clear|play now,\s*wait,\s*skip or watch for gameplay)\b/i,
  },
  {
...
const NUMBERED_TITLE_FALSE_POSITIVE_RE =
  /^(?:dodging|avoiding)\b|\b(?:playstation|ps|xbox|xbox series|series x|series s|nintendo switch|switch|steam deck|game pass|directx|unreal engine)\b/i;

const NUMBERED_TITLE_ALIAS_PAIRS = [
  {
    claim: /\bgrand\s+theft\s+auto\s+(?:6|vi)\b/i,
