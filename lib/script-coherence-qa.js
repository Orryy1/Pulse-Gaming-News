...
const VAGUE_FILLER_PATTERNS = [
  ...
  },
  {
    ...
    re: /\bnobody (?:expected|saw (?:it )?coming|noticed)\b/i,
  },
  {
    id: "formulaic_pivot",
    re: /\b(?:but here'?s where it gets interesting|here'?s where it gets interesting|this is the part nobody is reporting|this is bigger than you think|but hold on|but wait)\b/i,
  },
  {
  ...
  },
  {
    ...
    re: /\b(?:signals? for pulse|pulse signals?|signal for pulse|signal first,\s*certainty later|signal over certainty|that'?s the signal|this is the signal|our read is signal|the read is signal)\b/i,
  },
  {
    id: "internal_tracking_language",
    re: /\b(?:safe(?:st)? read is|safe(?:st)? takeaway is|we(?:'re| are) tracking (?:confirmation|the official follow[- ]up|whether)|tracking the official follow[- ]up)\b/i,
  },
];
