...
const REQUEST_SCHEMA_VERSION =
  "pulse-autonomous-green-supplement-request-v1";
const SUPPLEMENT_SCHEMA_VERSION =
...
const RIGHTS_LEDGER_SCHEMA_VERSION =
    ...
    if (
      ...
      scriptSections.some(
        (section) =>
          ...
          !ALLOWED_SCRIPT_SECTIONS.has(section),
      )
    ) {
...
function validateMediaItem(value, prefix) {
  ...
  if (
    (OWNED_RIGHTS_BASES.has(rightsBasis) &&
      licenceDocumentSha256 !== null) ||
    (LICENSED_RIGHTS_BASES.has(rightsBasis) &&
      licenceDocumentSha256 === null)
  ) {
...
function validateFinalMediaInventory(value, request) {
  ...
  const narrationItems = items.filter(
    (item) =>
      item.media_role === "NARRATION" &&
      item.asset_sha256 === narrationSha256,
  );
...
async function materialiseAutonomousGreenSupplement(value, options = {}) {
  ...
  const autonomousPackage = validatePackage(
    packageObservation.value,
    request,
  );
  ...
  if (
    promptControl.source_intake_sha256 !== claimMap.source_intake_sha256
  ) {
  ...
  const itemById = new Map(
    inventory.items.map((item) => [item.item_id, item]),
  );
  if (
  ...
  for (const reference of request.media_rights_evidence) {
    ...
    if (
      rightsObservation.sha256 !== expectedItem.rights_evidence_sha256
    ) {
      fail("autonomous_green_supplement_rights_evidence_binding_mismatch");
    }
    validateRightsEvidence(
      rightsObservation.value,
      request,
      expectedItem,
    );
