...
async function materialiseAutonomousOfficialJitAdmissionPacket(
  ...
  const attemptCount = Number(request.attempt_count);
  if (!Number.isSafeInteger(attemptCount) || attemptCount <= 0) {
    fail("autonomous_jit_packet_attempt_count_invalid");
  }
  ...
  if (
    ...
    path.basename(attemptOutputRoot) !== `attempt-${attemptCount}`
  ) {
  ...
  const controlResult = await materialiseAutonomousAdmissionControlProofs(
    {
      ...
      repos: request.repos,
      env: request.env,
      publicationAdmissionLease: request.publication_admission_lease,
    },
    {
      clock: options.clock,
      fileSystem: options.fileSystem,
  ...
  fullArtifacts.publication_admission_owner_proof =
    controlResult.publication_admission_owner_proof;
  ...
  const authority = await createAutonomousOfficialPublicationAuthority(
    {
      ...
      admission_controls: {
        ...
        publication_admission_owner_proof_sha256:
          controlResult.publication_admission_owner_proof.sha256,
        publication_admission_owner_checked_at: controlResult.generated_at,
      },
    },
    {
      clock: options.clock,
  ...
  return Object.freeze({
    ...
    resolved_plan: Object.freeze({
      schema_version: resolvedPlan.schema_version,
      resolved_plan_sha256: canonicalSha256(resolvedPlan),
    }),
