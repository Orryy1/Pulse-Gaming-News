...
const REPORT_CONTROLS_FIELDS = new Set([
  "kill_switch",
  "scheduler_and_publication_admission_ownership",
  "kill_switch_proof",
...
function validateReport(report, nowMs) {
  ...
  for (const observation of [
    ...
    report.controls.publication_admission_owner_proof,
  ...
  if (
    ...
    report.controls?.scheduler_and_publication_admission_ownership !==
      "SINGLE_OWNER" ||
...
function validateControls(controls, report, nowMs) {
  ...
  const ownerHash = exactSha256(
    controls.publication_admission_owner_proof_sha256,
    "autonomous_publication_authority_publication_admission_owner_hash_invalid",
  );
  if (
    killHash !== report.lineage.kill_switch_proof_sha256 ||
    ownerHash !==
      report.lineage.publication_admission_owner_proof_sha256 ||
    report.controls?.kill_switch_proof?.observed_sha256 !== killHash ||
    report.controls?.publication_admission_owner_proof?.observed_sha256 !==
      ownerHash
  ) {
  ...
  const ownerCheckedAtMs = exactTimestamp(
    controls.publication_admission_owner_checked_at,
    "autonomous_publication_authority_publication_admission_owner_time_invalid",
  );
...
async function createAutonomousOfficialPublicationAuthority(
  ...
  const authorityClaims = {
    ...
    admission_controls: {
      ...
      publication_admission_owner_proof_sha256: controls.ownerHash,
      publication_admission_owner_checked_at:
        admissionControls.publication_admission_owner_checked_at,
    },
