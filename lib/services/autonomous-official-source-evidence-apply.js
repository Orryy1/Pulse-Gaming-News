...
const APPLY_REQUEST_SCHEMA_VERSION =
  "pulse-autonomous-official-source-evidence-apply-request-v4";
const APPLY_REPORT_SCHEMA_VERSION =
  "pulse-autonomous-official-source-evidence-apply-report-v4";
const APPLY_RESULT_SCHEMA_VERSION =
  "pulse-autonomous-official-source-evidence-apply-result-v4";
const APPLY_MATERIALISER_ID =
...
const VISUAL_REVIEW_REQUIREMENT = Object.freeze({
  ...
  autonomous_exception_authority_type: AUTHORITY,
...
const PUBLICATION_ADMISSION_OWNER_PROOF_FIELDS = new Set([
  "schema_version",
  "story_id",
  "checked_at",
  "valid_until",
  "scheduler_owner_sha256",
  "publication_admission_owner_sha256",
  "publication_admission_claimed_job_authority_sha256",
  "active_scheduler_owner_count",
  "active_publication_admission_owner_count",
  "scheduler_owner_healthy",
  "publication_admission_owner_healthy",
  "publication_admission_lease_expires_at",
]);
...
function validateAutonomousGreenSupplement({
  request,
  artifacts,
  ownedVisualAssets,
  scriptSha256,
  ...
  const hashes = object(supplement?.hashes);
  const safety = object(supplement?.safety);
  const validated = object(supplement?.validated);
  const promptInjection = object(supplement?.prompt_injection);
  const mediaItems = Array.isArray(supplement?.media_items)
    ? supplement.media_items
    : [];
  const expectedAssetSha256s = [
    artifacts.owned_programme.observed_sha256,
    artifacts.narration_audio.observed_sha256,
    ...ownedVisualAssets.map((asset) => asset.observed_sha256),
  ].sort();
  const observedAssetSha256s = mediaItems
    .map((item) => text(item?.asset_sha256).toLowerCase())
    .sort();
  const expectedHashes = {
    source_intake_sha256:
      artifacts.story_intake.observed_sha256,
    script_sha256: scriptSha256,
    narration_sha256: artifacts.narration_audio.observed_sha256,
    motion_manifest_sha256:
      artifacts.owned_motion_manifest.observed_sha256,
    render_manifest_sha256:
      artifacts.renderer_manifest.observed_sha256,
    final_mp4_sha256: artifacts.final_mp4.observed_sha256,
    qa_report_sha256: artifacts.deterministic_qa.observed_sha256,
    publication_metadata_sha256:
      artifacts.publication_metadata.observed_sha256,
  };
  const exactHashBindings = Object.entries(expectedHashes).every(
    ([field, expected]) =>
      text(hashes?.[field]).toLowerCase() === expected,
  );
  if (
    supplement?.schema_version !==
      "pulse-autonomous-green-supplement-v1" ||
    text(supplement?.mode).toUpperCase() !== MODE ||
    text(supplement?.verdict).toUpperCase() !== "GREEN" ||
    text(supplement?.authority_scope) !==
      "LOCAL_PROOF_EVIDENCE_ONLY" ||
    text(supplement?.story_id) !== request.story_id ||
    text(supplement?.channel_id) !== "pulse-gaming" ||
    text(supplement?.lane_id) !== "breaking_short" ||
    text(supplement?.platform) !== "youtube" ||
    !SHA256_PATTERN.test(text(supplementSha256).toLowerCase()) ||
    canonicalSha256(supplementBody) !==
      text(supplementSha256).toLowerCase() ||
    !exactHashBindings ||
    promptInjection?.verdict !== "PASS" ||
    validated?.final_media_inventory_complete !== true ||
    validated?.prompt_injection_verdict !== "PASS" ||
    validated?.distinct_lineage_digests !== true ||
    validated?.distinct_rights_evidence_digests !== true ||
    validated?.final_media_item_count !== expectedAssetSha256s.length ||
    new Set(observedAssetSha256s).size !==
      observedAssetSha256s.length ||
    JSON.stringify(observedAssetSha256s) !==
      JSON.stringify(expectedAssetSha256s) ||
    safety?.local_proof_only !== true ||
    safety?.publish_authority !== false ||
    safety?.scheduler_authority !== false ||
    safety?.database_authority !== false ||
    safety?.oauth_or_token_authority !== false ||
    safety?.network_authority !== false ||
    safety?.network_used !== false ||
    safety?.platform_contacted !== false
  ) {
    throw new AutonomousOfficialSourceEvidenceApplyError(
...
function validateAutonomousVisualGateDecision({
  request,
  artifacts,
}) {
  const decision = artifacts.autonomous_visual_gate_decision.value;
  const {
    decision_sha256: decisionSha256,
    ...decisionBody
  } = object(decision) || {};
  const policy = object(decision?.visual_review_policy);
  const bindings = object(decision?.bindings);
  const finalBinding = object(bindings?.final_mp4);
  const visualQaBinding = object(bindings?.visual_qa);
  const modelEvidence = object(decision?.model_evidence);
  const controls = object(decision?.controls);
  ...
  const modelIds = Array.isArray(modelEvidence?.model_ids)
    ...
    : [];
  const visualQaModels = Array.isArray(
    visualQa?.model_aggregation?.requested_models,
  )
    ? visualQa.model_aggregation.requested_models.map((model) => text(model))
    : [];
  ...
  if (
    !exactKeys(decision, expectedDecisionFields) ||
    !exactKeys(policy, [
      "policy_id",
      "policy_version",
      "gate",
    ...
    !exactKeys(bindings, ["final_mp4", "visual_qa"]) ||
    !exactKeys(finalBinding, ["path", "sha256"]) ||
    !exactKeys(visualQaBinding, [
      "path",
      "raw_sha256",
      "canonical_sha256",
    ]) ||
    !exactKeys(modelEvidence, [
      "strategy",
      "model_ids",
    ...
    decision?.schema_version !==
      "pulse-governed-autonomous-visual-gate-decision-v1" ||
    decision?.mode !== MODE ||
    text(decision?.story_id) !== request.story_id ||
    text(decision?.channel_id) !== "pulse-gaming" ||
    text(decision?.lane_id) !== "breaking_short" ||
    text(decision?.platform) !== "youtube" ||
    text(decision?.verdict).toUpperCase() !== "PASS" ||
    text(decision?.decision_authority) !== "SYSTEM_POLICY" ||
    text(decision?.authority_scope) !== AUTHORITY ||
    ...
    !SHA256_PATTERN.test(text(decisionSha256).toLowerCase()) ||
    canonicalSha256(decisionBody) !== text(decisionSha256).toLowerCase() ||
    path.resolve(text(finalBinding?.path)) !==
      artifacts.final_mp4.resolved_path ||
    text(finalBinding?.sha256).toLowerCase() !==
      artifacts.final_mp4.observed_sha256 ||
    path.resolve(text(visualQaBinding?.path)) !==
      artifacts.multimodal_visual_qa.resolved_path ||
    text(visualQaBinding?.raw_sha256).toLowerCase() !==
      artifacts.multimodal_visual_qa.observed_sha256 ||
    text(visualQaBinding?.canonical_sha256).toLowerCase() !==
      canonicalSha256(artifacts.multimodal_visual_qa.value) ||
    modelEvidence?.strategy !== "UNANIMOUS_PASS" ||
    modelIds.length < 2 ||
    new Set(modelIds).size !== modelIds.length ||
    JSON.stringify(modelIds) !== JSON.stringify(visualQaModels) ||
    visualQa?.schema_version !== policy?.required_report_schema ||
    visualQa?.model_aggregation?.strategy !== "UNANIMOUS_PASS" ||
    visualQa?.model_aggregation?.review_count !== modelIds.length ||
    visualQa?.model_aggregation?.pass_count !== modelIds.length ||
    visualQa?.model_aggregation?.all_reviews_must_pass !== true ||
...
function validateLineageAndQa({ request, observations, scriptSha256 }) {
  ...
  if (
    ...
    finalComposite?.human_visual_review_required !== true ||
    canonicalSha256(finalComposite?.visual_review_requirement) !==
      canonicalSha256(VISUAL_REVIEW_REQUIREMENT) ||
  ...
  validateAutonomousGreenSupplement({
    request,
    artifacts,
    ownedVisualAssets: observations.ownedVisualAssets,
    scriptSha256,
  });
  validateAutonomousVisualGateDecision({
    request,
    artifacts,
  });
...
function validateControlProof({
  request,
  killSwitch,
  publicationAdmissionOwner,
  nowMs,
}) {
  const blockers = [];
  ...
  try {
    ...
    const ownerCheckedAt = exactTimestamp(
      publicationAdmissionOwner?.checked_at,
      "publication_admission_owner_checked_at_invalid",
    );
    ownerValidUntil = exactTimestamp(
      publicationAdmissionOwner?.valid_until,
      "publication_admission_owner_valid_until_invalid",
    );
    const leaseExpiresAt = exactTimestamp(
      publicationAdmissionOwner?.publication_admission_lease_expires_at,
      "publication_admission_owner_lease_expiry_invalid",
    );
    if (
      !object(publicationAdmissionOwner) ||
      Object.keys(publicationAdmissionOwner).length !==
        PUBLICATION_ADMISSION_OWNER_PROOF_FIELDS.size ||
      Object.keys(publicationAdmissionOwner).some(
        (field) => !PUBLICATION_ADMISSION_OWNER_PROOF_FIELDS.has(field),
      )
    ) {
      blockers.push("publication_admission_owner_proof_schema_closed");
    }
    if (
      publicationAdmissionOwner?.schema_version !==
        "pulse-publication-admission-owner-proof-v1" ||
      text(publicationAdmissionOwner?.story_id) !== request.story_id ||
      !SHA256_PATTERN.test(
        text(publicationAdmissionOwner?.scheduler_owner_sha256),
      ) ||
      !SHA256_PATTERN.test(
        text(publicationAdmissionOwner?.publication_admission_owner_sha256),
      ) ||
      !SHA256_PATTERN.test(
        text(
          publicationAdmissionOwner
      ...
      publicationAdmissionOwner?.active_scheduler_owner_count !== 1 ||
      publicationAdmissionOwner?.active_publication_admission_owner_count !==
        1 ||
      publicationAdmissionOwner?.scheduler_owner_healthy !== true ||
      publicationAdmissionOwner?.publication_admission_owner_healthy !== true ||
      ownerCheckedAt > nowMs ||
      nowMs - ownerCheckedAt > MAX_JIT_AGE_MS ||
      nowMs >= ownerValidUntil ||
      nowMs >= leaseExpiresAt
    ) {
      blockers.push("fresh_publication_admission_owner_proof_required");
    }
  } catch (error) {
    blockers.push(...(error.codes || [error.message]));
  }
...
function buildLineage({ observations, scriptSha256, rendererSha256 }) {
  return {
    ...
    autonomous_green_supplement_sha256:
      observations.artifacts.autonomous_green_supplement.observed_sha256,
    autonomous_visual_gate_decision_sha256:
      observations.artifacts.autonomous_visual_gate_decision.observed_sha256,
    kill_switch_proof_sha256:
      observations.artifacts.kill_switch_proof.observed_sha256,
    publication_admission_owner_proof_sha256:
      observations.artifacts.publication_admission_owner_proof.observed_sha256,
  };
}
...
async function materialiseAutonomousOfficialSourceEvidence(
  ...
  const controls = validateControlProof({
    ...
    publicationAdmissionOwner:
      observations.artifacts.publication_admission_owner_proof.value,
    nowMs,
  });
  ...
  const reportPayload = {
    ...
    controls: {
      ...
      publication_admission_owner_proof: publicObservation(
        observations.artifacts.publication_admission_owner_proof,
      ),
    },
    qa: {
      deterministic: "PASS",
