...
function tokens(value) {
  return new Set(
    text(value, 8_000)
      ...
      .filter(
        (token) =>
          token.length >= 3 && !GENERIC_QUERY_TOKENS.has(token),
      ),
  );
}
...
function relevanceScore(row, requestTokens, subjects) {
  ...
  for (const token of requestTokens) {
    ...
    overlap += 1;
    if (!subjects.has(token)) propositionOverlap += 1;
  }
  ...
  return Object.freeze({
    ...
    async search(request = {}) {
      ...
      for (const row of Array.isArray(rows) ? rows : []) {
        ...
        const requiredPropositionOverlap =
          relevance.subjectMatches > 0 ? 1 : 2;
        if (
          relevance.propositionOverlap <
          requiredPropositionOverlap
        ) {
          continue;
        }
        seenUrls.add(url);
