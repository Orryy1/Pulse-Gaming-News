...
async function runContentQa(story, opts = {}) {
  ...
  if (!story.exported_path) {
    ...
    try {
      const exists = await fs.pathExists(story.exported_path);
      if (!exists) {
        failures.push("exported_mp4_not_on_disk");
      } else {
        const stat = await fs.stat(story.exported_path);
        if (stat.size < minMp4) {
          failures.push(
  ...
  if (story.story_image_path) {
    try {
      const exists = await fs.pathExists(story.story_image_path);
