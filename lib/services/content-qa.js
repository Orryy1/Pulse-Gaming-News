...
const DEFAULT_MIN_RETENTION_SHORT_WORDS = 45;
...
function isGeneralRedditSource(story) {
  ...
  if (hasNonRedditPrimarySource(story)) return false;
...
function hasNonRedditPrimarySource(story) {
  ...
  if (!sourceLabel || !sourceUrl) return false;
  if (/^(?:reddit|r\/|community|discussion)$/i.test(sourceLabel)) return false;
  if (/(?:^|\/\/)(?:www\.)?reddit\.com\b|(?:^|\/\/)i\.redd\.it\b/i.test(sourceUrl)) return false;
  return true;
}
...
const GTA_VI_TTS_SUBJECT_RE = /\b(?:g\s+t\s+a\s+|gta\s+|gtavi\b|grand\s+theft\s+auto\s+)/;
const GTA_VI_TTS_VERSION_RE = /(?:v\s+i|6|six)\b/;
const GTA_VI_TTS_STUTTER_RE =
  /(?:(?:s|si|sy|sigh|see|sea|c|size|sice|sic|sick|sig|sixty)\s+six|(?:see|sea|c|s|si|suh|sir|size|sice|sic|sick|sig)\s+a\s+six|(?:suh|sir|size|sice|sic|sick|sig)\s+six|six\s+six)\b/;
...
function normaliseSpokenGateText(value) {
  return String(value || "")
    .toLowerCase()
    .normalize("NFKD")
    .replace(/[\u0300-\u036f]/g, "")
...
function hasRiskyGtaViTtsText(value) {
  const normalised = normaliseSpokenGateText(value);
  if (/\bgtavi\b/.test(normalised)) return true;
  if (!GTA_VI_TTS_SUBJECT_RE.test(normalised)) return false;
  return new RegExp(
    `${GTA_VI_TTS_SUBJECT_RE.source}(?:${GTA_VI_TTS_VERSION_RE.source}|${GTA_VI_TTS_STUTTER_RE.source})`,
  ).test(normalised);
}
...
function hasGtaViPronunciationContext(value) {
  ...
  return (
    /\bgtavi\b/.test(normalised) ||
    /\bg\s+t\s+a\s+(?:6|six|v\s+i|vi)\b/.test(normalised) ||
    /\bgta\s+(?:6|six|v\s+i|vi)\b/.test(normalised) ||
    /\bgrand\s+theft\s+auto\s+(?:6|six|v\s+i|vi)\b/.test(normalised) ||
    /\brockstar(?:s)?\s+next\s+grand\s+theft\s+auto\b/.test(normalised) ||
    /\bthe\s+next\s+grand\s+theft\s+auto\b/.test(normalised)
  );
}
...
function timestampWordTimestampSource(payload = {}) {
  const meta = timestampMeta(payload);
  return String(
...
function hasGtaViTimestampProfileRisk(story = {}, payload = {}) {
  if (!payload || typeof payload !== "object" || Object.keys(payload).length === 0) {
    return false;
  }
  const profile = timestampPronunciationProfile(payload);
  if (profile === TTS_PRONUNCIATION_PROFILE_VERSION) return false;
  const meta = timestampMeta(payload);
  const words = arrayField(payload.words)
    .map((word) => String(word?.word || word?.text || word?.token || ""))
    .join(" ");
  return [
    story.title,
    story.public_title,
...
function hasGtaViTimestampAsrVerificationRisk(story = {}, payload = {}) {
  if (!payload || typeof payload !== "object" || Object.keys(payload).length === 0) {
    return false;
  }
  if (timestampWordTimestampSource(payload) === "local_whisper_word_alignment") {
    return false;
  }
  const meta = timestampMeta(payload);
  const words = arrayField(payload.words)
...
function timestampTranscriptFields(payload = {}) {
  const meta = payload?.meta && typeof payload.meta === "object" ? payload.meta : {};
  const fields = [
    ["meta.transcript", meta.transcript],
    ["meta.spoken_text", meta.spoken_text],
    ["meta.text", meta.text],
  ...
  const words = arrayField(payload.words)
    ...
    .filter(Boolean)
    .join(" ");
  if (words) fields.push(["words", words]);
...
function addPathCandidate(candidates, value, baseDir = "") {
  const cleaned = String(value || "").trim();
  if (!cleaned) return;
  const candidate =
    baseDir && !path.isAbsolute(cleaned) ? path.resolve(baseDir, cleaned) : cleaned;
  if (!candidates.includes(candidate)) candidates.push(candidate);
}

function addTimestampFields(candidates, source = {}, baseDir = "") {
  ...
  for (const field of [
    ...
    "timestamps_path",
    "timestamp_path",
    "resolved_audio_timestamps_path",
    "audio_timestamps_path",
  ]) {
    addPathCandidate(candidates, source[field], baseDir);
  }
  for (const nested of [
    source.audio,
    source.narration,
    source.audio_manifest,
    source.final_audio_manifest,
    source.narration_manifest,
    source.caption_manifest,
  ]) {
    if (nested) addTimestampFields(candidates, parseJsonObject(nested), baseDir);
  }
}

function addPackageTimestampCandidates(candidates, artifactDir = "") {
  const dir = String(artifactDir || "").trim();
  if (!dir) return;
  addPathCandidate(candidates, path.join(dir, "audio", "word_timestamps.json"));
  addPathCandidate(candidates, path.join(dir, "word_timestamps.json"));
  addPathCandidate(candidates, path.join(dir, "audio_timestamps.json"));
  addPathCandidate(candidates, path.join(dir, "narration_timestamps.json"));
  addPathCandidate(candidates, path.join(dir, "audio", "narration_timestamps.json"));
}

async function addManifestTimestampCandidates(candidates, manifestPath, fs) {
  const cleaned = String(manifestPath || "").trim();
  if (!cleaned || !fs?.pathExists || !fs?.readJson) return;
  let resolved = cleaned;
  try {
    resolved = path.isAbsolute(cleaned)
      ? cleaned
      : await mediaPaths.resolveExisting(cleaned, { fs });
    if (!resolved || !(await fs.pathExists(resolved))) return;
    const manifest = await fs.readJson(resolved);
    const baseDir = path.dirname(resolved);
    addTimestampFields(candidates, manifest, baseDir);
    addPackageTimestampCandidates(candidates, baseDir);
  } catch {
    return;
  }
}

async function readStoryTimestampPayload(story = {}, fs = fsExtra) {
  if (!story || typeof story !== "object" || typeof fs?.readJson !== "function") {
    return null;
  }
  const extra = parseJsonObject(story._extra || story.extra);
  const candidates = [];
  addTimestampFields(candidates, story);
  addTimestampFields(candidates, extra);
  ...
  for (const filePath of [
    ...
    extra.captions_path,
    extra.caption_path,
    extra.canonical_manifest_path,
    extra.platform_publish_manifest_path,
  ]) {
    ...
    const resolved = path.isAbsolute(cleaned)
      ? cleaned
      : await mediaPaths.resolveExisting(cleaned, { fs });
    if (resolved) addPackageTimestampCandidates(candidates, path.dirname(resolved));
  }

  for (const manifestPath of [
    story.canonical_manifest_path,
    story.platform_publish_manifest_path,
    story.audio_manifest_path,
    story.narration_manifest_path,
    story.caption_manifest_path,
    extra.canonical_manifest_path,
    extra.platform_publish_manifest_path,
    extra.audio_manifest_path,
    extra.narration_manifest_path,
    extra.caption_manifest_path,
  ]) {
    await addManifestTimestampCandidates(candidates, manifestPath, fs);
  }
  ...
  for (const candidate of candidates) {
    try {
      const resolved = path.isAbsolute(candidate)
        ? candidate
        : await mediaPaths.resolveExisting(candidate, { fs });
...
async function runContentQa(story, opts = {}) {
  ...
  const minWords =
    typeof opts.minWords === "number"
      ? opts.minWords
      : durationLane === RETENTION_DURATION_LANE || story?.allow_retention_short_video === true
        ? DEFAULT_MIN_RETENTION_SHORT_WORDS
        : DEFAULT_MIN_WORDS;
  const maxWords =
  ...
  const durationQa = classifyShortDuration({
    ...
    minVideoSeconds: story.min_video_duration_seconds || story.target_video_duration_seconds_min,
    maxVideoSeconds: story.max_video_duration_seconds || story.target_video_duration_seconds_max,
    story,
  });
  ...
  // --- 3b. Gold-standard media-house benchmark --------------------
  // The reference workbook is style guidance, not an asset library.
  // This gate turns it into measurable production checks: first
  // three seconds, motion density, source lock, card hierarchy,
  // captions, transitions, SFX, rights risk and stale wording. By
  // default this warns so legacy stamped renders remain observable;
  // Visual V3/V4 proof/publish paths can opt into hard blocking via
  // opts/env/story metadata.
  try {
    const requireGoldStandardBenchmark =
      opts.requireGoldStandardBenchmark === true ||
      story.require_gold_standard_benchmark === true ||
      story.media_house_benchmark_required === true ||
      env.REQUIRE_GOLD_STANDARD_BENCHMARKS === "true";
    const hasBenchmarkContext = Boolean(
      requireGoldStandardBenchmark ||
        story.media_house_benchmark ||
        story.reference_pack_used ||
        story.visual_director_plan ||
        story.visual_v4_director_plan ||
        story.director_plan ||
        env.GOLD_STANDARD_BENCHMARK_WARNINGS === "true",
    );
    if (hasBenchmarkContext) {
      const benchmark = runMediaHouseBenchmark({
        ...
        directorPlan:
          ...
          story.visual_v4_director_plan ||
          story.director_plan,
        requireGate: requireGoldStandardBenchmark,
      });
      if (benchmark.failures.length > 0) failures.push(...benchmark.failures);
      if (benchmark.warnings.length > 0) warnings.push(...benchmark.warnings);
    }
  } catch (err) {
    warnings.push(`gold_standard_benchmark_error:${err.code || "unknown"}`);
  }
  ...
  if (typeof story.tts_script === "string" && hasRiskyGtaViTtsText(story.tts_script)) {
    failures.push("risky_gta_vi_tts_script:tts_script");
  }
  const timestampPayload = await readStoryTimestampPayload(story, fs);
  if (timestampPayload) {
    if (hasGtaViTimestampProfileRisk(story, timestampPayload)) {
      failures.push("risky_gta_vi_timestamp_profile_stale");
    }
    if (hasGtaViTimestampAsrVerificationRisk(story, timestampPayload)) {
      failures.push("risky_gta_vi_timestamp_not_asr_verified");
    }
    for (const [field, value] of timestampTranscriptFields(timestampPayload)) {
      if (hasRiskyGtaViTtsText(value)) {
        failures.push(`risky_gta_vi_timestamp_transcript:${field}`);
        break;
      }
    }
  }
