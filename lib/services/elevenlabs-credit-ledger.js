...
function validateLedger(ledger) {
  ...
  for (const [keyHash, entry] of Object.entries(ledger.entries)) {
    const migratedFromKeyHash = String(
      entry?.migrated_from_key_hash || "",
    );
    if (
      ...
      !/^[a-f0-9]{64}$/.test(
        String(entry.input_fingerprint || ""),
      ) ||
      (migratedFromKeyHash &&
        (!/^[a-f0-9]{64}$/.test(migratedFromKeyHash) ||
          migratedFromKeyHash === keyHash ||
  ...
  for (const entry of Object.values(ledger.entries)) {
    ...
    if (!migratedFromKeyHash) continue;
    const source = ledger.entries[migratedFromKeyHash];
    if (
      !source ||
      source.migrated_from_key_hash ||
      !["provider_succeeded", "completed"].includes(
        source.state,
      ) ||
      source.input_fingerprint !== entry.input_fingerprint ||
      source.provider_result.relative_path !==
        entry.provider_result.relative_path ||
      source.provider_result.sha256 !==
        entry.provider_result.sha256 ||
      source.provider_result.byte_length !==
        entry.provider_result.byte_length
    ) {
      throw ledgerError("elevenlabs_credit_ledger_invalid");
    }
  }
