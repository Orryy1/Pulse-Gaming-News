...
const MEASURED_FLASH_PROFILE = Object.freeze({
  duration_band_id:
...
const SCRIPT_CONTROL_TOKEN_PATTERN =
  /\[(?:PAUSE|VISUAL(?:\s*:[^\]\r\n]*)?)\]/i;
...
function buildContract(
  ...
  if (narrationTiming) {
    const targetDurationSeconds =
      narrationTiming.evidence.target.duration_seconds;
    return Object.freeze({
      contract: {
        editorial_lane_id: "what_changes_for_players",
        hook_type: "direct",
        duration_band_id:
          MEASURED_FLASH_PROFILE.duration_band_id,
        ...
        target_duration_review: {
          ...
          reviewed_by:
            `SYSTEM_MEASUREMENT:${NARRATION_TIMING_EVIDENCE_SCHEMA_VERSION}`,
          reviewed_at:
            narrationTiming.evidence.reviewed_at,
          narration_timing_evidence_path:
            narrationTiming.path,
          narration_timing_evidence_sha256:
            narrationTiming.file_sha256,
          narration_provider_result_sha256:
            narrationTiming.evidence.provider_result.sha256,
          narration_audio_sha256:
            narrationTiming.evidence.audio.sha256,
          narration_audio_duration_seconds:
            narrationTiming.evidence.audio.duration_seconds,
          narration_alignment_end_seconds:
            narrationTiming.evidence.alignment.end_seconds,
          narration_alignment_sha256:
            narrationTiming.evidence.alignment.sha256,
          narration_tail_seconds:
            narrationTiming.evidence.target
              .narration_tail_seconds,
        },
      },
      target_duration_seconds: targetDurationSeconds,
...
function normaliseScript(value) {
  ...
  if (SCRIPT_CONTROL_TOKEN_PATTERN.test(supplied)) {
    fail(
...
function assessGovernedAutonomousBreakingScriptClaimSupport({
  script,
  confirmed_claims: confirmedClaims,
} = {}) {
  try {
    if (
      !Array.isArray(confirmedClaims) ||
      confirmedClaims.length < 1 ||
      confirmedClaims.length > 32
    ) {
      fail(
        "autonomous_breaking_candidate_claim_inventory_ambiguous",
      );
    }
    const claims = confirmedClaims.map((claim) => ({
      claim_key: text(claim?.claim_key),
      text: visibleClaimText(String(claim?.text ?? "")),
    }));
    if (
      claims.some((claim) => !claim.claim_key) ||
      new Set(claims.map((claim) => claim.claim_key)).size !==
        claims.length
    ) {
      fail(
        "autonomous_breaking_candidate_claim_inventory_ambiguous",
    ...
    const bindings = partitionScript(
      normaliseScript(script),
      claims,
    );
    return Object.freeze({
      verdict: "GREEN",
      blockers: Object.freeze([]),
      script_claim_bindings: Object.freeze(bindings),
    });
  } catch (error) {
    const blocker =
      error instanceof
      GovernedAutonomousBreakingCandidateContractCompilerError
        ? error.code
        : "autonomous_breaking_candidate_script_claim_assessment_failed";
    return Object.freeze({
      verdict: "HOLD",
      blockers: Object.freeze([blocker]),
      script_claim_bindings: Object.freeze([]),
    });
  }
}
...
function explicitEditorialFormats(story, extra) {
  return [
...
async function compileGovernedAutonomousBreakingCandidateContract(
  ...
  const storyExtra = assertLegacyProjectionAvailable(story);
  ...
  if (
    ...
    value.narration_timing_evidence !== null
  ) {
    let timingFile;
    try {
      timingFile = await readBoundJson({
        filePath: value.narration_timing_evidence?.path,
        expectedSha256:
          value.narration_timing_evidence?.file_sha256,
        roots,
        code:
          "autonomous_breaking_candidate_narration_timing_file",
      });
    } catch (error) {
      if (
        error?.code ===
        "autonomous_breaking_candidate_narration_timing_file_sha256_mismatch"
      ) {
        fail(
          "autonomous_breaking_candidate_narration_timing_file_hash_mismatch",
          error,
        );
      }
      throw error;
    ...
    try {
      ...
      fail(
        "autonomous_breaking_candidate_narration_timing_invalid",
        error,
      );
    }
    narrationTiming = Object.freeze({
      path: timingFile.path,
      file_sha256: timingFile.file_sha256,
      evidence,
    });
  }
  const { contract, target_duration_seconds: targetDuration } =
    buildContract(
      narrationTiming ? MEASURED_FLASH_PROFILE : profile,
      wordCount,
      scriptSha256,
      generated.raw,
      narrationTiming,
  ...
  const fastNewsLaneDecision =
    createGovernedFastNewsLaneDecision({
      ...
      source_evidence_sha256:
        primaryAssessment.packet_sha256,
      explicit_formats: explicitEditorialFormats(
        story,
        storyExtra,
      ),
    });
  ...
  const lockedIntake = {
    ...
    freshness: {
      ...
      stale_reframe_option: {
        allowed: false,
        reason:
          "Fast-news wording is bound to the current official-source evidence.",
      },
    },
    visual_brief: {
  ...
  const compiledCandidate = {
    ...
    candidate_revision: revisionBody,
    candidate_revision_sha256: candidateRevisionSha256,
  ...
  validateGovernedAutonomousCompiledCandidateBinding({
    ...
    source_evidence_sha256:
      primaryAssessment.packet_sha256,
    locked_intake_binding:
      compiledCandidate.locked_intake_binding,
    creative_package: creativePackage,
    runtime_policy: runtimePolicy,
  });
  return Object.freeze(compiledCandidate);
}
