...
function createGovernedAutonomousNarrationAdapter(
  ...
  return async function generateNarration(value) {
    ...
    const idempotencyKey =
      keyPrefix +
      sha256Bytes(
        Buffer.from(JSON.stringify(narrationIdentity), "utf8"),
      );
    ...
    const lease = await options.creditGovernor.preflight({
      ...
      legacyIdempotencyKeyHashes: [
        sha256Bytes(legacyIdempotencyKey),
      ],
    });
    let response;
    ...
    let providerResultEvidence =
      lease.replayAvailable === true
        ? lease.providerResultEvidence
        : null;
    if (lease.replayAvailable !== true) {
      try {
    ...
    if (
      !plainObject(providerResultEvidence) ||
      !/^[a-f0-9]{64}$/.test(
        text(providerResultEvidence.sha256).toLowerCase(),
      ) ||
      !Number.isInteger(
        Number(providerResultEvidence.byte_length),
      ) ||
      Number(providerResultEvidence.byte_length) <= 0 ||
      !text(providerResultEvidence.relative_path)
    ) {
      fail("autonomous_narration_provider_result_evidence_invalid");
    }
    ...
    return Object.freeze({
      provider: {
        ...
        provider_result: {
          relative_path: text(
            providerResultEvidence.relative_path,
          ),
          sha256: text(
            providerResultEvidence.sha256,
          ).toLowerCase(),
          byte_length: Number(
            providerResultEvidence.byte_length,
          ),
        },
      },
      ...
      network_used: lease.replayAvailable !== true,
    });
  };
}
