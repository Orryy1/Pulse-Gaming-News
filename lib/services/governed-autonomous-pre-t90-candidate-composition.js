...
function exactDatabaseStoryBinding(result, storyId, hashes) {
  const supplied =
    result.intake?.database_story_binding;
  if (!storyId.startsWith("official_")) {
    if (supplied !== undefined && supplied !== null) {
      fail("pre_t90_database_story_binding_unexpected");
    }
    return null;
  }
  if (!object(supplied)) {
    fail("pre_t90_database_story_binding_required", {
      story_id: storyId,
    });
  }
  try {
    return validateGovernedAutonomousDatabaseStoryBinding(
      supplied,
      {
        canonical_story_id: storyId,
        final_script_sha256: hashes.script_sha256,
      },
    );
  } catch (error) {
    fail("pre_t90_database_story_binding_invalid", {
      story_id: storyId,
...
function composeCandidate(candidate, role, timing) {
  ...
  const databaseStoryBinding = exactDatabaseStoryBinding(
    result,
    storyId,
    hashes,
  );
...
function validateResultCandidate(candidate, role, timing) {
  ...
  } else if (candidate.database_story_binding !== null) {
    fail("pre_t90_database_story_binding_unexpected");
  }
  assertFalse(
