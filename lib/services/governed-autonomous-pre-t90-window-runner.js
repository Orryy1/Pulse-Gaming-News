...
function sourceRequestFor({
  ...
  return {
    ...
    artifacts: {
      ...
      single_owner_proof: control.single_owner_proof,
    },
    owned_visual_assets:
...
async function runGovernedAutonomousPreT90WindowPreparation(
  ...
  const applyComposition =
    ...
    applyGovernedAutonomousPreT90CandidateComposition;
  const withPublisherLease =
    options.runWithPublisherLease ||
    runWithPublisherLease;

  let leased;
  try {
    leased = await withPublisherLease({
      leases: options.repos.runtimeLeases,
      channelId: CHANNEL_ID,
      operation: "governed_autonomous_pre_t90_window_preparation",
      metadata: {
        scheduled_for: input.scheduled_for,
        primary_story_id: indexedPrimary.receipt.story_id,
        standby_story_id: indexedStandby.receipt.story_id,
      },
      task: async ({ assertHealthy, lease }) => {
        if (
          ...
          fail("pre_t90_runner_publisher_lease_invalid");
        }
    ...
    fail("pre_t90_runner_publisher_lease_failed", error);
  }
  if (
    !object(leased) ||
    leased.marker !==
    ...
    fail("pre_t90_runner_publisher_lease_unavailable");
  }
