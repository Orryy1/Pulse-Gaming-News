...
function fail(code) {
  throw new GovernedAutonomousProductionCoordinatorError(code);
}
...
function normaliseRequest(value) {
  ...
  const storyId = (() => {
    ...
    try {
      parsed = JSON.parse(
        fs.readFileSync(value.locked_intake.inventory_path, "utf8"),
      );
    } catch {
      fail("autonomous_production_inventory_unreadable");
    }
    const candidate = text(parsed?.story?.id);
    if (!STORY_ID_PATTERN.test(candidate)) {
...
async function materialiseGovernedAutonomousOfficialCandidate(
  ...
  try {
    ...
    const narrationPlan = await buildGovernedNarrationPlan({
      ...
      licenceAttestedAt: request.generated_at,
