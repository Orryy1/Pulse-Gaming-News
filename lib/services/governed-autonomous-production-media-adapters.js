...
function svgForScene(scene) {
  ...
  const storyMark = `PULSE / ${text(scene?.role).replaceAll("_", " ").toUpperCase()}`;
  ...
  return [
    ...
    `  <text x="${AUTHORED_TEXT_LEFT}" y="${AUTHORED_TEXT_TOP + 50}" fill="${accent}" font-family="Arial, sans-serif" font-size="30" font-weight="700" letter-spacing="5">${layout}</text>`,
    `  ${headlineBlock.svg}`,
    `  ${supportingBlock.svg}`,
