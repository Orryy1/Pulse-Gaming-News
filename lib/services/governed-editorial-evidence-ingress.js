...
const ATTEMPT_REVISION_SCHEMA =
  "pulse-governed-editorial-evidence-attempt-revision-v1";
const DEFAULT_MAX_AGE_HOURS = 7 * 24;
const DEFAULT_REFRESH_BUCKET_HOURS = 6;
...
function buildEvidenceAttemptRevision({
  fingerprint,
  now,
  bucketHours = DEFAULT_REFRESH_BUCKET_HOURS,
} = {}) {
  const generatedAt = isoTimestamp(now);
  const normalisedFingerprint = text(
    fingerprint,
  ).toLowerCase();
  const normalisedBucketHours = Number(
    bucketHours,
  );
  if (!generatedAt) {
    throw new Error(
      "governed_editorial_evidence_attempt_revision_now_invalid",
    );
  }
  if (!/^[a-f0-9]{64}$/.test(normalisedFingerprint)) {
    throw new Error(
      "governed_editorial_evidence_attempt_revision_fingerprint_invalid",
    );
  }
  if (
    !Number.isInteger(normalisedBucketHours) ||
    normalisedBucketHours < 1 ||
    normalisedBucketHours > 24
  ) {
    throw new Error(
      "governed_editorial_evidence_attempt_revision_bucket_invalid",
    );
  }
  const bucketMs =
    normalisedBucketHours * 60 * 60 * 1000;
  const bucketStartedMs =
    Math.floor(Date.parse(generatedAt) / bucketMs) *
    bucketMs;
  const contract = {
    schema_version: ATTEMPT_REVISION_SCHEMA,
    evidence_fingerprint_sha256: normalisedFingerprint,
    bucket_hours: normalisedBucketHours,
    bucket_started_at: new Date(
      bucketStartedMs,
    ).toISOString(),
    bucket_ends_at: new Date(
      bucketStartedMs + bucketMs,
    ).toISOString(),
  };
  return {
    ...contract,
    revision_sha256: sha256(contract),
  };
}

function evidenceAttemptWorkIdentity(request = {}) {
  ...
  return sha256({
    ...
    channel_id: text(request.channel_id),
    story_id: text(request.story_id),
    payload: stablePayload,
    priority: Number(request.priority),
    requires_gpu:
      request.requires_gpu === true ||
      Number(request.requires_gpu) === 1,
    max_attempts: Number(request.max_attempts),
  });
}

function sameEvidenceAttemptWork(existing, request) {
  if (!existing) return false;
  return (
    text(existing.idempotency_key) ===
      text(request.idempotency_key) &&
    evidenceAttemptWorkIdentity(existing) ===
      evidenceAttemptWorkIdentity(request)
  );
}
...
function enqueueGovernedEditorialEvidence({
  ...
  const attemptRevision =
    attemptRevisionHours === null ||
    attemptRevisionHours === undefined
      ? null
      : buildEvidenceAttemptRevision({
          fingerprint,
          now: assessment.generated_at,
          bucketHours: attemptRevisionHours,
        });
  const payload = {
    schema_version: INGRESS_SCHEMA,
  ...
  const request = {
    ...
    idempotency_key:
      attemptRevision
        ? `governed-editorial-evidence:v2:${assessment.story.id}:` +
          `${fingerprint}:${attemptRevision.revision_sha256}`
  ...
  if (
    ...
    if (existing) {
      if (!sameEvidenceAttemptWork(existing, request)) {
      ...
      return {
        ...
        attempt_revision_sha256:
          attemptRevision?.revision_sha256 || null,
        job: existing,
      };
    }
  }
  ...
  return {
    queued: true,
    reason: attemptRevision
      ? "governed_editorial_evidence_revision_enqueued"
      : "governed_editorial_evidence_enqueued",
    assessment,
    fingerprint_sha256: fingerprint,
    attempt_revision_sha256:
      attemptRevision?.revision_sha256 || null,
...
module.exports = {
  ATTEMPT_REVISION_SCHEMA,
  DEFAULT_MAX_AGE_HOURS,
  DEFAULT_REFRESH_BUCKET_HOURS,
