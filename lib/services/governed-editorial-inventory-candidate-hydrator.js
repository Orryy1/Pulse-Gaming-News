...
const MAXIMUM_REPAIR_CLAIMS = 32;
const MAXIMUM_REPAIR_CLAIM_CHARACTERS = 4_000;
const MAXIMUM_REPAIR_CONTEXT_CHARACTERS = 12_000;
...
function boundedConfirmedClaims(sourcePacket) {
  if (
    sourcePacket?.verdict !== "OFFICIAL_CONFIRMED" ||
    sourcePacket?.verification_status !== "CONFIRMED" ||
    sourcePacket?.confirmation_basis !== "official_first_party" ||
    sourcePacket?.verified_for_planning !== true ||
    !Array.isArray(sourcePacket?.blockers) ||
    sourcePacket.blockers.length !== 0
  ) {
    return [];
  }
  const groups = Array.isArray(sourcePacket.confirmed_claims)
    ? sourcePacket.confirmed_claims
    : [];
  if (
    groups.length < 1 ||
    groups.length > MAXIMUM_REPAIR_CLAIMS
  ) {
    return [];
  }
  const claims = [];
  ...
  for (const group of groups) {
    ...
    const exactEvidence = evidence[0];
    const claimText = String(exactEvidence?.text ?? "");
    if (
      (text(exactEvidence?.claim_key) &&
        text(exactEvidence?.claim_key) !== claimKey) ||
      text(exactEvidence?.source_class).toUpperCase() !==
        "OFFICIAL_FIRST_PARTY" ||
      !claimText.trim() ||
      claimText.length > MAXIMUM_REPAIR_CLAIM_CHARACTERS ||
      normaliseSha256(exactEvidence?.claim_text_sha256) !==
        sha256(Buffer.from(claimText, "utf8"))
    ) {
      return [];
    }
    totalCharacters += claimKey.length + claimText.length;
    if (totalCharacters > MAXIMUM_REPAIR_CONTEXT_CHARACTERS) {
      return [];
    }
...
async function hydrateGovernedEditorialInventoryCandidates({
  ...
  for (const candidate of exactCandidates) {
    ...
    hydrated.push({
      ...
      confirmed_claims: boundedConfirmedClaims(
        assessment.source_packet,
      ),
      source_evidence_sha256:
        bindings.source_evidence.canonical_sha256,
