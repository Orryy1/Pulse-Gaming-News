...
async function assessRegistry(registryPath, rootDir, allowedRoots) {
  ...
  if (
    ...
    blockers.push("editorial_inventory_generated_at_invalid");
  }
  ...
  for (const name of referenceNames) {
    observed[name] = await observeBoundFile({
      ...
      allowedRoots,
      prefix: name,
      parseJson: name === "weekly_source_evidence",
    });
    blockers.push(...observed[name].blockers);
  }
  const publicationEvidence =
    observed.weekly_source_evidence?.value || {};
  if (
    text(publicationEvidence.story_id) !==
    storyAssessment.story.id
  ) {
    blockers.push(
      "weekly_source_evidence_story_id_mismatch",
    );
  }
  if (
    text(publicationEvidence.source_url) !==
    storyAssessment.story.primary_source_url
  ) {
    blockers.push(
      "weekly_source_evidence_primary_url_mismatch",
    );
  }
  const publicationClaims = [
    ...(Array.isArray(publicationEvidence.claims)
      ? publicationEvidence.claims
      : []),
    ...(Array.isArray(
      publicationEvidence.official_source_snapshot?.claims,
    )
      ? publicationEvidence.official_source_snapshot.claims
      : []),
  ];
  const identityBinding = assessStorySubjectEvidenceBinding({
    story: storyAssessment.story,
    claims: publicationClaims,
    source_urls: [
      publicationEvidence.source_url,
      publicationEvidence.official_source_snapshot?.source_url,
    ],
  });
  if (!identityBinding.bound) {
    blockers.push(
...
function governedRevisionLineage(entry, rootDir) {
  const registryPath = path.resolve(text(entry?.registry_path));
  const inventoryDir = path.dirname(registryPath);
  if (path.basename(inventoryDir).toLowerCase() !== "inventory") {
    return null;
  }
  const revisionOrStoryDir = path.dirname(inventoryDir);
  const possibleRevisionSha256 = path
    .basename(revisionOrStoryDir)
    .toLowerCase();
  const possibleRevisionsDir = path.dirname(revisionOrStoryDir);
  const isRevision =
    SHA256_PATTERN.test(possibleRevisionSha256) &&
    path.basename(possibleRevisionsDir).toLowerCase() ===
      "revisions";
  const lineageRoot = isRevision
    ? path.dirname(possibleRevisionsDir)
    : revisionOrStoryDir;
  if (!isContained(rootDir, lineageRoot)) return null;
  return {
    lineage_root: lineageRoot,
    lineage_key: pathIdentity(lineageRoot),
    revision_sha256: isRevision ? possibleRevisionSha256 : null,
    kind: isRevision ? "GOVERNED_REVISION" : "LEGACY_INVENTORY",
  };
}

function compareCurrentRevision(left, right, rootDir) {
  const timestampDifference =
    Date.parse(right.generated_at) - Date.parse(left.generated_at);
  if (timestampDifference) return timestampDifference;
  const lineageRank = (entry) =>
    governedRevisionLineage(entry, rootDir)?.kind ===
    "GOVERNED_REVISION"
      ? 1
      : 0;
  const lineageDifference =
    lineageRank(right) - lineageRank(left);
  if (lineageDifference) return lineageDifference;
  const hashDifference = String(
    right.registry_file_sha256 || "",
  ).localeCompare(String(left.registry_file_sha256 || ""));
  if (hashDifference) return hashDifference;
  return right.registry_path.localeCompare(left.registry_path);
}

function selectCurrentStoryRevisions(assessed, rootDir) {
  ...
  for (const entry of assessed) {
    ...
    const group = byStoryId.get(storyId) || [];
    group.push(entry);
    byStoryId.set(storyId, group);
  }

  for (const group of byStoryId.values()) {
    if (group.length === 1) {
      selected.push(group[0]);
      continue;
    }
    const lineages = group.map((entry) =>
      governedRevisionLineage(entry, rootDir),
    );
    const lineageKey = lineages[0]?.lineage_key;
    const oneGovernedLineage =
      Boolean(lineageKey) &&
      lineages.every(
        (lineage) => lineage?.lineage_key === lineageKey,
      );
    if (!oneGovernedLineage) {
    ...
    const current = group
      ...
      .sort((left, right) =>
        compareCurrentRevision(left, right, rootDir),
      )[0];
    if (current) {
      const currentLineage = governedRevisionLineage(
        current,
        rootDir,
      );
      current.revision_lineage = {
        lineage_root: lineages[0].lineage_root,
        revision_sha256: currentLineage.revision_sha256,
        physical_revision_count: group.length,
      };
      selected.push(current);
    }
    selected.push(
      ...group.filter((entry) => entry.blockers.length > 0),
    );
  }
...
async function scanGovernedEditorialInventory({
  ...
  const currentAssessments = selectCurrentStoryRevisions(
    assessed,
    resolvedRoot,
  );
  const entries = currentAssessments
    .filter((entry) => entry.blockers.length === 0)
    .sort((left, right) =>
      left.story.id.localeCompare(right.story.id),
    );
  const rejected = currentAssessments
    .filter((entry) => entry.blockers.length > 0)
    .sort((left, right) =>
