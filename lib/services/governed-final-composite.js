...
function validateCombinedOwnedMotionManifest({
  ...
  const opening = manifest?.opening_treatment || {};
  const openingRecord = resolveManifestRecord({
    baseDir: manifestDir,
    record: opening.asset,
    prefix: "owned_motion_opening_treatment_asset",
  });
  const openingAsset = resolvedAssets.find(
    (asset) =>
      samePath(asset.path, openingRecord.path) &&
      asset.sha256 === openingRecord.sha256,
  );
  const firstImageAsset = resolvedAssets.find(
    (asset) => asset.media_type === "image",
  );
  const openingErrors = [];
  if (
    text(opening.source) !==
    "owned_motion_opening_treatment_v1"
  ) {
    openingErrors.push("owned_motion_opening_treatment_source_invalid");
  }
  if (!text(opening.first_frame_text)) {
    openingErrors.push("owned_motion_first_frame_text_required");
  }
  if (
    !openingAsset ||
    openingAsset.media_type !== "image" ||
    text(opening.role).toLowerCase() !== openingAsset.role ||
    firstImageAsset !== openingAsset
  ) {
    openingErrors.push(
      "owned_motion_opening_treatment_asset_binding_invalid",
    );
  }
  if (openingErrors.length) {
    throw new GovernedFinalCompositeError(openingErrors);
  }
  const openingTreatment = {
    role: openingAsset.role,
    first_frame_text: text(opening.first_frame_text),
    source: text(opening.source),
    asset: openingAsset,
  };
...
async function deriveCombinedOwnedMotionManifest({
  ...
  const sourceOpeningAsset = resolveManifestRecord({
    baseDir: sourceDir,
    record: manifest?.opening_treatment?.asset,
    prefix: "source_owned_motion_opening_treatment_asset",
  });
  const combined = {
    ...manifest,
    generated_at: generatedAt,
    opening_treatment: {
      ...manifest.opening_treatment,
      asset: {
        path: normalisePathForRecord(
          targetDir,
          sourceOpeningAsset.path,
        ),
        sha256: sourceOpeningAsset.sha256,
      },
    },
    combination: {
      mode: "LOCAL_PROOF",
...
function validateGovernedNarrationManifest({
  ...
  const narratorSpeed = Number(manifest?.narration?.speed);
  const expectedNarratorVersion = deriveNarratorVersion({
    generatorIdentity: manifest?.generator_identity,
    provider,
    modelId: manifest?.narration?.model_id,
    voiceId: manifest?.narration?.voice_id,
    speed: narratorSpeed,
  });
  if (
    !Number.isFinite(narratorSpeed) ||
    narratorSpeed < 0.7 ||
    narratorSpeed > 1.2 ||
    text(manifest?.narration?.narrator_version) !==
      expectedNarratorVersion
  ) {
    errors.push("narration_narrator_version_binding_invalid");
  }
...
function deriveRendererEvidence(ownedMotion, sourceMedia = null) {
  ...
  return {
    timing: {
      ...
      first_frame_text:
        ownedMotion.openingTreatment.first_frame_text,
...
async function materialiseGovernedFinalCompositeToStage(
  ...
  const experimentDimensions =
    story.intake?.experiment_dimensions;
  if (
    experimentDimensions !== undefined &&
    (!experimentDimensions ||
      typeof experimentDimensions !== "object" ||
      Array.isArray(experimentDimensions) ||
      typeof experimentDimensions.eligible !== "boolean")
  ) {
    throw new GovernedFinalCompositeError(
      "controlled_experiment_observation_experiment_eligibility_required",
    );
  }
  if (experimentDimensions !== undefined) {
    const expectedFields = (
      experimentDimensions.eligible
        ? [
            "eligible",
            "experiment_id",
            "matrix_version",
  ...
  const hasGovernedStoryTarget = Number.isFinite(
    story.targetDurationSeconds,
  );
  const finalDurationSeconds = hasGovernedStoryTarget
    ? story.targetDurationSeconds
    : DURATION_SECONDS;
  const sourceVisualDurationSeconds = hasGovernedStoryTarget
    ? story.targetDurationSeconds
    : HYPERFRAMES_DURATION_SECONDS;
  ...
  const controlledExperimentObservationPath = path.join(
    outputRoot,
  ...
  const invocation = buildFfmpegInvocation({
    ...
    preserveRenderedCaptions,
    governedStoryTarget: hasGovernedStoryTarget,
    ffmpegPath: options.ffmpegPath || "ffmpeg",
  });
  await writeAtomic(
    filterPath,
  ...
  let controlledExperimentObservation = null;
  let controlledExperimentObservationRecord = null;
  if (experimentDimensions !== undefined) {
    ...
    try {
      const eligible =
        experimentDimensions?.eligible === true;
        ...
        buildControlledExperimentObservation(
          {
            ...
            experiment: eligible
              ? {
                  eligible: true,
                  experiment_id:
                    experimentDimensions.experiment_id,
                  matrix_version:
                    experimentDimensions.matrix_version,
                  expected_cell_id:
                    experimentDimensions.expected_cell_id,
              ...
              : {
                  ...
                  ineligibility_reason:
                    experimentDimensions?.ineligibility_reason,
                },
            creative_static: eligible
              ? {
                  runtime_seconds:
                    finalProbe.technical.duration_seconds,
                  hook_type:
                    story.intake?.contract?.hook_type,
                  narrator_version:
                    governedNarration.narrator_version,
                  first_frame_text:
                    rendererManifest.timing.first_frame_text,
                  motion_ratio:
                    rendererEvaluation.evidence.motion_ratio,
                  topic: experimentDimensions.topic,
                  game: experimentDimensions.game,
                  subject_platform:
                    experimentDimensions.subject_platform,
                  source_type: story.intake?.source_type,
                  consequence_lane:
                    story.intake?.contract
                      ?.editorial_lane_id,
                  renderer_version:
                    rendererManifest.renderer.version,
                  qa_result:
                    qaReport.verdict.toLowerCase(),
                }
              : null,
      ...
      throw new GovernedFinalCompositeError(
        error?.code ||
          "controlled_experiment_observation_invalid",
      );
    }
    await writeJsonAtomic(
    ...
    controlledExperimentObservationRecord = {
      path: normalisePathForRecord(
        outputRoot,
        controlledExperimentObservationPath,
      ),
      file_sha256: hashFile(
        controlledExperimentObservationPath,
      ),
      observation_sha256:
        controlledExperimentObservation.observation_sha256,
      eligible:
        controlledExperimentObservation.experiment.eligible,
    };
  }
  ...
  return {
    ...
    ...(controlledExperimentObservationRecord
      ? {
          controlled_experiment_observation_path:
            controlledExperimentObservationPath,
          controlled_experiment_observation_file_sha256:
            controlledExperimentObservationRecord.file_sha256,
          controlled_experiment_observation_sha256:
            controlledExperimentObservationRecord.observation_sha256,
        }
      : {}),
