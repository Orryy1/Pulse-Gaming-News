...
const {
  ATTRIBUTION_TEXT: SOURCE_MEDIA_ATTRIBUTION_TEXT,
...
function validateSourceMediaProjectBindings({
  ...
  try {
    rawManifest = JSON.parse(
      fs.readFileSync(sourceMedia.manifest_path, "utf8"),
    );
  } catch {
    throw new GovernedFinalCompositeError(
      "source_media_manifest_validation_failed",
  ...
  const rawComponents = Array.isArray(rawManifest?.components)
    ? rawManifest.components
    : [];
  const errors = [];
  if (!domHtml.includes(SOURCE_MEDIA_ATTRIBUTION_TEXT)) {
  ...
  const bindings = sourceMedia.components.map((component) => {
    ...
    const rawComponent = rawComponents.find(
      (candidate) =>
        text(candidate?.component_id) === component.component_id,
    );
    const usage = rawComponent?.editorial?.usage_seconds;
  ...
  if (sourceMedia) {
    ...
    if (
      text(manifest?.combination?.source_media_policy)
        .toUpperCase() !== SOURCE_MEDIA_POLICY
    ) {
      sourceErrors.push(
...
async function materialiseGovernedFinalCompositeToStage(
  ...
  let captionSafeZone;
  try {
    captionSafeZone = validateAssCaptionSafeZone({
      ass,
