...
function resolveOpeningTreatment(intake, role) {
  const explicit = cleanText(
    intake?.story?.visual_brief?.opening_treatment
      ?.first_frame_text,
  );
  const roleText = {
    hook_slam:
      explicit ||
      cleanText(intake?.story?.hook) ||
      cleanText(intake?.story?.title),
    shield_lock: "NEW JOB REVEALED",
    mode_switch: "COMBAT MODE",
    branching_balance: "STORY + SCALING",
    raid_grid: "EIGHT-PLAYER RAID",
    platform_release: "NEW PLATFORM",
  };
  const firstFrameText = cleanText(roleText[role]);
  if (!firstFrameText) {
    throw new Error("owned_motion_opening_treatment_text_required");
  }
  return {
    role,
    first_frame_text: firstFrameText,
...
function buildOwnedMotionPlan({
  ...
  const openingTreatment = resolveOpeningTreatment(
    intake,
    sceneRoles[0],
  );
  const assets = [
  ...
  return {
    ...
    opening_treatment: openingTreatment,
    video: {
      width: WIDTH,
      height: HEIGHT,
...
function hookSlamSvg(intake, plan) {
  ...
  const hook = plan.opening_treatment.first_frame_text;
...
async function materializeOwnedMotion({
  ...
  try {
    ...
    const openingAsset = manifestAssets.find(
      (asset) =>
        asset.media_type === "image" &&
        asset.role === plan.opening_treatment?.role,
    );
    if (
      !openingAsset ||
      visualInputs[0]?.path !== openingAsset.path ||
      visualInputs[0]?.sha256 !== openingAsset.sha256
    ) {
      throw new Error(
