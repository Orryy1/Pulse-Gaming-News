...
function orderProjectSceneLayers(sceneAssets) {
  return [
    ...sceneAssets.filter(
      (asset) => asset.scene.role === "owned_motion_backbone",
    ),
    ...sceneAssets.filter(
      (asset) => asset.scene.role !== "owned_motion_backbone",
    ),
...
function renderHyperframesIndex({
  ...
  const clips = sceneAssets
    .map((asset, index) => {
      ...
      const layerClass =
        scene.role === "owned_motion_backbone"
          ? "programme-backbone"
          : "programme-foreground";
      const common = [
        `id="${htmlEscape(scene.asset_id)}"`,
        `class="clip ${layerClass}"`,
  ...
  return [
    ...
    "      .programme-backbone { z-index: 0; }",
    "      .programme-foreground { z-index: 1; }",
    "    </style>",
    "  </head>",
    "  <body>",
...
async function materialiseHyperframesProject({
  ...
  const layeredProjectSceneAssets =
    orderProjectSceneLayers(projectSceneAssets);
  const primaryFiles = [
    {
      name: "index.html",
      bytes: Buffer.from(
        renderHyperframesIndex({
          ...
          sceneAssets: layeredProjectSceneAssets,
        }),
        "utf8",
      ),
    },
  ...
  for (const asset of layeredProjectSceneAssets) {
    records.push({
  ...
  const manifest = {
    ...
    authored_scene_assets: layeredProjectSceneAssets.map((asset) => ({
