...
function renderHyperframesIndex({
  ...
  const clips = sceneAssets
    .map((asset, index) => {
      ...
      const source = `../${slashPath(asset.relative_path)}`;
      const common = [
...
async function materialiseHyperframesProject({
  ...
  const manifest = {
    ...
    authored_scene_assets: sceneAssets.map((asset) => ({
      asset_id: asset.scene.asset_id,
      role: asset.scene.role,
      path: relativeRecordPath(projectRoot, asset.path),
      sha256: asset.sha256,
...
async function materialiseGovernedOwnedProgrammePack(
  ...
  const stageRoot = path.join(
    parentRoot,
    `.${storyId}.owned-programme-pack-staging-${process.pid}-${crypto.randomUUID()}`,
  );
  let staged;
  try {
    if (fs.existsSync(storyRoot)) {
