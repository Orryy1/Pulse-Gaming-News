...
function validateRenderer({
  ...
  if (
    !timestampInput ||
    !narration.timestamps?.sha256 ||
    timestampInput.sha256 !== narration.timestamps.sha256
  ) {
    errors.push("renderer_word_timestamps_binding_invalid");
  } else {
    const timestampValue = parseJsonBytes(
      fs.readFileSync(timestampInput.absolutePath),
      "renderer_word_timestamps_invalid_json",
    );
    if (
      timestampValue?.schema_version !== "pulse-word-timestamps-v1" ||
      text(timestampValue?.story_id) !== story.storyId
    ) {
      errors.push("renderer_word_timestamps_schema_invalid");
    }
  }
  if (
