...
function validateOptionalControlledExperimentObservation({
  ...
  const record = manifest?.controlled_experiment_observation;
  ...
  if (
    ...
    throw new GovernedPublicationReviewError(
      "controlled_experiment_observation_record_fields_invalid",
    );
  }
  const observationFile = resolveBoundJson({
    baseDir: manifestDir,
    record: {
      path: record.path,
      sha256: record.file_sha256,
    },
    prefix: "controlled_experiment_observation",
  });
  const storyIntake = resolveBoundJson({
    baseDir: manifestDir,
    record: manifest.story_intake,
    prefix: "controlled_experiment_story_intake",
  });
  const narrationManifest = resolveBoundJson({
    baseDir: manifestDir,
    record: manifest.governed_narration_manifest,
    prefix: "controlled_experiment_narration_manifest",
  });
  try {
    ...
    if (
      ...
      throw new GovernedPublicationReviewError(
        "controlled_experiment_observation_record_self_sha256_mismatch",
      );
    }
    return {
    ...
    if (error instanceof ControlledExperimentObservationError) {
      throw new GovernedPublicationReviewError(error.code);
    }
    throw error;
  }
}
...
async function validatePublicationReviewManifest({
  ...
  const controlledExperimentObservation =
    validateOptionalControlledExperimentObservation({
      ...
      qaReport,
      renderer,
      rendererCanonicalSha256: canonicalRendererSha,
    });
  ...
  const preflightEvidence = {
    ...
    ...(controlledExperimentObservation
      ? {
          story_intake_sha256:
            controlledExperimentObservation.story_intake.sha256,
          narration_manifest_sha256:
            controlledExperimentObservation.narration_manifest
              .sha256,
          renderer_manifest_file_sha256: renderer.sha256,
          controlled_experiment_observation:
            controlledExperimentObservation.observation,
...
function buildReviewProvenance({
  ...
  return {
    ...
    ...(validation.licensedAudio
      ...
      : {}),
    ...(validation.controlledExperimentObservation
      ? {
          controlled_experiment_observation: {
            path:
              validation.controlledExperimentObservation.path,
            file_sha256:
              validation.controlledExperimentObservation
                .file_sha256,
            observation_sha256:
