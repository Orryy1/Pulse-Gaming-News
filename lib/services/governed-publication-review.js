...
function validateSourceMediaRightsEvidence({
  ...
  if (
    ...
    text(value?.attribution_text) !== ATTRIBUTION_TEXT ||
  ...
  if (
    text(descriptionEvidence?.notice) !== ATTRIBUTION_TEXT ||
  ...
  if (sourceMedia) {
    for (const component of sourceMedia.components) {
      ...
      if (
        ...
        text(item?.attribution_text) !== ATTRIBUTION_TEXT
      ) {
    ...
    if (
      ...
      text(motionItem?.attribution_decision).toUpperCase() !==
        "REQUIRED_AND_SUPPLIED" ||
      text(motionItem?.attribution_text) !== ATTRIBUTION_TEXT
    ) {
...
async function validatePublicationReviewManifest({
  ...
  if (sourceMediaPolicy === LICENSED_SOURCE_MEDIA_POLICY) {
    if (
      manifest?.policy?.third_party_media_used !== true ||
      manifest?.policy?.attribution_required !== true
    ) {
