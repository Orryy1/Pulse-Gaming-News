...
const REVIEWED_25_SECOND_VIDEO_FAILURE =
...
async function resolveGovernedReviewedContentQaAuthority({
  ...
  const rendererDurationSeconds = Number(
    rendererManifest?.output?.duration_seconds,
  );
  ...
  if (
    !Number.isFinite(rendererDurationSeconds) ||
    rendererDurationSeconds <= 0
  ) {
    blockers.push("governed_review_renderer_duration_invalid");
  }
  if (
  ...
  try {
    ...
    if (
      !Number.isFinite(rendererDurationSeconds) ||
      rendererDurationSeconds < contract.min_seconds ||
      rendererDurationSeconds > contract.max_seconds
    ) {
      blockers.push("governed_review_renderer_duration_outside_contract");
    }
  } catch {
  ...
  const authority = Object.freeze({
    ...
    videoRuntime: Object.freeze({
      durationSeconds: rendererDurationSeconds,
...
function reconcileGovernedReviewedVideoQa(result, authority) {
  ...
  if (originalResult === "fail" && failures.length === 0) {
    retained.push("video_qa_unclassified_failure");
  } else if (
    originalResult !== "pass" &&
    originalResult !== "warn" &&
    originalResult !== "fail"
  ) {
    retained.push("video_qa_result_invalid");
  }
  const runtime = authority?.videoRuntime;
  const exactReviewed25SecondAuthority =
    VALIDATED_AUTHORITIES.has(authority) &&
    runtime?.durationSeconds === 25 &&
    runtime?.durationBandId === "what_changes_short_25_32" &&
    runtime?.minSeconds === 25 &&
    runtime?.maxSeconds === 32;
  for (const failure of failures) {
    if (
      exactReviewed25SecondAuthority &&
      failure === REVIEWED_25_SECOND_VIDEO_FAILURE
    ) {
      resolved.push(failure);
      continue;
    }
