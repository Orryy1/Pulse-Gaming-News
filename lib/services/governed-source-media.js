...
function validateGovernedSourceMediaManifest({
  ...
  if (manifest?.schema_version === GAME_MEDIA_ADMISSION_SCHEMA) {
    try {
      const admission = validateGovernedGameMediaAdmission({
        manifestPath: manifestFile.path,
        expectedManifestSha256: manifestFile.sha256,
        expectedStoryId,
        validationBoundaryAt,
      });
      return {
        ...
        rights_review: {
          ...
          evidence: {
            ...
            publisher: "PER_ASSET",
            licence_evidence_url: null,
          },
        },
        components: admission.assets.map((asset) => ({
          component_id: asset.asset_id,
          media_type: asset.media_type,
          source: {
            page_url: asset.source.page_url || null,
            direct_media_url:
              asset.source.direct_media_url || null,
            publisher: asset.game.publisher,
          },
          rights_basis: asset.rights_basis,
          licence_evidence_url:
            asset.permission.evidence_url || null,
          review_status: "ACCEPTED",
          attribution: {
            required: Boolean(asset.attribution),
            text: asset.attribution?.text || "",
            delivery: asset.attribution?.delivery || [],
          },
          editorial: {
            purpose: asset.transformation.purpose,
            treatment:
              asset.transformation.operations.join("+"),
            operations: asset.transformation.operations,
            third_party_music_used: false,
            source_audio_disposition:
              asset.transformation.source_audio_disposition,
            usage_seconds:
              asset.transformation.usage_seconds,
          },
          asset: {
            path: asset.materialised.path,
            sha256: asset.materialised.sha256,
            size_bytes: asset.materialised.size_bytes,
          },
          game: asset.game,
          media_kind: asset.media_kind,
          editorial_role: asset.editorial_role,
          admission_binding: asset.binding,
        })),
        game_media_admission: admission,
      };
    } catch (error) {
      if (error instanceof GovernedGameMediaAdmissionError) {
        throw new GovernedSourceMediaError(error.codes);
      }
      throw error;
    }
  }
