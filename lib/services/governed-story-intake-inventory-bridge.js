...
async function materializeGovernedLockedStoryIntakeFromInventory({
  ...
  const inventoryStoryId = text(registry?.story?.id);
  const exactIdentityUrl = text(canonicalIdentityUrl);
  const registryPrimarySourceUrl = text(
    registry?.story?.primary_source_url,
  );
  const inventoryStoryIdIsValid =
    /^[A-Za-z0-9._:-]{1,128}$/.test(inventoryStoryId);
  const canonicalStoryId =
    `official_${canonicalHash(exactIdentityUrl)}`;
  const canonicalIdentityMatchesOfficialId =
    canonicalStoryId === inventoryStoryId;
  const canonicalIdentityMatchesRegistrySource =
  ...
  if (
    !inventoryStoryIdIsValid ||
    !canonicalUrl(exactIdentityUrl) ||
    parsedIdentityUrl?.protocol !== "https:" ||
    parsedIdentityUrl?.username ||
    parsedIdentityUrl?.password ||
    (
      !canonicalIdentityMatchesOfficialId &&
      !canonicalIdentityMatchesRegistrySource
    )
  ) {
    throw new GovernedStoryIntakeInventoryBridgeError([
  ...
  const candidate = hydrated.candidates.find(
    (item) =>
      item.lane_id === "breaking_short" &&
      item.story_id === inventoryStoryId &&
      item.governed_editorial_inventory_bindings,
  );
  if (!candidate || hydrated.rejected.length) {
    throw new GovernedStoryIntakeInventoryBridgeError([
  ...
  if (
    text(primaryPacket.value.story_id) !== inventoryStoryId ||
  ...
  if (
    text(publicationSource.value.story_id) !== inventoryStoryId ||
  ...
  for (
    ...
    if (
      text(packet.value.story_id) !== inventoryStoryId ||
  ...
  const proofBase = {
    ...
    legacy_story_id: inventoryStoryId,
    story_id: canonicalStoryId,
    stages: {
      exact_ready_inventory_binding: "PASS",
