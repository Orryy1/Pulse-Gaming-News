...
const BREAKING_NEWS_HIGH_CADENCE_CONTRACT = Object.freeze({
  editorial_lane_id: "what_changes_for_players",
  hook_type: "direct",
  duration_band_id:
    "what_changes_breaking_high_cadence_35_42",
});
const BREAKING_NEWS_FLASH_CONTRACT = Object.freeze({
  editorial_lane_id: "what_changes_for_players",
  hook_type: "direct",
  duration_band_id:
    "what_changes_breaking_flash_18_24",
});
const BREAKING_NEWS_FLASH_MIN_WORDS = 36;
const BREAKING_NEWS_FLASH_MAX_WORDS = 48;
const BREAKING_NEWS_HIGH_CADENCE_MIN_WORDS = 100;
const BREAKING_NEWS_HIGH_CADENCE_MAX_WORDS = 120;
const GOVERNED_INTAKE_CONTRACTS = Object.freeze({
  [BREAKING_NEWS_FLASH_CONTRACT.duration_band_id]:
    Object.freeze({
      required: BREAKING_NEWS_FLASH_CONTRACT,
      min_words: BREAKING_NEWS_FLASH_MIN_WORDS,
      max_words: BREAKING_NEWS_FLASH_MAX_WORDS,
      delivery_profile_id: "breaking_news_flash_v1",
      target_duration_review_required: true,
    }),
  [REQUIRED_CONTRACT.duration_band_id]: Object.freeze({
    required: REQUIRED_CONTRACT,
    min_words: SCRIPT_MIN_WORDS,
    max_words: SCRIPT_MAX_WORDS,
    delivery_profile_id: null,
    target_duration_review_required: false,
  }),
  [BREAKING_NEWS_HIGH_CADENCE_CONTRACT.duration_band_id]:
    Object.freeze({
      required: BREAKING_NEWS_HIGH_CADENCE_CONTRACT,
      min_words: BREAKING_NEWS_HIGH_CADENCE_MIN_WORDS,
      max_words: BREAKING_NEWS_HIGH_CADENCE_MAX_WORDS,
...
const FRESHNESS_TIMESTAMP_PATTERN =
...
const EXPERIMENT_DIMENSION_FIELDS = new Set([
  "eligible",
  "experiment_id",
  "matrix_version",
  "expected_cell_id",
...
function validFreshnessTimestamp(value) {
  return (
    typeof value === "string" &&
    FRESHNESS_TIMESTAMP_PATTERN.test(value) &&
    validTimestamp(value)
  );
}

function validateFreshnessContract(
  value,
  ...
  if (
    ...
    if (required) {
      errors.push("story_freshness_contract_required");
    } else if (present) {
      errors.push("story_freshness_contract_invalid");
    }
    return null;
  }
  if (
    Object.keys(value).length !== FRESHNESS_FIELDS.size ||
    Object.keys(value).some(
      (field) => !FRESHNESS_FIELDS.has(field),
    )
  ) {
    errors.push("freshness_fields_invalid");
  }

  const timestampFields = [
    ["discovered_at", "freshness_discovered_at_invalid"],
    [
      "source_last_checked_at",
      "freshness_source_last_checked_at_invalid",
    ],
    ["publish_by", "freshness_publish_by_invalid"],
    ["stale_after", "freshness_stale_after_invalid"],
  ];
  for (const [field, code] of timestampFields) {
    if (!validFreshnessTimestamp(value[field])) errors.push(code);
  }

  const discoveredAt = Date.parse(value.discovered_at);
  const sourceLastCheckedAt = Date.parse(
    value.source_last_checked_at,
  );
  const publishBy = Date.parse(value.publish_by);
  const staleAfter = Date.parse(value.stale_after);
  if (
    Number.isFinite(discoveredAt) &&
    Number.isFinite(sourceLastCheckedAt) &&
    sourceLastCheckedAt < discoveredAt
  ) {
    errors.push("freshness_source_check_before_discovery");
  }
  if (
    Number.isFinite(discoveredAt) &&
    Number.isFinite(publishBy) &&
    publishBy < discoveredAt
  ) {
    errors.push("freshness_publish_by_before_discovery");
  }
  if (
    Number.isFinite(sourceLastCheckedAt) &&
    Number.isFinite(publishBy) &&
    sourceLastCheckedAt > publishBy
  ) {
    errors.push("freshness_source_check_after_publish_by");
  }
  if (
    Number.isFinite(publishBy) &&
    ...
    errors.push("freshness_publish_by_after_stale_after");
  }
  if (typeof value.reverification_required !== "boolean") {
    errors.push("freshness_reverification_required_invalid");
  }
  const staleReframe = value.stale_reframe_option;
  if (
    !staleReframe ||
    typeof staleReframe !== "object" ||
    Array.isArray(staleReframe)
  ) {
    errors.push("freshness_stale_reframe_option_invalid");
  } else {
    if (
      Object.keys(staleReframe).length !==
        STALE_REFRAME_FIELDS.size ||
      Object.keys(staleReframe).some(
        (field) => !STALE_REFRAME_FIELDS.has(field),
      )
    ) {
      errors.push("freshness_stale_reframe_fields_invalid");
    }
    if (typeof staleReframe.allowed !== "boolean") {
      errors.push("freshness_stale_reframe_allowed_invalid");
    }
    if (!String(staleReframe.reason || "").trim()) {
      errors.push("freshness_stale_reframe_reason_required");
    }
  }
...
function validateTargetDurationReview({
  contract,
  contractProfile,
  ...
  if (
    contractProfile?.target_duration_review_required !== true
  ) {
    return;
  }
  ...
  if (
    ...
    errors.push("target_duration_review_required");
    return;
  }
  if (review.status !== "APPROVED") {
    errors.push("target_duration_review_status_invalid");
  }
  if (
    typeof contract.target_duration_seconds !== "number" ||
    !Number.isFinite(contract.target_duration_seconds)
  ) {
    errors.push("target_duration_seconds_required");
  }
  if (
    typeof review.target_duration_seconds !== "number" ||
    !Number.isFinite(review.target_duration_seconds) ||
    review.target_duration_seconds !==
      contract.target_duration_seconds
  ) {
    errors.push("target_duration_review_target_mismatch");
  }
  if (
    String(review.script_sha256 || "").toLowerCase() !==
  ...
  if (!String(review.reviewed_by || "").trim()) {
    errors.push("target_duration_review_actor_required");
  }
  if (!validTimestamp(review.reviewed_at)) {
    errors.push("target_duration_review_timestamp_invalid");
  }
}

function validateExperimentDimensions(
  value,
  { resolvedContract, errors } = {},
) {
  if (value === undefined) return null;
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    errors.push("experiment_dimensions_invalid");
    return null;
  }
  const fields = Object.keys(value);
  if (fields.some((field) => !EXPERIMENT_DIMENSION_FIELDS.has(field))) {
    errors.push("experiment_dimensions_fields_invalid");
  }
  if (typeof value.eligible !== "boolean") {
    errors.push("experiment_dimensions_eligible_invalid");
    return null;
  }
  if (value.eligible === false) {
    if (
      typeof value.ineligibility_reason !== "string" ||
      !value.ineligibility_reason.trim()
    ) {
      errors.push(
    ...
    if (
      fields.some(
        (field) =>
          !["eligible", "ineligibility_reason"].includes(field),
      )
    ) {
      errors.push("experiment_dimensions_ineligible_fields_invalid");
    }
    return structuredClone(value);
  }
  if (
    ...
    errors.push("experiment_dimensions_eligible_fields_invalid");
  }
  ...
  for (const field of requiredFields) {
    if (
      ...
      errors.push(`experiment_dimensions_${field}_required`);
    }
  }
  const experimentId =
    typeof value.experiment_id === "string"
      ...
      : "";
  const matrixVersion =
    typeof value.matrix_version === "string"
      ? value.matrix_version.trim()
      : "";
  const expectedCellId =
    typeof value.expected_cell_id === "string"
      ? value.expected_cell_id.trim()
      : "";
  if (
    experimentId &&
    experimentId !==
    String(resolvedContract?.experiment_id || "").trim()
  ) {
    errors.push("experiment_dimensions_experiment_id_mismatch");
  }
  if (
    matrixVersion &&
    matrixVersion !==
    String(
      resolvedContract?.experiment_matrix_version || "",
    ).trim()
  ) {
    errors.push("experiment_dimensions_matrix_version_mismatch");
  }
  if (
    expectedCellId &&
    expectedCellId !==
    String(resolvedContract?.experiment_cell_id || "").trim()
  ) {
    errors.push("experiment_dimensions_cell_contract_mismatch");
  }
  return structuredClone(value);
}
...
function validateStoryIntakeManifest({ manifestPath } = {}) {
  ...
  const declaredCanonicalIdentityUrl = String(
    manifest?.canonical_identity_url || "",
  ).trim();
  const canonicalIdentityUrl =
    declaredCanonicalIdentityUrl || sourceUrl;
  if (
    declaredCanonicalIdentityUrl &&
    !canonicalUrl(declaredCanonicalIdentityUrl)
  ) {
    errors.push("canonical_identity_url_invalid");
  }
  ...
  const canonicalSourceHash = canonicalHash(canonicalIdentityUrl);
  const storyId =
  ...
  if (!contract || typeof contract !== "object" || Array.isArray(contract)) {
    ...
    contractProfile =
      GOVERNED_INTAKE_CONTRACTS[contract.duration_band_id] || null;
    const requiredContract =
      contractProfile?.required || REQUIRED_CONTRACT;
    for (const [key, expected] of Object.entries(requiredContract)) {
    ...
    try {
      ...
      if (
        !contractProfile ||
        resolved.editorial_lane_id !==
          requiredContract.editorial_lane_id ||
        resolved.hook_type !== requiredContract.hook_type ||
        resolved.duration_band_id !==
          requiredContract.duration_band_id ||
        resolved.min_words !== contractProfile.min_words ||
        resolved.max_words !== contractProfile.max_words ||
        resolved.delivery_profile_id !==
          contractProfile.delivery_profile_id ||
        resolved.target_duration_review_required !==
          contractProfile.target_duration_review_required
      ) {
  ...
  const minimumWords =
    contractProfile?.min_words ?? SCRIPT_MIN_WORDS;
  const maximumWords =
    contractProfile?.max_words ?? SCRIPT_MAX_WORDS;
  if (wordCount < minimumWords || wordCount > maximumWords) {
    errors.push("script_word_count_out_of_range");
  }
  const freshness = validateFreshnessContract(
    manifest?.freshness,
    {
      required:
        contractProfile?.target_duration_review_required === true,
  ...
  const experimentDimensions = validateExperimentDimensions(
    manifest?.experiment_dimensions,
    {
      resolvedContract,
      errors,
    },
  ...
  if (!String(manifest?.source_evidence_path || "").trim()) {
    ...
    try {
      ...
      if (
        declaredCanonicalIdentityUrl &&
        String(
          evidence?.canonical_identity_url || "",
        ).trim() !== declaredCanonicalIdentityUrl
      ) {
        errors.push("source_evidence_canonical_identity_url_mismatch");
      }
      if (
        evidence?.story_id &&
        String(evidence.story_id).trim() !== storyId
      ) {
        errors.push("source_evidence_story_id_mismatch");
      }
  ...
  return {
    ...
    contract: {
      ...
      ...(resolvedContract?.target_duration_seconds !== null &&
      resolvedContract?.target_duration_seconds !== undefined
        ? {
            target_duration_seconds:
              resolvedContract.target_duration_seconds,
          }
        : {}),
      ...(contract?.target_duration_review
        ? {
...
function ingestTransaction({
  ...
  const governanceExtra = {
    ...
    ...(validation.freshness
      ? { freshness: structuredClone(validation.freshness) }
      : {}),
    ...(assetValidation
      ? {
...
module.exports = {
  ACTIONS,
  BREAKING_NEWS_HIGH_CADENCE_CONTRACT,
  BREAKING_NEWS_HIGH_CADENCE_MAX_WORDS,
  BREAKING_NEWS_HIGH_CADENCE_MIN_WORDS,
  GovernedStoryIntakeError,
  GOVERNED_INTAKE_CONTRACTS,
