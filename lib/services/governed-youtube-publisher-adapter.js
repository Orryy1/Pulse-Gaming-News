...
function normaliseExactYoutubeStagedBinding(
  ...
  const eventId = value.scheduledEventId ?? value.scheduled_event_id;
  const binding = {
...
function publisherAuthorityInput(input, repos, binding, operation) {
  return {
    db: repos.db,
    runtimeAuthority: input.runtimeAuthority,
    claimedJobAuthority: input.claimedJobAuthority,
    admissionContext: buildAdmittedPublicationOperation(operation, {
      channelId: binding.channelId,
      storyId: binding.storyId,
      platform: binding.platform,
      scheduledEventId: binding.scheduledEventId,
      scheduledFor: binding.scheduledFor,
      dispatchIdempotencyKey: binding.idempotencyKey,
...
async function verifyExactGovernedYoutubePrivatePrestage(
  ...
  return requiredFunction(
    ...
    task: async ({ assertHealthy }) => {
      ...
      const containUnexpectedObject = async (candidate) => {
          const externalId = text(
