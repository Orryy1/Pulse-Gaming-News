...
function buildPreparedAuthority({
  ...
  if (variant.type === AUTONOMOUS_APPROVAL_TYPE) {
    laneId = text(variant.jitPreparation?.lane_id);
    ...
    const projectedLegacyStoryId = text(
    ...
    const laneEligibility =
      evaluateGovernedBreakingLaneEligibility({
        ...
        expectedFastNewsStoryId:
          projectedLegacyStoryId || exactStoryId,
