  ...
  async _tick() {
    ...
    try {
      ...
      const heartbeatNow = async () => {
        const held = await this._heartbeat();
        assertLeaseHealthy();
        return held;
      };
      const result = await handler(jobRow, {
        ...
        claimGeneration: jobRow.claim_generation,
        attempt: jobRow.attempt_count,
        deadlineAt: new Date(this._currentLeaseDeadlineMs).toISOString(),
        leaseDeadline: new Date(this._currentLeaseDeadlineMs).toISOString(),
        assertLeaseHealthy,
        assertLeaseActive: assertLeaseHealthy,
        heartbeatNow,
      });
      assertLeaseHealthy();
