  ...
  constructor(result = {}) {
    const blockers = Array.isArray(result.blockers)
      ? result.blockers
          ...
          .slice(0, 20)
      : [];
    super(`retryable_job_outcome:${blockers[0] || "handler_requested_retry"}`);
...
class JobsRunner {
  constructor({
    ...
    if (runtimeAuthority) {
      const normalised = buildRuntimeGenerationLeaseMetadata(runtimeAuthority);
      this.runtimeAuthority = Object.freeze({
        runtime_instance_id: normalised.runtime_instance_id,
        child_pid: normalised.process_id,
        child_started_at: normalised.process_started_at,
        authority_fingerprint: normalised.authority_fingerprint,
      });
    } else {
  ...
  async stop() {
    ...
    this.log(`[jobs-runner] ${this.workerId} stopped drained=${drained}`);
    return { drained };
  }
  ...
  _startLocalLease(jobRow, generation, claimStartedAt) {
    this._currentLeaseDurationMs = serverLeaseDurationMs(jobRow, this.leaseMs);
    return this._armLeaseDeadline(
      jobRow,
      generation,
      claimStartedAt + this._localLeaseWindowMs(this._currentLeaseDurationMs),
    );
  }
  ...
  async _tick() {
    if (!this.running) return;
    let jobRow = null;
    let claimedRepos = null;
    const claimStartedAt = Date.now();
    try {
      claimedRepos = this.reposProvider();
      const { jobs } = claimedRepos;
    ...
    this.current = jobRow;
    let claimedJobAuthority = null;
    if (
      this.runtimeAuthority &&
      (claimedJobKindRequiresPublisherAuthority(jobRow.kind) ||
        claimedJobKindRequiresPublicationAdmissionAuthority(jobRow.kind))
    ) {
      try {
        claimedJobAuthority = buildClaimedJobAuthority({
          db: claimedRepos?.db,
          jobId: jobRow.id,
        ...
        this.log(
          `[jobs-runner] job ${jobRow.id} claimed authority invalid: ${error.message}`,
        );
        try {
          claimedRepos.jobs.fail(
            jobRow.id,
            this.workerId,
            jobRow.claim_token,
            error,
          ...
          /* best-effort claim finalisation */
        }
        this.current = null;
        this._schedule(50);
        return;
    ...
    if (!handler) {
      ...
      try {
        ...
        jobs.fail(jobRow.id, this.workerId, jobRow.claim_token, new Error(msg));
      } catch (err) {
    ...
    try {
      ...
      const result = await handler(jobRow, {
        ...
        claimedJobAuthority,
        assertClaimedJobAuthority: claimedJobAuthority
          ? () =>
              assertClaimedJobAuthority({
                db: claimedRepos?.db,
