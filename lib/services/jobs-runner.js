...
const QUIET_SUCCESS_KINDS = new Set([
  "jobs_reap",
  "publish_schedule_recovery_monitor",
]);

class JobHandlerTimeoutError extends Error {
  constructor(jobRow, timeoutMs) {
    super(
      `job handler timed out after ${timeoutMs}ms ` +
        `(job=${jobRow?.id ?? "unknown"} kind=${jobRow?.kind || "unknown"})`,
    );
    this.name = "JobHandlerTimeoutError";
    this.code = "JOB_HANDLER_TIMEOUT";
    this.jobId = jobRow?.id ?? null;
    this.kind = jobRow?.kind || null;
...
class JobsRunner {
  constructor({
    ...
    this.leaseMs = Number.isFinite(Number(leaseMs)) && Number(leaseMs) > 0
      ? Number(leaseMs)
      : undefined;
    this.claimGuard = typeof claimGuard === "function" ? claimGuard : null;
    this.handlerTimeoutMsByKind =
      handlerTimeoutMsByKind && typeof handlerTimeoutMsByKind === "object"
        ? { ...handlerTimeoutMsByKind }
        : {};
    this.stopOnHandlerTimeout = stopOnHandlerTimeout === true;
    this.onHandlerTimeout =
      typeof onHandlerTimeout === "function" ? onHandlerTimeout : null;
    this.log = log;
  ...
  _handlerTimeoutMs(jobRow) {
    const configured = Number(this.handlerTimeoutMsByKind[jobRow?.kind]);
    return Number.isFinite(configured) && configured > 0
      ? Math.floor(configured)
      : null;
  }

  async _runHandler(handler, jobRow, context) {
    const timeoutMs = this._handlerTimeoutMs(jobRow);
    if (!timeoutMs) return handler(jobRow, context);

    let timeoutHandle = null;
    try {
      return await Promise.race([
        Promise.resolve().then(() => handler(jobRow, context)),
        new Promise((resolve, reject) => {
          timeoutHandle = setTimeout(
            () => reject(new JobHandlerTimeoutError(jobRow, timeoutMs)),
            timeoutMs,
          );
        }),
      ]);
  ...
  async _tick() {
    ...
    if (this.claimGuard) {
      let guard;
      try {
        guard = await this.claimGuard({
          now: new Date(),
          workerId: this.workerId,
          kinds: this.kinds,
        });
      } catch (err) {
        this.log(`[jobs-runner] claim guard failed closed: ${err.message}`);
        this._schedule(this.pollIntervalMs);
        return;
      }
      if (guard?.allow_claim === false) {
        const retryAfterMs = Math.max(
          500,
          Math.min(
            60_000,
            Number(guard.retry_after_ms) || this.pollIntervalMs,
          ),
        );
        this._schedule(retryAfterMs);
        return;
      }
    }
    let jobRow = null;
    ...
    try {
      const result = await this._runHandler(handler, jobRow, {
        workerId: this.workerId,
        repos: getRepos(),
      });
      const { jobs } = getRepos();
      ...
      const handlerTimedOut = err?.code === "JOB_HANDLER_TIMEOUT";
      if (handlerTimedOut && this.stopOnHandlerTimeout) {
        this.running = false;
        if (this._heartbeatHandle) clearInterval(this._heartbeatHandle);
        this._heartbeatHandle = null;
      }
      ...
      if (handlerTimedOut && this.onHandlerTimeout) {
        try {
          await this.onHandlerTimeout(err, jobRow);
        } catch (handlerErr) {
          this.log(
            `[jobs-runner] onHandlerTimeout handler threw: ${handlerErr.message}`,
          );
        }
      }
    } finally {
      ...
      try {
        const { workers } = getRepos();
        workers.heartbeat(this.workerId, {
          status: this.running ? "idle" : "offline",
        });
      } catch {
        /* ignore */
      }
      ...
      if (this.running) this._schedule(50);
    }
  }
  ...
  _heartbeat() {
    ...
    try {
      ...
      if (this.current) {
        jobs.heartbeat(this.current.id, this.workerId, this.leaseMs);
        workers.heartbeat(this.workerId, { status: "busy" });
      } else {
        workers.heartbeat(this.workerId, { status: "idle" });
      }
