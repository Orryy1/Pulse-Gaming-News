...
function summariseWorkerPools(bootstrapState, env) {
  const runners = Array.isArray(bootstrapState?.runners)
...
function summariseElevenLabsCreditMonitor(bootstrapState) {
  ...
  const thresholdStateKey =
    /^sha256:[a-f0-9]{16}$/i.test(
  ...
  const lastErrorCode =
    ...
    status.last_error_code === undefined
      ? null
      : /^elevenlabs_[a-z0-9_]{1,120}$/.test(
  ...
  return {
    ...
    allow_paid_synthesis:
      active &&
      verdict !== "BLOCKED" &&
      status.allow_paid_synthesis === true,
    included_credits_remaining: safeFinite(
      status.included_credits_remaining,
    ),
    remaining_percent: safeFinite(status.remaining_percent),
    safe_spendable_credits: safeFinite(
      status.safe_spendable_credits,
    ),
...
function buildMultiLaneRuntimeObservability({
  ...
  return {
    ...
    elevenlabs_credit_monitor:
      summariseElevenLabsCreditMonitor(bootstrapState),
...
function buildAutonomousScheduleSummary(runtime = {}) {
  ...
  if (profile === "governed_multi_lane") {
    return {
      profile,
      hunts: [
        "Continuous breaking watcher plus governed discovery windows",
      ],
      lanes: [
        "breaking_short",
        "evergreen_short",
