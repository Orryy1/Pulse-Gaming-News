...
const DEFAULT_SCHEDULER_BRIDGE_REPORT_MAX_AGE_MS = 6 * 60 * 60 * 1000;
const MAX_SCHEDULER_BRIDGE_CLOCK_SKEW_MS = 5 * 60 * 1000;
...
function schedulerBridgeReportFreshness(candidateReport = {}, options = {}) {
  const maxAgeMs = Number(options.schedulerBridgeReportMaxAgeMs) > 0
    ? Number(options.schedulerBridgeReportMaxAgeMs)
    : DEFAULT_SCHEDULER_BRIDGE_REPORT_MAX_AGE_MS;
  const nowMs = Number.isFinite(Date.parse(options.now))
    ? Date.parse(options.now)
    : Date.now();
  if (!candidateReport || typeof candidateReport !== "object" || !Object.keys(candidateReport).length) {
    return {
      status: "missing",
      usable: false,
      generated_at: null,
      ...
      reason: "scheduler_bridge_candidate_report_missing",
    };
  }
  const generatedAt = String(candidateReport.generated_at || "").trim();
  ...
  if (!generatedAt || !Number.isFinite(generatedAtMs)) {
    return {
      ...
      age_seconds: null,
      max_age_seconds: Math.round(maxAgeMs / 1000),
      reason: "scheduler_bridge_candidate_report_timestamp_invalid",
    };
  }
  const ageMs = nowMs - generatedAtMs;
  if (ageMs < -MAX_SCHEDULER_BRIDGE_CLOCK_SKEW_MS) {
    return {
      ...
      max_age_seconds: Math.round(maxAgeMs / 1000),
      reason: "scheduler_bridge_candidate_report_timestamp_in_future",
    };
  }
  if (ageMs > maxAgeMs) {
    return {
      ...
      age_seconds: Math.round(ageMs / 1000),
      max_age_seconds: Math.round(maxAgeMs / 1000),
      reason: "scheduler_bridge_candidate_report_stale",
    };
  }
  return {
    status: "fresh",
...
function buildPipelineBacklog(stories, options = {}) {
  ...
  const rawSchedulerBridgeReport =
    options.schedulerBridgeCandidateReport || options.bridgeCandidateReport || {};
  const bridgeReportFreshness = schedulerBridgeReportFreshness(
    rawSchedulerBridgeReport,
    options,
  );
  const schedulerBridgeReport = bridgeReportFreshness.usable
    ? rawSchedulerBridgeReport
    : {};
  ...
  return {
    ...
    scheduler_bridge_report_freshness: bridgeReportFreshness,
...
function renderPipelineBacklogMarkdown(report = {}) {
  ...
  const bridgeFreshness = report.scheduler_bridge_report_freshness;
  if (bridgeFreshness) {
    lines.push(`- report freshness: ${bridgeFreshness.status || "unknown"}`);
    if (bridgeFreshness.reason) lines.push(`- freshness blocker: ${bridgeFreshness.reason}`);
  }
...
module.exports = {
  ...
  schedulerBridgeReportFreshness,
