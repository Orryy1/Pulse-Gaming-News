...
function isInstagramPlatform(platform = "") {
  return /instagram/i.test(String(platform || ""));
}
...
function classifyPlatformVideoQa(probe, opts = {}) {
  ...
  const instagram = isInstagramPlatform(platform);
  ...
  if (!video) {
    ...
    if (!Number.isFinite(width) || !Number.isFinite(height) || width <= 0 || height <= 0) {
      ...
      if (instagram && width > 1920) {
        failures.push(`instagram_video_width_above_1920 (${width})`);
      }
    }

    if (instagram) {
      const videoBitrate = numberOrNull(video.bit_rate);
      if (Number.isFinite(videoBitrate) && videoBitrate > 25_000_000) {
        failures.push(`instagram_video_bitrate_above_25mbps (${video.bit_rate})`);
      }
    }
  }
  ...
  if (!audio) {
    ...
    if (Number.isFinite(sampleRate) && sampleRate > 48000) {
      failures.push(`audio_sample_rate_too_high_for_social (${audio.sample_rate})`);
    }
    if (instagram) {
      const audioBitrate = numberOrNull(audio.bit_rate);
      if (Number.isFinite(audioBitrate) && audioBitrate > 132_000) {
        failures.push(`instagram_audio_bitrate_above_128kbps (${audio.bit_rate})`);
      }
    }
  }

  if (instagram) {
    const formatSize = numberOrNull(probe.format?.size);
    const maxSize = 300 * 1024 * 1024;
    if (Number.isFinite(formatSize) && formatSize > maxSize) {
      failures.push(`instagram_file_size_above_300mb (${probe.format.size})`);
    }
  }
...
class PlatformVideoQaUploadError extends Error {
  constructor({ platform, failures = [], warnings = [], qaResult } = {}) {
    const safePlatform = platform || "shorts";
    const failureText = failures.length ? failures.join("; ") : "unknown_failure";
    super(`Platform video QA failed for ${safePlatform}: ${failureText}`);
    this.name = "PlatformVideoQaUploadError";
    this.code = "platform_video_qa_failed";
    this.platform = safePlatform;
    this.failures = failures;
    this.warnings = warnings;
    this.qaResult = qaResult || null;
  }
}

async function assertPlatformVideoQaPass(mp4Path, opts = {}) {
  const result = await runPlatformVideoQa(mp4Path, opts);
  if (result.result === "fail") {
    throw new PlatformVideoQaUploadError({
