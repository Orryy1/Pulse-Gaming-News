...
function outsideCadenceAuthorisationPayload(value = {}) {
  return {
    ...
    confirmed_authorisation_id: text(
      value.confirmed_authorisation_id,
    ),
...
function fingerprintOutsideCadenceAuthorisation(value) {
  return sha256Text(
    JSON.stringify(outsideCadenceAuthorisationPayload(value)),
  );
}
...
function persistedOutsideCadenceAuthorisation(value, binding = {}) {
  ...
  const persisted = {
    ...
    confirmed_authorisation_id:
      normalised.confirmed_authorisation_id,
    one_shot: true,
  ...
  return {
    ...persisted,
    binding_sha256:
      fingerprintOutsideCadenceAuthorisation(persisted),
  };
}
...
function validatePersistedOutsideCadenceAuthorisation(
  ...
  if (
    payload.confirmed_authorisation_id !== payload.authorisation_id
  ) {
    blockers.push(
  ...
  if (
    ...
    if (
      authorisedAtMs < noEarlierThanMs ||
      authorisedAtMs > noLaterThanMs
    ) {
...
async function admitPublication({
  ...
  const outsideCadenceEvidence = isGuardedUtcSchedule(
    new Date(scheduledAt),
  )
    ? null
    : persistedOutsideCadenceAuthorisation(
        outsideCadenceAuthorisation,
        {
          storyId: story.id,
          channelId,
