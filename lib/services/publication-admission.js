...
function assertPublicationAdmissionLeaseHealthy(lease) {
  if (lease === null || lease === undefined) return true;
  if (typeof lease.assertHealthy !== "function") {
    throw new Error("publication_admission_lease_invalid");
  }
  return lease.assertHealthy();
}

function assertPublicationAdmissionLeaseInTransaction(lease) {
  if (lease === null || lease === undefined) return true;
  if (typeof lease.assertHealthyInTransaction !== "function") {
    throw new Error("publication_admission_lease_invalid");
  }
  return lease.assertHealthyInTransaction();
}
...
async function admitPublication({
  ...
  dispatchJob = null,
  publicationAdmissionLease = null,
  ...
  const transaction = repos.db.transaction(() => {
    ...
    assertPublicationAdmissionLeaseInTransaction(publicationAdmissionLease);
  ...
  assertPublicationAdmissionLeaseHealthy(publicationAdmissionLease);
...
const AUTONOMOUS_RELEASE_BOUNDARY_FIELDS = new Set([
  ...
  "publication_admission_owner_revalidation_required",
]);
...
function validateAutonomousAuthorityEnvelope(
  ...
  if (
    ...
    boundary.publication_admission_owner_revalidation_required !== true ||
