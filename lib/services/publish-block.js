...
function isRetryablePlatformBlock(row = {}) {
  const platform = String(row.platform || "").trim();
  const status = String(row.status || "").trim();
  const reason = String(row.block_reason || "").trim();

  if (platform !== "facebook_reel" || status !== "blocked") return false;
  return ["page_not_eligible", "facebook_page_reels_gate"].includes(reason);
}
