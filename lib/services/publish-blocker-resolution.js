...
function staleTemporalReviewDecision(story = {}) {
  return cleanText(
    story.stale_temporal_review?.decision ||
      story.stale_temporal_review?.status ||
      story.stale_temporal_review_decision ||
      "",
  );
}

function hasResolvedStaleTemporalReview(story = {}) {
  return /^(reject_stale_current_news_candidate|defer_until_new_source_updates_story)$/i.test(
    staleTemporalReviewDecision(story),
  );
}
...
function commandForLane(lane, storyId) {
  ...
  const commands = {
    ...
    voice_mastering_repair:
      `npm run voice:repair-final-audio -- --story-id ${id} --dry-run`,
    script_runtime_rewrite:
    ...
    public_copy_package_repair:
      `npm run ops:goal-public-copy-repair -- --story-packages output/goal-contract/production_cutover_story_packages.json --story-id ${id} --out-dir output/goal-contract --json`,
    stale_script_qa_recheck:
    ...
    rights_ledger_repair:
      `npm run ops:bridge-live-rights-repair -- --story-id ${id} --out-dir output/goal-contract --json`,
    platform_media_repair:
      `npm run ops:local-media-repair -- --story-id ${id} --dry-run`,
    produce_or_render:
      `npm run ops:goal-production-render -- --story-id ${id}`,
    stale_story_refresh:
      `npm run hunt && npm run ops:next-publish-candidates -- --preflight-qa`,
    stale_temporal_review:
      `npm run ops:goal-stale-temporal-review -- --json`,
    duration_recut:
...
function classifyPublishBlocker({
  ...
  const finalRenderPath = cleanText(
    story.exported_path ||
    story.final_render_path ||
    story.finalRenderPath ||
    story.render_manifest?.final_render_path ||
    story.render_manifest?.mp4_path,
  );
  ...
  if (/^already_has_public_platform_id/.test(lowerReason)) {
    ...
    priority = 5;
  } else if (/^live_db_legacy_debt:/.test(lowerReason)) {
    resolutionLane = "already_handled";
    action =
      "Keep this as legacy live DB debt; bridge preflight cannot repair it because the story is not scheduler-visible.";
    safetyGate = "read_only_live_db_legacy_debt";
    priority = 4;
  } else if (/^upstream_skipped:/.test(lowerReason)) {
    resolutionLane = "already_handled";
    action = "Honour the upstream skip and keep the story out of active repair pressure.";
    safetyGate = "read_only_state_check";
    priority = 5;
  } else if (
    /stale_temporal_claim|current_wording_on_old_event/.test(lowerReason) &&
    hasResolvedStaleTemporalReview(story)
  ) {
    resolutionLane = "already_handled";
    action = "Confirm stale-temporal review is still reflected in dry-run skipped stories.";
    safetyGate = "read_only_state_check";
    priority = 6;
  } else if (/stale_temporal_claim|current_wording_on_old_event/.test(lowerReason)) {
    resolutionLane = "stale_temporal_review";
    action =
      "Write a stale-temporal review artefact so old current-news wording is rejected, deferred or refreshed before any publish attempt.";
    canApplyAutomatically = true;
    safetyGate = "stale_temporal_review_required_no_publish";
    priority = 74;
    ...
    /approved_voice:local_voice_mastering_missing|local_voice_mastering_missing/.test(lowerReason) &&
    !finalRenderPath
  ) {
    resolutionLane = "produce_or_render";
    action =
      "Build the final V4 MP4 first; approved-voice mastering cannot be repaired against a missing final render.";
    canApplyAutomatically = true;
    safetyGate = "render_and_governance_required";
    priority = v4Ready ? 92 : 76;
  } else if (/approved_voice:local_voice_mastering_missing|local_voice_mastering_missing/.test(lowerReason)) {
    resolutionLane = "voice_mastering_repair";
    action =
      "Rebuild or verify local voice mastering evidence for the final audio path, then rerun approved-voice QA.";
    canApplyAutomatically = true;
    safetyGate = "local_voice_mastering_repair_required";
    priority = 85;
    ...
    priority = 86;
  } else if (/script_validation_review_required_public_row_repair/.test(lowerReason)) {
    resolutionLane = "stale_script_qa_recheck";
    action =
      "Rerun current preflight because the persisted public-row repair label may pre-date the corrected public copy.";
    canApplyAutomatically = true;
    ...
    safetyGate = "fresh_preflight_required_no_db_mutation";
    priority = 89;
  } else if (/public_output:placeholder_title|public_output:internal_qa|public_output:generic_title/.test(lowerReason)) {
    resolutionLane = "public_copy_package_repair";
    action =
      "Repair the governed public-copy package, then regenerate audio, captions and final render before scheduler preflight.";
    canApplyAutomatically = true;
    safetyGate = "public_output_package_repair_required";
    priority = 88;
  } else if (/top_comment_used_as_fact|placeholder_title|source backed|internal_qa|script_coherence|glued_sentence_in_tts_script/.test(lowerReason)) {
    ...
    priority = 80;
  } else if (/instagram.*(?:url processing failed|media upload|container creation|only photo)|error code 2207076|meta.*media|video_pixel_format_not_yuv420p/.test(lowerReason)) {
    ...
    priority = 81;
  } else if (/rights_risk_above_reference|rights_ledger|missing[_ -]?rights|rights record|asset rights/.test(lowerReason)) {
    resolutionLane = "rights_ledger_repair";
    action = "Repair the asset rights ledger and source evidence, then rerun rights, benchmark and scheduler preflight checks.";
    canApplyAutomatically = true;
    safetyGate = "rights_qa_and_preflight_required";
    priority = 83;
  } else if (/thin_visuals|risky_article_context_dominated_deck|article_context_dominated|safe non-article|missing_motion|visual|motion|freeze_segment_too_long|cumulative_freeze_too_high/.test(lowerReason)) {
...
function classifyStaleMissingStoryBlocker(row = {}) {
  const storyId = cleanText(row.id);
  return {
    story_id: storyId || "unknown",
    title: cleanText(row.title).slice(0, 180),
    blocker: cleanText(row.reason) || "unknown",
    resolution_lane: "already_handled",
    priority: 4,
    action:
      "Treat this as stale blocker evidence because the story is no longer present in the current live story set; refresh publish-unblock inputs before assigning repair work.",
    safe_next_command: commandForLane("already_handled", storyId),
    can_apply_automatically: false,
    safety_gate: "read_only_stale_blocker_row",
...
function repairMissingInputForLane(lane, blocker) {
  const reason = cleanText(blocker);
  const suffix = reason ? ` (${reason})` : "";
  const messages = {
    audio_regeneration: `final narration audio, clean transcript and caption/timestamp parity${suffix}`,
    voice_mastering_repair: `local voice mastering evidence and approved-voice QA metadata${suffix}`,
    script_runtime_rewrite: `runtime-compliant narration script and matching audio/caption timing${suffix}`,
    canonical_script_rewrite: `canonical public copy with aligned title, thumbnail, script and source labels${suffix}`,
    public_copy_package_repair: `governed public-copy package plus rerender work order${suffix}`,
    stale_script_qa_recheck: `fresh script QA evidence for the current approved script${suffix}`,
    script_generation_retry: `valid script-generation output from a timeout-safe retry${suffix}`,
    visual_v4_motion_enrichment: `distinct V4 motion clips and source families that meet benchmark motion density${suffix}`,
    rights_ledger_repair: `complete rights ledger and source evidence for every asset used${suffix}`,
    platform_media_repair: `platform-ready media encode, container and upload preflight evidence${suffix}`,
    produce_or_render: `final MP4 evidence plus render QA artefacts${suffix}`,
    stale_story_refresh: `fresh source state or a reject/defer decision for stale coverage${suffix}`,
    stale_temporal_review: `stale-temporal review artefact and refreshed wording decision${suffix}`,
    governance_green_approval_promotion: `operator-confirmed live-row promotion evidence${suffix}`,
  };
  return messages[lane] || `operator triage input for blocker${suffix}`;
}

function expectedOutputForLane(lane) {
  const outputs = {
    audio_regeneration: "narration_manifest, word_timestamps, caption_manifest and fresh audio QA evidence",
    voice_mastering_repair: "final audio loudness/mastering report plus refreshed approved-voice metadata evidence",
    script_runtime_rewrite: "runtime-compliant script package with regenerated narration, captions and preflight evidence",
    canonical_script_rewrite: "canonical public-copy package with coherence report and no placeholder/internal QA language",
    public_copy_package_repair: "public_copy_repair_report plus audio and final-render regeneration work orders",
    stale_script_qa_recheck: "fresh scheduler preflight result showing the stale script QA label is cleared or still blocked",
    script_generation_retry: "valid selected_script plus script_scorecard and generation failure evidence cleared",
    visual_v4_motion_enrichment: "updated V4 motion/source work order with distinct motion families and benchmark recheck evidence",
    rights_ledger_repair: "rights_ledger plus rights_risk_report recognised by bridge and scheduler preflight",
    platform_media_repair: "re-encoded platform media package plus platform_upload_preflight_report",
    produce_or_render: "final_render_manifest, MP4 path and render QA evidence",
    stale_story_refresh: "fresh story/source manifest or a machine-readable reject/defer decision",
    stale_temporal_review: "stale_temporal_review artefact and updated public wording recommendation",
    governance_green_approval_promotion: "DB backup, audit trail and post-apply scheduler preflight evidence",
  };
  return outputs[lane] || "machine-readable repair result plus refreshed preflight evidence";
}

function validationCommandForLane(lane, storyId) {
  const id = storyId || "<story-id>";
  if (lane === "platform_media_repair") {
    return `npm run ops:platform-doctor -- --json && npm run ops:publish-unblock -- --story-id ${id} --json`;
  }
  if (lane === "rights_ledger_repair") {
    return `npm run ops:bridge-live-rights-repair -- --story-id ${id} --dry-run --json && npm run ops:next-publish-candidates -- --preflight-qa --story-id ${id}`;
  }
  if (lane === "visual_v4_motion_enrichment") {
    return `npm run ops:v4-source-deficit -- --story-id ${id} --json && npm run ops:next-publish-candidates -- --preflight-qa --story-id ${id}`;
  }
  if (lane === "audio_regeneration") {
    return `npm run ops:local-tts-publish-refresh -- --story-id ${id} --dry-run && npm run ops:next-publish-candidates -- --preflight-qa --story-id ${id}`;
  }
  if (lane === "voice_mastering_repair") {
    return `npm run voice:repair-final-audio -- --story-id ${id} --dry-run && npm run ops:next-publish-candidates -- --preflight-qa --story-id ${id}`;
  }
  if (/script/.test(lane)) {
    return `npm run ops:reprocess-script-failures -- --story-id ${id} --dry-run --json && npm run ops:next-publish-candidates -- --preflight-qa --story-id ${id}`;
  }
  return `npm run ops:next-publish-candidates -- --preflight-qa --story-id ${id} && npm run ops:publish-unblock -- --story-id ${id} --json`;
}

function repairWorkOrderForItem(item = {}, returnedIds = new Set()) {
  const lane = item.resolution_lane || "manual_triage";
  const operatorApprovalRequired = /operator_confirmed|operator_review/.test(item.safety_gate || "") ||
    lane === "manual_triage";
  return {
    ...
    blocker_type: item.blocker,
    priority: item.priority,
    command: item.safe_next_command,
    recommended_command: item.safe_next_command,
    exact_missing_input: repairMissingInputForLane(lane, item.blocker),
    expected_output: expectedOutputForLane(lane),
    db_mutation_required: /operator_confirmed_db_mutation/.test(item.safety_gate || ""),
    operator_approval_required: operatorApprovalRequired,
    post_repair_validation_command: validationCommandForLane(lane, item.story_id),
...
function buildPublishRunway({
  ...
  returnedItems = [],
  publishWindows24h = 5,
  targetPublishableCandidates = 10,
} = {}) {
  const rows = Array.isArray(items) ? items : [];
  const liveCandidates = Number(candidateCount || 0);
  const windowCount = Number.isFinite(Number(publishWindows24h)) && Number(publishWindows24h) > 0
    ? Number(publishWindows24h)
    : 5;
  const targetCount = Math.max(
    windowCount,
    Number.isFinite(Number(targetPublishableCandidates)) && Number(targetPublishableCandidates) > 0
      ? Number(targetPublishableCandidates)
      : 10,
  );
  const coveredWindows = Math.min(liveCandidates, windowCount);
  const uncoveredWindows = Math.max(0, windowCount - liveCandidates);
  const reserveCandidates = Math.max(0, liveCandidates - windowCount);
  const reserveTarget = Math.max(0, targetCount - windowCount);
  const reserveShortfall = Math.max(0, targetCount - liveCandidates);
  ...
  const freshContentNeeded =
    liveCandidates < targetCount &&
    greenPackAvailable === 0 &&
    repairableBacklog === 0;

  let status = "fresh_content_required";
  let nextAction = "Run fresh hunt, scoring and candidate preflight.";
  if (liveCandidates >= targetCount) {
    status = "publishable_now_with_reserve";
    nextAction =
      "Let the guarded scheduler run and keep the reserve topped up before each publish window.";
  } else if (liveCandidates >= windowCount) {
    status = "publishable_now_reserve_short";
    nextAction =
      "Refill the fresh GREEN reserve while the guarded scheduler continues; do not wait for the last candidate to be consumed.";
  } else if (liveCandidates > 0) {
    status = "publishable_now";
    nextAction =
      "Run safe repair or fresh intake before uncovered publish windows arrive; keep the guarded scheduler on the existing GREEN candidates.";
  ...
  return {
    ...
    target_publishable_candidates: targetCount,
    covered_publish_windows_24h: coveredWindows,
    uncovered_publish_windows_24h: uncoveredWindows,
...
function buildRepairOrchestration({
  ...
  const autoRepairBacklog = rows
    ...
    .map((item) => repairWorkOrderForItem(item, returnedIds));
  const operatorConfirmed = rows
    .filter((item) => /operator_confirmed/.test(item.safety_gate || ""))
    .map((item) => ({
      ...repairWorkOrderForItem(item, returnedIds),
      requires_operator_confirmation: true,
    }));
  const operatorReviewBacklog = rows
    .filter((item) => item.resolution_lane === "manual_triage")
    .map((item) => ({
      ...repairWorkOrderForItem(item, returnedIds),
      requires_operator_confirmation: true,
    }));
  ...
  if (operatorReviewBacklog.length > 0) {
    stages.push({
      id: "operator_review_backlog",
      description:
        "Manual-triage blockers stay visible as operator work orders instead of disappearing from the repair backlog.",
      requires_operator_confirmation: true,
      items: operatorReviewBacklog,
    });
  }
  stages.push({
  ...
  return {
    ...
    counts: {
      ...
      operator_review_backlog: operatorReviewBacklog.length,
...
function buildPublishBlockerResolutionPlan({
  ...
  const hasStoryUniverse = storyById.size > 0;
  const items = rows
    .map((row) => {
      const rowId = String(row.id || "");
      if (hasStoryUniverse && rowId && !storyById.has(rowId)) {
        return {
          ...classifyStaleMissingStoryBlocker(row),
          selection_score: 0,
        };
      }
      const story = storyById.get(rowId) || {
        id: row.id,
        title: row.title,
      };
      return {
...
function formatPublishBlockerResolutionMarkdown(plan = {}) {
  ...
  lines.push(`- publish windows covered: ${Number(runway.covered_publish_windows_24h || 0)}/${Number(runway.publish_windows_24h || 0)}`);
  lines.push(`- reserve candidates: ${Number(runway.reserve_candidates || 0)}/${Number(runway.reserve_target || 0)}`);
  lines.push(`- reserve shortfall: ${Number(runway.reserve_shortfall || 0)}`);
