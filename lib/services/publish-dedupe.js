...
const DEFAULT_TOPIC_KEY_RECENT_DAYS = 14;
const TOPIC_KEY_STOPWORDS = new Set([
  "a",
  "an",
  "and",
  "are",
...
const TOPIC_KEY_HOUSE_WORDS = new Set([
  "already",
  "big",
  "changes",
  "changed",
...
function titleTopicWords(title) {
  return String(title || "")
    .toLowerCase()
    .replace(/&/g, " and ")
    .replace(/[^a-z0-9]+/g, " ")
    ...
    .filter((word) => !TOPIC_KEY_STOPWORDS.has(word))
    .filter((word) => !TOPIC_KEY_HOUSE_WORDS.has(word));
}

function titleTopicKey(title) {
  const words = titleTopicWords(title);
  return [...new Set(words)].sort().join("_");
}

function titleTopicOverlap(a, b) {
  const wordsA = new Set(titleTopicWords(a));
  const wordsB = new Set(titleTopicWords(b));
  if (!wordsA.size || !wordsB.size) return { match: false, shared: [], score: 0 };
  const shared = [...wordsA].filter((word) => wordsB.has(word));
  const smaller = Math.min(wordsA.size, wordsB.size);
  const score = smaller ? shared.length / smaller : 0;
  return {
    match: shared.length >= 4 && score >= 0.55,
    shared,
    score,
  };
}
...
function decidePublish(story, platform, repos, options = {}) {
  ...
  const {
    ...
    topicKeyRecentDays = DEFAULT_TOPIC_KEY_RECENT_DAYS,
  } = options;
  ...
  const topicKey = titleTopicKey(story.title);
  if (topicKey && repos.db) {
    try {
      const recentRows = repos.db
        .prepare(`
          SELECT pp.*, s.id AS story_id_ref, s.title AS story_title
          FROM platform_posts pp
          JOIN stories s ON s.id = pp.story_id
          WHERE pp.platform = ?
            ...
            AND pp.external_id IS NOT NULL
            AND s.id != ?
            AND COALESCE(pp.published_at, pp.updated_at, pp.created_at) >= datetime('now', ?)
          ORDER BY COALESCE(pp.published_at, pp.updated_at, pp.created_at) DESC
          LIMIT 100
        `)
        .all(platform, story.id, `-${Number(topicKeyRecentDays) || DEFAULT_TOPIC_KEY_RECENT_DAYS} days`);
      const topicMatch = recentRows.find((row) => titleTopicKey(row.story_title) === topicKey);
      if (topicMatch) {
        return {
          decision: "block_dupe",
          reason: "topic-key",
      ...
      const topicOverlapMatch = recentRows.find(
        (row) => titleTopicOverlap(row.story_title, story.title).match,
      );
      if (topicOverlapMatch) {
        return {
          decision: "block_dupe",
          reason: "topic-overlap",
          existing: topicOverlapMatch,
        };
      }
    } catch (err) {
