...
/**
 * Returns true iff the legacy story row has a platform id that would
 * have passed the old "non-sentinel, non-null" check. Mirrors the
 * publisher.js checks so callers migrating one platform at a time don't
...
function platformIdPresent(s, platform) {
  switch (platform) {
    ...
    case "tiktok":
      return (
        !!s.tiktok_post_id &&
        !String(s.tiktok_post_id).startsWith("DUPE_") &&
    ...
    case "instagram":
      return (
        ...
        !String(s.instagram_media_id).startsWith("DUPE_")
      );
    case "facebook_reel":
    case "facebook":
      return (
        !!s.facebook_post_id && !String(s.facebook_post_id).startsWith("DUPE_")
      );
    case "twitter_video":
    case "twitter":
      return (
        !!s.twitter_post_id && !String(s.twitter_post_id).startsWith("DUPE_")
      );
    default:
      return false;
  }
