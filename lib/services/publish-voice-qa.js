"use strict";

const path = require("node:path");
const fsExtra = require("fs-extra");

const mediaPaths = require("../media-paths");
const { hasApprovedPulseCta } = require("../pulse-cta");
const {
  evaluateApprovedVoicePath,
} = require("../studio/v2/approved-voice-path");

const ALWAYS_BLOCKING_VOICE_FAILURES = new Set([
  "gta_vi_spoken_stutter",
  "gta_vi_spoken_roman_split",
  "gta_vi_opening_spoken_six_risk",
  "spoken_outro_misheard",
  "local_tts_tempo_stretch_applied",
  "local_tts_non_native_rate_applied",
  "managed_tts_non_native_rate_applied",
]);

function bool(value) {
  return value === true || /^(true|1|yes|on)$/i.test(String(value || ""));
}

function timestampPathForAudio(audioPath) {
  return String(audioPath || "").replace(/\.(mp3|wav|m4a)$/i, "_timestamps.json");
}

function addCandidate(target, value) {
  const cleaned = normaliseText(value);
  if (cleaned && !target.includes(cleaned)) target.push(cleaned);
}

function timestampPathCandidatesForStory(story = {}, audioPath = "", audioAbs = "") {
  const candidates = [];
  const manifests = [
    story,
    story.audio_manifest,
    story.final_audio_manifest,
    story.narration_manifest,
    story.caption_manifest,
  ].filter((value) => value && typeof value === "object");

  for (const source of manifests) {
    addCandidate(candidates, source.resolved_word_timestamps_path);
    addCandidate(candidates, source.word_timestamps_path);
    addCandidate(candidates, source.timestamps_path);
    addCandidate(candidates, source.timestamp_path);
  }

  const legacy = timestampPathForAudio(audioPath);
  addCandidate(candidates, legacy);

  const relativeWordPath = normaliseText(story.word_timestamps_path || story.audio_manifest?.word_timestamps_path);
  if (audioAbs && relativeWordPath && !path.isAbsolute(relativeWordPath)) {
    const audioDir = path.dirname(audioAbs);
    const artifactDir = /[\\/]audio$/i.test(audioDir) ? path.dirname(audioDir) : audioDir;
    addCandidate(candidates, path.resolve(artifactDir, relativeWordPath));
  }

  return candidates;
}

function normaliseText(value) {
  return String(value || "")
    .replace(/\s+/g, " ")
    .trim();
}

function hasRequiredOutro(value) {
  return hasApprovedPulseCta(value);
}

function wordTextFromPayload(payload = {}) {
  const words = Array.isArray(payload.words)
    ? payload.words
    : Array.isArray(payload.alignment?.words)
      ? payload.alignment.words
      : [];
  return normaliseText(
    words
      .map((word) => word?.word || word?.text || word?.token || "")
      .filter(Boolean)
      .join(" "),
  );
}

function transcriptFromPayload(payload = {}, fallback = "") {
  const meta = payload.meta || {};
  const wordText = wordTextFromPayload(payload);
  const candidates = [
    wordText,
    typeof meta.spoken_text === "string" ? meta.spoken_text : "",
    typeof meta.transcript === "string" ? meta.transcript : "",
    typeof meta.text === "string" ? meta.text : "",
    Array.isArray(payload.characters) && payload.characters.length > 0
      ? payload.characters.join("")
      : "",
    fallback,
  ]
    .map(normaliseText)
    .filter(Boolean);

  const withOutro = candidates.find(hasRequiredOutro);
  if (withOutro) return withOutro;

  return candidates.sort((a, b) => b.length - a.length)[0] || "";
}

function acousticFromMeta(meta = {}) {
  if (meta.acoustic && typeof meta.acoustic === "object") return meta.acoustic;
  const diagnostics = meta.voiceDiagnostics || meta.voice_diagnostics || {};
  if (diagnostics.acoustic && typeof diagnostics.acoustic === "object") {
    return diagnostics.acoustic;
  }
  if (diagnostics.metrics && typeof diagnostics.metrics === "object") {
    return diagnostics.metrics;
  }
  return null;
}

function captionTimingRepairReason(meta = {}) {
  const repair = captionTimingRepair(meta);
  if (!repair) return null;
  return String(repair.reason || repair.repairReason || "unknown").trim() || "unknown";
}

function captionTimingRepair(meta = {}) {
  const repair =
    meta.timestampRepair ||
    meta.timestamp_repair ||
    meta.captionTimingRepair ||
    meta.caption_timing_repair ||
    null;
  if (!repair || typeof repair !== "object") return null;
  const repaired = repair.repaired === true || bool(repair.repaired);
  if (!repaired) return null;
  return repair;
}

function captionTimingRepairIsUsable(repair = {}) {
  const inspection =
    repair.repairedInspection ||
    repair.repaired_inspection ||
    repair.inspection ||
    null;
  if (!inspection || typeof inspection !== "object") return false;
  return inspection.usable === true && String(inspection.reason || "usable") === "usable";
}

function allowRepairedCaptionTimingForPublish(env = process.env) {
  return bool(env.ALLOW_REPAIRED_CAPTION_TIMING_FOR_PUBLISH);
}

function strictVoiceQaEnabled(story = {}, env = process.env) {
  if (bool(env.REQUIRE_APPROVED_VOICE_FOR_PUBLISH)) return true;
  if (String(env.REQUIRE_APPROVED_VOICE_FOR_PUBLISH || "").toLowerCase() === "false") {
    return false;
  }
  return (
    String(env.DEPLOYMENT_MODE || "").toLowerCase() === "local" &&
    bool(env.AUTO_PUBLISH)
  );
}

function shouldInspectVoice(story = {}, env = process.env) {
  return strictVoiceQaEnabled(story, env) || Boolean(story.audio_path || story.audioPath);
}

async function runPublishVoiceQa(story, opts = {}) {
  const fs = opts.fs || fsExtra;
  const env = opts.env || process.env;
  const failures = [];
  const warnings = [];

  if (!story || typeof story !== "object") {
    return { result: "fail", failures: ["approved_voice:no_story"], warnings };
  }

  const strict = strictVoiceQaEnabled(story, env);

  function recordProblem(code) {
    if (strict) failures.push(code);
    else warnings.push(code);
  }

  function problemResult(extra = {}) {
    return {
      result: failures.length > 0 ? "fail" : warnings.length > 0 ? "warn" : "pass",
      failures,
      warnings,
      inspected: true,
      strict,
      ...extra,
    };
  }

  if (!shouldInspectVoice(story, env)) {
    return { result: "pass", failures, warnings, inspected: false };
  }

  const audioPath = story.audio_path || story.audioPath || null;
  if (!audioPath) {
    recordProblem("approved_voice:audio_path_missing");
    return problemResult();
  }

  const audioAbs = await mediaPaths.resolveExisting(audioPath, { fs });
  if (!audioAbs || !(await fs.pathExists(audioAbs))) {
    recordProblem("approved_voice:audio_file_missing");
    return problemResult();
  }

  const timestampCandidates = timestampPathCandidatesForStory(story, audioPath, audioAbs);
  let timestampsAbs = null;
  for (const timestampCandidate of timestampCandidates) {
    const resolved = await mediaPaths.resolveExisting(timestampCandidate, { fs });
    if (resolved && (await fs.pathExists(resolved))) {
      timestampsAbs = resolved;
      break;
    }
  }
  if (!timestampsAbs) {
    recordProblem("approved_voice:metadata_missing");
    return problemResult();
  }

  let payload;
  try {
    payload = await fs.readJson(timestampsAbs);
  } catch (err) {
    recordProblem(`approved_voice:metadata_unreadable:${err.code || "unknown"}`);
    return problemResult();
  }

  const meta = payload && typeof payload === "object" ? payload.meta || {} : {};
  const provider = String(meta.provider || "").trim();
  const source = String(meta.source || "").trim();
  if (!provider || !source) {
    recordProblem("approved_voice:metadata_missing");
    return problemResult();
  }

  const repair = captionTimingRepair(meta);
  const repairReason = repair
    ? String(repair.reason || repair.repairReason || "unknown").trim() || "unknown"
    : null;
  if (provider.toLowerCase() === "local" && repairReason) {
    const code = `approved_voice:caption_timing_repaired:${repairReason}`;
    if (strict && !allowRepairedCaptionTimingForPublish(env)) failures.push(code);
    else if (captionTimingRepairIsUsable(repair)) warnings.push(code);
    else recordProblem(code);
  }

  const narration = {
    provider,
    source,
    audioPath: audioAbs,
    transcript: transcriptFromPayload(payload, story.tts_script || story.full_script || ""),
    acoustic: acousticFromMeta(meta),
    elevenlabs:
      meta.elevenlabs && typeof meta.elevenlabs === "object"
        ? meta.elevenlabs
        : null,
    voiceMastering:
      meta.voiceMastering ||
      meta.voice_mastering ||
      meta.mastering ||
      null,
    approvedLocalVoice: meta.approvedLocalVoice,
    acceptedLocalVoice:
      meta.acceptedLocalVoice ||
      meta.localVoiceReference ||
      meta.voiceReference ||
      null,
  };

  const approved = evaluateApprovedVoicePath({
    narration,
    env,
    requireExistingAudio: true,
  });

  for (const failure of approved.blockers) {
    const code = `approved_voice:${failure}`;
    if (isAlwaysBlockingVoiceFailure(failure)) failures.push(code);
    else recordProblem(code);
  }
  warnings.push(...approved.warnings.map((warning) => `approved_voice:${warning}`));

  return {
    result: failures.length > 0 ? "fail" : warnings.length > 0 ? "warn" : "pass",
    failures,
    warnings,
    inspected: true,
    strict,
    voice: {
      provider: approved.provider,
      source: approved.source,
      verdict: approved.verdict,
      local_voice: approved.local_voice,
      local_voice_reference_approved: approved.local_voice_reference_approved,
      transcript: approved.transcript,
      acoustic: approved.acoustic,
    },
  };
}

function isAlwaysBlockingVoiceFailure(failure) {
  return ALWAYS_BLOCKING_VOICE_FAILURES.has(String(failure || "").trim());
}

module.exports = {
  isAlwaysBlockingVoiceFailure,
  runPublishVoiceQa,
  shouldInspectVoice,
  strictVoiceQaEnabled,
  timestampPathForAudio,
  captionTimingRepair,
  captionTimingRepairReason,
  captionTimingRepairIsUsable,
  allowRepairedCaptionTimingForPublish,
};
