...
const DEFAULT_MAX_PUBLIC_POSTS_PER_24H = 2;
...
function shouldHardGatePublishCadence(env = {}, hardGateKeys = []) {
  const liveGuarded =
    String(env?.PULSE_OPERATING_MODE || "").trim().toUpperCase() ===
    "LIVE_GUARDED";
  if (liveGuarded || isActivePrimaryAutoPublisher(env)) return true;
...
function latestPublishedStory({ stories = [], now = new Date() } = {}) {
  ...
  for (const story of Array.isArray(stories) ? stories : []) {
    // A real external ID is public-object evidence even when the legacy
    // row says "failed" or review-blocked. Count it conservatively until
    // platform-side reconciliation proves otherwise; ignoring it can
    // create a duplicate or violate the rolling cadence.
...
function countPublicPostsInWindow({
  ...
  for (const story of Array.isArray(stories) ? stories : []) {
    // See latestPublishedStory(): conflicted rows with real platform
    // evidence still consume cadence until reconciliation.
    if (!storyHasRealPlatformId(story)) continue;
    const at = parseDate(storyPublishedAt(story));
