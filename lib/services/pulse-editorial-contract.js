...
const DURATION_BANDS = Object.freeze({
  what_changes_breaking_flash_18_24: Object.freeze({
    id: "what_changes_breaking_flash_18_24",
    lane_id: "what_changes_for_players",
    variant: "breaking_flash",
    label:
      "What Changes governed measured Breaking Flash runtime: 18–24 seconds",
    format_family: "short",
    min_seconds: 18,
    max_seconds: 24,
...
function resolvePulseScriptContract({
  ...
  // Older processor rows wrote the selected band here as { min, max }.
  // Accept only that exact redundant shape; scalar targets remain strict.
  const legacyDurationBandRange =
    rawTargetDuration !== null &&
    typeof rawTargetDuration === "object" &&
    !Array.isArray(rawTargetDuration) &&
    Object.keys(rawTargetDuration).length === 2 &&
    Object.hasOwn(rawTargetDuration, "min") &&
    Object.hasOwn(rawTargetDuration, "max") &&
    Number(rawTargetDuration.min) === durationBand.min_seconds &&
    Number(rawTargetDuration.max) === durationBand.max_seconds;
  const hasTargetDuration =
    !legacyDurationBandRange &&
    rawTargetDuration !== null &&
