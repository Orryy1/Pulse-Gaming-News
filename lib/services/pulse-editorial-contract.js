...
function resolveDurationBand({ story = {}, lane } = {}) {
  ...
  if (explicitBandId) {
    ...
    return {
      ...
      selection: {
        basis: "explicit_story_contract",
        cohort_count: explicitBand.format_family === "short" ? 2 : 1,
        bucket:
          explicitBand.format_family === "short"
            ? SHORT_DURATION_BANDS_BY_LANE[editorialLane.id].indexOf(
                explicitBand.id,
              )
            : 0,
        audit_hash: null,
...
function resolvePulseScriptContract({
  ...
  return {
    ...
    experiment_id:
      lane.format_family === "short" ? CONTROLLED_EXPERIMENT.id : null,
    ...
    seconds_per_word: normalisedSecondsPerWord,
    min_words: Math.ceil(durationBand.min_seconds / normalisedSecondsPerWord),
    max_words: Math.floor(durationBand.max_seconds / normalisedSecondsPerWord),
    experiment_cell_id:
      lane.format_family === "short"
