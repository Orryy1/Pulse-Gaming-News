...
const LANE_BY_NAME = {
  ...
  produce_late_morning: "normal",
  produce_afternoon: "normal",
  produce_mid_afternoon: "normal",
  produce_primary: "normal",
  publish_morning: "normal",
  publish_late_morning: "normal",
  publish_afternoon: "normal",
  publish_mid_afternoon: "normal",
  ...
  overnight_produce_sweep: "normal",
  overnight_analytics_backfill: "analytics",
  overnight_claude_analyst: "analytics",
  overnight_morning_digest: "analytics",
  live_performance_analyst_30m: "analytics",
...
function buildSchedulerPlan(schedules) {
  ...
  const hasLateMorningPublish = items.some(
    (i) => i.name === "publish_late_morning" && i.enabled,
  );
  const hasAfternoonPublish = items.some(
    (i) => i.name === "publish_afternoon" && i.enabled,
  );
  const hasMidAfternoonPublish = items.some(
    (i) => i.name === "publish_mid_afternoon" && i.enabled,
  );
  const hasEveningPublish = items.some(
    (i) => i.name === "publish_primary" && i.enabled,
  );
  const publishSlotCount = items.filter(
    (i) => i.kind === "publish" && i.enabled,
  ).length;
  ...
  return {
    ...
    cadence_status: {
      ...
      late_morning_publish: hasLateMorningPublish,
      afternoon_publish: hasAfternoonPublish,
      mid_afternoon_publish: hasMidAfternoonPublish,
      evening_publish: hasEveningPublish,
      daily_publish_slots: publishSlotCount,
    },
