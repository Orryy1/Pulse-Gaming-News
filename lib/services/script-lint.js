...
const BANNED_PHRASES = [
  ...
  {
    re: /\b(?:a\s+)?verified insider claims\b/i,
    reason: "unsupported_verified_insider",
  },
  {
    re: /\baccording to (?:a )?verified reddit post\b/i,
    reason: "fake_verified_reddit_source",
  },
  {
    re: /\bthe community is buzzing\b/i,
    reason: "community_buzzing_filler",
  },
  {
    re: /\b(?:changes?|changed|changing) everything\b/i,
    reason: "changes_everything",
  },
  {
    re: /\b(?:but here(?: is|'?s) where it gets interesting|this is bigger than you think|this is the part nobody is reporting)\b/i,
    reason: "formulaic_pivot",
  },
  {
    re: /\b(?:for pulse|direction of travel|signal first|signal over certainty|safe(?:st)? read|safe(?:st)? takeaway|tracking confirmation|tracking the official follow[- ]up)\b/i,
    reason: "internal_pulse_language",
  },
  {
    re: /\b(?:the source context is|the story angle is manual review|that is the player consequence|use official wording for the follow-up)\b/i,
    reason: "repair_boilerplate_leaked",
  },
  {
    re: /\b(?:so the clean read is this|clean read|broader launch data|source-backed update|not a blank cheque|wait-and-see column|the disciplined version is simple|the useful detail is the review-score framing|the useful caveat is)\b/i,
    reason: "boring_source_bound_recap",
  },
  {
    ...
    reason: "internal_source_policy_language",
  },
  {
    re: /\bthe catch is what matters after the reveal cut\b/i,
    reason: "generic_reveal_catch_template",
  },
  {
    re: /\b(?:the catch is simple|the debate is simple|the real question is|the debate finally has a real stress test)\b/i,
    reason: "generic_simple_pivot",
  },
];
...
const CURIOSITY_MARKERS = [
  ...
  /\b(?:the\s+(?:reason|catch|twist)|(?:has|with)\s+(?:a\s+|one\s+)?(?:real\s+|awkward\s+)?catch|(?:a|one|real|awkward)\s+catch)\b/i,
  /\btrade[- ]off\b/i,
  /\brisk\b/i,
  /\bwho gets access\b/i,
  /\bwhether\b.{0,140}\bor just\b/i,
  /\bif\b.{0,80}\b(?:holds|lands|survives|proves|turns)\b/i,
  /the kicker/i,
  /only the opening beat/i,
  /one high score can hide/i,
  /the uncomfortable bit/i,
  /where .* wins might not be/i,
  /hype (?:either )?turns into trust/i,
  /paywall confusion/i,
];
...
function lintScript(script, opts = {}) {
  ...
  if (HYBRID_SPOKEN_YEAR_RE.test(script)) failures.push("hybrid_spoken_year");
