...
// Legacy word-count bounds for callers that have not selected an editorial
// duration contract. Canonical Pulse callers pass the selected contract and
// use its derived word range instead.
 ...
 * @param {{ minWords?: number, maxWords?: number, contract?: object }} [opts]
 */
function lintScript(script, opts = {}) {
  ...
  if (
    ...
    (!Number.isFinite(Number(contract.min_words)) ||
      ...
      Number(contract.min_words) < 0 ||
      Number(contract.max_words) < Number(contract.min_words))
  ) {
    return {
      result: "fail",
      failures: ["script_contract_invalid"],
      warnings: [],
      word_count: 0,
      word_range: null,
      duration_band_id: contract.duration_band_id || null,
    };
  }
  const minWords = contract
    ? Number(contract.min_words)
    : typeof opts.minWords === "number"
      ? opts.minWords
      : DEFAULT_MIN_WORDS;
  const maxWords = contract
    ? Number(contract.max_words)
    : typeof opts.maxWords === "number"
      ? opts.maxWords
      : DEFAULT_MAX_WORDS;
  ...
  if (words < minWords) {
    ...
    const overflow = `script_too_long (${words} words, max ${maxWords})`;
    if (contract) failures.push(overflow);
    else warnings.push(overflow);
  }
