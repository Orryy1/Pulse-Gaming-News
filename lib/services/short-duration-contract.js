...
const DEFAULT_MIN_RETENTION_SHORT_VIDEO_SECONDS = 22;
const DEFAULT_MIN_NORMAL_PRODUCTION_VIDEO_SECONDS = 35;
const DEFAULT_MAX_SHORT_VIDEO_SECONDS = 75;
const DEFAULT_MAX_NORMAL_PRODUCTION_VIDEO_SECONDS = 60;
...
const RETENTION_DURATION_LANE = "pulse_retention_short";
const NORMAL_PRODUCTION_DURATION_LANE = "normal_production";
const BREAKING_NEWS_DURATION_LANE = "breaking_news";
...
const RETENTION_DURATION_LANE_ALIASES = new Set([
  RETENTION_DURATION_LANE,
  "retention_short",
  "standard_retention_short",
]);

const NORMAL_PRODUCTION_DURATION_LANE_ALIASES = new Set([
...
const BREAKING_NEWS_DURATION_LANE_ALIASES = new Set([
  BREAKING_NEWS_DURATION_LANE,
  "breaking_news_short",
  "breaking_news_measurement_required",
  "verified_first_party_breaking",
]);
...
function normaliseDurationLane(value) {
  ...
  if (RETENTION_DURATION_LANE_ALIASES.has(lane)) return RETENTION_DURATION_LANE;
  if (NORMAL_PRODUCTION_DURATION_LANE_ALIASES.has(lane)) {
    return NORMAL_PRODUCTION_DURATION_LANE;
  }
  if (BREAKING_NEWS_DURATION_LANE_ALIASES.has(lane)) {
    return BREAKING_NEWS_DURATION_LANE;
  }
...
function durationBoundsForLane(lane) {
  ...
  if (durationLane === RETENTION_DURATION_LANE) {
    return {
      durationLane,
      minVideoSeconds: DEFAULT_MIN_RETENTION_SHORT_VIDEO_SECONDS,
  ...
  if (durationLane === BREAKING_NEWS_DURATION_LANE) {
    return {
      durationLane,
      minVideoSeconds: DEFAULT_MIN_NORMAL_PRODUCTION_VIDEO_SECONDS,
      maxVideoSeconds: DEFAULT_MAX_NORMAL_PRODUCTION_VIDEO_SECONDS,
...
module.exports = {
  ...
  DEFAULT_MIN_RETENTION_SHORT_VIDEO_SECONDS,
