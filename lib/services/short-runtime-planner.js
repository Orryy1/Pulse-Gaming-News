...
} = require("../stabilisation/brand-content-contract");
...
function resolveStabilisationRuntimeBand(story = {}) {
  const storyType = String(story.stabilisation_story_type || "")
    .trim()
    .toLowerCase();
  if (INITIAL_RUNTIME_BANDS[storyType]) {
    return {
      ...INITIAL_RUNTIME_BANDS[storyType],
      durationLane: storyType,
      source: "initial_story_type",
    };
  }

  const laneId = String(story.stabilisation_lane_id || "")
    .trim()
    .toLowerCase();
  const experimentBand = String(story.experiment_band || "")
    .trim()
    .toLowerCase();
  const band = EXPERIMENT_RUNTIME_BANDS[laneId]?.[experimentBand];
  if (band && Number.isFinite(band.min_seconds)) {
    return {
      ...band,
      durationLane: `${laneId}:${experimentBand}`,
      source: "controlled_12_video_experiment",
    };
  }
  return null;
}
...
function classifyShortScriptRuntime({
  ...
  story,
  format,
  minSeconds: requestedMinSeconds,
  maxSeconds: requestedMaxSeconds,
  reviewMaxSeconds: requestedReviewMaxSeconds,
  secondsPerWord = DEFAULT_SECONDS_PER_WORD,
  measuredAudioSeconds,
} = {}) {
  const fmt = format || storyFormatCandidate(story);
  const stabilisationBand = resolveStabilisationRuntimeBand(story);
  const extendedShort =
    !stabilisationBand && isExtendedShortFormat(fmt);
  const minSeconds =
    Number.isFinite(Number(requestedMinSeconds))
      ? Number(requestedMinSeconds)
      : stabilisationBand?.min_seconds ?? FLASH_MIN_SECONDS;
  const maxSeconds =
    Number.isFinite(Number(requestedMaxSeconds))
      ? Number(requestedMaxSeconds)
      : stabilisationBand?.max_seconds ?? FLASH_MAX_SECONDS;
  const reviewMaxSeconds =
    Number.isFinite(Number(requestedReviewMaxSeconds))
      ? Number(requestedReviewMaxSeconds)
      : stabilisationBand?.max_seconds ?? REVIEW_MAX_SECONDS;
  ...
  if (firstPartyAnnouncement.qualifies && !stabilisationBand) {
    const measured =
  ...
  return {
    ...
    reviewMaxWords,
    format: fmt,
    durationLane: stabilisationBand?.durationLane || null,
    stabilisationAuthoritative: Boolean(stabilisationBand),
    runtimePolicySource: stabilisationBand?.source || "legacy_flash_contract",
  };
}
...
module.exports = {
  ...
  resolveStabilisationRuntimeBand,
  FLASH_MIN_SECONDS,
  FLASH_MAX_SECONDS,
  REVIEW_MAX_SECONDS,
