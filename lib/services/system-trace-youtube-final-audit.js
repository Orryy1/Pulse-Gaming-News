...
function defaultContract() {
  return {
    ...
    episodes: EXPECTED_EPISODES.map(([storyId, publishAtUtc]) => [
      storyId,
      publishAtUtc,
    ]),
  };
...
function resolveContract(value) {
  if (value === undefined) return defaultContract();
  ...
  const episodes = Array.isArray(value?.episodes)
    ? value.episodes.map((entry) => [clean(entry?.[0]), exactUtc(entry?.[1])])
    : [];
  const storyIds = episodes.map(([storyId]) => storyId);
  const publishTimes = episodes.map(([, publishAtUtc]) => publishAtUtc);
  if (
    !sha256Hex(manifestSha256) ||
    !channelId ||
    episodes.length !== 7 ||
    episodes.some(([storyId, publishAtUtc]) => !storyId || !publishAtUtc) ||
    new Set(storyIds).size !== episodes.length ||
    new Set(publishTimes).size !== episodes.length
  ) {
    throw new Error("system_trace_youtube_final_audit_contract_invalid");
  }
  return { manifestSha256, channelId, episodes };
}
...
function exactPackageCaptionBinding(episode, packageEvidence, contract) {
  const blockers = [];
  ...
  if (
    ...
    canonical?.destination?.channel_id !== contract.channelId ||
...
function manifestBlockers(manifest, manifestSha256, contract) {
  const blockers = [];
  if (clean(manifestSha256).toLowerCase() !== contract.manifestSha256) {
  ...
  if (
    ...
    manifest?.channel?.id !== contract.channelId ||
    manifest?.channel?.title !== "Pulse Gaming" ||
    !Array.isArray(manifest?.episodes) ||
    manifest.episodes.length !== contract.episodes.length
  ) {
    blockers.push("exact_closed_buffer_manifest_required");
  }
  for (const [index, expected] of contract.episodes.entries()) {
...
function receiptBlockers(
  ...
  if (privateReceipt?.receipt_type === "governed_youtube_dispatch") {
    ...
    const binding = exactPackageCaptionBinding(episode, packageEvidence, contract);
  ...
  if (
    ...
    scheduleReceipt?.channel_id !== contract.channelId ||
...
function videoChecks(observed, episode, videoId, contract) {
  const publishAt = exactUtc(episode.publish_at_utc);
  return {
    video_id_exact: observed.id === videoId,
    channel_id_exact: observed.channel_id === contract.channelId,
...
async function auditEpisode(
  ...
  if (videoResponse && matchingVideos.length !== 1) {
    ...
    Object.assign(
      result.checks,
      videoChecks(result.observed.video, episode, videoId, contract),
    );
  }
...
async function auditSystemTraceYouTubeBuffer(input = {}) {
  ...
  return {
    ...
    channel: { id: contract.channelId, title: "Pulse Gaming" },
