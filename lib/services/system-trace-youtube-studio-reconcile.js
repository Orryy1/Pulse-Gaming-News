...
function normaliseIntent(input = {}) {
  return {
    ...
    tags: Array.isArray(input.tags)
      ? input.tags.map((tag) => String(tag ?? "")).filter((tag) => tag.length > 0)
      : [],
...
function validateIntent(intent) {
  ...
  if (blockers.length) {
    const error = new Error("system_trace_youtube_studio_reconcile_input_blocked");
...
function exactBlockers(observed, intent) {
  ...
  if (observed.privacy_status !== "private") blockers.push("privacy_status_mismatch");
  if (observed.upload_status !== "processed") blockers.push("upload_status_mismatch");
  if (observed.processing_status !== "succeeded") blockers.push("processing_status_mismatch");
...
async function executeSystemTraceYouTubeStudioReconcile(input = {}) {
  ...
  try {
    await input.client.videos.update(
      snippetRepairRequest(intent),
      { ...REQUEST_OPTIONS },
    );
  } catch (error) {
    updateError = error;
  ...
  error.receipt = {
    ...receiptBase(intent, generatedAt),
    verdict: "RED",
    status: updateError ? "PRIVATE_RECONCILIATION_OUTCOME_UNKNOWN" : "PRIVATE_RECONCILIATION_FAILED",
    verification_outcome: updateError
...
async function verifyExactSystemTraceYouTubeStudioVideo(input = {}) {
  ...
  if (blockers.length) {
    const error = new Error("system_trace_youtube_studio_exact_readback_blocked");
    error.blockers = blockers;
...
function assertExactDeferredRedReceipt(receipt, intent, sourceReceiptSha256) {
  ...
  if (
    !receipt?.readback ||
    !isDeepStrictEqual(exactBlockers(receipt.readback, intent), ["default_language_missing"])
  ) blockers.push("source_receipt_readback_mismatch");
  if (blockers.length) {
    const error = new Error("system_trace_youtube_studio_deferred_source_receipt_blocked");
    error.blockers = blockers;
    throw error;
  }
}

async function executeDeferredSystemTraceYouTubeStudioReconciliation(input = {}) {
  const intent = normaliseIntent(input.intent);
  validateIntent(intent);
...
function assertExactDeferredGreenProof(proof, intentInput, sourceReceiptSha256) {
  ...
  if (blockers.length) {
    const error = new Error("system_trace_youtube_studio_deferred_green_proof_blocked");
    error.blockers = blockers;
    throw error;
  }
  return proof;
