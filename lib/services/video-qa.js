 ...
 *   5. Reused motion sequences and sustained low-cadence/stutter
 *      patterns, measured from a low-resolution perceptual sample
...
function analyzeTemporalFrames(
  ...
  for (
    ...
    maxWindowNearStaticRatio = Math.max(
      maxWindowNearStaticRatio,
      nearStatic / pairCount,
    );
  }
...
function validateTemporalVideoQaReport(
  ...
  if (Number(temporal.repeated_motion_seconds || 0) > 0) {
...
function classifyVideoQa({
  ...
  if (requireTemporalScan && !temporal) {
    ...
    const repeatedMotion = Array.isArray(temporal.repeated_motion_sequences)
      ? temporal.repeated_motion_sequences
      : [];
    if (repeatedMotion.length > maxRepeatedMotionSequences) {
      failures.push(
