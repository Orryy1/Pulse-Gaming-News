...
const HOST_SOURCE_NAMES = new Map([
  ...
  ["slayersclub.bethesda.net", "Slayers Club"],
...
function sourceNameFromUrl(url) {
  try {
    ...
    for (const [knownHost, sourceName] of HOST_SOURCE_NAMES.entries()) {
      if (host.endsWith(`.${knownHost}`)) return sourceName;
    }
...
function specificContextSentences(story = {}, sourceMaterial = "") {
  ...
  if (isForzaSteamPlayerStory(story, sourceMaterial)) {
    ...
    sentences.push(
      ...
      "For Xbox, the comparison is clear: the premium-only opening has already beaten the last game's biggest Steam moment.",
  ...
  sentences.push(
    "The tension is whether this changes price, access, platform support or trust once the official details land.",
...
function buildXboxPcBackwardCompatibilityFallback({
  story = {},
  sourceMaterial = "",
  minWords = 80,
  maxWords = 180,
  ...
  const evidence = normaliseText(sourceMaterial);
  const requiredFacts = [
    /backward compatibility on pc/i,
    /four classic original xbox games/i,
    /blinx(?:: the time sweeper)?/i,
    /conker: live and reloaded/i,
    /crimson skies(?:: high road to revenge)?/i,
    /fuzion frenzy/i,
    /(?:digital )?licen[cs]es?[^.]{0,100}carr(?:y|ies) over/i,
    /achievements?[^.]{0,100}(?:coming months|later this year)/i,
  ];
  if (
    source?.key !== "xbox_wire" ||
    !requiredFacts.every((pattern) => pattern.test(evidence))
  ) {
    return null;
  }
  ...
  const bodySentences = [
    ...
    "That turns a nostalgia drop into a test of Xbox Play Anywhere.",
    "If Microsoft expands the catalogue quickly, old console purchases become a real portable PC library.",
  ];
  const fullScript = `${hook} ${bodySentences.join(" ")} ${EXACT_CTA}`;
  ...
  return {
    ...
    suggested_thumbnail_text: "4 XBOX CLASSICS",
    suggested_title: "4 Xbox Classics Hit PC, Achievements Come Later",
    content_pillar: "Confirmed Drop",
    script_generation_status: "script_ready",
    script_source: "source_bound_first_party_breaking",
    format_route: "breaking_news_measurement_required",
    runtime_route: "breaking_news_measurement_required",
...
function buildSourceBoundFallbackScript(story = {}, options = {}) {
  ...
  const verifiedBreakingFallback = buildXboxPcBackwardCompatibilityFallback({
    story,
    sourceMaterial,
    minWords,
    maxWords,
  });
  if (verifiedBreakingFallback) return verifiedBreakingFallback;
  ...
  const pads = [
    ...
    "If the detail holds, it changes the buy, download, trust or timing decision instead of becoming background noise.",
  ];
