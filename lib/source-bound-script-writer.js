...
function titleSubject(title = "") {
  ...
  return known ? known[0] : "This gaming story";
}
...
function isForzaSteamPlayerStory(story = {}, sourceMaterial = "") {
  ...
  return /\b(?:Steam|concurrent|players?|early[-\s]?access|premium|Steam peak|Steam record|\$120|120 dollars)\b/i.test(
    storyAndSourceText(story, sourceMaterial),
  );
}
...
function buildHook(story = {}, sourceMaterial = "") {
  ...
  return sentenceCase(`${titleSubject(title)} just got a source-backed update`);
}
...
function specificContextSentences(story = {}, sourceMaterial = "") {
  ...
  if (isForzaReviewScoreStory(story, sourceMaterial)) {
    sentences.push(
      "The useful detail is the review-score framing, not a blank cheque to invent launch data.",
      ...
      "For players, the practical question is whether the praised setting and driving model hold up once the full audience arrives.",
      "The disciplined version is simple: cite the score, keep the caveat clear and do not turn critic momentum into fake certainty.",
    );
    if (/Japan/i.test(sourceMaterial) || /japan/i.test(title)) {
      sentences.push(
  ...
  sentences.push(
    "The useful caveat is that this is one sourced update, not a blank cheque to invent extra details.",
    ...
    "A smaller factual story is better than pretending there is certainty the source does not provide.",
    "For now, the safest public version is the concrete claim, the named source and what players can actually do with it.",
  );
  return sentences;
}
...
function sourceLeadSentence(story = {}, sourceName = "the source") {
  ...
  return `${sourceName} reports the core detail plainly: ${sentenceCase(title)}`;
}
...
function buildSourceBoundFallbackScript(story = {}, options = {}) {
  ...
  const pads = [
    ...
    "For players, the question is practical: does this change what you buy, download, trust or watch next?",
  ];
