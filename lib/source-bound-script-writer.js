...
function specificContextSentences(story = {}, sourceMaterial = "") {
  ...
  if (isForzaReviewScoreStory(story, sourceMaterial)) {
    sentences.push(
      ...
      "Keep the claim tight: cite the score and do not turn critic momentum into fake certainty.",
    );
    if (/Japan/i.test(sourceMaterial) || /japan/i.test(title)) {
      sentences.push(
  ...
  sentences.push(
    ...
    "Keep the claim to the source, then explain what players can actually do with it.",
  );
  return sentences;
}
...
function buildSourceBoundFallbackScript(story = {}, options = {}) {
  ...
  const pads = [
    "The key is to keep the claim tight and useful.",
    "Anything outside the report should stay out of the narration.",
    "That keeps the story useful without padding it into fake certainty.",
