...
function parseTransitionMetadata(value) {
  try {
    ...
    for (const item of parsed.participants) {
      ...
      const processStartSource = String(
        item.process_start_source || "",
      ).trim();
      if (
        !participantId ||
...
function inspectTransitionOwnerProcess(candidate = {}) {
  ...
  const expectedStartedAt = String(
    candidate.process_started_at || "",
  ).trim();
...
function ownedSnapshot(leases, ownerId, participantIdentity, role) {
  ...
  const exactParticipant = metadata.participants.find(
    (item) => sameParticipant(item, participantIdentity),
  );
...
function acquireLiveRuntimeTransitionLease({
  ...
  const processInspector =
    participantProcessInspector || ownerProcessInspector;
  let handle;
  try {
    ...
    sealForHandoff({ childParticipantIdentity } = {}) {
      ...
      try {
        for (let attempt = 0; attempt < MAX_METADATA_CAS_ATTEMPTS; attempt += 1) {
          ...
          if (
            !snapshot ||
            snapshot.metadata.admission_state !==
              TRANSITION_ADMISSION_OPEN ||
          ...
          if (
            handle.leases.compareAndSwapMetadata({
              ...
              metadata: sealedMetadataWithParticipants(
                snapshot.metadata,
                [...snapshot.metadata.participants, handoffChild],
              ),
              now: new Date(),
              leaseMs: duration,
              allowExpired: true,
...
function borrowLiveRuntimeTransitionLease({
  ...
  try {
    ...
    for (let attempt = 0; attempt < MAX_METADATA_CAS_ATTEMPTS; attempt += 1) {
      const current = handle.leases.get(
      ...
      if (
        ...
        currentMetadata.participants.length >=
