...
function canonicalAuthorityContextSha256(value) {
  if (
    typeof value !== "string" ||
    !/^[0-9a-f]{64}$/.test(value)
  ) {
    throw new Error("live_runtime_transition_authority_context_invalid");
  }
  return value;
}

function isCanonicalAuthorityContextSha256(value) {
  return typeof value === "string" && /^[0-9a-f]{64}$/.test(value);
}

function optionalAuthorityContextSha256(value) {
  return value === null || value === undefined
    ? null
    : canonicalAuthorityContextSha256(value);
}

function authorityContextMetadata(authorityContextSha256) {
  return authorityContextSha256 === null
    ? {}
...
function transitionMetadata(
  ...
  return {
    ...
    ...authorityContextMetadata(authorityContextSha256),
...
function parseTransitionMetadata(value) {
  try {
    ...
    if (
      ...
      (Object.hasOwn(parsed, "authority_context_sha256") &&
        !isCanonicalAuthorityContextSha256(
          parsed.authority_context_sha256,
        )) ||
    ...
    return {
      transition_owner_schema_version: TRANSITION_OWNER_METADATA_SCHEMA_VERSION,
      authority_context_sha256: Object.hasOwn(
        parsed,
...
function metadataWithParticipants(metadata, participants) {
  return {
    transition_owner_schema_version: TRANSITION_OWNER_METADATA_SCHEMA_VERSION,
    ...authorityContextMetadata(metadata.authority_context_sha256),
...
function freshlyAssertAuthorityContext(
  authorityContextProvider,
  admittedAuthorityContextSha256,
  failureCode,
) {
  if (authorityContextProvider === null || authorityContextProvider === undefined) {
    return admittedAuthorityContextSha256;
  }
  if (typeof authorityContextProvider !== "function") {
    throw new Error(failureCode);
  }
  let measured;
  try {
    measured = canonicalAuthorityContextSha256(authorityContextProvider());
  } catch {
    throw new Error(failureCode);
  }
  if (measured !== admittedAuthorityContextSha256) {
    throw new Error(failureCode);
  }
  return measured;
}

function liveTransitionRequiresFreshAuthorityProvider(value) {
  return /^live(?:-|:)/.test(String(value || "").trim().toLowerCase());
}
...
function validateLiveRuntimeTransitionLeaseRow(
  ...
  if (
    authorityContextSha256 !== null &&
    (!isCanonicalAuthorityContextSha256(authorityContextSha256) ||
      metadata.authority_context_sha256 !== authorityContextSha256)
  ) {
    return null;
  }
...
function acquireLiveRuntimeTransitionLease({
  ...
  const requiresFreshAuthority = liveTransitionRequiresFreshAuthorityProvider(
    action,
  );
  const authorityContext = requiresFreshAuthority
    ? canonicalAuthorityContextSha256(authorityContextSha256)
    : optionalAuthorityContextSha256(authorityContextSha256);
  ...
  try {
    if (
      ...
      typeof authorityContextProvider !== "function"
    ) {
  ...
  return Object.freeze({
    ...
    lease_ms: duration,
    authority_context_sha256: authorityContext,
    borrowed: false,
    assertCurrentAuthority() {
      ...
      try {
        ...
        return freshlyAssertAuthorityContext(
          authorityContextProvider,
          authorityContext,
          lostCode,
        );
        ...
        for (
          ...
          if (
            handle.leases.compareAndSwapMetadata({
              ...
              metadata: metadataWithParticipants(
                snapshot.metadata,
...
function borrowLiveRuntimeTransitionLease({
  ...
  try {
    ...
    for (let attempt = 0; attempt < MAX_METADATA_CAS_ATTEMPTS; attempt += 1) {
      ...
      if (
        !currentMetadata ||
        currentMetadata.authority_context_sha256 !== authorityContext ||
