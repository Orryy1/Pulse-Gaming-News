...
function boundedAuthorityHold(blockers, values = {}) {
  return {
    ...
    runtime_instance_id: values.runtime_instance_id || null,
...
async function inspectLiveBoundedAuthorityObservation({ expected, probes }) {
  ...
  if (
    ...
    (taskDefinition?.blockers || []).length !== 0
  ) {
  ...
  if (!safeLiveHealthIdentity(health, binding.release_sha)) {
...
async function inspectQuiescentBoundedAuthorityObservation({
  ...
  if ((taskDefinition?.blockers || []).length !== 0) {
  ...
  if (
    ...
    (taskInstances?.blockers || []).length !== 0
  ) {
    blockers.push("quiescent_task_instance_present");
  }
  if (conflicts?.clear !== true || (conflicts?.blockers || []).length !== 0) {
  ...
  if (!["absent", "governed_stale", "exact_stale"].includes(owner?.state)) {
...
function inspectLiveActivationReceipt({
  ...
  const expectedAuthorityBinding = buildLiveTaskAuthorityBinding({
    taskName: profile?.task_name,
    nodePath: nodePath || receiptAuthorityBinding?.node_path,
    checkoutRealPath:
      checkoutRealPath || receiptAuthorityBinding?.checkout_real_path,
    releaseSha: commit,
    profileSha256: profileFingerprint(profile),
    databaseIdentitySha256:
      databaseIdentitySha256 ||
      receiptAuthorityBinding?.database_identity_sha256,
  });
...
async function inspectLiveTaskBoundHandoff({
  ...
  const child = await inspectWindowsAuthorityProcess({
    pid: childPid,
    expected: { parent_pid: Number(supervisorPid) },
    runPowerShell,
  });
  ...
  if (!job.process_ids.includes(Number(childPid))) {
    return { ok: false, blockers: ["live_task_child_not_in_job"] };
  }
  return {
    ok: true,
    task_name: profile.task_name,
