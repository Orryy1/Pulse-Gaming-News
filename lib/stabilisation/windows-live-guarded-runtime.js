...
async function settleLiveChildExit({
  ...
  if (terminationReason === "live_runtime_health_lost") {
    ...
    return restartImpl(restartOptions);
  }
...
async function runLiveSupervision({
  ...
  const restartSupervision = restartImpl || runLiveSupervision;

  return superviseLiveChildSession({
    child,
    ownerPath,
    profile,
    repoRoot,
    ...
    restartDelayMs,
    delayImpl,
    restartImpl: restartSupervision,
    restartOptions: {
      repoRoot,
      expectedCommit,
      profilePath,
