...
function buildLiveProcessCommandAuthority({
  ...
  const supervisorArguments = buildLiveScheduledTaskArguments({
    profile,
    repoRoot,
    commit,
  });
  const supervisorCommandLine = `${quoteWindowsArgument(canonicalNodePath)} ${supervisorArguments}`;
  const childCommandLine = `${quoteWindowsArgument(canonicalNodePath)} server.js`;
  return Object.freeze({
    supervisor_command_line: supervisorCommandLine,
    supervisor_command_sha256: sha256(supervisorCommandLine),
    child_command_line: childCommandLine,
    child_command_sha256: sha256(childCommandLine),
  });
}
      ...
      const exact =
        ...
        (owner.schema_version !== LIVE_OWNER_SCHEMA ||
          (validRuntimeInstanceId(owner.runtime_instance_id) &&
            ...
            validAbsoluteWindowsPath(expectedSupervisorExecutablePath) &&
            canonicalPath(owner.supervisor_executable_path).toLowerCase() ===
              canonicalPath(expectedSupervisorExecutablePath).toLowerCase() &&
            SHA256_PATTERN.test(
              String(expectedSupervisorCommandSha256 || ""),
            ) &&
            owner.supervisor_command_sha256 ===
              expectedSupervisorCommandSha256 &&
            validAbsoluteWindowsPath(expectedChildExecutablePath) &&
            canonicalPath(owner.child_executable_path).toLowerCase() ===
              canonicalPath(expectedChildExecutablePath).toLowerCase() &&
            SHA256_PATTERN.test(String(expectedChildCommandSha256 || "")) &&
            owner.child_command_sha256 === expectedChildCommandSha256 &&
    ...
    if (fs.existsSync(ownerPath)) {
      ...
      if (
        ...
        (staleOwner?.schema_version === LIVE_OWNER_SCHEMA &&
          (!staleReviewedProcessAuthority ||
            canonicalPath(
              staleOwner?.supervisor_executable_path,
            ).toLowerCase() !==
              canonicalPath(
                staleReviewedProcessAuthority.expectedSupervisorExecutablePath,
              ).toLowerCase() ||
            staleOwner?.supervisor_command_sha256 !==
              staleReviewedProcessAuthority.expectedSupervisorCommandSha256 ||
            canonicalPath(staleOwner?.child_executable_path).toLowerCase() !==
              canonicalPath(
                staleReviewedProcessAuthority.expectedChildExecutablePath,
              ).toLowerCase() ||
            staleOwner?.child_command_sha256 !==
              staleReviewedProcessAuthority.expectedChildCommandSha256))
      ) {
