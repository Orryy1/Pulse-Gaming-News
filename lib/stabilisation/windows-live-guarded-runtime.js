...
function validateObservedLiveAuthorityEvidence(authority) {
  ...
  const exactPersistentJobMembership =
    Array.isArray(processIds) &&
    processIds.length === 2 &&
    processIds.every(validPositivePid) &&
    new Set(processIds).size === 2 &&
  ...
  return (
    ...
    exactPersistentJobMembership &&
    hasNoBlockers(authority?.job_membership) &&
    authority?.job_membership?.supervisor_present === true &&
...
function decodeTaskXmlValue(value) {
  let valid = true;
  ...
  if (!valid || /&(?:#\d+|#x[a-f0-9]+|[a-z][\w.-]*);/i.test(decoded)) {
    throw new Error("task_xml_entity_invalid");
  }
  return decoded;
}

function parseTaskXmlAttributes(source) {
  ...
  while (index < source.length) {
    ...
    if (!nameMatch) throw new Error("task_xml_attribute_invalid");
    const name = nameMatch[1];
    index += name.length;
    while (/\s/.test(source[index] || "")) index += 1;
    if (source[index] !== "=") throw new Error("task_xml_attribute_invalid");
    index += 1;
    while (/\s/.test(source[index] || "")) index += 1;
    const quote = source[index];
    if (quote !== '"' && quote !== "'") {
      throw new Error("task_xml_attribute_invalid");
    }
    index += 1;
    const end = source.indexOf(quote, index);
    if (end < 0 || Object.prototype.hasOwnProperty.call(attributes, name)) {
      throw new Error("task_xml_attribute_invalid");
    }
    attributes[name] = decodeTaskXmlValue(source.slice(index, end));
    index = end + 1;
  }
  return attributes;
}

function findTaskXmlTagEnd(source, start) {
  let quote = null;
...
function parseClosedTaskXml(xml) {
  ...
  while (index < source.length) {
    if (source[index] !== "<") {
      ...
      const text = decodeTaskXmlValue(source.slice(index, end));
      if (stack.length === 0) {
        if (text.trim()) throw new Error("task_xml_text_outside_root");
      } else {
    ...
    if (
      ...
      throw new Error("task_xml_construct_forbidden");
    }
    const end = findTaskXmlTagEnd(source, index + 1);
    if (end < 0) throw new Error("task_xml_tag_invalid");
    const raw = source.slice(index + 1, end).trim();
    if (raw.startsWith("/")) {
      const closingName = raw.slice(1).trim();
      if (
        !/^[A-Za-z_][\w.-]*$/.test(closingName) ||
        stack.length === 0 ||
        stack[stack.length - 1].name !== closingName
      ) {
        throw new Error("task_xml_nesting_invalid");
      }
      stack.pop();
      index = end + 1;
      continue;
    ...
    if (!nameMatch) throw new Error("task_xml_tag_invalid");
    const name = nameMatch[1];
    const node = {
      name,
      attributes: parseTaskXmlAttributes(body.slice(name.length)),
      children: [],
      text: "",
    };
  ...
  if (stack.length !== 0 || roots.length !== 1) {
    throw new Error("task_xml_root_invalid");
  }
  return roots[0];
}

function taskXmlChildren(node, name) {
  return Array.isArray(node?.children)
    ? node.children.filter((child) => child.name === name)
    : [];
}

function taskXmlChild(node, name) {
  const matches = taskXmlChildren(node, name);
  return matches.length === 1 ? matches[0] : null;
}

function taskXmlHasExactChildren(node, expectedNames) {
  if (!node || node.text.trim()) return false;
  const actual = node.children.map((child) => child.name).sort();
  const expected = [...expectedNames].sort();
  return JSON.stringify(actual) === JSON.stringify(expected);
}

function taskXmlHasExactAttributes(node, expected) {
  if (!node) return false;
  const actualKeys = Object.keys(node.attributes).sort();
  const expectedKeys = Object.keys(expected).sort();
  return (
    JSON.stringify(actualKeys) === JSON.stringify(expectedKeys) &&
    expectedKeys.every((key) => node.attributes[key] === expected[key])
  );
}

function taskXmlLeafText(node) {
  return node && node.children.length === 0 ? node.text.trim() : null;
}
...
function validateLiveScheduledTaskXml({
  ...
  let root;
  try {
    root = parseClosedTaskXml(xml);
  } catch {
    return {
      valid: false,
      enabled: true,
      blockers: ["task_xml_structure_invalid"],
    };
  }
  ...
  const registration = taskXmlChild(root, "RegistrationInfo");
  const triggers = taskXmlChild(root, "Triggers");
  const principals = taskXmlChild(root, "Principals");
  const settings = taskXmlChild(root, "Settings");
  const actions = taskXmlChild(root, "Actions");
  requireMatch(
    root.name === "Task" &&
      taskXmlHasExactAttributes(root, {
        version: "1.4",
        xmlns: "http://schemas.microsoft.com/windows/2004/02/mit/task",
      }) &&
      taskXmlHasExactChildren(root, [
        "RegistrationInfo",
        "Triggers",
        "Principals",
        "Settings",
    ...
    "task_xml_structure_invalid",
  );
  const registrationChildren = registration?.children.map((child) => child.name);
  requireMatch(
    registration &&
      !registration.text.trim() &&
      taskXmlHasExactAttributes(registration, {}) &&
      Array.isArray(registrationChildren) &&
      registrationChildren.filter((name) => name === "Description").length === 1 &&
      registrationChildren.filter((name) => name === "URI").length <= 1 &&
      registrationChildren.every((name) => ["Description", "URI"].includes(name)) &&
      taskXmlLeafText(taskXmlChild(registration, "Description")) ===
        "Pulse Gaming reviewed live guarded YouTube runtime" &&
      (taskXmlChildren(registration, "URI").length === 0 ||
        taskXmlLeafText(taskXmlChild(registration, "URI")) ===
          `\\${LIVE_TASK_NAME}`),
    "task_registration_invalid",
  );

  const bootTrigger = taskXmlChild(triggers, "BootTrigger");
  requireMatch(
    taskXmlHasExactAttributes(triggers, {}) &&
      taskXmlHasExactChildren(triggers, ["BootTrigger"]),
    "task_trigger_set_invalid",
  );
  requireMatch(bootTrigger !== null, "boot_trigger_invalid");
  requireMatch(
    taskXmlHasExactAttributes(bootTrigger, {}) &&
      taskXmlHasExactChildren(bootTrigger, ["Enabled", "Delay"]) &&
      taskXmlLeafText(taskXmlChild(bootTrigger, "Enabled")) === "true",
    "boot_trigger_invalid",
  );
  requireMatch(
    taskXmlLeafText(taskXmlChild(bootTrigger, "Delay")) === "PT30S",
    "boot_trigger_delay_invalid",
  );

  const principal = taskXmlChild(principals, "Principal");
  requireMatch(
    taskXmlHasExactAttributes(principals, {}) &&
      taskXmlHasExactChildren(principals, ["Principal"]),
    "task_principal_set_invalid",
  );
  requireMatch(
    taskXmlHasExactAttributes(principal, { id: "PulseLiveGuarded" }) &&
      taskXmlHasExactChildren(principal, ["UserId", "RunLevel"]),
    "task_principal_set_invalid",
  );
  requireMatch(
    taskXmlLeafText(taskXmlChild(principal, "UserId")) === "S-1-5-18",
    "system_identity_required",
  );
  requireMatch(
    taskXmlChildren(principal, "LogonType").length === 0,
    "explicit_logon_type_forbidden_for_system_xml",
  );
  requireMatch(
    taskXmlLeafText(taskXmlChild(principal, "RunLevel")) ===
      "HighestAvailable",
    "highest_run_level_required",
  );

  const settingsFields = [
    ["MultipleInstancesPolicy", "IgnoreNew", "task_overlap_policy_invalid"],
    ["DisallowStartIfOnBatteries", "false", "task_battery_policy_invalid"],
    ["StopIfGoingOnBatteries", "false", "task_battery_policy_invalid"],
    ["StartWhenAvailable", "true", "task_catchup_policy_invalid"],
    ["RunOnlyIfNetworkAvailable", "false", "task_network_policy_invalid"],
    ["AllowHardTerminate", "true", "task_termination_policy_invalid"],
    ["AllowStartOnDemand", "true", "task_start_on_demand_policy_invalid"],
    ["Enabled", null, "task_enabled_state_invalid"],
    ["Hidden", "true", "task_hidden_policy_invalid"],
    ["RunOnlyIfIdle", "false", "task_idle_policy_invalid"],
    [
    ...
    ["WakeToRun", "true", "task_wake_policy_invalid"],
    ["ExecutionTimeLimit", "PT0S", "task_execution_limit_invalid"],
    ["Priority", "4", "task_priority_invalid"],
  ];
  requireMatch(
    taskXmlHasExactAttributes(settings, {}) &&
      taskXmlHasExactChildren(settings, [
        ...settingsFields.map(([name]) => name),
        "IdleSettings",
        "RestartOnFailure",
      ]),
    "task_settings_set_invalid",
  );
  for (const [name, expectedValue, blocker] of settingsFields) {
    const node = taskXmlChild(settings, name);
    requireMatch(
      taskXmlHasExactAttributes(node, {}) &&
        node?.children.length === 0 &&
        (expectedValue === null || taskXmlLeafText(node) === expectedValue),
      blocker,
    );
  }
  const idleSettings = taskXmlChild(settings, "IdleSettings");
  requireMatch(
    taskXmlHasExactAttributes(idleSettings, {}) &&
      taskXmlHasExactChildren(idleSettings, ["StopOnIdleEnd", "RestartOnIdle"]) &&
      taskXmlLeafText(taskXmlChild(idleSettings, "StopOnIdleEnd")) === "true" &&
      taskXmlLeafText(taskXmlChild(idleSettings, "RestartOnIdle")) === "false",
    "task_idle_policy_invalid",
  );
  const enabledText = taskXmlLeafText(taskXmlChild(settings, "Enabled"));
  const enabled = enabledText === "false" ? false : true;
  requireMatch(
    enabledText === "true" || enabledText === "false",
    "task_enabled_state_invalid",
  );
  const restartOnFailure = taskXmlChild(settings, "RestartOnFailure");
  requireMatch(
    taskXmlHasExactAttributes(restartOnFailure, {}) &&
      taskXmlHasExactChildren(restartOnFailure, ["Interval", "Count"]) &&
      taskXmlLeafText(taskXmlChild(restartOnFailure, "Interval")) === "PT1M" &&
      taskXmlLeafText(taskXmlChild(restartOnFailure, "Count")) === "999",
  ...
  const execAction = taskXmlChild(actions, "Exec");
  requireMatch(
    taskXmlHasExactAttributes(actions, { Context: "PulseLiveGuarded" }) &&
      taskXmlHasExactChildren(actions, ["Exec"]),
    "task_action_principal_binding_invalid",
  );
  requireMatch(
    taskXmlHasExactAttributes(execAction, {}) &&
      taskXmlHasExactChildren(execAction, [
        "Command",
        "Arguments",
        "WorkingDirectory",
      ]),
    "task_action_count_invalid",
  );
  requireMatch(
    canonicalPath(taskXmlLeafText(taskXmlChild(execAction, "Command"))).toLowerCase() ===
      canonicalPath(nodeExecutable).toLowerCase(),
    "task_node_identity_invalid",
  );
  requireMatch(
    canonicalPath(
      taskXmlLeafText(taskXmlChild(execAction, "WorkingDirectory")),
    ).toLowerCase() === canonicalPath(resolvedRoot).toLowerCase(),
    "task_working_directory_invalid",
  );
  const actionArguments = taskXmlLeafText(
    taskXmlChild(execAction, "Arguments"),
  );
  requireMatch(
    typeof actionArguments === "string" &&
      actionArguments.includes(toolPath),
    "task_tool_identity_invalid",
  );
  requireMatch(
    actionArguments === expectedCommand,
    "task_supervision_command_invalid",
  );
  requireMatch(
    /\bsupervise\b/.test(actionArguments || "") &&
      /--noninteractive\b/.test(actionArguments || ""),
    "task_noninteractive_supervisor_invalid",
  );
  requireMatch(
    !SECRET_KEY_PATTERN.test(String(xml || "")),
...
function inspectExactLiveTaskPresence({
  profile,
  execFileSyncImpl = execFileSync,
} = {}) {
  ...
  const literalTaskName = `'${LIVE_TASK_NAME.replace(/'/g, "''")}'`;
  const literalTaskPath = `'\\${LIVE_TASK_NAME.replace(/'/g, "''")}'`;
  const script = [
    ...
    "$rootFound = $false",
    `try { $null = $root.GetTask(${literalTaskPath}); $rootFound = $true } catch { if ($_.Exception.HResult -ne -2147024894) { throw } }`,
    ...
    [
      ...
      "if ([string]::IsNullOrWhiteSpace($folderPath)) { throw 'task_folder_identity_invalid' }",
      "$visitKey = $folderPath.ToUpperInvariant()",
      "if ($visited.ContainsKey($visitKey)) { throw 'task_folder_cycle_detected' }",
      "$visited[$visitKey] = $true",
      "if ($visited.Count -gt 256) { throw 'task_folder_limit_exceeded' }",
      "$tasks = $folder.GetTasks(1)",
      "$totalTasks += [int]$tasks.Count",
      "if ($totalTasks -gt 4096) { throw 'task_enumeration_limit_exceeded' }",
      "for ($taskIndex = 1; $taskIndex -le $tasks.Count; $taskIndex += 1) {",
      "$candidate = $tasks.Item($taskIndex)",
      "$candidateName = [string]$candidate.Name",
      "$candidatePath = [string]$candidate.Path",
      `if ($candidateName -ieq ${literalTaskName}) {`,
      "$matches += @($candidate)",
      `if ($candidateName -ceq ${literalTaskName} -and $candidatePath -ceq ${literalTaskPath}) { $exactRootMatches += @($candidate) }`,
      "}",
      "}",
      "$children = $folder.GetFolders(0)",
      "for ($childIndex = 1; $childIndex -le $children.Count; $childIndex += 1) { [void]$folders.Add($children.Item($childIndex)) }",
      "}",
    ].join("; "),
    "$present = ($rootFound -and $matches.Count -eq 1 -and $exactRootMatches.Count -eq 1)",
    `$exactName = if ($present) { ${literalTaskName} } else { $null }`,
    "[pscustomobject]@{RootFound=[bool]$rootFound;Present=[bool]$present;ExactName=$exactName;MatchCount=[int]$matches.Count;ExactRootCount=[int]$exactRootMatches.Count} | ConvertTo-Json -Compress",
  ].join("; ");
  try {
    const output = String(
      execFileSyncImpl(
    ...
    if (
      ...
      typeof value.RootFound !== "boolean" ||
      typeof value.Present !== "boolean" ||
      !Number.isInteger(value.MatchCount) ||
      value.MatchCount < 0 ||
      value.MatchCount > 4096 ||
      !Number.isInteger(value.ExactRootCount) ||
      value.ExactRootCount < 0 ||
      value.ExactRootCount > 1
    ) {
      return { available: false, present: null };
    }
    const provenAbsent =
      value.RootFound === false &&
      value.Present === false &&
      value.ExactName === null &&
      value.MatchCount === 0 &&
      value.ExactRootCount === 0;
    const provenPresent =
      value.RootFound === true &&
      value.Present === true &&
      value.ExactName === LIVE_TASK_NAME &&
      value.MatchCount === 1 &&
      value.ExactRootCount === 1;
    if (!provenAbsent && !provenPresent) {
      return { available: false, present: null };
    }
    return { available: true, present: value.Present };
  } catch {
    return { available: false, present: null };
  }
}
...
function inspectLiveWindowsTask({
  ...
  const presence = inspectExactLiveTaskPresence({ profile, execFileSyncImpl });
...
function installLiveScheduledTask({
  ...
  taskXmlBuilder = buildLiveScheduledTaskXml,
  ...
  try {
    ...
    try {
      ...
      execFileSyncImpl(
        "schtasks.exe",
        ["/Create", "/TN", profile.task_name, "/XML", xmlPath],
        {
          encoding: "utf8",
          timeout: 15_000,
          windowsHide: true,
...
function setLiveScheduledTaskEnabled({
  ...
  try {
    ...
    const mutationBoundaryTask = taskInspector({
      profile,
      repoRoot,
      expectedCommit,
      execFileSyncImpl,
    });
    const currentCommandFingerprint = String(
      current.command_fingerprint || "",
    )
      .trim()
      .toLowerCase();
    if (
      mutationBoundaryTask?.state !== current.state ||
      (currentCommandFingerprint &&
        (!SHA256_PATTERN.test(currentCommandFingerprint) ||
          String(mutationBoundaryTask.command_fingerprint || "")
            .trim()
            .toLowerCase() !== currentCommandFingerprint))
    ) {
      throw new Error("live_task_enablement_mutation_boundary_identity_lost");
    }
    execFileSyncImpl(
      "schtasks.exe",
...
async function startLiveScheduledTask({
  ...
  const assertBoundaryAuthority = ({ postLaunch = false } = {}) => {
    ...
    if (postLaunch !== true) currentTaskAuthority = task;
  ...
  const cleanupOwnAttempt = async () => {
    ...
    try {
      ...
      const endBoundaryTask = inspectExactTask();
      const beforeEndCommandFingerprint = String(
        beforeEndTask.command_fingerprint || "",
      )
        .trim()
        .toLowerCase();
      if (
        endBoundaryTask?.state !== "managed_current" ||
        (beforeEndCommandFingerprint &&
          (!SHA256_PATTERN.test(beforeEndCommandFingerprint) ||
            String(endBoundaryTask.command_fingerprint || "")
              .trim()
              .toLowerCase() !== beforeEndCommandFingerprint))
      ) {
        cleanup.blockers.push("start_cleanup_managed_task_identity_lost");
        return cleanup;
      }
    ...
    if (
      ...
      try {
        ...
        const disableBoundaryTask = inspectExactTask();
        const beforeDisableCommandFingerprint = String(
          beforeDisableTask.command_fingerprint || "",
        )
          .trim()
          .toLowerCase();
        if (
          disableBoundaryTask?.state !== "managed_current" ||
          (beforeDisableCommandFingerprint &&
            (!SHA256_PATTERN.test(beforeDisableCommandFingerprint) ||
              String(disableBoundaryTask.command_fingerprint || "")
                .trim()
                .toLowerCase() !== beforeDisableCommandFingerprint))
        ) {
          cleanup.blockers.push("start_cleanup_disable_authority_lost");
        } else {
          ...
          execFileSyncImpl(
            "schtasks.exe",
            ["/Change", "/TN", profile.task_name, "/DISABLE"],
            {
              encoding: "utf8",
              timeout: 10_000,
              windowsHide: true,
  ...
  try {
    ...
    const runBoundaryTask = inspectExactTask();
    const currentTaskCommandFingerprint = String(
      currentTaskAuthority?.command_fingerprint || "",
    )
      .trim()
      .toLowerCase();
    if (
      runBoundaryTask?.state !== "managed_current" ||
      (currentTaskCommandFingerprint &&
        (!SHA256_PATTERN.test(currentTaskCommandFingerprint) ||
          String(runBoundaryTask.command_fingerprint || "")
            .trim()
            .toLowerCase() !== currentTaskCommandFingerprint))
    ) {
      throw new Error("live_task_start_mutation_boundary_identity_lost");
    }
    failureStage = "start_task_run_failed";
    launched = true;
    execFileSyncImpl(
      "schtasks.exe",
      ["/Run", "/TN", profile.task_name],
      {
        encoding: "utf8",
        timeout: 10_000,
        windowsHide: true,
...
function uninstallLiveScheduledTask({
  ...
  try {
    ...
    const currentCommandFingerprint = String(
      ...
      .toLowerCase();
    if (
      mutationBoundaryTask?.state !== "managed_disabled" ||
      (currentCommandFingerprint &&
        (!SHA256_PATTERN.test(currentCommandFingerprint) ||
          String(mutationBoundaryTask.command_fingerprint || "")
            .trim()
      ...
      throw new Error("live_task_uninstall_mutation_boundary_identity_lost");
    }
    execFileSyncImpl(
      "schtasks.exe",
    ...
    if (removed.state !== "absent") {
      throw new Error("live_task_post_uninstall_absence_failed");
    }
    assertLiveTransitionLeaseAuthority(transitionLease);
    return lifecycleReceiptWriter({
      profile,
...
async function inspectLiveTaskBoundHandoff({
  ...
  const persistentProcessIds = job.process_ids;
  if (
    !validPositivePid(job.probe_pid) ||
    !Array.isArray(persistentProcessIds) ||
    persistentProcessIds.length !== 2 ||
    persistentProcessIds.some((pid) => !validPositivePid(pid)) ||
    new Set(persistentProcessIds).size !== 2 ||
    persistentProcessIds.includes(job.probe_pid) ||
    !persistentProcessIds.includes(Number(supervisorPid)) ||
    !persistentProcessIds.includes(Number(childPid))
  ) {
  ...
  return {
    ...
    child,
    job_membership: {
      process_ids: [...persistentProcessIds],
...
async function startLiveSupervisionGeneration({
  ...
  try {
    ...
    if (
      ...
      taskAuthority.job_membership.process_ids.length !== 2 ||
      new Set(taskAuthority.job_membership.process_ids).size !== 2 ||
