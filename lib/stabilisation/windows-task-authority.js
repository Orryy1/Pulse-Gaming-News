...
try {
  ...
  [pscustomobject]@{ ProbePid=$PID;ProcessIds=@($processIds) } | ConvertTo-Json -Compress
} finally {
...
async function inspectCurrentWindowsJobMembership({ runPowerShell } = {}) {
  ...
  if (!Object.prototype.hasOwnProperty.call(value, "ProbePid")) {
    return {
      ok: false,
      process_ids: [],
  ...
  const probePid = value.ProbePid;
  if (!Number.isInteger(probePid) || probePid <= 0) {
    return {
      ok: false,
      process_ids: [],
      blockers: ["windows_job_probe_identity_invalid"],
  ...
  if (!Array.isArray(value.ProcessIds)) {
    return {
      ok: false,
      process_ids: [],
      blockers: ["windows_job_membership_invalid"],
  ...
  const processIds = value.ProcessIds;
  if (
  ...
  if (processIds.filter((pid) => pid === probePid).length !== 1) {
    return {
      ok: false,
      process_ids: [],
      blockers: ["windows_job_probe_identity_invalid"],
  ...
  const persistentProcessIds = processIds.filter((pid) => pid !== probePid);
  if (new Set(persistentProcessIds).size !== persistentProcessIds.length) {
    return {
      ok: false,
      process_ids: [],
      blockers: ["windows_job_membership_invalid"],
  ...
  return {
    ok: true,
    probe_pid: probePid,
    process_ids: persistentProcessIds.sort((left, right) => left - right),
    blockers: [],
  };
}
...
async function observeBoundedWindowsAuthority({
  ...
  if (!Array.isArray(job.blockers) || job.blockers.length !== 0) {
    return failedAuthorityObservation(
      taskName,
      ["windows_job_membership_probe_failed"],
      { task_instance: taskInstance, supervisor, child },
    );
  }
  const probePid = job.probe_pid;
  const persistentProcessIds = job.process_ids;
  if (
    !Number.isInteger(probePid) ||
    probePid <= 0 ||
    !Array.isArray(persistentProcessIds) ||
    persistentProcessIds.includes(probePid)
  ) {
    return failedAuthorityObservation(
      taskName,
      ["windows_job_probe_identity_invalid"],
      { task_instance: taskInstance, supervisor, child },
    );
  }
  if (
    persistentProcessIds.some(
      (pid) => !Number.isInteger(pid) || pid <= 0,
    ) ||
    new Set(persistentProcessIds).size !== persistentProcessIds.length
  ) {
    return failedAuthorityObservation(
      taskName,
      ["windows_authority_job_membership_invalid"],
      { task_instance: taskInstance, supervisor, child },
    );
  }
  const supervisorPresent = persistentProcessIds.includes(supervisorPid);
  const childPresent = persistentProcessIds.includes(childPid);
  const jobMembership = {
  ...
  if (
    ...
    persistentProcessIds.some(
      (pid) => pid !== supervisorPid && pid !== childPid,
    )
  ) {
    return failedAuthorityObservation(
      taskName,
