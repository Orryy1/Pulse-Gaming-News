...
function improvesVisualDiversity(candidate, accepted, deckEntities) {
...
function steamStillAssetsForApp(entity, app) {
...
async function storyWithMultiEntityStoreAssets(story, options = {}) {
  ...
  for (const target of targets) {
    ...
    const assets = app ? steamStillAssetsForApp(target, app).slice(0, maxAssetsPerEntity) : [];
    coverage.push({
      entity: target,
      source: "steam",
...
function buildStillImageEnrichmentPlan(story, options = {}) {
  ...
  for (const candidate of candidates) {
    ...
    if (!improvesVisualDiversity(candidate, accepted, deckEntities)) {
...
function renderStillImageEnrichmentMarkdown(report) {
  ...
  lines.push("");
  lines.push("| story | fetch | reject | before | after | improves | deck change |");
  lines.push("| --- | ---: | ---: | --- | --- | --- | --- |");
