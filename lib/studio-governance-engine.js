...
function authoritativeFinalRenderInputAssets(story = {}) {
  const renderManifest = asObject(story.render_manifest || story.renderManifest);
  const selection = asObject(
    renderManifest.selected_input_assets || story.selected_input_assets,
  );
  const rows = asArray(selection.assets);
  const declaredCount = Number(selection.asset_count);
  const failures = [
    ...asArray(selection.blockers),
    ...asArray(selection.failures),
    ...asArray(selection.rejection_reasons),
  ];
  if (
  ...
  return rows
    .map((row, index) => {
      const sourceKind = cleanText(
        row?.kind || row?.asset_type || row?.role || row?.source_type,
      ).toLowerCase();
      const kind = /narrat|music|sfx|sound|audio|voice/.test(sourceKind)
        ? "audio"
        : /video|motion|clip|footage/.test(sourceKind)
          ? "video"
          : "visual";
      return normaliseAsset(row, index, kind);
    })
    .filter((asset) => {
...
function collectPublishAssets(story = {}) {
  const authoritativeInputs = authoritativeFinalRenderInputAssets(story);
  if (authoritativeInputs) return authoritativeInputs;
...
function rightsRecordsRepresentSameAsset(left = {}, right = {}) {
  const leftId = normaliseText(left.asset_id);
  const rightId = normaliseText(right.asset_id);
  if (leftId && rightId && leftId === rightId) {
    return true;
  }
  const leftPath = normaliseText(left.path);
  const rightPath = normaliseText(right.path);
  if (leftPath && rightPath && leftPath === rightPath) return true;
  const leftSource = normaliseText(left.source_url);
  const rightSource = normaliseText(right.source_url);
...
function rightsRecordCompleteness(record = {}) {
  return [
    record.licence_basis ? 100 : 0,
    record.evidence_file ? 20 : 0,
    record.allowed_platforms?.length ? 10 + record.allowed_platforms.length : 0,
    record.source_type ? 5 : 0,
    record.source_url ? 5 : 0,
    record.path ? 5 : 0,
    Number.isFinite(Number(record.risk_score)) ? 2 : 0,
  ].reduce((sum, value) => sum + value, 0);
}

function mergeRightsRecords(left = {}, right = {}) {
  const leftScore = rightsRecordCompleteness(left);
  const rightScore = rightsRecordCompleteness(right);
  const primary = rightScore > leftScore ? right : left;
  const secondary = primary === right ? left : right;
  const riskScores = [primary.risk_score, secondary.risk_score]
    .map((value) => Number(value))
    .filter((value) => Number.isFinite(value));
  const livePublishAllowed =
    primary.live_publish_allowed === false || secondary.live_publish_allowed === false
      ? false
      : primary.live_publish_allowed === true || secondary.live_publish_allowed === true
        ? true
        : undefined;
  return {
    ...secondary,
    ...primary,
    asset_id: primary.asset_id || secondary.asset_id,
    path: primary.path || secondary.path,
    source_url: primary.source_url || secondary.source_url,
    source_type: primary.source_type || secondary.source_type,
    licence_basis: primary.licence_basis || secondary.licence_basis,
    allowed_platforms: primary.allowed_platforms?.length ? primary.allowed_platforms : secondary.allowed_platforms || [],
    expiry: primary.expiry || secondary.expiry || null,
    credit_required: primary.credit_required === true || secondary.credit_required === true,
    commercial_use_allowed:
      primary.commercial_use_allowed === false || secondary.commercial_use_allowed === false ? false : true,
    live_publish_allowed: livePublishAllowed,
    requires_human_legal_review_before_publish:
      primary.requires_human_legal_review_before_publish === true ||
      secondary.requires_human_legal_review_before_publish === true,
    risk_score: riskScores.length ? Math.max(...riskScores) : primary.risk_score,
    evidence_file: primary.evidence_file || secondary.evidence_file,
  };
}

function preferCompleteRightsRecords(records = []) {
  const selected = [];
  for (const record of records) {
    const existingIndex = selected.findIndex((candidate) => rightsRecordsRepresentSameAsset(candidate, record));
    if (existingIndex === -1) {
      selected.push(record);
    } else {
      selected[existingIndex] = mergeRightsRecords(selected[existingIndex], record);
    }
  }
  return selected;
}
...
function collectRightsLedger(story = {}, options = {}) {
  ...
  return preferCompleteRightsRecords(records);
}

function rightsRecordMatchesAsset(record = {}, asset = {}) {
  const recordId = normaliseText(record.asset_id);
  const assetId = normaliseText(asset.asset_id);
  if (recordId && assetId && recordId === assetId) {
    return true;
  }
  const recordPath = normaliseText(record.path);
  const assetPath = normaliseText(asset.path);
  if (recordPath && assetPath && recordPath === assetPath) return true;
  const recordSource = normaliseText(record.source_url);
  const assetSource = normaliseText(asset.source_url);
  if (recordSource && assetSource && recordSource === assetSource) {
    if (recordId && assetId && recordId !== assetId) return false;
    if (recordPath && assetPath && recordPath !== assetPath) return false;
    return true;
  }
  return false;
}
...
function runRightsLedgerGate(story = {}, options = {}) {
  ...
  for (const asset of assets) {
    ...
    if (record.live_publish_allowed === false) failures.push("rights:live_publish_not_allowed");
    if (record.requires_human_legal_review_before_publish === true) {
      failures.push("rights:human_legal_review_required_before_publish");
    }
...
function subjectAliasKeys(subject) {
  const key = normaliseText(subject);
  const aliases = new Set([key]);
  if (/^assassin s creed\s+/.test(key)) {
    aliases.add(key.replace(/^assassin s creed\s+/, ""));
  }
...
function runAffiliateDisclosureGate(story = {}, options = {}) {
  ...
  const financeCryptoVertical = classifyFinanceOrCrypto(story, manifest);
  if (
    financeCryptoVertical !== "non_financial" &&
    manifest?.compliance?.finance_or_crypto &&
    manifest.compliance.review_required
  ) {
...
function runAiDisclosureGate(story = {}) {
  ...
  const disclosed = [
    ...
    youtube.synthetic_media_disclosed,
    youtube.altered_synthetic_disclosure_present,
    youtube.ai_disclosure_applied,
    tiktok.ai_generated_content_label,
    tiktok.synthetic_media_disclosed,
    story.youtube_synthetic_media_disclosed,
    story.ai_disclosure_applied,
  ].some(affirmativeDisclosureValue);
  const required = disclosed || [
    aiUsage.label_required,
    aiUsage.realistic_altered_or_synthetic,
    story.ai_disclosure_required,
    story.realistic_synthetic_media,
    story.synthetic_media_realistic,
    story.synthetic_media_required,
    story.altered_synthetic_disclosure_required,
    youtube.synthetic_media_required,
    youtube.altered_synthetic_disclosure_required,
    youtube.ai_disclosure_required,
    tiktok.ai_generated_content_label_required,
  ].some(affirmativeDisclosureValue) ||
    cleanText(story.youtube_altered_or_synthetic_content).toUpperCase() === "YES";
...
function runPlatformPolicyGate(story = {}) {
  ...
  const warnings = [];
  const youtube = disclosureObject(story, "youtube");
  const paidPromotionDisclosed = [
    youtube.paid_promotion_toggle,
    youtube.paid_promotion_disclosed,
    youtube.paid_promotion_disclosure_present,
    story.youtube_paid_promotion_disclosed,
    story.paid_promotion_disclosure_present,
  ].some(affirmativeDisclosureValue);
  const needsPaidPromotionToggle = paidPromotionDisclosed || [
    youtube.paid_promotion,
    youtube.paid_promotion_disclosure_required,
  ...
  return {
    ...
    failures: unique(failures),
    warnings: unique(warnings),
    paid_promotion_required: needsPaidPromotionToggle,
    paid_promotion_present: paidPromotionDisclosed,
  };
}
...
function classifyFinanceOrCrypto(story = {}, commercial = null) {
  ...
  const text = [
    ...
    .filter(Boolean)
    .join(" ");
  const gamingContext = /\b(?:game|gaming|player|players|xbox|playstation|nintendo|steam|switch|ps5|rpg|trailer|release date|developer|studio|publisher|combat|gameplay|early access|survival game|survival games)\b/i.test(text);
  const financeCryptoTopic = /\b(?:crypto|bitcoin|ethereum|blockchain|token|wallet|exchange|leverage|web3|stocks?|shares?|market|mortgage|pension|tax|isa|bank|fees?)\b/i.test(text);
  if (gamingContext && !financeCryptoTopic) return "non_financial";
...
function buildPlatformPolicyReport(report = {}) {
  const aiDisclosureGate = report.ai_disclosure_gate || {};
  const affiliateDisclosureGate = report.affiliate_disclosure_gate || {};
  const platformPolicyGate = report.platform_policy_gate || {};
  return {
    schema_version: 1,
    ...
    disclosure_requirements: {
      affiliate: affiliateDisclosureGate.disclosure_required === true,
      altered_or_synthetic_content: aiDisclosureGate.disclosure_required === true,
      paid_promotion: platformPolicyGate.paid_promotion_required === true,
    },
    disclosure_evidence: {
      altered_or_synthetic_content: {
        required: aiDisclosureGate.disclosure_required === true,
        present: aiDisclosureGate.disclosure_present === true,
      },
      paid_promotion: {
        required: platformPolicyGate.paid_promotion_required === true,
        present: platformPolicyGate.paid_promotion_present === true,
      },
    },
