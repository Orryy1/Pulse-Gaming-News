...
function rightsRecordsRepresentSameAsset(left = {}, right = {}) {
  if (left.asset_id && right.asset_id && normaliseText(left.asset_id) === normaliseText(right.asset_id)) {
    return true;
  }
  if (left.path && right.path && normaliseText(left.path) === normaliseText(right.path)) return true;
  if (left.source_url && right.source_url && normaliseText(left.source_url) === normaliseText(right.source_url)) {
    return true;
  }
  return false;
}
