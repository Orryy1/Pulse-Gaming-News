...
const AI_TELL_PATTERNS = [
  ...
  /\b(?:retention|editorial|caption|visual|cta)\s*:\s*/gi,
];
...
function normaliseNumberPronunciation(text) {
  return String(text || "")
    .replace(/\b130,000\b/g, "one hundred and thirty thousand")
...
function buildStudioEditorial(story, opts = {}) {
  const raw = story?.full_script || story?.body || story?.hook || "";
  const tightened = tightenScript(stripGenericCta(raw), story);
  const maxWords = Number(opts.maxWords || process.env.STUDIO_EDITORIAL_MAX_WORDS || 140);
