...
function buildEntityBadgeFilters(scene, fontOpt, layout = {}) {
  ...
  const kickerY = Number.isFinite(Number(layout.kickerY)) ? Number(layout.kickerY) : 112;
  const labelY = Number.isFinite(Number(layout.labelY)) ? Number(layout.labelY) : 152;
  const fade =
    "alpha='if(lt(t\\,0.12)\\,0\\,if(lt(t\\,0.24)\\,(t-0.12)/0.12\\,if(gt(t\\,2.24)\\,if(gt(t\\,2.56)\\,0\\,1-(t-2.24)/0.32)\\,1)))'";
  return [
    `drawtext=text='${kicker}':${fontOpt}:fontcolor=${ACCENT}:fontsize=22:x=${x}:y=${kickerY}:box=1:boxcolor=black@0.46:boxborderw=10:${fade}`,
    `drawtext=text='${label}':${fontOpt}:fontcolor=white:fontsize=${labelFontSize}:x=${x}:y=${labelY}:box=1:boxcolor=black@0.38:boxborderw=13:${fade}`,
  ];
}
...
function safeClipTailPadFilter(scene, duration) {
  ...
  const padS = Math.max(0.05, sceneDuration - safeClipDuration);
  return [`tpad=stop_mode=clone:stop_duration=${padS.toFixed(2)}`];
}

function buildClipFilter({ slot, duration, scene, fontOpt }) {
  const safePad = safeClipTailPadFilter(scene, duration);
  return [
    `[${slot}:v]setrange=tv`,
    "scale=1080:1920:force_original_aspect_ratio=increase",
    "crop=1080:1920:(iw-1080)/2:(ih-1920)/2",
...
function buildFreezeFrameFilter({ slot, duration, scene, fontOpt }) {
  ...
  const caption = scene.caption ? ffEscape(String(scene.caption).toUpperCase()) : null;
...
function buildOpenerFilter({ slot, scene, story, fontOpt }) {
  ...
  const safePad = scene.isClipBacked ? safeClipTailPadFilter(scene, scene.duration) : [];
  ...
  return [
    ...
    ...hookDraws,
    ...buildEntityBadgeFilters(scene, fontOpt, { x: 74, kickerY: 250, labelY: 290 }),
    ...safePad,
...
function buildTakeawayCardFilter({ slot, scene, fontOpt }) {
  ...
  return [
    ...
    `drawtext=text='   ${cta}   ':${fontOpt}:fontcolor=black:fontsize=34:x=(w-tw)/2:y=h/2+106:box=1:boxcolor=${ACCENT}@0.98:boxborderw=18:${fadeIn(0.82, 0.42)}`,
    `drawtext=text='FOLLOW PULSE GAMING SO YOU NEVER MISS A BEAT':${fontOpt}:fontcolor=white@0.76:fontsize=27:x=(w-tw)/2:y=h/2+198:${fadeIn(1.18, 0.35)}`,
    `trim=duration=${scene.duration},setpts=PTS-STARTPTS`,
    `format=yuv420p,setsar=1[v${slot}]`,
  ].join(",");
}
...
function sceneInputReadDuration(scene, multiplier = 1, extraS = 0) {
  ...
  if (isClipLikeScene(scene) && Number.isFinite(safeClipDuration) && safeClipDuration > 0) {
    return Math.max(0.1, Math.min(requested, safeClipDuration));
  }
  return requested;
}
