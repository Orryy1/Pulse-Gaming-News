...
function buildFooterSafetyMaskFilters() {
  return [
    "drawbox=x=0:y=ih-174:w=iw:h=174:color=black@0.16:t=fill",
    `drawbox=x=0:y=ih-174:w=iw:h=2:color=${ACCENT}@0.18:t=fill`,
  ];
}

function buildMotionFilter({ slot, duration, motion, scene, fontOpt }) {
  const frames = Math.max(1, Math.round(duration * FPS));
  const zoomTravel = scene?.motionIntensity === "high" ? 0.42 : 0.34;
...
function buildTimelineFallbackFilter({ slot, scene, fontOpt }) {
  ...
  const lines = [
    ...
    `drawtext=text='${kicker}':${fontOpt}:fontcolor=${ACCENT}:fontsize=36:x=96:y=438:${fadeIn(0.06, 0.22)}`,
    `drawtext=text='${heading}':${fontOpt}:fontcolor=black@0.70:fontsize=66:x=100:y=507:${fadeIn(0.16, 0.28)}`,
    `drawtext=text='${heading}':${fontOpt}:fontcolor=white:fontsize=66:x=96:y=502:${fadeIn(0.16, 0.28)}`,
  ];
  bullets.forEach((b, i) => {
    const numText = String(i + 1).padStart(2, "0");
    const start = 0.28 + i * 0.14;
