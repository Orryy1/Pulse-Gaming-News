...
const DEFAULT_BADGE_START_S = 0.12;
const DEFAULT_BADGE_END_S = 1.16;
const CLIP_TAIL_TRIM_S = 0.18;
...
function buildEntityBadgeFilters(scene, fontOpt, layout = {}) {
  const shouldRender =
    layout.force === true ||
    scene?.showEntityBadge === true ||
    (scene?.type === SCENE_TYPES.OPENER && scene?.hideEntityBadge !== true);
  if (!shouldRender || scene?.showEntityBadge === false || scene?.suppressEntityBadge === true) {
    return [];
  }
  ...
  const labelFontSize = label.length > 16 ? 24 : 28;
  ...
  const labelY = Number.isFinite(Number(layout.labelY)) ? Number(layout.labelY) : 292;
  const startS = Number.isFinite(Number(layout.startS))
    ? Number(layout.startS)
    : DEFAULT_BADGE_START_S;
  const endS = Number.isFinite(Number(layout.endS))
    ? Number(layout.endS)
    : DEFAULT_BADGE_END_S;
  const enable = `enable='between(t\\,${startS.toFixed(2)}\\,${endS.toFixed(2)})'`;
  return [
    `drawtext=text='${kicker}':${fontOpt}:fontcolor=${ACCENT}:fontsize=18:x=${x}:y=${kickerY}:box=1:boxcolor=black@0.36:boxborderw=8:${enable}`,
    `drawtext=text='${label}':${fontOpt}:fontcolor=white:fontsize=${labelFontSize}:x=${x}:y=${labelY}:box=1:boxcolor=black@0.30:boxborderw=10:${enable}`,
  ];
}

function buildFooterSafetyMaskFilters() {
  return [
    "drawbox=x=0:y=ih-560:w=iw:h=560:color=black@0.86:t=fill",
    `drawbox=x=0:y=ih-560:w=iw:h=2:color=${ACCENT}@0.32:t=fill`,
    "drawbox=x=0:y=ih-410:w=iw:h=410:color=black@1.0:t=fill",
  ];
}

function buildMotionFilter({ slot, duration, motion, scene, fontOpt }) {
  const frames = Math.max(1, Math.round(duration * FPS));
  const isTrailerFrame =
    scene?.type === SCENE_TYPES.CLIP_FRAME ||
    scene?.sourceType === "official_trailer_frame";
  const zoomTravel =
    scene?.motionIntensity === "high" ? 0.52 : isTrailerFrame ? 0.46 : 0.34;
  const inc = Math.round(10000 * (zoomTravel / frames)) / 10000;
  const maxZoom = Number((1 + zoomTravel).toFixed(2));
  let zoompan;
  switch (motion) {
    case "pullBackCentre":
      zoompan = `zoompan=z=if(eq(on\\,1)\\,${maxZoom}\\,max(zoom-${inc}\\,1.0)):x=iw/2-(iw/zoom/2):y=ih/2-(ih/zoom/2):d=${frames}:s=1080x1920:fps=${FPS}`;
      break;
    case "pushPanRight":
      zoompan = `zoompan=z=min(zoom+${inc}\\,${maxZoom}):x=(iw-iw/zoom)*on/${frames}:y=ih/2-(ih/zoom/2):d=${frames}:s=1080x1920:fps=${FPS}`;
      break;
    case "pushPanLeft":
      zoompan = `zoompan=z=min(zoom+${inc}\\,${maxZoom}):x=(iw-iw/zoom)*(1-on/${frames}):y=ih/2-(ih/zoom/2):d=${frames}:s=1080x1920:fps=${FPS}`;
      break;
    case "driftDown":
      zoompan = `zoompan=z=min(zoom+${inc}\\,${maxZoom}):x=iw/2-(iw/zoom/2):y=(ih-ih/zoom)*on/${frames}:d=${frames}:s=1080x1920:fps=${FPS}`;
      break;
    default:
      zoompan = `zoompan=z=min(zoom+${inc}\\,${maxZoom}):x=iw/2-(iw/zoom/2):y=ih/2-(ih/zoom/2):d=${frames}:s=1080x1920:fps=${FPS}`;
  }
  return [
    `[${slot}:v]setrange=tv`,
...
function safeClipRenderDuration(scene, duration) {
  ...
  if (
    ...
    safeClipDuration <= 0 ||
    safeClipDuration > sceneDuration + 0.05
  ) {
    return sceneDuration;
  }
  return Number(Math.max(0.1, safeClipDuration - CLIP_TAIL_TRIM_S).toFixed(3));
}
...
function buildOpenerFilter({ slot, scene, story, fontOpt }) {
  ...
  return [
    ...base,
    ...(scene.isClipBacked ? buildFooterSafetyMaskFilters() : []),
...
function buildTakeawayCardFilter({ slot, scene, fontOpt }) {
  ...
  const frames = Math.max(1, Math.ceil(Number(scene.duration || 4) * FPS));
  ...
  return [
    ...
    "scale=1080:1920:force_original_aspect_ratio=increase",
    "crop=1080:1920:(iw-1080)/2:(ih-1920)/2",
    `zoompan=z=min(zoom+0.0012\\,1.08):x=iw/2-(iw/zoom/2):y=ih/2-(ih/zoom/2):d=${frames}:s=1080x1920:fps=${FPS}`,
    "boxblur=10:3",
    ...
    `drawtext=text='PULSE  PULSE  PULSE  PULSE':${fontOpt}:fontcolor=white@0.18:fontsize=66:x='mod(t*920\\,w+880)-880':y=1688:enable='gte(t\\,1.05)'`,
...
function buildFlashStatCardFilter({ slot, scene, fontOpt }) {
  ...
  return [
    ...
    `drawbox=x='mod(t*680\\,1320)-240':y=0:w=96:h=ih:color=${ACCENT}@0.10:t=fill`,
    `drawbox=x='w-mod(t*520\\,1320)':y=0:w=54:h=ih:color=white@0.07:t=fill`,
    `drawbox=x=68:y=410:w=944:h=520:color=black@0.60:t=fill:${enableAfter(0)}`,
    `drawbox=x='-260+mod(t*520\\,1540)':y=410:w=240:h=520:color=white@0.10:t=fill`,
    `drawbox=x='1040-mod(t*640\\,1420)':y=410:w=170:h=520:color=${ACCENT}@0.12:t=fill`,
    `drawbox=x=68:y=410:w=944:h='if(lt(t\\,0.5)\\,4+(520-4)*t/0.5\\,520)':color=${ACCENT}@0.95:t=4`,
    `drawtext=text='MUST KNOW':${fontOpt}:fontcolor=${ACCENT}:fontsize=34:x=118:y=474`,
    `drawtext=text='${heading}':${fontOpt}:fontcolor=black@0.72:fontsize=${headingSize}:x=123:y=567`,
    `drawtext=text='${heading}':${fontOpt}:fontcolor=white:fontsize=${headingSize}:x=118:y=562`,
    ...
    `drawbox=x=650:y=604:w='1+220*mod(t*0.62\\,1)':h=18:color=${ACCENT}@0.50:t=fill`,
    `drawbox=x=650:y=646:w='1+310*mod(t*0.44\\,1)':h=18:color=white@0.22:t=fill`,
    `drawbox=x=650:y=688:w='1+260*mod(t*0.53\\,1)':h=18:color=${ACCENT}@0.34:t=fill`,
    `drawbox=x=68:y='980+mod(t*210\\,360)':w=944:h=3:color=${ACCENT}@0.22:t=fill`,
    trim,
    `format=yuv420p,setsar=1[v${slot}]`,
  ].join(",");
}
...
function buildTimelineFallbackFilter({ slot, scene, fontOpt }) {
  ...
  const fadeIn = (start, dur = 0.4) =>
    `alpha='if(lt(t\\,${start})\\,0\\,if(lt(t-${start}\\,${dur})\\,(t-${start})/${dur}\\,1))'`;
  const enableAfter = (start) => `enable='gte(t\\,${Number(start).toFixed(2)})'`;
  const lines = [
    `[${slot}:v]setrange=tv`,
    "scale=1080:1920:force_original_aspect_ratio=increase",
    "crop=1080:1920:(iw-1080)/2:(ih-1920)/2",
    `fps=${FPS}`,
    "boxblur=12:4",
    "eq=brightness=-0.14:saturation=1.06:contrast=1.18",
    "drawbox=x=0:y=0:w=iw:h=ih:color=black@0.30:t=fill",
    `drawbox=x='mod(t*620\\,1320)-240':y=0:w=110:h=ih:color=${ACCENT}@0.12:t=fill`,
    `drawbox=x=56:y=330:w=968:h=730:color=black@0.54:t=fill`,
    `drawbox=x=56:y=330:w=968:h=730:color=${ACCENT}@0.95:t=4`,
    `drawbox=x='-380+mod(t*430\\,1780)':y=330:w=260:h=730:color=white@0.08:t=fill`,
    `drawbox=x=56:y=1038:w='1+967*mod(t*0.38\\,1)':h=12:color=${ACCENT}@0.32:t=fill`,
    `drawbox=x=96:y=390:w='if(lt(t\\,0.42)\\,1+(520-1)*t/0.42\\,520)':h=6:color=${ACCENT}@0.95:t=fill`,
    `drawtext=text='${kicker}':${fontOpt}:fontcolor=${ACCENT}:fontsize=36:x=96:y=438`,
    `drawtext=text='${heading}':${fontOpt}:fontcolor=black@0.70:fontsize=66:x=100:y=507`,
    `drawtext=text='${heading}':${fontOpt}:fontcolor=white:fontsize=66:x=96:y=502`,
  ];
  bullets.forEach((b, i) => {
    const numText = String(i + 1).padStart(2, "0");
    const start = i * 0.12;
    const yBase = 665 + i * 106;
    const panelY = yBase - 20;
    lines.push(
      `drawbox=x=96:y=${panelY}:w=888:h=82:color=white@0.10:t=fill:${enableAfter(start)}`,
    );
    lines.push(
      `drawbox=x=96:y=${panelY}:w='if(lt(t\\,${start + 0.2})\\,1\\,1+(888-1)*(t-${(start + 0.2).toFixed(2)})/0.34)':h=82:color=${ACCENT}@0.10:t=fill:enable='gte(t\\,${(start + 0.2).toFixed(2)})'`,
    );
    lines.push(
      `drawtext=text='${numText}':${fontOpt}:fontcolor=${ACCENT}:fontsize=42:x=134:y=${yBase}:${fadeIn(start, 0.28)}`,
    );
    const copy = ffEscape(String(b || "").slice(0, 60));
    lines.push(
      `drawtext=text='${copy}':${fontOpt}:fontcolor=white:fontsize=34:x=220:y=${yBase + 4}:${fadeIn(start, 0.28)}`,
    );
  });
  lines.push(
    `drawbox=x=96:y=1018:w='if(lt(t\\,1.3)\\,1\\,1+(888-1)*(t-1.3)/0.44)':h=5:color=${ACCENT}@0.95:t=fill`,
  );
  lines.push(
    `drawbox=x=56:y='1110+mod(t*180\\,420)':w=968:h=3:color=white@0.12:t=fill`,
  );
...
function buildTransitionPlan(scenes, opts = {}) {
  const transitions = [];
