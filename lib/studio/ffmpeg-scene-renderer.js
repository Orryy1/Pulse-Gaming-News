...
function badgeKicker(scene) {
  ...
  if (/trailer.*frame|official_trailer_frame/.test(type))
    return "OFFICIAL FRAME";
  if (/movie|trailer|clip/.test(type) || scene?.type === SCENE_TYPES.CLIP)
...
function buildEntityBadgeFilters(scene, fontOpt) {
  const entity =
    scene?.entity ||
    inferEntityFromSource(scene?.source || scene?.backgroundSource);
  if (!entity) return [];
  ...
  const fade =
    "alpha='if(lt(t\\,0.12)\\,0\\,if(lt(t\\,0.24)\\,(t-0.12)/0.12\\,if(gt(t\\,2.24)\\,if(gt(t\\,2.56)\\,0\\,1-(t-2.24)/0.32)\\,1)))'";
  return [
    `drawtext=text='${kicker}':${fontOpt}:fontcolor=${ACCENT}:fontsize=22:x=74:y=112:box=1:boxcolor=black@0.46:boxborderw=10:${fade}`,
    `drawtext=text='${label}':${fontOpt}:fontcolor=white:fontsize=${labelFontSize}:x=74:y=152:box=1:boxcolor=black@0.38:boxborderw=13:${fade}`,
  ];
}
...
function buildFreezeFrameFilter({ slot, duration, scene, fontOpt }) {
  ...
  const caption = scene.caption
    ? ffEscape(String(scene.caption).toUpperCase())
    : null;
  ...
  if (caption) {
    filters.push(
      `drawbox=x=76:y=h/2-112:w=928:h=224:color=black@0.68:t=fill:${enable}`,
      `drawbox=x=76:y=h/2-112:w=928:h=6:color=${ACCENT}@0.98:t=fill:${enable}`,
      `drawtext=text='${caption}':${fontOpt}:fontcolor=black@0.72:fontsize=62:x=(w-tw)/2+4:y=h/2-32+4:${fade}:${enable}`,
      `drawtext=text='${caption}':${fontOpt}:fontcolor=white:fontsize=62:x=(w-tw)/2:y=h/2-32:${fade}:${enable}`,
    );
  }
...
function buildTakeawayCardFilter({ slot, scene, story, fontOpt }) {
  ...
  const ctaDraws = governedCta
    ? [
        `drawtext=text='${ffEscape(governedCta)}':${fontOpt}:fontcolor=black:fontsize=26:x=(w-tw)/2:y=h/2+132:box=1:boxcolor=${ACCENT}@0.98:boxborderw=16:${fadeIn(0.82, 0.42)}`,
      ]
    : [];
  return [
    `[${slot}:v]setrange=tv`,
    ...
    "drawbox=x=78:y=h/2-260:w=924:h=520:color=black@0.40:t=fill:enable='gte(t\\,0)'",
    `drawbox=x=78:y=h/2-260:w=10:h=520:color=${ACCENT}@0.98:t=fill:enable='gte(t\\,0.05)'`,
    `drawbox=x=78:y=h/2-260:w='if(lt(t\\,0.55)\\,1+(924-1)*t/0.55\\,924)':h=6:color=${ACCENT}@0.98:t=fill`,
    `drawbox=x=78:y=h/2+254:w='if(lt(t\\,0.55)\\,1+(924-1)*t/0.55\\,924)':h=6:color=white@0.82:t=fill`,
    `drawtext=text='END LOCK':${fontOpt}:fontcolor=${ACCENT}:fontsize=34:x=128:y=h/2-198:${fadeIn(0, 0.32)}`,
    `drawtext=text='${text}':${fontOpt}:fontcolor=black@0.70:fontsize=74:x=(w-tw)/2+4:y=h/2-54+4:${fadeIn(0.32, 0.46)}`,
    `drawtext=text='${text}':${fontOpt}:fontcolor=white:fontsize=74:x=(w-tw)/2:y=h/2-54:${fadeIn(0.32, 0.46)}`,
    ...ctaDraws,
    `trim=duration=${scene.duration},setpts=PTS-STARTPTS`,
    `format=yuv420p,setsar=1[v${slot}]`,
  ].join(",");
...
function buildFlashStatCardFilter({ slot, scene, fontOpt }) {
  const heading = ffEscape(
    String(scene.statLabel || "MUST KNOW").toUpperCase(),
  );
  const sub = ffEscape(
    String(scene.sublabel || "THE DETAIL THAT MATTERS").toUpperCase(),
  );
...
function dispatchSceneFilter({ slot, scene, story, fontOpt }) {
  ...
  switch (scene.type) {
    ...
    case SCENE_TYPES.CARD_TAKEAWAY:
      return buildTakeawayCardFilter({ slot, scene, story, fontOpt });
...
function buildSceneInput(scene) {
  ...
  const mediaStartS = Number(
    scene.mediaStartS ?? scene.media_start_s ?? scene.startS ?? 0,
  );
  const seek =
    Number.isFinite(mediaStartS) && mediaStartS > 0
      ? `-ss ${mediaStartS.toFixed(2)} `
      : "";

  if (scene.type === SCENE_TYPES.SPEED_RAMP) {
    const sourceReadDur = (Number(scene.duration || 4) * 2.2).toFixed(2);
    return `${seek}-t ${sourceReadDur} -i "${esc(file)}"`;
  }
  if (scene.type === SCENE_TYPES.FREEZE_FRAME) {
    const playInS = Math.min(
      0.8,
      Math.max(0.45, Number(scene.duration || 4) * 0.22),
    );
    return `${seek}-t ${(playInS + 0.2).toFixed(2)} -i "${esc(file)}"`;
  }
