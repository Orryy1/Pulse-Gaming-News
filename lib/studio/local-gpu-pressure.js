...
function isLocalTtsResidentReadySummary(summary = {}) {
  const safeSummary = summary || {};
  const voice = safeSummary.voice || {};
  return (
    safeSummary.ok === true &&
    String(safeSummary.status || "").toLowerCase() === "ok" &&
    safeSummary.ready === true &&
    voice.loaded === true &&
...
async function inspectLocalGpuPressure({
  ...
  localTtsResidentReady = false,
  localTtsHealth = null,
  coordinateSharedGpu = null,
  inspectSharedGpu = inspectPulseGpuAvailability,
} = {}) {
  ...
  if (shouldCoordinate) {
    try {
      sharedGpu = await inspectSharedGpu({ env });
    } catch (error) {
      return {
        ok: false,
        status: "busy",
        failure_code: "shared_gpu_scheduler_unavailable",
        reason: `Shared GPU scheduler unavailable: ${error.message || error}`,
        selected: null,
        gpus: [],
        thresholds: {},
        shared_gpu: null,
    ...
    if (sharedGpu?.ok === false) {
      return {
        ok: false,
        status: "busy",
        failure_code: "shared_gpu_reserved",
        reason: sharedGpu.reason || "Shared GPU is reserved by another studio",
        selected: null,
        gpus: [],
        thresholds: {},
        shared_gpu: sharedGpu,
      };
    }
  }

  const coldMinFreeMb = positiveNumber(
    env.LOCAL_TTS_MIN_GPU_FREE_MB || env.STUDIO_V2_LOCAL_TTS_MIN_GPU_FREE_MB,
    12288,
  );
  const residentMinFreeMb = positiveNumber(
    env.LOCAL_TTS_RESIDENT_MIN_GPU_FREE_MB ||
      env.STUDIO_V2_LOCAL_TTS_RESIDENT_MIN_GPU_FREE_MB,
    6144,
  );
  const residentReady =
    localTtsResidentReady === true || isLocalTtsResidentReadySummary(localTtsHealth);
  const minFreeMb = residentReady
    ? Math.min(coldMinFreeMb, residentMinFreeMb)
    : coldMinFreeMb;
  ...
  const thresholds = {
    ...
    maxUtilPercent,
    gpuIndex,
    localTtsResidentReady: residentReady,
  };
...
function formatLocalGpuPressure(report = {}) {
  ...
  if (report.shared_gpu?.owner?.studio) {
    parts.push(`shared_owner=${report.shared_gpu.owner.studio}`);
  }
  return parts.join(" ");
}
...
module.exports = {
  ...
  isLocalTtsResidentReadySummary,
  parseNvidiaSmiCsv,
};
