...
function classifyLocalTtsHealthFailure(summary = {}) {
  ...
  if (/unknown local voice fallback|refusing default fallback voice/.test(haystack)) {
    return classification("unsafe_voice", "local TTS default voice fallback is not allowed");
  }
...
function classifyLocalTtsFailure(err) {
  ...
  if (/local_tts_gpu_busy|gpu_saturated|gpu.*saturat|gpu.*busy|gpu.*headroom/.test(haystack)) {
    return classification("gpu_saturated", "local TTS GPU is too busy for clean generation");
  }
  if (/local_tts_busy|inference queue wait exceeded/.test(haystack)) {
    return classification("tts_busy", "local TTS inference lane is currently occupied");
  }
  ...
  if (
    /status code 5\d\d|http 5\d\d|internal server error|model failed to load|paging file is too small/.test(
      haystack,
    )
  ) {
    return classification("server_error", "local TTS server returned a recoverable generation error");
  }
  if (
    /econnrefused|enotfound|server_down|fetch failed|local tts server is not reachable|health endpoint is unreachable/.test(
      haystack,
    )
  ) {
  ...
  if (
    /unsafe voice|unsafe_local_tts_voice|unapproved.*voice|reference.*missing|unknown voice_id|refusing default fallback voice|unknown local voice fallback/.test(haystack)
  ) {
