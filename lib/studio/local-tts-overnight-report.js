...
function buildAutonomousRecoveryStatus({ localReady, recoveryPlan = {} } = {}) {
  const workOrders = list(recoveryPlan.work_orders);
  const safeRetryOrders = workOrders.filter((order) =>
    order &&
  ...
  const operatorRequiredOrders = workOrders.filter((order) =>
    order && order.operator_approval_required === true,
  );
  const retryPreflightOrders = safeRetryOrders.filter((order) =>
    order && order.preflight_required === true,
  );
  const retryApplyReadyOrders = safeRetryOrders.filter((order) =>
    order && order.preflight_required !== true,
  );
  const safeRetryStoryIds = uniqueTruthy(
    safeRetryOrders.map((order) => order.story_id),
  );

  let status = "complete";
  if (!localReady) status = "blocked_tts_not_ready";
  else if (recoveryPlan.blocked_by_voice_quality) status = "blocked_voice_quality";
  else if (retryApplyReadyOrders.length > 0) status = "ready_for_local_tts_retry_apply";
  else if (retryPreflightOrders.length > 0) status = "ready_for_local_tts_retry_preflight";
  else if (operatorRequiredOrders.length > 0) {
    status = "operator_review_required_for_duration_repair";
  } else if (workOrders.length > 0) {
    status = "local_recovery_work_present";
  }

  return {
    status,
    current_local_tts_ready: localReady === true,
    safe_to_run_local_tts_retry:
      localReady === true &&
      recoveryPlan.blocked_by_voice_quality !== true &&
      retryApplyReadyOrders.length > 0,
    safe_to_run_local_tts_retry_preflight:
      localReady === true &&
      recoveryPlan.blocked_by_voice_quality !== true &&
      safeRetryOrders.length > 0,
    safe_retry_work_order_count: safeRetryOrders.length,
    retry_preflight_work_order_count: retryPreflightOrders.length,
    retry_apply_ready_work_order_count: retryApplyReadyOrders.length,
    operator_required_work_order_count: operatorRequiredOrders.length,
    total_work_order_count: workOrders.length,
    safe_retry_story_ids: safeRetryStoryIds,
    safe_retry_preflight_commands:
      retryPreflightOrders.map((order) => order.preflight_command).filter(Boolean),
    safe_retry_commands:
      retryApplyReadyOrders.map((order) => order.apply_command).filter(Boolean),
    apply_after_preflight_commands:
      retryPreflightOrders.map((order) => order.apply_command).filter(Boolean),
    next_action:
      retryApplyReadyOrders.length > 0
        ? "run_local_tts_retry_apply_work_orders_before_the_next_candidate_refill"
        : retryPreflightOrders.length > 0
          ? "run_local_tts_retry_preflight_work_orders_before_applying_audio"
        : operatorRequiredOrders.length > 0
          ? "review_duration_repair_work_orders_before_applying_audio"
          : localReady === true
            ? "no_local_tts_recovery_work_required"
            : "restart_and_prewarm_local_tts_before_retrying_audio",
    safety: {
      local_only: true,
      production_db_mutation_required: false,
      platform_posting_risk: false,
      token_or_oauth_mutation_required: false,
      duration_repairs_require_operator_review: operatorRequiredOrders.length > 0,
    },
  };
}
...
function buildLocalTtsOvernightReport({
  ...
  const autonomousRecovery = buildAutonomousRecoveryStatus({
    localReady,
    recoveryPlan,
  });
...
function renderLocalTtsOvernightMarkdown(report) {
  ...
  if (report.autonomous_recovery) {
    lines.push(`- autonomous_status=${report.autonomous_recovery.status}`);
    lines.push(
      `- safe_to_run_local_tts_retry=${report.autonomous_recovery.safe_to_run_local_tts_retry}`,
    );
    lines.push(
      `- safe_retry_work_orders=${report.autonomous_recovery.safe_retry_work_order_count}`,
    );
    lines.push(
      `- retry_preflight_work_orders=${report.autonomous_recovery.retry_preflight_work_order_count}`,
    );
    lines.push(
      `- retry_apply_ready_work_orders=${report.autonomous_recovery.retry_apply_ready_work_order_count}`,
    );
    lines.push(
      `- operator_required_work_orders=${report.autonomous_recovery.operator_required_work_order_count}`,
    );
  }
  lines.push(
