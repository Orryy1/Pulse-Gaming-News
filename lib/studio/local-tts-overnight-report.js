...
function buildLocalTtsOvernightReport({
  ...
  const applied = list(audioApply.applied).map((row) => ({
    ...
    word_count: numberOrNull(row.text_word_count ?? row.word_count),
    estimated_seconds: numberOrNull(row.estimated_seconds),
    ...
    timestamp_verdict: timestampVerdict(row),
    verdict: proofVerdict(row, expectedVoiceId),
    voice_verdict: proofVerdict(row, expectedVoiceId),
  })).map((row) => ({
    ...row,
    wpm: wordsPerMinute(row.word_count, row.duration_seconds),
  }));
...
function renderLocalTtsOvernightMarkdown(report) {
  ...
  if (!ready.length) {
    ...
    for (const row of ready) {
      lines.push(
        `- ${row.story_id}: measured=${row.duration_seconds ?? "unknown"}s | estimated=${row.estimated_seconds ?? "unknown"}s | ${row.word_count ?? "unknown"} words | ${row.wpm ?? "unknown"} WPM | ${row.resolved_audio_path || row.output_audio_path}`,
      );
    }
  }
