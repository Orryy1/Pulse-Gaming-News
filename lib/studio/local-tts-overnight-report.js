...
function buildRecoveryPlan(proofRejected, skipped, proofReady = []) {
  ...
  const retryWorkOrders = retryTtsStoryIds.map((storyId) => ({
    ...
    exact_missing_input: "fresh local Liam proof audio after TTS health is green",
    recommended_command: `npm run ops:local-script-extension -- --story-id ${storyId} --apply-local-audio --apply-limit 1`,
    expected_output: "test/output/local_script_extension_audio_apply.json",
    db_mutation_required: false,
