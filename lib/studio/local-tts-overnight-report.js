...
function buildLocalTtsOvernightReport({
  ...
  const applied = list(audioApply.applied).map((rawRow) => {
    const row = {
...
function renderLocalTtsOvernightMarkdown(report) {
  ...
  if (!ready.length) {
    ...
    for (const row of ready) {
      lines.push(
        `- ${row.story_id}: measured=${row.duration_seconds ?? "unknown"}s | estimated=${row.estimated_seconds ?? "unknown"}s | ${row.word_count ?? "unknown"} words | ${row.wpm ?? "unknown"} WPM | pitch=${row.acoustic?.medianPitchHz ?? "unknown"}Hz | outro=${row.spoken_outro_present} | ${row.resolved_audio_path || row.output_audio_path}`,
      );
    }
  }
