...
const DEFAULT_MIN_READY_PROOFS = 3;
...
function evaluateGenerationEvidence(
  rows,
  expectedVoiceId = DEFAULT_ACCEPTED_LOCAL_VOICE_ID,
  options = {},
) {
  const minReadyProofs = Math.max(
    1,
    numberOrNull(options.minReadyProofs) ?? DEFAULT_MIN_READY_PROOFS,
  );
  const strictUnresolved = bool(options.strictUnresolved);
  ...
  if (readyRows.length > 0 && readyRows.length < minReadyProofs) {
    blockers.push("local_tts_generation_evidence_below_minimum");
  }
  if (strictUnresolved && (unresolvedRejected.length > 0 || unresolvedSkipped.length > 0)) {
    blockers.push("local_tts_unresolved_generation_failures");
  } else if (unresolvedRejected.length > 0 || unresolvedSkipped.length > 0) {
    warnings.push("local_tts_historical_generation_failures_present");
  }

  return {
    ok: blockers.length === 0,
    ready_count: readyRows.length,
    min_ready_proofs: minReadyProofs,
...
function buildLocalTtsProofPromotionReport({
  ...
  const generation = evaluateGenerationEvidence(rows, expectedVoiceId, {
    minReadyProofs: numberOrNull(env.LOCAL_TTS_PROMOTION_MIN_READY_PROOFS),
    strictUnresolved: bool(env.LOCAL_TTS_PROMOTION_STRICT_FAILURES),
  });
  ...
  // Historical failed proof attempts are retained inside the generation gate
  // for auditability, but they should not block or amber a proof-render
  // promotion once the current ready-pool threshold is met.
  ...
  return {
    ...
    gates: {
      ...
      generation_evidence: {
        ...
        min_ready_proofs: generation.min_ready_proofs,
...
function renderLocalTtsProofPromotionMarkdown(report) {
  ...
  lines.push(
    `- generation_evidence=${report.gates.generation_evidence.ok} ready=${report.gates.generation_evidence.ready_count}/${report.gates.generation_evidence.min_ready_proofs} unresolved_rejected=${report.gates.generation_evidence.unresolved_rejected_count} unresolved_skipped=${report.gates.generation_evidence.unresolved_skipped_count}`,
  );
  lines.push(
