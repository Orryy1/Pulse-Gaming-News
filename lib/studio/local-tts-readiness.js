...
function summariseVoice(voice, voiceId) {
  if (!voice) {
    return {
      ...
      loaded: false,
      refResolved: false,
      referenceValidationPresent: false,
      referenceTechnicalReady: false,
      referenceProductionReady: false,
      referenceHashVerified: false,
      referenceRightsStatus: "UNVERIFIED",
      present: false,
    };
  }
  const referenceValidation =
    voice.reference_validation &&
    typeof voice.reference_validation === "object" &&
    !Array.isArray(voice.reference_validation)
      ? voice.reference_validation
      : null;
  return {
    ...
    referenceValidationPresent: referenceValidation !== null,
    referenceTechnicalReady: bool(referenceValidation?.technical_ready),
    referenceProductionReady: bool(referenceValidation?.production_ready),
    referenceHashVerified: bool(referenceValidation?.hash_matches),
    referenceRightsStatus: String(
      referenceValidation?.rights_status || "UNVERIFIED",
    ),
    present: true,
  };
}
...
function summariseLocalTtsHealth(health, voiceId) {
  ...
  if (voice.present && !voice.referenceValidationPresent) {
    reasons.push(
      `voice ${voice.alias || voiceId} reference validation is missing`,
    );
  } else if (voice.present && !voice.referenceProductionReady) {
    reasons.push(
      `voice ${voice.alias || voiceId} reference is not production-ready ` +
        `(rights=${voice.referenceRightsStatus})`,
    );
  }
