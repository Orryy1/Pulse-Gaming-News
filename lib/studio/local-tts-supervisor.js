...
function resolveLocalTtsRuntimePaths({
  ...
  const venvPythonw =
    platform === "win32"
      ...
      : null;
  const defaultPython =
    platform === "win32" && venvPythonw && fs.existsSync(venvPythonw)
      ? venvPythonw
...
function isFreshStartLock(lockPath, { now = Date.now(), ttlMs = 300000 } = {}) {
...
function acquireStartLock(lockPath, { now = Date.now(), ttlMs = 300000 } = {}) {
...
async function startLocalTtsServer({
  ...
  const lockTtlMs = parsePositiveMs(env.LOCAL_TTS_START_LOCK_TTL_MS, 300000);
  if (!acquireStartLock(spec.startLockPath, { ttlMs: lockTtlMs })) {
    return {
      pid: null,
      skipped: true,
  ...
  stampStartLockPid(spec.startLockPath, child.pid);
