...
function resolveWindowlessPythonPath(
  ...
  if (path.basename(resolved).toLowerCase() !== "python.exe") return resolved;
  const pythonwPath = path.join(path.dirname(resolved), "pythonw.exe");
  return fsImpl.existsSync(pythonwPath) ? pythonwPath : resolved;
}
