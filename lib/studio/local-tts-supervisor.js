...
const DEFAULT_RECENT_BOOT_BYPASS_MIN_AGE_MS = 60 * 1000;
...
function defaultPortListening({ host = "127.0.0.1", port, timeoutMs = 750 } = {}) {
...
function hasFreshDeadStartLockPid(
  lockPath,
  {
    now = Date.now(),
    ttlMs = DEFAULT_START_LOCK_TTL_MS,
  ...
  try {
    ...
    if (!Number.isFinite(startedAt) || now - startedAt >= ttlMs) return false;
...
function classifyLocalTtsDoctorAction(summary, options = {}) {
  ...
  if (status === "ok" && summary?.ready !== true && /^(warming|loading|starting)$/i.test(phase)) {
    return {
      action: allowRestart ? "restart" : "manual_restart_required",
      verdict: allowRestart ? "amber" : "red",
      reason: `local TTS service is stuck in ${phase} phase`,
    };
  }
...
async function startLocalTtsServer({
  ...
  processExists = defaultProcessExists,
  isPortListening = defaultPortListening,
  allowRecentBootBypassWhenNoListener = envFlagEnabled(
    env.LOCAL_TTS_BYPASS_RECENT_BOOT_COOLDOWN_WHEN_UNREACHABLE,
  ),
} = {}) {
  ...
  const freshDeadStartLockPid = hasFreshDeadStartLockPid(spec.startLockPath, {
    now,
    ttlMs: lockTtlMs,
    processExists,
  });
  if (freshDeadStartLockPid) {
    fs.rmSync(spec.startLockPath, { force: true });
  }
  let recentBootBypass = null;
  if (recentBoot.recent && !freshDeadStartLockPid) {
    const bypassMinAgeMs = parsePositiveMs(
      env.LOCAL_TTS_RECENT_BOOT_BYPASS_MIN_AGE_MS,
      DEFAULT_RECENT_BOOT_BYPASS_MIN_AGE_MS,
    );
    if (
      allowRecentBootBypassWhenNoListener === true &&
      Number.isFinite(recentBoot.ageMs) &&
      recentBoot.ageMs >= bypassMinAgeMs
    ) {
      const listening = await isPortListening({
        host: spec.env.HOST || env.LOCAL_TTS_HOST || "127.0.0.1",
        port: spec.env.PORT || env.LOCAL_TTS_PORT || "8765",
        timeoutMs: parsePositiveMs(env.LOCAL_TTS_PORT_PROBE_TIMEOUT_MS, 750),
      });
      if (listening !== true) {
        recentBootBypass = {
          reason: "recent_boot_without_listener",
          recent_boot: recentBoot,
          bypass_min_age_ms: bypassMinAgeMs,
        };
      } else {
        return {
          pid: null,
  ...
  if (recentBoot.recent && !freshDeadStartLockPid && !recentBootBypass) {
    return {
      pid: null,
      skipped: true,
      reason: "recent_boot_cooldown",
  ...
  return {
    pid: child.pid || null,
    ...(recentBootBypass ? { recent_boot_bypass: recentBootBypass } : {}),
    spec: {
      pythonPath: spec.pythonPath,
      args: spec.args,
      serverDir: spec.serverDir,
...
async function waitForLocalTtsHealth({
  ...
  while (Date.now() <= deadline) {
    ...
    const phase = String(lastSummary.phase || "unknown");
    if (
      lastSummary.status !== "unreachable" &&
      !/^(warming|loading|starting)$/i.test(phase)
    ) {
      return lastSummary;
    }
    if (
      ...
      Number(processId) > 0 &&
      processExists(Number(processId)) !== true
    ) {
      return {
        ...lastSummary,
        processExited: true,
        reasons: [
          ...(lastSummary.reasons || []),
          `local TTS process ${processId} exited before binding`,
        ],
      };
    }
...
function renderLocalTtsDoctorMarkdown(report) {
  ...
  if (report.generation_smoke) {
    lines.push(
      report.generation_smoke.ok === true
        ? `- generation smoke: ok size=${report.generation_smoke.size_bytes ?? "-"} attempts=${report.generation_smoke.attempts ?? "-"}`
        : `- generation smoke: failed ${report.generation_smoke.error || "unknown"}`,
    );
  }
  if (report.started) {
