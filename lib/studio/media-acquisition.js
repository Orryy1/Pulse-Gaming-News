"use strict";

const path = require("node:path");
const fs = require("fs-extra");
const { execFileSync } = require("node:child_process");

const CLIP_TAGS = ["A", "B", "C", "D"];
const DEFAULT_FFPROBE_DURATION_TIMEOUT_MS = 10000;

function resolveFfprobeDurationTimeoutMs(options = {}) {
  const raw = options.timeoutMs ?? process.env.FFPROBE_DURATION_TIMEOUT_MS;
  const number = Number(raw);
  if (Number.isFinite(number) && number >= 1000) return Math.min(120000, Math.round(number));
  return DEFAULT_FFPROBE_DURATION_TIMEOUT_MS;
}

function ffprobeDuration(file, options = {}) {
  try {
    const out = execFileSync(
      "ffprobe",
      [
        "-v",
        "quiet",
        "-show_entries",
        "format=duration",
        "-of",
        "csv=p=0",
        file,
      ],
      {
        encoding: "utf8",
        timeout: resolveFfprobeDurationTimeoutMs(options),
      },
    ).trim();
    return Number.parseFloat(out);
  } catch {
    return null;
  }
}

function inferStillKind(filename) {
  const f = String(filename || "").toLowerCase();
  if (f.includes("trailerframe")) return "trailer-frame";
  if (f.includes("article")) return "article";
  if (f.includes("steam")) return "steam";
  if (f.includes("pexels") || f.includes("unsplash") || f.includes("bing")) {
    return "stock";
  }
  return "unknown";
}

async function listMatchingFiles(dir, predicate) {
  if (!(await fs.pathExists(dir))) return [];
  const names = await fs.readdir(dir);
  return names.filter(predicate).map((name) => path.join(dir, name));
}

function studioMediaRoots(root) {
  const roots = [];
  const mediaRoot = String(process.env.MEDIA_ROOT || "").trim();
  if (mediaRoot) roots.push(path.resolve(mediaRoot));
  roots.push(root);
  return [...new Set(roots)];
}

async function rootHasStoryMedia(root, storyId) {
  for (const relDir of [
    path.join("output", "image_cache"),
    path.join("output", "video_cache"),
  ]) {
    const dir = path.join(root, relDir);
    if (!(await fs.pathExists(dir))) continue;
    const names = await fs.readdir(dir);
    if (names.some((name) => name.startsWith(`${storyId}_`))) return true;
  }
  return false;
}

async function activeStudioMediaRoots(root, storyId) {
  const roots = studioMediaRoots(root);
  const mediaRoot = String(process.env.MEDIA_ROOT || "").trim();
  if (!mediaRoot) return roots;
  const primary = path.resolve(mediaRoot);
  if (await rootHasStoryMedia(primary, storyId)) return [primary];
  return roots;
}

function cacheDir(root, kind) {
  return path.join(root, "output", kind);
}

function writeCacheDir(root, kind) {
  return cacheDir(studioMediaRoots(root)[0], kind);
}

async function firstExisting(roots, relPath) {
  for (const base of roots) {
    const p = path.join(base, relPath);
    if (await fs.pathExists(p)) return p;
  }
  return path.join(roots[0], relPath);
}

async function listMatchingFilesAcross(roots, relDir, predicate) {
  const out = [];
  const seen = new Set();
  for (const base of roots) {
    const dir = path.join(base, relDir);
    for (const p of await listMatchingFiles(dir, predicate)) {
      if (!seen.has(p)) {
        seen.add(p);
        out.push(p);
      }
    }
  }
  return out;
}

async function discoverLocalStudioMedia({ root, storyId }) {
  const roots = await activeStudioMediaRoots(root, storyId);
  const imageCacheRel = path.join("output", "image_cache");
  const videoCacheRel = path.join("output", "video_cache");

  const articleHeroes = [];
  const articlePath = await firstExisting(
    roots,
    path.join(imageCacheRel, `${storyId}_article.jpg`),
  );
  if (await fs.pathExists(articlePath)) {
    articleHeroes.push({ path: articlePath, kind: "article", score: 85 });
  }

  const trailerFrames = [];
  for (let i = 1; i <= 8; i++) {
    const p = await firstExisting(
      roots,
      path.join(imageCacheRel, `${storyId}_trailerframe_${i}.jpg`),
    );
    if (await fs.pathExists(p)) {
      trailerFrames.push({
        path: p,
        kind: "trailer-frame",
        score: 100 - i,
      });
    }
  }

  const clips = [];
  const seenClipPaths = new Set();
  for (const tag of CLIP_TAGS) {
    const p = await firstExisting(
      roots,
      path.join(videoCacheRel, `${storyId}_clip_${tag}.mp4`),
    );
    if (await fs.pathExists(p)) {
      const durationS = ffprobeDuration(p);
      if (!Number.isFinite(durationS) || durationS <= 0.2) {
        await fs.remove(p).catch(() => {});
        continue;
      }
      seenClipPaths.add(p);
      clips.push({
        path: p,
        tag,
        kind: "trailer-clip",
        durationS,
        score: 120,
      });
    }
  }
  const genericClips = await listMatchingFilesAcross(
    roots,
    videoCacheRel,
    (name) =>
      name.startsWith(`${storyId}_`) &&
      /\.(mp4|webm)$/i.test(name) &&
      !name.includes("_smartcrop"),
  );
  const genericTrailerCandidates = [];
  for (const p of genericClips) {
    if (seenClipPaths.has(p)) continue;
    const durationS = ffprobeDuration(p);
    if (!Number.isFinite(durationS) || durationS <= 0.2) {
      await fs.remove(p).catch(() => {});
      continue;
    }
    seenClipPaths.add(p);
    const base = path.basename(p);
    if (!base.includes("_clip_")) {
      genericTrailerCandidates.push(p);
    }
    clips.push({
      path: p,
      tag: base.replace(`${storyId}_`, "").replace(/\.[^.]+$/, ""),
      kind: "story-video-clip",
      durationS,
      score: 110,
    });
  }

  let trailerPath = await firstExisting(
    roots,
    path.join(videoCacheRel, `${storyId}_trailer.mp4`),
  );
  if (!(await fs.pathExists(trailerPath)) && genericTrailerCandidates.length > 0) {
    trailerPath = genericTrailerCandidates[0];
  } else if (!(await fs.pathExists(trailerPath)) && clips.length > 0) {
    trailerPath = clips[0].path;
  }
  const stockFillers = (
    await listMatchingFilesAcross(
      roots,
      imageCacheRel,
      (name) =>
        name.startsWith(`${storyId}_`) &&
        /\.(jpe?g|png)$/i.test(name) &&
        inferStillKind(name) === "stock" &&
        !name.includes("_smartcrop"),
    )
  ).map((p) => ({ path: p, kind: "stock", score: 5, _stock: true }));

  const otherImages = (
    await listMatchingFilesAcross(
      roots,
      imageCacheRel,
      (name) =>
        name.startsWith(`${storyId}_`) &&
        /\.(jpe?g|png)$/i.test(name) &&
        inferStillKind(name) !== "stock" &&
        !name.includes("_smartcrop") &&
        !name.includes("trailerframe") &&
        !name.includes("_article.jpg"),
    )
  ).map((p) => ({
    path: p,
    kind: inferStillKind(path.basename(p)),
    score: 40,
  }));

  return {
    clips,
    trailerFrames,
    articleHeroes,
    publisherAssets: otherImages,
    stockFillers,
    trailerPath: (await fs.pathExists(trailerPath)) ? trailerPath : null,
  };
}

function buildClipSlicePlan({ trailerPath, storyId, outputDir }) {
  if (!trailerPath) return [];
  const trailerDuration = ffprobeDuration(trailerPath) || 0;
  return [
    { tag: "A", startS: 1.2, durationS: 5.0 },
    { tag: "B", startS: 7.5, durationS: 5.0 },
    { tag: "C", startS: 15.0, durationS: 5.0 },
  ]
    .filter((slice) => trailerDuration === 0 || slice.startS < trailerDuration - 0.5)
    .map((slice) => ({
      ...slice,
      durationS:
        trailerDuration > 0
          ? Math.max(0.5, Math.min(slice.durationS, trailerDuration - slice.startS))
          : slice.durationS,
      input: trailerPath,
      output: path.join(outputDir, `${storyId}_clip_${slice.tag}.mp4`),
    }));
}

function buildFrameExtractionPlan({ trailerPath, storyId, outputDir }) {
  if (!trailerPath) return [];
  return [2.0, 5.5, 9.0, 13.0, 17.0, 21.0].map((timeS, index) => ({
    timeS,
    input: trailerPath,
    output: path.join(outputDir, `${storyId}_trailerframe_${index + 1}.jpg`),
  }));
}

async function ensureTrailerClipSlices({ root, storyId, media }) {
  if (media.clips.length >= 3 || !media.trailerPath) return media;
  const videoCache = writeCacheDir(root, "video_cache");
  await fs.ensureDir(videoCache);
  const plan = buildClipSlicePlan({
    trailerPath: media.trailerPath,
    storyId,
    outputDir: videoCache,
  });
  for (const item of plan) {
    if (!(await fs.pathExists(item.output))) {
      execFileSync("ffmpeg", [
        "-y",
        "-hide_banner",
        "-loglevel",
        "error",
        "-ss",
        String(item.startS),
        "-t",
        String(item.durationS),
        "-i",
        item.input,
        "-vf",
        "scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920",
        "-an",
        "-c:v",
        "libx264",
        "-crf",
        "21",
        "-pix_fmt",
        "yuv420p",
        item.output,
      ]);
    }
    const duration = ffprobeDuration(item.output);
    if (!Number.isFinite(duration) || duration <= 0.2) {
      await fs.remove(item.output).catch(() => {});
    }
  }
  return discoverLocalStudioMedia({ root, storyId });
}

async function ensureTrailerFrames({ root, storyId, media }) {
  if (media.trailerFrames.length >= 6 || !media.trailerPath) return media;
  const imageCache = writeCacheDir(root, "image_cache");
  await fs.ensureDir(imageCache);
  const plan = buildFrameExtractionPlan({
    trailerPath: media.trailerPath,
    storyId,
    outputDir: imageCache,
  });
  for (const item of plan) {
    if (await fs.pathExists(item.output)) continue;
    execFileSync("ffmpeg", [
      "-y",
      "-hide_banner",
      "-loglevel",
      "error",
      "-ss",
      String(item.timeS),
      "-i",
      item.input,
      "-frames:v",
      "1",
      "-vf",
      "format=yuvj420p",
      "-q:v",
      "2",
      item.output,
    ]);
  }
  return discoverLocalStudioMedia({ root, storyId });
}

function rankSourceDiversity(media) {
  const groups = {
    clips: media.clips.length,
    trailerFrames: media.trailerFrames.length,
    articleHeroes: media.articleHeroes.length,
    publisherAssets: media.publisherAssets.length,
    stockFillers: media.stockFillers.length,
  };
  const topicalSources =
    groups.clips + groups.trailerFrames + groups.articleHeroes + groups.publisherAssets;
  const stockSuppressed = groups.stockFillers > 0 && topicalSources >= 3;
  const score = Math.min(100, topicalSources * 12 + groups.clips * 14);
  return {
    groups,
    topicalSources,
    stockSuppressed,
    sourceMixScore: score,
    verdict: stockSuppressed
      ? "stock-suppressed"
      : groups.stockFillers > 0
        ? "stock-risk"
        : "topical-only",
  };
}

module.exports = {
  discoverLocalStudioMedia,
  ensureTrailerClipSlices,
  ensureTrailerFrames,
  buildClipSlicePlan,
  buildFrameExtractionPlan,
  rankSourceDiversity,
  ffprobeDuration,
  resolveFfprobeDurationTimeoutMs,
  activeStudioMediaRoots,
};
