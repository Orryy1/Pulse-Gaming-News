...
const BASE_SOURCE_ASSET_ID_FIELDS = Object.freeze([
...
function canonicaliseMotionSourceUrl(value) {
  ...
  try {
    ...
    if (
      ...
      parsed.hostname = "video.steamstatic.com";
      parsed.port = "";
...
function canonicaliseBaseSourceAssetId(value) {
  const text = cleanText(value)
    .toLowerCase()
    .replace(/\s+/g, "-")
    .replace(/^asset:/, "");
  if (
    ...
    text.length < 3 ||
    text.length > 256 ||
    !/^[a-z0-9][a-z0-9._:/-]*$/.test(text) ||
    /(?:^|[-_/:.])(?:placeholder|dummy|pending|tbd|unknown|missing)(?:$|[-_/:.])/.test(text)
  ) {
    return "";
  }
  return `asset:${text}`;
...
function resolveMotionSourceIdentity(clip, { clipIndex = null } = {}) {
  ...
  for (const entry of fieldEntries(clip, BASE_SOURCE_ASSET_ID_FIELDS)) {
    addEvidence(
      entry,
      canonicaliseBaseSourceAssetId(entry.value),
      "base_source_asset_id",
    );
  }
