...
async function mergeSegmentAlignments(segmentRelPaths, options = {}) {
  ...
  const interSegmentGapS = Math.max(0, Number(options.interSegmentGapS || 0));
  ...
  for (let segmentIndex = 0; segmentIndex < segmentRelPaths.length; segmentIndex++) {
    ...
    if (segmentIndex < segmentRelPaths.length - 1) {
      cumulativeOffset += interSegmentGapS;
    }
  }
...
function resolveLocalInterSegmentPauseS({
  ...
  if (String(provider || "").toLowerCase() !== "local") return 0;
  const durations = (segmentDurations || [])
    .map((duration) => Number(duration))
    .filter((duration) => Number.isFinite(duration) && duration > 0);
  if (durations.length <= 1) return 0;

  const targetDurationS = clampNumber(
    Number(env.STUDIO_V2_LOCAL_TTS_TARGET_DURATION_S || 61.2),
    50,
    75,
  );
  const maxPauseS = clampNumber(
    Number(env.STUDIO_V2_LOCAL_TTS_MAX_INTERSEGMENT_PAUSE_S || 0.95),
    0,
    1.5,
  );
  ...
  if (missingS <= 0) return 0;
  return Number(Math.min(maxPauseS, missingS / (durations.length - 1)).toFixed(3));
}
...
async function ensureProductionVoice({
    ...
    : [
        {
          label: "full",
          text: fallbackText,
          rate: isLocal ? 1.0 : brand.voiceSettings?.speaking_rate || 1.1,
        },
      ];
  ...
  try {
    ...
    if (voiceSegments.length > 1) {
      ...
      const interSegmentGapS = resolveLocalInterSegmentPauseS({
        provider,
        segmentDurations,
        env: process.env,
      });
      if (interSegmentGapS > 0) {
        cumulativeS = 0;
        segmentTimeline = segmentTimeline.map((segment, index) => {
          const startS = cumulativeS;
          const endS = startS + segment.durationS;
          cumulativeS = endS + (index < segmentTimeline.length - 1 ? interSegmentGapS : 0);
          return {
            ...segment,
            startS: Number(startS.toFixed(3)),
            endS: Number(endS.toFixed(3)),
            interSegmentGapAfterS:
              index < segmentTimeline.length - 1 ? interSegmentGapS : 0,
          };
        });
      }
              ...
              provider === "local" && voiceSegments.length > 1
                ? {
                    ...
                    target_duration_s: Number(
                      (process.env.STUDIO_V2_LOCAL_TTS_TARGET_DURATION_S || 61.2),
                    ),
                    method: "concat_inserted_silence_between_native_rate_segments",
                  }
                : null,
