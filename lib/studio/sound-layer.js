...
} = require("./local-tts-segmentation");
...
const LOCAL_INTER_SEGMENT_PAUSE_VERSION = 5;
const DEFAULT_LOCAL_TTS_TARGET_DURATION_S = 62.5;
const DEFAULT_LOCAL_TTS_MAX_INTERSEGMENT_PAUSE_S = 0.42;
const HARD_LOCAL_TTS_MAX_INTERSEGMENT_PAUSE_S = 0.55;
const DEFAULT_LOCAL_TTS_MAX_OUTRO_LEAD_GAP_S = 0.25;
const DEFAULT_LOCAL_TTS_MAX_TITLE_FRAGMENT_GAP_S = 0.08;
...
const LEGACY_STUDIO_OUTRO_LINE =
...
async function ensureFreshLocalLiam({
  ...
  const outputMp3 = path.join(audioDir, `${outputStem}.mp3`);
  const outputTs = path.join(audioDir, `${outputStem}_timestamps.json`);
  const { text, displayText } = normaliseFreshLocalLiamEditorialText(editorial);
...
function normaliseFreshLocalLiamEditorialText(editorial = {}) {
  const rawText = editorial?.scriptForTTS || editorial?.fullScript || "";
  const text = productionAudio.cleanForTTS(rawText);
  const displayText =
...
function cleanProductionVoiceSegmentText(text) {
  ...
  const defaultCta = cleanTerminalCtaForSegment(LEGACY_STUDIO_OUTRO_LINE);
...
function buildProductionVoiceSegments(editorial, env = process.env) {
  ...
  const cleaned = rawSegments
    .map((segment) => {
      const cleanText = cleanProductionVoiceSegmentText(segment.text);
      return {
        ...segment,
...
function mergeSplitTitleContinuationSegments(segments) {
  ...
  for (const segment of segments) {
    ...
    if (previous && isSplitTitleFragmentBoundary(previous, segment)) {
      ...
      previous.rawText = [
        previous.rawText || previous.text || previous.cleanText || "",
        segment.rawText || segment.text || segment.cleanText || "",
      ]
        .join(" ")
        .replace(/\s+/g, " ")
        .trim();
...
function resolveLocalInterSegmentGapSchedule({
  ...
  if (!gapCount) return [];
  const baseGapS = clampNumber(
    Number(interSegmentPausePlan?.gapS || 0),
    0,
    HARD_LOCAL_TTS_MAX_INTERSEGMENT_PAUSE_S,
  );
  const maxPauseS = clampNumber(
    Number(interSegmentPausePlan?.maxPauseS || baseGapS),
    0,
    HARD_LOCAL_TTS_MAX_INTERSEGMENT_PAUSE_S,
  );
...
function isSplitTitleFragmentBoundary(left, right) {
  ...
  const rawLeft = String(left?.rawText || left?.text || "").trim();
  const leftEndedWithTitleSeparator = /[:\uFF1A\uFE55,\/\-\u2013\u2014]\s*$/.test(rawLeft);
  if (leftEndedWithTitleSeparator) return true;
...
function splitLongVoiceSegments(segments, opts = {}, env = process.env) {
  ...
  for (const segment of segments || []) {
    ...
    if (!text) continue;
    const chunks = splitLocalTtsRequestSegments(text, env, opts);
    if (chunks.length <= 1) {
