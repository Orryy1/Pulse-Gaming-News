...
function resolveStudioOutroLine(story = {}) {
  return resolveGovernedCtaCopy(story);
}

function buildProductionVoiceSegments(editorial, { story = null } = {}) {
  ...
  const contextualCta = resolveStudioOutroLine(story);
  const existingNarration = rawSegments
  ...
  if (
    contextualCta &&
    !existingNarration.includes(contextualCta.toLowerCase())
  ) {
    rawSegments.push({
      label: "cta",
      text: contextualCta,
...
function applyLocalVoiceRateMultiplier(segments, env = process.env) {
  ...
  const baseSpeed = clampNumber(
    Number(env.STUDIO_V2_LOCAL_TTS_BASE_SPEED || 1.4),
    0.5,
    3.0,
  );
  const effectiveRateCap = clampNumber(
    Number(env.STUDIO_V2_LOCAL_TTS_EFFECTIVE_RATE_CAP || 2.35),
    1.0,
    2.4,
  );
...
async function ensureProductionVoice({
  ...
  const segments = buildProductionVoiceSegments(editorial, { story });
