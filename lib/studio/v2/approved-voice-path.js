...
function hasSplitGtaViRomanNarration(transcript) {
  const normalised = normaliseText(transcript);
  return (
    /\bgtavi\s+(?:now|just|has|is|starts?|turns?|looks|pre|preorders?|cover|delay|launch|release)\b/.test(normalised) ||
    /\b(?:g\s+t\s+a|gta|grand\s+theft\s+auto)\s+v\s+i\s+(?:now|just|has|is|starts?|turns?|looks|pre|preorders?|cover|delay|launch|release)\b/.test(normalised)
  );
}
