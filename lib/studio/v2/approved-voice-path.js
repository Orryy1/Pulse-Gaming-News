...
function hasMalformedGtaViSpokenStutter(transcript) {
  const normalised = normaliseText(transcript);
  return (
    /\b(?:g\s+t\s+a|gta|grand\s+theft\s+auto)\s+(?:s|si|sy|sigh|see|sea|c)\s+six\b/.test(normalised) ||
    /\b(?:g\s+t\s+a|gta|grand\s+theft\s+auto)\s+six\s+six\b/.test(normalised)
  );
}
...
function hasRiskyGtaViOpening(transcript) {
  ...
  return (
    ...
    /\b(?:g\s+t\s+a|gta|grand\s+theft\s+auto)\s+(?:s|si|sy|sigh|see|sea|c)\s+six\b/.test(opening) ||
    /\b(?:g\s+t\s+a|gta|grand\s+theft\s+auto)\s+six\s+six\b/.test(opening) ||
