 ...
 * cue list. The cue planner stays varied and low-level, with a hard cap that
 * is high enough for creator-grade movement but still below the forensic
...
const MAX_STUDIO_SFX_CUES = 4;
const MIN_STUDIO_CUE_SPACING_S = 2.4;
...
function findTransitionAtOrAfter(transitions = [], atS = 0) {
  return (transitions || [])
    .slice()
    .sort((a, b) => Number(a?.offset || 0) - Number(b?.offset || 0))
    .find((transition) => Number(transition?.offset || 0) >= atS);
}
...
function buildStudioSfxCues({ scenes = [], transitions = [] } = {}) {
  ...
  const totalDurationS = totalSceneDuration(scenes);
  const candidates = [];
  ...
  const firstHandoff = findTransitionAtOrAfter(transitions, 1.4);
  if (firstHandoff) {
    addCueCandidate(candidates, {
      family: "transition",
      kind: "hook-handoff",
      atS: Number(firstHandoff.offset || 0) + 0.3,
      category: "transition",
      preferred: ["whoosh-short", "swipe-tonal", "whoosh-sub"],
      vol: 0.075,
      durationS: 0.3,
      priority: 88,
      reason: "hook-to-context motion handoff",
    });
  }

  const midHandoff = findTransitionAtOrAfter(
    transitions,
    Math.max(7, totalDurationS * 0.42),
  );
  if (midHandoff) {
    addCueCandidate(candidates, {
      family: "transition",
      kind: "midroll-swipe",
      atS: Number(midHandoff.offset || 0),
      category: "transition",
      preferred: ["swipe-tonal", "whoosh-up", "whoosh-down"],
      vol: 0.065,
      durationS: 0.28,
      priority: 68,
      reason: "mid-story pace reset",
    });
  }
...
function resolveAudioPlan({ story, scenes = [], transitions = [] } = {}) {
  ...
  return {
    ...
    decisions: {
      ...
      forensicSafeCueCount: sfxCues.length <= 4,
