...
function classifyFinalRenderVoice({
  ...
  const warnings = normaliseBlockers(approved.warnings);
  const verdict =
    blockers.length > 0
      ? "reject"
      : warnings.length > 0
...
function renderFinalVoiceAuditMarkdown(report) {
  ...
  if (!report.rows.length) {
    ...
    for (const row of report.rows) {
      lines.push(
        `- ${row.story_id || "unknown"}: ${row.verdict} blockers=${row.blockers.join(", ") || "clear"} warnings=${row.warnings.join(", ") || "none"} reuse_for_tiktok=${!row.do_not_reuse_for_tiktok_dispatch}`,
      );
    }
  }
  lines.push("");
