...
function voiceLoudnessWarnings({ approved = {}, narration = {} } = {}) {
  const warnings = [];
  ...
  const wpm = numberOrNull(narration.wpm ?? narration.localPace?.wpm ?? narration.pace?.wpm);

  if (localVoice && integratedLufs === null) warnings.push("voice_loudness_unverified");
  else if (integratedLufs !== null && integratedLufs > -12) warnings.push("voice_loudness_too_hot");
...
function classifyFinalRenderVoice({
  ...
  const warnings = normaliseBlockers([
    ...(approved.warnings || []),
    ...voiceLoudnessWarnings({ approved, narration }),
  ]);
  const verdict =
    blockers.length > 0
      ? "reject"
  ...
  return {
    ...
    voice_path: {
      ...
      wpm: numberOrNull(narration.wpm ?? narration.localPace?.wpm ?? narration.pace?.wpm),
    },
  };
}
