...
function storyIdFromPath(mp4Path) {
  ...
  if (/^visual_v4_render(?:_.+)?$/i.test(base)) {
    const parent = path.basename(path.dirname(String(mp4Path || "")));
    const grandparent = path.basename(path.dirname(path.dirname(String(mp4Path || ""))));
    if (grandparent === "platform_variants") {
      return path.basename(path.dirname(path.dirname(path.dirname(String(mp4Path || "")))));
    }
...
function rowCounts(rows = []) {
  return {
    ...
    unknown: rows.filter((row) => !["pass", "review", "reject", "skip"].includes(row.verdict)).length,
  };
}

function verdictForRows(rows = [], inspectionBlockers = []) {
  const counts = rowCounts(rows);
  if (inspectionBlockers.length > 0) return "AMBER";
  if (rows.length === 0) return "AMBER";
  if (counts.reject > 0 || counts.unknown > 0) return "RED";
  if (counts.review > 0) return "AMBER";
  return "GREEN";
}
...
function voiceLoudnessWarnings({ approved = {}, narration = {}, env = process.env } = {}) {
  ...
  const maxLocalWpm = numberOrNull(env.STUDIO_FINAL_VOICE_MAX_LOCAL_WPM) ?? 220;
  const targetMaxLocalWpm = numberOrNull(env.STUDIO_FINAL_VOICE_TARGET_MAX_LOCAL_WPM) ?? 190;
  ...
  if (localVoice && wpm !== null && wpm > maxLocalWpm) warnings.push("voice_pace_too_fast");
  else if (localVoice && wpm !== null && wpm > targetMaxLocalWpm) {
    warnings.push("voice_pace_above_target");
  }

  return warnings;
}

function voiceLoudnessBlockers(warnings = []) {
  return normaliseBlockers((warnings || []).filter((warning) =>
    warning === "voice_segment_loudness_jump" ||
    warning === "voice_segment_loudness_late_jump" ||
    warning === "voice_pace_too_fast",
  ));
}
...
function classifyFinalRenderVoice({
  ...
  const loudnessWarnings = voiceLoudnessWarnings({ approved, narration, env });
  const loudnessBlockers = voiceLoudnessBlockers(loudnessWarnings);
  const blockers = normaliseBlockers([
    ...(approved.blockers || []),
    ...loudnessBlockers,
  ]);
  const warnings = normaliseBlockers([
    ...(approved.warnings || []),
    ...loudnessWarnings.filter((warning) => !loudnessBlockers.includes(warning)),
  ]);
  const verdict =
    blockers.length > 0
      ? "reject"
...
function buildFinalVoiceAudit({
  ...
  const activePathSet = new Set(
    (Array.isArray(activeVideoPaths) ? activeVideoPaths : [])
      .map(normaliseComparablePath)
      .filter(Boolean),
  );
  const scopedToActiveManifest = scopeToActiveManifest === true || activePathSet.size > 0;
  const activeRows = scopedToActiveManifest
    ? rows.filter((row) => activePathSet.has(normaliseComparablePath(row.mp4_path)))
    : rows;
  const quarantinedRows = scopedToActiveManifest
    ? rows.filter((row) => !activePathSet.has(normaliseComparablePath(row.mp4_path)))
    : [];
  const counts = rowCounts(rows);
  const activeCounts = rowCounts(activeRows);
  const quarantinedCounts = rowCounts(quarantinedRows);
  const inspectionBlockers = rows.length === 0 ? ["no_final_mp4s_inspected"] : [];
  const activeInspectionBlockers =
    scopedToActiveManifest && activeRows.length === 0
      ? ["no_active_manifest_mp4s_inspected"]
      : inspectionBlockers;
  return {
    schema_version: 1,
    generated_at: generatedAt,
    verdict: scopedToActiveManifest
      ? verdictForRows(activeRows, activeInspectionBlockers)
      : verdictForRows(rows, inspectionBlockers),
    readiness_scope: scopedToActiveManifest ? "active_manifest" : "all_inspected_files",
    counts,
    active_counts: activeCounts,
    quarantined_counts: quarantinedCounts,
    active_row_count: activeRows.length,
    quarantined_row_count: quarantinedRows.length,
    inspection_blockers: inspectionBlockers,
    active_inspection_blockers: activeInspectionBlockers,
    rows,
    safety: {
      read_only: true,
      mutates_media: false,
...
function renderFinalVoiceAuditMarkdown(report) {
  ...
  lines.push(`Readiness scope: ${report.readiness_scope || "all_inspected_files"}`);
  lines.push(
    `Counts: pass=${report.counts.pass} review=${report.counts.review} reject=${report.counts.reject} skip=${report.counts.skip}`,
  );
  if (report.active_counts) {
    lines.push(
      `Active: pass=${report.active_counts.pass} review=${report.active_counts.review} reject=${report.active_counts.reject} skip=${report.active_counts.skip}`,
    );
  }
  if (report.quarantined_counts) {
    lines.push(
      `Quarantined debt: pass=${report.quarantined_counts.pass} review=${report.quarantined_counts.review} reject=${report.quarantined_counts.reject} skip=${report.quarantined_counts.skip}`,
    );
  }
  if (Array.isArray(report.inspection_blockers) && report.inspection_blockers.length) {
    lines.push(`Inspection blockers: ${report.inspection_blockers.join(", ")}`);
  }
  if (Array.isArray(report.active_inspection_blockers) && report.active_inspection_blockers.length) {
    lines.push(`Active inspection blockers: ${report.active_inspection_blockers.join(", ")}`);
  }
  lines.push("");
  lines.push("## Rows");
