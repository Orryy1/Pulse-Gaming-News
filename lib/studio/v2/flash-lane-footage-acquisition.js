...
function cleanEntity(value) {
  return normaliseText(value).trim();
}
...
function attemptedSegmentsByEntity(segments) {
  const byEntity = {};
  for (const segment of segments) {
    const entity = cleanEntity(segment.entity);
    if (!entity) continue;
    if (!byEntity[entity]) byEntity[entity] = [];
    byEntity[entity].push({
      reason: rejectedReasonForSegment(segment),
      media_start_s: numberOr(segment.media_start_s ?? segment.start_s, null),
    });
  }
  return byEntity;
...
function buildShoppingList({
  ...
  const coverageSet = new Set(coverageEntities);
  return entities
    .filter((entity) => !validatedEntities.includes(entity) || coverageSet.has(entity))
    .map((entity) => {
      const items = rejectedItems[entity] || [];
      const isCoverageItem = coverageSet.has(entity);
      const reasons = uniq(items.map((item) => item.reason));
      const attemptedStarts = attemptedStartsForRejectedItems(
        isCoverageItem ? attemptedItems?.[entity] || items : items,
      );
      ...
      return {
        ...
        reasons: requiresAlternateOfficialSource
          ? uniq([
              ...
              ...(isCoverageItem ? ["find_additional_validated_clip_window_for_existing_entity"] : []),
              "alternate_official_source_required",
            ])
          : uniq([
              ...reasonsForShoppingItem(reasons, attemptedStarts),
...
function buildSingleFlashLaneFootageAcquisitionPlan({
  ...
  const attemptedItems = attemptedSegmentsByEntity(segments);
  const sourceFamilyStatus = sourceFamilyStatusByEntity(segments);
  let shoppingList = buildShoppingList({
    entities,
    validatedEntities,
    rejectedItems,
    attemptedItems,
  ...
  if (shoppingList.length === 0 && validatedSegments.length < minValidatedClipWindows) {
    shoppingList = buildShoppingList({
      ...
      attemptedItems,
      sourceFamilyStatus,
      coverageEntities: validatedEntities,
    });
  }
