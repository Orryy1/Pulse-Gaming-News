...
function suggestedWindowsForReasons(reasons, attemptedStarts = []) {
  ...
  const attempted = new Set(attemptedStarts.map((start) => Number(start).toFixed(2)));
  let starts = primaryStarts.filter((start) => !attempted.has(Number(start).toFixed(2)));
  if (starts.length < 3) {
    starts = starts.concat(fallbackStarts.filter((start) => !attempted.has(Number(start).toFixed(2))));
  }
  ...
  return entities
    ...
    .map((entity) => {
      ...
      return {
        ...
        suggested_windows: suggestedWindowsForReasons(reasons, attemptedStarts),
        reasons: reasonsForShoppingItem(reasons, attemptedStarts),
        acceptance_criteria: [
