...
function segmentFacts(segments) {
  const validated = segments.filter((segment) => segment.allowed_for_flash_lane === true);
...
function buildFlashLaneFootageBackboneReport({
  ...
  minClipDominance = 0.65,
  maxProjectedClipRefs = 12,
  maxCandidateWindowsPerSource = 3,
  includeFrameAnchoredWindows = true,
  minSegmentQualityScore = MIN_FLASH_SEGMENT_QUALITY_SCORE,
} = {}) {
  const resolvedStoryId = storyId || story?.id || null;
  const editorial = resolveRequiredPulseEditorialContract({
    story,
    editorialContract,
  });
  const contract = editorial.contract;
  const targetRuntimeS = contract
    ? Number(
        (
          (Number(contract.min_seconds) + Number(contract.max_seconds)) /
          2
        ).toFixed(3),
      )
    : null;
  const plans = storyPlans(frameReport, resolvedStoryId);
  const segments = storySegments(segmentValidationReport, resolvedStoryId);
  const frame = frameFacts(plans);
  const segment = segmentFacts(segments);
  const projectedClipRefCount = Math.min(
    Math.max(1, Number(maxProjectedClipRefs) || 12),
    Math.max(
      minValidatedClipRefs,
      Number.isFinite(targetRuntimeS)
        ? Math.ceil((targetRuntimeS * minClipDominance) / 5)
        : minValidatedClipRefs,
    ),
  );
  const candidateClipRefs = resolvedStoryId
    ? buildOfficialTrailerClipsFromFrameReport(frameReport, resolvedStoryId, {
        segmentValidationReport,
        requireValidatedSegments: true,
        maxClips: Math.max(projectedClipRefCount, Math.max(1, Number(maxProjectedClipRefs) || 12)),
  ...
  const qualityFilteredClipRefs = candidateClipRefs.filter((ref) => {
    ...
    return !Number.isFinite(score) || score >= minimumQuality;
  });
  const validatedClipRefs = balanceClipRefsByEntity(qualityFilteredClipRefs, projectedClipRefCount, {
  ...
  const projectedClipDominance =
    Number.isFinite(targetRuntimeS) && targetRuntimeS > 0
      ? Number((projectedClipSeconds / targetRuntimeS).toFixed(2))
      : null;
  const thresholds = {
    minValidatedClipRefs,
  ...
  if (segment.validated_entities.length < minValidatedEntities) {
    blockers.push("footage_backbone_entity_coverage_too_thin");
  }
  if (
    Number.isFinite(projectedClipDominance) &&
    projectedClipDominance < minClipDominance
  ) {
    blockers.push("footage_backbone_clip_dominance_too_low");
  }
  const verdict = editorial.blocker
    ? "editorial_contract_blocked"
    : blockers.length
  ...
  return {
    ...
    blockers,
    thresholds,
    projected_clip_ref_count: projectedClipRefCount,
