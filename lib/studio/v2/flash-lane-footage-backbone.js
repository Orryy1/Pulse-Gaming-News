...
const FRAME_SUPPORTED_MAX_FLASH_CLIP_REFS_PER_SOURCE = 4;
...
function clipStart(ref) {
  const start = Number(ref?.mediaStartS ?? ref?.media_start_s);
  return Number.isFinite(start) ? start : null;
}

function clipSourceCount(refs) {
  return uniq(asArray(refs).map((ref) => clipSourceKey(ref))).length;
}

function candidateStartGap(candidate, starts) {
  const start = clipStart(candidate);
  if (!Number.isFinite(start) || !Array.isArray(starts) || starts.length === 0) return Infinity;
  return Math.min(...starts.map((selectedStart) => Math.abs(start - selectedStart)));
}

function takeNextBalancedCandidate(group, sourceCounts, selectedStartsBySource, opts) {
  ...
  for (let index = 0; index < group.length; index += 1) {
    ...
    const selectedStarts = selectedStartsBySource.get(source) || [];
    const start = clipStart(candidate);
    if (
      Number.isFinite(start) &&
      selectedStarts.some((selectedStart) => Math.abs(start - selectedStart) < minStartGapS)
    ) {
      continue;
    }

    if (sourceCount < Math.max(1, maxPerSource - 1)) {
      bestIndex = index;
      break;
    }

    const spread = candidateStartGap(candidate, selectedStarts);
  ...
  return group.splice(bestIndex, 1)[0];
}
...
function balanceClipRefsByEntity(refs, maxRefs, opts = {}) {
  ...
  const selectedStartsBySource = new Map();
  ...
  while (balanced.length < cap) {
    ...
    for (const entity of entityOrder) {
      ...
      const next = takeNextBalancedCandidate(group, sourceCounts, selectedStartsBySource, {
        maxPerSource,
        minStartGapS,
      });
      if (!next) continue;
      const source = clipSourceKey(next);
      const sourceCount = sourceCounts.get(source) || 0;
      const start = clipStart(next);
      const selectedStarts = selectedStartsBySource.get(source) || [];
      sourceCounts.set(source, sourceCount + 1);
      if (Number.isFinite(start)) {
        selectedStarts.push(start);
        selectedStartsBySource.set(source, selectedStarts);
      }
      balanced.push(next);
      added = true;
...
function buildFlashLaneFootageBackboneReport({
  ...
  const candidateWindowsPerSource = Math.max(
    Math.max(1, Number(maxCandidateWindowsPerSource) || MAX_FLASH_CLIP_REFS_PER_SOURCE),
    FRAME_SUPPORTED_MAX_FLASH_CLIP_REFS_PER_SOURCE,
  );
  const candidateClipRefs = storyId
    ? buildOfficialTrailerClipsFromFrameReport(frameReport, storyId, {
        ...
        maxCandidateWindowsPerSource: candidateWindowsPerSource,
  ...
  const qualityFilteredSourceCount = clipSourceCount(qualityFilteredClipRefs);
  const maxClipRefsPerSource =
    qualityFilteredSourceCount >= 3 && frame.accepted_frames >= 3
      ? FRAME_SUPPORTED_MAX_FLASH_CLIP_REFS_PER_SOURCE
  ...
  const validatedClipRefs = balanceClipRefsByEntity(qualityFilteredClipRefs, maxProjectedClipRefCount, {
    maxPerSource: maxClipRefsPerSource,
    minStartGapS: 4,
  });
  ...
  return {
    ...
    max_clip_refs_per_source: maxClipRefsPerSource,
    quality_filtered_source_count: qualityFilteredSourceCount,
