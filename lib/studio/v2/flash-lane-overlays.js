...
const { hasVerifiedRedditReaction } = require("../../reddit-discussion-enrichment");
...
function numericBoxValue(value, frame = DEFAULT_FRAME) {
  ...
  const widthOffset = text.match(/^(?:w|iw)-(\d+(?:\.\d+)?)$/);
...
function isSteamPlayerMetricStory(story = {}) {
  ...
  return (
    ...
    /\b(?:concurrent players|players|peak|record|ccu)\b/i.test(text)
  );
}
...
function extractStoryBeats({ story = {}, entities = [] } = {}) {
  const text = storyText(story);
  const beats = [];
  const hasSteamPlayerStat = isSteamPlayerMetricStory(story);
  const positiveSubscriptionValue =
    /\b(?:free|freebie|included|no extra cost|comes with|available through|on game pass|game pass launch|ps plus|playstation plus)\b/i.test(
      text,
    ) &&
    !/\b(?:does not count|not count|doesn't count|excluding|excluded from|outside|not included)\b[^.]{0,90}\b(?:game pass|ps plus|playstation plus)\b/i.test(
      text,
    );
  ...
  if (hasSteamPlayerStat) {
    addBeat(beats, "Steam peak", "source_backed_player_metric");
  }
  if (hasSteamPlayerStat && /\b(?:early access|premium early|before standard launch)\b/i.test(text)) {
    addBeat(beats, "Early access", "storefront_metric_context");
  }
  ...
  if (/\b(?:down this quarter|revenue|player.*down|daily ccu|top 50)\b/i.test(text)) {
...
function hookChipLabel({ story = {}, beats = [] } = {}) {
  ...
  if (beats.some((beat) => beat.label === "STEAM PEAK")) return "HARD NUMBERS";
  ...
  if (/\b(?:reports?|according to|source|says)\b/i.test(text)) return "SOURCE CHECK";
  if (/\b(?:rumour|rumor|reportedly|allegedly)\b/i.test(text)) return "RUMOUR WATCH";
  return "WAIT... WHAT?";
}
...
function sourceLabel(story = {}) {
  ...
  const text = `${story.title || ""} ${story.hook || ""} ${story.body || ""} ${story.full_script || ""}`;
  const outlet = text.match(/\b(GamesRadar|IGN|GameSpot|Eurogamer|PC Gamer|VGC|Polygon|Kotaku)\b/i)?.[1];
  if (outlet) return cleanLabel(outlet).toUpperCase();
  const namedSource = story.publisher || story.source_name || story.outlet || "";
  if (namedSource) return cleanLabel(namedSource).toUpperCase();
...
function commentOverlay(story = {}) {
  const sourceType = String(story.source_type || "").toLowerCase();
  if (sourceType === "reddit" && isSteamPlayerMetricStory(story)) {
    return {
      allowed: false,
      source_type: "stat_led_story",
      reason: "Hard-number stories should not spend a Flash Lane beat on a loose Reddit comment.",
    };
  }
  ...
  if (hasVerifiedRedditReaction(story)) {
    const related = String(story.comment_source_type || "").toLowerCase() === "related_reddit_discussion";
    return {
      allowed: true,
      source_type: related ? "related_reddit_discussion" : "real_reddit_comment",
      label: `r/${cleanLabel(related ? story.reddit_discussion?.subreddit : story.subreddit || "gaming")}`,
      reason: related
        ? "Verified related Reddit discussion used as labelled audience reaction."
        : "Reddit source with a real top comment.",
    };
  }
  ...
  return {
    allowed: false,
    source_type: sourceType === "rss" ? "rss_description_only" : "unknown_non_reddit_text",
...
function position(anchor, width) {
  switch (anchor) {
    case "upper_right":
      return { x: `iw-${width}`, y: "128", align: "right" };
    case "mid_left":
...
function chipFilters(item, fontOpt) {
  ...
  const textX = p.align === "right" ? "w-tw-28" : `${p.x}+28`;
  const kicker =
