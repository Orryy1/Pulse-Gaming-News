...
function buildFlashLaneOverlayPlan({ story = {}, scenes = [], durationS = 0 } = {}) {
  ...
  return {
    ...
    timeline: uniqueTimeline(timeline).sort((a, b) => a.at_s - b.at_s),
    safety: {
      local_only: true,
...
function position(anchor) {
  switch (anchor) {
    ...
    case "upper_left":
    default:
      // Scene-level source/entity badges occupy y=112..200 for the
      // first moments of each scene. Global chips are applied after
      // scene concat, so keeping the upper-left lane lower prevents
      // cross-scene overlays from colliding with those badges.
      return { x: "64", y: "250", align: "left" };
  }
}
