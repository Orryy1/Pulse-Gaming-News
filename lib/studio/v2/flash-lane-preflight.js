...
const MIN_FLASH_RUNTIME_S = 50;
const MIN_CREATOR_REWARDS_RUNTIME_S = 61;
...
function buildFlashLaneNarrationPlan({ scriptWordCount, narrationDurationS } = {}) {
  ...
  if (Number.isFinite(duration) && duration > 0) {
    if (duration < MIN_FLASH_RUNTIME_S) issues.push("narration_too_short_for_flash_lane");
    else if (duration < MIN_CREATOR_REWARDS_RUNTIME_S) {
      warnings.push("narration_below_creator_rewards_runtime_target");
    }
  ...
  } else if (issues.includes("narration_too_short_for_flash_lane")) {
    recommendation = "expand_or_slow_slightly_before_render";
  } else if (warnings.includes("narration_below_creator_rewards_runtime_target")) {
    recommendation = "publishable_short_consider_expanding_for_creator_rewards";
  }

  return {
    targetRuntimeS: [MIN_FLASH_RUNTIME_S, MAX_FLASH_RUNTIME_S],
    creatorRewardsTargetRuntimeS: [MIN_CREATOR_REWARDS_RUNTIME_S, MAX_FLASH_RUNTIME_S],
...
function buildFlashLaneProofPreflight({
  ...
  if (
    ...
    blockers.push("flash_lane_runtime_outside_50_to_75_seconds");
  } else if (
    Number.isFinite(narrationDurationS) &&
    narrationDurationS < MIN_CREATOR_REWARDS_RUNTIME_S
  ) {
    warnings.push("flash_lane_below_creator_rewards_runtime_target");
  }
  ...
  if (actualClipDominance < TARGET_ACTUAL_CLIP_DOMINANCE) {
    ...
    const mixedOfficialFramesCarryGap =
      exhaustedValidatedClipRefs &&
      availableTrailerFrames >= 6 &&
      supportingMotionScenes >= 3 &&
      motionDominance >= 0.55 &&
      cardRatio <= MAX_CARD_RATIO &&
      Number(directorMetrics.distinctSceneBeats || 0) >= MIN_CARD_HEAVY_DISTINCT_SCENE_BEATS;
    if (trailerFramesCarryGap || mixedOfficialFramesCarryGap) {
  ...
  return {
    ...
    thresholds: {
      ...
      minCreatorRewardsRuntimeS: MIN_CREATOR_REWARDS_RUNTIME_S,
...
function buildFlashLaneProofReadinessSummary({ preflight, overlayPlan, scenes } = {}) {
  ...
  const visualQualityClean =
    (visualMetrics.weakClipSegments || []).length === 0 &&
    (visualMetrics.unvalidatedOfficialSegments || []).length === 0 &&
    (visualMetrics.badFrameTasteScenes || []).length === 0 &&
    (visualMetrics.ratingSlateScenes || []).length === 0;
  const frameSupportedMotionIsProductionReady =
    motionDominance !== null &&
    motionDominance >= 0.7 &&
    Number(metrics.actualClipScenes || 0) >= 3 &&
    uniqueClipSources !== null &&
    uniqueClipSources >= 3 &&
    distinctSceneBeats !== null &&
    distinctSceneBeats >= MIN_CARD_HEAVY_DISTINCT_SCENE_BEATS &&
    cardRatio !== null &&
    cardRatio <= Math.min(MAX_CARD_RATIO, 0.16) &&
    storyBeatOverlayCount >= requiredBeatOverlayMinimum &&
    visualQualityClean;
  const notes = [];
  ...
  for (const warning of warnings) {
    if (
      warning === "flash_lane_clip_dominance_supported_by_trailer_frames" &&
      frameSupportedMotionIsProductionReady
    ) {
      notes.push(warning);
    } else {
  ...
  if (preflight?.verdict === "block" || blockers.length > 0) {
    verdict = "blocked";
  } else if (activeWarnings.length === 0 && !cardHeavyWithoutBeatCoverage) {
  ...
  if (!blockers.length && activeWarnings.length) reasons.push(...activeWarnings);
