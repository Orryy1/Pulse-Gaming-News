...
function minUniqueClipSourcesForRuntime(runtimeS) {
  const duration = Number(runtimeS);
  if (!Number.isFinite(duration) || duration <= 0) return 3;
  return Math.max(2, Math.min(3, Math.ceil(duration / 15)));
}

function minDistinctSceneBeatsForRuntime(runtimeS) {
  const duration = Number(runtimeS);
  if (!Number.isFinite(duration) || duration <= 0) return 8;
  return Math.max(5, Math.ceil(duration / 5));
}
...
function buildFlashLaneVisualDirector({ scenes, media, narrationDurationS } = {}) {
  ...
  const minUniqueClipSources = minUniqueClipSourcesForRuntime(runtimeS);
  const minDistinctSceneBeats = minDistinctSceneBeatsForRuntime(runtimeS);
  ...
  if (uniqueClipSources < minUniqueClipSources) {
    blockers.push(
  ...
  if (distinctSceneBeats < minDistinctSceneBeats) {
  ...
  if (
    ...
    recommendation =
      "acquire_more_unique_official_footage_or_downgrade_format";
...
module.exports = {
  ...
  isActualClipScene,
  minDistinctSceneBeatsForRuntime,
  minUniqueClipSourcesForRuntime,
