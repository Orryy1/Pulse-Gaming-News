...
const FRAME_SUPPORTED_MAX_CLIP_SCENES_PER_SOURCE = 4;
const MIN_SAFE_CLIP_START_S = 36;
...
function sceneClipTimingProvenance(scene) {
  return scene?.clipTimingProvenance || scene?.clip_timing_provenance || scene?.provenance || {};
}

function validatedSegmentStart(scene) {
  const provenance = sceneClipTimingProvenance(scene);
  const candidates = [
    provenance.segment_original_start_s,
    provenance.segmentOriginalStartS,
    provenance.segment_recommended_start_s,
    provenance.segmentRecommendedStartS,
    sceneStart(scene),
  ];
...
function usesValidatedSegmentWindow(scene) {
  const provenance = sceneClipTimingProvenance(scene);
  const policy = [
    provenance.clip_start_policy,
    provenance.segment_selection_policy,
    provenance.segmentSelectionPolicy,
  ]
    .filter(Boolean)
    .join(" ");
  return (
    provenance.segment_validated === true ||
    provenance.allowed_for_flash_lane === true ||
    /validated/i.test(policy)
  );
}

function clipReuseSourceId(scene) {
  const id = normaliseSource(sourceId(scene));
  if (!id) return "";
  if (!usesValidatedSegmentWindow(scene)) return id;
  const start = validatedSegmentStart(scene);
  return start === null ? id : `${id}@${start.toFixed(2)}`;
}

function clipBeatSourceId(scene) {
  const id = normaliseSource(sourceId(scene));
  if (!id) return "";
  if (usesValidatedSegmentWindow(scene)) return clipReuseSourceId(scene);
  return `${id}@${sceneStart(scene) ?? "x"}`;
}
...
function countDistinctSceneBeats(scenes) {
  ...
  for (const scene of scenes) {
    ...
    if (isActualClipScene(scene)) {
      beats.add(`clip:${clipBeatSourceId(scene) || sourceId(scene)}:${type}`);
...
function buildFlashLaneVisualDirector({ scenes, media, narrationDurationS } = {}) {
  ...
  for (const scene of actualClipScenes) {
    const id = clipReuseSourceId(scene);
  ...
  const maxAllowedClipScenesPerSource =
    is60sLane && uniqueClipSources >= MIN_UNIQUE_CLIP_SOURCES_60S
      ? FRAME_SUPPORTED_MAX_CLIP_SCENES_PER_SOURCE
      : MAX_CLIP_SCENES_PER_SOURCE;
  const overusedClipSources = [...clipCounts.entries()]
    .filter(([, count]) => count > maxAllowedClipScenesPerSource)
  ...
  return {
    ...
    thresholds: {
      ...
      maxClipScenesPerSource: maxAllowedClipScenesPerSource,
