...
} = require("../local-tts-voice-id");
const { resolveTtsOutputFormat } = require("../../../audio");
...
const DEFAULT_MIN_SPECTRAL_ROLLOFF_85_HZ = 850;
const DEFAULT_MIN_HIGH_FREQUENCY_RATIO_GT_5KHZ = 0.0008;
...
function acousticValue(acoustic = {}, key, ...aliases) {
  const values = [typeof key === "string" ? acoustic[key] : key];
  for (const alias of aliases) {
    values.push(typeof alias === "string" ? acoustic[alias] : alias);
  }
  return numberOrNull(...values);
}

function localTtsClarityMetrics(acoustic, env = process.env) {
  const minRolloff =
    numberOrNull(env.STUDIO_FLASH_VOICE_MIN_ROLLOFF_85_HZ) ||
    DEFAULT_MIN_SPECTRAL_ROLLOFF_85_HZ;
  const minHighRatio =
    numberOrNull(env.STUDIO_FLASH_VOICE_MIN_HIGH_FREQ_RATIO_GT_5KHZ) ||
    DEFAULT_MIN_HIGH_FREQUENCY_RATIO_GT_5KHZ;
  const rolloff85Hz = acousticValue(
    acoustic,
    "spectralRolloff85Hz",
    "rolloff85Hz",
    "rolloff_85_hz_median",
    "spectral_rolloff_85_hz",
  );
  const highFrequencyRatioGt5Khz = acousticValue(
    acoustic,
    "highFrequencyRatioGt5Khz",
    "highFrequencyRatio",
    "high_freq_ratio_gt_5khz_median",
    "high_frequency_ratio_gt_5khz",
  );
  ...
  return {
    ...
    highFrequencyRatioGt5Khz,
    thresholds: {
      minRolloff85Hz: minRolloff,
      minHighFrequencyRatioGt5Khz: minHighRatio,
    },
  };
}

function localTtsClarityBlocker(acoustic, env = process.env) {
  if (!acoustic || typeof acoustic !== "object") return null;
  const clarity = localTtsClarityMetrics(acoustic, env);
  const { rolloff85Hz, highFrequencyRatioGt5Khz, thresholds } = clarity;
  const minRolloff = thresholds.minRolloff85Hz;
  const lowRolloff = rolloff85Hz !== null && rolloff85Hz < minRolloff;
  const lowHighFrequency =
    highFrequencyRatioGt5Khz !== null &&
    highFrequencyRatioGt5Khz < thresholds.minHighFrequencyRatioGt5Khz;
  if (!lowRolloff && !lowHighFrequency) return null;
  return {
    code: "local_tts_filtered_voice_risk",
    rolloff85Hz,
    highFrequencyRatioGt5Khz,
    thresholds,
  };
...
function evaluateAcousticProfile(acoustic, env = process.env) {
  ...
  const clarityMetrics = localTtsClarityMetrics(acoustic, env);
  const clarity = localTtsClarityBlocker(acoustic, env);
  ...
  return {
    ...
    metrics: {
      ...
      silenceRatio,
      clippingRatio,
      spectralRolloff85Hz: clarityMetrics.rolloff85Hz,
      highFrequencyRatioGt5Khz: clarityMetrics.highFrequencyRatioGt5Khz,
      thresholds: {
        minPitchHz: minPitch,
        ...
        minLufs,
        maxLufs,
        minRolloff85Hz: clarityMetrics.thresholds.minRolloff85Hz,
        minHighFrequencyRatioGt5Khz:
          clarityMetrics.thresholds.minHighFrequencyRatioGt5Khz,
      },
    },
  };
}
...
function evaluateVoiceCandidate({
  ...
  const warnings = [...(narrationPlan.warnings || [])];
  ...
  const tempoStretch = localVoice ? detectLocalTtsTempoStretch(candidate) : null;
  if (localVoice && tempoStretch.applied) {
    blockers.push("local_tts_tempo_stretch_applied");
  }
  const nonNativeRate = localVoice ? detectLocalTtsNonNativeRate(candidate) : null;
...
function buildGenerationPlan({ dryRun = true, story, contract } = {}) {
  ...
  return {
    ...
    target_runtime_seconds: contract?.runtime_target_seconds || {
      min: 50,
      max: 75,
      creator_rewards_min: 61,
    },
    candidate_outputs_under: "test/output/flash-lane-voice-workbench",
    next_step:
      contract?.next_action === "generate_approved_flash_lane_voice"
        ? "generate_or_supply_50_to_75_second_voice_candidates"
...
function resolveLocalGenerationRate(engine, rate) {
  const requestedRate = Number(rate);
  const safeRequestedRate = Number.isFinite(requestedRate) && requestedRate > 0 ? requestedRate : 1.0;
  const boundedRate = Math.min(2.4, Math.max(0.6, safeRequestedRate));
  return {
    rate: boundedRate,
    requestedRate: safeRequestedRate,
    adjusted: Math.abs(boundedRate - safeRequestedRate) > 1e-3,
    reason:
      Math.abs(boundedRate - safeRequestedRate) > 1e-3
        ? "local_tts_rate_clamped"
        : null,
  };
}
...
async function generateLocalVoiceCandidate({
  ...
  const resolvedVoiceId = canonicalLocalTtsVoiceId(voiceId);
  const resolvedRate = resolveLocalGenerationRate(engine, rate);
  ...
  const request = {
    ...
    requestedRate: resolvedRate.requestedRate,
    rateAdjusted: resolvedRate.adjusted,
    rateAdjustmentReason: resolvedRate.reason,
    voiceId: resolvedVoiceId,
    requestedVoiceId: String(resolvedVoiceId) !== String(voiceId) ? voiceId : null,
    baseUrl,
    story_id: story?.id || null,
  ...
  const stem = `${safeName(story?.id || "story")}_${safeName(engine)}_${String(resolvedRate.rate).replace(/[^0-9]+/g, "_")}`;
  ...
  const response = await fetchImpl(
    `${String(baseUrl).replace(/\/+$/, "")}/v1/text-to-speech/${encodeURIComponent(resolvedVoiceId)}/with-timestamps`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        text,
        output_format: resolveTtsOutputFormat("local", env),
        voice_settings: {
          stability: 0.2,
          similarity_boost: 0.8,
          style: 0.75,
          speaking_rate: resolvedRate.rate,
        },
      }),
    },
  );
  ...
  const transcript = transcriptFromAlignment(body.alignment || {});
  await fs.writeJson(
    timestampsPath,
    {
      ...(body.alignment || {}),
      meta: {
        ...
        generation: {
          ...
          engine,
          rate: resolvedRate.rate,
          requested_rate: resolvedRate.adjusted ? resolvedRate.requestedRate : null,
          rate_adjustment: resolvedRate.adjusted
            ? { reason: resolvedRate.reason }
            : null,
