...
function scriptForGovernedNarration(text, story = {}) {
  const base = String(text || "").trim();
  if (!story?.cta_policy) {
    throw new Error("pulse_cta_policy_missing");
  }
  const validation = validatePulseCta({
    script: {
      cta: story.cta || "",
      full_script: base,
    },
    decision: story.cta_policy,
  });
  if (validation.result !== "pass") {
    throw new Error(
      `governed_narration_cta_invalid:${validation.failures.join(",")}`,
    );
  }
  return base;
}
...
function localVoiceApproved(candidate = {}, env = process.env) {
  return bool(candidate.approvedLocalVoice) || bool(env.STUDIO_V2_LOCAL_VOICE_APPROVED);
}
...
function evaluateVoiceCandidate({
  ...
  let ctaValidation = null;
  if (!story?.cta_policy) {
    blockers.push("pulse_cta_policy_missing");
  } else if (transcript) {
    ctaValidation = validatePulseCta({
      script: {
        cta: story.cta || "",
    ...
    blockers.push(...ctaValidation.failures);
  } else {
    warnings.push("spoken_cta_unverified");
  }

  if (isLocalVoiceCandidate(candidate) && !localVoiceApproved(candidate, env)) {
    warnings.push("local_voice_requires_human_approval");
  }
  ...
  return {
    ...
    transcript: {
      ...
      cta_policy_verified: ctaValidation?.result === "pass",
      contextual_cta_present: ctaValidation?.cta_detected ?? null,
    },
    approval: {
      local_voice: isLocalVoiceCandidate(candidate),
...
function buildGenerationPlan({ dryRun = true, story, contract } = {}) {
  ...
  return {
    ...
    target_runtime_seconds: contract?.runtime_target_seconds || null,
    candidate_outputs_under: "test/output/flash-lane-voice-workbench",
    next_step:
      contract?.next_action === "generate_approved_flash_lane_voice"
        ? `generate_or_supply_${contract.duration_band_id || "selected_band"}_voice_candidates`
...
async function generateLocalVoiceCandidate({
  ...
  const text = scriptForGovernedNarration(
