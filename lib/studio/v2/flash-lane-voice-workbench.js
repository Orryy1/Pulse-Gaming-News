...
const DEFAULT_MIN_SPECTRAL_ROLLOFF_85_HZ = 1600;
const DEFAULT_MIN_HIGH_FREQUENCY_RATIO_GT_5KHZ = 0.0015;
...
function localTtsClarityBlocker(acoustic, env = process.env) {
  ...
  const rolloff85Hz = acousticValue(
    acoustic,
    "spectralRolloff85Hz",
    acoustic.rolloff85Hz,
    acoustic.rolloff_85_hz_median,
    acoustic.spectral_rolloff_85_hz,
  );
  const highFrequencyRatioGt5Khz = acousticValue(
    acoustic,
    "highFrequencyRatioGt5Khz",
    acoustic.highFrequencyRatio,
    acoustic.high_freq_ratio_gt_5khz_median,
    acoustic.high_frequency_ratio_gt_5khz,
  );

  const lowRolloff = rolloff85Hz !== null && rolloff85Hz < minRolloff;
  const lowHighFrequency =
    highFrequencyRatioGt5Khz !== null &&
    highFrequencyRatioGt5Khz < minHighRatio;
...
function evaluateAcousticProfile(acoustic, env = process.env) {
  ...
  return {
    ...
    metrics: {
      ...
      silenceRatio,
      clippingRatio,
      spectralRolloff85Hz: clarity?.rolloff85Hz ?? null,
      highFrequencyRatioGt5Khz: clarity?.highFrequencyRatioGt5Khz ?? null,
      thresholds: {
        minPitchHz: minPitch,
        ...
        maxLufs,
        minRolloff85Hz:
          clarity?.thresholds?.minRolloff85Hz ??
          DEFAULT_MIN_SPECTRAL_ROLLOFF_85_HZ,
        minHighFrequencyRatioGt5Khz:
          clarity?.thresholds?.minHighFrequencyRatioGt5Khz ??
          DEFAULT_MIN_HIGH_FREQUENCY_RATIO_GT_5KHZ,
      },
    },
  };
...
function resolveLocalGenerationRate(engine, rate) {
  ...
  const normalisedEngine = String(engine || "voxcpm2").toLowerCase();
  if (normalisedEngine.includes("vox") && Math.abs(safeRequestedRate - 1.0) > 1e-3) {
    return {
      rate: 1.0,
      requestedRate: safeRequestedRate,
      adjusted: true,
      reason: "voxcpm_phase_vocoder_rate_disabled",
    };
  }
  return {
    rate: safeRequestedRate,
    requestedRate: safeRequestedRate,
    adjusted: false,
    reason: null,
  };
