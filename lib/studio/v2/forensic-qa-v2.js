...
function analyseSubtitleDensity({
  ...
  if (failingDenseCues.length || failingLineCues.length) {
    verdict = "fail";
  ...
  } else if (denseCues.length || multiLineCues.length) {
    verdict = "warn";
    if (denseCues.length) {
...
async function analyseRenderedFrameTaste({
  ...
  const ratingOrTitleFrameCount = badFrames.filter(
    (frame) => frame.group === "title_or_rating_slate",
  ).length;
  const lowInformationFrameCount = badFrames.filter(
    (frame) => frame.group === "low_information_frame",
  ).length;
  const badFrameCount = badFrames.length;
  let verdict = "pass";
  const reasons = [];
  ...
  if (lowInformationFrameCount > 0 && verdict !== "fail") {
    verdict = "warn";
  }
...
async function runForensicQa(options = {}) {
  ...
  subtitles.density = analyseSubtitleDensity({ dialogues: subtitleDialogues });
