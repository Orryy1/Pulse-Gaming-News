...
const CAPTION_GAP_WARN_S = 2.75;
const CAPTION_TAIL_WARN_S = 3;
const MAX_PORTABLE_MEDIA_TOOL_PATH = 247;
...
function decodedVisualEvidencePaths(outputDir, safeStoryId) {
  const digest = createHash("sha256")
    .update(`${path.resolve(outputDir)}\0${safeStoryId}`)
    .digest("hex")
    .slice(0, 16);
  let reportPath = path.join(outputDir, `${safeStoryId}_decoded_visual_gate.json`);
  let frameDir = path.join(outputDir, `${safeStoryId}_frames`);
  if (path.join(frameDir, "frame_001.jpg").length >= MAX_PORTABLE_MEDIA_TOOL_PATH) {
    frameDir = path.join(outputDir, `frames_${digest}`);
  }
  if (reportPath.length >= MAX_PORTABLE_MEDIA_TOOL_PATH) {
    reportPath = path.join(outputDir, `decoded_visual_${digest}.json`);
  }
  if (path.join(frameDir, "frame_001.jpg").length >= MAX_PORTABLE_MEDIA_TOOL_PATH) {
    frameDir = path.join(os.tmpdir(), "pulse-decoded-visual", digest);
  }
  return { reportPath, frameDir };
}
...
function analyseSubtitleTimeline({
  ...
  warnCoverageRatio = 0.65,
  failCoverageRatio = 0.35,
  maxTailAfterLastCueS = CAPTION_TAIL_WARN_S,
} = {}) {
  ...
  const tailAfterLastCueS =
    chronologicalDialogues.length && safeNumber(durationS, 0) > lastCueEndS
      ? safeNumber(durationS, 0) - lastCueEndS
      : 0;
  ...
  const warningTimelineIssue =
    overrunS > 0.1 ||
    tailAfterLastCueS > maxTailAfterLastCueS ||
    gaps.length ||
  ...
  return {
    ...
    tailAfterLastCueS:
      tailAfterLastCueS > maxTailAfterLastCueS
        ? round(tailAfterLastCueS, 2)
        : 0,
    maxTailAfterLastCueS,
    gapsOver2s: gaps,
    nearZeroDurationS,
...
function analyseAudioRecurrence({
  ...
  scheduledTimesS = [],
  strictScheduledRecurrence = false,
  scheduledRepeatSimilarityFloor = 0.9,
  strictScheduledRepeatWarnCount = 7,
  strictScheduledRepeatFailCount = 9,
}) {
  ...
  const scheduledRepeatCount = worstScheduledCluster
    ...
    : 0;
  const scheduledRepeatSimilarity = safeNumber(
    worstScheduledCluster?.averageSimilarity,
    0,
  );
  const strictScheduledRepeatLooksLikeSfx =
    strictScheduledRecurrence &&
    declaredSfxCueCount >= 3 &&
    scheduledRepeatCount >= strictScheduledRepeatWarnCount &&
    scheduledRepeatSimilarity >= scheduledRepeatSimilarityFloor;
  const scheduledRepeatLooksLikeSfx =
    candidates.length > 0 ||
    declaredSfxCueCount > 4 ||
    strictScheduledRepeatLooksLikeSfx;
  const scheduledFailCount = strictScheduledRepeatLooksLikeSfx
    ? strictScheduledRepeatFailCount
    : 8;
  const scheduledWarnCount = strictScheduledRepeatLooksLikeSfx
    ? strictScheduledRepeatWarnCount
    : 4;
  let verdict = "pass";
  const reasons = [];

  if (declaredSfxCueCount > 7) {
    verdict = "fail";
    reasons.push(`declared SFX cue count is ${declaredSfxCueCount}`);
  } else if (declaredSfxCueCount > 4) {
    verdict = "warn";
    reasons.push(`declared SFX cue count is ${declaredSfxCueCount}`);
  }
  ...
  if (scheduledRepeatLooksLikeSfx && scheduledRepeatCount >= scheduledFailCount) {
    verdict = "fail";
    reasons.push(
    ...
    scheduledRepeatCount >= scheduledWarnCount &&
    verdict !== "fail"
  ) {
    verdict = "warn";
    reasons.push(
...
function probeFrameRate(mp4Path) {
  try {
    ...
    return parseFrameRate(stream.avg_frame_rate) || parseFrameRate(stream.r_frame_rate) || 30;
  } catch {
    return 30;
  }
}

function buildFrameExtractionFilter({ fps = 30, intervalS = 1.5 } = {}) {
  const safeFps = Math.max(1, safeNumber(fps, 30));
  const safeInterval = Math.max(0.25, safeNumber(intervalS, 1.5));
  const stepFrames = Math.max(1, Math.round(safeFps * safeInterval));
  return [
    `select='not(mod(n\\,${stepFrames}))'`,
    "scale=270:480:force_original_aspect_ratio=decrease",
    "pad=270:480:(ow-iw)/2:(oh-ih)/2:0x0D0D0F",
  ].join(",");
}
...
async function extractFrames({ mp4Path, frameDir, intervalS = 1.5 }) {
  ...
  const frameRate = probeFrameRate(mp4Path);
  execFileSync(
    "ffmpeg",
    [
      "-y",
      ...
      mp4Path,
      "-vf",
      buildFrameExtractionFilter({ fps: frameRate, intervalS }),
      "-fps_mode",
      "vfr",
      "-q:v",
      "3",
...
async function frameHash(framePath) {
  const width = 9;
  const height = 8;
  const [data, darknessData] = await Promise.all([
    sharp(framePath)
  ...
  const darknessMean =
    darknessData.reduce((total, value) => total + value, 0) /
    Math.max(1, darknessData.length);
  const darknessVariance =
    darknessData.reduce(
      (total, value) => total + (value - darknessMean) ** 2,
      0,
    ) / Math.max(1, darknessData.length);
  const activePixelCount = darknessData.reduce(
  ...
  return {
    ...
    luminanceStdDev: Math.sqrt(darknessVariance),
    activePixelRatio: activePixelCount / Math.max(1, darknessData.length),
    peakLuminance: darknessData.reduce(
...
function isDecodedBlackFrame(frame = {}) {
  if (Number(frame.luminance) >= 8) return false;
  return (
    Number(frame.luminanceStdDev) < 6 ||
    Number(frame.activePixelRatio) < 0.04 ||
    Number(frame.peakLuminance) < 32
  );
}
...
function buildValidatedReadableCardRanges(report = {}) {
  const allowedKinds = /^(?:source|quote|takeaway|context|timeline|proof|stat|chart|breaking|title)$/;
  const explicitWindows = [
    ...(Array.isArray(report?.card_visible_windows) ? report.card_visible_windows : []),
    ...(Array.isArray(report?.cardVisibleWindows) ? report.cardVisibleWindows : []),
  ];
  const ranges = [];

  function addRange({ startS, endS, durationS, minimumS, maximumS, kind }) {
    const normalisedKind = String(kind || "").trim().toLowerCase();
    const start = safeNumber(startS, null);
    const duration = safeNumber(durationS, null);
    const end = safeNumber(endS, duration != null && start != null ? start + duration : null);
    const observedDuration = duration != null ? duration : end != null && start != null ? end - start : null;
    const minimum = safeNumber(minimumS, normalisedKind === "source" ? 1.6 : MIN_READABLE_CARD_DURATION_S);
    const maximum = safeNumber(maximumS, normalisedKind === "source" ? 3.2 : 6.5);
    if (
      !allowedKinds.test(normalisedKind) ||
      start == null ||
      end == null ||
      observedDuration == null ||
      observedDuration + 0.01 < minimum ||
      observedDuration > maximum + 0.01 ||
      end <= start
    ) {
      return;
    }
  ...
  for (const window of explicitWindows) {
    addRange({
      startS: window?.start_s ?? window?.startS,
      endS: window?.end_s ?? window?.endS,
      durationS: window?.duration_s ?? window?.durationS ?? window?.duration,
      minimumS:
        window?.minimum_readable_duration_s ??
        window?.minimumReadableDurationS ??
        window?.minimum_visible_duration_s,
      maximumS:
        window?.maximum_visible_duration_s ??
        window?.maximumVisibleDurationS ??
        window?.max_readable_card_duration_s,
      kind: window?.kind ?? window?.readableCardKind ?? window?.readable_card_kind,
    });
  }
  if (ranges.length) return ranges;

  let cursor = 0;
  for (const scene of Array.isArray(report?.sceneList) ? report.sceneList : []) {
    const duration = safeNumber(scene?.duration ?? scene?.durationS, 0);
    const kind = String(
      scene?.readableCardKind ||
      scene?.readable_card_kind ||
      String(scene?.type || "").replace(/^card[._-]?/i, ""),
    ).trim().toLowerCase();
    if (kind) {
      addRange({
        startS: cursor,
        endS: cursor + duration,
        durationS: duration,
        minimumS:
          scene?.minimumReadableDurationS ??
          scene?.minimum_readable_duration_s ??
          scene?.minimum_visible_duration_s,
        maximumS:
          scene?.maximumVisibleDurationS ??
          scene?.maximum_visible_duration_s ??
          scene?.max_readable_card_duration_s,
        kind,
      });
    }
    cursor += duration;
...
function intentionalReadableCardRangeAt(timeS, ranges = []) {
  return (Array.isArray(ranges) ? ranges : [])
    .find((range) => timeInRange(timeS, range, 0.8)) || null;
}

function subtitleCueRangeAt(timeS, ranges = []) {
  return (Array.isArray(ranges) ? ranges : []).find((range) => (
    cleanCaptionText(range?.text || range?.rawText) &&
    timeInRange(timeS, range, 0.08)
  )) || null;
}

function isVerifiedSubtitleOverlayFalsePositive(taste = {}, prescan = {}, cue = null) {
  if (!cue || String(taste?.reason || "") !== "white_text_on_dark_card") return false;
  const score = safeNumber(taste?.score, 0);
  const tags = new Set(Array.isArray(taste?.tags) ? taste.tags : []);
  const saturation = safeNumber(prescan?.saturation_mean, 0);
  const edgeDensity = safeNumber(prescan?.edge_density, 0);
  const textOverlayLikelihood = safeNumber(prescan?.text_overlay_likelihood, 0);
  const whiteTextOnDarkLikelihood = safeNumber(
    prescan?.white_text_on_dark_likelihood,
    1,
  );
  const brightRatio = safeNumber(prescan?.bright_pixel_ratio, 0);
  const darkRatio = safeNumber(prescan?.dark_pixel_ratio, 1);
  const centralLuminance = safeNumber(prescan?.central_luminance_oval, 0);
  const baseSubtitleMotionSignal = (
    score >= 45 &&
    score <= 75 &&
    saturation >= 0.45 &&
    edgeDensity >= 0.1 &&
    textOverlayLikelihood <= 0.12 &&
    whiteTextOnDarkLikelihood >= 0.35 &&
    whiteTextOnDarkLikelihood <= 0.7 &&
    brightRatio <= 0.12 &&
    darkRatio >= 0.35 &&
    darkRatio <= 0.72 &&
    centralLuminance >= 0.28
  );
  const detailedOwnedMotionSignal = (
    score >= 45 &&
    score <= 85 &&
    saturation >= 0.55 &&
    edgeDensity >= 0.18 &&
    textOverlayLikelihood <= 0.18 &&
    whiteTextOnDarkLikelihood >= 0.35 &&
    whiteTextOnDarkLikelihood <= 0.7 &&
    brightRatio <= 0.12 &&
    darkRatio >= 0.35 &&
    ...
    centralLuminance >= 0.28 &&
    tags.has("colourful") &&
    tags.has("detail_rich")
  );
  return (
    (baseSubtitleMotionSignal || detailedOwnedMotionSignal) &&
    prescan?.black_frame !== true &&
    !["fail", "warn"].includes(String(prescan?.blur_verdict || "").toLowerCase())
  );
}

function isLikelySubtitleOverlayFalsePositive(taste = {}, prescan = {}) {
  const reason = String(taste?.reason || "");
  if (reason !== "white_text_on_dark_card") return false;
  const score = safeNumber(taste?.score, 0);
  const tags = new Set(Array.isArray(taste?.tags) ? taste.tags : []);
  const saturation = safeNumber(prescan?.saturation_mean, 0);
  const edgeDensity = safeNumber(prescan?.edge_density, 0);
  const textOverlayLikelihood = safeNumber(prescan?.text_overlay_likelihood, 0);
  const brightRatio = safeNumber(prescan?.bright_pixel_ratio, 0);
  const darkRatio = safeNumber(prescan?.dark_pixel_ratio, 0);
  const centralLuminance = safeNumber(prescan?.central_luminance_oval, 0);
  const centralBrightRatio = safeNumber(prescan?.central_bright_pixel_ratio, 0);
  const centralDarkRatio = safeNumber(prescan?.central_dark_pixel_ratio, 1);
  const centredSubjectSignal =
    centralLuminance >= 0.32 &&
    centralDarkRatio <= darkRatio - 0.06 &&
    centralBrightRatio >= brightRatio + 0.008;
  return (
    score >= 45 &&
    darkRatio >= 0.42 &&
    darkRatio <= 0.78 &&
    ...
    edgeDensity >= 0.12 &&
    tags.has("colourful") &&
    (tags.has("detail_rich") || centredSubjectSignal)
  );
}
...
async function analyseRenderedFrameTaste({
  ...
  for (const frame of frames || []) {
    ...
    const verifiedSubtitleCue = subtitleCueRangeAt(frame.timeS, subtitleCueRanges);
    if (isVerifiedSubtitleOverlayFalsePositive(taste, prescan, verifiedSubtitleCue)) {
      taste = {
        verdict: "pass",
        reason: "verified_subtitle_overlay_taste_ignored",
        score: taste?.score ?? null,
        tags: [
          ...new Set([
            ...(Array.isArray(taste?.tags) ? taste.tags : []),
            "subtitle_overlay",
            "verified_subtitle_cue",
          ]),
        ],
      };
    } else if (isLikelySubtitleOverlayFalsePositive(taste, prescan)) {
      taste = {
        verdict: "pass",
        reason: "subtitle_overlay_taste_ignored",
        score: taste?.score ?? null,
        tags: [...new Set([...(Array.isArray(taste?.tags) ? taste.tags : []), "subtitle_overlay"])],
      };
    }
    if (taste?.reason === "promotional_store_slate_frame") {
      // The scalar-only source heuristic also matches bright gameplay after
      // Pulse overlays are composited. Raw-source admission still rejects it;
      // explicit slates and low-information frames remain hard failures here.
      taste = {
        verdict: "pass",
        reason: "promotional_slate_heuristic_ignored_on_composite",
        score: taste?.score ?? null,
        tags: [
          ...new Set([
            ...(Array.isArray(taste?.tags) ? taste.tags : []),
    ...
    const intentionalReadableCardRange = intentionalReadableCardRangeAt(
      frame.timeS,
      intentionalReadableCardRanges,
    );
    if (
      taste?.verdict === "fail" &&
      RENDERED_FRAME_TITLE_OR_RATING_REASONS.has(taste?.reason) &&
      intentionalReadableCardRange
    ) {
      taste = {
        verdict: "pass",
        reason: "validated_readable_editorial_card_taste_routed",
        score: taste?.score ?? null,
        tags: [
          ...new Set([
            ...(Array.isArray(taste?.tags) ? taste.tags : []),
            "validated_readable_editorial_card",
            `card_kind_${intentionalReadableCardRange.kind}`,
          ]),
        ],
      };
    }
...
async function analyseVisualRepetition({ frames, minGapS = 3, maxHamming = 6, ignoreRangesS = [] }) {
  ...
  const blackFrames = hashed
    .filter(isDecodedBlackFrame)
    .map((frame) => ({
      frame: path.basename(frame.path),
      timeS: frame.timeS,
      luminance: round(frame.luminance, 2),
      luminanceStdDev: round(frame.luminanceStdDev, 2),
      activePixelRatio: round(frame.activePixelRatio, 4),
      peakLuminance: frame.peakLuminance,
    }));
  let verdict = "pass";
  ...
  return {
    ...
    uniqueHashCount: new Set(hashed.map((frame) => frame.hash)).size,
...
const MOTION_SCENE_TYPES = new Set([
  "clip",
  "punch",
  "speed-ramp",
  "freeze-frame",
]);
const MIN_READABLE_CARD_DURATION_S = 2.2;

function sceneType(scene) {
  return String(scene?.type || scene?.sceneType || "unknown");
}
...
function sceneSourceIdentity(scene) {
  return String(
    scene?.source ||
...
function isMotionScene(scene) {
  ...
  return (
    MOTION_SCENE_TYPES.has(type) ||
    (type === "opener" && scene?.isClipBacked === true)
  );
}

function isReadableCardScene(scene) {
  const type = sceneType(scene);
  return (
    /^card\./.test(type) ||
    type === "outro" ||
    sceneText(scene?.readableCardKind || scene?.readable_card_kind) !== "" ||
    (scene?.premiumLane === "hyperframes" && /card/i.test(String(scene?.label || type)))
  );
}

function sceneCardAssetReuseKey(scene) {
  if (!isReadableCardScene(scene)) return null;
  const explicitAsset = String(
    scene?.prerenderedMp4 ||
      scene?.assetPath ||
      scene?.path ||
      "",
  ...
  if (/^studio_v4_overlay_chain$/i.test(source)) return null;
  return source || null;
}

function sceneSourceReuseKey(scene) {
  const source = sceneSourceIdentity(scene);
  if (/^studio_v4_overlay_chain$/i.test(source) && sceneText(scene?.id)) {
    return `${source}:${sceneText(scene.id)}`;
  }
  const isClipBeat =
    isMotionScene(scene);
...
function sceneMotionWindowKey(scene) {
  if (!isMotionScene(scene)) return null;
  const source = sceneSourceIdentity(scene);
  const start = Number(
    scene?.mediaStartS ??
      scene?.media_start_s ??
  ...
  if (!Number.isFinite(start)) return `${source} @unknown-start`;
  return `${source} @${start.toFixed(1)}s`;
}
...
function sceneDurationS(scene) {
  ...
  const start = safeNumber(scene?.start_s ?? scene?.startS ?? scene?.start, null);
  const end = safeNumber(scene?.end_s ?? scene?.endS ?? scene?.end, null);
...
function normaliseProductionManifestScenes(report = {}) {
  const direct = Array.isArray(report?.sceneList)
    ? report.sceneList
  ...
  const scenes = [];
  const plan =
    report?.clip_scene_plan && typeof report.clip_scene_plan === "object"
      ? report.clip_scene_plan
      : report?.clipScenePlan && typeof report.clipScenePlan === "object"
        ? report.clipScenePlan
        : {};
  const clipPlan = Array.isArray(report?.clip_scene_plan)
    ? report.clip_scene_plan
    : Array.isArray(report?.clipScenePlan)
      ? report.clipScenePlan
      : Array.isArray(plan?.scenes)
        ? plan.scenes
        : [];
  const clips = clipPlan.length
    ? clipPlan
    : Array.isArray(report?.clips)
      ? report.clips
      : [];
  for (const clip of clips) {
    const readableCardKind = sceneText(clip?.readableCardKind || clip?.readable_card_kind);
    scenes.push({
      ...clip,
      type: clip?.type || clip?.sceneType || (readableCardKind ? `card.${readableCardKind}` : "clip"),
      source:
        clip?.source ||
        clip?.backgroundSource ||
        clip?.baseSourceKey ||
        ...
        clip?.assetPath ||
        clip?.sourceRootKey ||
        clip?.source_root_key ||
        "unknown",
      duration: sceneDurationS(clip),
      premiumLane: clip?.premiumLane || (readableCardKind ? "hyperframes" : clip?.premiumLane),
    });
  }

  // Visual V4 repeats planned card scenes in card_visible_windows as placement
  // evidence. Those windows describe the same render, not an additional reuse.
  const plannedCardAssets = new Set(
    scenes.map((scene) => sceneCardAssetReuseKey(scene)).filter(Boolean),
  );

  const cardWindows = [
    ...(Array.isArray(report?.overlay_card_windows) ? report.overlay_card_windows : []),
    ...(Array.isArray(report?.card_visible_windows) ? report.card_visible_windows : []),
  ];
  for (const card of cardWindows) {
    const normalisedCard = {
      ...card,
      type: card?.type || card?.sceneType || "card.overlay",
      source:
        card?.source ||
        card?.backgroundSource ||
        card?.prerenderedMp4 ||
        card?.assetPath ||
        card?.path ||
        "studio_v4_overlay_chain",
      duration: sceneDurationS(card),
      premiumLane: card?.premiumLane || "hyperframes",
    };
    const cardAsset = sceneCardAssetReuseKey(normalisedCard);
    if (cardAsset && plannedCardAssets.has(cardAsset)) continue;
    scenes.push(normalisedCard);
  }

  return scenes;
}

function repeatedEntriesFromCounts(counts) {
...
function sceneBreakdown(report) {
  const scenes = normaliseProductionManifestScenes(report);
  const typeCounts = {};
  const sourceCounts = {};
  const motionWindowCounts = {};
  const cardAssetCounts = {};
  const unreadableCardScenes = [];
  for (const scene of scenes) {
    const type = sceneType(scene);
    const source = sceneSourceReuseKey(scene);
    typeCounts[type] = (typeCounts[type] || 0) + 1;
    sourceCounts[source] = (sourceCounts[source] || 0) + 1;
    const motionWindow = sceneMotionWindowKey(scene);
    if (motionWindow) {
      motionWindowCounts[motionWindow] = (motionWindowCounts[motionWindow] || 0) + 1;
    }
    const cardAsset = sceneCardAssetReuseKey(scene);
    if (cardAsset) {
      cardAssetCounts[cardAsset] = (cardAssetCounts[cardAsset] || 0) + 1;
    }
    const duration = sceneDurationS(scene);
    if (isReadableCardScene(scene) && duration > 0 && duration < MIN_READABLE_CARD_DURATION_S) {
      unreadableCardScenes.push({
        type,
        label: scene?.label || null,
        source: sceneSourceIdentity(scene),
        durationS: round(duration, 3),
        minimumReadableDurationS: MIN_READABLE_CARD_DURATION_S,
        premiumLane: scene?.premiumLane || null,
      });
    }
  }
  const repeatedSources = repeatedEntriesFromCounts(sourceCounts);
  const repeatedMotionWindows = repeatedEntriesFromCounts(motionWindowCounts);
  const repeatedCardAssets = repeatedEntriesFromCounts(cardAssetCounts);
  return {
    sceneCount: scenes.length,
    typeCounts,
    uniqueSourceCount: Object.keys(sourceCounts).length,
    repeatedSources,
    repeatedMotionWindows,
    repeatedCardAssets,
    unreadableCardScenes,
    cardAssetReuse: {
      verdict: repeatedCardAssets.length ? "fail" : "pass",
      repeatedCardAssetCount: repeatedCardAssets.length,
    },
    cardReadability: {
      verdict: unreadableCardScenes.length ? "fail" : "pass",
      minReadableDurationS: MIN_READABLE_CARD_DURATION_S,
      unreadableCardCount: unreadableCardScenes.length,
    },
...
async function runDecodedVisualGate({
  storyId = "story",
  mp4Path,
  outputDir = path.join(TEST_OUT, "decoded-visual-gate"),
  renderReport = {},
  frameIntervalS = 1,
  subtitlePath = null,
  subtitleCueRanges = null,
} = {}) {
  const safeStoryId = String(storyId || "story").replace(/[^a-z0-9_-]+/gi, "_");
  const { reportPath, frameDir } = decodedVisualEvidencePaths(outputDir, safeStoryId);
  const blockers = [];
  let frames = [];
  let visualRepetition = null;
  let renderedFrameTaste = null;

  await fs.ensureDir(outputDir);
  try {
    if (!mp4Path || !(await fs.pathExists(mp4Path))) {
      throw new Error("render file does not exist");
    }
    frames = await extractFrames({
      mp4Path,
      frameDir,
      intervalS: Math.max(0.25, safeNumber(frameIntervalS, 1)),
    });
    if (frames.length < 3) {
      blockers.push("decoded_visual_frame_sample_insufficient");
    } else {
      const ignoreRangesS = buildVisualRepeatIgnoreRanges(renderReport);
      const intentionalReadableCardRanges = buildValidatedReadableCardRanges(renderReport);
      const verifiedSubtitleCueRanges = Array.isArray(subtitleCueRanges)
        ? subtitleCueRanges
        : parseSubtitleCues({ subtitlePath });
      visualRepetition = await analyseVisualRepetition({ frames, ignoreRangesS });
      renderedFrameTaste = await analyseRenderedFrameTaste({
        frames,
        intentionalReadableCardRanges,
        subtitleCueRanges: verifiedSubtitleCueRanges,
      });
      renderedFrameTaste.intentionalReadableCardRanges = intentionalReadableCardRanges;
      renderedFrameTaste.verifiedSubtitleCueCount = verifiedSubtitleCueRanges.length;

      if (visualRepetition.blackFrames.length > 0) {
        blockers.push("decoded_visual_black_frames_present");
      }
      const frozenSample =
        visualRepetition.frameCount >= 4 &&
        visualRepetition.uniqueHashCount <= Math.max(1, Math.floor(visualRepetition.frameCount * 0.15));
      if (frozenSample) blockers.push("decoded_visual_frozen_output");
      if (visualRepetition.verdict === "fail" || visualRepetition.repeatPairCount > 4) {
        blockers.push("decoded_visual_repetition_failed");
      }
      if (renderedFrameTaste.verdict === "fail") {
        blockers.push("decoded_visual_frame_taste_failed");
      }
    }
  } catch (error) {
    blockers.push("decoded_visual_media_unreadable");
    const report = {
      ...
      mp4_path: mp4Path ? relativeToRoot(mp4Path) : null,
      report_path: relativeToRoot(reportPath),
      frame_count: 0,
      visual_repetition: null,
      rendered_frame_taste: null,
  ...
  const report = {
    ...
    mp4_path: relativeToRoot(mp4Path),
    report_path: relativeToRoot(reportPath),
    frame_dir: relativeToRoot(frameDir),
    frame_count: frames.length,
    frame_interval_s: Math.max(0.25, safeNumber(frameIntervalS, 1)),
    sample_frames: frames.slice(0, 24).map((frame) => ({
      time_s: frame.timeS,
      path: relativeToRoot(frame.path),
    })),
    visual_repetition: visualRepetition,
    rendered_frame_taste: renderedFrameTaste,
    generated_at: new Date().toISOString(),
  };
  await fs.writeJson(reportPath, report, { spaces: 2 });
  return report;
...
function plannedSfxCueCount(report = {}) {
  const v4 = report?.visualV4 || {};
  const counts = [
    safeNumber(v4?.sfxPlan?.cue_count, null),
    arrayCount(v4?.sfxPlan?.cues),
    safeNumber(v4?.soundTransitionPlan?.sfx?.cue_count, null),
    arrayCount(v4?.soundTransitionPlan?.sfx?.cues),
  ].filter((count) => Number.isFinite(count) && count > 0);
  return counts.length ? Math.max(...counts) : 0;
}

function renderedSfxCueCount(report = {}) {
  const value = report?.auto?.sfxEventCount;
  if (typeof value === "number") return safeNumber(value, 0);
  return safeNumber(value?.value, 0);
}

function analyseRenderConsistency(report = {}) {
  const issues = [];
  const visualV4 = report?.visualV4 || null;
  const visualV4Verdict = String(visualV4?.verdict || "").trim();
  if (/blocked|fail/i.test(visualV4Verdict)) {
    issues.push({
      severity: "fail",
      code: "visual_v4_blocked_render",
      message:
        "Visual V4 marked the render as blocked, so a generated MP4 must not pass forensic QA as production-ready.",
      evidence: {
        verdict: visualV4Verdict,
        blockers: visualV4?.blockers || [],
        warnings: visualV4?.warnings || [],
      },
    });
  }

  const planned = plannedSfxCueCount(report);
  const rendered = renderedSfxCueCount(report);
  const missing = Math.max(0, planned - rendered);
  if (planned >= 4 && missing >= 2) {
    const renderedRatio = planned > 0 ? rendered / planned : 1;
    issues.push({
      severity: renderedRatio < 0.7 || missing >= 4 ? "fail" : "warn",
      code: "sfx_plan_render_mismatch",
      message:
        "The V4 director planned more SFX cues than the renderer declared, so impact beats may be missing.",
      evidence: {
        plannedSfxCueCount: planned,
        renderedSfxCueCount: rendered,
        missingSfxCueCount: missing,
      },
    });
  }

  return {
    verdict: issues.some((issue) => issue.severity === "fail")
      ? "fail"
      : issues.length
        ? "warn"
        : "pass",
    plannedSfxCueCount: planned,
    renderedSfxCueCount: rendered,
    visualV4Verdict: visualV4Verdict || null,
    issues,
  };
}

function analyseReportTextHygiene(report = {}, options = {}) {
  const maxSamples = Math.max(1, safeNumber(options.maxSamples, 12));
  const maxDepth = Math.max(1, safeNumber(options.maxDepth, 8));
  const samples = [];
  const seen = new WeakSet();

  function visit(value, pointer, depth) {
    if (samples.length >= maxSamples || depth > maxDepth) return;
    if (typeof value === "string") {
      const hygiene = classifyTextHygiene(value);
      if (hygiene.severity !== "clean") {
        samples.push({
          path: pointer || "$",
          severity: hygiene.severity,
          issues: hygiene.issues,
          text: value.slice(0, 220),
          normalised: String(hygiene.normalised || "").slice(0, 220),
        });
      }
      return;
    }
    ...
    for (const [key, item] of Object.entries(value)) {
      ...
      visit(item, nextPointer, depth + 1);
  ...
  visit(report, "", 0);
  return {
    verdict: samples.some((sample) => sample.severity === "fail")
      ? "fail"
      : samples.length
        ? "warn"
        : "pass",
...
function buildIssues({
  ...
  if ((scene.repeatedMotionWindows || []).length > 0) {
    issues.push({
      severity: "fail",
      code: "scene_motion_window_reuse",
      message:
        "Scene report shows exact repeated motion windows; repeated clips must be replaced with distinct footage beats.",
      evidence: (scene.repeatedMotionWindows || []).slice(0, 8),
    });
  }
  if ((scene.repeatedCardAssets || []).length > 0) {
    issues.push({
      severity: "fail",
      code: "scene_card_asset_reuse",
      message:
        "Scene report shows repeated source, quote, timeline or HyperFrames card assets; each designed card beat must be unique and story-specific.",
      evidence: (scene.repeatedCardAssets || []).slice(0, 8),
    });
  }
  if ((scene.unreadableCardScenes || []).length > 0) {
    issues.push({
      severity: "fail",
      code: "unreadable_card_duration",
      message:
        "One or more source, quote, timeline or HyperFrames card scenes are too short to read on mobile.",
      evidence: (scene.unreadableCardScenes || []).slice(0, 8),
    });
  }
  ...
  const consistency =
    renderConsistency ||
    (renderReport ? analyseRenderConsistency(renderReport) : { verdict: "pass", issues: [] });
  for (const issue of consistency.issues || []) {
    issues.push(issue);
  }
  const textHygiene =
    reportTextHygiene ||
    (renderReport ? analyseReportTextHygiene(renderReport) : { verdict: "pass" });
  if (textHygiene.verdict && textHygiene.verdict !== "pass") {
    issues.push({
      severity: textHygiene.verdict === "fail" ? "fail" : "warn",
      code: "report_text_hygiene",
      message:
        "Render report contains mojibake, raw entities or control characters in nested text fields.",
      evidence: {
        sampleCount: textHygiene.sampleCount,
        samples: (textHygiene.samples || []).slice(0, 8),
      },
    });
  }
  return issues;
...
function renderReportDurationS(report = {}) {
  const candidates = [
    report?.runtime?.durationS,
    report?.runtime?.mp4DurationS,
    report?.rendered_duration_s,
    report?.renderedDurationS,
    report?.duration_s,
    report?.durationS,
    report?.audio_duration_s,
  ];
  for (const candidate of candidates) {
    const duration = safeNumber(candidate, 0);
...
async function runForensicQa(options = {}) {
  ...
  const reportDurationS = renderReportDurationS(renderReport);
  const runtime = {
  ...
  const frameIntervalS = Math.max(0.25, safeNumber(options.frameIntervalS, 1.5));
  const frames = await extractFrames({ mp4Path, frameDir, intervalS: frameIntervalS });
  ...
  visual.taste = await analyseRenderedFrameTaste({
    frames,
    intentionalReadableCardRanges,
    subtitleCueRanges: subtitleDialogues,
  });
  visual.taste.intentionalReadableCardRanges = intentionalReadableCardRanges;
  visual.frameDir = relativeToRoot(frameDir);
  visual.frameIntervalS = frameIntervalS;
  ...
  const renderConsistency = analyseRenderConsistency(renderReport);
  const reportTextHygiene = analyseReportTextHygiene(renderReport);
