...
function resolveArtifactPath(artifactPath) {
  return path.isAbsolute(String(artifactPath || ""))
    ? artifactPath
    : path.resolve(ROOT, String(artifactPath || ""));
}

function buildGauntletHtml(report, outputDir = TEST_OUT) {
  const rows = report.candidates
    .map((s) => {
      const forensicLink = s.paths.forensicHtml
        ? `<a href="${escapeHtml(toPosix(path.relative(outputDir, resolveArtifactPath(s.paths.forensicHtml))))}">forensic</a>`
        : "";
      const mp4Rel = toPosix(
        path.relative(outputDir, resolveArtifactPath(s.paths.mp4)),
      );
      return `<tr>
  ...
  const bestVideo = best
    ? `<video controls preload="metadata" src="${escapeHtml(toPosix(path.relative(outputDir, resolveArtifactPath(best.paths.mp4))))}"></video>`
    : "";
  return `<!doctype html>
<html lang="en">
<head>
...
async function runGauntlet(options = {}) {
  ...
  await fs.writeFile(htmlPath, buildGauntletHtml(report, outputDir));
  report.outputs = {
    json: rel(jsonPath),
    markdown: rel(mdPath),
    html: rel(htmlPath),
