...
function summariseCandidate({ candidate, studioReport, forensicReport, loudness }) {
  ...
  return {
    ...
    studio: {
      ...
      hyperframesCardCount:
        ...
        (premiumLane.decisions || []).filter((d) => d.renderer === "hyperframes")
          .length,
      premiumShellVerdict:
        premiumLane.hyperframesPremiumShellGate?.verdict ||
        premiumLane.premiumShellVerdict ||
        "unknown",
      premiumShellPassCount:
        premiumLane.hyperframesPremiumShellGate?.passCount ??
    ...
    forensic: {
      ...
      audioPresence: forensicReport?.audio?.presence?.verdict || "unknown",
      subtitleVerdict: forensicReport?.subtitles?.verdict || "unknown",
      subtitleDensity: forensicReport?.subtitles?.density?.verdict || "unknown",
...
function rankCandidate(summary) {
  ...
  if (
    (summary.studio.hyperframesCardCount || 0) >= 4 &&
    summary.studio.premiumShellVerdict === "pass"
  ) {
    score += 2;
  }
