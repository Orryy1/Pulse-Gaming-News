...
async function buildStorySourceCard({
  ...
  const projectDir = path.join(TEMPLATE_DIR, `hf-source-${storyId}`);
  await copyProjectScaffold(path.join(TEMPLATE_DIR, "hf-source"), projectDir);
  await writeMeta(projectDir, `hf-source-${storyId}`);
  ...
  const outPath = path.join(TEST_OUT, `hf_source_card_${storyId}.mp4`);
...
async function buildStoryContextCard({
  ...
  const projectDir = path.join(TEMPLATE_DIR, `hf-context-${storyId}`);
  await copyProjectScaffold(path.join(TEMPLATE_DIR, "hf-context"), projectDir);
  await writeMeta(projectDir, `hf-context-${storyId}`);
  ...
  const outPath = path.join(TEST_OUT, `hf_context_card_${storyId}.mp4`);
  lintAndRender(projectDir, outPath);
  return { outPath, projectDir, fontSize };
}
...
async function buildStoryTakeawayCard({
  ...
  const projectDir = path.join(TEMPLATE_DIR, `hf-takeaway-${storyId}`);
  await copyProjectScaffold(path.join(TEMPLATE_DIR, "hf-takeaway"), projectDir);
  await writeMeta(projectDir, `hf-takeaway-${storyId}`);
  ...
  const outPath = path.join(TEST_OUT, `hf_takeaway_card_${storyId}.mp4`);
  lintAndRender(projectDir, outPath);
  return { outPath, projectDir, fontSize };
}
 ...
 */
async function buildAllStoryCards({ story, pkg, options = {} }) {
