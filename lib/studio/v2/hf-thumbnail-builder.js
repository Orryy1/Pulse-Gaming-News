...
const { selectThumbnailSubjectImage } = require("../../thumbnail-safety");
const { buildThumbnailCandidatePng } = require("../../thumbnail-candidate");
const { applyProduceSelection } = require("../../produce-selection");
...
function pickSubjectImage(story) {
  ...
  for (const img of Array.isArray(story?.downloaded_images)
    ...
    : []) {
    const resolved = img?.path ? mediaPaths.resolveExistingSync(img.path) : null;
    if (resolved && fs.existsSync(resolved)) {
      candidates.push({ ...img, path: resolved, storedPath: img.path });
    }
  }

  const cacheCandidates = [
    {
      path: path.join(IMAGE_CACHE, `${story.id}_trailerframe_2_smartcrop_v2.jpg`),
      type: "trailer_frame",
      source: "trailer",
      priority: 94,
    },
    {
      path: path.join(IMAGE_CACHE, `${story.id}_trailerframe_3_smartcrop_v2.jpg`),
      type: "trailer_frame",
      source: "trailer",
      priority: 93,
    },
    {
      path: path.join(IMAGE_CACHE, `${story.id}_trailerframe_1_smartcrop_v2.jpg`),
      type: "trailer_frame",
      source: "trailer",
      priority: 92,
    },
    {
      path: path.join(IMAGE_CACHE, `${story.id}_trailerframe_2.jpg`),
      type: "trailer_frame",
      source: "trailer",
      priority: 88,
    },
    {
      path: path.join(IMAGE_CACHE, `${story.id}_trailerframe_1.jpg`),
      type: "trailer_frame",
      source: "trailer",
      priority: 87,
    },
    {
      path: path.join(IMAGE_CACHE, `${story.id}_article.jpg`),
      type: "article_hero",
      source: "article",
      priority: 62,
    },
  ...
  for (const c of cacheCandidates) {
    if (c.path && fs.existsSync(c.path)) candidates.push(c);
  }

  const selected = selectThumbnailSubjectImage(story, candidates);
  return selected?.image?.path || null;
}
...
/**
 * Batch-build YouTube thumbnails for every approved+exported story
 * that doesn't yet have one.
 *
 * Stamps `story.hf_thumbnail_path` with a repo-relative path under
 * `output/thumbnails/` (resolved through media-paths so MEDIA_ROOT
 * is honoured on Railway). The intermediate MP4 is deleted; only
 * the JPEG is kept.
 *
 * Best-effort: per-story failures are logged and skipped. Never
 * throws so the produce pipeline can't be blocked by a thumbnail
 * issue.
 */
async function buildThumbnailsForApprovedStories({
  log = console.log,
  storyIds = null,
} = {}) {
  const db = require(path.join(ROOT, "lib", "db"));
  const mediaPaths = require(path.join(ROOT, "lib", "media-paths"));

  const stories = await db.getStories();
  if (!Array.isArray(stories) || stories.length === 0) {
    log("[hf-thumb] no stories in canonical store");
    return { built: 0, skipped: 0, failed: 0 };
  }

  const OUT_DIR_REL = path.join("output", "thumbnails");
  const outDirAbs = mediaPaths.writePath(OUT_DIR_REL);
  await fs.ensureDir(outDirAbs);

  const channelId = process.env.CHANNEL || "pulse-gaming";

  const scopedStories = Array.isArray(storyIds)
    ? require("../../services/exact-story-production-scope").filterStoriesToExactScope(
        stories,
        { storyIds },
      )
    : stories;
  const targets = [];
  for (const s of applyProduceSelection(scopedStories, {
    stage: "hf-thumb",
    log,
  })) {
    if (!s.approved || !s.exported_path) continue;
    if (s.hf_thumbnail_path) {
      // If the recorded path still exists on disk, skip. Otherwise
      // self-heal by clearing it so we rebuild this cycle.
      const existing = await mediaPaths.resolveExisting(s.hf_thumbnail_path);
      if (existing && (await fs.pathExists(existing))) continue;
      s.hf_thumbnail_path = null;
    }
    targets.push(s);
  }

  if (targets.length === 0) {
    log("[hf-thumb] no stories needing thumbnails");
    return { built: 0, skipped: stories.length, failed: 0 };
  }

  log(`[hf-thumb] building ${targets.length} thumbnails`);

  let built = 0;
  let failed = 0;
  for (const story of targets) {
    const mp4Rel = path.join(OUT_DIR_REL, `${story.id}.mp4`);
    const jpgRel = path.join(OUT_DIR_REL, `${story.id}.jpg`);
    const mp4Abs = mediaPaths.writePath(mp4Rel);
    try {
      const result = await buildStoryThumbnail({
        story,
        channelId,
        outPath: mp4Abs,
      });
      try {
        const candidateRel = path.join(
          OUT_DIR_REL,
          `${story.id}_thumbnail_candidate.png`,
        );
        const candidate = await buildThumbnailCandidatePng({
          story,
          outPath: candidateRel,
          subjectImagePath: result.subjectImagePath,
        });
        story.thumbnail_candidate_path = candidate.relativePath;
        story.thumbnail_safety_status = candidate.qa.result;
        story.thumbnail_safety_warnings = candidate.qa.warnings;
      } catch (candidateErr) {
        log(
          `[hf-thumb] ${story.id} thumbnail_candidate FAILED: ${candidateErr.message}`,
        );
      }
      story.hf_thumbnail_path = jpgRel;
      // The intermediate MP4 is just a render carrier — delete to
      // keep the persistent volume tidy. The JPEG is what YouTube
      // wants for thumbnails.set.
      try {
        if (result.mp4Path && (await fs.pathExists(result.mp4Path))) {
          await fs.remove(result.mp4Path);
        }
      } catch {
        /* non-fatal — the MP4 cleanup is best effort */
      }
      built++;
      log(`[hf-thumb] ${story.id} → ${jpgRel}`);
    } catch (err) {
      failed++;
      log(`[hf-thumb] ${story.id} FAILED: ${err.message}`);
    }
  }

  await db.saveStories(stories);
  log(`[hf-thumb] built=${built} failed=${failed} target=${targets.length}`);
  return { built, failed, skipped: stories.length - targets.length };
}
