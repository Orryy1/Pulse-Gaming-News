...
function uniqueOfficialSourceRecords(frameReport, storyId) {
  ...
  for (const plan of Array.isArray(frameReport?.plans) ? frameReport.plans : []) {
    ...
    for (const frame of Array.isArray(plan.frames) ? plan.frames : []) {
      ...
      seen.add(key);
      records.push({ sourceUrl, sourceType, entity, ...urlKind, referenceReportSource: false });
    }
  }
  return records;
}
...
function isOfficialReference(record) {
  ...
  return (
    ...
    /(steam_movie|steam_storefront_video_reference|igdb_video|official_trailer|publisher_video|platform_video)/.test(sourceType) &&
