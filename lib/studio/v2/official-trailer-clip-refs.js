...
const OFFICIAL_REFERENCE_SOURCE_TYPE_RE =
  /(steam_movie|steam_storefront_video_reference|igdb_video|official_trailer|publisher_video|platform_video|licensed_direct_media_url|official_publisher_or_developer_trailer_page|official_game_website_media_page|platform_storefront_video_reference|official_platform_product_page)/;
...
function exploratoryStartsForSource(baseStarts, record = {}) {
  const duration = sourceDurationSeconds(record);
  if (!Number.isFinite(duration) || duration > MIN_OFFICIAL_CLIP_START_S + SAFE_FRAME_LEAD_OUT_S) {
    return uniqueNumberList(baseStarts.filter((seconds) => seconds >= MIN_OFFICIAL_CLIP_START_S));
  }
  const minimum = Math.max(4, minimumClipStartForSource(record));
  const maximum = maximumClipStartForSource(record);
...
function buildExploratoryClipRefs(frameReport, storyId, opts = {}) {
  ...
  const pairs = records
    .flatMap((record, recordIndex) =>
      exploratoryStartsForSource(starts, record).map((start) => ({
        record,
        recordIndex,
        start,
      })),
