...
function isOfficialReference(record) {
  ...
  return (
    ...
    !isRatingBoardReference(record)
  );
}

function isRatingBoardReference(record) {
  const text = [
    record?.movie_name,
    record?.movieName,
    record?.name,
    ...
    .filter(Boolean)
    .join(" ");
  return RATING_REFERENCE_RE.test(text);
}
...
function buildOfficialTrailerClipsFromAcquisitionPlan(acquisitionPlan, referenceReport, storyId = null) {
  ...
  for (const plan of acquisitionPlansForStory(acquisitionPlan, storyId)) {
    ...
    for (const item of asArray(plan.shopping_list)) {
      ...
      const matchingRecords = records.filter((record) => normaliseEntityLabel(record.entity) === entityKey);
      for (const record of matchingRecords) {
        for (const window of asArray(item.suggested_windows)) {
          ...
          refs.push({
            ...
            mediaStartS: Number(Math.max(MIN_OFFICIAL_CLIP_START_S, start).toFixed(2)),
            provenance: {
              target_time_seconds: null,
              frame_local_path: null,
              content_hash: null,
  ...
  return dedupeClipRefs(
    asArray(segmentValidationReport?.segments)
      ...
      .map((segment) => {
        ...
        const mediaStartS = hasTrimTiming ? recommendedStart : start;
        const durationS = hasTrimTiming ? recommendedDuration : duration;
        return {
          ...
          durationS: Number.isFinite(durationS) && durationS > 0 ? Number(durationS.toFixed(2)) : 5,
          mediaStartS: Number(mediaStartS.toFixed(2)),
          provenance: {
            target_time_seconds: null,
            frame_local_path: null,
            content_hash: null,
