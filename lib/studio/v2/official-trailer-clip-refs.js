...
function boundClipDurationForSource(start, requestedDuration, record = {}) {
  const duration = Number(requestedDuration);
  if (!Number.isFinite(duration) || duration <= 0) return 0;
  const sourceDuration = sourceDurationSeconds(record);
  if (!Number.isFinite(sourceDuration)) return Number(duration.toFixed(2));
  const availableTail = Math.max(0, sourceDuration - Number(start || 0) - SOURCE_TAIL_GUARD_S);
  return Number(Math.min(duration, availableTail).toFixed(2));
}
...
function canonicalSteamMediaAssetIdentity(value) {
  ...
  const trailer = pathname.match(/\/store_trailers\/(\d+)\/(\d+)(?:\/|$)/i);
  if (trailer) return `steam_store_trailer:${trailer[1]}:${trailer[2]}`;
  const extra = pathname.match(/\/store_item_assets\/steam\/apps\/(\d+)\/extras\/([^/]+)$/i);
  if (!extra) return null;
  const asset = extra[2].replace(/\.(?:mp4|webm|mov|m4v)$/i, "").toLowerCase();
  return asset ? `steam_store_extra:${extra[1]}:${asset}` : null;
}

function clipRefVariantPriority(ref = {}) {
  const source = String(ref?.path || "").trim().toLowerCase();
  const sourceType = String(ref?.sourceType || ref?.source_type || "").toLowerCase();
  let score = 0;
  if (/hls_264_master\.m3u8(?:[?#]|$)/.test(source)) score += 600;
  else if (/\.mp4(?:[?#]|$)/.test(source)) score += 500;
  else if (/\.webm(?:[?#]|$)/.test(source)) score += 450;
  else if (/dash_h264\.mpd(?:[?#]|$)/.test(source)) score += 400;
  else if (/\.m3u8(?:[?#]|$)/.test(source)) score += 350;
  else if (/\.mpd(?:[?#]|$)/.test(source)) score += 300;
  if (sourceType.includes("steam_movie")) score += 50;
  if (ref?.movieName || ref?.movie_name || ref?.provenance?.movie_name) score += 10;
  return score;
}

function clipRefDedupeKey(ref) {
  const source = String(ref?.path || "").trim();
  const assetIdentity = canonicalSteamMediaAssetIdentity(source);
  return [
...
function dedupeClipRefs(refs) {
  ...
  for (const ref of refs) {
    ...
    if (clipRefVariantPriority(ref) > clipRefVariantPriority(out[existingIndex])) {
      out[existingIndex] = ref;
    }
  }
  return out;
}
  ...
  for (const plan of Array.isArray(referenceReport?.plans) ? referenceReport.plans : []) {
    ...
    for (const reference of Array.isArray(plan.references) ? plan.references : []) {
      ...
      records.push({
        ...
        localMaterializationAllowed:
          reference.local_materialization_allowed ??
          reference.provenance?.local_materialization_allowed,
        livePublishAllowed:
          reference.live_publish_allowed ?? reference.provenance?.live_publish_allowed,
        requiresHumanLegalReviewBeforePublish:
          reference.requires_human_legal_review_before_publish ??
          reference.provenance?.requires_human_legal_review_before_publish,
        sourceAudioAllowed:
          reference.source_audio_allowed ?? reference.provenance?.source_audio_allowed,
        rightsGrant: reference.rights_grant ?? reference.provenance?.rights_grant,
        rightsVerdict: reference.rights_verdict || reference.provenance?.rights_verdict || null,
        ...
        referenceTitle:
          ...
          reference.provenance?.reference_title ||
          reference.provenance?.source_title ||
          null,
        sourceOwner: reference.source_owner || reference.provenance?.source_owner || null,
        youtubeVideoId: reference.youtube_video_id || reference.provenance?.youtube_video_id || null,
        officialChannel:
          reference.official_channel ||
          reference.official_channel_url ||
          reference.provenance?.official_channel ||
          reference.provenance?.official_channel_url ||
          null,
        officialChannelId:
          reference.official_channel_id ||
          reference.provenance?.official_channel_id ||
          null,
        sourceIdentityPath:
          reference.source_identity_path ||
          reference.provenance?.source_identity_path ||
          null,
        sourceIdentitySha256:
          reference.source_identity_sha256 ||
          reference.provenance?.source_identity_sha256 ||
          null,
...
function rightsProvenanceFields(record = {}) {
  return {
    ...
    local_materialization_allowed: record.localMaterializationAllowed,
    live_publish_allowed: record.livePublishAllowed,
    requires_human_legal_review_before_publish:
      record.requiresHumanLegalReviewBeforePublish,
    source_audio_allowed: record.sourceAudioAllowed,
    rights_grant: record.rightsGrant,
    rights_verdict: record.rightsVerdict || null,
...
function referenceRecordProvenanceFields(record = {}) {
  if (!record || typeof record !== "object" || !record.sourceUrl) return {};
  return {
    ...
    official_channel: record.officialChannel || null,
    official_channel_id: record.officialChannelId || null,
    source_identity_path: record.sourceIdentityPath || null,
    source_identity_sha256: record.sourceIdentitySha256 || null,
...
function buildExploratoryClipRefs(frameReport, storyId, opts = {}) {
  ...
  for (const { record, start } of pairs) {
      ...
      const renderDurationS = boundClipDurationForSource(start, exploratoryDurationS, record);
      if (renderDurationS < 1) continue;
      refs.push({
        ...
        provenance: {
          ...
          exploratory_duration_s: renderDurationS,
          requested_exploratory_duration_s: exploratoryDurationS,
...
function buildOfficialTrailerClipsFromFrameReport(frameReport, storyId, opts = {}) {
  ...
  const officialReferenceRecords = uniqueOfficialReferenceRecords(opts.referenceReport, storyId);
  ...
  const allRefs = candidateFrames
    ...
    .map(({ frame, startCandidate }) => {
      const targetSeconds = Number(frame.target_time_seconds);
      const exactSourceKey = clipSourceKey({
        path: String(frame.source_url || "").trim(),
        entity: frame.entity || null,
      });
      const matchingReference = officialReferenceRecords.find(
        (record) =>
          clipSourceKey({ path: record.sourceUrl, entity: record.entity }) === exactSourceKey,
      );
      return {
        path: String(frame.source_url || "").trim(),
        source: "official-trailer-reference",
        ...
        provenance: {
          ...
          ...referenceRecordProvenanceFields(matchingReference),
        },
      };
    });
