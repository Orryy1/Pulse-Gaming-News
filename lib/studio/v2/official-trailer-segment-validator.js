...
const QA_GUARDED_EARLY_SEGMENT_START_S = 6;
...
const VALIDATOR_RULESET_VERSION = 10;
const FLASH_LANE_ALLOWED_MOTION_CLASSES = new Set([
  "gameplay_action",
...
function isOfficialPlatformProductPageReference(record = {}) {
  const provenance = record.provenance || {};
  return /official_platform_product_page/i.test(
    String(record.sourceType || record.source_type || provenance.sourceType || provenance.source_type || ""),
  );
}
...
function minimumSegmentStartForSource(record = {}) {
  ...
  if (
    isOfficialPlatformProductPageReference(record) &&
    Number.isFinite(duration) &&
    duration <= MIN_OFFICIAL_SEGMENT_START_S
  ) {
    return 0;
  }
  if (
    record.allowEarlyExploratoryWindow === true ||
    record.allow_early_exploratory_window === true ||
    record.provenance?.allowEarlyExploratoryWindow === true ||
    record.provenance?.allow_early_exploratory_window === true
  ) {
    return QA_GUARDED_EARLY_SEGMENT_START_S;
  }
  ...
  const maximumSampleOffset = Math.max(...DEFAULT_SAMPLE_OFFSETS_S);
  const maximumStartForSampling = Number(Math.max(0, duration - maximumSampleOffset - 0.1).toFixed(2));
  const sourceRelativeStart = Number(Math.max(4, duration * 0.28).toFixed(2));
  if (maximumStartForSampling < sourceRelativeStart) {
    return Number(Math.max(0, Math.min(maximumStartForSampling, duration * 0.28)).toFixed(2));
  }
...
function canonicalClipSourceUrlForWindow(clip = {}) {
  return clipSourceUrl(clip).replace(/[?#].*$/, "").toLowerCase();
}

function clipSourceWindowIdentity(clip = {}) {
  return [
    normaliseFamilyValue(clip?.story_id || clip?.storyId || clip?.provenance?.story_id),
    normaliseFamilyValue(clip?.entity || clip?.provenance?.entity),
    canonicalClipSourceUrlForWindow(clip),
  ].join("|");
}

function clipSourceWindowRange(clip = {}) {
  const start = numberOr(clip?.mediaStartS ?? clip?.media_start_s, 0);
  const duration = Math.max(1, numberOr(clip?.durationS ?? clip?.duration_s, 5));
  return {
    start,
    end: start + duration,
  };
...
function sourceWindowsOverlap(a = {}, b = {}) {
  return Math.min(a.end, b.end) - Math.max(a.start, b.start) > 0.25;
}

function overlapsAcceptedSourceWindow(clip = {}, acceptedWindows = []) {
  const identity = clipSourceWindowIdentity(clip);
  if (!identity.trim()) return false;
  const range = clipSourceWindowRange(clip);
  return acceptedWindows.some((window) => window.identity === identity && sourceWindowsOverlap(range, window.range));
}
...
function isOfficialTrailerClip(clip) {
  return (
    ...
    /(steam_movie|steam_storefront_video_reference|igdb_video|official_trailer|publisher_video|platform_video|official_platform_product_page)/i.test(
...
function officialTrailerSegmentPreflightRejectReason(clip) {
  ...
  if (
    localPublisherPressKitMasterCandidate(clip) &&
    !verifiedLocalPublisherPressKitMaster(clip)
  ) {
    return "publisher_press_kit_requires_green_commercial_editorial_approval";
  }
...
function clipSourceProvenance(clip) {
  const provenance = clip?.provenance && typeof clip.provenance === "object"
    ? clip.provenance
    : {};
  const referenceUrl =
    clip?.reference_url ||
    ...
    null;
  const sourceMasterSha256 =
    clip?.source_master_sha256 ||
    clip?.sourceMasterSha256 ||
    clip?.source_sha256 ||
    clip?.sourceSha256 ||
    provenance.source_master_sha256 ||
    provenance.sourceMasterSha256 ||
  ...
  const youtubeVideoId =
    ...
    clip?.youtubeVideoId ||
    provenance.youtube_video_id ||
    provenance.youtubeVideoId ||
    null;
  const allowedPlatforms =
    clip?.allowedPlatforms ??
    clip?.allowed_platforms ??
    provenance.allowedPlatforms ??
    provenance.allowed_platforms;
  const restrictedPlatforms =
    clip?.restrictedPlatforms ??
    clip?.restricted_platforms ??
    provenance.restrictedPlatforms ??
    provenance.restricted_platforms;
  const platformRestrictions =
    clip?.platformRestrictions ??
    clip?.platform_restrictions ??
    provenance.platformRestrictions ??
    provenance.platform_restrictions;
  return {
    provider: clip?.provider || provenance.provider || null,
    source_family: clip?.sourceFamily || clip?.source_family || provenance.source_family || null,
    reference_title:
      clip?.reference_title ||
      clip?.movie_name ||
      clip?.movieName ||
      ...
      provenance.title ||
      null,
    movie_id: clip?.movie_id || clip?.movieId || provenance.movie_id || provenance.movieId || null,
    store_app_id: clip?.store_app_id || clip?.storeAppId || provenance.store_app_id || null,
    store_app_title: clip?.store_app_title || clip?.storeAppTitle || provenance.store_app_title || null,
    source_owner:
      clip?.source_owner || clip?.sourceOwner || provenance.source_owner || provenance.sourceOwner || null,
    licence_basis:
      clip?.licence_basis ||
      clip?.license_basis ||
      clip?.licenceBasis ||
      clip?.licenseBasis ||
      provenance.licence_basis ||
      provenance.license_basis ||
      provenance.licenceBasis ||
      provenance.licenseBasis ||
      null,
    allowed_use:
      clip?.allowed_use || clip?.allowedUse || provenance.allowed_use || provenance.allowedUse || null,
    allowed_render_use:
      clip?.allowed_render_use ||
      clip?.allowedRenderUse ||
      provenance.allowed_render_use ||
      provenance.allowedRenderUse ||
      null,
    allowed_platforms: Array.isArray(allowedPlatforms) ? [...allowedPlatforms] : undefined,
    restricted_platforms: Array.isArray(restrictedPlatforms) ? [...restrictedPlatforms] : undefined,
    platform_restrictions:
      platformRestrictions && typeof platformRestrictions === "object"
        ? structuredClone(platformRestrictions)
        : undefined,
    commercial_use_allowed:
      clip?.commercial_use_allowed ??
      clip?.commercialUseAllowed ??
      provenance.commercial_use_allowed ??
      provenance.commercialUseAllowed,
    local_materialization_allowed:
      clip?.local_materialization_allowed ??
      clip?.localMaterializationAllowed ??
      provenance.local_materialization_allowed ??
      provenance.localMaterializationAllowed,
    live_publish_allowed:
      clip?.live_publish_allowed ??
      clip?.livePublishAllowed ??
      provenance.live_publish_allowed ??
      provenance.livePublishAllowed,
    requires_human_legal_review_before_publish:
      clip?.requires_human_legal_review_before_publish ??
      clip?.requiresHumanLegalReviewBeforePublish ??
      provenance.requires_human_legal_review_before_publish ??
      provenance.requiresHumanLegalReviewBeforePublish,
    source_audio_allowed:
      clip?.source_audio_allowed ??
      clip?.sourceAudioAllowed ??
      provenance.source_audio_allowed ??
      provenance.sourceAudioAllowed,
    rights_grant:
      clip?.rights_grant ??
      clip?.rightsGrant ??
      provenance.rights_grant ??
      provenance.rightsGrant,
    rights_verdict:
      clip?.rights_verdict ||
      clip?.rightsVerdict ||
      provenance.rights_verdict ||
      provenance.rightsVerdict ||
      null,
    credit_required:
      clip?.credit_required ??
      clip?.creditRequired ??
      provenance.credit_required ??
      provenance.creditRequired,
    evidence_reference:
      clip?.evidence_reference ||
      clip?.evidenceReference ||
    ...
    source_sha256: sourceMasterSha256,
    reference_url: referenceUrl,
    official_channel:
      clip?.official_channel ||
      clip?.officialChannel ||
      provenance.official_channel ||
      provenance.officialChannel ||
      null,
    official_channel_id:
      clip?.official_channel_id ||
      clip?.officialChannelId ||
      provenance.official_channel_id ||
      provenance.officialChannelId ||
      null,
    approval_status:
      clip?.approval_status || clip?.approvalStatus || provenance.approval_status || provenance.approvalStatus || null,
    rights_status:
      clip?.rights_status || clip?.rightsStatus || provenance.rights_status || provenance.rightsStatus || null,
    rights_risk_class:
      ...
      null,
    risk_score: clip?.risk_score ?? clip?.riskScore ?? provenance.risk_score ?? provenance.riskScore,
  };
}
...
function rejectedClipSegment(clip, index, reason) {
  const sourceUrl = String(clip?.path || clip?.source_url || clip?.sourceUrl || "").trim();
  ...
  return {
    ...
    segment_validation_ineligible_reason:
      sourceUrlKind.segment_validation_ineligible_reason || null,
    ...
    trim_recommended: false,
    recommended_media_start_s: null,
    recommended_duration_s: null,
    trim_sample_orders: [],
    samples: [],
  };
}
...
function guardSegmentSample(clip, sample, qa = {}) {
  ...
  const frameLike = {
    ...
    source_duration_s: sourceDurationSeconds(clip),
    allow_early_exploratory_window:
      clip?.allowEarlyExploratoryWindow === true ||
      clip?.allow_early_exploratory_window === true ||
      clip?.provenance?.allowEarlyExploratoryWindow === true ||
      clip?.provenance?.allow_early_exploratory_window === true,
    qa: guarded,
  };
...
function scoreGameplayActionSample(qa = {}) {
  ...
  const lowerBandOverlay = numberOr(prescan.lower_band_overlay_likelihood, 0);
  const lowerBandSpanRows = numberOr(prescan.lower_band_overlay_span_rows, 0);
  const lowerBandMaxRowRatio = numberOr(prescan.lower_band_overlay_max_row_ratio, 0);
  ...
  const cinematicCandidate =
    ...
    text <= 0.12 &&
    rounded >= 68;
  const stableGameplayHudCandidate =
    lowerBandOverlay >= 0.011 &&
    lowerBandSpanRows >= 1 &&
    lowerBandMaxRowRatio >= 0.82 &&
    edge >= 0.025 &&
    saturation >= 0.45 &&
    text <= 0.12 &&
    whiteText <= 0.25 &&
  ...
  const candidate =
    ...
    taste?.verdict !== "fail" &&
    (standardCandidate || cinematicCandidate || stableGameplayHudCandidate);
  let reason = stableGameplayHudCandidate
    ? "stable_gameplay_hud_candidate"
    : "gameplay_action_candidate";
  if (!candidate) {
...
function officialDirectMediaCleanMotionCandidate({ clip, passing, actionScore } = {}) {
  ...
  const directOfficialSource =
    /(?:platform_storefront_video_reference|official_game_website_media_page|official_publisher|official_studio|official_source|official_platform_product_page)/.test(
      sourceType,
    );
...
function sourceMotionTitleText(clip = {}) {
  return [
    clip?.reference_title,
    clip?.referenceTitle,
    clip?.movie_name,
...
function sourceMotionEntityTexts(clip = {}) {
  return [
    clip?.entity,
    clip?.canonical_entity,
    clip?.canonicalEntity,
    clip?.subject,
    clip?.provenance?.entity,
    clip?.provenance?.canonical_entity,
    clip?.provenance?.canonicalEntity,
    clip?.provenance?.subject,
  ].filter(Boolean);
}

function normaliseMotionMatchText(value) {
  return String(value || "")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, " ")
    .trim()
...
function sourceMotionTextIncludesEntity(clip = {}) {
  const sourceText = normaliseMotionMatchText(sourceMotionTitleText(clip));
  if (!sourceText) return false;
  return sourceMotionEntityTexts(clip).some((entity) => {
    const entityText = normaliseMotionMatchText(entity);
    if (entityText.length < 4) return false;
    if (sourceText.includes(entityText)) return true;
    const tokens = entityText.split(" ").filter((token) => token.length >= 3);
    return tokens.length >= 2 && tokens.every((token) => sourceText.includes(token));
  });
}

function sourceMotionTitleLooksLikeTrailer(clip = {}) {
  const text = sourceMotionTitleText(clip);
  return /\b(?:gameplay|official|launch|story|announcement|announce|trailer|teaser|pv\d+)\b/.test(text);
}

function licensedSteamStoreTrailerDirectMedia(clip = {}) {
  const sourceType = String(clip?.sourceType || clip?.source_type || "").toLowerCase();
  return (
    sourceType.includes("licensed_direct_media_url") &&
    /(?:^|\/)store_trailers\//i.test(clipSourceUrl(clip)) &&
    sourceMotionTextIncludesEntity(clip)
  );
}

function platformStorefrontSteamTrailerDirectMedia(clip = {}) {
  const sourceType = String(clip?.sourceType || clip?.source_type || "").toLowerCase();
  return (
    sourceType.includes("platform_storefront") &&
    /(?:^|\/)store_trailers\//i.test(clipSourceUrl(clip)) &&
    sourceMotionTextIncludesEntity(clip)
  );
}

function platformStorefrontSteamExtraDirectMedia(clip = {}) {
  const sourceType = String(clip?.sourceType || clip?.source_type || "").toLowerCase();
  const sourceUrl = clipSourceUrl(clip);
  return (
    sourceType.includes("platform_storefront") &&
    /^https:\/\/shared(?:\.[a-z0-9-]+)*\.steamstatic\.com\/store_item_assets\/steam\/apps\/\d+\/extras\//i.test(
      sourceUrl,
    ) &&
    mediaSourceUrlKindFields(sourceUrl).source_url_kind === "direct_video" &&
    sourceMotionTextIncludesEntity(clip)
  );
}

function youtubeVideoIdFromReferenceUrl(value) {
  try {
...
function verifiedOfficialYoutubePublisherTrailer(clip = {}) {
  const sourceType = String(clip?.sourceType || clip?.source_type || "").toLowerCase();
  if (!/^official_youtube_channel(?:_url)?$/.test(sourceType)) return false;
  const provenance = clip?.provenance && typeof clip.provenance === "object"
    ? clip.provenance
    : {};
  const originalProvenance =
    provenance.original_provenance && typeof provenance.original_provenance === "object"
  ...
  const youtubeVideoId = String(
    clip?.youtube_video_id ||
      clip?.youtubeVideoId ||
      provenance.youtube_video_id ||
      originalProvenance.youtube_video_id ||
      "",
  ).trim();
  const referenceUrl = String(
    clip?.reference_url ||
      clip?.referenceUrl ||
      provenance.reference_url ||
      originalProvenance.reference_url ||
      "",
  ).trim();
  const officialChannel = String(
    clip?.official_channel ||
      clip?.officialChannel ||
      provenance.official_channel ||
      originalProvenance.official_channel ||
      "",
  ).trim();
  const sourceSha256 = String(
    clip?.source_sha256 ||
      clip?.sourceSha256 ||
      provenance.source_sha256 ||
      originalProvenance.source_sha256 ||
      "",
  ).trim();
  const sourceIdentityPath = String(
    clip?.source_identity_path ||
      clip?.sourceIdentityPath ||
      provenance.source_identity_path ||
      originalProvenance.source_identity_path ||
      "",
  ).trim();
  const sourceIdentitySha256 = String(
    clip?.source_identity_sha256 ||
      clip?.sourceIdentitySha256 ||
      provenance.source_identity_sha256 ||
      originalProvenance.source_identity_sha256 ||
      "",
  ).trim();
  const sourceVerified =
    clip?.source_verified === true ||
    provenance.source_verified === true ||
    originalProvenance.source_verified === true;
  const commercialUseAllowed =
    clip?.commercial_use_allowed ??
    provenance.commercial_use_allowed ??
    originalProvenance.commercial_use_allowed;
  const allowedRenderUse = String(
    clip?.allowed_render_use ||
      provenance.allowed_render_use ||
      originalProvenance.allowed_render_use ||
      "",
  ).trim();
  return (
    sourceVerified &&
    /^[A-Za-z0-9_-]{11}$/.test(youtubeVideoId) &&
    youtubeVideoIdFromReferenceUrl(referenceUrl) === youtubeVideoId &&
    /^https:\/\/(?:www\.)?youtube\.com\/@[^/]+\/?$/i.test(officialChannel) &&
    /^[a-f0-9]{64}$/i.test(sourceSha256) &&
    Boolean(sourceIdentityPath) &&
    /^[a-f0-9]{64}$/i.test(sourceIdentitySha256) &&
    commercialUseAllowed === true &&
    allowedRenderUse === "transformative_editorial_short_form"
  );
}

function localPublisherPressKitMasterCandidate(clip = {}) {
  const sourceType = String(clip?.sourceType || clip?.source_type || "").toLowerCase();
  if (!/^official_(?:game_website_media_page|publisher_or_developer_trailer_page)$/.test(sourceType)) {
    return false;
  }
  if (mediaSourceUrlKindFields(clipSourceUrl(clip)).source_url_kind !== "local_video_file") {
    return false;
  }
  ...
  const originalProvenance =
    provenance.original_provenance && typeof provenance.original_provenance === "object"
      ? provenance.original_provenance
      : {};
  const source = String(provenance.source || originalProvenance.source || "").trim().toLowerCase();
  return /^official_publisher_press_kit(?:_|$)/.test(source);
}

function verifiedLocalPublisherPressKitMaster(clip = {}) {
  if (!localPublisherPressKitMasterCandidate(clip)) return false;
  ...
  const originalAssetSha256 = String(
    clip?.original_asset_sha256 ||
      clip?.originalAssetSha256 ||
      provenance.original_asset_sha256 ||
      originalProvenance.original_asset_sha256 ||
      "",
  ).trim();
  const referenceUrl = String(
    clip?.reference_url ||
  ...
  const evidenceReference = String(
    clip?.evidence_reference ||
      clip?.evidenceReference ||
      provenance.evidence_reference ||
      originalProvenance.evidence_reference ||
      "",
  ).trim();
  const sourceOwner = String(
    clip?.source_owner ||
      ...
      originalProvenance.source_owner ||
      "",
  ).trim();
  const commercialUseAllowed =
    clip?.commercial_use_allowed ??
  ...
  const approvalStatus = String(
    clip?.approval_status ||
      ...
      originalProvenance.approval_status ||
      "",
  ).trim();
  const rightsStatus = String(
    clip?.rights_status ||
      ...
      originalProvenance.rights_status ||
      "",
  ).trim().toUpperCase();
  const sourceIdentityBound =
    sourceSha256 === sourceIdentitySha256 || originalAssetSha256 === sourceIdentitySha256;

  return (
    sourceVerified &&
    /^official_publisher_press_kit(?:_|$)/.test(source) &&
    /^[a-f0-9]{64}$/i.test(sourceSha256) &&
    Boolean(sourceIdentityPath) &&
    /^[a-f0-9]{64}$/i.test(sourceIdentitySha256) &&
    sourceIdentityBound &&
    ...
    Boolean(sourceOwner) &&
    /\.json$/i.test(evidenceReference) &&
    commercialUseAllowed === true &&
    allowedRenderUse === "transformative_editorial_short_form" &&
    approvalStatus === "approved_for_commercial_editorial_use" &&
    rightsStatus === "GREEN"
  );
}

function officialPublisherDirectMediaTrailer(clip = {}) {
  const sourceType = String(clip?.sourceType || clip?.source_type || "").toLowerCase();
  const sourceUrl = clipSourceUrl(clip);
  const urlKind = mediaSourceUrlKindFields(sourceUrl);
  const verifiedOfficialYoutube = verifiedOfficialYoutubePublisherTrailer(clip);
  const verifiedLocalPublisherPressKit = verifiedLocalPublisherPressKitMaster(clip);
  const sourceText = sourceMotionTitleText(clip);
  const provenanceText = [
    ...
    clip?.allowed_render_use,
    clip?.allowedRenderUse,
    clip?.provenance?.provider,
    clip?.provenance?.source,
    clip?.provenance?.rights_risk_class,
    clip?.provenance?.rightsRiskClass,
    clip?.provenance?.allowed_render_use,
    clip?.provenance?.allowedRenderUse,
  ]
    .filter(Boolean)
    .join(" ")
    .toLowerCase();
  const directPublisherType =
    sourceType.includes("licensed_direct_media_url") ||
    /official_(?:game_site_news_page|platform_news_page|game_website_media_page|publisher_or_developer_trailer_page|social_media_video)/.test(
      sourceType,
    ) ||
    verifiedOfficialYoutube ||
    verifiedLocalPublisherPressKit;
  if (!directPublisherType) return false;
  if (
    !["direct_video", "hls_manifest", "dash_manifest"].includes(urlKind.source_url_kind) &&
    !(
      (verifiedOfficialYoutube || verifiedLocalPublisherPressKit) &&
  ...
  if (blockedDirectMediaMotionSourceText(clip)) return false;
  if (!sourceMotionTextIncludesEntity(clip)) return false;
  const hasOfficialMarker = /\bofficial\b/.test(sourceText) || /\bofficial\b/.test(provenanceText);
  if (!hasOfficialMarker) return false;
  return (
    sourceMotionTitleLooksLikeTrailer(clip) ||
    /\b(?:direct|showcase|reveal|announcement|announce|news|trailer|teaser)\b/.test(sourceText)
  );
}

function averageVisualTasteScore(samples = []) {
  const scores = samples
    .map((sample) => Number(sample?.qa?.visual_taste?.score))
    .filter((score) => Number.isFinite(score));
  if (!scores.length) return 0;
  return Number((scores.reduce((sum, score) => sum + score, 0) / scores.length).toFixed(1));
}

function averagePrescanMetric(samples = [], key) {
  const values = samples
    .map((sample) => Number(sample?.qa?.prescan?.[key]))
    .filter((value) => Number.isFinite(value));
  if (!values.length) return 0;
  return Number((values.reduce((sum, value) => sum + value, 0) / values.length).toFixed(4));
}

function storefrontTrailerMotionCandidate({ clip, passing, actionScore } = {}) {
  const sourceType = String(clip?.sourceType || clip?.source_type || "").toLowerCase();
  const sourceUrl = clipSourceUrl(clip);
  const urlKind = mediaSourceUrlKindFields(sourceUrl);
  const platformStorefrontSteamTrailer = platformStorefrontSteamTrailerDirectMedia(clip);
  const storefrontSource =
    ...
    platformStorefrontSteamTrailer;
  if (!storefrontSource) return false;
  if (!["direct_video", "hls_manifest", "dash_manifest"].includes(urlKind.source_url_kind)) return false;
  if (blockedDirectMediaMotionSourceText(clip)) return false;
  const titleLooksLikeTrailer =
    sourceMotionTitleLooksLikeTrailer(clip) ||
    (sourceType.includes("steam_movie") && sourceMotionTitleText(clip).length > 0) ||
    platformStorefrontSteamTrailer;
  if (!titleLooksLikeTrailer) return false;
  if (!Array.isArray(passing) || passing.length < 3) return false;
  if (uniqueContentHashCount(passing) < Math.min(MIN_PASSING_SAMPLES, passing.length)) return false;
  if (actionScore < 62) return false;
  if (averageVisualTasteScore(passing) < 78) return false;
  return passing.every((sample) => {
    if (sample.qa?.thumbnail_safe === false) return false;
    if (asArray(sample.qa?.failures).length) return false;
    if (sample.qa?.visual_taste?.verdict === "fail") return false;
    const score = numberOr(sample.qa?.gameplay_action_score, 0);
    const edge = numberOr(sample.qa?.prescan?.edge_density, 0);
    const saturation = numberOr(sample.qa?.prescan?.saturation_mean, 0);
    const text = numberOr(sample.qa?.prescan?.text_overlay_likelihood, 0);
    const whiteText = numberOr(sample.qa?.prescan?.white_text_on_dark_likelihood, 0);
    return score >= 58 && edge >= 0.085 && saturation >= 0.25 && text <= 0.12 && whiteText <= 0.25;
  });
}

function officialStorefrontCinematicMotionCandidate({ clip, passing, actionScore } = {}) {
  const sourceType = String(clip?.sourceType || clip?.source_type || "").toLowerCase();
  const sourceUrl = clipSourceUrl(clip);
  const urlKind = mediaSourceUrlKindFields(sourceUrl);
  const licensedSteamStoreTrailer = licensedSteamStoreTrailerDirectMedia(clip);
  const platformStorefrontSteamTrailer = platformStorefrontSteamTrailerDirectMedia(clip);
  const platformStorefrontSteamExtra = platformStorefrontSteamExtraDirectMedia(clip);
  const officialPublisherTrailer = officialPublisherDirectMediaTrailer(clip);
  const storefrontSource =
    sourceType.includes("steam_movie") ||
    sourceType.includes("steam_storefront_video_reference") ||
    sourceType.includes("platform_storefront_video_reference") ||
    platformStorefrontSteamTrailer ||
    platformStorefrontSteamExtra ||
    licensedSteamStoreTrailer ||
    officialPublisherTrailer;
  if (!storefrontSource) return false;
  if (
    !["direct_video", "hls_manifest", "dash_manifest"].includes(urlKind.source_url_kind) &&
    !(officialPublisherTrailer && urlKind.source_url_kind === "local_video_file")
  ) {
    return false;
  }
  if (blockedDirectMediaMotionSourceText(clip)) return false;
  ...
  if (!Array.isArray(passing) || passing.length < MIN_PASSING_SAMPLES) return false;
  if (uniqueContentHashCount(passing) < Math.min(MIN_PASSING_SAMPLES, passing.length)) return false;
  const calmStorefrontMotion =
    passing.length >= 3 &&
    actionScore >= 59 &&
    averageVisualTasteScore(passing) >= 78 &&
    averagePrescanMetric(passing, "edge_density") >= 0.09 &&
    passing.some((sample) => numberOr(sample.qa?.prescan?.edge_density, 0) >= 0.12) &&
    passing.some((sample) => numberOr(sample.qa?.gameplay_action_score, 0) >= 61) &&
    passing.every((sample) => {
      ...
      const text = numberOr(sample.qa?.prescan?.text_overlay_likelihood, 0);
      const whiteText = numberOr(sample.qa?.prescan?.white_text_on_dark_likelihood, 0);
      const dark = numberOr(sample.qa?.prescan?.dark_pixel_ratio, 0);
      const bright = numberOr(sample.qa?.prescan?.bright_pixel_ratio, 0);
      const tasteScore = numberOr(sample.qa?.visual_taste?.score, 0);
      return (
        score >= 58 &&
        saturation >= 0.27 &&
        text <= 0.08 &&
  ...
  if (calmStorefrontMotion) return true;
  if (actionScore < 62) return false;
  if (averageVisualTasteScore(passing) < 80) return false;
  if (averagePrescanMetric(passing, "edge_density") < 0.075) return false;
  if (!passing.some((sample) => numberOr(sample.qa?.prescan?.edge_density, 0) >= 0.1)) return false;
  const duration = numberOr(clip?.durationS ?? clip?.duration_s, 5);
  const shortMutedHighDetail =
    duration <= 3.25 &&
    passing.length >= MIN_PASSING_SAMPLES &&
    averageVisualTasteScore(passing) >= 84 &&
    averagePrescanMetric(passing, "edge_density") >= 0.08 &&
    passing.some((sample) => numberOr(sample.qa?.prescan?.edge_density, 0) >= 0.12) &&
    passing.every((sample) => {
      if (sample.qa?.thumbnail_safe === false) return false;
      if (asArray(sample.qa?.failures).length) return false;
      ...
      const dark = numberOr(sample.qa?.prescan?.dark_pixel_ratio, 0);
      const bright = numberOr(sample.qa?.prescan?.bright_pixel_ratio, 0);
      return saturation >= 0.1 && text <= 0.08 && whiteText <= 0.2 && dark <= 0.75 && bright <= 0.45;
    });
  if (shortMutedHighDetail) return true;
  ...
  return passing.every((sample) => {
    ...
    const mutedHighDetail =
      ...
      bright <= 0.45;
    return (
      (saturation >= 0.35 || mutedHighDetail) &&
      text <= 0.08 &&
      whiteText <= 0.2 &&
      dark <= 0.85 &&
      bright <= 0.45
...
function officialProductMotionCandidate({ clip, passing, actionScore } = {}) {
  const sourceType = String(clip?.sourceType || clip?.source_type || "").toLowerCase();
  if (!sourceType.includes("official_platform_product_page")) return false;
  if (officialDirectMediaCleanMotionCandidate({ clip, passing, actionScore })) return true;
  if (!Array.isArray(passing) || passing.length < MIN_PASSING_SAMPLES) return false;
  if (uniqueContentHashCount(passing) < Math.min(MIN_PASSING_SAMPLES, passing.length)) return false;
  const cleanSamples = passing.every((sample) => {
    if (sample.qa?.thumbnail_safe === false) return false;
    if (asArray(sample.qa?.failures).length) return false;
    if (sample.qa?.visual_taste?.verdict === "fail") return false;
    return true;
  });
  if (!cleanSamples) return false;
  const strictProductMotion = actionScore >= 66 && averageVisualTasteScore(passing) >= 88 && passing.every((sample) => {
    const score = numberOr(sample.qa?.gameplay_action_score, 0);
    const edge = numberOr(sample.qa?.prescan?.edge_density, 0);
    const saturation = numberOr(sample.qa?.prescan?.saturation_mean, 0);
    const text = numberOr(sample.qa?.prescan?.text_overlay_likelihood, 0);
    const whiteText = numberOr(sample.qa?.prescan?.white_text_on_dark_likelihood, 0);
    return score >= 62 && edge >= 0.02 && saturation >= 0.55 && text <= 0.12 && whiteText <= 0.25;
  });
  if (strictProductMotion) return true;
  const lifestyleHardwareMotion =
    actionScore >= 64 &&
    averageVisualTasteScore(passing) >= 84 &&
    passing.every((sample) => {
      const score = numberOr(sample.qa?.gameplay_action_score, 0);
      ...
      const text = numberOr(sample.qa?.prescan?.text_overlay_likelihood, 0);
      const whiteText = numberOr(sample.qa?.prescan?.white_text_on_dark_likelihood, 0);
      return score >= 61 && edge >= 0.055 && saturation >= 0.16 && text <= 0.08 && whiteText <= 0.2;
    });
  if (lifestyleHardwareMotion) return true;
  const cleanControllerDetailMotion =
    actionScore >= 64 &&
    averageVisualTasteScore(passing) >= 82 &&
    passing.every((sample) => {
      const score = numberOr(sample.qa?.gameplay_action_score, 0);
      const edge = numberOr(sample.qa?.prescan?.edge_density, 0);
      const saturation = numberOr(sample.qa?.prescan?.saturation_mean, 0);
      ...
      const dark = numberOr(sample.qa?.prescan?.dark_pixel_ratio, 0);
      const bright = numberOr(sample.qa?.prescan?.bright_pixel_ratio, 0);
      const letterbox = numberOr(sample.qa?.prescan?.letterbox_bar_ratio, 0);
      return (
        score >= 63 &&
        edge >= 0.12 &&
        saturation >= 0.28 &&
  ...
  return cleanControllerDetailMotion;
}
...
function officialStorefrontDetailTrimCandidateFromSamples(samples, clip = {}) {
  const sourceType = String(clip?.sourceType || clip?.source_type || "").toLowerCase();
  const sourceUrl = clipSourceUrl(clip);
  const urlKind = mediaSourceUrlKindFields(sourceUrl);
  const storefrontSource =
    ...
    sourceType.includes("steam_storefront_video_reference") ||
    sourceType.includes("platform_storefront_video_reference");
  if (!storefrontSource) return null;
  if (!["direct_video", "hls_manifest", "dash_manifest"].includes(urlKind.source_url_kind)) return null;
  if (blockedDirectMediaMotionSourceText(clip)) return null;
  if (!sourceMotionTitleLooksLikeTrailer(clip)) return null;
  ...
  for (const item of ordered) {
    ...
    const qa = sample.qa || {};
    const prescan = qa.prescan || {};
    const taste = qa.visual_taste || {};
    const score = numberOr(qa.gameplay_action_score, 0);
    const edge = numberOr(prescan.edge_density, 0);
    const text = numberOr(prescan.text_overlay_likelihood, 0);
    const whiteText = numberOr(prescan.white_text_on_dark_likelihood, 0);
    const dark = numberOr(prescan.dark_pixel_ratio, 0);
    ...
    const isCleanDetail =
      sample.status === "accepted" &&
      qa.thumbnail_safe !== false &&
      !asArray(qa.failures).length &&
      taste.verdict !== "fail" &&
      score >= MIN_SEGMENT_ACTION_SCORE &&
      edge >= 0.14 &&
      text <= 0.12 &&
      whiteText <= 0.2 &&
      dark <= 0.45 &&
  ...
  const candidates = runs
    .filter((run) => run.length >= MIN_PASSING_SAMPLES)
    .map((run) => {
      const runSamples = run.map((item) => item.sample);
      const uniqueHashes = uniqueContentHashCount(runSamples);
      const actionScore = averageGameplayScore(runSamples);
      const tasteScore = averageVisualTasteScore(runSamples);
      return {
        run,
        runSamples,
        uniqueHashes,
    ...
    .filter((candidate) => {
      const requiredHashes = Math.min(MIN_PASSING_SAMPLES, candidate.runSamples.length);
      return (
        candidate.uniqueHashes >= requiredHashes &&
        candidate.actionScore >= MIN_SEGMENT_ACTION_SCORE &&
        candidate.tasteScore >= 86
      );
    })
    .sort((a, b) => {
      if (b.runSamples.length !== a.runSamples.length) return b.runSamples.length - a.runSamples.length;
...
function classifySegment(samples, clip = {}) {
  ...
  const storefrontDetailTrimCandidate = officialStorefrontDetailTrimCandidateFromSamples(finished, clip);
  ...
  if (storefrontDetailTrimCandidate) {
    return {
      status: "validated",
      segment_validated: true,
      allowed_for_flash_lane: true,
      reason: "trimmed_official_storefront_detail_motion_samples_passed",
      action_sample_count: storefrontDetailTrimCandidate.action_sample_count,
      action_score: storefrontDetailTrimCandidate.action_score,
      segment_motion_class: "gameplay_action",
      trim_recommended: true,
      recommended_media_start_s: storefrontDetailTrimCandidate.recommended_media_start_s,
      recommended_duration_s: storefrontDetailTrimCandidate.recommended_duration_s,
      trim_sample_orders: storefrontDetailTrimCandidate.trim_sample_orders,
      sample_rejection_reasons: rejectionReasons,
    };
  }
  ...
  if (
    officialProductMotionCandidate({
      clip,
      passing,
      actionScore,
    })
    ...
    return {
      ...
      segment_validated: true,
      allowed_for_flash_lane: true,
      reason: "official_product_motion_samples_passed",
      action_sample_count: actionSamples.length,
      action_score: actionScore,
  ...
  if (
    storefrontTrailerMotionCandidate({
      clip,
      passing,
      actionScore,
    })
    ...
    return {
      ...
      segment_validated: true,
      allowed_for_flash_lane: true,
      reason: "official_storefront_trailer_motion_samples_passed",
      action_sample_count: actionSamples.length,
      action_score: actionScore,
      segment_motion_class: "gameplay_action",
      sample_rejection_reasons: rejectionReasons,
  ...
  if (
    officialStorefrontCinematicMotionCandidate({
      clip,
      passing,
      actionScore,
    })
    ...
    return {
      ...
      segment_validated: true,
      allowed_for_flash_lane: true,
      reason: "official_storefront_cinematic_motion_samples_passed",
      action_sample_count: actionSamples.length,
      action_score: actionScore,
...
async function processClipSegment(clip, index, options) {
  ...
  if (preflightRejectReason) {
    return {
      ...rejectedClipSegment(clip, index, preflightRejectReason),
...
function buildSegmentValidationReport({ applyLocal, outputRoot, segments, status = "completed", completed = true } = {}) {
  return {
    schema_version: 1,
...
function mergeOfficialTrailerSegmentReports(previousReport = null, currentReport = null, options = {}) {
  ...
  const currentReferenceSourceUrls = new Set(
    asArray(options.currentReferenceSourceUrls)
      .map((sourceUrl) => String(sourceUrl || "").trim())
      .filter(Boolean),
  );
  let previousStaleSourceSegmentCount = 0;
  const previousSegmentsForMerge = currentReferenceSourceUrls.size && scopedStoryIds.size
    ? previousSegments.filter((segment) => {
        const storyId = segmentStoryId(segment);
        if (!storyId || !scopedStoryIds.has(storyId)) return true;
        const sourceUrl = clipSourceUrl(segment);
        if (!sourceUrl || currentReferenceSourceUrls.has(sourceUrl)) return true;
        previousStaleSourceSegmentCount += 1;
        return false;
      })
    : previousSegments;
  const byKey = new Map();
  for (const segment of previousSegmentsForMerge) {
  ...
  return {
    ...
    merge: {
      ...
      previous_merged_segment_count: previousSegmentsForMerge.length,
      previous_stale_source_segment_count: previousStaleSourceSegmentCount,
      current_reference_source_count: currentReferenceSourceUrls.size,
...
async function runOfficialTrailerSegmentValidation(clipRefs = [], options = {}) {
  ...
  const acceptedSourceWindows = [];
  for (const [index, clip] of clips.entries()) {
    const clipKey = segmentKeyForClipRef(clip);
    if (seenClipKeys.has(clipKey)) {
      segments.push(rejectedClipSegment(clip, index, "duplicate_source_window"));
      if (typeof options.onProgress === "function") {
        await options.onProgress(buildSegmentValidationReport({
          applyLocal,
          outputRoot,
    ...
    if (overlapsAcceptedSourceWindow(clip, acceptedSourceWindows)) {
      segments.push(rejectedClipSegment(clip, index, "overlapping_source_window"));
      if (typeof options.onProgress === "function") {
        await options.onProgress(buildSegmentValidationReport({
          applyLocal,
          outputRoot,
    ...
    const segment = await processClipSegment(clip, index, workerOptions);
    segments.push(segment);
    if (segment.allowed_for_flash_lane === true) {
      acceptedSourceWindows.push({
        identity: clipSourceWindowIdentity(clip),
        range: clipSourceWindowRange(clip),
        clip_key: clipKey,
      });
    }
    if (typeof options.onProgress === "function") {
  ...
  return buildSegmentValidationReport({
    applyLocal,
    outputRoot,
    segments,
    status: "completed",
...
function applySegmentValidationToClipRefs(clipRefs = [], report = null) {
  ...
  return asArray(clipRefs).map((clip) => {
    ...
    const allowedForFlashLane =
      ...
      FLASH_LANE_ALLOWED_MOTION_CLASSES.has(String(segment.segment_motion_class || "")) &&
      Number.isFinite(actionScore) &&
      actionScore >= (
        segment.segment_motion_class === "official_storefront_cinematic_motion"
          ? 62
          : segment.segment_motion_class === "official_product_motion"
            ? 64
