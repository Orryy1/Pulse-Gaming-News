...
} = require("../v4/premium-card-timing-policy");

const MIN_PREMIUM_HYPERFRAMES_CARDS = 4;
const MIN_HYPERFRAMES_READABLE_HOLD_S = V5_READABLE_CARD_TIMING.minimum_visible_duration_s;
const MAX_HYPERFRAMES_READABLE_HOLD_S = V5_READABLE_CARD_TIMING.maximum_visible_duration_s;
  ...
  return {
    ...
    contract_version: String(
      contract.contract_version ||
        contract.version ||
        shell.card_timing_contract_version ||
        shell.timing_policy_version ||
        "",
    ).trim(),
...
function internalReadableHoldFloorS({ readableText = "", wordCount = null, kind = "context" } = {}) {
  const text = String(readableText || "").trim();
  const words = Number(wordCount) || text.split(/\s+/).filter(Boolean).length;
  const timingKind = String(kind).toLowerCase() === "source" && words > MAX_CONCISE_SOURCE_LOCK_WORDS
    ? "context"
    : kind;
  const fallbackText = text || Array(Math.max(1, words || 1)).fill("WORD").join(" ");
  return v5CardTimingContract(timingKind, fallbackText).minimum_visible_duration_s;
}
...
function evaluateHyperframesPremiumShellEvidence({
  ...
  const unifiedCheckPasses = isPassingCheck(checks.check);
  for (const name of ["lint", "validate", "inspect"]) {
    const hasDedicatedCheck = checks[name] != null;
    if (!(hasDedicatedCheck ? isPassingCheck(checks[name]) : unifiedCheckPasses)) {
      blockers.push(`hyperframes_${name}_not_passed`);
    }
  }
  if (!isPassingCheck(checks.render)) blockers.push("hyperframes_render_not_passed");
  if (checks.check?.skipped === true) {
    blockers.push("hyperframes_check_skipped");
  }
  ...
  if (!readability.contract_version) {
    blockers.push("hyperframes_card_timing_contract_version_missing");
  } else if (readability.contract_version !== PREMIUM_CARD_TIMING_V5_VERSION) {
    blockers.push(
      `hyperframes_card_timing_contract_stale:${readability.contract_version}:${PREMIUM_CARD_TIMING_V5_VERSION}`,
    );
  }
  ...
  const currentTiming = v5CardTimingContract(kind, readability.readable_text);
  evidence.cardTimingContractVersion = readability.contract_version || null;
  evidence.currentCardTimingContract = currentTiming;
  if (currentTiming.content_fits_maximum === false) {
    blockers.push("hyperframes_card_copy_exceeds_momentum_budget");
  }
  if (
    readability.planned_visible_duration_s != null &&
    readability.planned_visible_duration_s > currentTiming.maximum_visible_duration_s + 0.001
  ) {
    blockers.push("hyperframes_readable_hold_above_momentum_ceiling");
  }
  if (
    readability.max_readable_card_duration_s != null &&
    readability.max_readable_card_duration_s > currentTiming.maximum_visible_duration_s + 0.001
  ) {
    blockers.push("hyperframes_readable_maximum_above_momentum_ceiling");
  }
