...
function assertPromotionPacketStoryRequest(stillDeckReport = {}, { storyId } = {}) {
  if (!storyId) return;
  const reportStoryId = stillDeckReport.story_id || stillDeckReport.storyId || null;
  if (!reportStoryId) {
    throw new Error(`still-deck report does not declare a story id for requested story ${storyId}`);
  }
  if (String(reportStoryId) !== String(storyId)) {
    throw new Error(`still-deck report is for ${reportStoryId}, not ${storyId}`);
  }
}
