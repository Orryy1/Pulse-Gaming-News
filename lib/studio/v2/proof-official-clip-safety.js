...
function resolveOfficialTrailerClipRefsForProof({
  ...
  allowPartialValidatedOfficialClips = false,
  targetRuntimeS = 66,
} = {}) {
  ...
  if (segmentValidationReport) {
    ...
    if (footageBackboneReport.verdict !== "ready_for_flash_render_preflight" || clipRefs.length === 0) {
      if (allowPartialValidatedOfficialClips && renderSafe.clipRefs.length > 0) {
        return {
          clipRefs: renderSafe.clipRefs,
          allClipRefs: clipRefs,
          footageBackboneReport,
          safety: {
            status: "partial_validated_diagnostic_only",
            reason:
              "partial validated official clips were allowed only for an explicit local diagnostic render",
