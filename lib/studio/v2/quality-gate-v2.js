...
function gradeSpokenWPM(pkg, audioDurationS, realignedWords = []) {
  const tightenedScript = pkg?.script?.tightened || "";
  const wordCount = Array.isArray(realignedWords) && realignedWords.length > 0
    ? realignedWords.length
    : tightenedScript.split(/\s+/).filter(Boolean).length;
  ...
  return {
    value: Number(wpm.toFixed(1)),
    grade: g,
    wordCount,
    source:
      Array.isArray(realignedWords) && realignedWords.length > 0
        ? "alignment"
        : "package",
  };
}
...
function gradeSourceDiversity(scenes) {
  ...
  for (const s of scenes) {
    ...
    const rawId =
      ...
      s.label ||
      "(unknown)";
    let id = String(rawId).replace(/_smartcrop_v2(_[a-z]+)?\.jpe?g$/i, ".jpg");
    const isFootageBeat =
      ["clip", "punch", "speed-ramp", "freeze-frame"].includes(type) ||
      ((type === "opener" || s.sceneType === "opener") && s.isClipBacked);
    const start = Number(s.mediaStartS ?? s.media_start_s ?? s.startS);
    if (isFootageBeat && Number.isFinite(start)) {
      id = `${id}#${start.toFixed(1)}s`;
    }
    sources.add(id);
  }
...
function gradeCaptionGaps(assPath, audioDurationS) {
  ...
  const gaps = [];
  if (intervals.length > 0 && intervals[0][0] > CAPTION_GAP_WARN_S) {
    gaps.push({ fromS: 0, toS: intervals[0][0], type: "leading" });
  }
  for (let i = 1; i < intervals.length; i++) {
    const gap = intervals[i][0] - intervals[i - 1][1];
    if (gap > CAPTION_GAP_WARN_S) {
      gaps.push({
        fromS: intervals[i - 1][1],
        toS: intervals[i][0],
        type: "internal",
      });
    }
  }
  const lastEnd = intervals.length ? intervals[intervals.length - 1][1] : 0;
  if (
    intervals.length > 0 &&
    Number.isFinite(Number(audioDurationS)) &&
    Number(audioDurationS) - lastEnd > CAPTION_GAP_WARN_S
  ) {
    gaps.push({
      fromS: lastEnd,
      toS: Number(audioDurationS),
      type: "tail",
    });
  }
  const g = grade(
...
function gradeVoicePath(audioMeta) {
  ...
  const source = audioMeta?.source || "";
  let lane = "unknown";
  let g = "amber";
  let blocker = null;
  let localVoiceReference = null;
  if (/silent/i.test(provider) || /silent/i.test(source)) {
    lane = "silent-fixture";
    g = "red";
    ...
    if (
      ...
      lane =
        source === "local-production-chatterbox-path"
          ? "local-production-chatterbox"
          : source === "provided-local-tts-audio"
            ? "approved-provided-local-tts"
            : "local-production-voxcpm";
      const humanApproved = localVoiceHumanApproved(audioMeta, process.env);
      const reference = localVoiceReferenceStatus(audioMeta, process.env);
      localVoiceReference = reference.reference;
      if (!humanApproved) blocker = "unapproved_local_tts_voice_path";
      else if (!reference.approved) blocker = reference.blocker;
      g = blocker ? "red" : "green";
    } else {
      lane = String(source).includes("voxcpm") ? "legacy-local-voxcpm" : "fresh-local";
      g =
  ...
  return { value: lane, grade: g, voiceId, blocker, localVoiceReference };
}
...
function gradeMotionDensity(scenes, transitions, audioDurationS) {
  ...
  const motionSceneCount = scenes.filter((scene) => {
  ...
  const motionSceneRatio = scenes.length > 0 ? motionSceneCount / scenes.length : 0;
  const g = grade(
    perMin,
    (v) => v >= 18,
    (v) => v >= 12 || (v >= 9.5 && motionSceneRatio >= 0.7),
  );
  return {
    value: Number(perMin.toFixed(1)),
    grade: g,
    motionSceneRatio: Number(motionSceneRatio.toFixed(2)),
  };
}
...
function gradeSfx(soundLayerPayload) {
  ...
  if (process.env.STUDIO_V2_SFX_MODE === "off") {
    return { value: cueCount, grade: "green", disabled: true };
  }
  const g = grade(
    cueCount,
    (v) => v >= 2,
...
function gradeBrandNameIntegrity({ pkg, assPath }) {
  let subtitleText = "";
  if (assPath && fs.existsSync(assPath)) {
    subtitleText = extractAssPlainText(fs.readFileSync(assPath, "utf8"));
  }
  const qa = runBrandNameQa({
    title: pkg?.title,
    script_raw: pkg?.script?.raw,
    script_tightened: pkg?.script?.tightened,
    subtitles: subtitleText,
  });
  return {
    value: qa.failures.length,
    grade:
...
function gradeFlashLanePreflight(preflight) {
  if (!preflight) {
    return {
      value: 0,
      grade: "green",
  ...
  return {
    ...
    grade: blockers.length > 0 ? "red" : warnings.length > 0 ? "amber" : "green",
    verdict: preflight.verdict || (blockers.length > 0 ? "block" : "allow"),
    blockers,
    warnings,
  };
}
...
function deriveVerdict(auto) {
  ...
  if (auto.spokenWPM?.grade === "red") {
    lane = "reject";
    reasons.push("spoken pacing is outside publishable range");
  }
  if (auto.brandNameIntegrity?.grade === "red") {
    lane = "reject";
    reasons.push("brand-name integrity failed");
  }
  if (auto.voicePathUsed?.grade === "red") {
    lane = "reject";
    const voiceBlocker = auto.voicePathUsed.blocker;
    reasons.push(
      auto.voicePathUsed.value === "silent-fixture"
        ? "silent fixture audio is not a valid pilot proof"
        : voiceBlocker === "local_tts_voice_reference_unverified" ||
            voiceBlocker === "local_tts_voice_reference_mismatch"
          ? "local TTS voice reference unverified"
        : "unapproved local TTS voice path",
    );
  }
  if (auto.flashLanePreflight?.grade === "red") {
    lane = "reject";
    reasons.push("Flash Lane preflight blocked");
  }
...
function buildQualityReportV2({
  ...
  const auto = {
    ...
    spokenWPM: gradeSpokenWPM(pkg, audioDurationS, realignedWords),
    ...
    brandNameIntegrity: gradeBrandNameIntegrity({ pkg, assPath }),
    flashLanePreflight: gradeFlashLanePreflight(flashLanePreflight),
  };
