...
function buildFreezeFrameScene({
  ...
  // AUTHORED MOMENT: 60ms full-frame white flash at the freeze
  // instant. Reads as a deliberate "photo finish" / shutter-snap
  // beat. Drawn BEFORE the caption stack so the caption pops out of
  // the flash rather than being washed white.
  if (authored) {
    const flashStart = playInS.toFixed(2);
    const flashEnd = (playInS + 0.06).toFixed(2);
    filterParts.push(
      `drawbox=x=0:y=0:w=iw:h=ih:color=white@0.85:t=fill:enable='between(t\\,${flashStart}\\,${flashEnd})'`,
    );
  }
