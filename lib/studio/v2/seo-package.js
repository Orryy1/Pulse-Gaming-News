...
function buildDescription({ story, pkg, channel }) {
  ...
  const governedCta =
    channel.channelId === "pulse-gaming"
      ? resolveGovernedCtaCopy(story)
      : "";
  const bodyWithoutSelectedCta = governedCta
    ? tightened.replace(governedCta, "").replace(/\s+/g, " ").trim()
    : tightened;
  const bodyTrimmed = trimToLength(bodyWithoutSelectedCta, 720);
...
/**
 * Public engagement generation is frozen during stabilisation. A comment can
 * only enter the upload request through the separate hash-bound operator
 * approval recorded on the story.
 */
