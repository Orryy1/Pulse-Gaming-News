...
function buildAudioMixFilters({ voiceIdx, musicIdx, sfxCues, targetDurationS }) {
  ...
  //    threshold=0.045 (~-27 dBFS), ratio=7, attack=8ms, release=320ms.
  ...
  if (musicIdx >= 0) {
    lines.push(`[${musicIdx}:a]volume=0.135[bgm_raw]`);
    lines.push(
      `[bgm_raw][voiceSC]sidechaincompress=threshold=0.045:ratio=7:attack=8:release=320:knee=3:level_sc=1[bgm]`,
    );
  }
