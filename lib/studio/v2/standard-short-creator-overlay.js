...
function isMotionScene(scene) {
  const type = String(scene?.type || "").toLowerCase();
  return (
    type.includes("clip") || type.includes("frame") || type.includes("motion")
  );
}

function sceneDuration(scene, fallback = 4) {
  const value = Number(
    scene?.duration ?? scene?.durationS ?? scene?.duration_s,
  );
  return Number.isFinite(value) && value > 0 ? value : fallback;
}

function extractEntities({ story = {}, scenes = [] } = {}) {
  const sceneEntities = asArray(scenes).map((scene) =>
    cleanLabel(scene.entity),
  );
  ...
  const textEntities = known.filter((entity) =>
    new RegExp(`\\b${entity.replace(/\s+/g, "\\s+")}\\b`, "i").test(text),
  );
    ...
    return {
      ...
      source_type:
        story.source_type === "rss"
          ? "rss_description_only"
          : "unknown_non_reddit_text",
      label: null,
...
function buildEntityPopups(entities, scenes) {
  return entities.map((entity, index) => {
    const matchedScene = asArray(scenes).find(
      (scene) => cleanLabel(scene.entity) === entity,
    );
    return {
      entity,
      kind: "entity_popup",
      ...
      reason:
        "Named subject should appear as a quick creator-style overlay instead of a full-screen card.",
    };
  });
}

function resolveOverlayCtaPolicy(story = {}) {
  const decision = story.cta_policy || null;
  const governedCta = resolveGovernedCtaCopy(story);
  const rendererApplied = Boolean(governedCta);
  return {
    policy_version: decision?.policy_version || null,
    include_cta: decision?.include_cta === true,
    renderer_applied: rendererApplied,
    cta_copy: governedCta || null,
    audit_hash: decision?.audit_hash || null,
    reason: rendererApplied
      ? "governed_selected_cta"
      : "cta_omitted_or_contract_invalid",
  };
}
...
function buildTimeline({
  ...
  const timeline = [];
  const source = titleCaseSource(
    story.subreddit || story.publisher || story.source_name,
  );
  timeline.push({
    kind: "source_badge",
    label: source,
  ...
  if (ctaPolicy.renderer_applied) {
    timeline.push({
      kind: "cta_chip",
      label: ctaPolicy.cta_copy,
      at_s: Math.max(0, Number(durationS || 0) - 3),
      duration_s: 3,
      style: "single_line_creator_cta",
      policy_version: ctaPolicy.policy_version,
      audit_hash: ctaPolicy.audit_hash,
    });
  }
...
function buildRecommendations({ cardRatio, motionRatio, blockers }) {
  ...
  if (cardRatio > 0.45)
    recommendations.push("replace_fullscreen_cards_with_popups");
  if (motionRatio < 0.25)
    recommendations.push("add_validated_footage_or_gameplay_backbone");
  if (blockers.includes("standard_short_duration_outside_selected_band")) {
...
function buildStandardShortCreatorOverlayPlan({
  ...
  const cardRatio =
    totalScenes > 0 ? Number((cardScenes / totalScenes).toFixed(2)) : 0;
  const motionRatio =
    totalScenes > 0 ? Number((motionScenes / totalScenes).toFixed(2)) : 0;
  const entities = extractEntities({ story, scenes: sceneList });
  const commentOverlay = buildCommentOverlay(story);
  const ctaPolicy = resolveOverlayCtaPolicy(story);
  const editorial = resolveRequiredPulseEditorialContract({
    story,
    editorialContract,
  });
  const blockers = editorial.blocker ? [editorial.blocker] : [];
  if (totalScenes === 0) blockers.push("standard_short_has_no_scenes");
  if (cardRatio > cardRatioLimit)
    blockers.push("standard_short_card_ratio_too_high");
  if (
    editorial.contract &&
    (!Number.isFinite(Number(durationS)) ||
      Number(durationS) < Number(editorial.contract.min_seconds) ||
      Number(durationS) > Number(editorial.contract.max_seconds))
  ) {
    blockers.push("standard_short_duration_outside_selected_band");
  }

  const warnings = [];
  if (motionRatio < 0.25) warnings.push("standard_short_motion_ratio_low");
  if (entities.length === 0)
    warnings.push("standard_short_has_no_named_entities");
  if (commentOverlay.source_type === "rss_description_only")
    warnings.push("rss_description_comment_overlay_disabled");
  ...
  return {
    ...
    verdict: blockers.length
      ? "needs_standard_overlay_rebuild"
      : "ready_for_standard_short_overlay",
    blockers,
    warnings,
    duration_s: durationS,
    duration_band_id: editorial.contract?.duration_band_id || null,
    scene_inventory: {
...
function renderStandardShortCreatorOverlayMarkdown(plan) {
  ...
  lines.push("");
  lines.push(
    `- max words per punch: ${plan.caption_rules.max_words_per_punch}`,
  );
  ...
  for (const item of plan.timeline) {
    lines.push(
      `- ${item.at_s}s ${item.kind}: ${item.label || item.entity || "unlabelled"}`,
    );
  }
  lines.push("");
