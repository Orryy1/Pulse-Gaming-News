...
const SEGMENT_VALIDATION_PASS_REASONS = new Set([
  "segment_samples_passed",
  "trimmed_segment_samples_passed",
]);

function segmentValidationRejectReason(segment = {}) {
  const status = String(segment.status || "").toLowerCase();
  const reason = String(segment.validation_reason || "").trim();
  if (status === "validated") return null;
  if (SEGMENT_VALIDATION_PASS_REASONS.has(reason)) return null;
  if (reason === "segment_contains_title_or_rating_card") return "segment_title_or_rating_card";
  if (reason === "segment_contains_black_frame") return "segment_contains_black_frame";
  if (reason === "segment_contains_low_detail_frame") return "segment_contains_low_detail_frame";
  if (reason === "segment_samples_too_repetitive") return "segment_samples_too_repetitive";
  if (reason === "segment_lacks_gameplay_action_samples") {
    return "segment_lacks_gameplay_action_samples";
  }
  if (reason === "segment_sample_extract_failed") return "segment_sample_extract_failed";
  return status === "rejected" ? "segment_not_validated" : null;
}
...
function framesFromSegmentValidationReport(segmentValidationReport, storyId) {
  ...
  for (const segment of Array.isArray(segmentValidationReport?.segments)
    ...
    const segmentRejectReason = segmentValidationRejectReason(segment);
    const sample = bestSegmentValidationSample(segment);
    if (sample) {
      const seekSeconds = Number(
        sample.target_time_seconds ??
          sample.targetTimeSeconds ??
          sample.seek_seconds ??
          sample.seekSeconds,
      );
      const segmentStart = Number(segment.media_start_s ?? segment.mediaStartS);
      const sampleOffset = Number(sample.offset_s ?? sample.offsetS);
      frames.push({
        ...sample,
        __frameRejectReason: segmentRejectReason || sample.__frameRejectReason || null,
        ...
        target_time_seconds: Number.isFinite(seekSeconds)
          ? seekSeconds
          : Number.isFinite(segmentStart) && Number.isFinite(sampleOffset)
            ? Number((segmentStart + sampleOffset).toFixed(3))
            : null,
        media_start_s: Number.isFinite(segmentStart) ? segmentStart : null,
        offset_s: Number.isFinite(sampleOffset) ? sampleOffset : null,
...
function buildFrameAsset(frame, resolved, order) {
  ...
  const provenance = {
    ...
    target_time_seconds:
      ...
      frame.targetTimeSeconds ??
      frame.seek_seconds ??
      frame.seekSeconds ??
      null,
    media_start_s: frame.media_start_s ?? frame.mediaStartS ?? null,
    offset_s: frame.offset_s ?? frame.offsetS ?? null,
