...
function sourceAssetsFromPlan(plan) {
  const sourceAssets =
    Array.isArray(plan?.applied_assets) && plan.applied_assets.length
      ? plan.applied_assets
      : Array.isArray(plan?.would_fetch) && plan.would_fetch.length
        ? plan.would_fetch
        : Array.isArray(plan?.visual_deck?.items)
          ? plan.visual_deck.items
          : [];
