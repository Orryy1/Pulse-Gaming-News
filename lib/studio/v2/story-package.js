...
const { resolveStudioDbPath } = require("./studio-db-path");
...
async function activeStudioMediaRoots(storyId) {
  const roots = studioMediaRoots();
  const mediaRoot = String(process.env.MEDIA_ROOT || "").trim();
...
async function resolvedStoryMediaPath(storedPath) {
  if (!storedPath) return null;
  const resolved = await mediaPaths.resolveExisting(storedPath);
  return resolved && (await fs.pathExists(resolved)) ? resolved : null;
}
...
async function buildMediaInventory(storyId, story = null) {
  const roots = await activeStudioMediaRoots(storyId);

  const trailerClips = [];
  const seenClips = new Set();
  for (const root of roots) {
    const vc = path.join(root, "output", "video_cache");
    if (!(await fs.pathExists(vc))) continue;
    const files = await fs.readdir(vc);
    for (const f of files) {
      if (
        f.startsWith(`${storyId}_`) &&
        ...
        try {
          ...
          addUnique(trailerClips, seenClips, {
            path: p,
            durationS: d,
            source: f.includes("_clip_") ? "trailer" : "story-video-clip",
          });
        } catch {}
      }
    }
  }
  for (const clipPath of Array.isArray(story?.video_clips)
    ? story.video_clips
    : []) {
    const p = await resolvedStoryMediaPath(clipPath);
    if (!p) continue;
    try {
      const d = parseFloat(
        execSync(
  ...
  function addStillFromFile(p, f) {
    if (f.includes("_trailerframe_")) {
      addUnique(trailerFrames, seenStills, { path: p, source: "trailer-frame" });
    } else if (f.includes("_article_inline")) {
      addUnique(articleInline, seenStills, { path: p, source: "article-inline" });
    } else if (f.includes("_article")) {
      addUnique(articleHeroes, seenStills, { path: p, source: "article-hero" });
    } else if (
      f.includes("_pexels") ||
      f.includes("_unsplash") ||
      f.includes("_bing")
    ) {
      addUnique(stockFiller, seenStills, { path: p, source: "stock" });
    } else if (f.includes("_steam") || f.includes("_igdb")) {
      addUnique(articleHeroes, seenStills, { path: p, source: "topical-art" });
    }
  }

  for (const root of roots) {
    const cache = path.join(root, "output", "image_cache");
    if (!(await fs.pathExists(cache))) continue;
    ...
    for (const f of files) {
      ...
      addStillFromFile(path.join(cache, f), f.toLowerCase());
    }
  }
  ...
  for (const img of Array.isArray(story?.downloaded_images)
    ...
    : []) {
    const p = await resolvedStoryMediaPath(img?.path);
    if (!p) continue;
    addStillFromFile(p, path.basename(p).toLowerCase());
  }

  if (story?.image_path) {
    const p = await resolvedStoryMediaPath(story.image_path);
    if (p) {
      addUnique(articleHeroes, seenStills, {
        path: p,
...
async function buildStoryPackage(storyId, opts = {}) {
  ...
  // Media inventory + diversity score
  const inventory = await buildMediaInventory(storyId, story);
  ...
  candidateScenes.push({
    type: "card.quote",
    available: hasVerifiedRedditReaction(story),
  });
  candidateScenes.push({
    type: "card.release",
    available: true,
  ...
  if (hasVerifiedRedditReaction(story)) {
    quoteCandidates.push({
      ...
      score: Number(story.top_comment_score) || null,
      subreddit: story.reddit_discussion?.subreddit || story.subreddit || null,
    });
  }
