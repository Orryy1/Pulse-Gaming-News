...
function resolveReportPath(reportPath) {
  return path.isAbsolute(String(reportPath || ""))
    ? reportPath
    : path.resolve(ROOT, String(reportPath || ""));
}
...
async function loadGateInputFromGauntlet({
  ...
  const canonicalSummary = requireCanonical
    ? find(`${storyId}:canonical`)
    : null;
  if (!candidateSummary) {
    throw new Error(`candidate not found in gauntlet: ${storyId}:${variant}`);
  }
  if (requireCanonical && !canonicalSummary) {
    throw new Error(`canonical not found in gauntlet: ${storyId}:canonical`);
  }
  const candidateReport = await fs.readJson(
    resolveReportPath(candidateSummary.paths.studioReport),
  );
  const canonicalReport = canonicalSummary
    ? await fs.readJson(resolveReportPath(canonicalSummary.paths.studioReport))
    : null;
  return {
    gauntlet,
    ...
    calibration: requireCanonical
      ? gauntlet.candidates
          ...
          .map((summary) => ({
            summary,
            reportPath: resolveReportPath(summary.paths.studioReport),
          }))
      : [],
  };
}
...
async function runStudioGateV21({
  ...
  const fullReport = {
    ...
    storyId,
    variant,
    canonicalKey: input.canonical?.key || null,
  ...
  const shouldWriteRootMarkdown =
    path.resolve(outputDir) === path.resolve(TEST_OUT);
  if (shouldWriteRootMarkdown) {
    await fs.writeFile(rootMdPath, markdown);
  }
  fullReport.outputs = {
    json: rel(jsonPath),
    markdown: rel(mdOutputPath),
    rootMarkdown: shouldWriteRootMarkdown ? rel(rootMdPath) : null,
  };
  await fs.writeJson(jsonPath, fullReport, { spaces: 2 });
  return fullReport;
}
