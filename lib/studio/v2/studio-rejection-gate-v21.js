...
function resolvePremiumShellGate({ summary = null, report = null } = {}) {
  const studio = summary?.studio || {};
  const premiumLane = report?.premiumLane || {};
  const shellGate =
    premiumLane.hyperframesPremiumShellGate ||
    premiumLane.premiumShellGate ||
    studio.hyperframesPremiumShellGate ||
    studio.premiumShellGate ||
    {};
  const verdict =
    studio.premiumShellVerdict ||
    premiumLane.premiumShellVerdict ||
    shellGate.verdict ||
    "unknown";
  const passCount =
    studio.premiumShellPassCount ??
    premiumLane.premiumShellPassCount ??
    shellGate.passCount ??
    0;
  return {
    verdict,
    passCount: Number(passCount || 0),
    blockers: shellGate.blockers || [],
    requiredPassCount:
      shellGate.requiredPassCount || MIN_PREMIUM_HYPERFRAMES_CARDS,
  };
}
...
function normaliseGateCandidate({ key, summary = null, report = null } = {}) {
  ...
  const premiumLane = report?.premiumLane || {};
  const premiumShellGate = resolvePremiumShellGate({ summary, report });
  ...
  return {
    ...
    audioPresence: forensic.audioPresence || "unknown",
    subtitleVerdict: forensic.subtitleVerdict || "unknown",
    subtitleDensity: forensic.subtitleDensity || "unknown",
    ...
    premiumShellVerdict: premiumShellGate.verdict,
    premiumShellPassCount: premiumShellGate.passCount,
    premiumShellRequiredPassCount: premiumShellGate.requiredPassCount,
    premiumShellBlockers: premiumShellGate.blockers,
...
function evaluateStudioRejectionGate({
  ...
  if (candidate.audioPresence && candidate.audioPresence !== "pass") {
    addReason(
      candidate.audioPresence === "fail" ? hardFails : warnings,
      "audio_presence",
      "Audio presence QA did not find a reliable narration track.",
      { audioPresence: candidate.audioPresence },
    );
  } else if (candidate.audioPresence === "pass") {
    greenSignals.push("audio_presence_pass");
  }
  ...
  if (candidate.subtitleDensity && candidate.subtitleDensity !== "pass") {
    addReason(
      candidate.subtitleDensity === "fail" ? hardFails : warnings,
      "subtitle_density",
      "Caption density is too heavy for a high-retention short.",
      { subtitleDensity: candidate.subtitleDensity },
    );
  } else if (candidate.subtitleDensity === "pass") {
    greenSignals.push("subtitle_density_pass");
  }
  ...
  const premiumShellMissing =
    candidate.premiumShellMissing === true ||
    (candidate.hyperframesCardCount >= MIN_PREMIUM_HYPERFRAMES_CARDS &&
      candidate.premiumLaneVerdict === "pass" &&
      candidate.premiumShellVerdict !== "pass");
  ...
  } else if (premiumShellMissing) {
    addReason(
      hardFails,
      "premium_shell_missing",
      "HyperFrames premium shell proof did not pass.",
      {
        premiumShellVerdict: candidate.premiumShellVerdict,
        premiumShellPassCount: candidate.premiumShellPassCount,
        premiumShellRequiredPassCount: candidate.premiumShellRequiredPassCount,
        premiumShellBlockers: candidate.premiumShellBlockers,
      },
    );
  } else if (candidate.hyperframesCardCount >= MIN_PREMIUM_HYPERFRAMES_CARDS) {
    greenSignals.push("premium_card_lane_pass");
    greenSignals.push("premium_shell_pass");
  }
