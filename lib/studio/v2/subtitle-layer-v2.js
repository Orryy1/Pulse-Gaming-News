 ...
 *     consecutive misses.
 *
 *   - Caption position: 70% down the frame (y=1342 of 1920) to
 *     stay clear of lower-third branding, baked-in source captions
 *     and bottom-edge cutoff risk.
 */
...
const EDITORIAL_WORDS_PER_PHRASE = 5;
const EDITORIAL_PHRASE_MAX_CHARS = 32;
...
Style: Pop,Impact,82,&H00FFFFFF,&H00FFFFFF,&H00000000,&HC8000000,-1,0,0,0,100,100,1,0,1,5,3,2,66,66,430,1
Style: PopEmphasis,Impact,92,${bgr},${bgr},&H00000000,&HC8000000,-1,0,0,0,100,100,1,0,1,5,3,2,66,66,430,1
...
function tryMatchDecimalCurrencyExpansion(tokens, ti, displayToken) {
  const units = currencyUnitWordsForDisplayToken(displayToken);
  if (!units) return null;
  const body = String(displayToken || "")
    .trim()
    .replace(/^US\$/i, "")
    .replace(/^\$/, "")
    .replace(/[,:;!?]+$/g, "");
  ...
  const majorMatch = tryMatchNumberExpansion(tokens, ti, major);
  if (!majorMatch) return null;
  const unitIndex = consumeTrailingCurrencyUnit(tokens, majorMatch.lastIdx, units);
  if (unitIndex === majorMatch.lastIdx) return null;
  const minorMatch = tryMatchNumberExpansion(tokens, unitIndex + 1, minor);
  return minorMatch ? { lastIdx: minorMatch.lastIdx } : null;
}
...
function tryMatchRomanNumeralExpansion(tokens, ti, roman) {
  if (ti >= tokens.length) return null;
  const aliases = ROMAN_TITLE_SPOKEN[String(roman || "").toLowerCase()];
  if (!aliases) return null;
  return aliases.has(normWord(tokens[ti]?.word)) ? { lastIdx: ti } : null;
}
...
const PROTECTED_PHRASE_ALIASES = [
  ...
  },
  {
    ...
    spoken: [
      ["denshattack"],
      { words: ["densha", "attack"], segments: [2] },
    ],
  },
  {
  ...
  {
    display: ["black", "flag", "resynced"],
    spoken: [["black", "flag", "reesynced"]],
  },
  {
    display: ["gta", "5"],
    spoken: [
      { words: ["g", "t", "a", "five"], segments: [3, 1] },
      ["gta", "five"],
      { words: ["grand", "theft", "auto", "five"], segments: [3, 1] },
    ],
  },
  {
    display: ["gta", "v"],
  ...
  },
  {
    display: ["gta", "6"],
    spoken: [
      { words: ["g", "t", "a", "six"], segments: [3, 1] },
      ["gta", "six"],
      { words: ["grand", "theft", "auto", "six"], segments: [3, 1] },
    ],
  },
  {
    display: ["gta", "vi"],
  ...
  },
  {
    ...
    spoken: [["playstation", "four"]],
  },
  {
    display: ["playstation", "5"],
    spoken: [["playstation", "five"]],
  },
  {
    display: ["rtx", "2060era"],
    spoken: [
      { words: ["rtx", "twenty", "sixty", "era"], segments: [1, 3] },
      { words: ["rtx", "twenty", "sixty"], segments: [1, 2] },
    ],
  },
  {
    display: ["rtx", "2060"],
    spoken: [{ words: ["rtx", "twenty", "sixty"], segments: [1, 2] }],
  },
];
...
function tryMatchCanonicalRomanTitleExpansion(scriptTokens, scriptIndex, timestamps, ti) {
  ...
  const previousTitleToken = normWord(String(scriptTokens[scriptIndex - 1] || ""));
  const spoken = ROMAN_TITLE_SPOKEN[cleanScript] || NUMERIC_TITLE_SPOKEN[cleanScript];
  ...
  if ((cleanScript === "ii" || (cleanScript === "2" && previousTitleToken === "hades")) && first === "sequel") {
    return { lastIdx: ti };
  }
  return null;
}
...
function tryMatchProtectedPhraseAlias(scriptTokens, scriptIndex, timestamps, ti) {
  for (const phrase of PROTECTED_PHRASE_ALIASES) {
    ...
    for (const spokenEntry of phrase.spoken || []) {
      const spoken = Array.isArray(spokenEntry) ? spokenEntry : spokenEntry?.words;
      if (!Array.isArray(spoken) || ti + spoken.length > timestamps.length) continue;
      ...
      const segmentCounts = Array.isArray(spokenEntry?.segments) &&
        spokenEntry.segments.length === display.length
        ? spokenEntry.segments.map((value) => Math.max(1, Number(value) || 1))
        : display.map(() => 1);
      const alignments = [];
      let cursor = ti;
      for (const count of segmentCounts) {
        alignments.push({
          startIdx: cursor,
          endIdx: Math.min(ti + spoken.length - 1, cursor + count - 1),
        });
        cursor += count;
      }
      return {
...
function isProtectedTitleSuffixMerge(buffer, nextWord) {
  if (!Array.isArray(buffer) || buffer.length === 0) return false;
  const suffix = normWord(nextWord?.word);
  if (!suffix) return false;
  if (!ROMAN_TITLE_SPOKEN[suffix] && !NUMERIC_TITLE_SPOKEN[suffix]) return false;
  const prefixWords = buffer.map((word) => normWord(word?.word)).filter(Boolean);
  if (!prefixWords.length) return false;
  return ROMAN_TITLE_PREFIXES.some(
    (prefix) =>
      prefix.length === prefixWords.length &&
      prefix.every((part, index) => prefixWords[index] === part),
  );
}

function phraseTailMatches(tokens, phrase) {
  if (!Array.isArray(tokens) || !Array.isArray(phrase) || tokens.length < phrase.length) {
    return false;
  }
  const start = tokens.length - phrase.length;
  return phrase.every((part, index) => tokens[start + index] === part);
}

function isProtectedCaptionContinuation(buffer, nextWord) {
  const tokens = [
    ...((Array.isArray(buffer) ? buffer : []).map((word) => normWord(word?.word))),
    normWord(nextWord?.word),
  ].filter(Boolean);
  if (tokens.length < 2) return false;
  return PROTECTED_CAPTION_PHRASES.some((phrase) => phraseTailMatches(tokens, phrase));
}

function isProtectedCaptionPrefix(buffer) {
  const tokens = (Array.isArray(buffer) ? buffer : [])
    .map((word) => normWord(word?.word))
    .filter(Boolean);
  if (!tokens.length) return false;
  return PROTECTED_CAPTION_PHRASES.some((phrase) => {
    if (tokens.length >= phrase.length) return false;
    const prefixMatch = tokens.every((part, index) => phrase[index] === part);
    const suffixStart = phrase.length - tokens.length;
    const suffixMatch = tokens.every((part, index) => phrase[suffixStart + index] === part);
  ...
  for (let scriptIndex = 0; scriptIndex < scriptTokens.length; scriptIndex += 1) {
    ...
    if (protectedPhraseMatch) {
      for (let offset = 0; offset < protectedPhraseMatch.scriptTokenCount; offset += 1) {
        const alignment = protectedPhraseMatch.alignments?.[offset];
        const startIdx = alignment?.startIdx ?? ti + offset;
        const endIdx = alignment?.endIdx ?? ti + offset;
        out.push({
          word: scriptTokens[scriptIndex + offset],
          start: timestamps[startIdx].start,
          end: timestamps[endIdx].end,
        });
      }
    ...
    const romanNumeralMatch = tryMatchRomanNumeralExpansion(timestamps, ti, lcScript);
    if (romanNumeralMatch) {
      out.push({
        word: scriptToken,
        start: timestamps[ti].start,
        end: timestamps[romanNumeralMatch.lastIdx].end,
      });
      ti = romanNumeralMatch.lastIdx + 1;
      consecutiveMisses = 0;
      continue;
    }
    ...
    const decimalCurrencyMatch = tryMatchDecimalCurrencyExpansion(
      timestamps,
      ti,
      scriptToken,
    );
    if (decimalCurrencyMatch) {
      out.push({
        word: scriptToken,
        start: timestamps[ti].start,
        end: timestamps[decimalCurrencyMatch.lastIdx].end,
      });
      ti = decimalCurrencyMatch.lastIdx + 1;
      consecutiveMisses = 0;
      continue;
    }
...
function planCaptionDensity(words, opts = {}) {
  ...
  for (const w of words) {
    ...
    const protectedTitleSuffixMerge = isProtectedTitleSuffixMerge(buffer, w);
    const protectedCaptionContinuation = isProtectedCaptionContinuation(buffer, w);
    if (
      buffer.length > 0 &&
      !protectedTitleSuffixMerge &&
      !protectedCaptionContinuation &&
    ...
    previousEnd = w.end;
    if ((buffer.length >= wordsPerCaption && !isProtectedCaptionPrefix(buffer)) || endsSentence) {
      flush();
    }
  }
  flush();
  ...
  return mergeDanglingPhrases(out, {
    ...
    minPhraseDurationS: Number(opts.minPhraseDurationS) || PHRASE_MIN_DURATION_S,
  });
}
...
function groupIntoPhrases(words, opts = {}) {
  return planCaptionDensity(words, {
    ...
    minPhraseDurationS: opts.minPhraseDurationS,
    preferOneLine: true,
  });
}
...
function mergeSingleWordSentenceTailWithBoundedShift(left, right, opts) {
  if (!left || right?.words?.length !== 1 || !phraseEndsSentence(right)) return null;
  const words = [...(left.words || []), ...(right.words || [])];
  if (words.length > (opts.danglingMergeMaxWords || opts.wordsPerPhrase)) return null;
  if (words.map((word) => word.word).join(" ").length > opts.phraseMaxChars) return null;
  const gap = Number(right.start) - Number(left.end);
  if (Number.isFinite(gap) && gap > MAX_WORD_GAP_S) return null;
  const maxDuration = Number(opts.maxPhraseDurationS);
  const minDuration = Number(opts.minPhraseDurationS) || PHRASE_MIN_DURATION_S;
  const combinedDuration = Number(right.end) - Number(left.start);
  if (
    !Number.isFinite(maxDuration) ||
    maxDuration <= 0 ||
    !Number.isFinite(combinedDuration) ||
    combinedDuration > maxDuration + minDuration
  ) {
    return null;
  }
  const merged = mergePair(left, right);
  if (combinedDuration > maxDuration) {
    merged.start = Number(right.end) - maxDuration;
  }
  return merged;
}

function rebalanceSingleWordSentenceTail(previous, current, opts) {
  const previousWords = [...(previous?.words || [])];
  const currentWords = [...(current?.words || [])];
  if (previousWords.length < 3 || currentWords.length !== 1) return null;

  const moved = previousWords.pop();
  const left = {
    words: previousWords,
    start: previous.start,
    end: previousWords[previousWords.length - 1].end,
  };
  const right = {
    words: [moved, ...currentWords],
    start: moved.start,
    end: current.end,
  };
  const maxDuration = Number(opts.maxPhraseDurationS);
  const withinDuration = (phrase) =>
    !Number.isFinite(maxDuration) ||
    maxDuration <= 0 ||
    Number(phrase.end) - Number(phrase.start) <= maxDuration;
  const withinText = (phrase) => phraseText(phrase).length <= opts.phraseMaxChars;
  const withinWords = (phrase) => phrase.words.length <= opts.wordsPerPhrase;
  const gap = Number(right.start) - Number(left.end);

  if (
    withinDuration(left) &&
    withinDuration(right) &&
    withinText(left) &&
    withinText(right) &&
    withinWords(left) &&
    withinWords(right) &&
    (!Number.isFinite(gap) || gap <= MAX_WORD_GAP_S)
  ) {
    return { left, right };
  }
  return null;
...
const DANGLING_FUNCTION_WORD_RE = /^(?:a|an|the|and|or|to|of|for|with|in|on)$/i;
const WEAK_SEMANTIC_TAIL_RE =
  /^(?:am|are|be|been|being|can|could|did|do|does|had|has|have|is|may|might|must|shall|should|was|were|will|would)$/i;

function phraseEndsWithDanglingFunctionWord(phrase) {
  const last = phrase?.words?.[phrase.words.length - 1]?.word || "";
  return DANGLING_FUNCTION_WORD_RE.test(normWord(last));
}

function phraseEndsWithWeakSemanticTail(phrase) {
  const last = phrase?.words?.[phrase.words.length - 1]?.word || "";
  const normalised = normWord(last);
  return (
    DANGLING_FUNCTION_WORD_RE.test(normalised) ||
    WEAK_SEMANTIC_TAIL_RE.test(normalised)
  );
}

function phraseFitsPremiumBounds(phrase, opts) {
  const words = Array.isArray(phrase?.words) ? phrase.words : [];
  if (!words.length) return false;
  if (words.length > opts.wordsPerPhrase) return false;
  if (phraseText(phrase).length > opts.phraseMaxChars) return false;
  const maxDuration = Number(opts.maxPhraseDurationS);
  if (
    Number.isFinite(maxDuration) &&
    maxDuration > 0 &&
    Number.isFinite(Number(phrase.start)) &&
    Number.isFinite(Number(phrase.end)) &&
    Number(phrase.end) - Number(phrase.start) > maxDuration
  ) {
    return false;
  }
...
function boundShiftedFunctionPhraseDuration(phrase, opts) {
  if (phraseFitsPremiumBounds(phrase, opts)) return phrase;
  const maxDuration = Number(opts.maxPhraseDurationS);
  const minDuration = Number(opts.minPhraseDurationS) || PHRASE_MIN_DURATION_S;
  const duration = Number(phrase?.end) - Number(phrase?.start);
  if (
    !Number.isFinite(maxDuration) ||
    maxDuration <= 0 ||
    !Number.isFinite(duration) ||
    duration > maxDuration + minDuration ||
    (Array.isArray(phrase?.words) && phrase.words.length > opts.wordsPerPhrase) ||
    phraseText(phrase).length > opts.phraseMaxChars
  ) {
    return null;
  }
  return {
    ...phrase,
    start: Number(phrase.end) - maxDuration,
  };
}

function splitWordsIntoPremiumPhrases(words, opts) {
  const sourceWords = [...(Array.isArray(words) ? words : [])];
  if (sourceWords.length < 4) return null;
  const minDuration = Number(opts.minPhraseDurationS) || PHRASE_MIN_DURATION_S;
  const candidates = [];
  for (let splitIndex = 2; splitIndex <= sourceWords.length - 2; splitIndex += 1) {
    const leftWords = sourceWords.slice(0, splitIndex);
    const rightWords = sourceWords.slice(splitIndex);
    if (leftWords.some((word) => /[.!?]$/.test(String(word?.word || "")))) continue;
    const left = {
      words: leftWords,
    ...
    if (
      !phraseFitsPremiumBounds(left, opts) ||
      !phraseFitsPremiumBounds(right, opts) ||
      phraseEndsWithDanglingFunctionWord(left) ||
      phraseEndsWithDanglingFunctionWord(right) ||
      Number(left.end) - Number(left.start) + 0.001 < minDuration ||
      Number(right.end) - Number(right.start) + 0.001 < minDuration
    ) {
      continue;
    }
    candidates.push({
      ...
      right,
      score:
        Math.abs(leftWords.length - rightWords.length) * 10 +
        Math.abs(
          Number(left.end) -
            Number(left.start) -
...
function reflowPrependedWordsThroughFollowingPhrases(phrases, phraseIndex, prependedWords, opts) {
  const working = (Array.isArray(phrases) ? phrases : []).map((phrase) => ({
    ...phrase,
    words: [...(Array.isArray(phrase?.words) ? phrase.words : [])],
  }));
  let carry = [...(Array.isArray(prependedWords) ? prependedWords : [])];
  for (let index = phraseIndex; carry.length && index < working.length; index += 1) {
    const currentWords = [...carry, ...working[index].words];
    const overflow = [];
    let bounded = null;
    while (currentWords.length) {
      ...
      bounded = boundShiftedFunctionPhraseDuration(candidate, opts);
      if (bounded && !phraseEndsWithDanglingFunctionWord(bounded)) break;
      const moved = currentWords.pop();
      overflow.unshift(moved);
      bounded = null;
    }
    if (!bounded || !currentWords.length) return null;
    if (overflow.some((word) => /[.!?]$/.test(String(word?.word || "")))) return null;
    working[index] = bounded;
    carry = overflow;
  }
  return carry.length ? null : working;
}

function rebalanceDanglingFunctionWordTails(phrases, opts) {
  const out = (Array.isArray(phrases) ? phrases : []).map((phrase) => ({
    ...phrase,
    words: [...(Array.isArray(phrase?.words) ? phrase.words : [])],
  }));
  for (let index = 0; index < out.length - 1; index += 1) {
    let left = out[index];
    let right = out[index + 1];
    while (left.words.length > 1 && phraseEndsWithDanglingFunctionWord(left)) {
      const moved = left.words[left.words.length - 1];
      const nextLeftWords = left.words.slice(0, -1);
      const candidateLeft = {
        words: nextLeftWords,
        start: left.start,
        end: nextLeftWords[nextLeftWords.length - 1].end,
      };
      const candidateRight = {
        words: [moved, ...right.words],
        start: moved.start,
        end: right.end,
      };
      let boundedRight = boundShiftedFunctionPhraseDuration(candidateRight, opts);
      let reflowed = null;
      if (
        !boundedRight &&
        index + 2 < out.length
      ) {
        reflowed = reflowPrependedWordsThroughFollowingPhrases(
          out,
          index + 1,
          [moved],
          opts,
        );
        boundedRight = reflowed?.[index + 1] || null;
      }
      const splitRight = boundedRight
        ? null
        : splitWordsIntoPremiumPhrases(candidateRight.words, opts);
      const nextRight = splitRight?.left || boundedRight;
      const gap = Number(nextRight?.start) - Number(candidateLeft.end);
      if (
        !phraseFitsPremiumBounds(candidateLeft, opts) ||
        !nextRight ||
        (Number.isFinite(gap) && gap > MAX_WORD_GAP_S)
      ) {
        break;
      ...
      out[index] = left;
      if (splitRight) {
        out[index + 1] = splitRight.left;
        out.splice(index + 2, 0, splitRight.right);
      } else if (reflowed) {
        for (let reflowIndex = index + 1; reflowIndex < out.length; reflowIndex += 1) {
          out[reflowIndex] = reflowed[reflowIndex];
        }
        right = out[index + 1];
      } else {
        out[index + 1] = right;
...
function plannedPhraseDwell(phrase, nextPhrase, opts) {
  const minDuration = Number(opts.minPhraseDurationS) || PHRASE_MIN_DURATION_S;
  const maxDuration = Number(opts.maxPhraseDurationS);
  const start = Number(phrase?.start);
  let end = Number(phrase?.end);
  if (!Number.isFinite(start) || !Number.isFinite(end)) return 0;
  if (Number.isFinite(maxDuration) && maxDuration > 0 && end - start > maxDuration) {
    end = start + maxDuration;
  }
  if (end - start < minDuration) end = start + minDuration;
  const nextStart = Number(nextPhrase?.start);
...
function rebalanceShortPhraseDwells(phrases, opts) {
  const out = (Array.isArray(phrases) ? phrases : []).map((phrase) => ({
    ...phrase,
    words: [...(Array.isArray(phrase?.words) ? phrase.words : [])],
  }));
  ...
  for (let index = 1; index < out.length; index += 1) {
    ...
    if (plannedPhraseDwell(current, out[index + 1], opts) + 0.001 >= minDuration) {
      continue;
    }
    const previous = out[index - 1];
    if (
      phraseEndsSentence(previous) ||
    ...
    const movedWords = [];
    let repair = null;
    while (previousWords.length > 2) {
      const moved = previousWords.pop();
      movedWords.unshift(moved);
      if (/[.!?]$/.test(String(moved?.word || ""))) break;
      const candidatePrevious = {
        words: [...previousWords],
        start: previousWords[0].start,
        end: previousWords[previousWords.length - 1].end,
      };
      const candidateCurrent = {
        words: [...movedWords, ...currentWords],
        start: movedWords[0].start,
        end: current.end,
      };
      if (
        phraseFitsPremiumBounds(candidatePrevious, opts) &&
        phraseFitsPremiumBounds(candidateCurrent, opts) &&
        !phraseEndsWithWeakSemanticTail(candidatePrevious) &&
        !phraseEndsWithDanglingFunctionWord(candidateCurrent) &&
        plannedPhraseDwell(candidateCurrent, out[index + 1], opts) + 0.001 >=
          minDuration
      ) {
        repair = {
    ...
    if (repair) {
      out[index - 1] = repair.previous;
      out[index] = repair.current;
    }
  }
  return out;
}

function mergeShortDanglingPhrases(phrases, opts) {
  const out = [];
  for (let i = 0; i < phrases.length; i++) {
    ...
    if (canMergePhrases(previous, current, opts)) {
      ...
      const mergedSentenceTail = mergeSingleWordSentenceTailWithBoundedShift(
        previous,
        current,
        opts,
      );
      if (mergedSentenceTail) {
        out[out.length - 1] = mergedSentenceTail;
        continue;
      }
      const balanced = rebalanceSingleWordSentenceTail(previous, current, opts);
      if (balanced) {
        out[out.length - 1] = balanced.left;
        out.push(balanced.right);
      } else {
        out.push(current);
      }
    }
...
function mergeDanglingPhrases(phrases, opts) {
  let repaired = mergeShortDanglingPhrases(phrases, opts);
  repaired = rebalanceDanglingFunctionWordTails(repaired, opts);
  repaired = mergeShortDanglingPhrases(repaired, opts);
  repaired = rebalanceDanglingFunctionWordTails(repaired, opts);
  repaired = rebalanceShortPhraseDwells(repaired, opts);
  return rebalanceDanglingFunctionWordTails(repaired, opts);
}
  ...
  if (
    ...
    const fallbackText =
      !transcriptMismatch && strictEndCoverage === false && cleanWords.length > 0
        ...
        : displayScriptText;
    const fallback = normaliseCaptionDisplayWords(wordsFromScriptText(fallbackText, duration), {
      textKey: "word",
    });
    if (fallback.length > 0) return fallback;
  }
  return normaliseCaptionDisplayWords(cleanWords, { textKey: "word" });
}
...
function buildWordPopDialogues(phrases, storyTokens, options = {}) {
  ...
  for (let phraseIndex = 0; phraseIndex < phrases.length; phraseIndex++) {
    ...
    const segments =
      motionStyle === "flash"
        ? [
            "{\\an2\\move(540,1378,540,1342,0,130)\\fad(35,70)\\bord6\\shad4\\t(0,130,\\fscx106\\fscy106)\\t(130,260,\\fscx100\\fscy100)}",
          ]
        : motionStyle === "editorial"
          ? [
              "{\\an2\\move(540,1372,540,1346,0,170)\\fad(45,90)\\bord6\\shad4\\t(0,170,\\fscx104\\fscy104)\\t(170,340,\\fscx100\\fscy100)}",
            ]
        : [];
...
function buildKineticAss({
  ...
  const subtitleScriptText = normaliseCaptionDisplayText(
    scriptText ||
  ...
  const realigned =
    realign === false
      ? words || []
      : realignTimestampsToScript(subtitleScriptText, words || []);
  ...
  const safeMaxWordsPerPhrase =
    motionStyle === "flash"
      ...
      : motionStyle === "editorial"
        ? Math.min(Number(maxWordsPerPhrase) || WORDS_PER_PHRASE, EDITORIAL_WORDS_PER_PHRASE)
      : maxWordsPerPhrase;
  const safeMaxPhraseChars =
    motionStyle === "flash"
      ? Math.min(Number(maxPhraseChars) || PHRASE_MAX_CHARS, FLASH_PHRASE_MAX_CHARS)
      : motionStyle === "editorial"
        ? Math.min(Number(maxPhraseChars) || PHRASE_MAX_CHARS, EDITORIAL_PHRASE_MAX_CHARS)
      : maxPhraseChars;
...
module.exports = {
  ...
  EDITORIAL_WORDS_PER_PHRASE,
  EDITORIAL_PHRASE_MAX_CHARS,
};
