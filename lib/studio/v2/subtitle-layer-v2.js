...
function planCaptionDensity(words, opts = {}) {
  ...
  for (const w of words) {
    ...
    if (
      buffer.length > 0 &&
      (buffer.length >= wordsPerCaption || nextText.length > charsPerCaption)
    ) {
      flush();
    }
...
function hasCorruptTimings(words, duration) {
  ...
  const leadingGap =
    dur > 0 && Number.isFinite(firstStart) ? firstStart : 0;
  const trailingGap =
    dur > 0 && Number.isFinite(lastEnd) ? dur - lastEnd : 0;
  const maxEdgeGap = dur > 0 ? Math.max(2.5, dur * 0.12) : Infinity;
  return (
    invalid > 0 ||
    hugeGaps > 1 ||
    clampRatio > 0.2 ||
    leadingGap > maxEdgeGap ||
    trailingGap > maxEdgeGap
  );
}
...
function prepareSubtitleWords({ words, duration, scriptText }) {
  ...
  if (hasCorruptTimings(cleanWords, duration) || hasLowTranscriptCoverage(cleanWords, scriptText)) {
