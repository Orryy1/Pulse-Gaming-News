...
function buildAssHeader(emphasisHex, captionLayout = null) {
  const bgr = emphasisHex ? hexToAssBgr(emphasisHex) : DEFAULT_EMPHASIS_BGR;
  const layout = captionLayout || {};
  const playResX = Number(layout?.canvas?.width) || 1080;
  const playResY = Number(layout?.canvas?.height) || 1920;
  const fontSize = Number(layout.font_size) || 96;
  const emphasisFontSize =
    Number(layout.emphasis_font_size) || 108;
  const outline = Number(layout.outline) || 6;
  const shadow = Number(layout.shadow) || 4;
  const alignment = Number(layout?.anchor?.alignment) || 2;
  const marginLeft = Number(layout.margin_left) || 40;
  const marginRight = Number(layout.margin_right) || 40;
  const marginVertical = Number(layout.margin_vertical) || 250;
  return `[Script Info]
ScriptType: v4.00+
PlayResX: ${playResX}
PlayResY: ${playResY}
...
Style: Pop,Impact,${fontSize},&H00FFFFFF,&H00FFFFFF,&H00000000,&HC8000000,-1,0,0,0,100,100,2,0,1,${outline},${shadow},${alignment},${marginLeft},${marginRight},${marginVertical},1
Style: PopEmphasis,Impact,${emphasisFontSize},${bgr},${bgr},&H00000000,&HC8000000,-1,0,0,0,100,100,2,0,1,${outline},${shadow},${alignment},${marginLeft},${marginRight},${marginVertical},1
...
function buildWordPopDialogues(phrases, storyTokens, options = {}) {
  const events = [];
  const layout = options.captionLayout || null;
  const anchorTag = layout
    ? `{\\an${Number(layout?.anchor?.alignment) || 2}\\pos(${Number(layout?.anchor?.x)},${Number(layout?.anchor?.y)})}`
    : "";
  ...
  for (const phrase of phrases) {
    ...
    const text = `${anchorTag}${segments.join("").trimEnd()}`;
    events.push(
...
function buildKineticAss({
  ...
  const header = captionLayout
    ? buildAssHeader(emphasisHex, captionLayout)
    : emphasisHex
      ? buildAssHeader(emphasisHex)
      : ASS_HEADER;
  return `${header}\n${safeEvents.join("\n")}\n`;
}
...
function extractAssDialogueText(ass) {
  return String(ass || "")
    ...
    .map((line) => stripAssTags(line.split(",,").slice(1).join(",,") || ""))
    .filter(Boolean);
}
