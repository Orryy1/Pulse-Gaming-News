...
const MIN_READABLE_INFO_CARD_DURATION_S = 5.5;
const MAX_READABLE_INFO_CARD_DURATION_S = 7.5;
...
function isInformationOverlay(event = {}) {
  return /^(?:steam_chart|review_score_card|ranking_snap|source_lock|caveat_card|price_snap)$/.test(
    String(event.kind || ""),
  );
}

function readableOverlayDuration(event = {}) {
  const raw = Number(event.durationS ?? event.duration_s ?? event.duration);
  if (isInformationOverlay(event)) {
    return clamp(
      Number.isFinite(raw) && raw > 0 ? raw : MIN_READABLE_INFO_CARD_DURATION_S,
      MIN_READABLE_INFO_CARD_DURATION_S,
      MAX_READABLE_INFO_CARD_DURATION_S,
    );
  }
  return clamp(Number.isFinite(raw) && raw > 0 ? raw : 2.4, 1.2, 5.5);
}
...
function fitEventIntoWindows(event, desiredStart, durationS, safeWindows = []) {
  const minDuration = isInformationOverlay(event) ? MIN_READABLE_INFO_CARD_DURATION_S : 0.8;
  const duration = Math.max(minDuration, Number(durationS) || minDuration);
  for (const window of safeWindows) {
    ...
    if (endLimit - startLimit < minDuration) continue;
    if (desiredStart > endLimit - 0.35) continue;
    const fittedDuration = Math.min(duration, endLimit - Math.max(desiredStart, startLimit));
    if (fittedDuration < minDuration) continue;
...
function placeEventsInWindows(events, durationS, safeWindows = []) {
  ...
  for (const event of sorted) {
    const duration = readableOverlayDuration(event);
...
function buildVisualV3OverlayFilter({
  ...
  const normalisedEvents = events.map((event) => ({
    ...event,
    durationS: readableOverlayDuration({
      ...event,
