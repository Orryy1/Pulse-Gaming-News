...
function modelSetupStatus(model, { env = process.env, localTtsDoctorReport = {} } = {}) {
  ...
  if (model.id === "voicebox_local") {
    const config = resolveVoiceboxConfig(env);
    if (truthy(env.VOICEBOX_TTS_ENABLED) || isVoiceboxLocalTtsEngine(env)) {
      return config.profileId || config.profileName
        ? "configured_optional"
        : "missing_voicebox_profile";
    }
    return "not_detected_optional_setup";
  }
...
function localLiamVoiceId({ env = process.env, voiceId = null, localTtsDoctorReport = {} } = {}) {
  ...
  return canonicalLocalTtsVoiceId(
    voiceId ||
    doctorVoice ||
