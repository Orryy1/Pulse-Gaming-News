...
function buildStudioV4CanonicalPacket({
  ...
  const useAuthoritativeV4MotionPlan = motionClips.length > 0;
  const benchmarkStory = {
    ...storyForPlanning,
    downloaded_images: useAuthoritativeV4MotionPlan
      ? []
      : storyForPlanning.downloaded_images,
    game_images: useAuthoritativeV4MotionPlan ? [] : storyForPlanning.game_images,
    media_provenance: useAuthoritativeV4MotionPlan
      ? []
      : storyForPlanning.media_provenance,
  ...
  return {
    ...
    benchmark_asset_scope: useAuthoritativeV4MotionPlan
      ? {
          mode: "authoritative_visual_v4_motion_plan",
          legacy_downloaded_images_excluded: parseArray(storyForPlanning.downloaded_images).length,
          legacy_game_images_excluded: parseArray(storyForPlanning.game_images).length,
          legacy_media_provenance_excluded: parseArray(storyForPlanning.media_provenance).length,
        }
      : {
          mode: "story_asset_inventory",
          legacy_downloaded_images_excluded: 0,
          legacy_game_images_excluded: 0,
          legacy_media_provenance_excluded: 0,
        },
