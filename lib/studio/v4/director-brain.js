...
function uniqueMotionClips(footagePlan = {}, targetCount = 5) {
  ...
  for (const clip of asArray(footagePlan.motion_inventory?.accepted_local_clips)) {
    ...
    const baseSourceFamily = normaliseMotionSourceAsset(
    ...
    if (!family) continue;
    const dedupeKey = baseSourceFamily || family;
...
function motionReadinessEvidence({ footagePlan = {}, motionShots = [] } = {}) {
  ...
  const distinctSourceAssets = new Set(
    materialisedMotionShots.map((shot) =>
      normaliseMotionSourceAsset(shot.base_source_family || shot.source_family),
    ),
  );
  return {
...
function buildCoreShots({
  ...
  const shots = [
    ...
    },
    {
      ...
      durationS: readableCardDurationS("SOURCE", sourceLabel(story)),
