...
function buildMotionShots({ footagePlan = {}, durationS }) {
  ...
  const clips = uniqueMotionClips(footagePlan, minActual);
  const selected = clips.slice(0, Math.max(0, minActual));
...
function buildCoreShots({
  ...
  if (!commaMetric && !score && !price) {
    const gameplayProof = /\b(?:gameplay|trailer footage|combat|hands[-\s]?on|real footage)\b/i.test(text);
    shots.push({
      id: gameplayProof ? "gameplay_proof_card" : "source_proof_card",
      kind: "proof_card",
      startS: 4.45,
      durationS: 2.35,
      label: gameplayProof ? "REAL GAMEPLAY" : "SOURCE LOCKED",
      detail: gameplayProof ? "Show the proof before the take" : "One claim, one source",
      priority: 84,
      visual_treatment:
