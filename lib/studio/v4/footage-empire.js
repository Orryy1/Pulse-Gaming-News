...
function localMotionInventory(localMotionClips = []) {
  ...
  const officialProductMotionClips = accepted.filter((asset) => {
    ...
    return (
      ...
      reason.includes("official product motion samples passed") &&
      (rights.includes("official reference only") || rights.includes("reference only by default"))
    );
  });
...
function buildReadiness({ requirements, inventory, trustedSources }) {
  ...
  const hashDistinctWindowFamilySubstitute =
    !requirements.steam_metric_story &&
    !requirements.review_score_story &&
  ...
  if (
    ...
    !hashDistinctWindowFamilySubstitute
  ) {
  ...
  if (hashDistinctWindowFamilySubstitute) {
    warnings.push("distinct_family_floor_satisfied_by_hash_distinct_official_windows");
  }
