...
function isHardwareAccessoryProductStory(story = {}) {
  const entityIdentity = normaliseMatchText([
    story.vertical,
    story.story_type,
    story.canonical_subject,
    story.canonical_game,
  ...
  const headlineIdentity = normaliseMatchText([
    story.title,
    story.suggested_title,
    story.short_title,
    story.suggested_thumbnail_text,
  ].filter(Boolean).join(" "));
  const hardwareProductTerm =
    /\b(?:controller|headset|headphones|dualsense|accessory|accessories|peripheral|keyboard|mouse|monitor|storage|ssd|capture card|racing wheel|wheel|steam deck|playstation portal|handheld|hardware)\b/.test(identity);
  const hardwareProductIdentity =
    /\b(?:controller|headset|headphones|dualsense|accessory|accessories|peripheral|keyboard|mouse|monitor|storage|ssd|capture card|racing wheel|wheel|steam deck|playstation portal|handheld|hardware)\b/.test(identity);
  const hardwareCommercialContext =
    /\b(?:price|deal|discount|bundle|edition|preorder|pre order|accessory|accessories|stock|retail|hardware|product|upgrade|leak|leaked|revealed|announced)\b/.test(text);
  const consoleEntityIdentity =
    /\b(?:ps5 pro|playstation 5 pro|ps5|playstation 5|xbox series|switch 2|console)\b/.test(entityIdentity);
  const explicitHardwareTest =
    /\bhardware (?:test|comparison|review|benchmark)\b/.test(storyText);
  const consoleProductContext =
    (/\b(?:ps5|playstation 5|xbox series|switch 2|console)\b/.test(entityIdentity) &&
    ...
    (/\bps5 pro\b/.test(headlineIdentity) && (consoleEntityIdentity || explicitHardwareTest)) ||
    /\b(?:ps5|playstation 5) edition upgrade\b/.test(headlineIdentity);
  return hardwareProductIdentity || (hardwareProductTerm && hardwareCommercialContext) || consoleProductContext;
}
...
function materializedWindowHashSurrogates(asset = {}) {
  ...
  const officialSource =
    ...
    sourceType.includes("official game website");
  ...
  const officialStorefrontTrailerMotion =
    ...
    (
      ...
      (
        ...
        (
          reason.includes("segment samples passed") ||
          reason.includes("official direct media clean motion samples passed")
        )
      )
    ) &&
    (
...
function localMotionInventory(localMotionClips = []) {
  ...
  for (const [index, rawAsset] of asArray(localMotionClips).entries()) {
    ...
    const baseSourceFamily = cleanText(
      asset.base_source_family ||
        asset.baseSourceFamily ||
        asset.provenance?.base_source_family ||
        asset.provenance?.baseSourceFamily,
    );
    const explicitSourceAssetKey = cleanText(
      asset.source_asset_key ||
        asset.sourceAssetKey ||
        asset.provenance?.source_asset_key ||
        asset.provenance?.sourceAssetKey,
    );
    ...
    const normalised = {
      ...
      base_source_family: baseSourceFamily || null,
      path: pathValue,
      local_materialized_path: cleanText(
        asset.local_materialized_path || asset.local_materialised_path || pathValue,
      ),
      source_url: sourceUrl,
      canonical_source_url: cleanText(
        asset.canonical_source_url || asset.canonicalSourceUrl,
      ) || undefined,
      youtube_video_id: cleanText(
        asset.youtube_video_id || asset.youtubeVideoId,
      ) || undefined,
      source_master_path: cleanText(
        asset.source_master_path || asset.sourceMasterPath,
      ) || undefined,
      source_master_sha256: cleanText(
        asset.source_master_sha256 || asset.sourceMasterSha256,
      ) || undefined,
      source_duration_s:
        Number(asset.source_duration_s ?? asset.sourceDurationS) || undefined,
      source_asset_key: sourceAssetKeyFromUrl(
        explicitSourceAssetKey || baseSourceFamily || sourceUrl || pathValue,
      ),
      ...
      segmentValidationPassed:
        typeof asset.segmentValidationPassed === "boolean"
          ? asset.segmentValidationPassed
          : typeof asset.segment_validation_passed === "boolean"
            ? asset.segment_validation_passed
            : undefined,
      ...
      source_safety_blocked: asset.source_safety_blocked === true,
      generator_project_id: cleanText(asset.generator_project_id) || undefined,
      generator_master_sha256:
        cleanText(asset.generator_master_sha256) || undefined,
      type: localClipType(asset),
      media_kind: cleanText(asset.media_kind || asset.mediaKind) || undefined,
      ...
      allowed_use: cleanText(
        asset.allowed_use || asset.allowed_render_use || asset.allowedRenderUse,
      ) || undefined,
      allowed_platforms: Array.isArray(asset.allowed_platforms)
        ? [...asset.allowed_platforms]
        : undefined,
      commercial_use_allowed:
        typeof asset.commercial_use_allowed === "boolean"
          ...
          : undefined,
      credit_required:
        typeof asset.credit_required === "boolean" ? asset.credit_required : undefined,
      approval_status: cleanText(asset.approval_status) || undefined,
      rights_status: cleanText(asset.rights_status) || undefined,
      evidence_reference: cleanText(asset.evidence_reference) || undefined,
      rights_evidence_file: cleanText(
        asset.rights_evidence_file || asset.evidence_file,
      ) || undefined,
      evidence_sha256: cleanText(
        asset.evidence_sha256 || asset.rights_evidence_sha256,
      ) || undefined,
      rights_evidence_sha256: cleanText(
        asset.rights_evidence_sha256 || asset.evidence_sha256,
      ) || undefined,
      rights_evidence_size_bytes:
        Number(asset.rights_evidence_size_bytes ?? asset.evidence_size_bytes) || undefined,
      risk_score: Number.isFinite(Number(asset.risk_score))
      ...
      asset_sha256: cleanText(
        asset.asset_sha256 || asset.materialised_output_sha256 || asset.sha256,
      ) || undefined,
      asset_size_bytes:
        Number(
      ...
      materialised_output_sha256:
        cleanText(asset.materialised_output_sha256) || undefined,
      materialised_output_size_bytes:
        Number(asset.materialised_output_size_bytes) || undefined,
      owned_rights_record:
        asset.owned_rights_record && typeof asset.owned_rights_record === "object"
          ? { ...asset.owned_rights_record }
          : undefined,
      owned_rights_evaluation:
        asset.owned_rights_evaluation &&
        typeof asset.owned_rights_evaluation === "object"
          ? { ...asset.owned_rights_evaluation }
          : undefined,
      materialized_file_evidence:
        asset.materialized_file_evidence &&
        typeof asset.materialized_file_evidence === "object"
          ? { ...asset.materialized_file_evidence }
          : undefined,
      sampled_visual_fingerprint: cleanText(
        asset.sampled_visual_fingerprint,
      ) || undefined,
      base_source_asset_id: cleanText(asset.base_source_asset_id) || undefined,
      base_source_identity_basis:
        cleanText(asset.base_source_identity_basis) || undefined,
      motion_source_identity:
        asset.motion_source_identity &&
        typeof asset.motion_source_identity === "object"
          ? { ...asset.motion_source_identity }
          : undefined,
      source_identity_provenance:
        asset.source_identity_provenance &&
        typeof asset.source_identity_provenance === "object"
          ? { ...asset.source_identity_provenance }
          : undefined,
      source_identity_conflicts: Array.isArray(asset.source_identity_conflicts)
        ? [...asset.source_identity_conflicts]
        : undefined,
      validation_provenance:
        asset.validation_provenance &&
        typeof asset.validation_provenance === "object"
          ? { ...asset.validation_provenance }
          : undefined,
      transformation_provenance:
        asset.transformation_provenance &&
        typeof asset.transformation_provenance === "object"
          ? { ...asset.transformation_provenance }
          : undefined,
      validation_provenance_conflicts:
        Array.isArray(asset.validation_provenance_conflicts)
          ? [...asset.validation_provenance_conflicts]
          : undefined,
      provenance:
