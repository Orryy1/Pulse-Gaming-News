...
function entityTokens(value = "") {
  return cleanText(value)
    .toLowerCase()
    .replace(/fh6/g, "forza horizon 6")
    .replace(/gta\s*vi/g, "grand theft auto vi")
    .replace(/gta\s*6/g, "grand theft auto vi")
    .replace(/[^a-z0-9]+/g, " ")
    .split(/\s+/)
    .filter((token) => token.length > 1 && !ENTITY_TOKEN_STOPWORDS.has(token));
}
...
function sourceContradictsPrimaryStory(candidate = {}, operatorRow = {}, directMediaRow = {}) {
  const primaryStoryEntity = firstText(
    candidate.primary_story_entity,
    candidate.story_entity,
    directMediaRow.primary_story_entity,
    operatorRow.primary_story_entity,
  );
  if (!primaryStoryEntity) return false;
  const sourceIdentity = [
    operatorRow.source_title,
    operatorRow.source_owner,
    candidate.source_owner,
    directMediaRow.source_owner,
    operatorRow.source_family,
    candidate.source_family,
    directMediaRow.source_family,
    operatorRow.official_source_url,
    candidate.official_source_url,
    directMediaRow.official_source_url,
  ].join(" ");
  return !hasTokenOverlap(primaryStoryEntity, sourceIdentity);
}
...
function mediaReferenceRejectReason({ candidate = {}, directMediaRow = {}, operatorRow = {} } = {}) {
  const directUrl = firstText(
    operatorRow.approved_direct_media_url,
    operatorRow.direct_media_url_if_available,
    operatorRow.direct_media_url,
    directMediaRow.direct_media_url,
    directMediaRow.approved_direct_media_url,
    candidate.approved_direct_media_url,
    candidate.direct_media_url_if_available,
    candidate.source_url,
  );
  return officialMediaReferenceRejectReason({
    ...candidate,
    ...directMediaRow,
    ...operatorRow,
    source_url: firstText(operatorRow.source_url, directMediaRow.source_url, candidate.source_url, directUrl),
    url: directUrl,
    reference_url: firstText(operatorRow.reference_url, directMediaRow.reference_url, candidate.reference_url),
    official_source_url: firstText(
      operatorRow.official_source_url,
      directMediaRow.official_source_url,
      candidate.official_source_url,
    ),
...
function collectCandidates(sourceFamilyReport = {}) {
  ...
  function merge(entry = {}, row = {}) {
    ...
    byKey.set(key, {
      ...
      primary_story_entity: firstText(row.primary_story_entity, row.entity, existing.primary_story_entity),
...
function storyEntityMapFromSourceFamilyReport(sourceFamilyReport = {}) {
  const byStoryId = new Map();
  for (const row of asArray(sourceFamilyReport.rows)) {
    const storyId = cleanText(row.story_id);
    const entity = firstText(row.primary_story_entity, row.entity, row.title);
    if (storyId && entity && !byStoryId.has(storyId)) byStoryId.set(storyId, entity);
  }
  return byStoryId;
}

function collectDirectMediaRows(directMediaReport = {}, storyEntitiesById = new Map()) {
  const byKey = new Map();
  for (const row of rowsFromPayload(directMediaReport)) {
    ...
    byKey.set(sourceKey(storyId, family), {
      ...row,
      source_family: family,
      primary_story_entity: firstText(row.primary_story_entity, storyEntitiesById.get(storyId)),
...
function mergeOperatorOnlyCandidates(candidates = [], operatorRows = new Map()) {
  ...
  for (const row of operatorRows.values()) {
    ...
    merged.push({
      ...
      primary_story_entity: cleanText(row.primary_story_entity),
...
function classifySource({
  ...
  const metadataRejectReason = mediaReferenceRejectReason({ candidate, directMediaRow, operatorRow });

  const rightsGate = trustedCreator ? (marker.complete ? "licence_marker" : "blocked") : "official_source";

  if (sourceContradictsPrimaryStory(candidate, operatorRow, directMediaRow)) {
    return {
      ...row,
      status: "blocked",
      access_mode: direct.ok ? "approved_direct_media_url" : local.ok ? "local_operator_file" : "source_reference",
      rights_gate: rightsGate,
      blocking_reason: "source_entity_mismatch_with_story",
  ...
  if (metadataRejectReason) {
    return {
      ...row,
      status: "blocked",
      access_mode: direct.ok ? "approved_direct_media_url" : local.ok ? "local_operator_file" : "source_reference",
      rights_gate: rightsGate,
      blocking_reason: metadataRejectReason,
      approved_media_url: direct.ok ? direct.url : "",
      local_operator_file_path: local.ok ? local.path : "",
      source_url_kind: direct.source_url_kind || "",
      segment_validation_eligible: false,
...
function buildLicensedDirectMediaAcquisitionReport({
  ...
  const storyEntitiesById = storyEntityMapFromSourceFamilyReport(sourceFamilyReport);
  const directRows = collectDirectMediaRows(directMediaReport, storyEntitiesById);
