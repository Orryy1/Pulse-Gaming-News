...
function buildVisualV4MotionPack({
  ...
  const consumedDuplicateAssetCandidates = new Set();
  ...
  if (accepted.length < targetClipCount) {
    ...
    for (const item of repeatCandidatesByTimeline) {
      ...
      if (targetClipCount && accepted.length >= targetClipCount) break;
  ...
  if (accepted.length < targetClipCount) {
    const repeatAssetsByTimeline = duplicateAssetCandidates
      .map((item, index) => ({ ...item, duplicateIndex: index }))
      .sort(segmentStartSort);
    for (const item of repeatAssetsByTimeline) {
      if (accepted.length >= maxClipCount) break;
      if (targetClipCount && accepted.length >= targetClipCount) break;
      const sourceAssetKey = segmentSourceAssetKey(item.segment);
      if (isShortTrimmedMontage(item.segment)) {
        rejected.push({ ...item.base, reason: "repeat_short_trimmed_montage_not_allowed" });
        consumedDuplicateAssetCandidates.add(item.duplicateIndex);
        continue;
      }
      if (hasWindowConflict(item.segment, accepted)) {
        rejected.push({ ...item.base, reason: "source_asset_window_too_close" });
      ...
      if (!segmentHasHashDistinctWindowProof(item.segment)) {
        continue;
      }
      const sourceAssetWindowCount = accepted.filter(
        (clip) => segmentSourceAssetKey(clip) === sourceAssetKey,
      ).length;
      if (sourceAssetWindowCount >= MAX_HASH_DISTINCT_WINDOWS_PER_SOURCE_ASSET) {
        rejected.push({ ...item.base, reason: "source_asset_repeat_limit_reached" });
        consumedDuplicateAssetCandidates.add(item.duplicateIndex);
        continue;
      }
      if (hasSampleHashConflict(item.segment, accepted)) {
        rejected.push({ ...item.base, reason: "source_asset_sample_hash_repeated" });
        consumedDuplicateAssetCandidates.add(item.duplicateIndex);
        continue;
      }
      acceptSegment(item.segment);
  ...
  duplicateAssetCandidates.forEach((item, index) => {
    if (consumedDuplicateAssetCandidates.has(index)) return;
