"use strict";

const { mediaSourceUrlKindFields } = require("../../media-source-url-kind");
const {
  officialMediaReferenceRejectReason,
} = require("../../official-media-reference-preflight");
const {
  canonicaliseMotionSourceUrl,
} = require("../motion-source-identity");
const { buildFootageEmpirePlan } = require("./footage-empire");

const MIN_SEGMENT_ACTION_SCORE = 70;
const MIN_CLIP_DURATION_S = 1.2;
const PREMIUM_BENCHMARK_TARGET_MOTION_SCENES = 8;
const MAX_HASH_DISTINCT_WINDOWS_PER_SOURCE_ASSET = 4;
const MIN_SAMPLE_HASHES_FOR_DISTINCT_WINDOW = 2;
const PROMO_CARD_SOURCE_RE =
  /\b(?:pre[-_\s]?order|bonus|welcome[-_\s]?pack|car[-_\s]?pass|voucher|edition[-_\s]?(?:card|pack)|promo[-_\s]?card|static[-_\s]?card|storefront[-_\s]?card|legend[-_\s]?video)\b/i;
const OFFICIAL_TRAILER_MOTION_RE =
  /\b(?:trailer|gameplay|gameplay[-_\s]?reveal|launch|showcase|deep[-_\s]?dive|overview|footage|direct[-_\s]?feed)\b/i;
const STATIC_PROMO_MEDIA_RE =
  /\b(?:cover[-_\s]?art|key[-_\s]?art|box[-_\s]?art|poster|wallpaper|screenshot|still|pre[-_\s]?order|bonus|edition[-_\s]?(?:card|pack)|voucher|static[-_\s]?card|storefront[-_\s]?card)\b/i;
const SPECIALISED_VISUAL_SOURCE_RE =
  /\b(?:accessibility|colourblind|colorblind|high[-_\s]?contrast|visual[-_\s]?aid)\b/i;
const CHARACTER_SPECIFIC_MOTION_CONTEXT_RE =
  /\b(?:character|fighter|roster|new[-_\s]?fighter|new[-_\s]?character|dlc[-_\s]?fighter|gameplay[-_\s]?reveal|ranked|rushdown|zoner|footsie|footsies|meter|combo|combos|knife[-_\s]?feints?|eskrima|space[-_\s]?control)\b/i;
const STORY_MOTION_LOCK_STOP_TOKENS = new Set([
  "about",
  "ahead",
  "arrives",
  "back",
  "backs",
  "beat",
  "because",
  "before",
  "capcom",
  "character",
  "combat",
  "control",
  "could",
  "dangerous",
  "defenders",
  "eskrima",
  "fairness",
  "finally",
  "fighter",
  "footage",
  "gameplay",
  "gamespot",
  "giving",
  "headline",
  "just",
  "knife",
  "look",
  "looks",
  "made",
  "meter",
  "mode",
  "motion",
  "players",
  "player",
  "pressure",
  "problem",
  "ranked",
  "real",
  "reveal",
  "revealed",
  "rushdown",
  "shorts",
  "shows",
  "split",
  "splits",
  "space",
  "street",
  "story",
  "trailer",
  "trailers",
  "video",
  "while",
  "why",
  "zoner",
  "zoners",
]);

function asArray(value) {
  if (Array.isArray(value)) return value;
  if (!value) return [];
  if (typeof value === "string") {
    const trimmed = value.trim();
    if (!trimmed) return [];
    if (trimmed.startsWith("[") || trimmed.startsWith("{")) {
      try {
        const parsed = JSON.parse(trimmed);
        return Array.isArray(parsed) ? parsed : [parsed];
      } catch {
        return [];
      }
    }
  }
  return typeof value === "object" ? [value] : [];
}

function cleanText(value) {
  return String(value || "").replace(/\s+/g, " ").trim();
}

function normaliseFamily(value) {
  return (
    cleanText(value)
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "_")
      .replace(/^_+|_+$/g, "") || null
  );
}

function numberOrNull(value) {
  const number = Number(value);
  return Number.isFinite(number) ? number : null;
}

function storyId(story = {}) {
  return cleanText(story.id || story.story_id);
}

function storyProductMotionText(story = {}) {
  return [
    story.canonical_subject,
    story.canonical_game,
    story.canonical_company,
    story.title,
    story.suggested_title,
    story.short_title,
    story.suggested_thumbnail_text,
    story.full_script,
    story.tts_script,
  ]
    .map(cleanText)
    .filter(Boolean)
    .join(" ")
    .toLowerCase();
}

function comparableText(value) {
  return cleanText(value)
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}

function comparableSubjectMatches(haystack, subject) {
  const needle = comparableText(subject);
  if (needle.length < 4) return false;
  const text = comparableText(haystack);
  if (!text) return false;
  if (text.includes(needle)) return true;
  const tokens = needle.split(" ").filter((token) => token.length >= 3);
  return tokens.length >= 2 && tokens.every((token) => text.includes(token));
}

function comparableTokens(value) {
  return comparableText(value).split(" ").filter(Boolean);
}

function textContainsComparableToken(haystack, token) {
  const text = comparableText(haystack);
  if (!text || !token) return false;
  return new RegExp(`(^| )${token.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}( |$)`).test(text);
}

function storySpecificMotionLockTokens(story = {}) {
  const context = [
    story.title,
    story.selected_title,
    story.short_title,
    story.suggested_title,
    story.suggested_thumbnail_text,
    story.thumbnail_headline,
    story.full_script,
    story.tts_script,
    story.narration_script,
    ...asArray(story.confirmed_claims),
  ]
    .map(cleanText)
    .filter(Boolean)
    .join(" ");
  if (!CHARACTER_SPECIFIC_MOTION_CONTEXT_RE.test(context)) return [];

  const candidateText = [
    story.motion_subject,
    story.featured_character,
    story.character_name,
    story.title,
    story.short_title,
    story.suggested_thumbnail_text,
    story.thumbnail_headline,
  ]
    .map(cleanText)
    .filter(Boolean)
    .join(" ");
  const excluded = new Set([
    ...comparableTokens(story.canonical_game),
    ...comparableTokens(story.canonical_company),
    ...comparableTokens(story.source_name),
    ...comparableTokens(story.primary_source_name),
    ...STORY_MOTION_LOCK_STOP_TOKENS,
  ]);
  const seen = new Set();
  return comparableTokens(candidateText)
    .filter((token) => token.length >= 4)
    .filter((token) => !/^\d+$/.test(token))
    .filter((token) => !excluded.has(token))
    .filter((token) => {
      if (seen.has(token)) return false;
      seen.add(token);
      return true;
    })
    .slice(0, 4);
}

function storyAllowsOfficialGameProductMotion(story = {}, segment = {}) {
  const subject = cleanText(story.canonical_game || story.canonical_subject);
  if (!subject) return false;
  const haystack = [
    segment.entity,
    segment.source_family,
    segment.sourceFamily,
    segment.reference_title,
    segment.referenceTitle,
    segment.movie_name,
    segment.movieName,
    segment.title,
    segment.source_title,
    segment.sourceTitle,
    segment.provenance?.entity,
    segment.provenance?.source_family,
    segment.provenance?.reference_title,
  ].join(" ");
  return comparableSubjectMatches(haystack, subject);
}

function segmentIdentityText(segment = {}) {
  return [
    segment.entity,
    segment.source_family,
    segment.sourceFamily,
    segment.reference_title,
    segment.referenceTitle,
    segment.movie_name,
    segment.movieName,
    segment.title,
    segment.source_title,
    segment.sourceTitle,
    segment.provenance?.entity,
    segment.provenance?.source_family,
    segment.provenance?.reference_title,
  ]
    .map(cleanText)
    .filter(Boolean)
    .join(" ");
}

function segmentMatchesCurrentStoryIdentity(story = {}, segment = {}) {
  const haystack = segmentIdentityText(segment);
  return [
    story.canonical_subject,
    story.canonical_game,
    story.game_title,
    story.public_title,
    story.suggested_title,
    story.title,
  ]
    .map(cleanText)
    .filter((subject) => subject.length >= 4)
    .some((subject) => comparableSubjectMatches(haystack, subject));
}

function storySpecificMotionLockRejectReason(story = {}, segment = {}) {
  const lockTokens = storySpecificMotionLockTokens(story);
  if (!lockTokens.length) return null;
  const haystack = segmentIdentityText(segment);
  return lockTokens.some((token) => textContainsComparableToken(haystack, token))
    ? null
    : "story_subject_motion_mismatch";
}

function segmentLooksOfficialDirectMedia(segment = {}) {
  const haystack = [
    segment.source_type,
    segment.sourceType,
    segment.provider,
    segment.rights_risk_class,
    segment.rightsRiskClass,
    segment.allowed_render_use,
    segment.allowedRenderUse,
    segment.provenance?.rights_risk_class,
    segment.provenance?.allowed_render_use,
  ]
    .map(cleanText)
    .join(" ")
    .toLowerCase();
  return [
    "licensed_direct_media",
    "official_direct_media",
    "official_game_website_media_page",
    "official_game_site_media_page",
    "official_game_site_direct_video",
    "official_game_page_direct_video",
    "official_platform_product_page",
    "platform_storefront_video_reference",
    "official_source",
  ].some((marker) => haystack.includes(marker));
}

function allowsRefreshedStoryIdSegment(segment = {}, options = {}) {
  return segmentLooksOfficialDirectMedia(segment) && segmentMatchesCurrentStoryIdentity(options.story || {}, segment);
}

function storyAllowsOfficialProductMotion(story = {}, segment = {}) {
  const text = storyProductMotionText(story);
  if (/\b(?:ps5|playstation\s*5|xbox\s*(?:wireless\s*)?controller|controller|console|hardware|steam\s*deck|switch\s*2|dualsense|accessor(?:y|ies)|peripheral)\b/i.test(
    text,
  )) {
    return true;
  }
  return storyAllowsOfficialGameProductMotion(story, segment);
}

function segmentLooksOwnedExplainerMotion(segment = {}) {
  const text = [
    segment.media_kind,
    segment.mediaKind,
    segment.source_type,
    segment.sourceType,
    segment.source_kind,
    segment.sourceKind,
    segment.rights_risk_class,
    segment.rightsRiskClass,
    segment.licence_basis,
    segment.license_basis,
    segment.provenance?.media_kind,
    segment.provenance?.source_type,
    segment.provenance?.rights_risk_class,
    segment.provenance?.licence_basis,
  ]
    .map(cleanText)
    .join(" ")
    .toLowerCase();
  return (
    text.includes("owned_explainer_motion") ||
    text.includes("internally_generated_motion_graphic") ||
    text.includes("owned_generated_motion") ||
    text.includes("owned_generated_editorial_motion_graphic")
  );
}

function segmentStoryId(segment = {}) {
  return cleanText(
    segment.story_id ||
      segment.storyId ||
      segment.provenance?.story_id ||
      segment.provenance?.storyId,
  );
}

function segmentSourceUrl(segment = {}) {
  return cleanText(segment.source_url || segment.sourceUrl || segment.path || segment.source);
}

function previousClipMotionSourceUrl(clip = {}) {
  const candidates = [
    clip.source_url,
    clip.sourceUrl,
    clip.local_materialized_path,
    clip.media_path,
    clip.path,
  ];
  for (const candidate of candidates) {
    const text = cleanText(candidate);
    if (!text) continue;
    if (mediaSourceUrlKindFields(text).segment_validation_eligible === true) {
      return text;
    }
  }
  return segmentSourceUrl(clip);
}

function sourceFamilyFromUrl(value) {
  const text = cleanText(value);
  if (!text) return null;
  try {
    const parsed = new URL(text);
    const host = parsed.hostname.replace(/^www\./i, "");
    const steam = text.match(/store_trailers\/(\d+)\/(\d+)/i);
    if (steam) return normaliseFamily(`steam_${steam[1]}_${steam[2]}`);
    return normaliseFamily(host.split(".").slice(0, -1).join("_") || host);
  } catch {
    const withoutExt = text.replace(/\.[a-z0-9]+(?:[?#].*)?$/i, "");
    return normaliseFamily(withoutExt.split(/[\\/]/).pop());
  }
}

function sourceAssetKeyFromUrl(value) {
  const text = cleanText(value);
  if (!text) return null;
  const steam = text.match(/store_trailers\/(\d+)\/(\d+)/i);
  if (steam) return `steam:${steam[1]}:${steam[2]}`;
  try {
    const parsed = new URL(text);
    parsed.search = "";
    parsed.hash = "";
    const host = parsed.hostname.toLowerCase().replace(/^www\./, "");
    if (host.endsWith("steamstatic.com") && parsed.pathname.includes("/store_item_assets/steam/apps/")) {
      const pathname = parsed.pathname.toLowerCase();
      const basePath = pathname.replace(/\/extras\/([^/.]+)\.(?:mp4|webm|mov|mkv)$/i, "/extras/$1");
      if (basePath !== pathname) return `steamstatic:${basePath}`;
    }
    return parsed.href.toLowerCase();
  } catch {
    return text.replace(/[?#].*$/, "").toLowerCase();
  }
}

function sourceAssetKeyFromIdentity(value) {
  const text = cleanText(value);
  if (!text) return null;
  const urlWrapped = text.match(/^url:(https?:\/\/.+)$/i);
  if (urlWrapped) return sourceAssetKeyFromUrl(urlWrapped[1]);
  if (/^https?:\/\//i.test(text)) return sourceAssetKeyFromUrl(text);
  return text
    .replace(/[?#].*$/, "")
    .replace(/_window_\d+(?:_\d+)?$/i, "")
    .toLowerCase();
}

function segmentSourceFamily(segment = {}) {
  const explicitFamily =
    normaliseFamily(segment.source_family) ||
    normaliseFamily(segment.sourceFamily) ||
    normaliseFamily(segment.provenance?.source_family) ||
    normaliseFamily(segment.provenance?.sourceFamily);
  const provider = normaliseFamily(segment.provider);
  const urlFamily = sourceFamilyFromUrl(segmentSourceUrl(segment));
  const steamUrlFamily = urlFamily && urlFamily.startsWith("steam_") ? urlFamily : null;
  if (steamUrlFamily && explicitFamily && explicitFamily.startsWith("steam_")) {
    return steamUrlFamily;
  }
  if (explicitFamily && explicitFamily !== "steam") return explicitFamily;
  const storeAppId = cleanText(segment.store_app_id || segment.storeAppId || segment.provenance?.store_app_id);
  const movieId = cleanText(segment.movie_id || segment.movieId || segment.provenance?.movie_id);
  if (
    (explicitFamily === "steam" || provider === "steam") &&
    /^\d+$/.test(storeAppId) &&
    /^\d+$/.test(movieId)
  ) {
    return `steam_${storeAppId}_${movieId}`;
  }
  if (!explicitFamily && steamUrlFamily && (!provider || provider === "steam")) {
    return steamUrlFamily;
  }
  return (
    explicitFamily ||
    normaliseFamily(segment.trusted_footage_source_id) ||
    provider ||
    normaliseFamily(segment.source) ||
    urlFamily ||
    "unknown"
  );
}

function segmentBaseSourceFamily(segment = {}) {
  return cleanText(
    segment.base_source_family ||
      segment.baseSourceFamily ||
      segment.source_asset_key ||
      segment.sourceAssetKey ||
      segment.provenance?.base_source_family ||
      segment.provenance?.baseSourceFamily ||
      segment.provenance?.source_asset_key ||
      segment.provenance?.sourceAssetKey,
  );
}

function segmentSourceIdentityValue(segment = {}, fields = []) {
  const owners = [
    segment,
    segment.source_identity,
    segment.motion_source_identity,
    segment.provenance,
  ].filter((owner) => owner && typeof owner === "object");
  for (const owner of owners) {
    for (const field of fields) {
      const value = cleanText(owner[field]);
      if (value) return value;
    }
  }
  return "";
}

function segmentCanonicalSourceIdentity(segment = {}) {
  const youtubeVideoId = segmentSourceIdentityValue(segment, [
    "youtube_video_id",
    "source_youtube_id",
  ]);
  if (/^[A-Za-z0-9_-]{6,32}$/.test(youtubeVideoId)) {
    return `youtube:${youtubeVideoId}`;
  }
  const canonicalSourceUrl = segmentSourceIdentityValue(segment, [
    "canonical_source_url",
    "master_source_url",
    "original_source_url",
    "reference_url",
  ]);
  return canonicaliseMotionSourceUrl(canonicalSourceUrl);
}

function segmentSourceAssetKey(segment = {}) {
  return (
    segmentCanonicalSourceIdentity(segment) ||
    sourceAssetKeyFromIdentity(segmentBaseSourceFamily(segment)) ||
    sourceAssetKeyFromUrl(segmentSourceUrl(segment))
  );
}

function sourceUrlForTrustedRow(row = {}) {
  return cleanText(
    row.reference_url ||
      row.canonical_source_url ||
      row.official_source_url ||
      row.approved_media_url ||
      row.source_url ||
      row.url,
  );
}

function trustedReportRowUsable(row = {}) {
  const status = cleanText(row.status || row.verdict || row.readiness_status).toLowerCase();
  const blocker = cleanText(row.blocking_reason || row.blocker || row.reason);
  if (blocker) return false;
  if (status && /(?:^|[_\s-])(?:blocked|reject|rejected|failed|fail|unsafe|red)(?:$|[_\s-])/.test(status)) return false;
  if (
    status &&
    !/(?:^|[_\s-])(?:ready|accepted|approved|validated|green|pass|passed|publishable)(?:$|[_\s-])/.test(status)
  ) {
    return false;
  }
  const rightsGate = cleanText(row.rights_gate || row.rightsGate).toLowerCase();
  const sourceTier = cleanText(row.source_tier || row.sourceTier).toLowerCase();
  const accessMode = cleanText(row.access_mode || row.accessMode).toLowerCase();
  const rightsRisk = cleanText(row.rights_risk_class || row.rightsRiskClass).toLowerCase();
  const allowedRenderUse = cleanText(row.allowed_render_use || row.allowedRenderUse).toLowerCase();
  const sourceType = cleanText(row.source_type || row.sourceType).toLowerCase();
  const officialReferenceSignal =
    /\bofficial\b/.test(rightsRisk) ||
    /\bstorefront_promotional_video\b/.test(rightsRisk) ||
    /\b(?:steam_movie|platform_storefront|licensed_direct_media_url)\b/.test(sourceType);
  const renderUseBlocked =
    /\b(?:blocked|forbidden|disallowed|unsafe|not_allowed|do_not_render)\b/.test(allowedRenderUse);
  const verifiedOfficialReference =
    row.source_verified !== false &&
    row.segment_validation_eligible !== false &&
    !renderUseBlocked &&
    officialReferenceSignal;
  return (
    rightsGate === "official_source" ||
    /\bofficial\b/.test(sourceTier) ||
    /\bapproved_direct_media_url\b/.test(accessMode) ||
    verifiedOfficialReference
  );
}

function planReferences(report = {}) {
  return asArray(report.plans).flatMap((plan) =>
    asArray(plan.references).map((reference) => ({
      ...reference,
      story_id: reference.story_id || plan.story_id || plan.storyId || null,
    })),
  );
}

function trustedReportReferences(report = {}) {
  return [
    ...asArray(report.story_candidates),
    ...asArray(report.rows).filter(trustedReportRowUsable),
    ...asArray(report.accepted_references).filter(trustedReportRowUsable),
    ...asArray(report.render_ready_sources).filter(trustedReportRowUsable),
    ...planReferences(report).filter(trustedReportRowUsable),
  ];
}

function clipWindow(asset = {}) {
  const timing = selectedTiming(asset);
  if (timing.mediaStartS === null || timing.durationS === null) return null;
  const start = Number(timing.mediaStartS);
  const end = Number((start + Number(timing.durationS)).toFixed(2));
  return { start, end };
}

function hasWindowConflict(segment = {}, accepted = [], gapSeconds = 1) {
  const assetKey = segmentSourceAssetKey(segment);
  const candidateWindow = clipWindow(segment);
  if (!assetKey || !candidateWindow) return false;
  for (const clip of accepted) {
    if (segmentSourceAssetKey(clip) !== assetKey) continue;
    const existingWindow = clipWindow(clip);
    if (!existingWindow) continue;
    const tooClose =
      candidateWindow.start < existingWindow.end + gapSeconds &&
      existingWindow.start < candidateWindow.end + gapSeconds;
    if (tooClose) return true;
  }
  return false;
}

function sampleContentHash(sample = {}) {
  return cleanText(
    sample.qa?.content_hash ||
      sample.qa?.contentHash ||
      sample.content_hash ||
      sample.contentHash ||
      sample.hash ||
      sample.frame_hash ||
      sample.frameHash,
  );
}

function segmentSampleContentHashes(segment = {}) {
  return new Set(
    asArray(segment.samples)
      .filter((sample) => {
        const status = cleanText(sample.status || "accepted").toLowerCase();
        const qa = sample.qa || {};
        return (
          status !== "rejected_qa" &&
          qa.thumbnail_safe !== false &&
          qa.black_frame !== true &&
          asArray(qa.failures).length === 0 &&
          qa.visual_taste?.verdict !== "fail"
        );
      })
      .map(sampleContentHash)
      .filter(Boolean),
  );
}

function segmentHasHashDistinctWindowProof(segment = {}) {
  if (segment.segment_validated !== true) return false;
  if (segment.allowed_for_flash_lane !== true) return false;
  return segmentSampleContentHashes(segment).size >= MIN_SAMPLE_HASHES_FOR_DISTINCT_WINDOW;
}

function clipSampleContentHashes(clip = {}) {
  return new Set(asArray(clip.provenance?.sample_content_hashes).map(cleanText).filter(Boolean));
}

function hasSampleHashConflict(segment = {}, accepted = []) {
  const assetKey = segmentSourceAssetKey(segment);
  if (!assetKey) return false;
  const candidateHashes = segmentSampleContentHashes(segment);
  if (!candidateHashes.size) return false;
  for (const clip of accepted) {
    if (segmentSourceAssetKey(clip) !== assetKey) continue;
    const clipHashes = clipSampleContentHashes(clip);
    for (const hash of candidateHashes) {
      if (clipHashes.has(hash)) return true;
    }
  }
  return false;
}

function trustedSourceFamilies(report = {}, story = {}) {
  const wantedStoryId = storyId(story);
  const families = new Set();
  for (const candidate of trustedReportReferences(report)) {
    const candidateStoryId = cleanText(candidate.story_id);
    if (wantedStoryId && candidateStoryId && candidateStoryId !== wantedStoryId) continue;
    const family = segmentSourceFamily(candidate);
    if (family && family !== "unknown") families.add(family);
  }
  for (const source of asArray(report.accepted_sources)) {
    const family = segmentSourceFamily(source);
    if (family && family !== "unknown") families.add(family);
    const urlFamily = sourceFamilyFromUrl(sourceUrlForTrustedRow(source));
    if (urlFamily && urlFamily !== "unknown") families.add(urlFamily);
  }
  return families;
}

function selectedTiming(segment = {}) {
  const trim = segment.trim_recommended === true;
  const recommendedStart = numberOrNull(
    segment.recommended_media_start_s ?? segment.recommendedMediaStartS,
  );
  const recommendedDuration = numberOrNull(
    segment.recommended_duration_s ?? segment.recommendedDurationS,
  );
  const rawStart = numberOrNull(segment.media_start_s ?? segment.mediaStartS);
  const rawDuration = numberOrNull(segment.duration_s ?? segment.durationS);
  return {
    mediaStartS: trim && recommendedStart !== null ? recommendedStart : rawStart,
    durationS: trim && recommendedDuration !== null ? recommendedDuration : rawDuration,
    trimRecommended: trim && recommendedStart !== null && recommendedDuration !== null,
  };
}

function segmentHasGameplaySampleProof(segment = {}) {
  const sourceType = cleanText(segment.source_type || segment.sourceType).toLowerCase();
  const actionScore = numberOrNull(segment.action_score ?? segment.actionScore);
  if (segment.segment_validated !== true) return false;
  if (segment.allowed_for_flash_lane !== true) return false;
  if (cleanText(segment.segment_motion_class) !== "gameplay_action") return false;
  if (!sourceType.includes("official_social_media_video")) return false;
  if (actionScore === null || actionScore < MIN_SEGMENT_ACTION_SCORE) return false;

  const acceptedSamples = asArray(segment.samples).filter((sample) => {
    const status = cleanText(sample.status || "accepted").toLowerCase();
    const qa = sample.qa || {};
    return (
      status !== "rejected_qa" &&
      qa.thumbnail_safe !== false &&
      qa.black_frame !== true &&
      asArray(qa.failures).length === 0 &&
      qa.visual_taste?.verdict !== "fail"
    );
  });
  if (acceptedSamples.length < 3) return false;
  const gameplayTagged = acceptedSamples.filter((sample) => {
    const qa = sample.qa || {};
    const tags = asArray(qa.visual_taste?.tags);
    return qa.gameplay_action_candidate === true || tags.includes("gameplay_candidate");
  });
  return gameplayTagged.length >= 2;
}

function segmentHasOfficialTrailerGameplayProof(segment = {}) {
  const descriptor = [
    segment.reference_title,
    segment.referenceTitle,
    segment.movie_name,
    segment.movieName,
    segment.title,
    segment.source_title,
    segment.sourceTitle,
    segment.source_url,
    segment.sourceUrl,
    segment.provenance?.reference_title,
    segment.provenance?.source_url,
  ]
    .map(cleanText)
    .filter(Boolean)
    .join(" ")
    .replace(/[_-]+/g, " ");
  const sourceUrl = segmentSourceUrl(segment);
  const urlKind = mediaSourceUrlKindFields(sourceUrl);
  const directOfficialVideo =
    segmentLooksOfficialDirectMedia(segment) &&
    urlKind.segment_validation_eligible === true &&
    urlKind.source_url_kind !== "html_or_unknown_page";
  if (!directOfficialVideo) return false;
  if (!OFFICIAL_TRAILER_MOTION_RE.test(descriptor)) return false;
  if (STATIC_PROMO_MEDIA_RE.test(descriptor)) return false;
  if (segment.segment_validated !== true) return false;
  if (segment.allowed_for_flash_lane !== true) return false;
  if (cleanText(segment.segment_motion_class) !== "gameplay_action") return false;
  return true;
}

function promoCardSourceFamilyRisk(segment = {}) {
  const haystack = [
    segmentSourceFamily(segment),
    segment.clip_key,
    segment.reference_title,
    segment.referenceTitle,
    segment.movie_name,
    segment.movieName,
    segment.title,
    segment.source_title,
    segment.sourceTitle,
    segment.provenance?.reference_title,
    segment.provenance?.clip_key,
  ]
    .map(cleanText)
    .filter(Boolean)
    .join(" ")
    .replace(/[_-]+/g, " ");
  return (
    PROMO_CARD_SOURCE_RE.test(haystack) &&
    !segmentHasGameplaySampleProof(segment) &&
    !segmentHasOfficialTrailerGameplayProof(segment)
  );
}

function specialisedVisualSourceFamilyRisk(segment = {}) {
  const haystack = [
    segmentSourceFamily(segment),
    segment.clip_key,
    segment.reference_title,
    segment.referenceTitle,
    segment.movie_name,
    segment.movieName,
    segment.title,
    segment.source_title,
    segment.sourceTitle,
    segment.provenance?.reference_title,
    segment.provenance?.clip_key,
  ]
    .map(cleanText)
    .filter(Boolean)
    .join(" ")
    .replace(/[_-]+/g, " ");
  return SPECIALISED_VISUAL_SOURCE_RE.test(haystack);
}

function candidateRejectionReason(segment = {}, options = {}) {
  const sourceUrl = segmentSourceUrl(segment);
  const urlKind = mediaSourceUrlKindFields(sourceUrl);
  const metadataReject = officialMediaReferenceRejectReason(segment);
  const storyFilter = cleanText(options.storyId);
  const segmentId = segmentStoryId(segment);
  const actionScore = numberOrNull(segment.action_score ?? segment.actionScore);
  const timing = selectedTiming(segment);
  const validationReason = cleanText(segment.validation_reason || segment.reason);
  const motionClass = cleanText(segment.segment_motion_class);
  const sourceType = cleanText(segment.source_type || segment.sourceType).toLowerCase();
  const platformStorefrontCinematicMotion =
    motionClass === "official_storefront_cinematic_motion" &&
    sourceType.includes("platform_storefront");
  const platformStorefrontStoryMatched =
    platformStorefrontCinematicMotion &&
    storyAllowsOfficialProductMotion(options.story || {}, segment);
  const officialProductMotionAllowed =
    motionClass === "official_product_motion" &&
    sourceType.includes("official_platform_product_page") &&
    storyAllowsOfficialProductMotion(options.story || {}, segment);
  const officialStorefrontCinematicMotionAllowed =
    motionClass === "official_storefront_cinematic_motion" &&
    (
      validationReason === "official_storefront_cinematic_motion_samples_passed" ||
      validationReason === "official_storefront_trailer_motion_samples_passed"
    ) &&
    (
      sourceType.includes("steam_movie") ||
      sourceType.includes("steam_storefront_video_reference") ||
      sourceType.includes("platform_storefront_video_reference") ||
      sourceType.includes("licensed_direct_media_url") ||
      platformStorefrontStoryMatched
    );
  const screenshotDerivedMotionAllowed =
    motionClass === "screenshot_derived_motion_clip" &&
    validationReason === "screenshot_derived_motion_materialized" &&
    sourceType.includes("screenshot_derived_motion") &&
    urlKind.source_url_kind === "local_video_file";
  const ownedExplainerMotionAllowed =
    motionClass === "owned_explainer_motion" &&
    validationReason === "owned_explainer_motion_materialized" &&
    segmentLooksOwnedExplainerMotion(segment) &&
    urlKind.source_url_kind === "local_video_file";
  const minActionScore =
    validationReason === "official_product_motion_samples_passed"
      ? Math.min(64, options.minActionScore || MIN_SEGMENT_ACTION_SCORE)
      : validationReason === "short_direct_media_detail_motion_samples_passed"
      ? Math.min(68, options.minActionScore || MIN_SEGMENT_ACTION_SCORE)
      : validationReason === "official_storefront_cinematic_motion_samples_passed" ||
        validationReason === "official_storefront_trailer_motion_samples_passed"
      ? Math.min(62, options.minActionScore || MIN_SEGMENT_ACTION_SCORE)
      : validationReason === "screenshot_derived_motion_materialized"
      ? Math.min(60, options.minActionScore || MIN_SEGMENT_ACTION_SCORE)
      : options.minActionScore || MIN_SEGMENT_ACTION_SCORE;

  if (storyFilter && segmentId && segmentId !== storyFilter && !allowsRefreshedStoryIdSegment(segment, options)) {
    return "story_id_mismatch";
  }
  if (!sourceUrl) return "segment_source_missing";
  if (urlKind.segment_validation_eligible !== true) {
    return urlKind.segment_validation_ineligible_reason || "segment_source_url_not_direct_media";
  }
  if (metadataReject) return `metadata_guard_${metadataReject}`;
  const storySubjectRejectReason = storySpecificMotionLockRejectReason(options.story || {}, segment);
  if (storySubjectRejectReason) return storySubjectRejectReason;
  if (platformStorefrontCinematicMotion && !platformStorefrontStoryMatched) {
    return "story_subject_motion_mismatch";
  }
  if (segment.segment_validated !== true) return "segment_not_validated";
  if (segment.allowed_for_flash_lane !== true) return "segment_not_allowed_for_flash_lane";
  if (promoCardSourceFamilyRisk(segment)) return "promo_card_source_family";
  if (specialisedVisualSourceFamilyRisk(segment)) return "specialised_visual_source_family";
  if (
    motionClass !== "gameplay_action" &&
    !officialProductMotionAllowed &&
    !officialStorefrontCinematicMotionAllowed &&
    !screenshotDerivedMotionAllowed &&
    !ownedExplainerMotionAllowed
  ) {
    return "segment_not_gameplay_action";
  }
  if (actionScore === null || actionScore < minActionScore) {
    return "segment_action_score_too_low";
  }
  if (timing.mediaStartS === null) return "segment_start_missing";
  if (timing.durationS === null || timing.durationS < MIN_CLIP_DURATION_S) {
    return "segment_duration_too_short";
  }
  return null;
}

function clipFromSegment(segment = {}, index = 0, trustedFamilies = new Set()) {
  const sourceUrl = segmentSourceUrl(segment);
  const baseSourceFamily = segmentBaseSourceFamily(segment);
  const sourceAssetKey = segmentSourceAssetKey(segment);
  const canonicalSourceUrl = segmentSourceIdentityValue(segment, [
    "canonical_source_url",
    "master_source_url",
    "original_source_url",
    "reference_url",
  ]);
  const youtubeVideoId = segmentSourceIdentityValue(segment, [
    "youtube_video_id",
    "source_youtube_id",
  ]);
  const sourceMasterSha256 = segmentSourceIdentityValue(segment, [
    "source_master_sha256",
    "master_sha256",
    "base_source_sha256",
  ]);
  const localPath = cleanText(
    segment.local_materialized_path ||
      segment.localMaterializedPath ||
      segment.media_path ||
      segment.mediaPath,
  );
  const urlKind = mediaSourceUrlKindFields(sourceUrl);
  const family = segmentSourceFamily(segment);
  const timing = selectedTiming(segment);
  const actionScore = numberOrNull(segment.action_score ?? segment.actionScore);
  const samples = asArray(segment.samples);
  const allowedRenderUse = cleanText(
    segment.allowed_render_use ||
      segment.allowedRenderUse ||
      segment.allowed_use ||
      segment.allowedUse ||
      segment.provenance?.allowed_render_use,
  );
  const rightsRiskClass = cleanText(
    segment.rights_risk_class ||
      segment.rightsRiskClass ||
      segment.provenance?.rights_risk_class,
  );
  const licenceBasis = cleanText(
    segment.licence_basis ||
      segment.license_basis ||
      segment.rights_basis ||
      segment.rightsBasis ||
      segment.provenance?.licence_basis,
  );
  const ownedExplainerMotion = segmentLooksOwnedExplainerMotion(segment);

  return {
    id: cleanText(segment.clip_id || segment.id || `v4_motion_${index + 1}_${family}`),
    type: "motion_clip",
    source_family: family,
    base_source_family: sourceAssetKey || baseSourceFamily || sourceAssetKeyFromUrl(sourceUrl) || family,
    path: localPath || sourceUrl,
    source_url: sourceUrl,
    canonical_source_url: canonicalSourceUrl || null,
    youtube_video_id: youtubeVideoId || null,
    source_master_sha256: sourceMasterSha256 || null,
    source_kind: urlKind.source_url_kind,
    source_url_kind: urlKind.source_url_kind,
    source_type: segment.source_type || segment.sourceType || null,
    provider: segment.provider || null,
    entity: segment.entity || segment.provenance?.entity || null,
    mediaStartS: Number(timing.mediaStartS.toFixed(2)),
    durationS: Number(timing.durationS.toFixed(2)),
    validated: true,
    segmentValidationPassed: true,
    allowed_render_use: allowedRenderUse || "reference_only_by_default",
    rights_risk_class: rightsRiskClass || (ownedExplainerMotion ? "owned_generated_motion" : "official_reference_only"),
    licence_basis: licenceBasis || null,
    trusted_source_matched:
      trustedFamilies.size === 0 || trustedFamilies.has(family) || family === "unknown"
        ? trustedFamilies.has(family)
        : false,
    provenance: {
      source: "visual_v4_motion_pack",
      source_report:
        segment.source_report ||
        segment.provenance?.source_report ||
        "official_trailer_segment_validation",
      story_id: segmentStoryId(segment) || null,
      clip_key: segment.clip_key || null,
      base_source_family: sourceAssetKey || baseSourceFamily || sourceAssetKeyFromUrl(sourceUrl) || family,
      canonical_source_url: canonicalSourceUrl || null,
      youtube_video_id: youtubeVideoId || null,
      source_master_sha256: sourceMasterSha256 || null,
      reference_title:
        segment.reference_title ||
        segment.movie_name ||
        segment.movieName ||
        segment.title ||
        null,
      movie_id: segment.movie_id || segment.movieId || null,
      store_app_id: segment.store_app_id || segment.storeAppId || null,
      store_app_title: segment.store_app_title || segment.storeAppTitle || null,
      validation_reason: segment.validation_reason || null,
      segment_motion_class: segment.segment_motion_class || null,
      segment_action_score: actionScore,
      segment_action_sample_count: numberOrNull(segment.action_sample_count),
      segment_validation_samples: samples.length,
      segment_validation_reported_at: null,
      segment_trim_recommended: timing.trimRecommended,
      segment_original_start_s: numberOrNull(segment.media_start_s ?? segment.mediaStartS),
      segment_original_duration_s: numberOrNull(segment.duration_s ?? segment.durationS),
      segment_render_start_s: Number(timing.mediaStartS.toFixed(2)),
      segment_render_duration_s: Number(timing.durationS.toFixed(2)),
      sample_paths: samples.map((sample) => sample.local_path || sample.planned_local_path).filter(Boolean),
      sample_content_hashes: Array.from(segmentSampleContentHashes(segment)),
    },
  };
}

function segmentFromPreviousClip(clip = {}) {
  const ownedExplainerMotion = segmentLooksOwnedExplainerMotion(clip);
  const sourceUrl = previousClipMotionSourceUrl(clip);
  const timing = selectedTiming({
    media_start_s: clip.mediaStartS ?? clip.media_start_s,
    duration_s: clip.durationS ?? clip.duration_s,
  });
  const family = segmentSourceFamily(clip);
  const provenance = clip.provenance || {};
  const actionScore = numberOrNull(
    clip.action_score ??
      clip.actionScore ??
      provenance.segment_action_score ??
      provenance.action_score,
  );
  const provenanceValidated =
    provenance.segment_validated === true || provenance.segmentValidationPassed === true;
  const topLevelValidated = clip.validated === true || clip.segmentValidationPassed === true;
  const provenanceAllowed =
    provenance.allowed_for_flash_lane === true || provenance.allowedForFlashLane === true;
  const topLevelAllowed =
    clip.allowed_for_flash_lane !== false && clip.allowedForFlashLane !== false;
  if (!sourceUrl || !family || family === "unknown") return null;
  if (!topLevelValidated && !provenanceValidated) return null;
  if (!topLevelAllowed && !provenanceAllowed) return null;
  if ((timing.mediaStartS === null && !ownedExplainerMotion) || timing.durationS === null) return null;
  const mediaStartS = timing.mediaStartS === null && ownedExplainerMotion ? 0 : timing.mediaStartS;
  return {
    story_id: provenance.story_id || null,
    clip_id: clip.id || null,
    clip_key:
      provenance.clip_key ||
      [sourceUrl, clip.entity || provenance.entity || "", mediaStartS].join("|"),
    source_url: sourceUrl,
    source_family: family,
    base_source_family: cleanText(
      clip.base_source_family ||
        clip.baseSourceFamily ||
        provenance.base_source_family ||
        provenance.baseSourceFamily,
    ) || null,
    source_type: clip.source_type || clip.sourceType || null,
    media_kind: clip.media_kind || clip.mediaKind || null,
    provider: clip.provider || null,
    entity: clip.entity || provenance.entity || null,
    media_start_s: mediaStartS,
    duration_s: timing.durationS,
    segment_validated: true,
    allowed_for_flash_lane: true,
    segment_motion_class:
      provenance.segment_motion_class ||
      (ownedExplainerMotion ? "owned_explainer_motion" : "gameplay_action"),
    action_score: actionScore === null ? MIN_SEGMENT_ACTION_SCORE : actionScore,
    action_sample_count: numberOrNull(provenance.segment_action_sample_count) || 1,
    validation_reason:
      provenance.validation_reason ||
      (ownedExplainerMotion ? "owned_explainer_motion_materialized" : "previous_motion_pack_clip_preserved"),
    story_selected_motion_clip:
      clip.story_selected_motion_clip === true ||
      provenance.story_selected_motion_clip === true ||
      cleanText(provenance.source_report) === "story_validated_motion_clips",
    allowed_render_use:
      clip.allowed_render_use ||
      clip.allowedRenderUse ||
      clip.allowed_use ||
      clip.allowedUse ||
      (ownedExplainerMotion ? "finished_editorial_video_only" : null),
    rights_risk_class:
      clip.rights_risk_class ||
      clip.rightsRiskClass ||
      (ownedExplainerMotion ? "owned_generated_motion" : null),
    rights_basis: clip.rights_basis || clip.rightsBasis || null,
    licence_basis: clip.licence_basis || clip.license_basis || null,
    source_report: "previous_visual_v4_motion_pack",
    local_materialized_path: cleanText(
      clip.local_materialized_path ||
        clip.localMaterializedPath ||
        clip.media_path ||
        clip.mediaPath ||
        clip.path,
    ) || null,
    source_still_url:
      cleanText(clip.source_url || clip.sourceUrl) !== sourceUrl
        ? cleanText(clip.source_url || clip.sourceUrl)
        : null,
    samples: asArray(provenance.sample_paths).map((samplePath) => ({ local_path: samplePath })),
  };
}

function previousMotionPackSegments(previousMotionPack = {}) {
  return asArray(previousMotionPack.clips)
    .map(segmentFromPreviousClip)
    .filter(Boolean);
}

function isStorySelectedMotionSegment(segment = {}) {
  return (
    segment.story_selected_motion_clip === true ||
    segment.provenance?.story_selected_motion_clip === true ||
    cleanText(segment.source_report || segment.provenance?.source_report) ===
      "story_validated_motion_clips"
  );
}

function motionFloorSatisfied(footagePlan = {}) {
  const budget = footagePlan.motion_budget || {};
  const blockers = new Set(asArray(footagePlan.readiness?.blockers));
  return (
    Number(budget.available_motion_clips || 0) >=
      Number(budget.required_motion_scenes || 0) &&
    Number(budget.available_distinct_families || 0) >=
      Number(budget.required_distinct_families || 0) &&
    Number(budget.available_distinct_source_assets || 0) >=
      Number(budget.required_distinct_families || 0) &&
    !blockers.has("actual_motion_clip_minimum_not_met") &&
    !blockers.has("distinct_motion_families_minimum_not_met") &&
    !blockers.has("distinct_motion_source_assets_minimum_not_met")
  );
}

function segmentSelectionScore(segment = {}) {
  let score = Number(segment.action_score || segment.actionScore || 0);
  const timing = selectedTiming(segment);
  const reason = cleanText(segment.validation_reason || segment.reason);
  const shortTrimmedMontage = isShortTrimmedMontage(segment);
  if (
    cleanText(segment.source_report || segment.provenance?.source_report) ===
      "previous_visual_v4_motion_pack" &&
    !shortTrimmedMontage
  ) {
    score += 20;
  }
  if (shortTrimmedMontage) score -= 26;
  if (reason === "segment_samples_passed") score += 8;
  if (Number(timing.durationS || 0) >= 4.5) score += 4;
  return score;
}

function isShortTrimmedMontage(segment = {}) {
  const timing = selectedTiming(segment);
  const reason = cleanText(segment.validation_reason || segment.reason);
  return (
    (timing.trimRecommended || /^trimmed_/i.test(reason)) &&
    Number(timing.durationS || segment.duration_s || segment.durationS || 0) <= 3.2
  );
}

function segmentSort(a, b) {
  const scoreDelta = segmentSelectionScore(b) - segmentSelectionScore(a);
  if (scoreDelta) return scoreDelta;
  const startDelta =
    Number(a.media_start_s ?? a.mediaStartS ?? 0) - Number(b.media_start_s ?? b.mediaStartS ?? 0);
  return startDelta || 0;
}

function segmentStartSort(a, b) {
  const aStart = Number(selectedTiming(a.segment || a).mediaStartS ?? 0);
  const bStart = Number(selectedTiming(b.segment || b).mediaStartS ?? 0);
  return aStart - bStart || segmentSelectionScore(b.segment || b) - segmentSelectionScore(a.segment || a);
}

function buildVisualV4MotionPack({
  story = {},
  trustedFootageReport = {},
  segmentValidationReport = {},
  previousMotionPack = {},
  maxClips = 12,
  minActionScore = MIN_SEGMENT_ACTION_SCORE,
  generatedAt = new Date().toISOString(),
} = {}) {
  const wantedStoryId = storyId(story);
  const trustedFamilies = trustedSourceFamilies(trustedFootageReport, story);
  const accepted = [];
  const rejected = [];
  const usedFamilies = new Set();
  const usedSourceAssets = new Set();
  const sourceAssetFamilies = new Map();
  const duplicateFamilyCandidates = [];
  const duplicateAssetCandidates = [];
  const requirementsProbe = buildFootageEmpirePlan({
    story,
    trustedFootageReport,
    localMotionClips: [],
    generatedAt,
  });
  const requiredMotionScenes = Number(
    requirementsProbe.motion_budget?.required_motion_scenes || 0,
  );
  const requiredDistinctFamilies = Number(
    requirementsProbe.motion_budget?.required_distinct_families || 0,
  );

  function acceptSegment(segment, options = {}) {
    const family = options.sourceFamilyOverride || segmentSourceFamily(segment);
    const sourceAssetKey = segmentSourceAssetKey(segment);
    const canonicalEntity = cleanText(
      story.canonical_subject || story.canonical_game || story.game_title || story.primary_entity,
    );
    const segmentForClip =
      canonicalEntity && !cleanText(segment.entity || segment.provenance?.entity)
        ? { ...segment, entity: canonicalEntity }
        : segment;
    const clip = clipFromSegment(segmentForClip, accepted.length, trustedFamilies);
    if (options.sourceFamilyOverride) {
      clip.source_family = family;
      clip.id = cleanText(segment.clip_id || segment.id || `v4_motion_${accepted.length + 1}_${family}`);
      clip.trusted_source_matched =
        trustedFamilies.size === 0 || family === "unknown" ? trustedFamilies.has(family) : trustedFamilies.has(family);
    }
    clip.provenance.segment_validation_reported_at =
      segmentValidationReport.generated_at || null;
    accepted.push(clip);
    usedFamilies.add(family);
    if (sourceAssetKey) {
      usedSourceAssets.add(sourceAssetKey);
      if (!sourceAssetFamilies.has(sourceAssetKey)) sourceAssetFamilies.set(sourceAssetKey, family);
    }
  }

  function processCandidateSegments(candidateSegments) {
    for (const segment of candidateSegments) {
      const base = {
        story_id: segmentStoryId(segment) || null,
        source_family: segmentSourceFamily(segment),
        base_source_family: segmentSourceAssetKey(segment) || segmentBaseSourceFamily(segment) || null,
        source_url: segmentSourceUrl(segment) || null,
        canonical_source_url:
          segmentSourceIdentityValue(segment, [
            "canonical_source_url",
            "master_source_url",
            "original_source_url",
            "reference_url",
          ]) || null,
        youtube_video_id:
          segmentSourceIdentityValue(segment, [
            "youtube_video_id",
            "source_youtube_id",
          ]) || null,
        media_start_s: numberOrNull(segment.media_start_s ?? segment.mediaStartS),
        duration_s: numberOrNull(segment.duration_s ?? segment.durationS),
        action_score: numberOrNull(segment.action_score ?? segment.actionScore),
        reference_title:
          segment.reference_title ||
          segment.referenceTitle ||
          segment.movie_name ||
          segment.movieName ||
          segment.title ||
          null,
      };
      const reason = candidateRejectionReason(segment, {
        storyId: wantedStoryId,
        minActionScore,
        story,
      });
      if (reason) {
        rejected.push({ ...base, reason });
        continue;
      }

      const sourceAssetKey = segmentSourceAssetKey(segment);
      if (sourceAssetKey && usedSourceAssets.has(sourceAssetKey)) {
        duplicateAssetCandidates.push({ segment, base });
        continue;
      }
      const family = segmentSourceFamily(segment);
      if (usedFamilies.has(family)) {
        duplicateFamilyCandidates.push({ segment, base });
        continue;
      }
      acceptSegment(segment);
      if (accepted.length >= Math.max(1, Number(maxClips) || 12)) break;
    }
  }

  const maxClipCount = Math.max(1, Number(maxClips) || 12);
  const targetClipCount = Math.min(
    maxClipCount,
    Math.max(
      requiredMotionScenes || 0,
      5,
      PREMIUM_BENCHMARK_TARGET_MOTION_SCENES,
    ),
  );
  const previousSegments = previousMotionPackSegments(previousMotionPack);
  const storySelectedSegments = previousSegments.filter(isStorySelectedMotionSegment).sort(segmentSort);
  const broadDiscoverySegments = [
    ...previousSegments.filter((segment) => !isStorySelectedMotionSegment(segment)),
    ...asArray(segmentValidationReport.segments),
  ].sort(segmentSort);

  processCandidateSegments(storySelectedSegments);
  const selectedFootagePlan = storySelectedSegments.length
    ? buildFootageEmpirePlan({
        story,
        trustedFootageReport,
        localMotionClips: accepted,
        generatedAt,
      })
    : null;
  const selectedClipsMeetMotionFloor =
    selectedFootagePlan && motionFloorSatisfied(selectedFootagePlan);

  if (!selectedClipsMeetMotionFloor && accepted.length < maxClipCount) {
    processCandidateSegments(broadDiscoverySegments);
  }

  duplicateFamilyCandidates.forEach((item) => {
    rejected.push({ ...item.base, reason: "source_family_already_used" });
  });
  duplicateAssetCandidates.forEach((item) => {
    rejected.push({ ...item.base, reason: "source_asset_already_used" });
  });

  const footagePlan = buildFootageEmpirePlan({
    story,
    trustedFootageReport,
    localMotionClips: accepted,
    generatedAt,
  });

  return {
    schema_version: 1,
    generated_at: generatedAt,
    execution_mode: "visual_v4_motion_pack_builder",
    local_only: true,
    story_id: wantedStoryId || null,
    title: story.title || null,
    readiness: footagePlan.readiness,
    clips: accepted,
    rejected_candidates: rejected,
    motion_budget: footagePlan.motion_budget,
    trusted_source_pipeline: footagePlan.trusted_source_pipeline,
    handoff: {
      visual_v4_local_motion_clips: accepted,
    },
    safety: {
      local_only: true,
      planner_only: true,
      video_downloads_started: false,
      retained_video_files: false,
      browser_scraping_started: false,
      yt_dlp_started: false,
      oauth_triggered: false,
      production_db_mutated: false,
      railway_mutated: false,
      social_posting_triggered: false,
      elevenlabs_required: false,
    },
  };
}

function renderVisualV4MotionPackMarkdown(pack = {}) {
  const lines = [];
  lines.push("# Visual V4 Motion Pack");
  lines.push("");
  lines.push(`Generated: ${pack.generated_at || "unknown"}`);
  lines.push(`Story: ${pack.story_id || "unknown"}`);
  lines.push(`Readiness: ${pack.readiness?.status || "unknown"}`);
  lines.push("");
  lines.push("## Summary");
  lines.push("");
  lines.push(`- clips: ${asArray(pack.clips).length}`);
  lines.push(`- rejected candidates: ${asArray(pack.rejected_candidates).length}`);
  lines.push(
    `- distinct source families: ${pack.motion_budget?.available_distinct_families ?? 0}/${pack.motion_budget?.required_distinct_families ?? "unknown"}`,
  );
  lines.push(
    `- motion scenes: ${pack.motion_budget?.available_motion_clips ?? 0}/${pack.motion_budget?.required_motion_scenes ?? "unknown"}`,
  );
  lines.push("");
  lines.push("## Clips");
  lines.push("");
  lines.push("| family | start | duration | score | source kind | rights |");
  lines.push("| --- | ---: | ---: | ---: | --- | --- |");
  for (const clip of asArray(pack.clips)) {
    lines.push(
      [
        clip.source_family || "unknown",
        clip.mediaStartS ?? "unknown",
        clip.durationS ?? "unknown",
        clip.provenance?.segment_action_score ?? "unknown",
        clip.source_url_kind || clip.source_kind || "unknown",
        clip.rights_risk_class || "unknown",
      ]
        .map((value) => String(value).replace(/\|/g, "/"))
        .join(" | ")
        .replace(/^/, "| ")
        .replace(/$/, " |"),
    );
  }
  if (!asArray(pack.clips).length) {
    lines.push("| none | 0 | 0 | 0 | none | none |");
  }
  lines.push("");
  lines.push("## Rejected");
  lines.push("");
  lines.push("| family | reason | source |");
  lines.push("| --- | --- | --- |");
  for (const rejected of asArray(pack.rejected_candidates).slice(0, 20)) {
    lines.push(
      `| ${rejected.source_family || "unknown"} | ${rejected.reason || "unknown"} | ${rejected.source_url || "none"} |`,
    );
  }
  if (!asArray(pack.rejected_candidates).length) {
    lines.push("| none | none | none |");
  }
  lines.push("");
  lines.push("## Safety");
  lines.push("");
  lines.push("- Local manifest only.");
  lines.push("- No downloads, retained video files, browser scraping, yt-dlp, DB mutation, OAuth or posting.");
  return lines.join("\n") + "\n";
}

function applyVisualV4MotionPackToStory(story = {}, pack = {}) {
  const clips = asArray(pack.handoff?.visual_v4_local_motion_clips || pack.clips);
  story.visual_v4_motion_pack = pack;
  story.visual_v4_local_motion_clips = clips;
  story.visual_v4_motion_pack_status = pack.readiness?.status || "unknown";
  story.visual_v4_motion_pack_clip_count = clips.length;
  return story;
}

module.exports = {
  buildVisualV4MotionPack,
  renderVisualV4MotionPackMarkdown,
  applyVisualV4MotionPackToStory,
};
