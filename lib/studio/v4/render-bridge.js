...
function sourceAssetKeyForClip(clip = {}) {
  return cleanText(
    sourceUrlForClip(clip) ||
...
function sourceWindowForClip(clip = {}, shot = {}) {
  const source = sourceAssetKeyForClip(clip);
  const start =
    numberOrNull(clip.mediaStartS ?? clip.media_start_s) ??
    numberOrNull(clip.materialized_media_start_s ?? clip.source_start_s) ??
    numberOrNull(shot.mediaStartS ?? shot.media_start_s);
  const duration =
    numberOrNull(shot.durationS ?? shot.duration_s) ??
    numberOrNull(clip.durationS ?? clip.duration_s);
  if (!source || start === null || duration === null || duration <= 0) return null;
  return {
    source,
    start,
    end: start + duration,
...
function sourceWindowsOverlap(a = {}, b = {}) {
  ...
  return overlapEnd - overlapStart > 0.25;
}
...
function buildStudioV4RenderBridge({
  ...
  for (const shot of shots.filter((item) => item?.kind === "motion_clip")) {
    ...
    const sourceWindow = sourceWindowForClip(clip, shot);
    if (
      sourceWindow &&
      usedSourceWindows.some((usedWindow) => sourceWindowsOverlap(sourceWindow, usedWindow))
    ) {
      rejected.push({
        id: shot.id || null,
        source_family: family,
        source_url: sourceUrlForClip(clip),
        mediaStartS: sourceWindow.start,
        durationS: sourceWindow.end - sourceWindow.start,
        reason: "duplicate_source_window",
      });
      continue;
    }
    ...
    if (sourceWindow) usedSourceWindows.push(sourceWindow);
  }
