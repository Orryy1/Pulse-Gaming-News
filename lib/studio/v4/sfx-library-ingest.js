...
function explicitRoleForPath(filePath) {
  const normalised = normalisePath(filePath).toLowerCase();
  const segments = normalised.split("/");
  for (const role of ["impact", "transition", "ui_tick", "riser", "sub_hit", "glitch"]) {
    if (segments.includes(role) || normalised.includes(`epidemic_${role}_`)) return role;
  }
  return "";
}
...
function roleForPath(filePath) {
  ...
  const explicitRole = explicitRoleForPath(filePath);
