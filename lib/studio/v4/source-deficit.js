...
function clipFamilyEvidenceComplete(pack = {}) {
  const clips = asArray(pack.clips);
  if (!clips.length) return false;
  const budget = pack.motion_budget || {};
  const availableMotionClips = numberOrZero(budget.available_motion_clips);
  if (!availableMotionClips) return true;
  return clips.length >= availableMotionClips;
}
...
function buildRow({
  ...
  const currentMotionClips = numberOrZero(
    budget.available_motion_clips ?? clips.length,
  );
  const requiredMotionClips = numberOrZero(budget.required_motion_scenes);
  const currentMotionFamilies = numberOrZero(
    clipFamilyEvidenceComplete(pack) ? families.length : budget.available_distinct_families,
  );
  ...
  const visualFamilyBlockers =
    clips.length > 0 &&
    requiredMotionFamilies > 0 &&
  ...
  return {
    ...
    blockers: [...new Set([...asArray(pack.readiness?.blockers), ...visualFamilyBlockers])],
