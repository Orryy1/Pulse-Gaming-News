...
function markDirectVideoEnrichmentRequests(packs = [], workOrder = {}) {
  ...
  if (!requestedStoryIds.size && !realVisualRequiredStoryIds.size) return asArray(packs);
  return asArray(packs).map((pack) => {
    const storyId = storyIdFromPack(pack);
    if (!requestedStoryIds.has(storyId) && !realVisualRequiredStoryIds.has(storyId)) return pack;
    return {
      ...pack,
...
function buildStudioV4SourceFamilyAcquisitionReport({
  ...
  const noDeadEndBlockers = rows.every(rowHasAcquisitionRunway);
