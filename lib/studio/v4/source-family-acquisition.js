...
const { visualReadinessFamily } = require("../../visual-evidence-classifier");
...
const ENTITY_SUPPORT_MODIFIER_TOKENS = new Set([
  "official",
  "wireless",
  "limited",
  "edition",
...
function supportTextMentionsSpecificEntity(supportText, entity) {
  const text = normaliseMatchText(supportText);
  const normalised = normaliseMatchText(entity);
  if (!text || !normalised) return false;
  if (titleContainsSpecificEntity(text, normalised)) return true;
  ...
  return overlap.some(
    (token) => !ENTITY_SUPPORT_MODIFIER_TOKENS.has(token) && !BROAD_PLATFORM_OR_COMPANY_ENTITIES.has(token),
  );
}
...
function knownGameAliasFromText(value = "") {
  ...
  if (/\bDoom\s*:?\s*The\s+Dark\s+Ages\b/i.test(text)) return "Doom: The Dark Ages";
  if (/\bCyberpunk 2077\b/i.test(text)) return "Cyberpunk 2077";
  if (/\bHellraiser\s*:?\s+Revival\b/i.test(text)) return "Hellraiser: Revival";
  if (/\bOcarina\b/i.test(text)) return "Ocarina of Time";
  if (/\bBlack Ops\b/i.test(text)) return "Call of Duty: Black Ops";
  if (/\bGuilty\s+Gear\s*[-:]?\s*Strive\b/i.test(text)) return "GUILTY GEAR -STRIVE-";
  if (/\bHalo\b/i.test(text)) return "Halo: Campaign Evolved";
    ...
    asArray(workOrder.jobs)
      .filter((job) => {
        ...
        return (
          ...
          typeText.includes("direct video motion clips") ||
          typeText.includes("additional direct video motion required") ||
          typeText.includes("materialise validated real motion clips") ||
          typeText.includes("official direct video sources")
        );
      })
...
function referencePlanStoryEntities(referencePlan = {}, pack = {}) {
  ...
  function canAddEntity(entity, options = {}) {
    if (options.allowSibling === true) return true;
    return (
  ...
  function addEntity(entity, options = {}) {
    ...
    if (text && canAddEntity(text, options)) entities.push(text);
  }
  ...
  for (const reference of asArray(referencePlan.references)) {
    ...
    if (
      titleContainsSpecificEntity(titleText, entity) ||
      supportTextMentionsSpecificEntity(packEntitySupportText(referencePlan, pack), entity)
    ) {
      addEntity(entity, { allowSibling: true });
    }
  }
...
function storyLooksLikeAdaptationOrStreaming(text = "") {
  const source = cleanText(text);
  if (!source) return false;
  const hasScreenOrStreamerContext =
    /\b(?:netflix|hbo|max|prime\s+video|amazon\s+prime|disney\+|disney\s+plus|crunchyroll|paramount\+|paramount\s+plus|hulu|peacock|streaming|streamer|anime|live[- ]action|tv\s+series|television\s+series|movie|film|show|season\s+\d+)\b/i.test(
      source,
    );
  const hasAdaptationContext =
    /\b(?:adapt(?:s|ed|ing|ation)|live[- ]action|tv\s+series|television\s+series|anime|movie|film|show|series)\b/i.test(
      source,
    );
  return hasScreenOrStreamerContext && hasAdaptationContext;
}
...
function visualSearchPolicyForEntity(entity = "", pack = {}) {
  ...
  if (
    ...
    /\b(?:exclusive|exclusives|feedback|strategy|review|reevaluate|re-evaluate|moneti[sz]ation|subscription|game\s+pass|dashboard|account|sign[- ]?in|login|cross[- ]?platform|platform\s+holder|first[- ]?party|layoff|layoffs|studio\s+closure|business\s+pressure)\b/i.test(text)
  ) {
    return {
  ...
  if (storyLooksLikeAdaptationOrStreaming(text)) {
    return {
      blockers: ["adaptation_story_requires_source_card_or_approved_media"],
      accepted_sources: [
        "official streamer or studio source",
        "official publisher or rights-holder announcement",
        "licensed trailer or promotional media",
        "operator-approved source cards",
      ],
      visual_plan_type: "adaptation_owned_or_approved_media_plan",
      visual_plan_reason:
        "Adaptation, anime and streaming stories need official announcement cards or approved promo media, not gameplay from a related game.",
    };
  }
...
const TITLE_ENTITY_REQUIREMENTS = [
  { title: "gta vi", requiredEntity: "grand theft auto vi", suggestedEntity: "Grand Theft Auto VI" },
  { title: "grand theft auto vi", requiredEntity: "grand theft auto vi", suggestedEntity: "Grand Theft Auto VI" },
  { title: "doom the dark ages", requiredEntity: "doom the dark ages", suggestedEntity: "Doom: The Dark Ages" },
  { title: "cyberpunk 2077", requiredEntity: "cyberpunk 2077" },
  { title: "hellraiser revival", requiredEntity: "hellraiser revival" },
  { title: "halo", requiredEntity: "halo", suggestedEntity: "Halo: Campaign Evolved" },
  { title: "black ops", requiredEntity: "call of duty black ops" },
  { title: "ocarina", requiredEntity: "ocarina of time" },
  ...
  { title: "xbox", requiredEntity: "xbox", suggestedEntity: "Xbox" },
...
function titleSpecificEntityMismatchBlockers(entity = "", pack = {}) {
  ...
  const key = normaliseMatchText(entity);
  if (!title || !key) return [];
  if (titleContainsSpecificEntity(title, entity)) return [];
  const matchedRequirements = TITLE_ENTITY_REQUIREMENTS.filter((requirement) =>
    title.includes(requirement.title),
  );
  if (matchedRequirements.some((requirement) => key.includes(requirement.requiredEntity))) {
    return [];
  }
  return matchedRequirements.some((requirement) => !key.includes(requirement.requiredEntity))
...
function titleRequiredEntity(title = "") {
  ...
  return match ? firstText(match.suggestedEntity, match.requiredEntity) : "";
}

const TITLE_SHAPED_ENTITY_PATTERNS = [
  /\b(?:face|faces|gets|got|returns|confirms|confirmed|ends|shows|goes|reveals|reports|claims|has|have|will|could|may|just|finally)\b/,
  /\b(?:trust debt|trust problem|price test|remake pressure|hidden [a-z0-9 ]{0,30}clue|strategy trust problem)\b/,
];

function titleShapedPrimaryEntityBlockers(entity = "", pack = {}) {
  const key = normaliseMatchText(entity);
  const title = normaliseMatchText(pack.title || pack.selected_title || pack.canonical_title);
  if (!key || !title) return [];
  const explicitCanonicalFields = [
    pack.canonical_subject,
    pack.canonical_game,
  ].map(normaliseMatchText).filter(Boolean);
  if (!explicitCanonicalFields.some((field) => field === key)) return [];
  const titleMatchesEntity =
    key === title ||
    (key.length >= 12 && title.includes(key)) ||
    (title.length >= 12 && key.includes(title));
  if (!titleMatchesEntity) return [];
  if (TITLE_SHAPED_ENTITY_PATTERNS.some((pattern) => pattern.test(key))) {
    return ["title_shaped_primary_entity"];
  }
  return [];
}
...
function governedVisualPlanAllowedAssetClasses(planType) {
  ...
  if (planType === "broad_platform_owned_explainer_plan") {
    return [
      "premium HyperFrames explainer cards",
      "owned platform strategy graphics",
      "official platform-holder source cards",
      "approved logo usage with rights record",
      "operator-approved direct platform media",
    ];
  }
  ...
  if (planType === "adaptation_owned_or_approved_media_plan") {
    return [
      "official streamer or studio trailer media",
      "official announcement source cards",
      "owned adaptation context cards",
      "licensed promotional stills or clips",
      "premium HyperFrames explainer cards",
    ];
  }
  return [
...
function governedVisualPlanForEntity({
  ...
  return {
    ...
    required_artefacts: [
      ...
      ...(cleanText(planType).includes("owned_explainer")
        ? [
            "hyperframes_lint_validate_inspect_report.json",
            "premium_shell_gate_report.json",
          ]
        : []),
    ],
    required_approvals: [
...
function sourceSearchBlockersForPrimaryEntity(entity = "", pack = {}, referencePlan = {}) {
  ...
  blockers.push(...titleShapedPrimaryEntityBlockers(text, pack));
...
function candidateFromStoryPackageReference(item = {}, pack = {}, referencePlan = {}, origin = "story_package_reference") {
  const sourceUrl = firstText(item.source_url, item.sourceUrl, item.url, item.href);
  const referenceUrl = firstText(
    item.reference_url,
    item.referenceUrl,
    item.reference_page_url,
    item.official_source_url,
    pack.primary_source_url,
    sourceUrl,
  );
  const entity = firstText(
    item.entity,
    item.canonical_subject,
    item.game,
    item.title_entity,
    primaryEntityForPack(pack, referencePlan),
  );
  const candidate = candidateFromReference(
    {
      ...
      ...item,
      title: firstText(item.title, item.label, item.display_name, item.name),
      entity,
      source_url: sourceUrl,
      reference_url: referenceUrl,
    },
  ...
  return {
    ...
    evidence_of_officialness: firstText(
      item.evidence_of_officialness,
      item.evidence,
      item.officialness_evidence,
      "Story package supplied this official motion reference.",
    ),
  ...
  return Array.from(
    new Set(
      asArray(pack.clips)
        .map((clip, index) =>
          normaliseFamily(
            visualReadinessFamily(clip, index) ||
              clip.source_family ||
              clip.sourceFamily,
          ),
        )
...
function clipBaseSourceRoot(clip = {}, index = 0) {
  const explicit = cleanText(
    clip.base_source_family ||
      ...
      clip.provenance?.baseSourceFamily ||
      clip.provenance?.source_root_key ||
      clip.provenance?.sourceRootKey,
  );
  if (explicit) return explicit.replace(/\\/g, "/").toLowerCase();
  return (
    visualReadinessFamily(clip, index) ||
    cleanText(clip.source_family || clip.sourceFamily).replace(/\\/g, "/").toLowerCase()
  );
}

function baseSourceRootCounts(pack = {}) {
  const counts = new Map();
  asArray(pack.clips).forEach((clip, index) => {
    const root = clipBaseSourceRoot(clip, index);
    if (!root) return;
...
function overusedBaseSourceRoots(pack = {}) {
  return Array.from(baseSourceRootCounts(pack).entries())
    .filter(([, count]) => count > 1)
    .map(([root, count]) => ({ root, count }))
    .sort((a, b) => b.count - a.count || a.root.localeCompare(b.root));
}

function currentMotionFamilyCount(pack = {}) {
  const clips = asArray(pack.clips);
  const availableMotionClips = numberOrZero(pack.motion_budget?.available_motion_clips);
  if (clips.length && (!availableMotionClips || clips.length >= availableMotionClips)) {
    return currentFamilyNames(pack).length;
  }
  return numberOrZero(pack.motion_budget?.available_distinct_families);
}

function hasOfficialProductMotionGap(pack = {}) {
  return asArray(pack.readiness?.blockers).some((blocker) =>
    [
...
function hasVisualSourceFamilyGap(pack = {}) {
  const required = numberOrZero(pack.motion_budget?.required_distinct_families);
  const availableMotionClips = numberOrZero(pack.motion_budget?.available_motion_clips);
  const clips = asArray(pack.clips);
  return (
    clips.length > 0 &&
    (!availableMotionClips || clips.length >= availableMotionClips) &&
    required > 0 &&
    currentMotionFamilyCount(pack) < required
  );
}

function hasBaseSourceRootOveruse(pack = {}) {
  const clips = asArray(pack.clips);
  const availableMotionClips = numberOrZero(pack.motion_budget?.available_motion_clips);
  return (
    clips.length > 0 &&
    (!availableMotionClips || clips.length >= availableMotionClips) &&
    overusedBaseSourceRoots(pack).length > 0
  );
}
...
function collectSourceFamilyCandidates({ pack = {}, trustedFootageReport = {}, referenceReport = {} } = {}) {
  ...
  for (const item of [
    ...asArray(trustedFootageReport.accepted_references),
    ...asArray(trustedFootageReport.render_ready_sources),
    ...asArray(trustedFootageReport.rows),
  ].filter(trustedReportRowUsable)) {
    const itemStoryId = cleanText(item.story_id);
    if (itemStoryId && itemStoryId !== storyId) continue;
    const candidate = candidateFromTrustedReport(item, pack, referencePlan);
    if (!itemStoryId) {
  ...
  for (const ref of asArray(pack.official_motion_references)) {
    add(candidateFromStoryPackageReference(ref, pack, referencePlan, "story_package_official_motion_reference"));
  }
  for (const ref of asArray(pack.trailer_references)) {
    add(candidateFromStoryPackageReference(ref, pack, referencePlan, "story_package_trailer_reference"));
  }
...
function safeNextCommands(
  ...
  const paths = {
    ...
    officialSearchAutofillJson:
      cleanText(commandPaths.officialSearchAutofillJson) ||
      "test/output/official_search_intake_autofill.json",
    officialSearchAutofillMd:
      cleanText(commandPaths.officialSearchAutofillMd) ||
      "test/output/official_search_intake_autofill.md",
    officialSearchAutofillTemplate:
      cleanText(commandPaths.officialSearchAutofillTemplate) ||
      "test/output/visual_v4_source_family_intake_template_autofill.json",
    storyJsonPath: cleanText(commandPaths.storyJsonPath),
    sourceFamilyReport:
      cleanText(commandPaths.sourceFamilyReport) ||
      "test/output/studio_v4_source_family_acquisition.json",
  ...
  const storyJsonFlag = paths.storyJsonPath ? ` --story-json ${paths.storyJsonPath}` : "";
  ...
  if (hasSourceIntake) {
    commands.push(
      ...
      },
      {
        ...
        command:
          `npm run media:intake-official-sources -- --input ${paths.officialDirectMediaIntakeTemplate}${suffix}${storyJsonFlag}`,
      },
    );
  } else if (hasOfficialSearch) {
    commands.push(
      {
        step: "autofill_official_source_intake_from_search_template",
        command:
          `npm run media:autofill-official-source-intake -- --input ${paths.officialSearchTemplate}` +
          ` --merge-input ${paths.sourceFamilyIntakeTemplate}${suffix}` +
          ` --output-json ${paths.officialSearchAutofillJson}` +
          ` --output-md ${paths.officialSearchAutofillMd}` +
          ` --output-template ${paths.officialSearchAutofillTemplate}`,
      },
      {
        step: "discover_direct_media_from_autofilled_official_sources",
        command:
          `npm run media:discover-direct-media -- --input ${paths.officialSearchAutofillTemplate}${suffix}` +
          ` --output-json ${paths.officialDirectMediaDiscoveryJson}` +
          ` --output-md ${paths.officialDirectMediaDiscoveryMd}` +
          ` --output-template ${paths.officialDirectMediaIntakeTemplate}`,
      },
      {
        step: "validate_operator_supplied_official_sources",
        command:
      ...
      },
      {
        ...
        command:
          `manual: if autofill finds no official source, use ${paths.officialSearchTemplate} to add official source URLs to ${paths.sourceFamilyIntakeTemplate}${storyId ? ` for ${storyId}` : ""}`,
      },
    );
  ...
  commands.push(
    {
      ...
      command:
        `npm run ops:v4-licensed-direct-media --${suffix}` +
        ` --source-family-report ${paths.sourceFamilyReport}` +
        ` --direct-media-report ${paths.officialDirectMediaDiscoveryJson}` +
        ` --operator-intake ${paths.officialDirectMediaIntakeTemplate}` +
        ` --output-json ${paths.licensedDirectMediaReport}` +
        ` --output-md ${paths.licensedDirectMediaReport.replace(/\.json$/i, ".md")}` +
        ` --intake-template ${paths.licensedDirectMediaReport.replace(/\.json$/i, "_intake_template.json")}`,
    },
    {
      step: "resolve_trailer_references",
      command:
        `npm run media:resolve-trailers --${suffix}${storyJsonFlag}` +
        ` --official-source-intake-report ${paths.licensedDirectMediaReport}` +
        ` --trusted-footage-registry-report ${paths.trustedFootageRegistryReport}` +
        ` --segment-validation-report ${paths.segmentValidationReport}` +
        ` --write-latest-report`,
    },
    {
...
function buildRow({ pack = {}, trustedFootageReport = {}, referenceReport = {}, commandPaths = {} } = {}) {
  const storyId = storyIdFromPack(pack);
  const officialProductMotionGap = hasOfficialProductMotionGap(pack);
  const directVideoEnrichmentRequested =
    pack.direct_video_enrichment_requested === true || officialProductMotionGap;
  const realVisualMediaRequiredAfterOwnedExplainerFailed =
    pack.real_visual_media_required_after_owned_explainer_failed === true;
  const readinessStatus = cleanText(pack.readiness?.status || "unknown");
  const visualSourceFamilyGap = hasVisualSourceFamilyGap(pack);
  const overusedRoots = overusedBaseSourceRoots(pack);
  const baseSourceRootOveruse = overusedRoots.length > 0;
  const blockingCurrentMotionReadiness =
    readinessStatus !== "v4_motion_ready" || visualSourceFamilyGap || baseSourceRootOveruse;
  ...
  const requiredMotionClips = Math.max(
    numberOrZero(budget.required_motion_scenes),
    officialProductMotionGap ? 5 : 0,
  );
  const currentMotionFamilies = currentMotionFamilyCount(pack);
  const requiredMotionFamilies = Math.max(
    numberOrZero(budget.required_distinct_families),
    officialProductMotionGap ? 5 : 0,
  );
  ...
  const referenceSourceProofCovered = asArray(referencePlan.source_proof_covered_target_entities);
  const candidateSourceProofCovered =
    !referenceSourceProofCovered.length &&
    cleanText(primaryStoryEntity) &&
    sourceFamilyCandidates.some((candidate) =>
      candidateMatchesStory(candidate, [primaryStoryEntity]),
    )
      ? [primaryStoryEntity]
      : [];
  const sourceProofCovered = referenceSourceProofCovered.length
    ? referenceSourceProofCovered
    : candidateSourceProofCovered;
  const sourceProofMissing = sourceProofCovered.length
    ? []
    : asArray(referencePlan.source_proof_missing_target_entities);

  return {
    story_id: storyId,
    title: cleanText(pack.title),
    primary_story_entity: primaryStoryEntity,
    source_proof_covered_target_entities: sourceProofCovered,
    source_proof_missing_target_entities: sourceProofMissing,
    ...
    blockers: [
      ...
      ...(directVideoEnrichmentRequested
        ...
        : []),
      ...(officialProductMotionGap
        ? ["visual_evidence:official_product_motion_gap"]
        : []),
    ],
    ...
    overused_base_source_roots: overusedRoots,
    base_source_root_overuse_count: overusedRoots.length,
...
function buildSourceFamilyIntakeTemplate(rows = []) {
  ...
  for (const row of asArray(rows)) {
    for (const candidate of asArray(row.source_family_candidates)) {
      ...
      const entity = cleanText(candidate.entity || row.primary_story_entity || row.title);
      const entry = {
...
function buildAcquisitionRunway(
  ...
  if (asArray(canonicalEntityRepairEntries).length > 0) {
    return withWorkOrderCounts({
      ...
      next_action:
        "Repair malformed, generic or title-shaped canonical entities before source-family acquisition can safely search official media.",
      source_intake_rows: 0,
  ...
  const rows = packs
    .filter(
      (pack) =>
        ...
        hasVisualSourceFamilyGap(pack) ||
        hasBaseSourceRootOveruse(pack) ||
    ...
    .filter(
      (row) =>
        ...
        row.base_source_root_overuse_count > 0 ||
