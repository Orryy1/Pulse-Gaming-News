...
const DIRECT_MOTION_VISUAL_SELECTOR_V5 = Object.freeze({
  ...
  persistent_letterbox_min_ratio: 0.08,
  persistent_letterbox_min_consecutive_samples: 2,
  persistent_lower_band_min_likelihood: 0.03,
  persistent_lower_band_min_consecutive_samples: 2,
...
function isVerifiedOwnedSupportCardClip(clip) {
  const mediaKind = String(clip?.media_kind || clip?.mediaKind || "").toLowerCase();
  const sourceType = String(clip?.source_type || clip?.sourceType || "").toLowerCase();
  const designRole = String(
    clip?.generator_design_role || clip?.generatorDesignRole || "",
  ).toLowerCase();
  const readableCardKind = String(
    clip?.readable_card_kind || clip?.readableCardKind || "",
  ).toLowerCase();
  const cardKind = String(clip?.card_kind || clip?.cardKind || "").toLowerCase();
  const rightsEvaluation = clip?.owned_rights_evaluation || clip?.ownedRightsEvaluation || {};
  const rightsBlockers = Array.isArray(rightsEvaluation.blockers)
    ? rightsEvaluation.blockers.filter(Boolean)
    : [];
  const grant = clip?.owned_generated_rights_grant || clip?.ownedGeneratedRightsGrant || {};
  ...
  return (
    ...
    designRole === "support_card" &&
    clip?.hyperframes_card === true &&
    allowedCardKinds.has(readableCardKind) &&
    cardKind === readableCardKind &&
    clip?.materialized === true &&
    clip?.counts_towards_motion_readiness === true &&
    clip?.source_safety_blocked === false &&
    Boolean(String(clip?.generator_project_id || "").trim()) &&
    isSha256(clip?.generator_master_sha256) &&
    isSha256(clip?.materialised_output_sha256 || clip?.materialized_output_sha256) &&
    isSha256(clip?.evidence_file_sha256) &&
    isSha256(clip?.rights_evidence_file_sha256) &&
    String(rightsEvaluation.status || "").toLowerCase() === "pass" &&
    rightsEvaluation.verified === true &&
    rightsBlockers.length === 0 &&
    String(grant.grant_type || "").toLowerCase() === "owned_generated" &&
    String(grant.allowed_use || "").toLowerCase() ===
      "commercial_editorial_and_platform_native_derivatives" &&
    grant.commercial_use_allowed === true &&
    grant.derivative_use_allowed === true
  );
}
...
function isGeneratedCardClip(clip) {
  ...
  return (
    ...
    isVerifiedOwnedSupportCardClip(clip)
  );
}
...
function scoreDirectMotionVisualSamples(samples = []) {
  ...
  const letterboxFlags = rows.map(
    (sample) =>
      finiteNumber(sample?.letterbox_bar_ratio) >=
      DIRECT_MOTION_VISUAL_SELECTOR_V5.persistent_letterbox_min_ratio,
  );
  let letterboxRun = 0;
  let longestLetterboxRun = 0;
  for (const flagged of letterboxFlags) {
    letterboxRun = flagged ? letterboxRun + 1 : 0;
    longestLetterboxRun = Math.max(longestLetterboxRun, letterboxRun);
  }
  const letterboxCount = letterboxFlags.filter(Boolean).length;
  const lowerBandOverlayFlags = rows.map(
    (sample) =>
      finiteNumber(sample?.lower_band_overlay_likelihood) >=
      DIRECT_MOTION_VISUAL_SELECTOR_V5.persistent_lower_band_min_likelihood,
  );
  let lowerBandOverlayRun = 0;
  let longestLowerBandOverlayRun = 0;
  for (const flagged of lowerBandOverlayFlags) {
    lowerBandOverlayRun = flagged ? lowerBandOverlayRun + 1 : 0;
    longestLowerBandOverlayRun = Math.max(
      longestLowerBandOverlayRun,
      lowerBandOverlayRun,
    );
  }
  const lowerBandOverlayCount = lowerBandOverlayFlags.filter(Boolean).length;
  ...
  if (
    longestLetterboxRun >=
    DIRECT_MOTION_VISUAL_SELECTOR_V5.persistent_letterbox_min_consecutive_samples
  ) {
    reasons.push("direct_motion_persistent_letterbox_risk");
  }
  if (
    longestLowerBandOverlayRun >=
    DIRECT_MOTION_VISUAL_SELECTOR_V5.persistent_lower_band_min_consecutive_samples
  ) {
    reasons.push("direct_motion_persistent_lower_band_overlay_risk");
  }
  if (
    longestDarkLowInformationRun >=
  ...
  return {
    ...
    metrics: {
      ...
      letterbox_sample_count: letterboxCount,
      longest_letterbox_run: longestLetterboxRun,
      lower_band_overlay_sample_count: lowerBandOverlayCount,
      longest_lower_band_overlay_run: longestLowerBandOverlayRun,
...
function isFooterOnlyLowerBandFailure(report = {}) {
  ...
  const footerReasonSet = new Set([
    "direct_motion_frame_taste_failed",
    "direct_motion_persistent_lower_band_overlay_risk",
  ]);
  const onlyFooterReasons =
    reasons.length > 0 && reasons.every((reason) => footerReasonSet.has(reason));
  const persistentThinFooter =
    !reasons.includes("direct_motion_persistent_lower_band_overlay_risk") ||
    Number(report?.metrics?.longest_lower_band_overlay_run || 0) >=
      DIRECT_MOTION_VISUAL_SELECTOR_V5.persistent_lower_band_min_consecutive_samples;
  const failedTasteIsFooterOnly =
    !reasons.includes("direct_motion_frame_taste_failed") ||
    (
  ...
  return (
    onlyFooterReasons &&
    persistentThinFooter &&
    failedTasteIsFooterOnly &&
...
function resolveDirectMotionFrameDir(
  filePath,
  outputDir,
  {
    cwd = process.cwd(),
    platform = process.platform,
    maxSafeFramePathLength = 239,
  } = {},
) {
  const resolvedFilePath = path.resolve(filePath);
  const resolvedOutputDir = path.resolve(
    outputDir || path.join(os.tmpdir(), "pulse-direct-motion-v5"),
  );
  const safeId = crypto
    .createHash("sha1")
    ...
    .digest("hex")
    .slice(0, 12);
  const requestedFrameDir = path.join(resolvedOutputDir, safeId);
  const requestedLongestFramePath = path.join(
    requestedFrameDir,
    `frame_${String(DIRECT_MOTION_VISUAL_SELECTOR_V5.sample_count).padStart(2, "0")}.jpg`,
  );
  const compacted =
    platform === "win32" &&
    requestedLongestFramePath.length > maxSafeFramePathLength;
  const outputIdentity = crypto
    .createHash("sha1")
    .update(`${resolvedOutputDir}|${resolvedFilePath}`)
    .digest("hex")
    .slice(0, 12);
  const frameDir = compacted
    ? path.join(
        path.resolve(cwd, "output", "qa", "direct-motion-v5-frames"),
        outputIdentity,
        safeId,
      )
    : requestedFrameDir;
  ...
  return {
    frame_dir: frameDir,
    requested_frame_dir: requestedFrameDir,
    compacted,
    max_safe_frame_path_length: maxSafeFramePathLength,
  };
}
...
async function inspectDirectMotionClip(clip, { outputDir } = {}) {
  ...
  const frameStorage = resolveDirectMotionFrameDir(filePath, outputDir);
  const frameDir = frameStorage.frame_dir;
  ...
  for (const clip of Array.isArray(clips) ? clips : []) {
    ...
    if (
      ...
      try {
        ...
        let repairedInspectionAttempts = 1;
        let repairedReport = attachExactClipEvidence(
          repairedClip,
          await inspectClip(repairedClip, {
            outputDir: path.join(outputDir, "repaired-frame-samples"),
          }),
        );
        const repairedReasons = (Array.isArray(repairedReport.reasons)
          ? repairedReport.reasons
          : [])
          .map((value) => String(value || "").trim())
          .filter(Boolean);
        if (
          !repairedReport.eligible &&
          repairedReasons.length === 1 &&
          repairedReasons[0] === "direct_motion_visual_analysis_failed"
        ) {
          repairedInspectionAttempts += 1;
          repairedReport = attachExactClipEvidence(
            repairedClip,
            await inspectClip(repairedClip, {
              outputDir: path.join(outputDir, "repaired-frame-samples-retry"),
            }),
          );
        }
        const repairEvidence = {
          ...
          inspection_attempts: repairedInspectionAttempts,
        };
