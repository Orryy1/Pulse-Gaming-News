...
const KINETIC_TYPOGRAPHY_V5 = Object.freeze({
  ...
  dangling_merge_max_words: 5,
  max_single_word_caption_ratio: 0.12,
  reveal_mode: "phrase",
  motion_style: "editorial",
});

const DANGLING_FUNCTION_WORD_TAIL_RE = /\b(?:A|AN|THE|AND|OR|TO|OF|FOR|WITH|IN|ON)$/;
...
function inspectPremiumCaptionCadence(ass = "") {
  ...
  const maximumWords = captions.length
    ...
    : 0;
  const danglingFunctionWordTailCount = captions.filter((caption) =>
    DANGLING_FUNCTION_WORD_TAIL_RE.test(caption.text),
  ).length;
  const blockers = [];
  ...
  if (danglingFunctionWordTailCount > 0) {
    blockers.push("caption_dangling_function_word_tail");
  }
  return {
    ...
    metrics: {
      ...
      dangling_function_word_tail_count: danglingFunctionWordTailCount,
    },
    captions,
  };
}
