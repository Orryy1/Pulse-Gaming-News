...
function inspectPremiumEditRhythm({ scenes = [], coveredDurationS = null } = {}) {
  ...
  const sourceLockCardIndexes = cardIndexes.filter(
    (index) => isSourceLockKind(governedV5CardKind(sequence[index])),
  );
  ...
  if (sourceLockCardIndexes.includes(sequence.length - 1)) {
    blockers.push("source_lock_card_at_episode_end");
  }
  ...
  return {
    ...
    metrics: {
      ...
      source_lock_card_scene_count: sourceLockCardIndexes.length,
      source_lock_card_scene_indexes: sourceLockCardIndexes,
