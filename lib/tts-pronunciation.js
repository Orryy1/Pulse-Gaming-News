...
const TTS_PRONUNCIATION_PROFILE_VERSION = "gta-safe-next-title-v11";
...
const GTA_SIX_STUTTER_RE =
  /\b(?:(the)\s+)?(?:G\.?\s*T\.?\s*A\.?|Grand\s+Theft\s+Auto)\s+(?:(?:s\s*i|s(?:i|y|igh)?|see(?:[-\s]+see)?|sea|c|size|sice|sic|sick|sig|sixty)[-\s]*(?:six|6)|(?:see|sea|c|s|si|suh|sir|size|sice|sic|sick|sig)[-\s]+a[-\s]+(?:six|6)|(?:suh|sir|size|sice|sic|sick|sig)[-\s]+(?:six|6)|siix)\b/gi;
