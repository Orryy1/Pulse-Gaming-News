...
const TTS_PRONUNCIATION_PROFILE_VERSION = "gta-roman-title-v3";
...
const GTA_TITLE_RE = /\bG\.?\s*T\.?\s*A\.?\s+(VI|6|six|V|5|five)\b/gi;
const GRAND_THEFT_AUTO_TITLE_RE =
  /\bGrand\s+Theft\s+Auto\s+(VI|6|six|V|5|five)\b/gi;
...
const RULES = [
  ...
  // Soften to "online RPG".
  {
    ...
    apply(text) {
      return text
        ...
        .replace(GRAND_THEFT_AUTO_TITLE_RE, (_match, version) => {
          ...
          const spoken = lower === "vi" || lower === "6" || lower === "six" ? "six" : "five";
          return `Grand Theft Auto ${spoken}`;
        })
    ...
    description: "GTA V/VI title forms and six-stutters -> Grand Theft Auto five/six for stable local narration",
  },
  {
