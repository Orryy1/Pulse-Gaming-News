...
const TTS_PRONUNCIATION_PROFILE_VERSION = "gta-safe-next-title-v9";
...
const GRAND_THEFT_AUTO_TITLE_RE =
  /\b(?:(the)\s+)?Grand\s+Theft\s+Auto\s+(V\s*I|VI|6|six|V|5|five)(['’]s)?\b/gi;
const GTA_TITLE_WITH_POSSESSIVE_RE =
  /\b(?:(the)\s+)?G\.?\s*T\.?\s*A\.?\s+(V\s*I|VI|6|six|V|5|five)(['’]s)?\b/gi;
const GTA_SIX_STUTTER_RE =
  /\b(?:(the)\s+)?(?:G\.?\s*T\.?\s*A\.?|Grand\s+Theft\s+Auto)\s+(?:(?:s\s*i|s(?:i|y|igh)?|see(?:[-\s]+see)?|sea|c)[-\s]*(?:six|6)|(?:see|sea|c|s|si|suh|sir)[-\s]+a[-\s]+(?:six|6)|(?:suh|sir)[-\s]+(?:six|6)|siix)\b/gi;
