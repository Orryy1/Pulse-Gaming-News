...
const TTS_PRONUNCIATION_PROFILE_VERSION = "gta-safe-next-title-v6";
...
const GRAND_THEFT_AUTO_TITLE_RE =
  /\b(?:(the)\s+)?Grand\s+Theft\s+Auto\s+(V\s*I|VI|6|six|V|5|five)\b/gi;
const GTA_SIX_STUTTER_RE =
  /\b(?:(the)\s+)?(?:G\.?\s*T\.?\s*A\.?|Grand\s+Theft\s+Auto)\s+s(?:i|y|igh)?[-\s]*six\b/gi;
...
function gtaSpokenVersion(version, article) {
  ...
  return article ? `${article} Grand Theft Auto five` : "Grand Theft Auto five";
}
  ...
  // Soften to "online RPG".
  {
    ...
    apply(text) {
      return text
        ...
        .replace(GRAND_THEFT_AUTO_TITLE_RE, (_match, article, version) => {
          return gtaSpokenVersion(version, article);
        })
        .replace(GTA_TITLE_RE, (_match, article, version) => {
          return gtaSpokenVersion(version, article);
        })
