...
const TTS_PRONUNCIATION_PROFILE_VERSION = "gta-safe-next-title-v8";
...
const GTA_SIX_STUTTER_RE =
  /\b(?:(the)\s+)?(?:G\.?\s*T\.?\s*A\.?|Grand\s+Theft\s+Auto)\s+(?:(?:s\s*i|s(?:i|y|igh)?|see(?:[-\s]+see)?|sea|c)[-\s]*(?:six|6)|siix)\b/gi;
