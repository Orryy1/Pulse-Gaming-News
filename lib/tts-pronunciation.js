...
const TTS_PRONUNCIATION_PROFILE_VERSION = "canonical-title-continuity-v26";
...
const ARKNIGHTS_ENDFIELD_TITLE_RE = /\bArknights\s*(?::\s*)?Endfield\b/gi;
const BLACK_FLAG_RESYNCED_TITLE_RE = /\bBlack\s+Flag\s+Resynced\b/g;
...
const RULES = [
  ...
  {
    name: "arknights_endfield_title",
    apply(text) {
      return text.replace(ARKNIGHTS_ENDFIELD_TITLE_RE, "Arknights End Field");
    },
    description: "Arknights: Endfield -> continuous provider-safe title pronunciation",
  },
  {
    name: "black_flag_resynced_title",
    apply(text) {
      return text.replace(BLACK_FLAG_RESYNCED_TITLE_RE, "Black Flag re synced");
    },
    description: "Black Flag Resynced -> clear two-syllable TTS alias without punctuation or an invented spelling",
  },
  {
  ...
  },
  {
    ...
    apply(text) {
      return text.replace(
        /\b(Version)\s+(\d+)\.\s*(\d+)\b/gi,
        (_match, label, whole, fraction) =>
          `${label} ${whole} point ${fraction.split("").join(" ")}`,
      );
    },
    description: "Version 1.4 -> Version 1 point 4 for stable spoken version numbers",
  },
  {
  ...
  },
  {
    ...
    apply(text) {
      return text
        .replace(/\bPS\s*5\b/gi, "PlayStation five")
        .replace(/\bPS\s*4\b/gi, "PlayStation four");
    },
    description: "PS5 / PS4 and Whisper-spaced variants -> PlayStation five / four",
  },
  {
    name: "source_brand_names",
    apply(text) {
      return text
        .replace(/\bHypergryph\b/gi, "Hypergriff")
    ...
    description: "Publisher and storefront brand names -> clear spoken words",
  },
];
