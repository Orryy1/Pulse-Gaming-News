...
const TTS_PRONUNCIATION_PROFILE_VERSION = "title-colon-pause-v2";
...
const RULES = [
  ...
  // Soften to "online RPG".
  {
    name: "gta_title",
    apply(text) {
      return text.replace(GTA_TITLE_RE, (_match, version) => {
        const lower = String(version || "").toLowerCase();
        const spoken = lower === "vi" || lower === "6" || lower === "six" ? "six" : "five";
        return `Grand Theft Auto ${spoken}`;
      });
    },
    description: "GTA V/VI title forms -> Grand Theft Auto five/six for stable local narration",
  },
  {
