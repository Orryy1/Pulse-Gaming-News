...
const TTS_PRONUNCIATION_PROFILE_VERSION = "gta-safe-next-title-v15";
...
const TITLE_PAUSE_PUNCT_RE =
  /\b([A-Z][A-Za-z0-9'&-]*(?:\s+(?:[A-Z][A-Za-z0-9'&-]*|\d+|I|II|III|IV|V|VI|VII|VIII|IX|X|XI|XII|a|an|and|of|on|or|the|to|vs|versus|with)){0,5})\s*(:|;|[-–—]|[.?!])\s+([A-Z0-9][A-Za-z0-9'&-]*(?:\s+(?:[A-Z][A-Za-z0-9'&-]*|\d+|I|II|III|IV|V|VI|VII|VIII|IX|X|XI|XII|a|an|and|of|on|or|the|to|vs|versus|with)){0,6})\b/g;
...
function normaliseTitleColonPauses(text) {
  return String(text || "").replace(TITLE_PAUSE_PUNCT_RE, (match, left, punct, right) => {
    ...
    const knownJoinedTitle =
      /^(?:Halo Campaign Evolved|The Expanse Osiris Reborn|Doom The Dark Ages(?: Revelations)?|Marvel Tokon Fighting Souls|Gears of War E-Day)$/i.test(joined);
    const titleLike =
      strong >= 3 &&
      (leftWords.length > 1 ||
        rightWords.length > 1 ||
