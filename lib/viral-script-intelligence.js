...
function scoreHook({ story, script }) {
  ...
  if (!/\b(?:needed|warning|catch|paid|headline|problem|mistake|win|panic|risk|wild)\b/i.test(hook)) {
    score -= 18;
  }
...
function scoreCuriosity(script = "") {
  ...
  if (/\b(?:not full demand|before the standard launch|why|what matters|if the wider launch)\b/i.test(text)) {
    score += 22;
  }
...
function scoreInsight({ story, script }) {
  ...
  if (cleanText(story.source_name) && script.includes(story.source_name)) score += 8;
...
function scoreSourceSafety({ story, script, coherence }) {
  let score = 86;
  if (cleanText(story.source_name) && !script.includes(story.source_name)) score -= 18;
...
function storyLooksLikeForzaSteam(story = {}, script = "") {
  return /\bforza horizon 6\b/i.test(`${story.title || ""} ${script}`) &&
    /\b(?:steam|steamdb|numbers?|concurrent)\b/i.test(`${story.title || ""} ${script}`);
}
