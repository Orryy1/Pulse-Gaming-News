...
function genericSourceSignalTemplate(script = "") {
  return /\b(?:has one detail worth checking before it becomes background noise|the important part is whether it changes timing,\s*access,\s*price,\s*performance or actual footage|not every update deserves a spotlight|use this as a watch signal,\s*not a verdict)\b/i.test(
    script,
  );
}
...
function genericTitleTemplate(story = {}) {
  return /\b(?:just got a new signal|just changed the watchlist|now has a real question|just got a new reason to watch|is worth watching again|reviews just sent a signal|just dropped a new clue|deal has one catch|needs one real proof point)\b/i.test(
    story.title || "",
  );
}
