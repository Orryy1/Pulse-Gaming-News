...
function hookSurfacesMonetisationConflict(script = "") {
  ...
  const explicitConflict = /\b(?:micro[- ]?transactions?|moneti[sz]ation|DLC|paid[-\s]+(?:content|extras?|add[- ]?ons?|packs?|bundles?|passes?|expansions?|upgrades?|skins?|characters?|maps?|missions?|cosmetics?|currency|items?)|in[- ]game purchases?|storefront|season pass|full[- ]price|price\s+(?:cut|drop|fell|hike|increase|jump|rise|rose|tag)|costs?\s+(?:more|less|extra))\b/i;
  const price = /[$\u00a3\u20ac]\s?\d/i;
