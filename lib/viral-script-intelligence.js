...
function scoreHook({ story, script }) {
  ...
  if (!/\b(?:needed|warning|catch|paid|headline|problem|mistake|win|panic|risk|wild|signal|verdict|argument|free access|real story|gameplay|leak(?:ed)?|date|deal|drop(?:ped)?|review|score|handmade|job market|brutal|fake|pitch|console|release|hardware|demand|launch|more than|xcom|anti-cheat|vanguard|waiting room|subscription|reinstall|ownership|lapsed|friction|kernel)\b/i.test(hook)) {
    score -= 18;
  }
...
function buildBlockers({ story, script, lint, coherence }) {
  ...
  for (const failure of asArray(coherence.failures)) {
    ...
    if (
      /(?:vague_filler:internal_audience_scaffold|vague_filler:internal_tracking_language|generic_uncertainty_boilerplate|internal_pulse_framing|abstract_signal_language)/.test(failure) &&
