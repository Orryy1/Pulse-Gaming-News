 ...
 *   - lower_band_overlay_likelihood — bright achromatic glyph edges
 *     spread across the footer band (proxy for baked trailer labels).
...
const LOWER_BAND_SCAN_WIDTH = 540;
const LOWER_BAND_FRAME_HEIGHT = 960;
const LOWER_BAND_SCAN_HEIGHT = 96;
const LOWER_BAND_OVERLAY_THRESHOLD = 0.12;
...
function computeLowerBandOverlaySignalsFromSample(
  rgbBuffer,
  width,
  height,
  { sampleIsLowerBand = false } = {},
) {
  const empty = {
    lower_band_overlay_likelihood: 0,
    lower_band_overlay_span_rows: 0,
    lower_band_overlay_analysed_rows: 0,
    lower_band_overlay_max_row_ratio: 0,
  };
  if (
    !Buffer.isBuffer(rgbBuffer) ||
    !Number.isInteger(width) ||
    !Number.isInteger(height) ||
    width < 32 ||
    height < 32 ||
    rgbBuffer.length < width * height * 3
  ) {
    return empty;
  }
  ...
  const px = (x, y) => {
    ...
    return [rgbBuffer[index], rgbBuffer[index + 1], rgbBuffer[index + 2]];
  };
  const luminance = (x, y) => {
    const [r, g, b] = px(x, y);
    return 0.299 * r + 0.587 * g + 0.114 * b;
  };
  const radius = Math.max(1, Math.round(width / 135));
  const yStart = sampleIsLowerBand ? 0 : Math.floor(height * 0.9);
  const minimumHits = Math.max(3, Math.ceil(width * 0.13));
  const minimumSpan = width * 0.35;
  const minimumTextLikeRuns = Math.max(4, Math.ceil(width / 120));
  let analysedRows = 0;
  let maxRowRatio = 0;
  const candidateRowHits = [];

  for (let y = yStart + radius; y < height - radius; y += 1) {
    analysedRows += 1;
    let hits = 0;
    let minX = width;
    let maxX = -1;
    ...
    for (let x = radius; x < width - radius; x += 1) {
      ...
      const saturation =
        (maxChannel - Math.min(r, g, b)) / Math.max(1, maxChannel);
      const localMin = Math.min(
        luminance(x - radius, y),
        luminance(x + radius, y),
        luminance(x, y - radius),
        luminance(x, y + radius),
      );
      if (
        lum >= 185 &&
        ...
        if (currentRunLength === 0) hitRuns += 1;
        currentRunLength += 1;
      } else {
    ...
    longestRunLength = Math.max(longestRunLength, currentRunLength);
    const rowRatio = hits / Math.max(1, width - radius * 2);
    const dominantRunRatio =
      hits > 0 ? longestRunLength / hits : 1;
    maxRowRatio = Math.max(maxRowRatio, rowRatio);
    if (
      hits >= minimumHits &&
      maxX >= minX &&
      maxX - minX >= minimumSpan &&
      hitRuns >= minimumTextLikeRuns &&
      dominantRunRatio <= 0.6
    ) {
      candidateRowHits.push(hits);
    }
  }

  const maximumCandidateHits = candidateRowHits.length
    ? Math.max(...candidateRowHits)
    : 0;
  const minimumCandidateHits = candidateRowHits.length
    ? Math.min(...candidateRowHits)
    : 0;
  const rowHitVariation = maximumCandidateHits > 0
    ? (maximumCandidateHits - minimumCandidateHits) / maximumCandidateHits
    : 0;
  const uniformRepeatedScenePattern =
    candidateRowHits.length >= 5 &&
    rowHitVariation < 0.08;
  const spanRows = uniformRepeatedScenePattern ? 0 : candidateRowHits.length;

  return {
    lower_band_overlay_likelihood:
      analysedRows > 0 ? spanRows / analysedRows : 0,
    lower_band_overlay_span_rows: spanRows,
    lower_band_overlay_analysed_rows: analysedRows,
    lower_band_overlay_max_row_ratio: maxRowRatio,
  };
}
...
function classifyTrailerFrameTaste(input = {}) {
  ...
  const textOverlay = numberOrZero(prescan.text_overlay_likelihood);
  const lowerBandOverlay = numberOrZero(
    prescan.lower_band_overlay_likelihood,
  );
  ...
  if (lowerBandOverlay >= LOWER_BAND_OVERLAY_THRESHOLD) {
    tags.push("lower_band_text_overlay");
  }
  ...
  if (lowerBandOverlay >= LOWER_BAND_OVERLAY_THRESHOLD) {
    return fail("lower_band_text_overlay");
  }
  ...
  if (
    ...
    return fail("promotional_store_slate_frame");
  }
  if (
    textOverlay >= 0.18 &&
    brightRatio >= 0.34 &&
...
async function prescanImage(filePath, opts = {}) {
  ...
  const result = {
    ...
    lower_band_overlay_likelihood: null,
    lower_band_overlay_span_rows: null,
    lower_band_overlay_analysed_rows: null,
    lower_band_overlay_max_row_ratio: null,
  ...
  try {
    const lowerBandRaw = await sharpLib(filePath)
      .resize(LOWER_BAND_SCAN_WIDTH, LOWER_BAND_FRAME_HEIGHT, { fit: "cover" })
      .extract({
        left: 0,
        top: LOWER_BAND_FRAME_HEIGHT - LOWER_BAND_SCAN_HEIGHT,
        width: LOWER_BAND_SCAN_WIDTH,
        height: LOWER_BAND_SCAN_HEIGHT,
      })
      .removeAlpha()
      .raw()
      .toBuffer();
    const lowerBand = computeLowerBandOverlaySignalsFromSample(
      lowerBandRaw,
      LOWER_BAND_SCAN_WIDTH,
      LOWER_BAND_SCAN_HEIGHT,
      { sampleIsLowerBand: true },
    );
    result.lower_band_overlay_likelihood =
      lowerBand.lower_band_overlay_likelihood;
    result.lower_band_overlay_span_rows =
      lowerBand.lower_band_overlay_span_rows;
    result.lower_band_overlay_analysed_rows =
      lowerBand.lower_band_overlay_analysed_rows;
    result.lower_band_overlay_max_row_ratio =
      lowerBand.lower_band_overlay_max_row_ratio;
    result.trailer_frame_taste = classifyTrailerFrameTaste({
      ...sig,
      ...lowerBand,
    });
  } catch {
    // Whole-frame prescan remains valid when the secondary footer scan fails.
  }
...
module.exports = {
  ...
  computeLowerBandOverlaySignalsFromSample,
  computeContentHash,
