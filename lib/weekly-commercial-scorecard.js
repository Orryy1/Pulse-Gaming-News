...
function normaliseEvidence(evidence = {}) {
  return {
    ...
    expected_sha256:
      clean(evidence.expected_sha256 || evidence.sha256).toLowerCase() || null,
  };
}
...
function inspectMaterialisedEvidence(evidence = {}, { evidenceRoot = null } = {}) {
  ...
  try {
    ...
    if (
      ...
      evidence.expected_sha256 !== actualSha256
    ) {
      return {
        integrity_status: "sha256_mismatch",
        integrity_verified: false,
        resolved_path: canonicalCandidate,
...
function normaliseAllocationScope(entry = {}) {
  const scope = entry.scope || {};
  const normalised = {
    story_id: clean(scope.story_id || entry.story_id) || null,
    platform: clean(scope.platform || entry.platform).toLowerCase() || null,
    external_id: clean(scope.external_id || entry.external_id) || null,
    cohort_id: clean(scope.cohort_id || entry.cohort_id) || null,
  };
  return Object.values(normalised).some(Boolean) ? normalised : null;
}

function mostSevereVerdict(...verdicts) {
  const rank = { GREEN: 1, AMBER: 2, RED: 3 };
  return verdicts
    .map((verdict) => clean(verdict).toUpperCase())
    .filter((verdict) => rank[verdict])
    .sort((left, right) => rank[right] - rank[left])[0] || "AMBER";
}
...
function renderWeeklyCommercialScorecardMarkdown(scorecard = {}) {
  ...
  const allocation = scorecard.commercial_evidence_allocation || null;
  ...
  if (allocation) {
    lines.push(
      "",
      "## Evidence Allocation",
      `- Verdict: ${allocation.verdict || "AMBER"}`,
      `- Source records: ${allocation.source_record_count || 0}`,
      `- Allocated records: ${allocation.allocated_record_count || 0}`,
      `- Unavailable records: ${allocation.unavailable_record_count || 0}`,
      `- Rejected records: ${allocation.rejected_record_count || 0}`,
      `- Publication snapshot: ${allocation.publication_snapshot?.status || "not_provided"}`,
    );
  }
  lines.push(
    "",
...
function buildWeeklyCommercialScorecard({
  ...
  const rows = revenueInputRows.map((entry, index) => {
    ...
    return {
      id,
      source_record_id: clean(entry.source_record_id) || null,
      stage,
      ...
      allocation_basis: clean(entry.allocation_basis) || null,
      scope: normaliseAllocationScope(entry),
  ...
  const entryIntegrityVerdict = claimedEntryCount === 0
    ? "AMBER"
    : inputLedgerBound && allClaimedEntriesRecognised && invalidEvidenceRows.length === 0
      ? "GREEN"
      : "RED";
  const allocationVerdict = allocationBridge
    ? clean(allocationBridge.verdict).toUpperCase() || "AMBER"
    : null;
  const commercialIntegrityVerdict = allocationVerdict
    ? mostSevereVerdict(entryIntegrityVerdict, allocationVerdict)
    : entryIntegrityVerdict;
  const allocationAdvisories = allocationVerdict === "AMBER"
    ? ["commercial_evidence_allocation_incomplete"]
    : [];
  if (allocationVerdict === "RED") {
    blockers.push("commercial_evidence_allocation_red");
    blockers.push(...asArray(allocationBridge.blockers));
  }
  ...
  return {
    ...
    accounting_policy: {
      ...
      unknown_amount_representation: "null_not_zero",
    ...
    costs: {
      direct_production_cost_gbp: directProductionCostEvidenceBacked
        ? directProductionCost
        : null,
      direct_production_cost_status: directProductionCostEvidenceBacked
        ? "evidence_backed"
        : "unknown_no_primary_evidence",
      labour_replacement_cost_gbp: labourReplacementCostEvidenceBacked
        ? labourReplacementCost
        : null,
      labour_replacement_cost_status: labourReplacementCostEvidenceBacked
        ? "evidence_backed"
        : "unknown_no_primary_evidence",
      total_economic_cost_gbp: fullyLoadedCostEvidenceBacked
        ? money(directProductionCost + labourReplacementCost)
        : null,
    ...
    economics: {
      contribution_margin_gbp: directProductionCostEvidenceBacked
        ? contributionMargin
        : null,
      contribution_margin_status: directProductionCostEvidenceBacked
        ? "evidence_backed"
        : "unavailable_missing_direct_cost_evidence",
      contribution_margin_rate: directProductionCostEvidenceBacked && cashReceived > 0
        ? Math.round((contributionMargin / cashReceived) * 10_000) / 10_000
        : null,
      profit_gbp: fullyLoadedCostEvidenceBacked ? profit : null,
    ...
    commercial_evidence_allocation: allocationBridge || null,
    commercial_evidence_integrity: {
      ...
      advisories: allocationAdvisories,
    },
    blockers: uniqueBlockers,
    safety: {
