...
const FB_TOKEN_PATH = path.join(__dirname, '..', 'tokens', 'facebook_token.json');
const IG_TOKEN_PATH = path.join(__dirname, '..', 'tokens', 'instagram_token.json');
...
async function exchangeForLongLived(shortToken) {
  ...
  console.log(`Long-lived user token acquired (expires in ${Math.round(expiresIn / 86400)} days)`);

  // Step 2: Get page access token (these never expire as long as the user token is valid)
  ...
  if (pages.length === 0) {
    console.error('No Facebook Pages found. Make sure your app has pages_manage_posts permission.');
    process.exit(1);
  }
  ...
  // Use first page (or match FACEBOOK_PAGE_ID if set)
...
async function checkStatus() {
  ...
  for (const [name, tokenPath] of [['Facebook', FB_TOKEN_PATH], ['Instagram', IG_TOKEN_PATH]]) {
    if (await fs.pathExists(tokenPath)) {
      ...
      if (data.expires_at === 0) {
        ...
        console.log(`  Expires: ${expiry.toISOString()} (${daysLeft > 0 ? daysLeft + ' days left' : 'EXPIRED'})`);
      }
  ...
  if (!found) {
    console.log('No token files found. Run: node scripts/fb_auth.js exchange YOUR_SHORT_TOKEN');
  }
}
...
async function main() {
  ...
  if (command === 'exchange') {
    ...
    if (!shortToken) {
      console.error('Usage: node scripts/fb_auth.js exchange YOUR_SHORT_LIVED_TOKEN');
    ...
    console.log('Usage:');
    console.log('  node scripts/fb_auth.js exchange TOKEN  - exchange short-lived token for permanent page token');
    console.log('  node scripts/fb_auth.js status          - check current token status');
    console.log('  node scripts/fb_auth.js pages           - list pages + IG accounts');
    ...
    console.log('     instagram_basic, instagram_content_publish, pages_show_list');
    console.log('  4. Copy token and run: node scripts/fb_auth.js exchange YOUR_TOKEN');
  }
}
