...
app.get("/api/health", (req, res) => {
  ...
  const editorial = require(
    "./lib/services/governed-editorial-client"
  ).editorialRuntimeSummary(process.env);
  ...
  res.json({
    ...
    schedulerActive,
    schedulerExpected,
    multiLaneRuntime: currentMultiLaneRuntimeObservability(),
    circuitBreakers,
    build,
    runtime,
    deployment,
...
app.get("/api/autonomous/status", requireAuth, (req, res) => {
  const operatingContract = resolveOperatingContract();
  const multiLaneRuntime = currentMultiLaneRuntimeObservability();
  res.json({
    ...
    nextHuntRun: hunterInterval
      ...
      : null,
    schedule: buildAutonomousScheduleSummary(multiLaneRuntime),
    platforms: {
      youtube: {
...
function currentMultiLaneRuntimeObservability() {
  ...
  try {
    bootstrapState = require("./lib/bootstrap-queue").state();
  } catch {
    /* queue bootstrap is absent or has not started */
  }
  ...
  let schedules;
  try {
    schedules = schedulesForProfile(schedulerProfile);
  } catch {
  ...
  return buildMultiLaneRuntimeObservability({
    bootstrapState,
    env: process.env,
    schedulerActive: currentSchedulerActive(),
    schedulerProfile,
...
async function _registerLegacyDevCronRegistry() {
  ...
  if (legacyCompilationSchedulerEnabled(process.env)) {
    // Legacy weekly compilation is LOCAL_PROOF-only and opt-in. The governed
    // weekly lane owns production in every managed runtime profile.
    cron.schedule(
    "0 14 * * 0",
    async () => {
    ...
    console.log("[server] Legacy weekly compilation: Sunday 14:00 UTC");
  ...
  // Monthly topic compilations - 1st of each month at 10:00 UTC
    ...
    console.log(
      "[server] Legacy monthly topic compilations: 1st of month at 10:00 UTC",
    );
  } else {
    console.log(
      "[server] Legacy weekly/monthly compilation disabled; governed weekly lane is authoritative.",
    );
  }
