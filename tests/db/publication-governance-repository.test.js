...
test("autonomous approval requires one exact immutable non-operator authority", () => {
  const { db, governance } = fixture();
  for (const state of [
    "DISCOVERED",
    "VERIFIED",
    ...
    governance.appendLifecycle({
      ...
      toState: state,
      evidence: lifecycleEvidence(state),
      idempotencyKey: `story-1:autonomous-gate:${state}`,
    });
  }
  ...
  assert.throws(
    () =>
      governance.appendLifecycle({
        ...
        actorType: "system",
        evidence: lifecycleEvidence("AUTONOMOUSLY_APPROVED", {
          publication_authority_audit_id: 1,
          authority_binding_sha256: "a".repeat(64),
        }),
        idempotencyKey: "story-1:autonomous-gate:approval",
      }),
    /autonomous_approval_authority_required/,
  );
  ...
  const authorityInput = {
    ...
    lifecycleIdempotencyKey: "story-1:autonomous-gate:approval",
    reason: "Exact governed candidate passed autonomous authority gates",
    evidence: {
      exact_candidate_bound: true,
      external_publish_authorised: false,
      authority_binding_sha256: "a".repeat(64),
    },
    idempotencyKey: "story-1:autonomous-gate:authority",
  };
  const authority =
    governance.recordAutonomousPublicationAuthority(authorityInput);
  const replay =
    governance.recordAutonomousPublicationAuthority(authorityInput);
  assert.equal(replay.id, authority.id);
  assert.equal(authority.story_id, "story-1");
  assert.equal(authority.platform, "youtube");
  assert.equal(authority.decision, "APPROVED");
  assert.equal(
    Object.keys(authority).some((field) => /actor|operator/i.test(field)),
    false,
  );
  assert.deepEqual(
    governance.getPublicationAuthorityDecision(authority.id),
    authority,
  );

  assert.throws(
    () =>
      governance.recordAutonomousPublicationAuthority({
        ...authorityInput,
        reason: "Changed after the immutable decision",
      }),
    /publication_idempotency_conflict/,
  );
  ...
  const event = governance.appendLifecycle({
    ...
    toState: "AUTONOMOUSLY_APPROVED",
    actorType: "system",
    publicationAuthorityAuditId: authority.id,
    evidence: lifecycleEvidence("AUTONOMOUSLY_APPROVED", {
      publication_authority_audit_id: authority.id,
      authority_type: authority.authority_type,
      authority_binding_sha256: authority.authority_binding_sha256,
    }),
    idempotencyKey: authority.lifecycle_idempotency_key,
  });
  assert.equal(event.to_state, "AUTONOMOUSLY_APPROVED");
  assert.equal(event.actor_type, "system");
  assert.equal(event.actor_id, null);
  assert.equal(event.publication_authority_audit_id, authority.id);

  const eventReplay = governance.appendLifecycle({
    storyId: "story-1",
    platform: "youtube",
    toState: "AUTONOMOUSLY_APPROVED",
    actorType: "system",
  ...
  assert.equal(eventReplay.id, event.id);
  ...
  governance.appendLifecycle({
    ...
    toState: "SCHEDULED",
    actorType: "system",
    evidence: lifecycleEvidence("SCHEDULED"),
    idempotencyKey: "story-1:autonomous-gate:SCHEDULED",
  });
  assert.equal(
    governance.getState("story-1", "youtube").lifecycle_state,
    "SCHEDULED",
...
test("autonomous authority cannot cross candidates, actors or approval paths", () => {
  const { db, governance } = fixture();
  for (const storyId of ["story-1", "story-2"]) {
    for (const state of [
      ...
      governance.appendLifecycle({
        ...
        toState: state,
        evidence: lifecycleEvidence(state),
        idempotencyKey: `${storyId}:autonomous-isolation:${state}`,
      });
    }
  }
  const authority =
    governance.recordAutonomousPublicationAuthority({
      ...
      lifecycleIdempotencyKey:
        "story-1:autonomous-isolation:approval",
      evidence: {
        exact_candidate_bound: true,
        authority_binding_sha256: "b".repeat(64),
      },
      idempotencyKey: "story-1:autonomous-isolation:authority",
    });
  ...
  assert.throws(
    () => append({ storyId: "story-2" }),
    /autonomous_approval_authority_required/,
  );
  assert.throws(
    () => append({ actorType: "operator", actorId: "operator-1" }),
  ...
  assert.throws(
    () =>
      append({
        idempotencyKey:
          "story-1:autonomous-isolation:different-lifecycle",
      }),
    /autonomous_approval_authority_required/,
  );
  assert.throws(
    () =>
      append({
        evidence: lifecycleEvidence("AUTONOMOUSLY_APPROVED", {
          publication_authority_audit_id: authority.id,
          authority_type: authority.authority_type,
          authority_binding_sha256: "c".repeat(64),
        }),
      }),
    /autonomous_approval_authority_required/,
  );
  ...
  append();
  const humanAudit = governance.recordOperatorDecision({
    actorId: "operator-1",
    action: "approve_publication",
    targetType: "platform_publication",
    targetId: "story-1:youtube",
    decision: "APPROVED",
    reason: "Attempted cross-conversion",
    evidence: { review_complete: true },
    idempotencyKey:
      "story-1:autonomous-isolation:human-audit",
  });
  assert.throws(
    () =>
      governance.appendLifecycle({
        ...
        actorType: "operator",
        actorId: "operator-1",
        operatorDecisionId: humanAudit.id,
        evidence: lifecycleEvidence("HUMAN_APPROVED", {
          operator_decision_id: humanAudit.id,
        }),
        idempotencyKey: "story-1:autonomous-isolation:cross-convert",
      }),
    /invalid_publication_lifecycle_transition:AUTONOMOUSLY_APPROVED->HUMAN_APPROVED/,
  );
  db.close();
});
