...
test("duration padding keeps the approved CTA once at the end", () => {
  ...
  assert.equal(
    out,
    "A short source-backed script. Extra context goes here. Follow for more gaming news.",
  );
  assert.equal(
    (out.match(/Follow for more gaming news/g) || []).length,
    1,
  );
});
...
test("second duration padding attempt adds a source-safe follow-up sentence", () => {
  ...
  assert.equal(
    runScriptCoherenceQa(
      { ...story, full_script: out.full_script, cta: "Follow for more gaming news" },
