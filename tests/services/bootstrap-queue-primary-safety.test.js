...
test("server.js: autonomous status describes the stabilisation profile truthfully", () => {
  const src = fs.readFileSync(SERVER_PATH, "utf8");
  assert.match(src, /profile:\s*"stabilisation_30d"/);
  assert.match(src, /maximum:\s*"2 YouTube Shorts per rolling 24 hours"/);
  assert.match(src, /minimumGap:\s*"4 hours"/);
  assert.match(src, /humanReviewRequired:\s*true/);
  assert.match(src, /youtube_only_guarded/);
  assert.match(src, /operator_disabled/);
  assert.doesNotMatch(
    src,
    /strategy:\s*"1 Short per window = 3 Shorts\/day across all platforms"/,
  );
});
