...
function goodStory(overrides = {}) {
  return {
    ...
    full_script:
      "A dead franchise just got resurrected with a clear source trail. Big studios are responding to a shift in the market that took three years to build and thirty seconds to explode. The numbers are staggering and the timing is surgical. Ubisoft confirmed the reveal is set for later this month and the embargo lifts at midday across every major territory. Sources have verified the timeline through two separate trade outlets and an internal calendar invite that leaked last week. Players are already speculating about what this means for the series going forward, and the marketing team is quietly scrubbing old posts in preparation for the new positioning. Follow Pulse Gaming so you never miss a beat.",
...
test("runContentQa: compact GTAVI tts_script fails before audio generation", async () => {
  const story = goodStory({
    title: "GTAVI Starts The Preorder Fight",
    full_script:
      "GTA VI now has one real preorder catch. Xbox Wire says pre-orders open on June 25 after Rockstar put Jason and Lucia on the official cover art. The cover is hype. The store page is the test. It has to show price, editions, bonuses and which version is actually worth buying early. That is where fans split. Wait for value, or lock in because this is the safest blockbuster in games. If the first store page lands cleanly, the reveal becomes the first real fight over which version is worth buying. Follow Pulse Gaming so you never miss a beat.",
    tts_script:
      "GTAVI now has one real preorder catch. Xbox Wire says pre orders open on June 25 after Rockstar put Jason and Lucia on the official cover art. Follow Pulse Gaming so you never miss a beat.",
  });
  const qa = await runContentQa(story, {
    fs: fakeFs({ [story.exported_path]: { size: 5 * 1024 * 1024 } }),
  });
...
test("runContentQa: GTA VI timestamp evidence without current pronunciation profile fails before publish", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "pulse-content-qa-gta-profile-"));
  const videoPath = path.join(root, "story.mp4");
  const timestampPath = path.join(root, "story_timestamps.json");
  await fs.outputFile(videoPath, Buffer.alloc(5 * 1024 * 1024, 1));
  await fs.outputJson(timestampPath, {
...
test("runContentQa: strict local publish accepts explicit V4 word timestamp metadata path", async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "pulse-v4-voice-qa-"));
...
test("runContentQa: strict local publish blocks malformed GTA VI narration stutters", async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "pulse-gta-voice-stutter-"));
  const artifactDir = path.join(tmp, "story");
  const audioDir = path.join(artifactDir, "audio");
  const mp4 = path.join(artifactDir, "visual_v4_render.mp4");
  const audio = path.join(audioDir, "narration.mp3");
  ...
  try {
    const qa = await runContentQa(
      goodStory({
        ...
        audio_path: audio,
        word_timestamps_path: timestamps,
        full_script: goodStory().full_script.replace("A dead franchise", "Grand Theft Auto VI"),
      }),
      {
        env: {
          DEPLOYMENT_MODE: "production",
    ...
    assert.strictEqual(qa.result, "fail", JSON.stringify(qa));
    assert.ok(qa.failures.includes("approved_voice:gta_vi_opening_spoken_six_risk"));
  } finally {
    await fs.remove(tmp).catch(() => {});
  }
});
