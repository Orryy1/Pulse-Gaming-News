...
} = require("../../lib/goal38-definition-missing");

test("Goal 38 records a missing campaign definition without inventing requirements", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "pulse-goal38-missing-"));
  ...
  await fs.outputFile(docPath, [
    ...
    "",
    "Outputs: `goal37_readiness_report.json`.",
    "",
    "## Acceptance criteria",
    "",
  ].join("\n"));

  const report = await buildGoal38DefinitionMissing({
    campaignDocPath: docPath,
    upstreamGoal37Report: { verdict: "BLOCKED" },
    storyPackages: [{ story_id: "story-a" }, { story_id: "story-b" }],
    generatedAt: "2026-05-26T13:40:09.138Z",
  });

  assert.equal(report.goal, "38_goal_definition_missing");
  assert.equal(report.verdict, "BLOCKED");
  assert.equal(report.direct_definition_verdict, "BLOCKED");
  assert.equal(report.summary.goal_number, 38);
  assert.equal(report.summary.story_count, 2);
  assert.equal(report.summary.publish_now_count, 0);
  assert.ok(report.blockers.includes("campaign:goal38_definition_missing"));
  assert.ok(report.blockers.includes("campaign:goal38_outputs_missing"));
  assert.ok(report.blockers.includes("campaign:goal38_acceptance_criteria_missing"));
  assert.ok(report.blockers.includes("upstream:goal37_goal_definition_missing_blocked"));
...
test("Goal 38 recognises a defined section while carrying upstream blockers", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "pulse-goal38-defined-"));
  const docPath = path.join(root, "docs", "codex-main-goal.md");
  await fs.outputFile(docPath, [
    "# Campaign",
    "",
    "### 38. Archive Retention Policy",
    "",
...
test("Goal 38 writes machine-readable and human-readable definition-gap artefacts", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "pulse-goal38-write-"));
  const docPath = path.join(root, "docs", "codex-main-goal.md");
  ...
  const written = await writeGoal38DefinitionMissing(report, { outputDir: outDir });
