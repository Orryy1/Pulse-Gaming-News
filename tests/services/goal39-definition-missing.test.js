...
test("Goal 39 records a missing campaign definition without inventing requirements", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "pulse-goal39-missing-"));
  ...
  await fs.outputFile(docPath, [
    ...
    "",
    "Outputs: `goal38_readiness_report.json`.",
    "",
    "## Acceptance criteria",
    "",
  ].join("\n"));
  ...
  const report = await buildGoal39DefinitionMissing({
    ...
    upstreamGoal38Report: { verdict: "BLOCKED" },
    storyPackages: [{ story_id: "story-a" }, { story_id: "story-b" }],
    generatedAt: "2026-05-26T14:10:14.186Z",
  });

  assert.equal(report.goal, "39_goal_definition_missing");
  assert.equal(report.verdict, "BLOCKED");
  assert.equal(report.direct_definition_verdict, "BLOCKED");
  assert.equal(report.summary.goal_number, 39);
  assert.equal(report.summary.story_count, 2);
  assert.equal(report.summary.publish_now_count, 0);
  assert.ok(report.blockers.includes("campaign:goal39_definition_missing"));
  assert.ok(report.blockers.includes("campaign:goal39_outputs_missing"));
  assert.ok(report.blockers.includes("campaign:goal39_acceptance_criteria_missing"));
  assert.ok(report.blockers.includes("upstream:goal38_goal_definition_missing_blocked"));
...
test("Goal 39 recognises a defined section while carrying upstream blockers", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "pulse-goal39-defined-"));
  const docPath = path.join(root, "docs", "codex-main-goal.md");
  await fs.outputFile(docPath, [
    "# Campaign",
    "",
    "### 39. Archive Retention Policy",
    "",
...
test("Goal 39 writes machine-readable and human-readable definition-gap artefacts", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "pulse-goal39-write-"));
  const docPath = path.join(root, "docs", "codex-main-goal.md");
  ...
  const written = await writeGoal39DefinitionMissing(report, { outputDir: outDir });
