...
test("official trailer segment validator accepts entity-matched muted Steam storefront media", async () => {
  const outputRoot = tempOutputRoot("official-steam-platform-storefront-muted-motion");
  await cleanTempRoot(outputRoot);
  let call = 0;
  ...
  const report = await runOfficialTrailerSegmentValidation(
    [
      clip({
        path: "https://shared.fastly.steamstatic.com/store_item_assets/steam/apps/2764370/extras/starward.mp4",
        sourceType: "platform_storefront",
        sourceFamily: "steam_2764370_starward_media",
        referenceTitle: "Starward",
        sourceDurationS: 5.58,
        mediaStartS: 1.33,
        durationS: 5,
        entity: "Starward",
      }),
    ],
    {
      ...
      inspectFrame: async (outputPath) => {
        call += 1;
        const samples = [
          { edge_density: 0.299, saturation_mean: 0.145, score: 94.9 },
          { edge_density: 0.335, saturation_mean: 0.203, score: 100 },
          { edge_density: 0.355, saturation_mean: 0.133, score: 100 },
        ];
        const sample = samples[call - 1] || samples[0];
        return {
          ...passingQa(outputPath),
          content_hash: `starward-storefront-${call}`,
          prescan: {
            likely_is_logo: false,
            text_overlay_likelihood: 0,
            white_text_on_dark_likelihood: 0,
...
test("official trailer segment validator rejects muted Steam storefront media with an entity mismatch", async () => {
  const outputRoot = tempOutputRoot("official-steam-platform-storefront-entity-mismatch");
  await cleanTempRoot(outputRoot);
  let call = 0;
  ...
  const report = await runOfficialTrailerSegmentValidation(
    ...
    ],
    {
      ...
      inspectFrame: async (outputPath) => {
        ...
        return {
          ...passingQa(outputPath),
          content_hash: `mismatched-starward-storefront-${call}`,
          prescan: {
            likely_is_logo: false,
            text_overlay_likelihood: 0,
            white_text_on_dark_likelihood: 0,
