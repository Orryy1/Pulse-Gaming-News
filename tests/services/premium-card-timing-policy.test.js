...
test("premium card timing gives source verification enough time to register without stalling", () => {
  ...
  assert.equal(PREMIUM_CARD_TIMING_V5_VERSION, "pulse_card_timing_v5");
...
test("premium card timing keeps proof beats readable without allowing momentum-killing holds", () => {
  ...
  assert.equal(shortProof.minimum_visible_duration_s, 2.2);
  assert.ok(shortProof.planned_visible_duration_s >= 2.2);
  assert.ok(shortProof.planned_visible_duration_s <= 2.8);
  assert.equal(shortProof.maximum_visible_duration_s, 2.8);
  assert.ok(shortProof.maximum_visible_duration_s < 3);
  assert.equal(longProof.planned_visible_duration_s, 2.8);
...
test("premium card timing reports when readable copy exceeds the momentum ceiling", () => {
  ...
  assert.equal(timing.planned_visible_duration_s, 2.8);
...
test("premium card timing recognises source-lock aliases", () => {
  ...
  assert.equal(v5CardTimingContract("hyperframes_source_card", "VGC").planned_visible_duration_s, 2.4);
});
