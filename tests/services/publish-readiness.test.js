...
test("applyEnabledPlatformAutoPublishReadinessScope: autonomous executor handoff holds non-selected dry-run blockers", () => {
  const pillars = {
    ...
    human_review_approval_gate: {
      verdict: "amber",
      reason: "human_review_approval_gate_stale:148.8h",
      raw: {
        approved_action_count: 0,
        invalid_decision_count: 0,
        guarded_dispatch_eligible: false,
    ...
    guarded_dispatch_executor_preflight: {
      ...
      raw: {
        dispatch_ready_action_count: 4,
        selected_action_count: 4,
        handoff_ready_action_count: 4,
        blocked_selected_action_count: 0,
        ready_for_live_executor_handoff: true,
      },
    },
  ...
  assert.equal(scoped.pillars.strict_dry_run_control.raw.blocked_action_count_still_visible, 0);
  assert.equal(
    scoped.pillars.human_review_approval_gate.raw.enabled_platform_scope_override,
    "autonomous_green_dry_run_actions_supersede_human_review_approval_gate",
  );
...
test("pillarRenderMetadata blocks HyperFrames shell failures nested in render_manifest", () => {
  ...
  try {
    fs.writeFileSync(
      ...
      JSON.stringify({
        generated_at: "2026-06-01T00:00:00.000Z",
        ready_stories: [{ story_id: "active-hf-nested" }],
        safety: {
          no_publish_triggered: true,
          no_network_uploads: true,
          no_db_mutation: true,
    ...
    fs.writeFileSync(
      ...
      JSON.stringify([
        {
          id: "active-hf-nested",
          title: "Nested HyperFrames Story",
          exported_path: "C:/renders/active-hf-nested.mp4",
          render_lane: "visual_v4_production",
          render_quality_class: "premium",
          render_manifest: {
            premiumLane: {
              ...
              hyperframesPremiumShellGate: {
                ...
                passCount: 3,
                requiredPassCount: 4,
                blockers: ["source:hyperframes_validate_not_passed"],
              },
            },
          },
        },
    ...
    assert.deepEqual(
      pillar.raw.active_hyperframes_premium_shell_blockers[0].blockers,
      ["source:hyperframes_validate_not_passed"],
    );
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
...
test("pillarRenderMetadata blocks partial HyperFrames renders without shell proof", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pulse-render-metadata-hf-partial-shell-"));
  const planPath = path.join(dir, "dry_run_publish_plan.json");
  const bridgePath = path.join(dir, "scheduler_bridge_candidates.json");
  try {
    fs.writeFileSync(
      ...
      JSON.stringify({
        generated_at: "2026-06-01T00:00:00.000Z",
        ready_stories: [{ story_id: "active-hf-partial" }],
        safety: {
          no_publish_triggered: true,
          no_network_uploads: true,
          no_db_mutation: true,
    ...
    fs.writeFileSync(
      ...
      JSON.stringify([
        {
          id: "active-hf-partial",
          title: "Partial HyperFrames Story",
          exported_path: "C:/renders/active-hf-partial.mp4",
          render_lane: "visual_v4_production",
          render_quality_class: "premium",
          premiumLane: {
            rendererSplit: "ffmpeg-backbone-story-specific-hyperframes-cards",
    ...
    assert.equal(pillar.raw.active_hyperframes_premium_shell_blockers[0].hyperframes_card_count, 2);
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test("buildPublishReadinessReport: local TTS green doctor overrides stale blocked retry status", async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pulse-publish-readiness-tts-doctor-green-"));
  const outputDir = path.join(dir, "test", "output");
  fs.mkdirSync(outputDir, { recursive: true });
  const ttsPath = path.join(outputDir, "local_tts_overnight_report.json");
  const doctorPath = path.join(outputDir, "local_tts_doctor.json");
  try {
    fs.writeFileSync(
      ...
      JSON.stringify({
        verdict: "RED",
        autonomous_recovery: {
          status: "blocked_tts_not_ready",
          current_local_tts_ready: false,
          safe_to_run_local_tts_retry: false,
          safe_to_run_local_tts_retry_preflight: false,
          safe_retry_work_order_count: 2,
          retry_preflight_work_order_count: 2,
          retry_apply_ready_work_order_count: 0,
          operator_required_work_order_count: 0,
    ...
    const report = await pr.buildPublishReadinessReport({
      ...
      db: {
        async getStories() {
          return ["tts-one", "tts-two"].map((id, index) => ({
            id,
            title: `TTS recovery candidate ${index + 1}`,
            qa_failed: true,
            qa_failures: ["audio_generation_failed:server_down"],
    ...
    const recovery = report.pillars.recent_failed_candidates.raw.local_tts_recovery;
    assert.equal(recovery.status, "ready_for_local_tts_retry_preflight");
