...
function parseArgs(argv = process.argv.slice(2)) {
  const args = {
    ...
    upstreamGoal27ReportPath: path.join(ROOT, "output", "goal-27", "goal27_readiness_report.json"),
    outDir: path.join(ROOT, "output", "goal-28"),
    generatedAt: null,
    json: false,
    help: false,
  };
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (arg === "--upstream-goal27-report") args.upstreamGoal27ReportPath = argv[++index] || args.upstreamGoal27ReportPath;
...
function usage() {
  return [
    "Usage: npm run ops:goal28-definition-missing -- [options]",
    "",
    "Options:",
    "  --campaign-doc <path>              Campaign goal document",
    "  --story-packages <path>            Story package manifest for carry-forward context",
    "  --upstream-goal27-report <path>    Goal 27 readiness report",
    "  --out-dir <dir>                    Output directory for Goal 28 proof",
    "  --generated-at <iso>               Fixed timestamp for deterministic reports",
    "  --json                             Print JSON report",
    "",
    "LOCAL_PROOF only. This command records missing Goal 28 contract blockers and an operator request. It does not invent requirements, publish, post externally, mutate production rows or change OAuth/token state.",
  ].join("\n");
}
...
async function main(argv = process.argv.slice(2)) {
  ...
  const upstreamGoal27Report = await readJsonIfPresent(path.resolve(args.upstreamGoal27ReportPath), {});
  ...
  const written = await writeGoal28DefinitionMissing(report, {
    outputDir: path.resolve(args.outDir),
  });
  if (args.json) console.log(JSON.stringify(report, null, 2));
  else console.log(renderGoal28DefinitionMissingMarkdown(report).trimEnd());
  return { report, written };
}

if (require.main === module) {
  main().catch((error) => {
    console.error(`[goal28-definition-missing] FAILED: ${error.stack || error.message}`);
    process.exit(1);
  });
}
