...
function parseArgs(argv = process.argv.slice(2)) {
  const args = {
    ...
    upstreamGoal41ReportPath: path.join(ROOT, "output", "goal-41", "goal41_readiness_report.json"),
    outDir: path.join(ROOT, "output", "goal-42"),
    generatedAt: null,
    json: false,
    help: false,
  };
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (arg === "--upstream-goal41-report") args.upstreamGoal41ReportPath = argv[++index] || args.upstreamGoal41ReportPath;
...
function usage() {
  return [
    "Usage: npm run ops:goal42-definition-missing -- [options]",
    "",
    "Options:",
    "  --campaign-doc <path>              Campaign goal document",
    "  --story-packages <path>            Story package manifest for carry-forward context",
    "  --upstream-goal41-report <path>    Goal 41 readiness report",
    "  --out-dir <dir>                    Output directory for Goal 42 proof",
    "  --generated-at <iso>               Fixed timestamp for deterministic reports",
    "  --json                             Print JSON report",
    "",
    "LOCAL_PROOF only. This command records missing Goal 42 contract blockers and an operator request. It does not invent requirements, publish, post externally, mutate production rows or change OAuth/token state.",
  ].join("\n");
}
...
async function main(argv = process.argv.slice(2)) {
  ...
  const upstreamGoal41Report = await readJsonIfPresent(path.resolve(args.upstreamGoal41ReportPath), {});
  ...
  const written = await writeGoal42DefinitionMissing(report, {
    outputDir: path.resolve(args.outDir),
  });
  if (args.json) console.log(JSON.stringify(report, null, 2));
  else console.log(renderGoal42DefinitionMissingMarkdown(report).trimEnd());
  return { report, written };
}

if (require.main === module) {
  main().catch((error) => {
    console.error(`[goal42-definition-missing] FAILED: ${error.stack || error.message}`);
    process.exit(1);
  });
}
