...
const VALUE_FLAGS = new Map([
  ...
  ["--guarded-dispatch-result", "guardedDispatchResultPath"],
  ["--story-id", "storyId"],
  ...
  ["--confirm-rights-ledger-sha256", "confirmRightsLedgerSha256"],
  [
...
function usage() {
  return [
    "Governed reviewed video-QA refusal repair",
    "",
    "Usage:",
    "  node tools/governed-reviewed-video-qa-repair.js [options]",
    "",
    "Dry-run is the default. It verifies the exact reviewed 25-second",
    "media, current and persisted duration failure, immutable review chain,",
    "guarded dispatch result, expired admission, stopped runtime and fresh",
    "backup without changing the database.",
    "",
    "Apply additionally requires:",
    "  --apply --database <db> --backup-evidence <file>",
    "  --guarded-dispatch-result <guarded-result.json>",
    ...
    "  --confirm-renderer-manifest-sha256 <sha256>",
    "  --confirm-source-evidence-sha256 <sha256>",
    "  --confirm-guarded-dispatch-result-sha256 <sha256>",
    "  --confirm-rights-ledger-sha256 <sha256>",
    "  --confirm-publication-metadata-sha256 <sha256>",
    ...
    "changes OAuth or tokens, retries dispatch, or enables autonomous posting.",
  ].join("\n");
}
...
function parseCliArgs(argv = process.argv.slice(2)) {
  ...
  if (errors.length) {
    const error = new Error(
      "governed_reviewed_video_qa_repair_cli_arguments_invalid",
    );
    error.codes = errors;
    throw error;
  }
...
function renderMarkdown(result) {
  ...
  return [
    ...
    `- Auth boundary entered: ${stateLabel(result.safety?.auth_boundary_entered)}`,
    `- Uploader invoked: ${stateLabel(result.safety?.uploader_invoked)}`,
    `- Platform calls performed by repair: ${result.safety?.platform_calls_performed ? "yes" : "no"}`,
    `- OAuth or token changes by repair: ${result.safety?.oauth_or_tokens_mutated ? "yes" : "no"}`,
    "",
...
function preflightArtifactOutput(outDir) {
  const resolvedOutDir = path.resolve(
    outDir || path.join("output", "governed-reviewed-video-qa-repair"),
  );
...
function writeArtifacts(result, outDir) {
  ...
  const stem = [
    "governed-reviewed-video-qa-repair",
...
async function run(
  ...
  const execute =
    dependencies.execute || executeGovernedReviewedVideoQaRepair;
  const result = await execute({
    ...args,
    apply: args.apply === true,
    env,
  });
  const write = dependencies.writeArtifacts || writeArtifacts;
  let artifacts;
  try {
  ...
  try {
    ...
    process.stderr.write(
      `${JSON.stringify(
        {
          schema_version:
            "pulse-governed-reviewed-video-qa-repair-cli-error-v1",
          verdict: "ERROR",
          errors: Array.isArray(error?.codes)
            ...
            : [
                String(
                  error?.message ||
                    "governed_reviewed_video_qa_repair_failed",
                ),
              ],
        },
        null,
