...
const DISPLAY_PIPELINE_EVIDENCE_ROOT = String.raw`D:\pulse-evidence\system-trace-display-pipeline-buffer-20260820`;
const DEFAULT_MANIFEST_FILE = "system-trace-youtube-buffer.json";
const DISPLAY_PIPELINE_MANIFEST_FILE =
...
function parseArgs(argv = process.argv.slice(2)) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--campaign") {
      parsed.campaign = argv[++index] || null;
      if (parsed.campaign !== "display-pipeline") {
        throw new Error("campaign_not_supported");
      }
    } else if (arg === "--confirm-story-id")
      parsed.confirmStoryId = argv[++index] || null;
...
function usage() {
  return [
    ...
    "",
    "Required:",
    "  --campaign display-pipeline  Use the fixed 22–28 August display buffer",
...
function assertPackContract(pack, episode) {
  ...
  if (
    !clean(request?.video_file) ||
    !/^[a-f0-9]{64}$/.test(clean(request?.video_sha256))
  ) {
...
async function main(
  ...
  const manifestFile =
    args.campaign === "display-pipeline"
      ? DISPLAY_PIPELINE_MANIFEST_FILE
      : DEFAULT_MANIFEST_FILE;
  const selectedEvidenceRoot =
    evidenceRoot ||
    (args.campaign === "display-pipeline"
      ? DISPLAY_PIPELINE_EVIDENCE_ROOT
      : DEFAULT_EVIDENCE_ROOT);
  const manifestInput = await readRegularJson(
    path.join(fixedRepoRoot, "videos", manifestFile),
    "fixed_manifest",
  );
  const manifest = manifestInput.document;
  const episode = manifest?.episodes?.find(
  ...
  const packageRoot = await fs.realpath(
    path.resolve(fixedRepoRoot, episode.project_dir),
  );
  ...
  const fixedEvidenceRoot = path.resolve(selectedEvidenceRoot);
  ...
  if (!pathInside(packageRoot, videoPath))
    throw new Error("video_must_be_inside_package");
  ...
  if (await fs.pathExists(fixedReceiptPath)) {
    const existingInput = await readRegularJson(
      fixedReceiptPath,
      "existing_private_receipt",
    );
    if (receiptReplayIsExact(existingInput.document, intent)) {
      const client = await authenticatedYoutubeClientFactory();
      const readback = await verifyExactSystemTraceYouTubeStudioVideo({
        client,
        intent,
      });
      log(
  ...
  // Authentication is a precondition, not a reserved remote-mutation attempt.
  // Fail before creating the immutable receipt when no remote call can occur.
  const client = await authenticatedYoutubeClientFactory();
  const reserved = {
    schema_version: 1,
