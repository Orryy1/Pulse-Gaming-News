...
powershell -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "if (Test-Path '%TTS_START_LOCK%') { $age = (Get-Date) - (Get-Item '%TTS_START_LOCK%').LastWriteTime; if ($age.TotalMinutes -lt 5) { exit 0 } } exit 1"
if "%ERRORLEVEL%"=="0" (
